...
export type WorkflowExposureInput = {
  /** Every registered route, paired with its access result. */
  routes: ReadonlyArray<{
    route: RouteDefinition;
    access: RouteAccessResult;
  }>;
  primaryWorkflow: WorkflowProfileCode;
  secondaryWorkflows: ReadonlyArray<WorkflowProfileCode>;
};

export type WorkflowExposureItem = {
  route: RouteDefinition;
  access: RouteAccessResult;
  /** Why the item is in this bucket (debug + analytics-free). */
  bucketReason:
...
export type WorkflowExposureResult = {
  primaryItems: ReadonlyArray<WorkflowExposureItem>;
  secondaryItems: ReadonlyArray<WorkflowExposureItem>;
  moreAdvancedItems: ReadonlyArray<WorkflowExposureItem>;
  recommendedItems: ReadonlyArray<WorkflowExposureItem>;
  allToolsItems: ReadonlyArray<WorkflowExposureItem>;
};
 ...
 */
export function resolveWorkflowExposure(
  input: WorkflowExposureInput,
): WorkflowExposureResult {
  const primaryItems: WorkflowExposureItem[] = [];
  const secondaryItems: WorkflowExposureItem[] = [];
  const moreAdvancedItems: WorkflowExposureItem[] = [];
  const recommendedItems: WorkflowExposureItem[] = [];
  const allToolsItems: WorkflowExposureItem[] = [];
  ...
  for (const entry of input.routes) {
    ...
    const taggedSecondary = input.secondaryWorkflows.some((w) =>
  ...
  for (const entry of input.routes) {
    if (primaryIds.has(entry.route.id)) continue;
    if (!entry.route.workflowTags.includes(input.primaryWorkflow)) continue;
