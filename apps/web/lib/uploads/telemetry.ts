...
export type UploadTelemetryType = (typeof UPLOAD_TELEMETRY_TYPES)[number];

export type UploadTelemetryEmitter = {
  emit(type: UploadTelemetryType, count?: number): void;
  /** Flush any pending counts now. Returns the number of events
   *  posted. Useful for tests + `beforeunload`. */
  flush(): Promise<number>;
};

const DEFAULT_FLUSH_DELAY_MS = 4_000;
const MAX_BATCH = 50;
const VALID_TYPES = new Set<string>(UPLOAD_TELEMETRY_TYPES);

export function createUploadTelemetry(input: {
  teamId: string;
  flushDelayMs?: number;
}): UploadTelemetryEmitter {
  const pending = new Map<UploadTelemetryType, number>();
