...
export type WorkflowUsageSignals = {
  /** Recent reviewer-queue / SLA activity volume. */
  reviewerQueueActivity: number;
  /** Active cases / matters in the workspace. */
  caseActivity: number;
  /** Active legal-hold / governance configuration activity. */
  governanceActivity: number;
  /** Operational incidents / queue pressure activity. */
  operationalIncidentActivity: number;
  /** Public verification / media-record activity. */
  mediaVerificationActivity: number;
  /** Relationship / timeline reconstruction usage. */
  investigationActivity: number;
};

export type WorkflowSuggestion = {
  recommended: WorkflowProfileCode;
  /** Bounded confidence 0..100. Useful for sorting multiple signals. */
  confidence: number;
  /** Short, operationally-toned reason rendered next to the suggestion. */
  reason: string;
};

const EMPTY_SIGNALS: WorkflowUsageSignals = {
...
export function suggestWorkflow(
  signals: Partial<WorkflowUsageSignals> = {},
): WorkflowSuggestion {
  const merged = { ...EMPTY_SIGNALS, ...signals };
  const total =
    merged.reviewerQueueActivity +
    merged.caseActivity +
  ...
  if (total === 0) {
    return {
      ...
      confidence: 0,
      reason:
        "Default starting point for capture, verification, and documentation workflows.",
    };
  }
  ...
  const ranked: Array<{
  ...
  }> = [
    {
      ...
      reason:
        "Operational incident and queue pressure signal is high. Operational Administration prioritises Command Center surfaces.",
    },
    {
      code: "REVIEW_OPERATIONS",
      score: merged.reviewerQueueActivity,
      reason:
        "Reviewer queue activity is high. Review & Operations prioritises intake queues, SLA tracking, and assignments.",
    },
    {
      code: "GOVERNANCE_COMPLIANCE",
      score: merged.governanceActivity,
      reason:
        "Governance and legal-hold activity is high. Governance & Compliance prioritises retention, audit posture, and policy workflows.",
    },
    {
      code: "INVESTIGATION_RECONSTRUCTION",
      score: merged.investigationActivity,
      reason:
        "Timeline and relationship usage is high. Investigation & Reconstruction prioritises cross-case intelligence and discovery.",
    },
    {
      code: "LEGAL_CASEWORK",
      score: merged.caseActivity,
      reason:
        "Case / matter activity is high. Legal & Case Workflows prioritises matters, custody timelines, and verification packages.",
    },
    {
      code: "MEDIA_VERIFICATION",
      score: merged.mediaVerificationActivity,
      reason:
        "Media verification activity is high. Media & Publication Verification prioritises authenticity, public verify, and export readiness.",
    },
  ];
