...
const DEFAULT_PROFILE: PlatformContextPersonaProfile = {
  teamId: null,
...
export function usePersonaProfile(): PlatformContextPersonaProfile {
  const { envelope } = usePlatformContext();
  return envelope?.personaProfile ?? DEFAULT_PROFILE;
}

export function usePrimaryPersona(): WorkspacePersonaProfile {
 ...
 */
export function useIsOperatorPersona(): boolean {
