...
export type StateObservabilityEvent =
  (typeof STATE_OBSERVABILITY_EVENTS)[number];
...
/**
 * The complete set of payload field types that may be emitted. Anything
 * outside this union is a TypeScript compile error — so evidence
 * content, raw GPS, secrets, tokens, blobs, etc. cannot be emitted
 * even by accident.
 *
 * Specifically forbidden by the absence of compatible types:
 *   - strings longer than ~64 chars (no email, no path, no blob)
 *   - numbers with floating-point precision (no raw GPS coordinates)
 *   - any object/array (no nested PII, no envelope dumps)
 */
export type SafePayloadValue =
  | boolean
  | null
  | undefined
  // Bounded labels: workspace scope, persona code, surface name,
  // route id, error code. Each capped at 64 chars at runtime.
  | string
  // Bounded counts / versions. Integers only (validated at runtime).
  | number;

export type SafePayload = Readonly<Record<string, SafePayloadValue>>;

export interface RecordedEvent {
  readonly event: StateObservabilityEvent;
  readonly tMs: number;
...
function sanitize(payload: SafePayload): SafePayload {
  const out: Record<string, SafePayloadValue> = {};
 ...
 */
export function redactWorkspaceId(id: string | null | undefined): string {
  if (!id) return "workspace_none";
...
export function recordedEvents(): ReadonlyArray<RecordedEvent> {
  return ring.slice();
}
 ...
 */
export function __testForceEnable(enabled: boolean): void {
