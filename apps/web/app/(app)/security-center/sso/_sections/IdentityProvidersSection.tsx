...
/**
 * SSO CONNECTIONS — create, lifecycle, signing and domain policy.
 *
 * PV-DUP-001 — ONE SSO CONSOLE. This section was the whole of
 * `/admin/identity/providers`: the same endpoint (`/v1/admin/identity/providers`)
 * as `/security-center/sso`, behind a different gate, in a different navigation
 * family. Two places configured the same tenant SSO, and a change made in one
 * was invisible in the other's navigation. It now renders inside the canonical
 * SSO console, beside the SAML configuration it feeds, and the old URL
 * redirects there.
 *
 * Hard rules (unchanged from the providers console):
 *   - A client secret is shown ONCE on create (`clientSecretOnce`) and dropped
 *     from state after dismissal or a workspace switch.
 *   - Reads only ever carry the preview (`ck-***-abcd`) and key fingerprints —
 *     never a private key.
 *   - Creating, transitioning and changing signing policy route through step-up.
 *   - A 402/403/404 is a DENIAL, rendered as one — never as "nothing here".
 *
 * The page owns one refresh key: this section reloads when it changes and
 * reports its own changes through `onChanged`, so the SAML section below and
 * this one never show two different lists.
 */
...
import { FederationReadinessSection } from "./FederationReadinessSection";
import { AdmInline } from "../../../../../components/admin/AdminSurfaces";
...
const CREATE_FORM_ID = "sso-new-connection-form";
...
export function IdentityProvidersSection({
  ...
  /** Bumped by the page whenever any section changed a connection. */
  refreshKey: number;
  /** Report a change so every section on the page re-reads. */
  onChanged: () => void;
}) {
  ...
  const [providersCap, setProvidersCap] = useState<number | undefined>(undefined);
  ...
  /** The control that opened the form — focus returns here on cancel. */
  const triggerRef = useRef<HTMLButtonElement | null>(null);
  /** The form's first field — focus moves here when the form opens. */
  ...
  // The opened form is announced by focus landing in it, not by appearing
  // somewhere below the fold with nothing to say it did.
  useEffect(() => {
  ...
  const submitCreate = useCallback(async () => {
    ...
    try {
      ...
      if (code === "STEP_UP_CANCEL") {
        ...
        setError(toSafeUserError(err, { message: "Could not create connection." }).message);
      }
    } finally {
      setBusy(null);
    }
  }, [teamId, createForm, onChanged, stepUp, stamp, isStale]);
    ...
    async (id: string, nextStatus: string) => {
      ...
      if (nextStatus === "REVOKED" || nextStatus === "DISABLED") {
        const ok = await confirm({
          ...
          confirmLabel: nextStatus === "REVOKED" ? "Revoke provider" : "Disable provider",
          tone: "danger",
      ...
      try {
        await stepUp.runStepUpAction(async (headers) =>
          apiFetch(`/v1/admin/identity/providers/${encodeURIComponent(id)}/transition`, {
            method: "POST",
            headers: { "content-type": "application/json", ...(headers ?? {}) },
        ...
        if (code === "STEP_UP_CANCEL") {
          ...
          setError(toSafeUserError(err, { message: "Transition failed." }).message);
        }
      } finally {
        setBusy(null);
      }
    },
    [teamId, onChanged, stepUp, confirm, stamp, isStale],
  );
  ...
  const updatePolicy = useCallback(
    async (
      ...
      try {
        ...
        await stepUp.runStepUpAction(async (headers) =>
          apiFetch(`/v1/admin/identity/providers/${encodeURIComponent(id)}/policy`, {
            method: "POST",
            headers: { "content-type": "application/json", ...(headers ?? {}) },
        ...
        if (code === "STEP_UP_CANCEL") {
          ...
          setError(toSafeUserError(err, { message: "Could not update signing policy." }).message);
        }
      } finally {
        setBusy(null);
      }
    },
    [teamId, stepUp, onChanged, stamp, isStale],
  );
  ...
  return (
    <>
      ...
      <PageSection
        title="Connections"
        description="Every identity provider for this workspace: create one, move it through its lifecycle, and set its signing and domain policy."
      >
        <div style={{ display: "flex", justifyContent: "flex-end", marginBottom: 8 }}>
          <Button
            ...
            data-testid="sso-new-connection"
          >
            New connection
          </Button>
        </div>
              ...
              >
                <Field label="Provider">
                  <select
                    ...
                    onChange={(e) =>
                      setCreateForm((p) => ({ ...p, provider: e.target.value as SsoProvider }))
                    }
                  >
                  ...
                  <>
                    <Field label="Issuer URL (OIDC discovery)">
                      <input
                        ...
                        onChange={(e) => setCreateForm((p) => ({ ...p, issuerUrl: e.target.value }))}
                      />
                    </Field>
                    ...
                    <Field label="Client secret">
                      <input
                        ...
                        onChange={(e) =>
                          setCreateForm((p) => ({ ...p, clientSecret: e.target.value }))
                        }
                      />
                    </Field>
                  </>
                ...
                <Field label="Allowed email domains (comma-separated)">
                  <input
                    ...
                    onChange={(e) =>
                      setCreateForm((p) => ({ ...p, allowedEmailDomains: e.target.value }))
                    }
        ...
        {providers === null ? (
          ...
          <EmptyState
            ...
            purpose="Connect an identity provider (Okta, Entra ID, Google Workspace, or generic OIDC/SAML) with New connection above to enable single sign-on and just-in-time provisioning for this workspace."
          />
        ) : (
          <div
            style={{
          ...
          >
              ...
              <tbody>
                {providers.map((p) => (
                  <React.Fragment key={p.id}>
                    <tr>
                      <td>
                        <span style={{ fontWeight: 600 }}>{PROVIDER_LABELS[p.provider]}</span>
                      </td>
                      <td>
                        {p.displayName}
                        {p.clientSecretPreview ? (
                          <div className="adm-help" style={{ fontFamily: "monospace", fontSize: 11 }}>
                      ...
                      <td>
                        <span className="adm-help">{p.jitDefaultRole ?? "disabled"}</span>
                      </td>
                      <td>
                      ...
                      <td>
                        <span className="adm-help">{formatCellDateTime(p.lastUsedAtUtc)}</span>
                      </td>
                      <td>
            ...
            {/* The connection list reads under a fixed server cap, which the
                route ships rather than keeping to itself. */}
            <ResultCount
...
function PolicyPanel({
  ...
  const keyReasonId = `sso-key-reason-${connection.id}`;
  const domainReasonId = `sso-domain-reason-${connection.id}`;
  ...
  const signingSourceLabel =
    connection.samlSpSigningKeySource === "stored"
      ...
      : connection.samlSpSigningKeySource === "env"
        ...
        : "Not configured";
  const nothingToInstall = keyDraft.trim() === "" && certDraft.trim() === "";
  ...
  return (
    ...
    >
      {isSaml ? (
        <div style={{ display: "flex", flexDirection: "column", gap: 8 }}>
          ...
          <div className="adm-help" style={{ fontSize: 12 }}>
            ...
            {" · "}SP certificate: {connection.samlSpCertificateConfigured ? "configured" : "none"}
          </div>
          <label style={{ display: "flex", alignItems: "center", gap: 8, fontSize: 13 }}>
            <input
              ...
              disabled={busy}
              onChange={(e) => onUpdate({ samlSignRequests: e.target.checked })}
            />
            <span>Sign AuthnRequests</span>
          </label>
          {connection.samlSignRequests && connection.samlSpSigningKeySource === "none" ? (
          ...
          <textarea
            ...
            aria-label="New SAML SP signing key (PEM)"
          ...
          <textarea
            ...
            aria-label="SAML SP certificate (base64), optional"
            placeholder="SP certificate (base64, no PEM header) — optional"
            className="adm-input"
            style={{ minHeight: 60, fontFamily: "monospace", fontSize: 11 }}
          />
          <div style={{ display: "flex", gap: 8, flexWrap: "wrap", alignItems: "center" }}>
            <Button
              ...
              size="sm"
              loading={busy}
              disabled={busy || nothingToInstall}
              aria-describedby={nothingToInstall ? keyReasonId : undefined}
              onClick={() => {
                const patch: { samlSpPrivateKey?: string; samlSpCertificate?: string } = {};
                if (keyDraft.trim() !== "") patch.samlSpPrivateKey = keyDraft;
                if (certDraft.trim() !== "") patch.samlSpCertificate = certDraft;
                onUpdate(patch);
                setKeyDraft("");
                setCertDraft("");
              }}
            ...
            {nothingToInstall ? (
              <span id={keyReasonId} className="adm-help" style={{ fontSize: 12 }}>
                Paste a signing key or a certificate to install.
              </span>
            ) : null}
          </div>
        </div>
      ...
      <div style={{ display: "flex", flexDirection: "column", gap: 8 }}>
        ...
        <div className="adm-help" style={{ fontSize: 12 }}>
          Verified organization domains: <strong>{verifiedDomainCount}</strong>
        </div>
        <label style={{ display: "flex", alignItems: "center", gap: 8, fontSize: 13 }}>
          <input
            type="checkbox"
            checked={connection.restrictToVerifiedDomains}
            disabled={busy || (noVerifiedDomains && !connection.restrictToVerifiedDomains)}
            aria-describedby={
              noVerifiedDomains && !connection.restrictToVerifiedDomains ? domainReasonId : undefined
            }
            onChange={(e) => onUpdate({ restrictToVerifiedDomains: e.target.checked })}
          />
        ...
        {noVerifiedDomains && !connection.restrictToVerifiedDomains ? (
          <div
            id={domainReasonId}
            className="adm-help"
            style={{ fontSize: 12, color: "var(--warning-strong)" }}
          >
            Verify at least one organization domain before enabling this restriction.
          </div>
        ) : null}
      </div>
    </div>
