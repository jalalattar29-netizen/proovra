              ...
              <tbody>
                {status.components.map((c) => (
                  <tr key={c.key} data-status-component={c.key} data-status-component-health={c.health}>
                    ...
                    <td style={td}>{formatUserDateTime(c.lastUpdatedUtc)}</td>
                  </tr>
                ))}
              </tbody>
            </table>
                ...
                <tbody>
                  {status.maintenanceWindows.map((w) => (
                    <tr key={w.id} data-maintenance-window={w.id}>
                      ...
                      <td style={td}>{formatUserDateTime(w.startsAtUtc)}</td>
                      <td style={td}>{formatUserDateTime(w.endsAtUtc)}</td>
      ...
      {incidents.map((i) => (
        ...
        >
          ...
          <p style={{ fontSize: 11, color: "#475569" }}>
            ...
            {formatUserDateTime(i.startedAtUtc)}
            {i.resolvedAtUtc ? (
              <> · Resolved {formatUserDateTime(i.resolvedAtUtc)}</>
            ) : null}
          </p>
          {i.updates.length > 0 ? (
            <ul style={{ marginTop: 6, paddingLeft: 18 }}>
              {i.updates.map((u) => (
                <li key={u.id} style={{ fontSize: 12 }}>
                  <code>{u.state}</code> · {formatUserDateTime(u.createdAtUtc)} —{" "}
                  {u.body}
                </li>
              ))}
            </ul>
