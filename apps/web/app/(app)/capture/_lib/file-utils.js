/* eslint-env browser */
// Committed compiled output of file-utils.ts. The .ts source is the canonical
// edit point; this .js artifact must remain in sync. ESLint scans both; the
// browser env directive prevents no-undef errors on MediaRecorder, File, etc.
import { MAX_SESSION_BYTES, MAX_SESSION_FILES, MAX_SINGLE_FILE_BYTES, } from "./templates";
export function normalizeClientMimeType(mimeType, fallback = "application/octet-stream") {
...
export function inferEvidenceTypeFromMimeType(mimeType) {
...
export function deriveBatchEvidenceType(files) {
...
export function isScreenshotLikeFileName(fileName) {
    return /screenshot|screen shot|bildschirmfoto|screen-recording|screenrecording|screencap/i.test(fileName);
}
export function buildSessionItemSignals(file, relativePath, existingItems) {
    const now = Date.now();
    const folderPathPresent = Boolean(relativePath);
    const mimeType = normalizeClientMimeType(file.type);
    const duplicateMatch = existingItems.some((existing) => existing.file.name === file.name &&
...
export function deriveSessionItemTypeLabel(mimeType) {
...
export function formatEvidenceTypeLabel(kind) {
    switch (kind) {
        case "PHOTO":
            return "Photo";
        case "VIDEO":
...
export function getStepRequirementLabel(step) {
    return step.required ? "Required" : "Optional";
}
export function getChecklistStepById(plan, stepId) {
    if (!plan || !stepId)
        return null;
...
export function getChecklistStepStatus(params) {
    ...
    if (!step) {
        return {
            ...
            description: "Assign this material to a collection plan step so reviewers understand why it was collected.",
        };
    }
    return {
...
export function roundGpsCoordinate(value, precision = 3) {
    const factor = Math.pow(10, precision);
    return Math.round(value * factor) / factor;
}
export function validateIncomingFiles(incomingFiles, existingItems) {
    ...
    if (tooLarge) {
        return `${tooLarge.name} is too large. Maximum single file size is ${formatFileSize(MAX_SINGLE_FILE_BYTES)}.`;
    }
    ...
    if (existingBytes + incomingBytes > MAX_SESSION_BYTES) {
        return `Session is too large. Maximum staged session size is ${formatFileSize(MAX_SESSION_BYTES)}.`;
    }
    return null;
}
export function buildAudioRecordingFileName(extension) {
