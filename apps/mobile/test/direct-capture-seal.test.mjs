...
test("one act has one name across all three capture surfaces", () => {
  // "Finalize Evidence" here, "Finish & Sign" there, "Evidence saved." against
  // "Evidence created successfully" — three names for one act is how a single
  // product reads as two.
  const read = (rel) => readFileSync(resolve(HERE, rel), "utf8");
  for (const file of [
    "../app/(stack)/capture.tsx",
    "../app/(stack)/screen-capture.tsx",
    "../app/(stack)/continuous-capture.tsx",
  ]) {
    const src = read(file);
    assert.ok(
      !/Finalize Evidence/.test(src),
      `${file} still calls it "Finalize Evidence"`,
    );
  ...
  for (const file of [
    ...
    assert.match(read(file), /Evidence created successfully/);
    assert.match(read(file), /Finish &amp; Sign/);
  }
});
