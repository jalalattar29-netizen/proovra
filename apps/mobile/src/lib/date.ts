/**
 * apps/mobile date helpers — thin wrappers over the single shared timestamp
 * layer (@proovra/shared), mirroring apps/web/lib/date.ts. Per the PROOVRA
 * Global Timestamp Display Policy the authenticated dashboard renders
 * timestamps in the VIEWER's IANA time zone (e.g. "03 Jul 2026, 02:48:42
 * Europe/Berlin") — never "GMT+2" — and audit surfaces render UTC. No direct
 * Intl/toLocale formatting lives here; the viewer time zone is resolved inside
 * the shared (allowlisted) helper.
 */
import {
