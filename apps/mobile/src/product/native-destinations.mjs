...
export const NATIVE_DESTINATIONS = {
  ...
  "/evidence": {
    ...
    gaps: [
      ...
      "S15 CROSS-CHECK (2026-09-22): 5 of 15 web routes unconsumed natively. Saved-view UPDATE, DELETE and set-default are absent (creation and listing are present, so a view can be made and read but not renamed, removed or defaulted); GET /:id/parts and GET /:id/original are per-record reads the detail screen owns. The saved-view three are a real asymmetry on a row marked CODE_PARITY and are named here.",
  ...
  "/evidence/[id]": {
    ...
    gaps: [
      ...
      "S15 CROSS-CHECK (2026-09-22). The capability map now resolves native calls, so the ledger's parity claim was tested mechanically: the routes this row's web sources consume, minus the routes any native file consumes. It found 11 of 16 unconsumed. Three of those (lock, archive, trash) WERE consumed through a call shape the analyzer could not read and are now resolvable. The rest are listed here rather than left implied by a CODE_PARITY label.",
      "CLOSED by the same pass: the lifecycle controls were offered UNCONDITIONALLY and there was no UNLOCK at all - a record could be locked from the phone and never released there, and Archive showed on records already archived, under legal hold, or inside a retention window. Every one of those was a refusal the server was always going to make. All of them now read the canonical `evidence.lifecycle` projection, whose own type states 'Every field is a RESULT. Nothing here lets a client re-derive a verdict', and a blocked action shows the server's reason code as a sentence.",
      "STILL ABSENT and named rather than implied: editing the record label (PATCH /label), creating and deleting relationships, the reviewer-workflow PATCH and its event feed, and downloading the ORIGINAL file. The first three are reviewer-ops surfaces the web gates behind the same enterprise projection as annotations; the original download is a custody-writing GET and needs its own confirmation design on a phone, which is a product decision rather than a port.",
  ...
  "/search": {
    ...
    gaps: [
      ...
      "S15 CROSS-CHECK (2026-09-22): 10 of 11 web routes unconsumed natively. The web /search surface carries SAVED VIEWS (list/create/update/delete), typeahead SUGGEST, relationship lookup, and three OPERATOR reads - diagnostics, reconcile and semantic backfill. The operator three are not product surfaces and are correctly absent. Saved views, suggest and relationships are real absences on a row marked CODE_PARITY, and are named here rather than left implied.",
  ...
  "/settings": {
    ...
    gaps: [
      ...
      "S15 CROSS-CHECK (2026-09-22): 8 of 19 web routes unconsumed natively, and the earlier 'one remaining absence' line was too confident. Cookie consent is genuinely not portable. The AI POLICY trio (GET/PUT /v1/teams/ai-policy, GET ai-assistance-status, ai-usage) and GET /v1/platform/rbac/matrix are workspace-administration reads the web Settings pane carries and Native does not. GET /v1/users/legal-acceptance is read by the legal-acceptance screen under a different route row. POST /v1/identity/data-export/:id/download mints an export download and is the same custody-writing class as the evidence original.",
    ],
  },
  "/reports": {
  ...
  "/evidence-requests/[id]": {
    ...
    gaps: [
      ...
      "S15 CROSS-CHECK (2026-09-22): 3 of 5 web routes unconsumed natively - the per-RESPONSE review and request-more actions, and the deliverable WAIVE. The transitions on the request itself are ported; acting on an individual response is not, and is named here rather than implied by CODE_PARITY.",
