...
| `20280660000000_uc5_ios_screen_capture_acquisition_mode` | EXPAND | yes — apply BEFORE the UC-5 image. Widens the two `acquisition_mode` CHECK constraints (`evidence_acquisition_mode_check`, `capture_sessions_acquisition_mode_check`) to admit `DIRECT_SCREEN_CAPTURE_IOS`, the fourth `isDirectCapture` mode (the PROOVRA iOS app recording the device screen through Apple's user-authorised system broadcast — ReplayKit `RPSystemBroadcastPickerView` plus a PROOVRA Broadcast Upload Extension — in a server-issued capture session). **WHAT:** a CHECK IN-list cannot be extended in place, so each constraint is dropped and re-added with the same name and a strictly wider value set — a constraint SWAP, not a removal. **SAFE:** every existing row already satisfies the narrower set; the pre-UC-5 image writes only the seven prior modes (still admitted), the UC-5 image writes the new mode (admitted only by the widened constraint), so apply before that image. Idempotent (`DROP CONSTRAINT IF EXISTS` + `ADD`). **NOT REHEARSED — AND THIS ROW DOES NOT CLAIM OTHERWISE.** Its UC-1/UC-2/UC-3 siblings each close "Clean-boot rehearsed on disposable PostgreSQL 16; drift OK"; that has NOT been done for this one, because no database is reachable in the environment it was authored in (`TEST_DATABASE_URL` unset, Docker daemon down — recorded as ENVIRONMENT_BLOCKED_DATABASE_PROOF). The operation is byte-for-byte the same shape as the three siblings that WERE rehearsed, which is the strongest statement available here and is not the same statement. **A CLEAN-BOOT REHEARSAL IS OWED BEFORE THIS IS APPLIED.** Rollback is code-first. **NOT APPLIED TO PRODUCTION.** |
