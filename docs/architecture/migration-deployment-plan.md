...
| `20280102000000_billing_payment_abandoned` | EXPAND | no — ONE new value on the `PaymentStatus` enum, `ABANDONED`, with no backfill and no row rewritten. **Why:** PayPal exposes no operation that cancels an unapproved order, so a months-old approval attempt had one honest action ("Re-check") that kept returning the same answer, and the only alternative was a "Cancel payment" button claiming PayPal had stopped something it had not. ABANDONED is the **customer's own act**, written by `abandonPendingPayment` only after a reconciliation confirms the transaction is still open with nothing captured; where the provider knows better the provider's answer is written instead, and where the provider cannot be reached nothing is written at all. It asserts nothing about the provider and reverses no charge, because none was made. The shared transition treats it as terminal for the customer while still letting a proven `SUCCEEDED` through. **No backfill:** an existing PENDING row stays PENDING; marking history abandoned would assert a decision nobody made. Adding an enum value is permitted inside a transaction on PostgreSQL 12+ provided it is not used in the same transaction, and this one uses neither. **Nothing dropped, renamed, retyped or narrowed.** Apply BEFORE deploying API. A rollback before any row reaches it is indistinguishable from today. |
| `20280104000000_operational_incident_platform_uniqueness` | EXPAND | no — ONE partial unique index and nothing else: `operational_incidents_platform_fingerprint_uk ON operational_incidents (fingerprint) WHERE team_id IS NULL`. No column added, altered or dropped; no row read, written or deleted. **WHY:** `@@unique([teamId, fingerprint])` reads as "one row per condition per workspace, and one row per platform condition" and means only the first. A standard PostgreSQL unique index treats NULL as distinct from NULL, so `(NULL,'x')` and `(NULL,'x')` are two different keys — measured against a fully-migrated PostgreSQL 16 before this was written, and both rows insert with no error. Every PLATFORM-scope and LEGACY_UNSCOPED incident has been un-deduplicated since the column became nullable, and `recordIncident`'s read-then-create had nothing underneath it: two evaluators observing one global condition in the same moment wrote two rows and nothing merged them. Those siblings are what a reader counts as separate faults. **WHY IT CARRIES NO DELETE:** a unique index cannot be created over existing duplicates, so a database holding them must CONVERGE first, and convergence merges and deletes rows. That is irreversible, and `full-migration-audit.mjs` classifies `DELETE FROM` in a post-baseline migration as CRITICAL with no guarded form — correctly, because a release should not quietly merge production records while nobody is watching. The two halves are split by AUDIENCE: this migration is safe unattended, and `services/api/scripts/incident-convergence.mjs` merges duplicates after a reviewed dry-run and an explicit operator decision. That script DISCOVERS every referencing column from the catalog and REFUSES to run on one it has no reviewed disposition for — the first draft knew about three relations and the catalog returned twelve. **BEHAVIOUR ON A DIRTY DATABASE:** it RAISES, naming the duplicate group count, the excess row count and the exact commands, with the remediation in DETAIL and HINT. It does NOT skip: a silent skip leaves the invariant unenforced on precisely the database that needed it while the deploy goes green, and nobody finds out until two rows appear again. A stopped deploy is the correct outcome, because shipping code that assumes an invariant onto a database that does not hold it is the defect. **REHEARSED against live PostgreSQL 16:** applies on a clean database (index present); refuses on one seeded with 2 duplicate groups / 3 excess rows; and after `incident-convergence.mjs --apply` plus `prisma migrate resolve --rolled-back`, re-deploys cleanly. Convergence itself was rehearsed on three duplicate rows and preserved earliest first-seen, latest last-seen, the summed occurrence count (42 = 10+25+7), the worst severity by RANK (CRITICAL, not the lexical MAX which returns HIGH), OPEN status from a sibling, all timeline events plus one `merged` event naming the folded ids, and three SLA cycles renumbered 1,2,3 rather than dropped to fit `UNIQUE(incident_id, cycle_number)`. **Apply BEFORE deploying API.** The index enforces an invariant the writer already assumes, so applying it first narrows the window rather than opening one; the writer's P2002 recovery is already live and is a no-op until the index exists. Idempotent — `CREATE UNIQUE INDEX IF NOT EXISTS`, and a no-op when the operator script created the index first. A rollback drops one index and returns the table to the un-deduplicated state it has been in all along. |
| `20280110000000_signer_control_state` | EXPAND | no — ONE new table, `signer_control_state`, plus a CHECK constraint on `status` and one index. No existing column added, altered or dropped; no existing row read, written or deleted; no other table touched. **WHY:** `revokeSigner()` and `retireSigner()` validated a reason, bumped a counter, emitted a security event and returned ok — they wrote no state at all, never checked the signer id existed, and `getCurrentActiveSigners()` recomputes the active set FROM ENVIRONMENT VARIABLES on every request with `status` hardcoded to `active`. So a signer "revoked" behind step-up and a typed REVOKE reappeared ACTIVE on the next page load and kept signing, while the dialog said it "is withdrawn immediately and cannot be used again. This cannot be undone." This table is the durable overlay that makes the two operations real: the environment says which keys the deployment HAS, this says which of them may still be USED, and the signing boundary in both the API and the worker reads it immediately before producing any signature. **WHY IT CARRIES NO BACKFILL:** absence already means ACTIVE, which is what keeps a deployment that has never opened the console signing normally. Registration is `INSERT … ON CONFLICT DO NOTHING` at read time and never an upsert — an upsert would resurrect a revoked signer on the next boot, which is the original defect in a new place. **HOLDS NO SECRET:** `signer_id` is the composite identity the read model already uses (`purpose:provider:keyId:keyVersion`); no key material, no KMS credential, no token, no actor PII beyond the acting user id. **PLAIN CREATE TABLE, not IF NOT EXISTS:** Prisma applies a migration exactly once, so the guard buys nothing, while `IF NOT EXISTS` would silently succeed against a table of a different shape and hide the drift — the repo gate classifies it CRITICAL for that reason. **REHEARSED against live PostgreSQL 16:** applies on a database migrated from zero (256 of 256) and on one already carrying the full history; a REVOKED row then survives an API restart and a worker restart, the signing boundary refuses it in both processes, discovery does not reset it to ACTIVE, and an untouched signer still signs. **Apply BEFORE deploying API.** An empty table changes no behaviour, so applying it first is inert; deploying the code first would leave the console writing to a table that does not exist. A rollback drops one empty table and returns retire/revoke to the no-ops they were. |
| `20280115000000_worker_lease_and_heartbeat_retention` | EXPAND | no — ONE new table, `worker_leases`, ONE enum, `WorkerLeaseState`, its own index, and ONE guarded index on the existing `worker_telemetry_snapshots`. No existing column added, altered or dropped; no existing row read, written or deleted. **WHY:** `worker_telemetry_snapshots` is append-only — one row per worker per heartbeat interval, with no retention — so at the shipped 60s interval it grows 1,440 rows per worker per day for ever. Liveness was derived by scanning the newest 200 of those rows and deduplicating them in memory, which additionally drops instances out of a fleet large enough to overflow the window, and which cannot represent a clean shutdown AT ALL — a drained worker and a killed worker both look like a last heartbeat followed by silence, so a deliberate scale-down read as a crash for the length of the stale threshold. The lease is CURRENT STATE: one row per instance, upserted in place, carrying the worker's own lifecycle (STARTING, LIVE, DRAINING, STOPPED) plus last-seen, build revision and queue subscriptions. STALE is DERIVED from a LIVE lease past the freshness window with NO stop marker — a crash cannot write a marker, which is exactly what makes its absence evidence. **WHY THE HISTORY TABLE SURVIVES:** the operations dashboard, the trust status probes and the reconcile/telemetry health probes all still read it, so dropping it here would be a silent capability change wearing the costume of a cleanup. It is BOUNDED instead — 30-day retention, swept in capped batches under a PostgreSQL advisory lock so exactly one instance sweeps per tick rather than the whole fleet running overlapping DELETEs over the same rows. **WHY IT CARRIES NO BACKFILL:** a lease is current state, not history; every running worker writes its own row on its next heartbeat (at most one interval), and synthesising one would be a claim about a process this migration has not heard from. An empty table reads as NOT_MEASURED — "no worker has ever registered" — which is the honest answer during the window between migrating and deploying, and is not a claim that the fleet is unhealthy. **THE GUARDED INDEX:** the retention predicate is `heartbeat_at_utc < cutoff`, and every existing index on that table leads with a different column, so none can serve a bare age scan. It is created inside a DO block that checks `information_schema` first, because it indexes a column this migration does not create — the failure class the Phase O gate exists to prevent. **HOLDS NO SECRET:** worker id, lifecycle state, timestamps, a build revision string and queue NAMES; no payloads, no URLs, no credentials. **REHEARSED against live PostgreSQL 16:** applies on a database migrated from zero and on one already carrying the full history, converging on identical schema in both. **Apply BEFORE deploying API and Worker.** An empty lease table changes nothing for the running image, which does not read it; deploying the code first would leave the worker upserting into a table that does not exist and the Admin fleet view reading UNAVAILABLE. A rollback drops one table and one enum, removes one index, and returns liveness to the heartbeat scan it used before. |
| `20280125000000_intake_customer_id` | EXPAND | no — TWO nullable columns, `workflow_intake_links.customer_id` and `evidence.intake_customer_id`, and one tenant-scoped index on each. No existing column altered or dropped; no existing row read, written or deleted. **WHY:** an organization refers to the person behind a submission by ITS OWN identifier — a claim number, a policy number, a case reference — and had no way to attach that to an intake link or to find the resulting record by it. PROOVRA neither issues nor validates the value. **WHY TWO COLUMNS:** the link is AUTHORITATIVE — it is where the organization stated the association — and the evidence column is a SNAPSHOT taken when the submission became a record, so a later correction on the organization's side cannot silently rewrite a finished record's provenance, and so search can filter an indexed column instead of joining session → link on every query. **WHY NO BACKFILL:** there is nothing to derive it from; the value exists only where an operator typed it, and inventing one would assert a customer association nobody made. **A CONSEQUENCE WORTH RECORDING:** because the snapshot is written only at submission and only from this migration forward, evidence submitted before it reached production keeps `intake_customer_id` NULL whatever its link says — so evidence search also matches the link's own value, and those records stay findable by the identifier their organisation uses. **HOLDS NO SECRET:** an identifier the organization already holds and supplied. **Apply BEFORE deploying API.** Both columns are nullable and unread by the previous image; deploying the code first would leave it writing columns that do not exist. |
| `20280401000000_search_projection_version` | EXPAND | no — ONE `ADD COLUMN IF NOT EXISTS evidence_search_documents.projection_version INTEGER NOT NULL DEFAULT 1` and one index on `(team_id, projection_version)`. No existing column altered or dropped; no existing row read, written or deleted. **WHY:** a search document is a CACHE of `buildEvidenceProjection`, and when the projection began indexing External Intake identity — Customer ID, recipient name, address, phone — every document already written became unable to answer a question about those fields while still looking entirely valid. Nothing could detect it: the source row was untouched so no timestamp moved, and the reindex only ever selected evidence rows with NO document (`esd.id IS NULL`), so a record that already had one was invisible to the cron sweep, `POST /v1/search/reconcile` and the backfill CLI alike. Proven against a live database: a document with the identity stripped from its body failed all four identity probes, a full workspace reconcile reported 14 documents indexed, and the stripped body came back byte-for-byte identical. **THE DEFAULT IS 1, NOT THE CURRENT VERSION:** every existing row predates the identity fields and must report as stale so the reindex refreshes it — defaulting to the current version would declare the whole production index healthy and leave the defect behind a green number. **WHY NO BACKFILL HERE:** the repair is a REINDEX, not an UPDATE; stamping the version without rebuilding the body would mark stale documents current and permanently hide the very records this exists to find. The refresh runs through the same canonical `indexEvidence`, per workspace, in bounded batches, under the existing durable per-workspace lock, and is idempotent. **HOLDS NO SECRET:** an integer naming which builder wrote a row. **REHEARSED against live PostgreSQL 16:** applies on a database migrated from zero (262 of 262) and on one already carrying the previous schema with 229 documents, converging on an identical column signature with the index present in both, 229 rows conserved and every pre-existing row taking version 1 with no UPDATE. **Apply BEFORE deploying API.** The column has a default so the ALTER does not rewrite the table on PostgreSQL 11+, and the previous image neither reads nor writes it. A rollback drops one integer column and one index. |
| `20280120000000_break_glass_single_active_grant` | EXPAND | no — ONE partial unique index and nothing else: `emergency_access_grants_active_org_user_uk ON emergency_access_grants (organization_id, emergency_user_id) WHERE status = 'ACTIVE'`. No table, column, type, enum or constraint added, altered or dropped; no row read, written or deleted. **WHY:** `activateBreakGlass` created unconditionally. Measured against a live fixture, FOUR simultaneous activations — each carrying its own genuinely approved, single-use step-up challenge — produced FOUR overlapping ACTIVE grants for the same organization and the same emergency user. Every one was individually authorized, so this is not an authorization hole; it is an operational one. The staff console lists live emergency access over a customer's data, and revoking the grant an operator can SEE leaves three they cannot — emergency access that survives its own revocation is the precise failure mode break-glass exists to bound. **WHY THE DATABASE AND NOT THE SERVICE:** a read-then-create cannot close it — two callers both read "none active" and both create. The exclusion has to be an index, and the service's P2002 recovery turns the collision into a typed `BREAK_GLASS_ALREADY_ACTIVE` 409 rather than a 500. **WHY PARTIAL:** only ACTIVE rows are exclusive. Revoked and expired grants are history and must accumulate freely; a full unique index would make it impossible to grant emergency access to the same person twice, ever. **BEHAVIOUR ON A DIRTY DATABASE:** a database already holding overlapping ACTIVE grants cannot build the index, and the deploy STOPS — the migration RAISEs, naming the conflicting group count, with the remediation in the HINT. It does NOT skip: converging live emergency access is an operator decision about who currently holds privilege over customer data, not something a migration should do unattended, and a silent skip would leave the invariant unenforced on precisely the database that needed it while the deploy went green. **THE COLUMN GUARD:** the CREATE is preceded by an `information_schema` existence check naming `organization_id`, `emergency_user_id` and `status` literally, because it indexes columns this migration does not create — the failure class the Phase O gate exists to prevent. **HOLDS NO SECRET:** an index over two foreign keys and a status string. **REHEARSED against live PostgreSQL 16:** applies on a database migrated from zero and on one already carrying the full history, converging on an identical partial index in both. **Apply BEFORE deploying API.** The index enforces an invariant the writer already assumes, so applying it first narrows the concurrency window rather than opening one, and the typed-409 recovery is a no-op until the index exists. A rollback drops one index and returns activation to the unconstrained create it was. |

| `20280215000000_admin_audit_identity_contract` | EXPAND | no — NINE nullable columns on `admin_audit_logs` (`actor_type`, `actor_display`, `actor_authority`, `target_display`, `previous_state`, `requested_state`, `resulting_state`, `reason_code`, `event_version`) and ONE index, `admin_audit_logs_actor_type_created_at_idx`. No existing column altered or dropped; no existing row read, written or deleted. **WHY:** `user_id` alone could not answer *who acted*. It is NULL for every automated event, so a scheduled worker and an unattributed action were the same row shape; it recorded no authority, so an action taken as platform staff looked identical to one taken as a workspace member; and it resolved through a live join, so the record stopped being readable exactly when it matters most — after the account is renamed, anonymised or deleted. Nor could a row answer *what changed*: one `outcome` column with three values and no state fields, so a refused request and a successful one differed by a word, and neither said what was ASKED FOR or what storage now holds. Three state columns rather than two, because a refusal has a requested state and no resulting change — collapsing them makes a refusal indistinguishable from a success. **EXPAND-ONLY AND SAFE BEFORE CODE DEPLOYMENT:** every column is nullable and NO backfill runs. A deployment still on the previous image writes NULL into all nine and continues unchanged; the reader presents a NULL `actor_type` as UNKNOWN_LEGACY, which is the honest answer for a row written before the contract existed and is not a claim about who acted. **WHY IT CARRIES NO BACKFILL:** an audit trail is append-only. Historical rows do not carry the identity these columns record, and inventing plausible values would be fabricating an account of who did what — the one thing this table exists NOT to do. Their hashes also legitimately cover the values they were written with, so an UPDATE would invalidate them. `event_version` DEFAULTs to 1 so existing rows describe themselves as the pre-contract shape without an UPDATE touching a single one; new rows are written as 2. **HASHING:** these columns are bound into chain **V4**. V1–V3 rows are never rehashed and still verify with the algorithm of their own `chain_version`, linking across versions through `prev_hash` exactly as V1→V2→V3 already does. An attribution field that verification did not cover would be WORSE than an absent one — it would look authoritative and remain editable, so verification would pass while the record lied. **THE INDEX NEEDS NO GUARD:** it indexes `actor_type`, which this migration creates, so it cannot reference a column that does not exist — the failure class the Phase O gate exists to prevent does not apply. **HOLDS NO SECRET:** a safe display label, an authority name, state names and a reason code. The label is a display name, or a MASKED email when the account has none; the raw address is never copied into the trail. **REHEARSED against live PostgreSQL 16:** applies on a database migrated from zero (260 migrations) and on one already carrying the previous schema with 25 historical rows, converging on identical schema in both — row conservation 25 → 25, every historical row keeping NULL identity, `event_version` 1 and its original hash. **Apply BEFORE deploying API and Worker.** The writers emit V4 and populate the nine columns; deploying the code first would have every audit write fail on unknown columns, which for a fail-closed audit path means sensitive Admin actions refusing rather than proceeding unlogged. A rollback drops nine nullable columns and one index, and returns the writers to V3. |
| `20280301000000_evidence_credit_admin_grant` | EXPAND | no — ONE enum value (`EvidenceCreditEntryType.ADMIN_GRANT`), ONE nullable column (`evidence_credit_ledger_entries.grant_ref`) and ONE partial UNIQUE index (`evidence_credit_ledger_grant_ref_key`). No existing column altered or dropped; no existing row read, written or deleted. **WHY:** the wallet had one way to gain credits — a completed Stripe or PayPal payment — so support remediation, goodwill and controlled internal testing had no mechanism, and the workaround for that is always to write a PURCHASE row: a payment that never happened, in the one table whose purpose is to prove where every credit came from, which the customer's own billing history reads. **EXPAND-ONLY AND SAFE BEFORE CODE DEPLOYMENT:** the column is nullable with no default and no backfill; an enum GAINS a value. A deployment still on the previous image never writes ADMIN_GRANT and never reads `grant_ref`. **WHY NO BACKFILL:** the ledger is append-only and no historical row is a grant — every credit that has entered a wallet arrived through a provider payment, and relabelling one would invent a support decision nobody made. **THE INDEX PREDICATE IS `grant_ref IS NOT NULL`:** PostgreSQL refuses to use a newly added enum value inside the transaction that added it, so naming ADMIN_GRANT would fail this migration on a fresh database; only grant rows carry a `grant_ref`, so the predicates select the same rows. It is the idempotency guarantee at the database, the same shape as the existing PURCHASE `(provider, provider_ref)` guard, so a retried grant cannot double-credit even when two requests race. **HOLDS NO SECRET:** `grant_ref` is `<targetUserId>:<operator reference>`, minted server-side; the reason and the acting admin live in the platform audit trail. **Apply BEFORE deploying the API.** The route writes ADMIN_GRANT rows; deploying the code first would fail every grant on an unknown enum value. A rollback drops one nullable column and one index and leaves the enum value unused. |
| `20280501000000_workspace_invite_lifecycle_hardening` | BACKFILL | yes — and it must be applied BEFORE the API image that reads it. **WHAT:** on `team_invites` — the invitation that actually grants WORKSPACE membership — it adds `token_hash VARCHAR(64)` (backfilled, then NOT NULL), makes `token` nullable, adds `revoked_at`, `revoked_by_user_id`, `accepted_by_user_id`, `last_resent_at`, `resend_count INTEGER NOT NULL DEFAULT 0`, one UNIQUE index on the hash, and one PARTIAL UNIQUE index on (`team_id`, lower(`email`)) WHERE `accepted_at IS NULL AND revoked_at IS NULL`. No column is dropped and no row is deleted. **WHY:** this table was the least protected of the invitation tables. The token was stored in PLAINTEXT and uniquely indexed, so a backup was a set of live workspace credentials; the whole row including the token was returned by the create and resend responses, so any workspace ADMIN reading the API held live tokens for every pending invitation; there was no revocation STATE at all, so "revoking" meant DELETING the row and destroying the record that it was ever sent; and there was no resend record, so a chased invitation could not be told from an ignored one. `organization_invites` solved all four in `20260927000000_p2_7x_stage6_invite_token_hash`; this is deliberately the same shape, statement for statement, so the two tables converge rather than diverging into a second design. **WHY BACKFILL AND NOT EXPAND:** it writes to existing rows. `token_hash` is computed from the token each pending invitation already carries, so in-flight links keep resolving after the read path switches to the hash. Rows holding no token cannot be accepted and cannot be hashed, so they get a random unmatchable value — the NOT NULL stays honest without inventing a hash a real token could collide with. Both UPDATEs are conditioned on `token_hash IS NULL`, so re-running is a no-op. **THE PARTIAL INDEX IS A CORRECTION, NOT A CONVENIENCE:** one pending invitation per address per workspace used to be a read-then-write in the route, which two concurrent requests could both pass. **APPLY BEFORE THE API:** the previous image neither reads nor writes any new column and `token` still holds its value, so applying early is safe; the new image reads `token_hash`, so deploying it first would fail every invitation lookup. **THE RAW TOKEN IS DELIBERATELY RETAINED** — clearing and dropping it is a separate later CONTRACT_DROP once no deployed service reads it, so rolling the SERVICE back does not strand live invitations. **REHEARSED** on disposable PostgreSQL 16 in both directions: clean boot from an empty database, and an upgrade over a database carrying pending plaintext-token invitations, which still resolved afterwards. Readiness: `SELECT count(*) FROM team_invites WHERE token_hash IS NULL` — must be 0. **NOT APPLIED TO PRODUCTION.** |
| `20280503000000_collaboration_scale_indexes` | EXPAND | yes — TWO `CREATE INDEX IF NOT EXISTS` statements and nothing else: `team_members (team_id, created_at, id)` and `collaboration_team_assignments (team_id, created_at DESC, id DESC)`. **WHY:** both collaboration reads grew a cursor in the same pass — the workspace detail used to load EVERY membership row on every page load, and `listAssignments` answered `take: 200` with no cursor, which is a truncation rather than a page. A keyset page without a matching index is a sequential scan plus a sort of the whole partition on every page, so the pagination added to make a large workspace readable would have made it slower instead; each index matches its query's ORDER BY exactly, including direction. An index changes how a query is answered, never what it answers, so it is safe on either image and idempotent on re-run. Rehearsed clean-boot and upgrade on disposable PostgreSQL 16; `db:drift-check` in sync. **NOT APPLIED TO PRODUCTION.** |
| `20280510000000_enterprise_contract_collaboration_limits` | EXPAND | yes — two nullable `INTEGER` columns on `enterprise_contracts` (`collaboration_teams_max`, `collaboration_team_members_max`) plus two CHECK constraints refusing a non-positive value. **WHY:** `resolveEnterpriseContractLimits` already turned a contract row into enforceable seats, storage, evidence records and AI operations; collaboration was not in that list. So how many Collaboration Teams a workspace may hold came from a flat catalog `1000`, and how many people may sit inside one came from a flat catalog `500` — and because the per-group ceiling is reconciled down to the seat entitlement with a `min()`, an organization contracted for 800 seats could put only 500 of them in a single group. A self-serve placeholder was capping a signed agreement, and no contract could state how many groups it bought because there was nowhere to write it. **SEMANTICS ARE THE EXISTING ONES**, deliberately imitated rather than invented: `NULL` means the contract is SILENT and the canonical ENTERPRISE catalog default governs (surfaces render "Contract-managed"); a stated value is ENFORCED and wins outright, above or below the catalog, with no `max()`; and a non-ACTIVE contract grants neither, because `resolveEnterpriseContractLimits` fails closed on DRAFT/PENDING_ACTIVATION/SUSPENDED/TERMINATED before either column is read. **NO BACKFILL IS POSSIBLE OR CORRECT:** an existing contract states nothing about collaboration and resolves exactly as it did before; writing a number here would be inventing a contract term. The CHECK constraints mirror `normalizeContractAllowance` so a hand-written UPDATE cannot introduce what the service rejects — a zero would be read downstream as "silent" and silently promoted to the catalog default, hiding the error. Safe on either image; idempotent. **NOT APPLIED TO PRODUCTION.** |
| `20280511000000_collaboration_activity_keyset_index` | EXPAND | yes — one `CREATE INDEX IF NOT EXISTS` on `collaboration_team_activity (team_id, created_at DESC, id DESC)`. **WHY:** `listTeamActivity` paged on `id` while ordering by `created_at` alone, so two rows sharing a timestamp had no defined order between requests and a page boundary could move — rows repeating or vanishing between pages. Ties are the NORMAL case here, not an edge case: `recordActivity` writes inside the mutation's own transaction and several events routinely commit together with identical timestamps. The service now orders by the stable pair, which is the treatment `listAssignments` received when it was paginated; this index is the other half. The existing `(team_id, created_at DESC)` index cannot serve it — a keyset walk needs the tiebreaker in the index, or PostgreSQL sorts the whole team partition on every page, making the pagination slower than the truncation it replaced. An index changes how a query is answered, never what it answers. Phase O-final guard pattern. **NOT APPLIED TO PRODUCTION.** |
| `20280512000000_enterprise_contract_legal_hold` | EXPAND | yes — ONE nullable `BOOLEAN` column on `enterprise_contracts` (`legal_hold_enabled`), behind the same Phase O-final table-existence guard its neighbours use. No existing column altered or dropped, no index, no constraint, no row read or written. **WHY:** Legal Hold is an ENTERPRISE governance capability whose ACTIVATION is a term of the agreement, and the contract had nowhere to state it. `resolveEnterpriseContractLimits` could already turn a contract row into enforceable seats, storage, evidence records, AI operations and collaboration capacity — but not a capability, so the runtime gate `FEATURE_LEGAL_HOLD` was fed only by `EntitlementGrant` rows written by `applyProductLine`, which is not the contract. An ENTERPRISE workspace with no product line applied resolved to the default `false` and could not place a hold the catalog advertised, while two governance routes reached the same canonical writer and checked no entitlement at all. **SEMANTICS DELIBERATELY DIFFER FROM THE NUMERIC NEIGHBOURS:** for an ALLOWANCE, `NULL` means the contract is silent and the catalog default governs — safe, because an allowance has a published default. A CAPABILITY has no safe default: reading silence as a grant would be the `plan === ENTERPRISE` shortcut wearing a contract's clothes and would hand a governance capability to every Enterprise customer. So `NULL` and `false` both mean NOT GRANTED, and only an explicit `true` grants — and only while the contract is ACTIVE, because the resolver fails closed on DRAFT/PENDING_ACTIVATION/SUSPENDED/TERMINATED before the term is read. **NO BACKFILL IS POSSIBLE OR CORRECT:** no contract has ever stated this term, `NULL` is exactly the behaviour every existing row has today, and writing `true` anywhere would be inventing a contract nobody signed. **SAFE ON EITHER IMAGE:** an older image never selects the column; a newer one reading it finds the same answer the product gave before the migration. **TOUCHES NO HOLD:** commercial state governs ADMISSION to creating a NEW hold and nothing else — an ACTIVE hold keeps blocking destruction through downgrade, expiry, entitlement removal and suspension, because the canonical destruction formula takes only lifecycle facts and has no plan or entitlement parameter. Rollback is code-first: revert the API image and LEAVE the column: dropping it is a CONTRACT_DROP requiring its own migration and its own decision. Clean-boot rehearsed against disposable PostgreSQL 16 — full chain applied from an empty database. **NOT APPLIED TO PRODUCTION.** |
| `20280601000000_uc0_acquisition_provenance_foundation` | BACKFILL | yes — additive only, and it carries its own deterministic backfill. **WHAT:** `evidence.acquisition_mode` + `acquisition_mode_source` (nullable, bounded CHECKs, a pair CHECK, a `(team_id, acquisition_mode)` index and a BEFORE UPDATE set-once trigger); `evidence_parts.artifact_class NOT NULL DEFAULT 'ORIGINAL'` (true for every existing part — no capture manifest was ever stored and derivatives are never parts); `reports.acquisition_mode_snapshot`; direct-capture session columns on `capture_sessions` with a unique `nonce_sha256` and enum values `ACTIVE`/`BOUND`/`INTERRUPTED`; `capture_device_attestations.verifier_version`; and on `evidence_part_derived_assets` a lineage descriptor (`transformation`, `parameters_sha256`, `source_offset_ms`, `variant_key DEFAULT 'default'`) with the canonical `(team, part, kind, variant)` unique index. **WHY:** PROOVRA had no truthful persisted answer to how evidence entered it — `capture_method` was overwritten by completion with a STRUCTURE value and `uploadSource` was inverted for the mobile and citizen routes. **BACKFILL (owner decision D9):** only rows proven by the UNIQUE `workflow_intake_sessions.evidence_id` join are set to `SECURE_INTAKE_LINK`, marked `BACKFILL_INTAKE_SESSION_LINK`, and only while NULL; nothing is inferred from any other field, so every other historical row stays NULL ("not recorded"). **SAFE ON EITHER IMAGE:** the narrower `(team, part, kind)` unique index is RETAINED so an older image's `ON CONFLICT` target still exists; it is removed only by a later CONTRACT migration. The trigger only refuses CHANGING a non-null acquisition value, which no code writes. Clean-boot rehearsed on disposable PostgreSQL 16 + pgvector; drift and raw-schema verification OK; re-executed for idempotency. Rollback is code-first: revert the images and LEAVE the schema expanded. **NOT APPLIED TO PRODUCTION.** |
| `20280602000000_derived_asset_variant_contract` | CONTRACT_DROP | **no — apply ONLY after the UC-0 image is live everywhere.** One guarded `DROP INDEX IF EXISTS "evidence_part_derived_assets_team_part_kind_uk"` preceded by a `DO`-block that RAISEs unless the composite `(team, part, kind, variant)` unique key exists. **WHAT:** retires the narrow `(team, part, kind)` unique key so a second variant of one kind on the same part can coexist (e.g. UC-4 keyframes); the composite key added by `20280601000000` keeps identical variants idempotent. **WHY LATER:** the pre-UC-0 image upserts `ON CONFLICT (team_id, evidence_part_id, asset_kind)` against the narrow key, so dropping it under that image would fail every derived-asset write — the hazard exists in one direction only, because the UC-0 image upserts against the composite key and never needs the narrow one. **SAFE:** an index is not data; `DROP INDEX IF EXISTS` loses no rows and is idempotent; the guard makes it impossible to leave derived assets with no uniqueness. Clean-boot rehearsed on disposable PostgreSQL 16 (the full chain from empty leaves only the composite key); drift and raw-schema verification OK; multiple variants of one kind proven to coexist while an identical variant stays idempotent. Rollback is code-first: the index can be recreated. **NOT APPLIED TO PRODUCTION.** |
| `20280650000000_uc4_screen_intelligence` | EXPAND | yes — apply BEFORE the UC-4 image. Three additive changes for DERIVED screen intelligence. **WHAT:** (1) widens `media_intelligence_runs_kind_bounded` to admit run kind `reconstruct_screen` (UC-4 reuses the durable `MediaIntelligenceRun` lifecycle on the existing media-intelligence queue — no second queue); (2) widens `evidence_part_derived_assets_kind_bounded` to admit `video_keyframe` + `screen_reconstruction` (UC4-C declared these in code but a bounded CHECK enumerated only the five original kinds, so `recordDerivedAsset` would have been rejected at runtime — CHECKs are invisible to `prisma migrate diff`, so raw-schema-verify never surfaced it); (3) adds two labels to `EvidenceExtractedTextKind` (`OCR_SCREEN` = DERIVED_MACHINE_EXTRACTED, `SCREEN_RECONSTRUCTION` = DERIVED_RECONSTRUCTED) so UC-4 derived text persists through the canonical extracted-text authority. Both CHECKs are dropped and re-added with the same name and a strictly wider value set — a constraint SWAP, not a removal; enum `ADD VALUE IF NOT EXISTS` is additive. **SAFE:** every existing row already satisfies the narrower sets; the pre-UC-4 image writes none of the new kinds/labels (still admitted), the UC-4 image writes them (admitted only by the widened constraints), so apply before that image. Idempotent (`DROP CONSTRAINT IF EXISTS` + `ADD`; `ADD VALUE IF NOT EXISTS`). Clean-boot rehearsed on disposable PostgreSQL 16; raw-schema-verify OK 880/0. Rollback is code-first. **NOT APPLIED TO PRODUCTION.** |
| `20280640000000_uc3_continuous_screen_capture_acquisition_mode` | EXPAND | yes — apply BEFORE the UC-3 image. Widens the two `acquisition_mode` CHECK constraints to admit `DIRECT_SCREEN_CAPTURE_ANDROID_CONTINUOUS`, the third `isDirectCapture` mode (the PROOVRA Android app recording a continuous screen session — ordered segments — via MediaProjection). **WHAT:** a CHECK IN-list cannot be extended in place, so each constraint is dropped and re-added with the same name and a strictly wider value set — a constraint SWAP, not a removal. **SAFE:** every existing row already satisfies the narrower set; the pre-UC-3 image writes only the five prior modes (still admitted), the UC-3 image writes the new mode (admitted only by the widened constraint), so apply before that image. Idempotent (`DROP CONSTRAINT IF EXISTS` + `ADD`). Clean-boot rehearsed on disposable PostgreSQL 16; drift OK. Rollback is code-first. **NOT APPLIED TO PRODUCTION.** |
| `20280660000000_uc5_ios_screen_capture_acquisition_mode` | EXPAND | yes — apply BEFORE the UC-5 image. Widens the two `acquisition_mode` CHECK constraints (`evidence_acquisition_mode_check`, `capture_sessions_acquisition_mode_check`) to admit `DIRECT_SCREEN_CAPTURE_IOS`, the fourth `isDirectCapture` mode (the PROOVRA iOS app recording the device screen through Apple's user-authorised system broadcast — ReplayKit `RPSystemBroadcastPickerView` plus a PROOVRA Broadcast Upload Extension — in a server-issued capture session). **WHAT:** a CHECK IN-list cannot be extended in place, so each constraint is dropped and re-added with the same name and a strictly wider value set — a constraint SWAP, not a removal. **SAFE:** every existing row already satisfies the narrower set; the pre-UC-5 image writes only the seven prior modes (still admitted), the UC-5 image writes the new mode (admitted only by the widened constraint), so apply before that image. Idempotent (`DROP CONSTRAINT IF EXISTS` + `ADD`). **CLEAN-BOOT REHEARSED (2026-09-22).** The rehearsal this row previously recorded as OWED has been executed, and the row is updated because it was done rather than because the release wanted it done. Disposable PostgreSQL 16 (`pgvector/pgvector:pg16` — the plain `postgres:16` image fails earlier, at `20271119000000_search_document_embedding_after_extension`, which refuses to leave `evidence_search_documents.embedding` undeclared when pgvector is absent). Every registered migration applied by `prisma migrate deploy` on an empty database, ending "All migrations have been successfully applied". Verified after: `db:drift-check` OK (schema and migrations in sync), `db:raw-schema-verify` OK (880 registered objects, 0 unregistered divergences, 0 proposed for removal), and the widened constraint read back from the live catalogue — `evidence_acquisition_mode_check` admits `DIRECT_SCREEN_CAPTURE_IOS`. Drift OK. Rollback is code-first. **NOT APPLIED TO PRODUCTION.** |
| `20280680000000_artifact_recovery_progress` | EXPAND | yes — apply BEFORE the image that writes it. Adds nullable `reports.pdf_sha256`, `verification_packages.report_version` / `report_sha256`, `report_generation_requests.report_version` / `stage` / `intent` / `client_request_key`, and one DO-guarded unique index on `report_generation_requests(evidence_id, client_request_key)` (NULL keys stay distinct, so existing rows are unaffected). **WHAT:** purely additive; nothing existing is dropped, narrowed, renamed or backfilled. **WHY:** package-only recovery must embed the STORED report only after verifying its bytes against a recorded hash; a package must reference the report version it certifies; a retry must resume from the report version its request committed instead of minting another; an explicit new version must be replayable after a lost response. **SAFE:** the prior build never reads or writes these columns. **CLEAN-BOOT REHEARSED (2026-09-26)** on disposable PostgreSQL 16 (`pgvector/pgvector:pg16`): `prisma migrate deploy` applied every migration from an empty database, `db:drift-check` OK, `db:raw-schema-verify` OK (880 objects, 0 unregistered divergences). Behaviour proven by `services/api/test/point5/report-package-recovery.integration.test.ts` against a real database and the real worker processor. Rollback is code-first; the columns are inert to any build that does not read them. **NOT APPLIED TO PRODUCTION.** |
| `20280670000000_bd2_durable_batch_analysis_jobs` | EXPAND | yes — apply BEFORE the image that reads it. Creates `batch_analysis_jobs` and `batch_analysis_job_items` plus the two enums they use. **WHAT:** purely additive. Nothing existing is dropped, narrowed, renamed or backfilled, and there is NO backfill of its own — the rows it replaces only ever lived in the memory of a process that has since exited, so there is nothing to migrate. **WHY:** `batch-analysis.service.ts` kept jobs in a plain object on a module singleton, so a restart lost every job and two API instances could not see each other's (BD-2). **SAFE:** a deployment still running the prior build is unaffected — it keeps its jobs in memory and never reads these tables — so the ordering requirement is one-way only: table first, then the image. **CLEAN-BOOT REHEARSED (2026-09-22)** on disposable PostgreSQL 16 (`pgvector/pgvector:pg16`): `prisma migrate deploy` applied all 278 migrations from empty, `db:drift-check` OK, `db:raw-schema-verify` OK (880 objects, 0 divergences). Behaviour proven by `services/api/test/batch-analysis-durability.integration.test.ts` — 15 cases against a real database, including that only ONE instance can claim a job and that a second connection sees the row. Rollback is code-first; the tables are inert to any build that does not read them. **NOT APPLIED TO PRODUCTION.** |
| `20280630000000_uc2_screen_capture_acquisition_mode` | EXPAND | yes — apply BEFORE the UC-2 image. Widens the two `acquisition_mode` CHECK constraints (`evidence_acquisition_mode_check`, `capture_sessions_acquisition_mode_check`) to admit `DIRECT_SCREEN_CAPTURE_ANDROID`, the second `isDirectCapture` mode (the PROOVRA Android app via MediaProjection, in a server-issued capture session). **WHAT:** a CHECK IN-list cannot be extended in place, so each constraint is dropped and re-added with the same name and a strictly wider value set — a constraint SWAP, not a removal. **SAFE:** every existing row already satisfies the narrower set; the pre-UC-2 image writes only the four prior modes (still admitted), the UC-2 image writes the new mode (admitted only by the widened constraint), so apply before that image. Idempotent (`DROP CONSTRAINT IF EXISTS` + `ADD`). Clean-boot rehearsed on disposable PostgreSQL 16; drift OK. Rollback is code-first. **NOT APPLIED TO PRODUCTION.** |
| `20280610000000_uc1_direct_web_capture_acquisition_mode` | EXPAND | yes — apply BEFORE the UC-1 image. Widens the two `acquisition_mode` CHECK constraints (`evidence_acquisition_mode_check`, `capture_sessions_acquisition_mode_check`) to admit `DIRECT_WEB_CAPTURE_EXTENSION`, the first `isDirectCapture` mode (the PROOVRA browser extension, in a server-issued capture session). **WHAT:** a CHECK IN-list cannot be extended in place, so each constraint is dropped and re-added with the same name and a strictly wider value set — a constraint SWAP, not a removal. **SAFE:** every existing row already satisfies the narrower set; the pre-UC-1 image writes only the three UC-0 modes (still admitted), the UC-1 image writes the new mode (admitted only by the widened constraint), so apply before that image. Idempotent (`DROP CONSTRAINT IF EXISTS` + `ADD`). Clean-boot rehearsed on disposable PostgreSQL 16; drift OK. Rollback is code-first. **NOT APPLIED TO PRODUCTION.** |
| `20280620000000_uc1_extension_oauth_codes` | EXPAND | yes — apply BEFORE the UC-1 image. One new table `extension_auth_codes` backing the first-party extension Authorization Code + PKCE (S256) flow. **WHAT:** `CREATE TABLE IF NOT EXISTS extension_auth_codes` (id, `code_hash` unique, `user_id` FK → `users(id)` ON DELETE CASCADE behind an existence guard, `client_id`, `redirect_uri`, `code_challenge`, `scope`, `expires_at_utc`, `used_at_utc` nullable, `created_at_utc`) + a unique index on `code_hash` + an index on `expires_at_utc`. The authorization code is NEVER stored — only its SHA-256; it is single-use (atomic `UPDATE ... WHERE used_at_utc IS NULL`), short-lived, and bound to `client_id`, `redirect_uri` and `code_challenge`. **SAFE:** EXPAND — nothing existing is altered, no column changed, no row read or written. The pre-UC-1 image never touches the table; the UC-1 image's `/v1/oauth/extension/authorize` + `/token` routes read it, so apply before that image. Idempotent (`CREATE TABLE/INDEX IF NOT EXISTS` + guarded FK `ADD`). **NOT A SECOND AUTH SYSTEM:** the token endpoint mints an ordinary short-lived `AUTH_JWT` that the canonical `requireAuth` already accepts. Clean-boot rehearsed on disposable PostgreSQL 16; drift OK. Rollback is code-first: revert the image and LEAVE the table. **NOT APPLIED TO PRODUCTION.** |
...
13 migrations. Each mutates rows; none drops anything.
...
| `20280210000000_intake_identity_search` | `SELECT count(*) FROM workflow_intake_links WHERE recipient_phone IS NOT NULL AND recipient_phone_e164 IS NULL AND (normalised LIKE '+%' OR digits LIKE '00%')` — must be 0 | links whose typed number IS normalisable but carry no canonical form |
