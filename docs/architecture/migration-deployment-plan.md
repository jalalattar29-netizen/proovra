...
| `20271225000000_operational_incident_legacy_column_drop` | CONTRACT_DROP | NO — apply AFTER the API and Worker images are running and healthy. Drops the 28 legacy columns once the expand half above has backfilled and proven conservation. Separate from that half because `verify-migration-artifact.mjs` refuses a CONTRACT_DROP that removes something unless it declares `safeBeforeCodeDeployment: false`, and folding the two together would have forced a false statement in one direction or the other. Re-proves conservation immediately before dropping and RAISEs rather than destroying the only copy of any value. No shipped image reads these columns, so it is invisible to the application |
...
7. once those are healthy, apply
   `20271225000000_operational_incident_legacy_column_drop`, which removes the
   legacy columns. It is deliberately last: a removal should always wait, and
   nothing needs it to have happened for Operations to work.
