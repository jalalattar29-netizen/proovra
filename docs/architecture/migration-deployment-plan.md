...
| `20271224000000_operational_incident_naming_convergence` | CONTRACT_DROP | yes, and it is the one that must apply BEFORE the API and Worker images — the writer is broken until it does. Removes the LEGACY camelCase column family from `operational_incidents` and `operational_incident_events`: the duplicate columns a generation of the model without `@map` created beside the canonical snake_case ones, documented on OTHER tables by `20260620200000_reviewer_ops_naming_drift_repair`, whose stated cleanup ("a separate cleanup migration will drop them") was never written for these. Measured, not theorised: `safe_summary` is `VARCHAR(400) NOT NULL` with no default, so its legacy twin is too, and the real `recordIncident` fails **P2011 / 23502 `null value in column "safeSummary"`** at `create()` while the LOOKUP succeeds — which is why every read-only surface kept working and `/readyz` answered ok. Two silent riders go with it: the UNIQUE sat on `("teamId", fingerprint)`, so Prisma's writes to `team_id` were deduplicated by nothing, and the events foreign key sat on `"incidentId"`. Order inside the migration is load-bearing: backfill canonical-from-legacy where canonical IS NULL, PROVE no row still holds a legacy value its canonical column lacks (RAISEs if one does), rebind the unique/indexes/FK onto canonical columns, and only then drop the 27 legacy columns. Divergence between the two halves is REPORTED as a NOTICE and not refused — it is the normal state of a table written through one family while the other sat beside it (`occurrenceCount` keeps its insert default while `occurrence_count` increments), and the canonical column wins because every client shipped since `@map` reads and writes only it. Idempotent: on any database that never drifted — every clean boot and CI — it does nothing except assert the canonical unique exists |
...
4. apply `20271224000000_operational_incident_naming_convergence` — this one is
   not optional and not deferrable: until it runs, `recordIncident` fails
   P2011/23502 on every category, so the workspace records no operational
   conditions at all while every read surface looks healthy;
