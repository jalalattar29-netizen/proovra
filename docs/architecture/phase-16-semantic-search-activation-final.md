...
| `runExtractionInline` (the OCR / transcript writer) | `services/api/src/services/intelligence/extraction.service.ts` | **Corrected 2026-08-17, Phase 13 §4.** This row previously named `services/worker/src/media-intelligence.processor.ts` and claimed it "inherits the new behaviour automatically because it calls `indexEvidenceText`". It did not call it, and nothing else did either: `indexEvidenceText` had ZERO production callers, so `evidence_semantic_chunks` had no producer and every reader downstream of it — the hybrid ranker, the `mi-embed` worker, the backfill route, the UI status chip — read an index that was always empty. The live-indexing hook is now `runExtractionInline`, which fires it on the COMPLETED `EvidenceExtractedText` write alongside the keyword re-index and entity-extraction triggers, gated on `teamId`, on `SEMANTIC_SEARCH_ENABLED`, and (inside `indexEvidenceText`) on the workspace AI policy. |
