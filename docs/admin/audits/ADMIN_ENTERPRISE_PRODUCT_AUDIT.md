...
**Runtime disposition for every UI-reachable mutation action**

- unique mutation actions discovered: **undefined**
- of those, reachable from a UI control: **554**
- executed end to end in a browser or through the API with a durable re-read: **undefined**
- **still unexecuted: undefined**

*Why they are not done:* The harness is built, proven and resumable (PV_FROM/PV_TO), and it runs against a dedicated disposable fixture. What remains is execution time plus, for the BLOCKED set, a per-endpoint precondition the generic payload deriver cannot synthesise — a typed confirmation phrase, a second factor of a different kind, or a seeded row of a shape the fixture does not yet carry. Each is named in stage2.blockedReasons.

*What blocks them:* Nothing external. Database disposable, step-up satisfiable, no third-party sandbox needed.
...
- `UI-reachable mutations == passed + defective + refusal-only + blocked + not-yet-attempted` → **PASS**
- `every UI-reachable mutation has a final disposition` → **FAIL**
...
## Stage 2 — mutation runtime register (checkpoint)

| | |
| --- | ---: |
| UI-reachable mutation actions | 554 |
| Attempted with a valid session | **22** |
| Not yet attempted | **532** |
| BLOCKED_SUCCESS_BRANCH_UNREACHED | 12 |
| RUNTIME_PROVEN_REFUSAL_ONLY | 8 |
| RUNTIME_PROVEN_PASS | 2 |

**Method.** Not guessed. The harness sends an empty body, reads the field contract out of the endpoint's own 400 (three error shapes are in use and all three are parsed), fills each named field from the seeded fixture by field name and expected type, and resends. Path parameters are real rows of the right KIND, looked up from the table the segment names, so a 404 means the authority refused rather than that the id was invented.

**Step-up.** Satisfied for real where required: an SMS contact factor enrolled through the product's own route, the one-time code read back from the fixture's recording transport, and the verified challenge id presented in x-proovra-step-up-challenge-id.

**Pass criterion.** A 2xx is an execution. Every other outcome is recorded as the branch it is and is NOT counted as a pass.

### Why the sweep stopped at 22

The sweep is valid for the first 22 actions only. Action #21, POST /v1/admin/identity/emergency-revoke, returned 200 and revoked every session for the workspace INCLUDING the harness own. Every action after it answered 401, and re-authentication then failed because the harness had already enrolled an SMS contact factor on that same account in order to satisfy step-up, so sign-in became MFA-gated.

**Is this a product defect?** No. Emergency revoke did exactly what it is for. It is a harness sequencing defect, recorded here because 532 rows of 401 would otherwise read as findings.

**Fix for the next run**

- Run session-destroying actions LAST, or execute them with a throwaway persona that is not the harness actor.
- Enrol the step-up contact factor on a dedicated actor, or complete the MFA challenge at re-login (the pending token is not accepted as a Bearer on /v1/identity/mfa/challenge/verify; the correct credential path still needs tracing).
- Both are changes to scratchpad/stage2/mutation-harness.mjs only. No product change is implied.

### Pre-existing breakage on origin/main: `pnpm run build:web`

- **Error** apps/web/lib/evidence/generation-labels.ts:105 and :109 — no-fallthrough (2 errors), which fails `next build`.
- **Introduced by** ec92bc6a, the current origin/main HEAD. `git diff origin/main -- <that file>` is empty from this branch, so it is untouched by the safety fixes.
- **The code** A deliberate grouped fall-through: five case labels share one `return false`. The code is correct; the lint rule rejects the shape.
- **Consequence** The release web build is RED on origin/main today, independently of this work.
- **Not fixed here** Out of the two defects this stage was scoped to. Flagged for a decision rather than folded in silently.
- **Fixture build** The fixture launcher build path succeeds, so the Admin/Enterprise E2E fixture remains runnable.

### Stage 1 safety-fix verification

| Gate | Result |
| --- | --- |
| retentionReproduction | 11/11 green (services/api/test/retention-effective-policy-uuid-sentinel.integration.test.ts), red before the fix with the exact production error text. |
| fixtureIsolation | 16/16 green (apps/web/__tests__/local-fixture-env-isolation.test.mjs), 2 of them red before the fix. |
| apiSuite | 802 files / 24,782 tests, 0 failures. |
| webSuite | 3,057 tests, 0 failures. |
| integrationSubset | 3 files / 35 tests on a clean database, 0 failures. |
| typecheck | all workspaces clean. |
| buildApi | exit 0. |
| cleanDbBoot | migrate + db:preflight (0 fail / 1 warn / 4 pass) + API boot healthy on a fresh database. |
| architectureEngine | AuditEngineIntegrity = PASS; two consecutive generations byte-identical. |
| buildWeb | RED — but not from these changes. See stage2.preExistingMainBreakage. |
