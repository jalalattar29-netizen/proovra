...
| | PWA | Native |
|---|---|---|
| Entry | `apps/web/app/pricing/page.tsx` | `apps/mobile/app/(stack)/billing.tsx` |
| Files in recursive tree | 143 | 141 |
| Max import depth | 8 | 9 |
| **Unresolved imports** | **0** | **0** |
| JSX elements | 368 | 189 |
...
| Extra in native | 22 |
| Web elements carrying no literal label | 355 |
...
| | PWA | Native |
|---|---:|---:|
| Style properties resolved to literals | 401 | 239 |
...
| Classes with no matching CSS rule (UNRESOLVED) | 553 (250 distinct) | — |
| className built from a runtime expression (UNRESOLVED) | 33 | — |
| Competing rules for one class (cascade decides at runtime) | 10 | — |
| Native theme tokens not resolvable to a literal | — | 83 |

**PWA-only colours on this route (first 17):** `rgba(0, 0, 0, 0.7)` `#102126` `#1b3136` `rgba(158, 216, 207, 0.2)` `rgba(158, 216, 207, 0.1)` `rgba(0, 0, 0, 0.4)` `rgba(158, 216, 207, 0.15)` `#e2e8f0` `#cbd5e1` `#f0f4f8` `#64748b` `#0f1d36` `#d64545` `rgba(107, 91, 255, 0.28)` `#f1f4f9` `rgba(15, 23, 42, 0.035)` `rgba(255, 255, 255, 0.28)`

## L6 · Conditional states

| | PWA | Native |
|---|---:|---:|
| Conditional branches (ternary / && / .map) | 1138 | 1125 |
...
- Web classes with no CSS rule: **553**.
- Runtime-built `className`: **33**.
