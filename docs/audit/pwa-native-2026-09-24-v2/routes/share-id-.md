...
| | PWA | Native |
|---|---|---|
| Entry | `apps/web/app/share/[id]/page.tsx` | `apps/mobile/app/(stack)/evidence/[id].tsx` |
| Files in recursive tree | 139 | 152 |
| Max import depth | 8 | 6 |
| **Unresolved imports** | **0** | **0** |
| JSX elements | 252 | 383 |
...
| Extra in native | 106 |
| Web elements carrying no literal label | 239 |

### Adjudication of the unpaired web labels

| Verdict | Count |
|---|---:|
| `COPY_OR_COMPOSITION_DIFF` | 16 |
...
| | PWA | Native |
|---|---:|---:|
| Style properties resolved to literals | 257 | 258 |
...
| Classes with no matching CSS rule (UNRESOLVED) | 251 (147 distinct) | — |
...
| | PWA | Native |
|---|---:|---:|
| Conditional branches (ternary / && / .map) | 1067 | 1460 |
...
- `COPY_OR_COMPOSITION_DIFF`: **16** — needs a per-element read.
- Web classes with no CSS rule: **251**.
