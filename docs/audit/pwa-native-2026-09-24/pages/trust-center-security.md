...
**PWA entry:** `apps/web/app/(app)/trust-center/security/page.tsx`
...
|---:|---|---:|---|
| 0 | `apps/web/app/(app)/trust-center/security/page.tsx` | 2 | `(entry)` |
...
**Stock Tailwind utilities used (41).** `apps/web/tailwind.config.ts` declares `theme: { extend: {} }`, so these carry DEFAULT Tailwind values and are not connected to the PROOVRA token set:
...
| PWA elements identified | 182 |
| Native elements identified | 77 |
| Pairable PWA elements | 21 |
| Paired | 0 |
| Missing in Native | 7 |
| Extra in Native | 13 |
...
| PWA style items SOURCE-UNRESOLVED | 85 |
