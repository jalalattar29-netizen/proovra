...
export function safeBooleanLabel(value, trueLabel = "Yes", falseLabel = "No", unknownLabel = "N/A") {
    if (value === true)
        return trueLabel;
    if (value === false)
        return falseLabel;
...
export function redactIdentifier(value, visible = 6) {
    const t = safe(value, "");
    if (!t)
        return "Not included in external report";
    if (t.length <= visible * 2 + 3)
        return t;
    return `${t.slice(0, visible)}[redacted-middle]${t.slice(-visible)}`;
}
export function buildPublicEvidenceReference(evidenceId) {
    const t = safe(evidenceId, "");
    if (!t)
        return "Not recorded";
    return t;
}
export function buildPublicSigningKeyReference(keyId, version) {
    const id = safe(keyId, "");
    if (!id)
        return "Not recorded";
    const publicRef = id
...
export function formatBytesHuman(bytesStr) {
...
export function prettifySummaryText(input) {
...
export function normalizeTimestampFailureReason(reason) {
