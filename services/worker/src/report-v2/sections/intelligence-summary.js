...
export function renderIntelligenceSummarySection(section) {
    const hasAnything = (section.document?.totalRecords ?? 0) > 0 ||
    ...
    const intro = renderCallout({
        ...
        body: "PROOVRA's intelligence layer surfaces provider suggestions to " +
    ...
    `
        : "";
    const providerTable = section.providers.length > 0
        ? `
    ...
    const providerQualityTable = section.providerQuality && section.providerQuality.length > 0
        ? `
...
export async function buildTrustReferencesForReport(input) {
    ...
    try {
        const [trustRows, methodRows, aiRows, secRows, spRows, policyCount, activeGrants, crossOrgActive] = await Promise.all([
        ...
        return {
            trustCenter: trustRows,
            methodology: methodRows,
            aiDisclosure: aiRows,
            security: secRows,
            subprocessors: spRows,
            governance: {
                policyCount: policyCount,
                activeGrants: activeGrants,
                crossOrgActive: crossOrgActive,
            },
        };
    }
    catch {
...
function chipRow(rows) {
    if (rows.length === 0)
        return `<p class="muted small">—</p>`;
    return `<p class="muted small">${rows
        .map((r) => `<span class="redaction-chip">${escapeHtml(String(Object.values(r)[0]))}: ${r.count}</span>`)
        .join(" ")}</p>`;
}
