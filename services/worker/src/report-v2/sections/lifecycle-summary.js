...
export async function loadLifecycleSummary(input) {
    ...
    try {
        // Retention
        const [totalPolicies, activePolicies, overridePolicies, templateGroups, activePolicy] = await Promise.all([
        ...
        // Archive — per-tier
        const archiveTiers = ["HOT", "WARM", "COLD", "DEEP_ARCHIVE"];
        const countByTier = { HOT: 0, WARM: 0, COLD: 0, DEEP_ARCHIVE: 0 };
        const costEstimateUsdMicrosByTier = {
            HOT: 0,
            WARM: 0,
            COLD: 0,
            DEEP_ARCHIVE: 0,
        ...
        const transfer = recentTransfer
            ? {
                transferId: recentTransfer.id,
                state: recentTransfer.state,
                toOrganizationSlug: recentTransfer.toOrganizationSlug,
                evidenceCount: Array.isArray(recentTransfer.evidenceIds)
                    ? recentTransfer.evidenceIds.length
                    : 0,
                createdAt: recentTransfer.createdAt.toISOString(),
                completedAtUtc: recentTransfer.completedAtUtc?.toISOString() ??
                    null,
            }
            : null;
        ...
        const destruction = recentDestruction
            ? {
                requestId: recentDestruction.id,
                state: recentDestruction.state,
                evidenceCount: recentDestruction.certificate
                    ?.evidenceCount ??
                    (Array.isArray(recentDestruction.evidenceIds)
                        ? recentDestruction.evidenceIds.length
                        : 0),
                certifiedAtUtc: recentDestruction.certifiedAtUtc?.toISOString() ?? null,
                certificateHash: recentDestruction.certificate?.certificateHash ?? null,
            }
            : null;
        ...
        const violationRows = await prisma.intelligenceActivityEvent
            ...
            .catch(() => []);
        const violations = violationRows.map((r) => ({ code: r.eventCode, count: r._count._all }));
 ...
 */
export function renderLifecycleSummarySection(data) {
    if (!data)
        return "";
    const hasContent = data.retention.totalPolicies > 0 ||
    ...
    const intro = renderCallout({
        ...
        body: "PROOVRA records bounded lifecycle signals — retention policies, legal holds, archive " +
    ...
    <p class="muted small">${data.retention.totalPolicies} configured · ${data.retention.activePolicies} active · ${data.retention.overridePolicies} override${data.retention.activePolicy
    ...
    ${Object.keys(data.retention.templateBreakdown).length > 0
        ? `<p class="muted small">${Object.entries(data.retention.templateBreakdown)
            .map(([tmpl, cnt]) => `<span class="redaction-chip">${escapeHtml(tmpl)}: ${cnt}</span>`)
            .join(" ")}</p>`
        : ""}
  `;
    // Legal holds block
    ...
    ${Object.keys(data.legalHolds.byKind).length > 0
        ? `<p class="muted small">${Object.entries(data.legalHolds.byKind)
            .map(([kind, cnt]) => `<span class="redaction-chip">${escapeHtml(kind)}: ${cnt}</span>`)
            .join(" ")}</p>`
        : ""}
  `;
    // Archive tiers block
    ...
    <p class="muted small">${archiveTiers
        ...
        .map((t) => `<span class="redaction-chip">${t}: ${data.archive.countByTier[t]} evidence · $${((data.archive.costEstimateUsdMicrosByTier[t] ?? 0) / 1_000_000).toFixed(4)}</span>`)
        .join(" ")}</p>
  `;
    // Transfer block
  ...
  `
        : "";
    // Violations block
    const violationsBlock = data.totalViolations > 0
        ? `
  ...
  `
        : "";
    return renderPageSection("Evidence Lifecycle Summary", `
      ${intro}
      ${retentionBlock}
      ${holdBlock}
      ${archiveBlock}
