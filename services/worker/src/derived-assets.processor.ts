 ...
 * PHASE 12 — POINT 5.
 *
 * The payload used to carry `{ teamId, evidenceId, evidencePartId, assetKind }`
 * and this processor believed every field. `teamId` scoped the raw SQL that
 * reads the source bytes, so a tampered value read another workspace's evidence
 * part; `assetKind` selected which producer ran, so a tampered value ran an
 * ffmpeg pipeline against an image.
 *
 * All four now come from the `EvidencePartDerivedAsset` row the command names.
 * That row was committed by the authorized synchronous path, which had already
 * proved the part belongs to the workspace.
 */
export async function processDerivedAssetJob(
  job: Job<unknown>,
): Promise<{ ok: true; status: string }> {
  const requestId = randomUUID();
  const decoded = decodeCanonicalJob(JOB_NAMES.GENERATE_DERIVED_ASSET, job, {
    requestId,
  });

  const request = await prisma.evidencePartDerivedAsset.findUnique({
  ...
  if (!request) {
    // The row is not going to appear on a retry. Bounded no-op rather than a
    // burned attempt budget.
    logger.warn(
      { requestId, jobId: job.id },
      "derived_assets.request_not_found",
    );
  ...
  const evidencePartId = request.evidencePartId;
  const assetKind = request.assetKind as DerivedAssetKind;

  // A settled asset is not regenerated by a replayed job. Without this a
  // duplicate delivery re-fetches the source bytes, re-runs sharp or ffmpeg,
  // re-uploads the object and rewrites a row whose checksum is already
  // recorded against the evidence.
  //
  // PHASE 12 — POINT 5. This guard tested `status === "READY"`, and READY is a
  // value this row CANNOT HOLD: `evidence_part_derived_assets_status_bounded`
  // permits PENDING / PROCESSING / COMPLETED / FAILED / UNSUPPORTED, and every
  // write in this module goes through `recordDerivedAsset` with one of those.
  // So the guard never fired and every replay re-ran the pipeline in full —
  // duplicate storage cost and duplicate encode cost on completed work,
  // silently, because the no-op log line it would have emitted was
  // unreachable too.
  //
  // UNSUPPORTED is terminal for the same reason COMPLETED is: it records a
  // capability decision (no ffmpeg, reserved kind) that a retry of the same
  // job cannot change. Re-running it would re-take the same decision and
  // rewrite the same row.
  if (request.status === "COMPLETED" || request.status === "UNSUPPORTED") {
    logger.info(
      {
        requestId,
        jobId: job.id,
        derivedAssetId: request.id,
        status: request.status,
      },
      "derived_assets.replay_noop",
    );
