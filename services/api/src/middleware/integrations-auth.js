 ...
 */
import { integrationsFeatureDisabledReason, scopesGrantPermission, verifyApiKeyDetailed, } from "../services/integrations/api-keys.service.js";
import { safeRecordApiCredentialUsage, } from "../services/integrations/api-key-usage.service.js";
...
export async function requireApiKey(req, reply) {
    ...
    if (credential.ipAllowlist.length > 0) {
        ...
        if (!isIpAddressAllowed(clientIp, credential.ipAllowlist)) {
            ...
            void (async () => {
                try {
                    const { recordRiskSignal } = await import("../services/identity-security/risk.service.js");
    ...
    try {
        ...
        reply.header("x-ratelimit-reset", String(Math.floor(resetAtMs / 1000)));
    }
    catch {
 ...
 */
export function requireApiScope(req, reply, permission) {
 ...
 */
export async function runWithApiAudit(req, reply, action, handler) {
    ...
    try {
        ...
        if (cred) {
            ...
            safeRecordApiCredentialUsage({
                ...
                failureReason: code >= 400 ? classifyStatusFailure(code) : null,
                requestId: req.id ?? null,
            });
        }
        return result;
 ...
 */
export function serviceAccountActor(req) {
    const cred = req.apiCredential;
    if (!cred)
        return null;
    return {
