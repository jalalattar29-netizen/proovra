...
import { SEMRESATTRS_DEPLOYMENT_ENVIRONMENT, SEMRESATTRS_SERVICE_NAME, SEMRESATTRS_SERVICE_NAMESPACE, SEMRESATTRS_SERVICE_VERSION, } from "@opentelemetry/semantic-conventions";
let started = false;
...
let lastBootstrapAtUtc = null;
let lastBootstrapOutcome = null;
let lastBootstrapFailureCode = null;
let spansCreatedCount = 0;
let lastExportErrorCode = null;
...
let lastSpanName = null;
let lastSpanAtUtc = null;
function isEnabled() {
...
export function initOpenTelemetry(input) {
    input.log.info({
        serviceName: (process.env.OTEL_SERVICE_NAME ?? "").trim() || input.defaultServiceName,
        endpointConfigured: (process.env.OTEL_EXPORTER_OTLP_ENDPOINT ?? "").trim().length > 0,
        protocol: (process.env.OTEL_EXPORTER_OTLP_PROTOCOL ?? "").trim() || "http/protobuf",
    ...
    if (!isEnabled()) {
        ...
        input.log.info({ reason: "OTEL_ENABLED not true", defaultServiceName: input.defaultServiceName }, "otel.bootstrap_disabled");
        return false;
    }
    const serviceName = (process.env.OTEL_SERVICE_NAME ?? "").trim() || input.defaultServiceName;
    const serviceNamespace = parseResourceAttribute("service.namespace") ?? "proovra";
    const environment = parseResourceAttribute("deployment.environment") ??
    ...
    try {
        ...
        input.log.info({
            ...
            serviceVersion,
            endpointConfigured: (process.env.OTEL_EXPORTER_OTLP_ENDPOINT ?? "").trim().length > 0,
            protocol: (process.env.OTEL_EXPORTER_OTLP_PROTOCOL ?? "").trim() ||
                "http/protobuf",
 ...
 */
export function __resetOtelStateForTests() {
    started = false;
...
export const PROOVRA_SPAN_NAMES = {
    // Phase O1.1 / O1.2 / O1.3 / O1.4 — bounded runtime emission. EVERY
    // entry below is wired by a real `withProovraSpan(PROOVRA_SPAN_NAMES.X, …)`
    // call OR by a `wrapJobHandlerWithOtelContext(PROOVRA_SPAN_NAMES.X, …)`
    // registration in `services/worker/src/index.ts`.
    //
    // The contract test `services/worker/test/phase-o1-4-span-emission.test.ts`
    // asserts every entry has a real call site. Per the O1.4 hard rule,
    // no enum-only entries are permitted — adding a name without
    // wiring the emission FAILS the contract test.
    //
    ...
    // Phase O1.5A — Capture + Evidence + Upload + Finalize. Every
    // entry below has a runtime emission asserted by the contract
    // test `phase-o1-4-span-emission.test.ts`.
    ...
    // Phase O1.5B — Integrity + Custody chain + TSA + OTS.
    ...
    // Phase O1.5C — Report pipeline (5 spans).
    ...
    // Phase O1.5C — Verification Package pipeline (6 spans).
    ...
    // Phase O1.5D — Reviewer Ops (5 spans).
    ...
    // Phase O1.5D — C2PA validate + SIU followup/timeline (3 spans).
    ...
    // Phase O1.5E — SMTP + external review (2 spans).
 ...
 */
export async function withProovraSpan(name, attributes, fn) {
...
/**
 * Phase O1.5B — sync-safe bounded span helper.
 *
 * Wraps a synchronous function in a PROOVRA span without changing
 * its return type to a Promise. Use for critical sync code paths
 * like `evaluateCustodyChain` that the spec forbids converting to
 * async. Returns whatever the wrapped function returns; on error
 * the span is marked failed and the error is re-thrown UNCHANGED so
 * the caller's control flow + error class is preserved.
 *
 * Safe when OTEL is disabled — no-op tracer is a pass-through.
 */
export function withProovraSpanSync(name, attributes, fn) {
    const tracer = trace.getTracer("proovra");
...
function boundedAttributes(raw) {
    const out = {};
...
function parseResourceAttribute(key) {
