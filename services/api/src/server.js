...
import { adminIdentityRoutes, adminIdentityRuntimeRoutes, } from "./routes/admin-identity.routes.js";
...
import { AppError, ErrorCode, createErrorResponse, isAppError, } from "./errors.js";
...
function isProovraOrigin(origin) {
    const value = normalizeOrigin(origin);
    return (value === "https://proovra.com" ||
...
function buildRequestContext(req) {
    ...
    if (req.evidenceId)
        context.evidenceId = req.evidenceId;
    if (req.geo && Object.keys(req.geo).length > 0)
        context.geo = req.geo;
    return context;
}
function emitOperationalAlert(logger, params) {
    logger.error({
        alert: true,
        severity: "critical",
...
function normalizeUnknownError(err) {
    ...
    if (err instanceof ZodError) {
        ...
        return new AppError(ErrorCode.VALIDATION_ERROR, "Request validation failed", firstIssue
            ? {
...
function buildZodWirePayload(err, requestId) {
    const issues = Array.isArray(err.issues) ? err.issues : [];
    const fields = issues.slice(0, ZOD_FIELD_LIMIT).map((issue) => {
    ...
    const baseSummary = firstField
        ...
        : "Invalid input.";
    const summary = (overflow ? `${baseSummary} (+${issues.length - ZOD_FIELD_LIMIT} more)` : baseSummary).slice(0, ZOD_MESSAGE_LIMIT);
    return {
        status: 400,
        body: {
            error: {
...
function readPrismaDiagnostic(err) {
    const meta = (err.meta ?? {});
    const readMetaString = (key) => {
...
export async function buildServer() {
    ...
    app.setErrorHandler((err, req, reply) => {
        ...
        if (appError) {
            ...
            const errorResponse = createErrorResponse(appError.code, req.id, appError.details, appError.message);
        ...
        const errorResponse = createErrorResponse(ErrorCode.INTERNAL_SERVER_ERROR, req.id);
    ...
    const schemaValidationFailFast = (process.env.SCHEMA_VALIDATION_FAIL_FAST ?? "true").toLowerCase() !== "false";
    try {
