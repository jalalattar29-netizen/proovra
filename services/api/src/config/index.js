 ...
 */
export function resolveSecret(envName) {
    const raw = (process.env[envName] ?? "").trim();
    if (raw.length > 0)
        return raw;
    if (PROD()) {
        throw new ProductionConfigError(`Required secret "${envName}" is not set. Refusing to use a fallback in production.`);
    }
...
function flagOk(envName) {
    const v = (process.env[envName] ?? "").trim();
    return v.length > 0
        ? { configured: true, reason: null }
        : { configured: false, reason: `${envName}_missing` };
}
function allOk(...envNames) {
    const missing = envNames.filter((n) => (process.env[n] ?? "").trim().length === 0);
    if (missing.length === 0)
 ...
 */
export function collectStartupViolations() {
    const out = [];
    ...
    if (envBool("COMMUNICATIONS_ENABLED")) {
        if ((process.env.COMMUNICATIONS_RECIPIENT_HASH_SECRET ?? "").trim().length ===
            0) {
            out.push({
    ...
    if (PROD() && envBool("SAML_ENABLED")) {
        const acsUrl = (process.env.SAML_SP_ACS_URL ??
            ...
            "").trim();
        if (acsUrl.startsWith("http://localhost") ||
    ...
    // report job.
    if (PROD()) {
        const pdfSigningOn = (process.env.PDF_SIGNING_ENABLED ?? "").trim().toLowerCase() === "true";
        const pdfOptOutAck = (process.env.PDF_ARTIFACT_SIGNATURE_OPT_OUT_ACK ?? "")
            .trim()
            .toLowerCase() === "true";
    ...
    // never intended.
    {
        ...
        if (stripeKey.length > 0) {
            const isSecret = stripeKey.startsWith("sk_live_") || stripeKey.startsWith("sk_test_");
            if (!isSecret) {
                out.push({
...
export function runStartupConfigValidation(log) {
    ...
    if (PROD()) {
        ...
        if (!pdfSigningOn && pdfOptOutAck) {
            ...
            log.info({ pdfSigningEnabled: true }, "phase_a2.startup.pdf_signing_enabled");
        }
    }
    ...
    if (violations.length === 0)
        return;
    if (PROD()) {
        log.error({ violations }, "phase20.startup.config_violations_in_production");
        throw new ProductionConfigError(`Missing required production config: ${violations.map((v) => v.envName).join(", ")}`);
    }
    log.warn({ violations }, "phase20.startup.config_violations_non_production");
}
function scrubSnapshotForLog(snapshot) {
