...
function refreshTtlMs() {
    ...
    // set it.
    const raw = Number.parseInt((process.env.AWS_SECRETS_REFRESH_TTL_MS ??
...
async function hydrateOnce(log) {
    if (!state.awsEnabled)
        return;
    log.info({ secretName: configuredSecretName(), region: configuredRegion() }, "aws_secrets.hydration_started");
    try {
        const res = await getClient().send(new GetSecretValueCommand({ SecretId: configuredSecretName() }));
...
async function refreshLoop(log) {
    if (!state.awsEnabled)
        return;
 ...
 */
export async function initSecretsManager(log) {
    state.awsEnabled = isEnabled();
    if (!state.awsEnabled) {
        state.fallbackMode = "env_only";
        log.info({ reason: "AWS_SECRETS_ENABLED not true" }, "aws_secrets.disabled");
        return;
    }
    await hydrateOnce(log);
 ...
 * test files.
 */
export function __resetSecretsManagerForTests() {
    if (state.refreshTimer)
        clearInterval(state.refreshTimer);
