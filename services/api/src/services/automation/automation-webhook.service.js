...
// ---------------------------------------------------------------------------
export const WEBHOOK_RETRY_BACKOFF_SECONDS = [5, 30, 300];
 ...
 */
export function computeNextAttemptAt(attemptThatJustFailed, nowMs = Date.now()) {
...
export const WEBHOOK_SIGNATURE_ALGO = "sha256";
 ...
 * afterwards.
 */
export function createDestinationSecret() {
 ...
 */
function encryptSecretForStorage(plaintext) {
 ...
 */
export function decryptStoredSecret(envelope) {
    if (!envelope.startsWith("e3.2:"))
        return null;
    try {
...
function resolveStorageKeyMaterial() {
    ...
    const raw = getSecret("AUTH_JWT_SECRET") ?? "dev-jwt-secret-not-for-production";
    return createHash("sha256").update(`e3.2-webhook-secret-wrap|${raw}`).digest();
}
function expandKeystream(key, length) {
 ...
 * a fully-checked URL.
 */
export function validateDestinationUrlStatic(input) {
    let parsed;
    try {
    ...
    if (isIP(host) > 0) {
        ...
        if (ipReason)
            return { ok: false, reason: ipReason };
    }
    return {
        ok: true,
 ...
 */
export async function validateDestinationUrlWithDns(input) {
    ...
    if (isIP(staticResult.hostname) > 0)
        return staticResult;
    let resolved;
    try {
...
export async function deliverWebhookOnce(input) {
    ...
    const timeout = setTimeout(() => controller.abort(), input.timeoutMs ?? WEBHOOK_TIMEOUT_MS);
    try {
