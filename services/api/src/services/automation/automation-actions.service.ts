...
export type ActionExecutionResult = {
  ...
  skipped?: boolean;
  reason?: string;
  /** Operator-safe summary fields. Never include secrets / raw evidence. */
...
async function actionNotifyUser(
  ...
  const cfg = input.actionConfig as { userId?: string; template?: string };
 ...
 */
async function actionNotifyRole(
  input: ActionExecutionInput,
  prisma: PrismaClient,
): Promise<ActionExecutionResult> {
  const cfg = input.actionConfig as { role?: string; template?: string };
 ...
 */
async function actionAssignReviewer(
  input: ActionExecutionInput,
  prisma: PrismaClient,
): Promise<ActionExecutionResult> {
  const cfg = input.actionConfig as {
 ...
 */
async function actionAddOperationalComment(
  input: ActionExecutionInput,
  prisma: PrismaClient,
): Promise<ActionExecutionResult> {
  const cfg = input.actionConfig as { body?: string; visibility?: string };
 ...
 */
async function actionWebhookDelivery(
  input: ActionExecutionInput,
  prisma: PrismaClient,
): Promise<ActionExecutionResult> {
  const cfg = input.actionConfig as {
    ...
    eventType?: string;
    metadataPassThrough?: string[];
  };
  const destinationId =
    typeof cfg.destinationId === "string" ? cfg.destinationId : null;
 ...
 * the run row).
 */
function hashShort(id: string): string {
  return id.slice(0, 8);
}
