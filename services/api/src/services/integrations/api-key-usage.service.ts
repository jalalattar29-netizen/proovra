...
import type {
  ApiCredentialUsageLog as DbUsageLog,
  PrismaClient,
} from "@prisma/client";
...
export type ServiceAccountFailureReason =
  | "feature_disabled"
  | "missing_bearer"
  | "invalid_key"
  | "scope_denied"
...
export type RecordApiCredentialUsageInput = {
  ...
  statusCode: number;
  success: boolean;
  failureReason?: ServiceAccountFailureReason | string | null;
  requestId?: string | null;
};

export async function recordApiCredentialUsage(
  input: RecordApiCredentialUsageInput,
  client: PrismaClient = defaultPrisma,
): Promise<DbUsageLog | null> {
 ...
 */
export function safeRecordApiCredentialUsage(
  input: RecordApiCredentialUsageInput,
  client: PrismaClient = defaultPrisma,
): void {
...
export type ListApiCredentialUsageInput = {
  apiCredentialId: string;
  teamId: string;
  limit?: number;
};

export async function listApiCredentialUsage(
  input: ListApiCredentialUsageInput,
  client: PrismaClient = defaultPrisma,
): Promise<DbUsageLog[]> {
...
export function projectApiCredentialUsage(log: DbUsageLog): {
  id: string;
  routePath: string;
  method: string;
  action: string;
