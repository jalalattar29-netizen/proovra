...
export async function getWebhookDelivery(input, client = defaultPrisma) {
 ...
 */
export async function manuallyRetryWebhookDelivery(input, client = defaultPrisma) {
    ...
    if (!endpoint)
        throw new WebhookDeliveryOpError("endpoint_not_found");
...
export async function cancelWebhookDelivery(input, client = defaultPrisma) {
    const delivery = await getWebhookDelivery(input, client);
    if (!delivery)
        throw new WebhookDeliveryOpError("delivery_not_found");
...
export function readWebhookDeliveryRetentionDays() {
    ...
    if (!Number.isFinite(n))
        return RETENTION_DEFAULT_DAYS;
    if (n < RETENTION_MIN_DAYS)
        return RETENTION_MIN_DAYS;
    if (n > RETENTION_MAX_DAYS)
 ...
 */
export async function cleanupOldWebhookDeliveries(input = {}, client = defaultPrisma) {
    const retentionDays = (() => {
        if (typeof input.retentionDays === "number") {
            const r = Math.floor(input.retentionDays);
            if (r < RETENTION_MIN_DAYS)
                return RETENTION_MIN_DAYS;
            if (r > RETENTION_MAX_DAYS)
                return RETENTION_MAX_DAYS;
            return r;
        }
...
// -----------------------------------------------------------------------------
export function projectWebhookDeliveryDetail(d) {
    return {
        id: d.id,
