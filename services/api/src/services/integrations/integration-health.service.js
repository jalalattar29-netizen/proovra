 ...
 */
export async function computeIntegrationsHealthSnapshot(teamId, client = defaultPrisma) {
    const now = new Date();
    const windowStart = new Date(now.getTime() - HEALTH_RECENT_WINDOW_HOURS * 60 * 60 * 1000);
    // -------------------------------------------------------------------
    ...
    for (const g of deliveryStatusGroups) {
        ...
        if (g.status === "SENT")
            ...
            failedLast24h = count;
        else if (g.status === "RETRY_SCHEDULED")
            retryScheduledLast24h = count;
        else if (g.status === "PENDING")
            pendingLast24h = count;
    }
    const deliveriesLast24h = sentLast24h + failedLast24h + retryScheduledLast24h + pendingLast24h;
    const successRateLast24h = deliveriesLast24h === 0 ? null : sentLast24h / deliveriesLast24h;
    // -------------------------------------------------------------------
    // Pending or retry-scheduled count — full window, not 24h, since
    // an old stuck row matters even if it's past the 24h cutoff.
    // -------------------------------------------------------------------
    const pendingOrRetryScheduledCount = await client.integrationWebhookDelivery.count({
    ...
    let averageLatencyMsLast24h = null;
    ...
    return {
        ...
        oldestPendingDeliveryAt: oldestPending?.createdAt?.toISOString() ?? null,
