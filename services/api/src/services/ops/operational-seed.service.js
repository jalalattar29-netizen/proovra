...
import { acknowledgeEscalation, resolveEscalation, } from "../reviewer-ops/escalation-engine.service.js";
import { runReconcile, } from "../reviewer-ops/reviewer-operations-engine.service.js";
...
// Env gates
// -----------------------------------------------------------------------------
export function assertSeedingEnabled() {
    const enabled = (process.env.OPERATIONAL_SEEDING_ENABLED ?? "false").toLowerCase() ===
        "true";
    if (!enabled) {
        throw new OperationalSeedError("SEEDING_DISABLED");
    }
    if (process.env.NODE_ENV === "production") {
        const allow = (process.env.OPERATIONAL_SEEDING_ALLOW_PRODUCTION ?? "false").toLowerCase() ===
            "true";
        if (!allow) {
...
export function assertSeedingSecret(presented) {
...
async function auditSeedRun(result, actorUserId, requestId) {
...
async function auditSeedCleanup(result, actorUserId, requestId) {
    await appendPlatformAuditLog({
        userId: actorUserId,
...
// -----------------------------------------------------------------------------
async function loadSeedRunAuditRow(seedRunId, client) {
    ...
    if (!meta || typeof meta !== "object")
        return null;
    const createdResourceIds = meta.createdResourceIds;
    ...
    return {
        createdResourceIds: {
            ...
            incidentIds: Array.isArray(createdResourceIds.incidentIds)
                ...
                : [],
            workloadSnapshotsCreated: typeof createdResourceIds.workloadSnapshotsCreated === "number"
...
async function findSeedableEvidence(teamId, needed, client) {
...
async function findSeedReviewer(teamId, client) {
...
export async function runOperationalSeed(input, client = defaultPrisma) {
    ...
    if (seedable.length < count) {
        warnings.push(`Only ${seedable.length} of ${count} evidence rows available; seeding the smaller set.`);
    }
    if (input.dryRun) {
        const dryResult = {
            seedRunId,
    ...
    for (const ev of seedable) {
        const wf = await ensureReviewWorkflow({ evidenceId: ev.id, teamId: input.teamId, workspaceType: "TEAM" }, client);
    ...
    if (defaults.expectEscalations) {
        bump("operational_seed_created_escalations_total", reconcileResult.escalationsCreated);
    }
    ...
    if (input.scenario === "full_lifecycle" && created.escalationIds[0]) {
        ...
        try {
            ...
            warnings.push(`full_lifecycle transition failed for escalation ${escalationId}: ` +
    ...
    if (input.scenario === "escalation_storm" || input.scenario === "full_lifecycle") {
        try {
            const incident = await recordIncident({
                ...
                safeSummary: "Demo / staging seed scenario — generated by the operational seeding flow. " +
            ...
            warnings.push("Seed incident creation failed: " +
                (err instanceof Error ? err.message : "unknown_error"));
        }
    }
    ...
    if (input.scenario === "baseline" && reconcileResult.workloadReviewersComputed === 0) {
        try {
            const extra = await snapshotWorkspaceWorkload({ teamId: input.teamId }, client);
            created.workloadSnapshotsCreated += extra.reviewersComputed;
        }
        catch (err) {
            warnings.push("Extra workload snapshot failed: " +
                (err instanceof Error ? err.message : "unknown_error"));
        }
    }
    const result = {
...
export async function listSeedRuns(teamId, client = defaultPrisma) {
    ...
    return rows
        ...
        const seedScenario = meta && typeof meta.seedScenario === "string"
            ? meta.seedScenario
            : "unknown";
        const teamIdRaw = meta && typeof meta.teamId === "string" ? meta.teamId : null;
        const createdResourceIds = (meta?.createdResourceIds ?? null);
        return {
