...
import { getSecret, MIGRATED_SECRETS, } from "../config/runtime-secrets.js";
function env(name) {
...
export { brandName as getEmailBrandName, emailShell as renderEmailShell, fromHeader as getEmailFromHeader, safeHtml as escapeEmailHtml, supportEmail as getEmailSupportAddress, webBaseUrl as getEmailWebBaseUrl, };
function fromHeader() {
...
function inviteAcceptUrl(invitationToken) {
...
function buildFollowUpContent(step) {
    if (step === 1) {
        return {
            ...
            intro: "Just following up in case your demo request is still active. The links below are usually the fastest way to evaluate the workflow before a live walkthrough.",
    ...
    if (step === 2) {
        return {
            ...
            intro: "If you are still evaluating fit, these materials usually answer the practical questions faster than another confirmation email.",
    ...
    return {
        ...
        intro: "This is a final follow-up in case the request is still relevant. If the workflow is still active on your side, you can use the links below to continue or book a walkthrough.",
...
export async function sendCustomEmailViaResend(input) {
    ...
    return withProovraSpan(PROOVRA_SPAN_NAMES.SMTP_EMAIL_SEND, {
...
export function getEmailService() {
    ...
    singleton = {
        ...
        `.trim(),
            ...
            const text = `Reset your ${app} password\n\n` +
        ...
        async sendMfaRecoveryAdminDigestEmail(adminEmail, teamDisplayName, pendingCount, adminSpaUrl, snoozeUrl) {
            const app = brandName();
        ...
        `.trim()
            ...
            const html = emailShell({
                ...
                preheader: `You have ${safeCount} pending recovery request${safeCount === 1 ? "" : "s"} awaiting your review.`,
                bodyHtml: `
          <div style="margin:0 0 10px 0;">
          ...
          <div style="margin:0 0 10px 0;">
            <strong>${safeCount}</strong> pending request${safeCount === 1 ? "" : "s"}
          </div>
        ...
        `.trim(),
                ...
                ctaUrl: adminSpaUrl,
                secondaryText: "If you are no longer an administrator of this workspace, you can ignore this email.",
            });
            const text = `Pending MFA recovery requests for ${teamDisplayName} on ${app}\n\n` +
            ...
            return resend.emails.send({
                from,
                to: adminEmail,
                subject: `${safeCount} pending MFA recovery request${safeCount === 1 ? "" : "s"} — ${teamDisplayName}`,
                html,
                text,
            });
        },
        async sendMfaRecoveryVerificationEmail(email, verificationUrl) {
            const app = brandName();
            const html = emailShell({
        ...
        `.trim(),
                ...
                secondaryText: "If you did not initiate this recovery request, ignore this email and notify your administrator.",
            });
            const text = `Verify your ${app} MFA recovery request\n\n` +
        ...
        async sendTeamInvitation(email, orgName, invitationToken) {
        ...
        `.trim(),
            ...
            const text = `Batch complete: ${batchName}\n` +
                `Org: ${orgName}\n` +
                `Total: ${total}\nSucceeded: ${succeeded}\nFailed: ${failed}\n\n` +
        ...
        async sendDemoRequestNotification(params) {
            const qualificationReasons = Array.isArray(params.qualificationReasons)
                ...
                : [];
            const reasonsHtml = qualificationReasons.length > 0
                ? qualificationReasons
                    .map((reason) => `<li style="margin:0 0 6px 0;">${safeHtml(prettyReason(reason))}</li>`)
                    .join("")
          ...
          <ul style="margin:0; padding-left:18px; color:#334155;">
            <li style="margin:0 0 6px 0;"><a href="${safeHtml(quickLinks.replyToLeadMailto)}" style="color:#2563eb; text-decoration:none;">Reply to lead</a></li>
            <li style="margin:0 0 6px 0;"><a href="${safeHtml(quickLinks.sampleReportUrl)}" style="color:#2563eb; text-decoration:none;">Open sample report</a></li>
            <li style="margin:0 0 6px 0;"><a href="${safeHtml(quickLinks.verificationDemoUrl)}" style="color:#2563eb; text-decoration:none;">Open verification demo</a></li>
            <li style="margin:0 0 6px 0;"><a href="${safeHtml(quickLinks.methodologyUrl)}" style="color:#2563eb; text-decoration:none;">Open methodology</a></li>
            <li style="margin:0 0 6px 0;"><a href="${safeHtml(quickLinks.pricingUrl)}" style="color:#2563eb; text-decoration:none;">Open pricing</a></li>
            <li style="margin:0 0 6px 0;"><a href="${safeHtml(quickLinks.requestDemoUrl)}" style="color:#2563eb; text-decoration:none;">Open request demo page</a></li>
            <li style="margin:0 0 6px 0;"><a href="${safeHtml(quickLinks.contactSalesUrl)}" style="color:#2563eb; text-decoration:none;">Open contact sales page</a></li>
            ${quickLinks.bookingUrl
                    ? `<li style="margin:0 0 6px 0;"><a href="${safeHtml(quickLinks.bookingUrl)}" style="color:#2563eb; text-decoration:none;">Open booking</a></li>`
                    : ""}
          </ul>
        `
                : "";
          ...
          <div style="margin:0 0 10px 0;"><strong>Priority:</strong> ${safeHtml(params.priority ?? "-")}</div>
          <div style="margin:0 0 10px 0;"><strong>Lead quality:</strong> ${safeHtml(params.leadQuality ?? "-")}</div>
          <div style="margin:0 0 10px 0;"><strong>Lead track:</strong> ${safeHtml(params.leadTrack ?? "-")}</div>
          <div style="margin:0 0 10px 0;"><strong>Recommended action:</strong> ${safeHtml(params.recommendedAction ?? "-")}</div>
          <div style="margin:0 0 10px 0;"><strong>Response SLA:</strong> ${safeHtml(params.responseSlaHours != null
                    ? `${params.responseSlaHours}h`
                    : "-")}</div>
          <div style="margin:0 0 10px 0;"><strong>Qualification score:</strong> ${safeHtml(params.qualificationScore != null
        ...
        `.trim(),
            ...
            const text = [
                ...
                `Response SLA: ${params.responseSlaHours != null ? `${params.responseSlaHours}h` : "-"}`,
                `Qualification score: ${params.qualificationScore != null
                    ? String(params.qualificationScore)
                    : "-"}`,
        ...
        <ul style="margin:0; padding-left:18px; color:#334155;">
          <li style="margin:0 0 6px 0;"><a href="${safeHtml(params.sampleReportUrl)}" style="color:#2563eb; text-decoration:none;">View a sample report</a></li>
          <li style="margin:0 0 6px 0;"><a href="${safeHtml(params.verificationDemoUrl)}" style="color:#2563eb; text-decoration:none;">Open the verification demo</a></li>
          <li style="margin:0 0 6px 0;"><a href="${safeHtml(params.methodologyUrl)}" style="color:#2563eb; text-decoration:none;">Review the verification methodology</a></li>
          <li style="margin:0 0 6px 0;"><a href="${safeHtml(params.pricingUrl)}" style="color:#2563eb; text-decoration:none;">Review pricing</a></li>
          ${params.bookingUrl
                ? `<li style="margin:0 0 6px 0;"><a href="${safeHtml(params.bookingUrl)}" style="color:#2563eb; text-decoration:none;">Book a walkthrough</a></li>`
                : ""}
        </ul>
      `;
            const html = emailShell({
          ...
          <div style="margin:0 0 10px 0;">
            Our team will review your request and reply ${safeHtml(params.responseWindowText)} if the workflow looks relevant for a live walkthrough.
          </div>
          <div style="margin:0 0 10px 0;">
        ...
        <ul style="margin:0; padding-left:18px; color:#334155;">
          <li style="margin:0 0 6px 0;"><a href="${safeHtml(params.sampleReportUrl)}" style="color:#2563eb; text-decoration:none;">Sample report</a></li>
          <li style="margin:0 0 6px 0;"><a href="${safeHtml(params.verificationDemoUrl)}" style="color:#2563eb; text-decoration:none;">Verification demo</a></li>
          <li style="margin:0 0 6px 0;"><a href="${safeHtml(params.methodologyUrl)}" style="color:#2563eb; text-decoration:none;">Verification methodology</a></li>
          <li style="margin:0 0 6px 0;"><a href="${safeHtml(params.pricingUrl)}" style="color:#2563eb; text-decoration:none;">Pricing</a></li>
          <li style="margin:0 0 6px 0;"><a href="${safeHtml(params.requestDemoUrl)}" style="color:#2563eb; text-decoration:none;">Request demo</a></li>
          <li style="margin:0 0 6px 0;"><a href="${safeHtml(params.contactSalesUrl)}" style="color:#2563eb; text-decoration:none;">Contact sales</a></li>
          ${params.bookingUrl
                ? `<li style="margin:0 0 6px 0;"><a href="${safeHtml(params.bookingUrl)}" style="color:#2563eb; text-decoration:none;">Book a walkthrough</a></li>`
                : ""}
        </ul>
