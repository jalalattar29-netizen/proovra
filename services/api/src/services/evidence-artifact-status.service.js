...
import { PDF_SIGNING_UNAVAILABLE_COPY, PDF_UNSIGNED_OPT_OUT_WARNING_COPY, } from "@proovra/shared";
 ...
 */
const PDF_STATUSES_RUNTIME = new Set([
    "SIGNED",
    "UNSIGNED_OPT_OUT",
    "SIGNING_UNAVAILABLE",
    "SIGNING_FAILED",
...
function normalizePdfSignatureStatus(raw) {
    if (!raw)
        return null;
    return PDF_STATUSES_RUNTIME.has(raw)
        ? raw
        : null;
}
/**
 ...
 * deterministic.
 */
function resolveWarningCopy(status, stored) {
    if (status === "SIGNED" || status === "NOT_APPLICABLE")
        return null;
    if (stored && stored.trim().length > 0)
        return stored;
    if (status === "UNSIGNED_OPT_OUT")
        return PDF_UNSIGNED_OPT_OUT_WARNING_COPY;
    if (status === "SIGNING_UNAVAILABLE")
        return PDF_SIGNING_UNAVAILABLE_COPY;
    return null;
}
export async function buildEvidenceArtifactStatus(params) {
    ...
    const finalized = params.evidenceStatus === prismaPkg.EvidenceStatus.SIGNED ||
    ...
    const blockedMeta = readBlockedMetadata(params.evidenceVerificationPackageMetadata ?? null);
    const packageBlocked = finalized && !latestPackage && blockedMeta !== null;
    const packagePending = finalized &&
        !latestPackage &&
    ...
    // legacy row.
    const pdfSignature = latestReport
        ? (() => {
            const status = normalizePdfSignatureStatus(latestReport.pdfSignatureStatus) ??
                "SIGNING_UNAVAILABLE";
            return {
                status,
                signedAtUtc: status === "SIGNED" && latestReport.pdfSignedAtUtc
                    ? latestReport.pdfSignedAtUtc.toISOString()
                    : null,
                signerKeyId: status === "SIGNED" ? (latestReport.pdfSignerKeyId ?? null) : null,
                warning: resolveWarningCopy(status, latestReport.pdfSigningWarning ?? null),
            };
        })()
        : null;
    ...
    const manifestSignature = latestPackage
        ? {
            status: "SIGNED",
            signerKeyId: (process.env.PACKAGE_SIGNING_KEY_ID ?? "").trim() || null,
        }
        : null;
    return {
        evidenceId,
 ...
 * as not-blocked.
 */
function readBlockedMetadata(raw) {
    ...
    return {
        ...
        blockedAtUtc: typeof obj.blockedAtUtc === "string" ? obj.blockedAtUtc : null,
    };
}
