...
import {
  ...
  type TrustLifecycleCategory,
  type TrustLifecycleCode,
} from "@proovra/shared";
...
export type EmitTrustEventInput = {
  prisma?: PrismaClient;
  teamId: string;
  code: TrustLifecycleCode;
  actorUserId?: string | null;
  targetType?: TrustAuditTargetType | null;
...
export async function emitTrustEvent(
  input: EmitTrustEventInput,
): Promise<{ ok: boolean; id?: string }> {
  if (!(TRUST_LIFECYCLE_CODES as ReadonlyArray<string>).includes(input.code)) {
    return { ok: false };
  }
  const prisma = input.prisma ?? defaultPrisma;
  const category: TrustLifecycleCategory = classifyTrustLifecycleCategory(
    input.code,
  );
  try {
...
export async function emitTrustArticleEvent(input: {
  ...
  articleId: string;
  /**
   * Any TRUST_ARTICLE_* lifecycle code:
   * CREATED / UPDATED / PUBLISHED / SUPERSEDED / REVIEWED / MARKED_STALE.
   */
  code: TrustLifecycleCode;
  actorUserId?: string | null;
  reason?: string | null;
  /** Optional bounded payload — e.g. { fromVersion, toVersion } for SUPERSEDED. */
  payload?: Record<string, unknown> | null;
}): Promise<{ ok: boolean; id?: string }> {
...
export async function emitAccessReviewDecisionEvent(input: {
  ...
  if (
    !(ACCESS_REVIEW_ITEM_DECISIONS as ReadonlyArray<string>).includes(
      input.decision,
    )
  ) {
    return { ok: false };
