...
import { TRUST_ARTICLE_DRIFT_STATES, } from "@proovra/shared";
 ...
 */
export async function runTrustArticleDriftScan(input) {
    ...
    for (const row of rows) {
        ...
        const nextState = missing.length === 0 ? "CURRENT" : "STALE";
        await prisma.trustCenterArticle.update({
            where: { id: row.id },
            data: {
                driftState: nextState,
                lastReferenceCheckAt: new Date(),
                missingReferences: missing.length === 0 ? Prisma.JsonNull : missing,
            },
        });
 ...
 */
export async function listStaleTrustArticles(input) {
    const prisma = input.prisma ?? defaultPrisma;
    const rows = await prisma.trustCenterArticle.findMany({
 ...
 */
export async function markArticleNeedsReview(input) {
    const prisma = input.prisma ?? defaultPrisma;
 ...
 */
function parseImplementationReferences(value) {
