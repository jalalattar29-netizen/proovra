 ...
 */
import { AI_DISCLOSURE_SECTIONS, METHODOLOGY_SECTIONS, SECURITY_SECTIONS, TRUST_ARTICLE_DRIFT_STATES, TRUST_ARTICLE_KINDS, TRUST_ARTICLE_STATES, TRUST_CENTER_SECTIONS, } from "@proovra/shared";
...
export async function upsertTrustArticle(input) {
    if (!TRUST_ARTICLE_KINDS.includes(input.kind)) {
...
// Helpers
// ---------------------------------------------------------------------------
function validateSection(kind, section) {
    switch (kind) {
        case "TRUST_CENTER":
            return TRUST_CENTER_SECTIONS.includes(section);
        case "METHODOLOGY":
            return METHODOLOGY_SECTIONS.includes(section);
        case "AI_DISCLOSURE":
            return AI_DISCLOSURE_SECTIONS.includes(section);
        case "SECURITY":
            return SECURITY_SECTIONS.includes(section);
    }
}
...
function normalizeDriftStateForProjection(value) {
    if (!value)
        return undefined;
    return TRUST_ARTICLE_DRIFT_STATES.includes(value)
        ? value
        : undefined;
}
// Compile-time guard.
function _assertStatesIntact() {
    ...
    const _s = "PLATFORM_TRUST";
    void _s;
    const _m = "HOW_VERIFICATION_WORKS";
    void _m;
    const _ai = "MODELS_USED";
    void _ai;
    const _sec = "AUTHENTICATION";
    void _sec;
}
...
const SEED_ARTICLES = [
    // -------- TRUST_CENTER --------
    {
        ...
        summary: "PROOVRA's bounded value: capture, verification, provenance, review, redaction, governance. We delegate provider-tier extraction to vendors with bounded adapters.",
        body: "PROOVRA is an evidence operations platform. Every operation is workspace-anchored, every state transition is recorded, every paid provider call is budget-gated, every reviewer correction is append-only. Bounded provenance is the contract.",
    ...
    },
    {
        ...
        summary: "Every evidence artifact carries a SHA-256 hash, an OpenTimestamps proof, a custody event chain, and a verification package. The verify page renders the chain offline.",
        body: "Verification is anchored by: (1) cryptographic file hash, (2) trusted timestamp via OpenTimestamps, (3) custody event chain, (4) signed verification package. Each step is reproducible by an external auditor with bounded inputs.",
        implementationReferences: [
    ...
    },
    {
        ...
        summary: "Originals are immutable. Object-locked storage. Cryptographic hashes computed at capture. Derivatives are tracked separately.",
        body: "Evidence rows record the original file SHA-256 at capture, and storage is configured with Object Lock so the bytes cannot be silently replaced. Reviewer corrections and redactions produce DERIVATIVES — the original remains intact.",
        implementationReferences: [
    ...
    },
    {
        ...
        summary: "Every original capture is timestamped at upload + anchored via OpenTimestamps for independent verification.",
        body: "The platform records UTC timestamps on every evidence row, every custody event, every workflow state. OpenTimestamps proofs are produced for the file hash so a third party can attest the file existed at the timestamped moment.",
        implementationReferences: [
    ...
    },
    {
        ...
        summary: "Public, vendor-agnostic blockchain anchoring. Proofs are verifiable using the upstream `opentimestamps-client`.",
        body: "We use OpenTimestamps because it is open standard, public, vendor-agnostic, and verifiable offline against the public Bitcoin blockchain. Proof files are shipped inside the verification package ZIP.",
        implementationReferences: [
            "services/worker/src/jobs/ots-anchoring",
            "services/worker/src/jobs/verification-package",
        ],
    ...
    },
    {
        ...
        summary: "Every access, every transition, every export writes a bounded custody event row. The chain is append-only.",
        body: "Custody events record evidence_id, actor_user_id, event_type, occurred_at_utc, plus a bounded payload. There is no delete path. The chain is rendered into the verification package and into the reviewer / portal surfaces.",
        implementationReferences: [
    ...
    },
    {
        ...
        slug: "provenance",
        title: "Provenance",
        summary: "Capture device attestations (App Attest / Play Integrity), signed canonical-JSON envelopes, server-verified Ed25519 signatures.",
        body: "Mobile captures carry a bounded TrustEnvelope: device key + attestation + canonical-JSON-signed body. The server verifies the Ed25519 signature against the registered device key before accepting the capture.",
        implementationReferences: [
    ...
    },
    {
        ...
        summary: "Authentication, authorization, RBAC, MFA, SAML, SCIM, KMS encryption, object lock, audit logging — see the Security Documentation Center.",
        body: "Defence in depth. RBAC governs every server-side operation. MFA + SAML governs identity. SCIM governs provisioning. KMS governs encryption. Object Lock governs evidence immutability.",
        implementationReferences: [
    ...
    },
    {
        ...
        slug: "ai-governance",
        title: "AI governance",
        summary: "Provider-tier extraction (Azure DI / Deepgram / OpenAI / Rekognition) gated by budget + audit + reviewer corrections. AI output is never ground truth.",
        body: "Every paid provider call passes through `runProviderOperation`: budget gate → adapter dispatch → bounded ingest → usage event → lifecycle event. Reviewer corrections are append-only; the final confidence band is fused from provider + reviewer bands. See the AI Disclosure Center.",
        implementationReferences: [
            "services/api/src/services/intelligence/media-intelligence.service.ts",
    ...
    },
    {
        ...
        slug: "reliability",
        title: "Reliability",
        summary: "Operational status surfaced via the in-platform Status Page; Better Stack integration available when bound. Honest degradation when not.",
        body: "Status components include API, Verification, Capture, Reports, AI Services, Azure, Deepgram, AWS, Background Workers, Queue Health, Storage Health. Incidents include investigating / identified / monitoring / resolved + postmortem.",
        implementationReferences: [
    ...
    },
    {
        ...
        summary: "All evidence is processed in the customer's workspace. Provider calls send only the bounded bytes required for the requested operation. See Subprocessors.",
        body: "Data is workspace-anchored on every read and write. Provider calls send only the bytes required for the requested operation (e.g. Azure DI receives the document; Deepgram receives the audio). No cross-tenant aggregation.",
        implementationReferences: [
    ...
    },
    {
        ...
        slug: "privacy",
        title: "Privacy",
        summary: "Reviewer corrections, audit events, and executive metrics surface bounded counts + ids + bands. Raw OCR / transcript / entity text never reaches manifests or dashboards.",
        body: "Privacy is enforced by the data contract: every executive surface and every audit row is bounded. Raw provider payloads are stored on the per-record `payload` JSONB column and never republished into manifests, audit, executive or report surfaces.",
        implementationReferences: [
    ...
    },
    {
        ...
        summary: "AWS, Azure, Deepgram, OpenAI, Better Stack, Sentry, Cloudflare. Each with purpose, region, data categories, state, effective date — see the Subprocessors page.",
        body: "The Subprocessor Registry is the authoritative list. Every entry is versioned; every change writes a SubprocessorVersion row with a `changeSummary` for audit.",
        implementationReferences: [
    ...
    },
    {
        ...
        slug: "governance",
        title: "Governance",
        summary: "Organizations → Departments → Workspaces. Delegated admin tiers. Policy registry with org / dept / workspace assignment + inheritance + override.",
        body: "Phase 4A added tiered delegated admin (Global / Org / Department / Workspace / Reviewer Lead / Security Officer / Compliance Officer), department isolation, policy inheritance, and access reviews. See the Governance Dashboard.",
        implementationReferences: [
    ...
    },
    {
        ...
        slug: "transparency",
        title: "Transparency",
        summary: "Federated audit timeline across six bounded sources. Failure reasons surfaced. Budget breaches surfaced. Reviewer correction lifecycle surfaced.",
        body: "The Audit & Transparency Center federates provider usage, reviewer corrections, redaction activity, policy audit, video timeline, external review activity, and the canonical intelligence_activity_events table — surfacing bounded counts + ids + labels for every operator action.",
        implementationReferences: [
            "services/api/src/services/intelligence/audit-transparency.service.ts",
        ],
    ...
    // -------- METHODOLOGY --------
    {
        ...
        summary: "Hash → trusted timestamp → custody chain → verification package. Every step reproducible offline.",
    ...
    },
    {
        ...
        summary: "SHA-256 is computed at upload. The bounded hex digest is recorded on the evidence row and shipped in the manifest.",
    ...
    },
    {
        ...
        summary: "Server records UTC timestamps for every transition; OpenTimestamps anchors the file hash to public blockchain.",
    ...
    },
    {
        ...
        summary: "OTS aggregates hashes into a Merkle tree, anchors the tree root on Bitcoin, then issues a `.ots` proof file per hash.",
    ...
    },
    {
        ...
        summary: "Mobile captures sign a canonical-JSON TrustEnvelope. The server verifies the signature against the registered device key.",
    ...
    },
    {
        ...
        summary: "Verification packages are signed ZIPs containing manifests + proofs + bounded provenance. Offline-verifiable.",
    ...
    },
    {
        ...
        summary: "Trust classes derive from device attestation strength + signature verification result + custody continuity.",
    ...
    },
    {
        ...
        summary: "Detection → reviewer decision → approval → derivative. Bounded detection providers; reviewer overrides every decision.",
    ...
    },
    {
        ...
        summary: "Vendor-agnostic adapter pattern (Azure / Deepgram / OpenAI / Rekognition) with budget gating + reviewer correction version chain.",
    ...
    // -------- AI_DISCLOSURE --------
    {
        ...
        slug: "models-used",
        title: "Models used",
        summary: "Azure Document Intelligence (OCR / layout / tables / forms); Deepgram (ASR / diarisation); OpenAI (entity extraction / document summary / text embeddings when SEMANTIC_SEARCH_ENABLED + opt-in outbound); AWS Rekognition (faces / text / labels). Local REGEX_PII fallback when OpenAI is not bound.",
        body: "Models are selected per operation by the provider adapter (services/api/src/services/intelligence/providers). Bounded REGEX_PII fallback runs locally when OpenAI is not configured so PII detection remains useful in air-gapped deployments. Embedding models (text-embedding-3-small by default) are gated behind both SEMANTIC_SEARCH_ENABLED=true and SEMANTIC_EMBEDDINGS_SEND_CONTENT_OUTBOUND=true; both default to false. Model selections are pinned per provider and recorded on every usage event so operators can audit which model produced which output.",
        implementationReferences: [
            "services/api/src/services/intelligence/providers",
    ...
    },
    {
        ...
        summary: "Only the bytes required for the requested operation (e.g. Azure DI receives the document file; Deepgram receives the audio file; AWS Rekognition receives the image). The adapter abstracts the request shape so the orchestrator never serialises tenant metadata into the payload.",
        body: "Bounded per operation. Azure DI receives the document file + region-pinned endpoint. Deepgram receives the audio bytes + a bounded model parameter. OpenAI entity-extraction receives the OCR or transcript text (never the raw file, never identifiers). OpenAI embeddings receive chunked text only when SEMANTIC_EMBEDDINGS_SEND_CONTENT_OUTBOUND=true. AWS Rekognition receives the image bytes. Across all providers, payloads never include reviewer identities, custody events, organization names, policy state, version chains, or any cross-workspace data. Provider TLS endpoints are pinned at the adapter layer.",
        implementationReferences: [
            "services/api/src/services/intelligence/providers",
            "services/api/src/services/search/embedding-provider.ts",
        ],
    ...
    },
    {
        ...
        summary: "Provider raw confidence is mapped to a bounded band (LOW / MEDIUM / HIGH / VERY_HIGH) via classifyIntelligenceConfidence. Cut-offs: >=0.95 VERY_HIGH, >=0.8 HIGH, >=0.5 MEDIUM, else LOW. Reviewer override always wins. The final fused band is what surfaces on executive + reviewer + reporting surfaces.",
        body: "Provider vendors emit confidence on different scales with different semantics; the bounded band normalises them so an Azure HIGH means the same thing to a reviewer as a Deepgram HIGH. The reviewer's accept / reject / correct decision is the source of truth — if a reviewer corrects an AI output, the final band defers to the reviewer band regardless of the provider band. Confidence bands are never represented as legal certainty: they are signals to prioritise review effort, never to bypass it.",
        implementationReferences: [
            "packages/shared/src/media-intelligence-platform.ts",
            "services/api/src/services/intelligence/reviewer-correction.service.ts",
        ],
    ...
    },
    {
        ...
        summary: "Every AI / provider output flows into the Reviewer Workspace. Reviewers accept, reject, or correct each output. AI output is NEVER ground truth — the reviewer decision is. Corrections are append-only with explicit version chain so the history is reproducible.",
        body: "The Reviewer Workspace renders the AI output alongside the original evidence, the bounded confidence band, the provider that produced the output, and the reviewer's prior decisions on related items. The reviewer's verdict writes a ReviewerCorrection row with versionNumber + parentCorrectionId + supersedesCorrectionId — there is no in-place mutation. Disagreement workflow (challenge -> second review -> supervisor -> resolution) is supported via the reviewer-disagreement.service.ts. QC sampling assigns randomised double-review to measure accuracy. The fused final confidence band always defers to the latest reviewer correction.",
        implementationReferences: [
            "services/api/src/services/intelligence/reviewer-correction.service.ts",
    ...
    },
    {
        ...
        slug: "limitations",
        title: "Limitations",
        summary: "AI is never ground truth. The reviewer is. AI provides signals — confidence bands, candidate entities, suggested redactions — that prioritise human attention; it never substitutes for human judgement on factual truth, authorship, identity, or legal admissibility.",
    ...
    },
    {
        ...
        summary: "Every provider call writes a provider_usage_events row + an intelligence_activity_events lifecycle row. The Audit & Transparency Center federates both alongside reviewer corrections, redaction activity, policy audit, video timeline, and external review activity.",
        body: "Operators can answer 'when did this provider call happen, who initiated it, did it succeed, why did it fail, what did it cost' from one place. The federation surfaces bounded counts, ids, and labels — never raw provider payloads. Date-range filtering + trend math (executive-metrics service) lets operators detect anomalies and quality drift over time. The verification package includes a snapshot of the relevant audit slice so external auditors can reproduce the activity trail offline.",
        implementationReferences: [
            "services/api/src/services/intelligence/audit-transparency.service.ts",
        ],
    ...
    },
    {
        ...
        summary: "SEMANTIC_EMBEDDINGS_SEND_CONTENT_OUTBOUND defaults to FALSE. Outbound content (sending chunk text to an external embedding provider like OpenAI) requires explicit operator opt-in. The default posture is air-gapped semantic disabled — operators must affirmatively enable outbound traffic.",
        body: "The dual-gate (SEMANTIC_SEARCH_ENABLED + SEMANTIC_EMBEDDINGS_SEND_CONTENT_OUTBOUND) is enforced INSIDE every provider implementation that would talk to an external service (embedding-provider.ts:261 and :545) — not relied on at a single call site, so accidentally enabling one without the other still blocks outbound traffic. Operators who run on-prem or air-gapped deployments can leave outbound disabled and either run keyword-only or wire a local embedding model in future. This default reflects PROOVRA's posture: customer data does not leave the platform without an explicit operator decision documented in env config + audit logs.",
        implementationReferences: [
            "services/api/src/services/search/embedding-provider.ts",
        ],
    ...
    },
    {
        ...
        summary: "Workspace roles + reviewer roles + external-portal roles + 7 delegated-admin tiers. Capability projection drives the UI; the server re-enforces on every mutation.",
    ...
    },
    {
        ...
        summary: "SAML SSO supported for external reviewer grants. Identity is validated server-side.",
    ...
    },
    {
        ...
        summary: "SCIM 2.0 user + group provisioning routes, token management, drift preview/reconciliation, and sync-failure replay exist in the product today. Availability remains configuration-dependent and the supported subset is intentionally bounded.",
        body: "PROOVRA exposes token-authenticated `/v2/scim` routes for Users and Groups, backed by scoped `ScimProvisioningToken` credentials, soft deprovisioning, and audit emission. Operators manage tokens, reconciliation preview/execute, and sync-failure replay from the SCIM Operations Center. The implementation is intentionally bounded rather than full-RFC-complete: PATCH support is limited, provisioning still requires operator configuration, and workspace readiness can remain disabled until the org explicitly enables the posture.",
        implementationReferences: [
    ...
    },
    {
        ...
        slug: "encryption",
        title: "Encryption",
        summary: "Evidence + derivatives persist to an S3-compatible storage provider (AWS S3, Cloudflare R2, or Google Cloud Storage) over TLS; at-rest encryption is supplied by the storage provider per its bucket configuration. Production endpoints are TLS-enforced.",
        body: "PROOVRA does not currently configure per-tenant KMS keys or PROOVRA-managed SSE-KMS headers in the application layer — the storage layer relies on the provider's default at-rest encryption (S3 SSE-S3 by default on AWS; R2 + GCS encrypt at rest by default). In-transit security is enforced: storage.ts rejects http:// endpoints in production unless S3_ALLOW_INSECURE=true is set explicitly. Operators who require customer-managed keys must configure them at the bucket level outside PROOVRA. The KMS section of this Security Center documents the deferred-Signer/KMS posture honestly.",
        implementationReferences: [
    ...
    },
    {
        ...
        slug: "audit-logging",
        title: "Audit logging",
        summary: "Append-only audit federation across seven bounded sources: provider usage, reviewer corrections, redaction activity, policy audit, video timeline, external review activity, intelligence lifecycle. Trust + governance lifecycle events emit alongside.",
        body: "The audit-transparency.service.ts federator unifies bounded counts + ids + labels from intelligence_activity_events, provider_usage_events, reviewer_corrections, redaction_activity, policy_audit, video_timeline, and external_review_activity. The Phase 4A trust + governance lifecycle codes (TRUST_ARTICLE_CREATED / SUPERSEDED / PUBLISHED, DELEGATED_ADMIN_GRANTED / REVOKED, POLICY_BLOCK / VIOLATION, ACCESS_REVIEW_DECIDED, etc.) emit through the same emitter so operators can answer who-did-what-when from one place. Every audit row is append-only; there is no delete path. SecurityEvent rows additionally capture identity-security events (login, MFA, step-up, quarantine, revoke, webhook signature failure).",
        implementationReferences: [
            "services/api/src/services/intelligence/audit-transparency.service.ts",
    ...
    },
    {
        ...
        summary: "Incidents tracked in the Status Page incident table with a bounded state machine: INVESTIGATING -> IDENTIFIED -> MONITORING -> RESOLVED, with optional POSTMORTEM_DRAFT and POSTMORTEM_PUBLISHED follow-ups. Updates are append-only and emit STATUS_INCIDENT_UPDATED lifecycle codes.",
    ...
    },
    {
        ...
        slug: "deletion",
        title: "Deletion",
        summary: "Deletion runs through bounded lifecycle workflows: retention-policy evaluation, legal-hold gates, destruction reviews, destruction certificates, and lifecycle-ledger tombstoning. Originals under Object Lock cannot be deleted until retention eligibility is satisfied.",
        body: "Destructive outcomes are gated twice: first by lifecycle-orchestrator.service.ts and then again by the destruction orchestrator worker at execution time. The worker emits a destruction certificate plus lineage hash, preserves tombstone rows, and records the DESTROYED transition through the lifecycle ledger rather than mutating history in place. Storage payload deletion is delegated to the existing purge processor, but the governance execution row captures the boundary and failure state so operators can audit the outcome end to end.",
        implementationReferences: [
            "services/api/src/services/governance-lifecycle/lifecycle-orchestrator.service.ts",
            "services/api/src/services/lifecycle/legal-hold.service.ts",
    ...
    },
    {
        ...
        summary: "Operators and adaptive-auth signals can immediately revoke an authenticated session via the Phase 19 revokeSession() pathway. Revocation invalidates the AuthenticatedSession row, emits a SecurityEvent, and the next request from the revoked session is rejected by the auth middleware.",
        body: "services/api/src/services/identity-security/session-revocation.service.ts.revokeSession() marks the AuthenticatedSession row revoked + emits a session_revoked SecurityEvent with bounded reason. The middleware/auth.ts validator rejects revoked sessions on the next request. Three production callers: (1) the /v1/identity-security/sessions/:id/revoke route for operator-initiated revoke, (2) services/access-control/adaptive-auth.service.ts which auto-revokes on high-risk signals, (3) services/access-control/session-inventory.service.ts which supports bulk revoke from the operator session-inventory surface. External reviewer portal sessions revoke via the parallel portal-session.service.ts pathway.",
        implementationReferences: [
            "services/api/src/services/identity-security/session-revocation.service.ts",
            "services/api/src/services/access-control/adaptive-auth.service.ts",
            "services/api/src/services/access-control/session-inventory.service.ts",
