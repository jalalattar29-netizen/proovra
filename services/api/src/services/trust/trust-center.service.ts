...
const SEED_ARTICLES: ReadonlyArray<SeedArticle> = [
  ...
  // -------- AI_DISCLOSURE --------
  {
    ...
    body:
      "Models are selected per operation by the provider adapter (services/api/src/services/intelligence/providers). The entity-extraction and document-summary adapters make NO outbound OpenAI call (no `callOpenAI` seam exists); they always run a bounded LOCAL REGEX_PII entity pass and a deterministic truncated summary, and are labeled `LOCAL_ENTITY_EXTRACTION` / `LOCAL_DOCUMENT_SUMMARY` accordingly (legacy rows recorded under the former OPENAI_* labels are renamed by the mi_provider_local_value_rename migration). Binding OPENAI_API_KEY does not change their behaviour and their provider probe reports NOT_CONFIGURED. The only real OpenAI path is embeddings (text-embedding-3-small by default), gated behind both SEMANTIC_SEARCH_ENABLED=true and SEMANTIC_EMBEDDINGS_SEND_CONTENT_OUTBOUND=true; both default to false. Model selections are pinned per provider and recorded on every usage event so operators can audit which model produced which output.",
    implementationReferences: [
      "services/api/src/services/intelligence/providers",
