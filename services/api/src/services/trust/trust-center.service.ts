...
const SEED_ARTICLES: ReadonlyArray<SeedArticle> = [
  ...
  // -------- AI_DISCLOSURE --------
  {
    ...
    title: "Models used",
    summary:
      "Azure Document Intelligence (OCR / layout / tables / forms); Deepgram (speech recognition / speaker separation); AWS Rekognition (faces / text / labels); OpenAI text embeddings (text-embedding-3-small) ONLY when semantic search and outbound content are both switched on (both default off). Entity extraction and document summarisation are NOT wired to a live OpenAI call — they run as a bounded local, deterministic fallback (pattern-based entity detection and a truncated summary).",
    body:
      "A model is chosen per operation by that operation's provider adapter. Entity extraction and document summarisation make no outbound call to OpenAI at all: they always run a bounded local pattern-based entity pass and a deterministic truncated summary, and they are recorded under local labels so their origin is unambiguous in the activity log. Configuring an OpenAI key does not change that — those capabilities continue to report themselves as not configured. The only path that reaches OpenAI is text embeddings, and it requires semantic search and outbound content to be switched on independently; both are off by default. Model selections are pinned per provider and recorded on every usage event, so you can audit which model produced which output.",
    implementationReferences: [
      "services/api/src/services/intelligence/providers",
  ...
  },
  {
    ...
    summary:
      "Bounded ProviderAdapter set: AZURE_DOCUMENT_INTELLIGENCE, DEEPGRAM_TRANSCRIPT, AWS_REKOGNITION_FACES, AWS_REKOGNITION_TEXT, AWS_REKOGNITION_LABELS, OPENAI_EMBEDDINGS (opt-in, default off), MANUAL_OPERATOR — each a real binding that reports READY when configured. LOCAL_ENTITY_EXTRACTION and LOCAL_DOCUMENT_SUMMARY run a LOCAL fallback only (no live OpenAI call); their probe reports NOT_CONFIGURED rather than asserting an operational provider binding. Every call is recorded in provider_usage_events.",
    body:
  ...
  },
  {
    ...
    body:
      "Bounded per operation. Azure Document Intelligence receives the document file and a region-pinned endpoint. Deepgram receives the audio and a bounded model setting. AWS Rekognition receives the image. Entity extraction and document summarisation send nothing outbound at all — that text is processed locally by a bounded pattern-based pass and truncation, so no OpenAI call is made for those operations. OpenAI embeddings are the only path that reaches OpenAI, and they receive chunked text only when outbound content is switched on, which it is not by default. Across all providers, what we send never includes reviewer identities, custody events, organization names, policy state, correction histories, or anything from another workspace. Encrypted provider endpoints are pinned where the call is made.",
    implementationReferences: [
      "services/api/src/services/intelligence/providers",
      "services/api/src/services/search/embedding-provider.ts",
    ],
  ...
  },
  {
    ...
    body:
      "The Reviewer Workspace shows the AI output beside the original evidence, the bounded confidence band, the provider that produced it, and the reviewer's earlier decisions on related items. A reviewer's verdict is written as a new correction record carrying its version number and links to the record it follows and the one it replaces — nothing is edited in place. Disagreements follow a defined path: challenge, second review, supervisor, resolution. Quality sampling assigns randomised double review to measure accuracy. The final confidence band always defers to the most recent reviewer correction.",
    implementationReferences: [
      "services/api/src/services/intelligence/reviewer-correction.service.ts",
  ...
  },
  {
    ...
    body:
      "Each correction is appended, never overwritten: every record carries a version number, a link to its immediate predecessor and a link to the original it replaces. Reverting appends a record marking the reversal; superseding appends a record referencing the one before it. The full chain can be reconstructed for any item, and it travels inside the verification package so an external auditor can reproduce the decision trail offline. Every created, accepted, reverted and superseded correction raises a lifecycle event into the Audit & Transparency Center.",
    implementationReferences: [
      "services/api/src/services/intelligence/reviewer-correction.service.ts",
    ],
  ...
  },
  {
    ...
    body: `AI assistance throughout PROOVRA carries the standing disclaimer: "${AI_LEGAL_DISCLAIMER}". It is enforced on our servers, not in the interface: a policy gate blocks AI output containing forbidden language — claims such as "this proves what happened", "legally admissible", "the person in the image is" or "definitively shows" — before that output can reach you. The standing limitations are repeated on every executive summary. Confidence bands describe a model's assessment of its own output, not external truth, and we monitor providers for drift in what their confidence scores mean.`,
    implementationReferences: [
  ...
  },
  {
    ...
    body:
      "Mitigations: (1) budget limits stop runaway spend on a misbehaving provider, (2) every provider call is captured in the audit record so anomalies are reviewable, (3) per-provider quality rankings detect drift over time, (4) correction analytics surface systematic provider error patterns, (5) standing limitations appear on every executive surface to reinforce that AI is signal and not truth, and (6) the policy gate blocks AI output that drifts into forbidden ground-truth language. Prompt injection hidden inside scanned or transcribed content is mitigated by the bounded local fallback and by treating all extracted text as untrusted until a reviewer confirms it.",
    implementationReferences: [
  ...
  },
  {
    ...
    summary:
      "The cost of every provider call is recorded against that call. Budgets can be set per case, project, team, workspace or provider, each with a defined enforcement mode. Breaches are written to the audit record and shown in the Budget Center.",
    body:
      "A budget applies only to what it names, so a per-project limit never blocks an unrelated project. Threshold status is shown as HEALTHY, APPROACHING or EXCEEDED, with spend, remaining, projected burn and breach history for each scope. Embedding spend for semantic search is governed by the same model, with per-workspace caps enforced before any outbound call is made. We do not absorb breaches silently: you see the verdict and decide whether to raise the cap or stop the work.",
    implementationReferences: [
      "services/api/src/services/intelligence/provider-budget.service.ts",
  ...
  // -------- AI_DISCLOSURE (additions: Phase 15/16 semantic search disclosure) --------
  {
    ...
    summary:
      "When semantic search is switched on and a provider is configured, text taken from documents and transcripts is turned into vectors so results can be found by meaning rather than exact words. Content is never sent to an external provider unless outbound content is separately switched on. Per-workspace budget limits apply, and if anything fails the search falls back to keyword matching.",
    body:
      "Three embedding providers exist: a disabled one, which is the default and produces nothing; a local deterministic one used only in testing; and OpenAI, which is gated behind the two switches above. When embeddings are available, search blends keyword and meaning-based ranking; when they are not, it falls back to keyword matching and says plainly in the interface that semantic search is off. Embeddings are scoped to your workspace — there is no shared vector pool across customers — and per-workspace spend caps are enforced before any outbound call is made.",
    implementationReferences: [
      "services/api/src/services/search/embedding-provider.ts",
  ...
  },
  {
    ...
    summary:
      "Sending content out defaults to OFF. Passing text to an external embedding provider such as OpenAI requires an explicit operator decision to switch it on. The default posture is that semantic search is disabled and nothing leaves the platform; turning it back off preserves keyword search and the local audit history you already have.",
    body:
      "Both switches are checked inside every provider that could talk to an external service, rather than once at a single point in the code. That matters: switching on one without the other still blocks outbound traffic, so no single mistake can start sending your content. Deployments that must stay isolated can leave outbound off and run keyword search. When it is off, the system records that honestly rather than presenting semantic AI as available. The principle is simple: your content does not leave the platform without an explicit operator decision, and that decision is recorded in the audit log.",
    implementationReferences: [
      "services/api/src/services/search/embedding-provider.ts",
  ...
  // -------- SECURITY (additions: undisclosed controls with proven implementation) --------
  {
    ...
    body:
      "services/api/src/services/operations/signer-registry.service.ts exposes a deterministic SignerRecord for each purpose: provider (aws_kms | local_pem | disabled), keyId, keyVersion, kmsKeyArn (when applicable), algorithm, status (active / staged / retiring / retired / revoked / degraded), activatedAt, retiredAt. The rotation history comes from signer_staged / signer_promoted / signer_retired / signer_revoked audit events; the active state comes from env (SIGNER_PROVIDER, SIGNING_KEY_ID, SIGNING_KEY_VERSION, PACKAGE_SIGNING_KEY_ID, PACKAGE_SIGNING_KEY_VERSION, KMS_KEY_ID). Historical artefacts are never mutated when keys rotate — the registry tracks FUTURE signing only so existing signed outputs remain reproducibly verifiable. A signed Report records the key that signed it in Report.pdfSignerKeyId. A Verification Package records its signature inside the package itself, in MANIFEST.json.sig, which is the artefact a verifier reads; the API does not restate a package signer key id, because it holds no per-package record of one and reporting the current key would misattribute older packages after a rotation. Private key material is never returned by any API.",
    implementationReferences: [
      "services/api/src/services/operations/signer-registry.service.ts",
      "docs/security/signer-governance.md",
    ],
