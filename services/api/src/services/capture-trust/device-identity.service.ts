...
import {
  ...
  type CaptureSignatureAlgorithm,
...
export type RegisterDeviceInput = {
  ...
  /** Hex public key (Ed25519 = 64 chars, P-256 uncompressed = 130 chars). */
  publicKeyHex: string;
...
export type DeviceDenialReason =
  | "DEVICE_NOT_FOUND"
...
export type RegisterDeviceResult =
  | { ok: true; deviceId: string; publicKeyFingerprint: string }
  | { ok: false; denial: DeviceDenialReason };
...
export async function registerDevice(
  ...
  if (
    !(DEVICE_ATTESTATION_PROVIDERS as ReadonlyArray<string>).includes(
...
export async function revokeDevice(
  ...
  if (!device) return { ok: false, denial: "DEVICE_NOT_FOUND" };
...
export async function listDevicesForTeam(
  teamId: string,
