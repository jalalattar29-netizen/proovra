...
import { CAPTURE_SIGNATURE_ALGORITHMS, signatureVerdictIsFatal, } from "@proovra/shared";
import { prisma as defaultPrisma } from "../../db.js";
import { canonicalize, CanonicalJsonError } from "./canonical-json.js";
// -----------------------------------------------------------------------------
// Public verifier
// -----------------------------------------------------------------------------
export async function verifyCaptureSignature(input) {
    ...
    if (!CAPTURE_SIGNATURE_ALGORITHMS.includes(input.payload.algorithm)) {
    ...
    try {
        ...
        if (err instanceof CanonicalJsonError) {
            return result("INVALID_CANONICAL_JSON", computedAssetHash, 0, device.id);
        }
        return result("INVALID_CANONICAL_JSON", computedAssetHash, 0, device.id);
    }
    ...
    if (!signatureBytes) {
        return result("INVALID_SIGNATURE", computedAssetHash, canonicalBytes.length, device.id);
    }
    let valid = false;
    try {
    ...
    return result(valid ? "VALID" : "INVALID_SIGNATURE", computedAssetHash, canonicalBytes.length, device.id);
}
// -----------------------------------------------------------------------------
// Helpers
// -----------------------------------------------------------------------------
function result(verdict, computedAssetHash, canonicalBytesLength, deviceId) {
    return {
        verdict,
...
function structuralValidate(p) {
    if (typeof p !== "object" || p === null)
        return "not-object";
    const o = p;
    if (o["schemaVersion"] !== "PROOVRA_CAPTURE_SIG_V1")
        return "schema-version";
    if (typeof o["assetHash"] !== "string" || o["assetHash"].length !== 64) {
        return "asset-hash";
    }
    if (typeof o["captureMode"] !== "string")
        return "capture-mode";
    if (typeof o["provenanceClass"] !== "string")
        return "provenance-class";
    if (typeof o["deviceKeyId"] !== "string")
        return "device-key-id";
    if (typeof o["algorithm"] !== "string")
        return "algorithm";
    if (typeof o["captureSessionId"] !== "string")
        return "capture-session-id";
    if (typeof o["signedAtUtc"] !== "string")
        return "signed-at-utc";
    if (typeof o["signedAtMonotonicNs"] !== "string")
        return "monotonic";
    if (typeof o["nonceHex"] !== "string" || o["nonceHex"].length !== 64) {
        return "nonce-hex";
    }
 ...
 */
function ed25519RawPubkeyToPem(hex) {
 ...
 */
function p256RawPubkeyToPem(hex) {
    const raw = Buffer.from(hex, "hex");
