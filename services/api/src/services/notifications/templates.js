 ...
 */
import { escapeEmailHtml, getEmailBrandName, getEmailWebBaseUrl, renderEmailShell, } from "../email.service.js";
// -----------------------------------------------------------------------------
// renderTransactionalTemplate
// -----------------------------------------------------------------------------
export function renderTransactionalTemplate(eventType, ctx) {
    switch (eventType) {
        ...
        case "EVIDENCE_REQUEST_FULFILLED":
            return renderRequestStatusUpdate(expectCtx(ctx, "Simple"), "fulfilled");
        case "EVIDENCE_REQUEST_CANCELLED":
            return renderRequestStatusUpdate(expectCtx(ctx, "Simple"), "cancelled");
        ...
        case "REVIEW_REASSIGNED":
            return renderReviewVariant(expectCtx(ctx, "ReviewAssigned"), "Review reassigned", "You have been assigned as the new reviewer");
        case "REVIEW_ESCALATED":
            return renderReviewVariant(expectCtx(ctx, "ReviewAssigned"), "Review escalated", "A review you own has been escalated");
        case "REVIEW_OVERDUE_REMINDER":
            return renderReviewVariant(expectCtx(ctx, "ReviewAssigned"), "Review overdue", "A review you own is past its due date");
        case "REVIEW_NEEDS_MORE_INFO":
            return renderReviewVariant(expectCtx(ctx, "ReviewAssigned"), "Review needs more information", "Additional information has been requested");
        case "REVIEW_RESPONSE_RECEIVED":
            return renderReviewVariant(expectCtx(ctx, "ReviewAssigned"), "Review response received", "A response has been received on a review you own");
        ...
        case "DISCUSSION_MENTION_RECEIVED":
            return renderReviewVariant(expectCtx(ctx, "ReviewAssigned"), "You were mentioned", "You were @mentioned in a workspace discussion");
        case "DISCUSSION_REPLY_RECEIVED":
            return renderReviewVariant(expectCtx(ctx, "ReviewAssigned"), "Discussion reply", "A discussion you participate in has a new reply");
        case "DISCUSSION_RESOLVED":
            return renderReviewVariant(expectCtx(ctx, "ReviewAssigned"), "Discussion resolved", "A discussion you participate in has been resolved");
        case "DISCUSSION_REOPENED":
            return renderReviewVariant(expectCtx(ctx, "ReviewAssigned"), "Discussion reopened", "A discussion you participate in has been reopened");
        case "CONTRIBUTOR_REPLY_RECEIVED":
            return renderReviewVariant(expectCtx(ctx, "ReviewAssigned"), "Contributor reply", "An external contributor replied on a discussion you own");
        default: {
            const _exhaustive = eventType;
            throw new Error(`renderTransactionalTemplate: no renderer for ${_exhaustive}`);
        }
    }
}
function expectCtx(ctx, kind) {
    if (ctx.kind !== kind) {
        throw new Error(`renderTransactionalTemplate: expected context kind "${kind}" but got "${ctx.kind}"`);
    }
    return ctx.data;
}
// -----------------------------------------------------------------------------
...
// -----------------------------------------------------------------------------
function renderEvidenceRequestSent(d) {
    ...
    const due = d.dueAtUtcIso
        ? `<div style="margin:0 0 12px 0;"><strong>Please respond by:</strong> ${escapeEmailHtml(new Date(d.dueAtUtcIso).toLocaleString())}</div>`
        : "";
    const deliverableList = d.deliverableSummaries.length > 0
        ? `
        <div style="margin:0 0 12px 0;"><strong>What's needed:</strong></div>
        <ul style="margin:0 0 16px 18px; padding:0;">
          ${d.deliverableSummaries
            .map((item) => `<li style="margin:0 0 6px 0;">${escapeEmailHtml(item.title)}${item.required ? " (required)" : ""}</li>`)
            .join("")}
        </ul>
      `
        : "";
  ...
  `.trim();
    const html = renderEmailShell({
        ...
        secondaryText: "If you didn't expect this request, you can ignore this email.",
    });
    const text = [
        ...
        ...d.deliverableSummaries.map((item) => `  - ${item.title}${item.required ? " (required)" : ""}`),
        "",
...
function renderEvidenceRequestReminder(d) {
...
function renderResponseReceived(d) {
    const workspaceName = escapeEmailHtml(d.workspaceName);
    const title = escapeEmailHtml(d.requestTitle);
    const body = `
    <div style="margin:0 0 12px 0;">
...
function renderNeedsMoreInfo(d) {
    const workspaceName = escapeEmailHtml(d.workspaceName);
    const title = escapeEmailHtml(d.requestTitle);
...
function renderRequestStatusUpdate(d, decision) {
    const labels = {
        fulfilled: "Fulfilled",
        cancelled: "Cancelled",
        closed: "Closed",
...
function renderReviewAssigned(d) {
    const workspaceName = escapeEmailHtml(d.workspaceName);
    const title = escapeEmailHtml(d.requestTitle);
    const body = `
    <div style="margin:0 0 12px 0;">
...
function renderReviewVariant(d, subjectPrefix, headline) {
    const workspaceName = escapeEmailHtml(d.workspaceName);
    const title = escapeEmailHtml(d.requestTitle);
...
function renderIntakeLinkCreated(d) {
    const workspaceName = escapeEmailHtml(d.workspaceName);
...
function renderIntakeSubmitted(d) {
    const workspaceName = escapeEmailHtml(d.workspaceName);
    const body = `
    <div style="margin:0 0 12px 0;">
