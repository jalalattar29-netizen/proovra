...
export type UploadSizeLimits = {
  maxUploadFileSizeBytes: number;
  multipartThresholdBytes: number;
  multipartPartSizeBytes: number;
};

export function getUploadSizeLimits(): UploadSizeLimits {
  return {
    maxUploadFileSizeBytes: readIntEnv(
    ...
    multipartThresholdBytes: readIntEnv(
    ...
    multipartPartSizeBytes: readIntEnv(
...
export function getStaleThresholds(): StaleThresholds {
  return {
    stalledMinutes: readIntEnv(
      "UPLOAD_STALLED_MINUTES",
      DEFAULT_UPLOAD_STALLED_MINUTES,
      UPLOAD_STALLED_MINUTES_MIN,
      UPLOAD_STALLED_MINUTES_MAX,
    ),
    abandonedHours: readIntEnv(
...
export type UploadSizeCheckOutcome =
  | { ok: true; recommendMultipart: boolean }
  | {
      ok: false;
...
export function checkUploadSize(
  sizeBytes: number | bigint | null | undefined,
  limits: UploadSizeLimits = getUploadSizeLimits(),
): UploadSizeCheckOutcome {
  const n =
