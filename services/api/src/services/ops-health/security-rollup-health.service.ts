...
export async function evaluateSecurityRollupHealth(input: {
  ...
  let latestAnomaly: { createdAt: Date } | null = null;
  let canonicalEventCount: number = 0;
  let rollupOk = true;
  ...
  if (!rollupOk && !canonicalOk) {
    return finalize({
      status: "FAILED",
      reason:
        "Both the security anomaly classifier read AND the canonical SecurityEvent log read failed. Detection cannot be confirmed.",
      recoverable: false,
  ...
  const ageH =
    (Date.now() - latestAnomaly.createdAt.getTime()) / 3_600_000;
