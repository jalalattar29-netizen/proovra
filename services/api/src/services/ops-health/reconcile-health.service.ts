...
export async function evaluateReconcileHealth(input: {
  ...
  if (heartbeat === null && !workerAlive) {
    return finalize({
      status: "DISCONNECTED",
      reason:
        "No reviewer reconcile run has been recorded yet, and no recent worker heartbeat is present. Reviewer reconcile activates when the worker first runs the reviewer ops scheduler.",
      recoverable: true,
  ...
  if (heartbeat === null && workerAlive) {
    return finalize({
      status: "STALE",
      reason:
        "No reviewer reconcile run recorded for this workspace yet — worker is alive, so the scheduler may not have fired the reviewer reconcile loop for this team in its current cycle.",
      recoverable: true,
      lastSuccessfulRunAt: null,
      retrying: true,
      degradedSince: null,
