...
import { buildInitialDemoFollowUp, buildInitialDemoRouting, } from "./demo-follow-up.service.js";
...
function inferPriority(input) {
    ...
    if (input.leadTrack === prismaPkg.DemoLeadTrack.ENTERPRISE ||
...
async function calculateSpamAssessment(input, context) {
    let spamScore = 0;
    const reasons = [];
...
export async function createDemoRequest(rawInput, context = {}) {
    const parsed = createDemoRequestSchema.safeParse(rawInput);
    if (!parsed.success) {
        const firstIssue = parsed.error.issues[0];
        throw new AppError(ErrorCode.VALIDATION_ERROR, firstIssue?.message || "Invalid demo request payload");
    }
    ...
    const leadQuality = scoredLead.leadQuality;
    const leadTrack = scoredLead.leadTrack;
    const recommendedAction = scoredLead.recommendedAction;
    ...
    const notificationEmail = envString("DEMO_REQUEST_NOTIFICATION_EMAIL") ??
        envString("SUPPORT_EMAIL") ??
        "support@proovra.com";
    let emailSentAt = null;
    let autoReplySentAt = null;
    let webhookSentAt = null;
    if (emailService.isConfigured()) {
        ...
        if (autoReplyEnabled && !created.isSpam) {
            await emailService.sendDemoRequestAutoReply({
                ...
                responseWindowText: created.leadTrack === prismaPkg.DemoLeadTrack.ENTERPRISE
