...
export function sanitizePageContextPath(path) {
...
export class AiChatService {
    ...
    async analyzeChat(userId, payload) {
        const guard = this.costGuard.canSendChatMessage(userId);
        if (!guard.allowed) {
            return {
                status: "blocked",
                summary: "AI chat support is temporarily blocked due to usage or budget limits.",
        ...
        const result = await this.provider.run(AiTask.SUPPORT_CHAT, sanitizeChatPayload(payload));
        const cleanedResult = {
            ...result,
            warnings: result.status === "ok"
                ? result.warnings.slice(0, 1)
                : result.warnings,
            suggestions: result.status === "ok"
                ? result.suggestions.slice(0, 2)
                : result.suggestions,
            flags: result.status === "ok"
                ? []
                : result.flags,
        };
