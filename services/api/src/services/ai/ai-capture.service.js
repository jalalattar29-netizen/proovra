...
function inferEvidenceTypeFromMimeType(mimeType) {
 ...
 */
function redactFileName(originalName) {
...
function buildExtensionCategory(fileName) {
...
function bucketSizeBytes(sizeBytes) {
    const n = typeof sizeBytes === "number" && Number.isFinite(sizeBytes) ? sizeBytes : 0;
    if (n <= 0)
        return "empty";
    if (n < 100 * 1024)
        return "tiny_lt_100KB";
    if (n < 1024 * 1024)
        return "small_lt_1MB";
    if (n < 10 * 1024 * 1024)
        return "medium_lt_10MB";
    if (n < 100 * 1024 * 1024)
        return "large_lt_100MB";
    if (n < 1024 * 1024 * 1024)
        return "very_large_lt_1GB";
    return "massive_gt_1GB";
}
function buildRedactedItems(items) {
...
function buildDeterministicSessionReview(payload) {
    ...
    if (missingSteps.length > 0) {
        ...
        warnings.push(`Required intake steps are not complete. ${missingSteps.length} required step(s) remain.`);
    ...
    if (payload.planMode === "CHECKLIST_REQUIRED" && payload.collectionPlan.locationRequirement === "required" && !payload.useLocation) {
        flags.push({
            ...
            detail: "The selected plan requires location metadata, but location consent is not enabled.",
        });
        warnings.push("Location metadata is required by the selected intake plan.");
        suggestions.push("Enable location metadata or choose a plan that does not require location.");
    }
    ...
    payload.items.forEach((item) => {
        ...
        const step = payload.collectionPlan.steps.find((candidate) => candidate.id === item.checklistStepId);
        if (item.clientSignals?.genericMime) {
            ...
            suggestions.push("Provide a more specific file type or confirm the file mapping before finalizing.");
        }
        ...
        if (item.clientSignals?.oldLastModified) {
            ...
            suggestions.push("Confirm whether the file age is appropriate for the intake scenario.");
        }
        if (item.clientSignals?.screenshotLike) {
            ...
            suggestions.push("Review whether the screenshot is the best match for the selected requirement.");
        }
        if (!step) {
            return;
        }
        if (step.acceptedKinds && !step.acceptedKinds.includes(itemKind)) {
            ...
            suggestions.push("Confirm whether the current file type is suitable for the mapped requirement.");
        }
        if (isCloseUpStep(step)) {
            if (itemKind !== "PHOTO" && itemKind !== "VIDEO") {
                ...
                suggestions.push("Use a photo or video file for close-up requirements when available.");
            }
            else {
                flags.push({
                    severity: "info",
                    title: "Close-up requirement mapped",
                    detail: "The item is mapped to a close-up requirement, but visual close-up quality requires human review or future AI vision review.",
                    affectedItemId: item.id,
                    affectedStepId: step.id,
                });
            }
    ...
    if (unmappedItems.length > 5) {
        warnings.push("There are more than five unmapped items, which may indicate an unorganized intake session.");
        suggestions.push("Review unmapped items and assign checklist steps for better intake clarity.");
    }
    const summary = warnings.length > 0 || flags.length > 0
...
function buildDeterministicItemReview(payload) {
    const warnings = [];
    const suggestions = [];
    const flags = [];
    ...
    if (expectedKinds.length > 0 && !expectedKinds.includes(itemKind)) {
        flags.push({
            ...
            detail: `Selected step ${payload.selectedStep.title} accepts ${expectedKinds.join(", ")} but the item is ${itemKind}.`,
            affectedItemId: payload.item.id,
            affectedStepId: payload.selectedStep.id,
        });
        suggestions.push("Confirm that this file is appropriate for the selected intake requirement.");
    }
    if (isCloseUpStep(payload.selectedStep)) {
        if (itemKind !== "PHOTO" && itemKind !== "VIDEO") {
            ...
            suggestions.push("Use a photo or video if the requirement specifically asks for a close-up capture.");
        }
        else {
            flags.push({
                severity: "info",
    ...
    if (payload.item.clientSignals?.genericMime) {
        ...
        suggestions.push("Use a more specific file type or review the file metadata for accuracy.");
    }
    if (payload.item.clientSignals?.duplicateStatus === "duplicate") {
        warnings.push("This item has a duplicate signal. A reviewer should confirm whether it is intended.");
        flags.push({
            severity: "warning",
...
export class AiCaptureService {
    provider;
    costGuard;
    constructor(provider, costGuard) {
        this.provider = provider;
        this.costGuard = costGuard;
    }
    async analyzeSession(userId, payload) {
        ...
        if (!guard.allowed) {
            return {
                ...deterministic,
                status: "blocked",
                summary: "AI capture review is blocked due to usage or budget limits.",
        ...
        const providerResult = await this.provider.run(AiTask.CAPTURE_SESSION_REVIEW, redactSessionPayloadForProvider(payload));
    ...
    async analyzeItem(userId, payload) {
        ...
        if (!guard.allowed) {
            return {
                ...deterministic,
                status: "blocked",
                summary: "AI item review is blocked due to usage or budget limits.",
                warnings: [...deterministic.warnings, guard.reason ?? "Usage limit reached."],
                suggestions: [...deterministic.suggestions],
                legalDisclaimer: AI_LEGAL_DISCLAIMER,
            };
        ...
        // for the privacy rationale (Issue #5: never send raw filenames to AI).
        const providerResult = await this.provider.run(AiTask.CAPTURE_ITEM_REVIEW, redactItemPayloadForProvider(payload));
        if (providerResult.status === "ok") {
            this.costGuard.recordCaptureAnalysis(userId, payload.collectionPlan.id);
            return compareFlags(providerResult, deterministic);
        }
