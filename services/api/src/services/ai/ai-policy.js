...
const LEGAL_DISCLAIMER = "AI assistance is advisory and does not determine factual truth, authorship, or legal admissibility.";
...
function containsUnsafeLanguage(value) {
    const normalized = normalizeText(value);
    return forbiddenPatterns.some((pattern) => pattern.test(normalized));
}
function isUnsafeResult(result) {
    if (containsUnsafeLanguage(result.summary))
        return true;
    if (containsUnsafeLanguage(result.legalDisclaimer))
        return true;
    for (const warning of result.warnings) {
        if (containsUnsafeLanguage(warning))
            return true;
    }
    for (const suggestion of result.suggestions) {
        if (containsUnsafeLanguage(suggestion))
            return true;
    }
    for (const flag of result.flags) {
        if (containsUnsafeLanguage(flag.title))
            return true;
        if (containsUnsafeLanguage(flag.detail))
            return true;
    }
    return false;
}
function normalizeResult(result) {
    ...
    const flags = Array.isArray(result.flags)
        ? result.flags
            ...
            affectedItemId: typeof flag?.affectedItemId === "string" && flag.affectedItemId.trim()
                ? flag.affectedItemId.trim()
                : undefined,
            affectedStepId: typeof flag?.affectedStepId === "string" && flag.affectedStepId.trim()
...
export function applyAiPolicy(result) {
    ...
    if (isUnsafeResult(safeResult)) {
        return {
            status: "blocked",
            summary: "AI guidance contained unsupported language and was blocked to protect review quality.",
            warnings: [
