 ...
 */
import { DISCUSSION_BODY_MAX_BYTES, isAllowedDiscussionThreadTransition, parseMentionTokens, sanitiseDiscussionBody, } from "@proovra/shared";
...
// Helpers
// -----------------------------------------------------------------------------
function emitAudit(client, evidenceId, actorUserId, eventType, metadata) {
...
async function loadEvidenceWorkspace(client, evidenceId, teamId) {
...
export async function createDiscussionThread(input, client = defaultPrisma) {
...
export async function getDiscussionThread(threadId, client = defaultPrisma) {
    return client.discussionThread.findUnique({ where: { id: threadId } });
}
export async function listDiscussionThreads(input, client = defaultPrisma) {
...
// Status transitions
// -----------------------------------------------------------------------------
async function applyStatusTransition(client, thread, to, patch) {
...
export async function resolveDiscussionThread(input, client = defaultPrisma) {
    const thread = await client.discussionThread.findUnique({
        where: { id: input.threadId },
    });
    if (!thread)
        throw new DiscussionError("thread_not_found");
...
export async function reopenDiscussionThread(input, client = defaultPrisma) {
    const reason = input.reason.trim();
    if (!reason)
        throw new DiscussionError("body_empty", { field: "reason" });
    const thread = await client.discussionThread.findUnique({
        where: { id: input.threadId },
    });
    if (!thread)
...
export async function assignDiscussionThread(input, client = defaultPrisma) {
    const thread = await client.discussionThread.findUnique({
        where: { id: input.threadId },
    });
    if (!thread)
        throw new DiscussionError("thread_not_found");
    if (thread.assignedToUserId === input.assignedToUserId)
        return thread;
    const updated = await applyStatusTransition(client, thread, thread.status, {
        assignedToUserId: input.assignedToUserId,
    ...
    await client.discussionParticipant.upsert({
        ...
        update: { role: "RESOLVER", revokedAtUtc: null },
    });
...
export async function escalateDiscussionThread(input, client = defaultPrisma) {
    const reason = input.reason.trim();
    if (!reason)
        throw new DiscussionError("body_empty", { field: "reason" });
    const thread = await client.discussionThread.findUnique({
...
export async function postMessage(input, client = defaultPrisma) {
    ...
    if (!body)
        throw new DiscussionError("body_empty");
    ...
    const message = await client.discussionMessage.create({
        data: {
            ...
            contributorIntakeSessionId: input.contributorIntakeSessionId ?? null,
    ...
    emitAudit(client, thread.evidenceId, input.authorUserId ?? null, input.authorKind === "CONTRIBUTOR"
    ...
    if (input.authorKind === "USER" &&
        ...
        input.authorUserId) {
        await resolveMentionsForMessage(message, body, input.authorUserId, client);
    }
    return message;
}
async function resolveMentionsForMessage(message, body, actorUserId, client) {
    ...
    for (const m of members) {
        ...
        if (m.user.displayName) {
            ...
            if (token)
                userByToken.set(token, m.user.id);
        }
        if (m.user.email) {
            const local = m.user.email.split("@")[0]?.toLowerCase() ?? "";
            if (local)
                userByToken.set(local, m.user.id);
        }
    }
    const out = [];
    for (const tok of tokens) {
        ...
        try {
            ...
            })).evidenceId, actorUserId, "MENTION_CREATED", {
                threadId: message.threadId,
                messageId: message.id,
                mentionedUserId: userId,
                token: tok,
...
export async function listMessagesForThread(threadId, client = defaultPrisma) {
...
export async function grantContributorAccess(input, client = defaultPrisma) {
    const thread = await client.discussionThread.findUnique({
        where: { id: input.threadId },
    });
    if (!thread)
    ...
    emitAudit(client, thread.evidenceId, input.actorUserId, "CONTRIBUTOR_ACCESS_GRANTED", {
        threadId: thread.id,
        intakeSessionId: input.intakeSessionId,
        contributorLabel: input.contributorLabel?.slice(0, 180) ?? null,
    });
...
export async function revokeContributorAccess(input, client = defaultPrisma) {
    const thread = await client.discussionThread.findUnique({
        where: { id: input.threadId },
    });
    if (!thread)
    ...
    emitAudit(client, thread.evidenceId, input.actorUserId, "CONTRIBUTOR_ACCESS_REVOKED", {
        threadId: thread.id,
        intakeSessionId: input.intakeSessionId,
    });
    return updated;
}
export async function countDiscussionThreads(input, client = defaultPrisma) {
    ...
    for (const r of rows) {
        if (r.status === "OPEN")
            out.open = r._count._all;
        else if (r.status === "IN_PROGRESS")
            out.inProgress = r._count._all;
        else if (r.status === "RESOLVED")
            out.resolved = r._count._all;
        else if (r.status === "CLOSED")
            out.closed = r._count._all;
    }
    if (input.meUserId) {
...
export function projectDiscussionThread(t) {
    return {
        id: t.id,
...
export function projectDiscussionMessage(m) {
    return {
        id: m.id,
        threadId: m.threadId,
...
export function projectDiscussionParticipant(p) {
    return {
        id: p.id,
        userId: p.userId,
 ...
 */
export async function canReadThread(thread, ctx, client = defaultPrisma) {
    if (ctx.kind === "user") {
        return ctx.isMember;
    }
    if (thread.visibility !== "CONTRIBUTOR_SCOPED")
