...
import { clipSafeSummary, isAllowedIncidentStatusTransition, isValidIncidentFingerprint, } from "@proovra/shared";
...
export async function recordIncident(input, client = defaultPrisma) {
    ...
    const sanitisedMetadata = input.metadata != null ? safeJsonSnapshot(input.metadata) : null;
    ...
    if (!existing) {
        ...
        const data = {
            ...
            status: existing.status === prismaPkg.IncidentStatus.RESOLVED ||
...
export async function acknowledgeIncident(input, client = defaultPrisma) {
    return transitionIncident(input, prismaPkg.IncidentStatus.ACKNOWLEDGED, "acknowledged", client);
}
export async function resolveIncident(input, client = defaultPrisma) {
    return transitionIncident(input, prismaPkg.IncidentStatus.RESOLVED, "resolved", client);
}
export async function suppressIncident(input, client = defaultPrisma) {
    return transitionIncident(input, prismaPkg.IncidentStatus.SUPPRESSED, "suppressed", client);
}
/**
 ...
 */
export async function assignIncident(input, client = defaultPrisma) {
    const existing = await client.operationalIncident.findFirst({
        where: { id: input.incidentId, teamId: input.teamId },
    });
    if (!existing)
        throw new IncidentError("incident_not_found");
...
async function transitionIncident(input, next, eventType, client) {
    const existing = await client.operationalIncident.findFirst({
        where: { id: input.incidentId, teamId: input.teamId },
    });
    if (!existing)
        throw new IncidentError("incident_not_found");
    if (!isAllowedIncidentStatusTransition(existing.status, next)) {
    ...
    try {
        await client.operationalIncidentEvent.create({
            data: {
                incidentId: updated.id,
                eventType,
                safeMessage: eventType === "resolved" && input.resolutionNote
                    ? clipSafeSummary(input.resolutionNote)
...
export async function listIncidents(input, client = defaultPrisma) {
...
export function projectIncident(i) {
    return {
        id: i.id,
        category: i.category,
        severity: i.severity,
...
// Gauges
// -----------------------------------------------------------------------------
async function refreshIncidentGauges(client) {
