 ...
 */
export async function resolveTenancyForWrite(input, client = defaultPrisma) {
    if (input.teamId) {
        ...
        if (input.expectedOrganizationId &&
 ...
 */
export async function getOrganizationIdForTeam(teamId, client = defaultPrisma) {
    const team = await client.team.findUnique({
        where: { id: teamId },
...
export async function resolveEvidenceTenancyForRead(input, client = defaultPrisma) {
...
export async function checkEvidenceTenancyInvariant(evidenceId, client = defaultPrisma) {
