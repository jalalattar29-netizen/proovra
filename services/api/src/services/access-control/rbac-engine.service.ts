...
export async function evaluateInheritedPermission(
  input: {
    teamId: string;
    userId: string;
...
function pickAllowSource(kind: string | null | undefined): RbacDecisionSource {
  switch (kind) {
...
function pickDenySource(reason: string | null | undefined): RbacDecisionSource {
  if (!reason) return "EXPLICIT_DENY";
  if (reason.includes("scope")) return "SERVICE_ACCOUNT";
  if (reason.includes("contributor")) return "CONTRIBUTOR";
...
export async function buildPermissionSnapshot(
  input: { teamId: string; userId: string },
  client: PrismaClient = defaultPrisma,
): Promise<RbacMemberSnapshot> {
  const member = await loadMemberAccessSnapshot(
    { teamId: input.teamId, userId: input.userId },
    client,
  );
  if (!member) throw new RbacEngineError("RBAC_MEMBER_NOT_FOUND");
  ...
  bump("rbac_permission_matrix_viewed_total" as never); // not in catalog; safe no-op
  ...
  return {
    ...
    userId: input.userId,
    canonicalRole,
    status: member.status as RbacMemberSnapshot["status"],
    permissions,
...
// -----------------------------------------------------------------------------
// Effective role matrix — admin overview of which role implies which
// permissions. Pure helper that materialises the catalog × role matrix.
// -----------------------------------------------------------------------------

export type EffectiveRoleMatrixRow = {
  role: CanonicalRole;
  permissions: ReadonlyArray<{ permission: Permission; allowed: boolean }>;
};

export function computeEffectiveRoleMatrix(): ReadonlyArray<EffectiveRoleMatrixRow> {
...
export async function grantTemporaryElevation(
  input: TemporaryElevationInput & { grantedByUserId: string; ttlSeconds: number },
  client: PrismaClient = defaultPrisma,
): Promise<{ grantId: string; expiresAtUtc: string }> {
  if (!PERMISSION_SET.has(input.permission)) {
    throw new RbacEngineError("RBAC_PERMISSION_UNKNOWN");
  }
...
export async function evaluateServiceAccountPermission(
  input: { apiCredentialId: string; permission: Permission },
  client: PrismaClient = defaultPrisma,
): Promise<RbacDecision> {
  bump("rbac_permission_eval_total");
  if (!PERMISSION_SET.has(input.permission)) {
