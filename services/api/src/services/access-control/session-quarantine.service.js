 ...
 */
import { SESSION_QUARANTINE_DEFAULT_HOURS, SESSION_QUARANTINE_MAX_HOURS, } from "@proovra/shared";
...
function projectQuarantine(row) {
    if (!row.quarantinedAtUtc)
        return null;
    return {
        sessionId: row.id,
        teamId: row.teamId,
...
export async function quarantineSession(input, client = defaultPrisma) {
    ...
    const hours = Math.min(Math.max(input.releaseHours ?? SESSION_QUARANTINE_DEFAULT_HOURS, 1), SESSION_QUARANTINE_MAX_HOURS);
    const now = new Date();
...
export async function releaseQuarantine(input, client = defaultPrisma) {
    const row = await client.authenticatedSession.findFirst({
        where: { id: input.sessionId, teamId: input.teamId },
    });
    if (!row || !row.quarantinedAtUtc)
        return false;
...
// -----------------------------------------------------------------------------
export async function sweepQuarantineReleases(input, client = defaultPrisma) {
    const now = new Date();
    ...
    if (due.length > 0)
        bump("quarantine_release_total", due.length);
    await refreshQuarantineGauge(input.teamId, client);
...
// -----------------------------------------------------------------------------
export async function isSessionQuarantined(input, client = defaultPrisma) {
    const row = await client.authenticatedSession.findFirst({
        where: { id: input.sessionId, teamId: input.teamId },
...
// Listing
// -----------------------------------------------------------------------------
export async function listQuarantinedSessions(input, client = defaultPrisma) {
...
// -----------------------------------------------------------------------------
export async function emergencyOrgRevoke(input, client = defaultPrisma) {
...
// -----------------------------------------------------------------------------
async function refreshQuarantineGauge(teamId, client) {
