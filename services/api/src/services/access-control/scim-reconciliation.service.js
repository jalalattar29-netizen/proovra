...
// Drift detection
// ---------------------------------------------------------------------------
export async function detectScimDrift(input, client = defaultPrisma) {
    ...
    const dupeKey = (m) => `${m.provider}:${m.externalSubjectId}`;
    const seen = new Map();
    ...
    items.sort((a, b) => a.category === b.category ? a.id.localeCompare(b.id) : a.category.localeCompare(b.category));
    ...
    const previewId = stableId("preview", input.teamId, String(generatedAt.getTime()));
    const report = {
        previewId,
        teamId: input.teamId,
...
export async function executeScimReconciliation(input, client = defaultPrisma) {
    purgeExpiredPreviews();
    ...
    if (cached.expiresAtUtc <= Date.now()) {
        return {
            ok: false,
            code: "stale_preview",
            message: "Preview is older than 5 minutes. Re-run the drift scan and try again.",
        };
    }
    if (input.itemIds.length === 0) {
        return {
            ...
            message: "No drift items selected. Pick the rows to reconcile before executing.",
        };
    }
    const selected = cached.report.items.filter((i) => input.itemIds.includes(i.id));
    const details = [];
    let appliedCount = 0;
    let skippedCount = 0;
...
export async function listScimSyncFailures(input, client = defaultPrisma) {
...
export async function replayScimSyncFailure(input, client = defaultPrisma) {
    ...
    if (row.eventType === "scim_invalid_token") {
        return {
            ...
            message: "Terminal failure: bad SCIM token. Issue a new token in the Tokens tab.",
        };
    }
...
function humaniseScimFailure(eventType) {
    switch (eventType) {
