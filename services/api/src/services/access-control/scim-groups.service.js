 ...
 */
import { SCIM_GROUP_MAPPED_ROLES, SCIM_GROUP_SCHEMA_URI, SCIM_LIST_RESPONSE_SCHEMA_URI, ScimGroupPatchOpSchema, ScimGroupSchema, } from "@proovra/shared";
import { prisma as defaultPrisma } from "../../db.js";
import { bump } from "../ops/metrics.service.js";
import { safeEmitSecurityEvent } from "../security/security-event.service.js";
import { appendPlatformAuditLog } from "../platform-audit-log.service.js";
async function loadGroupMembers(teamId, mappedRole, client) {
...
async function buildResource(ctx, row, client) {
    const members = await loadGroupMembers(ctx.teamId, row.mappedRole, client);
    return {
...
export async function scimCreateGroup(ctx, input, client = defaultPrisma) {
    ...
    if (parsed.data.members && parsed.data.members.length > 0) {
        await applyGroupMembership(ctx, row, parsed.data.members.map((m) => m.value), "add", client);
    }
...
// Read / list
// -----------------------------------------------------------------------------
export async function scimReadGroup(ctx, groupId, client = defaultPrisma) {
...
export async function scimListGroups(ctx, input, client = defaultPrisma) {
    const startIndex = Math.max(1, Math.floor(input.startIndex ?? 1));
    const count = Math.min(Math.max(Math.floor(input.count ?? 50), 1), 200);
    let displayEq;
    let externalIdEq;
    if (input.filter) {
        const m = /^(displayName|externalId)\s+eq\s+"([^"]+)"$/i.exec(input.filter.trim());
        if (m) {
            if (m[1].toLowerCase() === "displayname")
                displayEq = m[2];
            else
                externalIdEq = m[2];
        }
...
export async function scimPatchGroup(ctx, groupId, input, client = defaultPrisma) {
    ...
    for (const op of parsed.data.Operations) {
        ...
        if (op.op === "replace" &&
            ...
            SCIM_GROUP_MAPPED_ROLES.includes(op.value)) {
            await client.scimGroup.update({
                where: { id: row.id },
...
// -----------------------------------------------------------------------------
export async function scimDeleteGroup(ctx, groupId, client = defaultPrisma) {
    const row = await client.scimGroup.findFirst({
        where: { id: groupId, teamId: ctx.teamId },
    });
    if (!row)
...
// -----------------------------------------------------------------------------
function extractMemberUserIds(value) {
    if (!value)
        return [];
...
async function applyGroupMembership(ctx, group, userIds, op, client) {
    let changed = 0;
    for (const userId of userIds) {
        ...
        if (!member || member.status !== "ACTIVE")
            continue;
        const desiredRole = op === "add" ? group.mappedRole : "MEMBER";
