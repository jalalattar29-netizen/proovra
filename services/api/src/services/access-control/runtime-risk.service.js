 ...
 */
import { HIGH_RISK_INCIDENT_DEDUP_HOURS, HIGH_RISK_INCIDENT_DEFAULT_THRESHOLD, RUNTIME_RISK_RECOMPUTE_DEFAULT_MINUTES, isSessionDueForRiskRecompute, sessionRiskLevel, } from "@proovra/shared";
...
// Sweep
// -----------------------------------------------------------------------------
export async function runtimeRiskRecomputeSweep(input, client = defaultPrisma) {
    const window = input.recomputeWindowMinutes ?? RUNTIME_RISK_RECOMPUTE_DEFAULT_MINUTES;
    ...
    for (const c of candidates) {
        if (!isSessionDueForRiskRecompute({
        ...
        try {
            const result = await detectAndScoreSession({ teamId: input.teamId, sessionId: c.id }, client);
