...
export function readTwilioConfigFromEnv() {
    ...
    if (!accountSid)
        return { config: null, reason: "account_sid_missing" };
    if (!apiKey)
        return { config: null, reason: "api_key_missing" };
    if (!apiSecret)
        return { config: null, reason: "api_secret_missing" };
    const messagingServiceSid = (process.env.TWILIO_MESSAGING_SERVICE_SID ?? "").trim() || null;
    const verifyServiceSid = (process.env.TWILIO_VERIFY_SERVICE_SID ?? "").trim() || null;
    const smsFromNumber = (process.env.TWILIO_SMS_FROM_NUMBER ?? "").trim() || null;
    const whatsappNumber = (process.env.TWILIO_WHATSAPP_NUMBER ?? "").trim() || null;
...
export class TwilioMessagingProvider {
    ...
    async sendWhatsApp(input) {
        if (!this.cfg.whatsappNumber && !this.cfg.messagingServiceSid) {
            return this.failure("WHATSAPP", "provider_unsupported_channel", "whatsapp_unconfigured", "TWILIO_WHATSAPP_NUMBER not configured.");
        }
        return this.sendMessage(input, "WHATSAPP");
    }
    async sendMessage(input, channel) {
        const url = `${TWILIO_API_BASE}/Accounts/${encodeURIComponent(this.cfg.accountSid)}/Messages.json`;
        const params = new URLSearchParams();
        const to = channel === "WHATSAPP" ? `whatsapp:${input.toE164}` : input.toE164;
        params.set("To", to);
        ...
        if (this.cfg.messagingServiceSid) {
            ...
            return this.failure(channel, "provider_unconfigured", "no_send_target", "No MessagingServiceSid or from-number configured.");
        }
        ...
        if (!result.ok) {
            return this.failure(channel, mapTransportFailure(result), result.errorCode, result.errorMessage);
        }
        const body = result.json;
        const sid = body && typeof body["sid"] === "string" ? body["sid"] : null;
        const status = body && typeof body["status"] === "string"
            ? body["status"]
            : "queued";
        if (!sid) {
            return this.failure(channel, "provider_internal_error", "missing_sid", "Twilio did not return a message SID.");
        }
        return {
            ok: true,
    ...
    verifyWebhookSignature(input) {
        ...
        if (this.cfg.apiSecret)
            keys.push(this.cfg.apiSecret);
        if (this.cfg.authToken)
            keys.push(this.cfg.authToken);
    ...
    parseDeliveryWebhook(input) {
        ...
        if (fields.Body || fields.From) {
            ...
            const channel = from.startsWith("whatsapp:")
                ? "WHATSAPP"
                : "SMS";
            const event = {
                provider: "TWILIO",
    ...
    failure(channel, reason, errorCode, errorMessage) {
        return {
            ok: false,
            provider: "TWILIO",
            channel,
...
function mapTransportFailure(result) {
    ...
    if (code === "21617" || code === "21618")
        return "provider_rejected_body";
    return "provider_internal_error";
}
function mapTransportFailureForVerify(result) {
...
function mapTwilioStatusToCommunication(status) {
    switch (status) {
        case "queued":
        case "accepted":
        case "scheduled":
