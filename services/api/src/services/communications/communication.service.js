...
import { classifyCommunicationProviderError, communicationRetryDelaySeconds, isCriticalCommunicationPurpose, isPhoneChannel, isRetryEligibleCommunicationPurpose, maskPhonePreview, normaliseToE164, COMMUNICATION_RETRY_MAX_ATTEMPTS, } from "@proovra/shared";
...
function readRateLimits() {
    const perRecipient = numberFromEnv(process.env.COMMUNICATIONS_MAX_SENDS_PER_RECIPIENT_PER_DAY, DEFAULT_PER_RECIPIENT_PER_DAY, 1, 10_000);
    const perTeam = numberFromEnv(process.env.COMMUNICATIONS_MAX_SENDS_PER_TEAM_PER_DAY, DEFAULT_PER_TEAM_PER_DAY, 1, 1_000_000);
    const retryBase = numberFromEnv(process.env.COMMUNICATIONS_RETRY_BASE_SECONDS, DEFAULT_RETRY_BASE_SECONDS, 10, 24 * 3600);
    return {
...
function numberFromEnv(raw, fallback, min, max) {
    if (!raw)
        return fallback;
...
export function hashRecipientPhone(phoneE164) {
...
// -----------------------------------------------------------------------------
export async function enqueueOutboundMessage(input, client = defaultPrisma) {
    ...
    if (!input.forceCritical &&
        ...
        (await isRecipientOptedOut({ teamId: input.teamId, recipientHash, channel: input.channel }, client))) {
        const cancelled = await client.communicationMessage.create({
            data: {
                teamId: input.teamId,
                channel: input.channel,
    ...
    if (recipientCount >= limits.perRecipientPerDay ||
        ...
        const cancelled = await client.communicationMessage.create({
            data: {
                ...
                errorMessage: recipientCount >= limits.perRecipientPerDay
        ...
        safeEmitSecurityEvent({
            ...
            details: {
                ...
                channel: input.channel,
                recipientHash,
                scope: recipientCount >= limits.perRecipientPerDay ? "recipient" : "team",
            },
        }, client);
...
// -----------------------------------------------------------------------------
export async function dispatchPendingMessage(messageId, client = defaultPrisma) {
    const provider = getMessagingProvider();
    const row = await client.communicationMessage.findUnique({
        where: { id: messageId },
    });
    if (!row)
        throw new Error("communication_message_not_found");
...
async function resolvePhoneFromSource(row, client) {
    ...
    if (row.relatedIntakeLinkId) {
        ...
        if (link.revokedAtUtc)
            return null;
        if (link.expiresAtUtc && link.expiresAtUtc.getTime() <= Date.now())
            return null;
...
// -----------------------------------------------------------------------------
async function updateAfterProviderAttempt(row, result, _provider, client) {
    if (result.ok) {
        const updated = await client.communicationMessage.update({
            where: { id: row.id },
            data: {
    ...
    const classification = classifyCommunicationProviderError(result.errorCode, result.errorMessage);
    const purpose = row.purpose;
    const allowRetry = classification === "transient" &&
...
// Opt-out check
// -----------------------------------------------------------------------------
async function isRecipientOptedOut(input, client) {
    ...
    if (input.channel === "SMS")
        return pref.smsOptOut;
    if (input.channel === "WHATSAPP")
        return pref.whatsappOptOut;
    return false;
}
// -----------------------------------------------------------------------------
// Body preview safety
...
export function projectCommunicationMessage(row) {
    return {
        id: row.id,
        channel: row.channel,
