...
import { normaliseToE164, maskPhonePreview, } from "@proovra/shared";
...
export async function startVerification(input, client = defaultPrisma) {
    const provider = getMessagingProvider();
    const phone = normaliseToE164(input.phoneE164OrRaw);
    if (!phone)
        throw new VerificationError("invalid_phone");
...
export async function checkVerification(input, client = defaultPrisma) {
    const provider = getMessagingProvider();
    const phone = normaliseToE164(input.phoneE164OrRaw);
    if (!phone)
        throw new VerificationError("invalid_phone");
...
// Read projection
// -----------------------------------------------------------------------------
export function projectVerificationAttempt(row) {
    return {
        id: row.id,
        channel: row.channel,
        status: row.status,
