...
function normalizeTsaFailureReason(error) {
...
function parseOpenSslReplyText(text) {
...
export async function createEvidenceTimestamp(params) {
    if (!enabled())
        return null;
    return withProovraSpan(PROOVRA_SPAN_NAMES.TSA_TIMESTAMP_REQUEST, {
...
async function createEvidenceTimestampInner(params) {
    ...
    try {
        ...
        const { stdout } = await execFileAsync("openssl", ["ts", "-reply", "-in", responseFile, "-text"], { timeout: timeoutMs() });
        ...
        const granted = await withProovraSpan(PROOVRA_SPAN_NAMES.TSA_TIMESTAMP_VERIFY, {
        ...
        async () => {
            ...
            await withProovraSpan(PROOVRA_SPAN_NAMES.INTEGRITY_TIMESTAMP_VERIFY, {
