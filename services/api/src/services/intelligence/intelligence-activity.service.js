 ...
 */
import { INTELLIGENCE_LIFECYCLE_CODES, classifyLifecycleCategory, } from "@proovra/shared";
...
export async function emitLifecycleEvent(input) {
    if (!INTELLIGENCE_LIFECYCLE_CODES.includes(input.code)) {
        return { ok: false };
    }
    const prisma = input.prisma ?? defaultPrisma;
    const category = classifyLifecycleCategory(input.code);
    try {
