...
export function setEmbeddingProvider(p) {
...
function chunkText(text, chunkBytes = DEFAULT_CHUNK_BYTES) {
    const chunks = [];
 ...
 */
export async function indexEvidenceText(input, client = defaultPrisma) {
...
export async function searchSemantic(input, client = defaultPrisma) {
    if (!isSemanticSearchEnabled() || !activeProvider) {
...
export async function listChunksForEvidence(evidenceId, client = defaultPrisma) {
