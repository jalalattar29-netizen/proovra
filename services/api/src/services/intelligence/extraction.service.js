 ...
 */
import { AI_ADVISORY_DISCLAIMER, } from "@proovra/shared";
...
let activeOcrProvider = noopProvider;
let activeTranscriptProvider = noopProvider;
export function setOcrProvider(p) {
...
export function setTranscriptProvider(p) {
...
export function configuredTranscriptProviderName() {
    return (process.env.TRANSCRIPT_PROVIDER?.trim() ||
...
// Job lifecycle
// -----------------------------------------------------------------------------
export async function enqueueIntelligenceJob(input, client = defaultPrisma) {
    try {
 ...
 */
export async function runExtractionInline(input, client = defaultPrisma) {
    const provider = input.jobKind === "OCR" ? activeOcrProvider : activeTranscriptProvider;
    ...
    try {
        ...
        const msg = err instanceof Error ? err.message.slice(0, 200) : "extraction_failed";
    ...
    const truncated = typeof result.text === "string" && result.text.length > MAX_TEXT_BYTES
        ? result.text.slice(0, MAX_TEXT_BYTES)
        : result.text;
    const wordCount = truncated && truncated.length > 0
    ...
    if (input.teamId) {
        try {
            const reason = input.jobKind === "OCR" ? "ocr_completed" : "transcript_completed";
    ...
    if (truncated && truncated.length > 0) {
        try {
            const { extractAndPersistEntities } = await import("./entity-extraction.service.js");
            const source = input.jobKind === "OCR" ? "OCR" : "TRANSCRIPT";
...
// -----------------------------------------------------------------------------
export async function listExtractedTexts(evidenceId, client = defaultPrisma) {
    return client.evidenceExtractedText.findMany({
        where: { evidenceId },
        orderBy: { createdAt: "desc" },
    });
}
export async function listIntelligenceJobs(input, client = defaultPrisma) {
...
// -----------------------------------------------------------------------------
export function projectExtractedTextSummary(row) {
    return {
        id: row.id,
...
export function projectExtractedTextDetail(row) {
    return {
        id: row.id,
        evidenceId: row.evidenceId,
        kind: row.kind,
...
export function projectIntelligenceJob(row) {
    return {
        id: row.id,
        evidenceId: row.evidenceId,
        teamId: row.teamId,
