...
export async function projectProviderQuality(input: {
...
}): Promise<ProviderQualityProjection> {
  ...
  const usageRows = await prisma.providerUsageEvent
    ...
    .catch(() => [] as Array<{
      provider: string;
      _count: { _all: number };
      _sum: { estimatedCostUsdMicros: bigint | null };
    }>);
  ...
  const failureRows = await prisma.providerUsageEvent
    ...
    .catch(() => [] as Array<{ provider: string; _count: { _all: number } }>);
  ...
  const recordRows = await prisma.mediaIntelligenceRecord
    ...
    .catch(() => [] as Array<{
      provider: string;
      _count: { _all: number };
      _avg: { providerConfidence: number | null };
    }>);
  ...
  const correctionRowsRaw = await prisma.reviewerCorrection
    ...
    .catch(() => [] as Array<{
      ...
      record: { provider: string; state: string };
    }>);
  ...
  for (const c of correctionRowsRaw) {
    ...
    b.correctionCount += 1;
    if (c.state === "ACCEPTED") b.acceptanceCount += 1;
    if (c.state === "REVERTED" || c.revertedAt) b.revertCount += 1;
  }
  ...
  for (const r of acceptedRecordRows) {
    ...
    if (r.state === "ACCEPTED") b.acceptedRecords += r._count._all;
    if (r.state === "REJECTED") {
  ...
  for (const r of usageRows) allProviders.add(r.provider);
  for (const r of recordRows) allProviders.add(r.provider);
  for (const r of failureRows) allProviders.add(r.provider);
  for (const p of perProvider.keys()) allProviders.add(p);

  const usageByProvider = new Map(usageRows.map((r: (typeof usageRows)[number]) => [r.provider, r]));
  const failureByProvider = new Map(failureRows.map((r: (typeof failureRows)[number]) => [r.provider, r]));
  const recordByProvider = new Map(recordRows.map((r: (typeof recordRows)[number]) => [r.provider, r]));
  ...
  for (const p of allProviders) {
    ...
    const correctionRatePct =
      recordCount === 0 ? 0 : round1((b.correctionCount / recordCount) * 100);
    const acceptanceRatePct =
      b.correctionCount === 0
        ? 0
        : round1((b.acceptanceCount / b.correctionCount) * 100);
    const rejectionRatePct =
      recordCount === 0 ? 0 : round1((b.rejectedRecords / recordCount) * 100);
    const revertRatePct =
      b.correctionCount === 0
        ? 0
    ...
    rows.push({
      provider: p as MediaIntelligenceProvider,
      callCount,
      failureCount,
      failureRatePct,
      recordCount,
...
export async function projectReviewerQuality(input: {
  ...
  teamId: string;
  range: ExecutiveMetricsRange;
}): Promise<ReviewerQualityProjection> {
  const prisma = input.prisma ?? defaultPrisma;
  const since = new Date(Date.now() - rangeWindowMs(input.range));
  ...
  const perReviewer = new Map<string, Bucket>();
  for (const c of corrections) {
    ...
    b.authored += 1;
    if (c.state === "ACCEPTED") b.accepted += 1;
    if (c.state === "REVERTED" || c.revertedAt) b.reverted += 1;
    if (c.supersededByCorrectionId) b.superseded += 1;
    if (c.acceptedAt) {
      b.acceptLatenciesMs.push(c.acceptedAt.getTime() - c.createdAt.getTime());
    }
  }
  const rows: ReviewerQualityRow[] = [];
  for (const [reviewerId, b] of perReviewer) {
    ...
    const supersededRatePct =
      b.authored === 0 ? 0 : (b.superseded / b.authored) * 100;
...
export async function projectTeamQuality(input: {
  ...
  for (const r of records) {
    ...
    if (r.state === "ACCEPTED" || r.state === "CORRECTED") accepted += 1;
  ...
  const correctionDensity =
    recordCount === 0 ? 0 : round1((correctionCount / recordCount) * 100);
  const averageProviderConfidence = confCount === 0 ? 0 : round2(confSum / confCount);
  const reviewQualityScore = round1(
    Math.max(0, 100 - correctionDensity * 2 - (rejected * 100) / Math.max(recordCount, 1)),
  );
  const teamRow: TeamQualityRow = {
    scope: "TEAM",
  ...
  const caseAggregates = await prisma.providerUsageEvent
    ...
    .catch(() => [] as Array<{ caseId: string | null; _count: { _all: number } }>);

  const caseRows: TeamQualityRow[] = [];
  for (const c of caseAggregates) {
    ...
    for (const r of caseRecords) {
      ...
      if (r.state === "ACCEPTED" || r.state === "CORRECTED") caseAccepted += 1;
      if (r.state === "REJECTED") caseRejected += 1;
      if (r.providerConfidence !== null) {
...
async function evidenceIdsForCase(
  prisma: PrismaClient,
  teamId: string,
  caseId: string,
  sinceUtc: Date,
  ...
  const rows = await prisma.providerUsageEvent
    ...
    .catch(() => [] as Array<{ evidenceId: string | null }>);
  return rows.map((r: (typeof rows)[number]) => r.evidenceId).filter((x: string | null): x is string => x !== null);
}
