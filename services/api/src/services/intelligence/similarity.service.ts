...
import type {
  EvidenceSimilarity as DbSimilarity,
  EvidenceSimilarityKind as DbSimilarityKind,
  PrismaClient,
} from "@prisma/client";
import {
  filenameTokens,
    ...
    return await client.evidenceSimilarity.upsert({
      where: {
        sourceEvidenceId_targetEvidenceId_kind: {
          sourceEvidenceId: a,
          targetEvidenceId: b,
          kind: input.kind as DbSimilarityKind,
        },
      },
      create: {
        teamId: input.teamId,
...
export async function detectHashDuplicatesForEvidence(
  evidenceId: string,
...
export async function detectFilenameSimilarForEvidence(
  evidenceId: string,
  client: PrismaClient = defaultPrisma,
): Promise<DbSimilarity[]> {
  const me = await client.evidence.findUnique({
...
export async function detectTextSimilarForEvidence(
  input: { evidenceId: string; kind: "OCR_SIMILAR" | "TRANSCRIPT_SIMILAR" },
  client: PrismaClient = defaultPrisma,
): Promise<DbSimilarity[]> {
  const sourceTextKind =
    input.kind === "OCR_SIMILAR" ? ["OCR_PDF", "OCR_IMAGE"] : ["TRANSCRIPT_AUDIO", "TRANSCRIPT_VIDEO"];
  const me = await client.evidence.findUnique({
  ...
  const myText = myTexts
    ...
    .filter(Boolean)
    .join("\n");
  if (myText.trim().length < 64) return [];
...
export async function reconcileSimilaritiesForEvidence(
  evidenceId: string,
  client: PrismaClient = defaultPrisma,
): Promise<{ hash: number; filename: number; ocr: number; transcript: number }> {
  const [hash, filename, ocr, transcript] = await Promise.all([
    ...
    detectTextSimilarForEvidence(
      { evidenceId, kind: "TRANSCRIPT_SIMILAR" },
      client,
    ),
  ]);
  return {
