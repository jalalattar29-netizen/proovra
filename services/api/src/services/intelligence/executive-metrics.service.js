 ...
 */
import { EXECUTIVE_METRICS_LIMITATIONS, EXECUTIVE_METRICS_SCHEMA_VERSION, EXECUTIVE_TRENDS_SCHEMA_VERSION, buildTrendMetric, rangeWindowMs, } from "@proovra/shared";
...
export async function projectExecutiveTrends(input) {
    ...
    const t = (c, p) => buildTrendMetric(c, p);
    return {
        ...
        ai: {
            ...
            averageProviderConfidence: t(current.averageProviderConfidence, previous.averageProviderConfidence),
        },
        sla: {
            jobFailureRatePct: t(current.jobFailureRatePct, previous.jobFailureRatePct),
            providerAvailabilityPct: t(current.providerAvailabilityPct, previous.providerAvailabilityPct),
        },
        cost: {
...
async function aggregateWindow(input) {
    ...
    const evidenceTotal = await safeCount(() => prisma.evidence.count({ where: { teamId } }));
    ...
    const captures = await safeCount(() => prisma.evidence.count({
        where: { teamId, createdAt: within },
    }));
    const capturesWithSignature = await safeCount(() => prisma.evidence.count({
        where: { teamId, createdAt: within, fileSha256: { not: null } },
    }));
    const captureSuccessRatePct = captures === 0
        ? 100
    ...
    // Review.
    const reviewed = await safeCount(() => prisma.evidenceReviewWorkflow.count({
        where: { teamId, completedAt: within },
    }));
    ...
    try {
        verifications = await prisma.evidence.count({
            where: { teamId, lastVerifiedAtUtc: within },
        });
        publicVerifyViews = await prisma.evidence.count({
            where: { teamId, lastPublicVerifyViewAtUtc: within },
        });
    }
    catch {
        /* swallow */
