...
export async function searchEvidence(input, client = defaultPrisma) {
    ...
    for (const t of titleHits) {
        const isFilenameMatch = !!t.displayFileName?.toLowerCase().includes(q.toLowerCase()) ||
    ...
    for (const h of textHits)
        if (!byId.has(h.evidenceId))
            extraIds.add(h.evidenceId);
    ...
    for (const t of textHits) {
        ...
        if (!hit)
            continue;
        if (t.kind.startsWith("TRANSCRIPT"))
            hit.matches.transcript = true;
        else
            hit.matches.ocr = true;
    }
    for (const e of entityHits) {
        const hit = byId.get(e.evidenceId);
        if (hit)
            hit.matches.entity = true;
    }
