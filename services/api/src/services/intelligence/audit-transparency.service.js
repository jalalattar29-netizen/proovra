 ...
 */
import { AUDIT_TRANSPARENCY_CATEGORIES, classifyLifecycleCategory, } from "@proovra/shared";
import { prisma as defaultPrisma } from "../../db.js";
export async function listAuditTransparency(input) {
...
function classifyRedactionCategory(code) {
    if (code.startsWith("DETECTION"))
        return "DETECTION";
    if (code.startsWith("APPROVAL") || code.startsWith("VERSION_APPROVED"))
        return "APPROVAL";
    if (code.startsWith("DERIVATIVE"))
        return "REDACTION";
    if (code.startsWith("REVIEW"))
        return "REVIEW";
    return "REDACTION";
}
function mapLifecycleCategoryToAudit(c) {
    switch (c) {
...
// Compile-time guard.
function _assertCategoriesBound() {
    const _x = "POLICY";
    void _x;
    void AUDIT_TRANSPARENCY_CATEGORIES;
    const _y = "RECORD_INGESTED";
    void _y;
