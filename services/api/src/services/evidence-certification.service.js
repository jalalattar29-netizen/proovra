...
import { CertificationStatus as PrismaCertificationStatus } from "@prisma/client";
...
function serializeCertification(item) {
    return {
        id: item.id,
...
export async function getLatestEvidenceCertification(evidenceId, declarationType) {
    return prisma.evidenceCertification.findFirst({
        where: { evidenceId, declarationType },
        orderBy: [{ version: "desc" }, { updatedAt: "desc" }],
    });
}
export async function listEvidenceCertifications(evidenceId) {
...
export async function requestEvidenceCertification(params) {
    const latest = await getLatestEvidenceCertification(params.evidenceId, params.declarationType);
    if (latest &&
        (latest.status === PrismaCertificationStatus.REQUESTED ||
            latest.status === PrismaCertificationStatus.ATTESTED)) {
        const error = new Error("A certification request already exists for this declaration type");
...
export async function attestEvidenceCertification(params) {
    ...
    if (latest.status === PrismaCertificationStatus.REVOKED) {
        const error = new Error("Certification record has been revoked");
        error.statusCode = 409;
        throw error;
    }
    ...
    const attestorOrganization = normalizeOptionalText(params.attestorOrganization, 180);
    ...
    const statementSnapshot = params.statementSnapshot === undefined
        ? undefined
        : params.statementSnapshot === null
            ? prismaPkg.Prisma.JsonNull
            : params.statementSnapshot;
...
export async function revokeEvidenceCertification(params) {
    ...
    if (latest.status === PrismaCertificationStatus.REVOKED) {
        const error = new Error("Certification record already revoked");
        error.statusCode = 409;
        throw error;
    }
