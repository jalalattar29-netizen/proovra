 ...
 * just log the result.
 */
export async function reconcileTeamGraph(teamId, client = defaultPrisma) {
    ...
    try {
        ...
        for (const ev of evidence) {
            const label = (ev.displayFileName ??
                ev.originalFileName ??
                ev.type ??
                "Evidence record")
                .toString()
            ...
            const upserted = await upsertNode(client, teamId, "EVIDENCE", ev.id, label, visibility);
            if (upserted)
                nodesUpserted += 1;
        }
        ...
        try {
            const cases = (await client.$queryRawUnsafe(`SELECT "id", "name"
           FROM "cases"
           WHERE "team_id" = $1
           ORDER BY "created_at" ASC`, teamId));
            ...
            for (const c of cases) {
                ...
                const upserted = await upsertNode(client, teamId, "CASE", c.id, label, "WORKSPACE_INTERNAL");
                if (upserted)
                    nodesUpserted += 1;
            }
            ...
            // a no-op.
            try {
                await client.$executeRawUnsafe(`UPDATE "investigation_graph_nodes" n
            ...
            const evidenceWithCase = (await client.$queryRawUnsafe(`SELECT "id", "case_id"
           FROM "evidence"
           WHERE "team_id" = $1
             AND "case_id" IS NOT NULL
             AND "deleted_at" IS NULL`, teamId));
            for (const ev of evidenceWithCase) {
                ...
                if (!evidenceNodeId || !caseNodeId)
                    continue;
                const created = await upsertEdge(client, teamId, evidenceNodeId, caseNodeId, "BELONGS_TO_CASE", "SYSTEM", "HIGH", "Evidence record is assigned to this case in the workspace.");
                if (created)
                    edgesUpserted += 1;
            }
        }
        ...
        try {
            const reports = (await client.$queryRawUnsafe(`SELECT r."id", r."evidence_id", r."version", r."generated_at_utc"
           FROM "reports" r
           JOIN "evidence" e ON e."id" = r."evidence_id"
           WHERE e."team_id" = $1
             AND e."deleted_at" IS NULL
           ORDER BY r."generated_at_utc" ASC`, teamId));
            const seenReportExternalIds = new Set();
            for (const r of reports) {
                ...
                const upserted = await upsertNode(client, teamId, "REPORT", externalId, label, "WORKSPACE_INTERNAL");
                if (upserted)
                    nodesUpserted += 1;
                // GENERATED_REPORT edge: Evidence → Report.
                const evidenceNodeId = await findNodeId(client, teamId, "EVIDENCE", r.evidence_id);
                const reportNodeId = await findNodeId(client, teamId, "REPORT", externalId);
                if (evidenceNodeId && reportNodeId) {
                    const created = await upsertEdge(client, teamId, evidenceNodeId, reportNodeId, "GENERATED_REPORT", "SYSTEM", "HIGH", `Evidence record generated this report (version ${r.version}).`);
                    if (created)
                        edgesUpserted += 1;
                }
            }
        ...
        try {
            const packages = (await client.$queryRawUnsafe(`SELECT vp."id", vp."evidence_id", vp."version", vp."generated_at_utc"
           FROM "verification_packages" vp
           JOIN "evidence" e ON e."id" = vp."evidence_id"
           WHERE e."team_id" = $1
             AND e."deleted_at" IS NULL
           ORDER BY vp."generated_at_utc" ASC`, teamId));
            for (const p of packages) {
                const externalId = `${p.evidence_id}:v${p.version}`;
                const label = `Verification package v${p.version}`.slice(0, 240);
                const upserted = await upsertNode(client, teamId, "VERIFICATION_PACKAGE", externalId, label, "WORKSPACE_INTERNAL");
                if (upserted)
                    nodesUpserted += 1;
                const evidenceNodeId = await findNodeId(client, teamId, "EVIDENCE", p.evidence_id);
                const pkgNodeId = await findNodeId(client, teamId, "VERIFICATION_PACKAGE", externalId);
                if (evidenceNodeId && pkgNodeId) {
                    const created = await upsertEdge(client, teamId, evidenceNodeId, pkgNodeId, "GENERATED_PACKAGE", "SYSTEM", "HIGH", `Evidence record generated this verification package (version ${p.version}).`);
                    if (created)
                        edgesUpserted += 1;
                }
            }
        ...
        try {
            const exports = (await client.$queryRawUnsafe(`SELECT "id", "evidence_id", "snapshot_kind"::text AS "snapshot_kind",
           ...
           ORDER BY "created_at" ASC`, teamId));
            for (const x of exports) {
                ...
                const upserted = await upsertNode(client, teamId, "EXPORT", x.id, label, "WORKSPACE_INTERNAL");
                if (upserted)
                    nodesUpserted += 1;
                // EXPORTED_AS edge: only when the export is scoped to an
                // evidence row (some exports are workspace-wide).
                if (x.evidence_id) {
                    const evidenceNodeId = await findNodeId(client, teamId, "EVIDENCE", x.evidence_id);
                    const exportNodeId = await findNodeId(client, teamId, "EXPORT", x.id);
                    if (evidenceNodeId && exportNodeId) {
                        const created = await upsertEdge(client, teamId, evidenceNodeId, exportNodeId, "EXPORTED_AS", "SYSTEM", "HIGH", `Evidence record was exported (${x.snapshot_kind}).`);
                        if (created)
                            edgesUpserted += 1;
                    }
                }
        ...
        try {
            const workflows = (await client.$queryRawUnsafe(`SELECT w."id", w."evidence_id", w."status"::text AS "status",
                w."priority"::text AS "priority"
           FROM "evidence_review_workflows" w
           JOIN "evidence" e ON e."id" = w."evidence_id"
           WHERE w."team_id" = $1
             AND e."deleted_at" IS NULL`, teamId));
            for (const w of workflows) {
                const label = `Review task — ${w.status} (${w.priority})`.slice(0, 240);
                const upserted = await upsertNode(client, teamId, "REVIEW_TASK", w.id, label, "WORKSPACE_INTERNAL");
                if (upserted)
                    nodesUpserted += 1;
                const evidenceNodeId = await findNodeId(client, teamId, "EVIDENCE", w.evidence_id);
                const taskNodeId = await findNodeId(client, teamId, "REVIEW_TASK", w.id);
                if (evidenceNodeId && taskNodeId) {
                    const created = await upsertEdge(client, teamId, evidenceNodeId, taskNodeId, "REVIEWED_BY", "SYSTEM", "HIGH", `Evidence record has a workspace review task (${w.status}).`);
                    if (created)
                        edgesUpserted += 1;
                }
            }
        ...
        try {
            const escalations = (await client.$queryRawUnsafe(`SELECT "id", "workflow_id", "evidence_id", "reason",
                "severity", "status"
           ...
           WHERE "team_id" = $1`, teamId));
            for (const x of escalations) {
                ...
                const upserted = await upsertNode(client, teamId, "ESCALATION", x.id, label, "WORKSPACE_INTERNAL");
                if (upserted)
                    nodesUpserted += 1;
                const reviewTaskNodeId = await findNodeId(client, teamId, "REVIEW_TASK", x.workflow_id);
                const escalationNodeId = await findNodeId(client, teamId, "ESCALATION", x.id);
                if (reviewTaskNodeId && escalationNodeId) {
                    const created = await upsertEdge(client, teamId, reviewTaskNodeId, escalationNodeId, "ESCALATED_FROM", "SYSTEM", severityToConfidence(x.severity), `Review task escalated (${x.reason}).`);
                    if (created)
                        edgesUpserted += 1;
                }
            }
        ...
        try {
            const incidents = (await client.$queryRawUnsafe(`SELECT "id", "related_evidence_id",
           ...
           WHERE "team_id" = $1`, teamId));
            for (const inc of incidents) {
                ...
                const upserted = await upsertNode(client, teamId, "INCIDENT", inc.id, label, "WORKSPACE_INTERNAL");
                if (upserted)
                    nodesUpserted += 1;
                if (inc.related_evidence_id) {
                    const evidenceNodeId = await findNodeId(client, teamId, "EVIDENCE", inc.related_evidence_id);
                    const incidentNodeId = await findNodeId(client, teamId, "INCIDENT", inc.id);
                    if (evidenceNodeId && incidentNodeId) {
                        const created = await upsertEdge(client, teamId, evidenceNodeId, incidentNodeId, "REFERENCES_SAME_INCIDENT", "SYSTEM", severityToConfidence(inc.severity), `Evidence record correlated with an operational incident (${inc.category}).`);
                        if (created)
                            edgesUpserted += 1;
                    }
                }
        ...
        try {
            const grants = (await client.$queryRawUnsafe(`SELECT "id", "scope_kind", "state",
           ...
           WHERE "team_id" = $1`, teamId));
            for (const g of grants) {
                ...
                const upserted = await upsertNode(client, teamId, "EXTERNAL_REVIEW", g.id, label, "WORKSPACE_INTERNAL");
                if (upserted)
                    nodesUpserted += 1;
                ...
                const reviewNodeId = await findNodeId(client, teamId, "EXTERNAL_REVIEW", g.id);
                if (reviewNodeId) {
                    let scopeNodeId = null;
                    if (g.scope_kind === "EVIDENCE" && g.evidence_id) {
                        scopeNodeId = await findNodeId(client, teamId, "EVIDENCE", g.evidence_id);
                    }
                    else if (g.scope_kind === "CASE" && g.case_id) {
                        scopeNodeId = await findNodeId(client, teamId, "CASE", g.case_id);
                    }
                    else if (g.scope_kind === "PACKAGE" && g.package_id) {
                        scopeNodeId = await findNodeId(client, teamId, "VERIFICATION_PACKAGE", g.package_id);
                    }
                    if (scopeNodeId) {
                        const created = await upsertEdge(client, teamId, scopeNodeId, reviewNodeId, "REVIEWED_BY", "SYSTEM", "HIGH", `Scoped to an external reviewer grant (${g.scope_kind}/${g.state}).`);
                        if (created)
                            edgesUpserted += 1;
                    }
                }
        ...
        const signals = (await client.$queryRawUnsafe(`SELECT "id", "evidence_id", "signal_type", "safe_summary", "severity"
         FROM "media_intelligence_signals"
         WHERE "team_id" = $1
           AND "status" IN ('PENDING', 'ACKNOWLEDGED')
         ORDER BY "created_at_utc" ASC`, teamId));
        for (const sig of signals) {
            ...
            const nodeKind = isOcr
                ? "OCR"
                : isTranscript
                    ? "TRANSCRIPT"
                    : "MEDIA_SIGNAL";
            const upserted = await upsertNode(client, teamId, nodeKind, sig.id, sig.safe_summary.slice(0, 240), "WORKSPACE_INTERNAL");
            if (upserted)
                nodesUpserted += 1;
            // Edge from evidence → signal.
            const evidenceNodeId = await findNodeId(client, teamId, "EVIDENCE", sig.evidence_id);
            if (evidenceNodeId) {
                const signalNodeId = await findNodeId(client, teamId, nodeKind, sig.id);
                if (signalNodeId) {
                    const edgeType = isOcr
                        ? "HAS_OCR"
                        : isTranscript
                            ? "HAS_TRANSCRIPT"
                            : "HAS_MEDIA_SIGNAL";
                    const created = await upsertEdge(client, teamId, evidenceNodeId, signalNodeId, edgeType, "SYSTEM", confidenceForSeverity(sig.severity), sig.safe_summary.slice(0, 240));
                    if (created)
                        edgesUpserted += 1;
                }
            }
        ...
        const dupePairs = (await client.$queryRawUnsafe(`SELECT DISTINCT epa."evidence_id" AS "a_evidence_id",
        ...
        for (const pair of dupePairs) {
            ...
            if (aNode && bNode) {
                const created = await upsertEdge(client, teamId, aNode, bNode, "SAME_HASH_AS", "SYSTEM", "HIGH", "Exact byte-identical material observed in both records.");
                if (created)
                    edgesUpserted += 1;
            }
        }
 ...
 * the query.
 */
export async function buildEvidenceSubgraph(input, client = defaultPrisma) {
    const { MAX_GRAPH_TRAVERSAL_DEPTH, MAX_GRAPH_NODES_PER_RESPONSE, MAX_GRAPH_EDGES_PER_RESPONSE, } = await import("./graph-catalog.js");
    const depth = Math.max(0, Math.min(input.depth ?? 2, MAX_GRAPH_TRAVERSAL_DEPTH));
    bump("graph_query_total");
    // Find the seed node for this evidence.
    const seedRows = (await client.$queryRawUnsafe(`SELECT "id", "team_id", "node_kind", "external_id", "safe_label",
       ...
       LIMIT 1`, input.teamId, input.evidenceId));
    ...
    let frontier = new Set([seedNode.id]);
    let truncated = false;
    for (let hop = 0; hop < depth; hop++) {
        ...
        const edges = (await client.$queryRawUnsafe(`SELECT "id", "team_id", "source_node_id", "target_node_id",
         ...
         LIMIT $3`, input.teamId, frontierIds, MAX_GRAPH_EDGES_PER_RESPONSE - collectedEdges.size));
        ...
        if (nextFrontier.size > 0) {
            ...
            const nodes = (await client.$queryRawUnsafe(`SELECT "id", "team_id", "node_kind", "external_id", "safe_label",
                "visibility_scope", "stale_at_utc",
                "created_at_utc", "updated_at_utc"
           FROM "investigation_graph_nodes"
           WHERE "team_id" = $1
             AND "id" = ANY($2::uuid[])
             AND "stale_at_utc" IS NULL`, input.teamId, newIds));
...
export async function createManualRelationship(input, client = defaultPrisma) {
    try {
        // Both endpoints must belong to this team (anti-enumeration).
        const endpoints = (await client.$queryRawUnsafe(`SELECT "id"
         FROM "investigation_graph_nodes"
         WHERE "team_id" = $1
           AND "id" = ANY($2::uuid[])
           AND "stale_at_utc" IS NULL`, input.teamId, [input.sourceNodeId, input.targetNodeId]));
        ...
        // Insert audit row.
        const auditRows = (await client.$queryRawUnsafe(`INSERT INTO "manual_relationships"
         ("team_id", "source_node_id", "target_node_id",
          "edge_type", "created_by_user_id", "safe_note", "status")
       VALUES ($1, $2, $3, $4, $5, $6, 'ACTIVE')
       RETURNING "id"`, input.teamId, input.sourceNodeId, input.targetNodeId, input.edgeType, input.actorUserId, input.safeNote?.slice(0, 400) ?? null));
        const manualRelationshipId = auditRows[0].id;
        // Upsert the corresponding graph edge.
        const edgeRows = (await client.$queryRawUnsafe(`INSERT INTO "investigation_graph_edges"
       ...
       DO UPDATE SET
         ...
         "stale_at_utc" = NULL,
         "updated_at_utc" = NOW()
       RETURNING "id"`, input.teamId, input.sourceNodeId, input.targetNodeId, input.edgeType, input.safeNote?.slice(0, 240) ?? "Manually linked by operator.", input.actorUserId));
...
export async function retractManualRelationship(input, client = defaultPrisma) {
    try {
        // Mark the manual relationship RETRACTED + stale the graph edge.
        const rels = (await client.$queryRawUnsafe(`UPDATE "manual_relationships"
         SET "status" = 'RETRACTED',
             "retracted_by_user_id" = $3,
             "retracted_at_utc" = NOW(),
             "retraction_reason" = $4
         WHERE "id" = $1 AND "team_id" = $2 AND "status" = 'ACTIVE'
         RETURNING "source_node_id", "target_node_id", "edge_type"`, input.manualRelationshipId, input.teamId, input.actorUserId, input.reason?.slice(0, 240) ?? null));
        if (rels.length === 0) {
            // Already retracted / not found / wrong team — idempotent.
            return { ok: true };
        }
        const rel = rels[0];
        await client.$executeRawUnsafe(`UPDATE "investigation_graph_edges"
         SET "stale_at_utc" = NOW(),
             "updated_at_utc" = NOW()
         WHERE "team_id" = $1 AND "source_node_id" = $2
           AND "target_node_id" = $3 AND "edge_type" = $4
           AND "source_kind" = 'MANUAL'`, input.teamId, rel.source_node_id, rel.target_node_id, rel.edge_type);
...
export async function listDuplicateEdges(input, client = defaultPrisma) {
    const limit = Math.max(1, Math.min(input.limit ?? 100, 200));
    bump("graph_duplicate_list_total");
    let rows = [];
    try {
        if (input.evidenceId) {
            rows = (await client.$queryRawUnsafe(`SELECT e."id" AS "edge_id",
                e."edge_type",
                e."confidence",
                e."safe_summary",
          ...
          LIMIT $3`, input.teamId, input.evidenceId, limit + 1));
        }
        else {
            rows = (await client.$queryRawUnsafe(`SELECT e."id" AS "edge_id",
                e."edge_type",
...
export async function listGraphSeedNodes(input, client = defaultPrisma) {
    const cap = Math.max(1, Math.min(input.perKindLimit ?? 25, 50));
    const requestedKinds = input.kinds && input.kinds.length > 0
        ? input.kinds
        : ["CASE", "INCIDENT", "REPORT", "EVIDENCE"];
    const collected = [];
    for (const kind of requestedKinds) {
        try {
            const rows = (await client.$queryRawUnsafe(`SELECT "id", "node_kind", "external_id", "safe_label", "updated_at_utc"
           FROM "investigation_graph_nodes"
           WHERE "team_id" = $1
             AND "node_kind" = $2
             AND "stale_at_utc" IS NULL
           ORDER BY "updated_at_utc" DESC
           LIMIT $3`, input.teamId, kind, cap));
            for (const r of rows) {
                collected.push({
                    id: r.id,
...
function evidenceVisibility(_status) {
...
async function upsertNode(client, teamId, nodeKind, externalId, safeLabel, visibility) {
    try {
        await client.$executeRawUnsafe(`INSERT INTO "investigation_graph_nodes"
       ...
       DO UPDATE SET
         ...
         "stale_at_utc" = NULL,
         "updated_at_utc" = NOW()`, teamId, nodeKind, externalId, safeLabel, visibility);
...
async function findNodeId(client, teamId, nodeKind, externalId) {
    try {
        const rows = (await client.$queryRawUnsafe(`SELECT "id" FROM "investigation_graph_nodes"
         WHERE "team_id" = $1 AND "node_kind" = $2 AND "external_id" = $3
         LIMIT 1`, teamId, nodeKind, externalId));
...
async function upsertEdge(client, teamId, sourceNodeId, targetNodeId, edgeType, sourceKind, confidence, safeSummary) {
    try {
        await client.$executeRawUnsafe(`INSERT INTO "investigation_graph_edges"
         ("team_id", "source_node_id", "target_node_id", "edge_type",
       ...
       DO UPDATE SET
         ...
         "stale_at_utc" = NULL,
         "updated_at_utc" = NOW()`, teamId, sourceNodeId, targetNodeId, edgeType, sourceKind, confidence, safeSummary);
        bump("graph_edge_created_total");
        return true;
    }
    catch {
 ...
 * nodes excluded).
 */
export async function searchGraph(input, client = defaultPrisma) {
    const { MAX_GRAPH_NODES_PER_RESPONSE } = await import("./graph-catalog.js");
    const limit = Math.max(1, Math.min(input.limit ?? 50, MAX_GRAPH_NODES_PER_RESPONSE));
    bump("graph_query_total");
    try {
        const where = [`"team_id" = $1`, `"stale_at_utc" IS NULL`];
        const params = [input.teamId];
        const kindFilter = input.nodeKinds && input.nodeKinds.length > 0 ? input.nodeKinds : null;
        if (kindFilter) {
        ...
        params.push(limit + 1);
        const rows = (await client.$queryRawUnsafe(`SELECT "id", "team_id", "node_kind", "external_id", "safe_label",
              "visibility_scope", "stale_at_utc", "created_at_utc", "updated_at_utc"
         FROM "investigation_graph_nodes"
 ...
 */
export async function buildInvestigationTimeline(input, client = defaultPrisma) {
    const { MAX_GRAPH_NODES_PER_RESPONSE } = await import("./graph-catalog.js");
    const limit = Math.max(1, Math.min(input.limit ?? 50, MAX_GRAPH_NODES_PER_RESPONSE));
    ...
    try {
        ...
        const lifecycleWhere = [`le."team_id" = $1`];
        const runsWhere = [`r."team_id" = $1`];
        const signalsWhere = [`s."team_id" = $1`];
        if (input.rootNodeId) {
            params.push(input.rootNodeId);
            const p = params.length;
            nodeWhere.push(`n."id" = $${p}`);
            edgeWhere.push(`(e."source_node_id" = $${p} OR e."target_node_id" = $${p})`);
        }
        let evidenceParamIndex = null;
        if (input.evidenceId) {
        ...
        if (input.fromUtc) {
            ...
            edgeWhere.push(`(e."created_at_utc" >= $${p} OR (e."stale_at_utc" IS NOT NULL AND e."stale_at_utc" >= $${p}))`);
            lifecycleWhere.push(`le."created_at" >= $${p}`);
            runsWhere.push(`(r."created_at_utc" >= $${p} OR r."started_at_utc" >= $${p} OR r."completed_at_utc" >= $${p})`);
            signalsWhere.push(`(s."created_at_utc" >= $${p} OR s."acknowledged_at_utc" >= $${p})`);
        }
        if (input.toUtc) {
 ...
 */
export async function buildCaseSubgraph(input, client = defaultPrisma) {
    bump("graph_query_total");
    const seedRows = (await client.$queryRawUnsafe(`SELECT "id" FROM "investigation_graph_nodes"
       WHERE "team_id" = $1 AND "node_kind" = 'CASE'
         AND "external_id" = $2 AND "stale_at_utc" IS NULL
       LIMIT 1`, input.teamId, input.caseId));
    if (seedRows.length === 0) {
        return { ok: true, nodes: [], edges: [], truncated: false };
    }
    return buildSubgraphFromNodeId({ teamId: input.teamId, seedNodeId: seedRows[0].id, depth: input.depth }, client);
}
/** Internal helper: traverse from any seed node id (not just
 *  evidence). Used by buildCaseSubgraph. */
async function buildSubgraphFromNodeId(input, client) {
    const { MAX_GRAPH_TRAVERSAL_DEPTH, MAX_GRAPH_NODES_PER_RESPONSE, MAX_GRAPH_EDGES_PER_RESPONSE, } = await import("./graph-catalog.js");
    const depth = Math.max(0, Math.min(input.depth ?? 2, MAX_GRAPH_TRAVERSAL_DEPTH));
    const seedRows = (await client.$queryRawUnsafe(`SELECT "id", "team_id", "node_kind", "external_id", "safe_label",
            "visibility_scope", "stale_at_utc", "created_at_utc", "updated_at_utc"
       ...
       LIMIT 1`, input.seedNodeId, input.teamId));
    if (seedRows.length === 0) {
        return { ok: true, nodes: [], edges: [], truncated: false };
    }
    const seedNode = seedRows[0];
