...
export async function stripeRequestRaw(path, method, body) {
    const res = await fetch(`${stripeApiBase()}${path}`, {
        method,
        headers: {
            Authorization: `Bearer ${must("STRIPE_SECRET_KEY")}`,
...
export function verifyStripeSignature(rawBody, signature, options = {}) {
    ...
    if (!timestamp || !sig) {
        throw new StripeSignatureError("Invalid signature header", "MISSING_FIELDS");
    }
    ...
    if (!Number.isFinite(tsSec) ||
        ...
        throw new StripeSignatureError("Stripe signature timestamp is not a valid unix epoch.", "TIMESTAMP_INVALID");
    }
    const nowMs = options.nowMs ?? Date.now();
    const tsMs = tsSec * 1000;
    const deltaSec = Math.abs(nowMs - tsMs) / 1000;
    if (deltaSec > STRIPE_SIGNATURE_TOLERANCE_SEC) {
        throw new StripeSignatureError(`Stripe signature timestamp is outside the ${STRIPE_SIGNATURE_TOLERANCE_SEC}s tolerance window (delta=${Math.round(deltaSec)}s).`, "TIMESTAMP_OUT_OF_TOLERANCE");
    }
    ...
    if (expected.length !== sig.length) {
        throw new StripeSignatureError("Invalid webhook signature", "BAD_SIGNATURE");
    }
