...
export function hashDeviceCookieValue(value: string): string {
  return hashOpaqueValue(value.trim());
}

const SIGNAL_KIND_SET = new Set<string>(RISK_SIGNAL_KINDS);
...
const SIGNAL_TTL_HOURS: Readonly<Record<RiskSignalKind, number | null>> = {
  NEW_DEVICE: 24 * 7,
  NEW_IP: 24 * 7,
  NEW_USER_AGENT: 24 * 3,
  NEW_COUNTRY: 24 * 14,
...
export type RecordRiskSignalInput = {
  teamId: string;
  signalKind: RiskSignalKind;
  reason: string;
  subjectKind: "USER" | "SERVICE_ACCOUNT" | "CONTRIBUTOR_SESSION";
...
export async function recordRiskSignal(
...
): Promise<prismaPkg.RiskSignal | null> {
  if (!SIGNAL_KIND_SET.has(input.signalKind)) return null;
  ...
  let row: prismaPkg.RiskSignal | null = null;
  try {
...
function signalKindToSecurityEvent(
  kind: RiskSignalKind,
):
...
const HIGH_THRESHOLD = (() => {
  ...
  if (!Number.isFinite(n) || n < 1 || n > 100) return 75;
  return n;
})();
...
const MEDIUM_THRESHOLD = (() => {
  ...
  const n = raw ? Number.parseInt(raw, 10) : NaN;
  if (!Number.isFinite(n) || n < 1 || n > 100) return 40;
  return n;
})();

export async function getRiskSnapshotForUser(
...
export async function getRiskSnapshotForServiceAccount(
  input: { teamId: string; apiCredentialId: string },
  client: PrismaClient = defaultPrisma,
): Promise<RiskSnapshot> {
  const now = new Date();
  const rows = await client.riskSignal.findMany({
...
export async function getRiskSnapshotForContributorSession(
  input: { teamId: string; intakeSessionId: string },
  client: PrismaClient = defaultPrisma,
): Promise<RiskSnapshot> {
  const now = new Date();
  const rows = await client.riskSignal.findMany({
...
function computeSnapshot(rows: prismaPkg.RiskSignal[]): RiskSnapshot {
  const signals = rows
    .filter((r) => SIGNAL_KIND_SET.has(r.signalKind))
    .map((r) => ({
      kind: r.signalKind as RiskSignalKind,
      reason: r.reason,
...
//
// Compares the inbound request's IP / UA / deviceIdHash against the
// user's trusted-device baseline. Fires NEW_DEVICE / NEW_IP /
// NEW_USER_AGENT signals when divergence is detected. Best-effort.
// -----------------------------------------------------------------------------

export type ObserveSessionContextInput = {
  teamId: string;
  userId: string;
  ip?: string | null;
  userAgent?: string | null;
...
export async function observeSessionContext(
  input: ObserveSessionContextInput,
  client: PrismaClient = defaultPrisma,
): Promise<void> {
  try {
...
export async function maybeFireFailedOtpBurst(
  input: { teamId: string; userId: string },
  client: PrismaClient = defaultPrisma,
): Promise<void> {
  try {
