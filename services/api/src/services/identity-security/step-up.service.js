...
import { VerificationError, checkVerification, startVerification, } from "../communications/verification.service.js";
...
export async function startStepUpChallenge(input, client = defaultPrisma) {
...
export async function checkStepUpChallenge(input, client = defaultPrisma) {
    ...
    try {
        ...
        if (err instanceof VerificationError) {
            ...
            await maybeFireFailedOtpBurst({ teamId: input.teamId, userId: input.userId }, client);
...
export async function consumeApprovedChallenge(input, client = defaultPrisma) {
    const row = await client.stepUpChallenge.findFirst({
        where: { id: input.challengeId, teamId: input.teamId },
    });
    if (!row)
...
// Projection
// -----------------------------------------------------------------------------
export function projectStepUpChallenge(c) {
    return {
        id: c.id,
        purpose: c.purpose,
