...
import { TRUSTED_DEVICE_TTL_DAYS_DEFAULT, TRUSTED_DEVICE_TTL_DAYS_MAX, isValidDeviceCookieValue, maskIpPreview, summariseUserAgent, } from "@proovra/shared";
import { prisma as defaultPrisma } from "../../db.js";
import { appendPlatformAuditLog } from "../platform-audit-log.service.js";
import { safeEmitSecurityEvent } from "../security/security-event.service.js";
import { hashDeviceCookieValue, hashIpAddress } from "./risk.service.js";
export async function markDeviceTrusted(input, client = defaultPrisma) {
    if (!isValidDeviceCookieValue(input.deviceCookieValue))
        return null;
...
function clampTtlDays(value) {
    if (value === null)
        return TRUSTED_DEVICE_TTL_DAYS_DEFAULT;
...
// isCurrentlyTrusted
// -----------------------------------------------------------------------------
export async function isCurrentlyTrusted(input, client = defaultPrisma) {
    if (!isValidDeviceCookieValue(input.deviceCookieValue))
        return false;
    const deviceIdHash = hashDeviceCookieValue(input.deviceCookieValue);
...
export async function revokeTrustedDevice(input, client = defaultPrisma) {
    const existing = await client.trustedDevice.findFirst({
...
export async function listTrustedDevicesForUser(input, client = defaultPrisma) {
    return client.trustedDevice.findMany({
        where: { teamId: input.teamId, userId: input.userId },
        orderBy: { lastSeenAtUtc: "desc" },
    });
}
export async function listTrustedDevicesForTeam(input, client = defaultPrisma) {
    return client.trustedDevice.findMany({
        where: {
            teamId: input.teamId,
            ...(input.activeOnly
...
// -----------------------------------------------------------------------------
export function projectTrustedDevice(d) {
    return {
        id: d.id,
        userId: d.userId,
