 ...
 */
export async function generateIncidentsForWorkspace(ctx) {
    const rules = [];
    let recorded = 0;
    let failed = 0;
    for (const rule of [
...
// ---------- Per-rule scanners ---------------------------------------------
async function scanReportBacklog(ctx) {
    ...
    if (backlog < REPORT_BACKLOG_HIGH)
        return null;
    const severity = backlog >= REPORT_BACKLOG_CRITICAL ? "CRITICAL" : "HIGH";
    return {
...
async function scanPackageBacklog(ctx) {
    const backlog = await prisma.evidence.count({
        where: {
            teamId: ctx.teamId,
            status: "REPORTED",
    ...
    if (backlog < PACKAGE_BACKLOG_HIGH)
        return null;
    const severity = backlog >= PACKAGE_BACKLOG_CRITICAL ? "CRITICAL" : "HIGH";
    return {
...
async function scanStaleReviews(ctx) {
    const since = new Date(Date.now() - STALE_REVIEW_HOURS * 60 * 60 * 1000);
    ...
    if (stale < STALE_REVIEW_HIGH_COUNT)
        return null;
    return {
...
async function scanRetryStorms(ctx) {
...
async function scanStaleTelemetry(ctx) {
    ...
    if (!recent)
        return null; // healthy_empty — no rows yet is not a problem
    const ageMinutes = (Date.now() - recent.sampledAtUtc.getTime()) / 60_000;
    if (ageMinutes < TELEMETRY_STALE_MINUTES)
        return null;
    return {
...
async function scanWorkerHeartbeatStaleness(_ctx) {
    ...
    if (!recent)
        return null;
    const ageMinutes = (Date.now() - recent.heartbeatAtUtc.getTime()) / 60_000;
    if (ageMinutes < WORKER_HEARTBEAT_STALE_MINUTES)
        return null;
    ...
    return {
        ...
        category: "WORKER",
        severity: ageMinutes > WORKER_HEARTBEAT_STALE_MINUTES * 4 ? "CRITICAL" : "HIGH",
...
async function scanUnsignedFinalizedAged(ctx) {
    ...
    // signal.
    const cutoff = new Date(Date.now() - UNSIGNED_FINALIZED_AGED_DAYS * 24 * 60 * 60 * 1000);
    ...
    if (aged < UNSIGNED_FINALIZED_HIGH_COUNT)
        return null;
    return {
...
async function scanCoordinationBacklogStale(ctx) {
    const cutoff = new Date(Date.now() - COORDINATION_STALE_DAYS * 24 * 60 * 60 * 1000);
    ...
    if (total < COORDINATION_STALE_HIGH_COUNT)
        return null;
    return {
        fingerprint: `dashboard:coordination:stale_backlog:${ctx.teamId}`,
        category: "GOVERNANCE",
        severity: total >= COORDINATION_STALE_HIGH_COUNT * 3 ? "HIGH" : "WARNING",
