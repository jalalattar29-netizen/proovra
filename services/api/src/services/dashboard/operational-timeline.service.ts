...
export type OperationalTimelineFamily =
  | "EVIDENCE"
  | "CASE"
  | "CUSTODY"
  | "REPORT"
...
export type OperationalTimelineConfidence = "DIRECT" | "INFERRED";
...
async function projectOne(input: {
  ...
  eventFamily: OperationalTimelineFamily;
  eventType: string;
  severity: string;
  occurredAtUtc: Date;
  sourceTable: string;
  sourceId: string;
  confidence?: OperationalTimelineConfidence;
  summary: string;
  route?: string | null;
}): Promise<void> {
...
> {
  ...
  return rows.map((r) => ({
    id: r.id,
    eventFamily: r.eventFamily as OperationalTimelineFamily,
    ...
    caseId: r.caseId,
    route: r.route,
    confidence: r.confidence as OperationalTimelineConfidence,
  }));
}
