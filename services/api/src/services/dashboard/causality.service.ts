...
export async function detectCausalityForWorkspace(input: {
  ...
  for (const wf of workflows) {
    if (!wf.sourceIncidentId) continue;
  ...
  for (const r of reportIncidents) {
    for (const p of packageIncidents) {
      ...
      links.push({
        ...
        explanation:
          "Report backlog blocks downstream verification package generation.",
      });
    }
  }
  ...
  const auditFingerprintIncidents = incidents.filter((i) =>
    i.fingerprint.includes("unsigned_aged"),
  );
  for (const g of governanceIncidents) {
    for (const a of auditFingerprintIncidents) {
      ...
      links.push({
        ...
        explanation:
          "Governance incident contributes to audit readiness pressure.",
      });
    }
  }

  // Rule E — telemetry stale → worker incidents (CAUSED_BY)
  const telemetryIncidents = incidents.filter((i) =>
    i.fingerprint.includes("telemetry:"),
  );
  const heartbeatStaleIncidents = incidents.filter((i) =>
  ...
  for (const t of telemetryIncidents) {
    for (const w of heartbeatStaleIncidents) {
      ...
      links.push({
        ...
        explanation:
          "Worker heartbeat staleness explains stale queue telemetry samples.",
      });
    }
  }
...
function buildChains(input: {
  ...
  const governanceIncs = input.incidents.filter(
    (i) => i.category === "GOVERNANCE",
  );
  ...
  if (reportIncs.length + packageIncs.length >= 2) {
    ...
    const wfs = input.workflows.filter(
      (w) =>
        (w.workflowType === "REPORT_RETRY" || w.workflowType === "PACKAGE_RETRY") &&
        w.sourceIncidentId !== null &&
        allIncs.some((i) => i.id === w.sourceIncidentId),
    );
    const corrs = input.correlations.filter(
      (c) => c.correlationType === "PIPELINE_DEGRADATION",
    );
    out.push({
  ...
  const reviewWorkflows = input.workflows.filter(
    (w) => w.workflowType === "REVIEW_ESCALATION",
  );
  if (reviewWorkflows.length >= 2) {
    const incs = input.incidents.filter((i) =>
      reviewWorkflows.some((w) => w.sourceIncidentId === i.id),
    );
    out.push({
  ...
  if (governanceIncs.length >= 1 && packageIncs.length >= 1) {
    const wfs = input.workflows.filter(
      (w) =>
        w.workflowType === "GOVERNANCE_ESCALATION" ||
        w.workflowType === "EXPORT_BLOCKER_RESOLUTION" ||
        w.workflowType === "AUDIT_READINESS",
    );
    const corrs = input.correlations.filter(
      (c) =>
        c.correlationType === "GOVERNANCE_ESCALATION" ||
    ...
    out.push({
      ...
      linkedCorrelationIds: corrs.map((c) => c.id),
      linkedCaseIds: [],
      linkedEvidenceIds: dedupEvidence(
        [...governanceIncs, ...packageIncs],
        wfs,
      ),
    });
  }
  ...
  if (telemetryIncs.length >= 2 || workerIncs.length >= 3) {
    ...
    const wfs = input.workflows.filter(
      (w) =>
        w.workflowType === "TELEMETRY_RECOVERY" ||
  ...
  const integrityIncs = input.incidents.filter(
    (i) => i.category === "GOVERNANCE" && i.fingerprint.includes("unsigned_aged"),
  );
  const integrityWfs = input.workflows.filter(
    (w) =>
      w.workflowType === "INTEGRITY_REVIEW" ||
      w.workflowType === "AUDIT_READINESS",
  );
  ...
  const coordIncs = input.incidents.filter((i) =>
    i.fingerprint.includes("coordination:"),
  );
  const coordWfs = input.workflows.filter(
    (w) =>
      w.workflowType === "COORDINATION_RESOLUTION" ||
...
function maxSeverity(items: string[]): Severity {
  let maxRank = 0;
...
function dedupEvidence(
  incidents: IncidentRow[],
  workflows: Array<{ evidenceId: string | null }>,
): string[] {
  const set = new Set<string>();
  for (const i of incidents) {
    if (i.relatedEvidenceId) set.add(i.relatedEvidenceId);
  }
  for (const w of workflows) {
    if (w.evidenceId) set.add(w.evidenceId);
  }
