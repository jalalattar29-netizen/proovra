...
export async function correlateWorkspaceIncidents(input: {
  teamId: string;
}): Promise<{ persisted: number; failed: number; patterns: string[] }> {
  let persisted = 0;
  let failed = 0;
  ...
  try {
    const since = new Date(
      Date.now() - CORRELATION_WINDOW_HOURS * 60 * 60 * 1000,
    );
  ...
  if (incidents.length === 0) return { persisted: 0, failed: 0, patterns: [] };
...
function detectPatterns(
  ...
  const integrationIncidents = incidents.filter(
  ...
  if (reportIncidents.length >= 1 && packageIncidents.length >= 1) {
    candidates.push({
      ...
      rootOperationalCause:
        "Report and verification package pipelines are both degraded.",
      ...
      recommendedAction:
        "Investigate the upstream generator (worker queue or storage backend); the two pipelines share infrastructure.",
  ...
  if (infra.length >= 2) {
    candidates.push({
      ...
      rootOperationalCause:
        "Storage and/or database subsystem under pressure.",
      ...
      recommendedAction:
        "Check storage backend capacity + DB connection pool saturation. Other pipelines depend on these.",
  ...
  if (governanceIncidents.length >= 2) {
    candidates.push({
      ...
      rootOperationalCause:
        "Multiple governance incidents firing concurrently — policy or hold conflicts may be blocking exports.",
      operationalSummary: `${governanceIncidents.length} governance incidents active in the last ${CORRELATION_WINDOW_HOURS}h.`,
      recommendedAction:
        "Open the governance dashboard; check active legal holds + retention policy versions.",
  ...
  if (governanceIncidents.length >= 1 && packageIncidents.length >= 1) {
    candidates.push({
      ...
      rootOperationalCause:
        "Audit readiness degraded — governance incidents are blocking package generation.",
      ...
      recommendedAction:
        "Resolve the governance incident first; the package backlog will clear automatically once the blocker is removed.",
  ...
  if (integrationIncidents.length >= 2) {
    candidates.push({
      ...
      rootOperationalCause:
        "Multiple integration incidents firing concurrently — upstream connector congestion is propagating.",
      operationalSummary: `${integrationIncidents.length} integration incidents active in the last ${CORRELATION_WINDOW_HOURS}h.`,
      recommendedAction:
        "Check the integration health page; consider throttling the noisiest connector temporarily.",
...
function maxSeverity(rows: IncidentRow[]): "INFO" | "WARNING" | "HIGH" | "CRITICAL" {
  let max: "INFO" | "WARNING" | "HIGH" | "CRITICAL" = "INFO";
  let maxRank = 0;
  for (const r of rows) {
    const rank = severityRank(r.severity);
    if (rank > maxRank) {
      maxRank = rank;
      max = r.severity as "INFO" | "WARNING" | "HIGH" | "CRITICAL";
    }
  }
  return max;
}
