 ...
 */
function classifySeverity(category, count, worstSecurityEventSeverity) {
    if (category === "ADMIN_ROLE_CHANGE" ||
        category === "BLOCKED_EXPORT_ATTEMPT" ||
        category === "STEP_UP_FAILED") {
        // Critical-leaning categories — escalate faster.
        if (worstSecurityEventSeverity === "HIGH" || count >= 5)
            return "HIGH";
        if (count >= 3)
            return "WARNING";
        return "INFO";
    ...
    // Default scaling.
    if (count >= 10 || worstSecurityEventSeverity === "HIGH")
        return "HIGH";
    if (count >= CLASSIFIER_MIN_BURST)
        return "WARNING";
    return "INFO";
}
function describeAnomaly(c, count) {
    switch (c) {
...
export async function runClassifierForWorkspace(input) {
    let persisted = 0;
    let failed = 0;
    const windowStart = new Date(Date.now() - CLASSIFIER_WINDOW_HOURS * 60 * 60 * 1000);
        ...
        for (const e of events) {
            ...
            if (entry) {
                ...
                if (e.severity === "HIGH" ||
                    ...
                    entry.worstSeverity = e.severity;
                }
                if (e.createdAt > entry.lastSeen)
