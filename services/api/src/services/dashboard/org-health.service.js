...
export async function recordOrgHealthSnapshotForWorkspace(input) {
    try {
        // Real counters
        const [openIncidents, criticalIncidents, activeWorkflows, resolvedWorkflows7d, failedWorkflows, queueSamples7d, reviewerCriticalCount, reviewerWorkloadCount, auditBlockerWorkflows, reportBacklog, packageBacklog,] = await Promise.all([
        ...
        const healthScore = clamp100(100 -
            Math.min(50, openIncidents * 3) -
            Math.min(30, criticalIncidents * 10) -
            Math.min(20, failedWorkflows * 5));
        const operationalMaturityScore = clamp100(100 -
            Math.min(40, activeWorkflows * 2) -
            Math.min(20, failedWorkflows * 5) +
            Math.min(20, Math.floor(resolvedWorkflows7d / 2)));
        const governanceMaturityScore = clamp100(100 - Math.min(60, auditBlockerWorkflows * 8));
        const auditReadinessScore = clamp100(100 -
            Math.min(50, auditBlockerWorkflows * 8) -
            Math.min(30, packageBacklog * 1));
        const reviewerMaturityScore = clamp100(100 -
            Math.min(50, reviewerCriticalCount * 12) -
            Math.min(20, Math.max(0, reviewerWorkloadCount - 5)));
        const queueReliabilityScore = clamp100(queueSamples7d > 0 ? 80 + Math.min(20, queueSamples7d) : 50);
        const artifactReliabilityScore = clamp100(100 - Math.min(60, reportBacklog) - Math.min(40, packageBacklog));
