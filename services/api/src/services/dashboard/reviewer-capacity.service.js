    ...
    try {
        ...
        for (const r of active) {
            ...
            if (r.dueAt && r.dueAt < now)
                b.overdueCount += 1;
            else if (r.dueAt && r.dueAt < dueSoonCutoff)
                b.dueSoonCount += 1;
            if (r.updatedAt < staleCutoff)
                b.staleCount += 1;
        }
        ...
        const completed7Map = new Map();
        const completed30Map = new Map();
        ...
        for (const b of byReviewer.values()) {
            ...
            const saturationLevel = b.assignedCount >= SATURATION_CRITICAL
                ? "CRITICAL"
            ...
            const overloadPenalty = Math.min(80, b.assignedCount * 2 + b.overdueCount * 5 + b.staleCount * 3);
        ...
        if (snapshots.length >= 2) {
            const overloaded = snapshots.filter((s) => s.saturationLevel === "HIGH" || s.saturationLevel === "CRITICAL");
            const lowSat = snapshots.filter((s) => s.saturationLevel === "LOW");
            for (const src of overloaded) {
                if (lowSat.length === 0)
                    continue;
                const target = lowSat.reduce((acc, cur) => cur.capacityScore > acc.capacityScore ? cur : acc);
