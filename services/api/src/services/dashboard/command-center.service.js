...
import { backfillIntegritySnapshots, listWorkspaceIntegritySnapshots, } from "./integrity-snapshot.service.js";
import { listWorkspaceAccessAnomalies, runClassifierForWorkspace, } from "./access-anomaly.service.js";
import { listLatestQueueSnapshots, recordDbDerivedSnapshotsForWorkspace, } from "./queue-telemetry.service.js";
import { listLatestWorkerTelemetry } from "./worker-telemetry.service.js";
import { backfillCaseEvidenceLinks, listCaseSharedEvidenceClusters, listEvidenceLinkedToMultipleCases, } from "./case-evidence-link.service.js";
import { listWorkspaceTimelineEvents, projectWorkspaceTimeline, } from "./operational-timeline.service.js";
...
import { correlateWorkspaceIncidents, listWorkspaceCorrelations, } from "./incident-correlation.service.js";
import { generateWorkflowsForWorkspace, listWorkspaceWorkflows, } from "./workflow-generator.service.js";
import { detectCausalityForWorkspace, listWorkspaceCausalityChains, } from "./causality.service.js";
import { computeReviewerCapacityForWorkspace, listReviewerCapacity, listReviewerRoutingRecommendations, } from "./reviewer-capacity.service.js";
import { projectOperationalGraphForWorkspace, getOperationalGraphSummary, } from "./operational-graph.service.js";
import { getLatestOrgHealthSnapshot, recordOrgHealthSnapshotForWorkspace, } from "./org-health.service.js";
import { resolveCapabilityMatrix, } from "./persona-resolver.service.js";
...
const ROUTING_CATALOG = {
    overdue_review: {
        ...
        operationalExplanation: "A review workflow is past its due time and remains open.",
        recommendedAction: "Open the review and either complete, reassign, or escalate it.",
    ...
    stalled_review: {
        ...
        affectedDomain: "review_ops",
        affectedEntityType: "review_workflow",
        operationalExplanation: "A review has been in progress without any recorded touch beyond the stall window.",
        recommendedAction: "Reassign the reviewer or move the workflow forward.",
        primaryRoute: "/reviewer-ops",
        secondaryRoute: null,
        sourceTable: "EvidenceReviewWorkflow",
        requiredPermission: "evidence_request.review",
    ...
    unassigned_review: {
        ...
        affectedDomain: "review_ops",
        affectedEntityType: "review_workflow",
        operationalExplanation: "A queued review has remained unassigned beyond the bounded queue window.",
        recommendedAction: "Assign a reviewer or rebalance the queue.",
        primaryRoute: "/reviewer-ops",
        secondaryRoute: null,
        sourceTable: "EvidenceReviewWorkflow",
    ...
    open_escalation: {
        ...
        operationalExplanation: "A reviewer escalation is open and requires triage.",
        recommendedAction: "Acknowledge, resolve, or reassign the escalation.",
    ...
    stuck_upload: {
        ...
        operationalExplanation: "Evidence has been in the UPLOADING state without progress beyond the stall window.",
        recommendedAction: "Review the upload session and resume or cancel.",
    ...
    missing_report: {
        ...
        affectedEntityType: "evidence",
        operationalExplanation: "Evidence is finalized (SIGNED) but no report row has been generated.",
        recommendedAction: "Confirm the report pipeline is healthy and trigger generation if required.",
        primaryRoute: "/evidence",
    ...
    missing_package: {
        ...
        affectedEntityType: "evidence",
        operationalExplanation: "Evidence is REPORTED but no verification package row has been generated.",
        recommendedAction: "Confirm the verification-package pipeline is healthy.",
        primaryRoute: "/evidence",
        secondaryRoute: "/ops/observability",
    ...
    failed_report: {
        ...
        operationalExplanation: "An operational incident classified as a report-generation failure is open.",
        recommendedAction: "Open the runbook and acknowledge or resolve the incident.",
    ...
    failed_package: {
        ...
        affectedDomain: "packages",
        affectedEntityType: "incident",
        operationalExplanation: "An operational incident classified as a package-generation failure is open.",
        recommendedAction: "Open the runbook and acknowledge or resolve the incident.",
        primaryRoute: "/ops/observability",
        secondaryRoute: null,
        sourceTable: "OperationalIncident",
    ...
    retry_storm: {
        ...
        affectedEntityType: "incident",
        operationalExplanation: "A repeated operational incident is firing above the retry-storm threshold.",
        recommendedAction: "Investigate the underlying job/worker and resolve the root cause.",
        primaryRoute: "/ops/observability",
        secondaryRoute: null,
        sourceTable: "OperationalIncident",
        requiredPermission: null,
    ...
    governance_conflict: {
        ...
        operationalExplanation: "Governance reports an active policy conflict.",
        recommendedAction: "Open the governance policy editor and reconcile.",
    ...
    policy_conflict: {
        ...
        affectedDomain: "governance",
        affectedEntityType: "policy",
        operationalExplanation: "An active retention policy conflict was detected.",
        recommendedAction: "Open the governance policy editor and reconcile.",
        primaryRoute: "/governance/policy",
        secondaryRoute: "/governance",
        sourceTable: "EvidenceRetentionPolicy",
    ...
    evidence_no_case: {
        ...
        affectedEntityType: "evidence",
        operationalExplanation: "Recently created evidence is not linked to any case.",
        recommendedAction: "Link the evidence to the relevant investigation or matter.",
        primaryRoute: "/evidence",
    ...
    unsigned_evidence_old: {
        ...
        affectedDomain: "evidence_pipeline",
        affectedEntityType: "evidence",
        operationalExplanation: "Evidence has been unsigned for longer than the bounded age window.",
        recommendedAction: "Finalize or revoke the evidence to clear the audit-readiness gap.",
        primaryRoute: "/evidence",
        secondaryRoute: null,
        sourceTable: "Evidence",
        requiredPermission: "evidence.finalize",
    ...
    blocked_export: {
        ...
        affectedDomain: "governance",
        affectedEntityType: "evidence",
        operationalExplanation: "An evidence verification package was blocked by the governance gate.",
        recommendedAction: "Open the evidence detail page and review the governance block reason.",
        primaryRoute: "/evidence",
        secondaryRoute: "/governance",
        sourceTable: "Evidence",
    ...
    operational_incident: {
        ...
        affectedDomain: "operational_health",
        affectedEntityType: "incident",
        operationalExplanation: "A high-severity operational incident is open and unacknowledged.",
        recommendedAction: "Open the runbook and acknowledge or resolve the incident.",
        primaryRoute: "/ops/observability",
 ...
 */
function enrichPressureItems(items, viewerRole) {
    const now = Date.now();
    return items.map((it) => {
        const meta = ROUTING_CATALOG[it.category];
        const ageMs = it.occurredAt !== null ? now - new Date(it.occurredAt).getTime() : null;
        ...
        const canCurrentUserAct = meta.requiredRoles.includes(viewerRole);
...
// ---------------------------------------------------------------------------
async function detectWorkspaceScope(teamId) {
...
// ---------------------------------------------------------------------------
async function runOperationalPressure(teamId, scope, viewerRole) {
    const now = new Date();
    const stuckUploadCutoff = new Date(Date.now() - STUCK_UPLOAD_HOURS * 60 * 60 * 1000);
    const stalledReviewCutoff = new Date(Date.now() - STALLED_REVIEW_HOURS * 60 * 60 * 1000);
    const unsignedAgeCutoff = new Date(Date.now() - UNSIGNED_EVIDENCE_AGE_DAYS * 24 * 60 * 60 * 1000);
    const collected = [];
    let anyOk = false;
    let anyFailed = false;
    const pushKindResults = (items, sliced = PRESSURE_PER_KIND) => {
        for (const item of items.slice(0, sliced))
            collected.push(item);
    };
    ...
    if (scope === "TEAM") {
        try {
            ...
            anyOk = true;
            pushKindResults(queuedTooLong.map((r) => ({
    ...
    try {
        ...
        anyOk = true;
        pushKindResults(incidents.map((i) => ({
    ...
    const items = enrichPressureItems(collected.slice(0, PRESSURE_TOTAL), viewerRole);
    const counts = {
...
// ---------------------------------------------------------------------------
async function runCaseOperations(teamId, scope) {
    try {
        const now = new Date();
        ...
        const casesWithEvidenceGapsCount = Math.max(0, activeCasesCount - casesWithEvidenceCount);
        ...
        for (const r of reviewByCase) {
            ...
            entry.open += 1;
            if (r.dueAt && r.dueAt.getTime() < now.getTime())
                entry.overdue += 1;
        ...
        for (const e of evidenceCounts) {
            if (e.caseId)
                evidenceCountMap.set(e.caseId, e._count._all);
        }
        const holdsSet = new Set();
        for (const h of holdsByCase)
            holdsSet.add(h.caseId);
        let openEscalationsByCase = new Map();
        if (scope === "TEAM") {
            try {
                ...
                for (const e of escalations) {
                    ...
                    if (!cid)
                        continue;
                    openEscalationsByCase.set(cid, (openEscalationsByCase.get(cid) ?? 0) + 1);
                }
            }
            catch {
        ...
        const topCases = recentCases.map((c) => ({
            caseId: c.id,
            caseName: c.name,
...
// ---------------------------------------------------------------------------
async function runReviewerOrchestration(teamId, scope) {
    if (scope === "PERSONAL") {
    ...
    try {
        ...
        const inactivityCutoff = new Date(Date.now() - REVIEWER_INACTIVITY_HOURS * 60 * 60 * 1000);
        const [queueDepth, overdueCount, dueSoonCount, unassignedCount, openEscalationsCount, completedLast7dCount, completedPrev7dCount,] = await Promise.all([
        ...
        if (reviewerIds.length > 0) {
            ...
            const overdueById = new Map(overdueGroups
                .filter((g) => g.assignedToUserId)
                .map((g) => [g.assignedToUserId, g._count._all]));
            const dueSoonById = new Map(dueSoonGroups
                .filter((g) => g.assignedToUserId)
                .map((g) => [g.assignedToUserId, g._count._all]));
...
// ---------------------------------------------------------------------------
async function runPipelineDetail(teamId) {
    try {
        const stuckCutoff = new Date(Date.now() - STUCK_UPLOAD_HOURS * 60 * 60 * 1000);
        const [grouped, stuckUploading, reportsReady, packagesReady, publicVerify] = await Promise.all([
            prisma.evidence.groupBy({
                by: ["status"],
                where: { teamId },
                _count: { _all: true },
        ...
        const [failedReports, failedPackages, blockedPackages] = await Promise.all([
            ...
            prisma.evidence
                ...
                .then((rows) => rows.reduce((n, r) => {
                const meta = r.verificationPackageMetadata;
...
// ---------------------------------------------------------------------------
async function runGovernancePosture(teamId, scope) {
    if (scope === "PERSONAL") {
        return { status: "not_applicable", data: null };
    }
    let anyOk = false;
...
// ---------------------------------------------------------------------------
async function runOrganizationalIntelligence(teamId) {
    try {
        const since24h = new Date(Date.now() - 24 * 60 * 60 * 1000);
        const since7d = new Date(Date.now() - 7 * 24 * 60 * 60 * 1000);
        const [evidenceCreatedLast24h, evidenceCreatedLast7d, evidenceFinalizedLast7d, reportsGeneratedLast7d, packagesGeneratedLast7d, activityLast7d,] = await Promise.all([
            prisma.evidence.count({
...
// ---------------------------------------------------------------------------
async function runTimeline(teamId) {
    const items = [];
    let anyOk = false;
    let anyFailed = false;
...
// ---------------------------------------------------------------------------
async function runAuditReadiness(teamId, scope) {
    try {
        const unsignedAgeCutoff = new Date(Date.now() - UNSIGNED_EVIDENCE_AGE_DAYS * 24 * 60 * 60 * 1000);
        const counters = [];
...
// ---------------------------------------------------------------------------
async function runRecentEvidence(teamId) {
    try {
...
async function runIncidents(teamId) {
    ...
    let workflows = [];
    try {
        workflows = (await listWorkspaceWorkflows({ teamId, limit: 24 }));
    }
    catch {
        workflows = [];
    }
...
async function runLegacyPipelineSummary(teamId) {
    try {
...
async function runLegacyReviewerWorkload(teamId, scope) {
    if (scope === "PERSONAL")
        return { status: "not_applicable", data: null };
    try {
        const now = new Date();
        const [queuedCount, assignedCount, inReviewCount, overdueCount, openEscalationsCount] = await Promise.all([
            prisma.evidenceReviewWorkflow.count({
                where: { teamId, status: "QUEUED" },
            }),
            prisma.evidenceReviewWorkflow.count({
...
export async function buildCommandCenter(input) {
    ...
    const [pressure, caseOps, reviewerOrch, pipelineDetail, governance, organizational, timeline, auditReadiness, recentEvidence, incidents, legacyPipeline, legacyReviewer,] = await Promise.all([
    ...
    const [investigation, queueCongestionResult, integrityResult, securityResult, workloadEngineResult,] = await Promise.all([
    ...
    const [relationshipResult, crossCaseV2Result, reconstructedTimelineResult, deepIntegrityResult, securityClassifierResult, queueWorkerTelemetryResult, coordinationResult,] = await Promise.all([
    ...
    const orgIntelligenceV2Result = await runOrgIntelligenceV2(input.teamId, pressure, {
    ...
    const [reviewerCapacityResult, operationalGraphResult, orgHealthResult,] = await Promise.all([
    ...
    const [telemetryHealth, reconcileHealth, securityRollupHealth] = await Promise.all([
        evaluateTelemetryHealth({ teamId: input.teamId }).catch(() => ({
            status: "DEGRADED",
            severity: "warning",
            reason: "Telemetry health evaluator could not complete on this cycle.",
            recoverable: true,
        ...
        evaluateReconcileHealth({ teamId: input.teamId }).catch(() => ({
            status: "DEGRADED",
            severity: "warning",
            reason: "Reconcile health evaluator could not complete on this cycle.",
            recoverable: true,
            lastSuccessfulRunAt: null,
            retrying: true,
            degradedSince: new Date().toISOString(),
        ...
        evaluateSecurityRollupHealth({ teamId: input.teamId }).catch(() => ({
            status: "DEGRADED",
            severity: "warning",
            reason: "Security rollup health evaluator could not complete on this cycle.",
            recoverable: true,
            lastSuccessfulRunAt: null,
            retrying: true,
            degradedSince: new Date().toISOString(),
    ...
    const summary = {
        status: "ok",
        data: {
            evidenceActiveCount: (pipelineDetail.data?.evidence.uploaded ?? 0) +
            ...
            governanceAttentionCount: (governance.data?.activeLegalHoldsCount ?? 0) +
            ...
            auditReadinessFlags: auditReadiness.counters.filter((c) => c.severity !== "info" && c.value > 0).length,
        },
    };
    return {
...
// ---------------------------------------------------------------------------
async function runReviewerCapacityBoard(teamId, scope) {
    const meta = {
        status: "ok",
        warnings: [],
        unsupportedSignals: [],
...
async function runOperationalGraphSection(teamId) {
    const meta = {
        status: "ok",
        warnings: [],
        unsupportedSignals: [],
...
async function runOrganizationalHealthSection(teamId) {
    const meta = {
        status: "ok",
        warnings: [],
        unsupportedSignals: [],
...
// ---------------------------------------------------------------------------
async function runInvestigationIntelligence(teamId, scope) {
    const meta = {
        status: "ok",
        warnings: [],
        unsupportedSignals: [],
    ...
    meta.unsupportedSignals.push("cross_case_same_submitter (no canonical submitter-identity table)");
    try {
        ...
        const [evidenceByCase, reviewByCase, overdueByCase, escalationsByCase, holdsByCase, blockedExportsByCase, missingReportsByCase,] = await Promise.all([
            prisma.evidence.groupBy({
                by: ["caseId"],
                where: { caseId: { in: caseIds } },
                _count: { _all: true },
        ...
        // Build per-case maps.
        const evidenceCountByCase = new Map();
        for (const g of evidenceByCase) {
            if (g.caseId)
                evidenceCountByCase.set(g.caseId, g._count._all);
        }
        const overdueCountByCase = new Map();
        ...
        const escalationsCountByCase = new Map();
        for (const e of escalationsByCase) {
            const cid = e.workflow?.evidence?.caseId;
            if (typeof cid === "string") {
                escalationsCountByCase.set(cid, (escalationsCountByCase.get(cid) ?? 0) + 1);
            }
        }
        const holdsByCaseSet = new Set();
        for (const h of holdsByCase)
            holdsByCaseSet.add(h.caseId);
        ...
        const missingReportsByCaseMap = new Map();
        for (const m of missingReportsByCase) {
            if (m.caseId)
                missingReportsByCaseMap.set(m.caseId, (missingReportsByCaseMap.get(m.caseId) ?? 0) + 1);
        }
            ...
            const riskLevel = score >= 10 ||
                ...
                : score >= 5
                    ...
                    : score >= 1
                        ? "LOW"
                        : "NONE";
            const recommendedAction = riskLevel === "NONE"
        ...
        if (scope === "TEAM") {
            try {
                ...
                meta.warnings.push("Multi-case reviewer overload check could not complete.");
            }
        }
...
// ---------------------------------------------------------------------------
async function runQueueCongestion(teamId, scope) {
    const meta = {
        status: "ok",
        warnings: [],
        unsupportedSignals: [
    ...
    try {
        ...
        if (scope === "TEAM") {
            ...
            const oldestAgeMs = oldestQueued
                ...
                : null;
            const severity = queuedDepth >= 25 ? "high" : queuedDepth >= 10 ? "warning" : "info";
            items.push({
        ...
        items.push({
            ...
            oldestAgeMs: null,
            severity: packagePending >= 15 ? "high" : packagePending >= 5 ? "warning" : "info",
        ...
        if (scope === "TEAM") {
            ...
            items.push({
                ...
                oldestAgeMs: null,
                severity: openEscalations >= 5 ? "high" : openEscalations >= 1 ? "warning" : "info",
...
// ---------------------------------------------------------------------------
async function runCustodyIntegrityAnomalies(teamId) {
    const meta = {
        status: "ok",
        warnings: [],
        unsupportedSignals: [
    ...
    try {
        ...
        for (const row of sample) {
            const m = row.verificationPackageMetadata;
            if (m && m.blocked === true) {
                items.push({
                    evidenceId: row.id,
            ...
            if (items.length >= INTEGRITY_ANOMALY_LIMIT * 2)
                break;
        }
        return {
            meta,
...
// ---------------------------------------------------------------------------
async function runAccessSecurityAnomalies(teamId) {
    const meta = {
        status: "ok",
        warnings: [],
        unsupportedSignals: [
    ...
    try {
        ...
        const items = rows.map((r) => ({
            ...
            severity: (r.severity === "HIGH" ? "high" : "warning"),
...
// ---------------------------------------------------------------------------
async function runWorkloadEngine(teamId, scope) {
    const meta = {
        status: "ok",
        warnings: [],
        unsupportedSignals: [],
    ...
    try {
        ...
        const reviewers = grouped
            ...
            const saturationScore = Math.min(10, Math.round(g._count._all + overdue * 1.5));
            return {
                userId: g.assignedToUserId,
                displayName: userById.get(g.assignedToUserId)?.displayName ?? null,
                email: userById.get(g.assignedToUserId)?.email ?? null,
        ...
        const meanSaturation = reviewers.reduce((s, r) => s + r.saturationScore, 0) /
            Math.max(1, reviewers.length);
        const teamOverdue = reviewers.reduce((s, r) => s + r.overdueCount, 0);
        const health = bottlenecks >= 2 || teamOverdue >= 8
            ? "CRITICAL"
...
// ---------------------------------------------------------------------------
function classifyTimelineDomain(kind) {
    switch (kind) {
...
function buildTimelineGroupings(events) {
    const now = Date.now();
...
function annotateTimelineEvents(events) {
...
// =============================================================================
function buildUnsupportedSignalsCatalog(metas) {
    const result = [];
...
async function runRelationshipIntelligence(teamId) {
    const meta = {
        status: "ok",
        warnings: [],
        unsupportedSignals: [
    ...
    try {
        ...
        for (const g of dupHashes) {
            ...
            const caseIds = Array.from(new Set(members.map((m) => m.caseId).filter((c) => c !== null)));
            clusters.push({
                ...
                recommendedAction: "Open each record and confirm whether the duplication is intentional. Consider linking duplicates to the same case.",
    ...
    try {
        ...
        for (const r of rels) {
            clusters.push({
                ...
                recommendedAction: "Open the related evidence to confirm the operational link.",
    ...
    try {
        ...
        for (const g of submitterGroups) {
            ...
            const caseIds = Array.from(new Set(sample.map((s) => s.caseId).filter((c) => c !== null)));
            clusters.push({
                ...
                recommendedAction: "Confirm the submission pattern aligns with the expected intake plan.",
                route: "/evidence",
                confidence: "direct",
            });
        }
    ...
    meta.unsupportedSignals.push("intake_session_clustering (Evidence has no intakeSessionId column; clustering requires join through EvidenceRequest)");
    return {
        meta: { ...meta, status: clusters.length === 0 ? "ok" : "ok" },
        clusters: clusters.slice(0, RELATIONSHIP_CLUSTER_LIMIT * 2),
    };
}
async function runCrossCaseIntelligenceV2(teamId, scope) {
    const meta = {
        status: "ok",
        warnings: [],
        unsupportedSignals: [
    ...
    try {
        ...
        if (multiCaseEvidence.length > 0) {
            ...
            signals.push({
                ...
                recommendedAction: "Review the cross-linked evidence to confirm the linkage is intended.",
                route: "/cases",
            });
        }
    }
    ...
    try {
        ...
        if (clusters.length >= 2) {
            signals.push({
                ...
                recommendedAction: "Open the cross-case cluster view to confirm investigation scope.",
                route: "/cases",
            });
        }
    }
    ...
    try {
        ...
        if (blockedCases.size >= 2) {
            signals.push({
                ...
                recommendedAction: "Review the governance policy or hold causing the cross-case block.",
                route: "/governance",
            });
        }
        ...
        if (evidenceIds.length >= 3) {
            ...
            if (cases.size >= 2) {
                ...
                signals.push({
                    ...
                    recommendedAction: "Open the runbook for the underlying incident.",
        ...
        if (heldButStale.length >= 2) {
            signals.push({
                ...
                recommendedAction: "Review whether preservation is still required or update the hold reason.",
                route: "/governance",
            });
        }
    }
...
async function runReconstructedTimeline(teamId) {
    const meta = {
        status: "ok",
        warnings: [],
        unsupportedSignals: [
    ...
    const events = [];
    const since = new Date(Date.now() - RECONSTRUCTED_TIMELINE_WINDOW_DAYS * 24 * 60 * 60 * 1000);
    ...
    meta.unsupportedSignals.push("admin_audit_log_workspace_scope (AdminAuditLog has no teamId; workspace scoping would require joining through resourceId for every resourceType)");
    // Reports + packages
    try {
    ...
    try {
        ...
        for (const s of security) {
            events.push({
                ...
                severity: (s.severity === "HIGH" ? "high" : "warning"),
...
async function runDeepIntegrityWatch(teamId) {
    const meta = {
        status: "ok",
        warnings: [],
        unsupportedSignals: [
    ...
    try {
        ...
        if (snapshots.length === 0) {
            ...
            try {
                ...
                meta.warnings.push("EvidenceIntegritySnapshot lazy-backfill could not complete; falling back to live derivation.");
            }
        }
        else {
            for (const s of snapshots) {
                ...
                items.push({
                    ...
                    recommendedAction: "Open the evidence detail and consult the integrity panel.",
        ...
        meta.warnings.push("EvidenceIntegritySnapshot read could not complete; falling back to live derivation.");
    }
    try {
        ...
        for (const r of tsaUnavailable) {
            items.push({
                ...
                confidence: "direct",
                explanation: "Evidence is REPORTED but no TSA token was recorded. Timestamp signal is unavailable for this record.",
                recommendedAction: "Confirm the TSA pipeline is healthy and reprocess if required.",
                href: `/evidence/${r.id}`,
            });
        }
        ...
        for (const r of otsUnavailable) {
            const failed = r.otsStatus === "FAILED" || r.otsStatus === "ERRORED";
            items.push({
                evidenceId: r.id,
                title: r.title ?? "Untitled evidence",
                ...
                recommendedAction: "Confirm the OTS pipeline is healthy and reprocess if required.",
                href: `/evidence/${r.id}`,
            });
        }
        // REPORTED for > N hours without a package.
        const cutoff = new Date(Date.now() - REPORT_BUT_NO_PACKAGE_AGE_HOURS * 60 * 60 * 1000);
        ...
        for (const r of reportFinalizedNoPackage) {
            items.push({
                ...
                recommendedAction: "Confirm the package generator is healthy and reprocess if required.",
                href: `/evidence/${r.id}`,
            });
        }
        // Package blocked.
        ...
        for (const row of sample) {
            ...
            if (m && m.blocked === true) {
                items.push({
                    ...
                    confidence: "direct",
                    explanation: "Package generation was blocked by governance gate. The block reason is recorded in the metadata blob.",
                    recommendedAction: "Open the evidence detail and review the governance block reason.",
                    href: `/evidence/${row.id}`,
                });
            }
            if (items.length >= DEEP_INTEGRITY_LIMIT * 2)
                break;
        }
    }
    catch {
...
function classifySecurityEventType(eventType) {
    ...
    if (t.includes("permission_denied"))
        return "permission_denied_burst";
...
async function runAccessSecurityClassifier(teamId) {
    const meta = {
        status: "ok",
        warnings: [],
        unsupportedSignals: [
    ...
    try {
        ...
        if (persisted.length === 0) {
            try {
                ...
                meta.warnings.push("AccessAnomaly classifier lazy-run could not complete; falling back to live grouping.");
            }
            ...
            if (refreshed.length > 0) {
                return {
                    ...
                    anomalies: refreshed.map((r) => ({
                        ...
                        recommendedAction: recommendAnomalyAction(r.category),
        ...
        meta.warnings.push("AccessAnomaly read could not complete; falling back to live grouping.");
    }
    try {
        const since24h = new Date(Date.now() - 24 * 60 * 60 * 1000);
        const rows = await prisma.securityEvent.findMany({
        ...
        for (const r of rows) {
            ...
            const sev = r.severity === "HIGH" ? "high" : "warning";
            const entry = grouped.get(key);
            if (entry) {
                entry.count += 1;
                if (r.createdAt > entry.lastSeen)
                    entry.lastSeen = r.createdAt;
                if (r.createdAt < entry.firstSeen)
        ...
        for (const v of grouped.values()) {
            if (v.category === "uncategorized" && v.count < 3)
                continue;
            anomalies.push({
                category: v.category,
                severity: v.severity,
...
function describeAnomalyCategory(c, count) {
    switch (c) {
...
function recommendAnomalyAction(c) {
    switch (c) {
        case "repeated_failed_access":
...
async function runQueueWorkerTelemetry(teamId, scope) {
    const meta = {
        status: "ok",
        warnings: [],
        unsupportedSignals: [],
    ...
    try {
        ...
        const heartbeatHours = heartbeatAtUtc
            ...
            : null;
        const reconcileHealth = heartbeatHours === null
            ? "UNAVAILABLE"
            : heartbeatHours <= 1
                ? "FRESH"
                : "STALE";
        const [reviewQueueDepth, reportQueuePending, packageQueuePending, oldestQueued, retryStormIncidents,] = await Promise.all([
            scope === "TEAM"
                ? prisma.evidenceReviewWorkflow.count({
                    where: { teamId, status: "QUEUED" },
                })
        ...
        return {
            ...
            data: {
                ...
                reconcileFreshnessHours: heartbeatHours !== null
                ...
                reportQueuePending,
                packageQueuePending,
                oldestQueuedAgeHours: oldestQueuedAgeHours !== null
...
async function runCoordinationSignals(teamId, scope) {
    const meta = {
        status: "ok",
        warnings: [],
        unsupportedSignals: [],
    ...
    const [caseCommentBacklog, reviewerCommentOpen, annotationOpen,] = await Promise.all([
    ...
    try {
        ...
        for (const a of annotations) {
            signals.push({
                ...
                entityType: "evidence",
                detectedAt: a.createdAt.toISOString(),
                explanation: "A new annotation was recorded; resolution tracking is not yet modeled in this workspace.",
                route: `/evidence/${a.evidenceId}`,
            });
        }
        // Recent legal notes.
...
function runPredictiveRisk(input) {
    ...
    if (reviewer) {
        if (reviewer.dueSoonCount >= 5 && reviewer.queueDepth >= 10) {
            forecasts.push({
                ...
                likelyImpact: "Without rebalancing, several reviews will breach SLA inside the next operational window.",
                recommendedAction: "Reassign or expedite the due-soon workflows.",
                confidence: reviewer.dueSoonCount >= 12 ? "high" : "medium",
                evidenceCount: reviewer.dueSoonCount,
                caseCount: 0,
            });
        }
        const delta = reviewer.completedLast7dCount - reviewer.completedPrev7dCount;
        if (delta <= -3 && reviewer.queueDepth >= 10) {
            forecasts.push({
                ...
                likelyImpact: "Backlog growth without capacity intervention.",
                recommendedAction: "Investigate reviewer availability and redistribute workload.",
                confidence: "medium",
    ...
    if (pipeline) {
        if (pipeline.reports.failed >= 2 || pipeline.reports.queued >= 10) {
            forecasts.push({
                ...
                likelyImpact: "Report pipeline backlog will continue to grow without operator intervention.",
                recommendedAction: "Investigate the report generator and the affected runbooks.",
        ...
        if (pipeline.packages.blocked >= 3 || pipeline.packages.failed >= 2) {
            forecasts.push({
                ...
                severity: pipeline.packages.blocked >= 5 || pipeline.packages.failed >= 4
                    ? "high"
                    : "warning",
                reason: `Packages blocked: ${pipeline.packages.blocked} · packages failed: ${pipeline.packages.failed}.`,
                likelyImpact: "Deliverable generation will stall without governance review or runbook intervention.",
                recommendedAction: "Address blocked packages in Governance and failed packages in Operations Center.",
                confidence: "medium",
                evidenceCount: pipeline.packages.blocked + pipeline.packages.failed,
                caseCount: 0,
            });
        }
    }
    if (governance) {
        if (governance.policyConflictsCount >= 1 ||
            governance.blockedExportsCount >= 3) {
            forecasts.push({
                id: "fc_governance",
                forecastType: "governance_pressure_rising",
                severity: governance.policyConflictsCount >= 3 ||
                ...
                likelyImpact: "Governance gate denials will continue to interfere with downstream operations.",
                recommendedAction: "Open the governance control plane and reconcile policy + holds.",
                confidence: governance.policyConflictsCount >= 3 ||
                    governance.blockedExportsCount >= 6
                    ? "high"
                    : "medium",
    ...
    const flagged = audit.filter((c) => c.severity !== "info" && c.value > 0).length;
    if (flagged >= 3) {
        forecasts.push({
            id: "fc_audit",
            ...
            likelyImpact: "An external audit pass would surface unresolved gaps without operator action.",
            recommendedAction: "Resolve the flagged audit-readiness counters before the next reporting cycle.",
            confidence: "medium",
            evidenceCount: audit.reduce((s, c) => (c.severity !== "info" ? s + c.value : s), 0),
            caseCount: 0,
        });
    }
    if (input.retryStorm >= 1) {
        forecasts.push({
            ...
            likelyImpact: "Repeat failures will continue to consume worker capacity.",
            recommendedAction: "Open the runbook and address the underlying job.",
...
async function runOrgIntelligenceV2(teamId, pressure, workload) {
    const meta = {
        status: "ok",
        warnings: [],
        unsupportedSignals: [],
    ...
    try {
        ...
        for (const item of pressure.items) {
            domainCounts.set(item.affectedDomain, (domainCounts.get(item.affectedDomain) ?? 0) + 1);
        }
        ...
        for (const item of pressure.items) {
            sourceCounts.set(item.sourceTable, (sourceCounts.get(item.sourceTable) ?? 0) + 1);
        }
        ...
        const orgHealth = critical > 0 || workload.health === "CRITICAL"
            ? "CRITICAL"
        ...
        for (const b of bottleneckDomains.slice(0, 3)) {
            if (b.pressureItems > 0) {
                recommendedActions.push(`Address pressure in domain · ${b.domain} (${b.pressureItems} items)`);
            }
        }
        if (workload.health === "CRITICAL" || workload.health === "DEGRADED") {
            recommendedActions.push("Rebalance reviewer workload — team health is " + workload.health);
        }
        return {
            meta,
            data: {
...
async function buildProjectionSummary(teamId) {
    ...
    if (row) {
        const ageSec = Math.max(0, Math.floor((Date.now() - row.sampledAtUtc.getTime()) / 1000));
        const projectionStatus = ageSec <= PROJECTION_FRESH_THRESHOLD_SEC ? "fresh" : "stale";
        return {
            projectionStatus,
