 ...
 */
export async function refreshOrgHealthProjection(input, client = defaultPrisma) {
    if (!input.teamId) {
    ...
    const [evidenceCount, caseCount, pendingReportCount, pendingPackageCount,] = await Promise.all([
 ...
 */
export async function readLatestOrgHealthProjection(input, client = defaultPrisma) {
