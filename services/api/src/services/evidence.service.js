...
import { assertWorkspaceAllowsEvidenceCreation, resolveWorkspaceScopeForUser, assertWorkspaceAllowsStorageGrowth, } from "./billing-enforcement.service.js";
...
import { ensureUploadSession, safeTransitionUploadSession, } from "./reliability/upload-session.service.js";
...
import { buildStrictDepartmentScopeWhere, resolveUserDepartmentScope, } from "./governance/department-scope.service.js";
function must(name) {
...
function normalizeUploadMimeType(input) {
...
function resolveIdentityLevel(params) {
    ...
    if (params.provider === prismaPkg.AuthProvider.GOOGLE ||
...
export async function createEvidence(params) {
    ...
    if (effectiveTeamId) {
        ...
        if (!membership) {
            const err = new Error("Forbidden team workspace");
            err.statusCode = 403;
    ...
    const guestIdentity = owner.provider === prismaPkg.AuthProvider.GUEST
    ...
    const organizationVerifiedSnapshot = workspaceTeam?.verificationState ===
    ...
    const workspaceNameSnapshot = workspaceTeam?.evidenceWorkspaceLabel?.trim() ||
        workspaceTeam?.name?.trim() ||
        null;
    const organizationNameSnapshot = workspaceTeam?.legalName?.trim() ||
        workspaceTeam?.name?.trim() ||
        null;
    const created = await prisma.$transaction(async (tx) => {
            ...
            payload: {
                ...
                departmentScopeContext: departmentScopeContext
                    ? {
                        ...
                        allowedDepartmentIds: departmentScopeContext.allowedDepartmentIds,
        ...
        await appendCustodyEventTx(tx, {
            ...
            payload: {
                ...
                meaning: "A presigned upload URL was issued for the initial intake location. No bytes have been confirmed uploaded yet, and the final evidence structure may still become multipart during completion.",
            },
        });
    ...
    ensureUploadSession({
        ...
        .then(() => safeTransitionUploadSession({
        evidenceId: created.id,
        to: "PRESIGNED",
    }))
        .catch(() => null);
