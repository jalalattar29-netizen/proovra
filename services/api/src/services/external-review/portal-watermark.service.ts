...
import {
  canonicalJson,
  type SignedWatermark,
  type WatermarkPayload,
} from "@proovra/shared";

const WATERMARK_TTL_S_MAX = 4 * 60 * 60; // 4h
const WATERMARK_SCHEMA = "PROOVRA_WATERMARK_V1" as const;
...
export type BuildWatermarkInput = {
  grantId: string;
  sessionId: string;
  ...
  grantExpiresAtUtc: Date;
};

export function buildSignedWatermark(
  input: BuildWatermarkInput,
): SignedWatermark | null {
  const k = secret();
  if (!k) return null;
  const now = new Date();
  const grantTtlS = Math.floor(
    (input.grantExpiresAtUtc.getTime() - now.getTime()) / 1000,
  );
  if (grantTtlS <= 0) return null;
 ...
 */
export function verifySignedWatermark(
  signed: SignedWatermark,
): { valid: boolean; reason?: string } {
  const k = secret();
  if (!k) return { valid: false, reason: "PORTAL_WATERMARK_HMAC_MISSING" };
  ...
  try {
    ...
    if (Date.now() > expires) return { valid: false, reason: "EXPIRED" };
    return { valid: true };
  } catch {
