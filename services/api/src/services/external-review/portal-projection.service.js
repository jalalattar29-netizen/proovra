 ...
 */
import { EXTERNAL_PORTAL_LIMITATIONS, EXTERNAL_PORTAL_SCHEMA_VERSION, externalPortalCapabilitiesForRole, } from "@proovra/shared";
...
export async function projectPortalDashboard(input) {
    ...
    const decidedMap = new Map(decisions.map((d) => [d.workflowId, d.submittedAtUtc]));
    ...
    const watermark = s.watermarkPolicy === "NEVER"
        ? null
    ...
    const role = s.role ?? "EXTERNAL_REVIEWER";
    const proj = {
