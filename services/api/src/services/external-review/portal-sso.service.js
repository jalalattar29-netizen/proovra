...
import { SamlAssertionError, validateSamlResponse, } from "../security/saml-assertion.service.js";
...
// startPortalSsoFlow
// ---------------------------------------------------------------------------
export async function startPortalSsoFlow(input) {
...
// ---------------------------------------------------------------------------
export async function completePortalSsoFlow(input) {
    const prisma = input.prisma ?? defaultPrisma;
    ...
    try {
        ...
        const code = err instanceof SamlAssertionError ? err.code : "SAML_PARSE_FAILED";
        await emitPortalActivity({
            prisma,
            teamId: input.teamId,
            grantId: input.grantId,
    ...
    const identityOk = identityMatchesGrant(assertedEmail, expectedEmail, role.allowedDomains);
    if (!identityOk) {
        await emitPortalActivity({
            prisma,
            teamId: input.teamId,
...
function extractEmail(a) {
    ...
    const candidates = [
        ...
        a.attributes["mail"]?.[0],
        a.attributes["http://schemas.xmlsoap.org/ws/2005/05/identity/claims/emailaddress"]?.[0],
        a.attributes["urn:oid:0.9.2342.19200300.100.1.3"]?.[0],
    ];
...
function identityMatchesGrant(assertedEmail, expectedEmail, allowedDomains) {
