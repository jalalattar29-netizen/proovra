 ...
 */
import { EXTERNAL_REVIEWER_ROLES, WATERMARK_POLICIES, } from "@proovra/shared";
import { prisma as defaultPrisma } from "../../db.js";
import { issueExternalReviewGrant, transitionExternalReviewGrant, } from "./external-review-grant.service.js";
import { emitPortalActivity } from "./portal-activity.service.js";
export async function issueInvitation(input) {
    if (!EXTERNAL_REVIEWER_ROLES.includes(input.role)) {
        return { ok: false, denial: "POLICY_REJECTED" };
    }
    if (input.watermarkPolicy &&
        !WATERMARK_POLICIES.includes(input.watermarkPolicy)) {
        return { ok: false, denial: "POLICY_REJECTED" };
    }
...
export async function acceptInvitation(input) {
    ...
    const res = await transitionExternalReviewGrant({
        ...
        teamId: input.teamId,
        toState: "ACTIVE",
        actorUserId: input.acceptedByUserId ?? "00000000-0000-0000-0000-000000000000",
    }, prisma);
    if (!res.ok) {
        return { ok: false, denial: mapGrantDenial(res.reason) };
    }
...
export async function listInvitationsForTeam(input) {
    ...
    const totalMap = new Map(totals.map((t) => [t.grantId, t._count ?? 0]));
    ...
    return assignments.map((a) => {
        ...
        const expired = expiresAtUtc !== null && expiresAtUtc.getTime() < nowMs;
...
// Helpers
// ---------------------------------------------------------------------------
function mapGrantDenial(reason) {
    switch (reason) {
        case "INVALID_TOKEN":
