...
// Helpers
// -----------------------------------------------------------------------------
function assertSessionUploadEligible(session) {
...
function evidenceTypeFromMime(mime) {
...
function workflowTemplateSnapshotFromLink(link) {
...
// -----------------------------------------------------------------------------
export async function createOrLoadExternalEvidence(pair, seed, client = defaultPrisma) {
    ...
    try {
        const { applyRetentionPolicyOnCreate } = await import("./governance.service.js");
...
export async function addExternalEvidencePart(input, client = defaultPrisma) {
    assertSessionUploadEligible(input.session);
    // Load or create the backing Evidence once.
    const evidence = await createOrLoadExternalEvidence({ link: input.link, session: input.session }, { mimeType: input.mimeType, originalFileName: input.originalFileName }, client);
    const bucket = typeof evidence.storageBucket === "string" && evidence.storageBucket.length > 0
    ...
    const key = `evidence/${evidence.id}/parts/${String(partIndex).padStart(3, "0")}-${fileName ?? `part-${partIndex + 1}`}`;
...
// -----------------------------------------------------------------------------
export async function updateExternalEvidencePartMapping(input, client = defaultPrisma) {
    assertSessionUploadEligible(input.session);
    const part = await client.evidencePart.findUnique({
        where: { id: input.partId },
    });
    if (!part)
        throw new ExternalIntakeOrchestrationError("part_not_found");
 ...
 * link was created.
 */
function assertSubmissionReady(link, parts) {
    const snapshot = link.workflowTemplateSnapshot;
    if (!snapshot || !Array.isArray(snapshot.steps))
        return;
    if (snapshot.planMode !== "CHECKLIST_REQUIRED")
        return;
    const mappedStepIds = new Set(parts
...
export async function submitExternalIntake(input, client = defaultPrisma) {
    assertSessionUploadEligible(input.session);
