...
export async function swallowCustodyAppendError(err, ctx) {
    const preview = asMessage(err).slice(0, MAX_ERR_PREVIEW);
    const errName = err instanceof Error ? err.name : typeof err === "string" ? "string" : "unknown";
 ...
 * route handler.
 */
export function noteCustodyFailure(err) {
