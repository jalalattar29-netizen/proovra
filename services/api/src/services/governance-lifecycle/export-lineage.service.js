...
import { GOVERNANCE_EXPORT_SNAPSHOT_KINDS, GovernanceExportSnapshotInputSchema, } from "@proovra/shared";
...
const VALID_KINDS = new Set(GOVERNANCE_EXPORT_SNAPSHOT_KINDS);
export async function captureExportSnapshot(input, client = defaultPrisma) {
    ...
    let retentionPolicyVersionId = null;
    let retentionPolicyId = null;
    let retentionDays = null;
    let retentionImmutable = false;
    let custodyAnchorHash = null;
    let exportEligibility = {
        outcome: "ALLOWED",
    ...
    if (input.evidenceId) {
        ...
        if (!ev) {
            throw new GovernanceExportSnapshotError("EXPORT_SNAPSHOT_EVIDENCE_NOT_FOUND");
        }
...
// Read
// -----------------------------------------------------------------------------
export async function getExportSnapshot(input, client = defaultPrisma) {
    const row = await client.governanceExportSnapshot.findFirst({
        where: { id: input.id, teamId: input.teamId },
    });
    return row ? projectSnapshot(row) : null;
}
export async function listExportSnapshots(input, client = defaultPrisma) {
...
// -----------------------------------------------------------------------------
export function verifyExportSnapshotHash(snapshot) {
