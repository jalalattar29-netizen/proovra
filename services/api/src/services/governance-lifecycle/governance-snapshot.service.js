...
import { canonicalCanEnterPendingDestruction, canonicalEvaluateExportEligibility, canonicalEvaluatePackageEligibility, exportEligibilityLabel, packageEligibilityLabel, } from "@proovra/shared";
...
function lifecycleIsTerminal(state) {
    return state === "DESTROYED";
}
// -----------------------------------------------------------------------------
// Warning derivation
// -----------------------------------------------------------------------------
function deriveWarnings(snap) {
    const out = [];
    ...
    if (snap.destruction.activeReviewId && snap.destruction.activeReviewStatus) {
        if (["PENDING", "UNDER_REVIEW", "DEFERRED", "APPROVED"].includes(snap.destruction.activeReviewStatus)) {
            out.push({
    ...
    if (snap.immutableStorage.driftDetected) {
        out.push({
            ...
            label: "Storage governance drift — database immutability flag and object-lock state disagree (storage state only; not a content-integrity finding)",
            severity: "HIGH",
        });
    }
    ...
    if (snap.review.activeEscalationId) {
        out.push({
            ...
            label: `Open reviewer escalation (${(snap.review.activeEscalationSeverity ?? "WARNING").toLowerCase()})`,
            severity: snap.review.activeEscalationSeverity === "CRITICAL"
                ? "CRITICAL"
                : "HIGH",
        });
    }
...
export async function buildGovernanceSnapshot(input, client = defaultPrisma) {
    ...
    const [directHoldCount, caseHoldCount, retentionPolicyVersion, activeDestructionReview, workflow, openIncidents, activeEscalation,] = await Promise.all([
    ...
    if (workflow?.activeEscalationId) {
        ...
        if (row)
            escalation = { id: row.id, severity: row.severity };
    }
    ...
    const driftIncident = openIncidents.find((i) => i.runbookSlug === IMMUTABLE_DRIFT_RUNBOOK) ?? null;
    ...
    const exportDecision = canonicalEvaluateExportEligibility({
        ...
        destructionReviewStatus: activeDestructionReview?.status ?? null,
    });
    ...
    const retentionExpired = evidence.retentionUntilUtc != null &&
