...
import { ANALYTICS_WINDOWS, GOVERNANCE_ANALYTICS_METRICS, analyticsWindowToMilliseconds, } from "@proovra/shared";
import { prisma as defaultPrisma } from "../../db.js";
function normalizeWindow(w) {
    if (typeof w === "string" && ANALYTICS_WINDOWS.includes(w)) {
        return w;
    }
    return "24h";
}
export async function computeGovernanceAnalytics(input, client = defaultPrisma) {
    const window = normalizeWindow(input.window);
    const windowMs = analyticsWindowToMilliseconds(window);
    const since = new Date(Date.now() - windowMs);
    const [destructionQueueDepth, destructionBlockedCount, destructionExecutedCount, retentionConflictsRows, retentionExpirationVelocity, holdPressure, lifecycleTransitions, lifecycleBlockedTransitions, governanceIncidentOpen, immutableDriftCount, exportBlocks, reviewOverdue, incidentSeverityRows, notificationSeverityRows, lifecycleByStateRows, destructionByStatusRows, immutableDriftRows, topRetentionConflictsRows, topReconciliationRunsRows,] = await Promise.all([
    ...
    const retentionConflictCount = retentionConflictsRows.filter((r) => r._count._all > 1).length;
    const metrics = {
    ...
    return {
        ...
        lifecycleByState: Object.fromEntries(lifecycleByStateRows.map((r) => [r.lifecycleState, r._count._all])),
        destructionByStatus: Object.fromEntries(destructionByStatusRows.map((r) => [r.status, r._count._all])),
        immutableDriftByOutcome: Object.fromEntries(immutableDriftRows.map((r) => [r.outcome, r._count._all])),
