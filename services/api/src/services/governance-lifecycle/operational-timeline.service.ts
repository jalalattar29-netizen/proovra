...
export type TimelineEntry = {
  /** Source row id. */
  id: string;
  /** Which underlying stream this came from. */
  kind: TimelineEntryKind;
  /** Specific event type within the stream (e.g. "hold_placed",
   *  "escalation_resolved", "operational_incident_open"). */
  eventType: string;
  /** Bounded operator-readable label. */
  label: string;
  ...
  /** Optional bounded safe summary (operator-readable, no PII). */
...
function lifecycleLabel(eventType: string, toState: string | null): string {
  switch (eventType) {
...
function lifecycleSeverity(eventType: string): TimelineEntry["severity"] {
  switch (eventType) {
    case "destruction_executed":
      return "HIGH";
    case "hold_placed":
...
function reviewLabel(eventType: string, toStatus: string | null): string {
  switch (eventType) {
    case "ASSIGNED":
...
function reviewSeverity(eventType: string): TimelineEntry["severity"] {
  switch (eventType) {
    case "SLA_BREACHED":
    case "ESCALATED":
    case "REJECTED":
...
export async function buildOperationalTimeline(
  ...
  for (const evt of reviewEvents) {
    ...
    const nextValueStatus =
      evt.nextValue &&
      typeof evt.nextValue === "object" &&
      !Array.isArray(evt.nextValue) &&
      typeof (evt.nextValue as Record<string, unknown>).status === "string"
        ? ((evt.nextValue as Record<string, unknown>).status as string)
        : null;
    entries.push({
      id: evt.id,
      kind: "review",
  ...
  entries.sort((a, b) =>
    a.occurredAtUtc < b.occurredAtUtc ? 1 : a.occurredAtUtc > b.occurredAtUtc ? -1 : 0,
  );
