...
import { COLLABORATION_TEAM_ASSIGNMENT_PRIORITIES, COLLABORATION_TEAM_ASSIGNMENT_STATUSES, COLLABORATION_TEAM_ASSIGNMENT_TARGETS, COLLABORATION_TEAM_INVITE_CHANNELS, COLLABORATION_TEAM_INVITE_TOKEN_PREFIX, COLLABORATION_TEAM_INVITE_TOKEN_RANDOM_BYTES, COLLABORATION_TEAM_ROLES, COLLABORATION_TEAM_TYPES, collaborationTeamRoleHasPermission, getCollaborationTeamPlanLimits, } from "@proovra/shared";
...
const E = {
    notFound: (what) => new CollaborationTeamError("team_not_found", `${what} not found.`, 404),
    forbidden: (perm) => new CollaborationTeamError("team_forbidden", `Permission required: ${perm}.`, 403),
    invalid: (msg) => new CollaborationTeamError("team_invalid", msg, 400),
    conflict: (msg) => new CollaborationTeamError("team_conflict", msg, 409),
    rateLimited: (msg) => new CollaborationTeamError("team_rate_limited", msg, 429),
    planLimit: (msg) => new CollaborationTeamError("team_plan_limit", msg, 402),
};
// =============================================================================
// Token helpers
// =============================================================================
function generateInviteToken() {
    ...
    const raw = COLLABORATION_TEAM_INVITE_TOKEN_PREFIX + base32Encode(buf).replace(/=+$/, "");
...
function validateName(name) {
    ...
    if (trimmed.length === 0)
        throw E.invalid("Team name is required.");
...
function validateTeamType(t) {
    if (!t)
        return "GENERAL";
    if (COLLABORATION_TEAM_TYPES.includes(t))
        return t;
    throw E.invalid(`team_type must be one of ${COLLABORATION_TEAM_TYPES.join(", ")}.`);
}
function validateRole(r) {
    if (!r)
        return "MEMBER";
    if (COLLABORATION_TEAM_ROLES.includes(r))
        return r;
    throw E.invalid(`role must be one of ${COLLABORATION_TEAM_ROLES.join(", ")}.`);
}
function validateChannel(c) {
    if (!c || !COLLABORATION_TEAM_INVITE_CHANNELS.includes(c))
        throw E.invalid("channel must be EMAIL, SMS, or LINK.");
    return c;
}
function validateAssignmentTarget(t) {
    if (!t ||
        !COLLABORATION_TEAM_ASSIGNMENT_TARGETS.includes(t))
...
function validatePriority(p) {
    if (!p)
        return "NORMAL";
    if (COLLABORATION_TEAM_ASSIGNMENT_PRIORITIES.includes(p))
        return p;
    throw E.invalid(`priority must be one of ${COLLABORATION_TEAM_ASSIGNMENT_PRIORITIES.join(", ")}.`);
}
function validateAssignmentStatus(s) {
    if (!s ||
        !COLLABORATION_TEAM_ASSIGNMENT_STATUSES.includes(s))
        throw E.invalid(`status must be one of ${COLLABORATION_TEAM_ASSIGNMENT_STATUSES.join(", ")}.`);
    return s;
}
...
function validatePhone(p) {
    if (!p || typeof p !== "string")
        throw E.invalid("Phone is required.");
...
// Permission helpers
// =============================================================================
async function requireMemberWithPermission(client, teamId, actorUserId, permission) {
    const team = await client.collaborationTeam.findUnique({
        where: { id: teamId },
        select: { id: true, workspaceId: true, status: true },
    });
    if (!team)
        throw E.notFound("Team");
...
async function resolveWorkspacePlan(client, workspaceId) {
    const ws = await client.team.findUnique({
        where: { id: workspaceId },
        select: { billingPlan: true },
    });
    return (ws?.billingPlan ?? "FREE");
}
// =============================================================================
// Public API
// =============================================================================
...
// Teams CRUD
// -----------------------------------------------------------------------------
export async function createCollaborationTeam(input, client = defaultPrisma) {
    ...
    if (existingActive >= limits.maxTeams) {
        throw E.planLimit(`Plan ${plan} allows up to ${limits.maxTeams} active team(s). Archive or upgrade to add more.`);
    }
...
export async function listCollaborationTeams(input, client = defaultPrisma) {
    ...
    return teams.map((t) => ({
        ...
        pendingInviteCount: t._count.invites ?? 0,
        openAssignmentCount: t._count.assignments ?? 0,
        lastActivityAt: t.activity[0]?.createdAt ?? null,
        viewerRole: t.members[0] && t.members[0].status === "ACTIVE"
            ? t.members[0].role
            : null,
    }));
}
export async function getCollaborationTeamDetail(input, client = defaultPrisma) {
    const team = await client.collaborationTeam.findUnique({
    ...
    if (!team)
        throw E.notFound("Team");
    const viewer = team.members.find((m) => m.userId === input.actorUserId && m.status === "ACTIVE");
    if (!viewer)
        throw E.forbidden("team.read");
    return {
        id: team.id,
        ...
        assignmentCount: team._count.assignments,
    };
}
export async function updateCollaborationTeam(input, client = defaultPrisma) {
    const { team } = await requireMemberWithPermission(client, input.teamId, input.actorUserId, "team.update_settings");
    const data = {};
    const meta = {};
    let renamed = false;
...
export async function archiveCollaborationTeam(input, client = defaultPrisma) {
    const { team } = await requireMemberWithPermission(client, input.teamId, input.actorUserId, "team.archive");
...
// Members
// -----------------------------------------------------------------------------
export async function addExistingMember(input, client = defaultPrisma) {
    const { team } = await requireMemberWithPermission(client, input.teamId, input.actorUserId, "team.member.invite");
    ...
    if (!ws)
        throw E.invalid("Target user is not an active member of the parent workspace.");
    ...
    if (activeCount >= limits.maxMembersPerTeam)
        throw E.planLimit(`Plan ${plan} allows up to ${limits.maxMembersPerTeam} active members per team.`);
...
export async function changeMemberRole(input, client = defaultPrisma) {
    const { team, role: actorRole } = await requireMemberWithPermission(client, input.teamId, input.actorUserId, "team.member.change_role");
    ...
    if (member.role === "LEAD" && newRole !== "LEAD") {
        ...
        if (leadCount <= 1)
            throw E.conflict("Cannot demote the last LEAD. Transfer leadership first.");
    }
    await client.$transaction(async (tx) => {
        ...
        await recordActivity(tx, {
            ...
            workspaceId: team.workspaceId,
            actorUserId: input.actorUserId,
            eventType: member.role !== "LEAD" && newRole === "LEAD"
                ? "LEAD_TRANSFERRED"
                : "MEMBER_ROLE_CHANGED",
            targetType: "USER",
...
export async function suspendMember(input, client = defaultPrisma) {
    const { team } = await requireMemberWithPermission(client, input.teamId, input.actorUserId, "team.member.suspend");
    const member = await client.collaborationTeamMember.findUnique({
        where: { id: input.memberId },
...
export async function removeMember(input, client = defaultPrisma) {
    const { team } = await requireMemberWithPermission(client, input.teamId, input.actorUserId, "team.member.remove");
    const member = await client.collaborationTeamMember.findUnique({
        where: { id: input.memberId },
...
async function enforceInviteLimits(client, args) {
    ...
    if (pending >= limits.maxPendingInvitesPerTeam)
        throw E.planLimit(`Plan ${plan} allows up to ${limits.maxPendingInvitesPerTeam} pending invites per team.`);
    ...
    if (recent >= limits.maxInvitesPer24h)
        throw E.rateLimited(`You've issued ${recent} invites in the last 24h. Plan limit: ${limits.maxInvitesPer24h}.`);
}
export async function createEmailInvite(input, client = defaultPrisma) {
    const { team } = await requireMemberWithPermission(client, input.teamId, input.actorUserId, "team.member.invite");
    ...
    const expiresAtUtc = new Date(Date.now() + (input.expiresInMs ?? DEFAULT_INVITE_TTL_MS));
...
export async function createSmsInvite(input, client = defaultPrisma) {
    const { team } = await requireMemberWithPermission(client, input.teamId, input.actorUserId, "team.member.invite");
...
export async function createLinkInvite(input, client = defaultPrisma) {
    const { team } = await requireMemberWithPermission(client, input.teamId, input.actorUserId, "team.member.invite");
    const role = validateRole(input.role);
    const plan = await resolveWorkspacePlan(client, team.workspaceId);
    const limits = getCollaborationTeamPlanLimits(plan);
...
export async function revokeInvite(input, client = defaultPrisma) {
    const { team } = await requireMemberWithPermission(client, input.teamId, input.actorUserId, "team.invite.revoke");
    const invite = await client.collaborationTeamInvite.findUnique({
        where: { id: input.inviteId },
        select: { id: true, teamId: true, status: true },
    });
    if (!invite || invite.teamId !== input.teamId)
        throw E.notFound("Invite");
...
export async function recordInviteDeliveryResult(input, client = defaultPrisma) {
 ...
 */
export async function acceptInvite(input, client = defaultPrisma) {
    ...
    if (invite.status === "ACCEPTED" &&
        ...
        if (existing)
            return { teamId: invite.teamId, memberId: existing.id };
    }
    ...
    if (!wsMembership)
        throw E.invalid("Join the parent workspace first; the team invite cannot be accepted standalone.");
    ...
    if (activeCount >= limits.maxMembersPerTeam)
        throw E.planLimit(`This team has reached its member limit (${limits.maxMembersPerTeam}).`);
    ...
    const result = await client.$transaction(async (tx) => {
        ...
        await tx.collaborationTeamInvite.update({
            ...
            data: {
                ...
                status: nextStatus,
                acceptedByUserId: nextStatus === "ACCEPTED" ? input.actorUserId : invite.acceptedByUserId,
                acceptedAtUtc: nextStatus === "ACCEPTED" ? new Date() : null,
            },
        });
        await recordActivity(tx, {
            teamId: invite.teamId,
...
// Activity feed
// -----------------------------------------------------------------------------
export async function listTeamActivity(input, client = defaultPrisma) {
    await requireMemberWithPermission(client, input.teamId, input.actorUserId, "team.activity.read");
...
// Assignments
// -----------------------------------------------------------------------------
export async function createAssignment(input, client = defaultPrisma) {
    const { team } = await requireMemberWithPermission(client, input.teamId, input.actorUserId, "team.assignment.create");
...
export async function updateAssignment(input, client = defaultPrisma) {
    ...
    const isCompleteOnly = input.status === "COMPLETED" &&
        input.priority === undefined &&
        input.assigneeUserId === undefined &&
        input.dueAtUtc === undefined &&
        input.note === undefined;
    if (isCompleteOnly) {
        await requireMemberWithPermission(client, input.teamId, input.actorUserId, "team.assignment.complete");
    }
    else {
        await requireMemberWithPermission(client, input.teamId, input.actorUserId, "team.assignment.reassign");
    }
    const data = {};
    const eventEmissions = [];
    const meta = { assignmentId: assignment.id };
...
export async function listAssignments(input, client = defaultPrisma) {
    await requireMemberWithPermission(client, input.teamId, input.actorUserId, "team.read");
