...
import { COLLABORATION_TEAM_BILLING_ERROR_HTTP_STATUS, COLLABORATION_TEAM_BILLING_UPGRADE_CTA, getCollaborationTeamPlanLimits, } from "@proovra/shared";
...
export const SUBSCRIPTION_GRACE_PERIOD_MS = SUBSCRIPTION_GRACE_PERIOD_DAYS * 24 * 60 * 60 * 1000;
// =============================================================================
// Error class
// =============================================================================
/**
 ...
 * never drift.
 */
async function resolveUserPlan(client, userId) {
 ...
 */
async function resolveCollaborationTeamOwnerUserId(client, teamId) {
 ...
 */
export async function assertCanCreateCollaborationTeam(ownerUserId, client = defaultPrisma) {
 ...
 */
export async function assertCollaborationTeamMemberLimit(teamId, addingCount = 1, client = defaultPrisma) {
 ...
 */
export async function assertCanInviteCollaborationTeamMember(teamId, channel, client = defaultPrisma) {
    const ownerUserId = await resolveCollaborationTeamOwnerUserId(client, teamId);
    const plan = await resolveUserPlan(client, ownerUserId);
    const limits = getCollaborationTeamPlanLimits(plan);
    // -------- channel gate --------
 ...
 */
export async function assertCanCreateGuest(teamId, client = defaultPrisma) {
    const ownerUserId = await resolveCollaborationTeamOwnerUserId(client, teamId);
    const plan = await resolveUserPlan(client, ownerUserId);
    const limits = getCollaborationTeamPlanLimits(plan);
    // Bounded plan-tier derivation. PRO+ unlocks the guest surface.
    const guestsAllowed = plan === prismaPkg.PlanType.PRO || plan === prismaPkg.PlanType.TEAM;
 ...
 */
export async function assertSubscriptionActiveOrGraceAllowed(userId, client = defaultPrisma) {
    ...
    if (pastDueSubscription) {
        const periodEndMs = pastDueSubscription.currentPeriodEnd?.getTime() ?? null;
        const inGracePeriod = periodEndMs !== null &&
    ...
    const status = matchingPlanRow.status;
    throwBillingError({
...
export { COLLABORATION_TEAM_BILLING_ERROR_CODES, COLLABORATION_TEAM_BILLING_ERROR_HTTP_STATUS, COLLABORATION_TEAM_BILLING_UPGRADE_CTA, isCollaborationTeamBillingErrorCode, } from "@proovra/shared";
