...
const ALLOWED_TRANSITIONS: Record<PublicVerifyState, PublicVerifyState[]> = {
...
async function applyTransition(
  evidenceId: string,
  from: PublicVerifyState,
  to: PublicVerifyState,
  patch: {
    publicVerifyPublishedAtUtc?: Date | null;
    publicVerifyPublishedByUserId?: string | null;
    publicVerifySuspendedAtUtc?: Date | null;
    publicVerifySuspendedByUserId?: string | null;
...
export async function publishPublicVerify(
...
export async function unpublishPublicVerify(
  input: PublicationActionInput,
  client: PrismaClient = defaultPrisma,
): Promise<DbEvidence> {
  const ev = await client.evidence.findUnique({
...
export async function suspendPublicVerify(
  input: PublicationActionInput,
  client: PrismaClient = defaultPrisma,
): Promise<DbEvidence> {
...
export async function restorePublicVerify(
  input: PublicationActionInput,
  client: PrismaClient = defaultPrisma,
): Promise<DbEvidence> {
...
export function projectPublicationState(ev: {
  publicVerifyState: PublicVerifyState;
  publicVerifyPublishedAtUtc: Date | null;
  publicVerifyPublishedByUserId: string | null;
  publicVerifySuspendedAtUtc: Date | null;
  publicVerifySuspendedByUserId: string | null;
  ...
  // Reason omitted in operator-facing projection that may be shared
  // workspace-wide; routes that include the reason explicitly opt in.
} {
  return {
