 ...
 */
import { DEPARTMENT_MEMBERSHIP_ROLES, DEPARTMENT_MEMBERSHIP_STATES, } from "@proovra/shared";
 ...
 *     same workspace.
 */
export async function grantDepartmentMembership(input) {
    const role = input.role ?? "MEMBER";
    if (!DEPARTMENT_MEMBERSHIP_ROLES.includes(role)) {
 ...
 */
export async function revokeDepartmentMembership(input) {
    const prisma = input.prisma ?? defaultPrisma;
 ...
 */
export async function listDepartmentMemberships(input) {
    const prisma = input.prisma ?? defaultPrisma;
