 ...
 */
export async function resolveUserDepartmentScope(input) {
    ...
    const allowedDepartmentIds = Array.from(new Set(memberships.map((m) => m.departmentId)));
    return {
        teamId: input.teamId,
        userId: input.userId,
        unrestricted: false,
 ...
 */
export function buildDepartmentScopeWhere(envelope) {
    if (envelope.unrestricted)
        return undefined;
 ...
 */
export function buildStrictDepartmentScopeWhere(envelope) {
    if (envelope.unrestricted)
        return undefined;
 ...
 */
export function assertDepartmentAccess(envelope, departmentId) {
    if (envelope.unrestricted)
        return { ok: true };
 ...
 */
export function auditCrossDepartmentAccess(input) {
