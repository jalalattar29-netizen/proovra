...
export async function reconcileRetention(input = {}, client = defaultPrisma) {
    ...
    for (const ev of candidates) {
        try {
            ...
            if (ev.retentionReconciliationFlaggedAtUtc &&
...
export async function listRetentionCandidates(input, client = defaultPrisma) {
    ...
    return rows
        ...
        .map((r) => ({
        id: r.id,
        retentionUntilUtc: r.retentionUntilUtc.toISOString(),
        retentionPolicySource: r.retentionPolicySource,
        flaggedAtUtc: r.retentionReconciliationFlaggedAtUtc?.toISOString() ?? null,
        caseId: r.caseId,
