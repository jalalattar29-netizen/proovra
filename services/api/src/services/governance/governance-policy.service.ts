...
export async function createPolicy(
  ...
  if (!(GOVERNANCE_POLICY_KINDS as ReadonlyArray<string>).includes(input.kind)) {
    return { ok: false, denial: "POLICY_REJECTED" };
  }
  if (!(GOVERNANCE_POLICY_ENFORCEMENT_MODES as ReadonlyArray<string>).includes(input.enforcementMode)) {
    return { ok: false, denial: "POLICY_REJECTED" };
  }
...
): Promise<{ ok: boolean; assignmentId?: string; denial?: "POLICY_REJECTED" }> {
  if (!(GOVERNANCE_POLICY_SCOPES as ReadonlyArray<string>).includes(input.scope)) {
    return { ok: false, denial: "POLICY_REJECTED" };
  }
  const prisma = input.prisma ?? defaultPrisma;
...
export async function listPolicies(input: {
  ...
  return rows.map((p) => ({
    id: p.id,
    kind: p.kind as GovernancePolicyKind,
    ...
    name: p.name,
    summary: p.summary ?? "",
    state: p.state as GovernancePolicyState,
    enforcementMode: p.enforcementMode as GovernancePolicyEnforcementMode,
...
export async function listPolicyAssignments(input: {
  ...
  scope?: GovernancePolicyScope;
  scopeTargetId?: string;
}): Promise<ReadonlyArray<GovernancePolicyAssignmentProjection>> {
  const prisma = input.prisma ?? defaultPrisma;
  ...
  return rows.map((a) => ({
    ...
    scope: ((a.scope ?? a.scopeKind) as GovernancePolicyScope),
...
export async function listPolicyAudit(input: {
  ...
  policyId?: string;
  limit?: number;
}): Promise<ReadonlyArray<GovernancePolicyAuditRow>> {
  const prisma = input.prisma ?? defaultPrisma;
...
export async function resolveEffectivePolicies(input: {
  ...
  const prisma = input.prisma ?? defaultPrisma;
  const scopes: Array<{ scope: GovernancePolicyScope; id: string }> = [];
  if (input.organizationId) scopes.push({ scope: "ORGANIZATION", id: input.organizationId });
  if (input.departmentId) scopes.push({ scope: "DEPARTMENT", id: input.departmentId });
  if (input.workspaceId) scopes.push({ scope: "WORKSPACE", id: input.workspaceId });
  ...
  const ordered: Array<typeof assignments[number]> = [];
  for (const s of scopes) {
    ordered.push(
      ...assignments.filter((a) => a.scope === s.scope && a.scopeTargetId === s.id),
    );
  }
  const byKey = new Map<string, GovernancePolicyProjection>();
  for (const a of ordered) {
    if (a.policy.state !== "ACTIVE") continue;
    if (input.kind && a.policy.kind !== input.kind) continue;
    const key = `${a.policy.kind}:${a.policy.slug}`;
    const proj: GovernancePolicyProjection = {
      id: a.policy.id,
      kind: a.policy.kind as GovernancePolicyKind,
      // R7-governance: same coalesce pattern as listPolicies — non-null projection contract held.
      slug: a.policy.slug ?? "",
      name: a.policy.name,
      summary: a.policy.summary ?? "",
      state: a.policy.state as GovernancePolicyState,
      enforcementMode: a.policy.enforcementMode as GovernancePolicyEnforcementMode,
      rule: (a.policy.rule as Record<string, unknown>) ?? {},
      version: a.policy.version ?? 1,
      createdByUserId: a.policy.createdByUserId ?? "",
      createdAtUtc: a.policy.createdAt.toISOString(),
    };
    if (!byKey.has(key) || a.isOverride) byKey.set(key, proj);
  }
...
function _assertEnumsIntact(): void {
  const _k: GovernancePolicyKind = "SECURITY";
  void _k;
