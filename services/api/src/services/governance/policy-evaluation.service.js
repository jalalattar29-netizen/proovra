 ...
 * dedicated engine.
 */
import { POLICY_EVALUATION_DECISIONS, } from "@proovra/shared";
import { prisma as defaultPrisma } from "../../db.js";
import { emitPolicyAudit, resolveEffectivePolicies, } from "./governance-policy.service.js";
// ===========================================================================
// Internal helpers
// ===========================================================================
const DECISION_STRENGTH = {
    ALLOW: 0,
    WARN: 1,
    BLOCK: 2,
};
 ...
 * is the consequence.
 */
export function applyEnforcementMode(mode, reason, policyId) {
    switch (mode) {
        case "BLOCK":
 ...
 */
function combine(running, candidate) {
...
function ruleBool(rule, key) {
    return rule[key] === true;
}
function ruleNumber(rule, key) {
    const v = rule[key];
    if (typeof v === "number" && Number.isFinite(v))
        return v;
    return null;
}
function ruleStringArray(rule, key) {
    const v = rule[key];
 ...
 */
const CONFIDENCE_BAND_ORDER = ["LOW", "MEDIUM", "HIGH"];
function confidenceBandRank(band) {
    if (!band)
        return -1;
 ...
 */
async function emitEvaluationAudit(prisma, teamId, surface, result, context) {
    ...
    const baseReason = boundReason(`${surface}:${result.decision.toLowerCase()}${result.reason ? `:${result.reason}` : ""}${context ? `:${context}` : ""}`);
...
async function loadEffective(prisma, teamId, kind, scope) {
    try {
...
export async function evaluateSecurityPolicy(input) {
    ...
    for (const policy of policies) {
        ...
        if (ruleBool(rule, "requireMfa") && !input.mfaSatisfied) {
            result = combine(result, applyEnforcementMode(policy.enforcementMode, "mfa_required", policy.id));
            continue;
        }
        if (ruleBool(rule, "requireSaml") && !input.samlAuthenticated) {
            result = combine(result, applyEnforcementMode(policy.enforcementMode, "saml_required", policy.id));
            continue;
        }
        const allowedActions = ruleStringArray(rule, "allowedActions");
        if (allowedActions && !allowedActions.includes(input.action)) {
            result = combine(result, applyEnforcementMode(policy.enforcementMode, "action_not_allowed", policy.id));
        }
    }
    await emitEvaluationAudit(prisma, input.teamId, "security", result, `action=${boundReason(input.action) ?? "unknown"}`);
    return result;
}
export async function evaluateRetentionPolicy(input) {
    const prisma = input.prisma ?? defaultPrisma;
    ...
    for (const policy of policies) {
        ...
        if (minDays !== null &&
            ageDays !== null &&
            ageDays < minDays) {
            result = combine(result, applyEnforcementMode(policy.enforcementMode, "min_retention_not_met", policy.id));
        }
        if (ruleBool(rule, "requireLegalHoldCheck") &&
            hasUnresolvedLegalHold) {
            result = combine(result, applyEnforcementMode(policy.enforcementMode, "legal_hold_active", policy.id));
        }
    }
    await emitEvaluationAudit(prisma, input.teamId, "retention", result, `action=${input.action.toLowerCase()}`);
    return result;
}
export async function evaluateReviewPolicy(input) {
    const prisma = input.prisma ?? defaultPrisma;
    ...
    try {
        ...
        for (const d of decisions)
            reviewers.add(d.reviewerUserId);
    ...
    for (const policy of policies) {
        ...
        if (ruleBool(rule, "requireQc") && isApprove && !qcPassed) {
            result = combine(result, applyEnforcementMode(policy.enforcementMode, "qc_pass_required", policy.id));
        }
        if (ruleBool(rule, "requireDualApproval") && approverCount < 2) {
            result = combine(result, applyEnforcementMode(policy.enforcementMode, "dual_approval_required", policy.id));
        }
    }
    await emitEvaluationAudit(prisma, input.teamId, "review", result, `decision=${boundReason(input.decision) ?? "unknown"}`);
    return result;
}
export async function evaluateIntelligencePolicy(input) {
    const prisma = input.prisma ?? defaultPrisma;
    ...
    if (input.evidenceId) {
        try {
            const rec = await prisma.mediaIntelligenceRecord
                ...
                .catch(() => null);
            recordBand = rec?.finalConfidenceBand ?? null;
        }
        catch {
    ...
    for (const policy of policies) {
        ...
        if (disallowedProviders && disallowedProviders.includes(input.provider)) {
            result = combine(result, applyEnforcementMode(policy.enforcementMode, "provider_disallowed", policy.id));
        }
        const disallowedOperations = ruleStringArray(rule, "disallowedOperations");
        if (disallowedOperations &&
            disallowedOperations.includes(input.operation)) {
            result = combine(result, applyEnforcementMode(policy.enforcementMode, "operation_disallowed", policy.id));
        }
        const minBand = ruleString(rule, "minConfidenceBand");
        if (minBand &&
            recordBand !== null &&
            confidenceBandRank(recordBand) < confidenceBandRank(minBand)) {
            result = combine(result, applyEnforcementMode(policy.enforcementMode, "confidence_below_band", policy.id));
        }
    }
    await emitEvaluationAudit(prisma, input.teamId, "intelligence", result, `provider=${boundReason(input.provider) ?? "unknown"}:op=${boundReason(input.operation) ?? "unknown"}`);
    return result;
}
export async function evaluateVerificationPolicy(input) {
    const prisma = input.prisma ?? defaultPrisma;
    ...
    for (const policy of policies) {
        const rule = ruleObject(policy);
        if (ruleBool(rule, "requireTrustReferences") &&
            publishedTrustArticleCount === 0) {
            result = combine(result, applyEnforcementMode(policy.enforcementMode, "trust_references_missing", policy.id));
        }
        if (ruleBool(rule, "blockedPublicExposure") &&
            input.action === "PUBLIC_VERIFY_EXPOSE") {
            result = combine(result, applyEnforcementMode(policy.enforcementMode, "public_exposure_blocked", policy.id));
        }
    }
    await emitEvaluationAudit(prisma, input.teamId, "verification", result, `action=${input.action.toLowerCase()}`);
    return result;
}
/**
 ...
 */
export async function evaluateRedactionPolicy(input) {
    const prisma = input.prisma ?? defaultPrisma;
    const result = {
        decision: "ALLOW",
    ...
    await emitEvaluationAudit(prisma, input.teamId, "redaction", result, null);
    return result;
}
// ===========================================================================
// Compile-time guard — keeps the bounded decision vocabulary honest.
// ===========================================================================
function _assertVocabularyIntact() {
    const _d = "ALLOW";
    void _d;
