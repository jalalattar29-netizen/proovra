...
import { enforceSensitiveAction, } from "../governance.service.js";
/**
 ...
 * verbatim).
 */
export async function runDestructiveActionGate(input, client = defaultPrisma) {
    ...
    const decision = await enforceSensitiveAction(input.action, {
        teamId: input.evidence.teamId,
    ...
    const custodyEventType = decision.code === "DELETE_BLOCKED_BY_LEGAL_HOLD"
    ...
    // mapping in evidence.routes.ts exactly.
    const statusCode = decision.code === "GOVERNANCE_CHECK_FAILED"
        ? 503
        : decision.code === "DELETE_RESTRICTED_TO_ADMIN"
            ? 403
            : 409;
    const message = input.routeLabel === "delete"
