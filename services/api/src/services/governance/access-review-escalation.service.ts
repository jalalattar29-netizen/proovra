...
export type EscalateAccessReviewItemInput = {
  prisma?: PrismaClient;
  teamId: string;
  itemId: string;
  escalatedByUserId: string;
  /** Bounded reason chip — NEVER PII. Truncated to 200 chars. */
  reason?: string | null;
};

export type EscalateAccessReviewItemResult = {
  ok: boolean;
 ...
 */
export async function escalateAccessReviewItem(
  input: EscalateAccessReviewItemInput,
): Promise<EscalateAccessReviewItemResult> {
  ...
  const primary = await emitTrustEvent({
    ...
    payload: { decision: "ESCALATED" satisfies AccessReviewItemDecision },
  });
...
export type ListEscalatedItemsInput = {
  prisma?: PrismaClient;
  teamId: string;
  limit?: number;
};
 ...
 */
export async function listEscalatedItems(
  input: ListEscalatedItemsInput,
