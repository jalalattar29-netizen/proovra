...
function nextFollowUpAtForStep(step) {
    if (step === 1)
        return addHours(new Date(), 72);
    if (step === 2)
        return addHours(new Date(), 24 * 7);
    return null;
}
function templateKeyForStep(step) {
    if (step === 1)
        return "followup_day_1";
    if (step === 2)
        return "followup_day_3";
    return "followup_day_7";
}
function isClosedStatus(status) {
    return (status === prismaPkg.DemoRequestStatus.QUALIFIED ||
...
export async function sendDemoFollowUpById(params) {
    ...
    if (item.followUpStatus === prismaPkg.DemoFollowUpStatus.STOPPED ||
    ...
    const nextStep = computedStep;
    if ((item.followUpStep ?? 0) > 0 && nextStep < item.followUpStep) {
    ...
    const nextStatus = item.status === prismaPkg.DemoRequestStatus.NEW ||
