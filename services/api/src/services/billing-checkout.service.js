...
import { createPayPalOrder, createPayPalSubscription, createPayPalStorageAddonCheckout as createPayPalStorageAddonCheckoutApi, } from "./paypal.service.js";
import { isPayPalRecurringPlan } from "./paypal-plan-map.service.js";
import { getStorageAddonDefinition } from "./billing.service.js";
import { getPlanPriceCents, getStorageAddonCurrency, getStorageAddonPriceCents, getStripePlanPriceId, getStripeStorageAddonPriceId, resolveCheckoutCurrency, } from "./billing-pricing.service.js";
function appBaseUrl() {
...
export async function createStripeCheckoutSession(params) {
    ...
    if (priceId) {
        ...
        searchParams.append("line_items[0][price_data][product_data][name]", `Proovra ${params.plan}`);
    ...
    if (mode === "subscription") {
        ...
        searchParams.append("subscription_data[metadata][amountCents]", String(amountCents));
        if (params.teamId) {
...
export async function createStripeStorageAddonCheckoutSession(params) {
    ...
    const searchParams = new URLSearchParams();
    searchParams.append("mode", mode);
    searchParams.append("success_url", `${appBase}/billing?success=1&kind=storage-addon`);
    searchParams.append("cancel_url", `${appBase}/billing?canceled=1&kind=storage-addon`);
    searchParams.append("payment_method_types[]", "card");
    searchParams.append("metadata[userId]", params.userId);
    searchParams.append("metadata[storageAddonKey]", params.addonKey);
    searchParams.append("metadata[billingCycle]", prismaPkg.StorageAddonBillingCycle.ONE_TIME);
    ...
    if (priceId) {
        ...
        searchParams.append("line_items[0][price_data][currency]", currency);
        searchParams.append("line_items[0][price_data][product_data][name]", `Proovra Storage Add-on ${params.addonKey}`);
