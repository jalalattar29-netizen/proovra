...
const STORAGE_ADDON_DEFINITIONS = [
    {
...
export function getStorageAddonDefinition(key) {
...
export async function setPersonalPlan(userId, plan) {
    if (plan === prismaPkg.PlanType.TEAM) {
        const err = new Error("TEAM is not a valid personal plan");
        err.statusCode = 409;
...
export async function activateTeamPlan(params) {
    if (params.plan !== prismaPkg.PlanType.TEAM) {
        const err = new Error("Only TEAM can activate a team workspace plan");
        err.statusCode = 409;
    ...
    if (team.ownerUserId !== params.ownerUserId) {
        const err = new Error("Only the team owner can manage this team billing");
        err.statusCode = 403;
        throw err;
    }
...
export async function recordPayment(params) {
    ...
    await trackBillingEvent({
        eventType: params.status === prismaPkg.PaymentStatus.SUCCEEDED
...
export async function upsertSubscription(params) {
    ...
    await trackBillingEvent({
        eventType: params.status === prismaPkg.SubscriptionStatus.CANCELED
...
export async function upsertWorkspaceStorageAddon(params) {
    if (params.billingCycle !== prismaPkg.StorageAddonBillingCycle.ONE_TIME) {
        const err = new Error("Only one-time storage add-ons are supported");
        err.statusCode = 400;
    ...
    const existingBySubscription = params.externalSubscriptionId
        ...
        : null;
    const existingByPayment = !existingBySubscription && params.externalPaymentId
        ? await prisma.workspaceStorageAddon.findUnique({
            where: {
    ...
    const data = {
        ...
        activatedAtUtc: params.status === prismaPkg.WorkspaceStorageAddonStatus.ACTIVE
            ? existing?.activatedAtUtc ?? new Date()
            : existing?.activatedAtUtc ?? null,
        currentPeriodEnd: null,
        expiresAtUtc: params.expiresAtUtc ?? null,
        canceledAtUtc: params.status === prismaPkg.WorkspaceStorageAddonStatus.CANCELED
            ? new Date()
            : null,
    ...
    await trackBillingEvent({
        eventType: params.status === prismaPkg.WorkspaceStorageAddonStatus.ACTIVE
            ? existing
...
export async function cancelWorkspaceStorageAddon(params) {
    ...
    if (!addon) {
        const err = new Error("Storage addon not found");
        err.statusCode = 404;
        throw err;
    }
    if (addon.ownerUserId !== params.ownerUserId) {
        const err = new Error("You are not allowed to manage this storage addon");
        err.statusCode = 403;
        throw err;
    }
...
export async function refreshTeamSeatState(teamId) {
    const team = await prisma.team.findUnique({
        where: { id: teamId },
        select: {
            id: true,
    ...
    if (!team)
        return null;
    const overSeatLimit = team.includedSeats > 0 ? team._count.members > team.includedSeats : false;
    const updated = await prisma.team.update({
        where: { id: teamId },
        data: {
            overSeatLimit,
...
export async function markTeamBillingCanceled(teamId, ownerUserId) {
    return cancelTeamPlan({
        teamId,
        ownerUserId,
    });
