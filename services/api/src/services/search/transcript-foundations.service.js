...
const TRANSCRIPT_VISIBILITY_SCOPE_SET = new Set(TRANSCRIPT_VISIBILITY_SCOPES);
...
export async function recordTranscriptSegment(input, client = defaultPrisma) {
    ...
    try {
        const rows = (await client.$queryRawUnsafe(`INSERT INTO "evidence_transcript_segments" (
       ...
       ON CONFLICT (
         ...
         "updated_at_utc" = NOW(),
         "indexed_at_utc" = NULL
       RETURNING "id"`, input.teamId, input.evidenceId, input.partId, input.segmentIndex, input.startMs, input.endMs, input.speakerId ? input.speakerId.slice(0, 64) : null, input.engine.slice(0, 40), input.languageHint ? input.languageHint.slice(0, 16) : null, safe, input.confidence, input.visibilityScope, redacted));
        const id = rows[0]?.id;
        if (!id) {
...
export async function listTranscriptSegmentsForEvidence(input, client = defaultPrisma) {
    const limit = Math.min(Math.max(input.limit ?? 100, 1), 2000);
    const raw = (await client.$queryRawUnsafe(`SELECT "id", "team_id", "evidence_id", "part_id", "segment_index",
            "start_ms", "end_ms", "speaker_id", "engine", "language_hint",
            "text", "confidence", "visibility_scope", "redacted",
...
export async function listIndexableTranscriptText(input, client = defaultPrisma) {
    ...
    const raw = (await client.$queryRawUnsafe(`SELECT "text"
       ...
       WHERE "team_id" = $1
         ...
         AND "redacted" = FALSE
       ORDER BY "part_id" NULLS FIRST, "segment_index" ASC`, input.teamId, input.evidenceId));
    return raw.map((r) => r.text);
}
export async function getTranscriptIndexingLagSeconds(teamId, client = defaultPrisma) {
