...
export function getDailyChunkCap(): number {
  return Math.floor(
    readEnvNumber(
      "SEMANTIC_MAX_CHUNKS_PER_WORKSPACE_PER_DAY",
      SEMANTIC_DEFAULT_MAX_CHUNKS_PER_DAY,
    ),
  );
}
...
export function getMonthlyBudgetEur(): number {
  return readEnvNumber(
    "SEMANTIC_MONTHLY_BUDGET_EUR",
    SEMANTIC_DEFAULT_MONTHLY_BUDGET_EUR,
  );
}

export function getUsdPerToken(): number {
  return readEnvNumber("SEMANTIC_USD_PER_TOKEN", SEMANTIC_DEFAULT_USD_PER_TOKEN);
}

export function getEurPerUsd(): number {
...
export function dateUtcKey(at: Date = new Date()): Date {
  const utc = new Date(
...
export function firstOfMonthUtc(at: Date = new Date()): Date {
...
export type SemanticUsageSummary = {
  today: { chunks: number; eur: number };
  monthToDate: { chunks: number; eur: number };
  dailyCap: number;
  monthlyBudgetEur: number;
};

export type CanEmbedDecision =
  | { allowed: true; remaining: number; reason: null }
  | {
      allowed: false;
      remaining: number;
      reason: "DAILY_CAP" | "MONTHLY_BUDGET";
    };
 ...
 */
export async function canEmbedMore(
  client: PrismaClient,
  args: { workspaceId: string; additionalChunks: number },
): Promise<CanEmbedDecision> {
  ...
  try {
    ...
    const monthEurMicros = monthRows.reduce(
 ...
 */
export async function recordSemanticChunkEmbedded(
  client: PrismaClient,
  args: {
    workspaceId: string;
    chunkId?: string | null;
 ...
 */
export async function getSemanticUsageSummary(
  client: PrismaClient,
  workspaceId: string,
): Promise<SemanticUsageSummary> {
  const dailyCap = getDailyChunkCap();
  ...
  try {
    ...
    const monthChunks = monthRows.reduce(
      (acc, r) => acc + (r.chunksEmbedded ?? 0),
      0,
    );
...
// Convenience for tests + the backfill summary surface.
export type { PrismaClient as SemanticBudgetPrismaClient } from "@prisma/client";
