...
import { resolveEmbeddingProviderFromEnv, logEmbeddingFailure, EmbeddingProviderError, } from "./embedding-provider.js";
import { canEmbedMore, estimateEurFromTokens, estimateTokensFromChars, recordSemanticChunkEmbedded, } from "./semantic-budget.service.js";
/**
...
export async function runSemanticBackfill(input, client = defaultPrisma) {
    ...
    for (let batch = 0; batch < maxBatches; batch += 1) {
      ...
      `;
            ...
            logEmbeddingFailure({
                status: "failed",
                errorCode: err instanceof Error ? `BACKFILL_SELECT:${err.name}` : "BACKFILL_SELECT",
                workspaceId: input.workspaceId,
            });
            break;
        }
        ...
        let vectors;
        try {
            vectors = await provider.embedBatch(candidates.map((c) => c.chunkText));
        }
        catch (err) {
