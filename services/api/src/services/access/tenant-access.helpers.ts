...
async function requireResourceAccess(
  ...
  args: {
    ...
    resourceKind: string;
    /**
     * Server-side resolver: fetch the resource and return its teamId.
     * Returning null means the resource does not exist.
     */
    resolveTeamId: (resourceId: string) => Promise<string | null>;
  },
): Promise<TenantAccessGrant | null> {
...
export async function requireActiveSpaceAccess(
  req: FastifyRequest,
  reply: FastifyReply,
  args: { teamId: string; permission: Permission },
): Promise<TenantAccessGrant | null> {
  const outcome = await authorizeOrFail(req, reply, {
    teamId: args.teamId,
    permission: args.permission,
...
export async function requireEvidenceAccess(
  req: FastifyRequest,
  reply: FastifyReply,
  args: { evidenceId: string; permission: Permission },
): Promise<TenantAccessGrant | null> {
...
export async function requireCaseAccess(
  req: FastifyRequest,
  reply: FastifyReply,
  args: { caseId: string; permission: Permission },
): Promise<TenantAccessGrant | null> {
  return requireResourceAccess(req, reply, {
...
export async function requireReportAccess(
  req: FastifyRequest,
  reply: FastifyReply,
  args: { reportId: string; permission: Permission },
): Promise<TenantAccessGrant | null> {
  return requireResourceAccess(req, reply, {
...
export async function requirePackageAccess(
  req: FastifyRequest,
  reply: FastifyReply,
  args: { packageId: string; permission: Permission },
): Promise<TenantAccessGrant | null> {
  return requireResourceAccess(req, reply, {
