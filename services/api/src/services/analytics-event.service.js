...
import { classifyRouteType, normalizeRoutePath, } from "../lib/route-classification.js";
...
export function cleanDisplayCity(value) {
    const decoded = safeDecodeMojibake(value);
...
export function classifyEventClass(eventType) {
    const normalized = eventType.trim().toLowerCase();
    if (NAVIGATION_EVENTS.has(normalized))
        return "navigation";
    if (AUTH_EVENTS.has(normalized))
        return "auth";
    if (EVIDENCE_EVENTS.has(normalized))
        return "evidence";
    if (REPORT_EVENTS.has(normalized))
        return "report";
    if (TEAM_EVENTS.has(normalized))
        return "team";
    if (BILLING_EVENTS.has(normalized))
        return "billing";
    return "custom";
}
export function classifySeverity(eventType) {
    const normalized = eventType.trim().toLowerCase();
    if (CRITICAL_EVENTS.has(normalized))
        return "critical";
    if (WARNING_EVENTS.has(normalized))
        return "warning";
    return "info";
}
...
function readGeoFromRequest(req) {
    ...
    const countryRaw = readHeader(req.headers, "cf-ipcountry") ??
        readHeader(req.headers, "x-vercel-ip-country");
    const cityRaw = readHeader(req.headers, "cf-ipcity") ??
        readHeader(req.headers, "x-vercel-ip-city");
    const region = readHeader(req.headers, "cf-region") ??
...
function toCreateMetadata(value) {
    if (value === undefined)
        return undefined;
    if (value === null)
...
export async function writeAnalyticsEvent(params) {
    const db = params.db ?? prisma;
    const geo = readGeoFromRequest(params.req);
    const normalizedPath = normalizeRoutePath(params.path ?? readRequestPath(params.req));
