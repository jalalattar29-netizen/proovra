...
import {
  ...
  type ReviewerOpsSavedViewFilter,
...
export type ReviewerOpsSavedViewProjection = {
  id: string;
  name: string;
...
function projectView(row: SavedSearchView): ReviewerOpsSavedViewProjection {
  ...
  const filter: ReviewerOpsSavedViewFilter = parsed.success
    ? parsed.data
    : { teamId: row.teamId };
  return {
    id: row.id,
...
export async function listReviewerOpsSavedViews(
  input: { teamId: string; userId: string; limit?: number },
  client: PrismaClient = defaultPrisma,
): Promise<ReadonlyArray<ReviewerOpsSavedViewProjection>> {
...
export type CreateReviewerOpsSavedViewInput = {
  teamId: string;
  actorUserId: string;
  name: string;
  description?: string | null;
...
export async function createReviewerOpsSavedView(
  input: CreateReviewerOpsSavedViewInput,
  client: PrismaClient = defaultPrisma,
): Promise<ReviewerOpsSavedViewProjection | null> {
  // Anti-cross-team: filter.teamId must match input.teamId.
  if (input.filter.teamId !== input.teamId) return null;
  try {
