...
import { ReviewerOpsError, assignReviewerToWorkflow, pauseReview, rejectReview, requestInformation, } from "./reviewer-operations-engine.service.js";
import { createEscalation, EscalationEngineError, } from "./escalation-engine.service.js";
// -----------------------------------------------------------------------------
// Public entry point
// -----------------------------------------------------------------------------
export async function executeBulkTriage(ctx, input, client = defaultPrisma) {
    ...
    for (const workflowId of uniqueIds) {
        try {
            ...
            const code = err instanceof ReviewerOpsError
                ? err.code
    ...
    if (failed > 0)
        bump("reviewer_bulk_action_partial_total");
...
// Per-row dispatcher
// -----------------------------------------------------------------------------
async function runOne(ctx, input, workflowId, client) {
    ...
    switch (action) {
        ...
        case "REASSIGN": {
            ...
            await assignReviewerToWorkflow(ctx, {
                workflowId,
        ...
        case "PAUSE": {
            ...
            await pauseReview(ctx, { workflowId, pausedReason: input.note }, client);
            return;
        }
        case "REQUEST_INFO": {
            if (!input.note) {
            ...
            await requestInformation(ctx, { workflowId, note: input.note }, client);
            return;
        }
        case "CLOSE": {
        ...
        case "PRIORITY_URGENT": {
            ...
            const priorityValue = action === "PRIORITY_HIGH"
                ? "HIGH"
