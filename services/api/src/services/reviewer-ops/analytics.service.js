 ...
 */
import { REVIEW_ESCALATION_REASONS, } from "@proovra/shared";
...
function boundRange(days) {
    const range = Math.min(Math.max(Math.floor(days ?? ANALYTICS_RANGE_DEFAULT_DAYS), 1), ANALYTICS_RANGE_MAX_DAYS);
...
// -----------------------------------------------------------------------------
export async function getEscalationAnalytics(input, client = defaultPrisma) {
    ...
    for (let d = new Date(startUtc); d <= endUtc; d = new Date(d.getTime() + 86400_000)) {
    ...
    const byDay = Array.from(buckets.values()).sort((a, b) => a.dayBucket < b.dayBucket ? -1 : 1);
    ...
    const meanResolutionMs = resolvedCount > 0 ? Math.round(totalMs / resolvedCount) : null;
    // ---------- Hotspots by reason ----------
    ...
    const hotspots = Array.from(hotspotMap.entries())
        ...
        meanResolutionMs: slot.resolvedCount > 0
...
// -----------------------------------------------------------------------------
export async function getReviewerPerformance(input, client = defaultPrisma) {
    const { startUtc, endUtc } = boundRange(input.rangeDays);
    ...
    for (const r of rows) {
        ...
        const isCompleted = r.status === "APPROVED_INTERNAL" ||
        ...
        if (r.completedAtUtc && r.completedAtUtc >= startUtc && r.completedAtUtc <= endUtc) {
            ...
            s.completedTotal += 1;
            if (r.status === "APPROVED_INTERNAL")
                s.approvedInRange += 1;
            if (r.status === "REJECTED_INSUFFICIENT")
                s.rejectedInRange += 1;
            if (r.assignedAtUtc) {
                s.totalResolutionMs +=
