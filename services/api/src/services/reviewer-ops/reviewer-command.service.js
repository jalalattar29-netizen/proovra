...
export async function buildReviewerCommand(input) {
    ...
    const now = new Date();
    const dueSoonCutoff = new Date(Date.now() + DUE_SOON_WINDOW_HOURS * 60 * 60 * 1000);
    // ----------- Summary tile counts -----------
    let summary = {
        status: "unavailable",
        data: null,
    ...
    try {
        const [assignedToMe, unassigned, dueSoon, overdue, openEscalations, inReview,] = await Promise.all([
    ...
    try {
        ...
        workflowPolicy = {
            ...
            data: {
                ...
                defaultFirstResponseDueHours: policy?.defaultFirstResponseDueHours ?? null,
...
function classifySlaTone(dueAt, now, dueSoonCutoff) {
    if (!dueAt)
        return "ok";
    if (dueAt.getTime() < now.getTime())
        return "overdue";
    if (dueAt.getTime() < dueSoonCutoff.getTime())
        return "due_soon";
    return "ok";
}
