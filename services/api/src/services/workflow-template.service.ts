...
export function mergeWorkflowTemplates(params: {
  ...
  for (const seed of params.seeds) {
    ...
    // applies at :205-211. Seeds that declare a category are subject to
    // the filter; seeds that omit a category (legacy "category-agnostic"
    // shape) bypass it, matching the NULL workspace_category DB row
    // behaviour.
    if (
      params.workspaceCategory &&
