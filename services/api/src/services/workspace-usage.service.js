...
import { formatBytesHuman, getPlanCapabilities, } from "./plan-catalog.service.js";
const GB = 1024n * 1024n * 1024n;
const STORAGE_ADDON_OFFERS = [
    {
...
function getAvailableStorageAddonOffers(scope) {
    ...
    if (scope.plan === prismaPkg.PlanType.PAYG) {
        return STORAGE_ADDON_OFFERS.filter((offer) => offer.key === prismaPkg.StorageAddonKey.PERSONAL_10_GB ||
            offer.key === prismaPkg.StorageAddonKey.PERSONAL_50_GB);
    }
    if (scope.plan === prismaPkg.PlanType.PRO) {
        return STORAGE_ADDON_OFFERS.filter((offer) => offer.workspaceType === "PERSONAL");
    }
    return [];
}
function getSuggestedUpgradePlan(scope) {
    if (scope.workspaceType === "TEAM") {
        return null;
    }
...
function getTeamMemberLimit(scope) {
    if (!scope.teamId) {
        return 0;
    }
    const caps = getPlanCapabilities(scope.plan);
    return Math.max(0, caps.maxMembersPerTeam || 0, caps.includedSeats || 0, scope.teamSeats || 0);
}
export async function getWorkspaceUsage(scope) {
    const caps = getPlanCapabilities(scope.plan);
    ...
    const [evidenceAggregate, reportAggregate, verificationPackageAggregate, evidenceCount, teamMemberCount,] = await Promise.all([
    ...
    const verificationPackageStorageBytes = toBigIntOrZero(verificationPackageAggregate._sum.sizeBytes);
    const storageBytesUsed = evidenceStorageBytes + reportStorageBytes + verificationPackageStorageBytes;
    ...
    const storageFromPlanAndAddons = baseStorageBytesLimit + extraStorageAddonBytes;
    const storageBytesLimit = storageBytesOverride && storageBytesOverride > 0n
        ? maxBigInt(storageBytesOverride, storageFromPlanAndAddons)
        : storageFromPlanAndAddons;
    const storageBytesRemaining = storageBytesLimit > storageBytesUsed
        ? storageBytesLimit - storageBytesUsed
        : 0n;
    const rawStorageRatio = storageBytesLimit > 0n
    ...
    const seatRemaining = seatLimit > teamMemberCount ? seatLimit - teamMemberCount : 0;
    ...
    const availableStorageAddons = getAvailableStorageAddonOffers(scope).map((offer) => ({
        key: offer.key,
        label: offer.label,
...
export async function assertWorkspaceStorageAvailable(params) {
    ...
    const incoming = typeof params.incomingBytes === "bigint"
    ...
    const nextRatio = usage.storageBytesLimit > 0n
        ? Number(nextValue) / Number(usage.storageBytesLimit)
        : 0;
    if (nextValue > usage.storageBytesLimit) {
        const err = new Error("Storage limit reached");
        err.statusCode = 409;
...
export async function assertTeamSeatAvailable(scope) {
    if (!scope.teamId)
        return;
    const usage = await getWorkspaceUsage(scope);
    const seatLimit = getTeamMemberLimit(scope);
    if (seatLimit <= 0) {
        const err = new Error("Team plan required");
        err.statusCode = 409;
    ...
    if (usage.teamMemberCount >= seatLimit) {
        const err = new Error("Team member limit reached");
        err.statusCode = 409;
