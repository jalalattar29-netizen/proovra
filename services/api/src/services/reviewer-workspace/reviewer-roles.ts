...
export type ReviewerRoleInput = {
  ...
  /**
   * Optional bounded reviewer-role override. Set by the workspace
   * admin via the future per-member surface. Bounded to the
   * REVIEWER_ROLES enum; unknown values fall back to the default.
   */
  reviewerRoleOverride?: string | null;
  /** True for platform-admin sessions. */
  isPlatformAdmin?: boolean;
};

export type ReviewerRoleResolution = {
  /** Bounded ReviewerRole, or null when the user cannot review at all. */
  role: ReviewerRole | null;
  capabilities: ReadonlyArray<ReviewerCapability>;
};

const DEFAULT_BY_WORKSPACE_ROLE: Readonly<
  Record<"OWNER" | "ADMIN" | "MEMBER" | "VIEWER", ReviewerRole | null>
> = {
  OWNER: "REVIEW_ADMIN",
  ADMIN: "SUPERVISOR",
  MEMBER: "REVIEWER",
...
export function resolveReviewerRole(
  ...
  if (!wsRole) return { role: null, capabilities: [] };
  ...
  if (wsRole !== "VIEWER" && input.reviewerRoleOverride) {
    const override = input.reviewerRoleOverride as ReviewerRole;
    if ((REVIEWER_ROLES as ReadonlyArray<string>).includes(override)) {
      return {
        role: override,
