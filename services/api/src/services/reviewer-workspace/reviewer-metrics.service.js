...
export async function getReviewerMetrics7d(input) {
    ...
    const workflowIds = Array.from(new Set(decisions.map((d) => d.workflowId)));
...
export async function getReviewerMetricsTeam(input) {
    ...
    return {
        ...
        escalationRate7dPct: total === 0 ? 0 : Math.round((escalated * 100) / total),
        disagreementRate7dPct: total === 0 ? 0 : Math.round((disagreements * 100) / total),
        qcFailureRate7dPct: qc.length === 0 ? 0 : Math.round((qcFails * 100) / qc.length),
        avgReviewDurationMs7d: avg,
    };
}
