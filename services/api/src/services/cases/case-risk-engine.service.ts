...
const REASON_LABELS: Record<RiskReasonCode, string> = {
...
function levelForScore(score: number): RiskLevel {
  if (score >= RISK_THRESHOLDS.NONE_MIN) return "NONE";
...
export async function listCaseEvidenceIds(input: {
  ...
  try {
    ...
    for (const r of links) set.add(r.evidenceId);
  } catch {
    /* best-effort */
  }
...
export type CaseRiskComputation = {
  riskScore: number;
  riskLevel: RiskLevel;
...
export async function computeCaseRisk(input: {
  ...
  if (openIncidents > 0) reasonCodes.push("INCIDENT_OPEN");
  if (overdueWorkflows > 0) reasonCodes.push("WORKFLOW_OVERDUE");
  if (activeWorkflows > 0 && overdueWorkflows === 0)
    reasonCodes.push("WORKFLOW_ACTIVE");
  if (integrityFailed > 0) reasonCodes.push("INTEGRITY_FAILED");
  if (integrityReviewRequired > 0)
    reasonCodes.push("INTEGRITY_REVIEW_REQUIRED");
  if (governanceBlockers > 0) reasonCodes.push("GOVERNANCE_BLOCKER");
  if (activeLegalHolds > 0) reasonCodes.push("LEGAL_HOLD_ACTIVE");
  if (reviewerPressure > 0) reasonCodes.push("REVIEWER_OVERLOAD");
  if (missingReports > 0) reasonCodes.push("REPORT_MISSING");
  if (missingPackages > 0) reasonCodes.push("PACKAGE_MISSING");
  if (evidenceGapCount > 0) reasonCodes.push("EVIDENCE_GAP");
  if (nonActiveLifecycle > 0) reasonCodes.push("CUSTODY_CONCERN");

  // Audit readiness: inverted gap count, bounded.
  const auditReadinessScore = clamp100(
    100 - Math.min(80, evidenceGapCount * 4 + governanceBlockers * 8),
  );
...
export function labelForReason(code: RiskReasonCode): string {
