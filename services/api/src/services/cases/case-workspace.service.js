...
// Helpers
// =============================================================================
function classifyCaseScope(caseTeamId) {
...
export async function buildCasesSummary(input) {
    ...
    const workspaceScope = memberCount <= 1 ? "PERSONAL" : "TEAM";
    ...
    try {
        const [totalCases, casesWithEvidence, casesWithActiveHolds, casesWithPendingReview,] = await Promise.all([
            ...
            prisma.case.count({
                where: {
                    ...
                    id: {
                        in: (await prisma.evidence.findMany({
                            where: {
                                teamId: input.teamId,
                                caseId: { not: null },
                            },
    ...
    try {
        ...
        if (rawCases.length === 0) {
            ...
            const [evidenceCounts, holdsByCase, recentReviewByCase] = await Promise.all([
            ...
            for (const row of evidenceCounts) {
                if (row.caseId)
                    evidenceCountByCase.set(row.caseId, row._count._all);
            }
            const holdsSet = new Set();
            for (const h of holdsByCase)
                holdsSet.add(h.caseId);
            const reviewCountByCase = new Map();
...
export async function buildCaseWorkspace(input) {
    ...
    const canManage = caseRow.ownerUserId === input.userId ||
        input.role === "OWNER" ||
        input.role === "ADMIN";
    // ----------- Overview counts -----------
    let overview = {
    ...
    try {
        ...
        const [linkedEvidenceCount, recentlyLinkedCount, activeCaseHoldsCount, affectedEvidenceHoldsCount, pendingReviewCount, openEscalationsCount,] = await Promise.all([
    ...
    // ----------- Review Coordination -----------
    let reviewCoordination = scope === "PERSONAL"
        ? { status: "not_applicable", data: null }
        : { status: "unavailable", data: null };
    if (scope === "TEAM") {
        try {
            const now = new Date();
            const [queuedCount, assignedCount, inReviewCount, needsInfoCount, overdueCount, openEscalationsCount,] = await Promise.all([
                prisma.evidenceReviewWorkflow.count({
    ...
    try {
        ...
        if (caseRow.updatedAt.getTime() - caseRow.createdAt.getTime() >
            60_000) {
            timelineItems.push({
