...
export async function buildMatterQueue(input) {
    ...
    const where = { teamId: input.teamId };
    ...
    for (const c of cases) {
        try {
            ...
            const [openIncidentCount, activeWorkflowCount, overdueWorkflowCount, governanceBlockerCount, activeLegalHoldCount, evidenceGapCount, activeAssignmentCount,] = await Promise.all([
            ...
            if (input.filter.hasOpenIncidents &&
                item.openIncidentCount === 0)
                continue;
            if (input.filter.hasGovernanceBlockers &&
                item.governanceBlockerCount === 0)
                continue;
            if (input.filter.hasOverdueWorkflows &&
