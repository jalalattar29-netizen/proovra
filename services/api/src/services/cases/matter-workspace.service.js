...
import { computeCaseRisk, listCaseEvidenceIds, recordCaseRiskSnapshot, } from "./case-risk-engine.service.js";
import { getCaseAssignmentRoles, resolveCaseViewerCapabilities, } from "./case-permission.service.js";
const SECTION_EVIDENCE_LIMIT = 50;
const SECTION_TIMELINE_LIMIT = 100;
const SECTION_HISTORY_LIMIT = 50;
function classifyCaseScope(teamId) {
...
export async function buildMatterWorkspace(input) {
    ...
    const accessRole = input.role ?? "VIEWER";
    const activeAssignmentRoleNames = await getCaseAssignmentRoles(caseRow.id, input.userId);
    ...
    const [commandSummary, evidence, relationships, workflows, incidentsAndCausality, reviewerCoordination, governance, custodyAndIntegrity, timeline, notes, deliverables, assignments, statusHistory,] = await Promise.all([
...
async function runCommandSummary(caseId, teamId, evidenceIds) {
    try {
        const since7d = new Date(Date.now() - 7 * 24 * 60 * 60 * 1000);
        const [linkedEvidenceCount, recentlyLinkedCount, activeCaseHoldsCount, affectedEvidenceHoldsCount, pendingReviewCount, openEscalationsCount, activeAssignmentCount,] = await Promise.all([
...
async function runEvidenceBoard(caseId, evidenceIds) {
    ...
    try {
        ...
        const byEv = new Map(links.map((l) => [
            l.evidenceId,
...
async function runRelationships(caseId, evidenceIds) {
    try {
        const links = await prisma.caseEvidenceLink.findMany({
            where: { caseId },
...
async function runWorkflows(caseId) {
    try {
...
async function runIncidentsAndCausality(caseId, evidenceIds) {
    try {
...
async function runReviewerCoordination(caseId, teamId, evidenceIds) {
    ...
    try {
        const [queuedCount, assignedCount, inReviewCount, needsInfoCount, overdueCount, openEscalationsCount, unresolvedReviewerCommentsCount, unresolvedAnnotationsCount, escalationsRaw,] = await Promise.all([
        ...
        if (teamId) {
            try {
                ...
                const ids = Array.from(new Set(owners
...
async function countReviews(evidenceIds, statuses) {
    if (evidenceIds.length === 0)
        return 0;
    try {
...
async function overdueReviews(evidenceIds) {
    if (evidenceIds.length === 0)
        return 0;
    try {
        return await prisma.evidenceReviewWorkflow.count({
...
async function runGovernance(caseId, evidenceIds, auditReadinessScore) {
    try {
        ...
        return {
            ...
            auditReadinessScore,
            blockerCount: caseHolds.filter((h) => h.status === "ACTIVE").length +
...
async function runCustodyAndIntegrity(evidenceIds) {
    if (evidenceIds.length === 0) {
        return {
            status: "ok",
...
async function runTimeline(caseId, evidenceIds) {
    try {
        ...
        const [statusEvents, assignEvents, linkEvents, commentEvents] = await Promise.all([
        ...
        const merged = [
            ...
            ...assignEvents.map((a) => ({
                id: `assign:${a.id}`,
                family: "CASE",
                eventType: a.status === "ACTIVE" ? "case.assignment_added" : "case.assignment_removed",
                severity: "INFO",
        ...
        merged.sort((a, b) => new Date(b.occurredAtUtc).getTime() -
...
async function runNotes(caseId, evidenceIds) {
    try {
        const [caseComments, unresolvedReviewerComments, unresolvedAnnotations] = await Promise.all([
            prisma.caseComment.findMany({
                where: { caseId },
                orderBy: { createdAt: "desc" },
                take: 25,
...
async function runDeliverables(evidenceIds) {
    if (evidenceIds.length === 0) {
        return {
            status: "ok",
            reports: [],
    ...
    try {
        const [reports, packages, externalLinks, signedCount, reportedCount, packagedCount] = await Promise.all([
        ...
        const deliverablesPending = Math.max(0, signedCount - reportsReady) +
...
async function runAssignments(caseId) {
    try {
...
async function runStatusHistory(caseId) {
    try {
