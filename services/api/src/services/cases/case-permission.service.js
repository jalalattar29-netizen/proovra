 ...
 */
export async function getCaseAssignmentRoles(caseId, userId) {
...
export function evaluateCaseMutationPermission(input) {
    ...
    switch (mutation) {
        case "ASSIGN":
            ...
            if (isOwnerOrAdmin || assignedAsOwner)
                return { allowed: true };
            return {
                allowed: false,
        ...
        case "STATUS_CHANGE":
            ...
            if (isOwnerOrAdmin || isTeamWriter)
                return { allowed: true };
            if (assignedAsOwner || assignedAsInvestigator)
                return { allowed: true };
            return {
                allowed: false,
...
export function evaluateCrossTeamAttach(input) {
    ...
    if (input.caseTeamId === null) {
        return {
            allowed: false,
            reason: "Cannot attach team-scoped evidence to a personal case. Personal cases may only hold personal evidence.",
    ...
    if (input.evidenceTeamId === null) {
        return {
            allowed: false,
            reason: "Cannot attach personal evidence to a team case. Move the evidence into the workspace first.",
            code: "CROSS_TEAM_ATTACH_BLOCKED",
        };
    }
    return {
        allowed: false,
        reason: "Evidence and case belong to different workspaces. Cross-workspace attachment is not permitted.",
        code: "CROSS_TEAM_ATTACH_BLOCKED",
    };
}
export function resolveCaseViewerCapabilities(input) {
    const evaluate = (mutation) => evaluateCaseMutationPermission({
        mutation,
    ...
    if (!assign.allowed)
        disabledReasons.assign = assign.reason;
    if (!changeStatus.allowed)
        disabledReasons.changeStatus = changeStatus.reason;
    ...
    if (!comment.allowed)
        disabledReasons.comment = comment.reason;
    ...
    const canMutate = changeStatus.allowed ||
