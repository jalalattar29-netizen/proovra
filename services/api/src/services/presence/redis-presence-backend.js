 ...
 */
import { buildPresenceKey, PRESENCE_HEARTBEAT_TTL_MS, PRESENCE_MAX_VIEWERS_PER_KEY, } from "./presence-backend.js";
...
function hashKey(teamId, resourceKind, resourceId) {
...
export class RedisPresenceBackend {
    redis;
    kind = "redis";
    constructor(redis) {
        this.redis = redis;
        ...
        try {
            ...
            if (len > PRESENCE_MAX_VIEWERS_PER_KEY) {
                ...
                const entries = Object.entries(all)
                    .map(([userId, raw]) => [userId, parseFieldValue(raw)])
                    .filter((e) => e[1] !== null)
