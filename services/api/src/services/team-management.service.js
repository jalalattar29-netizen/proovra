...
class TeamManagementService {
     ...
     */
    createOrganization(ownerId, name, slug, description) {
     ...
     */
    inviteToOrganization(organizationId, email, role = TeamRole.MEMBER, invitedBy) {
        const org = this.organizations.get(organizationId);
        if (!org) {
            throw new Error("Organization not found");
        }
        const normalizedEmail = this.normalizeEmail(email);
        const alreadyMember = org.members.some((m) => this.normalizeEmail(m.email) === normalizedEmail);
        if (alreadyMember) {
     ...
     * Accept invitation
     */
    acceptInvitation(token, userId, email) {
        let invitation = null;
        for (const inv of this.invitations.values()) {
     ...
     * Remove team member
     */
    removeTeamMember(organizationId, memberId) {
        const org = this.organizations.get(organizationId);
        if (!org)
            return false;
     ...
     */
    hasPermission(organizationId, userId, requiredRole) {
        const org = this.organizations.get(organizationId);
        if (!org)
            return false;
