...
let cached: AlertProvider | null = null;
...
export function setAlertProviderForTests(provider: AlertProvider | null): void {
  cached = provider;
...
const recentGlobalTimestamps: number[] = [];
...
function recordSent(category: AlertCategory, fingerprint: string, now: number): void {
  ...
  if (recentByFingerprint.size > 500) {
    ...
    if (oldestKey !== null) recentByFingerprint.delete(oldestKey);
  }
}
