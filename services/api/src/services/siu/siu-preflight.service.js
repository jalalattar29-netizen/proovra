...
import { SIU_STANDING_LIMITATIONS, } from "@proovra/shared";
import { loadSiuProfile } from "./siu-profile.service.js";
import { PROOVRA_SPAN_NAMES, withProovraSpan, } from "../../observability/otel.js";
export async function runSiuExportPreflight(input) {
    return withProovraSpan(PROOVRA_SPAN_NAMES.SIU_EXPORT_PREFLIGHT, { caseId: input.caseId, teamId: input.teamId }, () => runSiuExportPreflightInner(input));
}
async function runSiuExportPreflightInner(input) {
    ...
    const satisfied = required.filter((c) => c.status === "satisfied" || c.status === "mapped");
    const missingRequired = required.filter((c) => c.status === "missing" || c.status === "submitted");
    ...
    for (const ev of evidence) {
        if (ev.status === "FAILED_HASH_MISMATCH") {
            findings.push(blocker("EVIDENCE_INTEGRITY_FAILED", {
        ...
        if (ev.lifecycleState === "DESTROYED") {
    ...
    const openFollowUps = profile.followUps.filter((f) => f.status === "open" ||
        f.status === "sent" ||
        f.status === "received");
