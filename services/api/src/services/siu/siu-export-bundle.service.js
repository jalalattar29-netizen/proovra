...
import { SIU_EXPORT_SCHEMA_VERSION, SIU_STANDING_LIMITATIONS, } from "@proovra/shared";
const SIU_PROFILE_PII_FIELDS = [
    "claimantName",
    "claimantContact",
];
export async function buildSiuExportBundle(input) {
    return withProovraSpan(PROOVRA_SPAN_NAMES.SIU_EXPORT_GENERATE, {
        caseId: input.caseId,
        teamId: input.teamId,
        readiness: input.preflight.readiness,
    }, () => buildSiuExportBundleInner(input));
}
async function buildSiuExportBundleInner(input) {
    if (input.preflight.readiness === "blocked") {
        throw new Error("Refusing to build SIU export: preflight is blocked.");
    }
    if (input.preflight.readiness === "ready_with_warnings" &&
        (!input.warningExportReason || input.warningExportReason.length < 8)) {
        throw new Error("Refusing to build SIU export: ready_with_warnings requires a bounded reason.");
    }
    ...
    passthrough.on("data", (b) => buffers.push(b));
    ...
    function appendText(path, body, contentType) {
    ...
    await withProovraSpan(PROOVRA_SPAN_NAMES.SIU_TIMELINE_BUILD, { "proovra.operation": "siu_timeline_build" }, () => undefined);
    ...
    for (const ev of evidence) {
        ...
        const evVps = ev.verificationPackages;
        ...
        artifactInventory.push({
            evidenceId: ev.id,
            title: ev.title,
            reportGeneratedAtUtc: ev.reportGeneratedAtUtc?.toISOString() ?? null,
            verificationPackageGeneratedAtUtc: ev.verificationPackageGeneratedAtUtc?.toISOString() ?? null,
    ...
    appendJson("integrity-provenance-summary.json", {
        ...
        note: "Per-evidence integrity verdicts (hash, custody, TSA, OTS, C2PA) live inside each evidence's Verification Package ZIP (`verification/<id>/verification-package.zip`). This file is the bounded index.",
    });
    appendJson("custody-audit-summary.json", {
        generatedAtUtc: summary.generatedAtUtc,
        note: "Custody and audit events for each evidence are bundled inside that evidence's Verification Package (see `verification/`). This file restates the bounded SIU follow-up + indicator counts as an aggregate.",
        totals: summary.totals,
    });
    appendText("verification/offline-verification.md", buildOfflineVerificationGuidance(), "text/markdown");
    appendText("README.md", buildBundleReadme(summary, {
        reportsIncluded,
        reportsMissing,
        vpIncluded,
        vpMissing,
    ...
    // Package convention).
    const manifestShaBuffer = Buffer.from(`${manifestSha256}  manifest.json\n`, "utf8");
    ...
    const storageKey = bucket
        ...
        : null;
    let uploadFailureCode = null;
    let uploadFailureMessage = null;
    ...
    const exportRow = await prisma.caseSiuExport.create({
        data: {
            ...
            manifestSha256,
            artifactInclusionJson: inclusion,
...
export async function downloadSiuExportArtifact(input) {
    ...
    if (row.siuProfile.teamId !== input.teamId)
        return { notFound: true };
    ...
    return {
        stream,
        sizeBytes: row.artifactSizeBytes != null ? Number(row.artifactSizeBytes) : null,
        artifactSha256: row.artifactSha256,
        exportRowId: row.id,
    };
}
export async function listSiuExports(input) {
    ...
    return rows.map((r) => ({
        ...
        artifactSizeBytes: r.artifactSizeBytes != null ? r.artifactSizeBytes.toString() : null,
...
function extractInclusion(value) {
...
// ---------------------------------------------------------------------------
function buildClaimTimelinePayload(profile) {
    const events = [];
    events.push({
    ...
    return {
        ...
        generatedAtUtc: new Date().toISOString(),
        events,
        note: "Timeline is a bounded operational projection. It does not constitute a legal record and is not an admissibility claim.",
    };
}
function buildBundleManifest(input) {
    return {
        ...
        limitations: SIU_STANDING_LIMITATIONS,
        note: "This manifest enumerates files bundled at generation time. The bundle is NOT a legal-admissibility claim and SIU findings are operational signals only.",
    };
}
function buildBoundedExportNote(state) {
    switch (state) {
        case "ready":
...
function buildOfflineVerificationGuidance() {
    return [
...
function buildBundleReadme(summary, artifactCounts) {
    return [
