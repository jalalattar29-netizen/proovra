...
import type { SiuCapability } from "@proovra/shared";

export type SiuCapabilityDecision = {
  capability: SiuCapability;
  allowed: boolean;
  /** Bounded reason for the decision (≤80 chars). */
  reason:
    | "access_policy_allowed"
    | "case_owner_fallback"
    | "denied_by_access_policy"
    | "denied_by_default"
    | "denied_member_inactive";
};

export type EvaluateSiuCapabilityInput = {
  teamId: string;
  userId: string;
 ...
 * needs to change.
 */
export async function evaluateSiuCapability(
  input: EvaluateSiuCapabilityInput,
): Promise<SiuCapabilityDecision> {
