...
import { SIU_INVESTIGATION_STATUSES, SIU_SAVED_VIEW_VISIBILITY, } from "@proovra/shared";
// ---------------------------------------------------------------------------
...
export async function listCustomSavedViews(input) {
...
export async function createSavedView(input) {
    const row = await prisma.caseSiuSavedView.create({
        data: {
            ...
            filterJson: input.payload.filter,
            sortJson: input.payload.sort,
...
export async function updateSavedView(input) {
    ...
    const patch = {
        ...
        filterJson: input.payload.filter !== undefined
            ? input.payload.filter
            : undefined,
        sortJson: input.payload.sort !== undefined
            ? input.payload.sort
            : undefined,
