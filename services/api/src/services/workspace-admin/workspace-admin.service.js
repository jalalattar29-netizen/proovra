...
export async function buildWorkspaceAdmin(input) {
    ...
    try {
        const [policy, activeLegalHoldsCount, activeCaseLegalHoldsCount, retentionPoliciesCount, pendingDestructionCount,] = await Promise.all([
    ...
    if (scope === "TEAM") {
        try {
            const now = new Date();
            const [queuedCount, assignedCount, overdueCount, openEscalationsCount] = await Promise.all([
