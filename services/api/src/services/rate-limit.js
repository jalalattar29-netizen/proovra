...
function buildWindowReset(now, windowSec) {
    return now + windowSec * 1000;
}
function readRedisCooldownMs() {
 ...
 */
export async function clearAllRateLimitBuckets() {
    ...
    if (client) {
        try {
            ...
            let cursor = "0";
            do {
                const result = (await client.scan(cursor, "MATCH", "ratelimit:*", "COUNT", 200));
                cursor = result[0];
                const keys = result[1];
                if (keys.length > 0) {
