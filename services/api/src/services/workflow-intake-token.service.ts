...
export function isWorkflowIntakeFeatureEnabled(): boolean {
  if (process.env[FEATURE_FLAG_ENV] !== "true") return false;
  return readSecret() !== null;
}

export type WorkflowIntakeFeatureDisabledReason =
  | "feature_flag_off"
  | "secret_missing";
...
export function workflowIntakeFeatureDisabledReason():
  ...
  | null {
  if (process.env[FEATURE_FLAG_ENV] !== "true") return "feature_flag_off";
  if (readSecret() === null) return "secret_missing";
  return null;
}

function hmacToHexImpl(secret: Buffer, value: string): string {
...
export type VerifiedIntakeTokenLookup = {
  tokenHash: string;
  tokenVersion: number;
};
 ...
 */
export function lookupHashForIntakeToken(
  rawToken: string,
): VerifiedIntakeTokenLookup | null {
...
export function constantTimeEqualHex(a: string, b: string): boolean {
  if (a.length !== TOKEN_HASH_HEX_LENGTH) return false;
  if (b.length !== TOKEN_HASH_HEX_LENGTH) return false;
...
export const __testing = {
   ...
   */
  withSecret<T>(secret: string, fn: () => T): T {
    ...
    try {
      ...
      if (prevSecret === undefined) delete process.env[TOKEN_SECRET_ENV];
      else process.env[TOKEN_SECRET_ENV] = prevSecret;
      if (prevFlag === undefined) delete process.env[FEATURE_FLAG_ENV];
      else process.env[FEATURE_FLAG_ENV] = prevFlag;
    }
  },
};
