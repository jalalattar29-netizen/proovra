...
async function resolveAssigneeAndWorkspace(workflow, client) {
    let reviewer = null;
...
async function dispatchToAssignee(workflow, kind, initiatedByUserId, client) {
    const { reviewer, workspaceName, evidenceUrl } = await resolveAssigneeAndWorkspace(workflow, client);
...
// -----------------------------------------------------------------------------
export async function notifyReviewAssigned(input, client = defaultPrisma) {
    try {
        await dispatchToAssignee(input.workflow, input.isReassignment
            ? { eventType: "REVIEW_REASSIGNED", isReassignment: true }
            : { eventType: "REVIEW_REQUEST_ASSIGNED", isReassignment: false }, input.actorUserId, client);
    }
    catch {
        /* swallowed — caller flow is unaffected */
    }
}
export async function notifyReviewEscalated(input, client = defaultPrisma) {
    try {
        await dispatchToAssignee(input.workflow, { eventType: "REVIEW_ESCALATED" }, input.actorUserId, client);
    }
    catch {
        /* swallow */
    }
}
export async function notifyReviewNeedsMoreInfo(input, client = defaultPrisma) {
    try {
        await dispatchToAssignee(input.workflow, { eventType: "REVIEW_NEEDS_MORE_INFO" }, input.actorUserId, client);
    }
    catch {
        /* swallow */
    }
}
export async function notifyReviewResponseReceived(input, client = defaultPrisma) {
    try {
        await dispatchToAssignee(input.workflow, { eventType: "REVIEW_RESPONSE_RECEIVED" }, input.actorUserId ?? null, client);
    }
    catch {
        /* swallow */
    }
}
export async function notifyReviewSlaBreached(input, client = defaultPrisma) {
    try {
        await dispatchToAssignee(input.workflow, { eventType: "REVIEW_OVERDUE_REMINDER" }, null, client);
    }
    catch {
        /* swallow */
    }
