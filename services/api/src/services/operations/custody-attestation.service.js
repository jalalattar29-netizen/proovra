...
import { getCurrentActiveSigners, } from "./signer-registry.service.js";
/**
...
export function hashCanonicalPayload(p) {
 ...
 */
export async function signCustodyEvent(input, client = defaultPrisma) {
    return withProovraSpan(PROOVRA_SPAN_NAMES.CUSTODY_ATTESTATION_SIGN, { teamId: input.teamId, operation: "sign" }, async () => {
...
async function signCustodyEventInner(input, client) {
    ...
    if (existing) {
        ...
        for (const r of recent) {
            const d = (r.details ?? {});
            if (d.custodyEventId === input.custodyEventId &&
    ...
    try {
        if (signer.provider === "aws_kms") {
            const { KmsEvidenceSigner } = await import("../../signing/kms-signer.js");
            ...
            const sig = ed25519SignHexWithKeyPath(hash, "SIGNING_PRIVATE_KEY_PATH");
            signature = sig;
            algorithm = "ED25519";
        }
    }
        ...
        safeEmitSecurityEvent({
            ...
            details: {
                ...
                errorCode: err instanceof Error ? err.name.slice(0, 60) : "unknown",
            },
        });
        return {
            ok: false,
...
function pickCurrentCustodySigner() {
    const list = getCurrentActiveSigners();
    return (list.find((s) => s.signerPurpose === "custody_event" && s.status === "active") ?? null);
}
// ---------------------------------------------------------------------------
// Verification
// ---------------------------------------------------------------------------
 ...
 */
export async function verifyCustodyAttestation(input, client = defaultPrisma) {
    return withProovraSpan(PROOVRA_SPAN_NAMES.CUSTODY_ATTESTATION_VERIFY, { teamId: input.teamId, operation: "verify" }, async () => {
...
async function verifyCustodyAttestationInner(input, client) {
    ...
    const recomputed = hashCanonicalPayload(buildCanonicalPayload({
        id: row.id,
        evidenceId: row.evidenceId,
        eventType: row.eventType,
        atUtc: row.atUtc,
    ...
    if (recomputed !== att.canonicalPayloadHash) {
        ...
        return {
            outcome: "payload_hash_mismatch",
            summary: "Canonical payload hash does not match. Either the custody event was modified or the canonical projection changed.",
            attestation: att,
    ...
    try {
        if (att.provider === "aws_kms" && att.algorithm === "ED25519_SHA_512") {
            const { VerifyCommand, KMSClient } = await import("@aws-sdk/client-kms");
            ...
            const { ed25519VerifyHexSignature, loadPemFromPathEnv } = await import("../../crypto.js");
    ...
    if (!signatureOk) {
        ...
        return {
            outcome: "signature_invalid",
            summary: "Signature did not verify against the recorded signer / key version.",
            attestation: att,
            recomputedPayloadHash: recomputed,
            verifiedAtUtc: new Date().toISOString(),
        };
    ...
    return {
        outcome: "verified",
        summary: "Canonical payload hash matches the recorded value and the signature verifies under the recorded signer + key version.",
        attestation: att,
        recomputedPayloadHash: recomputed,
        verifiedAtUtc: new Date().toISOString(),
    };
}
export async function listAttestations(input, client = defaultPrisma) {
    ...
    for (const r of rows) {
        ...
        if (!att)
            continue;
        if (input.evidenceId && att.evidenceId !== input.evidenceId)
            continue;
        out.push({
...
export async function backfillCustodyAttestations(input, client = defaultPrisma) {
    return withProovraSpan(PROOVRA_SPAN_NAMES.CUSTODY_ATTESTATION_BACKFILL, {
        teamId: input.teamId,
        batchSize: Math.min(Math.max(input.batchSize ?? 50, 1), 200),
    }, () => backfillCustodyAttestationsInner(input, client));
}
async function backfillCustodyAttestationsInner(input, client) {
