...
export type RecoveryReportKind = (typeof RECOVERY_REPORT_KINDS)[number];
...
export type ValidationCheck = {
  ...
  detail: string;
  /** When this category sits outside the app's authority. */
  unsupported: boolean;
};
...
export async function validateBackup(
  ...
  return withProovraSpan(
    PROOVRA_SPAN_NAMES.RECOVERY_BACKUP_VALIDATE,
    { teamId: input.teamId },
    () => validateBackupInner(input, client),
  );
}

async function validateBackupInner(
  input: { teamId: string; actorUserId: string },
  client: PrismaClient,
): Promise<RecoveryValidationReport> {
  ...
  if (ol.mode === "verified") {
    ...
    checks.push({
      ...
      outcome: "warning",
      detail:
        "Object Lock is intentionally disabled. Exports persist normally but cannot be claimed as WORM.",
      unsupported: false,
    });
  ...
  try {
    ...
    if (recent.length === 0) {
      checks.push({
        ...
        outcome: "warning",
        detail:
          "No exports recorded for this workspace; nothing to validate.",
        unsupported: false,
      });
    } else {
      let missing = 0;
      ...
      checks.push({
        ...
        label: "Recent export artifacts present in S3",
        outcome:
          missing === 0 ? "passed" : missing === recent.length ? "failed" : "warning",
        detail:
          missing === 0
  ...
  checks.push({
    ...
    outcome: "unsupported",
    detail:
      "PROOVRA does not run application-layer database backups. The managed Postgres provider's backup schedule is authoritative. Validate via the provider's console.",
    unsupported: true,
  });
  ...
  checks.push({
    ...
    outcome: "unsupported",
    detail:
      "S3 backup is delegated to bucket-level configuration (versioning, replication, Object Lock). Validate via AWS console.",
    unsupported: true,
  });
...
export async function validateRestore(
  ...
  return withProovraSpan(
    PROOVRA_SPAN_NAMES.RECOVERY_RESTORE_VALIDATE,
    { teamId: input.teamId },
    () => validateRestoreInner(input, client),
  );
}

async function validateRestoreInner(
  input: { teamId: string; actorUserId: string },
  client: PrismaClient,
): Promise<RecoveryValidationReport> {
  ...
  if (ol.mode === "verified") {
    ...
    checks.push({
      ...
      outcome: "warning",
      detail:
        "Object Lock is not verified. Artifact restore depends on S3 versioning / external backups.",
      unsupported: false,
    });
  }
  ...
  checks.push({
    ...
    outcome: "unsupported",
    detail:
      "PROOVRA does not run a scripted full-DR rehearsal. Restore drill must be run by the infrastructure provider in a quarantined environment.",
    unsupported: true,
  });
  ...
  checks.push({
    ...
    outcome: "unsupported",
    detail:
      "Cross-region failover is delegated to the cloud provider. PROOVRA app code does not orchestrate it.",
    unsupported: true,
  });
...
export type RecoveryReportListItem = {
  reportId: string;
  kind: RecoveryReportKind;
  teamId: string;
...
export async function listRecoveryReports(
  input: { teamId: string; limit?: number },
  client: PrismaClient = defaultPrisma,
): Promise<ReadonlyArray<RecoveryReportListItem>> {
  ...
  return rows
    .map((r) => {
      ...
      const kind =
        ...
        (RECOVERY_REPORT_KINDS as readonly string[]).includes(details.kind)
          ? (details.kind as RecoveryReportKind)
          : null;
      const outcome =
        typeof details.outcome === "string" &&
        (VALIDATION_OUTCOMES as readonly string[]).includes(details.outcome)
          ? (details.outcome as ValidationOutcome)
          : null;
      const reportId =
        typeof details.reportId === "string" ? details.reportId : r.id;
      if (!kind) return null;
      return {
        reportId,
        kind,
    ...
    .filter((x): x is RecoveryReportListItem => x !== null);
}

export async function getRecoveryReport(
  input: { teamId: string; reportId: string },
  client: PrismaClient = defaultPrisma,
): Promise<RecoveryValidationReport | null> {
  ...
  for (const r of rows) {
    const details = (r.details ?? {}) as Record<string, unknown>;
    const report = (details.report ?? null) as RecoveryValidationReport | null;
    if (report && report.reportId === input.reportId) return report;
  }
  return null;
}
...
function aggregateOutcome(
  ...
  if (checks.every((c) => c.outcome === "unsupported")) return "unsupported";
  return "passed";
}
