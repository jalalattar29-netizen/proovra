...
const ENTRIES: ReadonlyArray<ReplaySafetyEntry> = [
  // ---- report queue ----
  {
    ...
    rationale:
      "Idempotent overall, but the job performs PDF signing + artifact moves. A partial-failure replay is the most risk-bearing path; gate with step-up.",
  },
  {
    queueName: "report",
    ...
    category: "forbidden",
    rationale:
      "Hard-deletes evidence rows + S3 objects. Re-running either no-ops (already gone) or attempts to delete data that has since been re-created. Operator should diagnose, not re-run.",
  },
  {
  ...
  // ---- ots-upgrade ----
  {
    ...
    category: "requires_step_up",
    rationale:
      "External blockchain anchor attempt. Safe to re-attempt but cumulative cost; step-up gates the operator confirmation.",
  },
  // ---- search-indexing ----
  {
  ...
  },
  {
    ...
    category: "safe",
    rationale:
      "DLQ replay re-enqueues the upstream analysis job using BullMQ's retry. Same idempotency guarantee as the live queue.",
  },
  // ---- MI subsystem queues ----
  {
