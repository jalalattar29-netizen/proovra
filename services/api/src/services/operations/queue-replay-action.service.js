...
import { getJobReplayCategory, getJobReplayRationale, } from "./queue-replay-safety.service.js";
...
export async function retryFailedJob(input) {
    return withProovraSpan(PROOVRA_SPAN_NAMES.QUEUE_JOB_RETRY, {
...
export async function replayFailedJob(input) {
    return withProovraSpan(PROOVRA_SPAN_NAMES.QUEUE_JOB_REPLAY, {
        queueName: input.queueName,
        action: "replay",
        teamId: input.teamId,
...
async function doReplayLike(action, input) {
    const reason = (input.reason ?? "").trim().slice(0, 240);
    if (reason.length === 0) {
        return {
            ok: false,
    ...
    if (category === "forbidden") {
        ...
        return {
            ...
            message: "This job kind is classified as forbidden for replay. See the replay safety matrix.",
        };
    }
    if (category === "unknown") {
        return {
            ...
            message: "Replay refused: this job kind is not in the replay safety matrix.",
        };
    }
    // Emit started.
    safeEmitSecurityEvent({
    ...
    bump("queue_replay_total");
    if (category === "safe")
        bump("queue_replay_safe_total");
    if (category === "requires_step_up")
        bump("queue_replay_step_up_total");
    try {
