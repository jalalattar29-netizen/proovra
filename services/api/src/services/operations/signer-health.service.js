 ...
 */
export async function probeSignerHealth() {
    return withProovraSpan(PROOVRA_SPAN_NAMES.SIGNER_HEALTH_CHECK, {
        provider: (process.env.SIGNER_PROVIDER ?? "local-pem")
            .trim()
            .toLowerCase(),
    }, () => probeSignerHealthInner());
}
async function probeSignerHealthInner() {
    bump("signer_health_check_total");
    const checkedAtUtc = new Date().toISOString();
    const providerRaw = (process.env.SIGNER_PROVIDER ?? "local-pem")
        .trim()
        .toLowerCase();
    const provider = providerRaw === "aws-kms" || providerRaw === "aws_kms"
        ? "aws_kms"
    ...
    if (provider === "disabled") {
        ...
        return {
            ...
            algorithm: null,
            recommendedAction: "Set SIGNER_PROVIDER=aws-kms (production) or local-pem (dev) to enable signing.",
        };
    }
    if (provider === "local_pem") {
        ...
        if (!privPath || !pubPath) {
            ...
            return {
                ...
                publicMaterialRef: null,
                algorithm: "ED25519",
                recommendedAction: "Set SIGNING_PRIVATE_KEY_PATH and SIGNING_PUBLIC_KEY_PATH.",
            };
        }
        if (!existsSync(privPath) || !existsSync(pubPath)) {
            ...
            return {
                ...
                algorithm: "ED25519",
                recommendedAction: "Mount the PEM files at the configured paths or rotate via the local-pem rotation procedure.",
            };
        }
        return {
            health: "healthy",
    ...
    try {
        ...
        if (!algos.includes("ED25519_SHA_512")) {
            bump("signer_health_degraded_total");
            return {
                ...
                recommendedAction: "Provision a KMS key with SigningAlgorithm=ED25519_SHA_512.",
            };
        }
        return {
            health: "healthy",
        ...
        return {
            ...
            publicMaterialRef: null,
            algorithm: "ED25519_SHA_512",
            recommendedAction: cls === "permission_denied"
