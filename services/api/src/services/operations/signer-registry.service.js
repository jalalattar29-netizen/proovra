...
function buildSignerId(purpose, provider, keyId, keyVersion) {
    return `${purpose}:${provider}:${keyId ?? "_"}:${keyVersion ?? "_"}`;
}
function evidenceAlgorithm(provider) {
    if (provider === "aws_kms")
 ...
 */
export function getCurrentActiveSigners() {
    const provider = envProvider();
    const now = new Date().toISOString();
    const kmsArn = provider === "aws_kms" ? envValue("KMS_KEY_ID") : null;
    ...
    const pkgKeyVersion = envValue("PACKAGE_SIGNING_KEY_VERSION") ?? evidKeyVersion;
    ...
    const verificationRef = provider === "aws_kms"
        ? "kms://GetPublicKey"
        : provider === "local_pem"
    ...
    const base = {
        ...
        kmsKeyArn: kmsArn,
        status: provider === "disabled" ? "degraded" : "active",
    };
    const list = [
        {
            ...base,
            signerId: buildSignerId("report_pdf", provider, evidKeyId, evidKeyVersion),
        ...
        {
            ...base,
            signerId: buildSignerId("verification_package", provider, pkgKeyId, pkgKeyVersion),
        ...
        {
            ...base,
            signerId: buildSignerId("export_manifest", provider, exportKeyId, exportKeyVersion),
        ...
        {
            ...base,
            signerId: buildSignerId("custody_event", provider, evidKeyId, evidKeyVersion),
...
export async function listStagedSigners(input, client = defaultPrisma) {
    const rows = await client.securityEvent.findMany({
        where: {
            teamId: input.teamId,
            eventType: { in: STAGED_EVENT_TYPES },
        },
    ...
    for (const row of rows.reverse()) {
        ...
        if (!SIGNER_PURPOSES.includes(purpose) ||
            !SIGNER_PROVIDERS.includes(provider)) {
            continue;
        }
        const keyId = typeof d.keyId === "string" ? d.keyId : null;
        const keyVersion = typeof d.keyVersion === "string" ? d.keyVersion : null;
        const signerId = typeof d.signerId === "string"
            ? d.signerId
            : buildSignerId(purpose, provider, keyId, keyVersion);
        const existing = map.get(signerId);
        const status = row.eventType === "signer_promoted"
            ? "retiring"
        ...
        map.set(signerId, {
            ...
            stagedAtUtc: existing?.stagedAtUtc ?? row.createdAt.toISOString(),
            stagedByUserId: typeof d.actorUserId === "string"
                ? d.actorUserId
                : existing?.stagedByUserId ?? "unknown",
            notes: typeof d.notes === "string" ? d.notes : existing?.notes ?? null,
        });
    }
    return Array.from(map.values());
}
function statusTransition(prev, next) {
...
// ---------------------------------------------------------------------------
export async function listAllSigners(input, client = defaultPrisma) {
    ...
    for (const s of staged) {
        out.push({
            ...
            status: s.status,
            activatedAtUtc: null,
            retiredAtUtc: s.status === "retired" || s.status === "revoked"
                ? s.stagedAtUtc
                : null,
            lastUsedAtUtc: null,
...
export async function getSignerById(input, client = defaultPrisma) {
