...
import { decodeExportId, hashManifest, resolveExportManifest, } from "./export-manifest.service.js";
// ---------------------------------------------------------------------------
...
// Verifier entrypoint
// ---------------------------------------------------------------------------
export async function verifyExportReproducibility(input, client = defaultPrisma) {
    const decoded = decodeExportId(input.exportId);
    if (!decoded) {
        return { ok: false, code: "not_found", message: "Unknown export id." };
    }
    const manifestResult = await resolveExportManifest({ teamId: input.teamId, exportId: input.exportId }, client);
    ...
    // ---- Artifact bytes ----
    const headResult = await safeHead(envelope.manifest.artifact.storageBucket, envelope.manifest.artifact.storageKey);
    if (!headResult.ok) {
        return {
            ok: true,
            report: {
                ...
                summary: headResult.reason === "not_found"
    ...
    const objectLockChecks = checks.filter((c) => c.field.startsWith("object_lock"));
    const objectLockOk = objectLockChecks.every((c) => c.ok);
    const manifestOk = checks.find((c) => c.field === "manifest_hash")?.ok ?? false;
    const artifactOk = checks.find((c) => c.field === "artifact.sha256")?.ok ?? false;
    let outcome;
    let summary;
    if (!manifestOk) {
...
async function safeHead(bucket, key) {
    try {
        ...
        const mode = res.objectLockMode === "GOVERNANCE" || res.objectLockMode === "COMPLIANCE"
            ? res.objectLockMode
            : null;
        ...
        const hold = holdRaw === "ON" ? "ON" : holdRaw === "OFF" ? "OFF" : null;
        return {
            ok: true,
            objectLockMode: mode,
