...
// ===========================================================================
export const PLAN_LINE_ENTITLEMENTS = {
    CAPTURE_AND_VERIFY: {
...
function classifyPeriod(key) {
    if (key.endsWith("PER_DAY"))
        return "DAY";
    if (key.endsWith("PER_MONTH"))
        return "MONTH";
    return "MONTH";
}
...
function projectGrant(key, row) {
    const kind = (["FEATURE", "QUOTA", "LIMIT"].includes(row.kind)
        ? row.kind
        : classifyKind(key));
    const value = kind === "FEATURE"
        ? row.valueBool ?? false
        : Number(row.valueNumber ?? 0);
    const source = (["PLAN", "CUSTOM", "PROMO", "DEFAULT"].includes(row.source)
        ? row.source
        : "PLAN");
    return {
        key,
...
// ===========================================================================
async function emitDenial(prisma, teamId, key, actorUserId, denial) {
    ...
    const code = denial === "QUOTA_EXCEEDED" || kind === "QUOTA"
...
export async function upsertEntitlementGrant(input) {
    ...
    const valueNumber = input.kind === "FEATURE"
        ? null
...
export async function applyProductLine(input) {
    if (!PRODUCT_LINES.includes(input.line)) {
