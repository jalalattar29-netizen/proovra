...
import { abortMultipartUpload, completeMultipartUpload, createPresignedPartUploadUrl, headCompletedObject, initiateMultipartUpload, verifyCompletedObject, } from "./storage-multipart.js";
// =============================================================================
...
export async function createUploadSession(input, client = defaultPrisma) {
    if (!Number.isInteger(input.expectedPartCount) ||
    ...
    const now = Date.now();
    const expiresAt = input.expiresAtUtc instanceof Date
        ? input.expiresAtUtc
        : input.expiresAtUtc
    ...
    if (input.idempotencyKey) {
        try {
            const existing = (await client.$queryRawUnsafe(`SELECT "id", "team_id", "evidence_id", "actor_user_id", "state",
    ...
    if (input.targetPartIndex != null &&
        (!Number.isInteger(input.targetPartIndex) || input.targetPartIndex < 0)) {
        return { ok: false, reason: "invalid_part_index" };
    }
    try {
        const rows = (await client.$queryRawUnsafe(`INSERT INTO "evidence_upload_sessions" (
       ...
       RETURNING "id", "team_id", "evidence_id", "actor_user_id", "state",
         ...
         "safe_note", "idempotency_key", "expires_at_utc",
         "completed_at_utc", "aborted_at_utc", "aborted_by_user_id",
         "abort_reason", "created_at_utc", "updated_at_utc"`, input.teamId, input.evidenceId, input.actorUserId, input.expectedPartCount, input.expectedTotalBytes ?? null, input.expectedSha256 ?? null, input.safeNote?.slice(0, 400) ?? null, input.idempotencyKey?.slice(0, 120) ?? null, expiresAt, input.targetPartIndex ?? null, input.originalFileName?.slice(0, 255) ?? null, input.expectedMimeType?.slice(0, 128) ?? null));
        ...
        const placeholders = Array.from({ length: input.expectedPartCount }, (_, i) => `($1, $2, ${i}, 'PENDING')`).join(", ");
        await client.$executeRawUnsafe(`INSERT INTO "evidence_upload_session_parts"
         ("session_id", "team_id", "part_index", "state")
         VALUES ${placeholders}
         ON CONFLICT ("session_id", "part_index") DO NOTHING`, session.id, input.teamId);
...
export async function resumeUploadSession(input, client = defaultPrisma) {
    try {
        const sessions = (await client.$queryRawUnsafe(`SELECT "id", "team_id", "evidence_id", "actor_user_id", "state",
              "expected_part_count", "expected_total_bytes", "expected_sha256",
              "safe_note", "idempotency_key", "expires_at_utc",
              "completed_at_utc", "aborted_at_utc", "aborted_by_user_id",
              "abort_reason", "created_at_utc", "updated_at_utc"
         ...
         LIMIT 1`, input.sessionId, input.teamId));
        if (sessions.length === 0) {
            return { ok: false, reason: "session_not_found" };
        }
        const session = projectSession(sessions[0]);
        const parts = (await client.$queryRawUnsafe(`SELECT "id", "session_id", "team_id", "part_index", "state",
         ...
         ORDER BY "part_index" ASC`, input.sessionId, input.teamId));
 ...
 * SHA-256 check.
 */
export async function markPartUploaded(input, client = defaultPrisma) {
    if (!Number.isInteger(input.partIndex) || input.partIndex < 0) {
        return { ok: false, reason: "invalid_part_index" };
    }
    if (input.clientSha256 != null &&
        !/^[a-f0-9]{64}$/.test(input.clientSha256)) {
        return { ok: false, reason: "hash_mismatch" };
    }
    const clientUploaded = input.clientUploadedAtUtc instanceof Date
        ? input.clientUploadedAtUtc
        : input.clientUploadedAtUtc
            ? new Date(input.clientUploadedAtUtc)
            : null;
    try {
        const rows = (await client.$queryRawUnsafe(`UPDATE "evidence_upload_session_parts"
         SET "state" = CASE
         ...
         RETURNING "id", "session_id", "team_id", "part_index", "state",
           "expected_bytes", "client_sha256", "server_sha256",
           "uploaded_at_utc_client", "verified_at_utc", "retry_count",
           "failure_reason", "created_at_utc", "updated_at_utc"`, input.sessionId, input.teamId, input.partIndex, input.clientSha256 ?? null, clientUploaded));
        if (rows.length === 0) {
            return { ok: false, reason: "invalid_part_index" };
        }
 ...
 */
export async function markPartVerified(input, client = defaultPrisma) {
    if (!/^[a-f0-9]{64}$/.test(input.serverSha256)) {
        return { ok: false, reason: "hash_mismatch" };
    }
    try {
        // Fetch current row to compare client hash + state.
        const current = (await client.$queryRawUnsafe(`SELECT "state", "client_sha256"
         FROM "evidence_upload_session_parts"
         WHERE "session_id" = $1 AND "team_id" = $2 AND "part_index" = $3
         LIMIT 1`, input.sessionId, input.teamId, input.partIndex));
        if (current.length === 0) {
            return { ok: false, reason: "invalid_part_index" };
        }
        const row = current[0];
        if (row.state === "VERIFIED") {
            ...
            const fetch = (await client.$queryRawUnsafe(`SELECT "id", "session_id", "team_id", "part_index", "state",
                "expected_bytes", "client_sha256", "server_sha256",
                "uploaded_at_utc_client", "verified_at_utc", "retry_count",
                "failure_reason", "created_at_utc", "updated_at_utc"
           FROM "evidence_upload_session_parts"
           WHERE "session_id" = $1 AND "team_id" = $2 AND "part_index" = $3`, input.sessionId, input.teamId, input.partIndex));
        ...
        if (claimed && claimed !== input.serverSha256) {
            ...
            const failed = (await client.$queryRawUnsafe(`UPDATE "evidence_upload_session_parts"
           SET "state" = 'FAILED',
               "server_sha256" = $4,
           ...
           RETURNING "id", "session_id", "team_id", "part_index", "state",
             "expected_bytes", "client_sha256", "server_sha256",
             "uploaded_at_utc_client", "verified_at_utc", "retry_count",
             "failure_reason", "created_at_utc", "updated_at_utc"`, input.sessionId, input.teamId, input.partIndex, input.serverSha256));
            safeEmitSecurityEvent({
                teamId: input.teamId,
 ...
 * the existing row.
 */
export async function completeUploadSession(input, client = defaultPrisma) {
    try {
        // Atomic check + flip via a CTE so we can refuse completion if
        // any part is still PENDING/UPLOADED_UNVERIFIED/FAILED in a
        // single round-trip.
        const sessions = (await client.$queryRawUnsafe(`SELECT "id", "state", "expected_part_count"
         FROM "evidence_upload_sessions"
         WHERE "id" = $1 AND "team_id" = $2 LIMIT 1`, input.sessionId, input.teamId));
        if (sessions.length === 0) {
            return { ok: false, reason: "session_not_found" };
        }
        ...
        const pending = (await client.$queryRawUnsafe(`SELECT "part_index"
         FROM "evidence_upload_session_parts"
        ...
        const rows = (await client.$queryRawUnsafe(`UPDATE "evidence_upload_sessions"
         SET "state" = 'COMPLETED',
         ...
         RETURNING "id", "team_id", "evidence_id", "actor_user_id", "state",
           ...
           "safe_note", "idempotency_key", "expires_at_utc",
           "completed_at_utc", "aborted_at_utc", "aborted_by_user_id",
           "abort_reason", "created_at_utc", "updated_at_utc"`, input.sessionId, input.teamId));
        if (rows.length === 0) {
...
async function fetchSession(teamId, sessionId, client) {
    const rows = (await client.$queryRawUnsafe(`SELECT "id", "team_id", "evidence_id", "actor_user_id", "state",
            "expected_part_count", "expected_total_bytes", "expected_sha256",
            "safe_note", "idempotency_key", "expires_at_utc",
            "completed_at_utc", "aborted_at_utc", "aborted_by_user_id",
            "abort_reason", "created_at_utc", "updated_at_utc"
       FROM "evidence_upload_sessions"
       WHERE "id" = $1 AND "team_id" = $2 LIMIT 1`, sessionId, teamId));
    if (rows.length === 0) {
        return { ok: false, reason: "session_not_found" };
    }
    return { ok: true, session: projectSession(rows[0]) };
}
export async function abortUploadSession(input, client = defaultPrisma) {
    try {
        const rows = (await client.$queryRawUnsafe(`UPDATE "evidence_upload_sessions"
         SET "state" = 'ABORTED',
             "aborted_at_utc" = NOW(),
         ...
         RETURNING "id", "team_id", "evidence_id", "actor_user_id", "state",
           ...
           "safe_note", "idempotency_key", "expires_at_utc",
           "completed_at_utc", "aborted_at_utc", "aborted_by_user_id",
           "abort_reason", "created_at_utc", "updated_at_utc"`, input.sessionId, input.teamId, input.actorUserId, input.reason.slice(0, 120)));
        if (rows.length === 0) {
            return { ok: false, reason: "session_already_terminal" };
        }
 ...
 */
export async function reapStaleUploadSessions(client = defaultPrisma) {
    try {
        const result = await client.$executeRawUnsafe(`UPDATE "evidence_upload_sessions"
         SET "state" = 'EXPIRED',
             "updated_at_utc" = NOW()
 ...
 */
export async function evaluateUploadSessionFinalizeGate(input, client = defaultPrisma) {
    ...
    let rows;
    try {
        rows = (await client.$queryRawUnsafe(`SELECT "id", "state", "completed_at_utc", "aborted_at_utc",
    ...
    const reasonForState = async (row) => {
        ...
        if (state === "FAILED") {
            ...
            try {
                const mismatch = (await client.$queryRawUnsafe(`SELECT 1
             FROM "evidence_upload_session_parts"
             WHERE "session_id" = $1 AND "team_id" = $2
               AND "failure_reason" = 'hash_mismatch'
             LIMIT 1`, row.id, input.teamId));
                if (mismatch.length > 0)
                    return "session_hash_mismatch";
            }
            catch {
    ...
    for (const row of rows) {
        ...
        let pending;
        try {
            pending = (await client.$queryRawUnsafe(`SELECT 1
           FROM "evidence_upload_session_parts"
           WHERE "session_id" = $1 AND "team_id" = $2 AND "state" <> 'VERIFIED'
           LIMIT 1`, row.id, input.teamId));
        }
        ...
        completedAtUtcs.push(row.completed_at_utc?.toISOString() ?? row.created_at_utc.toISOString());
    }
...
async function loadSessionWithMultipart(client, teamId, sessionId) {
    const rows = (await client.$queryRawUnsafe(`SELECT "id", "team_id", "evidence_id", "state",
 ...
 */
export async function initiateStorageMultipart(input, client = defaultPrisma) {
    const session = await loadSessionWithMultipart(client, input.teamId, input.sessionId);
    if (!session) {
        return { ok: false, reason: "session_not_found" };
    }
    ...
    try {
        await client.$executeRawUnsafe(`UPDATE "evidence_upload_sessions"
         SET "state" = 'UPLOADING',
         ...
         WHERE "id" = $1 AND "team_id" = $2
           ...
           AND "multipart_upload_id" IS NULL`, input.sessionId, input.teamId, storage.multipartUploadId, storage.bucket, storage.key);
    }
    catch {
 ...
 */
export async function presignStorageUploadPart(input, client = defaultPrisma) {
    if (!Number.isInteger(input.partIndex) ||
        input.partIndex < 0 ||
    ...
    await client
        .$executeRawUnsafe(`UPDATE "evidence_upload_session_parts"
         SET "presigned_at_utc" = NOW(),
             "presign_expires_at_utc" = $4,
             "updated_at_utc" = NOW()
         WHERE "session_id" = $1 AND "team_id" = $2 AND "part_index" = $3`, input.sessionId, input.teamId, input.partIndex, new Date(presign.expiresAt))
        .catch(() => null);
 ...
 */
export async function recordPartEtag(input, client = defaultPrisma) {
    if (!input.etag.trim()) {
        return { ok: false, reason: "missing_part_etag" };
    }
    if (input.partSizeBytes != null &&
        (input.partSizeBytes < 0 || !Number.isFinite(input.partSizeBytes))) {
        return { ok: false, reason: "invalid_part" };
    }
    try {
        const count = (await client.$executeRawUnsafe(`UPDATE "evidence_upload_session_parts"
         SET "part_etag" = $4,
             "part_size_bytes" = COALESCE($5, "part_size_bytes"),
             "updated_at_utc" = NOW()
         WHERE "session_id" = $1 AND "team_id" = $2 AND "part_index" = $3`, input.sessionId, input.teamId, input.partIndex, input.etag, input.partSizeBytes ?? null));
        if (count === 0) {
            return { ok: false, reason: "invalid_part_index" };
        }
        return { ok: true };
 ...
 * contract intact.
 */
export async function completeStorageMultipart(input, client = defaultPrisma) {
    const session = await loadSessionWithMultipart(client, input.teamId, input.sessionId);
    if (!session) {
        return { ok: false, reason: "session_not_found" };
    }
    ...
    const parts = (await client.$queryRawUnsafe(`SELECT "part_index", "part_etag"
       FROM "evidence_upload_session_parts"
       WHERE "session_id" = $1 AND "team_id" = $2
       ORDER BY "part_index" ASC`, input.sessionId, input.teamId));
    if (parts.length === 0) {
    ...
    if (input.verifyHash) {
        ...
        if (!verify.ok) {
            ...
            if (verify.reason === "hash_mismatch") {
                ...
                await client
                    .$executeRawUnsafe(`UPDATE "evidence_upload_sessions"
               SET "state" = 'FAILED',
                   "updated_at_utc" = NOW()
               WHERE "id" = $1 AND "team_id" = $2`, input.sessionId, input.teamId)
                    .catch(() => null);
            }
    ...
    await client.$executeRawUnsafe(`UPDATE "evidence_upload_sessions"
       SET "completed_object_etag" = $3,
           ...
           "updated_at_utc" = NOW()
       WHERE "id" = $1 AND "team_id" = $2`, input.sessionId, input.teamId, result.etag, head.contentLength);
    ...
    if (input.verifyHash && serverSha256) {
        try {
            await client.$executeRawUnsafe(`UPDATE "evidence_upload_session_parts"
           SET "state" = 'VERIFIED',
               ...
               "updated_at_utc" = NOW()
           WHERE "session_id" = $1 AND "team_id" = $2
             AND "state" <> 'VERIFIED'`, input.sessionId, input.teamId, serverSha256);
        }
        catch {
    ...
    if (input.verifyHash &&
        ...
        try {
           ...
           SET "bridged_evidence_part_id" = $3,
               "updated_at_utc" = NOW()
           WHERE "id" = $1 AND "team_id" = $2
             AND "bridged_evidence_part_id" IS NULL`, input.sessionId, input.teamId, partRow.id);
        }
        catch {
 ...
 */
export async function abortStorageMultipart(input, client = defaultPrisma) {
    const session = await loadSessionWithMultipart(client, input.teamId, input.sessionId);
    if (!session) {
        return { ok: false, reason: "session_not_found" };
    }
    ...
    await client.$executeRawUnsafe(`UPDATE "evidence_upload_sessions"
       SET "state" = 'ABORTED',
           ...
           "updated_at_utc" = NOW()
       WHERE "id" = $1 AND "team_id" = $2
         AND "state" NOT IN ('COMPLETED', 'EXPIRED')`, input.sessionId, input.teamId, input.actorUserId, input.reason.slice(0, 120));
 ...
 * success case.
 */
export async function reapStaleMultipartUploads(client = defaultPrisma) {
    let scanned = 0;
    let aborted = 0;
    let failed = 0;
    let rows;
    try {
        rows = (await client.$queryRawUnsafe(`SELECT "id", "team_id", "storage_bucket", "storage_key", "multipart_upload_id"
         FROM "evidence_upload_sessions"
    ...
    for (const row of rows) {
        ...
        try {
           ...
           SET "state" = 'EXPIRED',
               "aborted_at_storage_utc" = NOW(),
               "updated_at_utc" = NOW()
           WHERE "id" = $1 AND "team_id" = $2`, row.id, row.team_id);
            aborted += 1;
