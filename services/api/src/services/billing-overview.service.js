...
import { ensureEntitlement, getStorageAddonDefinition, } from "./billing.service.js";
...
function addonStatusSortValue(status) {
    switch (status) {
...
function toStorageAddonSummary(addon, teamName) {
...
export async function readBillingOverview(userId) {
    ...
    const [ownedTeams, allStorageAddons, recentPayments, personalSubscription] = await Promise.all([
    ...
    const activePersonalStorageAddons = allStorageAddons.filter((addon) => addon.teamId === null &&
        (addon.status === prismaPkg.WorkspaceStorageAddonStatus.ACTIVE ||
            addon.status === prismaPkg.WorkspaceStorageAddonStatus.PAST_DUE));
    const teams = await Promise.all(ownedTeams.map(async (team) => {
        ...
        const displayPlanForSeats = team.billingPlan === prismaPkg.PlanType.TEAM
        ...
        const activeTeamStorageAddons = teamStorageAddons.filter((addon) => addon.status === prismaPkg.WorkspaceStorageAddonStatus.ACTIVE ||
            addon.status === prismaPkg.WorkspaceStorageAddonStatus.PAST_DUE);
        const activeTeamStorageAddonBytes = activeTeamStorageAddons.reduce((sum, addon) => sum + addon.extraStorageBytes, 0n);
        const includedSeats = Math.max(team.includedSeats ?? 0, scope.teamSeats || 0, displaySeatCaps.includedSeats ?? 0);
        return {
            id: team.id,
            name: team.name,
            workspaceType: "TEAM",
            ...
            features: {
                ...
                verificationPackageIncluded: effectiveCaps.verificationPackageIncluded,
            ...
            seats: {
                ...
                usageRatio: includedSeats > 0 ? usage.teamMemberCount / includedSeats : 0,
                usagePercent: includedSeats > 0
                    ? Math.min(100, Math.round((usage.teamMemberCount / includedSeats) * 100))
                    : 0,
                nearLimit: includedSeats > 0 &&
    ...
    const paymentSummary = recentPayments.reduce((acc, payment) => {
        acc.total += 1;
    ...
    const activeTeamCount = teams.filter((team) => team.billingStatus === "ACTIVE" || team.billingStatus === "PAST_DUE").length;
    const overSeatLimitCount = teams.filter((team) => team.overSeatLimit).length;
    const nearStorageLimitCount = teams.filter((team) => team.workspaceHealth.storageNearLimit ||
        team.workspaceHealth.storageLimitReached).length;
    const personalActiveAddonBytes = activePersonalStorageAddons.reduce((sum, addon) => sum + addon.extraStorageBytes, 0n);
    const totalActiveAddonBytes = allStorageAddons
        .filter((addon) => addon.status === prismaPkg.WorkspaceStorageAddonStatus.ACTIVE ||
        addon.status === prismaPkg.WorkspaceStorageAddonStatus.PAST_DUE)
    ...
    return {
        ...
        summary: {
            ...
            activeStorageAddons: allStorageAddons.filter((addon) => addon.status === prismaPkg.WorkspaceStorageAddonStatus.ACTIVE ||
                addon.status === prismaPkg.WorkspaceStorageAddonStatus.PAST_DUE).length,
        ...
        workspaces: {
            personal: {
                ...
                features: {
                    ...
                    verificationPackageIncluded: personalCaps.verificationPackageIncluded,
        ...
        storageAddons: {
            all: allStorageAddons
                ...
                .sort((a, b) => addonStatusSortValue(a.status) - addonStatusSortValue(b.status))
                .map((addon) => {
                const teamName = addon.teamId != null
