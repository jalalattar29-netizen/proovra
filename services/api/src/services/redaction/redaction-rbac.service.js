 ...
 */
import { REDACTION_CAPABILITY_MATRIX, } from "@proovra/shared";
import { prisma as defaultPrisma } from "../../db.js";
// ---------------------------------------------------------------------------
// Role resolution
// ---------------------------------------------------------------------------
const PLATFORM_ROLE_TO_REDACTION = [
    {
 ...
 */
export async function resolveRedactionRoles(prisma, userId, teamId) {
...
function effectiveCapabilities(roles) {
    const caps = new Set();
 ...
 * action.
 */
export async function buildRedactionContext(userId, teamId, prismaClient = defaultPrisma) {
...
export async function assertRedactionCapability(input) {
    const ctx = await buildRedactionContext(input.userId, input.teamId, input.prisma ?? defaultPrisma);
