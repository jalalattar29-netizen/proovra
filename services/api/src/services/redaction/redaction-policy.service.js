...
import { assignPolicyVersion, createPolicy, createPolicyVersion, resolveEffectivePolicy, transitionPolicyVersion, } from "./redaction-policy-store.service.js";
...
export async function isPolicyAllowed(input) {
    ...
    for (const p of input.providers) {
        if (effective.providers[p] === false)
            continue;
        allowed.add(p);
    }
    return allowed;
...
export function __resetPolicyCacheForTests() {
