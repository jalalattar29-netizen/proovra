 ...
 */
import { VIDEO_TRACK_KINDS, VIDEO_TRACK_STATES, classifyConfidence, isValidBboxNormalized, } from "@proovra/shared";
import { prisma as defaultPrisma } from "../../../db.js";
import { emitVideoTimelineEvent } from "./video-timeline.service.js";
// ---------------------------------------------------------------------------
// Writes
// ---------------------------------------------------------------------------
export async function createVideoTrack(input) {
    if (!VIDEO_TRACK_KINDS.includes(input.kind)) {
...
export async function appendTrackDetection(input) {
...
export async function decideVideoTrack(input) {
    if (!VIDEO_TRACK_STATES.includes(input.toState)) {
        return { ok: false, denial: "POLICY_REJECTED" };
    }
    const prisma = input.prisma ?? defaultPrisma;
    const track = await prisma.videoTrack.findFirst({
...
export async function mergeTracks(input) {
    ...
    const evidenceIds = new Set(tracks.map((t) => t.evidenceId));
    const kinds = new Set(tracks.map((t) => t.kind));
    if (evidenceIds.size !== 1 || kinds.size !== 1) {
        return { ok: false, denial: "POLICY_REJECTED" };
    }
    const startFrame = Math.min(...tracks.map((t) => t.startFrame));
    const endFrame = Math.max(...tracks.map((t) => t.endFrame));
    // Pick the highest-confidence track to inherit decisions from.
    const anchor = tracks
        .slice()
        .sort((a, b) => bandRank(a.confidenceBand) > bandRank(b.confidenceBand) ? -1 : 1)[0] ?? tracks[0];
...
export async function splitTrack(input) {
    const prisma = input.prisma ?? defaultPrisma;
    const track = await prisma.videoTrack.findFirst({
        where: { id: input.trackId, teamId: input.teamId },
    });
    if (!track)
        return { ok: false, denial: "VERSION_NOT_FOUND" };
    if (input.splitAfterFrame < track.startFrame ||
...
// Compile-time guard.
function _assertClassifyConfidenceUsed() {
