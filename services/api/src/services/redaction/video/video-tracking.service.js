...
import { appendTrackDetection, createVideoTrack, } from "./video-track.service.js";
export async function groupDetectionsIntoTracks(input) {
    ...
    const sorted = [...input.detections].sort((a, b) => a.frameIndex - b.frameIndex);
    const open = [];
    const closedTrackIds = [];
    for (const det of sorted) {
        let bestMatch = null;
        for (let i = 0; i < open.length; i++) {
            const o = open[i];
            if (det.frameIndex - o.lastFrame > 5)
                continue; // bounded gap
    ...
    for (const o of open) {
        const mean = o.confidences.reduce((a, b) => a + b, 0) / o.confidences.length;
        await prisma.videoTrack.update({
            where: { id: o.trackId },
...
function defaultMethodForKind(kind) {
    switch (kind) {
        case "FACE":
            return "BLUR";
        case "LICENSE_PLATE":
