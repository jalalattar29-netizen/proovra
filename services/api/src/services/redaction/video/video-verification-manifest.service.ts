...
export async function buildVideoTrackingVerificationEntry(input: {
...
}): Promise<VideoTrackingVerificationManifestEntry> {
  ...
  const perKind: Partial<Record<VideoTrackKind, number>> = {};
  let accepted = 0;
  let rejected = 0;
  for (const t of tracks) {
    perKind[t.kind as VideoTrackKind] =
      (perKind[t.kind as VideoTrackKind] ?? 0) + 1;
    if (t.state === "ACCEPTED") accepted++;
    if (t.state === "REJECTED") rejected++;
  }
  return {
    evidenceId: input.evidenceId,
