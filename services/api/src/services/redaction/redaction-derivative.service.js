 ...
 */
import { REDACTION_ARTIFACT_KINDS, } from "@proovra/shared";
import { prisma as defaultPrisma } from "../../db.js";
import { emitRedactionActivity } from "./redaction-activity.service.js";
const ARTIFACT_TO_DERIVATIVE = {
    IMAGE: "IMAGE_REDACTED",
    PDF: "PDF_REDACTED",
    VIDEO: "VIDEO_REDACTED",
    AUDIO: "AUDIO_REDACTED",
};
export async function requestRedactionDerivative(input) {
    ...
    if (!REDACTION_ARTIFACT_KINDS.includes(artifactKind)) {
...
export async function markDerivativeReady(input) {
    const prisma = input.prisma ?? defaultPrisma;
    ...
    if (!derivative)
        return { ok: false, denial: "DERIVATIVE_NOT_READY" };
...
export async function markDerivativeFailed(input) {
    const prisma = input.prisma ?? defaultPrisma;
    const derivative = await prisma.redactionDerivative.findFirst({
        where: { id: input.derivativeId, teamId: input.teamId },
        select: {
