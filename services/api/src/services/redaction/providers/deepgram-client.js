 ...
 */
export async function transcribeAndScan(input, client = buildClient()) {
    if (!client) {
        return {
            state: "NOT_CONFIGURED",
    ...
    try {
        ...
        const sdk = client.listen.prerecorded;
        ...
        const out = input.audioBytes
            ...
            : await sdk.transcribeUrl({ url: input.audioUrl }, transcribeOptions);
        if (out.error) {
 ...
 */
export function scanTranscriptForCandidates(transcript, words) {
...
function maskTranscriptPreview(label, value) {
    const t = value.trim();
    if (t.length <= 4)
        return label;
    if (t.includes("@")) {
