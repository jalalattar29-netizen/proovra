 ...
 */
import { ORG_SECURITY_POLICY_DEFAULTS } from "@proovra/shared";
import { prisma as defaultPrisma } from "../../db.js";
import { appendPlatformAuditLog } from "../platform-audit-log.service.js";
import { safeEmitSecurityEvent } from "../security/security-event.service.js";
export async function getOrgSecurityPolicy(teamId, client = defaultPrisma) {
...
export async function upsertOrgSecurityPolicy(input, client = defaultPrisma) {
    const current = await getOrgSecurityPolicy(input.teamId, client);
    const merged = {
        mfaRequiredFlag: input.mfaRequiredFlag !== undefined
        ...
        restrictedIpRanges: input.restrictedIpRanges !== undefined
            ? normaliseCidrs(input.restrictedIpRanges)
            : current.restrictedIpRanges,
        reviewerSessionTimeoutSeconds: input.reviewerSessionTimeoutSeconds !== undefined
            ? clampTimeoutSeconds(input.reviewerSessionTimeoutSeconds)
            : current.reviewerSessionTimeoutSeconds,
        contributorSessionTimeoutSeconds: input.contributorSessionTimeoutSeconds !== undefined
            ? clampTimeoutSeconds(input.contributorSessionTimeoutSeconds)
            : current.contributorSessionTimeoutSeconds,
        ssoReadyFlag: input.ssoReadyFlag !== undefined
            ? input.ssoReadyFlag
            : current.ssoReadyFlag,
        scimReadyFlag: input.scimReadyFlag !== undefined
            ? input.scimReadyFlag
    ...
    safeEmitSecurityEvent({
        ...
        details: {
            ...
            changes: Object.fromEntries(Object.entries(input).filter(([k, v]) => k !== "teamId" &&
                k !== "actorUserId" &&
                k !== "ipAddress" &&
                k !== "userAgent" &&
                v !== undefined)),
