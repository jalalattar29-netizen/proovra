...
import { STALE_ACCESS_DAYS_DEFAULT, UNUSED_SERVICE_ACCOUNT_DAYS_DEFAULT, EXPIRING_ACCESS_WINDOW_DAYS_DEFAULT, isAllowedAccessReviewTransition, } from "@proovra/shared";
import { prisma as defaultPrisma } from "../../db.js";
import { appendPlatformAuditLog } from "../platform-audit-log.service.js";
import { safeEmitSecurityEvent } from "../security/security-event.service.js";
import { revokeMember, suspendMember, RbacError, } from "./rbac.service.js";
...
// -----------------------------------------------------------------------------
export async function regenerateAccessReviewQueue(input, client = defaultPrisma) {
    const staleDays = Math.max(7, Math.min(input.staleAccessDays ?? STALE_ACCESS_DAYS_DEFAULT, 365));
    const unusedDays = Math.max(7, Math.min(input.unusedServiceAccountDays ?? UNUSED_SERVICE_ACCOUNT_DAYS_DEFAULT, 365));
    const expiringDays = Math.max(1, Math.min(input.expiringWindowDays ?? EXPIRING_ACCESS_WINDOW_DAYS_DEFAULT, 90));
    const now = Date.now();
...
async function enqueueIfNotExists(client, input) {
...
export async function completeAccessReview(input, client = defaultPrisma) {
    ...
    if (!review)
        throw new AccessReviewError("review_not_found");
    const nextStatus = DECISION_TO_STATUS[input.decision];
    if (!isAllowedAccessReviewTransition(review.status, nextStatus)) {
    ...
    if ((input.decision === "REVOKE_MEMBER" || input.decision === "SUSPEND_MEMBER") &&
