...
// Service
// =============================================================================
export async function indexExistingOcrAndTranscript(input, client = defaultPrisma) {
    const batchSize = Math.max(1, Math.min(input.evidenceBatchSize ?? 500, 5000));
    const empty = (reason) => ({
        ok: false,
        ocrRowsPresent: 0,
        ocrEvidenceCount: 0,
    ...
    let ocrAvailRows = [];
    try {
        ocrAvailRows = (await client.$queryRawUnsafe(`SELECT "evidence_id",
        ...
        LIMIT $2`, input.teamId, batchSize));
    }
    catch (err) {
        return empty(err instanceof Error
            ? `ocr_query_failed:${err.message.slice(0, 40)}`
            : "ocr_query_failed");
    }
    // 2) Transcript availability + indexing counts.
    let transcriptAvailRows = [];
    try {
        transcriptAvailRows = (await client.$queryRawUnsafe(`SELECT "evidence_id",
...
async function upsertWholeEvidenceSignal(client, input) {
    try {
       ...
       ON CONFLICT (
         ...
         "updated_at_utc" = NOW()`, input.teamId, input.evidenceId, input.signalType, input.severity, input.confidence, input.safeSummaryText);
        return true;
    }
    catch {
