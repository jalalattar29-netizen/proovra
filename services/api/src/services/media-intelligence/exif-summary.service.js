 ...
 */
export async function upsertExifSummary(input, client = defaultPrisma) {
    try {
        const s = input.summary;
        await client.$executeRawUnsafe(`INSERT INTO "evidence_part_exif_summaries" (
       ...
       ON CONFLICT ("team_id", "evidence_part_id") DO UPDATE
         SET "exif_present" = EXCLUDED."exif_present",
             ...
             "updated_at_utc" = NOW()`, input.teamId, input.evidenceId, input.evidencePartId, s.exifPresent, s.dateTimeOriginalUtc ? new Date(s.dateTimeOriginalUtc) : null, s.createDateUtc ? new Date(s.createDateUtc) : null, s.dimensions?.width ?? null, s.dimensions?.height ?? null, s.cameraMake, s.cameraModel, s.software, s.hasGps, s.orientation, input.engineVersion ?? "exifr-v7-phase31-v1", input.extractedBytes, input.status ?? "OK", sanitizeError(input.lastError ?? null));
        return { ok: true };
    }
    catch (err) {
        return {
 ...
 */
export async function recordExifSummaryFailure(input, client = defaultPrisma) {
    try {
        await client.$executeRawUnsafe(`INSERT INTO "evidence_part_exif_summaries" (
         "team_id", "evidence_id", "evidence_part_id",
       ...
       ON CONFLICT ("team_id", "evidence_part_id") DO UPDATE
         SET "status" = EXCLUDED."status",
             ...
             "extracted_bytes" = EXCLUDED."extracted_bytes",
             "engine_version" = EXCLUDED."engine_version",
             "updated_at_utc" = NOW()`, input.teamId, input.evidenceId, input.evidencePartId, input.engineVersion ?? "exifr-v7-phase31-v1", input.extractedBytes ?? 0, input.status, sanitizeError(input.lastError ?? null));
        return { ok: true };
    }
    catch {
        return { ok: false };
 ...
 */
export async function listExifSummariesForEvidence(teamId, evidenceId, client = defaultPrisma) {
    try {
        const rows = (await client.$queryRawUnsafe(`SELECT "id", "team_id", "evidence_id", "evidence_part_id",
              "exif_present",
         ...
         ORDER BY "extracted_at_utc" DESC`, teamId, evidenceId));
 ...
 */
export async function getExifSummaryForPart(teamId, evidencePartId, client = defaultPrisma) {
    try {
        const rows = (await client.$queryRawUnsafe(`SELECT "id", "team_id", "evidence_id", "evidence_part_id",
              "exif_present",
              "date_time_original_utc", "create_date_utc",
...
function projectRow(raw) {
    const dims = raw.image_width_px != null && raw.image_height_px != null
