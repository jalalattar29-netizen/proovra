 ...
 */
export async function recordDerivedAsset(input, client = defaultPrisma) {
    try {
        const rows = (await client.$queryRawUnsafe(`INSERT INTO "evidence_part_derived_assets" (
       ...
       ON CONFLICT ("team_id", "evidence_part_id", "asset_kind") DO UPDATE
         SET "status" = EXCLUDED."status",
             ...
             "generated_at_utc" =
               ...
               END,
             "updated_at_utc" = NOW()
         RETURNING "id"`, input.teamId, input.evidenceId, input.evidencePartId, input.assetKind, input.status, input.derivedSha256 ?? null, input.sizeBytes ?? null, input.contentType ?? null, input.widthPx ?? null, input.heightPx ?? null, input.sourceSha256AtGeneration ?? null, input.storageBucket ?? null, input.storageKey ?? null, sanitizeError(input.lastError ?? null), input.engineVersion ?? "sharp-v0-phase31-v1"));
        const row = rows[0];
        if (!row)
            return { ok: false, reason: "upsert_returned_no_row" };
 ...
 */
export async function listDerivedAssetsForEvidence(teamId, evidenceId, client = defaultPrisma) {
    try {
        const rows = (await client.$queryRawUnsafe(`SELECT "id", "evidence_id", "evidence_part_id", "asset_kind",
              "status", "derived_sha256", "size_bytes", "content_type",
              "width_px", "height_px",
              "source_sha256_at_generation",
              "last_error", "engine_version",
        ...
        ORDER BY "asset_kind" ASC, "updated_at_utc" DESC`, teamId, evidenceId));
 ...
 */
export async function _getDerivedAssetStorageReference(teamId, id, client = defaultPrisma) {
    try {
        const rows = (await client.$queryRawUnsafe(`SELECT "storage_bucket", "storage_key", "content_type"
         FROM "evidence_part_derived_assets"
        WHERE "id" = $1 AND "team_id" = $2 AND "status" = 'COMPLETED'
        LIMIT 1`, id, teamId));
        const r = rows[0];
        if (!r || !r.storage_bucket || !r.storage_key)
            return null;
        return {
