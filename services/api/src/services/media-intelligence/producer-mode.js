...
const OCR_MODE_SET = new Set(OCR_PRODUCER_MODES);
const TRANSCRIPT_MODE_SET = new Set(TRANSCRIPT_PRODUCER_MODES);
export function getOcrProducerMode(env = process.env) {
...
export function getTranscriptProducerMode(env = process.env) {
...
export function summariseProducerModes(env = process.env) {
    const ocr = getOcrProducerMode(env);
    const transcript = getTranscriptProducerMode(env);
    const producesNewContent = ocr !== "NOT_CONFIGURED" &&
