...
// Entry point
// =============================================================================
export async function extractExifSafe(input) {
    ...
    if (input.mimeType &&
        !SUPPORTED_MIME_PREFIXES.some((p) => input.mimeType.toLowerCase().startsWith(p))) {
...
function projectSafeSummary(raw, allowRawGps) {
...
function pickDimension(a, b) {
    ...
    if (candidate <= 0)
        return null;
    if (candidate > 200_000)
        return null; // sanity ceiling
    return Math.floor(candidate);
}
function sanitizeShortString(v) {
    if (v == null)
        return null;
    ...
    if (!SAFE_STRING_CHARSET.test(s))
        return null;
    return s.slice(0, STRING_FIELD_MAX);
}
function pickOrientation(v) {
    if (v == null)
        return null;
 ...
 */
export function computeExifVsServerGapSeconds(exifIso, serverUtc) {
    if (!exifIso || !serverUtc)
        return null;
    try {
        const exifMs = new Date(exifIso).getTime();
        const serverMs = serverUtc.getTime();
        if (!Number.isFinite(exifMs) || !Number.isFinite(serverMs))
            return null;
