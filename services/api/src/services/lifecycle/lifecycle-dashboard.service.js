 ...
 */
import { ARCHIVE_TIERS, LIFECYCLE_DASHBOARD_SCHEMA_VERSION, PRODUCT_AND_LIFECYCLE_LIMITATIONS, } from "@proovra/shared";
import { countPolicyViolations } from "./policy-violation.service.js";
import { prisma as defaultPrisma } from "../../db.js";
const THIRTY_DAYS_MS = 30 * 24 * 60 * 60 * 1000;
const NINETY_DAYS_MS = 90 * 24 * 60 * 60 * 1000;
export async function projectLifecycleDashboard(input) {
    ...
    const archive = await (async () => {
        ...
        for (const r of rows) {
            ...
            const tier = ARCHIVE_TIERS.includes(r.toTier)
                ? r.toTier
                : null;
            if (!tier)
                continue;
        ...
        countByTier: { HOT: 0, WARM: 0, COLD: 0, DEEP_ARCHIVE: 0 },
        costMicrosByTier: {
            HOT: 0,
            WARM: 0,
            COLD: 0,
    ...
    const violations = await (async () => {
        ...
        const pvc = await countPolicyViolations({ prisma, teamId, since: since30 }).catch(() => ({ total: 0, byKind: {} }));
        const byCode = {
    ...
    const compliance = (() => {
        ...
        const holdCoverage = activeHolds === 0 && v === 0
            ? 1
            : Math.max(0, 1 - v / Math.max(1, activeHolds + v));
        const overallScore = Math.round(((retentionCoverage + holdCoverage) / 2) * 100) / 100;
        return {
            overallScore,
