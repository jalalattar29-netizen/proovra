...
import {
  ARCHIVE_TIERS,
  type ArchiveTier,
  type ArchiveTransitionProjection,
} from "@proovra/shared";
...
export const TIER_TO_S3_STORAGE_CLASS: Record<ArchiveTier, string> = {
  HOT: "STANDARD",
  WARM: "STANDARD_IA",
  COLD: "GLACIER",
...
const ASYNC_GLACIER_TIERS: ReadonlySet<ArchiveTier> = new Set([
  "COLD",
  "DEEP_ARCHIVE",
]);

/** Tiers that must go through RestoreObject before bytes are accessible. */
const RESTORE_REQUIRED_TIERS: ReadonlySet<ArchiveTier> = new Set([
  "COLD",
  "DEEP_ARCHIVE",
]);
 ...
 */
export const COST_PER_GB_PER_MONTH_USD_MICROS: Record<ArchiveTier, number> = {
  HOT: 23_000,
  WARM: 12_500,
  COLD: 4_000,
  DEEP_ARCHIVE: 990,
...
function computeMonthlyCostUsdMicros(sizeBytes: bigint, tier: ArchiveTier): number {
  const ratePerGb = COST_PER_GB_PER_MONTH_USD_MICROS[tier];
  if (sizeBytes <= 0n) return 0;
...
function isArchiveTier(value: string): value is ArchiveTier {
  return (ARCHIVE_TIERS as ReadonlyArray<string>).includes(value);
}

function boundFailureReason(err: unknown): string {
...
async function tryEmitWebhookEvent(
  client: PrismaClient,
  teamId: string,
  eventKind: "ARCHIVE_TIER_CHANGED" | "ARCHIVE_COMPLETED",
    ...
    await emitWebhookEvent(
      {
        ...
        eventType: eventKind as unknown as any,
        payload,
        attemptInline: false,
      },
      client,
...
export async function currentTierFor(input: {
  ...
  teamId: string;
  evidenceId: string;
  /** When true, also consults S3 headObject for the live storage class. */
  checkS3?: boolean;
}): Promise<ArchiveTier> {
  ...
  if (input.checkS3) {
    ...
    if (ev?.storageBucket && ev.storageKey) {
      try {
        ...
        if (liveS3Tier) return liveS3Tier;
      } catch {
  ...
  if (latest && isArchiveTier(latest.toTier)) return latest.toTier;
  return "HOT";
}

function s3StorageClassToTier(storageClass: string): ArchiveTier | null {
  for (const [tier, sc] of Object.entries(TIER_TO_S3_STORAGE_CLASS)) {
    if (sc === storageClass) return tier as ArchiveTier;
  }
  return null;
}
...
export type TransitionEvidenceTierInput = {
  prisma?: PrismaClient;
  teamId: string;
  evidenceId: string;
  toTier: ArchiveTier;
...
export type TransitionEvidenceTierResult = {
  ok: true;
  transitionId: string;
...
export async function transitionEvidenceTier(
  input: TransitionEvidenceTierInput,
): Promise<TransitionEvidenceTierResult> {
  ...
  if (!bucket || !key) {
    // eslint-disable-next-line @typescript-eslint/no-explicit-any
    await (prisma.archiveTierTransition.update as any)({
      where: { id: row.id },
...
export type RestoreEvidenceTierInput = {
  prisma?: PrismaClient;
  teamId: string;
  evidenceId: string;
  initiatedByUserId?: string | null;
  /** How many days the restored copy should remain accessible (default 7). */
  restoreDays?: number;
};

export type RestoreEvidenceTierResult = {
  ok: true;
  transitionId: string;
};
 ...
 */
export async function restoreEvidenceTier(
  input: RestoreEvidenceTierInput,
): Promise<RestoreEvidenceTierResult> {
  const prisma = input.prisma ?? defaultPrisma;
  ...
  if (!RESTORE_REQUIRED_TIERS.has(currentTier)) {
    throw Object.assign(
      new Error(`restore_not_required: evidence is in tier ${currentTier}`),
      { code: "restore_not_required", currentTier },
    );
  }
...
export async function listArchiveTransitions(input: {
  ...
  evidenceId?: string;
  toTier?: ArchiveTier;
}): Promise<ArchiveTransitionProjection[]> {
  const prisma = input.prisma ?? defaultPrisma;
...
export type ArchiveTierCostBucket = {
...
export async function projectArchiveCostsByTier(input: {
  ...
  for (const ev of evidenceRows) {
    const tier: ArchiveTier = latestByEvidence.get(ev.id) ?? "HOT";
...
export async function scheduleAutoTransitions(input: {
  ...
  if (evidenceRows.length === 0) return { scheduled: 0 };
