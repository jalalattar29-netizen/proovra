...
export async function buildLifecycleManifestEntry(input: {
  ...
  teamId: string;
  evidenceId?: string;
}): Promise<LifecycleManifestEntry> {
...
export async function buildRetentionManifestEntry(input: {
  ...
  return {
    ...
    policyId: row?.id ?? "",
    policyName: row?.name ?? "",
    template: (row?.template ?? "CUSTOM") as RetentionPolicyTemplate,
    years: row?.years ?? 0,
...
export async function buildLegalHoldManifestEntry(input: {
  ...
  teamId: string;
  evidenceId?: string;
}): Promise<LegalHoldManifestEntry> {
  const prisma = input.prisma ?? defaultPrisma;
  const { teamId, evidenceId } = input;
  ...
  return {
    ...
    generatedAtUtc: nowUtc(),
    holdId: row?.id ?? "",
    kind: (row?.scope ?? "EVIDENCE") as unknown as LegalHoldKind,
    state: (row?.status ?? "ACTIVE") as unknown as LegalHoldState,
...
export async function buildArchiveManifestEntry(input: {
  ...
  return {
    ...
    fromTier: (row?.fromTier ?? "HOT") as ArchiveTier,
    toTier: (row?.toTier ?? "HOT") as ArchiveTier,
...
export async function buildDestructionManifestEntry(input: {
  ...
  teamId: string;
  evidenceId?: string;
}): Promise<DestructionManifestEntry> {
  const prisma = input.prisma ?? defaultPrisma;
  const { teamId, evidenceId } = input;
  ...
  return {
    ...
    generatedAtUtc: nowUtc(),
    requestId: row?.id ?? "",
    state: (row?.state ?? "REQUESTED") as DestructionState,
    evidenceCount,
...
export async function buildExchangeManifestEntry(input: {
  ...
  return {
    ...
    generatedAtUtc: nowUtc(),
    packageId: row?.id ?? "",
    kind: (row?.kind ?? "EVIDENCE") as ExchangePackageKind,
    state: (row?.state ?? "DRAFT") as ExchangePackageState,
    evidenceCount,
...
export async function buildTransferManifestEntry(input: {
  ...
  return {
    ...
    state: (row?.state ?? "INITIATED") as ChainTransferState,
    evidenceCount,
    createdAt: row?.createdAt?.toISOString() ?? nowUtc(),
...
export type VerificationPackageLifecyclePreviewKind =
  (typeof VERIFICATION_PACKAGE_LIFECYCLE_PREVIEW_KINDS)[number];

export type VerificationPackageLifecyclePreviewEntry =
  | LifecycleManifestEntry
  | RetentionManifestEntry
  | LegalHoldManifestEntry
  | ArchiveManifestEntry
...
export async function buildLifecyclePackagePreview(input: {
  prisma?: PrismaClient;
  teamId: string;
  kind: VerificationPackageLifecyclePreviewKind;
  evidenceId?: string;
}): Promise<VerificationPackageLifecyclePreviewEntry> {
  const args = {
    prisma: input.prisma,
    teamId: input.teamId,
