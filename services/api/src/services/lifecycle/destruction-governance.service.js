...
import { DESTRUCTION_LIFECYCLE_CODES, } from "@proovra/shared";
import { prisma as defaultPrisma } from "../../db.js";
import { isUnderLegalHold, assertNoLegalHoldOrBlock, } from "./legal-hold.service.js";
...
// Constants
// -----------------------------------------------------------------------------
const CERTIFICATE_VERSION = "PROOVRA_DESTRUCTION_CERT_V1";
...
function signatureHmac(payload) {
    const secret = process.env.DESTRUCTION_CERT_SIGNING_SECRET ??
...
function isApproverEntryArray(value) {
...
function readApproverChain(value) {
    if (!isApproverEntryArray(value))
        return [];
    return value.map((e) => ({
        userId: e.userId,
...
function approverChainUserIds(chain) {
...
function hashSortedEvidenceIds(evidenceIds) {
...
// -----------------------------------------------------------------------------
async function checkLegalHoldBlocks(prisma, evidenceIds, teamId) {
    const blocked = [];
    for (const evidenceId of evidenceIds) {
        const result = await isUnderLegalHold({
            ...
            teamId,
            evidenceId,
        }).catch(() => ({ underHold: false, holdIds: [], sources: [] }));
        if (result.underHold)
            blocked.push(evidenceId);
    }
    if (blocked.length === 0)
        return { ok: true };
...
// -----------------------------------------------------------------------------
async function emitDestructionLifecycle(prisma, code, payload) {
    if (!DESTRUCTION_LIFECYCLE_CODES.includes(code)) {
        return;
    }
    ...
    try {
        ...
        await prisma.intelligenceActivityEvent
            .create({
            data: {
...
async function emitDestructionWebhook(prisma, code, body) {
...
export async function createDestructionRequest(input) {
    ...
    const approverChain = input.requiredApproverUserIds.map((userId) => ({ userId, approvedAtUtc: null }));
    const evidenceIdsJson = [...input.evidenceIds];
    const approverIdsJson = approverChain;
...
export async function recordApproval(input) {
    ...
    const nextState = allApproved ? "APPROVED" : "REQUESTED";
...
export async function rejectDestructionRequest(input) {
    const prisma = input.prisma ?? defaultPrisma;
    const row = await prisma.destructionRequest.findFirst({
        where: { id: input.requestId, teamId: input.teamId },
...
export async function executeDestruction(input) {
    const prisma = input.prisma ?? defaultPrisma;
    const row = await prisma.destructionRequest.findFirst({
        where: { id: input.requestId, teamId: input.teamId },
    ...
    const evidenceIds = Array.isArray(row.evidenceIds)
        ? row.evidenceIds.filter((v) => typeof v === "string")
        : [];
    const blockedAtExecute = [];
    for (const evidenceId of evidenceIds) {
        const assertResult = await assertNoLegalHoldOrBlock({
            ...
            evidenceId,
            action: "DESTROY",
        }).catch(() => ({ ok: false, denial: "LEGAL_HOLD_BLOCKED", holdIds: [] }));
        if (!assertResult.ok)
            blockedAtExecute.push(evidenceId);
    }
    ...
    try {
        ...
        if (bucket) {
            ...
            const allKeys = [
                ...evidenceRows.map((r) => r.storageKey).filter((k) => Boolean(k)),
                ...derivativeRows.map((r) => r.storageKey).filter((k) => Boolean(k)),
            ];
        ...
        if (err.denial === "DESTRUCTION_PARTIAL_FAILURE")
            throw err;
...
export async function certifyDestruction(input) {
    const prisma = input.prisma ?? defaultPrisma;
    const row = await prisma.destructionRequest.findFirst({
        where: { id: input.requestId, teamId: input.teamId },
        select: {
    ...
    const artifactBytes = Buffer.from(JSON.stringify(artifactBody, null, 2), "utf8");
    ...
    const approvalChainJson = approvalChainUserIds;
    // eslint-disable-next-line @typescript-eslint/no-explicit-any
    const cert = await prisma.destructionCertificate.create({
        data: {
            teamId: row.teamId,
            requestId: row.id,
            evidenceCount: evidenceIds.length,
    ...
    // eslint-disable-next-line @typescript-eslint/no-explicit-any
    await emitDestructionWebhook(prisma, "DESTRUCTION_COMPLETED", {
        teamId: row.teamId,
        requestId: row.id,
        state: "CERTIFIED",
    });
...
function projectCertificate(row) {
    const chain = Array.isArray(row.approvalChainUserIds)
        ? row.approvalChainUserIds.filter((v) => typeof v === "string")
        : [];
    return {
        id: row.id,
...
export async function listDestructionRequests(input) {
    const prisma = input.prisma ?? defaultPrisma;
...
export async function getDestructionCertificate(input) {
    const prisma = input.prisma ?? defaultPrisma;
...
export async function getDestructionCertificateArtifact(input) {
    const prisma = input.prisma ?? defaultPrisma;
    // eslint-disable-next-line @typescript-eslint/no-explicit-any
    const cert = await prisma.destructionCertificate.findFirst({
        where: { teamId: input.teamId, requestId: input.requestId },
