...
import { buildNavigationProjection, filterNavigationRegistry, } from "./navigation-registry.js";
import { CAPABILITY_SCHEMA_VERSION, NAVIGATION_SCHEMA_VERSION, isWorkspaceRole, } from "./types.js";
...
const ENTERPRISE_PLAN_KEYS = new Set(["TEAM"]);
const PRO_PLAN_KEYS = new Set(["PRO", "TEAM"]);
 ...
 */
export async function buildPlatformContext(input) {
    const now = new Date();
    ...
    try {
        isPlatformAdmin = await resolveIsPlatformAdmin(userRow.id, input.jwtRole ?? null);
    }
    catch {
    ...
    // -------------------------------------------------------------------------
    let workspaceStatus = "ok";
    let workspaceSource = "personal_bootstrap";
    const bootstrap = {
        attempted: false,
        reused: false,
        created: false,
        ...
        try {
            ...
            if (!team || !memberOk) {
                ...
                workspace = {
                    ...
                    plan: coercePlan(team.billingPlan),
                    membership: {
                        role,
                        isOwner: role === "OWNER",
    ...
    if (workspace.status !== "active") {
        if (personalTeamId) {
            ...
            if (!bootstrap.activeWorkspaceUpdated &&
    ...
    if (workspace.scope === "PERSONAL") {
        try {
            ...
            const personalPlan = coercePlan(personalEntitlement?.plan);
            if (personalPlan) {
    ...
    try {
        ...
        accountPlan = coercePlan(ent?.plan);
    }
    catch {
        accountPlan = null;
    }
    ...
    // -------------------------------------------------------------------------
    let availableWorkspacesStatus = "ok";
    const organizations = [];
    const personalTeams = [];
    try {
        ...
        for (const m of memberRows) {
            ...
            if (m.team.isPersonal) {
                personalTeams.push({
                    ...
                    billingPlan: m.team.billingPlan ?? null,
                });
                continue;
            }
            organizations.push({
                ...
                membershipStatus: m.status === "ACTIVE"
                    ...
                    : m.status === "PENDING"
                        ? "PENDING"
                        : "INACTIVE",
                plan: coercePlan(m.team.billingPlan),
                memberCount: m.team._count?.members ?? 0,
            });
        }
    }
    ...
    if (userRow.email && organizations.length > 0) {
        ...
        for (const org of organizations) {
            ...
            try {
                ...
                if (isSingleMember)
                    reasons.push("single_owner_member");
                if (isFreePlan)
                ...
                if (isOwner && isSingleMember) {
                    duplicatePersonalCandidates.push({
                        teamId: org.id,
                        name: org.name,
                        ownerUserId: teamRow.ownerUserId,
    ...
    // ===========================================================================
    const personalSpace = personalTeamId
        ? {
            status: "active",
            id: personalTeamId,
    ...
    // ===========================================================================
    let activeSpace;
    const workspaceIdNonEmpty = typeof workspace.id === "string" && workspace.id.length > 0;
    if (workspace.scope === "TEAM" && workspaceIdNonEmpty) {
        ...
        const personalId = workspaceIdNonEmpty
            ? workspace.id
    ...
    const wireVersion = input.requestedSchemaVersion === 3 ? 3 : 2;
    const envelope = {
        ...
        diagnostics: {
            ...
            workspaceSource,
            bootstrap,
            activeSpaceSource: activeSpace.type === "ORGANIZATION"
                ? "organization"
                : personalTeamId
                    ? bootstrap.created
                    ...
                    : "unavailable",
            staleWorkspaceHealed: workspaceSource === "personal_bootstrap_after_stale",
