 ...
 */
function appearsOnSurface(item, target) {
    const declared = item.surface ?? "SIDEBAR";
    if (declared === "BOTH")
        return target === "SIDEBAR" || target === "ACCOUNT_MENU";
    return declared === target;
}
function passesCapabilityGate(item, caps) {
    if (item.requiresCapability === null)
        return true;
 ...
 */
export function filterNavigationRegistry(caps) {
 ...
 */
export function buildNavigationProjection(caps) {
    const sidebarGroups = NAVIGATION_REGISTRY.map((group) => {
        const items = group.items
            .filter((item) => appearsOnSurface(item, "SIDEBAR"))
            .filter((item) => passesCapabilityGate(item, caps))
            .map(projectItem);
    ...
    for (const group of NAVIGATION_REGISTRY) {
        for (const item of group.items) {
            if (!appearsOnSurface(item, "ACCOUNT_MENU"))
                continue;
            if (!passesCapabilityGate(item, caps))
                continue;
            if (seen.has(item.id))
                continue;
            seen.add(item.id);
 ...
 */
export function buildPillarProjection(caps) {
    // Collect all sidebar-eligible items that pass the capability gate.
    const allItems = [];
    for (const group of NAVIGATION_REGISTRY) {
        for (const registryItem of group.items) {
            if (!appearsOnSurface(registryItem, "SIDEBAR"))
                continue;
            if (!passesCapabilityGate(registryItem, caps))
                continue;
            const pillar = DOMAIN_TO_PILLAR[registryItem.domain] ?? "ADMIN";
