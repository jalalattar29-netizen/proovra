 ...
 */
function findElement(parent, localName) {
    ...
    for (let i = 0; i < children.length; i++) {
        ...
        if (node && node.nodeType === 1) {
            const found = findElement(node, localName);
            if (found)
                return found;
        }
    }
 ...
 * in document order.
 */
function findAllElements(root, localName) {
    const results = [];
    function walk(node) {
 ...
 */
function computeCertFingerprint(base64Cert) {
 ...
 */
export function parseSamlMetadata(xmlString) {
    // Guard: must be a non-empty string
    if (typeof xmlString !== "string" || xmlString.trim().length === 0) {
        throw new SamlMetadataError("METADATA_PARSE_FAILED", "Metadata XML must be a non-empty string.");
    }
    ...
    if (xmlString.includes("<!ENTITY")) {
        throw new SamlMetadataError("METADATA_XML_UNSAFE", "Metadata XML contains DTD entity declarations which are not permitted.");
    }
    ...
    if (parseError) {
        throw new SamlMetadataError("METADATA_PARSE_FAILED", `XML parse error: ${parseError}`);
    }
    if (!doc || !doc.documentElement) {
        throw new SamlMetadataError("METADATA_PARSE_FAILED", "XML parser produced no document element.");
    }
    ...
    if (!entityId || entityId.trim() === "") {
        throw new SamlMetadataError("METADATA_MISSING_ENTITY_ID", "EntityDescriptor/@entityID is absent or empty.");
    }
    // -------------------------------------------------------------------------
    ...
    const ssoElements = findAllElements(doc, "SingleSignOnService");
    let ssoUrl = null;
    ...
    if (!ssoUrl) {
        throw new SamlMetadataError("METADATA_MISSING_SSO_URL", "No SingleSignOnService with HTTP-Redirect or HTTP-POST binding found.");
    }
    // -------------------------------------------------------------------------
    ...
    const idpSsoDescriptor = findElement(doc, "IDPSSODescriptor");
    if (idpSsoDescriptor) {
    ...
    if (!rawCert) {
        const allCerts = findAllElements(doc, "X509Certificate");
    ...
    if (!rawCert) {
        throw new SamlMetadataError("METADATA_MISSING_CERTIFICATE", "No X509Certificate found in IdP metadata.");
    }
    // -------------------------------------------------------------------------
    ...
    const nameIdEl = findElement(doc, "NameIDFormat");
    if (nameIdEl) {
 ...
 */
export async function fetchAndParseSamlMetadata(metadataUrl) {
    // Only allow HTTPS to prevent MITM / metadata substitution
    if (typeof metadataUrl !== "string" ||
        !metadataUrl.startsWith("https://")) {
        throw new SamlMetadataError("METADATA_FETCH_FAILED", "Metadata URL must use HTTPS.");
    }
    let response;
    try {
        ...
        throw new SamlMetadataError("METADATA_FETCH_FAILED", `Network error fetching metadata: ${msg}`);
    }
    if (!response.ok) {
        throw new SamlMetadataError("METADATA_FETCH_FAILED", `Metadata endpoint returned HTTP ${response.status}.`);
    }
    let xmlText;
    try {
        ...
        const msg = err instanceof Error ? err.message : String(err);
        throw new SamlMetadataError("METADATA_FETCH_FAILED", `Failed to read metadata response body: ${msg}`);
    }
