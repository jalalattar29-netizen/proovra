...
import { mfaRequiredForRole } from "@proovra/shared";
 ...
 */
function readGlobalFailMode() {
    const raw = (process.env.MFA_ENFORCEMENT_FAIL_MODE ?? "smart").toLowerCase();
    if (raw === "open" || raw === "closed" || raw === "smart")
        return raw;
    return "smart";
}
const PER_ORG_FAIL_MODE_VALUES = [
    "SMART",
    "FAIL_OPEN",
    "FAIL_CLOSED",
];
function normalisePerOrgFailMode(raw) {
    if (!raw)
        return null;
    const upper = raw.toUpperCase();
    if (!PER_ORG_FAIL_MODE_VALUES.includes(upper))
        return null;
    if (upper === "FAIL_OPEN")
        return "open";
 ...
 */
function pickStrictestPerOrgFailMode(policies) {
    let best = null;
 ...
 */
export async function resolveLoginMfaEnforcement(input) {
    ...
    try {
        ...
        activeFactorCount = status.factors.filter((f) => f.status === "ACTIVE").length;
    }
    catch {
    ...
    if (!membershipLookupOk) {
        return circuitBreakerOutcome("membership_lookup_failed", activeFactorCount, failMode, input.userId);
    }
    if (!factorLookupOk) {
        return circuitBreakerOutcome("factor_lookup_failed", activeFactorCount, failMode, input.userId, memberships.length > 0);
    }
    ...
    try {
        ...
        return circuitBreakerOutcome("policy_lookup_failed", activeFactorCount, failMode, input.userId, memberships.length > 0);
    }
    ...
    let strictestTeamId = null;
    let strictestLevel = null;
    let strictestRank = -1;
    for (const m of memberships) {
        ...
        if (p?.mfaPolicyLevel && typeof p.mfaPolicyLevel === "string") {
            ...
            if (candidates.includes(p.mfaPolicyLevel)) {
                level = p.mfaPolicyLevel;
            }
        }
        ...
        if (!mfaRequiredForRole(level, m.role))
            continue;
 ...
 */
function circuitBreakerOutcome(kind, activeFactorCount, failMode, userId, hasObservedMembership = false) {
    ...
    const looksOrgScoped = hasObservedMembership ||
