...
export async function listDigestPreferences(input) {
...
export async function updateDigestPreference(input) {
    ...
    if (changed) {
        void appendPlatformAuditLog({
            ...
            metadata: {
                teamId: input.teamId,
                digestEnabled: input.digestEnabled ?? (prior?.digestEnabled ?? true),
                suppressUntil: (suppressUntilDate ?? prior?.suppressUntil ?? null)?.toISOString() ??
                    null,
            },
        ...
        safeEmitSecurityEvent({
            ...
            details: {
                ...
                teamId: input.teamId,
                digestEnabled: input.digestEnabled ?? (prior?.digestEnabled ?? true),
                suppressUntilIso: (suppressUntilDate ?? prior?.suppressUntil ?? null)?.toISOString() ??
                    null,
            },
        });
    }
 ...
 */
export function shouldSendDigest(prefs, teamId, now = new Date()) {
