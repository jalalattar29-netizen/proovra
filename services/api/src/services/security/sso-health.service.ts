...
export type SsoConnectionHealthStatus =
  | "HEALTHY"
  | "DEGRADED"
  | "OUTAGE"
  | "DISABLED"
...
export type CertExpiryBand = "ok" | "warning" | "expiring" | "expired";

export type SsoConnectionHealth = {
  connectionId: string;
  provider: string;
  status: string;
...
export async function buildSsoHealthSnapshot(
  input: { teamId: string },
  client: PrismaClient = defaultPrisma,
): Promise<SsoHealthSnapshot> {
  const now = Date.now();
  ...
  for (const conn of connections) {
    ...
    const reason24 = new Map<SsoFailureReason, number>();
    const reason7 = new Map<SsoFailureReason, number>();
    ...
    for (const a of attempts) {
      ...
      if (a.status === "FAILED") {
        failed7++;
        if (recent24) failed24++;
        if (a.issuedAtUtc > (lastFailure ?? new Date(0))) lastFailure = a.issuedAtUtc;
        const r = classifyFailure(a.failureReason);
        reason7.set(r, (reason7.get(r) ?? 0) + 1);
        if (recent24) reason24.set(r, (reason24.get(r) ?? 0) + 1);
    ...
    let health: SsoConnectionHealthStatus;
    if (conn.status === "DISABLED" || conn.status === "REVOKED") {
      health = "DISABLED";
    } else if (
      !conn.samlCertFingerprint ||
      conn.status === "PENDING"
    ) {
      health = "UNCONFIGURED";
      ...
      (conn.consecutiveFailureCount ?? 0) >= OUTAGE_THRESHOLD ||
      cert.expiryBand === "expiring"
    ) {
      health = "DEGRADED";
    } else if (
      (conn.consecutiveFailureCount ?? 0) >= DEGRADED_THRESHOLD ||
    ...
    if (health === "DEGRADED") aggregatedDegraded = true;
    if (health === "OUTAGE") aggregatedOutage = true;

    // Failure breakdown
    const reasons: SsoFailureReason[] = [
      "invalid_signature",
      "expired_certificate",
      "audience_mismatch",
      "acs_mismatch",
  ...
  const overallStatus: SsoConnectionHealthStatus = !aggregatedAnyEnabled
    ? "DISABLED"
    : aggregatedOutage
      ? "OUTAGE"
      : aggregatedDegraded
...
function classifyFailure(raw: string | null): SsoFailureReason {
  if (!raw) return "unknown";
  const r = raw.toLowerCase();
  if (r.includes("signature")) return "invalid_signature";
  if (r.includes("cert") || r.includes("expired")) return "expired_certificate";
  if (r.includes("audience")) return "audience_mismatch";
  if (r.includes("acs") || r.includes("destination") || r.includes("recipient"))
    return "acs_mismatch";
  if (r.includes("nameid") || r.includes("subject")) return "missing_nameid";
  if (r.includes("email")) return "missing_email";
  if (r.includes("clock") || r.includes("notbefore") || r.includes("notonorafter"))
    return "clock_skew";
  if (r.includes("replay") || r.includes("nonce")) return "replay_detected";
  if (r.includes("unreachable") || r.includes("timeout") || r.includes("connect"))
    return "idp_unreachable";
  if (r.includes("metadata")) return "metadata_invalid";
  return "unknown";
}

function computeCertExpiry(
  notAfter: Date | string | null,
  nowMs: number,
): SsoConnectionHealth["cert"] {
  if (!notAfter) {
    return {
      fingerprint: null,
      notAfterUtc: null,
  ...
  const notAfterIso =
    notAfter instanceof Date ? notAfter.toISOString() : notAfter;
  ...
  let band: CertExpiryBand;
  if (days < 0) band = "expired";
  else if (days < CERT_EXPIRY_CRITICAL_DAYS) band = "expiring";
  else if (days < CERT_EXPIRY_WARN_DAYS) band = "warning";
  else band = "ok";
  return {
    fingerprint: null,
    notAfterUtc: notAfterIso,
...
function pickRecommendedAction(
  ...
  if (health === "DISABLED") return "Connection is disabled. Re-activate to allow SSO logins.";
  if (health === "UNCONFIGURED") return "Connection has no signing certificate. Ingest IdP metadata to configure.";
