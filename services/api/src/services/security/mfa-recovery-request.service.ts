...
function hashEmailToken(rawToken: string): string {
...
export interface CreateRecoveryRequestResult {
  ok: boolean;
  request?: {
    id: string;
  ...
  reason?:
    ...
    | "user_email_missing"
    | "rate_limited";
  /** Populated on `rate_limited` — when the throttle window resets. */
  retryAfter?: Date;
}
...
export async function createRecoveryRequest(
    ...
    return {
      ...
      request: {
        ...
        emailVerificationExpiresAt:
          existing.emailVerificationExpiresAt ?? existing.expiresAt,
      },
    };
  }
  ...
  const required = Math.max(
    1,
    Math.min(input.requiredApprovals ?? DEFAULT_RECOVERY_QUORUM, 3),
  );
  const expiresAt = new Date(
    Date.now() + MFA_RECOVERY_REQUEST_TTL_SECONDS * 1000,
  );
  const emailExpiresAt = new Date(
    Date.now() + MFA_RECOVERY_EMAIL_TTL_SECONDS * 1000,
  );
  ...
  try {
    ...
    if (emailService.isConfigured()) {
      await emailService.sendMfaRecoveryVerificationEmail(
        user.email,
        verificationUrl,
      );
    }
...
export interface VerifyRecoveryEmailInput {
  requestId: string;
  rawToken: string;
  /** When set, additionally enforces request.userId === actorUserId.
   *  Routes pass the authenticated user; the public verification
   *  page may pass null if the user is unauthenticated (the email
   *  link doubles as the auth proof). */
  actorUserId?: string | null;
}

export interface VerifyRecoveryEmailResult {
  ok: boolean;
  reason?:
    | "request_not_found"
    | "request_not_in_email_pending"
    | "token_invalid"
    | "token_expired"
    | "wrong_user";
}

export async function verifyRecoveryRequestEmail(
...
export interface ResendRecoveryEmailResult {
  ok: boolean;
  reason?:
    | "request_not_found"
    | "wrong_user"
    ...
    | "user_email_missing";
  nextResendAfter?: Date;
}

export async function resendRecoveryRequestEmail(
  ...
  const rawToken = randomBytes(32).toString("hex");
  const newEmailExpiresAt = new Date(
    Date.now() + MFA_RECOVERY_EMAIL_TTL_SECONDS * 1000,
  );
  const newBlockedUntil = new Date(
    Date.now() + MFA_RECOVERY_EMAIL_RESEND_COOLDOWN_SECONDS * 1000,
  );
...
export interface CancelRecoveryRequestInput {
  requestId: string;
  actorUserId: string;
  ipAddress?: string | null;
  userAgent?: string | null;
...
export interface CancelRecoveryRequestResult {
  ok: boolean;
  reason?:
    | "request_not_found"
    | "wrong_user"
...
export async function cancelRecoveryRequest(
  input: CancelRecoveryRequestInput,
...
export interface ApproveRecoveryRequestResult {
  ok: boolean;
  reason?:
    | "request_not_found"
    | "approver_not_admin"
    | "approver_not_in_team"
    | "already_approved"
    | "request_not_pending"
...
): Promise<ApproveRecoveryRequestResult> {
  const row = await prisma.mfaRecoveryRequest.findUnique({
    where: { id: input.requestId },
    select: {
      id: true,
...
export interface RejectRecoveryRequestInput {
  requestId: string;
  approverUserId: string;
  reason?: string | null;
}

export interface RejectRecoveryRequestResult {
  ok: boolean;
  reason?:
    | "request_not_found"
    | "approver_not_admin"
...
export async function rejectRecoveryRequest(
  input: RejectRecoveryRequestInput,
): Promise<RejectRecoveryRequestResult> {
  const row = await prisma.mfaRecoveryRequest.findUnique({
    where: { id: input.requestId },
    select: { id: true, userId: true, teamId: true, status: true },
  });
  ...
  if (!adminCheck) return { ok: false, reason: "approver_not_admin" };
  await prisma.mfaRecoveryRequest.update({
    where: { id: row.id },
    data: {
      status: "REJECTED",
...
export interface ListPendingRecoveryRequestsInput {
  teamId: string;
}

export async function listPendingRecoveryRequests(
  input: ListPendingRecoveryRequestsInput,
): Promise<
  ReadonlyArray<{
    id: string;
    userId: string;
    reason: string;
    status: "PENDING_ADMIN_REVIEW" | "EMAIL_VERIFICATION_PENDING";
    requiredApprovals: number;
...
export interface ReadRecoveryRequestDetailInput {
  requestId: string;
  /** Either the request's owner (user) OR an admin of the team. */
  actorUserId: string;
}
 ...
 */
export async function readRecoveryRequestDetail(
  input: ReadRecoveryRequestDetailInput,
): Promise<
  | {
      ok: true;
      detail: {
