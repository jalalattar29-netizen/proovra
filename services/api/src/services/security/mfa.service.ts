...
export async function verifyAndActivateEnrollment(input: {
  ...
  | { ok: true; factorId: string; recoveryCodes: ReadonlyArray<string> }
  | {
      ok: false;
      reason: "factor_not_found" | "factor_not_enrolling" | "code_invalid";
    }
> {
...
export async function verifyActiveTotp(input: {
  userId: string;
  code: string;
}): Promise<{ ok: boolean; factorId: string | null }> {
...
export async function revokeFactor(input: {
  ...
  factorId: string;
  reason?: string | null;
}): Promise<{ ok: boolean; reason?: "factor_not_found" }> {
  const factor = await prisma.mfaFactor.findUnique({
    where: { id: input.factorId },
  ...
  if (factor.status === "REVOKED") return { ok: true };
  await prisma.mfaFactor.update({
    where: { id: factor.id },
    data: {
      status: "REVOKED",
...
export async function consumeRecoveryCode(input: {
  ...
  if (row.batchInvalidatedAt) return { ok: false, reason: "invalidated" };
...
export async function regenerateRecoveryBatch(input: {
  userId: string;
}): Promise<{ recoveryCodes: ReadonlyArray<string> }> {
  const now = new Date();
  const recoveryCodes = generateRecoveryBatch();
  const batchId = randomUUID();
...
export async function readMfaStatus(input: { userId: string }): Promise<{
  hasMfa: boolean;
...
function hashPepperedSha256(input: string | null | undefined): string | null {
 ...
 */
async function gcStalePendingChallenges(): Promise<void> {
...
export interface CreateMfaPendingChallengeInput {
  userId: string;
...
export interface CreateMfaPendingChallengeResult {
  /** Opaque 32-char hex JTI. Also embedded in the signed token's `sid`. */
  jti: string;
  /** DB row id (UUID). */
  id: string;
  /** Absolute expiry. */
 ...
 * holds the secret.
 */
export async function createMfaPendingChallenge(
  input: CreateMfaPendingChallengeInput,
): Promise<CreateMfaPendingChallengeResult> {
  ...
  const row = await prisma.mfaPendingChallenge.create({
    data: {
      jti,
      userId: input.userId,
      purpose: (input.purpose ?? "LOGIN") as "LOGIN" | "STEP_UP",
      expiresAt,
...
export type ConsumeChallengeFailure =
  | "not_found"
  | "expired"
  | "already_consumed"
  | "wrong_user"
...
export interface ConsumeMfaPendingChallengeInput {
  jti: string;
  expectedUserId: string;
  expectedPurpose?: "LOGIN" | "STEP_UP";
}

export interface ConsumeMfaPendingChallengeResult {
  ok: boolean;
  reason?: ConsumeChallengeFailure;
  /** Populated on success — the row's id, useful for downstream audit. */
  challengeId?: string;
}
 ...
 */
export async function consumeMfaPendingChallenge(
  input: ConsumeMfaPendingChallengeInput,
): Promise<ConsumeMfaPendingChallengeResult> {
 ...
 */
export async function recordPendingChallengeFailure(
  jti: string,
): Promise<void> {
  try {
