...
function resolveKek() {
    ...
    if (envKey && envKey.length > 0) {
        ...
        if (decoded.length !== 32) {
            throw new Error("MFA_SECRET_KEK_BASE64 must decode to exactly 32 bytes (256 bits).");
        }
        return { key: decoded, kekId: PROD_DEFAULT_KEK_ID };
    }
    if (process.env.NODE_ENV === "production") {
        throw new Error("MFA_SECRET_KEK_BASE64 is required in production. The MFA service refuses to encrypt without an explicit KEK.");
    }
    // Dev / test fallback — stable per process so test runs are
    // reproducible. Not suitable for production.
    const derived = scryptSync("dev-only-mfa-kek-do-not-deploy", DEV_KEK_DERIVATION_SALT, 32, { N: 1 << 14, r: 8, p: 1 });
