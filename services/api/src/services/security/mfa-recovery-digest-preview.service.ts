...
export interface PreviewDigestInput {
  /** The admin previewing — service ALWAYS scopes to their own
   *  memberships. There is no admin-impersonation path. */
  actorUserId: string;
  /** When true, include teams the admin's preferences would
   *  currently suppress, flagged as such. Useful for "show me
   *  what I'd see if I un-snoozed". */
  includeSuppressed?: boolean;
}

export interface PreviewDigestTeam {
  teamId: string;
  teamName: string;
  pendingCount: number;
  /** Seconds since the oldest in-flight PENDING_ADMIN_REVIEW row
   *  was created. */
  oldestRequestAgeSeconds: number;
  adminRecoveryUrl: string;
  /** True when the admin's preferences would currently suppress
   *  digest emails for this team. Only ever true when the caller
   *  passed `includeSuppressed: true`. */
...
export interface PreviewDigestResult {
  ...
  teamCount: number;
  requestCount: number;
  teams: ReadonlyArray<PreviewDigestTeam>;
  /** Indicates whether ANY suppression applied while building the
   *  preview. Lets the UI surface a "you have N suppressed teams"
   *  banner without re-counting. */
  suppressedTeamCount: number;
}

export async function previewDigestForAdmin(
  input: PreviewDigestInput,
  ...
  for (const r of staleRows) {
    ...
    if (cur) {
      cur.count += 1;
      if (r.createdAt.getTime() < cur.oldest.getTime()) cur.oldest = r.createdAt;
    } else {
  ...
  for (const [teamId, stat] of perTeam) {
    ...
    built.push({
      ...
      oldestRequestAgeSeconds: Math.max(
        0,
        Math.floor((now.getTime() - stat.oldest.getTime()) / 1000),
      ),
      adminRecoveryUrl,
      suppressedByPreference: !allowed,
    });
    if (built.length >= MAX_TEAMS_PER_DIGEST) break;
  }
