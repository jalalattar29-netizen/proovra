...
// Scanner registry
//
// A scanner is anything that takes (evidenceId, teamId) and returns a
// terminal status + optional summary. The default `noopScanner` simply
// marks scans SKIPPED — useful so a feature-enabled deployment without
// a real scanner still produces an auditable record.
// -----------------------------------------------------------------------------
...
export type Scanner = (req: ScanRequest) => Promise<ScanResult>;
...
let activeScanner: Scanner = noopScanner;
 ...
 */
export function setActiveScanner(s: Scanner): () => void {
...
export async function enqueueScan(
  ...
  if (!isMalwareScanningEnabled()) return null;
  try {
...
export async function getLatestScanForEvidence(
  evidenceId: string,
  client: PrismaClient = defaultPrisma,
): Promise<DbScan | null> {
...
export async function listScansForTeam(
  input: {
    teamId: string;
    status?: FileSecurityScanStatus;
    limit?: number;
  },
  client: PrismaClient = defaultPrisma,
): Promise<DbScan[]> {
...
export async function countScansByTeam(
  ...
  for (const r of rows) {
    if (r.status === "PENDING") counts.pending = r._count._all;
    else if (r.status === "CLEAN") counts.clean = r._count._all;
    else if (r.status === "SUSPICIOUS") counts.suspicious = r._count._all;
    else if (r.status === "FAILED") counts.failed = r._count._all;
    else if (r.status === "SKIPPED") counts.skipped = r._count._all;
  }
  return counts;
}
 ...
 */
export function projectFileSecurityScan(row: DbScan): {
  id: string;
  evidenceId: string;
  teamId: string | null;
  status: string;
