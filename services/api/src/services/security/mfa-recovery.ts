...
export const RECOVERY_CODE_BATCH_SIZE = 10 as const;
/** Code length INCLUDING the hyphen separator (XXXXX-XXXXX). */
export const RECOVERY_CODE_LENGTH = 11 as const;
...
export function generateRecoveryCode(): string {
  ...
  while (chars.length < 10) {
    ...
    for (let i = 0; i < buf.length && chars.length < 10; i += 1) {
      ...
      if (idx >= RECOVERY_CODE_ALPHABET.length) continue;
...
export function generateRecoveryBatch(
  size: number = RECOVERY_CODE_BATCH_SIZE,
 ...
 */
export function computeLookupHash(rawCode: string): string {
  const normalized = normalizeRecoveryCode(rawCode);
  return createHash("sha256").update(normalized, "utf8").digest("hex");
}

export interface RecoveryVerifier {
  /** Per-row salt (16 bytes) generated at issuance time. */
  readonly salt: Buffer;
  /** scrypt(rawCode, salt) — DK_LEN bytes. */
 ...
 */
export function buildRecoveryVerifier(rawCode: string): RecoveryVerifier {
 ...
 */
export function verifyRecoveryVerifier(
  rawCode: string,
