...
export const SAML_MAPPING_PRIVILEGE_PURPOSE = "SAML_MAPPING_PRIVILEGE_UPDATE";
...
export function getSamlMappingSchema() {
    return {
        version: 1,
        fields: [
            {
                ...
                description: "IdP attribute name carrying the operator's email address. Used to identify or auto-provision the user.",
                kind: "string",
                required: false,
            ...
            },
            {
                ...
                description: "IdP attribute name carrying the operator-visible name. Used to populate the operator's profile display name on first login.",
                kind: "string",
                required: false,
            ...
            },
            {
                ...
                description: "Optional override for the stable external subject ID. Defaults to the assertion's NameID. Change only when the IdP's NameID is unstable.",
                kind: "string",
                required: false,
                suggestions: [],
            ...
            },
            {
                ...
                description: "IdP attribute name carrying group memberships (advisory only — authoritative role propagation flows through SCIM Groups when configured).",
                kind: "string",
                required: false,
            ...
            },
            {
                ...
                description: "Role assigned when no group claim matches and the user is being JIT-provisioned. Defaults to MEMBER.",
                kind: "enum",
                required: false,
                suggestions: [],
            ...
            },
            {
                ...
                description: "Per-group role override. When an assertion carries a matching group, the user is assigned that role instead of the default. OWNER/ADMIN assignments require step-up.",
                kind: "group_role_map",
                required: false,
                suggestions: [],
                privilegeAffecting: true,
...
export async function getCurrentSamlMapping(input, client = defaultPrisma) {
...
function normaliseMapping(raw) {
    ...
    return {
        ...
        defaultRole: obj.defaultRole === "MEMBER" || obj.defaultRole === "VIEWER"
            ? obj.defaultRole
            : null,
...
export async function previewSamlMapping(input, client = defaultPrisma) {
    const current = await getCurrentSamlMapping({ teamId: input.teamId, connectionId: input.connectionId }, client);
    if (!current) {
        return {
            ok: false,
    ...
    const newRoles = new Set((input.mapping.groupRoleMap ?? []).map((g) => g.role));
    const oldRoles = new Set((current.mapping.groupRoleMap ?? []).map((g) => g.role));
    if ((newRoles.has("OWNER") && !oldRoles.has("OWNER")) ||
        (newRoles.has("ADMIN") && !oldRoles.has("ADMIN"))) {
        privilegeAffecting = true;
        warnings.push({
            code: "GROUP_ROLE_INCLUDES_OWNER_OR_ADMIN",
            message: "Mapping grants OWNER or ADMIN via a group claim. This change requires step-up confirmation.",
        });
    }
    // SCIM overlap
    if (current.scimManaged && (input.mapping.groupClaim || (input.mapping.groupRoleMap ?? []).length > 0)) {
        warnings.push({
            ...
            message: "This connection is SCIM-managed. SAML group claims are ignored at assertion time; the mapping below has no operational effect until SCIM is disabled.",
        });
    }
    // Default-role downgrade
    if (current.mapping.defaultRole === "MEMBER" &&
        input.mapping.defaultRole === "VIEWER") {
        warnings.push({
            code: "DEFAULT_ROLE_DOWNGRADED",
            message: "Default fallback role downgraded from MEMBER to VIEWER. New JIT-provisioned users will have reduced privileges.",
        });
    }
    ...
    if (current.mapping.email && !input.mapping.email) {
        warnings.push({
            ...
            message: "Email mapping cleared. Future logins will fall back to the IdP's standard email attribute candidates.",
        });
    }
    // External ID override (changing this can effectively re-identify a user)
    if (input.mapping.externalId &&
        input.mapping.externalId !== current.mapping.externalId) {
        privilegeAffecting = true;
        warnings.push({
            code: "EXTERNAL_ID_OVERRIDE_RISKY",
            message: "Changing the external subject attribute will re-identify existing operators on next login. Step-up required.",
        });
    }
    // ---- Sample resolution ----
    let sampleResolution = null;
    if (input.sampleAttributes && Object.keys(input.sampleAttributes).length > 0) {
        ...
        let role = input.mapping.defaultRole ?? "MEMBER";
        let matchedGroup = null;
        if (sampleGroup) {
            const mapEntry = (input.mapping.groupRoleMap ?? []).find((g) => g.group === sampleGroup);
            if (mapEntry) {
                role = mapEntry.role;
...
export async function updateSamlMapping(input, client = defaultPrisma) {
    const conn = await client.ssoConnection.findFirst({
        where: { id: input.connectionId, teamId: input.teamId },
        select: { id: true },
    });
    ...
    const sanitised = {};
    if (input.mapping.email !== undefined)
        sanitised.email = input.mapping.email;
    if (input.mapping.name !== undefined)
        sanitised.name = input.mapping.name;
    ...
    await client.ssoConnection.update({
        ...
        data: { samlAttributeMapping: sanitised },
    });
