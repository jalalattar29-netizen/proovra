 ...
 */
export async function readRecoveryEventFeed(input) {
    const pageSize = Math.max(1, Math.min(input.limit ?? DEFAULT_PAGE_SIZE, MAX_PAGE_SIZE));
    const windowDays = Math.max(1, Math.min(input.windowDays ?? DEFAULT_WINDOW_DAYS, 60));
    ...
    const distinctTeamIds = [
        ...new Set(rows.map((r) => r.teamId).filter((t) => Boolean(t))),
    ];
 ...
 */
function pickShortIdSlice(details) {
    if (!details || typeof details !== "object")
        return null;
    const obj = details;
    const candidate = typeof obj.actorUserId === "string"
        ? obj.actorUserId
