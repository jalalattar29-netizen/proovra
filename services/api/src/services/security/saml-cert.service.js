 ...
 */
export function parseCertExpiry(base64Cert) {
    try {
 ...
 */
export function getCertExpiryStatus(notAfter, nowMs = Date.now()) {
    if (!notAfter)
        return "ok";
    const msUntilExpiry = notAfter.getTime() - nowMs;
    if (msUntilExpiry <= 0)
        return "expired";
    if (msUntilExpiry <= EXPIRY_30D_MS)
        return "expiring_30d";
    if (msUntilExpiry <= EXPIRY_60D_MS)
        return "expiring_60d";
    if (msUntilExpiry <= EXPIRY_90D_MS)
        return "expiring_90d";
    return "ok";
}
/**
 ...
 */
export function emitCertExpiryWarningIfNeeded(input) {
