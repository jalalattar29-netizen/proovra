...
export async function buildIdentitySessionTimeline(input, client = defaultPrisma) {
    ...
    const events = await client.securityEvent.findMany({
        where: {
            ...
            eventType: { in: IDENTITY_TIMELINE_EVENT_TYPES },
...
function humaniseIdentityEvent(eventType) {
