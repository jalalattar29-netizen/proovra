...
async function bumpWebhookFailureMetric() {
    try {
...
async function bumpReplayRejectionMetric() {
    try {
        const mod = await import("@proovra/shared-runtime/ops");
...
export async function auditWebhookSignatureVerification(input) {
    ...
    try {
        ...
        const errMessage = err instanceof Error ? err.message.slice(0, 200) : "unknown_error";
        safeEmitSecurityEvent({
            teamId: null,
            eventType: "webhook_signature_failure",
            severity: "HIGH",
