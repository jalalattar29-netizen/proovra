...
async function fetchDbMigrations(prisma) {
    try {
        return await prisma.$queryRawUnsafe(`SELECT migration_name, finished_at, rolled_back_at
...
function fingerprintDisk(names) {
...
export async function runMigrationDriftCheck(prisma = defaultPrisma) {
    ...
    for (const row of dbRows) {
        if (!disk.includes(row.migration_name)) {
            drift.push({
                ...
                source: "db_only",
                detail: "Migration recorded in the database but absent from the deployed prisma/migrations directory. Working tree may be out of date.",
    ...
    const status = hasRolledBack || hasInProgress
        ? "CRITICAL"
        : drift.length > 0
            ? "DEGRADED"
            : "HEALTHY";
