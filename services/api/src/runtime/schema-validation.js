...
// Validation
// -----------------------------------------------------------------------------
async function checkTable(prisma, name) {
    try {
        const result = await prisma.$queryRawUnsafe(`SELECT (to_regclass('public.' || $1) IS NOT NULL) AS present`, name);
...
async function checkColumn(prisma, table, column) {
    try {
...
async function checkEnum(prisma, name) {
    try {
        const result = await prisma.$queryRawUnsafe(`SELECT EXISTS (SELECT 1 FROM pg_type WHERE typname = $1) AS present`, name);
        return result[0]?.present ? "present" : "missing";
    }
    catch {
        return "unknown";
...
async function checkEnumValue(prisma, enumName, value) {
    try {
        const result = await prisma.$queryRawUnsafe(`SELECT EXISTS (
         SELECT 1
         FROM pg_type t
...
async function checkIndex(prisma, table, indexName) {
    try {
        const result = await prisma.$queryRawUnsafe(`SELECT EXISTS (
...
function fingerprintFailures(failures) {
    ...
    const keys = failures
        ...
        if (t.kind === "enum_value")
            return `enum_value:${t.enumName}.${t.value}`;
        if (t.kind === "index")
            return `index:${t.table}.${t.indexName}`;
...
function rollUpSubsystems(failures) {
    const ids = [
        "core_evidence",
    ...
    return ids.map((subsystem) => {
        const subsystemFailures = failures.filter((f) => f.target.subsystem === subsystem);
        const missingCritical = subsystemFailures.filter((f) => f.target.severity === "critical").length;
        const missingImportant = subsystemFailures.filter((f) => f.target.severity === "important").length;
        const missingOptional = subsystemFailures.filter((f) => f.target.severity === "optional").length;
        const status = missingCritical > 0
            ? "critical"
            : missingImportant > 0
                ? "degraded"
                : "healthy";
 ...
 */
export async function runSchemaValidation(prisma = defaultPrisma) {
...
export async function validateAtStartup(input) {
    ...
    const summary = `${report.status.toUpperCase()} schema drift (${report.failures.length} / ${report.checked} missing) — subsystems: ${failingSubsystems.join(",") || "none"} — fingerprint=${report.driftFingerprint}`;
    ...
    if (input.failFastOnCritical) {
        const err = new Error(`Critical schema drift detected at startup. ${summary}`);
        err.code = "SCHEMA_DRIFT_CRITICAL";
        throw err;
    }
    return report;
}
