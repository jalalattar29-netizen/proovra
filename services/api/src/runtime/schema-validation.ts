...
/**
 * How bad it is when an expected object is absent.
 *
 * ADM-013 PHASE 6 — the third tier, `optional`, is GONE, and its removal is
 * the point rather than a tidy-up.
 *
 * It meant "logged as info only", and `rollUpSubsystems` implemented that by
 * counting `missingOptional` and then not consulting it:
 *
 *     missingCritical > 0  ? "critical"
 *   : missingImportant > 0 ? "degraded"
 *   : "healthy"                          // missingOptional never read
 *
 * So a subsystem with a genuinely absent expected object reported `healthy`,
 * and the readiness page rendered green beside the sentence "2 of 111 expected
 * objects missing". Both statements came from the same function.
 *
 * The tier had exactly two members, and both were the same two objects the
 * canonical migration chain drops (see the catalog note on
 * `evidence_search_documents`). Rather than keep a severity whose only effect
 * is to hide its members from the status it feeds, the tier is removed: every
 * remaining entry is either `critical` (cannot serve) or `important` (this
 * subsystem degrades). Severity now decides HOW BAD an absence is, never
 * WHETHER it counts.
 */
export type SchemaSeverity = "critical" | "important";
...
export const EXPECTED_SCHEMA: ReadonlyArray<ExpectedSchemaObject> = [
  ...
  //
  // ADM-013 PHASE 6 — THE `tsv` COLUMN AND ITS GIN INDEX ARE NOT LISTED, AND
  // THAT IS A CORRECTION, NOT AN OMISSION.
  //
  // They were listed at severity `optional`, and they were the ONLY two
  // members of that tier. They were also permanently absent, for a reason no
  // amount of re-applying migrations can fix:
  //
  //   * 20260620100000_phase24_31_consolidated_drift_patches CREATES
  //     `tsv` as a STORED generated tsvector column, plus
  //     `evidence_search_documents_tsv_gin` over it.
  //   * 20260925000000_phase0_schema_catchup — auto-generated by
  //     `prisma migrate diff --to-schema schema.prisma` — then emits
  //     `ALTER TABLE "evidence_search_documents" DROP COLUMN "tsv"`, and
  //     dropping the column drops its dependent index with it.
  //
  // The diff is not wrong: `schema.prisma` has no way to express a Postgres
  // GENERATED column, so from its point of view `tsv` is drift. Every fresh
  // database therefore ends the canonical chain WITHOUT these two objects, and
  // the catalog expected them on every environment forever.
  //
  // Nothing reads them. `evidence-search.service.ts` runs free text through
  // Prisma `contains` (ILIKE) — its own comment says so — and the expression
  // index `evidence_search_documents_body_fts_gin` from the Phase 24 base
  // migration is what backs the query path that exists. Restoring `tsv` would
  // add a STORED generated column to the evidence search table, rewriting it
  // under lock, to satisfy an expectation no query has.
  //
  // So the expectation is retired. If a later phase wires real FTS, it adds
  // the column, the index AND the query that uses them in one change, and
  // re-registers them here at `important`.
  // ---------------------------------------------------------------------------
...
function rollUpSubsystems(
  ...
  return ids.map((subsystem) => {
    ...
    // ADM-013 PHASE 6 — every failure now lands in one of the two counters
    // above, and both are consulted. There is no third bucket a genuinely
    // missing object can fall into and be dropped from the status.
    //
    // The assertion is deliberate rather than defensive: if a future edit
    // reintroduces a severity the rollup does not know about, this throws at
    // the point of the mistake instead of quietly restoring the false green.
    if (missingCritical + missingImportant !== subsystemFailures.length) {
      throw new Error(
        `schema-validation: ${subsystemFailures.length - missingCritical - missingImportant} ` +
          `failure(s) in "${subsystem}" carry a severity the rollup does not classify. ` +
          "An unclassified absence would be counted and then ignored, which is how " +
          "readiness rendered green beside two missing expected objects.",
      );
    }
