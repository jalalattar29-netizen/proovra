...
export class KmsEvidenceSigner {
    ...
    async getPublicKeyPem() {
        ...
        const res = await this.kms.send(new GetPublicKeyCommand({
    ...
    async signFingerprintHex(messageHex) {
    ...
    async signPackageManifestDigestHex(digestHex) {
        const normalizedHex = digestHex.trim().toLowerCase();
        if (!/^[a-f0-9]{64}$/.test(normalizedHex)) {
            throw new Error("signPackageManifestDigestHex: digestHex must be a SHA-256 hex digest");
        }
        const message = Buffer.from(normalizedHex, "hex");
        const res = await this.kms.send(new SignCommand({
            KeyId: this.getKmsKeyId(),
        ...
        return {
            signatureBase64: Buffer.from(res.Signature).toString("base64"),
            keyId: process.env.PACKAGE_SIGNING_KEY_ID?.trim() || this.getBusinessKeyId(),
