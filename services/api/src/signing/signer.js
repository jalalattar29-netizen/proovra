...
class LocalPemEvidenceSigner {
    async signFingerprintHex(messageHex) {
        ...
        const signatureBase64 = ed25519SignHexWithKeyPath(normalizedHex, "SIGNING_PRIVATE_KEY_PATH");
        return {
            signatureBase64,
...
class AwsKmsEvidenceSigner {
    kmsSigner = new KmsEvidenceSigner();
    async signFingerprintHex(messageHex) {
        const normalizedHex = messageHex.trim().toLowerCase();
        if (!/^[a-f0-9]+$/.test(normalizedHex) || normalizedHex.length % 2 !== 0) {
            throw new Error("signFingerprintHex: messageHex must be valid hex");
...
export function getEvidenceSigner() {
    if (!cachedSigner) {
