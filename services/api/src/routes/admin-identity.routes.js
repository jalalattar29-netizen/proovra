 ...
 */
import { z } from "zod";
import { SCIM_SCOPES, SSO_CONNECTION_STATUSES, SsoConnectionCreateInputSchema, TEMPORARY_ELEVATION_DEFAULT_SECONDS, TEMPORARY_ELEVATION_MAX_SECONDS, TEMPORARY_ELEVATION_MIN_SECONDS, TemporaryElevationSchema, } from "@proovra/shared";
import { getAuthUserId } from "../auth.js";
import { prisma } from "../db.js";
import { requireAuth } from "../middleware/auth.js";
import { evaluateMemberAccess } from "../services/identity/access-policy.service.js";
import { requireStepUpForSensitiveAction } from "../services/identity-security/step-up-middleware.js";
import { RbacEngineError, buildPermissionSnapshot, computeEffectiveRoleMatrix, grantTemporaryElevation, } from "../services/access-control/rbac-engine.service.js";
import { SsoServiceError, createSsoConnection, listSsoConnections, transitionSsoConnection, } from "../services/access-control/sso.service.js";
import { createScimToken, listScimTokens, revokeScimToken, } from "../services/access-control/scim.service.js";
import { listActiveSessions, refreshHighRiskSessionGauge, revokeActiveSession, revokeAllSessionsForUserAdmin, sweepStaleSessions, } from "../services/access-control/session-inventory.service.js";
...
import { emergencyOrgRevoke, listQuarantinedSessions, quarantineSession, releaseQuarantine, sweepQuarantineReleases, } from "../services/access-control/session-quarantine.service.js";
...
import { SESSION_QUARANTINE_REASONS, } from "@proovra/shared";
async function requireIdentityAdmin(req, reply, teamId, permission = "identity.org_policy.read") {
...
function sendError(reply, err) {
    if (err instanceof SsoServiceError) {
        const status = err.code === "SSO_CONNECTION_NOT_FOUND"
            ? 404
    ...
    if (err instanceof RbacEngineError) {
        const status = err.code === "RBAC_MEMBER_NOT_FOUND"
            ? 404
...
// Routes
// -----------------------------------------------------------------------------
export async function adminIdentityRoutes(app) {
    // ===========================================================================
    // SSO — Identity providers
    // ===========================================================================
    app.get("/v1/admin/identity/providers", { preHandler: requireAuth }, async (req, reply) => {
    ...
    app.post("/v1/admin/identity/providers", { preHandler: requireAuth }, async (req, reply) => {
        ...
        const actor = await requireIdentityAdmin(req, reply, body.teamId, "identity.external_mapping.manage");
        if (!actor)
            return;
        ...
        try {
            ...
            if (sendError(reply, err))
                return;
            throw err;
        }
    });
    app.post("/v1/admin/identity/providers/:id/transition", { preHandler: requireAuth }, async (req, reply) => {
    ...
    // Permission matrix
    // ===========================================================================
    app.get("/v1/admin/identity/permission-matrix", { preHandler: requireAuth }, async (req, reply) => {
        const q = z
            .object({
            teamId: z.string().uuid(),
            ...
            .parse(req.query ?? {});
        const actor = await requireIdentityAdmin(req, reply, q.teamId, "identity.member.read");
        if (!actor)
            return;
        try {
    ...
    app.get("/v1/admin/identity/role-matrix", { preHandler: requireAuth }, async (req, reply) => {
        const q = TeamIdQuery.parse(req.query ?? {});
        const actor = await requireIdentityAdmin(req, reply, q.teamId);
        if (!actor)
            return;
    ...
    // Temporary elevation
    // ===========================================================================
    app.post("/v1/admin/identity/elevations", { preHandler: requireAuth }, async (req, reply) => {
        ...
        const body = parsed.data;
        const actor = await requireIdentityAdmin(req, reply, body.teamId, "identity.capability.grant");
        if (!actor)
            return;
        ...
        if (gate.sent)
            return;
        try {
            const ttl = Math.min(Math.max(body.ttlSeconds ?? TEMPORARY_ELEVATION_DEFAULT_SECONDS, TEMPORARY_ELEVATION_MIN_SECONDS), TEMPORARY_ELEVATION_MAX_SECONDS);
    ...
    // SCIM tokens
    // ===========================================================================
    app.get("/v1/admin/identity/scim/tokens", { preHandler: requireAuth }, async (req, reply) => {
        const q = TeamIdQuery.parse(req.query ?? {});
        const actor = await requireIdentityAdmin(req, reply, q.teamId, "identity.external_mapping.read");
        if (!actor)
            return;
        const tokens = await listScimTokens({ teamId: q.teamId });
        return reply.code(200).send({ tokens });
    });
    app.post("/v1/admin/identity/scim/tokens", { preHandler: requireAuth }, async (req, reply) => {
        const body = z
            .object({
            teamId: z.string().uuid(),
    ...
    app.post("/v1/admin/identity/scim/tokens/:id/revoke", { preHandler: requireAuth }, async (req, reply) => {
        const { id } = ParamsId.parse(req.params);
        const body = z
            .object({
            teamId: z.string().uuid(),
    ...
    // Active sessions
    // ===========================================================================
    app.get("/v1/admin/identity/sessions", { preHandler: requireAuth }, async (req, reply) => {
        const q = z
            .object({
            teamId: z.string().uuid(),
    ...
    app.post("/v1/admin/identity/sessions/:id/revoke", { preHandler: requireAuth }, async (req, reply) => {
        const { id } = ParamsId.parse(req.params);
        const body = z
            .object({
            teamId: z.string().uuid(),
            ...
            .parse(req.body ?? {});
        const actor = await requireIdentityAdmin(req, reply, body.teamId, "identity.contributor_session.revoke");
        if (!actor)
            return;
    ...
    app.post("/v1/admin/identity/sessions/user/:userId/revoke-all", { preHandler: requireAuth }, async (req, reply) => {
        const { userId } = z
    ...
    // ===========================================================================
    app.get("/v1/admin/identity/timeline", { preHandler: requireAuth }, async (req, reply) => {
        const q = z
            .object({
            teamId: z.string().uuid(),
    ...
    // ===========================================================================
    app.post("/v1/admin/identity/sessions/reconcile-stale", async (req, reply) => {
        const expected = process.env["IDENTITY_RECONCILE_CRON_SECRET"] ||
    ...
    // ===========================================================================
    app.post("/v1/admin/identity/sessions/:id/score", { preHandler: requireAuth }, async (req, reply) => {
        const { id } = ParamsId.parse(req.params);
...
function humaniseEventType(eventType) {
...
// =============================================================================
export async function adminIdentityRuntimeRoutes(app) {
    // ---------------------------------------------------------------------------
    // GET /v1/admin/identity/quarantined-sessions
    // ---------------------------------------------------------------------------
    app.get("/v1/admin/identity/quarantined-sessions", { preHandler: requireAuth }, async (req, reply) => {
        const q = z
            .object({
            teamId: z.string().uuid(),
            limit: z.coerce.number().int().min(1).max(500).optional(),
    ...
    // ---------------------------------------------------------------------------
    app.post("/v1/admin/identity/sessions/:id/quarantine", { preHandler: requireAuth }, async (req, reply) => {
        const { id } = ParamsId.parse(req.params);
        const body = z
            .object({
            teamId: z.string().uuid(),
    ...
    // ---------------------------------------------------------------------------
    app.post("/v1/admin/identity/sessions/:id/release", { preHandler: requireAuth }, async (req, reply) => {
        const { id } = ParamsId.parse(req.params);
        const body = z
            .object({
            teamId: z.string().uuid(),
    ...
    // ---------------------------------------------------------------------------
    app.post("/v1/admin/identity/emergency-revoke", { preHandler: requireAuth }, async (req, reply) => {
        const body = z
            .object({
            teamId: z.string().uuid(),
    ...
    // ---------------------------------------------------------------------------
    app.post("/v1/admin/identity/runtime/reconcile", async (req, reply) => {
        const expected = process.env["IDENTITY_RECONCILE_CRON_SECRET"] ||
            process.env["INTEGRATION_CRON_SECRET"] ||
            "";
        const got = req.headers["x-cron-secret"];
