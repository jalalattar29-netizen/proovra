...
function projectSessionForApi(session: UploadSessionRow) {
  return {
    id: session.id,
...
function projectPartForApi(part: UploadSessionPartRow) {
  return {
...
async function gateEvidenceForUpload(input: {
  ...
  | { ok: true }
  | { ok: false; status: 404 | 403; code: string; reason?: string }
> {
...
export async function integrationsUploadsRoutes(app: FastifyInstance) {
  ...
  app.post(
    ...
    async (req: FastifyRequest, reply: FastifyReply) => {
      ...
      if (!requireApiScope(req, reply, "integration.evidence.upload")) return;
