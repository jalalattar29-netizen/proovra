...
import { computeGovernanceAnalytics, } from "../services/governance-lifecycle/governance-analytics.service.js";
import { acknowledgeGovernanceNotification, countFailedGovernanceNotifications, countPendingGovernanceNotifications, GovernanceNotificationError, listGovernanceNotifications, } from "../services/governance-lifecycle/governance-notification.service.js";
import { captureExportSnapshot, getExportSnapshot, GovernanceExportSnapshotError, listExportSnapshots, verifyExportSnapshotHash, } from "../services/governance-lifecycle/export-lineage.service.js";
import { ANALYTICS_WINDOWS, GOVERNANCE_EXPORT_SNAPSHOT_KINDS, GOVERNANCE_NOTIFICATION_DELIVERY_STATUSES, GOVERNANCE_NOTIFICATION_KINDS, GOVERNANCE_NOTIFICATION_SEVERITIES, GOVERNANCE_RECONCILIATION_KINDS, } from "@proovra/shared";
...
export async function governanceOperationsRoutes(app) {
    // ===========================================================================
    // Analytics
    // ===========================================================================
    app.get("/v1/governance/analytics", { preHandler: requireAuth }, async (req, reply) => {
        const query = z
            .object({
        ...
        const result = await computeGovernanceAnalytics({
            teamId: query.teamId,
            window: query.window,
        });
    ...
    // Notifications
    // ===========================================================================
    app.get("/v1/governance/notifications", { preHandler: requireAuth }, async (req, reply) => {
        const query = z
            .object({
            teamId: z.string().uuid(),
    ...
    app.post("/v1/governance/notifications/:id/acknowledge", { preHandler: requireAuth }, async (req, reply) => {
    ...
    // Export snapshots
    // ===========================================================================
    app.post("/v1/governance/export-snapshots", { preHandler: requireAuth }, async (req, reply) => {
        const body = z
            .object({
            teamId: z.string().uuid(),
    ...
    app.get("/v1/governance/export-snapshots", { preHandler: requireAuth }, async (req, reply) => {
        const query = z
            .object({
            teamId: z.string().uuid(),
    ...
    app.get("/v1/governance/export-snapshots/:id", { preHandler: requireAuth }, async (req, reply) => {
        const { id } = ParamsId.parse(req.params);
    ...
    app.get("/v1/governance/export-snapshots/:id/verify", { preHandler: requireAuth }, async (req, reply) => {
        const { id } = ParamsId.parse(req.params);
        const query = z.object({ teamId: z.string().uuid() }).parse(req.query ?? {});
        const ok = await requireMember(req, reply, query.teamId);
        if (!ok)
    ...
    // ===========================================================================
    app.get("/v1/governance/reconciliation-runs", { preHandler: requireAuth }, async (req, reply) => {
        const query = z
            .object({
            teamId: z.string().uuid(),
    ...
    app.get("/v1/governance/destruction-executions", { preHandler: requireAuth }, async (req, reply) => {
        const query = z
            .object({
            teamId: z.string().uuid(),
    ...
    app.get("/v1/governance/immutable-storage-checks", { preHandler: requireAuth }, async (req, reply) => {
        const query = z
            .object({
            teamId: z.string().uuid(),
            evidenceId: z.string().uuid().optional(),
