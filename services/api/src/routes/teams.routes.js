...
import { assertTeamSeatAvailable, getWorkspaceUsage, } from "../services/workspace-usage.service.js";
import { getPersonalWorkspaceScope, getTeamWorkspaceScope, } from "../services/workspace-billing.service.js";
...
async function getActorMembership(teamId, userId) {
...
async function getTeamMemberByMemberId(teamId, memberId) {
...
async function createActivity(teamId, eventType, targetType, actorUserId, targetId, metadata) {
    try {
        const data = {
            teamId,
            eventType,
...
async function checkWorkspaceLegalHold(teamId) {
    try {
...
function auditTeamAction(req, params) {
...
async function assertUserCanCreateAnotherTeam(ownerUserId) {
    ...
    if (maxOwnedTeams <= 0) {
        const err = new Error("Your current plan does not allow team creation");
        err.statusCode = 409;
...
export async function teamsRoutes(app) {
        ...
        try {
            ...
            if (statusCode !== 500) {
                auditTeamAction(req, {
                    ...
                    metadata: {
                        name: body.name.trim(),
                        reason: err?.code ?? "team_creation_not_allowed",
                    },
                });
    ...
    app.get("/v1/teams", { preHandler: requireAuthAndLegal }, async (req, reply) => {
        ...
        const now = new Date();
        const items = await Promise.all(teams.map(async (team) => {
    ...
    app.get("/v1/teams/:id", { preHandler: requireAuthAndLegal }, async (req, reply) => {
    ...
    app.get("/v1/teams/:id/cases", { preHandler: requireAuthAndLegal }, async (req, reply) => {
        const teamId = z.string().uuid().parse(req.params.id);
        const userId = getAuthUserId(req);
    ...
    app.get("/v1/teams/:id/invites", { preHandler: requireAuthAndLegal }, async (req, reply) => {
        const teamId = z.string().uuid().parse(req.params.id);
        const userId = getAuthUserId(req);
        const actor = await getActorMembership(teamId, userId);
    ...
    app.patch("/v1/teams/:id", { preHandler: requireAuthAndLegal }, async (req, reply) => {
        const teamId = z.string().uuid().parse(req.params.id);
    ...
    app.delete("/v1/teams/:id", { preHandler: requireAuthAndLegal }, async (req, reply) => {
        const teamId = z.string().uuid().parse(req.params.id);
        const userId = getAuthUserId(req);
        const actor = await getActorMembership(teamId, userId);
    ...
    app.post("/v1/teams/:id/invites", { preHandler: requireAuthAndLegal }, async (req, reply) => {
        const teamId = z.string().uuid().parse(req.params.id);
        ...
        if (existingPendingInvite) {
            ...
            try {
                ...
                if (emailService.isConfigured()) {
                    ...
                    await emailService.sendTeamInvitation(email, team?.name || "PROOVRA", existingPendingInvite.token);
                    emailSent = true;
                }
            }
            catch (err) {
    ...
    app.delete("/v1/teams/:id/invites/:inviteId", { preHandler: requireAuthAndLegal }, async (req, reply) => {
    ...
    app.patch("/v1/teams/:id/members/:memberId", { preHandler: requireAuthAndLegal }, async (req, reply) => {
        const params = req.params;
        const teamId = z.string().uuid().parse(params.id);
    ...
    app.get("/v1/teams/:id/members/:memberId/removal-impact", { preHandler: requireAuthAndLegal }, async (req, reply) => {
        const params = req.params;
        const teamId = z.string().uuid().parse(params.id);
        const memberId = z.string().uuid().parse(params.memberId);
        const userId = getAuthUserId(req);
    ...
    app.delete("/v1/teams/:id/members/:memberId", { preHandler: requireAuthAndLegal }, async (req, reply) => {
        const params = req.params;
        const teamId = z.string().uuid().parse(params.id);
        const memberId = z.string().uuid().parse(params.memberId);
        const userId = getAuthUserId(req);
        ...
        const hasOwnedRecords = ownedEvidenceCount > 0 || ownedCaseCount > 0;
        if (hasOwnedRecords && !transferToUserId) {
            ...
            return reply.code(409).send({
                ...
                message: "This member still owns active evidence or cases in this team. Provide `transferToUserId` to re-assign ownership before removal.",
                impact: {
        ...
        if (transferToUserId) {
            if (transferToUserId === target.userId) {
                return reply.code(400).send({
                    ...
                    message: "Cannot transfer ownership to the member being removed.",
                });
            }
            ...
            if (!transferMember ||
                ...
                return reply.code(400).send({
                    code: "INVALID_TRANSFER_TARGET",
                    message: "Transfer target must be an active OWNER or ADMIN of this team.",
                });
            }
        }
        ...
        if (transferToUserId && hasOwnedRecords) {
            ...
            await createActivity(teamId, "ownership_transferred", "member", userId, target.userId).catch(() => null);
        }
        auditTeamAction(req, {
            userId,
            action: "teams.member_delete",
    ...
    app.post("/v1/teams/invites/:token/accept", { preHandler: requireAuthAndLegal }, async (req, reply) => {
    ...
    app.get("/v1/teams/:id/activity", { preHandler: requireAuthAndLegal }, async (req, reply) => {
        const teamId = z.string().uuid().parse(req.params.id);
        const userId = getAuthUserId(req);
        const actor = await getActorMembership(teamId, userId);
        if (!actor) {
    ...
    app.post("/v1/teams/:id/cases/link", { preHandler: requireAuthAndLegal }, async (req, reply) => {
    ...
    app.delete("/v1/teams/:id/cases/:caseId", { preHandler: requireAuthAndLegal }, async (req, reply) => {
        const params = req.params;
        const teamId = z.string().uuid().parse(params.id);
    ...
    //     "N results".
    // ===========================================================================
    app.get("/v1/teams/:id/external-collaborators", { preHandler: requireAuthAndLegal }, async (req, reply) => {
        const teamId = z.string().uuid().parse(req.params.id);
        const userId = getAuthUserId(req);
        const actor = await getActorMembership(teamId, userId);
        if (!actor || !hasRole(actor.role, prismaPkg.TeamRole.ADMIN)) {
        ...
        const externalGrants = grants.filter((g) => !teamMemberIds.has(g.userId));
        // 5. Resolve user identity (email + displayName only).
        const externalUserIds = Array.from(new Set(externalGrants.map((g) => g.userId)));
        ...
        const collaborators = Array.from(grantsByUser.entries()).map(([uid, grants]) => {
            const u = userById.get(uid);
            // Pick the earliest grant as the "first granted" timestamp.
            const earliestGrant = grants.reduce((acc, g) => (g.grantedAt < acc ? g.grantedAt : acc), grants[0]?.grantedAt ?? new Date().toISOString());
            return {
                userId: uid,
                email: u?.email ?? null,
        ...
        collaborators.sort((a, b) => a.firstGrantedAt < b.firstGrantedAt ? 1 : -1);
        return reply.code(200).send({
            teamId,
            summary: {
    ...
    // ===========================================================================
    app.get("/v1/teams/:id/access-review", { preHandler: requireAuthAndLegal }, async (req, reply) => {
        const teamId = z.string().uuid().parse(req.params.id);
        const userId = getAuthUserId(req);
        const actor = await getActorMembership(teamId, userId);
        if (!actor || !hasRole(actor.role, prismaPkg.TeamRole.ADMIN)) {
        ...
        const grants = teamCaseIds.length
            ...
            : [];
        const externalGrants = grants.filter((g) => !teamMemberIdSet.has(g.userId));
        const externalUserIds = Array.from(new Set(externalGrants.map((g) => g.userId)));
        ...
        const externalCollaborators = externalUserIds.map((uid) => {
            ...
            const firstGrantedAt = grantsList.reduce((acc, g) => (g.grantedAt < acc ? g.grantedAt : acc), grantsList[0]?.grantedAt ?? new Date().toISOString());
            return {
                kind: "EXTERNAL",
                userId: uid,
                email: u?.email ?? null,
    ...
    // ===========================================================================
    app.delete("/v1/teams/:id/external-grants/:grantId", { preHandler: requireAuthAndLegal }, async (req, reply) => {
        const teamId = z.string().uuid().parse(req.params.id);
        const grantId = z
            .string()
            .uuid()
            .parse(req.params.grantId);
        const userId = getAuthUserId(req);
        const actor = await getActorMembership(teamId, userId);
        if (!actor || !hasRole(actor.role, prismaPkg.TeamRole.ADMIN)) {
            return reply.code(403).send({ message: "Forbidden" });
        ...
        if (targetMembership) {
            return reply.code(422).send({
                ...
                message: "This grant belongs to a formal team member. Manage their access via /v1/teams/:id/members.",
            });
        }
        // Delete the grant.
    ...
    // ===========================================================================
    app.get("/v1/platform/rbac/matrix", { preHandler: requireAuthAndLegal }, async (_req, reply) => {
                    ...
                    capabilities: [
                        {
                            ...
                            description: "Browse the workspace evidence library and open individual records.",
                        },
                        {
                            id: "evidence.upload",
                            label: "Upload + finalize evidence",
                            roles: ["OWNER", "ADMIN", "MEMBER"],
                            description: "Create new evidence records via the capture flow, complete uploads, and finalize hash/signature pipeline.",
                        },
                        {
                            id: "evidence.manage",
                            label: "Manage evidence",
                            roles: ["OWNER", "ADMIN", "MEMBER"],
                            description: "Edit titles/tags/retention class; archive or soft-delete records.",
                        },
                    ],
                },
                {
                    ...
                    capabilities: [
                        {
                            ...
                            roles: ["OWNER", "ADMIN", "MEMBER"],
                            description: "Trigger a new report version. Report contents are operator-only.",
                        },
                        {
                            id: "report.download",
                            label: "Download report / verification package",
                            roles: ["OWNER", "ADMIN", "MEMBER", "VIEWER"],
                            description: "Download the signed PDF / verification archive. Custody event written on every download.",
                        },
                    ],
                },
                {
                    ...
                    capabilities: [
                        {
                            ...
                            label: "View cases",
                            roles: ["OWNER", "ADMIN", "MEMBER", "VIEWER"],
                            description: "Browse and open cases scoped to this workspace; see linked evidence and timeline.",
                        },
                        {
                            id: "case.create",
                            label: "Create cases",
                        ...
                        },
                        {
                            ...
                            roles: ["OWNER", "ADMIN"],
                            description: "Change case status. Closure cascades active assignments automatically.",
                        },
                        {
                            id: "case.assign_reviewers",
                            label: "Assign reviewers",
                            roles: ["OWNER", "ADMIN"],
                            description: "Assign / re-assign reviewer roles on a case.",
                        },
                    ],
                },
                {
                    ...
                    capabilities: [
                        {
                            ...
                            roles: ["OWNER", "ADMIN", "MEMBER"],
                            description: "See assigned reviews + workspace queue depth.",
                        },
                        {
                            id: "review.action",
                            label: "Approve / reject / pause / request-info",
                            roles: ["OWNER", "ADMIN", "MEMBER"],
                            description: "Drive a review through its lifecycle. Reason text required for non-approve actions.",
                        },
                        {
                            id: "review.escalation",
                            label: "Resolve escalations",
                            roles: ["OWNER", "ADMIN"],
                            description: "Resolve, suppress, reassign escalations.",
                        },
                    ],
                },
                {
                    ...
                    capabilities: [
                        {
                            ...
                            roles: ["OWNER", "ADMIN"],
                            description: "Send team invites. Subject to the workspace seat limit.",
                        },
                        {
                            id: "team.role_change",
                            label: "Change member role",
                            roles: ["OWNER", "ADMIN"],
                            description: "Promote or demote between ADMIN / MEMBER / VIEWER.",
                        },
                        {
                            id: "team.remove",
                            label: "Remove members (with ownership transfer)",
                            roles: ["OWNER", "ADMIN"],
                            description: "Offboard a member. Owned evidence / cases must be transferred first.",
                        },
                        {
                            id: "team.external_revoke",
                            label: "Revoke external access grants",
                            roles: ["OWNER", "ADMIN"],
                            description: "Revoke non-team-member access grants on the workspace's cases (Phase 2.6D).",
                        },
                        {
                            id: "team.delete",
                            label: "Delete the team workspace",
                            roles: ["OWNER"],
                            description: "Destructive. Only the OWNER can delete the team.",
                        },
                    ],
                },
                {
                    ...
                    capabilities: [
                        ...
                        },
                        {
                            ...
                            roles: ["OWNER"],
                            description: "Upgrade, downgrade, cancel subscriptions; manage storage add-ons.",
                        },
                        {
                            id: "security_center.view",
                            label: "Open Security Center",
                            roles: ["OWNER", "ADMIN"],
                            description: "View workspace MFA policy, trusted devices, sessions, SAML config, SCIM tokens.",
                        },
                        {
                            id: "security_center.manage",
                            label: "Manage workspace MFA policy / SSO config",
                            roles: ["OWNER", "ADMIN"],
                            description: "Change MFA enforcement level, manage SAML connections, revoke SCIM tokens. Step-up required.",
                        },
                    ],
                },
                {
                    ...
                    capabilities: [
                        {
                            ...
                            roles: ["OWNER", "ADMIN", "MEMBER"],
                            description: "See member invitations, role changes, ownership transfers, policy changes recorded in `team_activities`.",
                        },
                        {
                            id: "audit.platform",
                            label: "View platform audit log",
                            roles: [],
                            description: "PLATFORM_ADMIN only — not granted by team role.",
                        },
                    ],
                },
            ],
