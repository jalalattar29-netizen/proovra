 ...
 */
import { z } from "zod";
import { CAPTURE_INGEST_DENIAL_REASONS, CAPTURE_INGEST_WARNINGS, CAPTURE_MODES, CAPTURE_PROVENANCE_CLASSES, CAPTURE_SIGNATURE_ALGORITHMS, DEVICE_ATTESTATION_PROVIDERS, attestationVerdictKeepsClassA, clampProvenanceClass, demoteToClassC, } from "@proovra/shared";
import { getAuthUserId } from "../auth.js";
import { prisma } from "../db.js";
import { requireAuth } from "../middleware/auth.js";
import { verifyDeviceAttestation } from "../services/capture-trust/attestation-verifier.service.js";
import { listDevicesForTeam, getDevice as getDeviceById, registerDevice, revokeDevice, } from "../services/capture-trust/device-identity.service.js";
...
import { emitCaptureTrustEvent, readCaptureTrustTimeline, } from "../services/capture-trust/trust-event.service.js";
// =============================================================================
// Zod input schemas
// =============================================================================
...
// Route handlers
// =============================================================================
export async function captureTrustRoutes(app) {
    // ---------------------------------------------------------------------------
    // POST /v1/capture/devices — register a workspace-bound device
    // ---------------------------------------------------------------------------
    app.post("/v1/capture/devices", { preHandler: requireAuth }, async (req, reply) => {
    ...
    // GET /v1/capture/devices — list workspace devices
    // ---------------------------------------------------------------------------
    app.get("/v1/capture/devices", { preHandler: requireAuth }, async (req, reply) => {
        const teamId = await resolveTeamIdOrDeny(req, reply);
        if (!teamId)
            return reply;
    ...
    // ---------------------------------------------------------------------------
    app.get("/v1/capture/devices/:id", { preHandler: requireAuth }, async (req, reply) => {
        const teamId = await resolveTeamIdOrDeny(req, reply);
        if (!teamId)
            return reply;
    ...
    // POST /v1/capture/devices/:id/revoke — revoke a device
    // ---------------------------------------------------------------------------
    app.post("/v1/capture/devices/:id/revoke", { preHandler: requireAuth }, async (req, reply) => {
        const teamId = await resolveTeamIdOrDeny(req, reply);
        if (!teamId)
            return reply;
        const { id } = z.object({ id: z.string().uuid() }).parse(req.params);
    ...
    // ---------------------------------------------------------------------------
    app.post("/v1/capture/mobile/ingest", { preHandler: requireAuth }, async (req, reply) => {
        const teamId = await resolveTeamIdOrDeny(req, reply);
        if (!teamId)
            return reply;
        ...
        const warnings = [];
        let provenanceClass = clampProvenanceClass(payload.provenanceClass, payload.captureMode);
        ...
        if (sigResult.fatal) {
            const denial = sigResult.verdict === "UNKNOWN_DEVICE"
        ...
        if (body.attestation && sigResult.deviceId) {
            ...
            await emitCaptureTrustEvent({
                ...
                captureSessionId: payload.captureSessionId,
                evidenceId: null,
                code: att.verdict === "FAILED" || att.verdict === "REVOKED"
                    ? "ATTESTATION_FAILED"
        ...
        if (payload.provenanceClass === "A" &&
            ...
            const demoted = attestationVerdict === "FAILED" || attestationVerdict === "REVOKED"
    ...
    // ---------------------------------------------------------------------------
    app.get("/v1/capture/sessions/:id/trust-timeline", { preHandler: requireAuth }, async (req, reply) => {
        const teamId = await resolveTeamIdOrDeny(req, reply);
        if (!teamId)
            return reply;
        const { id } = z.object({ id: z.string().uuid() }).parse(req.params);
    ...
    // ---------------------------------------------------------------------------
    app.get("/v1/provenance/:evidenceId", { preHandler: requireAuth }, async (req, reply) => {
        const teamId = await resolveTeamIdOrDeny(req, reply);
        if (!teamId)
            return reply;
        const { evidenceId } = z
 ...
 */
async function resolveTeamIdOrDeny(req, reply) {
    const userId = getAuthUserId(req);
...
function denyIngest(reply, denial, provenanceClass) {
    if (!CAPTURE_INGEST_DENIAL_REASONS.includes(denial)) {
