...
import { canCreateIntakeLink, loadWorkspaceGovernancePolicy, } from "../services/governance.service.js";
import { EvidenceRequestInputSchema, } from "@proovra/shared";
...
export async function integrationsApiRoutes(app) {
    // ---------------------------------------------------------------------------
    // GET /v1/integrations/api/evidence
    // ---------------------------------------------------------------------------
    app.get("/v1/integrations/api/evidence", async (req, reply) => {
    ...
    // ---------------------------------------------------------------------------
    app.get("/v1/integrations/api/evidence/:id", async (req, reply) => {
        if (!(await requireApiKey(req, reply)))
            return;
        if (!requireApiScope(req, reply, "integration.evidence.read"))
            return;
    ...
    // ---------------------------------------------------------------------------
    app.post("/v1/integrations/api/intake-links", async (req, reply) => {
        if (!(await requireApiKey(req, reply)))
            return;
        if (!requireApiScope(req, reply, "integration.intake_link.create"))
            return;
        const cred = req.apiCredential;
            ...
            try {
                ...
                return reply.code(503).send({
                    error: {
                        ...
                        message: err instanceof Error ? err.message : "policy_lookup_failed",
                    },
                });
            }
    ...
    // ---------------------------------------------------------------------------
    app.post("/v1/integrations/api/evidence-requests", async (req, reply) => {
        if (!(await requireApiKey(req, reply)))
            return;
        if (!requireApiScope(req, reply, "integration.evidence_request.create"))
            return;
        const cred = req.apiCredential;
        return runWithApiAudit(req, reply, "evidence_request.create", async () => {
            const body = EvidenceRequestInputSchema.omit({ teamId: true }).parse(req.body ?? {});
            try {
                const { request } = await createEvidenceRequest({ ...body, teamId: cred.teamId }, { actorUserId: cred.credentialId });
                return reply.code(201).send({
                    evidenceRequest: {
                        id: request.id,
    ...
    // ---------------------------------------------------------------------------
    app.get("/v1/integrations/api/evidence-requests/:id", async (req, reply) => {
        if (!(await requireApiKey(req, reply)))
            return;
