...
import { changeCaseStatus, CaseError, } from "../services/cases/case-lifecycle.service.js";
...
import { resolveCaseDestructiveGate, evaluateCrossTeamAttach, } from "../services/cases/case-permission.service.js";
...
async function checkCaseLegalHold(caseId, teamId) {
    try {
...
function auditCaseAction(req, params) {
...
export async function casesRoutes(app) {
    ...
    app.get("/v1/cases/:id", { preHandler: requireAuthAndLegal }, async (req, reply) => {
    ...
    app.post("/v1/cases/:id/access", { preHandler: requireAuthAndLegal }, async (req, reply) => {
        const id = z.string().uuid().parse(req.params.id);
    ...
    app.get("/v1/cases/:id/export", { preHandler: requireAuthAndLegal }, async (req, reply) => {
        const id = z.string().uuid().parse(req.params.id);
        const ownerUserId = getAuthUserId(req);
        const item = await prisma.case.findUnique({
            where: { id },
    ...
    app.get("/v1/cases/:id/team-members", { preHandler: requireAuthAndLegal }, async (req, reply) => {
        const id = z.string().uuid().parse(req.params.id);
        const ownerUserId = getAuthUserId(req);
    ...
    app.patch("/v1/cases/:id", { preHandler: requireAuthAndLegal }, async (req, reply) => {
        const id = z.string().uuid().parse(req.params.id);
    ...
    app.delete("/v1/cases/:id", { preHandler: requireAuthAndLegal }, async (req, reply) => {
        const id = z.string().uuid().parse(req.params.id);
        const userId = getAuthUserId(req);
        const item = await prisma.case.findUnique({ where: { id } });
        if (!item) {
    ...
    app.post("/v1/cases/:id/evidence", { preHandler: requireAuthAndLegal }, async (req, reply) => {
        const id = z.string().uuid().parse(req.params.id);
    ...
    app.delete("/v1/cases/:id/evidence/:evidenceId", { preHandler: requireAuthAndLegal }, async (req, reply) => {
        const id = z.string().uuid().parse(req.params.id);
        const evidenceId = z
            .string()
            .uuid()
    ...
    app.get("/v1/cases/:id/available-evidence", { preHandler: requireAuthAndLegal }, async (req, reply) => {
        const id = z.string().uuid().parse(req.params.id);
        const ownerUserId = getAuthUserId(req);
        const caseItem = await prisma.case.findUnique({ where: { id } });
        if (!caseItem) {
    ...
    app.post("/v1/cases/:id/share-team", { preHandler: requireAuthAndLegal }, async (req, reply) => {
        const id = z.string().uuid().parse(req.params.id);
    ...
    app.post("/v1/cases/:id/share-email", { preHandler: requireAuthAndLegal }, async (req, reply) => {
        const id = z.string().uuid().parse(req.params.id);
    ...
    app.delete("/v1/cases/:id/access/:accessId", { preHandler: requireAuthAndLegal }, async (req, reply) => {
        const id = z.string().uuid().parse(req.params.id);
        const accessId = z
            .string()
            .uuid()
            .parse(req.params.accessId);
        const ownerUserId = getAuthUserId(req);
        const caseItem = await prisma.case.findUnique({ where: { id } });
        if (!caseItem) {
            auditCaseAction(req, {
    ...
    app.post("/v1/cases/bulk", { preHandler: requireAuthAndLegal }, async (req, reply) => {
        const userId = getAuthUserId(req);
        const parsed = BulkCasesBody.safeParse(req.body ?? {});
        if (!parsed.success) {
            return reply.code(400).send({
                code: "INVALID_BODY",
                message: "ids (1-100 UUIDs) and action (CLOSE | ARCHIVE | RESOLVE) are required.",
            });
        }
        ...
        for (const id of ids) {
            ...
            try {
                ...
                if (err instanceof CaseError) {
                    ...
                    req.log.warn({ err, caseId: id, action }, "cases.bulk.case_failed");
                    results.push({
                        id,
                        outcome: "SKIPPED",
    ...
    // ===========================================================================
    app.get("/v1/cases/:id/link-reconciliation", { preHandler: requireAuthAndLegal }, async (req, reply) => {
        const userId = getAuthUserId(req);
        ...
        const canonicalEvidenceIds = new Set(canonicalLinks.map((l) => l.evidenceId));
        ...
        const legacyOnly = legacyAttached
            ...
            attachmentKind: "legacy_case_id_only",
        }));
        ...
        const canonicalOnly = canonicalLinks
            ...
            role: l.role,
            attachmentKind: "canonical_link_only",
        }));
        return reply.code(200).send({
            caseId,
            summary: {
                ...
                inSync: legacyOnly.length === 0 && canonicalOnly.length === 0,
            },
            legacyOnly,
            canonicalOnly,
        });
