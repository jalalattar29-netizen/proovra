...
import { addCredits, setPersonalPlan, activateTeamPlan, cancelTeamPlan, cancelWorkspaceStorageAddon, getStorageAddonDefinition, } from "../services/billing.service.js";
import { stripeRequestRaw } from "../services/stripe.service.js";
import { createStripeCheckoutSession, createPayPalCheckout, createStripeStorageAddonCheckoutSession, createPayPalStorageAddonCheckout, } from "../services/billing-checkout.service.js";
...
import { buildPricingCatalogResponse, resolveCheckoutCurrency, } from "../services/billing-pricing.service.js";
...
function auditBillingAction(req, params) {
...
async function assertOwnedTeamForCheckout(userId, teamId) {
    ...
    if (team.ownerUserId !== userId) {
        const err = new Error("Only the team owner can purchase or manage this team subscription");
        err.statusCode = 403;
        throw err;
    }
    return team;
...
function assertCheckoutTarget(params) {
    ...
    if (params.plan === prismaPkg.PlanType.TEAM && !params.teamId) {
        const err = new Error("teamId is required for TEAM checkout");
        err.statusCode = 400;
        throw err;
    }
    if (params.plan !== prismaPkg.PlanType.TEAM && params.teamId) {
        const err = new Error("teamId is only allowed for TEAM checkout");
        err.statusCode = 400;
        throw err;
    }
}
...
async function readActiveStorageAddons(userId) {
...
async function assertStorageAddonAllowed(params) {
    if (params.billingCycle !== prismaPkg.StorageAddonBillingCycle.ONE_TIME) {
        const err = new Error("Storage add-ons are available only as one-time purchases");
        err.statusCode = 400;
        throw err;
    }
    ...
    if (params.teamId) {
        ...
        if (definition.workspaceType !== "TEAM") {
            const err = new Error("This storage add-on is not valid for team workspaces");
            err.statusCode = 400;
            throw err;
        }
        /**
        ...
        if (!caps.allowsTeamWorkspace) {
            const err = new Error("This workspace does not currently support team storage add-ons");
            err.statusCode = 409;
    ...
    if (definition.workspaceType !== "PERSONAL") {
        const err = new Error("This storage add-on is not valid for personal workspaces");
        err.statusCode = 400;
        throw err;
    }
    if (scope.plan === prismaPkg.PlanType.FREE) {
        const err = new Error("Please upgrade your base plan before purchasing extra storage");
        err.statusCode = 409;
        throw err;
    }
    if (scope.plan === prismaPkg.PlanType.PAYG) {
        if (params.addonKey !== prismaPkg.StorageAddonKey.PERSONAL_10_GB &&
            params.addonKey !== prismaPkg.StorageAddonKey.PERSONAL_50_GB) {
            const err = new Error("PAYG supports only +10 GB and +50 GB storage add-ons");
            err.statusCode = 400;
            throw err;
        }
        return {
    ...
    const err = new Error("Unsupported workspace plan for storage add-ons");
    err.statusCode = 400;
    throw err;
}
export async function billingRoutes(app) {
    ...
    app.get("/v1/billing/storage-addons", { preHandler: requireAuthAndLegal }, async (req, reply) => {
    ...
    app.get("/v1/billing/overview", { preHandler: requireAuthAndLegal }, async (req, reply) => {
        const userId = getAuthUserId(req);
    ...
    app.get("/v1/billing/subscription", { preHandler: requireAuthAndLegal }, async (req, reply) => {
        const userId = getAuthUserId(req);
    ...
    app.post("/v1/billing/subscription/cancel", { preHandler: requireAuthAndLegal }, async (req, reply) => {
        const userId = getAuthUserId(req);
        ...
        if (subscription.provider === prismaPkg.PaymentProvider.STRIPE) {
            ...
            try {
                await cancelPayPalSubscription(subscription.providerSubId, "Canceled by customer");
            }
            catch (err) {
                req.log.warn({
                    err,
     ...
     */
    app.post("/v1/billing/storage-addons/cancel", { preHandler: requireAuthAndLegal }, async (req, reply) => {
        const userId = getAuthUserId(req);
        ...
        if (addon.paymentProvider === prismaPkg.PaymentProvider.STRIPE) {
            await stripeRequestRaw(`/subscriptions/${addon.externalSubscriptionId}`, "DELETE");
        }
        else if (addon.paymentProvider === prismaPkg.PaymentProvider.PAYPAL) {
            try {
                await cancelPayPalSubscription(addon.externalSubscriptionId, "Canceled by customer");
            }
            catch (err) {
                req.log.warn({
                    err,
    ...
    app.post("/v1/billing/checkout/stripe", { preHandler: requireAuthAndLegal }, async (req, reply) => {
    ...
    app.post("/v1/billing/storage-addons/checkout/stripe", { preHandler: requireAuthAndLegal }, async (req, reply) => {
    ...
    app.post("/v1/billing/checkout/paypal", { preHandler: requireAuthAndLegal }, async (req, reply) => {
        const body = CheckoutBody.parse(req.body);
        const userId = getAuthUserId(req);
        assertPurchasablePlan(body.plan);
        assertCheckoutTarget({
        ...
        const resourceId = "subscription" in result
    ...
    app.post("/v1/billing/storage-addons/checkout/paypal", { preHandler: requireAuthAndLegal }, async (req, reply) => {
        const body = StorageAddonCheckoutBody.parse(req.body ?? {});
        if (body.billingCycle !== prismaPkg.StorageAddonBillingCycle.ONE_TIME) {
            return reply.code(400).send({
                message: "Storage add-ons are available only as one-time purchases",
    ...
    app.post("/v1/billing/credits", { preHandler: requireAuthAndLegal }, async (req, reply) => {
        const body = z
    ...
    app.post("/v1/billing/plan", { preHandler: requireAuthAndLegal }, async (req, reply) => {
        const body = z
            .object({
            plan: PlanTypeSchema,
            teamId: z.string().uuid().optional(),
