...
import { ANALYTICS_DEFAULT_WINDOW_DAYS, ANALYTICS_MAX_WINDOW_DAYS, ANALYTICS_MIN_WINDOW_DAYS, clampWindow, getArtifactReadinessAnalytics, getAutomationAnalytics, getGovernanceAnalytics, getOperationsOverview, getReviewerAnalytics, } from "../services/analytics/analytics.service.js";
// ---------------------------------------------------------------------------
// Query parsing
// ---------------------------------------------------------------------------
...
async function gateAnalyticsRead(req, reply, teamId) {
    ...
    const scope = team.isPersonal ? "PERSONAL" : "TEAM";
...
function buildInput(parsed, scope) {
    return {
...
// Route registrar
// ---------------------------------------------------------------------------
export async function analyticsOperationsRoutes(app) {
    // -----------------------------------------------------------------
    // GET /v1/analytics/operations
    // -----------------------------------------------------------------
    app.get("/v1/analytics/operations", { preHandler: requireAuth }, async (req, reply) => {
    ...
    // -----------------------------------------------------------------
    app.get("/v1/analytics/reviewer", { preHandler: requireAuth }, async (req, reply) => {
        const q = AnalyticsQuery.safeParse(req.query);
        if (!q.success) {
            return reply
                .code(400)
    ...
    // -----------------------------------------------------------------
    app.get("/v1/analytics/governance", { preHandler: requireAuth }, async (req, reply) => {
        const q = AnalyticsQuery.safeParse(req.query);
        if (!q.success) {
            return reply
                .code(400)
    ...
    // -----------------------------------------------------------------
    app.get("/v1/analytics/automation", { preHandler: requireAuth }, async (req, reply) => {
        const q = AnalyticsQuery.safeParse(req.query);
        if (!q.success) {
            return reply
                .code(400)
    ...
    // -----------------------------------------------------------------
    app.get("/v1/analytics/artifacts", { preHandler: requireAuth }, async (req, reply) => {
        const q = AnalyticsQuery.safeParse(req.query);
        if (!q.success) {
            return reply
                .code(400)
        ...
        if (!gate.ok)
            return;
        const envelope = await getArtifactReadinessAnalytics(buildInput(q.data, gate.scope));
        reply.send(envelope);
    });
    // -----------------------------------------------------------------
    ...
    // -----------------------------------------------------------------
    app.get("/v1/analytics/_window", { preHandler: requireAuth }, async (_req, reply) => {
        reply.send({
