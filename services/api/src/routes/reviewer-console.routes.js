...
import { buildDashboard, listReviewerOpsQueue, } from "../services/reviewer-ops/reviewer-operations-engine.service.js";
...
async function safeSection(fn, empty, request, surface) {
    try {
...
async function ensureReviewerActor(req, reply, teamId) {
...
export async function reviewerConsoleRoutes(app, _options = {}, prismaClient) {
    ...
    app.get("/v1/reviewer-ops/console", { preHandler: requireAuth }, async (req, reply) => {
        ...
        const [queueSection, mineSection, escalationSection, dashboardSection, workloadSection, savedViewsSection,] = await Promise.all([
            safeSection(() => listReviewerOpsQueue({
                teamId,
                meUserId: ctx.userId,
                queue: "UNASSIGNED",
                limit: SECTION_LIMIT,
            ...
            // empty case.
            { rows: [], nextCursor: null }, req, "queue"),
            safeSection(() => listReviewerOpsQueue({
                teamId,
                meUserId: ctx.userId,
                ...
                queue: "MY_REVIEWS",
                limit: SECTION_LIMIT,
            }, client), { rows: [], nextCursor: null }, req, "mine"),
            safeSection(() => 
            ...
            listEscalations({
                ...
                status: "OPEN",
                limit: SECTION_LIMIT,
            }, client), [], req, "escalations"),
            safeSection(() => buildDashboard({ teamId, meUserId: ctx.userId }, client), null, req, "dashboard"),
            safeSection(() => listLatestWorkloadSnapshots({ teamId, limit: SECTION_LIMIT }, client), [], req, "workload"),
            safeSection(() => listReviewerOpsSavedViews({ teamId, userId: ctx.userId }), [], req, "savedViews"),
        ]);
        ...
        let slaStatus = dashboardSection.status;
        try {
            const dv = dashboardSection.value;
            if (dv && dv.counts) {
                slaSnapshot = {
