...
import { EXPORT_KINDS, listExports, resolveExportManifest, } from "../services/operations/export-manifest.service.js";
...
async function requireOpsReader(req, reply, teamId) {
...
export async function operationsExportsRoutes(app) {
    // -------------------------------------------------------------------
    // List
    // -------------------------------------------------------------------
    app.get("/v1/operations/exports", { preHandler: requireAuth }, async (req, reply) => {
        const q = z
            .object({
    ...
    // -------------------------------------------------------------------
    app.get("/v1/operations/exports/object-lock", { preHandler: requireAuth }, async (req, reply) => {
        const q = z
    ...
    // -------------------------------------------------------------------
    app.get("/v1/operations/exports/:id", { preHandler: requireAuth }, async (req, reply) => {
        const params = z
    ...
    // -------------------------------------------------------------------
    app.get("/v1/operations/exports/:id/manifest", { preHandler: requireAuth }, async (req, reply) => {
        const params = z
            .object({ id: z.string().min(1).max(200) })
            .parse(req.params);
        const q = z
    ...
    // -------------------------------------------------------------------
    app.post("/v1/operations/exports/:id/verify", { preHandler: requireAuth }, async (req, reply) => {
        const params = z
            .object({ id: z.string().min(1).max(200) })
            .parse(req.params);
        const body = z
        ...
        if (result.report.outcome === "artifact_drift" ||
        ...
        safeEmitSecurityEvent({
            ...
            severity: result.report.outcome === "match" ? "INFO" : "WARNING",
            details: {
