...
import { MFA_POLICY_LEVELS, STEP_UP_PURPOSES, } from "@proovra/shared";
...
import { StepUpError, checkStepUpChallenge, projectStepUpChallenge, startStepUpChallenge, } from "../services/identity-security/step-up.service.js";
import { evaluateMfaRequirement, getMfaPolicy, updateMfaPolicy, } from "../services/identity-security/mfa-policy.service.js";
import { getRiskSnapshotForUser, } from "../services/identity-security/risk.service.js";
import { hashSessionId, listRevocationsForTeam, projectRevokedSession, revokeAllSessionsForUser, revokeSession, } from "../services/identity-security/session-revocation.service.js";
import { listTrustedDevicesForTeam, markDeviceTrusted, projectTrustedDevice, revokeTrustedDevice, } from "../services/identity-security/trusted-device.service.js";
import { requireStepUpForSensitiveAction } from "../services/identity-security/step-up-middleware.js";
import { changePasswordForUser, isPasswordPolicyCompliant, } from "../services/email-password-auth.service.js";
 ...
 */
async function requireSecurityActor(req, reply, teamId, permission) {
...
function handleStepUpError(reply, err) {
...
export async function identitySecurityRoutes(app) {
    // -------------------------------------------------------------------------
    // Step-up
    // -------------------------------------------------------------------------
    ...
    app.post("/v1/identity-security/step-up/start", { preHandler: requireAuth }, async (req, reply) => {
        ...
        try {
            ...
            if (handleStepUpError(reply, err))
                return;
            throw err;
        }
    });
    ...
    app.post("/v1/identity-security/step-up/check", { preHandler: requireAuth }, async (req, reply) => {
    ...
    app.post("/v1/identity-security/sessions/revoke", { preHandler: requireAuth }, async (req, reply) => {
        ...
        const result = await revokeSession({
            ...
            reason: (body.reason ?? "OPERATOR_REVOKED"),
    ...
    app.post("/v1/identity-security/sessions/revoke-all", { preHandler: requireAuth }, async (req, reply) => {
    ...
    app.get("/v1/identity-security/sessions", { preHandler: requireAuth }, async (req, reply) => {
    ...
    // Trusted devices
    // -------------------------------------------------------------------------
    app.get("/v1/identity-security/devices", { preHandler: requireAuth }, async (req, reply) => {
        const q = TeamIdQuery.parse(req.query ?? {});
        const actor = await requireSecurityActor(req, reply, q.teamId, "identity.member.read");
        if (!actor)
            return;
    ...
    app.post("/v1/identity-security/devices/trust", { preHandler: requireAuth }, async (req, reply) => {
    ...
    app.post("/v1/identity-security/devices/:id/revoke", { preHandler: requireAuth }, async (req, reply) => {
    ...
    // MFA policy
    // -------------------------------------------------------------------------
    app.get("/v1/identity-security/mfa-policy", { preHandler: requireAuth }, async (req, reply) => {
        const q = TeamIdQuery.parse(req.query ?? {});
        ...
        const requirement = await evaluateMfaRequirement({
            teamId: q.teamId,
            role: (await prisma.teamMember.findUnique({
    ...
    const PolicyBody = z.object({
        teamId: z.string().uuid(),
        level: z.enum(MFA_POLICY_LEVELS),
    ...
    app.put("/v1/identity-security/mfa-policy", { preHandler: requireAuth }, async (req, reply) => {
    ...
    // Risk snapshot
    // -------------------------------------------------------------------------
    app.get("/v1/identity-security/risk/me", { preHandler: requireAuth }, async (req, reply) => {
        const q = TeamIdQuery.parse(req.query ?? {});
        const actor = await requireSecurityActor(req, reply, q.teamId, "identity.member.read");
        if (!actor)
            return;
    ...
    app.get("/v1/identity-security/risk/user/:id", { preHandler: requireAuth }, async (req, reply) => {
        const { id } = ParamsId.parse(req.params);
        const q = TeamIdQuery.parse(req.query ?? {});
    ...
    // Reconcile cron
    // -------------------------------------------------------------------------
    app.post("/v1/identity-security/reconcile", async (req, reply) => {
    ...
    app.post("/v1/identity-security/password", { preHandler: requireAuth }, async (req, reply) => {
        const userId = getAuthUserId(req);
        ...
        if (!result.ok) {
            await appendPlatformAuditLog({
                ...
                action: "identity_security.password_change",
                category: "identity_security",
                severity: result.reason === "current_password_mismatch" ? "warning" : "info",
                source: "api_identity_security",
                outcome: "failure",
                resourceType: "user",
                resourceId: userId,
            ...
            if (result.reason === "current_password_mismatch" ||
        ...
        if (body.revokeOtherSessions) {
            const currentHash = req.sessionIdHash ?? null;
            const where = { userId, revokedAtUtc: null };
            if (currentHash)
                where.NOT = { sessionIdHash: currentHash };
    ...
    app.get("/v1/identity-security/my-sessions", { preHandler: requireAuth }, async (req, reply) => {
        const userId = getAuthUserId(req);
        const now = new Date();
    ...
    app.post("/v1/identity-security/my-sessions/revoke-others", { preHandler: requireAuth }, async (req, reply) => {
        const userId = getAuthUserId(req);
        const currentHash = req.sessionIdHash ?? null;
        const where = { userId, revokedAtUtc: null };
        if (currentHash)
    ...
    app.get("/v1/identity-security/security-events", { preHandler: requireAuth }, async (req, reply) => {
        const userId = getAuthUserId(req);
        const Limit = z.coerce.number().int().min(1).max(200).default(50);
        const limit = Limit.parse(req.query?.limit ?? 50);
