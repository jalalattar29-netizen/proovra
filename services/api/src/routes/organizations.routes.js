...
export async function organizationsRoutes(app) {
    ...
    // -------------------------------------------------------------------------
    app.get("/v1/me/orgs", { preHandler: requireAuthAndLegal }, async (req, reply) => {
        ...
        const now = new Date();
        const [memberCounts, workspaceCounts, pendingInviteCounts] = orgIds.length === 0
            ? [[], [], []]
            : await Promise.all([
    ...
    // -------------------------------------------------------------------------
    app.get("/v1/orgs/:id", { preHandler: requireAuthAndLegal }, async (req, reply) => {
        const orgId = UuidParam.parse(req.params.id);
        const userId = getAuthUserId(req);
        ...
        const [memberCount, workspaceCount, pendingInviteCount] = await Promise.all([
    ...
    // -------------------------------------------------------------------------
    app.get("/v1/orgs/:id/members", { preHandler: requireAuthAndLegal }, async (req, reply) => {
    ...
    // -------------------------------------------------------------------------
    app.get("/v1/orgs/:id/workspaces", { preHandler: requireAuthAndLegal }, async (req, reply) => {
        const orgId = UuidParam.parse(req.params.id);
        const userId = getAuthUserId(req);
        const access = await checkOrgAccess(prisma, { orgId, userId });
        if (access.kind !== "ok") {
        ...
        const canSeeBilling = access.role === "ORG_OWNER" ||
    ...
    // -------------------------------------------------------------------------
    app.post("/v1/orgs", { preHandler: requireAuthAndLegal }, async (req, reply) => {
    ...
    // -------------------------------------------------------------------------
    app.patch("/v1/orgs/:id", { preHandler: requireAuthAndLegal }, async (req, reply) => {
        const orgId = UuidParam.parse(req.params.id);
        const userId = getAuthUserId(req);
    ...
    // -------------------------------------------------------------------------
    app.post("/v1/orgs/:id/invites", { preHandler: requireAuthAndLegal }, async (req, reply) => {
        const orgId = UuidParam.parse(req.params.id);
        const userId = getAuthUserId(req);
        const body = InviteBody.parse(req.body);
        const inviteRole = body.role ?? "ORG_MEMBER";
        const access = await checkOrgAccess(prisma, {
            orgId,
            userId,
            minRole: "ORG_ADMIN",
        ...
        const result = await prisma.$transaction(async (tx) => {
            ...
            if (existingPending) {
                return { kind: "conflict", reason: "pending_invite_exists" };
            }
    ...
    // -------------------------------------------------------------------------
    app.post("/v1/org-invites/:token/accept", { preHandler: requireAuthAndLegal }, async (req, reply) => {
        const token = z
            .string()
            .min(16)
            .max(128)
        ...
        const result = await prisma.$transaction(async (tx) => {
            ...
            const recordRejection = async (reason) => {
                if (!invite)
                    return; // no org to scope the event to
                await emitOrgAuditEvent(tx, {
    ...
    // -------------------------------------------------------------------------
    app.get("/v1/orgs/:id/invites", { preHandler: requireAuthAndLegal }, async (req, reply) => {
        const orgId = UuidParam.parse(req.params.id);
        const userId = getAuthUserId(req);
        const access = await checkOrgAccess(prisma, {
            orgId,
    ...
    // -------------------------------------------------------------------------
    app.delete("/v1/orgs/:id/invites/:inviteId", { preHandler: requireAuthAndLegal }, async (req, reply) => {
        const orgId = UuidParam.parse(req.params.id);
        const inviteId = UuidParam.parse(req.params.inviteId);
        const userId = getAuthUserId(req);
        const access = await checkOrgAccess(prisma, {
            orgId,
            userId,
    ...
    // that case).
    // -------------------------------------------------------------------------
    app.post("/v1/orgs/:id/invites/:inviteId/resend", { preHandler: requireAuthAndLegal }, async (req, reply) => {
        const orgId = UuidParam.parse(req.params.id);
        const inviteId = UuidParam.parse(req.params.inviteId);
        const userId = getAuthUserId(req);
        const access = await checkOrgAccess(prisma, {
    ...
    // -------------------------------------------------------------------------
    app.patch("/v1/orgs/:id/members/:memberId", { preHandler: requireAuthAndLegal }, async (req, reply) => {
        const orgId = UuidParam.parse(req.params.id);
        ...
        const result = await prisma.$transaction(async (tx) => {
            ...
            if (target.userId === userId) {
                return { kind: "self_modify_blocked" };
            }
            // Only ORG_OWNER can change role TO or FROM ORG_OWNER.
            const involvesOwnerRole = target.role === "ORG_OWNER" || newRole === "ORG_OWNER";
            if (involvesOwnerRole && access.role !== "ORG_OWNER") {
                return { kind: "owner_change_requires_owner" };
            }
            ...
            if (target.role === "ORG_OWNER" && newRole !== "ORG_OWNER") {
                ...
                if (ownerCount <= 1) {
                    return { kind: "last_owner_protected" };
                }
            }
            if (target.role === newRole) {
                return { kind: "noop", membershipId: target.id };
            }
        ...
        switch (result.kind) {
            ...
            case "owner_change_requires_owner":
                return reply.code(403).send({
                    message: "Only ORG_OWNER can change a membership to or from ORG_OWNER.",
                });
    ...
    // another ORG_OWNER.
    // -------------------------------------------------------------------------
    app.delete("/v1/orgs/:id/members/:memberId", { preHandler: requireAuthAndLegal }, async (req, reply) => {
        const orgId = UuidParam.parse(req.params.id);
        const memberId = UuidParam.parse(req.params.memberId);
        const userId = getAuthUserId(req);
        const access = await checkOrgAccess(prisma, {
        ...
        const result = await prisma.$transaction(async (tx) => {
            ...
            if (target.userId === userId) {
                return { kind: "self_remove_blocked" };
            }
            if (target.role === "ORG_OWNER" && access.role !== "ORG_OWNER") {
                return { kind: "owner_remove_requires_owner" };
            }
            ...
            return { kind: "ok", membershipId: target.id };
        });
        switch (result.kind) {
            case "not_found":
                return reply.code(404).send({ message: "Membership not found." });
    ...
    // -------------------------------------------------------------------------
    app.get("/v1/orgs/:id/audit-events", { preHandler: requireAuthAndLegal }, async (req, reply) => {
        const orgId = UuidParam.parse(req.params.id);
        const userId = getAuthUserId(req);
        ...
        const actorIds = Array.from(new Set(events.map((e) => e.actorUserId).filter((x) => !!x)));
