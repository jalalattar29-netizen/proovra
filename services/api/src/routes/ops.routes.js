...
import { collectStartupViolations, getFeatureSnapshot, } from "../config/index.js";
...
import { bump, buildPrometheusExposition, setGauge, snapshotMetrics, } from "../services/ops/metrics.service.js";
...
import { IncidentError, acknowledgeIncident, assignIncident, listIncidents, projectIncident, resolveIncident, suppressIncident, } from "../services/observability/incident.service.js";
import { INCIDENT_CATEGORIES, INCIDENT_SEVERITIES, INCIDENT_STATUSES, } from "@proovra/shared";
...
async function runMasterReconcile() {
    const now = new Date();
    ...
    try {
        const { reapStaleMultipartUploads } = await import("../services/uploads/upload-session.service.js");
    ...
    try {
        ...
        const { reconcileTeamGraph } = await import("../services/graph/graph-builder.service.js");
...
export async function opsRoutes(app) {
    ...
    app.get("/v1/ops/health", { preHandler: requireAuth }, async (req, reply) => {
    ...
    app.get("/v1/ops/metrics", { preHandler: requireAuth }, async (req, reply) => {
            ...
            if (!user?.currentWorkspaceId) {
                return reply.code(400).send({
                    error: {
                        ...
                        message: "Select a workspace to view operational metrics.",
                        requestId: req.id,
                    },
                });
            }
    ...
    app.get("/v1/ops/alerts", { preHandler: requireAuth }, async (req, reply) => {
        const q = TeamIdQuery.parse(req.query ?? {});
        const actor = await requireOpsActor(req, reply, q.teamId);
        if (!actor)
            return;
    ...
    app.get("/admin/runtime/schema-status", { preHandler: requireAuth }, async (req, reply) => {
        const q = TeamIdQuery.parse(req.query ?? {});
        const actor = await requireOpsActor(req, reply, q.teamId);
        if (!actor)
            return;
    ...
    app.post("/v1/ops/reconcile", async (req, reply) => {
    ...
    // ---------------------------------------------------------------------------
    app.get("/v1/ops/incidents", { preHandler: requireAuth }, async (req, reply) => {
        const q = z
            .object({
    ...
    function handleIncidentError(reply, err) {
        if (err instanceof IncidentError) {
            const status = err.code === "incident_not_found"
                ? 404
    ...
    app.post("/v1/ops/incidents/:id/ack", { preHandler: requireAuth }, async (req, reply) => {
        ...
        try {
            ...
            if (handleIncidentError(reply, err))
                return;
            throw err;
        }
    });
    app.post("/v1/ops/incidents/:id/resolve", { preHandler: requireAuth }, async (req, reply) => {
        const { id } = ParamsIncidentId.parse(req.params);
    ...
    app.post("/v1/ops/incidents/:id/suppress", { preHandler: requireAuth }, async (req, reply) => {
        const { id } = ParamsIncidentId.parse(req.params);
        const body = TeamIdOnly.parse(req.body ?? {});
        const actor = await requireOpsActorAction(req, reply, body.teamId);
        if (!actor)
    ...
    app.post("/v1/ops/incidents/:id/assign", { preHandler: requireAuth }, async (req, reply) => {
        const { id } = ParamsIncidentId.parse(req.params);
    ...
    function handleWorkflowError(reply, err) {
        const e = err;
    ...
    app.get("/v1/ops/workflows", { preHandler: requireAuth }, async (req, reply) => {
        const q = z
            .object({
            teamId: z.string().uuid(),
        ...
        if (!actor)
            return;
        const { listWorkflows } = await import("../services/observability/workflow.service.js");
    ...
    app.get("/v1/ops/workflows/:id", { preHandler: requireAuth }, async (req, reply) => {
        ...
        if (!actor)
            return;
        const { getWorkflow } = await import("../services/observability/workflow.service.js");
    ...
    app.post("/v1/ops/workflows/:id/assign", { preHandler: requireAuth }, async (req, reply) => {
        const { id } = ParamsWorkflowId.parse(req.params);
        ...
        try {
            const { assignWorkflow } = await import("../services/observability/workflow.service.js");
            ...
            if (handleWorkflowError(reply, err))
                return;
            throw err;
        }
    });
    app.post("/v1/ops/workflows/:id/start", { preHandler: requireAuth }, async (req, reply) => {
        const { id } = ParamsWorkflowId.parse(req.params);
        ...
        if (!actor)
            return;
        try {
            const { startWorkflow } = await import("../services/observability/workflow.service.js");
    ...
    app.post("/v1/ops/workflows/:id/escalate", { preHandler: requireAuth }, async (req, reply) => {
        const { id } = ParamsWorkflowId.parse(req.params);
        const body = TeamIdOnlyWorkflow.parse(req.body ?? {});
        const actor = await requireOpsActorAction(req, reply, body.teamId);
        if (!actor)
            return;
        try {
            const { escalateWorkflow } = await import("../services/observability/workflow.service.js");
    ...
    app.post("/v1/ops/workflows/:id/mitigation", { preHandler: requireAuth }, async (req, reply) => {
        const { id } = ParamsWorkflowId.parse(req.params);
        ...
        if (!actor)
            return;
        try {
            const { addMitigation } = await import("../services/observability/workflow.service.js");
    ...
    app.post("/v1/ops/workflows/:id/resolve", { preHandler: requireAuth }, async (req, reply) => {
        const { id } = ParamsWorkflowId.parse(req.params);
        ...
        if (!actor)
            return;
        try {
            const { resolveWorkflow } = await import("../services/observability/workflow.service.js");
    ...
    app.post("/v1/ops/workflows/:id/suppress", { preHandler: requireAuth }, async (req, reply) => {
        const { id } = ParamsWorkflowId.parse(req.params);
        const body = TeamIdOnlyWorkflow.parse(req.body ?? {});
        const actor = await requireOpsActorAction(req, reply, body.teamId);
        if (!actor)
            return;
        try {
            const { suppressWorkflow } = await import("../services/observability/workflow.service.js");
    ...
    app.post("/v1/ops/workflows/:id/reopen", { preHandler: requireAuth }, async (req, reply) => {
        const { id } = ParamsWorkflowId.parse(req.params);
        const body = TeamIdOnlyWorkflow.parse(req.body ?? {});
        const actor = await requireOpsActorAction(req, reply, body.teamId);
        if (!actor)
            return;
        try {
            const { reopenWorkflow } = await import("../services/observability/workflow.service.js");
    ...
    app.post("/v1/ops/workflows/:id/schedule-retry", { preHandler: requireAuth }, async (req, reply) => {
        const { id } = ParamsWorkflowId.parse(req.params);
        ...
        if (!actor)
            return;
        try {
            const { scheduleRetry } = await import("../services/observability/workflow.service.js");
    ...
    app.get("/v1/ops/causality/chains", { preHandler: requireAuth }, async (req, reply) => {
        const q = z
            .object({
            teamId: z.string().uuid(),
        ...
        if (!actor)
            return;
        const { listWorkspaceCausalityChains } = await import("../services/dashboard/causality.service.js");
    ...
    app.get("/v1/ops/causality/chains/:id", { preHandler: requireAuth }, async (req, reply) => {
        const { id } = z
    ...
    app.post("/v1/ops/bulk-actions", { preHandler: requireAuth }, async (req, reply) => {
        const body = BulkActionBody.parse(req.body ?? {});
        const actor = await requireOpsActorAction(req, reply, body.teamId);
        if (!actor)
            return;
        const { runBulkAction } = await import("../services/dashboard/bulk-actions.service.js");
    ...
    app.get("/v1/ops/bulk-actions/:id", { preHandler: requireAuth }, async (req, reply) => {
        const { id } = z
            .object({ id: z.string().uuid() })
            .parse(req.params);
        const q = z
        ...
        if (!actor)
            return;
        const { getBulkActionRun } = await import("../services/dashboard/bulk-actions.service.js");
    ...
    const ReplayDlqBody = z
        ...
        .strict();
    app.post("/v1/ops/media-intelligence/runs/:runId/retry", { preHandler: requireAuth }, async (req, reply) => {
        const { runId } = RetryRunParams.parse(req.params);
        const body = RetryRunBody.parse(req.body ?? {});
        const actor = await requireOpsActorAction(req, reply, body.teamId);
        if (!actor)
            return;
        const { retryMediaIntelligenceJob } = await import("../queue/media-intelligence-queue.js");
    ...
    app.post("/v1/ops/media-intelligence/dlq/replay", { preHandler: requireAuth }, async (req, reply) => {
        const body = ReplayDlqBody.parse(req.body ?? {});
        const actor = await requireOpsActorAction(req, reply, body.teamId);
        if (!actor)
            return;
        const { replayMediaIntelligenceDlq } = await import("../queue/media-intelligence-queue.js");
    ...
    app.post("/v1/ops/upload-telemetry", { preHandler: requireAuth }, async (req, reply) => {
        let body;
        try {
        ...
        if (!rate.allowed) {
            reply.header("retry-after", String(Math.max(1, Math.ceil((rate.resetAtMs - Date.now()) / 1000))));
 ...
 */
async function requireOpsActorAction(req, reply, teamId) {
    // Local import — avoids re-running the file-level imports against
    // the action-flavor of requireOpsActor.
    const { evaluateMemberAccess } = await import("../services/identity/access-policy.service.js");
    const userId = getAuthUserId(req);
    const member = await prisma.teamMember.findUnique({
        where: { teamId_userId: { teamId, userId } },
        select: { id: true },
