 ...
 */
import { z } from "zod";
import { WORKFLOW_INTAKE_SESSION_STATUSES, WorkflowIntakeConsentSnapshotSchema, } from "@proovra/shared";
import { enforceRateLimit } from "../services/rate-limit.js";
import { validateUploadedFile } from "../services/security/file-validation.service.js";
import { workflowIntakeFeatureDisabledReason, } from "../services/workflow-intake-token.service.js";
import { getIntakeSession, openIntakeSession, projectIntakeLinkForExternalView, projectIntakeSessionForExternalView, recordIntakeConsent, transitionIntakeSession, validateIntakeToken, WorkflowIntakeSessionError, } from "../services/workflow-intake-session.service.js";
import { addExternalEvidencePart, ExternalIntakeOrchestrationError, submitExternalIntake, updateExternalEvidencePartMapping, } from "../services/external-intake-orchestration.service.js";
...
async function applyRateLimits(req, reply, tokenKeyMaterial) {
...
function evidenceKindFromMime(mime) {
...
function projectExternalPart(part) {
    return {
        ...
        sizeBytes: part.sizeBytes !== null && part.sizeBytes !== undefined
            ? part.sizeBytes.toString()
            : null,
        uploadedAtUtc: part.uploadedAtUtc?.toISOString() ?? null,
    };
}
function orchestrationErrorToReply(err, reply) {
    switch (err.code) {
...
function intakeErrorToReply(err, reply) {
    switch (err.code) {
        case "token_invalid":
        case "link_not_found":
            reply.code(404).send({
...
// Routes
// -----------------------------------------------------------------------------
export async function externalIntakeRoutes(app) {
    ...
    // the session.
    app.get("/v1/external-intake/:token", async (req, reply) => {
        ...
        try {
            ...
            const session = await openIntakeSession({
                ...
                submitterUserAgent: typeof req.headers["user-agent"] === "string"
            ...
            const { projectRequestForExternalView } = await import("../services/evidence-request.service.js");
    ...
    app.post("/v1/external-intake/:token/sessions/:sid/consent", async (req, reply) => {
        if (workflowIntakeFeatureDisabledReason()) {
            return sendFeatureDisabled(reply);
        }
    ...
    app.post("/v1/external-intake/:token/sessions/:sid/parts", async (req, reply) => {
        if (workflowIntakeFeatureDisabledReason()) {
            return sendFeatureDisabled(reply);
        }
        const params = ParamsTokenSession.parse(req.params);
        ...
        try {
            ...
            if (link.allowedAcceptedKinds.length > 0 &&
                !link.allowedAcceptedKinds.includes(evidenceKindFromMime(body.mimeType))) {
            ...
            if (typeof link.maxFileCountPerSession === "number" &&
    ...
    app.patch("/v1/external-intake/:token/sessions/:sid/parts/:partId", async (req, reply) => {
        if (workflowIntakeFeatureDisabledReason()) {
            return sendFeatureDisabled(reply);
        }
        const params = z
    ...
    // to SUBMITTED.
    app.post("/v1/external-intake/:token/sessions/:sid/submit", async (req, reply) => {
        if (workflowIntakeFeatureDisabledReason()) {
            return sendFeatureDisabled(reply);
        }
        const params = ParamsTokenSession.parse(req.params);
    ...
    app.post("/v1/external-intake/:token/sessions/:sid/transition", async (req, reply) => {
        if (workflowIntakeFeatureDisabledReason()) {
            return sendFeatureDisabled(reply);
        }
        const params = ParamsTokenSession.parse(req.params);
