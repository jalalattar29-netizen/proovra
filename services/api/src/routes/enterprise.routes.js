...
function auditEnterpriseAction(req, params) {
...
async function getRealUsageStats(userId) {
    ...
    const [todayCount, weekCount, monthCount, totalCount, evidenceTypes] = await Promise.all([
...
async function getRealQuotas(userId) {
    const month = startOfMonth();
    ...
    const teamMembersLimit = activeEntitlement?.teamSeats && activeEntitlement.teamSeats > 0
...
export async function enterpriseRoutes(app) {
    // ---------------------------------------------------------------------------
    ...
    const LEGACY_RETIRED_BODY = {
        code: "API_KEYS_LEGACY_RETIRED",
        detail: "The legacy /v1/api-keys surface (in-memory, user-scoped) has been retired. " +
    ...
    function emitLegacyEndpointAuditAndRespond(req, reply, legacyAction) {
    ...
    app.all("/v1/api-keys", { preHandler: [requireAuthAndLegal] }, async (req, reply) => emitLegacyEndpointAuditAndRespond(req, reply, "api_keys_root"));
    app.all("/v1/api-keys/*", { preHandler: [requireAuthAndLegal] }, async (req, reply) => emitLegacyEndpointAuditAndRespond(req, reply, "api_keys_subpath"));
    app.post("/v1/batch-analysis", { preHandler: [requireAuthAndLegal] }, async (req) => {
        ...
        try {
            ...
            if (evidence.length !== evidenceIds.length) {
                ...
                throw new AppError(ErrorCode.EVIDENCE_NOT_FOUND, "Some evidence items not found or you don't have access");
            }
    ...
    app.get("/v1/batch-analysis/:id", { preHandler: [requireAuthAndLegal] }, async (req) => {
        const userId = req.user.sub;
        const { id } = req.params;
        try {
    ...
    app.get("/v1/batch-analysis", { preHandler: [requireAuthAndLegal] }, async (req) => {
        const userId = req.user.sub;
        try {
            ...
            return {
                data: jobs.map((job) => ({
                    ...
                    createdAt: job.createdAt,
                    completedAt: job.completedAt,
                    progress: Math.round(((job.processedItems + job.failedItems) / job.totalItems) * 100),
                })),
            };
        }
        catch (_error) {
    ...
    app.post("/v1/batch-analysis/:id/process", { preHandler: [requireAuthAndLegal] }, async (req) => {
        const userId = req.user.sub;
        const { id } = req.params;
        try {
            const job = batchAnalysisService.getJob(userId, id);
                ...
                try {
                    ...
                    if (emailService.isConfigured()) {
                        ...
                        if (completedJob) {
                            ...
                            if (userEmail) {
                                void emailService.sendBatchComplete(userEmail, "Organization", completedJob.name, completedJob.totalItems, completedJob.failedItems, `${process.env.APP_URL || "https://app.proovra.com"}/batch/${completedJob.id}`);
                            }
                        }
                    }
                }
    ...
    app.get("/v1/batch-analysis/:id/results", { preHandler: [requireAuthAndLegal] }, async (req) => {
        const userId = req.user.sub;
        const { id } = req.params;
        try {
            const job = batchAnalysisService.getJob(userId, id);
    ...
    app.post("/v1/batch-analysis/:id/cancel", { preHandler: [requireAuthAndLegal] }, async (req) => {
        const userId = req.user.sub;
        const { id } = req.params;
        try {
    ...
    app.get("/v1/batch-analysis/:id/export", { preHandler: [requireAuthAndLegal] }, async (req, res) => {
        const userId = req.user.sub;
        const { id } = req.params;
        try {
            const job = batchAnalysisService.getJob(userId, id);
    ...
    app.get("/v1/quotas", { preHandler: [requireAuthAndLegal] }, async (req) => {
        const userId = req.user.sub;
        try {
            ...
            throw new AppError(ErrorCode.INTERNAL_SERVER_ERROR, "Failed to load quotas");
        }
    });
    app.get("/v1/usage-stats", { preHandler: [requireAuthAndLegal] }, async (req) => {
        const userId = req.user.sub;
        try {
            ...
            throw new AppError(ErrorCode.INTERNAL_SERVER_ERROR, "Failed to load usage statistics");
        }
    });
}
