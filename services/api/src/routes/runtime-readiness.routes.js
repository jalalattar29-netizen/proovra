...
async function requireReadinessActor(req, reply, teamId) {
...
export async function runtimeReadinessRoutes(app) {
    // ---------------------------------------------------------------------------
    ...
    // ---------------------------------------------------------------------------
    app.get("/admin/runtime/readiness", { preHandler: requireAuth }, async (req, reply) => {
        ...
        try {
            ...
            if (report.status === "CRITICAL")
                bump("runtime_readiness_critical_total");
    ...
    // ---------------------------------------------------------------------------
    app.get("/admin/runtime/queues", { preHandler: requireAuth }, async (req, reply) => {
        const q = TeamIdQuery.parse(req.query ?? {});
        const actor = await requireReadinessActor(req, reply, q.teamId);
        if (!actor)
            return;
    ...
    // ---------------------------------------------------------------------------
    app.get("/admin/runtime/workers", { preHandler: requireAuth }, async (req, reply) => {
        const q = TeamIdQuery.parse(req.query ?? {});
        const actor = await requireReadinessActor(req, reply, q.teamId);
        if (!actor)
            return;
    ...
    // GET /admin/runtime/migrations — migration drift detail.
    // ---------------------------------------------------------------------------
    app.get("/admin/runtime/migrations", { preHandler: requireAuth }, async (req, reply) => {
        const q = TeamIdQuery.parse(req.query ?? {});
        const actor = await requireReadinessActor(req, reply, q.teamId);
        if (!actor)
            return;
