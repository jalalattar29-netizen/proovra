...
import { getQueueInventory, getWorkerHealth, listFailedJobs, } from "../services/operations/queue-inventory.service.js";
import { getReplaySafetyMatrix, getJobReplayCategory, KNOWN_QUEUE_NAMES, } from "../services/operations/queue-replay-safety.service.js";
import { cancelJob, replayFailedJob, retryFailedJob, } from "../services/operations/queue-replay-action.service.js";
...
const KnownQueueName = z.enum(KNOWN_QUEUE_NAMES);
export async function operationsQueuesRoutes(app) {
    // -------------------------------------------------------------------
    // Inventory
    // -------------------------------------------------------------------
    app.get("/v1/operations/queues", { preHandler: requireAuth }, async (req, reply) => {
        const q = z
    ...
    // Worker health
    // -------------------------------------------------------------------
    app.get("/v1/operations/queues/workers", { preHandler: requireAuth }, async (req, reply) => {
        const q = z
            .object({ teamId: z.string().uuid() })
            .parse(req.query ?? {});
        const ctx = await requireOpsActor(req, reply, q.teamId);
        ...
        if (missingCount > 0)
            bump("worker_heartbeat_missing_total");
        return reply.code(200).send({ workers });
    });
    // -------------------------------------------------------------------
    // Replay safety matrix
    // -------------------------------------------------------------------
    app.get("/v1/operations/queues/replay-safety", { preHandler: requireAuth }, async (req, reply) => {
        const q = z
            .object({ teamId: z.string().uuid() })
            .parse(req.query ?? {});
        const ctx = await requireOpsActor(req, reply, q.teamId);
    ...
    // -------------------------------------------------------------------
    app.get("/v1/operations/queues/:queueName/failed", { preHandler: requireAuth }, async (req, reply) => {
        const params = z
    ...
    // Retry
    // -------------------------------------------------------------------
    app.post("/v1/operations/queues/:queueName/jobs/:jobId/retry", { preHandler: requireAuth }, async (req, reply) => {
        const params = z
        ...
        if (!result.ok) {
            const status = result.code === "replay_forbidden"
                ? 403
    ...
    // -------------------------------------------------------------------
    app.post("/v1/operations/queues/:queueName/jobs/:jobId/replay", { preHandler: requireAuth }, async (req, reply) => {
        const params = z
            .object({ queueName: KnownQueueName, jobId: z.string().min(1).max(200) })
            .parse(req.params);
        const body = z
    ...
    // Cancel
    // -------------------------------------------------------------------
    app.post("/v1/operations/queues/:queueName/jobs/:jobId/cancel", { preHandler: requireAuth }, async (req, reply) => {
        const params = z
            .object({ queueName: KnownQueueName, jobId: z.string().min(1).max(200) })
            .parse(req.params);
        const body = z
        ...
        if (!result.ok) {
            const status = result.code === "job_not_found"
                ? 404
                : 400;
            return reply
                .code(status)
