...
import { runGovernanceHandler, isPrismaTableOrColumnMissing, extractPrismaDiagnostic, } from "./_governance-error-bound.js";
import { requireIntegrationCronSecret } from "../middleware/cron-secret.js";
import { listLegalHoldsForEvidence, listLegalHoldsForTeam, loadWorkspaceGovernancePolicy, placeLegalHold, projectEffectivePolicy, projectLegalHold, releaseLegalHold, requirePermission, upsertWorkspaceGovernancePolicy, } from "../services/governance.service.js";
import { CaseLegalHoldError, isEvidenceUnderAnyLegalHold, listCaseLegalHolds, placeCaseLegalHold, projectCaseLegalHold, releaseCaseLegalHold, } from "../services/governance/case-legal-hold.service.js";
import { PublicationError, projectPublicationState, publishPublicVerify, restorePublicVerify, suspendPublicVerify, unpublishPublicVerify, } from "../services/governance/publication.service.js";
import { listRetentionCandidates, reconcileRetention, } from "../services/governance/retention-sweeper.service.js";
...
export async function governanceRoutes(app) {
    // GET policy
    app.get("/v1/governance/policy", { preHandler: requireAuth }, async (req, reply) => {
        const query = z
    ...
    // PUT policy (upsert)
    app.put("/v1/governance/policy", { preHandler: requireAuth }, async (req, reply) => {
        const body = z
            .object({
    ...
    app.get("/v1/governance/legal-holds", { preHandler: requireAuth }, async (req, reply) => {
        const query = z
            .object({
            teamId: z.string().uuid(),
    ...
    app.post("/v1/governance/legal-holds", { preHandler: requireAuth }, async (req, reply) => {
        const body = z
            .object({
            teamId: z.string().uuid(),
    ...
    app.post("/v1/governance/legal-holds/:id/release", { preHandler: requireAuth }, async (req, reply) => {
    ...
    app.get("/v1/governance/evidence/:id/status", { preHandler: requireAuth }, async (req, reply) => {
        const { id } = ParamsId.parse(req.params);
        const query = z
            .object({ teamId: z.string().uuid() })
            .parse(req.query ?? {});
    ...
    app.get("/v1/governance/evidence/:id/holds", { preHandler: requireAuth }, async (req, reply) => {
        const { id } = ParamsId.parse(req.params);
        const query = z
            .object({ teamId: z.string().uuid() })
            .parse(req.query ?? {});
    ...
    // ===========================================================================
    app.get("/v1/governance/case-legal-holds", { preHandler: requireAuth }, async (req, reply) => {
        const query = z
            .object({
            teamId: z.string().uuid(),
    ...
    app.post("/v1/governance/case-legal-holds", { preHandler: requireAuth }, async (req, reply) => {
        const body = z
            .object({
            teamId: z.string().uuid(),
    ...
    app.post("/v1/governance/case-legal-holds/:id/release", { preHandler: requireAuth }, async (req, reply) => {
        const { id } = ParamsId.parse(req.params);
        const body = z
            .object({
            teamId: z.string().uuid(),
        ...
        if (policy.requireLegalHoldReleaseApproval &&
    ...
    // ===========================================================================
    app.post("/v1/governance/evidence/:id/publish", { preHandler: requireAuth }, async (req, reply) => {
        const { id } = ParamsId.parse(req.params);
        const body = z
            .object({
            teamId: z.string().uuid(),
    ...
    app.post("/v1/governance/evidence/:id/unpublish", { preHandler: requireAuth }, async (req, reply) => {
        const { id } = ParamsId.parse(req.params);
        const body = z
            .object({
            teamId: z.string().uuid(),
    ...
    app.post("/v1/governance/evidence/:id/suspend", { preHandler: requireAuth }, async (req, reply) => {
        const { id } = ParamsId.parse(req.params);
        const body = z
            .object({
            teamId: z.string().uuid(),
    ...
    app.post("/v1/governance/evidence/:id/restore", { preHandler: requireAuth }, async (req, reply) => {
        const { id } = ParamsId.parse(req.params);
        const body = z
            .object({
            teamId: z.string().uuid(),
    ...
    // ===========================================================================
    app.get("/v1/governance/retention-candidates", { preHandler: requireAuth }, async (req, reply) => {
        const query = z
            .object({
            teamId: z.string().uuid(),
            limit: z.coerce.number().int().min(1).max(200).optional(),
    ...
    app.post("/v1/governance/reconcile-retention", async (req, reply) => {
    ...
    // ===========================================================================
    app.get("/v1/governance/evidence/:id/is-held", { preHandler: requireAuth }, async (req, reply) => {
        const { id } = ParamsId.parse(req.params);
        const query = z
            .object({ teamId: z.string().uuid() })
            .parse(req.query ?? {});
...
function publicationErrorToReply(err, reply) {
    if (err instanceof PublicationError) {
        const status = err.code === "evidence_not_in_workspace"
            ? 404
