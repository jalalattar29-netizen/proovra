...
import {
  ...
  readWorkspacePersonaProfile,
} from "../services/platform-context/persona-profile.service.js";
import {
...
} from "../services/platform-context/types.js";

const PROFILE_ENUM = z.enum(
  WORKSPACE_PERSONA_PROFILES as unknown as [string, ...string[]],
);
const DENSITY_ENUM = z.enum(
  OPERATIONAL_DENSITY_PREFERENCES as unknown as [string, ...string[]],
);
...
export async function workspacePersonaRoutes(app: FastifyInstance): Promise<void> {
  // ---------------------------------------------------------------------------
  // GET /v1/workspaces/:teamId/persona — read profile (TEAM_VIEW).
  // ---------------------------------------------------------------------------
  app.get(
    "/v1/workspaces/:teamId/persona",
    ...
    async (req, reply) => {
      ...
      const profile = await readWorkspacePersonaProfile({
        teamId: params.data.teamId,
        resolvedRolePersona: "INDIVIDUAL" satisfies Persona,
      });
    ...
    async (req, reply) => {
      ...
      const after = await prisma.workspacePersonaProfile.upsert({
        ...
        create: {
          ...
          secondaryUseCases: (update.secondaryUseCases ?? []) as unknown as object,
          ...
          operationalDensityPreference:
            update.operationalDensityPreference ?? "comfortable",
          featurePriorityOverrides:
            (update.featurePriorityOverrides ?? []) as unknown as object,
          onboardingState: (update.onboardingState ?? {}) as unknown as object,
        },
        update: {
...
function sanitiseUpdate(input: UpdatePersonaInput): {
