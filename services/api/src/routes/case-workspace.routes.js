...
import { buildCasesSummary, buildCaseWorkspace, } from "../services/cases/case-workspace.service.js";
...
import { CaseError, changeCaseStatus, addCaseAssignment, removeCaseAssignment, addCaseComment, resolveCaseComment, addEvidenceLink, removeEvidenceLink, removeLegacyEvidenceCaseId, } from "../services/cases/case-lifecycle.service.js";
import { evaluateCaseMutationPermission, getCaseAssignmentRoles, } from "../services/cases/case-permission.service.js";
/**
 ...
 */
async function gateCaseMutation(reply, mutation, caseId, member) {
    const accessRole = member.role;
...
function handleCaseError(reply, err) {
    if (err instanceof CaseError) {
        const status = err.code === "case_not_found" ||
...
async function requireCaseAccess(req, reply, caseId) {
    const userId = getAuthUserId(req);
...
export async function caseWorkspaceRoutes(app) {
    // ----------- Cases summary (for /cases list page) -----------
    app.get("/v1/cases/summary", { preHandler: requireAuth }, async (req, reply) => {
    ...
    app.get("/v1/cases/:id/workspace", { preHandler: requireAuth }, async (req, reply) => {
    ...
    app.get("/v1/cases/matter-queue", { preHandler: requireAuth }, async (req, reply) => {
    ...
    app.get("/v1/cases/:id/matter-workspace", { preHandler: requireAuth }, async (req, reply) => {
        const params = WorkspaceParams.parse(req.params);
        const member = await requireCaseAccess(req, reply, params.id);
        if (!member)
            return;
    ...
    app.get("/v1/cases/:id/risk", { preHandler: requireAuth }, async (req, reply) => {
        const params = WorkspaceParams.parse(req.params);
        const member = await requireCaseAccess(req, reply, params.id);
        if (!member)
            return;
    ...
    // insufficient.
    // ---------------------------------------------------------------------
    app.get("/v1/cases/:id/discussion-threads", { preHandler: requireAuth }, async (req, reply) => {
        const params = WorkspaceParams.parse(req.params);
        const member = await requireCaseAccess(req, reply, params.id);
        if (!member)
            return;
    ...
    // ---------------------------------------------------------------------
    app.get("/v1/cases/:id/evidence-requests", { preHandler: requireAuth }, async (req, reply) => {
        const params = WorkspaceParams.parse(req.params);
        const member = await requireCaseAccess(req, reply, params.id);
        if (!member)
            return;
        ...
        const rows = requests.map((r) => {
            ...
            const satisfied = r.deliverables.filter((d) => isSatisfied(d.status)).length;
            const completionPercent = total === 0 ? 0 : Math.round((satisfied / total) * 100);
    ...
    app.get("/v1/cases/:id/assignment-candidates", { preHandler: requireAuth }, async (req, reply) => {
        const params = WorkspaceParams.parse(req.params);
        ...
        if (!caseRow.teamId) {
            return reply.code(200).send({
                ...
                items: [],
                reason: "Personal cases do not support assignments. Switch to a team workspace to coordinate work.",
            });
        }
        ...
        return reply.code(200).send({
            ...
            items: page.map((m) => ({
                userId: m.user.id,
                displayName: [m.user.firstName, m.user.lastName].filter(Boolean).join(" ").trim() ||
                    m.user.displayName ||
                    m.user.email ||
                    null,
    ...
    app.get("/v1/cases/:id/linkable-evidence", { preHandler: requireAuth }, async (req, reply) => {
        const params = WorkspaceParams.parse(req.params);
        ...
        const linkedRoleByEv = new Map(existingLinks.map((l) => [l.evidenceId, String(l.role)]));
        return reply.code(200).send({
            items: page.map((e) => ({
                id: e.id,
    ...
    app.post("/v1/cases/:id/status", { preHandler: requireAuth }, async (req, reply) => {
        const params = WorkspaceParams.parse(req.params);
    ...
    app.post("/v1/cases/:id/assignments", { preHandler: requireAuth }, async (req, reply) => {
        const params = WorkspaceParams.parse(req.params);
    ...
    app.delete("/v1/cases/:id/assignments/:assignmentId", { preHandler: requireAuth }, async (req, reply) => {
        const params = z
    ...
    app.post("/v1/cases/:id/comments", { preHandler: requireAuth }, async (req, reply) => {
        const params = WorkspaceParams.parse(req.params);
    ...
    app.post("/v1/cases/:id/comments/:commentId/resolve", { preHandler: requireAuth }, async (req, reply) => {
        const params = z
        ...
        if (!access)
            return;
        const member = await gateCaseMutation(reply, "COMMENT_RESOLVE", params.id, access);
        if (!member)
            return;
        try {
    ...
    app.post("/v1/cases/:id/evidence-links", { preHandler: requireAuth }, async (req, reply) => {
        const params = WorkspaceParams.parse(req.params);
        const body = EvidenceLinkBody.parse(req.body ?? {});
        const access = await requireCaseAccess(req, reply, params.id);
        if (!access)
            return;
        const member = await gateCaseMutation(reply, "EVIDENCE_LINK", params.id, access);
        if (!member)
            return;
        try {
    ...
    app.delete("/v1/cases/:id/legacy-evidence-link/:evidenceId", { preHandler: requireAuth }, async (req, reply) => {
        const params = z
        ...
        if (!access)
            return;
        const member = await gateCaseMutation(reply, "EVIDENCE_UNLINK_LEGACY", params.id, access);
        if (!member)
            return;
        try {
    ...
    app.delete("/v1/cases/:id/evidence-links/:linkId", { preHandler: requireAuth }, async (req, reply) => {
        const params = z
    ...
    app.get("/v1/reports/artifacts", { preHandler: requireAuth }, async (req, reply) => {
