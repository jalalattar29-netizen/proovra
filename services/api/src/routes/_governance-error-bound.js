 ...
 */
export function isPrismaTableOrColumnMissing(err) {
    if (err == null || typeof err !== "object")
        return false;
    const code = err.code;
 ...
 * note text.
 */
export function extractPrismaDiagnostic(err) {
    const e = err;
    const meta = e?.meta ?? {};
 ...
 */
export async function runGovernanceHandler(reply, handler) {
    try {
        ...
        if (isPrismaSchemaDriftError(err)) {
            ...
            reply.code(503).send({
                error: {
                    ...
                    message: "Governance subsystem is currently degraded. Operators have been notified.",
                },
            });
            return undefined;
        }
