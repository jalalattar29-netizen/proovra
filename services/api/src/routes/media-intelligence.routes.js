...
// Routes
// =============================================================================
export async function mediaIntelligenceRoutes(app) {
    // ---------------------------------------------------------------------------
    // GET /v1/evidence/:evidenceId/media-intelligence?teamId=…
    // ---------------------------------------------------------------------------
    app.get("/v1/evidence/:evidenceId/media-intelligence", { preHandler: requireAuth }, async (req, reply) => {
        ...
        const rows = (await prisma.$queryRawUnsafe(`SELECT "id", "signal_type", "material_id", "severity", "confidence",
    ...
    // ---------------------------------------------------------------------------
    app.post("/v1/evidence/:evidenceId/media-intelligence/run", { preHandler: requireAuth }, async (req, reply) => {
        const { evidenceId } = ParamsEvidenceId.parse(req.params);
    ...
    // ---------------------------------------------------------------------------
    app.post("/v1/media-intelligence/signals/:signalId/action", { preHandler: requireAuth }, async (req, reply) => {
        ...
        const owner = (await prisma.$queryRawUnsafe(`SELECT "team_id" FROM "media_intelligence_signals"
           WHERE "id" = $1 LIMIT 1`, signalId));
        ...
        try {
            await prisma.$executeRawUnsafe(`UPDATE "media_intelligence_signals"
             SET "status" = $1,
                 "acknowledged_by_user_id" = $2,
                 "acknowledged_at_utc" = NOW(),
                 "updated_at_utc" = NOW()
             WHERE "id" = $3 AND "team_id" = $4`, body.action, actor.actorUserId, signalId, body.teamId);
    ...
    // ---------------------------------------------------------------------------
    app.get("/v1/investigation/overview", { preHandler: requireAuth }, async (req, reply) => {
        const { teamId } = TeamQuery.parse(req.query ?? {});
        const actor = await authorizeOrFail(req, reply, {
            teamId,
            permission: "evidence.read",
        ...
        let recentSignals = [];
        try {
            const rows = (await prisma.$queryRawUnsafe(`SELECT "id", "signal_type", "evidence_id", "severity",
    ...
    //   * Team-anchored: every SQL fragment binds team_id = $1.
    // ---------------------------------------------------------------------------
    app.get("/v1/investigation/reviewers", { preHandler: requireAuth }, async (req, reply) => {
        const { teamId } = TeamQuery.parse(req.query ?? {});
        const actor = await authorizeOrFail(req, reply, {
            teamId,
            permission: "evidence.read",
        ...
        try {
            const rows = (await prisma.$queryRawUnsafe(`SELECT "id", "workflow_id", "evidence_id", "reason",
        ...
        let recentGrants = [];
        try {
            const rows = (await prisma.$queryRawUnsafe(`SELECT "id", "scope_kind", "state",
        ...
        const indexingTotals = {
            ocrAvailable: typeof rawIndexingTotals?.ocrAvailable === "number"
                ? rawIndexingTotals.ocrAvailable
                : 0,
            ocrIndexed: typeof rawIndexingTotals?.ocrIndexed === "number"
                ? rawIndexingTotals.ocrIndexed
                : 0,
            transcriptAvailable: typeof rawIndexingTotals?.transcriptAvailable === "number"
                ? rawIndexingTotals.transcriptAvailable
                : 0,
            transcriptIndexed: typeof rawIndexingTotals?.transcriptIndexed === "number"
    ...
    // helper strips them).
    // ---------------------------------------------------------------------------
    app.get("/v1/evidence/:evidenceId/derived-assets", { preHandler: requireAuth }, async (req, reply) => {
        const { evidenceId } = ParamsEvidenceId.parse(req.params);
        const { teamId } = TeamQuery.parse(req.query ?? {});
        const actor = await authorizeOrFail(req, reply, {
            teamId,
        ...
        const { listDerivedAssetsForEvidence } = await import("../services/media-intelligence/derived-assets.service.js");
    ...
    //     the bucket / key.
    // ---------------------------------------------------------------------------
    app.get("/v1/evidence/:evidenceId/derived-assets/:assetId/bytes", { preHandler: requireAuth }, async (req, reply) => {
        ...
        const rows = (await prisma.$queryRawUnsafe(`SELECT "status", "content_type", "size_bytes",
                "derived_sha256"
           FROM "evidence_part_derived_assets"
          WHERE "id" = $1
            AND "team_id" = $2
            AND "evidence_id" = $3
          LIMIT 1`, assetId, teamId, evidenceId));
        const row = rows[0];
        if (!row) {
            return reply.code(404).send({ error: { code: "not_found" } });
        }
        ...
        const { _getDerivedAssetStorageReference } = await import("../services/media-intelligence/derived-assets.service.js");
        const storageRef = await _getDerivedAssetStorageReference(teamId, assetId);
        if (!storageRef) {
            return reply
                .code(503)
        ...
        reply.header("content-type", row.content_type ?? storageRef.contentType ?? "application/octet-stream");
    ...
    // ---------------------------------------------------------------------------
    app.post("/v1/evidence/:evidenceId/derived-assets/run", { preHandler: requireAuth }, async (req, reply) => {
        const { evidenceId } = ParamsEvidenceId.parse(req.params);
        const RunBody = z
            .object({
            teamId: z.string().uuid(),
        ...
        const { enqueueDerivedAssetGeneration } = await import("../queue/derived-assets-queue.js");
