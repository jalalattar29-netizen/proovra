 ...
 */
import { z } from "zod";
import { WORKFLOW_ACTOR_ROLES, WORKFLOW_INSTANCE_STATUSES, WorkflowIntakeModeSchema, } from "@proovra/shared";
import { getAuthUserId } from "../auth.js";
import { prisma } from "../db.js";
import { requireAuth } from "../middleware/auth.js";
import { evaluateMemberAccess } from "../services/identity/access-policy.service.js";
import { WorkflowEngineError, assignReviewer, createWorkflowInstance, getInstanceExportPolicySummary, getInstanceTimeline, getInstanceWithSteps, listInstances, mapEvidenceToStep, projectInstance, projectStep, projectStepForReviewer, transitionInstance, waiveStep, } from "../services/workflows/evidence-workflow-engine.service.js";
 ...
 */
async function requireWorkflowActor(req, reply, teamId) {
...
function handleEngineError(reply, err) {
    if (err instanceof WorkflowEngineError) {
        const status = err.code === "WORKFLOW_INSTANCE_NOT_FOUND" ||
...
export async function workflowInstancesRoutes(app) {
    // -------------------------------------------------------------------------
    // List
    // -------------------------------------------------------------------------
    app.get("/v1/workflows/instances", { preHandler: requireAuth }, async (req, reply) => {
        const q = z
            .object({
            teamId: z.string().uuid(),
            status: z
                .enum(WORKFLOW_INSTANCE_STATUSES)
                .optional(),
    ...
    // Get
    // -------------------------------------------------------------------------
    app.get("/v1/workflows/instances/:id", { preHandler: requireAuth }, async (req, reply) => {
        ...
        try {
            ...
            const isAssignedReviewer = instance.assignedReviewerUserId === actor.userId;
            ...
            if (handleEngineError(reply, err))
                return;
            throw err;
        }
    });
    ...
    const CreateBody = z.object({
        ...
        actorRole: z.enum(WORKFLOW_ACTOR_ROLES),
    ...
    app.post("/v1/workflows/instances", { preHandler: requireAuth }, async (req, reply) => {
    ...
    app.post("/v1/workflows/instances/:id/steps/:stepKey/map-evidence", { preHandler: requireAuth }, async (req, reply) => {
        const params = z
            .object({
            id: z.string().uuid(),
            stepKey: z.string().min(1).max(80),
    ...
    app.post("/v1/workflows/instances/:id/steps/:stepKey/waive", { preHandler: requireAuth }, async (req, reply) => {
        const params = z
            .object({
            id: z.string().uuid(),
            stepKey: z.string().min(1).max(80),
    ...
    // -------------------------------------------------------------------------
    app.post("/v1/workflows/instances/:id/submit", { preHandler: requireAuth }, async (req, reply) => {
        const { id } = ParamsId.parse(req.params);
    ...
    app.post("/v1/workflows/instances/:id/approve", { preHandler: requireAuth }, async (req, reply) => {
        const { id } = ParamsId.parse(req.params);
        const body = TeamIdQuery.parse(req.body ?? {});
        const actor = await requireWorkflowActor(req, reply, body.teamId);
        if (!actor)
    ...
    app.post("/v1/workflows/instances/:id/request-changes", { preHandler: requireAuth }, async (req, reply) => {
        const { id } = ParamsId.parse(req.params);
        const body = TeamIdQuery.parse(req.body ?? {});
        const actor = await requireWorkflowActor(req, reply, body.teamId);
        if (!actor)
    ...
    app.post("/v1/workflows/instances/:id/cancel", { preHandler: requireAuth }, async (req, reply) => {
        const { id } = ParamsId.parse(req.params);
        const body = TeamIdQuery.parse(req.body ?? {});
        const actor = await requireWorkflowActor(req, reply, body.teamId);
        if (!actor)
    ...
    app.post("/v1/workflows/instances/:id/assign-reviewer", { preHandler: requireAuth }, async (req, reply) => {
        const { id } = ParamsId.parse(req.params);
    ...
    // -------------------------------------------------------------------------
    app.get("/v1/workflows/instances/:id/timeline", { preHandler: requireAuth }, async (req, reply) => {
        const { id } = ParamsId.parse(req.params);
        const q = TeamIdQuery.parse(req.query ?? {});
        const actor = await requireWorkflowActor(req, reply, q.teamId);
        if (!actor)
    ...
    app.get("/v1/workflows/instances/:id/export-policy", { preHandler: requireAuth }, async (req, reply) => {
        const { id } = ParamsId.parse(req.params);
        const q = TeamIdQuery.parse(req.query ?? {});
        const actor = await requireWorkflowActor(req, reply, q.teamId);
        if (!actor)
    ...
    // -------------------------------------------------------------------------
    app.get("/v1/workflows/templates", { preHandler: requireAuth }, async (req, reply) => {
        const q = z
            .object({
            teamId: z.string().uuid(),
        ...
        if (!actor)
            return;
        const { listEffectiveWorkflowTemplates } = await import("../services/workflow-template.service.js");
        ...
        const filtered = q.sector
            ? rows.filter((r) => (r.template.workspaceCategory ?? "").toLowerCase() ===
