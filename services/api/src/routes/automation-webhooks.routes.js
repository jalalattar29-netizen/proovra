...
import { createDestinationSecret, validateDestinationUrlStatic, WEBHOOK_MAX_DESTINATIONS_PER_TEAM, } from "../services/automation/automation-webhook.service.js";
// ---------------------------------------------------------------------------
// Schemas
// ---------------------------------------------------------------------------
...
async function requireTeamCapability(req, reply, teamId, capability) {
    ...
    const team = await prisma.team.findUnique({
        where: { id: teamId },
        select: { id: true, isPersonal: true, billingPlan: true },
    });
    if (!team) {
...
// Routes
// ---------------------------------------------------------------------------
export async function automationWebhooksRoutes(app) {
    // -----------------------------------------------------------------
    // GET /v1/automation/webhooks?teamId=
    // -----------------------------------------------------------------
    app.get("/v1/automation/webhooks", { preHandler: requireAuth }, async (req, reply) => {
    ...
    // -----------------------------------------------------------------
    app.post("/v1/automation/webhooks", { preHandler: requireAuth }, async (req, reply) => {
        ...
        reply.code(201).send({
            ...
            revealedSecretNotice: "Store this secret now — it will never be shown again. Rotate via the /rotate-secret endpoint if lost.",
        });
    });
    // -----------------------------------------------------------------
    // PATCH /v1/automation/webhooks/:id
    // -----------------------------------------------------------------
    app.patch("/v1/automation/webhooks/:id", { preHandler: requireAuth }, async (req, reply) => {
    ...
    app.post("/v1/automation/webhooks/:id/enable", { preHandler: requireAuth }, (req, reply) => transitionEnabled(req, reply, true));
    app.post("/v1/automation/webhooks/:id/disable", { preHandler: requireAuth }, (req, reply) => transitionEnabled(req, reply, false));
    // -----------------------------------------------------------------
    ...
    // secret.
    // -----------------------------------------------------------------
    app.post("/v1/automation/webhooks/:id/rotate-secret", { preHandler: requireAuth }, async (req, reply) => {
        const params = ParamsId.safeParse(req.params);
        if (!params.success)
            return reply.code(400).send({ message: "Invalid id" });
        const existing = await prisma.automationWebhookDestination.findUnique({
        ...
        reply.send({
            ...
            revealedSecretNotice: "Store this secret now — it will never be shown again.",
        });
    });
    // -----------------------------------------------------------------
    // GET /v1/automation/webhook-deliveries?teamId=
    // -----------------------------------------------------------------
    app.get("/v1/automation/webhook-deliveries", { preHandler: requireAuth }, async (req, reply) => {
