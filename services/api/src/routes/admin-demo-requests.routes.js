...
import { processDueDemoFollowUps, sendDemoFollowUpById, } from "../services/demo-follow-up.service.js";
...
function isValidInternalKey(req) {
    const configured = envString("INTERNAL_API_KEY");
    if (!configured)
        return false;
    const provided = readHeaderValue(req.headers, "x-internal-key")?.trim();
    return !!provided && provided === configured;
}
async function requirePlatformAdminOrInternalKey(req, reply) {
...
export async function adminDemoRequestsRoutes(app) {
    app.get("/v1/admin/demo-requests", { preHandler: requirePlatformAdmin }, async (req, reply) => {
        const parsed = listQuerySchema.safeParse(req.query ?? {});
        if (!parsed.success) {
            return reply.code(400).send(createErrorResponse(ErrorCode.VALIDATION_ERROR, req.id, { reason: parsed.error.message }, "Invalid demo requests query"));
        }
        const { limit, status, priority, leadQuality, leadTrack, recommendedAction, routingTarget, followUpStatus, isSpam, search, } = parsed.data;
        const where = {
            ...(status ? { status } : {}),
            ...(priority ? { priority } : {}),
    ...
    app.get("/v1/admin/demo-requests/:id", { preHandler: requirePlatformAdmin }, async (req, reply) => {
        const params = req.params;
        const id = typeof params.id === "string" ? params.id : "";
        if (!id) {
            return reply.code(400).send(createErrorResponse(ErrorCode.VALIDATION_ERROR, req.id, { field: "id", reason: "Missing demo request id" }, "Missing demo request id"));
        }
        ...
        if (!item) {
            return reply.code(404).send(createErrorResponse(ErrorCode.NOT_FOUND, req.id, undefined, "Demo request not found"));
        }
        void appendPlatformAuditLog({
            userId: req.user?.sub ?? null,
    ...
    app.patch("/v1/admin/demo-requests/:id", { preHandler: requirePlatformAdmin }, async (req, reply) => {
        const params = req.params;
        const id = typeof params.id === "string" ? params.id : "";
        if (!id) {
            return reply.code(400).send(createErrorResponse(ErrorCode.VALIDATION_ERROR, req.id, { field: "id", reason: "Missing demo request id" }, "Missing demo request id"));
        ...
        if (!parsed.success) {
            return reply.code(400).send(createErrorResponse(ErrorCode.VALIDATION_ERROR, req.id, { reason: parsed.error.message }, "Invalid demo request update payload"));
        }
        ...
        const nextNotes = parsed.data.notes === undefined ? existing.notes : parsed.data.notes;
        const now = new Date();
        const data = {
            status: nextStatus,
        ...
        const shouldStampReviewed = parsed.data.status !== undefined &&
        ...
        if (parsed.data.followUpStatus !== undefined) {
            ...
            else if (parsed.data.followUpStatus === "PAUSED" ||
    ...
    app.post("/v1/admin/demo-requests/:id/route", { preHandler: requirePlatformAdmin }, async (req, reply) => {
        const params = req.params;
        const id = typeof params.id === "string" ? params.id : "";
        if (!id) {
            return reply.code(400).send(createErrorResponse(ErrorCode.VALIDATION_ERROR, req.id, { field: "id", reason: "Missing demo request id" }, "Missing demo request id"));
        ...
        if (!parsed.success) {
            return reply.code(400).send(createErrorResponse(ErrorCode.VALIDATION_ERROR, req.id, { reason: parsed.error.message }, "Invalid routing payload"));
        }
        const existing = await prisma.demoRequest.findUnique({
            where: { id },
            select: {
    ...
    app.post("/v1/admin/demo-requests/:id/follow-up/send", { preHandler: requirePlatformAdmin }, async (req, reply) => {
        const params = req.params;
        const id = typeof params.id === "string" ? params.id : "";
        if (!id) {
            return reply.code(400).send(createErrorResponse(ErrorCode.VALIDATION_ERROR, req.id, { field: "id", reason: "Missing demo request id" }, "Missing demo request id"));
        ...
        if (!parsed.success) {
            return reply.code(400).send(createErrorResponse(ErrorCode.VALIDATION_ERROR, req.id, { reason: parsed.error.message }, "Invalid follow-up payload"));
        }
        try {
            ...
            return reply.code(400).send(createErrorResponse(ErrorCode.INVALID_REQUEST, req.id, { reason }, "Unable to send follow-up"));
        }
    });
    app.post("/v1/admin/demo-requests/follow-up/run", { preHandler: requirePlatformAdminOrInternalKey }, async (req, reply) => {
        const parsed = runFollowUpBodySchema.safeParse(req.body ?? {});
        if (!parsed.success) {
            return reply.code(400).send(createErrorResponse(ErrorCode.VALIDATION_ERROR, req.id, { reason: parsed.error.message }, "Invalid follow-up run payload"));
        }
