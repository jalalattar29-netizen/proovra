 ...
 */
import { z } from "zod";
import { AI_ADVISORY_DISCLAIMER, EVIDENCE_ENTITY_KINDS, EVIDENCE_SIMILARITY_KINDS, EXTRACTED_TEXT_KINDS, INTELLIGENCE_JOB_KINDS, INTELLIGENCE_JOB_STATUSES, } from "@proovra/shared";
import { getAuthUserId } from "../auth.js";
import { prisma } from "../db.js";
import { requireAuth } from "../middleware/auth.js";
import { requirePermission } from "../services/governance.service.js";
import { configuredOcrProviderName, configuredTranscriptProviderName, enqueueIntelligenceJob, listExtractedTexts, listIntelligenceJobs, projectExtractedTextSummary, projectIntelligenceJob, } from "../services/intelligence/extraction.service.js";
import { listCrossEvidenceFindings, listEvidenceEntities, projectEvidenceEntity, } from "../services/intelligence/entity-extraction.service.js";
import { listSimilaritiesForEvidence, projectSimilarity, reconcileSimilaritiesForEvidence, } from "../services/intelligence/similarity.service.js";
...
import { isSemanticSearchEnabled, searchSemantic, } from "../services/intelligence/semantic.service.js";
...
import { AI_ASSISTANCE_KINDS, requestAiAssistance, } from "../services/intelligence/ai-assistance.service.js";
...
export async function intelligenceRoutes(app) {
    ...
    // ---------------------------------------------------------------------------
    app.get("/v1/intelligence/search", { preHandler: requireAuth }, async (req, reply) => {
        const query = z
            .object({
    ...
    // ---------------------------------------------------------------------------
    app.get("/v1/intelligence/jobs", { preHandler: requireAuth }, async (req, reply) => {
        const query = z
            .object({
            teamId: z.string().uuid(),
    ...
    // GET /v1/intelligence/evidence/:id
    // ---------------------------------------------------------------------------
    app.get("/v1/intelligence/evidence/:id", { preHandler: requireAuth }, async (req, reply) => {
    ...
    // POST /v1/intelligence/evidence/:id/enqueue
    // ---------------------------------------------------------------------------
    app.post("/v1/intelligence/evidence/:id/enqueue", { preHandler: requireAuth }, async (req, reply) => {
        const { id } = ParamsEvidenceId.parse(req.params);
        const body = z
            .object({
            teamId: z.string().uuid(),
    ...
    // POST /v1/intelligence/evidence/:id/reconcile-similarity
    // ---------------------------------------------------------------------------
    app.post("/v1/intelligence/evidence/:id/reconcile-similarity", { preHandler: requireAuth }, async (req, reply) => {
        const { id } = ParamsEvidenceId.parse(req.params);
        const body = z
            .object({ teamId: z.string().uuid() })
            .parse(req.body ?? {});
    ...
    // POST /v1/intelligence/evidence/:id/ai-assist
    // ---------------------------------------------------------------------------
    app.post("/v1/intelligence/evidence/:id/ai-assist", { preHandler: requireAuth }, async (req, reply) => {
        const { id } = ParamsEvidenceId.parse(req.params);
        const body = z
            .object({
            teamId: z.string().uuid(),
    ...
    // ---------------------------------------------------------------------------
    app.get("/v1/investigation/cross-evidence", { preHandler: requireAuth }, async (req, reply) => {
        const query = z
            .object({
            teamId: z.string().uuid(),
