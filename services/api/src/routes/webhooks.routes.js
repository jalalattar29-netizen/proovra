...
import { addCredits, ensureEntitlement, recordPayment, setPersonalPlan, activateTeamPlan, cancelTeamPlan, upsertSubscription, upsertWorkspaceStorageAddon, } from "../services/billing.service.js";
import { parseStripeEvent, verifyStripeSignature, } from "../services/stripe.service.js";
import { verifyPayPalWebhook, getPayPalSubscription, } from "../services/paypal.service.js";
...
function parsePlan(value) {
    if (value === prismaPkg.PlanType.FREE ||
...
function parseStorageAddonKey(value) {
    if (value === prismaPkg.StorageAddonKey.PERSONAL_10_GB ||
...
function parsePayPalSubscriptionStatus(status) {
...
function tryParseAddonContextFromCustomId(raw) {
    ...
    try {
        ...
        return {
            ...
            teamId: typeof parsed.teamId === "string"
                ? parsed.teamId
                : parsed.teamId === null
                    ? null
                    : undefined,
            storageAddonKey: parseStorageAddonKey(parsed.storageAddonKey),
            billingCycle: parsed.billingCycle === prismaPkg.StorageAddonBillingCycle.ONE_TIME
    ...
    return {
        ...
        billingCycle: out.billingCycle === prismaPkg.StorageAddonBillingCycle.ONE_TIME
            ? prismaPkg.StorageAddonBillingCycle.ONE_TIME
            : null,
    };
}
...
async function assertWebhookStorageAddonAllowed(params) {
    if (params.teamId) {
        ...
        if (team.ownerUserId !== params.userId) {
            const err = new Error("Storage add-on team ownership mismatch");
            err.statusCode = 403;
            throw err;
        }
        const isTeamAddon = params.addonKey === prismaPkg.StorageAddonKey.TEAM_100_GB ||
            params.addonKey === prismaPkg.StorageAddonKey.TEAM_500_GB ||
            params.addonKey === prismaPkg.StorageAddonKey.TEAM_1_TB;
        if (!isTeamAddon) {
            const err = new Error("Personal storage add-on cannot be attached to a team workspace");
            err.statusCode = 400;
            throw err;
        }
        const effectiveTeamActive = team.billingPlan === prismaPkg.PlanType.TEAM &&
        ...
        if (!effectiveTeamActive) {
            const err = new Error("Team storage add-ons require an active TEAM workspace");
            err.statusCode = 409;
            throw err;
        }
        return;
    ...
    const isPersonalAddon = params.addonKey === prismaPkg.StorageAddonKey.PERSONAL_10_GB ||
        params.addonKey === prismaPkg.StorageAddonKey.PERSONAL_50_GB ||
        params.addonKey === prismaPkg.StorageAddonKey.PERSONAL_200_GB;
    if (!isPersonalAddon) {
        const err = new Error("Team storage add-on cannot be attached to a personal workspace");
        err.statusCode = 400;
        throw err;
    }
    if (entitlement.plan === prismaPkg.PlanType.FREE) {
        const err = new Error("FREE plan cannot receive storage add-ons");
        err.statusCode = 409;
        throw err;
    }
    if (entitlement.plan === prismaPkg.PlanType.PAYG &&
        ...
        params.addonKey !== prismaPkg.StorageAddonKey.PERSONAL_50_GB) {
        const err = new Error("PAYG supports only PERSONAL_10_GB and PERSONAL_50_GB");
        err.statusCode = 400;
        throw err;
    }
}
async function syncPlanForSubscription(params) {
    ...
    if (params.plan === prismaPkg.PlanType.TEAM) {
        ...
        if (params.status === prismaPkg.SubscriptionStatus.PAST_DUE) {
            ...
            const alreadyActivated = existingTeam?.billingPlan === prismaPkg.PlanType.TEAM &&
...
export async function webhooksRoutes(app) {
    ...
    app.post("/stripe", async (req, reply) => {
        ...
        if (!sigCheck.ok) {
            return reply.code(400).send({
                message: sigCheck.reason === "missing_signature"
                    ? "Missing signature"
        ...
        if (event.type === "checkout.session.completed") {
            ...
            const storageAddonKey = parseStorageAddonKey(session.metadata?.storageAddonKey);
            ...
            if (userId && storageAddonKey) {
                try {
                    ...
                    const storageAddonBillingCycle = parseStorageAddonBillingCycle(session.metadata?.billingCycle);
                    const effectiveCurrency = (session.currency ??
                        session.metadata?.currency ??
                        "usd").toUpperCase();
                    const effectiveAmountCents = session.amount_total ??
        ...
        if (event.type === "customer.subscription.created" ||
            ...
            const storageAddonKey = parseStorageAddonKey(subscription.metadata?.storageAddonKey);
            ...
            const parsedCycle = parseStorageAddonBillingCycle(subscription.metadata?.billingCycle);
        ...
        if (event.type === "invoice.paid" ||
            ...
            const storageAddonKey = parseStorageAddonKey(invoice.metadata?.storageAddonKey);
            const parsedCycle = parseStorageAddonBillingCycle(invoice.metadata?.billingCycle);
            if (storageAddonKey && parsedCycle !== null) {
                req.log.warn({
                    provider: "STRIPE",
    ...
    app.post("/paypal", async (req, reply) => {
        ...
        const sigCheck = await auditWebhookSignatureVerification({
            ...
            verify: async () => {
                ...
                if (verification.verification_status !== "SUCCESS") {
                    throw new Error(`PayPal signature invalid: status=${verification.verification_status}`);
                }
            },
        });
        ...
        if (!paypalEventId) {
            ...
            // provider secret.
            req.log.warn({ provider: "PAYPAL", eventType: event.event_type }, "paypal.webhook_missing_event_id");
            return reply.code(200).send({ received: true });
        }
        ...
        try {
            ...
            if (code === "P2002") {
                ...
                const hashMatches = existing?.payloadHash != null &&
        ...
        try {
            if (event.event_type === "PAYMENT.CAPTURE.COMPLETED") {
                ...
                const addonContext = tryParseAddonContextFromCustomId(unit?.custom_id ?? event.resource.custom_id);
            ...
            if (event.event_type === "BILLING.SUBSCRIPTION.CREATED" ||
                ...
                const needsAddonRefresh = !addonContext.userId || !addonContext.storageAddonKey;
                if (needsPlanRefresh || needsAddonRefresh) {
                    try {
                        const liveSubscription = await getPayPalSubscription(subscriptionId);
                        parsed = parsePayPalCustomId(typeof liveSubscription.custom_id === "string"
