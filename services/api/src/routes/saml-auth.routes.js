...
import { signJwt, signMfaPendingToken, MFA_PENDING_TTL_SECONDS, } from "../services/jwt.js";
import { resolveLoginMfaEnforcement } from "../services/security/login-mfa-enforcement.service.js";
import { createMfaPendingChallenge, } from "../services/security/mfa.service.js";
import { validateSamlResponse, SamlAssertionError, } from "../services/security/saml-assertion.service.js";
import { handleSamlAssertion, SamlMappingError, } from "../services/security/saml-user-mapping.service.js";
import { buildSamlAuthnRequest, } from "../services/security/saml-authn-request.service.js";
import { parseSamlMetadata, SamlMetadataError, } from "../services/security/saml-metadata.service.js";
import { parseCertExpiry, getCertExpiryStatus, emitCertExpiryWarningIfNeeded, } from "../services/security/saml-cert.service.js";
import { consumeCallbackAttempt, markCallbackFailed, noteSsoFailure, noteSsoSuccess, persistSamlCallbackAttempt, } from "../services/access-control/sso-hardening.service.js";
...
const SAML_DEFAULT_NAME_ID_FORMAT = "urn:oasis:names:tc:SAML:1.1:nameid-format:emailAddress";
// ---------------------------------------------------------------------------
// Helpers (mirror sso-auth.routes.ts helpers exactly for session consistency)
// ---------------------------------------------------------------------------
function ipPreview(req) {
    const ip = req.ip ?? "";
    if (!ip)
        return null;
    if (ip.includes(".")) {
...
function sanitiseRedirectAfter(value) {
 ...
 */
function bounceToSamlError(reply, reason, redirectAfter) {
...
function setSessionCookie(req, reply, token) {
    ...
    const productionDomain = process.env["SSO_COOKIE_DOMAIN"] ||
        (host.includes("proovra.com") || origin.includes("proovra.com")
            ? ".proovra.com"
            : undefined);
    const secure = process.env["SSO_COOKIE_SECURE"] === "true" || !!productionDomain;
    const opts = {
        httpOnly: true,
        secure,
        sameSite: "lax",
...
function buildSpEntityId(connectionId) {
    const base = process.env["API_BASE_URL"] ?? "https://api.proovra.com";
...
// Routes
// ---------------------------------------------------------------------------
export async function samlAuthRoutes(app) {
    ...
    // -------------------------------------------------------------------------
    app.get("/v1/auth/saml/:connectionId/login", async (req, reply) => {
        if (!samlEnabled) {
            return reply
                .code(503)
        ...
        try {
            ...
            await noteSsoFailure({ connectionId: conn.id, reason: "saml_initiate_failed" }, prisma);
    ...
    // -------------------------------------------------------------------------
    app.post("/v1/auth/saml/acs", async (req, reply) => {
        if (!samlEnabled) {
            return reply
                .code(503)
                .send({ error: { code: "saml_disabled" } });
        ...
        const consume = await consumeCallbackAttempt({ stateRaw: relayStateRaw }, prisma);
        if (!consume.ok) {
        ...
        if (!conn || conn.status !== "ACTIVE") {
            await markCallbackFailed({ attemptId: attempt.id, reason: "connection_unavailable" }, prisma);
            return bounceToSamlError(reply, "saml_connection_unavailable", attempt.redirectAfter);
        }
        if (!conn.samlCertificate) {
            await markCallbackFailed({ attemptId: attempt.id, reason: "cert_not_configured" }, prisma);
            return bounceToSamlError(reply, "saml_not_configured", attempt.redirectAfter);
        }
        const spEntityId = conn.samlEntityId || buildSpEntityId(conn.id);
        try {
            // ---------------------------------------------------------------
            ...
            // ---------------------------------------------------------------
            if (attempt.samlAuthnRequestId &&
                assertion.inResponseTo !== attempt.samlAuthnRequestId) {
                throw new SamlAssertionError("SAML_IN_RESPONSE_TO_MISMATCH", "InResponseTo does not match stored AuthnRequest ID.");
            }
            // ---------------------------------------------------------------
            // Step 6: Map assertion to internal user
            // ---------------------------------------------------------------
            const attributeMapping = conn.samlAttributeMapping;
            ...
            if (decision.outcome === "MFA_REQUIRED" ||
                ...
                const url = new URL("/auth/mfa-challenge", process.env["WEB_BASE_URL"] || "https://www.proovra.com");
            ...
            await detectAndScoreSession({ teamId: conn.teamId, sessionId: session.id }, prisma).catch(() => null);
            ...
            const reason = err instanceof SamlAssertionError
                ? err.code.toLowerCase()
                : err instanceof SamlMappingError
                    ? err.code.toLowerCase()
                    : "saml_acs_failed";
            await markCallbackFailed({ attemptId: attempt.id, reason: reason.slice(0, 64) }, prisma);
            await noteSsoFailure({ connectionId: conn.id, reason }, prisma);
    ...
    // -------------------------------------------------------------------------
    app.get("/v1/auth/saml/metadata/:connectionId", async (req, reply) => {
        if (!samlEnabled) {
            return reply
                .code(503)
                .send({ error: { code: "saml_disabled" } });
    ...
    // -------------------------------------------------------------------------
    app.post("/v1/auth/saml/:connectionId/ingest-metadata", {
    ...
    // -------------------------------------------------------------------------
    app.post("/v1/auth/saml/:connectionId/test-connection", { config: { requireAuth: true } }, async (req, reply) => {
        if (!samlEnabled) {
        ...
        const certExpiryStatus = getCertExpiryStatus(conn.samlCertNotAfter ? new Date(conn.samlCertNotAfter) : null);
    ...
    // -------------------------------------------------------------------------
    app.put("/v1/auth/saml/:connectionId/certificate-next", { config: { requireAuth: true } }, async (req, reply) => {
        if (!samlEnabled) {
            return reply.code(503).send({ error: { code: "saml_disabled" } });
        }
        const { connectionId } = z
    ...
    // Requires: authenticated OWNER or ADMIN.
    // -------------------------------------------------------------------------
    app.delete("/v1/auth/saml/:connectionId/certificate-next", { config: { requireAuth: true } }, async (req, reply) => {
        if (!samlEnabled) {
            return reply.code(503).send({ error: { code: "saml_disabled" } });
        }
        const { connectionId } = z
