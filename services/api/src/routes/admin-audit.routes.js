...
import { appendPlatformAuditLog, listAdminAuditLogs, verifyAdminAuditChain, } from "../services/platform-audit-log.service.js";
...
function auditAdminAuditAccess(req, params) {
...
export async function adminAuditRoutes(app) {
    app.post("/v1/admin/audit-log", {
        ...
        if (!parsed.success) {
            return reply.code(400).send(createErrorResponse(ErrorCode.VALIDATION_ERROR, req.id, {
        ...
        if (!rate.allowed) {
            return reply.code(429).send(createErrorResponse(ErrorCode.RATE_LIMIT_EXCEEDED, req.id, undefined, "Too many audit log requests"));
        }
        ...
        try {
            ...
            if (err instanceof Error && err.message === "METADATA_TOO_LARGE") {
                return reply.code(400).send(createErrorResponse(ErrorCode.INVALID_REQUEST, req.id, { reason: "metadata exceeds maximum size" }, "Metadata too large"));
            }
            if (err instanceof Error && err.message === "METADATA_DEPTH_EXCEEDED") {
                return reply.code(400).send(createErrorResponse(ErrorCode.INVALID_REQUEST, req.id, { reason: "metadata nesting exceeds maximum depth" }, "Metadata too deeply nested"));
            }
            throw err;
        }
        return reply.code(201).send({ ok: true });
    });
    app.get("/v1/admin/audit-log", { preHandler: requirePlatformAdmin }, async (req, reply) => {
        const rate = await enforceRateLimit({
        ...
        const cursorId = typeof query.cursor === "string" && query.cursor.length > 0
            ? query.cursor
            : null;
    ...
    app.get("/v1/admin/audit-log/export", { preHandler: requirePlatformAdmin }, async (req, reply) => {
        const query = req.query;
        const { items } = await listAdminAuditLogs({
            limit: 100,
            action: query.action ?? null,
        ...
        return reply
            ...
            .header("content-disposition", `attachment; filename="admin-audit-export.csv"`)
            .send(lines.join("\r\n"));
    });
    app.get("/v1/admin/audit-log/verify", { preHandler: requirePlatformAdmin }, async (req, reply) => {
        const rate = await enforceRateLimit({
        ...
        if (!rate.allowed) {
            return reply.code(429).send(createErrorResponse(ErrorCode.RATE_LIMIT_EXCEEDED, req.id, undefined, "Too many verify requests"));
        }
        const query = req.query;
        const limitRaw = query.limit ? Number.parseInt(query.limit, 10) : NaN;
        const tailLimit = Number.isFinite(limitRaw) && limitRaw > 0 ? limitRaw : null;
        const result = await verifyAdminAuditChain(tailLimit != null ? { tailLimit } : undefined);
        auditAdminAuditAccess(req, {
