 ...
 */
import { z } from "zod";
import { WORKFLOW_INTAKE_LINK_STATUSES, WORKFLOW_INTAKE_MODES, } from "@proovra/shared";
import { getAuthUserId } from "../auth.js";
import { prisma } from "../db.js";
import { requireAuth } from "../middleware/auth.js";
import { hasRole } from "../services/rbac.js";
import { createWorkflowIntakeLink, getWorkflowIntakeLink, listWorkflowIntakeLinks, projectWorkflowIntakeLink, revokeWorkflowIntakeLink, sendIntakeLinkViaSms, WorkflowIntakeLinkError, } from "../services/workflow-intake-link.service.js";
...
async function requireAdmin(req, reply, teamId) {
...
function bigintFromOptional(value) {
...
// Route registration
// -----------------------------------------------------------------------------
export async function workflowIntakeLinksRoutes(app) {
    // -- Create -----------------------------------------------------------
    app.post("/v1/workflow/intake-links", { preHandler: requireAuth }, async (req, reply) => {
        ...
        {
            const { canCreateIntakeLink, loadWorkspaceGovernancePolicy } = await import("../services/governance.service.js");
        ...
        try {
            ...
            return reply.code(201).send({
                ...
                warning: "The raw token is shown exactly once. Capture it now — it is not retrievable later.",
            });
        }
        catch (err) {
            ...
            const message = err instanceof Error ? err.message : "Failed to create intake link";
            return reply.code(400).send({ message });
        }
    });
    // -- List -------------------------------------------------------------
    app.get("/v1/workflow/intake-links", { preHandler: requireAuth }, async (req, reply) => {
        if (workflowIntakeFeatureDisabledReason()) {
            return sendFeatureDisabled(reply);
        }
    ...
    // -- Get --------------------------------------------------------------
    app.get("/v1/workflow/intake-links/:id", { preHandler: requireAuth }, async (req, reply) => {
        if (workflowIntakeFeatureDisabledReason()) {
            return sendFeatureDisabled(reply);
        }
    ...
    // -- Revoke -----------------------------------------------------------
    app.post("/v1/workflow/intake-links/:id/revoke", { preHandler: requireAuth }, async (req, reply) => {
        if (workflowIntakeFeatureDisabledReason()) {
            return sendFeatureDisabled(reply);
        }
        const { id } = ParamsId.parse(req.params);
    ...
    app.post("/v1/workflow/intake-links/:id/send", { preHandler: requireAuth }, async (req, reply) => {
        if (workflowIntakeFeatureDisabledReason()) {
            return sendFeatureDisabled(reply);
        }
        const { id } = ParamsId.parse(req.params);
        ...
        if (!result.ok) {
            const status = result.reason === "link_not_found"
                ? 404
