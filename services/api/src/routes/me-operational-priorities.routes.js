...
export async function meOperationalPrioritiesRoutes(app) {
    app.get("/v1/me/operational-priorities", { preHandler: requireAuthAndLegal }, async (req, reply) => {
        ...
        const adminPendingInviteCount = adminPendingByOrg.reduce((sum, r) => sum + r._count.id, 0);
        ...
        const onboarding = {
            ...
            hasOwnedOrganization: membershipRows.some((m) => m.role === "ORG_OWNER"),
        };
        // -----------------------------------------------------------------
        ...
        if (pendingOrgInvites.length > 0) {
            ...
            items.push({
                ...
                meaning: pendingOrgInvites.length === 1
        ...
        if (adminPendingInviteCount > 0) {
            items.push({
                ...
                meaning: "As an admin you can resend or revoke these from each organization's detail page.",
                href: "/organizations",
                tone: "warning",
            });
        }
        if (!onboarding.hasAnyOrganization) {
            items.push({
                ...
                meaning: "Organizations group workspaces for governance, members, and audit. Create one, or accept an invite token, to enable team operations.",
                href: "/organizations",
                tone: "info",
            });
        }
        if (onboarding.hasAnyOrganization &&
            ...
            items.push({
                ...
                meaning: "Email-matched invites and audit-trail attribution require an email identity. Pure-guest accounts are limited to personal workspace operations.",
                href: "/settings",
                tone: "info",
            });
        }
