...
export async function platformContextRoutes(app) {
    app.get("/v1/platform/context", { preHandler: requireAuth }, async (req, reply) => {
        const userId = getAuthUserId(req);
        const jwtRole = typeof req.user?.role === "string" ? req.user.role : null;
     ...
     */
    app.post("/v1/platform/context/switch-workspace", { preHandler: requireAuth }, async (req, reply) => {
        const userId = getAuthUserId(req);
        const jwtRole = typeof req.user?.role === "string" ? req.user.role : null;
