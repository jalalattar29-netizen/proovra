...
import { abortStorageMultipart, abortUploadSession, completeStorageMultipart, completeUploadSession, createUploadSession, initiateStorageMultipart, markPartUploaded, markPartVerified, presignStorageUploadPart, recordPartEtag, resumeUploadSession, } from "../services/uploads/upload-session.service.js";
// =============================================================================
// Bounded validators
// =============================================================================
...
function sendDenial(reply, reason, extras) {
...
// Routes
// =============================================================================
export async function uploadSessionsRoutes(app) {
    // ===========================================================================
    // POST /v1/uploads/sessions — create (or reuse via idempotency key)
    // ===========================================================================
    app.post("/v1/uploads/sessions", { preHandler: requireAuth }, async (req, reply) => {
    ...
    // ===========================================================================
    app.get("/v1/uploads/sessions/:sessionId", { preHandler: requireAuth }, async (req, reply) => {
    ...
    // ===========================================================================
    app.get("/v1/uploads/sessions/:sessionId/status", { preHandler: requireAuth }, async (req, reply) => {
        const { sessionId } = SessionParamsSchema.parse(req.params);
        const { teamId } = TeamQuerySchema.parse(req.query ?? {});
        const actor = await authorizeOrFail(req, reply, {
            teamId,
    ...
    // ===========================================================================
    app.post("/v1/uploads/sessions/:sessionId/parts/:partIndex/uploaded", { preHandler: requireAuth }, async (req, reply) => {
    ...
    // mismatch.
    // ===========================================================================
    app.post("/v1/uploads/sessions/:sessionId/parts/:partIndex/verified", { preHandler: requireAuth }, async (req, reply) => {
        const { sessionId, partIndex } = PartParamsSchema.parse(req.params);
    ...
    // ===========================================================================
    app.post("/v1/uploads/sessions/:sessionId/complete", { preHandler: requireAuth }, async (req, reply) => {
        const { sessionId } = SessionParamsSchema.parse(req.params);
        ...
        if (!result.ok) {
            return sendDenial(reply, result.reason, result.pendingPartIndices
                ? { pendingPartIndices: result.pendingPartIndices }
                : undefined);
        }
    ...
    // ===========================================================================
    app.post("/v1/uploads/sessions/:sessionId/abort", { preHandler: requireAuth }, async (req, reply) => {
        const { sessionId } = SessionParamsSchema.parse(req.params);
    ...
    // ---------------------------------------------------------------------------
    app.post("/v1/uploads/sessions/:sessionId/multipart/initiate", { preHandler: requireAuth }, async (req, reply) => {
        const { sessionId } = SessionParamsSchema.parse(req.params);
    ...
    // ---------------------------------------------------------------------------
    app.post("/v1/uploads/sessions/:sessionId/parts/:partIndex/presign", { preHandler: requireAuth }, async (req, reply) => {
        const { sessionId, partIndex } = PartParamsSchema.parse(req.params);
    ...
    // ---------------------------------------------------------------------------
    app.post("/v1/uploads/sessions/:sessionId/multipart/complete", { preHandler: requireAuth }, async (req, reply) => {
        const { sessionId } = SessionParamsSchema.parse(req.params);
        ...
        if (!result.ok) {
            return sendDenial(reply, result.reason, result.missingEtags ? { missingPartIndices: result.missingEtags } : undefined);
        }
    ...
    // ---------------------------------------------------------------------------
    app.post("/v1/uploads/sessions/:sessionId/multipart/abort", { preHandler: requireAuth }, async (req, reply) => {
        const { sessionId } = SessionParamsSchema.parse(req.params);
        const body = AbortBodySchema.parse(req.body ?? {});
        const actor = await authorizeOrFail(req, reply, {
            teamId: body.teamId,
