...
import { GovernanceSnapshotError, buildGovernanceSnapshot, } from "../services/governance-lifecycle/governance-snapshot.service.js";
...
async function requireGovernanceReadActor(req, reply, teamId) {
...
export async function governanceSnapshotRoutes(app) {
    // ---------------------------------------------------------------------------
    // GET /v1/evidence/:id/governance-snapshot
    // ---------------------------------------------------------------------------
    app.get("/v1/evidence/:id/governance-snapshot", { preHandler: requireAuth }, async (req, reply) => {
        ...
        try {
            ...
            if (err instanceof GovernanceSnapshotError &&
    ...
    // ---------------------------------------------------------------------------
    app.get("/v1/evidence/:id/operational-timeline", { preHandler: requireAuth }, async (req, reply) => {
        const params = ParamsId.parse(req.params);
