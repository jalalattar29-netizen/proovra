...
import { isPreferenceEnabled, } from "../services/notifications/notification-preferences.service.js";
async function requireAuthAndLegal(req, reply) {
    await requireAuth(req, reply);
    await requireLegalAcceptance(req, reply);
}
export async function meInboxRoutes(app) {
    app.get("/v1/me/inbox", { preHandler: requireAuthAndLegal }, async (req, reply) => {
        ...
        const orgNameById = new Map(orgMemberships.map((m) => [m.organization.id, m.organization.name]));
        // -----------------------------------------------------------------
        ...
        const teamIdSet = new Set(teamMemberships.map((tm) => tm.team.id));
        ...
        for (const t of ownedPersonalTeams)
            teamIdSet.add(t.id);
        const teamIds = Array.from(teamIdSet);
        const teamNameById = new Map([
            ...teamMemberships.map((tm) => [tm.team.id, tm.team.name]),
            ...ownedPersonalTeams.map((t) => [t.id, t.name]),
        ]);
        // -----------------------------------------------------------------
        ...
        // -----------------------------------------------------------------
        const adminPendingByOrg = adminOrgIds.length === 0
            ? []
        ...
        const teamRoles = teamIds.length === 0
            ? []
        ...
        // memory.
        const conflictCandidates = adjudicatorTeamIds.length === 0
            ? []
        ...
        for (const r of conflictCandidates) {
            ...
            if (r.stage === "FIRST")
                entry.first = { decision: r.decision };
            else if (r.stage === "SECOND")
                entry.second = { decision: r.decision, decidedAt: r.decidedAt };
            else if (r.stage === "ADJUDICATION")
                entry.adjudication = true;
        ...
        const myFirstDecisions = teamIds.length === 0
            ? []
            : await prisma.workflowReviewDecision.findMany({
                where: {
        ...
        const secondsForMyFirsts = myFirstWorkflowIds.length === 0
            ? []
            : await prisma.workflowReviewDecision.findMany({
                where: {
        ...
        const haveSecond = new Set(secondsForMyFirsts.map((s) => s.workflowId));
        const pendingSecondForMe = myFirstDecisions.filter((d) => !haveSecond.has(d.workflowId));
        // -----------------------------------------------------------------
        // Source 3: unacknowledged governance notifications for teams
        // the caller is a member of.
        // -----------------------------------------------------------------
        const governanceRows = teamIds.length === 0
            ? []
        ...
        // exact thread.
        // -----------------------------------------------------------------
        const unreadMentions = teamIds.length === 0
            ? []
        ...
        // -----------------------------------------------------------------
        const assignedThreads = teamIds.length === 0
            ? []
        ...
        for (const inv of pendingOrgInvites) {
            items.push({
                ...
                body: `You have been invited to join ${inv.organization.name} as ${inv.role}. Accept before ${new Date(inv.expiresAt).toLocaleDateString()}.`,
                href: "/organizations",
        ...
        async function isAllowed(teamIdLookup, type) {
        ...
        for (const m of unreadMentions) {
            ...
            if (!(await isAllowed(m.teamId, "MENTION")))
                continue;
            items.push({
        ...
        for (const t of assignedThreads) {
            if (!(await isAllowed(t.teamId, "ASSIGNED_THREAD")))
                continue;
            items.push({
        ...
        const summary = {
            ...
            byCategory: {
                ...
                review_decision: items.filter((i) => i.category === "review_decision").length,
                discussion_mention: items.filter((i) => i.category === "discussion_mention").length,
                discussion_assigned: items.filter((i) => i.category === "discussion_assigned").length,
            },
        };
    ...
    // ---------------------------------------------------------------------
    app.get("/v1/me/inbox/summary", { preHandler: requireAuthAndLegal }, async (req, reply) => {
        const userId = getAuthUserId(req);
        const now = new Date();
        const teamMemberships = await prisma.teamMember.findMany({
            where: { userId },
...
// ---------------------------------------------------------------------------
function governanceKindTitle(kind) {
    switch (kind) {
...
function governanceKindBody(kind, occurrenceCount) {
    const occBlurb = occurrenceCount > 1 ? ` Seen ${occurrenceCount} times.` : "";
    switch (kind) {
        case "DESTRUCTION_PENDING":
...
function governanceKindHref(kind) {
    switch (kind) {
        case "DESTRUCTION_PENDING":
        case "DESTRUCTION_APPROVED":
        case "DESTRUCTION_EXECUTED":
