 ...
 */
import { z } from "zod";
import { REDACTION_APPROVAL_VERDICTS, REDACTION_ARTIFACT_KINDS, REDACTION_DECISION_STATES, REDACTION_DETECTION_PROVIDERS, REDACTION_METHODS, REDACTION_REGION_KINDS, } from "@proovra/shared";
...
import { emitRedactionActivity, listRedactionActivity, } from "../services/redaction/redaction-activity.service.js";
import { assertRedactionCapability } from "../services/redaction/redaction-rbac.service.js";
import { createRedactionVersion, listRedactionProjectsForTeam, openRedactionProject, transitionRedactionVersion, } from "../services/redaction/redaction-project.service.js";
import { addRedactionRegion, removeRedactionRegion, } from "../services/redaction/redaction-region.service.js";
import { listRedactionDetectionsForVersion, runRedactionDetection, } from "../services/redaction/redaction-detection.service.js";
...
import { getDerivativeForVersion, markDerivativeFailed, markDerivativeReady, requestRedactionDerivative, } from "../services/redaction/redaction-derivative.service.js";
import { projectRedactionProject, summariseRedactionQueueForTeam, } from "../services/redaction/redaction-projection.service.js";
import { REDACTION_BULK_DECISION_MAX_ROWS, bulkRecordDetectionDecisions, } from "../services/redaction/redaction-decision-bulk.service.js";
...
import { getRedactionDetectionPolicy, setRedactionDetectionPolicy, } from "../services/redaction/redaction-policy.service.js";
import { archivePolicy, assignPolicyVersion, createPolicy, createPolicyVersion, listAssignmentsForScope, listAuditForPolicy, listPolicies, listPolicyVersions, resolveEffectivePolicy, revokePolicyAssignment, transitionPolicyVersion, } from "../services/redaction/redaction-policy-store.service.js";
import { listVideoFrames, registerVideoFrameBatch, } from "../services/redaction/video/video-frame.service.js";
import { decideVideoTrack, listVideoTracksForEvidence, mergeTracks, propagateRangeDecision, splitTrack, } from "../services/redaction/video/video-track.service.js";
...
async function gate(reply, ctx, capability) {
...
// Route registration
// ---------------------------------------------------------------------------
export async function redactionRoutes(app) {
    // -------------------------------------------------------------------------
    ...
    // -------------------------------------------------------------------------
    app.get("/v1/redaction/workspace/summary", { preHandler: requireAuth }, async (req, reply) => {
        const ctx = await resolveWorkspace(req, reply);
        if (!ctx)
            return reply;
        if (!(await gate(reply, ctx, "redaction.view")))
            return reply;
    ...
    // -------------------------------------------------------------------------
    app.get("/v1/redaction/public/verify/:evidenceId", async (req, reply) => {
        const { evidenceId } = z
        ...
        const badge = {
            ...
            videoProvenance: totalFramesCount > 0
                ? {
    ...
    // Projects
    // -------------------------------------------------------------------------
    app.get("/v1/redaction/projects", { preHandler: requireAuth }, async (req, reply) => {
        const ctx = await resolveWorkspace(req, reply);
        if (!ctx)
            return reply;
        if (!(await gate(reply, ctx, "redaction.view")))
    ...
    app.post("/v1/redaction/projects", { preHandler: requireAuth }, async (req, reply) => {
        const ctx = await resolveWorkspace(req, reply);
        if (!ctx)
            return reply;
        ...
        try {
            ...
            if (!feOk.ok)
                return reply.code(403).send({ denial: "ENTITLEMENT_REQUIRED", entitlement: "FEATURE_REDACTION" });
        }
        catch { /* engine failure must not break route */ }
        if (!(await gate(reply, ctx, "redaction.region.author")))
            return reply;
    ...
    app.get("/v1/redaction/projects/:id", { preHandler: requireAuth }, async (req, reply) => {
        const ctx = await resolveWorkspace(req, reply);
        if (!ctx)
            return reply;
        if (!(await gate(reply, ctx, "redaction.view")))
    ...
    app.get("/v1/redaction/projects/:id/activity", { preHandler: requireAuth }, async (req, reply) => {
        const ctx = await resolveWorkspace(req, reply);
        if (!ctx)
            return reply;
        if (!(await gate(reply, ctx, "redaction.view")))
    ...
    // Versions
    // -------------------------------------------------------------------------
    app.post("/v1/redaction/projects/:id/versions", { preHandler: requireAuth }, async (req, reply) => {
        const ctx = await resolveWorkspace(req, reply);
        if (!ctx)
            return reply;
        if (!(await gate(reply, ctx, "redaction.region.author")))
    ...
    app.post("/v1/redaction/versions/:id/submit", { preHandler: requireAuth }, async (req, reply) => {
        const ctx = await resolveWorkspace(req, reply);
        if (!ctx)
            return reply;
        if (!(await gate(reply, ctx, "redaction.version.submit")))
            return reply;
        const { id } = z.object({ id: z.string().uuid() }).parse(req.params);
    ...
    app.post("/v1/redaction/versions/:id/approve", { preHandler: requireAuth }, async (req, reply) => {
        const ctx = await resolveWorkspace(req, reply);
        if (!ctx)
            return reply;
        if (!(await gate(reply, ctx, "redaction.version.approve")))
            return reply;
        const { id } = z.object({ id: z.string().uuid() }).parse(req.params);
    ...
    app.post("/v1/redaction/versions/:id/publish", { preHandler: requireAuth }, async (req, reply) => {
        const ctx = await resolveWorkspace(req, reply);
        if (!ctx)
            return reply;
        if (!(await gate(reply, ctx, "redaction.version.publish")))
            return reply;
        const { id } = z.object({ id: z.string().uuid() }).parse(req.params);
        ...
        if (!derivative || derivative.state !== "READY") {
            return reply
                .code(409)
                .send({ denial: "DERIVATIVE_NOT_READY" });
        }
        const res = await transitionRedactionVersion({
            teamId: ctx.teamId,
            versionId: id,
    ...
    // Regions
    // -------------------------------------------------------------------------
    app.post("/v1/redaction/versions/:id/regions", { preHandler: requireAuth }, async (req, reply) => {
        const ctx = await resolveWorkspace(req, reply);
        if (!ctx)
            return reply;
        if (!(await gate(reply, ctx, "redaction.region.author")))
    ...
    app.delete("/v1/redaction/regions/:id", { preHandler: requireAuth }, async (req, reply) => {
        const ctx = await resolveWorkspace(req, reply);
        if (!ctx)
            return reply;
        if (!(await gate(reply, ctx, "redaction.region.author")))
    ...
    // Detections
    // -------------------------------------------------------------------------
    app.post("/v1/redaction/versions/:id/detect", { preHandler: requireAuth }, async (req, reply) => {
        const ctx = await resolveWorkspace(req, reply);
        if (!ctx)
            return reply;
        if (!(await gate(reply, ctx, "redaction.detection.run")))
            return reply;
        const { id } = z.object({ id: z.string().uuid() }).parse(req.params);
    ...
    app.get("/v1/redaction/versions/:id/detections", { preHandler: requireAuth }, async (req, reply) => {
        const ctx = await resolveWorkspace(req, reply);
        if (!ctx)
            return reply;
        if (!(await gate(reply, ctx, "redaction.view")))
    ...
    // -------------------------------------------------------------------------
    app.post("/v1/redaction/versions/:id/decisions/bulk", { preHandler: requireAuth }, async (req, reply) => {
        const ctx = await resolveWorkspace(req, reply);
        if (!ctx)
            return reply;
        if (!(await gate(reply, ctx, "redaction.detection.review")))
            return reply;
    ...
    app.get("/v1/redaction/providers/health", { preHandler: requireAuth }, async (req, reply) => {
        const ctx = await resolveWorkspace(req, reply);
        if (!ctx)
            return reply;
        if (!(await gate(reply, ctx, "redaction.view")))
    ...
    app.get("/v1/redaction/evidence/:evidenceId/detection-manifest", { preHandler: requireAuth }, async (req, reply) => {
        const ctx = await resolveWorkspace(req, reply);
        if (!ctx)
            return reply;
        if (!(await gate(reply, ctx, "redaction.view")))
    ...
    app.get("/v1/redaction/policy", { preHandler: requireAuth }, async (req, reply) => {
        const ctx = await resolveWorkspace(req, reply);
        if (!ctx)
            return reply;
        if (!(await gate(reply, ctx, "redaction.view")))
    ...
    // ---------------------------------------------------------------------------
    app.get("/v1/redaction/policies", { preHandler: requireAuth }, async (req, reply) => {
        const ctx = await resolveWorkspace(req, reply);
        if (!ctx)
            return reply;
        if (!(await gate(reply, ctx, "redaction.view")))
    ...
    app.post("/v1/redaction/policies", { preHandler: requireAuth }, async (req, reply) => {
        const ctx = await resolveWorkspace(req, reply);
        if (!ctx)
            return reply;
        if (!(await gate(reply, ctx, "redaction.administer")))
            return reply;
    ...
    app.delete("/v1/redaction/policies/:id", { preHandler: requireAuth }, async (req, reply) => {
        const ctx = await resolveWorkspace(req, reply);
        if (!ctx)
            return reply;
        if (!(await gate(reply, ctx, "redaction.administer")))
    ...
    app.get("/v1/redaction/policies/:id/versions", { preHandler: requireAuth }, async (req, reply) => {
        const ctx = await resolveWorkspace(req, reply);
        if (!ctx)
            return reply;
        if (!(await gate(reply, ctx, "redaction.view")))
    ...
    app.post("/v1/redaction/policies/:id/versions", { preHandler: requireAuth }, async (req, reply) => {
        const ctx = await resolveWorkspace(req, reply);
        if (!ctx)
            return reply;
        if (!(await gate(reply, ctx, "redaction.administer")))
    ...
    app.post("/v1/redaction/policy-versions/:id/transition", { preHandler: requireAuth }, async (req, reply) => {
        const ctx = await resolveWorkspace(req, reply);
        if (!ctx)
            return reply;
        if (!(await gate(reply, ctx, "redaction.administer")))
    ...
    app.post("/v1/redaction/policies/:id/assignments", { preHandler: requireAuth }, async (req, reply) => {
        const ctx = await resolveWorkspace(req, reply);
        if (!ctx)
            return reply;
        if (!(await gate(reply, ctx, "redaction.administer")))
    ...
    app.delete("/v1/redaction/policy-assignments/:id", { preHandler: requireAuth }, async (req, reply) => {
        const ctx = await resolveWorkspace(req, reply);
        if (!ctx)
            return reply;
        if (!(await gate(reply, ctx, "redaction.administer")))
    ...
    app.get("/v1/redaction/policies/:id/audit", { preHandler: requireAuth }, async (req, reply) => {
        const ctx = await resolveWorkspace(req, reply);
        if (!ctx)
            return reply;
        if (!(await gate(reply, ctx, "redaction.view")))
    ...
    app.get("/v1/redaction/policy/effective", { preHandler: requireAuth }, async (req, reply) => {
        const ctx = await resolveWorkspace(req, reply);
        if (!ctx)
            return reply;
        if (!(await gate(reply, ctx, "redaction.view")))
    ...
    app.get("/v1/redaction/policy/assignments", { preHandler: requireAuth }, async (req, reply) => {
        const ctx = await resolveWorkspace(req, reply);
        if (!ctx)
            return reply;
        if (!(await gate(reply, ctx, "redaction.view")))
    ...
    app.patch("/v1/redaction/policy", { preHandler: requireAuth }, async (req, reply) => {
        const ctx = await resolveWorkspace(req, reply);
        if (!ctx)
            return reply;
        if (!(await gate(reply, ctx, "redaction.administer")))
    ...
    // ---------------------------------------------------------------------------
    app.get("/v1/redaction/videos/:evidenceId/frames", { preHandler: requireAuth }, async (req, reply) => {
        const ctx = await resolveWorkspace(req, reply);
        if (!ctx)
            return reply;
        if (!(await gate(reply, ctx, "redaction.view")))
    ...
    app.post("/v1/redaction/videos/:evidenceId/frames/batch", { preHandler: requireAuth }, async (req, reply) => {
        const ctx = await resolveWorkspace(req, reply);
        if (!ctx)
            return reply;
        if (!(await gate(reply, ctx, "redaction.administer")))
    ...
    app.get("/v1/redaction/videos/:evidenceId/tracks", { preHandler: requireAuth }, async (req, reply) => {
        const ctx = await resolveWorkspace(req, reply);
        if (!ctx)
            return reply;
        if (!(await gate(reply, ctx, "redaction.view")))
    ...
    app.post("/v1/redaction/videos/:evidenceId/tracks/group", { preHandler: requireAuth }, async (req, reply) => {
        const ctx = await resolveWorkspace(req, reply);
        if (!ctx)
            return reply;
        if (!(await gate(reply, ctx, "redaction.detection.run")))
    ...
    app.post("/v1/redaction/video-tracks/:id/decision", { preHandler: requireAuth }, async (req, reply) => {
        const ctx = await resolveWorkspace(req, reply);
        if (!ctx)
            return reply;
        if (!(await gate(reply, ctx, "redaction.detection.review")))
    ...
    app.post("/v1/redaction/video-tracks/merge", { preHandler: requireAuth }, async (req, reply) => {
        const ctx = await resolveWorkspace(req, reply);
        if (!ctx)
            return reply;
        if (!(await gate(reply, ctx, "redaction.detection.review")))
    ...
    app.post("/v1/redaction/video-tracks/:id/split", { preHandler: requireAuth }, async (req, reply) => {
        const ctx = await resolveWorkspace(req, reply);
        if (!ctx)
            return reply;
        if (!(await gate(reply, ctx, "redaction.detection.review")))
    ...
    app.post("/v1/redaction/videos/:evidenceId/decisions/range", { preHandler: requireAuth }, async (req, reply) => {
        const ctx = await resolveWorkspace(req, reply);
        if (!ctx)
            return reply;
        if (!(await gate(reply, ctx, "redaction.detection.review")))
    ...
    app.get("/v1/redaction/videos/:evidenceId/timeline", { preHandler: requireAuth }, async (req, reply) => {
        const ctx = await resolveWorkspace(req, reply);
        if (!ctx)
            return reply;
        if (!(await gate(reply, ctx, "redaction.view")))
    ...
    app.post("/v1/redaction/detections/:id/decision", { preHandler: requireAuth }, async (req, reply) => {
        const ctx = await resolveWorkspace(req, reply);
        if (!ctx)
            return reply;
        if (!(await gate(reply, ctx, "redaction.detection.review")))
    ...
    // Derivative pipeline
    // -------------------------------------------------------------------------
    app.post("/v1/redaction/versions/:id/derivative", { preHandler: requireAuth }, async (req, reply) => {
        const ctx = await resolveWorkspace(req, reply);
        if (!ctx)
            return reply;
    ...
    app.post("/v1/redaction/derivatives/:id/mark-ready", { preHandler: requireAuth }, async (req, reply) => {
        const ctx = await resolveWorkspace(req, reply);
        if (!ctx)
            return reply;
        if (!(await gate(reply, ctx, "redaction.administer")))
    ...
    app.post("/v1/redaction/derivatives/:id/mark-failed", { preHandler: requireAuth }, async (req, reply) => {
        const ctx = await resolveWorkspace(req, reply);
        if (!ctx)
            return reply;
        if (!(await gate(reply, ctx, "redaction.administer")))
    ...
    app.get("/v1/redaction/derivatives/:id", { preHandler: requireAuth }, async (req, reply) => {
        const ctx = await resolveWorkspace(req, reply);
        if (!ctx)
            return reply;
        if (!(await gate(reply, ctx, "redaction.derivative.download")))
    ...
    app.get("/v1/redaction/versions/:id/derivative", { preHandler: requireAuth }, async (req, reply) => {
        const ctx = await resolveWorkspace(req, reply);
        if (!ctx)
            return reply;
        if (!(await gate(reply, ctx, "redaction.view")))
