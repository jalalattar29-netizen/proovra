...
import { listRecentMfaEvents, readUserMfaPosture, requireUserReenrollment, resetTrustedDevicesForUser, revokeUserFactor, } from "../services/security/mfa-admin-lifecycle.service.js";
import { approveRecoveryRequest, cancelRecoveryRequest, createRecoveryRequest, listPendingRecoveryRequests, listRecoveryRequestApprovals, readRecoveryRequestDetail, rejectRecoveryRequest, resendRecoveryRequestEmail, verifyRecoveryRequestEmail, } from "../services/security/mfa-recovery-request.service.js";
import { getMfaPolicy, updateMfaPolicy, } from "../services/identity-security/mfa-policy.service.js";
import { listDigestPreferences, updateDigestPreference, } from "../services/security/mfa-digest-preference.service.js";
...
import { signMfaDigestSnoozeToken, verifyMfaDigestSnoozeToken, MFA_DIGEST_SNOOZE_TTL_SECONDS, } from "../services/security/mfa-digest-snooze-token.js";
 ...
 * proceeds either way.
 */
function readOptionalSessionUserId(req) {
    try {
...
function mapScopeFailure(reason) {
    switch (reason) {
...
export async function mfaAdminRoutes(app) {
    // ---------------------------------------------------------------------------
    // Read posture
    // ---------------------------------------------------------------------------
    app.get("/v1/identity/mfa-admin/posture/:teamId/:userId", { preHandler: requireAuth }, async (req, reply) => {
    ...
    // ---------------------------------------------------------------------------
    app.post("/v1/identity/mfa-admin/factors/:teamId/:userId/:factorId/revoke", { preHandler: requireAuth }, async (req, reply) => {
        const actorUserId = getAuthUserId(req);
        if (!actorUserId)
            throw new AppError(ErrorCode.UNAUTHORIZED, "Sign in.");
        ...
        if (!result.ok) {
            if (result.reason === "factor_not_found" ||
    ...
    // ---------------------------------------------------------------------------
    app.post("/v1/identity/mfa-admin/factors/:teamId/:userId/require-reenrollment", { preHandler: requireAuth }, async (req, reply) => {
        const actorUserId = getAuthUserId(req);
        if (!actorUserId)
            throw new AppError(ErrorCode.UNAUTHORIZED, "Sign in.");
        const params = TeamUserParams.parse(req.params);
    ...
    // ---------------------------------------------------------------------------
    app.post("/v1/identity/mfa-admin/trusted-devices/:teamId/:userId/reset", { preHandler: requireAuth }, async (req, reply) => {
        const actorUserId = getAuthUserId(req);
        if (!actorUserId)
            throw new AppError(ErrorCode.UNAUTHORIZED, "Sign in.");
        const params = TeamUserParams.parse(req.params);
    ...
    // Recent MFA events
    // ---------------------------------------------------------------------------
    app.get("/v1/identity/mfa-admin/events/:teamId", { preHandler: requireAuth }, async (req, reply) => {
        const actorUserId = getAuthUserId(req);
        if (!actorUserId)
            throw new AppError(ErrorCode.UNAUTHORIZED, "Sign in.");
    ...
    // ---------------------------------------------------------------------------
    app.get("/v1/identity/mfa-admin/recovery-requests/:teamId", { preHandler: requireAuth }, async (req, reply) => {
        const actorUserId = getAuthUserId(req);
        if (!actorUserId)
            throw new AppError(ErrorCode.UNAUTHORIZED, "Sign in.");
        const params = TeamParams.parse(req.params);
    ...
    // ---------------------------------------------------------------------------
    app.post("/v1/identity/mfa-admin/recovery-requests", { preHandler: requireAuth }, async (req, reply) => {
        const actorUserId = getAuthUserId(req);
        if (!actorUserId)
            throw new AppError(ErrorCode.UNAUTHORIZED, "Sign in.");
    ...
    // ---------------------------------------------------------------------------
    app.post("/v1/identity/mfa-admin/recovery-requests/:requestId/approve", { preHandler: requireAuth }, async (req, reply) => {
        const actorUserId = getAuthUserId(req);
        if (!actorUserId)
            throw new AppError(ErrorCode.UNAUTHORIZED, "Sign in.");
        ...
        if (!result.ok) {
            ...
            if (result.reason === "approver_not_admin" ||
    ...
    // ---------------------------------------------------------------------------
    app.post("/v1/identity/mfa-admin/recovery-requests/:requestId/reject", { preHandler: requireAuth }, async (req, reply) => {
        const actorUserId = getAuthUserId(req);
        if (!actorUserId)
            throw new AppError(ErrorCode.UNAUTHORIZED, "Sign in.");
        const params = RequestIdParams.parse(req.params);
    ...
    app.post("/v1/identity/mfa/recovery-requests/:requestId/verify-email", async (req, reply) => {
        const params = RequestIdParams.parse(req.params);
    ...
    // session signals.
    app.post("/v1/identity/mfa/recovery-requests/analytics/page-viewed", async (req, _reply) => {
        const actorUserId = readOptionalSessionUserId(req);
        void writeAnalyticsEvent({
    ...
    app.post("/v1/identity/mfa/recovery-requests/:requestId/resend-email", { preHandler: requireAuth }, async (req, reply) => {
        const actorUserId = getAuthUserId(req);
        if (!actorUserId)
            throw new AppError(ErrorCode.UNAUTHORIZED, "Sign in.");
        const params = RequestIdParams.parse(req.params);
        ...
        if (!result.ok) {
            ...
            if (result.reason === "resend_throttled" ||
    ...
    app.post("/v1/identity/mfa/recovery-requests/:requestId/cancel", { preHandler: requireAuth }, async (req, reply) => {
        const actorUserId = getAuthUserId(req);
        if (!actorUserId)
            throw new AppError(ErrorCode.UNAUTHORIZED, "Sign in.");
        const params = RequestIdParams.parse(req.params);
    ...
    // ---------------------------------------------------------------------------
    app.get("/v1/identity/mfa-admin/recovery-requests/detail/:requestId", { preHandler: requireAuth }, async (req, reply) => {
        const actorUserId = getAuthUserId(req);
        if (!actorUserId)
            throw new AppError(ErrorCode.UNAUTHORIZED, "Sign in.");
        const params = RequestIdParams.parse(req.params);
    ...
    // ---------------------------------------------------------------------------
    app.get("/v1/identity/mfa-admin/recovery-requests/:requestId/approvals", { preHandler: requireAuth }, async (req, reply) => {
        const actorUserId = getAuthUserId(req);
        if (!actorUserId)
            throw new AppError(ErrorCode.UNAUTHORIZED, "Sign in.");
        const params = RequestIdParams.parse(req.params);
    ...
    // ---------------------------------------------------------------------------
    app.get("/v1/identity/mfa-admin/policy/:teamId", { preHandler: requireAuth }, async (req, reply) => {
        const actorUserId = getAuthUserId(req);
        if (!actorUserId)
            throw new AppError(ErrorCode.UNAUTHORIZED, "Sign in.");
        const params = TeamParams.parse(req.params);
        ...
        const policy = await getMfaPolicy(params.teamId);
        return { policy };
    });
    app.patch("/v1/identity/mfa-admin/policy/:teamId", { preHandler: requireAuth }, async (req, reply) => {
        const actorUserId = getAuthUserId(req);
        if (!actorUserId)
            throw new AppError(ErrorCode.UNAUTHORIZED, "Sign in.");
        const params = TeamParams.parse(req.params);
    ...
    // ---------------------------------------------------------------------------
    app.get("/v1/identity/mfa-admin/digest-preferences", { preHandler: requireAuth }, async (req, reply) => {
        const actorUserId = getAuthUserId(req);
        if (!actorUserId)
            throw new AppError(ErrorCode.UNAUTHORIZED, "Sign in.");
    ...
    app.patch("/v1/identity/mfa-admin/digest-preferences", { preHandler: requireAuth }, async (req, reply) => {
        const actorUserId = getAuthUserId(req);
        if (!actorUserId)
            throw new AppError(ErrorCode.UNAUTHORIZED, "Sign in.");
        ...
        const suppressEnding = body.suppressUntil === null
            ? "cleared"
            : body.suppressUntil
    ...
    // ---------------------------------------------------------------------------
    app.get("/v1/identity/mfa-admin/digest-preferences/preview", { preHandler: requireAuth }, async (req, reply) => {
        const actorUserId = getAuthUserId(req);
        if (!actorUserId)
            throw new AppError(ErrorCode.UNAUTHORIZED, "Sign in.");
        const rawQuery = typeof req.query === "object" && req.query !== null
            ? req.query
            : {};
        const includeSuppressed = rawQuery.includeSuppressed === "true" ||
    ...
    // ---------------------------------------------------------------------------
    app.get("/v1/identity/mfa-admin/digest-preferences/snooze-link", async (req, reply) => {
        const rawQuery = typeof req.query === "object" && req.query !== null
            ? req.query
            : {};
        const rawToken = typeof rawQuery.token === "string" ? rawQuery.token : "";
        if (!rawToken) {
            reply.code(400);
        ...
        const suppressUntil = new Date(Date.now() + verified.payload.snoozeSeconds * 1000).toISOString();
        const result = await updateDigestPreference({
        ...
        return {
            ok: true,
            message: "Digest notifications snoozed. Security events and audit logs are unaffected.",
            suppressUntil,
        };
    });
    // ---------------------------------------------------------------------------
    ...
    // ---------------------------------------------------------------------------
    app.post("/v1/identity/mfa-admin/digest-preferences/preview/send-test", { preHandler: requireAuth }, async (req, reply) => {
        const actorUserId = getAuthUserId(req);
        if (!actorUserId)
            throw new AppError(ErrorCode.UNAUTHORIZED, "Sign in.");
        ...
        const adminSpaUrl = `${(process.env.WEB_BASE_URL || "https://www.proovra.com").replace(/\/$/, "")}/security-center/mfa-recovery`;
        ...
        const testSnoozeUrl = (() => {
            ...
            if (!secret)
                return null;
            return buildMfaDigestSnoozeUrl(actorUserId, null, secret, process.env.API_BASE_URL || "https://api.proovra.com");
        })();
        ...
        try {
            ...
            if (email.isConfigured()) {
                await email.sendMfaRecoveryAdminDigestEmail(user.email, `[TEST] preview for ${preview.adminUserId.slice(0, 8)}…`, preview.requestCount, adminSpaUrl, testSnoozeUrl);
            }
        }
        catch (err) {
            safeEmitSecurityEvent({
    ...
    // ---------------------------------------------------------------------------
    app.get("/v1/identity/mfa-admin/recovery-events", { preHandler: requireAuth }, async (req, reply) => {
        const actorUserId = getAuthUserId(req);
        if (!actorUserId)
            throw new AppError(ErrorCode.UNAUTHORIZED, "Sign in.");
        const rawQuery = typeof req.query === "object" && req.query !== null
            ? req.query
            : {};
        const limit = typeof rawQuery.limit === "string"
            ? Math.max(1, Math.min(200, Number.parseInt(rawQuery.limit, 10) || 100))
            : 100;
        const windowDays = typeof rawQuery.windowDays === "string"
...
// ---------------------------------------------------------------------------
const snoozeLinkJtiByExp = new Map();
const SNOOZE_LINK_GC_INTERVAL_MS = 60 * 60 * 1000;
let snoozeLinkLastGcAtMs = 0;
function snoozeLinkJtiSeen(jti, expSeconds) {
    const nowMs = Date.now();
    if (nowMs - snoozeLinkLastGcAtMs > SNOOZE_LINK_GC_INTERVAL_MS) {
        ...
        for (const [seenJti, exp] of snoozeLinkJtiByExp) {
            if (nowMs / 1000 >= exp)
                snoozeLinkJtiByExp.delete(seenJti);
        }
    }
    if (snoozeLinkJtiByExp.has(jti))
        return true;
...
const sendTestHistoryByUser = new Map();
function sendTestRateLimit(userId) {
    const nowMs = Date.now();
    const dayCutoff = nowMs - 24 * 60 * 60 * 1000;
    const prior = (sendTestHistoryByUser.get(userId) ?? []).filter((t) => t > dayCutoff);
    if (prior.length > 0) {
...
export function __resetMfaAdminRouteRateLimitersForTests() {
...
export function buildMfaDigestSnoozeUrl(userId, teamId, secret, baseUrl) {
