...
import { COMMUNICATION_CHANNELS, COMMUNICATION_PURPOSES, COMMUNICATION_STATUSES, isPhoneChannel, isRetryEligibleCommunicationPurpose, normaliseToE164, parseInboundCommand, } from "@proovra/shared";
...
import { buildProviderHealthSnapshot, getMessagingProvider, } from "../services/communications/provider-registry.js";
import { dispatchPendingMessage, projectCommunicationMessage, } from "../services/communications/communication.service.js";
import { VerificationError, checkVerification, projectVerificationAttempt, startVerification, } from "../services/communications/verification.service.js";
import { applyInboundOptIn, applyInboundOptOut, upsertContactPreference, upsertUserPreference, } from "../services/communications/communication-preference.service.js";
 ...
 */
async function requireCommunicationsActor(req, reply, teamId, permission) {
...
function handleVerificationError(reply, err) {
    if (err instanceof VerificationError) {
        ...
        if (err.code === "verification_check_failed" ||
...
export async function communicationsRoutes(app) {
    // -------------------------------------------------------------------------
    // Health
    // -------------------------------------------------------------------------
    app.get("/v1/communications/health", { preHandler: requireAuth }, async (req, reply) => {
    ...
    // -------------------------------------------------------------------------
    app.get("/v1/communications/messages", { preHandler: requireAuth }, async (req, reply) => {
        const q = z
            .object({
    ...
    app.post("/v1/communications/messages/:id/cancel-retry", { preHandler: requireAuth }, async (req, reply) => {
    ...
    app.post("/v1/communications/messages/:id/retry", { preHandler: requireAuth }, async (req, reply) => {
        const { id } = ParamsId.parse(req.params);
        const body = TeamIdQuery.parse(req.body ?? {});
        const actor = await requireCommunicationsActor(req, reply, body.teamId, "identity.org_policy.manage");
        if (!actor)
        ...
        if (row.status !== prismaPkg.CommunicationStatus.FAILED &&
            row.status !== prismaPkg.CommunicationStatus.RETRY_SCHEDULED) {
            reply.code(409).send({ error: { code: "not_retryable" } });
            return;
        }
        if (!isRetryEligibleCommunicationPurpose(row.purpose)) {
    ...
    app.post("/v1/communications/preferences", { preHandler: requireAuth }, async (req, reply) => {
    ...
    app.post("/v1/communications/verify/start", { preHandler: requireAuth }, async (req, reply) => {
        ...
        try {
            ...
            if (handleVerificationError(reply, err))
                return;
            throw err;
        }
    });
    ...
    app.post("/v1/communications/verify/check", { preHandler: requireAuth }, async (req, reply) => {
    ...
    // -------------------------------------------------------------------------
    app.post("/v1/communications/process-retries", async (req, reply) => {
    ...
    // -------------------------------------------------------------------------
    app.post("/v1/communications/webhooks/twilio/status", async (req, reply) => {
        await handleTwilioWebhook(req, reply, "status");
    });
    app.post("/v1/communications/webhooks/twilio/inbound", async (req, reply) => {
...
// -----------------------------------------------------------------------------
async function handleTwilioWebhook(req, reply, kind) {
    ...
    const signatureHeader = typeof req.headers["x-twilio-signature"] === "string"
    ...
    if (parsed.kind === "status") {
        ...
        try {
            await prisma.communicationMessage.update({
                ...
                data: {
                    status: ev.status,
                    deliveredAtUtc: ev.status === "DELIVERED" ? ev.occurredAtUtc : undefined,
                    failedAtUtc: ev.status === "FAILED" || ev.status === "UNDELIVERED"
                        ? ev.occurredAtUtc
                        : undefined,
    ...
    if (command === "STOP") {
        ...
        const { hashRecipientPhone } = await import("../services/communications/communication.service.js");
...
function buildPublicWebhookUrl(req) {
