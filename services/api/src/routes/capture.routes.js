...
import { getIntakeTemplate, listIntakeTemplates, snapshotIntakeTemplate, } from "../services/capture-intake-templates.js";
import { buildDefaultCaptureDraftExpiry, sanitizeCaptureSessionItem, } from "../services/capture-draft-governance.js";
...
function assertJsonSizeLimit(value, maxBytes, label) {
    const json = JSON.stringify(value ?? null);
    if (json.length > maxBytes) {
        const err = new Error(`${label} too large (max ${maxBytes} bytes)`);
        err.statusCode = 413;
        throw err;
    }
}
...
async function loadOwnedDraft(userId, id) {
    const session = await prisma.captureSession.findUnique({ where: { id } });
    if (!session) {
        const err = new Error("Capture session not found");
        err.statusCode = 404;
        throw err;
    }
...
export async function captureRoutes(app) {
    // -- Intake templates --------------------------------------------------
    app.get("/v1/capture/intake-templates", { preHandler: requireAuth }, async (_req, reply) => {
        const templates = listIntakeTemplates().map(snapshotIntakeTemplate);
        return reply.code(200).send({ templates });
    });
    app.get("/v1/capture/intake-templates/:id", { preHandler: requireAuth }, async (req, reply) => {
    ...
    // -- Capture sessions --------------------------------------------------
    app.get("/v1/capture/sessions", { preHandler: requireAuth }, async (req, reply) => {
    ...
    app.post("/v1/capture/sessions", { preHandler: requireAuth }, async (req, reply) => withProovraSpan(PROOVRA_SPAN_NAMES.CAPTURE_SESSION_CREATE, { "proovra.operation": "capture_session_create" }, async () => captureSessionCreateHandler(req, reply)));
    async function captureSessionCreateHandler(req, reply) {
        const ownerUserId = getAuthUserId(req);
        ...
        try {
            ...
            if (body.uploadState) {
                assertJsonSizeLimit(body.uploadState, MAX_UPLOAD_SNAPSHOT_BYTES, "uploadState");
            }
        }
        catch (err) {
        ...
        const session = await prisma.$transaction(async (tx) => {
            const created = await tx.captureSession.create({
                data: {
                    ...
                    itemsSnapshot: body.items === undefined
                        ? prismaPkg.Prisma.JsonNull
                        : // Phase D Blocker 2 — sanitize before persistence so the
                            // database never holds raw relative paths.
                            body.items.map((item, index) => sanitizeCaptureSessionItem(item, index)),
                    uploadStateSnapshot: body.uploadState === undefined
                        ? prismaPkg.Prisma.JsonNull
                        : body.uploadState,
    ...
    app.get("/v1/capture/sessions/:id", { preHandler: requireAuth }, async (req, reply) => withProovraSpan(PROOVRA_SPAN_NAMES.CAPTURE_REVIEW_BEGIN, { "proovra.operation": "capture_review_begin" }, async () => captureSessionReadHandler(req, reply)));
    async function captureSessionReadHandler(req, reply) {
        const ownerUserId = getAuthUserId(req);
    ...
    app.patch("/v1/capture/sessions/:id", { preHandler: requireAuth }, async (req, reply) => {
        ...
        const hasMappingChange = rawBody.intakeItemMappings !== undefined ||
        ...
        if (hasItems) {
            await withProovraSpan(PROOVRA_SPAN_NAMES.CAPTURE_ITEM_ADD, { "proovra.operation": "capture_item_add" }, () => undefined);
            await withProovraSpan(PROOVRA_SPAN_NAMES.CAPTURE_ITEM_REMOVE, { "proovra.operation": "capture_item_remove" }, () => undefined);
        }
        if (hasMappingChange || hasItems) {
            await withProovraSpan(PROOVRA_SPAN_NAMES.CAPTURE_ITEM_MAP, { "proovra.operation": "capture_item_map" }, () => undefined);
        }
        return captureSessionPatchHandler(req, reply);
    });
    async function captureSessionPatchHandler(req, reply) {
        const ownerUserId = getAuthUserId(req);
        const { id } = ParamsId.parse(req.params);
        ...
        try {
            ...
            if (body.templateId !== undefined) {
                if (body.templateId === null) {
                    ...
                    data.templateSnapshot = snapshotIntakeTemplate(template);
            ...
            if (body.items !== undefined) {
                ...
                data.itemsSnapshot = body.items.map((item, index) => sanitizeCaptureSessionItem(item, index));
    ...
    app.delete("/v1/capture/sessions/:id", { preHandler: requireAuth }, async (req, reply) => withProovraSpan(PROOVRA_SPAN_NAMES.CAPTURE_FINISH_SIGN, { "proovra.operation": "capture_finish_sign" }, async () => captureSessionDeleteHandler(req, reply)));
    async function captureSessionDeleteHandler(req, reply) {
        const ownerUserId = getAuthUserId(req);
        const { id } = ParamsId.parse(req.params);
        try {
            const session = await loadOwnedDraft(ownerUserId, id);
