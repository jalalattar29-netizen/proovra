...
import { AUTOMATION_ACTION_TYPES, AUTOMATION_TRIGGER_TYPES, CreateAutomationRuleInput, UpdateAutomationRuleInput, validateActionConfig, validateCondition, } from "../services/automation/automation.service.js";
// ---------------------------------------------------------------------------
// Helpers
// ---------------------------------------------------------------------------
 ...
 */
async function requireTeamCapability(req, reply, teamId, capability) {
...
export async function automationRoutes(app) {
    ...
    // -----------------------------------------------------------------
    app.get("/v1/automation/rules", { preHandler: requireAuth }, async (req, reply) => {
        ...
        const member = await requireTeamCapability(req, reply, q.data.teamId, "AUTOMATION_VIEW");
        if (!member)
            return;
    ...
    // -----------------------------------------------------------------
    app.post("/v1/automation/rules", { preHandler: requireAuth }, async (req, reply) => {
        ...
        const member = await requireTeamCapability(req, reply, body.data.teamId, "AUTOMATION_MANAGE");
        if (!member)
            return;
        ...
        // by Zod above.
        const actionType = body.data.actionType;
        const actionResult = validateActionConfig(actionType, body.data.actionConfigJson);
        if (!actionResult.ok) {
        ...
        const created = await prisma.automationRule.create({
            data: {
                ...
                conditionJson: conditionResult.condition ?? {},
                actionType,
                actionConfigJson: actionResult.config,
    ...
    // -----------------------------------------------------------------
    app.patch("/v1/automation/rules/:id", { preHandler: requireAuth }, async (req, reply) => {
        ...
        const member = await requireTeamCapability(req, reply, existing.teamId, "AUTOMATION_MANAGE");
        if (!member)
            return;
        const data = {
            updatedByUserId: member.userId,
        ...
        if (body.data.conditionJson !== undefined) {
            ...
            data.conditionJson = r.condition ?? {};
        }
        if (body.data.actionConfigJson !== undefined) {
            const r = validateActionConfig(existing.actionType, body.data.actionConfigJson);
            if (!r.ok) {
                return reply
                    .code(400)
                    .send({ message: "Invalid action config", errors: r.errors });
            }
            data.actionConfigJson = r.config;
        }
    ...
    // -----------------------------------------------------------------
    const transitionEnabled = async (req, reply, enabled) => {
        const params = ParamsId.safeParse(req.params);
        if (!params.success) {
    ...
    app.post("/v1/automation/rules/:id/enable", { preHandler: requireAuth }, (req, reply) => transitionEnabled(req, reply, true));
    app.post("/v1/automation/rules/:id/disable", { preHandler: requireAuth }, (req, reply) => transitionEnabled(req, reply, false));
    // -----------------------------------------------------------------
    // GET /v1/automation/runs?teamId=
    // -----------------------------------------------------------------
    app.get("/v1/automation/runs", { preHandler: requireAuth }, async (req, reply) => {
    ...
    // -----------------------------------------------------------------
    app.get("/v1/automation/runs/:id", { preHandler: requireAuth }, async (req, reply) => {
        const params = ParamsId.safeParse(req.params);
        if (!params.success) {
            return reply.code(400).send({ message: "Invalid id" });
        }
        ...
        const member = await requireTeamCapability(req, reply, row.teamId, "AUTOMATION_VIEW");
        if (!member)
            return;
