 ...
 */
import { z } from "zod";
import { EVIDENCE_REQUEST_PRIORITIES, EVIDENCE_REQUEST_STATUSES, EvidenceRequestInputSchema, } from "@proovra/shared";
import { getAuthUserId } from "../auth.js";
import { prisma } from "../db.js";
import { requireAuth } from "../middleware/auth.js";
import { createEvidenceRequest, editDraftEvidenceRequest, EvidenceRequestError, evidenceRequestsFeatureDisabledReason, getEvidenceRequest, listEvidenceRequestEvents, listEvidenceRequests, projectRequestForAuthenticatedView, reviewEvidenceRequestResponse, sendEvidenceRequest, transitionEvidenceRequest, waiveDeliverable, } from "../services/evidence-request.service.js";
// -----------------------------------------------------------------------------
// Helpers
// -----------------------------------------------------------------------------
...
// Routes
// -----------------------------------------------------------------------------
export async function evidenceRequestsRoutes(app) {
    // CREATE
    app.post("/v1/evidence-requests", { preHandler: requireAuth }, async (req, reply) => {
        if (sendFeatureDisabled(reply))
            return;
        ...
        try {
            ...
            if (err instanceof EvidenceRequestError)
                return errorToReply(err, reply);
            throw err;
        }
    });
    // LIST
    app.get("/v1/evidence-requests", { preHandler: requireAuth }, async (req, reply) => {
        if (sendFeatureDisabled(reply))
            return;
        const query = z
            .object({
    ...
    // GET ONE
    app.get("/v1/evidence-requests/:id", { preHandler: requireAuth }, async (req, reply) => {
        if (sendFeatureDisabled(reply))
            return;
    ...
    // SEND
    app.post("/v1/evidence-requests/:id/send", { preHandler: requireAuth }, async (req, reply) => {
        if (sendFeatureDisabled(reply))
            return;
        const { id } = ParamsId.parse(req.params);
    ...
    // CANCEL
    app.post("/v1/evidence-requests/:id/cancel", { preHandler: requireAuth }, async (req, reply) => {
        if (sendFeatureDisabled(reply))
            return;
        const { id } = ParamsId.parse(req.params);
        const existing = await getEvidenceRequest(id);
    ...
    // CLOSE
    app.post("/v1/evidence-requests/:id/close", { preHandler: requireAuth }, async (req, reply) => {
        if (sendFeatureDisabled(reply))
            return;
        const { id } = ParamsId.parse(req.params);
        const existing = await getEvidenceRequest(id);
    ...
    // NEEDS MORE INFO
    app.post("/v1/evidence-requests/:id/needs-more-info", { preHandler: requireAuth }, async (req, reply) => {
        if (sendFeatureDisabled(reply))
            return;
        const { id } = ParamsId.parse(req.params);
        const body = z
    ...
    // WAIVE DELIVERABLE
    app.post("/v1/evidence-requests/:id/deliverables/:deliverableId/waive", { preHandler: requireAuth }, async (req, reply) => {
        if (sendFeatureDisabled(reply))
            return;
        const params = z
            .object({
    ...
    // REVIEW RESPONSE
    app.post("/v1/evidence-requests/:id/responses/:responseId/review", { preHandler: requireAuth }, async (req, reply) => {
        if (sendFeatureDisabled(reply))
            return;
        const params = z
            .object({
    ...
    // PATCH (DRAFT only)
    app.patch("/v1/evidence-requests/:id", { preHandler: requireAuth }, async (req, reply) => {
        if (sendFeatureDisabled(reply))
            return;
        const { id } = ParamsId.parse(req.params);
        const body = z
    ...
    app.get("/v1/evidence-requests/:id/events", { preHandler: requireAuth }, async (req, reply) => {
        if (sendFeatureDisabled(reply))
            return;
        const { id } = ParamsId.parse(req.params);
        const existing = await getEvidenceRequest(id);
    ...
    app.post("/v1/evidence-requests/:id/assign", { preHandler: requireAuth }, async (req, reply) => {
        if (sendFeatureDisabled(reply))
            return;
        const { id } = ParamsId.parse(req.params);
        const body = z
