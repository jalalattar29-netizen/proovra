...
export async function organizationsGovernanceRoutes(app) {
    /**
     ...
     */
    app.post("/v1/orgs/:id/policies/retention", { preHandler: [requireAuth, requireLegalAcceptance] }, async (req, reply) => {
        ...
        await emitOrgAuditEvent(prisma, {
            ...
            targetId: orgId,
            metadata: {
                previousRetentionDays: existing && typeof existing.value === "object" && existing.value
                    ? existing.value.retentionDays ?? null
                    : null,
     ...
     * may read.
     */
    app.get("/v1/orgs/:id/policies/retention", { preHandler: [requireAuth] }, async (req, reply) => {
        const userId = getAuthUserId(req);
        const orgId = UuidParam.parse(req.params.id);
     ...
     * customer data.
     */
    app.get("/v1/orgs/:id/billing/rollup", { preHandler: [requireAuth] }, async (req, reply) => {
        const userId = getAuthUserId(req);
        const orgId = UuidParam.parse(req.params.id);
