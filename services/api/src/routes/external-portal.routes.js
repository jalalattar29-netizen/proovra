 ...
 */
import { z } from "zod";
import { BULK_INVITATION_MAX_ROWS, EXTERNAL_DECISION_VERDICTS, EXTERNAL_REVIEWER_ROLES, WATERMARK_POLICIES, } from "@proovra/shared";
...
import { emitPortalSessionRevoked, endPortalSession, establishPortalSession, } from "../services/external-review/portal-session.service.js";
import { acceptInvitation, issueInvitation, listInvitationsForTeam, revokeInvitation, } from "../services/external-review/portal-invitation.service.js";
import { projectPortalDashboard } from "../services/external-review/portal-projection.service.js";
import { emitPortalActivity, listPortalActivity, } from "../services/external-review/portal-activity.service.js";
import { listExternalDecisionsForWorkflow, submitExternalDecision, } from "../services/external-review/portal-decisions.service.js";
import { listCommentsForWorkflow, postCommentInWorkflow, } from "../services/external-review/portal-comments.service.js";
import { listDeliveriesForGrant, sendInvitationEmail, } from "../services/external-review/portal-invitation-email.service.js";
...
// Helpers
// ---------------------------------------------------------------------------
async function resolveInternalTeam(req, reply) {
 ...
 */
async function resolvePortalSession(req, reply) {
    ...
    const sess = await establishPortalSession({
        rawToken: raw,
        existingSessionId: typeof sessionHeader === "string" ? sessionHeader : null,
        ip: req.ip,
    ...
    const caps = externalPortalCapabilitiesForRole(s.role);
    return {
        grantId: s.grantId,
        teamId: s.teamId,
...
// Routes
// ---------------------------------------------------------------------------
export async function externalPortalRoutes(app) {
    // =========================================================================
    // INTERNAL — Invitation management
    // =========================================================================
    app.get("/v1/external-review/invitations", { preHandler: requireAuth }, async (req, reply) => {
    ...
    app.post("/v1/external-review/invitations", { preHandler: requireAuth }, async (req, reply) => {
        const ctx = await resolveInternalTeam(req, reply);
        if (!ctx)
            return reply;
        ...
        try {
            ...
            if (!feOk.ok)
                return reply.code(403).send({ denial: "ENTITLEMENT_REQUIRED", entitlement: "FEATURE_EXTERNAL_PORTAL" });
        }
    ...
    app.post("/v1/external-review/invitations/:id/revoke", { preHandler: requireAuth }, async (req, reply) => {
        const ctx = await resolveInternalTeam(req, reply);
        if (!ctx)
            return reply;
        if (!requireCap(ctx, "review.assign"))
    ...
    app.get("/v1/external-review/invitations/:id/activity", { preHandler: requireAuth }, async (req, reply) => {
        const ctx = await resolveInternalTeam(req, reply);
        if (!ctx)
            return reply;
        const { id } = z.object({ id: z.string().uuid() }).parse(req.params);
    ...
    // ---------------------------------------------------------------------------
    app.post("/v1/external-review/invitations/bulk", { preHandler: requireAuth }, async (req, reply) => {
        const ctx = await resolveInternalTeam(req, reply);
        if (!ctx)
            return reply;
        ...
        const res = await bulkIssueInvitations({
            teamId: ctx.teamId,
            invitedByUserId: ctx.userId,
            inviterDisplayName: user?.displayName ?? user?.email ?? "PROOVRA operator",
    ...
    app.post("/v1/external-review/invitations/bulk/revoke", { preHandler: requireAuth }, async (req, reply) => {
        const ctx = await resolveInternalTeam(req, reply);
        if (!ctx)
            return reply;
        if (!requireCap(ctx, "review.bulk") || !requireCap(ctx, "review.assign"))
    ...
    // ---------------------------------------------------------------------------
    app.post("/v1/external-review/invitations/:id/reveal-token", { preHandler: requireAuth }, async (req, reply) => {
        const ctx = await resolveInternalTeam(req, reply);
        if (!ctx)
            return reply;
    ...
    app.post("/v1/external-review/invitations/:id/resend", { preHandler: requireAuth }, async (req, reply) => {
        const ctx = await resolveInternalTeam(req, reply);
        if (!ctx)
            return reply;
        if (!requireCap(ctx, "review.assign"))
    ...
    app.get("/v1/external-review/invitations/:id/delivery", { preHandler: requireAuth }, async (req, reply) => {
        const ctx = await resolveInternalTeam(req, reply);
        if (!ctx)
            return reply;
        const { id } = z.object({ id: z.string().uuid() }).parse(req.params);
    ...
    // =========================================================================
    app.post("/v1/portal/auth", async (req, reply) => {
        ...
        if (!sess.ok)
            return reply.code(401).send({ denial: sess.denial });
    ...
    app.post("/v1/portal/logout", async (req, reply) => {
    ...
    app.get("/v1/portal/dashboard", async (req, reply) => {
        const s = await resolvePortalSession(req, reply);
        if (!s)
            return reply;
        const projection = await projectPortalDashboard({ session: s });
        return reply.code(200).send({ portal: projection });
    });
    app.get("/v1/portal/work/:workflowId/comments", async (req, reply) => {
        const s = await resolvePortalSession(req, reply);
        if (!s)
            return reply;
        const { workflowId } = z
    ...
    app.post("/v1/portal/work/:workflowId/comments", async (req, reply) => {
        const s = await resolvePortalSession(req, reply);
        if (!s)
            return reply;
    ...
    app.post("/v1/portal/work/:workflowId/decision", async (req, reply) => {
        const s = await resolvePortalSession(req, reply);
        if (!s)
            return reply;
    ...
    app.get("/v1/portal/work/:workflowId/decisions", async (req, reply) => {
        const s = await resolvePortalSession(req, reply);
        if (!s)
            return reply;
        const { workflowId } = z
    ...
    app.post("/v1/portal/work/:workflowId/view", async (req, reply) => {
        const s = await resolvePortalSession(req, reply);
        if (!s)
            return reply;
        const { workflowId } = z
    ...
    app.get("/v1/portal/activity", async (req, reply) => {
        const s = await resolvePortalSession(req, reply);
        if (!s)
            return reply;
    ...
    // -------------------------------------------------------------------------
    app.post("/v1/portal/sso/start/:grantId", async (req, reply) => {
        const { grantId } = z
    ...
    app.post("/v1/portal/sso/callback", async (req, reply) => {
    ...
    // -------------------------------------------------------------------------
    app.post("/v1/external-review/invitations/:id/sessions/revoke", { preHandler: requireAuth }, async (req, reply) => {
        const ctx = await resolveInternalTeam(req, reply);
        if (!ctx)
            return reply;
        if (!requireCap(ctx, "review.assign"))
    ...
    // -------------------------------------------------------------------------
    app.post("/v1/external-review/invitations/:id/send-email", { preHandler: requireAuth }, async (req, reply) => {
        const ctx = await resolveInternalTeam(req, reply);
        if (!ctx)
            return reply;
        const { id } = z.object({ id: z.string().uuid() }).parse(req.params);
