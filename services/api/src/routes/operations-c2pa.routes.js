...
import { BACKFILL_TARGET_FILTERS, cancelC2paBackfillRun, getC2paBackfillRun, listC2paBackfillRuns, previewC2paBackfill, startC2paBackfill, tickC2paBackfillRun, } from "../services/c2pa/c2pa-backfill.service.js";
...
function envFlags() {
    return {
        ...
        C2PA_RAW_MANIFEST_EXPORT_ENABLED: process.env.C2PA_RAW_MANIFEST_EXPORT_ENABLED ?? "false",
    };
}
export async function operationsC2paRoutes(app) {
    // -------------------------------------------------------------------
    // Overview
    // -------------------------------------------------------------------
    app.get("/v1/operations/c2pa", { preHandler: requireAuth }, async (req, reply) => {
        const q = z
    ...
    // Backfill preview
    // -------------------------------------------------------------------
    app.post("/v1/operations/c2pa/backfill/preview", { preHandler: requireAuth }, async (req, reply) => {
        const body = z
            .object({
    ...
    // -------------------------------------------------------------------
    app.post("/v1/operations/c2pa/backfill/start", { preHandler: requireAuth }, async (req, reply) => {
        const body = z
            .object({
            teamId: z.string().uuid(),
            filter: z.enum(BACKFILL_TARGET_FILTERS).optional(),
    ...
    // List runs
    // -------------------------------------------------------------------
    app.get("/v1/operations/c2pa/backfill", { preHandler: requireAuth }, async (req, reply) => {
        const q = z
            .object({ teamId: z.string().uuid() })
            .parse(req.query ?? {});
        const ctx = await requireOpsActor(req, reply, q.teamId);
    ...
    // Get one run
    // -------------------------------------------------------------------
    app.get("/v1/operations/c2pa/backfill/:id", { preHandler: requireAuth }, async (req, reply) => {
        const params = z
    ...
    // Cancel a run
    // -------------------------------------------------------------------
    app.post("/v1/operations/c2pa/backfill/:id/cancel", { preHandler: requireAuth }, async (req, reply) => {
        const params = z
            .object({ id: z.string().uuid() })
            .parse(req.params);
        const body = z
    ...
    // -------------------------------------------------------------------
    app.post("/v1/operations/c2pa/backfill/:id/tick", { preHandler: requireAuth }, async (req, reply) => {
        const params = z
            .object({ id: z.string().uuid() })
            .parse(req.params);
        const body = z
    ...
    // -------------------------------------------------------------------
    app.get("/v1/operations/c2pa/generation/readiness", { preHandler: requireAuth }, async (req, reply) => {
        const q = z
            .object({ teamId: z.string().uuid() })
            .parse(req.query ?? {});
        const ctx = await requireOpsActor(req, reply, q.teamId);
    ...
    // -------------------------------------------------------------------
    app.post("/v1/operations/c2pa/generate", { preHandler: requireAuth }, async (req, reply) => {
        const body = z
            .object({
            teamId: z.string().uuid(),
            targetKind: z
