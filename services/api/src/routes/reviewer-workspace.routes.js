 ...
 */
import { z } from "zod";
import { CODING_FIELD_TYPES, CODING_SCHEMA_CATEGORIES, DISAGREEMENT_STATES, QC_FAILURE_REASONS, QC_VERDICTS, REVIEWER_VERDICTS, } from "@proovra/shared";
import { getAuthUserId } from "../auth.js";
import { prisma } from "../db.js";
import { requireAuth } from "../middleware/auth.js";
import { assertFeatureEntitlement } from "../services/packaging/entitlement.service.js";
import { archiveSchema, bindSchemaToWorkflow, createSchema, getSchema, listSchemas, publishSchema, seedDefaultSchemas, } from "../services/reviewer-workspace/coding-schema.service.js";
import { evaluateRequiredFieldsCoverage, readCodingValuesForWorkflow, writeCodingValue, } from "../services/reviewer-workspace/coding-value.service.js";
import { fileDisagreement, listDisagreements, transitionDisagreement, } from "../services/reviewer-workspace/reviewer-disagreement.service.js";
import { assignSample, listSamples, renderQcVerdict, } from "../services/reviewer-workspace/qc-sample.service.js";
import { bulkResolveAnnotations, listAnnotationsForEvidence, postAnnotationReply, resolveAnnotation, } from "../services/reviewer-workspace/annotation-workspace.service.js";
import { bulkAssign, bulkCode, bulkDecide, } from "../services/reviewer-workspace/bulk-operations.service.js";
...
// Helpers
// =============================================================================
async function resolveTeam(req, reply) {
...
function denyWith(reply, status, denial) {
...
// Routes
// =============================================================================
export async function reviewerWorkspaceRoutes(app) {
    // ---------------------------------------------------------------------------
    // GET /v1/reviewer/workspace
    // ---------------------------------------------------------------------------
    app.get("/v1/reviewer/workspace", { preHandler: requireAuth }, async (req, reply) => {
    ...
    // ---------------------------------------------------------------------------
    app.get("/v1/reviewer/metrics", { preHandler: requireAuth }, async (req, reply) => {
        const ctx = await resolveTeam(req, reply);
        if (!ctx)
            return reply;
    ...
    // CODING SCHEMAS
    // ---------------------------------------------------------------------------
    app.get("/v1/coding/schemas", { preHandler: requireAuth }, async (req, reply) => {
        const ctx = await resolveTeam(req, reply);
        if (!ctx)
            return reply;
        const q = z
    ...
    app.post("/v1/coding/schemas", { preHandler: requireAuth }, async (req, reply) => {
        const ctx = await resolveTeam(req, reply);
        if (!ctx)
            return reply;
        ...
        try {
            ...
            if (!feOk.ok)
                return reply.code(403).send({ denial: "ENTITLEMENT_REQUIRED", entitlement: "FEATURE_REVIEWER_WORKSPACE" });
        }
        catch { /* engine failure must not break route */ }
        if (!requireCap(ctx, "review.schema.author"))
            return denyNoPermission(reply);
        ...
        if (!res.ok)
            return denyWith(reply, 409, res.denial);
        return reply.code(201).send({ schemaId: res.schemaId, version: res.version });
    });
    app.get("/v1/coding/schemas/:id", { preHandler: requireAuth }, async (req, reply) => {
        const ctx = await resolveTeam(req, reply);
        if (!ctx)
            return reply;
        ...
        if (!schema)
            return denyWith(reply, 404, "SCHEMA_NOT_FOUND");
        return reply.code(200).send({ schema });
    });
    app.post("/v1/coding/schemas/:id/publish", { preHandler: requireAuth }, async (req, reply) => {
        const ctx = await resolveTeam(req, reply);
        if (!ctx)
            return reply;
        if (!requireCap(ctx, "review.schema.author"))
    ...
    app.post("/v1/coding/schemas/:id/archive", { preHandler: requireAuth }, async (req, reply) => {
        const ctx = await resolveTeam(req, reply);
        if (!ctx)
            return reply;
        if (!requireCap(ctx, "review.schema.author"))
    ...
    app.post("/v1/coding/schemas/seed-defaults", { preHandler: requireAuth }, async (req, reply) => {
        const ctx = await resolveTeam(req, reply);
        if (!ctx)
            return reply;
        if (!requireCap(ctx, "review.schema.author"))
    ...
    app.post("/v1/reviewer/work/:workflowId/bind-schema", { preHandler: requireAuth }, async (req, reply) => {
        const ctx = await resolveTeam(req, reply);
        if (!ctx)
            return reply;
        if (!requireCap(ctx, "review.assign"))
    ...
    // CODING VALUES
    // ---------------------------------------------------------------------------
    app.get("/v1/reviewer/work/:workflowId/coding", { preHandler: requireAuth }, async (req, reply) => {
        const ctx = await resolveTeam(req, reply);
        if (!ctx)
            return reply;
        const { workflowId } = z
    ...
    app.post("/v1/reviewer/work/:workflowId/code", { preHandler: requireAuth }, async (req, reply) => {
        const ctx = await resolveTeam(req, reply);
        if (!ctx)
            return reply;
        if (!requireCap(ctx, "review.code"))
            return denyNoPermission(reply);
        const { workflowId } = z
            .object({ workflowId: z.string().uuid() })
            .parse(req.params);
    ...
    // DISAGREEMENTS
    // ---------------------------------------------------------------------------
    app.get("/v1/reviewer/disagreements", { preHandler: requireAuth }, async (req, reply) => {
        const ctx = await resolveTeam(req, reply);
        if (!ctx)
            return reply;
        const q = z
    ...
    app.post("/v1/reviewer/work/:workflowId/disagree", { preHandler: requireAuth }, async (req, reply) => {
        const ctx = await resolveTeam(req, reply);
        if (!ctx)
            return reply;
        if (!requireCap(ctx, "review.disagree"))
            return denyNoPermission(reply);
        const { workflowId } = z
            .object({ workflowId: z.string().uuid() })
            .parse(req.params);
    ...
    app.post("/v1/reviewer/disagreements/:id/transition", { preHandler: requireAuth }, async (req, reply) => {
        const ctx = await resolveTeam(req, reply);
        if (!ctx)
            return reply;
        if (!requireCap(ctx, "review.adjudicate"))
            return denyNoPermission(reply);
        const { id } = z.object({ id: z.string().uuid() }).parse(req.params);
    ...
    // QC SAMPLES
    // ---------------------------------------------------------------------------
    app.get("/v1/reviewer/qc/samples", { preHandler: requireAuth }, async (req, reply) => {
        const ctx = await resolveTeam(req, reply);
        if (!ctx)
            return reply;
        const q = z
    ...
    app.post("/v1/reviewer/qc/samples/:id/assign", { preHandler: requireAuth }, async (req, reply) => {
        const ctx = await resolveTeam(req, reply);
        if (!ctx)
            return reply;
        if (!requireCap(ctx, "review.assign"))
    ...
    app.post("/v1/reviewer/qc/samples/:id/verdict", { preHandler: requireAuth }, async (req, reply) => {
        const ctx = await resolveTeam(req, reply);
        if (!ctx)
            return reply;
        if (!requireCap(ctx, "review.qc.verdict"))
            return denyNoPermission(reply);
        const { id } = z.object({ id: z.string().uuid() }).parse(req.params);
    ...
    // ---------------------------------------------------------------------------
    app.get("/v1/reviewer/evidence/:evidenceId/annotations", { preHandler: requireAuth }, async (req, reply) => {
        const ctx = await resolveTeam(req, reply);
        if (!ctx)
            return reply;
        const { evidenceId } = z
        ...
        if (!res.ok)
            return denyWith(reply, 404, "EVIDENCE_NOT_FOUND");
        return reply.code(200).send({ annotations: res.annotations });
    });
    app.post("/v1/reviewer/annotations/:id/reply", { preHandler: requireAuth }, async (req, reply) => {
        const ctx = await resolveTeam(req, reply);
        if (!ctx)
            return reply;
        if (!requireCap(ctx, "review.annotate"))
            return denyNoPermission(reply);
        const { id } = z.object({ id: z.string().uuid() }).parse(req.params);
        const body = z
    ...
    app.post("/v1/reviewer/annotations/:id/resolve", { preHandler: requireAuth }, async (req, reply) => {
        const ctx = await resolveTeam(req, reply);
        if (!ctx)
            return reply;
        if (!requireCap(ctx, "review.annotate"))
    ...
    app.post("/v1/reviewer/annotations/bulk-resolve", { preHandler: requireAuth }, async (req, reply) => {
        const ctx = await resolveTeam(req, reply);
        if (!ctx)
            return reply;
        if (!requireCap(ctx, "review.annotate"))
    ...
    // ---------------------------------------------------------------------------
    app.post("/v1/reviewer/bulk/assign", { preHandler: requireAuth }, async (req, reply) => {
        const ctx = await resolveTeam(req, reply);
        if (!ctx)
            return reply;
    ...
    app.post("/v1/reviewer/bulk/decide", { preHandler: requireAuth }, async (req, reply) => {
        const ctx = await resolveTeam(req, reply);
        if (!ctx)
            return reply;
    ...
    app.post("/v1/reviewer/bulk/code", { preHandler: requireAuth }, async (req, reply) => {
        const ctx = await resolveTeam(req, reply);
        if (!ctx)
            return reply;
