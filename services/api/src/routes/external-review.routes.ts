...
import {
  ...
  recordExternalReviewAccess,
  transitionExternalReviewGrant,
  type ExternalReviewGrantRow,
} from "../services/external-review/external-review-grant.service.js";
import {
...
// =============================================================================
// Operator → reviewer projection (anti-leak shape)
// =============================================================================

/**
 * The reviewer-facing context shape. Intentionally NARROW — only the
 * fields a remote reviewer needs to render their review room. NEVER
 * includes internal IDs, signed URLs, storage keys, private notes,
 * legal notes, hidden workflow state, or workspace branding the
 * reviewer shouldn't see.
 */
type ReviewerContextProjection = {
  ...
  /** Bounded summary the operator wrote when inviting the reviewer. */
  safeNote: string | null;
  /** Optional redaction-policy version identifier — operators can
   *  surface "this view was redacted under policy v3.2" to the
   *  reviewer without exposing what was redacted. */
  redactionPolicyVersion: string | null;
};

function projectGrantForReviewer(
  grant: ExternalReviewGrantRow,
...
export async function externalReviewRoutes(app: FastifyInstance) {
  // ===========================================================================
