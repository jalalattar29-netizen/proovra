...
async function _emitEvidenceCreateSpans(rawBody) {
    ...
    if (rawBody?.captureSessionId) {
        await withProovraSpan(PROOVRA_SPAN_NAMES.CAPTURE_FINISH_SIGN, { "proovra.operation": "capture_finish_sign" }, _noop);
    }
}
import { CAPTURE_LOCATION_CONTEXT_DESCRIPTION, CAPTURE_LOCATION_LEGAL_BOUNDARY, CAPTURE_LOCATION_SOURCE_LABEL, CAPTURE_LOCATION_STATUS_LABEL, buildCaptureLocationExternalMapUrl, buildEvidenceTrustDecision, compareReviewerArtifactRolePriority, getReviewerArtifactRoleLabel, getReviewerEvidenceTypeLabel, getReviewerUploadModeLabel, deriveAnchorSemantics, hasCaptureLocationMetadata, isPrimaryReviewerArtifactRole, maskPublicEmailsInText, resolveReviewerArtifactRole, resolveEffectiveOtsStatus, } from "@proovra/shared";
import { resolveEvidenceTitle, detectEvidenceAssetKind, isPreviewableEvidenceKind, extensionFromMimeType, basenameFromStorageKey, getEvidencePartDisplayLabel, formatBytesForDisplay, buildContentCompositionSummary, buildPrimaryContentLabel, buildEvidenceDisplayDescriptor, resolveEvidenceContentAccessPolicyForSurface, buildEvidencePreviewPolicy, } from "@proovra/shared-evidence-presentation";
import { z } from "zod";
...
import { resolveWorkspaceScopeForUser, } from "../services/billing-enforcement.service.js";
...
import { presignGetObject, presignPutObject, headObject, getObjectRange, } from "../storage.js";
...
import { appendCustodyEvent, evaluateCustodyChain, classifyCustodyEventType, } from "../services/custody-events.service.js";
...
import { listEvidenceCertifications, requestEvidenceCertification, revokeEvidenceCertification, } from "../services/evidence-certification.service.js";
...
import { appendReviewerAuditEvent, listReviewerAuditEvents, } from "../services/evidence-review/reviewer-audit.service.js";
import { getEvidenceReviewerWorkflowSummary, listEvidenceReviewerWorkflowEvents, upsertEvidenceReviewerWorkflow, } from "../services/evidence-review/reviewer-workflow.service.js";
import { createEvidenceRelationship, deleteEvidenceRelationship, listEvidenceRelationships, updateEvidenceRelationship, } from "../services/evidence-review/relationship-summary.service.js";
...
const { EvidenceStatus, PlanType, VerificationSource, VerificationViewerType } = prismaPkg;
const evidenceAiProvider = createAiProvider();
const evidenceAiCostGuard = new AiCostGuard();
function assertEvidenceNotLocked(evidence) {
    if (evidence.lockedAt) {
        const err = new Error("Evidence is permanently locked");
        err.statusCode = 409;
        err.code = "EVIDENCE_LOCKED";
        throw err;
    }
}
function assertEvidenceDeletionAllowedByRetention(evidence) {
    ...
    if (mode === "COMPLIANCE" && retainUntil && retainUntil > new Date()) {
        const err = new Error("Evidence is under compliance retention and cannot be deleted before retention expiry");
        err.statusCode = 409;
        err.code = "COMPLIANCE_RETENTION_ACTIVE";
        throw err;
    }
}
function getErrorCode(err, fallback = "OPERATION_BLOCKED") {
...
function parseZipCentralDirectoryEntries(buffer) {
...
async function listZipEntryNames(bucket, key) {
    ...
    const cdBuffer = centralDirectoryOffset >= tailStart
...
async function inspectVerificationPackageArtifacts(bucket, key) {
    if (!bucket || !key) {
        return null;
    }
    try {
        ...
        console.warn("Unable to inspect verification package contents for artifact presence:", error);
        return null;
    }
}
function isVerificationPackageMetadata(value) {
    if (!value || typeof value !== "object")
        return false;
    const candidate = value;
    return (candidate.manifestPresent === true || candidate.manifestPresent === false) &&
...
async function requireAuthAndLegal(req, reply) {
...
function buildReviewerRepresentationLabel(params) {
    const prefix = params.artifactRole === "primary_evidence"
        ? "Primary"
...
function sortPublicEvidenceItems(items) {
    return [...items].sort((left, right) => {
        const roleOrder = compareReviewerArtifactRolePriority(left.artifactRole, right.artifactRole);
        if (roleOrder !== 0)
            return roleOrder;
...
function getPublicVerifyIdentityExposure() {
    const exposeAttribution = String(process.env.PUBLIC_VERIFY_EXPOSE_ATTRIBUTION ?? "false")
        .trim()
        .toLowerCase() === "true";
    const exposeAuthProviderCode = String(process.env.PUBLIC_VERIFY_EXPOSE_AUTH_PROVIDER_CODE ?? "false")
        .trim()
        .toLowerCase() === "true";
    return {
        exposeAttribution,
...
function auditEvidenceAction(req, params) {
...
function auditVerificationAction(req, params) {
    void appendPlatformAuditLog({
        userId: params.userId,
...
function normalizePublicPayloadValue(value) {
...
function mapRecordStatusLabel(status) {
...
function mapVerificationStatusLabel(status) {
    switch (String(status ?? "").toUpperCase()) {
...
function mapAuthProviderLabel(provider) {
...
function mapIdentityLevelLabel(level) {
...
function mapVerificationSourceLabel(source) {
...
function formatDisplayDateUtc(value) {
...
function buildEvidenceSubtitle(params) {
    ...
    return `${count} ${count === 1 ? "item" : "items"} • ${mapRecordStatusLabel(params.status)} • ${formatDisplayDateUtc(params.createdAt)}`;
}
function addDays(date, days) {
    return new Date(date.getTime() + days * 24 * 60 * 60 * 1000);
}
function getCompletedEvidenceLabel(itemCount) {
    const count = typeof itemCount === "number" && Number.isFinite(itemCount)
...
function normalizeTimestampStatus(status) {
    const text = typeof status === "string" ? status.trim().toUpperCase() : "";
    return text || null;
}
function normalizeOtsStatus(status) {
    const text = typeof status === "string" ? status.trim().toUpperCase() : "";
    return text || null;
}
...
function mapCaptureMethodLabel(captureMethod) {
...
function getTimestampDigestLabel(params) {
    const isMultipart = params.itemCount > 1 ||
...
function normalizeTrustDecisionSnapshot(value) {
...
function mapIntegrityHeadline(params) {
    const coreSignal = params.trustDecision?.signals.find((signal) => signal.key === "core_integrity");
    const explicitlyVerified = String(params.verificationStatus ?? "").toUpperCase() ===
        "RECORDED_INTEGRITY_VERIFIED";
    if (coreSignal?.status === "passed" &&
        explicitlyVerified &&
    ...
    if (params.overallIntegrity === true &&
...
function mapIntegritySummaryText(params) {
    ...
    const coreChecksPassed = params.canonicalHashMatches &&
...
function mapStorageStatusLabel(storage) {
    if (!storage)
        return "Not reported";
...
function mapTimestampStatusLabel(status) {
...
function mapOtsStatusLabel(status) {
...
function summarizePublicPayload(eventType, payload, context) {
    ...
    switch (eventType) {
        ...
        case prismaPkg.CustodyEventType.UPLOAD_AUTHORIZED: {
            const uploadMode = getReviewerUploadModeLabel({
                ...
                rawMode: normalizePublicPayloadValue(obj.uploadKind) ??
        ...
        case prismaPkg.CustodyEventType.UPLOAD_COMPLETED: {
            const itemCount = typeof obj.itemCount === "number" && Number.isFinite(obj.itemCount)
                ? obj.itemCount
                : null;
        ...
        case prismaPkg.CustodyEventType.OTS_APPLIED: {
            ...
            const calendar = normalizePublicPayloadValue(obj.calendar) ??
                normalizePublicPayloadValue(obj.otsCalendar);
            const bitcoinTxid = normalizePublicPayloadValue(obj.bitcoinTxid) ??
        ...
        case prismaPkg.CustodyEventType.REVIEW_READY: {
            const reviewerSummaryVersion = normalizePublicPayloadValue(obj.reviewerSummaryVersion);
            return [
        ...
        case prismaPkg.CustodyEventType.IDENTITY_SNAPSHOT_RECORDED: {
            const identityLevel = normalizePublicPayloadValue(obj.identityLevelSnapshot);
            const submittedByEmail = normalizePublicPayloadValue(obj.submittedByEmail);
            const authProvider = normalizePublicPayloadValue(obj.submittedByAuthProvider);
            return [
        ...
        default: {
            const safeEntries = Object.entries(obj)
                ...
                .slice(0, 5)
                .map(([key, value]) => `${key}: ${maskPublicEmailsInText(String(value))}`);
...
function toSafeEvidence(e) {
    return {
        ...
        storageObjectLockRetainUntilUtc: e.storageObjectLockRetainUntilUtc
            ...
            : null,
        storageObjectLockLegalHoldStatus: e.storageObjectLockLegalHoldStatus ?? null,
...
async function getEvidenceItemCount(evidenceId) {
    const count = await prisma.evidencePart.count({
        where: { evidenceId },
    });
    return count > 0 ? count : 1;
}
async function getStorageProtectionSummary(bucket, key, snapshot) {
    const snapshotMode = typeof snapshot?.storageObjectLockMode === "string"
        ? snapshot.storageObjectLockMode
        : null;
    const snapshotRetainUntil = snapshot?.storageObjectLockRetainUntilUtc instanceof Date
        ...
        : typeof snapshot?.storageObjectLockRetainUntilUtc === "string"
            ...
            : null;
    const snapshotLegalHold = typeof snapshot?.storageObjectLockLegalHoldStatus === "string"
        ? snapshot.storageObjectLockLegalHoldStatus
        : null;
    const snapshotRegion = typeof snapshot?.storageRegion === "string" && snapshot.storageRegion.trim()
    ...
    try {
        ...
        const retainUntil = meta.objectLockRetainUntilDate instanceof Date
...
function getStorageProtectionSummaryFromSnapshot(snapshot) {
    ...
    const retainUntil = snapshot.storageObjectLockRetainUntilUtc instanceof Date
        ? snapshot.storageObjectLockRetainUntilUtc.toISOString()
...
async function assertCaseAccess(userId, caseId) {
    ...
    if (item.ownerUserId === userId)
        return;
    if (item.access.some((a) => a.userId === userId))
        return;
...
async function getAccessibleEvidenceContext(userId) {
...
function buildEvidenceListBaseWhere(params) {
    const { query } = params;
    const archivedFilter = query.scope === "archived"
        ? { archivedAt: { not: null } }
        : query.scope === "active" || query.scope === "locked"
            ? { archivedAt: null }
            : {};
    const deletedFilter = query.scope === "deleted" ? { deletedAt: { not: null } } : { deletedAt: null };
    const lockedFilter = query.scope === "locked"
        ? { lockedAt: { not: null } }
        : query.scope === "active" || query.scope === "archived"
            ? { lockedAt: null }
            : {};
    const accessFilter = query.caseId
        ? { caseId: query.caseId }
        : {
            OR: [
    ...
    const statusFilter = query.status ? { status: query.status } : null;
...
function buildEvidenceListSearchFilter(search) {
    if (!search)
        return null;
...
function buildEvidenceListTypeFilter(type) {
    if (!type || type === "all")
        return null;
    switch (type) {
        case "photo":
        ...
        default: {
            ...
            if (enumCandidate in prismaPkg.EvidenceType) {
                return { type: enumCandidate };
            }
            return null;
        }
    }
}
function buildEvidenceListCaseAssignmentFilter(caseAssignment) {
    if (caseAssignment === "assigned")
        return { caseId: { not: null } };
    if (caseAssignment === "unassigned")
        return { caseId: null };
    return null;
}
function buildEvidenceListReportReadyFilter(reportReady) {
...
function buildEvidenceListCursorFilter(cursor, sort) {
    if (!cursor)
        return null;
...
function getEvidenceListOrderBy(sort) {
    const createdDirection = sort === "oldest" ? "asc" : "desc";
    return [{ createdAt: createdDirection }, { id: createdDirection }];
}
function mapEvidenceListItem(item) {
...
async function getEvidenceWithReadAccess(userId, evidenceId) {
...
async function getEvidenceWithOwnerAccess(userId, evidenceId) {
    const evidence = await prisma.evidence.findUnique({
        where: { id: evidenceId },
        select: SAFE_EVIDENCE_SELECT,
    });
...
async function canManageEvidenceCollaborativeContent(userId, evidence) {
    if (evidence.ownerUserId === userId) {
        return true;
    }
    if (!evidence.teamId) {
...
function buildMetadataCsv(items) {
    const rows = [
        [
            "Evidence ID",
            "Title",
...
function encodeEvidenceListCursor(value) {
    return Buffer.from(JSON.stringify(value), "utf8").toString("base64url");
}
function decodeEvidenceListCursor(value) {
...
function parseEvidenceStatusFilter(value) {
...
function normalizeEvidenceListTypeFilter(value) {
    const raw = typeof value === "string" ? value.trim().toLowerCase() : "";
    return raw || null;
}
function parseEvidenceListQuery(query) {
    const caseId = typeof query.caseId === "string" && query.caseId.trim()
        ? z.string().uuid().parse(query.caseId)
        : null;
    const scope = typeof query.scope === "string" && query.scope.trim().length > 0
    ...
    const cursor = decodeEvidenceListCursor(typeof query.cursor === "string" ? query.cursor : null);
    if (typeof query.cursor === "string" && query.cursor.trim() && !cursor) {
        const err = new Error("Invalid evidence list cursor");
        err.statusCode = 400;
        throw err;
    }
    const search = typeof query.search === "string" && query.search.trim().length > 0
        ...
        : null;
    const statusRaw = typeof query.status === "string" && query.status.trim().length > 0
        ? query.status
        : null;
    const status = parseEvidenceStatusFilter(statusRaw);
    if (statusRaw && !status) {
        const err = new Error("Invalid evidence status filter");
        err.statusCode = 400;
        throw err;
    }
    const type = normalizeEvidenceListTypeFilter(typeof query.type === "string" ? query.type : null);
    const caseAssignment = typeof query.caseAssignment === "string" && query.caseAssignment.trim().length > 0
        ? EvidenceListCaseAssignmentSchema.parse(query.caseAssignment.trim().toLowerCase())
        : "all";
    const reportReadyRaw = typeof query.reportReady === "string" && query.reportReady.trim().length > 0
        ? query.reportReady.trim().toLowerCase()
        : "all";
    if (!["all", "ready", "missing"].includes(reportReadyRaw)) {
        const err = new Error("Invalid report readiness filter");
        err.statusCode = 400;
        throw err;
    }
    const sort = typeof query.sort === "string" && query.sort.trim().length > 0
...
async function buildPublicEvidenceContent(params) {
    ...
    const buildRoleDecision = (input) => resolveReviewerArtifactRole({
    ...
    const roleAwareItems = multipart
        ? await Promise.all(params.parts.map(async (part) => {
            ...
            const resolvedRole = buildRoleDecision({
                ...
                fallbackRole: params.evidence.storageBucket === part.storageBucket &&
                    params.evidence.storageKey === part.storageKey
                    ? "primary_evidence"
                    : "supporting_evidence",
                fallbackRoleSource: params.evidence.storageBucket === part.storageBucket &&
                    params.evidence.storageKey === part.storageKey
                    ? "fallback_root"
                    : "fallback_first",
            });
            const isPrimary = isPrimaryReviewerArtifactRole(resolvedRole.artifactRole);
            const canPreviewThisItem = canExposeContent && isPreviewableEvidenceKind(kind);
            const canExposeDirectUrl = (canPreviewThisItem || canDownload) &&
            ...
            return {
                ...
                artifactRoleLabel: getReviewerArtifactRoleLabel(resolvedRole.artifactRole),
                ...
                previewDataUrl: canExposeContent ? preview?.previewDataUrl ?? null : null,
                previewTextExcerpt: canExposeContent ? preview?.previewTextExcerpt ?? null : null,
                previewCaption: canExposeContent ? preview?.previewCaption ?? null : null,
            };
        }))
        : singlePart || (params.evidence.storageBucket && params.evidence.storageKey)
            ? await Promise.all([
                (async () => {
                    ...
                    const sizeBytesValue = singlePart?.sizeBytes ?? params.evidence.sizeBytes;
                    ...
                    const previewable = canExposeContent && isPreviewableEvidenceKind(kind);
                    const canExposeDirectUrl = previewable || canDownload;
                    const label = getEvidencePartDisplayLabel({
                        partIndex: itemIndex,
                        mimeType,
                        originalFileName: singlePart?.originalFileName ??
            ...
            : [];
    const items = sortPublicEvidenceItems(roleAwareItems).map((item, index, list) => {
        if (list.some((candidate) => candidate.isPrimary)) {
            return {
                ...item,
                previewRole: item.previewRole === "metadata_only"
                    ? "metadata_only"
                    : item.isPrimary
                        ? "primary_preview"
                        : "secondary_preview",
    ...
    const primaryItem = items.find((item) => item.isPrimary) ?? (items.length > 0 ? items[0] : null);
    const summary = items.reduce((acc, item) => {
        acc.itemCount += 1;
        if (item.previewable)
            acc.previewableItemCount += 1;
        if (item.downloadable)
            acc.downloadableItemCount += 1;
        if (item.kind === "image")
            acc.imageCount += 1;
    ...
    const totalSizeBigInt = items.reduce((acc, item) => {
...
function buildPublicVerifyOverview(params) {
    const reportGeneratedAtUtc = params.latestReport?.generatedAtUtc
        ...
        : params.evidence.reportGeneratedAtUtc
            ...
            : null;
    const reportVersion = params.latestReport?.version ?? params.evidence.latestReportVersion ?? null;
    return {
        recordStatus: mapRecordStatusLabel(params.evidence.status),
        recordLifecycleStatus: params.evidence.status,
        verificationStatus: mapVerificationStatusLabel(params.evidence.verificationStatus),
        ...
        contentCompositionSummary: buildContentCompositionSummary(params.contentSummary),
        primaryContentLabel: buildPrimaryContentLabel(params.contentSummary?.primaryKind ?? null),
        ...
        evidenceStructure: params.itemCount > 1 ? "Multipart evidence package" : "Single evidence item",
        itemCount: params.itemCount,
        ...
        submittedByEmail: params.evidence.submittedByEmail
            ...
            : null,
        submittedByAuthProvider: mapAuthProviderLabel(params.evidence.submittedByAuthProvider),
        ...
        recordedIntegrityVerifiedAtUtc: params.evidence.recordedIntegrityVerifiedAtUtc
        ...
        lastVerifiedAtUtc: params.evidence.lastVerifiedAtUtc
            ...
            : null,
        lastVerifiedSource: mapVerificationSourceLabel(params.evidence.lastVerifiedSource),
        lastVerifiedSourceCode: params.evidence.lastVerifiedSource ?? null,
        lastVerifiedAtUtcLabel: "Last meaningful technical verification (report generation or reviewer technical-verification action). Public page views do not update this field.",
        ...
        lastPublicVerifyViewAtUtc: params.evidence.lastPublicVerifyViewAtUtc
            ...
            : null,
        lastPublicVerifyViewAtUtcLabel: "Most recent anonymous public verify page view (analytics only — not a technical verification).",
        currentPublicVerifyViewAtUtc: params.currentPublicVerifyViewAtUtc?.toISOString() ?? null,
        currentPublicVerifyViewAtUtcLabel: "Timestamp of the current public verify page request (analytics only — not a technical verification).",
        ...
        verificationPackageGeneratedAtUtc: params.evidence.verificationPackageGeneratedAtUtc
            ...
            : null,
        verificationPackageVersion: params.evidence.verificationPackageVersion ?? null,
...
function buildPublicVerifyHumanSummary(params) {
    return {
        ...
        whatIsVerified: "This verification checks the recorded integrity state of the evidence record, including fingerprint consistency, signature validation, recorded custody chain continuity, timestamp linkage, and OpenTimestamps linkage where available.",
        ...
        recordedIntegrityVerifiedAtUtc: params.overview.recordedIntegrityVerifiedAtUtc,
        ...
        currentPublicVerifyViewAtUtc: params.overview.currentPublicVerifyViewAtUtc,
        ...
        verificationPackageGeneratedAtUtc: params.overview.verificationPackageGeneratedAtUtc,
        ...
        externalPublicationAnchoredAtUtc: params.overview.externalPublicationAnchoredAtUtc,
    };
}
function buildPublicVerifyLimitations() {
    return {
        short: "This page verifies the recorded integrity state of the evidence record. It does not independently prove factual truth, authorship, context, or legal admissibility.",
        detailed: "Technical verification supports detection of post-completion changes to the recorded evidence state. It does not by itself establish who created the content, whether the depicted events are true, or whether any court, insurer, regulator, or authority must accept the material.",
    };
}
function buildPublicReviewGuidance(params) {
    return {
        ...
        contentReviewNote: params.previewableItemCount > 0
        ...
        legalAssessmentNote: "Use the evidence content together with the technical verification record; neither should be treated as a substitute for the other.",
        ...
        multipartReviewNote: params.itemCount > 1
...
function buildTechnicalMaterials(params) {
    ...
    let resolvedSemantics = explicitSemantics;
    ...
    return {
        ...
        fileSha256Label: resolvedSemantics === "single_file"
        ...
        multipartManifestSha256Label: "Reproducible SHA-256 of newline-joined per-part SHA-256 hashes in part-index order",
...
function mapPublicCustodyEvent(ev, context) {
    return {
...
async function getAnchorStatus(evidenceId) {
    ...
    const resolvedPublicUrl = anchor.publicUrl?.trim() ||
        (publicBaseUrl &&
            (anchor.receiptId || anchor.transactionId || anchor.anchorHash)
            ? `${publicBaseUrl.replace(/\/+$/, "")}/${encodeURIComponent(anchor.receiptId ?? anchor.transactionId ?? anchor.anchorHash ?? "")}`
            : null);
...
function resolveWorkspaceCapabilitySnapshot(params) {
    const teamWorkspace = params.evidence.teamId
        ? params.overview.workspaces.teams.find((team) => team.id === params.evidence.teamId) ?? null
        : null;
    if (teamWorkspace) {
        return {
            workspaceType: "TEAM",
            workspaceName: params.evidence.workspaceNameSnapshot?.trim() ||
                teamWorkspace.name ||
                "Team Workspace",
            ...
            verificationPackageIncluded: Boolean(teamWorkspace.features?.verificationPackageIncluded),
    ...
    return {
        ...
        workspaceName: params.evidence.workspaceNameSnapshot?.trim() || "Personal Workspace",
        plan: params.overview.workspaces.personal.plan,
        effectivePlan: params.overview.workspaces.personal.plan,
        reportsIncluded: Boolean(params.overview.workspaces.personal.features?.reportsIncluded),
        verificationPackageIncluded: Boolean(params.overview.workspaces.personal.features
            ?.verificationPackageIncluded),
        publicVerifyIncluded: Boolean(params.overview.workspaces.personal.features?.publicVerifyIncluded),
        billingStatus: params.overview.workspaces.personal.subscription?.status ?? null,
        storageUsedLabel: params.overview.workspaces.personal.storage?.usedLabel ?? null,
        storageLimitLabel: params.overview.workspaces.personal.storage?.limitLabel ?? null,
        storageRemainingLabel: params.overview.workspaces.personal.storage?.remainingLabel ?? null,
        seatsIncluded: null,
        seatsUsed: null,
        seatsRemaining: null,
        overSeatLimit: null,
...
function readBooleanClientSignal(source, key) {
...
function readStringClientSignal(source, key) {
    if (!source || typeof source !== "object" || Array.isArray(source)) {
        return null;
    }
    const value = source[key];
...
function buildSourceContext(params) {
    const folderPathPresent = params.parts.some((part) => readBooleanClientSignal(part.clientSignals, "folderPathPresent") === true);
    const screenshotLike = params.parts.some((part) => readBooleanClientSignal(part.clientSignals, "screenshotLike") === true);
    const genericMime = params.parts.some((part) => readBooleanClientSignal(part.clientSignals, "genericMime") === true);
    const oldLastModified = params.parts.some((part) => readBooleanClientSignal(part.clientSignals, "oldLastModified") === true);
    const duplicateSignals = params.parts
        .map((part) => readStringClientSignal(part.clientSignals, "duplicateStatus"))
        .filter((value) => Boolean(value));
    const locationIncluded = hasCaptureLocationMetadata({
        lat: decimalToNumber(params.evidence.lat),
        lng: decimalToNumber(params.evidence.lng),
    }) ||
        params.parts.some((part) => readBooleanClientSignal(part.clientSignals, "locationIncluded") === true);
    const captureMethod = params.evidence.captureMethod ?? null;
    const sourceType = folderPathPresent || captureMethod === prismaPkg.CaptureMethod.MULTIPART_PACKAGE
        ? "folder_upload"
    ...
    return {
        ...
        metadataAvailability: {
            nativeMetadataRecorded: params.parts.some((part) => Boolean(part.originalFileName || part.mimeType)),
...
function buildResolvedReviewerAlerts(params) {
    const baseAlerts = params.evidenceIntelligence?.reviewerAlerts?.filter((alert) => {
        if (alert.label !== "Public verification not configured")
            return true;
    ...
    if (!params.workspaceCapabilitySnapshot.publicVerifyIncluded) {
        operationalAlerts.push({
            ...
            detail: "Public verification is not included in the current workspace plan.",
        });
    }
        ...
        operationalAlerts.push({
            ...
            detail: "Public verification is supported for this workspace, but a public verification record is not published yet.",
        });
    }
    if (!params.latestReportVersion) {
        operationalAlerts.push({
            ...
            detail: "Generate a PDF report when a fixed review artifact is required.",
        });
    }
    if (params.workspaceCapabilitySnapshot.verificationPackageIncluded &&
        !params.latestVerificationPackageVersion) {
        operationalAlerts.push({
            severity: "warning",
            label: "Verification package not generated",
            detail: "Generate a verification package for offline or external review when needed.",
        });
    }
    if (!params.evidence.caseId) {
        operationalAlerts.push({
            ...
            detail: "Case assignment is not recorded for this evidence item.",
        });
    }
...
async function buildStorageLimitPayload(params) {
    ...
    const workspace = params.teamId != null
        ? overview.workspaces.teams.find((team) => team.id === params.teamId) ?? null
        : overview.workspaces.personal;
    const upgradeSuggestion = workspace && workspace.workspaceType === "PERSONAL"
...
function formatCaptureFileTimestamp(value) {
        ...
        : kind === "video"
            ...
            : kind === "audio"
                ...
                : kind === "pdf"
                    ...
                    : "PROOVRA-EVIDENCE";
    const partSuffix = params.multipart && typeof params.partIndex === "number"
...
function resolveOriginalAssetDisplayName(params) {
    ...
    if (originalName)
        return originalName;
    const fromStorageKey = sanitizeFileName(basenameFromStorageKey(params.storageKey ?? null, `evidence-file.${extensionFromMimeType(params.mimeType)}`));
    if (fromStorageKey &&
        fromStorageKey !== "0" &&
        fromStorageKey !== "1" &&
        fromStorageKey !== "2" &&
...
export async function evidenceRoutes(app) {
    ...
    app.post("/v1/evidence", { preHandler: requireAuthAndLegal }, async (req, reply) => {
        ...
        const normalizedChecksum = normalizeChecksumSha256Base64(body.checksumSha256Base64);
        const normalizedContentMd5 = normalizeContentMd5Base64(body.contentMd5Base64);
        ...
        try {
            ...
            if (err instanceof Error &&
                ...
                return reply.code(402).send({
                    ...
                    message: "Team workspace evidence creation requires an active TEAM plan.",
                    target: "TEAM",
                    requiredPlan: "TEAM",
                });
            }
        ...
        try {
            const result = await createEvidence({
                ...
                intakePlanJson: body.intakePlanJson === null || body.intakePlanJson === undefined
                    ? undefined
                    : body.intakePlanJson,
            });
            ...
            try {
                ...
                if (createdEvidence?.teamId) {
                    const { applyRetentionPolicyOnCreate } = await import("../services/governance.service.js");
                ...
                req.log?.warn?.({ err, evidenceId: result.id }, "governance.retention.apply_failed");
            }
            ...
            if (body.captureSessionId) {
                try {
                    ...
                    req.log?.warn?.({ err: sessionErr, captureSessionId: body.captureSessionId, evidenceId: result.id }, "capture_session_finalize_link_failed");
                }
            }
    ...
    app.patch("/v1/evidence/:id/label", { preHandler: requireAuth }, async (req, reply) => {
        const ownerUserId = getAuthUserId(req);
        ...
        const storage = await getStorageProtectionSummary(updated.storageBucket, updated.storageKey, {
    ...
    app.post("/v1/evidence/:id/parts", { preHandler: requireAuth }, async (req, reply) => {
        const ownerUserId = getAuthUserId(req);
        const id = z.string().uuid().parse(req.params.id);
        ...
        await withProovraSpan(PROOVRA_SPAN_NAMES.EVIDENCE_UPLOAD_PRESIGN, {
            "proovra.evidence_id": id,
            "proovra.operation": "evidence_upload_presign",
        }, () => undefined);
        await withProovraSpan(PROOVRA_SPAN_NAMES.EVIDENCE_UPLOAD_COMPLETE, {
            "proovra.evidence_id": id,
        ...
        try {
            ...
            return reply.code(statusCode).send({
                ...
                message: err instanceof Error ? err.message : "Evidence part cannot be created",
            });
        }
        try {
          ...
          `;
                ...
                if (evidence.status === EvidenceStatus.SIGNED ||
                    ...
                    evidence.lockedAt) {
                    const err = new Error("Evidence is immutable");
                    err.statusCode = 409;
                    throw err;
                }
                ...
                if (presignValidation.outcome === "block") {
                    const err = new Error("File validation blocked");
                    err.statusCode = 415;
                ...
                const normalizedMimeType = normalizeMimeType(body.mimeType) ?? "application/octet-stream";
                const safeOriginalFileName = sanitizeFileName(body.originalFileName);
                const ext = extensionFromMimeType(normalizedMimeType);
                const fallbackFileName = safeOriginalFileName ??
                ...
                const part = await tx.evidencePart.create({
                    data: {
                        ...
                        clientSignals: body.clientSignals === undefined
                            ? undefined
    ...
    app.get("/v1/evidence/:id/parts", { preHandler: requireAuth }, async (req, reply) => {
        const ownerUserId = getAuthUserId(req);
        const id = z.string().uuid().parse(req.params.id);
        req.evidenceId = id;
        req.log = req.log.child({ evidenceId: id });
        ...
        const enrichedParts = await Promise.all(parts.map(async (part) => {
            ...
            const storage = await getStorageProtectionSummary(part.storageBucket, part.storageKey, {
                storageRegion: part.storageRegion ?? null,
                storageObjectLockMode: part.storageObjectLockMode ?? null,
                storageObjectLockRetainUntilUtc: part.storageObjectLockRetainUntilUtc ?? null,
                storageObjectLockLegalHoldStatus: part.storageObjectLockLegalHoldStatus ?? null,
            });
            ...
            return {
                ...
                displaySizeLabel: formatBytesForDisplay(sizeBytes),
                isPrimary: evidence.storageBucket === part.storageBucket &&
        ...
        return reply.code(200).send({
            ...
            primary: evidence.storageBucket && evidence.storageKey
                ? {
    ...
    app.post("/v1/evidence/:id/lock", { preHandler: requireAuth }, async (req, reply) => {
        const ownerUserId = getAuthUserId(req);
        const id = z.string().uuid().parse(req.params.id);
        ...
        if (evidence.status !== prismaPkg.EvidenceStatus.SIGNED &&
    ...
    app.post("/v1/evidence/:id/archive", { preHandler: requireAuth }, async (req, reply) => {
        const ownerUserId = getAuthUserId(req);
        const id = z.string().uuid().parse(req.params.id);
        req.evidenceId = id;
        req.log = req.log.child({ evidenceId: id });
        ...
        {
            const { runDestructiveActionGate } = await import("../services/governance/destructive-action-gate.service.js");
        ...
        if (evidence.archivedAt) {
            const storage = await getStorageProtectionSummary(evidence.storageBucket, evidence.storageKey, {
                storageRegion: evidence.storageRegion,
                storageObjectLockMode: evidence.storageObjectLockMode,
                storageObjectLockRetainUntilUtc: evidence.storageObjectLockRetainUntilUtc,
                storageObjectLockLegalHoldStatus: evidence.storageObjectLockLegalHoldStatus,
            });
            return reply.code(200).send({
                evidence: {
    ...
    app.post("/v1/evidence/:id/unarchive", { preHandler: requireAuth }, async (req, reply) => {
        const ownerUserId = getAuthUserId(req);
        const id = z.string().uuid().parse(req.params.id);
        req.evidenceId = id;
        req.log = req.log.child({ evidenceId: id });
    ...
    app.delete("/v1/evidence/:id", { preHandler: requireAuth }, async (req, reply) => {
        const ownerUserId = getAuthUserId(req);
        const id = z.string().uuid().parse(req.params.id);
        req.evidenceId = id;
        req.log = req.log.child({ evidenceId: id });
        ...
        if (evidence.teamId) {
            const { gateRetentionAction } = await import("../services/governance/policy-runtime-gates.service.js");
            ...
            if (!retentionGate.ok) {
                ...
                return reply.code(409).send({
                    ...
                    denial: retentionGate.denial,
                    reason: retentionGate.reason,
                    message: "Evidence deletion is blocked by the workspace retention policy.",
                });
            }
        }
        if (evidence.deletedAt) {
    ...
    app.post("/v1/evidence/:id/restore", { preHandler: requireAuth }, async (req, reply) => {
        const ownerUserId = getAuthUserId(req);
        const id = z.string().uuid().parse(req.params.id);
    ...
    app.get("/v1/evidence", { preHandler: requireAuth }, async (req, reply) => {
        const ownerUserId = getAuthUserId(req);
        const parsedQuery = parseEvidenceListQuery(req.query);
        const { caseId, scope, limit, cursor, search, status, type, caseAssignment, reportReady, sort, } = parsedQuery;
        if (caseId) {
    ...
    app.post("/v1/evidence/bulk", { preHandler: requireAuth }, async (req, reply) => {
        ...
        if (body.action === "ADD_TO_CASE") {
            ...
            if (!canAccessCase && caseItem.teamId) {
                canAccessCase = Boolean(await prisma.teamMember.findUnique({
        ...
        for (const evidenceId of uniqueIds) {
            try {
                const evidence = body.action === "ADD_TO_CASE" ||
    ...
    // -------------------------------------------------------------------
    app.get("/v1/evidence/:id/c2pa", { preHandler: requireAuth }, async (req, reply) => {
        const userId = getAuthUserId(req);
        const id = z.string().uuid().parse(req.params.id);
        const evidence = await getEvidenceWithReadAccess(userId, id);
        const { loadEvidenceC2paPanel } = await import("../services/c2pa/c2pa-evidence-panel.service.js");
    ...
    app.post("/v1/evidence/:id/c2pa/retry", { preHandler: requireAuth }, async (req, reply) => {
        const userId = getAuthUserId(req);
        const id = z.string().uuid().parse(req.params.id);
        const evidence = await getEvidenceWithReadAccess(userId, id);
        const teamId = evidence.teamId;
        // Audit the retry intent first so denials still leave a trail.
        const { appendPlatformAuditLog } = await import("../services/platform-audit-log.service.js");
    ...
    app.patch("/v1/evidence/:id/comments/:commentId", { preHandler: requireAuth }, async (req, reply) => {
        const userId = getAuthUserId(req);
        const id = z.string().uuid().parse(req.params.id);
        ...
        const canManage = comment.authorUserId === userId ||
    ...
    app.delete("/v1/evidence/:id/comments/:commentId", { preHandler: requireAuth }, async (req, reply) => {
        const userId = getAuthUserId(req);
        const id = z.string().uuid().parse(req.params.id);
        const commentId = z.string().uuid().parse(req.params.commentId);
        const evidence = await getEvidenceWithReadAccess(userId, id);
    ...
    app.patch("/v1/evidence/:id/legal-notes/:noteId", { preHandler: requireAuth }, async (req, reply) => {
        const userId = getAuthUserId(req);
        const id = z.string().uuid().parse(req.params.id);
        ...
        const canManage = note.authorUserId === userId ||
            (await canManageEvidenceCollaborativeContent(userId, evidence));
        if (!canManage) {
            return reply.code(403).send({ message: "Forbidden" });
        }
    ...
    app.delete("/v1/evidence/:id/legal-notes/:noteId", { preHandler: requireAuth }, async (req, reply) => {
        const userId = getAuthUserId(req);
        const id = z.string().uuid().parse(req.params.id);
        const noteId = z.string().uuid().parse(req.params.noteId);
        const evidence = await getEvidenceWithReadAccess(userId, id);
    ...
    app.patch("/v1/evidence/:id/annotations/:annotationId", { preHandler: requireAuth }, async (req, reply) => {
        const userId = getAuthUserId(req);
        const id = z.string().uuid().parse(req.params.id);
        const annotationId = z
            .string()
            .uuid()
            .parse(req.params.annotationId);
        ...
        const canManage = annotation.authorUserId === userId ||
            (await canManageEvidenceCollaborativeContent(userId, evidence));
        if (!canManage) {
            return reply.code(403).send({ message: "Forbidden" });
        }
    ...
    app.delete("/v1/evidence/:id/annotations/:annotationId", { preHandler: requireAuth }, async (req, reply) => {
        const userId = getAuthUserId(req);
        const id = z.string().uuid().parse(req.params.id);
        const annotationId = z
            .string()
    ...
    app.get("/v1/evidence/:id/duplicates", { preHandler: requireAuth }, async (req, reply) => {
        ...
        const partHashValues = partHashes.map((part) => part.sha256).filter((value) => Boolean(value));
        const [exactHashMatches, fingerprintMatches, partHashMatches, possibleMetadataMatches] = await Promise.all([
            evidence.fileSha256
        ...
        return reply.code(200).send({
            ...
            limitation: "Duplicate detection is limited to accessible records and recorded hashes or metadata.",
        });
    });
    app.get("/v1/evidence/:id/reviewer-workflow", { preHandler: requireAuth }, async (req, reply) => {
        const userId = getAuthUserId(req);
        const id = z.string().uuid().parse(req.params.id);
        await getEvidenceWithReadAccess(userId, id);
        return reply.code(200).send(await getEvidenceReviewerWorkflowSummary(id));
    });
    app.patch("/v1/evidence/:id/reviewer-workflow", { preHandler: requireAuth }, async (req, reply) => {
        const userId = getAuthUserId(req);
        const id = z.string().uuid().parse(req.params.id);
        const body = ReviewerWorkflowUpdateBody.parse(req.body);
        const evidence = await getEvidenceWithReadAccess(userId, id);
        if (body.assignedToUserId) {
            const targetUserAccessible = evidence.ownerUserId === body.assignedToUserId ||
                (evidence.caseId
    ...
    app.get("/v1/evidence/:id/external-intake-summary", { preHandler: requireAuth }, async (req, reply) => {
        const userId = getAuthUserId(req);
        const id = z.string().uuid().parse(req.params.id);
        await getEvidenceWithReadAccess(userId, id);
        const { loadExternalIntakeSourceSummary } = await import("../services/external-intake-source-summary.service.js");
        const summary = await loadExternalIntakeSourceSummary(id);
        return reply.code(200).send({ summary });
    });
    app.get("/v1/evidence/:id/reviewer-workflow/events", { preHandler: requireAuth }, async (req, reply) => {
        const userId = getAuthUserId(req);
        const id = z.string().uuid().parse(req.params.id);
        await getEvidenceWithReadAccess(userId, id);
    ...
    app.get("/v1/evidence/:id/relationships", { preHandler: requireAuth }, async (req, reply) => {
        const userId = getAuthUserId(req);
        const id = z.string().uuid().parse(req.params.id);
        await getEvidenceWithReadAccess(userId, id);
        return reply.code(200).send({
    ...
    app.post("/v1/evidence/:id/relationships", { preHandler: requireAuth }, async (req, reply) => {
        const userId = getAuthUserId(req);
        const id = z.string().uuid().parse(req.params.id);
    ...
    app.patch("/v1/evidence/:id/relationships/:relationshipId", { preHandler: requireAuth }, async (req, reply) => {
        const userId = getAuthUserId(req);
        const id = z.string().uuid().parse(req.params.id);
        const relationshipId = z
            .string()
            .uuid()
            .parse(req.params.relationshipId);
    ...
    app.delete("/v1/evidence/:id/relationships/:relationshipId", { preHandler: requireAuth }, async (req, reply) => {
        const userId = getAuthUserId(req);
        const id = z.string().uuid().parse(req.params.id);
        const relationshipId = z
            .string()
    ...
    app.get("/v1/evidence/:id/reviewer-audit", { preHandler: requireAuth }, async (req, reply) => {
        const userId = getAuthUserId(req);
        const id = z.string().uuid().parse(req.params.id);
        await getEvidenceWithReadAccess(userId, id);
    ...
    app.get("/v1/evidence/:id/artifacts", { preHandler: requireAuth }, async (req, reply) => {
        const userId = getAuthUserId(req);
        const id = z.string().uuid().parse(req.params.id);
        await getEvidenceWithReadAccess(userId, id);
        return reply.code(200).send(await listEvidenceArtifacts(id));
    });
    app.get("/v1/evidence/:id/ai-categorization", { preHandler: requireAuth }, async (req, reply) => {
        const userId = getAuthUserId(req);
        const id = z.string().uuid().parse(req.params.id);
        await getEvidenceWithReadAccess(userId, id);
    ...
    app.post("/v1/evidence/:id/ai-categorization/run", { preHandler: requireAuth }, async (req, reply) => {
        const userId = getAuthUserId(req);
        const id = z.string().uuid().parse(req.params.id);
        const evidence = await getEvidenceWithReadAccess(userId, id);
        ...
        const metadataPayload = {
            ...
            verificationPackageReady: Boolean(evidence.verificationPackageVersion || evidence.verificationPackageGeneratedAtUtc),
            caseLinked: Boolean(evidence.caseId),
            workspaceLabel: evidence.workspaceNameSnapshot ?? null,
            checklistMetadataOnly: true,
        };
        const aiResult = await evidenceAiProvider.run(AiTask.EVIDENCE_METADATA_CATEGORIZATION, metadataPayload);
        ...
        const persisted = await prisma.evidenceAiCategorization.create({
            data: {
                ...
                categoriesJson: deterministicCategories,
                suggestedTagsJson: suggestedTags,
                riskFlagsJson: riskFlags,
                summary: aiResult.summary,
                legalDisclaimer: AI_LEGAL_DISCLAIMER,
                model: aiResult.status === "disabled"
                    ? null
    ...
    app.get("/v1/evidence/:id/review-workspace", { preHandler: requireAuth }, async (req, reply) => {
        const id = z.string().uuid().parse(req.params.id);
        const ownerUserId = getAuthUserId(req);
        req.evidenceId = id;
        req.log = req.log.child({ evidenceId: id });
        try {
            ...
            if (!signingKey) {
                ...
                return reply
                    ...
                    message: "The signing key referenced by this evidence record is not registered. Re-run `pnpm prisma:seed` against this environment, or contact support.",
                });
            }
            const [itemCount, overview, storage, anchor, parts, allCustodyEvents, latestReport, latestVerificationPackage, caseItem, publicVerifyCount, lastPublicVerify, authenticatedVerifyCount, reportDownloadCount, verificationPackageDownloadCount,] = await Promise.all([
            ...
            const forensicCustodyEvents = allCustodyEvents.filter((ev) => classifyCustodyEventType(ev.eventType) === "forensic");
            const accessCustodyEvents = allCustodyEvents.filter((ev) => classifyCustodyEventType(ev.eventType) === "access");
            const persistedVerificationPackageMetadata = isVerificationPackageMetadata(evidence.verificationPackageMetadata)
                ? evidence.verificationPackageMetadata
                : null;
            let verificationPackageIntegrity;
            if (persistedVerificationPackageMetadata) {
                verificationPackageIntegrity = {
                    ...
                    signedManifestPresent: persistedVerificationPackageMetadata.signedManifestPresent,
                    manifestDigestPresent: persistedVerificationPackageMetadata.signedManifestPresent,
                    checksumIndexPresent: persistedVerificationPackageMetadata.checksumIndexPresent,
                    offlineVerifierIncluded: persistedVerificationPackageMetadata.offlineVerifierIncluded,
                    auditExportIncluded: persistedVerificationPackageMetadata.auditExportIncluded ?? false,
                    custodyExportIncluded: persistedVerificationPackageMetadata.custodyExportIncluded ??
                        false,
                    accessExportIncluded: persistedVerificationPackageMetadata.accessExportIncluded ?? false,
                };
            }
            else if (latestVerificationPackage) {
                const inspectedArtifacts = await inspectVerificationPackageArtifacts(latestVerificationPackage.storageBucket, latestVerificationPackage.storageKey);
                verificationPackageIntegrity = {
                    available: true,
                    version: latestVerificationPackage.version,
                    generatedAtUtc: latestVerificationPackage.generatedAtUtc.toISOString(),
                    packageType: latestVerificationPackage.packageType ?? null,
                    manifestPresent: inspectedArtifacts?.manifestPresent ?? false,
                    signedManifestPresent: inspectedArtifacts?.signedManifestPresent ?? false,
                    manifestDigestPresent: inspectedArtifacts?.manifestDigestPresent ?? false,
                    checksumIndexPresent: inspectedArtifacts?.checksumIndexPresent ?? false,
                    offlineVerifierIncluded: inspectedArtifacts?.offlineVerifierIncluded ?? false,
                    auditExportIncluded: inspectedArtifacts?.auditExportIncluded ?? false,
                    custodyExportIncluded: inspectedArtifacts?.custodyExportIncluded ?? false,
                    accessExportIncluded: inspectedArtifacts?.accessExportIncluded ?? false,
                };
            }
            else {
                verificationPackageIntegrity = {
                ...
                for (const item of latestReport.embeddedPreviewsSnapshot) {
                    if (item &&
                        ...
                        reportPreviewMap.set(item.id, {
                            previewDataUrl: "previewDataUrl" in item &&
                                typeof item.previewDataUrl === "string"
                                ? item.previewDataUrl
                                : null,
                            previewTextExcerpt: "previewTextExcerpt" in item &&
                                typeof item.previewTextExcerpt === "string"
                                ? item.previewTextExcerpt
                                : null,
                            previewCaption: "previewCaption" in item &&
            ...
            const authenticatedContentAccessPolicy = resolveEvidenceContentAccessPolicyForSurface({
            ...
            const privateNoteByPartId = new Map(parts.map((part) => [part.id, part.privateNote ?? null]));
            const contentItems = content.items.map((item) => ({
                ...item,
                privateNote: privateNoteByPartId.get(item.id) ?? null,
            }));
            const defaultPreviewItem = contentItems.find((item) => item.previewable && item.viewUrl) ??
            ...
            const recomputedFingerprintHash = evidence.fingerprintCanonicalJson
                ...
                : null;
            const canonicalHashMatches = Boolean(recomputedFingerprintHash) &&
            ...
            const normalizedTsaStatus = String(evidence.tsaStatus ?? "")
                .trim()
                .toUpperCase();
            const timestampInputDigestHex = evidence.tsaInputDigestHex ?? evidence.fileSha256;
            const timestampStatusIsPositive = normalizedTsaStatus === "STAMPED" ||
                normalizedTsaStatus === "GRANTED" ||
                normalizedTsaStatus === "VERIFIED" ||
                normalizedTsaStatus === "SUCCEEDED";
            const timestampStatusIsUnavailable = normalizedTsaStatus === "FAILED" ||
                normalizedTsaStatus === "UNAVAILABLE" ||
                normalizedTsaStatus === "ERROR" ||
                normalizedTsaStatus.length === 0;
            const timestampDigestMatches = timestampStatusIsPositive
            ...
            const effectiveOtsAnchoredAtUtc = effectiveOtsStatus === "ANCHORED" ? evidence.otsAnchoredAtUtc : null;
            const otsHashMatches = evidence.otsHash && evidence.fingerprintHash
            ...
            const snapshotTrustDecision = normalizeTrustDecisionSnapshot(latestReport?.trustDecisionSnapshot) ??
                normalizeTrustDecisionSnapshot(latestVerificationPackage?.trustDecisionSnapshot) ??
                null;
            const liveTrustDecision = buildEvidenceTrustDecision({
                evidence: {
                    verificationStatus: evidence.verificationStatus ?? null,
                    recordedIntegrityVerifiedAtUtc: evidence.recordedIntegrityVerifiedAtUtc?.toISOString() ?? null,
                    fileSha256: evidence.fileSha256 ?? null,
                    fingerprintHash: evidence.fingerprintHash ?? null,
                    ...
                    verificationPackageVersion: latestVerificationPackage?.version ??
                        evidence.verificationPackageVersion ??
                        null,
                    verificationPackageGeneratedAtUtc: latestVerificationPackage?.generatedAtUtc?.toISOString() ??
            ...
            const mappedForensicEvents = forensicCustodyEvents.map((event) => mapPublicCustodyEvent(event, custodyDisplayContext));
            const mappedAccessEvents = accessCustodyEvents.map((event) => mapPublicCustodyEvent(event, custodyDisplayContext));
            const custodyLifecycle = buildPublicCustodyLifecycle({
                forensicEvents: mappedForensicEvents,
                accessEvents: mappedAccessEvents,
            });
            const reportGeneratedAtUtc = latestReport?.generatedAtUtc ?? evidence.reportGeneratedAtUtc ?? null;
            ...
            const accessEventsAfterReportGeneration = reportGeneratedAtUtc
                ...
                : accessCustodyEvents;
            const snapshotGeneratedAtUtc = trustDecisionConsistencySource === "REPORT_SNAPSHOT"
            ...
            const trustDecisionConsistency = buildTrustDecisionConsistency({
                ...
                latestReportGeneratedAtUtc: latestReport?.generatedAtUtc ?? evidence.reportGeneratedAtUtc ?? null,
                latestReportVersion: latestReport?.version ?? evidence.latestReportVersion ?? null,
                latestVerificationPackageGeneratedAtUtc: latestVerificationPackage?.generatedAtUtc ??
                    evidence.verificationPackageGeneratedAtUtc ??
                    null,
                latestVerificationPackageVersion: latestVerificationPackage?.version ??
                    evidence.verificationPackageVersion ??
                    null,
            ...
            const reviewDecision = evidenceIntelligence?.reviewerDecision ?? {
            ...
            return reply.code(200).send(toJsonSafe({
                evidence: {
                    ...toSafeEvidence(evidence),
                    itemCount,
                    display,
                    ...
                    primaryContentLabel: buildPrimaryContentLabel(content.summary.primaryKind),
                ...
                preservationMatrix: {
                    verificationStatus: evidence.verificationStatus ?? null,
                    verificationStatusLabel: mapVerificationStatusLabel(evidence.verificationStatus),
                    recordedIntegrityVerifiedAtUtc: evidence.recordedIntegrityVerifiedAtUtc?.toISOString() ?? null,
                    ...
                    ots: {
                        ...
                        anchoredAtUtc: effectiveOtsAnchoredAtUtc?.toISOString() ?? null,
                ...
                relationships: {
                    ...
                    itemCount: content.summary.itemCount,
                    note: !evidence.caseId && relationshipItems.length === 0
                ...
                classification: {
                    ...
                    intakeTemplate: typeof evidence.intakePlanJson === "object" &&
                        evidence.intakePlanJson &&
                        "selectedPlanId" in
                            evidence.intakePlanJson
                        ? evidence.intakePlanJson.selectedPlanId ?? null
                        : null,
                ...
                integrityDrift: {
                    ...
                    titleDiffersFromReportSnapshot: Boolean(latestReport?.displayTitleSnapshot) &&
                        latestReport?.displayTitleSnapshot !== display.displayTitle,
                    itemCountDiffersFromReportSnapshot: typeof latestReport?.itemCountSnapshot === "number" &&
                        latestReport.itemCountSnapshot !== itemCount,
                    postReportForensicEvents: forensicCustodyEvents.length -
                ...
                snapshot: {
                    ...
                    statusAtReportGeneration: latestReport?.verificationStatusSnapshot ?? null,
                    fixedArtifactNote: "PDF report is a fixed generated artifact. Public verification may show current verification or access state depending on implementation.",
                },
                ...
                artifactVersions: {
                    ...
                    latestReport: latestReport
                        ...
                        : {
                            ...
                            generatedAtUtc: evidence.reportGeneratedAtUtc?.toISOString() ?? null,
                        },
                    latestVerificationPackage: latestVerificationPackage
                        ...
                        : {
                            ...
                            packageType: null,
                            generatedAtUtc: evidence.verificationPackageGeneratedAtUtc?.toISOString() ??
                                null,
                        },
                ...
                parts: parts.map((part) => ({
                    ...
                    storage: getStorageProtectionSummaryFromSnapshot({
                        ...
                        storageObjectLockRetainUntilUtc: part.storageObjectLockRetainUntilUtc,
                        storageObjectLockLegalHoldStatus: part.storageObjectLockLegalHoldStatus,
                    }),
                    uploadedAtUtc: part.uploadedAtUtc?.toISOString() ?? null,
                    createdAt: part.createdAt.toISOString(),
                })),
                legalBoundary: "PROOVRA verifies the recorded integrity state of evidence records and supporting technical materials. It does not independently establish factual truth, authorship, identity, legal admissibility, or evidentiary weight.",
            }));
        }
        catch (err) {
            const statusCode = err instanceof Error && "statusCode" in err
    ...
    app.get("/v1/evidence/:id", { preHandler: requireAuth }, async (req, reply) => {
        ...
        try {
            ...
            const defaultPreviewItem = content.items.find((item) => item.previewable && item.viewUrl) ??
    ...
    app.post("/v1/evidence/:id/complete", { preHandler: requireAuth }, async (req, reply) => {
        const ownerUserId = getAuthUserId(req);
        const id = z.string().uuid().parse(req.params.id);
        req.evidenceId = id;
        req.log = req.log.child({ evidenceId: id });
        ...
        if (completionEvidence?.teamId) {
            const { enforceSensitiveAction, evidenceIsReviewed } = await import("../services/governance.service.js");
            const membership = await prisma.teamMember.findUnique({
                where: {
                    teamId_userId: {
        ...
        try {
            ...
            try {
                ...
                req.log?.warn?.({ err: workflowErr, evidenceId: id }, "capture_finalize_workflow_init_failed");
            }
            ...
            const storage = await getStorageProtectionSummary(refreshed.storageBucket, refreshed.storageKey, {
                storageRegion: refreshed.storageRegion,
                storageObjectLockMode: refreshed.storageObjectLockMode,
                storageObjectLockRetainUntilUtc: refreshed.storageObjectLockRetainUntilUtc,
                storageObjectLockLegalHoldStatus: refreshed.storageObjectLockLegalHoldStatus,
            });
            const itemCount = await getEvidenceItemCount(id);
            const parts = await prisma.evidencePart.findMany({
                where: { evidenceId: id },
            ...
            if (err instanceof Error &&
                ...
                return reply.code(statusCode).send({
                    ...
                    reason,
                    message: "Evidence cannot be finalized until the resumable upload session is server-verified.",
                });
            }
            if (err instanceof Error &&
                "code" in err &&
                ...
                const payload = await buildStorageLimitPayload({
                    ...
                    incomingBytes: typeof details?.incomingBytes === "string"
     ...
     */
    app.get("/v1/evidence/:id/artifacts/status", { preHandler: requireAuth }, async (req, reply) => {
        const ownerUserId = getAuthUserId(req);
        const id = z.string().uuid().parse(req.params.id);
        req.evidenceId = id;
        req.log = req.log.child({ evidenceId: id });
        ...
        const artifactStatus = await buildEvidenceArtifactStatus({
            ...
            evidenceVerificationPackageMetadata: evidenceRecord.verificationPackageMetadata ?? null,
        });
     ...
     */
    app.post("/v1/evidence/:id/reports/regenerate", { preHandler: requireAuth }, async (req, reply) => {
        const userId = getAuthUserId(req);
        const id = z.string().uuid().parse(req.params.id);
        req.evidenceId = id;
        req.log = req.log.child({ evidenceId: id });
        ...
        if (evidenceRecord.status ===
            ...
            return reply.code(409).send({
                ...
                message: "Report regeneration is not available for this record. The recomputed SHA-256 fingerprint did not match the value recorded at completion. Re-upload or recapture the source material as a new evidence record.",
            });
        }
    ...
    app.get("/v1/evidence/:id/report/latest", { preHandler: requireAuth }, async (req, reply) => {
        const ownerUserId = getAuthUserId(req);
        const id = z.string().uuid().parse(req.params.id);
        req.evidenceId = id;
        req.log = req.log.child({ evidenceId: id });
        // Phase O1.5A — bounded evidence.report.latest span.
        await withProovraSpan(PROOVRA_SPAN_NAMES.EVIDENCE_REPORT_LATEST, {
            "proovra.evidence_id": id,
            ...
            if (evidenceForGate?.teamId) {
                ...
                if (!decision.allowed) {
                    ...
                    return reply
                        ...
                        code: decision.code,
                        reason: decision.reason,
                        message: "Report download is blocked by workspace governance policy.",
                    });
                }
            }
        }
        ...
        const storage = await getStorageProtectionSummary(latest.storageBucket, latest.storageKey, {
            storageRegion: latest.storageRegion,
            storageObjectLockMode: latest.storageObjectLockMode,
            storageObjectLockRetainUntilUtc: latest.storageObjectLockRetainUntilUtc,
            storageObjectLockLegalHoldStatus: latest.storageObjectLockLegalHoldStatus,
        });
        return reply.code(200).send({
            evidenceId: id,
            ...
            snapshots: {
                ...
                verificationStatusLabel: mapVerificationStatusLabel(latest.verificationStatusSnapshot),
                identityLevel: latest.identityLevelSnapshot ?? null,
                identityLevelLabel: mapIdentityLevelLabel(latest.identityLevelSnapshot),
                submittedByEmail: latest.submittedByEmailSnapshot ?? null,
                submittedByAuthProvider: latest.submittedByAuthProviderSnapshot ?? null,
                submittedByAuthProviderLabel: mapAuthProviderLabel(latest.submittedByAuthProviderSnapshot),
                captureMethod: latest.captureMethodSnapshot ?? null,
                captureMethodLabel: mapCaptureMethodLabel(latest.captureMethodSnapshot),
    ...
    app.get("/v1/evidence/:id/original", { preHandler: requireAuth }, async (req, reply) => {
        const ownerUserId = getAuthUserId(req);
        const id = z.string().uuid().parse(req.params.id);
        req.evidenceId = id;
        req.log = req.log.child({ evidenceId: id });
        ...
        if (evidence.teamId) {
            ...
            if (!decision.allowed) {
                ...
                return reply
                    ...
                    code: decision.code,
                    reason: decision.reason,
                    message: "Original file download is blocked by workspace governance policy.",
                });
            }
        }
        const url = await presignGetObject({
        ...
        const matchingPrimaryPart = evidence.storageBucket && evidence.storageKey
        ...
        const resolvedOriginalFileName = cleanOptionalText(evidence.originalFileName) ??
            cleanOptionalText(matchingPrimaryPart?.originalFileName) ??
            null;
        const resolvedDisplayName = cleanOptionalText(evidence.displayFileName) ??
            resolveOriginalAssetDisplayName({
        ...
        return reply.code(200).send({
            ...
            displaySizeLabel: formatBytesForDisplay(evidence.sizeBytes?.toString() ?? null),
            lastAccessedByUserId: ownerUserId,
            lastAccessedAtUtc: accessedAt.toISOString(),
            storage,
        });
    });
    app.get("/v1/evidence/:id/verification-package", { preHandler: requireAuth }, async (req, reply) => {
        const ownerUserId = getAuthUserId(req);
        const id = z.string().uuid().parse(req.params.id);
        req.evidenceId = id;
        req.log = req.log.child({ evidenceId: id });
        // Phase O1.5A — bounded evidence.package.status span.
        await withProovraSpan(PROOVRA_SPAN_NAMES.EVIDENCE_PACKAGE_STATUS, {
            "proovra.evidence_id": id,
            ...
            if (evidenceForGate?.teamId) {
                ...
                if (!decision.allowed) {
                    ...
                    return reply
                        ...
                        code: decision.code,
                        reason: decision.reason,
                        message: "Verification package download is blocked by workspace governance policy.",
                    });
                }
                ...
                const { gateVerificationAction } = await import("../services/governance/policy-runtime-gates.service.js");
                ...
                if (!verifyGate.ok) {
                    ...
                    return reply.code(403).send({
                        ...
                        denial: verifyGate.denial,
                        reason: verifyGate.reason,
                        message: "Verification package publish is blocked by workspace verification policy.",
                    });
                }
            }
        }
        ...
        if (!latest) {
            ...
            const finalized = evidenceForState?.status === prismaPkg.EvidenceStatus.SIGNED ||
                evidenceForState?.status === prismaPkg.EvidenceStatus.REPORTED;
            const meta = evidenceForState?.verificationPackageMetadata;
            const blocked = meta != null && typeof meta === "object" && meta.blocked === true;
            ...
            if (blocked) {
                return reply.code(409).send({
                    ...
                    blockedAtUtc: typeof meta?.blockedAtUtc === "string" ? meta.blockedAtUtc : null,
                    message: "Verification package generation was blocked by governance policy.",
                });
            }
            if (finalized) {
                ...
                return reply.code(202).send({
                    ...
                    message: "Verification package is being generated. Poll /v1/evidence/:id/artifacts/status for completion.",
                });
            }
            return reply
                .code(404)
    ...
    app.get("/v1/evidence/:id/certifications", { preHandler: requireAuth }, async (req, reply) => {
        const ownerUserId = getAuthUserId(req);
        const id = z.string().uuid().parse(req.params.id);
        req.evidenceId = id;
        req.log = req.log.child({ evidenceId: id });
    ...
    app.post("/v1/evidence/:id/certifications/request", { preHandler: requireAuth }, async (req, reply) => {
        const ownerUserId = getAuthUserId(req);
        const id = z.string().uuid().parse(req.params.id);
    ...
    app.post("/v1/evidence/:id/certifications/revoke", { preHandler: requireAuth }, async (req, reply) => {
        const ownerUserId = getAuthUserId(req);
        const id = z.string().uuid().parse(req.params.id);
    ...
    app.get("/public/verify/:id", async (req, reply) => {
        ...
        // attribute set only.
        await withProovraSpan(PROOVRA_SPAN_NAMES.EVIDENCE_VERIFY_PUBLIC, { "proovra.operation": "evidence_verify_public" }, () => undefined);
        ...
        if (!ipRate.allowed) {
            const retryAfter = Math.max(1, Math.ceil((ipRate.resetAtMs - Date.now()) / 1000));
            req.log.warn({
                ip: req.ip,
                bucket: "ip",
                remaining: 0,
        ...
        if (!perEvidenceRate.allowed) {
            const retryAfter = Math.max(1, Math.ceil((perEvidenceRate.resetAtMs - Date.now()) / 1000));
            req.log.warn({
                ip: req.ip,
                evidenceId: id,
                bucket: "evidence",
        ...
        }
        {
            ...
            if (evidenceStatus === prismaPkg.EvidenceStatus.FAILED_HASH_MISMATCH) {
                auditVerificationAction(req, {
                    userId: null,
                    action: "verification.page_opened",
                    resourceId: id,
            ...
            const isFinalized = evidenceStatus === EvidenceStatus.SIGNED ||
            ...
            if (!isFinalized) {
                ...
                return reply.code(409).send({
                    ...
                    message: "This evidence record has not been finalized yet. A verification response is not available for pre-finalized records.",
                    status: evidenceStatus ?? null,
                });
            }
        }
        const [latestCustodianCertification, latestQualifiedPersonCertification] = await Promise.all([
        ...
        const publicCertifications = {
            custodian: latestCustodianCertification
                ? {
                    ...
                    requestedAtUtc: latestCustodianCertification.requestedAtUtc?.toISOString() ?? null,
                    attestedAtUtc: latestCustodianCertification.attestedAtUtc?.toISOString() ?? null,
                    attestorName: latestCustodianCertification.attestorName,
                    attestorTitle: latestCustodianCertification.attestorTitle,
                    attestorOrganization: latestCustodianCertification.attestorOrganization,
                    certificationHash: latestCustodianCertification.certificationHash,
                    revokedAtUtc: latestCustodianCertification.revokedAtUtc?.toISOString() ?? null,
            ...
            qualifiedPerson: latestQualifiedPersonCertification
                ? {
                    ...
                    requestedAtUtc: latestQualifiedPersonCertification.requestedAtUtc?.toISOString() ?? null,
                    attestedAtUtc: latestQualifiedPersonCertification.attestedAtUtc?.toISOString() ?? null,
                    ...
                    attestorOrganization: latestQualifiedPersonCertification.attestorOrganization,
                    certificationHash: latestQualifiedPersonCertification.certificationHash,
                    revokedAtUtc: latestQualifiedPersonCertification.revokedAtUtc?.toISOString() ?? null,
        ...
        if (!signingKey) {
            ...
            return reply
                ...
                message: "Verification is temporarily unavailable. Please retry in a few minutes.",
            });
        }
        ...
        if (persistedVerificationPackageMetadata) {
            verificationPackageIntegrity = {
                ...
                offlineVerifierIncluded: persistedVerificationPackageMetadata.offlineVerifierIncluded,
                auditExportIncluded: persistedVerificationPackageMetadata.auditExportIncluded ?? false,
                custodyExportIncluded: persistedVerificationPackageMetadata.custodyExportIncluded ?? false,
                accessExportIncluded: persistedVerificationPackageMetadata.accessExportIncluded ?? false,
            };
        }
        else if (verificationPackageAvailable) {
            const inspectedVerificationPackageArtifacts = await inspectVerificationPackageArtifacts(latestVerificationPackage?.storageBucket ?? null, latestVerificationPackage?.storageKey ?? null);
            verificationPackageIntegrity = {
                available: true,
                version: latestVerificationPackage?.version ?? null,
                generatedAtUtc: latestVerificationPackage?.generatedAtUtc
                    ...
                    : null,
                packageType: latestVerificationPackage?.packageType ?? null,
                manifestPresent: inspectedVerificationPackageArtifacts?.manifestPresent ?? false,
                signedManifestPresent: inspectedVerificationPackageArtifacts?.signedManifestPresent ?? false,
                manifestDigestPresent: inspectedVerificationPackageArtifacts?.manifestDigestPresent ?? false,
                checksumIndexPresent: inspectedVerificationPackageArtifacts?.checksumIndexPresent ?? false,
                offlineVerifierIncluded: inspectedVerificationPackageArtifacts?.offlineVerifierIncluded ?? false,
                auditExportIncluded: inspectedVerificationPackageArtifacts?.auditExportIncluded ?? false,
                custodyExportIncluded: inspectedVerificationPackageArtifacts?.custodyExportIncluded ?? false,
                accessExportIncluded: inspectedVerificationPackageArtifacts?.accessExportIncluded ?? false,
            };
            if (inspectedVerificationPackageArtifacts) {
                try {
                    await prisma.evidence.update({
                        ...
                        data: {
                            verificationPackageMetadata: {
                                manifestPresent: inspectedVerificationPackageArtifacts.manifestPresent,
                                signedManifestPresent: inspectedVerificationPackageArtifacts.signedManifestPresent,
                                checksumIndexPresent: inspectedVerificationPackageArtifacts.checksumIndexPresent,
                                offlineVerifierIncluded: inspectedVerificationPackageArtifacts.offlineVerifierIncluded,
                                auditExportIncluded: inspectedVerificationPackageArtifacts.auditExportIncluded,
                                custodyExportIncluded: inspectedVerificationPackageArtifacts.custodyExportIncluded,
                                accessExportIncluded: inspectedVerificationPackageArtifacts.accessExportIncluded,
                                packageVersion: "v1",
                                generatedAtUtc: evidence.verificationPackageGeneratedAtUtc?.toISOString() ??
                                    latestVerificationPackage?.generatedAtUtc?.toISOString() ??
                    ...
                    console.warn("Unable to backfill verification package metadata after ZIP inspection:", updateError);
                }
            }
        }
        else {
        ...
        if (Array.isArray(latestReport?.embeddedPreviewsSnapshot)) {
            for (const item of latestReport.embeddedPreviewsSnapshot) {
                if (item &&
                    ...
                    typeof item.id === "string") {
                    reportPreviewMap.set(item.id, {
                        previewDataUrl: "previewDataUrl" in item && typeof item.previewDataUrl === "string"
                            ? item.previewDataUrl
                            : null,
                        previewTextExcerpt: "previewTextExcerpt" in item &&
                            typeof item.previewTextExcerpt === "string"
                            ? item.previewTextExcerpt
                            : null,
                        previewCaption: "previewCaption" in item && typeof item.previewCaption === "string"
                            ? item.previewCaption
                            : null,
                    });
                }
        ...
        const recomputedFingerprintHash = sha256Hex(evidence.fingerprintCanonicalJson);
        const canonicalHashMatches = recomputedFingerprintHash === evidence.fingerprintHash;
        let signatureValid = false;
        try {
        ...
        const storageProtection = await getStorageProtectionSummary(evidence.storageBucket, evidence.storageKey, {
            storageRegion: evidence.storageRegion,
            storageObjectLockMode: evidence.storageObjectLockMode,
            storageObjectLockRetainUntilUtc: evidence.storageObjectLockRetainUntilUtc,
            storageObjectLockLegalHoldStatus: evidence.storageObjectLockLegalHoldStatus,
        ...
        const overallIntegrity = canonicalHashMatches &&
            signatureValid &&
            custodyChain.valid &&
        ...
        const evidenceStatusForGate = evidence.status;
        const isFinalizedForVerify = evidenceStatusForGate === EvidenceStatus.SIGNED ||
        ...
        if (isFinalizedForVerify) {
            ...
            const shouldEmitVerifyViewed = previousPublicViewAt === null ||
            ...
            void (async () => {
                ...
                if (failures.length > 0) {
                    for (const { i, r } of failures) {
                        ...
                        req.log.warn({
                            evidenceId: id,
                            err: r.reason instanceof Error
                                ? r.reason.message.slice(0, 200)
                                : String(r.reason).slice(0, 200),
                            code: r.reason instanceof Error && "code" in r.reason
                                ? r.reason.code ?? null
                                : null,
                            surface: which,
                ...
                try {
                    ...
                    req.log.warn({
                        evidenceId: id,
                        err: custodyErr instanceof Error
        ...
        const overview = buildPublicVerifyOverview({
            evidence: {
                ...
                organizationNameSnapshot: identityExposure.exposeAttribution
                    ...
                    : null,
                organizationVerifiedSnapshot: evidence.organizationVerifiedSnapshot ?? null,
                mimeType: evidence.mimeType,
                createdAt: evidence.createdAt,
                capturedAtUtc: evidence.capturedAtUtc,
                ...
                lastPublicVerifyViewAtUtc: evidence.lastPublicVerifyViewAtUtc ?? null,
        ...
        const custodyDisplayCounts = {
            ...
            accessAfterReportGeneration: accessEventsAfterReportGeneration.length,
            currentAccessEvents: accessCustodyEvents.length,
            totalDisplayedEvents: forensicCustodyEvents.length + accessCustodyEvents.length,
            totalDisplayedNow: forensicCustodyEvents.length + accessCustodyEvents.length,
            reportGeneratedAtUtc: reportGeneratedAtUtc
                ? reportGeneratedAtUtc.toISOString()
                : null,
        };
        ...
        const mediaIntelligenceAdvisory = evidence.teamId
            ? await (async () => {
                try {
                    const { projectVerifyMediaIntelligence } = await import("../services/media-intelligence/verify-projection.service.js");
                    return await projectVerifyMediaIntelligence({ teamId: evidence.teamId, evidenceId: evidence.id });
                }
                catch {
                    return null;
                }
        ...
        const captureTrust = evidence.teamId
            ? await (async () => {
                try {
                    const { projectVerifyCaptureTrust } = await import("../services/capture-trust/verify-trust-projection.service.js");
        ...
        return reply.code(200).send({
            ...
            contentExposureDecision: {
                ...
                rationale: publicVerifyAccessPolicy.mode === "metadata_only"
            ...
            storageAndTimestamping: {
                ...
                tsa: {
                    ...
                    digestMatchesTimestampInput: timestampDigestMatches,
                    digestMatchesFileHash: timestampStatusIsPositive && evidence.fileSha256 && evidence.tsaMessageImprint
                    ...
                    timestampedDigestNote: evidence.tsaInputKind && evidence.tsaInputKind !== "FILE_SHA256"
