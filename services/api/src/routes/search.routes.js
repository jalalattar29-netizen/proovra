 ...
 */
import { z } from "zod";
import { SAVED_VIEW_VISIBILITIES, SearchFilterSchema, SEARCH_DOCUMENT_TYPES, SEARCH_SORT_MODES, } from "@proovra/shared";
...
import { createRelationship, executeSearch, listRelationshipsForEvidence, } from "../services/search/evidence-search.service.js";
import { createSavedView, deleteSavedView, listSavedViewsForUser, } from "../services/search/saved-search.service.js";
import { indexEvidence, indexWorkflowInstance, } from "../services/search/evidence-indexing.service.js";
import { listSearchAudit, recordSearchAudit, } from "../services/search/search-audit.service.js";
...
import { isSemanticReadyAtRuntime, resolveEmbeddingProviderFromEnv, } from "../services/search/embedding-provider.js";
...
function auditSearchAction(req, params) {
 ...
 */
async function requireSearchActor(req, reply, teamId) {
 ...
 */
async function requireSearchOperator(req, reply, teamId) {
...
export async function searchRoutes(app) {
    app.get("/v1/search/evidence", { preHandler: [requireAuthAndLegal] }, async (req) => {
        try {
            ...
            if (error instanceof z.ZodError) {
                ...
                throw new AppError(ErrorCode.VALIDATION_ERROR, "Invalid search parameters", { fields: error.flatten() });
            }
            auditSearchAction(req, {
                userId: req.user?.sub ?? null,
                action: "search.evidence",
    ...
    app.get("/v1/search/cases", { preHandler: [requireAuthAndLegal] }, async (req) => {
        try {
            const querySchema = z.object({
                q: z.string().min(1).max(200).optional(),
                page: z.coerce.number().int().min(1).default(1),
    ...
    app.get("/v1/search/suggest", { preHandler: [requireAuthAndLegal] }, async (req) => {
        try {
            ...
            suggestions.push(...evidence.map((e) => ({
                type: "evidence",
                id: e.id,
    ...
    // -------------------------------------------------------------------------
    app.get("/v1/search", { preHandler: requireAuth }, async (req, reply) => {
        const raw = (req.query ?? {});
        ...
        const candidate = {
            ...
            updatedSinceUtc: typeof raw.updatedSinceUtc === "string" ? raw.updatedSinceUtc : undefined,
            updatedUntilUtc: typeof raw.updatedUntilUtc === "string" ? raw.updatedUntilUtc : undefined,
            ...
            mode: typeof raw.mode === "string"
                ? raw.mode.toUpperCase()
                : undefined,
        };
    ...
    // -------------------------------------------------------------------------
    app.get("/v1/search/saved-views", { preHandler: requireAuth }, async (req, reply) => {
        const q = z
            .object({
    ...
    // -------------------------------------------------------------------------
    app.post("/v1/search/saved-views", { preHandler: requireAuth }, async (req, reply) => {
        const body = z
            .object({
            teamId: z.string().uuid(),
    ...
    // -------------------------------------------------------------------------
    app.delete("/v1/search/saved-views/:id", { preHandler: requireAuth }, async (req, reply) => {
    ...
    // -------------------------------------------------------------------------
    app.get("/v1/search/relationships/:evidenceId", { preHandler: requireAuth }, async (req, reply) => {
        const { evidenceId } = z
    ...
    // -------------------------------------------------------------------------
    app.post("/v1/search/relationships", { preHandler: requireAuth }, async (req, reply) => {
        const body = z
            .object({
            teamId: z.string().uuid(),
    ...
    // -------------------------------------------------------------------------
    app.post("/v1/search/reindex/evidence/:id", { preHandler: requireAuth }, async (req, reply) => {
        const { id } = z.object({ id: z.string().uuid() }).parse(req.params);
        const body = z
    ...
    // -------------------------------------------------------------------------
    app.post("/v1/search/reindex/workflow/:id", { preHandler: requireAuth }, async (req, reply) => {
        const { id } = z.object({ id: z.string().uuid() }).parse(req.params);
        const body = z
            .object({ teamId: z.string().uuid() })
            .parse(req.body ?? {});
    ...
    // -------------------------------------------------------------------------
    app.get("/v1/search/audit", { preHandler: requireAuth }, async (req, reply) => {
        const q = z
            .object({
            teamId: z.string().uuid(),
    ...
    // -------------------------------------------------------------------------
    app.post("/v1/search/semantic/backfill", { preHandler: [requirePlatformAdmin] }, async (req, reply) => {
        const body = z
            .object({
    ...
    app.get("/v1/search/semantic/status", { preHandler: requireAuth }, async (req, reply) => {
        const q = z
            .object({ teamId: z.string().uuid() })
            .parse(req.query ?? {});
        const actor = await requireSearchActor(req, reply, q.teamId);
        ...
        return reply.code(200).send({
            ...
            fallbackReason: enabled && provider.name === "disabled" ? "PROVIDER_UNAVAILABLE" : null,
            usage,
        });
    });
