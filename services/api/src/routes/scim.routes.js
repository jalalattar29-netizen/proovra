 ...
 */
import { z } from "zod";
import { SCIM_USER_SCHEMA_URI, ScimGroupPatchOpSchema, ScimGroupSchema, ScimPatchOpSchema, ScimUserSchema, scimError, } from "@proovra/shared";
import { authenticateScimRequest, scimCreateUser, scimDeactivateUser, scimListUsers, scimReadUser, } from "../services/access-control/scim.service.js";
import { scimCreateGroup, scimDeleteGroup, scimListGroups, scimPatchGroup, scimReadGroup, } from "../services/access-control/scim-groups.service.js";
async function authenticateScim(req, reply, required) {
...
export async function scimRoutes(app) {
    // POST /v2/scim/Users — create
    app.post("/v2/scim/Users", async (req, reply) => {
    ...
    // GET /v2/scim/Users/:id — read
    app.get("/v2/scim/Users/:id", async (req, reply) => {
    ...
    app.get("/v2/scim/Users", async (req, reply) => {
        const ctx = await authenticateScim(req, reply, "users.read");
        if (!ctx)
            return;
        const q = z
    ...
    app.patch("/v2/scim/Users/:id", async (req, reply) => {
        const ctx = await authenticateScim(req, reply, "users.write");
        if (!ctx)
            return;
        const { id } = ParamsUserId.parse(req.params);
        ...
        // 501 Not Implemented.
        const op = parsed.data.Operations.find((o) => o.op === "replace" &&
    ...
    app.delete("/v2/scim/Users/:id", async (req, reply) => {
    ...
    // POST /v2/scim/Groups
    app.post("/v2/scim/Groups", async (req, reply) => {
        const ctx = await authenticateScim(req, reply, "users.write");
        if (!ctx)
            return;
    ...
    app.get("/v2/scim/Groups/:id", async (req, reply) => {
        const ctx = await authenticateScim(req, reply, "users.read");
        if (!ctx)
            return;
    ...
    // GET /v2/scim/Groups
    app.get("/v2/scim/Groups", async (req, reply) => {
        const ctx = await authenticateScim(req, reply, "users.read");
        if (!ctx)
            return;
        const q = z
    ...
    app.patch("/v2/scim/Groups/:id", async (req, reply) => {
        const ctx = await authenticateScim(req, reply, "users.write");
        if (!ctx)
            return;
        const { id } = ParamsGroupId.parse(req.params);
    ...
    app.delete("/v2/scim/Groups/:id", async (req, reply) => {
        const ctx = await authenticateScim(req, reply, "users.deactivate");
        if (!ctx)
            return;
        const { id } = ParamsGroupId.parse(req.params);
