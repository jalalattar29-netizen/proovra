import { z } from "zod";
import { AI_CHAT_LIMITS } from "@proovra/shared";
...
function auditAiAction(req, params) {
 ...
 */
async function bumpAiChatMetric(name) {
    try {
...
function emitAiChatAbuseSignal(req, userId, reason) {
 ...
 */
async function withAiTimeout(promise, ms) {
...
export async function aiRoutes(app) {
    app.post("/v1/ai/chat", { preHandler: [requireAuthAndLegal] }, async (req, reply) => {
        ...
        if (!userRate.allowed) {
            ...
            const retryAfter = Math.max(1, Math.ceil((userRate.resetAtMs - Date.now()) / 1000));
        ...
        try {
            ...
            throw new AppError(ErrorCode.VALIDATION_ERROR, "Invalid AI chat payload");
        }
        // Phase A3 — bounded timeout. The cost guard cannot save us
        // from a provider that simply never responds; this does.
        let result;
        try {
            result = await withAiTimeout(aiChatService.analyzeChat(userId, body), AI_CHAT_LIMITS.REQUEST_TIMEOUT_MS);
        }
        catch (err) {
            const isTimeout = err instanceof Error && err.message === "AI_REQUEST_TIMEOUT";
            if (isTimeout) {
                ...
                return reply.code(504).send({
                    ...
                    message: "AI assistant timed out. Try again in a moment. Evidence workflows are unaffected.",
                });
            }
            throw err;
        }
        auditAiAction(req, {
            ...
            outcome: result.status === "ok"
                ...
                : result.status === "blocked"
                    ? "blocked"
                    : "failure",
            severity: result.status === "error" || result.status === "blocked"
                ? "warning"
                : "info",
            metadata: {
    ...
    app.post("/v1/ai/capture/analyze-session", { preHandler: [requireAuthAndLegal] }, async (req) => {
        const userId = getAuthUserId(req);
    ...
    app.post("/v1/ai/capture/analyze-item", { preHandler: [requireAuthAndLegal] }, async (req) => {
        const userId = getAuthUserId(req);
