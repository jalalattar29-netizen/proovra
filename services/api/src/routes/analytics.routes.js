...
import { ANALYTICS_EVENT_NAMES, ANALYTICS_LIMITS, } from "@proovra/shared";
import { prisma } from "../db.js";
import { requirePlatformAdmin } from "../middleware/require-platform-admin.js";
import { verifyJwt } from "../services/jwt.js";
import { cleanDisplayCity, normalizeCity, normalizeCountryCode, writeAnalyticsEvent, } from "../services/analytics-event.service.js";
...
function auditAdminAnalyticsAction(req, params) {
...
function resolveTrackUserId(request, bodyUserId) {
...
function parseDateRangeKey(raw) {
    if (raw === "24h" || raw === "7d" || raw === "30d")
        return raw;
    return "7d";
}
function rangeWindowMs(key) {
    if (key === "24h")
...
function parseAdminEventFilter(raw) {
    const allowed = [
        "all",
        "page_view",
        "login_completed",
...
function readAdminAnalyticsQuery(request) {
...
function buildAnalyticsWhere(query, overrides) {
    const base = {
...
async function countDistinctSessions(where) {
...
function isTeamBillingActive(status) {
...
async function computeTeamWorkspaceHealth() {
    ...
    const evidenceToTeamId = new Map();
    for (const row of evidenceRows) {
        if (row.teamId) {
            evidenceToTeamId.set(row.id, row.teamId);
        }
    }
    const evidenceBytesByTeam = new Map();
    ...
    const reportBytesByTeam = new Map();
    ...
    const verificationBytesByTeam = new Map();
    ...
    for (const team of teams) {
        if (team.billingStatus === "ACTIVE")
            active += 1;
        else if (team.billingStatus === "PAST_DUE")
            pastDue += 1;
        else if (team.billingStatus === "CANCELED")
            canceled += 1;
        else
            inactive += 1;
        ...
        const storageLimit = team.storageBytesOverride && team.storageBytesOverride > 0n
            ? team.storageBytesOverride
            : caps.includedStorageBytes;
        const usedStorage = (evidenceBytesByTeam.get(team.id) ?? 0n) +
            (reportBytesByTeam.get(team.id) ?? 0n) +
            (verificationBytesByTeam.get(team.id) ?? 0n);
        const storageRatio = storageLimit > 0n ? Number(usedStorage) / Number(storageLimit) : 0;
...
async function getSummary() {
    const [registeredUsers, guestUsers, usersWithEvidenceRows, reportsGenerated, activeAnalyticsUserRows, entitlements, typeGroups, subscriptionsGrouped, paymentsGrouped, teamHealth,] = await Promise.all([
        prisma.user.count({
            where: {
                provider: { not: "GUEST" },
            },
    ...
    for (const row of entitlements) {
        if (row.plan === "FREE")
            subscriptionBreakdown.free = row._count.plan;
        if (row.plan === "PAYG")
            subscriptionBreakdown.payg = row._count.plan;
        if (row.plan === "PRO")
            subscriptionBreakdown.pro = row._count.plan;
        if (row.plan === "TEAM")
            subscriptionBreakdown.team = row._count.plan;
    }
    ...
    for (const row of typeGroups) {
        ...
        if (row.type === "PHOTO")
            evidenceByType.photos += c;
        else if (row.type === "VIDEO")
            evidenceByType.videos += c;
        else if (row.type === "DOCUMENT")
            evidenceByType.documents += c;
        else
            evidenceByType.other += c;
    }
    const billing = {
    ...
    for (const row of subscriptionsGrouped) {
        if (row.status === "ACTIVE")
            billing.activeSubscriptions = row._count.status;
        if (row.status === "TRIALING")
            billing.trialingSubscriptions = row._count.status;
        if (row.status === "PAST_DUE")
            billing.pastDueSubscriptions = row._count.status;
        if (row.status === "CANCELED")
            billing.canceledSubscriptions = row._count.status;
    }
    ...
    return {
        ...
        usersWithEvidence,
        totalEvidence: evidenceByType.photos +
...
async function getTrends(query) {
    const bucketCount = query.dateRangeKey === "24h" ? 1 : query.dateRangeKey === "7d" ? 7 : 30;
...
async function getFunnel(query) {
    const stepsConfig = [
    ...
    const counts = await Promise.all(stepsConfig.map((step) => prisma.analyticsEvent.count({
        where: buildAnalyticsWhere(query, {
    ...
    const steps = stepsConfig.map((step, index) => {
        ...
        const dropOff = index === 0 || conversion === null
            ? null
 ...
 */
const AnalyticsMetadataValueSchema = z.lazy(() => z.union([
    ...
    z
        ...
        .max(16),
    z
        .record(z.string().max(80), z.union([
        z.string().max(ANALYTICS_LIMITS.MAX_STRING),
        z.number().finite(),
        z.boolean(),
        z.null(),
    ]))
        .refine((r) => Object.keys(r).length <= ANALYTICS_LIMITS.MAX_METADATA_KEYS, { message: "metadata_too_large" }),
]));
const AnalyticsTrackSchema = z
    ...
    metadata: AnalyticsMetadataValueSchema.nullish().transform((v) => v ?? null),
})
    .strict();
function classifyAnalyticsValidationFailure(err) {
...
async function bumpAnalyticsMetric(name) {
    try {
...
async function emitAnalyticsRejection(request, reason) {
...
export default async function analyticsRoutes(app) {
    ...
    app.post("/v1/analytics/track", {
        ...
        if (!visitorRate.allowed) {
            ...
            const retryAfter = Math.max(1, Math.ceil((visitorRate.resetAtMs - Date.now()) / 1000));
            reply.header("Retry-After", String(retryAfter));
            return reply
                .code(429)
                .send({ success: false, error: "Rate limit exceeded" });
    ...
    app.get("/v1/admin/analytics/dashboard", ADMIN_PRE, async (request, reply) => {
        try {
            ...
            const [summary, geography, pages, recent, trends, funnel] = await Promise.all([
                getSummary(),
                getGeography(query),
                getPages(query),
                getRecent(query),
