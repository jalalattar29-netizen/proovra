...
import { assertFeatureEntitlement, assertQuotaEntitlement, applyProductLine, listEntitlements, upsertEntitlementGrant, } from "../services/packaging/entitlement.service.js";
import { createExchangePackage, generateSignedUrl, listPackages, recordPackageDelivery, recordPackageDownload, revokePackage, } from "../services/exchange/evidence-exchange.service.js";
import { listDeliveryActivity } from "../services/exchange/signed-delivery.service.js";
import { acceptChainTransfer, initiateChainTransfer, listChainTransfers, rejectChainTransfer, revokeChainTransfer, completeChainTransfer, } from "../services/exchange/chain-transfer.service.js";
import { createWebhookEndpoint, deactivateWebhookEndpoint, listWebhookEndpoints, } from "../services/packaging/webhooks/webhook-platform.service.js";
import { computeUpcomingExpirations, createRetentionPolicy, listRetentionPolicies, releaseRetentionPolicy, } from "../services/lifecycle/retention-engine.service.js";
import { createLegalHold, listLegalHolds, releaseLegalHold, } from "../services/lifecycle/legal-hold.service.js";
import { listArchiveTransitions, projectArchiveCostsByTier, transitionEvidenceTier, } from "../services/lifecycle/archive-tier.service.js";
import { createDestructionRequest, getDestructionCertificate, getDestructionCertificateArtifact, listDestructionRequests, recordApproval, rejectDestructionRequest, executeDestruction, } from "../services/lifecycle/destruction-governance.service.js";
...
import { VERIFICATION_PACKAGE_LIFECYCLE_PREVIEW_KINDS, buildLifecyclePackagePreview, } from "../services/lifecycle/lifecycle-manifest.service.js";
import { POLICY_VIOLATION_CODES, listPolicyViolations, countPolicyViolations, } from "../services/lifecycle/policy-violation.service.js";
import { LEGAL_HOLD_KINDS, PRODUCT_LINES, EXCHANGE_PACKAGE_KINDS, } from "@proovra/shared";
// ---------------------------------------------------------------------------
...
// Route registration
// ---------------------------------------------------------------------------
export async function productAndLifecycleRoutes(app) {
    // =========================================================================
    // Entitlements
    // =========================================================================
    app.get("/v1/packaging/entitlements", { preHandler: requireAuth }, async (req, reply) => {
    ...
    app.post("/v1/packaging/entitlements/apply-product-line", { preHandler: [requireAuth, requireDelegatedTier("ORG_ADMIN")] }, async (req, reply) => {
        const ctx = await resolveWorkspace(req, reply);
        if (!ctx)
            return reply;
        const body = z
    ...
    app.post("/v1/packaging/entitlements/grant", { preHandler: [requireAuth, requireDelegatedTier("ORG_ADMIN")] }, async (req, reply) => {
        const ctx = await resolveWorkspace(req, reply);
        if (!ctx)
            return reply;
        const body = z
            ...
            kind: z.enum(["FEATURE", "QUOTA", "LIMIT"]),
            source: z.enum(["PLAN", "CUSTOM", "PROMO"]).optional(),
    ...
    // Exchange packages
    // =========================================================================
    app.get("/v1/exchange/packages", { preHandler: requireAuth }, async (req, reply) => {
        const ctx = await resolveWorkspace(req, reply);
        if (!ctx)
            return reply;
    ...
    app.post("/v1/exchange/packages", { preHandler: requireAuth }, async (req, reply) => {
        const ctx = await resolveWorkspace(req, reply);
        if (!ctx)
            return reply;
        // Feature gate
    ...
    app.post("/v1/exchange/packages/:id/ready", { preHandler: [requireAuth, requireDelegatedTier("ORG_ADMIN")] }, async (req, reply) => {
        const ctx = await resolveWorkspace(req, reply);
        if (!ctx)
            return reply;
        ...
        const { markPackageReady } = await import("../services/exchange/evidence-exchange.service.js");
    ...
    app.post("/v1/exchange/packages/:id/sign-url", { preHandler: requireAuth }, async (req, reply) => {
        const ctx = await resolveWorkspace(req, reply);
        if (!ctx)
            return reply;
        const { id } = z.object({ id: z.string().uuid() }).parse(req.params);
    ...
    app.post("/v1/exchange/packages/:id/deliveries", { preHandler: requireAuth }, async (req, reply) => {
        const ctx = await resolveWorkspace(req, reply);
        if (!ctx)
            return reply;
        const { id } = z.object({ id: z.string().uuid() }).parse(req.params);
    ...
    app.post("/v1/exchange/deliveries/:id/download", { preHandler: requireAuth }, async (req, reply) => {
        const ctx = await resolveWorkspace(req, reply);
        if (!ctx)
            return reply;
        const { id } = z.object({ id: z.string().uuid() }).parse(req.params);
    ...
    app.post("/v1/exchange/packages/:id/revoke", { preHandler: [requireAuth, requireDelegatedTier("ORG_ADMIN")] }, async (req, reply) => {
        const ctx = await resolveWorkspace(req, reply);
        if (!ctx)
            return reply;
        const { id } = z.object({ id: z.string().uuid() }).parse(req.params);
    ...
    // Chain transfers
    // =========================================================================
    app.get("/v1/exchange/chain-transfers", { preHandler: requireAuth }, async (req, reply) => {
        const ctx = await resolveWorkspace(req, reply);
        if (!ctx)
            return reply;
    ...
    app.post("/v1/exchange/chain-transfers", {
        preHandler: [
            requireAuth,
    ...
    app.post("/v1/exchange/chain-transfers/:id/accept", { preHandler: [requireAuth, requireDelegatedTier("ORG_ADMIN")] }, async (req, reply) => {
        const ctx = await resolveWorkspace(req, reply);
        if (!ctx)
            return reply;
        const { id } = z.object({ id: z.string().uuid() }).parse(req.params);
    ...
    app.post("/v1/exchange/chain-transfers/:id/reject", { preHandler: [requireAuth, requireDelegatedTier("ORG_ADMIN")] }, async (req, reply) => {
        const ctx = await resolveWorkspace(req, reply);
        if (!ctx)
            return reply;
        const { id } = z.object({ id: z.string().uuid() }).parse(req.params);
    ...
    app.post("/v1/exchange/chain-transfers/:id/revoke", { preHandler: [requireAuth, requireDelegatedTier("ORG_ADMIN")] }, async (req, reply) => {
        const ctx = await resolveWorkspace(req, reply);
        if (!ctx)
            return reply;
        const { id } = z.object({ id: z.string().uuid() }).parse(req.params);
    ...
    app.post("/v1/exchange/chain-transfers/:id/complete", { preHandler: [requireAuth, requireDelegatedTier("ORG_ADMIN")] }, async (req, reply) => {
        const ctx = await resolveWorkspace(req, reply);
        if (!ctx)
            return reply;
        const { id } = z.object({ id: z.string().uuid() }).parse(req.params);
    ...
    // Webhooks
    // =========================================================================
    app.get("/v1/integrations/webhooks/endpoints", { preHandler: requireAuth }, async (req, reply) => {
        const ctx = await resolveWorkspace(req, reply);
        if (!ctx)
            return reply;
        const featureOk = await assertFeatureEntitlement({
    ...
    app.post("/v1/integrations/webhooks/endpoints", { preHandler: [requireAuth, requireDelegatedTier("ORG_ADMIN")] }, async (req, reply) => {
        const ctx = await resolveWorkspace(req, reply);
        if (!ctx)
            return reply;
        // Feature gate
    ...
    app.post("/v1/integrations/webhooks/endpoints/:id/deactivate", { preHandler: [requireAuth, requireDelegatedTier("ORG_ADMIN")] }, async (req, reply) => {
        const ctx = await resolveWorkspace(req, reply);
        if (!ctx)
            return reply;
        const { id } = z.object({ id: z.string().uuid() }).parse(req.params);
    ...
    app.get("/v1/integrations/webhooks/deliveries", { preHandler: requireAuth }, async (req, reply) => {
        const ctx = await resolveWorkspace(req, reply);
        if (!ctx)
            return reply;
        const q = z
    ...
    app.get("/v1/integrations/webhooks/lifecycle-deliveries", { preHandler: requireAuth }, async (req, reply) => {
        const ctx = await resolveWorkspace(req, reply);
        if (!ctx)
            return reply;
    ...
    app.post("/v1/integrations/webhooks/lifecycle-deliveries/:id/replay", { preHandler: [requireAuth, requireDelegatedTier("ORG_ADMIN")] }, async (req, reply) => {
        const ctx = await resolveWorkspace(req, reply);
        if (!ctx)
            return reply;
        const { id } = z.object({ id: z.string().uuid() }).parse(req.params);
    ...
    // Retention policies
    // =========================================================================
    app.get("/v1/lifecycle/retention/policies", { preHandler: requireAuth }, async (req, reply) => {
        const ctx = await resolveWorkspace(req, reply);
        if (!ctx)
            return reply;
    ...
    app.post("/v1/lifecycle/retention/policies", {
        preHandler: [
            requireAuth,
        ...
        const body = z
            ...
            scopeKind: z.enum(["WORKSPACE", "DEPARTMENT", "CASE"]),
    ...
    app.post("/v1/lifecycle/retention/policies/:id/release", {
        preHandler: [
            requireAuth,
            requireDelegatedTierAny(["ORG_ADMIN", "COMPLIANCE_OFFICER"]),
        ],
    ...
    app.get("/v1/lifecycle/retention/upcoming-expirations", { preHandler: requireAuth }, async (req, reply) => {
        const ctx = await resolveWorkspace(req, reply);
        if (!ctx)
            return reply;
    ...
    // Legal holds
    // =========================================================================
    app.get("/v1/lifecycle/legal-holds", { preHandler: requireAuth }, async (req, reply) => {
        const ctx = await resolveWorkspace(req, reply);
        if (!ctx)
            return reply;
        const featureOk = await assertFeatureEntitlement({
    ...
    app.post("/v1/lifecycle/legal-holds", {
        preHandler: [
            requireAuth,
            requireDelegatedTierAny(["ORG_ADMIN", "COMPLIANCE_OFFICER"]),
        ],
    ...
    app.post("/v1/lifecycle/legal-holds/:id/release", {
        preHandler: [
            requireAuth,
            requireDelegatedTierAny(["ORG_ADMIN", "COMPLIANCE_OFFICER"]),
        ],
    ...
    // Archive tier
    // =========================================================================
    app.get("/v1/lifecycle/archive/transitions", { preHandler: requireAuth }, async (req, reply) => {
        const ctx = await resolveWorkspace(req, reply);
        if (!ctx)
            return reply;
        const featureOk = await assertFeatureEntitlement({
    ...
    app.post("/v1/lifecycle/archive/transition", { preHandler: [requireAuth, requireDelegatedTier("ORG_ADMIN")] }, async (req, reply) => {
        const ctx = await resolveWorkspace(req, reply);
        if (!ctx)
            return reply;
        const body = z
    ...
    app.get("/v1/lifecycle/archive/costs", { preHandler: requireAuth }, async (req, reply) => {
        const ctx = await resolveWorkspace(req, reply);
        if (!ctx)
            return reply;
    ...
    // =========================================================================
    app.get("/v1/lifecycle/destruction/requests", { preHandler: requireAuth }, async (req, reply) => {
        const ctx = await resolveWorkspace(req, reply);
        if (!ctx)
            return reply;
        const featureOk = await assertFeatureEntitlement({
    ...
    app.post("/v1/lifecycle/destruction/requests", {
        preHandler: [
            requireAuth,
            requireDelegatedTierAny(["ORG_ADMIN", "COMPLIANCE_OFFICER"]),
        ],
        ...
        if (!res.ok)
            return reply.code(409).send({ denial: res.denial, holdIds: res.holdIds ?? [] });
        return reply.code(201).send({ requestId: res.requestId });
    });
    app.post("/v1/lifecycle/destruction/requests/:id/approve", {
        preHandler: [
            requireAuth,
    ...
    app.post("/v1/lifecycle/destruction/requests/:id/reject", {
        preHandler: [
            requireAuth,
            requireDelegatedTierAny(["ORG_ADMIN", "COMPLIANCE_OFFICER"]),
        ],
    ...
    app.post("/v1/lifecycle/destruction/requests/:id/execute", {
    ...
    app.get("/v1/lifecycle/destruction/requests/:id/certificate", { preHandler: requireAuth }, async (req, reply) => {
        const ctx = await resolveWorkspace(req, reply);
        if (!ctx)
            return reply;
        const { id } = z.object({ id: z.string().uuid() }).parse(req.params);
    ...
    app.get("/v1/lifecycle/destruction/requests/:id/certificate.json", { preHandler: requireAuth }, async (req, reply) => {
        const ctx = await resolveWorkspace(req, reply);
        if (!ctx)
            return reply;
        const { id } = z.object({ id: z.string().uuid() }).parse(req.params);
        ...
        void reply.header("Content-Disposition", `attachment; filename="destruction-cert-${id}.json"`);
        if (res.sha256)
            void reply.header("X-Proovra-Certificate-Sha256", res.sha256);
    ...
    app.get("/v1/lifecycle/destruction/requests/:id/certificate.pdf", { preHandler: requireAuth }, async (_req, reply) => {
        return reply.code(501).send({ denial: "PDF_NOT_IMPLEMENTED" });
    });
    // =========================================================================
    // Lifecycle dashboard + VP preview
    // =========================================================================
    app.get("/v1/lifecycle/dashboard", { preHandler: requireAuth }, async (req, reply) => {
        const ctx = await resolveWorkspace(req, reply);
        if (!ctx)
            return reply;
        ...
        try {
            ...
            req.log?.warn?.({ err: err instanceof Error ? err.message : "dashboard_projection_failed" }, "lifecycle_dashboard_projection_failed");
            dashboard = {
                retention: {},
                legalHolds: {},
                archive: {},
                ...
                degraded: true,
                degradedReason: "Lifecycle activity projection is temporarily unavailable. Counts will populate once it recovers.",
            };
        }
        ...
        try {
            ...
            req.log?.warn?.({ err: err instanceof Error ? err.message : "capability_status_failed" }, "lifecycle_capability_status_compute_failed");
        }
    ...
    app.get("/v1/lifecycle/verification-package/preview", {
        preHandler: [
            requireAuth,
            requireDelegatedTierAny(["ORG_ADMIN", "COMPLIANCE_OFFICER"]),
        ],
    ...
    app.get("/v1/lifecycle/violations", { preHandler: requireAuth }, async (req, reply) => {
        const ctx = await resolveWorkspace(req, reply);
        if (!ctx)
            return reply;
        const q = z
    ...
    app.get("/v1/lifecycle/violations/counts", { preHandler: requireAuth }, async (req, reply) => {
        const ctx = await resolveWorkspace(req, reply);
        if (!ctx)
            return reply;
        const q = z
