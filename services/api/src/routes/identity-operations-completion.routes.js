...
import { detectScimDrift, executeScimReconciliation, listScimSyncFailures, replayScimSyncFailure, } from "../services/access-control/scim-reconciliation.service.js";
import { buildSsoHealthSnapshot, } from "../services/security/sso-health.service.js";
import { getSamlMappingSchema, getCurrentSamlMapping, previewSamlMapping, updateSamlMapping, SAML_MAPPING_PRIVILEGE_PURPOSE, } from "../services/security/saml-mapping.service.js";
import { buildIdentitySessionTimeline, } from "../services/security/session-timeline.service.js";
import { requireStepUpForSensitiveAction } from "../services/identity-security/step-up-middleware.js";
import { bump } from "../services/ops/metrics.service.js";
async function requireIdentityAdmin(req, reply, teamId) {
...
// Plugin registration
// ---------------------------------------------------------------------------
export async function identityOperationsCompletionRoutes(app) {
    // ---------------------------------------------------------------------
    // SCIM drift preview
    // ---------------------------------------------------------------------
    app.get("/v1/scim/reconciliation/preview", { preHandler: requireAuth }, async (req, reply) => {
        const q = z
    ...
    // ---------------------------------------------------------------------
    app.post("/v1/scim/reconciliation/execute", { preHandler: requireAuth }, async (req, reply) => {
        const body = z
            .object({
    ...
    // ---------------------------------------------------------------------
    app.get("/v1/scim/sync-failures", { preHandler: requireAuth }, async (req, reply) => {
        const q = z
            .object({
            teamId: z.string().uuid(),
    ...
    app.post("/v1/scim/sync-failures/:id/replay", { preHandler: requireAuth }, async (req, reply) => {
    ...
    // ---------------------------------------------------------------------
    app.get("/v1/saml/mapping/schema", { preHandler: requireAuth }, async (_req, reply) => {
    ...
    app.get("/v1/saml/mapping/current", { preHandler: requireAuth }, async (req, reply) => {
        const q = z
            .object({
            teamId: z.string().uuid(),
    ...
    app.post("/v1/saml/mapping/preview", { preHandler: requireAuth }, async (req, reply) => {
        const body = z
            .object({
            teamId: z.string().uuid(),
            connectionId: z.string().uuid(),
    ...
    app.put("/v1/saml/mapping", { preHandler: requireAuth }, async (req, reply) => {
        const body = z
            .object({
            teamId: z.string().uuid(),
            connectionId: z.string().uuid(),
    ...
    // SSO health snapshot
    // ---------------------------------------------------------------------
    app.get("/v1/sso/health", { preHandler: requireAuth }, async (req, reply) => {
        const q = z
            .object({ teamId: z.string().uuid() })
            .parse(req.query ?? {});
        const ctx = await requireIdentityAdmin(req, reply, q.teamId);
        if (!ctx)
            return;
        const snapshot = await buildSsoHealthSnapshot({
            teamId: q.teamId,
        });
    ...
    // ---------------------------------------------------------------------
    app.get("/v1/identity/sessions/:sessionId/timeline", { preHandler: requireAuth }, async (req, reply) => {
        const params = z
        ...
        if (!ctx)
            return;
        const timeline = await buildIdentitySessionTimeline({
            teamId: q.teamId,
