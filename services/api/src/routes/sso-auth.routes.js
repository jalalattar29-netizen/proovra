...
import { handleOidcCallback, buildOidcAuthorizationUrl, SsoServiceError, } from "../services/access-control/sso.service.js";
...
import { consumeCallbackAttempt, getConnectionForAttempt, markCallbackFailed, noteSsoFailure, noteSsoSuccess, persistCallbackAttempt, } from "../services/access-control/sso-hardening.service.js";
...
function bounceToAuthError(reply, reason, redirectAfter) {
...
// Routes
// -----------------------------------------------------------------------------
export async function ssoAuthRoutes(app) {
    ...
    // GET /v1/auth/sso/:connectionId/initiate
    // -------------------------------------------------------------------------
    app.get("/v1/auth/sso/:connectionId/initiate", async (req, reply) => {
        if (!callbacksEnabled) {
        ...
        try {
            ...
            if (err instanceof SsoServiceError) {
                await noteSsoFailure({ connectionId: conn.id, reason: err.code }, prisma);
                return bounceToAuthError(reply, err.code.toLowerCase());
            }
            await noteSsoFailure({ connectionId: conn.id, reason: "initiate_failed" }, prisma);
    ...
    // GET /v1/auth/sso/callback
    // -------------------------------------------------------------------------
    app.get("/v1/auth/sso/callback", async (req, reply) => {
        if (!callbacksEnabled) {
            return reply.code(503).send({ error: { code: "sso_disabled" } });
        }
        const q = z
        ...
        const consume = await consumeCallbackAttempt({ stateRaw: q.state }, prisma);
        if (!consume.ok) {
            const reason = consume.reason === "REPLAYED"
                ? "sso_state_replayed"
        ...
        if (!conn) {
            await markCallbackFailed({ attemptId: attempt.id, reason: "connection_missing" }, prisma);
            return bounceToAuthError(reply, "sso_connection_unavailable");
        }
        ...
        try {
            ...
            if (decision.outcome === "ENROLLMENT_REQUIRED") {
                ...
                url.searchParams.set("next", attempt.redirectAfter ?? "/home");
            ...
            if (decision.outcome === "MFA_REQUIRED") {
                ...
                const productionDomain = process.env["SSO_COOKIE_DOMAIN"] ||
                    (host.includes("proovra.com") ||
                        ...
                        ? ".proovra.com"
                        : undefined);
                const secure = process.env["SSO_COOKIE_SECURE"] === "true" ||
                    !!productionDomain;
            ...
            await detectAndScoreSession({ teamId: result.teamId, sessionId: session.id }, prisma).catch(() => null);
            ...
            await markCallbackFailed({
                ...
                reason: err instanceof SsoServiceError ? err.code : "callback_failed",
            }, prisma);
            ...
            if (err instanceof SsoServiceError) {
                return bounceToAuthError(reply, err.code.toLowerCase(), attempt.redirectAfter);
            }
