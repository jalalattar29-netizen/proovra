...
async function resolvePublicKeyPemFromKms() {
    ...
    if (response.KeySpec !== "ECC_NIST_EDWARDS25519") {
        throw new Error(`Unexpected KMS key spec: ${response.KeySpec ?? "unknown"}`);
    }
    if (response.KeyUsage !== "SIGN_VERIFY") {
        throw new Error(`Unexpected KMS key usage: ${response.KeyUsage ?? "unknown"}`);
    }
 ...
 */
function validateAndCanonicalisePem(pem, sourceLabel) {
    if (!pem.includes("BEGIN PUBLIC KEY")) {
        throw new Error(`${sourceLabel} does not look like a PEM public key — ` +
    ...
    if (keyObject.asymmetricKeyType !== "ed25519") {
        throw new Error(`${sourceLabel} is not an Ed25519 key ` +
 ...
 */
function tryReadPemFromPath(path, sourceLabel) {
    ...
    try {
        ...
        throw new Error(`${sourceLabel} (resolved to "${abs}") exists but could not be read: ` +
 ...
 */
function buildCandidatePaths(envPath) {
    const candidates = [];
    const seen = new Set();
    const push = (p) => {
        if (!p)
...
export function resolvePublicKeyPemFromLocalPem() {
    ...
    if (inlinePem && inlinePem.length > 0) {
        const canonical = validateAndCanonicalisePem(inlinePem, 'env "SIGNING_PUBLIC_KEY_PEM"');
        return {
            pem: canonical,
    ...
    for (const candidate of candidates) {
        const found = tryReadPemFromPath(candidate, `Public key path "${candidate}"`);
        if (found) {
            const canonical = validateAndCanonicalisePem(found.pem, `File "${found.absolutePath}"`);
            // Was this the env-supplied path or the fixture fallback?
            const isEnvPath = envPath !== undefined &&
                envPath.length > 0 &&
                (candidate === envPath ||
    ...
    if (!isProductionEnv()) {
        const canonical = validateAndCanonicalisePem(TEST_ONLY_FALLBACK_PUBLIC_KEY_PEM, "TEST_ONLY built-in fallback");
        return {
            pem: canonical,
    ...
    throw new Error("Could not resolve a signing public key in production. Tried (in order):\n" +
...
async function upsertSigningKeyRow(keyId, version, publicKeyPem) {
...
async function main() {
    ...
    if (provider === "aws-kms") {
        ...
        providerLabel = `aws-kms (region=${process.env.AWS_REGION ?? "?"}, keyId=${process.env.KMS_KEY_ID ?? "?"})`;
    }
    else {
        ...
        if (resolved.isTestOnlyFallback) {
            ...
            console.warn("[seed-signing-key] WARNING: using TEST_ONLY built-in fallback " +
                "public key. This is acceptable for CI / dev but MUST NOT be used " +
                "in production. Verify NODE_ENV is not set to 'production'.");
        }
    }
    const evidenceKey = await upsertSigningKeyRow(signingKeyId, signingKeyVersion, publicKeyPem);
    // eslint-disable-next-line no-console
    ...
    if (packageKeyId &&
        ...
        if (Number.isFinite(packageKeyVersion)) {
            ...
            const packageKey = await upsertSigningKeyRow(packageKeyId, packageKeyVersion, publicKeyPem);
            // eslint-disable-next-line no-console
...
const isDirectInvocation = (() => {
    try {
        ...
        return (invokedPath.endsWith("seed-signing-key.ts") ||
