...
--   * Drop legacy CHECK constraints on converted columns before enum conversion.
...
--   * legacy CHECK constraints are removed only for columns converted to native enums
  ...
  IF EXISTS (
    ...
    FOR constraint_name IN
      ...
      WHERE n.nspname = 'public'
        ...
        AND c.contype = 'c'
        AND a.attname = 'last_verified_source_snapshot'
    LOOP
...
BEGIN
  IF EXISTS (
    ...
    FOR constraint_name IN
      ...
      JOIN pg_attribute a ON a.attrelid = t.oid AND a.attnum = cols.attnum
      WHERE n.nspname = 'public'
        AND t.relname = 'verification_views'
        AND c.contype = 'c'
        AND a.attname = 'verification_status_snapshot'
    LOOP
      EXECUTE format('ALTER TABLE "verification_views" DROP CONSTRAINT IF EXISTS %I', constraint_name);
    END LOOP;
  ...
  IF EXISTS (
    ...
    FOR constraint_name IN
      ...
      WHERE n.nspname = 'public'
        AND t.relname = 'teams'
        AND c.contype = 'c'
        AND a.attname = 'billing_plan'
    LOOP
      EXECUTE format('ALTER TABLE "teams" DROP CONSTRAINT IF EXISTS %I', constraint_name);
    END LOOP;
  ...
  IF EXISTS (
    ...
    FOR constraint_name IN
      ...
      WHERE n.nspname = 'public'
        AND t.relname = 'demo_requests'
        AND c.contype = 'c'
        AND a.attname = 'recommended_action'
    LOOP
      EXECUTE format('ALTER TABLE "demo_requests" DROP CONSTRAINT IF EXISTS %I', constraint_name);
    END LOOP;
