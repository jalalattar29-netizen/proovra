...
--
-- The columns themselves are NOT dropped in this release, and are not dropped
-- by any migration in this tree. The drop is a real CONTRACT_DROP, declares
-- `safeBeforeCodeDeployment: false`, and is held on `chore/operations-contract-
-- drop-later` until this release is live and healthy — because
-- `prisma migrate deploy` applies every pending migration in one pass and
-- cannot be asked to stop halfway, so co-shipping the two would race a removal
-- against the image deploy. Expand first, contract in a LATER release — the
-- discipline this repository already encodes, applied to its own incident
-- tables.
--
-- Nothing depends on that drop having happened. The legacy columns are inert
-- once their NOT NULLs are relaxed: no shipped image reads them and no writer
-- writes them.
--
