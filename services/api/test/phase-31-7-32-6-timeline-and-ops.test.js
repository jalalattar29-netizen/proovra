...
describe("Phase 31.7 — buildInvestigationTimeline", () => {
    ...
    it("TimelineEventKind union includes lifecycle + MI run + MI signal kinds", () => {
        const decl = src.match(/export type TimelineEventKind\s*=[\s\S]*?;/)?.[0];
    ...
    it("evidence-keyed stream UNIONs are only appended when evidenceId is provided", () => {
        ...
        expect(src).toMatch(/const evidenceStreams\s*=\s*input\.evidenceId[\s\S]*?:\s*""/);
    });
    ...
    it("never throws — every catch path returns a bounded result", () => {
        const fn = src.match(/export async function buildInvestigationTimeline\([\s\S]*?\n\}\s*\n/)?.[0];
        expect(fn).toBeTruthy();
        expect(fn).toMatch(/catch\s*\{[\s\S]*?return\s*\{\s*ok:\s*true,\s*events:\s*\[\]/);
    });
...
describe("Phase 31.7 — /v1/graph/timeline route", () => {
    ...
    it("accepts the new evidenceId param", () => {
        const block = src.match(/"\/v1\/graph\/timeline"[\s\S]*?\n\s*\}\s*,?\s*\n\s*\)/)?.[0];
...
describe("Phase 31.7 + 32.6 — /ops/media-graph page", () => {
    const src = readSource("../../../apps/web/app/(app)/ops/media-graph/page.tsx");
    ...
    it("reads only from a whitelisted set of endpoints", () => {
        ...
        for (const call of apiFetchCalls) {
            ...
            expect(matched, `unexpected endpoint called from /ops/media-graph: ${path}`).toBe(true);
        }
    });
    ...
    it("no forbidden truth-claim vocabulary in user-facing literals", () => {
        ...
        for (const lit of literals) {
            expect(lit, `ops page uses forbidden wording: ${lit}`).not.toMatch(forbidden);
        }
    });
