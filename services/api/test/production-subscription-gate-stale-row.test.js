...
const SHARED_CODES = readFileSync(fileURLToPath(new URL("../../../packages/shared/src/collaboration-team-billing-codes.ts", import.meta.url)), "utf8");
...
function extractSubscriptionGate(src) {
    const idx = src.indexOf("export async function assertSubscriptionActiveOrGraceAllowed");
    expect(idx, "assertSubscriptionActiveOrGraceAllowed must exist in billing-guards.ts").toBeGreaterThan(-1);
...
describe("Production fix — subscription gate no longer picks stale rows", () => {
    it("does NOT contain the legacy `findFirst({ where: { userId } })` shape", () => {
        ...
        expect(GATE).not.toMatch(/subscription\.findFirst\(\s*\{\s*where:\s*\{\s*userId\s*,?\s*\}/);
    });
    it("every subscription.findFirst inside the gate is plan-scoped", () => {
        ...
        for (const call of calls) {
            expect(call, `every subscription.findFirst in the gate must filter by plan — found: ${call.slice(0, 200)}`).toMatch(/\bplan\b/);
        }
    });
});
// ===========================================================================
...
describe("Production fix — corroboration policy is wired correctly", () => {
    it("Step 1 — looks up a live (ACTIVE/TRIALING) subscription for the current plan", () => {
        ...
        expect(GATE).toMatch(/status:\s*\{\s*in:\s*\[[\s\S]{0,200}SubscriptionStatus\.ACTIVE[\s\S]{0,200}SubscriptionStatus\.TRIALING/);
    });
    it("Step 2 — looks up a PAST_DUE row for the current plan and checks grace window", () => {
        ...
        expect(GATE).toMatch(/status:\s*prismaPkg\.SubscriptionStatus\.PAST_DUE/);
    ...
    it("Step 3 — falls through to ACTIVE when NO row matches the current plan", () => {
        ...
        expect(GATE).toMatch(/if\s*\(\s*!matchingPlanRow\s*\)\s*\{\s*return\s*\{[\s\S]{0,200}status:\s*"ACTIVE"/);
    });
