...
describe("E2 Test 1 — collaboration capability keys exist in CapabilityKey enum", () => {
    ...
    it.each(REQUIRED_CAPABILITY_KEYS)("CapabilityKey enum contains: %s", (key) => {
...
describe("E2 Test 2 — collaboration security-event vocabulary is preserved", () => {
    ...
    it.each(REQUIRED_EVENT_TYPES)("security-event vocabulary contains: %s", (eventType) => {
        expect(SECURITY_EVENTS).toMatch(new RegExp(`["']${eventType}["']`));
    });
});
// ===========================================================================
...
describe("E2 Test 3 — Prisma collaboration models present", () => {
    ...
    it.each(REQUIRED_MODELS)("Prisma schema declares model: %s", (modelName) => {
...
describe("E2 Test 4 — collaboration backend routes present", () => {
    ...
    it("case-workspace.routes.ts handles comments + assignments (per E2 inventory)", () => {
        ...
        expect(existsSync(apiPath("src/routes/case-workspace.routes.ts"))).toBe(true);
...
describe("E2 Test 5 — collaboration UI surfaces present", () => {
    ...
    it.each(REQUIRED_FILES)("$description exists at $rel", ({ rel }) => {
        expect(existsSync(webPath(rel)), `Required collaboration UI file missing: ${rel}`).toBe(true);
    });
});
// ===========================================================================
...
describe("E2 Test 7 — external review grants are bounded (Phase 27/28 invariants)", () => {
    it("external_review_grants table is created by Prisma migration", () => {
        ...
        // TABLE statement.
        const migrationFile = repoPath("services/api/prisma/migrations/20260620100000_phase24_31_consolidated_drift_patches/migration.sql");
        expect(existsSync(migrationFile), "Phase 27/28 migration file missing").toBe(true);
...
describe("E2 Test 10 — no new client-state library introduced", () => {
    it("apps/web/package.json has no React Query / SWR / Redux / Zustand", () => {
        ...
        for (const forbidden of [
            ...
            expect(deps[forbidden], `Forbidden library added in E2: ${forbidden} (no realtime/social/state-lib expansion)`).toBeUndefined();
        }
    });
});
// ===========================================================================
...
describe("E2 Test 11 — documentation + registry updated", () => {
    ...
    it("doc enumerates the 8 collaboration entities", () => {
        ...
        for (const entity of [
            ...
            expect(doc, `Collaboration model entity missing from doc: ${entity}`).toContain(entity);
        }
    });
    ...
    it("MASTER_PHASE_REGISTRY.md registers Phase E2 with explicit status", () => {
        ...
        expect(registry).toMatch(/\|\s*(Phase )?E2\s*\|[\s\S]*?(CLOSED|CLOSED_WITH_DEFERRED_ITEMS)/);
    });
    it("registry contains the 5 new DEF items (DEF-016 → DEF-020) per CR1.7 silent-debt rule", () => {
        const registry = readRepo("docs/recovery/MASTER_PHASE_REGISTRY.md");
        for (const def of ["DEF-016", "DEF-017", "DEF-018", "DEF-019", "DEF-020"]) {
            expect(registry, `New DEF item ${def} missing from registry §6 (CR1.7 silent-debt rule violation).`).toContain(def);
        }
    });
});
