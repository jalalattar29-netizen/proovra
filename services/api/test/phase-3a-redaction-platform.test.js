...
import { BULK_INVITATION_MAX_ROWS, REDACTION_ACTIVITY_CODES, REDACTION_APPROVAL_VERDICTS, REDACTION_ARTIFACT_KINDS, REDACTION_CAPABILITIES, REDACTION_CAPABILITY_MATRIX, REDACTION_CONFIDENCE_BANDS, REDACTION_DECISION_STATES, REDACTION_DENIAL_REASONS, REDACTION_DERIVATIVE_KINDS, REDACTION_DERIVATIVE_STATES, REDACTION_DETECTION_KINDS, REDACTION_DETECTION_PROVIDERS, REDACTION_DETECTION_PROVIDER_STATES, REDACTION_METHODS, REDACTION_PROJECTION_SCHEMA_VERSION, REDACTION_PROJECT_STATES, REDACTION_PROVENANCE_LIMITATIONS, REDACTION_REGION_KINDS, REDACTION_ROLES, REDACTION_VERSION_STATES, REDACTION_VERSION_TRANSITIONS, classifyConfidence, isAllowedVersionTransition, isValidAudioRangeMs, isValidBboxNormalized, isValidPdfPageRect, isValidPdfTextRange, isValidPolygonNormalized, isValidRegionGeometry, isValidVideoFrameBbox, redactionCapabilitiesForRole, } from "@proovra/shared";
...
const PERMISSIONS_SRC = readSource("../../../packages/shared/src/permissions.ts");
const SCHEMA = readSource("../../../services/api/prisma/schema.prisma");
const MIGRATION = readSource("../../../services/api/prisma/migrations/20261101000000_phase_3a_redaction_platform/migration.sql");
const SVC_ACTIVITY = readSource("../../../services/api/src/services/redaction/redaction-activity.service.ts");
const SVC_RBAC = readSource("../../../services/api/src/services/redaction/redaction-rbac.service.ts");
const SVC_PROJECT = readSource("../../../services/api/src/services/redaction/redaction-project.service.ts");
const SVC_REGION = readSource("../../../services/api/src/services/redaction/redaction-region.service.ts");
const SVC_DETECTION = readSource("../../../services/api/src/services/redaction/redaction-detection.service.ts");
const SVC_PROVIDERS = readSource("../../../services/api/src/services/redaction/redaction-detection-providers.service.ts");
const SVC_DECISION = readSource("../../../services/api/src/services/redaction/redaction-decision.service.ts");
const SVC_APPROVAL = readSource("../../../services/api/src/services/redaction/redaction-approval.service.ts");
const SVC_DERIVATIVE = readSource("../../../services/api/src/services/redaction/redaction-derivative.service.ts");
const SVC_PROJECTION = readSource("../../../services/api/src/services/redaction/redaction-projection.service.ts");
const SVC_VERIFICATION = readSource("../../../services/api/src/services/redaction/redaction-verification-manifest.service.ts");
const ROUTES = readSource("../../../services/api/src/routes/redaction.routes.ts");
const SERVER = readSource("../../../services/api/src/server.ts");
const NAV_REGISTRY = readSource("../../../services/api/src/services/platform-context/navigation-registry.ts");
const REPORT_SECTION = readSource("../../../services/worker/src/report-v2/sections/redaction-summary.ts");
const UI_PROJECTS = readSource("../../../apps/web/app/(app)/redaction/page.tsx");
const UI_PROJECT = readSource("../../../apps/web/app/(app)/redaction/[projectId]/page.tsx");
const UI_IMAGE_VIEWER = readSource("../../../apps/web/components/redaction/ImageRedactionViewer.tsx");
const UI_PDF_VIEWER = readSource("../../../apps/web/components/redaction/PdfRedactionViewer.tsx");
const UI_VIDEO_VIEWER = readSource("../../../apps/web/components/redaction/VideoRedactionViewer.tsx");
const UI_DETECTION = readSource("../../../apps/web/components/redaction/DetectionReviewPanel.tsx");
const UI_APPROVAL = readSource("../../../apps/web/components/redaction/ApprovalPanel.tsx");
const UI_HISTORY = readSource("../../../apps/web/components/redaction/VersionHistoryPanel.tsx");
// =============================================================================
// 1. Shared contracts
// =============================================================================
describe("Phase 3A — shared contracts", () => {
    it("artifact kinds cover image / pdf / video / audio", () => {
        expect([...REDACTION_ARTIFACT_KINDS].sort()).toEqual(["IMAGE", "PDF", "VIDEO", "AUDIO"].sort());
    });
    ...
    it("decision states are bounded to 5 values", () => {
        expect([...REDACTION_DECISION_STATES].sort()).toEqual(["ACCEPTED", "DEFERRED", "MODIFIED", "REJECTED", "SUGGESTED"].sort());
    });
    ...
    it("approval verdicts catalog is bounded", () => {
        expect([...REDACTION_APPROVAL_VERDICTS].sort()).toEqual(["APPROVE", "DEFER", "REJECT", "REQUEST_CHANGES"].sort());
    });
    ...
    it("RBAC roles + capabilities + matrix are bounded + monotonic", () => {
        ...
        // Approver ⊇ Reviewer.
        for (const c of reviewer)
            expect(approver.has(c)).toBe(true);
        // Admin ⊇ Approver.
        for (const c of approver)
    ...
    it("REVIEWER canonical role gets the bounded reviewer subset only", () => {
        ...
        expect(PERMISSIONS_SRC).toMatch(/REVIEWER:[\s\S]+?"redaction\.region\.author"[\s\S]+?\]/);
        expect(PERMISSIONS_SRC).not.toMatch(/REVIEWER:[\s\S]+?"redaction\.version\.approve"/);
    });
});
// =============================================================================
// 2. Prisma + migration
...
describe("Phase 3A — Prisma + migration", () => {
    ...
    it("RedactionDerivative has UNIQUE on versionId", () => {
        expect(SCHEMA).toMatch(/versionId\s+String\s+@unique\s+@map\("version_id"\)/);
    });
    it("Phase O-Final compliant migration — plain CREATE TABLE + guarded indexes", () => {
        ...
        expect(MIGRATION).not.toMatch(/CREATE TABLE IF NOT EXISTS "redaction_/);
...
describe("Phase 3A — backend services", () => {
    ...
    it("project service exposes openRedactionProject + createRedactionVersion + transitionRedactionVersion", () => {
        ...
        expect(SVC_PROJECT).toMatch(/export async function transitionRedactionVersion/);
...
describe("Phase 3A — HTTP routes", () => {
    it("mounts the full bounded surface", () => {
        for (const path of [
            ...
            expect(ROUTES).toMatch(new RegExp(path.replace(/\//g, "\\/").replace(/:/g, ":")));
        }
    });
...
describe("Phase 3A — integrations", () => {
    ...
    it("verify-page badge is public + provenance-only", () => {
        ...
        const block = ROUTES.match(/\/v1\/redaction\/public\/verify\/:evidenceId[\s\S]+?\}\s*,\s*\)/);
        expect(block).not.toBeNull();
        expect((block?.[0] ?? "").includes("requireAuth")).toBe(false);
    });
    it("report builder ships a redaction-summary section module", () => {
        expect(REPORT_SECTION).toMatch(/export function renderRedactionSummarySection/);
...
describe("Phase 3A — runtime helpers", () => {
    ...
    it("region geometry validators reject out-of-bounds shapes", () => {
        // BBOX normalized.
        expect(isValidBboxNormalized({ x: 0, y: 0, width: 1, height: 1 })).toBe(true);
        expect(isValidBboxNormalized({ x: 0.5, y: 0.5, width: 0.6, height: 0.6 })).toBe(false);
        expect(isValidBboxNormalized({ x: -0.1, y: 0, width: 0.5, height: 0.5 })).toBe(false);
        // Polygon normalized.
        expect(isValidPolygonNormalized({
            points: [
                { x: 0, y: 0 },
                { x: 0.5, y: 0 },
                { x: 0.5, y: 0.5 },
        ...
        expect(isValidPdfPageRect({
            pageIndex: 0,
            rect: { x: 0, y: 0, width: 0.5, height: 0.5 },
        })).toBe(true);
        expect(isValidPdfTextRange({
            pageIndex: 0,
            charStart: 10,
            charEnd: 25,
        })).toBe(true);
        ...
        expect(isValidRegionGeometry("BBOX_NORMALIZED", {
            x: 0,
            y: 0,
            width: 1,
            height: 1,
        })).toBe(true);
        expect(isValidRegionGeometry("PDF_PAGE_RECT", {
            pageIndex: 0,
            rect: { x: 0, y: 0, width: 0.5, height: 0.5 },
        })).toBe(true);
    });
    it("REGEX_PII detector finds emails + phones + national ids with bounded preview", async () => {
        const res = await regexPiiDetectorRun({
            ...
            artifactKind: "PDF",
            inlineText: "Contact jane.doe@acme.com or call +44 20 7946 0958. " +
