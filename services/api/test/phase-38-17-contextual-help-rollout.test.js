...
describe("Phase 38.17 — ContextualHelp mounted on 5 additional surfaces", () => {
    ...
    for (const entry of MOUNTS) {
        it(`${entry.name} (${entry.file}) imports and renders <ContextualHelp surface="${entry.surfaceEnum}">`, () => {
            ...
            expect(src).toMatch(new RegExp(`surface="${entry.surfaceEnum.replace(/\W/g, "\\$&")}"`));
        });
        ...
        it(`${entry.name} mounts ContextualHelp as collapsedByDefault`, () => {
            ...
            // import / usage block.
            const block = src.match(/<ContextualHelp[\s\S]{0,400}collapsedByDefault/);
            expect(block, `${entry.file} must mount ContextualHelp with collapsedByDefault`).not.toBeNull();
        });
    }
});
// =============================================================================
...
describe("Phase 38.17 — surface= values are within the bounded HelpSurface enum", () => {
    it("the canonical HelpSurface enum lists every value used in mounts", () => {
        ...
        for (const surface of [
            ...
            expect(helpSrc, `HelpSurface enum must include "${surface}"`).toMatch(new RegExp(`"${surface.replace(/\W/g, "\\$&")}"`));
        }
    });
});
// =============================================================================
...
describe("Phase 38.17 — total ContextualHelp mounts across codebase", () => {
    ...
    it("at least 7 .tsx files render <ContextualHelp (component definition + 7 mounts)", () => {
        const root = webPath(".");
        ...
        expect(mountCount, `expected ≥ 7 ContextualHelp mounts, found ${mountCount}`).toBeGreaterThanOrEqual(7);
    });
});
// =============================================================================
