...
describe("useActiveWorkspaceId hook", () => {
    ...
    const src = readSource("../../../apps/web/lib/platform-context/useTeamWorkspaceGate.ts");
...
describe("WorkspaceGateState renderer", () => {
    const src = readSource("../../../apps/web/app/(app)/reviewer-ops/WorkspaceGateState.tsx");
...
describe("Phase 32.8E — /reviewer-ops main console workspace wiring", () => {
    ...
    it("never short-circuits to a bare 'Switch to a workspace' string outside its bounded Shell", () => {
        ...
        if (matches) {
            throw new Error("Main /reviewer-ops console has an inline 'Switch to a workspace' render outside the bounded Shell.");
        }
    });
});
// -----------------------------------------------------------------------------
    ...
    for (const { name, rel, surface } of pages) {
        describe(name, () => {
            ...
            it("does NOT call /v1/users/me directly for workspace resolution", () => {
                ...
                expect(live).not.toMatch(/setTeamId\s*\(\s*r\?\.user\?\.currentWorkspaceId/);
            });
            ...
            it("never short-circuits to a bare 'Switch to a workspace' string outside the canonical gate", () => {
                ...
                if (inlineGate) {
                    throw new Error(`Page ${name} has an inline 'Switch to a workspace' render. The shared gate must own this string.`);
                }
            });
        });
    }
...
describe("/home is workspace-resolution-resilient (does not regress)", () => {
    ...
    const cc = readSource("../../../apps/web/components/command-center/CommandCenter.tsx");
    ...
    it("/home does NOT depend directly on user.currentWorkspaceId for primary load", () => {
        ...
        const live = cc
            .replace(/\/\*[\s\S]*?\*\//g, "")
            .replace(/\/\/[^\n]*/g, "");
        expect(live).not.toMatch(/apiFetch\([^)]*\/v1\/users\/me[^)]*\)[\s\S]{0,300}currentWorkspaceId/);
    });
});
