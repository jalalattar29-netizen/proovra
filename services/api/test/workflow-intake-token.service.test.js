...
import { __testing, constantTimeEqualHex, hmacForIntake, isWorkflowIntakeFeatureEnabled, issueIntakeToken, lookupHashForIntakeToken, workflowIntakeFeatureDisabledReason, } from "../src/services/workflow-intake-token.service.js";
...
describe("workflow intake token service — verification", () => {
    ...
    it("returns different hashes under different secrets", () => {
        ...
        const hashUnderSecretA = __testing.withSecret(TEST_SECRET, () => lookupHashForIntakeToken(token.rawToken));
        const hashUnderSecretB = __testing.withSecret("b".repeat(64), () => lookupHashForIntakeToken(token.rawToken));
