...
// Bounded sets
// ---------------------------------------------------------------------------
const SPAN_NAME_SET = new Set(Object.values(PROOVRA_SPAN_NAMES));
...
const COUNTER_ALLOWLIST_EXTRAS = new Set([
...
//    PROOVRA_SPAN_NAMES.
// ---------------------------------------------------------------------------
function extractSpanReferences(promql) {
...
function spanRefIsRealOrPrefixOfReal(ref) {
    // Exact match → real span.
    if (SPAN_NAME_SET.has(ref))
        return true;
    ...
    if (ref === "proovra.worker.graph.reconcile")
        return true;
    return false;
}
            ...
            for (const panel of json.panels ?? []) {
                for (const tgt of panel.targets ?? []) {
                    ...
                    for (const ref of refs) {
                        expect.soft(spanRefIsRealOrPrefixOfReal(ref), `Dashboard ${path} panel "${panel.title}" references phantom span: ${ref}`).toBe(true);
                    }
                }
            }
        });
        ...
        it(`${path} only references COUNTER_NAMES entries`, () => {
            ...
            for (const panel of json.panels ?? []) {
                for (const tgt of panel.targets ?? []) {
                    ...
                    const bucketTokens = expr.match(/[a-z][a-z0-9_]+_(bucket|seconds|milliseconds)\b/g) ??
                        [];
                    for (const tok of [...tokens, ...bucketTokens]) {
                        const ok = COUNTER_SET.has(tok) || COUNTER_ALLOWLIST_EXTRAS.has(tok);
                        expect.soft(ok, `Dashboard ${path} panel "${panel.title}" references phantom counter: ${tok}`).toBe(true);
                    }
                }
            }
        });
...
describe("O1.6 — alert PromQL references real spans + counters", () => {
    ...
    for (let i = 0; i < lines.length; i++) {
        if (/^\s*expr:\s*\|\s*$/.test(lines[i])) {
            ...
            for (let j = i + 1; j < lines.length; j++) {
                ...
                if (indent < baseIndent && lines[j].trim() !== "")
                    break;
                block.push(lines[j]);
            }
    ...
    it("every span_name referenced by an alert is a real span", () => {
        for (const expr of exprBlocks) {
            ...
            for (const ref of refs) {
                expect.soft(spanRefIsRealOrPrefixOfReal(ref), `Alert references phantom span: ${ref}`).toBe(true);
            }
        }
    });
    it("every counter referenced by an alert is a real counter", () => {
        for (const expr of exprBlocks) {
            ...
            for (const tok of [...tokens, ...bucketTokens]) {
                const ok = COUNTER_SET.has(tok) || COUNTER_ALLOWLIST_EXTRAS.has(tok);
                expect.soft(ok, `Alert references phantom counter: ${tok}`).toBe(true);
            }
        }
    });
});
...
describe("O1.6 — dashboard family coverage", () => {
    it("each span-name family has at least one dashboard referencing it", () => {
        ...
        const ALREADY_COVERED_BY_O1_2 = new Set([
        ...
        for (const family of families) {
            if (ALREADY_COVERED_BY_O1_2.has(family))
                continue;
