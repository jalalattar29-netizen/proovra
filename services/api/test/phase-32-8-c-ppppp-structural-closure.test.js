...
const MIGRATION = readApi("prisma/migrations/20260626100000_phase328cppppp_structural_intelligence_closure/migration.sql");
...
describe("Phase 32.8C+++++ — migration file source-contract", () => {
    it("creates queue_telemetry_snapshots idempotently", () => {
        expect(MIGRATION).toMatch(/CREATE TABLE IF NOT EXISTS "queue_telemetry_snapshots"/);
    });
    it("creates worker_telemetry_snapshots idempotently", () => {
        expect(MIGRATION).toMatch(/CREATE TABLE IF NOT EXISTS "worker_telemetry_snapshots"/);
    });
    ...
    it("creates operational_timeline_events idempotently with the idempotency unique", () => {
        expect(MIGRATION).toMatch(/CREATE TABLE IF NOT EXISTS "operational_timeline_events"/);
    ...
    it("ALTER TABLE on evidence_integrity_snapshots uses ADD COLUMN IF NOT EXISTS", () => {
        expect(MIGRATION).toMatch(/ALTER TABLE "evidence_integrity_snapshots"[\s\S]*?ADD COLUMN IF NOT EXISTS "tsa_issuer_common_name"/);
        expect(MIGRATION).toMatch(/ADD COLUMN IF NOT EXISTS "tsa_parse_status"/);
    });
    it("queue_telemetry_snapshots CASCADEs on team delete", () => {
        expect(MIGRATION).toMatch(/queue_telemetry_snapshots_team_id_fkey[\s\S]*?ON DELETE CASCADE/);
    });
...
describe("Phase 32.8C+++++ — case-comment.service.ts", () => {
    ...
    it("reader degrades to zero counts on failure", () => {
        expect(CASE_COMMENT).toMatch(/catch\s*\{\s*return\s*\{\s*openCount:\s*0,\s*resolvedCount:\s*0,\s*staleOpenCount:\s*0\s*\};\s*\}/);
    });
});
// =============================================================================
// PART 8 — TSA issuer derivation (no-op stub, UNAVAILABLE by default)
...
describe("Phase 32.8C+++++ — dashboard wiring", () => {
    it("imports the 5 new services", () => {
        expect(COMMAND_CENTER).toMatch(/from\s*"\.\/queue-telemetry\.service\.js"/);
        expect(COMMAND_CENTER).toMatch(/from\s*"\.\/worker-telemetry\.service\.js"/);
        expect(COMMAND_CENTER).toMatch(/from\s*"\.\/case-evidence-link\.service\.js"/);
        expect(COMMAND_CENTER).toMatch(/from\s*"\.\/operational-timeline\.service\.js"/);
    ...
    it("queue/worker telemetry section advertises new tables in sourceSummary", () => {
        expect(COMMAND_CENTER).toMatch(/QueueTelemetrySnapshot[^"\n]*Phase 32\.8C\+\+\+\+\+/);
        expect(COMMAND_CENTER).toMatch(/WorkerTelemetrySnapshot[^"\n]*Phase 32\.8C\+\+\+\+\+/);
    });
    ...
    it("cross-case section advertises CaseEvidenceLink in sourceSummary", () => {
        expect(COMMAND_CENTER).toMatch(/CaseEvidenceLink[^"\n]*Phase 32\.8C\+\+\+\+\+/);
    });
    ...
    it("coordination signals section no longer declares 'no resolved/unresolved column' as unsupported", () => {
        expect(COMMAND_CENTER).not.toMatch(/no resolved\/unresolved column on EvidenceAnnotation/);
    });
    it("coordination signals section advertises CaseComment + resolvedAtUtc sources", () => {
        expect(COMMAND_CENTER).toMatch(/CaseComment[^"\n]*Phase 32\.8C\+\+\+\+\+/);
        expect(COMMAND_CENTER).toMatch(/EvidenceReviewerComment\(resolvedAtUtc IS NULL — Phase 32\.8C\+\+\+\+\)/);
    });
    it("reconstructed timeline advertises OperationalTimelineEvent", () => {
        expect(COMMAND_CENTER).toMatch(/OperationalTimelineEvent[^"\n]*Phase 32\.8C\+\+\+\+\+/);
    });
    it("deep integrity unsupported signal is reframed (worker parser not yet deployed) — not the schema gap", () => {
        ...
        expect(COMMAND_CENTER).not.toMatch(/Evidence has tsaTokenBase64 \+ tsaGenTimeUtc but no tsaIssuer column/);
    });
