...
import { applyEnforcementMode, evaluateSecurityPolicy, } from "../src/services/governance/policy-evaluation.service.js";
import { POLICY_RUNTIME_GATE_REGISTRY, gateRedactionAction, gateRetentionAction, gateReviewDecision, gateSecurityAction, gateVerificationAction, } from "../src/services/governance/policy-runtime-gates.service.js";
import { auditCrossDepartmentAccess, buildDepartmentScopeWhere, buildStrictDepartmentScopeWhere, } from "../src/services/governance/department-scope.service.js";
import { probeAzureDocumentIntelligence, probeDeepgram, probeOpenAi, probeRekognition, } from "../src/services/trust/status-probes.service.js";
// ---------------------------------------------------------------------------
...
function makePolicy(enforcementMode, rule) {
    return {
        id: "policy-test-id",
        teamId: "team-1",
        kind: "SECURITY",
...
function makePrismaSinglePolicy(enforcementMode, rule) {
...
describe("1. Enforcement mode honored", () => {
    ...
    it("blockedPublicExposure AUDIT_ONLY → ALLOW (not BLOCK)", () => {
        const result = applyEnforcementMode("AUDIT_ONLY", "public_exposure_blocked", "p2");
        expect(result.decision).toBe("ALLOW");
        expect(result.reason).toBe("public_exposure_blocked");
    });
    it("requireDualApproval AUDIT_ONLY → ALLOW", () => {
        const result = applyEnforcementMode("AUDIT_ONLY", "dual_approval_required", "p3");
        expect(result.decision).toBe("ALLOW");
    });
    it("requireLegalHoldCheck AUDIT_ONLY → ALLOW", () => {
        const result = applyEnforcementMode("AUDIT_ONLY", "legal_hold_active", "p4");
        expect(result.decision).toBe("ALLOW");
    });
    it("minConfidenceBand AUDIT_ONLY → ALLOW", () => {
        const result = applyEnforcementMode("AUDIT_ONLY", "confidence_below_band", "p5");
        expect(result.decision).toBe("ALLOW");
    });
    it("requireTrustReferences AUDIT_ONLY → ALLOW", () => {
        const result = applyEnforcementMode("AUDIT_ONLY", "trust_references_missing", "p6");
        expect(result.decision).toBe("ALLOW");
    });
});
// ---------------------------------------------------------------------------
...
describe("4. auditCrossDepartmentAccess", () => {
    ...
    it("source contains emitTrustEvent with cross_department_admin_access reason", () => {
        const src = fs.readFileSync(path.resolve("src/services/governance/department-scope.service.ts"), "utf8");
...
describe("5. Trust article SUPERSEDED emission", () => {
    it("trust-center.service.ts source contains TRUST_ARTICLE_SUPERSEDED emit in upsertTrustArticle", () => {
        const src = fs.readFileSync(path.resolve("src/services/trust/trust-center.service.ts"), "utf8");
...
describe("6. Trust drift legacy casts removed", () => {
    it("trust-drift.service.ts does NOT contain 'as never' casts paired with TRUST_ARTICLE_MARKED_STALE or TRUST_ARTICLE_REVIEWED codes", () => {
        const src = fs.readFileSync(path.resolve("src/services/trust/trust-drift.service.ts"), "utf8");
        ...
        for (const line of lines) {
            if ((line.includes("TRUST_ARTICLE_MARKED_STALE") ||
                line.includes("TRUST_ARTICLE_REVIEWED")) &&
                line.includes("as never")) {
                throw new Error(`Found 'as never' on line with event code: ${line.trim()}`);
            }
        }
    });
});
...
describe("7. Security Center seed paths corrected", () => {
    ...
    it("reads trust-center.service.ts successfully", () => {
        src = fs.readFileSync(path.resolve("src/services/trust/trust-center.service.ts"), "utf8");
        expect(src.length).toBeGreaterThan(1000);
    });
    it("AUTHORIZATION implementationReferences does NOT reference access-grants.service.ts", () => {
        const src2 = fs.readFileSync(path.resolve("src/services/trust/trust-center.service.ts"), "utf8");
...
describe("8. External provider probes", () => {
    ...
    it("probeAll source contains all 4 new probe names", () => {
        const src = fs.readFileSync(path.resolve("src/services/trust/status-probes.service.ts"), "utf8");
...
describe("9. Verification package preview route", () => {
    it("trust-and-governance.routes.ts contains /v1/trust/verification-package/preview", () => {
        const src = fs.readFileSync(path.resolve("src/routes/trust-and-governance.routes.ts"), "utf8");
...
describe("10. Cross-org external grant validation", () => {
    it("cross-org-review.service.ts source contains external_review_grants query inside acceptCrossOrgReview", () => {
        const src = fs.readFileSync(path.resolve("src/services/governance/cross-org-review.service.ts"), "utf8");
...
describe("12. media-intelligence backslash fixed", () => {
    it("line ~200 of media-intelligence.service.ts does NOT start with a backslash-comment", () => {
        const lines = fs
            .readFileSync(path.resolve("src/services/intelligence/media-intelligence.service.ts"), "utf8")
            .split("\n");
...
describe("13. SECURITY gate wired into auth middleware", () => {
    it("auth.ts imports or calls gateSecurityAction", () => {
        const src = fs.readFileSync(path.resolve("src/middleware/auth.ts"), "utf8");
        // Broader check: either import or call site
        const hasGate = src.includes("gateSecurityAction") ||
...
describe("14. RETENTION gate wired into evidence routes/service", () => {
    it("gateRetentionAction appears in evidence.routes.ts or evidence.service.ts", () => {
        const routesSrc = fs.readFileSync(path.resolve("src/routes/evidence.routes.ts"), "utf8");
        ...
        if (!found) {
            ...
            let svcSrc = "";
            try {
                svcSrc = fs.readFileSync(path.resolve("src/services/evidence.service.ts"), "utf8");
            }
            catch {
...
describe("16. VERIFICATION gate wired", () => {
    it("gateVerificationAction appears in evidence.routes.ts or related service", () => {
        const src = fs.readFileSync(path.resolve("src/routes/evidence.routes.ts"), "utf8");
        const found = src.includes("gateVerificationAction") ||
...
describe("18. Department scope wired into executive metrics", () => {
    it("executive-metrics.service.ts references department scope logic", () => {
        const src = fs.readFileSync(path.resolve("src/services/intelligence/executive-metrics.service.ts"), "utf8");
        const hasDeptScope = src.includes("buildStrictDepartmentScopeWhere") ||
