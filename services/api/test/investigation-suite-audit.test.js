...
describe("Investigation Suite audit — EMPTY_STATE_COPY pins", () => {
    ...
    it("investigation duplicates renders the perceptual-similarity caveat (fix #4)", () => {
        ...
        expect(src).toMatch(/Perceptual similarity is not yet available on\s+this workspace/);
        const stripped = stripComments(src);
        expect(stripped).not.toMatch(/data unavailable/i);
    });
    it("investigation graph renders the new 'No graph yet' empty-state copy (fix #5)", () => {
        const src = readPage(PAGES.graph);
        expect(src).toMatch(/No graph yet — capture evidence and create cases to populate/);
        const stripped = stripComments(src);
        expect(stripped).not.toMatch(/data unavailable/i);
...
describe("Investigation Suite audit — NAV_VISIBILITY pin (fix #7)", () => {
    it("routeRegistry entry for investigation.reviewers is reviewer-only on the sidebar but reachable via cmd-K + All Tools", () => {
        ...
        expect(registry).toMatch(/id:\s*"investigation\.reviewers"[\s\S]{0,2000}sidebarEligible:\s*false/);
        expect(registry).toMatch(/id:\s*"investigation\.reviewers"[\s\S]{0,2000}commandPaletteVisible:\s*true/);
        expect(registry).toMatch(/id:\s*"investigation\.reviewers"[\s\S]{0,2000}allToolsVisible:\s*true/);
    });
    it("routeRegistry leaves backend gating unchanged for investigation.reviewers (capability + active space + fallback)", () => {
        ...
        expect(registry).toMatch(/id:\s*"investigation\.reviewers"[\s\S]{0,2000}requiredCapabilities:\s*\[\s*"EVIDENCE_VIEW"\s*\]/);
        expect(registry).toMatch(/id:\s*"investigation\.reviewers"[\s\S]{0,2000}requiredActiveSpace:\s*"PERSONAL_OR_ORG"/);
        expect(registry).toMatch(/id:\s*"investigation\.reviewers"[\s\S]{0,2000}fallbackBehavior:\s*"DEGRADED"/);
    });
});
// ---------------------------------------------------------------------------
