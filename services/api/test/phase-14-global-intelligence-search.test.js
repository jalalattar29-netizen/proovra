...
describe("Phase 14 — Stage 2 re-index triggers (backend)", () => {
    const EXTRACTION_PATH = resolve(API_ROOT, "src/services/intelligence/extraction.service.ts");
    const GRAPH_BUILDER_PATH = resolve(SHARED_RUNTIME_ROOT, "src/graph/graph-builder.service.ts");
    const EVIDENCE_COMPLETE_PATH = resolve(API_ROOT, "src/services/evidence-complete.service.ts");
    const MEDIA_INTEL_PATH = resolve(WORKER_ROOT, "src/media-intelligence.processor.ts");
    ...
    it("each enqueueSearchIndexingJob site in extraction.service.ts is wrapped in try/catch (best-effort)", () => {
        ...
        const guardedRe = /try\s*\{[\s\S]*?enqueueSearchIndexingJob\([\s\S]*?\}\s*catch/g;
...
describe("Phase 14 — Stage 3 /v1/intelligence/search consolidation (alias forward)", () => {
    const INTEL_ROUTES_PATH = resolve(API_ROOT, "src/routes/intelligence.routes.ts");
    const SEARCH_ROUTES_PATH = resolve(API_ROOT, "src/routes/search.routes.ts");
    const EVIDENCE_SEARCH_PATH = resolve(API_ROOT, "src/services/search/evidence-search.service.ts");
...
describe("Phase 14 — /search canonical route (frontend)", () => {
    ...
    it("workspace.search → /search remains the canonical route entry in routeRegistry", () => {
        ...
        // pin #10).
        expect(src).toMatch(/id:\s*"workspace\.search"[\s\S]{0,200}href:\s*"\/search"/);
    });
...
describe("Phase 14 — Stage 6 deep-link affordances (frontend)", () => {
    it("evidence/[id]/page.tsx imports EntityChipGroup (which links chips into /search?q=)", () => {
        const src = read(resolve(WEB_ROOT, "app/(app)/evidence/[id]/page.tsx"));
    ...
    it("components/intelligence/EntityChipGroup.tsx wires each chip into a /search?q= deep link", () => {
        const src = read(resolve(WEB_ROOT, "components/intelligence/EntityChipGroup.tsx"));
    ...
    it("reports surface (ReportsIndex.tsx) contains a /search?documentType=REPORT deep link", () => {
        ...
        const src = read(resolve(WEB_ROOT, "components/reports-experience/ReportsIndex.tsx"));
    ...
    it("investigation/graph/page.tsx contains a /search?q= deep link (entity affordance)", () => {
        const src = read(resolve(WEB_ROOT, "app/(app)/investigation/graph/page.tsx"));
        expect(src).toMatch(/\/search\?q=/);
    });
    it("investigation/timeline/page.tsx contains a /search? deep link", () => {
        const src = read(resolve(WEB_ROOT, "app/(app)/investigation/timeline/page.tsx"));
        expect(src).toMatch(/\/search\?/);
    });
    it("investigation/duplicates/page.tsx contains a /search? deep link", () => {
        const src = read(resolve(WEB_ROOT, "app/(app)/investigation/duplicates/page.tsx"));
        expect(src).toMatch(/\/search\?/);
    });
});
// ===========================================================================
...
describe("Phase 14 — bounded guards (no new schema / no v2 surfaces)", () => {
    ...
    it("Phase 14 added NO new Prisma migration (phase has no schema changes)", () => {
        ...
        const offenders = listDirs(MIGRATIONS_DIR).filter((d) => {
            if (!/phase[_-]?14|phase14/i.test(d))
                return false;
    ...
    it("no new search service file has 'v2' in its name", () => {
        const offenders = listFiles(SEARCH_SERVICES_DIR).filter((f) => /v2/i.test(f));
        expect(offenders, offenders.join("\n")).toEqual([]);
    });
    it("no new route file matches search.*v2 (would imply a Search v2 route)", () => {
        const offenders = listFiles(ROUTES_DIR).filter((f) => /search.*v2|v2.*search/i.test(f));
        expect(offenders, offenders.join("\n")).toEqual([]);
    });
});
