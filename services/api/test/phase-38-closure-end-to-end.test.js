...
describe("Phase 38-closure — persona mutation route", () => {
    it("declares both GET and PATCH endpoints on /v1/workspaces/:teamId/persona", () => {
        expect(ROUTE).toMatch(/app\.get\(\s*"\/v1\/workspaces\/:teamId\/persona"/);
        expect(ROUTE).toMatch(/app\.patch\(\s*"\/v1\/workspaces\/:teamId\/persona"/);
    });
...
describe("Phase 38-closure — terminology layer", () => {
    ...
    it("LAWYER renames Case → Matter (canonical product example)", async () => {
        const { resolveTerminology } = await import("../../../apps/web/lib/platform-context/useTerminology.js");
    ...
    it("every persona returns a complete terminology map (no missing keys)", async () => {
        ...
        for (const persona of personas) {
            ...
            for (const k of requiredKeys) {
                expect(typeof t[k] === "string" &&
                    t[k].length > 0, `${persona} must provide terminology[${k}]`).toBe(true);
            }
        }
    });
...
describe("Phase 38-closure — persona empty-state library", () => {
    ...
    it("returns a complete shape for every surface (default + per-persona override)", async () => {
        const { resolvePersonaEmptyState } = await import("../../../apps/web/lib/platform-context/personaEmptyStates.js");
        const surfaces = [
            "cases",
            "reports",
            "evidence",
...
describe("Phase 38-closure — topbar + onboarding", () => {
    ...
    it("onboarding wizard calls PATCH /v1/workspaces/:teamId/persona (no direct DB access)", () => {
        expect(WEB_WIZARD).toMatch(/apiFetch\(`\/v1\/workspaces\/\$\{encodeURIComponent\(workspaceId\)\}\/persona`/);
