...
describe("Phase 31.17 — REVIEW_TASK graph domain", () => {
    it("section header present + ordered after EXPORT and before MEDIA_SIGNAL", () => {
        ...
        const idxTask = RECONCILER_SRC.indexOf("Phase 31.17 — REVIEW_TASK domain reconciliation");
    ...
    it("SELECT JOIN'd evidence_review_workflows → evidence + team-anchored", () => {
        expect(RECONCILER_SRC).toMatch(/FROM "evidence_review_workflows" w[\s\S]*?JOIN "evidence" e ON e\."id" = w\."evidence_id"[\s\S]*?WHERE w\."team_id" = \$1[\s\S]*?AND e\."deleted_at" IS NULL/);
    });
    it("upserts REVIEW_TASK node with WORKSPACE_INTERNAL visibility", () => {
        expect(RECONCILER_SRC).toMatch(/upsertNode\(\s*client,\s*teamId,\s*"REVIEW_TASK"[\s\S]*?"WORKSPACE_INTERNAL"/);
    });
    it("emits REVIEWED_BY edge with SYSTEM source + HIGH confidence", () => {
        expect(RECONCILER_SRC).toMatch(/upsertEdge\([\s\S]*?"REVIEWED_BY"[\s\S]*?"SYSTEM"[\s\S]*?"HIGH"/);
    });
    it("REVIEW_TASK label NEVER pulls reviewer-private text fields", () => {
        const idx = RECONCILER_SRC.indexOf("Phase 31.17 — REVIEW_TASK domain reconciliation");
        const idxEnd = RECONCILER_SRC.indexOf("Phase 31.17 — ESCALATION domain reconciliation");
        ...
        expect(slice).toMatch(/SELECT w\."id", w\."evidence_id", w\."status"::text AS "status"[\s\S]*?w\."priority"::text AS "priority"/);
    ...
    it("stale-sweep for REVIEW_TASK is team-anchored on both UPDATE and sub-select", () => {
        const block = RECONCILER_SRC.match(/UPDATE "investigation_graph_nodes" n[\s\S]*?n\."node_kind" = 'REVIEW_TASK'[\s\S]*?\)\s*\)/)?.[0];
...
describe("Phase 31.17 — ESCALATION graph domain", () => {
    it("section header present", () => {
        expect(RECONCILER_SRC).toContain("Phase 31.17 — ESCALATION domain reconciliation");
    });
    it("SELECT on review_escalations is team-anchored", () => {
        expect(RECONCILER_SRC).toMatch(/FROM "review_escalations"\s+WHERE "team_id" = \$1/);
    });
    it("upserts ESCALATION node with WORKSPACE_INTERNAL visibility", () => {
        expect(RECONCILER_SRC).toMatch(/upsertNode\(\s*client,\s*teamId,\s*"ESCALATION"[\s\S]*?"WORKSPACE_INTERNAL"/);
    });
    it("emits ESCALATED_FROM edge with SYSTEM source + severityToConfidence", () => {
        expect(RECONCILER_SRC).toMatch(/upsertEdge\([\s\S]*?"ESCALATED_FROM"[\s\S]*?"SYSTEM"[\s\S]*?severityToConfidence/);
    });
    it("ESCALATION label NEVER pulls operator-private text fields", () => {
        const idx = RECONCILER_SRC.indexOf("Phase 31.17 — ESCALATION domain reconciliation");
        const idxEnd = RECONCILER_SRC.indexOf("Phase 31.17 — INCIDENT domain reconciliation");
        const slice = RECONCILER_SRC.slice(idx, idxEnd);
    ...
    it("stale-sweep for ESCALATION is team-anchored on both UPDATE and sub-select", () => {
        const block = RECONCILER_SRC.match(/UPDATE "investigation_graph_nodes" n[\s\S]*?n\."node_kind" = 'ESCALATION'[\s\S]*?\)\s*\)/)?.[0];
...
describe("Phase 31.17 — INCIDENT graph domain", () => {
    it("section header present", () => {
        expect(RECONCILER_SRC).toContain("Phase 31.17 — INCIDENT domain reconciliation");
    });
    it("SELECT on operational_incidents is team-anchored", () => {
        expect(RECONCILER_SRC).toMatch(/FROM "operational_incidents"\s+WHERE "team_id" = \$1/);
    });
    it("upserts INCIDENT node with WORKSPACE_INTERNAL visibility", () => {
        expect(RECONCILER_SRC).toMatch(/upsertNode\(\s*client,\s*teamId,\s*"INCIDENT"[\s\S]*?"WORKSPACE_INTERNAL"/);
    });
    it("emits REFERENCES_SAME_INCIDENT edge only when related_evidence_id is present", () => {
        expect(RECONCILER_SRC).toMatch(/if \(inc\.related_evidence_id\)[\s\S]*?upsertEdge\([\s\S]*?"REFERENCES_SAME_INCIDENT"[\s\S]*?"SYSTEM"[\s\S]*?severityToConfidence/);
    });
    it("INCIDENT label uses bounded category/severity/status enums — NEVER raw title or safe_summary", () => {
        const idx = RECONCILER_SRC.indexOf("Phase 31.17 — INCIDENT domain reconciliation");
    ...
    it("stale-sweep for INCIDENT is team-anchored on both UPDATE and sub-select", () => {
        const block = RECONCILER_SRC.match(/UPDATE "investigation_graph_nodes" n[\s\S]*?n\."node_kind" = 'INCIDENT'[\s\S]*?\)\s*\)/)?.[0];
...
describe("Phase 31.17 — severityToConfidence helper", () => {
    it("function defined + maps CRITICAL/HIGH → HIGH", () => {
        expect(RECONCILER_SRC).toMatch(/function severityToConfidence\([\s\S]*?CRITICAL[\s\S]*?HIGH[\s\S]*?HIGH/);
    });
    it("maps WARNING/MEDIUM → MEDIUM", () => {
        expect(RECONCILER_SRC).toMatch(/"WARNING"\s*\|\|\s*s === "MEDIUM"[\s\S]*?return "MEDIUM"/);
    });
    it("falls back to LOW", () => {
        const fn = RECONCILER_SRC.match(/function severityToConfidence[\s\S]*?return "LOW";\s*\n\}/)?.[0];
...
describe("Phase 31.17 — anti-leak invariants on new domain steps", () => {
    it("no storage internals anywhere in the 3 new steps", () => {
        const idxStart = RECONCILER_SRC.indexOf("Phase 31.17 — REVIEW_TASK domain reconciliation");
        const idxEnd = RECONCILER_SRC.indexOf("// 2. Materialize MEDIA_SIGNAL");
    ...
    it("each domain step is independently wrapped in try/catch", () => {
        ...
        const idxEnd = RECONCILER_SRC.indexOf("Phase 31.18 — EXTERNAL_REVIEW domain reconciliation");
        const slice = RECONCILER_SRC.slice(idxStart, idxEnd);
...
describe("Phase 32.16 — Relationship Inspector UI", () => {
    const src = readSource("../../../apps/web/app/(app)/investigation/relationships/page.tsx");
    ...
    it("no forbidden truth-claim vocabulary in user-facing literals", () => {
        ...
        const forbidden = /\b(tamper(ed|ing)?|forged|fake|authentic(ity)?|admissible|proves?|confirms?|manipulated|doctored)\b/i;
        for (const lit of literals) {
            expect(lit, `inspector forbidden wording: ${lit}`).not.toMatch(forbidden);
        }
    });
    ...
    it("links back to Case Graph Explorer + node-detail navigation present", () => {
        ...
        expect(src).toMatch(/\/investigation\/relationships\?caseId=\$\{encodeURIComponent\(caseId\)\}&nodeId=/);
        expect(src).toMatch(/\/investigation\/relationships\?caseId=\$\{encodeURIComponent\(caseId\)\}&edgeId=/);
    });
});
