...
import { DISAGREEMENT_STATES, REVIEWER_HOTKEY_CODES, REVIEWER_DEFAULT_HOTKEYS, isAllowedDisagreementTransition, reviewerCapabilitiesForRole, } from "@proovra/shared";
function readSource(rel) {
    return readFileSync(fileURLToPath(new URL(rel, import.meta.url)), "utf8");
}
const SHARED = readSource("../../../packages/shared/src/reviewer-workspace.ts");
const SHARED_INDEX = readSource("../../../packages/shared/src/index.ts");
const SCHEMA = readSource("../../../services/api/prisma/schema.prisma");
const ROLES = readSource("../../../services/api/src/services/reviewer-workspace/reviewer-roles.ts");
const CODING_SCHEMA = readSource("../../../services/api/src/services/reviewer-workspace/coding-schema.service.ts");
const CODING_VALUE = readSource("../../../services/api/src/services/reviewer-workspace/coding-value.service.ts");
const DISAGREE_SVC = readSource("../../../services/api/src/services/reviewer-workspace/reviewer-disagreement.service.ts");
const QC_SVC = readSource("../../../services/api/src/services/reviewer-workspace/qc-sample.service.ts");
const WORKSPACE_SVC = readSource("../../../services/api/src/services/reviewer-workspace/reviewer-workspace.service.ts");
const METRICS_SVC = readSource("../../../services/api/src/services/reviewer-workspace/reviewer-metrics.service.ts");
const ROUTES = readSource("../../../services/api/src/routes/reviewer-workspace.routes.ts");
...
const WORKSPACE_PAGE = readSource("../../../apps/web/app/(app)/review/workspace/page.tsx");
const SCHEMAS_PAGE = readSource("../../../apps/web/app/(app)/review/schemas/page.tsx");
const QC_PAGE = readSource("../../../apps/web/app/(app)/review/qc/page.tsx");
const DISAGREE_PAGE = readSource("../../../apps/web/app/(app)/review/disagreements/page.tsx");
const METRICS_PAGE = readSource("../../../apps/web/app/(app)/review/metrics/page.tsx");
const CODING_PANEL = readSource("../../../apps/web/components/reviewer-workspace/CodingPanel.tsx");
const HOTKEYS_LIB = readSource("../../../apps/web/lib/reviewer-workspace/reviewer-hotkeys.ts");
const REVIEWER_API = readSource("../../../apps/web/lib/reviewer-workspace/reviewer-api.ts");
// ===========================================================================
// 1 — Shared contracts
// ===========================================================================
...
describe("Phase 2A — Prisma schema", () => {
    ...
    it("extends EvidenceReviewWorkflow with coding-schema binding", () => {
        const block = SCHEMA.match(/^model EvidenceReviewWorkflow \{[\s\S]*?\n\}/m)[0];
...
describe("Phase 2A — pillar nav", () => {
    ...
    it("pillarRegistry maps every new route to the REVIEW pillar", () => {
        for (const id of [
            ...
            expect(PILLAR_REGISTRY).toMatch(new RegExp(`"${id}"[\\s,]+"REVIEW"`));
        }
    });
});
