...
import { EXTERNAL_REVIEW_SCOPE_KINDS, EXTERNAL_REVIEW_GRANT_DENIAL_CODES, } from "../src/services/external-review/external-review-grant.service.js";
...
describe("Phase 27/28 — external_review_grants SQL drift patch", () => {
    const src = readSource("../../../services/api/sql/drift-patches/2026-05-19-external-review-grants.sql");
    it("creates the table with IF NOT EXISTS (idempotent re-run)", () => {
        expect(src).toMatch(/CREATE TABLE IF NOT EXISTS\s+"external_review_grants"/i);
    });
    it("scope_kind catalog is bounded by CHECK constraint", () => {
        expect(src).toMatch(/CONSTRAINT "external_review_grants_scope_kind_bounded"[\s\S]*?CHECK[\s\S]*?'EVIDENCE'[\s\S]*?'CASE'[\s\S]*?'PACKAGE'/i);
    });
    it("state catalog is bounded by CHECK constraint matching the shared state machine", () => {
        expect(src).toMatch(/CONSTRAINT "external_review_grants_state_bounded"[\s\S]*?CHECK[\s\S]*?'INVITED'[\s\S]*?'ACTIVE'[\s\S]*?'EXPIRED'[\s\S]*?'REVOKED'[\s\S]*?'BLOCKED_BY_POLICY'/i);
    });
    it("enforces one-of (evidence_id, case_id, package_id) by scope_kind", () => {
        expect(src).toMatch(/CONSTRAINT "external_review_grants_scope_target_present"[\s\S]*?'EVIDENCE'.*?evidence_id.*?NOT NULL[\s\S]*?'CASE'.*?case_id.*?NOT NULL[\s\S]*?'PACKAGE'.*?package_id.*?NOT NULL/i);
    });
    it("forbids non-positive grant lifetimes", () => {
        expect(src).toMatch(/CONSTRAINT "external_review_grants_expires_after_created"[\s\S]*?expires_at_utc.*?>.*?created_at_utc/i);
    });
    it("token_hash is UNIQUE (no duplicate tokens — security incident if violated)", () => {
        expect(src).toMatch(/CREATE UNIQUE INDEX IF NOT EXISTS "external_review_grants_token_hash_uk"/i);
    });
    it("partial index on expiry sweep (state IN INVITED, ACTIVE) supports the reconciliation worker", () => {
        expect(src).toMatch(/CREATE INDEX IF NOT EXISTS "external_review_grants_expiry_sweep_idx"[\s\S]*?WHERE "state" IN \('INVITED', 'ACTIVE'\)/i);
    });
    it("operator queue index supports team-scoped active-grant listing", () => {
        expect(src).toMatch(/CREATE INDEX IF NOT EXISTS "external_review_grants_team_state_expires_idx"[\s\S]*?\("team_id", "state", "expires_at_utc" DESC\)/i);
    });
...
describe("Phase 27/28 — external-review-grant.service.ts source contract", () => {
    const src = readSource("../../../services/api/src/services/external-review/external-review-grant.service.ts");
    it("imports the canonical state machine + evaluator from @proovra/shared", () => {
        expect(src).toMatch(/import\s*\{[\s\S]*?evaluateExternalReviewAccess[\s\S]*?isAllowedExternalReviewTransition[\s\S]*?\}\s*from\s+"@proovra\/shared"/);
    });
    ...
    it("lookupExternalReviewGrantByToken is anti-enumeration safe (same reason for unknown vs revoked)", () => {
        ...
        expect(src).toMatch(/if \(!raw\) \{[\s\S]*?return \{ ok: false, reason: "grant_not_active" \}/);
    });
    ...
    it("transitionExternalReviewGrant validates against the canonical state machine", () => {
        expect(src).toMatch(/isAllowedExternalReviewTransition\(fromState, input\.toState\)/);
        expect(src).toMatch(/return \{ ok: false, reason: "invalid_transition" \}/);
    });
    it("revocation flips state + writes revoked_at_utc + revoked_by_user_id (audit trail)", () => {
        expect(src).toMatch(/revokedAt = input\.toState === "REVOKED" \? new Date\(\) : null/);
    ...
    it("recordExternalReviewAccess is best-effort (never throws to the reviewer's read path)", () => {
        expect(src).toMatch(/recordExternalReviewAccess[\s\S]*?try\s*\{[\s\S]*?\}\s*catch\s*\{[\s\S]*?\/\/ Best-effort/);
    });
    ...
    it("reviewer_email is normalised to lowercase + trimmed + length-bounded before storage", () => {
        expect(src).toMatch(/input\.reviewerEmail\.toLowerCase\(\)\.trim\(\)\.slice\(0,\s*320\)/);
    });
    it("SecurityEvent details hash the reviewer email (never the raw value)", () => {
        expect(src).toMatch(/reviewerEmailHash:\s*hashToken\(input\.reviewerEmail/);
    });
});
// =============================================================================
...
describe("Phase 27/28 — schema validation registration", () => {
    ...
    it("registers the external_review_grants table", () => {
        expect(src).toMatch(/name:\s*"external_review_grants"[\s\S]*?subsystem:\s*"governance_lifecycle"/);
    });
    it("registers state + token_hash + expires_at_utc columns at CRITICAL severity", () => {
        expect(src).toMatch(/table:\s*"external_review_grants",\s*column:\s*"state",\s*severity:\s*"critical"/);
        expect(src).toMatch(/table:\s*"external_review_grants",\s*column:\s*"token_hash",\s*severity:\s*"critical"/);
        expect(src).toMatch(/table:\s*"external_review_grants",\s*column:\s*"expires_at_utc",\s*severity:\s*"critical"/);
    });
    it("registers the token_hash unique index at CRITICAL severity", () => {
        expect(src).toMatch(/indexName:\s*"external_review_grants_token_hash_uk",\s*severity:\s*"critical"/);
    });
    it("registers scope_kind + allow_original_download as IMPORTANT (governance-load-bearing)", () => {
        expect(src).toMatch(/table:\s*"external_review_grants",\s*column:\s*"scope_kind",\s*severity:\s*"important"/);
        expect(src).toMatch(/table:\s*"external_review_grants",\s*column:\s*"allow_original_download",\s*severity:\s*"important"/);
    });
});
// =============================================================================
...
describe("Phase 27/28 — privacy + governance invariants", () => {
    ...
    it("SQL never grants a default that bypasses governance (state defaults to INVITED, never ACTIVE)", () => {
        const src = readSource("../../../services/api/sql/drift-patches/2026-05-19-external-review-grants.sql");
        expect(src).toMatch(/"state"\s+VARCHAR\(24\)\s+NOT NULL DEFAULT 'INVITED'/);
    ...
    it("allow_original_download + allow_package_download default to false-erring side (original NEVER auto-enabled)", () => {
        const src = readSource("../../../services/api/sql/drift-patches/2026-05-19-external-review-grants.sql");
        expect(src).toMatch(/"allow_original_download"\s+BOOLEAN NOT NULL DEFAULT FALSE/);
    });
    it("anti-enumeration: token lookup returns identical denial code for unknown + revoked tokens", () => {
        ...
        expect(src).toMatch(/if \(!raw\) \{[\s\S]*?reason: "grant_not_active"/);
        expect(src).toMatch(/:\s*"grant_not_active"/);
    });
    it("compile-time type-level invariant: scope + denial vocabularies are typed", () => {
        const scope = "EVIDENCE";
        const denial = "grant_not_active";
