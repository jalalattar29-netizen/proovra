...
describe("32.8 Test 1 — root nav stays bounded to the canonical primaries", () => {
    ...
    it("CANONICAL_PRIMARY_ROUTE_IDS contains exactly the Phase B canonical set", () => {
        ...
        const m = GROUPS_SRC.match(/CANONICAL_PRIMARY_ROUTE_IDS[\s\S]*?new Set\(\[([\s\S]*?)\]\)/);
        expect(m, "CANONICAL_PRIMARY_ROUTE_IDS not found").toBeTruthy();
        const block = m[1];
        const ids = Array.from(block.matchAll(/["']([^"']+)["']/g)).map((mm) => mm[1]);
...
describe("32.8 Test 2 — sidebar root groups bounded by ALLOWED_ROOT_GROUP_TITLES", () => {
    ...
    it.each(REQUIRED_GROUP_TITLES)("ALLOWED_ROOT_GROUP_TITLES contains: %s", (title) => {
    ...
    it("no unexpected group title is added to ALLOWED_ROOT_GROUP_TITLES", () => {
        ...
        const m = GROUPS_SRC.match(/ALLOWED_ROOT_GROUP_TITLES[\s\S]*?=\s*\[([\s\S]*?)\];/);
        ...
        const literals = Array.from(block.matchAll(/["']([^"']+)["']/g)).map((mm) => mm[1]);
        const constantRefs = Array.from(block.matchAll(/SIDEBAR_GROUP_[A-Z_]+\.title/g)).length;
        ...
        expect(totalCount, `ALLOWED_ROOT_GROUP_TITLES has ${totalCount} entries; expected exactly 6.`).toBe(6);
    });
});
// ===========================================================================
...
describe("32.8 Test 3 — degradation chip labels use operational (not engineering) language", () => {
    ...
    it.each(Object.entries(EXPECTED_LABELS))("DEGRADATION_CHIP_LABELS.%s = %s", (key, expected) => {
        const regex = new RegExp(`${key}:\\s*"${expected}"`);
        expect(GROUPS_SRC, `DEGRADATION_CHIP_LABELS.${key} must be "${expected}".`).toMatch(regex);
    });
    it("no raw 'Org' / 'Access' chip labels remain", () => {
        ...
        const m = GROUPS_SRC.match(/DEGRADATION_CHIP_LABELS\s*=\s*\{([\s\S]*?)\}\s*as\s+const/);
...
describe("32.8 Test 4 — engineering terminology not exposed as routeRegistry labels", () => {
    ...
    it("every routeRegistry label passes the forbidden-pattern checks", () => {
        ...
        const labels = Array.from(REGISTRY_SRC.matchAll(/\blabel:\s*"([^"]+)"/g)).map((m) => m[1]);
        expect(labels.length).toBeGreaterThan(10);
        const offenders = [];
        for (const label of labels) {
            for (const { label: reason, regex } of FORBIDDEN_LABEL_PATTERNS) {
                if (regex.test(label))
                    offenders.push({ label, reason });
            }
        }
        expect(offenders, `Forbidden engineering terms in routeRegistry labels:\n${offenders
...
describe("32.8 Test 5 — 4 canonical hubs exist with intact membership", () => {
    ...
    it.each(EXPECTED_HUBS)("hub with landingRouteId $landingId exists in HUB_DEFINITIONS", ({ landingId }) => {
        expect(HUBS_SRC).toMatch(new RegExp(`landingRouteId:\\s*["']${landingId}["']`));
    });
...
describe("32.8 Test 6 — canonical page-structure primitives available", () => {
    ...
    it.each(PRIMITIVES)("$symbol is exported from $rel", ({ rel, symbol }) => {
        const fullPath = webPath(rel);
        expect(existsSync(fullPath), `Required primitive file missing: ${rel}`).toBe(true);
        const src = readFileSync(fullPath, "utf8");
        const re = new RegExp(`export\\s+(function|const|class)\\s+${symbol}\\b`);
...
describe("32.8 Test 7 — no fake-widget / placeholder copy in user-facing JSX", () => {
    ...
    it("no .tsx file under apps/web/app and apps/web/components contains forbidden user-facing copy", () => {
        ...
        expect(offenders, `Forbidden user-facing copy found:\n${offenders.join("\n")}`).toEqual([]);
    });
});
// ===========================================================================
...
describe("32.8 Test 11 — documentation + registry updated", () => {
    ...
    it("MASTER_PHASE_REGISTRY.md registers Phase 32.8 with explicit status", () => {
        ...
        expect(registry).toMatch(/\|\s*(Phase )?32\.8\s*\|[\s\S]*?(CLOSED|CLOSED_WITH_DEFERRED_ITEMS)/);
    });
});
