...
import { RBAC_DECISION_OUTCOMES, RBAC_DECISION_SOURCES, RBAC_PERMISSION_DOMAINS, SCIM_SCOPES, SCIM_TOKEN_PREFIX, SEARCH_FORBIDDEN_OVERCLAIM_PHRASES, SSO_CONNECTION_STATUSES, SSO_PROVIDERS, applyInheritanceChain, isAllowedSsoConnectionTransition, ssoProviderProtocol, } from "@proovra/shared";
...
describe("Phase 26 — RBAC engine wiring", () => {
    const src = readSource("../src/services/access-control/rbac-engine.service.ts");
...
describe("Phase 26 — SSO service", () => {
    const src = readSource("../src/services/access-control/sso.service.ts");
...
describe("Phase 26 — SCIM service", () => {
    const src = readSource("../src/services/access-control/scim.service.ts");
...
describe("Phase 26 — pure helpers", () => {
    ...
    it("SSO catalog hosts the brief's IdPs", () => {
        for (const p of ["GOOGLE_WORKSPACE", "OKTA", "AZURE_AD"]) {
            expect(SSO_PROVIDERS.includes(p)).toBe(true);
        }
    });
