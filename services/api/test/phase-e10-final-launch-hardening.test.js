...
const FORBIDDEN_LAUNCH_CLAIM_PATTERNS = [
...
describe("E10 Test 6 — no fake launch claims in phase doc / runbooks", () => {
    ...
    const isClaimShaped = (body, pattern) => {
    ...
    it.each(FORBIDDEN_LAUNCH_CLAIM_PATTERNS)("phase doc does NOT assert %s as a claim", (pattern) => {
        expect(isClaimShaped(PHASE_DOC, pattern), `phase doc contains a claim matching ${pattern}`).toBe(false);
    });
    for (const runbookName of E10_RUNBOOKS) {
        describe(`runbook ${runbookName}`, () => {
            const body = readRunbook(runbookName);
            it.each(FORBIDDEN_LAUNCH_CLAIM_PATTERNS)("does NOT assert %s as a claim", (pattern) => {
                expect(isClaimShaped(body, pattern), `${runbookName} contains a claim matching ${pattern}`).toBe(false);
            });
        });
    }
});
...
describe("E10 Test 10 — master registry updated", () => {
    it("registry records Phase E10 with explicit closure status", () => {
        expect(REGISTRY).toMatch(/\|\s*Phase\s+E10\s*\|[\s\S]*?(CLOSED|CLOSED_WITH_DEFERRED_ITEMS)/);
    });
...
describe("E10 Test 11 — every open DEF carries a classification", () => {
    ...
    for (const m of defRows) {
        ...
        it(`${defId} row carries an explicit classification keyword`, () => {
            expect(isClassified(row), `${defId} row has no classification: ${row.slice(0, 200)}`).toBe(true);
        });
    }
});
// ===========================================================================
