...
import { LARGE_FILE_THRESHOLD_BYTES, UPLOAD_ROUTING, isResumableEnabled, routeFile, } from "../../../apps/web/lib/uploads/feature-flag.js";
import { UPLOAD_TELEMETRY_TYPES, createUploadTelemetry, } from "../../../apps/web/lib/uploads/telemetry.js";
...
describe("Phase 30.10 — routing policy", () => {
    it("flag off → ALWAYS legacy regardless of size", () => {
        ...
        expect(routeFile({ sizeBytes: 1_000_000_000, flagEnabled: false })).toBe("legacy");
    });
    ...
    it("flag on + ≥ threshold → resumable", () => {
        expect(routeFile({ sizeBytes: LARGE_FILE_THRESHOLD_BYTES, flagEnabled: true })).toBe("resumable");
        expect(routeFile({
    ...
    it("negative / NaN size → legacy (defensive)", () => {
        ...
        expect(routeFile({ sizeBytes: Number.NaN, flagEnabled: true })).toBe("legacy");
    });
});
// =============================================================================
...
describe("Phase 30.10 — useResumableUploads hook source contract", () => {
    const src = readSource("../../../apps/web/app/(app)/capture/_hooks/useResumableUploads.ts");
    ...
    it("recovery scan is gated on the env flag (no work when flag is off)", () => {
        ...
        expect(src).toMatch(/useEffect\([\s\S]*?if\s*\(!enabled\)\s*return[\s\S]*?runUploadRecovery/);
    });
...
describe("Phase 30.10 — capture page integration", () => {
    const src = readSource("../../../apps/web/app/(app)/capture/page.tsx");
    it("imports useResumableUploads + UploadOperationsPanel", () => {
        expect(src).toMatch(/import\s*\{\s*useResumableUploads\s*\}\s*from\s+"\.\/_hooks\/useResumableUploads"/);
        expect(src).toMatch(/import\s*\{\s*UploadOperationsPanel\s*\}\s*from\s+"\.\.\/\.\.\/\.\.\/components\/uploads\/UploadOperationsPanel"/);
    });
...
describe("Phase 30.10 — backend telemetry endpoint", () => {
    ...
    it("declares POST /v1/ops/upload-telemetry under requireAuth", () => {
        expect(opsSrc).toMatch(/app\.post\(\s*"\/v1\/ops\/upload-telemetry",\s*\{\s*preHandler:\s*requireAuth\s*\}/);
    });
    it("bounded event type catalog matches the client emitter exactly", () => {
        for (const type of UPLOAD_TELEMETRY_TYPES) {
            expect(opsSrc, `event type ${type} missing from server enum`).toContain(`"${type}"`);
        }
    });
    it("body schema bounded — events array .min(1).max(50)", () => {
        expect(opsSrc).toMatch(/events:\s*z\.array\(UploadTelemetryEvent\)\.min\(1\)\.max\(50\)/);
    });
    it("per-user rate limit (60/min)", () => {
        expect(opsSrc).toMatch(/enforceRateLimit\(\{\s*key:\s*`ops:upload-telemetry:\$\{userId\}`/);
    ...
    it("anti-enumeration: non-members of the team get 404 not_found", () => {
        expect(opsSrc).toMatch(/if\s*\(!member\)\s*\{[\s\S]*?reply\.code\(404\)\.send\(\{\s*error:\s*\{\s*code:\s*"not_found"\s*\}\s*\}\)/);
    });
...
describe("Phase 30.10 — client telemetry emitter", () => {
    it("UPLOAD_TELEMETRY_TYPES exhaustive + snake_case", () => {
        for (const t of [
            ...
            expect(UPLOAD_TELEMETRY_TYPES).toContain(t);
        }
    ...
    it("emit() drops unknown types silently (no oracle)", () => {
        ...
        expect(() => e.emit("bogus_type")).not.toThrow();
    });
...
describe("Phase 30.10 — existing capture orchestrator integrity (UPDATED by Phase 30.12)", () => {
    const orchestratorSrc = readSource("../../../apps/web/app/(app)/capture/_hooks/useCaptureSessionOrchestration.ts");
    ...
    it("Phase 30.12 — imports the resumable lib BUT gates the fork on `resumable && resumable.enabled`", () => {
        expect(orchestratorSrc).toMatch(/from\s+"[^"]*lib\/uploads\/feature-flag/);
        expect(orchestratorSrc).toMatch(/from\s+"[^"]*lib\/uploads\/retry/);
        // The fork guard requires all four conditions; flag off ⇒
        // never engages.
        expect(orchestratorSrc).toMatch(/if\s*\([\s\S]*?resumable\s*&&[\s\S]*?resumable\.enabled\s*&&[\s\S]*?evidenceTeamId\s*&&[\s\S]*?routeFile\(/);
    });
    it("legacy XHR PUT path still exists (per-part presign → XHR)", () => {
        expect(orchestratorSrc).toMatch(/\/v1\/evidence\/\$\{evidenceId\}\/parts/);
    ...
    it("legacy finalize still calls /v1/evidence/:id/complete", () => {
        expect(orchestratorSrc).toMatch(/\/v1\/evidence\/\$\{evidenceId\}\/complete/);
    });
