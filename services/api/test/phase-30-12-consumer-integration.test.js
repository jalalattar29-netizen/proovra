...
import { buildSessionReadiness, } from "../../../apps/web/app/(app)/capture/_lib/session-readiness.js";
...
describe("Phase 30.12 — bridge SQL drift patch", () => {
    const sql = readSource("../../../services/api/sql/drift-patches/2026-05-19-evidence-upload-session-bridge.sql");
    ...
    it("declares the partIndex non-negative constraint via DO block (idempotent)", () => {
        expect(sql).toMatch(/DO \$\$[\s\S]*?evidence_upload_sessions_target_part_index_nonneg[\s\S]*?CHECK[\s\S]*?>= 0/);
    });
    it("creates the bridged-part lookup index", () => {
        expect(sql).toMatch(/CREATE INDEX IF NOT EXISTS "evidence_upload_sessions_bridged_part_idx"/);
    });
    it("schema-validation registers each new column at the right severity", () => {
        ...
        expect(src).toMatch(/column:\s*"target_part_index",\s*severity:\s*"critical"/);
        expect(src).toMatch(/column:\s*"original_file_name",\s*severity:\s*"critical"/);
        expect(src).toMatch(/column:\s*"expected_mime_type",\s*severity:\s*"important"/);
        expect(src).toMatch(/column:\s*"bridged_evidence_part_id",\s*severity:\s*"critical"/);
    });
});
// =============================================================================
...
describe("Phase 30.12 — createUploadSession bridge fields", () => {
    ...
    it("CreateSessionInput type declares the 3 bridge fields as optional", () => {
        expect(src).toMatch(/targetPartIndex\?:\s*number\s*\|\s*null/);
        expect(src).toMatch(/originalFileName\?:\s*string\s*\|\s*null/);
        expect(src).toMatch(/expectedMimeType\?:\s*string\s*\|\s*null/);
    });
    it("INSERT carries the 3 bridge columns + bounded lengths", () => {
        const inserts = src.match(/INSERT INTO "evidence_upload_sessions"\s*\([\s\S]*?\)/g) ?? [];
    ...
    it("invalid targetPartIndex returns bounded denial (invalid_part_index)", () => {
        expect(src).toMatch(/input\.targetPartIndex != null[\s\S]*?reason:\s*"invalid_part_index"/);
    });
});
// =============================================================================
...
describe("Phase 30.12 — completeStorageMultipart bridge", () => {
    ...
    it("marks ALL parts VERIFIED with whole-object server SHA-256 when verifyHash", () => {
        expect(src).toMatch(/if\s*\(input\.verifyHash\s*&&\s*serverSha256\)[\s\S]*?UPDATE\s+"evidence_upload_session_parts"[\s\S]*?SET\s+"state"\s*=\s*'VERIFIED'/);
    });
    it("VERIFIED update uses COALESCE so existing per-part sha256 is preserved", () => {
        expect(src).toMatch(/SET\s+"state"\s*=\s*'VERIFIED'[\s\S]*?"server_sha256"\s*=\s*COALESCE\("server_sha256"/);
    });
    it("VERIFIED update clears failure_reason (idempotent recovery)", () => {
        expect(src).toMatch(/SET\s+"state"\s*=\s*'VERIFIED'[\s\S]*?"failure_reason"\s*=\s*NULL/);
    });
    it("creates bridging EvidencePart row only when bridge metadata is present", () => {
        ...
        expect(src).toMatch(/input\.verifyHash[\s\S]*?serverSha256[\s\S]*?target_part_index != null[\s\S]*?original_file_name[\s\S]*?!session\.bridged_evidence_part_id/);
    });
    it("bridging EvidencePart row sets sha256 from server-computed hash (NEVER ETag)", () => {
        const bridgeBlock = src.match(/client\.evidencePart\.create\([\s\S]*?select:\s*\{\s*id:\s*true\s*\}/)?.[0];
    ...
    it("bridge persists bridged_evidence_part_id idempotently (no double-bridge on retry)", () => {
        expect(src).toMatch(/UPDATE\s+"evidence_upload_sessions"[\s\S]*?SET\s+"bridged_evidence_part_id"\s*=\s*\$3[\s\S]*?WHERE[\s\S]*?"bridged_evidence_part_id"\s+IS\s+NULL/);
    });
    it("SessionWithMultipart includes bridge fields so the guard can read them", () => {
        expect(src).toMatch(/target_part_index:\s*number\s*\|\s*null[\s\S]*?original_file_name:\s*string\s*\|\s*null[\s\S]*?bridged_evidence_part_id:\s*string\s*\|\s*null/);
    });
});
// =============================================================================
...
describe("Phase 30.12 — route bodies accept bridge fields", () => {
    it("web CreateSessionBodySchema accepts the bridge fields", () => {
        ...
        expect(src).toMatch(/targetPartIndex:\s*z\.number\(\)\.int\(\)\.min\(0\)\.max\(9_999\)\.optional\(\)/);
        expect(src).toMatch(/originalFileName:\s*z\.string\(\)\.min\(1\)\.max\(255\)\.optional\(\)/);
        expect(src).toMatch(/expectedMimeType:\s*z\.string\(\)\.min\(1\)\.max\(128\)\.optional\(\)/);
    });
    it("web create handler forwards the bridge fields to createUploadSession", () => {
        const src = readSource("../../../services/api/src/routes/upload-sessions.routes.ts");
        expect(src).toMatch(/createUploadSession\(\{[\s\S]*?targetPartIndex:\s*body\.targetPartIndex/);
...
describe("Phase 30.12 — capture-page fork", () => {
    const src = readSource("../../../apps/web/app/(app)/capture/_hooks/useCaptureSessionOrchestration.ts");
    const noComments = src
        .replace(/\/\*[\s\S]*?\*\//g, "")
        .replace(/\/\/[^\n]*/g, "");
    it("imports routeFile + planChunks + UseResumableUploadsApi (type-only)", () => {
        expect(src).toMatch(/import\s*\{\s*routeFile\s*\}\s*from\s+"\.\.\/\.\.\/\.\.\/\.\.\/lib\/uploads\/feature-flag"/);
        expect(src).toMatch(/import\s*\{\s*planChunks\s*\}\s*from\s+"\.\.\/\.\.\/\.\.\/\.\.\/lib\/uploads\/retry"/);
        expect(src).toMatch(/import\s*type\s*\{\s*UseResumableUploadsApi\s*\}/);
    });
    it("orchestrator accepts `resumable` as an optional parameter", () => {
        expect(src).toMatch(/resumable\?:\s*UseResumableUploadsApi\s*\|\s*null/);
    });
    it("fork engages ONLY when all 4 conditions hold (resumable + enabled + teamId + size route)", () => {
        expect(src).toMatch(/if\s*\([\s\S]*?resumable\s*&&[\s\S]*?resumable\.enabled\s*&&[\s\S]*?evidenceTeamId\s*&&[\s\S]*?routeFile\([\s\S]*?\)\s*===\s*"resumable"/);
    });
    ...
    it("runResumableItemUpload NEVER calls /v1/evidence/:id/complete (finalize stays at orchestrator level)", () => {
        ...
        const helper = src.match(/async function runResumableItemUpload\([\s\S]*?\n\}/)?.[0];
    ...
    it("runResumableItemUpload passes targetPartIndex + originalFileName + expectedMimeType", () => {
        ...
        expect(src).toMatch(/expectedMimeType:\s*normalizeClientMimeType\(item\.mimeType\)/);
    });
    it("runResumableItemUpload calls /multipart/complete with verifyHash:true (gate requirement)", () => {
        expect(src).toMatch(/\/multipart\/complete[\s\S]*?verifyHash:\s*true/);
    });
    it("runResumableItemUpload calls /complete (session COMPLETED transition) AFTER /multipart/complete", () => {
        ...
        const helperStart = src.indexOf("async function runResumableItemUpload");
        expect(helperStart).toBeGreaterThan(-1);
        const tail = src.slice(helperStart);
        const multipartCompleteIdx = tail.indexOf("/multipart/complete");
        const sessionCompleteIdx = tail.indexOf("${sessionId}/complete", multipartCompleteIdx + 1);
    ...
    it("page.tsx passes `resumable` to the orchestrator", () => {
        const pageSrc = readSource("../../../apps/web/app/(app)/capture/page.tsx");
...
describe("Phase 30.12 — Review & Sign readiness with resumable blockers", () => {
    ...
    it("one upload_in_progress blocker → canFinalize false + precise label", () => {
        ...
        expect(readiness.blockers.find((b) => b.code === "resumable_upload_in_progress")).toBeDefined();
    });
    it("every blocker kind produces a distinct precise label (no generic disabled)", () => {
        ...
        for (const kind of kinds) {
            ...
            const issue = readiness.blockers.find((b) => b.code.startsWith("resumable_"));
    ...
    it("hash_mismatch label uses custody-careful wording (file integrity)", () => {
        ...
        const issue = readiness.blockers.find((b) => b.code === "resumable_hash_mismatch");
...
describe("Phase 30.12 — observability", () => {
    ...
    it("manifest resolver bumps the materials counter + mixed-evidence counter", () => {
        ...
        expect(src).toMatch(/totals\.legacy > 0\s*&&\s*totals\.sessions > 0[\s\S]*?bump\("unified_manifest_mixed_evidence_total"\)/);
    });
});
// =============================================================================
...
describe("Phase 30.12 — anti-leak", () => {
    it("orchestrator helper NEVER projects storage internals or signed URLs", () => {
        ...
        for (const banned of [
            ...
            expect(noComments, `orchestrator helper leaks ${banned}`).not.toContain(banned);
        }
    });
    ...
    it("createUploadSession service NEVER stores raw client timestamps as authoritative", () => {
        ...
        expect(src).not.toMatch(/SET\s+"completed_at_utc"\s*=\s*\$\d/);
    });
});
