...
const MIGRATION_SQL = readSource("../prisma/migrations/20260930000000_phase_a0_integrity_hard_gate/migration.sql");
const SCHEMA = readSource("../prisma/schema.prisma");
const EVIDENCE_ROUTES = readSource("../src/routes/evidence.routes.ts");
const EVIDENCE_COMPLETE = readSource("../src/services/evidence-complete.service.ts");
describe("Phase A0 — integrity hard-gate (API contract)", () => {
    it("migration adds FAILED_HASH_MISMATCH to EvidenceStatus", () => {
        expect(MIGRATION_SQL).toMatch(/ALTER\s+TYPE\s+"EvidenceStatus"\s+ADD\s+VALUE\s+IF\s+NOT\s+EXISTS\s+'FAILED_HASH_MISMATCH'/i);
    });
    it("migration adds INTEGRITY_REJECTED_HASH_MISMATCH to CustodyEventType", () => {
        expect(MIGRATION_SQL).toMatch(/ALTER\s+TYPE\s+"CustodyEventType"\s+ADD\s+VALUE\s+IF\s+NOT\s+EXISTS\s+'INTEGRITY_REJECTED_HASH_MISMATCH'/i);
    });
    it("schema.prisma EvidenceStatus enum includes FAILED_HASH_MISMATCH", () => {
        const enumBlock = SCHEMA.match(/enum\s+EvidenceStatus\s*\{[^}]*\}/)?.[0];
    ...
    it("schema.prisma CustodyEventType enum includes INTEGRITY_REJECTED_HASH_MISMATCH", () => {
        const enumBlock = SCHEMA.match(/enum\s+CustodyEventType\s*\{[\s\S]*?\}/)?.[0];
    ...
    it("regenerate endpoint refuses with 409 + EVIDENCE_INTEGRITY_FAILED for FAILED_HASH_MISMATCH", () => {
        const regenStart = EVIDENCE_ROUTES.indexOf('"/v1/evidence/:id/reports/regenerate"');
        expect(regenStart).toBeGreaterThan(0);
        const regenSlice = EVIDENCE_ROUTES.slice(regenStart, regenStart + 8_000);
        expect(regenSlice).toContain("prismaPkg.EvidenceStatus.FAILED_HASH_MISMATCH");
    ...
    it("completion service refuses retry on FAILED_HASH_MISMATCH", () => {
        expect(EVIDENCE_COMPLETE).toContain("evidence.status === EvidenceStatus.FAILED_HASH_MISMATCH");
    ...
    it("SECURITY_EVENT_TYPES includes evidence_integrity_rejected", () => {
        expect(SECURITY_EVENT_TYPES).toContain("evidence_integrity_rejected");
    });
});
