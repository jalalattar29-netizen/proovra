...
describe("Phase 31.18 — EXTERNAL_REVIEW graph domain", () => {
    it("declares a Phase 31.18 anchor section in the reconciler", () => {
        expect(RECONCILER_SRC).toMatch(/Phase 31\.18 — EXTERNAL_REVIEW domain reconciliation/);
    });
    it("reads from external_review_grants table, team-anchored", () => {
        const idx = RECONCILER_SRC.indexOf("Phase 31.18 — EXTERNAL_REVIEW domain reconciliation");
    ...
    it("wraps the EXTERNAL_REVIEW block in independent try/catch isolation", () => {
        ...
        const idxEnd = RECONCILER_SRC.indexOf("1j. Phase 13 — ENTITY domain reconciliation", idx) >= 0
...
describe("Phase 31.18 — per-kind stale-sweep hardening", () => {
    it("declares a Phase 31.18 anchor for the per-kind stale sweep", () => {
        expect(RECONCILER_SRC).toMatch(/Phase 31\.18 — per-kind stale-tombstone sweep/);
    });
    it("includes three separate stale sweeps anchored on MEDIA_SIGNAL / OCR / TRANSCRIPT", () => {
        const idx = RECONCILER_SRC.indexOf("Phase 31.18 — per-kind stale-tombstone sweep");
    ...
    it("uses kind discriminators matching the section-2 materializer", () => {
        ...
        expect(slice).toMatch(/"signal_type" NOT IN \('OCR_AVAILABLE', 'TRANSCRIPT_AVAILABLE'\)/);
    });
...
describe("Phase 31.18 — listDuplicateEdges helper", () => {
    ...
    it("filters to evidence-to-evidence edges of the three bounded kinds only", () => {
        ...
        const idxEnd = RECONCILER_SRC.indexOf("export async function listGraphSeedNodes", idx);
        expect(idxEnd).toBeGreaterThan(idx);
        const slice = RECONCILER_SRC.slice(idx, idxEnd);
...
describe("Phase 31.18 — new graph routes", () => {
    ...
    it("both new graph routes require authorizeOrFail + antiEnumeration: true + evidence.read", () => {
        ...
        expect(dupSlice).toMatch(/authorizeOrFail\(\s*req,\s*reply,\s*\{[\s\S]*?antiEnumeration: true[\s\S]*?\}\s*\)/);
        ...
        expect(seedsSlice).toMatch(/authorizeOrFail\(\s*req,\s*reply,\s*\{[\s\S]*?antiEnumeration: true[\s\S]*?\}\s*\)/);
...
describe("Phase 31.18 — /v1/investigation/reviewers route", () => {
    ...
    it("uses authorizeOrFail + antiEnumeration: true + evidence.read", () => {
        ...
        expect(slice).toMatch(/authorizeOrFail\(\s*req,\s*reply,\s*\{[\s\S]*?antiEnumeration: true[\s\S]*?\}\s*\)/);
    ...
    it("NEVER reads reviewer_email / token_hash / safe_note / *_reason / *_note from the source table", () => {
        ...
        const idxEnd = MI_ROUTES_SRC.indexOf("/v1/evidence/:evidenceId/derived-assets?teamId=", idx);
        expect(idxEnd).toBeGreaterThan(idx);
...
// =============================================================================
const DUPLICATES_PAGE = readSource("../../../apps/web/app/(app)/investigation/duplicates/page.tsx");
const GRAPH_NAV_PAGE = readSource("../../../apps/web/app/(app)/investigation/graph/page.tsx");
const REVIEWERS_PAGE = readSource("../../../apps/web/app/(app)/investigation/reviewers/page.tsx");
const TIMELINE_PAGE = readSource("../../../apps/web/app/(app)/investigation/timeline/page.tsx");
const SEARCH_PAGE = readSource("../../../apps/web/app/(app)/search/page.tsx");
...
function expectNoForbiddenUserFacing(src) {
...
describe("Phase 31.18 — Duplicate Review UI source contract", () => {
    ...
    it("calls only the whitelisted endpoints", () => {
        ...
        for (const c of calls) {
            ...
            const ok = c.includes("/v1/users/me") ||
...
describe("Phase 31.18 — Graph Navigation Explorer UI source contract", () => {
    ...
    it("calls only whitelisted endpoints", () => {
        ...
        expect(calls.length).toBeGreaterThan(0);
        for (const c of calls) {
            const ok = c.includes("/v1/users/me") || c.includes("/v1/graph/seeds");
            expect(ok).toBe(true);
        }
    });
...
describe("Phase 31.18 — Reviewer Intelligence Console UI source contract", () => {
    ...
    it("degraded state shows an operator-readable pill, not a fake counter", () => {
        ...
        expect(REVIEWERS_PAGE).toMatch(/data unavailable|No reviewer activity recorded yet/);
...
describe("Phase 31.19 — DISMISSED-signal graph hardening", () => {
    it("section-2 materializer filters out DISMISSED signals at SELECT time", () => {
        const idx = RECONCILER_SRC.indexOf("// 2. Materialize MEDIA_SIGNAL nodes");
        expect(idx).toBeGreaterThan(0);
        const idxEnd = RECONCILER_SRC.indexOf("Phase 31.18 — per-kind stale-tombstone sweep", idx);
        expect(idxEnd).toBeGreaterThan(idx);
        const slice = RECONCILER_SRC.slice(idx, idxEnd);
...
describe("Phase 31.19 — Reviewer Console actions API surface", () => {
    ...
    it("the ack/dismiss endpoint exists with bounded action enum", () => {
        expect(MI_ROUTES_SRC).toMatch(/app\.post\(\s*"\/v1\/media-intelligence\/signals\/:signalId\/action"/);
    });
});
describe("Phase 31.19 — Reviewer Console UI actions", () => {
    it("calls the ack/dismiss endpoint with bounded action enum", () => {
        expect(REVIEWERS_PAGE).toMatch(/\/v1\/media-intelligence\/signals\/\$\{encodeURIComponent\(signalId\)\}\/action/);
    ...
    it("optimistic removal does NOT depend on response body shape — anti-leak", () => {
        ...
        expect(REVIEWERS_PAGE).toMatch(/pendingSignals: prev\.pendingSignals\.filter\(\(s\) => s\.id !== signalId\)/);
    });
});
