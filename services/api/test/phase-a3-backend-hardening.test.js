...
import { ANALYTICS_EVENT_NAMES, ANALYTICS_LIMITS, ANALYTICS_REJECTION_REASONS, AI_CHAT_ABUSE_REASONS, AI_CHAT_LIMITS, SECURITY_EVENT_TYPES, VERIFY_VIEWED_LIMITS, WEBHOOK_PROVIDERS, WEBHOOK_SIGNATURE_FAILURE_REASONS, } from "@proovra/shared";
...
const WEBHOOK_AUDIT = readSource("../src/services/security/webhook-signature-audit.service.ts");
const EVIDENCE_ROUTES = readSource("../src/routes/evidence.routes.ts");
const METRICS_SERVICE = readSource("../../../packages/shared-runtime/src/ops/metrics.service.ts");
describe("Phase A3 — shared bounded vocabularies", () => {
    ...
    it("exports webhook provider + failure reason vocabularies", () => {
        ...
        expect(WEBHOOK_SIGNATURE_FAILURE_REASONS).toContain("timestamp_out_of_range");
    });
    it("exports the VERIFY_VIEWED debounce window (≥ 1 hour)", () => {
        expect(VERIFY_VIEWED_LIMITS.DEBOUNCE_WINDOW_MS).toBeGreaterThanOrEqual(60 * 60 * 1000);
    });
    it("registers the new SecurityEvent types", () => {
        expect(SECURITY_EVENT_TYPES).toContain("webhook_signature_failure");
        expect(SECURITY_EVENT_TYPES).toContain("analytics_request_rejected");
        expect(SECURITY_EVENT_TYPES).toContain("ai_chat_abuse_signal");
    });
...
describe("Phase A3 — analytics endpoint hardening", () => {
    ...
    it("declares the AnalyticsTrackSchema using z.enum(ANALYTICS_EVENT_NAMES)", () => {
        expect(ANALYTICS_ROUTES).toMatch(/eventType:\s*z\.enum\(ANALYTICS_EVENT_NAMES\)/);
        expect(ANALYTICS_ROUTES).toMatch(/AnalyticsTrackSchema\s*=\s*z[\s\S]*\.strict\(\)/);
    });
    ...
    it("does NOT log the raw payload on rejection or write failure", () => {
        ...
        // body or metadata.
        const trackBlock = ANALYTICS_ROUTES.slice(ANALYTICS_ROUTES.indexOf('"/v1/analytics/track"'), ANALYTICS_ROUTES.indexOf("app.get(") > 0
...
describe("Phase A3 — VERIFY_VIEWED debounced custody event", () => {
    it("appends VERIFY_VIEWED on the public verify success path", () => {
        expect(EVIDENCE_ROUTES).toContain("CustodyEventType.VERIFY_VIEWED");
    ...
    it("debounces to ≤ 1 event per 24 hours", () => {
        ...
        expect(EVIDENCE_ROUTES).toMatch(/debounceMs\s*=\s*24\s*\*\s*60\s*\*\s*60\s*\*\s*1000/);
    ...
    it("payload carries the bounded fields ONLY — no IP, no user agent", () => {
        ...
        // the file.
        const handlerStart = EVIDENCE_ROUTES.indexOf('app.get("/public/verify/:id"');
        expect(handlerStart).toBeGreaterThan(0);
        const handlerSlice = EVIDENCE_ROUTES.slice(handlerStart);
        const appendIdx = handlerSlice.indexOf("CustodyEventType.VERIFY_VIEWED");
    ...
    it("custody append never breaks the verify response", () => {
        ...
        expect(EVIDENCE_ROUTES).toContain("public_verify.custody_append_failed");
    });
});
