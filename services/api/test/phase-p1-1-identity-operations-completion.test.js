...
describe("Phase P1.1 — Backend services", () => {
    it("SCIM reconciliation service exports detect / execute / list / replay", () => {
        const src = readSource("../src/services/access-control/scim-reconciliation.service.ts");
    ...
    it("SAML mapping service exports schema / current / preview / update + privilege purpose", () => {
        const src = readSource("../src/services/security/saml-mapping.service.ts");
    ...
    it("SSO health service exports buildSsoHealthSnapshot + failure taxonomy", () => {
        const src = readSource("../src/services/security/sso-health.service.ts");
    ...
    it("Session timeline service exports buildIdentitySessionTimeline + bounded event allowlist", () => {
        const src = readSource("../src/services/security/session-timeline.service.ts");
...
describe("Phase P1.1 — Routes registration", () => {
    it("identity operations completion routes are registered in server.ts", () => {
        ...
        expect(server).toMatch(/app\.register\(\s*identityOperationsCompletionRoutes/);
    });
    it("routes file exposes all 9 endpoints", () => {
        const routes = readSource("../src/routes/identity-operations-completion.routes.ts");
        // SCIM
    ...
    it("destructive endpoints route through requireStepUpForSensitiveAction", () => {
        ...
        expect(routes).toMatch(/requireStepUpForSensitiveAction[\s\S]*purpose:\s*"SCIM_RECONCILIATION_EXECUTE"/);
        ...
        expect(routes).toMatch(/requireStepUpForSensitiveAction[\s\S]*purpose:\s*SAML_MAPPING_PRIVILEGE_PURPOSE/);
    });
...
describe("Phase P1.1 — Frontend surfaces", () => {
    it("SCIM page exposes Tokens / Drift / Replay tabs", () => {
        const p = readSource("../../../apps/web/app/(app)/admin/identity/scim/page.tsx");
    ...
    it("SSO Health dashboard page exists at /security-center/sso/health", () => {
        expect(exists("../../../apps/web/app/(app)/security-center/sso/health/page.tsx")).toBe(true);
        const p = readSource("../../../apps/web/app/(app)/security-center/sso/health/page.tsx");
    ...
    it("SAML Visual Mapping Builder exists at /security-center/sso/mapping", () => {
        expect(exists("../../../apps/web/app/(app)/security-center/sso/mapping/page.tsx")).toBe(true);
        const p = readSource("../../../apps/web/app/(app)/security-center/sso/mapping/page.tsx");
    ...
    it("Sessions page wires a Session Timeline drawer per row", () => {
        const p = readSource("../../../apps/web/app/(app)/admin/identity/sessions/page.tsx");
    ...
    it("/security-center/sso links to the new health + mapping pages", () => {
        const p = readSource("../../../apps/web/app/(app)/security-center/sso/page.tsx");
