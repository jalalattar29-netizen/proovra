...
const PLATFORM_CTX_SRC = readApi("src/services/platform-context/platform-context.service.ts");
...
describe("Phase R9 — routeAccessResolver source contract", () => {
    ...
    it("PERSONAL_OR_ORG branch consults the rescue helper before refusing", () => {
        ...
        expect(RESOLVER_SRC).toMatch(/requiredActiveSpace === "PERSONAL_OR_ORG"[\s\S]*?hasUsablePersonalOrOrgEvidence/);
    });
    it("ORGANIZATION_ONLY branch does NOT call the personal/workspace rescue", () => {
        ...
        const orgBlockStart = RESOLVER_SRC.indexOf('requiredActiveSpace === "ORGANIZATION_ONLY"');
        const orgBlockEnd = RESOLVER_SRC.indexOf('requiredActiveSpace === "PERSONAL_OR_ORG"');
...
describe("Phase R9 — PageRouteGate envelope plumbing", () => {
    ...
    const HAS_FRAGMENT_HOOK = /useWorkspaceFragment\b/.test(PAGE_GATE_SRC) &&
        /workspace:\s*workspaceFragment/.test(PAGE_GATE_SRC);
    const HAS_PERSONAL_HOOK = /usePersonalSpaceFragment\b/.test(PAGE_GATE_SRC) &&
    ...
    it("passes workspace fragment from envelope into resolveRouteAccess", () => {
        const legacy = /workspace:\s*envelope\?\.workspace[\s\S]*?id:\s*envelope\.workspace\.id/.test(PAGE_GATE_SRC);
        expect(legacy || HAS_FRAGMENT_HOOK).toBe(true);
    });
    it("passes personalSpace fragment from envelope into resolveRouteAccess", () => {
        const legacy = /personalSpace:\s*envelope\?\.personalSpace[\s\S]*?id:\s*envelope\.personalSpace\.id/.test(PAGE_GATE_SRC);
        expect(legacy || HAS_PERSONAL_HOOK).toBe(true);
    });
    it("still passes activeSpaceType (primary signal — fallback is additive)", () => {
        expect(PAGE_GATE_SRC).toMatch(/activeSpaceType[\s\S]{0,200}envelope\?\.activeSpace\?\.type/);
    });
});
// =============================================================================
...
describe("Phase R9 — buildPlatformContext activeSpace derivation", () => {
    it("emits type=ORGANIZATION only when workspace.scope='TEAM' AND workspace.id is non-empty", () => {
        ...
        expect(PLATFORM_CTX_SRC).toMatch(/workspace\.scope === "TEAM" && workspaceIdNonEmpty/);
        expect(PLATFORM_CTX_SRC).toMatch(/workspaceIdNonEmpty[\s\S]*?typeof workspace\.id === "string" && workspace\.id\.length > 0/);
    });
    it("falls back to type=PERSONAL when workspace.id is empty or scope is not TEAM", () => {
        ...
        expect(PLATFORM_CTX_SRC).toMatch(/personalId[\s\S]*?workspaceIdNonEmpty[\s\S]*?personalSpace\.id/);
...
describe("Phase R9 — entitlement projection is user-level (PRO without org)", () => {
    it("entitlement.findFirst is user-scoped (userId required; active:true added by the production billing-mismatch fix)", () => {
        ...
        const matches = PLATFORM_CTX_SRC.match(/entitlement\.findFirst\(\{\s*where:\s*\{\s*userId:\s*userRow\.id,\s*active:\s*true\s*\}/g);
...
describe("Phase R9 — signup eager bootstrap", () => {
    it("email-password registerWithEmailPassword imports ensurePersonalWorkspace", () => {
        expect(EMAIL_AUTH_SRC).toMatch(/import\s*\{\s*ensurePersonalWorkspace\s*\}\s*from\s*["'].*workspace-bootstrap\.service\.js["']/);
    });
    it("registerWithEmailPassword calls ensurePersonalWorkspace AFTER creating the user and BEFORE returning", () => {
        ...
        const fnStart = EMAIL_AUTH_SRC.indexOf("export async function registerWithEmailPassword");
        expect(fnStart).toBeGreaterThanOrEqual(0);
        const nextExport = EMAIL_AUTH_SRC.indexOf("export async function loginWithEmailPassword", fnStart + 1);
        ...
        const bootstrapIdx = body.indexOf("ensurePersonalWorkspace({ userId: user.id })");
    ...
    it("registerWithEmailPassword sets currentWorkspaceId only when NULL (no clobber)", () => {
        expect(EMAIL_AUTH_SRC).toMatch(/if\s*\(\s*!user\.currentWorkspaceId\s*&&\s*personal\.teamId\s*\)/);
        expect(EMAIL_AUTH_SRC).toMatch(/prisma\.user\.update\([\s\S]*?data:\s*\{\s*currentWorkspaceId:\s*personal\.teamId\s*\}/);
    });
    it("registerWithEmailPassword bootstrap failure is non-fatal (try/catch)", () => {
        ...
        expect(EMAIL_AUTH_SRC).toMatch(/try\s*\{[\s\S]*?ensurePersonalWorkspace[\s\S]*?\}\s*catch/);
    });
    it("auth.service upsertUserWithEmailLink imports ensurePersonalWorkspace", () => {
        expect(AUTH_SRC).toMatch(/import\s*\{\s*ensurePersonalWorkspace\s*\}\s*from\s*["'].*workspace-bootstrap\.service\.js["']/);
    });
    it("upsertUserWithEmailLink calls ensurePersonalWorkspace AFTER user upsert", () => {
        ...
        const bootstrapIdx = AUTH_SRC.indexOf("ensurePersonalWorkspace({ userId: user.id })");
    ...
    it("upsertUserWithEmailLink sets currentWorkspaceId only when NULL (no guest-conversion clobber)", () => {
        expect(AUTH_SRC).toMatch(/if\s*\(\s*!user\.currentWorkspaceId\s*&&\s*personal\.teamId\s*\)/);
    });
    it("upsertUserWithEmailLink bootstrap failure is non-fatal (try/catch wraps the bootstrap)", () => {
        expect(AUTH_SRC).toMatch(/try\s*\{[\s\S]*?ensurePersonalWorkspace[\s\S]*?\}\s*catch/);
    });
});
// =============================================================================
...
describe("Phase R9 — product contract", () => {
    it("no-organizations is NEVER treated as no-workspace in the resolver", () => {
        ...
        expect(RESOLVER_SRC).not.toMatch(/organizations\.length\s*<\s*1[\s\S]*?NEEDS/);
    });
