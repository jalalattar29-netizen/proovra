...
function listAppFiles() {
    ...
    return files
        ...
        if (f.rel.startsWith(".next/"))
            return false;
        if (f.rel.includes("__tests__/"))
            return false;
        if (f.rel.endsWith(".d.ts"))
            return false;
        return true;
    })
        .map((f) => ({
...
describe("R10 Group 1 — canonical CSS / ui.tsx upper-bound guards", () => {
    ...
    it("apps/web/components/command-center/command-center.css MUST NOT exceed pre-R10 baseline", () => {
        ...
        expect(countLines(text)).toBeLessThanOrEqual(PRE_R10_COMMAND_CENTER_CSS_LINES + 20);
    });
});
// ---------------------------------------------------------------------------
...
describe("R10 Group 6 — canonical primitives preserved in ui.tsx", () => {
    ...
    for (const name of REQUIRED_EXPORTS) {
        it(`ui.tsx exports ${name}`, () => {
            const pattern = new RegExp(`export\\s+(?:function|const|class)\\s+${name}\\b`);
...
describe("R10 Group 7 — no rogue floating action buttons", () => {
    ...
    const FAB_PATTERN = /style\s*=\s*\{[^}]*position\s*:\s*["']fixed["'][^}]*\bbottom\s*:[^}]*\b(?:right|left)\s*:[^}]*\}/;
    const ALLOWLIST = [
    ...
    for (const file of APP_FILES) {
        if (ALLOWLIST.some((a) => file.label.endsWith(a)))
            continue;
        if (!/<button|<Button|<a\s|<Link/.test(file.text))
            continue;
...
describe("R10 Group 9 — density-aware system preserved in globals.css", () => {
    ...
    it("globals.css defines a CSS custom-property token scale", () => {
        ...
        expect(/--background\b|--foreground\b|--primary\b/.test(GLOBALS_CSS)).toBe(true);
    });
});
// ---------------------------------------------------------------------------
...
describe("R10 Group 11 — PageRouteGate import preserved across protected app routes", () => {
    ...
    const PROTECTED_PAGES = APP_FILES.filter((f) => f.label.includes("/(app)/") && f.label.endsWith("/page.tsx"));
    ...
    it("at least 80% of protected pages reference PageRouteGate", () => {
        const gated = PROTECTED_PAGES.filter((f) => /PageRouteGate/.test(f.text)).length;
...
describe("R10 Group 14 — CSS hygiene", () => {
    it("no CSS file exceeds 10 MB (regression catch on accidental committed build output)", () => {
        ...
        for (const f of CSS_FILES) {
            if (statSync(f.abs).size > 10 * 1024 * 1024)
                oversize.push(f.label);
        }
