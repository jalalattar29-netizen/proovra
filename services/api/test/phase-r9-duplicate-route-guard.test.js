...
// files we ship.
const METHOD_RE = /\bapp\.(get|post|put|patch|delete|head|options)\s*\(\s*["']([^"']+)["']/gim;
/**
...
describe("Phase R9 — duplicate route registration guard", () => {
    it("no (method, path) pair is registered in more than one route file", () => {
        ...
        const report = duplicates
            ...
            .join("\n")}`)
            .join("\n");
        expect(duplicates, `Fastify routes registered in more than one .routes.ts file (would crash boot with FST_ERR_DUPLICATED_ROUTE):\n${report}`).toEqual([]);
    });
    it("GET /v1/governance/dashboard is registered exactly once", () => {
        ...
        const re = /\bapp\.get\s*\(\s*["']\/v1\/governance\/dashboard["']/g;
        const hits = [];
        for (const file of files) {
            const src = stripComments(readFileSync(file, "utf8"));
            re.lastIndex = 0;
        ...
        expect(hits, `GET /v1/governance/dashboard must be registered exactly once. Hits:\n${hits
            .map((f) => `  ${f}`)
            .join("\n")}`).toHaveLength(1);
    });
});
