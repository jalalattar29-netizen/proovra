...
const MIGRATION = readApi("prisma/migrations/20260630100000_phase328d_matter_workspace/migration.sql");
...
describe("Phase 32.8D — schema additions", () => {
    it("Case gains status / priority / description / referenceNumber / closedAtUtc / closureReason", () => {
        ...
        expect(block[0]).toMatch(/referenceNumber\s+String\?\s+@map\("reference_number"\)\s+@db\.VarChar\(60\)/);
        expect(block[0]).toMatch(/closedAtUtc\s+DateTime\?\s+@map\("closed_at_utc"\)/);
        expect(block[0]).toMatch(/closureReason\s+String\?\s+@map\("closure_reason"\)\s+@db\.VarChar\(400\)/);
    });
    ...
    it("CaseAssignment has unique on (caseId, assignedToUserId, role)", () => {
        expect(SCHEMA).toMatch(/@@unique\(\[caseId,\s*assignedToUserId,\s*role\]/);
    });
});
// =============================================================================
// PART 2 — Migration
...
describe("Phase 32.8D — migration", () => {
    ...
    it("ALTER TABLE on cases uses ADD COLUMN IF NOT EXISTS", () => {
        expect(MIGRATION).toMatch(/ALTER TABLE "cases"[\s\S]{0,800}ADD COLUMN IF NOT EXISTS "status"/);
    ...
    it("partial UNIQUE INDEX on (team_id, reference_number) is created with WHERE clause", () => {
        expect(MIGRATION).toMatch(/CREATE UNIQUE INDEX IF NOT EXISTS "case_team_reference_number_uniq"[\s\S]{0,200}WHERE "reference_number" IS NOT NULL/);
    });
...
describe("Phase 32.8D — risk engine", () => {
    ...
    it("listCaseEvidenceIds UNIONs legacy Evidence.caseId + canonical CaseEvidenceLink", () => {
        ...
        expect(RISK).toMatch(/prisma\.caseEvidenceLink\.findMany[\s\S]{0,200}caseId:\s*input\.caseId/);
    });
    ...
    it("recordCaseRiskSnapshot never throws", () => {
        expect(RISK).toMatch(/catch[\s\S]{0,80}\/\* advisory write — never throws \*\//);
    });
...
describe("Phase 32.8D — case lifecycle service", () => {
    ...
    it("active legal hold blocks CLOSED/ARCHIVED transitions", () => {
        expect(LIFECYCLE).toMatch(/if \(CLOSURE_STATUSES\.has\(input\.toStatus\)\)[\s\S]{0,400}active_legal_hold_blocks_closure/);
    });
    ...
    it("addEvidenceLink rejects cross-workspace evidence on team cases", () => {
        expect(LIFECYCLE).toMatch(/existing\.teamId && evidence\.teamId !== existing\.teamId/);
    });
...
describe("Phase 32.8D — routes", () => {
    ...
    it("every read route goes through requireWorkspaceMember or requireCaseAccess", () => {
        const matterQueueBlock = ROUTES.match(/app\.get\(\s*"\/v1\/cases\/matter-queue"[\s\S]*?\}\s*,\s*\)/);
        expect(matterQueueBlock).not.toBeNull();
        expect(matterQueueBlock[0]).toMatch(/requireWorkspaceMember/);
        const matterWsBlock = ROUTES.match(/app\.get\(\s*"\/v1\/cases\/:id\/matter-workspace"[\s\S]*?\}\s*,\s*\)/);
    ...
    it.skip("status mutation route restricts to OWNER/ADMIN/OPERATOR/INVESTIGATOR", () => {
        const block = ROUTES.match(/app\.post\(\s*"\/v1\/cases\/:id\/status"[\s\S]*?\}\s*,\s*\)/);
        expect(block).not.toBeNull();
