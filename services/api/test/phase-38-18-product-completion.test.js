...
const OPSUMMARY = readWeb("app/(app)/capture/_lib/CaptureOperationalSummary.tsx");
...
const TEAMS_HOME = readWeb("components/workspace-admin/WorkspaceAdministrationHome.tsx");
...
describe("Phase 38.18 — CaptureOperationalSummary integrated band", () => {
    ...
    it("renders nothing when no items are staged", () => {
        expect(OPSUMMARY).toMatch(/if\s*\(items\.length\s*===\s*0\)\s*return\s+null/);
    });
    ...
    it("emits structured data attributes for level/workflow/score/top-suggestion", () => {
        ...
        expect(OPSUMMARY).toMatch(/data-capture-operational-summary-top-suggestion=/);
    });
    ...
    it("operational summary is informational only — does NOT block finalization", () => {
        ...
        expect(CAPTURE).not.toMatch(/CaptureBottomBar[\s\S]{0,400}finishDisabled=\{[\s\S]{0,80}operationalSummary/);
    });
});
// =============================================================================
...
describe("Phase 38.18 — analyst-grade density on canonical enterprise classes", () => {
    ...
    for (const cls of COMPACT_TARGETS) {
        it(`compact-mode rule exists for ${cls}`, () => {
            ...
            expect(SHELL_CSS, `compact-mode rule must target ${cls}`).toMatch(new RegExp(`data-operational-density="compact"\\][^{]*${escaped}`));
        });
    }
    it("spacious-mode rules also extend the canonical classes (not just owned panels)", () => {
        expect(SHELL_CSS).toMatch(/data-operational-density="spacious"\][^{]*\.cc-page/);
        expect(SHELL_CSS).toMatch(/data-operational-density="spacious"\][^{]*\.cc-section/);
    });
});
// =============================================================================
...
describe("Phase 38.18 — ContextualHelp mounted on /teams + /search", () => {
    ...
    it("both new mounts are collapsedByDefault", () => {
        expect(TEAMS_HOME).toMatch(/<ContextualHelp[\s\S]{0,300}collapsedByDefault/);
        expect(SEARCH).toMatch(/<ContextualHelp[\s\S]{0,300}collapsedByDefault/);
    });
});
// =============================================================================
...
describe("Phase 38.18 — total ContextualHelp mounts across codebase", () => {
    ...
    it("at least 9 .tsx files mount <ContextualHelp (full HelpSurface enum coverage)", () => {
        ...
        for (const file of all) {
            ...
            if (normalized.endsWith("/ContextualHelp.tsx"))
                continue;
        ...
        expect(mountCount, `expected ≥ 9 ContextualHelp mounts, found ${mountCount}`).toBeGreaterThanOrEqual(9);
    });
});
// =============================================================================
