...
import { __setWebhookHttpClientForTests, attemptDelivery, } from "../src/services/integrations/webhook-dispatcher.js";
import { checkArchiveAgainstLimits, evaluateArchiveAdvisory, getArchiveLimits, validateUploadedFile, } from "../src/services/security/file-validation.service.js";
import { isMalwareScanningEnabled, } from "../src/services/security/file-security-scan.service.js";
function bytes(...arr) {
...
describe("archive limit helpers", () => {
    it("blocks over-entry archives", () => {
        ...
        if (!r.ok)
            expect(r.reason).toBe("too_many_entries");
    });
    it("blocks over-size archives", () => {
        ...
        expect(r.ok).toBe(false);
        if (!r.ok)
            expect(r.reason).toMatch(/uncompressed_size_exceeded|compression_ratio_exceeded/);
    });
    ...
    it("env overrides are picked up by getArchiveLimits()", () => {
        ...
        try {
            ...
            if (prev === undefined)
                delete process.env.MAX_ARCHIVE_ENTRY_COUNT;
            else
                process.env.MAX_ARCHIVE_ENTRY_COUNT = prev;
        }
    });
...
describe("webhook dispatcher SSRF hardening", () => {
    ...
    it("treats 302 responses as redirect_blocked (does not follow)", async () => {
        ...
        try {
            const { issueWebhookSecret } = await import("../src/services/integrations/webhooks.service.js");
            ...
            const outcome = await attemptDelivery(fakeRow, endpoint, prismaStub);
...
describe("malware scanning feature flag", () => {
    ...
    afterEach(() => {
        if (prev === undefined)
            ...
            process.env.MALWARE_SCANNING_ENABLED = prev;
    });
...
describe("server security headers (compile-time source check)", () => {
    it("server.ts adds CSP, COOP, CORP headers in addition to existing ones", async () => {
        ...
        const src = await readFile(fileURLToPath(new URL("../src/server.ts", import.meta.url)), "utf8");
...
describe("anti-enumeration — security routes never 403 on non-admin", () => {
    it("routes return 404 (not 403) when caller is not an admin member", async () => {
        const { readFile } = await import("node:fs/promises");
        const { fileURLToPath } = await import("node:url");
        const src = await readFile(fileURLToPath(new URL("../src/routes/security.routes.ts", import.meta.url)), "utf8");
