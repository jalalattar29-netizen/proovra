...
const REVIEWER_INSPECTOR_PAGE = readSource(
...
const REPORTS_INDEX = readSource(
  "../../../apps/web/components/reports-experience/ReportsIndex.tsx",
);
const SHARED_PRESENCE_DOC = readSource(
...
describe("Phase G3.2 — inline reviewer action UI (Reviewer Console)", () => {
  it("Queue/Mine rows expose Assign / Escalate / Request info / Open inspector buttons", () => {
    ...
    expect(REVIEWER_CONSOLE).toContain(
...
describe("Phase G3.2 — Matter Workspace tab filters", () => {
  it("HoldsTab consumes filterText (no underscore-prefixed unused param)", () => {
    expect(MATTER_WORKSPACE).toMatch(
      /function HoldsTab\(\{[\s\S]*?filterText = "",[\s\S]*?\}/,
    );
  ...
  it("DecisionsTab consumes filterText", () => {
    expect(MATTER_WORKSPACE).toMatch(
      /function DecisionsTab\(\{[\s\S]*?filterText = "",[\s\S]*?\}/,
    );
  ...
  it("CommunicationsTab consumes filterText", () => {
    expect(MATTER_WORKSPACE).toMatch(
      /function CommunicationsTab\(\{[\s\S]*?filterText = "",[\s\S]*?\}/,
    );
  ...
  it("AssignmentsTab consumes filterText", () => {
    expect(MATTER_WORKSPACE).toMatch(
      /function AssignmentsTab\(\{[\s\S]*?filterText = "",[\s\S]*?\}/,
    );
  });

  it("AuditTab consumes filterText", () => {
    expect(MATTER_WORKSPACE).toMatch(
      /function AuditTab\(\{[\s\S]*?filterText = "",[\s\S]*?\}/,
    );
  ...
  it("ExportTab consumes filterText", () => {
    expect(MATTER_WORKSPACE).toMatch(
      /function ExportTab\(\{[\s\S]*?filterText = "",[\s\S]*?\}/,
    );
...
describe("Phase G3.2 — Presence indicator final mounts", () => {
  ...
  it("Reviewer inspector page mounts PresenceIndicator with resourceKind=reviewer_workflow", () => {
    ...
    expect(REVIEWER_INSPECTOR_PAGE).toContain(
...
describe("Phase G3.2 — CollisionWarning wiring", () => {
  it("Evidence detail page mounts CollisionWarning with workflow updatedAt", () => {
    ...
    expect(EVIDENCE_DETAIL_PAGE).toContain(
      "workspace.reviewWorkflow.updatedAt",
    );
  });
...
describe("Phase G3.2 — vocabulary discipline across all G3.2 surfaces", () => {
  ...
  // We compare against the stripped-comments form so docstrings
  // that LIST the forbidden vocabulary (in order to forbid it)
  // don't trip the check. The active code in each surface still
  // gets the full ban.
  //
  // Per-surface `allow` lists encode the few phrases that appear in
  // the canonical anti-overclaim PRODUCT copy of a surface — i.e.
  // strings the operator is explicitly TOLD the product does NOT
  // claim. ReportsIndex carries the "we do not make court-ready
  // claims" disclaimer verbatim and must keep it.
  type SurfaceCheck = {
    name: string;
    src: string;
    allow?: ReadonlyArray<RegExp>;
  };
