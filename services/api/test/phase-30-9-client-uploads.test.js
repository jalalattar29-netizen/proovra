...
// =============================================================================
import { classifyRetry, planChunks, } from "../../../apps/web/lib/uploads/retry.js";
import { CLIENT_FAILURE_REASONS, CLIENT_PART_STATES, CLIENT_UPLOAD_SESSION_STATES, DEFAULT_CHUNK_SIZE_BYTES, MAX_CHUNK_SIZE_BYTES, MAX_CONCURRENCY, MAX_PART_RETRIES, MAX_RETRY_BACKOFF_MS, MIN_CHUNK_SIZE_BYTES, MIN_CONCURRENCY, MIN_RETRY_BACKOFF_MS, OFFLINE_DRAFT_STATES, } from "../../../apps/web/lib/uploads/types.js";
...
describe("Phase 30.9 — chunk planner", () => {
    it("clamps chunk size to [5 MiB, 64 MiB]", () => {
        ...
        const big = planChunks(500 * 1024 * 1024, 512 * 1024 * 1024);
...
describe("Phase 30.9 — bounded vocabularies", () => {
    it("CLIENT_UPLOAD_SESSION_STATES covers every operator-visible state", () => {
        for (const required of [
            ...
            expect(CLIENT_UPLOAD_SESSION_STATES).toContain(required);
        }
    ...
    it("CLIENT_FAILURE_REASONS is exhaustive + snake_case", () => {
        for (const required of [
            ...
            expect(CLIENT_FAILURE_REASONS).toContain(required);
        }
    ...
    it("RECOVERY_CLASSIFICATIONS is exhaustive + snake_case", () => {
        for (const c of [
            ...
            expect(RECOVERY_CLASSIFICATIONS).toContain(c);
        }
...
describe("Phase 30.9 — orchestrator custody-safety", () => {
    const src = readSource("../../../apps/web/lib/uploads/multipart-uploader.ts");
    ...
    it("safeToClose only true when no parts in flight + state is terminal/paused/ready", () => {
        ...
        const fn = src.match(/private computeSafeToClose\(\)[\s\S]*?\n  \}/)?.[0];
...
describe("Phase 30.9 — persistence + recovery + UI anti-leak", () => {
    const persistenceSrc = readSource("../../../apps/web/lib/uploads/persistence.ts");
    const recoverySrc = readSource("../../../apps/web/lib/uploads/recovery.ts");
    const panelSrc = readSource("../../../apps/web/components/uploads/UploadOperationsPanel.tsx");
    ...
    it("persistence project() strips part.etag (storage metadata, not needed for resume)", () => {
        const projectFn = persistenceSrc.match(/function projectSnapshot\([\s\S]*?\n\}/)?.[0];
    ...
    it("persistence stripClientOnlyPartState collapses transient states to PENDING (clean resume)", () => {
        const helper = persistenceSrc.match(/function stripClientOnlyPartState\([\s\S]*?\n\}/)?.[0];
    ...
    it("recovery classifies cleanupSafe correctly for terminal server states", () => {
        expect(recoverySrc).toMatch(/cleanupSafe:[\s\S]*?finalized[\s\S]*?aborted[\s\S]*?expired[\s\S]*?not_found/);
    });
...
describe("Phase 30.9 — persistence projection behavioral", () => {
    it("project() turns IN_FLIGHT parts into PENDING + drops etag", async () => {
        const { createUploadPersistence } = await import("../../../apps/web/lib/uploads/persistence.js");
...
describe("Phase 30.9 — recovery classification behavioral", () => {
    it("runs against a fake fetcher + persistence, classifies each known state", async () => {
        const { createUploadPersistence } = await import("../../../apps/web/lib/uploads/persistence.js");
        const { runUploadRecovery } = await import("../../../apps/web/lib/uploads/recovery.js");
        const persistence = createUploadPersistence();
        const baseEntry = {
            teamId: "team-1",
            evidenceId: "ev-1",
        ...
        expect(byId.get("s-completed")?.classification).toBe("ready_for_finalization");
