...
const TELEMETRY_HEALTH = readApi("src/services/ops-health/telemetry-health.service.ts");
const RECONCILE_HEALTH = readApi("src/services/ops-health/reconcile-health.service.ts");
const ROLLUP_HEALTH = readApi("src/services/ops-health/security-rollup-health.service.ts");
const PROJECTION_HEALTH = readApi("src/services/ops-health/projection-health.service.ts");
...
describe("Phase 32.8C+++++++ — section IDs match persona priority chip targets", () => {
    ...
    it("every required section id is registered in the SECTION_RENDERERS registry", () => {
        ...
        for (const id of REQUIRED_SECTION_IDS) {
            expect(CC_TSX, `missing SECTION_RENDERERS entry for "${id}"`).toMatch(new RegExp(`${id}:\\s*\\{\\s*ariaLabel:`));
        }
    });
    ...
    it("every required section has an aria-label in the registry", () => {
        ...
        for (const id of ["incidents", "routingQueue", "queueWorkerTelemetry"]) {
            const block = CC_TSX.match(new RegExp(`${id}:\\s*\\{\\s*ariaLabel:\\s*"[^"]+"`));
...
describe("Phase 32.8C+++++++ — ops-health evaluators", () => {
    ...
    it("telemetry evaluator distinguishes STALE (worker fresh) from FAILED (worker dead)", () => {
        expect(TELEMETRY_HEALTH).toMatch(/status:\s*"STALE"[\s\S]{0,400}heartbeat last recorded[\s\S]{0,200}worker is likely overloaded/i);
        expect(TELEMETRY_HEALTH).toMatch(/status:\s*"FAILED"[\s\S]{0,400}beyond the failure threshold/);
    });
    ...
    it("security rollup evaluator distinguishes DEGRADED (canonical alive) from FAILED (both reads dead)", () => {
        expect(ROLLUP_HEALTH).toMatch(/status:\s*"DEGRADED"[\s\S]{0,300}Detection remains operational/);
        expect(ROLLUP_HEALTH).toMatch(/status:\s*"FAILED"[\s\S]{0,300}canonical SecurityEvent log read failed/);
    });
...
describe("Phase 32.8C+++++++ — envelope opsHealth wiring", () => {
    ...
    it("each evaluator call has a .catch fallback so the envelope never throws", () => {
        expect(COMMAND_CENTER_API).toMatch(/evaluateTelemetryHealth\(\{\s*teamId:\s*input\.teamId\s*\}\)\.catch\(/);
        expect(COMMAND_CENTER_API).toMatch(/evaluateReconcileHealth\(\{\s*teamId:\s*input\.teamId\s*\}\)\.catch\(/);
        expect(COMMAND_CENTER_API).toMatch(/evaluateSecurityRollupHealth\(\{\s*teamId:\s*input\.teamId\s*\}\)\.catch\(/);
    });
    it("envelope return statement includes opsHealth on every branch", () => {
        expect(COMMAND_CENTER_API).toMatch(/opsHealth:\s*\{\s*telemetry:\s*telemetryHealth,\s*reconcile:\s*reconcileHealth,\s*securityRollup:\s*securityRollupHealth/);
    });
