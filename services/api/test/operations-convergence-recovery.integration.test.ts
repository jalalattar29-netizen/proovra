...
/**
 * The EXPAND half is the whole of what this release ships, and that is why it
 * is the only migration these cases run.
 *
 * It removes nothing and is safe before the code deploys — it is what unblocks
 * the writer. The CONTRACT half, which drops the legacy columns, is deliberately
 * NOT in this release: `prisma migrate deploy` applies every pending migration
 * in one go, so shipping both would race a removal against the image deploy.
 *
 * So every case below runs against the schema state the deployment actually
 * produces: canonical columns rebound, legacy columns STILL PRESENT. That is
 * the stricter of the two states rather than the weaker one — a writer that
 * works with the legacy family still sitting there is exactly the property
 * this release has to have.
 */
...
describe("Operations convergence recovery, across workspace kinds (live PostgreSQL 16)", () => {
  ...
  /** This release applies EXPAND and nothing else. */
  ...
  it("the EXPAND half ALONE recovers every workspace kind — which is what the deploy actually does first", async () => {
    // The production sequence for THIS release is: apply expand, then deploy
    // the code. Nothing is dropped, so the state asserted here — legacy columns
    // still present, every workspace kind fully working — is the deployed end
    // state rather than a transient window.
    await applyHybridDrift();
