...
export function isAutoPostgresEnabled() {
    ...
    return (process.env.RUN_LIVE_INTEGRATION_NO_TESTCONTAINERS !== "1" &&
 ...
 * supplied directly.
 */
export function assertLiveIntegrationEnv() {
    if (!isLiveIntegrationEnabled()) {
        throw new Error("RUN_LIVE_INTEGRATION must be set to '1' to use the integration harness. " +
    ...
    if (!testDatabaseUrl) {
        ...
        throw new Error("TEST_DATABASE_URL is required for live integration tests when testcontainers is disabled. " +
    ...
    if (!looksLikeTestDb && !explicitOverride) {
        throw new Error(`Refusing to run integration tests against DB "${dbName}" — the name must contain "test", ` +
...
export async function bootIntegrationHarness() {
    ...
    if (!testDatabaseUrl && isAutoPostgresEnabled()) {
        ...
        const migrateResult = spawnSync("pnpm", ["exec", "prisma", "migrate", "deploy"], {
            cwd: path.resolve(path.dirname(new URL(import.meta.url).pathname.replace(/^\//, "")), ".."),
        ...
        if (migrateResult.status !== 0) {
            ...
            throw new Error(`prisma migrate deploy failed against the testcontainer Postgres.\n` +
                `stdout:\n${migrateResult.stdout}\n` +
                `stderr:\n${migrateResult.stderr}`);
        }
    }
    if (!testDatabaseUrl) {
        throw new Error("bootIntegrationHarness: no TEST_DATABASE_URL resolved and " +
...
async function seedFixtures(prisma) {
    ...
    if (!secret) {
        throw new Error("AUTH_JWT_SECRET is required to mint integration test tokens. " +
            "Set it in your test environment alongside TEST_DATABASE_URL.");
    }
    const mintToken = (userId, email) => signJwt({ sub: userId, provider: "EMAIL", email }, secret, 60 * 60 * 1);
    // --------------------------------------------------------------------------
    ...
    // full custody chains.
    // --------------------------------------------------------------------------
    async function createEvidence(teamId, ownerUserId, title) {
    ...
    async function createCase(teamId, ownerUserId, name) {
        return prisma.case.create({
            data: { name, teamId, ownerUserId },
            select: { id: true },
        });
    }
    const evidenceA = await createEvidence(orgA.id, userAOwner.id, `OrgA evidence (${tag})`);
    const evidenceB = await createEvidence(orgB.id, userBOwner.id, `OrgB evidence (${tag})`);
    const evidencePersonal = await createEvidence(personalSpace.teamId, personalUser.id, `Personal evidence (${tag})`);
    const caseA = await createCase(orgA.id, userAOwner.id, `OrgA case (${tag})`);
    const caseB = await createCase(orgB.id, userBOwner.id, `OrgB case (${tag})`);
    const casePersonal = await createCase(personalSpace.teamId, personalUser.id, `Personal case (${tag})`);
    return {
        teamA: {
            teamId: orgA.id,
