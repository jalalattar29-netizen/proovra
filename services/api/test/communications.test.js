...
import { TwilioMessagingProvider, readTwilioConfigFromEnv, } from "../src/services/communications/twilio-provider.js";
import { buildProviderHealthSnapshot, getMessagingProvider, setMessagingProviderForTests, } from "../src/services/communications/provider-registry.js";
...
describe("Communications routes — auth posture", () => {
    it("webhook routes have NO requireAuth preHandler (signature IS the auth)", async () => {
        ...
        // pre-handler.
        const inboundBlock = src.match(/app\.post\(\s*"\/v1\/communications\/webhooks\/twilio\/inbound"[\s\S]{0,400}/);
        expect(inboundBlock).not.toBeNull();
        if (inboundBlock) {
            expect(inboundBlock[0]).not.toMatch(/preHandler:\s*requireAuth/);
        }
        const statusBlock = src.match(/app\.post\(\s*"\/v1\/communications\/webhooks\/twilio\/status"[\s\S]{0,400}/);
    ...
    it("operator routes use requireAuth + 404-on-non-member", async () => {
        ...
        expect(src).toMatch(/app\.get\(\s*"\/v1\/communications\/messages"[\s\S]{0,200}preHandler:\s*requireAuth/);
...
describe("Phase 18 — untouched files invariant", () => {
    it("services/worker/src/pdf/report.ts is NOT modified by Phase 18", async () => {
        const { readFile } = await import("node:fs/promises");
        const { fileURLToPath } = await import("node:url");
        const path = fileURLToPath(new URL("../../worker/src/pdf/report.ts", import.meta.url));
...
describe("Phase 18 migration — additive only", () => {
    it("uses IF NOT EXISTS / EXCEPTION blocks; never drops or renames", async () => {
        const { readFile } = await import("node:fs/promises");
        const { fileURLToPath } = await import("node:url");
        const src = await readFile(fileURLToPath(new URL("../prisma/migrations/20260527100000_add_communications_phase18/migration.sql", import.meta.url)), "utf8");
...
describe("Privacy contract — verification service never persists the code", () => {
    it("verification.service source does not store input.code anywhere except passing to the provider", async () => {
        const { readFile } = await import("node:fs/promises");
        const { fileURLToPath } = await import("node:url");
        const src = await readFile(fileURLToPath(new URL("../src/services/communications/verification.service.ts", import.meta.url)), "utf8");
