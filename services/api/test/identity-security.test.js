...
import { StepUpError, } from "../src/services/identity-security/step-up.service.js";
import { projectTrustedDevice } from "../src/services/identity-security/trusted-device.service.js";
import { projectRevokedSession, hashSessionId, } from "../src/services/identity-security/session-revocation.service.js";
import { hashDeviceCookieValue, hashIpAddress, } from "../src/services/identity-security/risk.service.js";
// -----------------------------------------------------------------------------
...
describe("projectTrustedDevice — privacy contract", () => {
    it("omits deviceIdHash and ipHash from the safe projection", () => {
        const projected = projectTrustedDevice({
            ...
            deviceIdHash: "deadbeef0000000000000000000000000000000000000000000000000000beef",
            uaPreview: "Chrome on macOS",
            ipPreview: "192.•••.•••.99",
            ipHash: "cafebabe0000000000000000000000000000000000000000000000000000babe",
            status: "ACTIVE",
        ...
        expect(projected.deviceIdHash).toBeUndefined();
        expect(projected.ipHash).toBeUndefined();
    });
});
// -----------------------------------------------------------------------------
...
describe("projectRevokedSession — privacy contract", () => {
    it("omits sessionIdHash and revokedBeforeIat from the safe projection", () => {
        const projected = projectRevokedSession({
            ...
            sessionIdHash: "deadbeef0000000000000000000000000000000000000000000000000000beef",
        ...
        const json = JSON.stringify(projected);
        expect(json).not.toContain("deadbeef");
        expect(projected.sessionIdHash).toBeUndefined();
        expect(projected.revokedBeforeIat).toBeUndefined();
    });
});
// -----------------------------------------------------------------------------
...
describe("HMAC helpers — deterministic + key-bound", () => {
    ...
    it("hash outputs never include the raw input", () => {
        expect(hashSessionId("secret-session-id-12345")).not.toMatch(/secret-session-id/);
...
describe("Identity-security routes — auth posture", () => {
    it("operator routes use requireAuth and 404-on-non-member", async () => {
        ...
        expect(src).toMatch(/app\.post\(\s*"\/v1\/identity-security\/step-up\/start"[\s\S]{0,200}preHandler:\s*requireAuth/);
...
describe("Privacy contract — step-up service never persists the OTP code", () => {
    it("step-up.service source does not store input.code anywhere", async () => {
        const { readFile } = await import("node:fs/promises");
        const { fileURLToPath } = await import("node:url");
        const src = await readFile(fileURLToPath(new URL("../src/services/identity-security/step-up.service.ts", import.meta.url)), "utf8");
...
describe("requireAuth — session revocation check", () => {
    it("auth middleware consults isSessionRevoked before populating req.user", async () => {
        const { readFile } = await import("node:fs/promises");
        const { fileURLToPath } = await import("node:url");
        const src = await readFile(fileURLToPath(new URL("../src/middleware/auth.ts", import.meta.url)), "utf8");
...
describe("JWT — Phase 19 sid + iat claims", () => {
    it("signJwt always populates iat and a fresh random sid", async () => {
        const { readFile } = await import("node:fs/promises");
        const { fileURLToPath } = await import("node:url");
        const src = await readFile(fileURLToPath(new URL("../src/services/jwt.ts", import.meta.url)), "utf8");
...
describe("Phase 19 migration — additive only", () => {
    it("uses IF NOT EXISTS / EXCEPTION blocks; never drops or renames", async () => {
        const { readFile } = await import("node:fs/promises");
        const { fileURLToPath } = await import("node:url");
        const src = await readFile(fileURLToPath(new URL("../prisma/migrations/20260528100000_add_identity_security_phase19/migration.sql", import.meta.url)), "utf8");
...
describe("RBAC auto-revoke — suspend + revoke trigger session revocation", () => {
    it("suspendMember + revokeMember call autoRevokeAllSessions", async () => {
        const { readFile } = await import("node:fs/promises");
        const { fileURLToPath } = await import("node:url");
        const src = await readFile(fileURLToPath(new URL("../src/services/identity/rbac.service.ts", import.meta.url)), "utf8");
        ...
        const suspendBlock = src.match(/export async function suspendMember[\s\S]{0,3000}autoRevokeAllSessions/);
        const revokeBlock = src.match(/export async function revokeMember[\s\S]{0,3000}autoRevokeAllSessions/);
