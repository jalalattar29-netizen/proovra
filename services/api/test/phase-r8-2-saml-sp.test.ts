...
const SAML_ASSERTION_SVC = readApi(
  "src/services/security/saml-assertion.service.ts",
);
const SAML_USER_MAPPING_SVC = readApi(
...
import {
  parseSamlMetadata,
  SamlMetadataError,
} from "../src/services/security/saml-metadata.service.js";
...
describe("R8.2 Group 3 — Source file hard rules", () => {
  it("saml-auth.routes.ts exists at src/routes/saml-auth.routes.ts and is > 15 000 bytes", () => {
    ...
    expect(apiPath("src/routes/saml-auth.routes.ts")).toMatch(
      /saml-auth\.routes\.ts$/,
    );
  });
...
describe("R8.2 Group 4 — saml-metadata.service pure-function tests", () => {
  it("parseSamlMetadata rejects DTD entity injection with METADATA_XML_UNSAFE", () => {
    ...
    expect(() => parseSamlMetadata(maliciousXml)).toThrowError(
      expect.objectContaining({
        name: "SamlMetadataError",
        code: "METADATA_XML_UNSAFE",
      } satisfies Partial<SamlMetadataError>),
    );
  });
...
describe("R8.2 Group 6 — saml-assertion.service pure-function tests", () => {
  it("validateSamlResponse throws SAML_XML_UNSAFE when payload contains <!ENTITY after base64 decode", () => {
    ...
    const samlResponseBase64 = Buffer.from(maliciousXml, "utf-8").toString(
      "base64",
    );
    ...
    expect(() =>
      ...
      } satisfies Partial<SamlAssertionError>),
    );
  });
