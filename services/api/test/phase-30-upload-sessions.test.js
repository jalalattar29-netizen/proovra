...
import { UPLOAD_SESSION_STATES, UPLOAD_PART_STATES, UPLOAD_SESSION_DENIAL_CODES, } from "../src/services/uploads/upload-session.service.js";
...
describe("Phase 30 — evidence_upload_sessions SQL drift patch", () => {
    const src = readSource("../../../services/api/sql/drift-patches/2026-05-19-evidence-upload-sessions.sql");
    it("creates both session + per-part tables with IF NOT EXISTS", () => {
        expect(src).toMatch(/CREATE TABLE IF NOT EXISTS\s+"evidence_upload_sessions"/i);
        expect(src).toMatch(/CREATE TABLE IF NOT EXISTS\s+"evidence_upload_session_parts"/i);
    });
    it("session state catalog is bounded by CHECK constraint matching the service contract", () => {
        expect(src).toMatch(/CONSTRAINT "evidence_upload_sessions_state_bounded"[\s\S]*?'INITIATED'[\s\S]*?'UPLOADING'[\s\S]*?'VERIFYING'[\s\S]*?'COMPLETED'[\s\S]*?'FAILED'[\s\S]*?'EXPIRED'[\s\S]*?'ABORTED'/i);
    });
    it("part state catalog is bounded by CHECK constraint", () => {
        expect(src).toMatch(/CONSTRAINT "evidence_upload_session_parts_state_bounded"[\s\S]*?'PENDING'[\s\S]*?'UPLOADED_UNVERIFIED'[\s\S]*?'VERIFIED'[\s\S]*?'FAILED'/i);
    });
    it("part count + total bytes + sha256 format constraints enforced at DB level", () => {
        expect(src).toMatch(/CONSTRAINT "evidence_upload_sessions_part_count_positive"[\s\S]*?>= 1[\s\S]*?<= 10000/i);
        expect(src).toMatch(/CONSTRAINT "evidence_upload_sessions_total_bytes_nonneg"/i);
        expect(src).toMatch(/CONSTRAINT "evidence_upload_sessions_sha256_format"[\s\S]*?\^\[a-f0-9\]\{64\}\$/i);
    });
    it("expiry must be in the future at creation", () => {
        expect(src).toMatch(/CONSTRAINT "evidence_upload_sessions_expires_after_created"[\s\S]*?expires_at_utc.*?>.*?created_at_utc/i);
    });
    it("per-part (session_id, part_index) is UNIQUE — duplicate parts impossible", () => {
        expect(src).toMatch(/CREATE UNIQUE INDEX IF NOT EXISTS "evidence_upload_session_parts_uk"[\s\S]*?\("session_id", "part_index"\)/i);
    });
    it("idempotency key is UNIQUE per-team — duplicate session POSTs collapse", () => {
        expect(src).toMatch(/CREATE UNIQUE INDEX IF NOT EXISTS "evidence_upload_sessions_team_idemp_uk"[\s\S]*?WHERE "idempotency_key" IS NOT NULL/i);
    });
    it("reaper partial index targets only resumable states (INITIATED / UPLOADING / VERIFYING)", () => {
        expect(src).toMatch(/CREATE INDEX IF NOT EXISTS "evidence_upload_sessions_reaper_idx"[\s\S]*?WHERE "state" IN \('INITIATED', 'UPLOADING', 'VERIFYING'\)/i);
    });
    it("part-state hash format constraints enforce 64-char hex at DB level", () => {
        expect(src).toMatch(/CONSTRAINT "evidence_upload_session_parts_client_sha_format"[\s\S]*?\^\[a-f0-9\]\{64\}\$/i);
        expect(src).toMatch(/CONSTRAINT "evidence_upload_session_parts_server_sha_format"[\s\S]*?\^\[a-f0-9\]\{64\}\$/i);
    });
    ...
    it("part-state cascades on session delete (no orphan rows)", () => {
        expect(src).toMatch(/REFERENCES "evidence_upload_sessions"\("id"\) ON DELETE CASCADE/);
    });
});
// =============================================================================
...
describe("Phase 30 — upload-session service source contract", () => {
    ...
    it("completed_at_utc is set ONLY in completeUploadSession + via NOW() (server clock)", () => {
        ...
        const completedSetters = noComments.match(/SET[\s\S]*?"completed_at_utc"\s*=\s*([^,\n]+)/g) ?? [];
    ...
    it("verified_at_utc is ONLY ever set via server-clock NOW() (Phase 30.12 update)", () => {
        ...
        const verifiedSetters = noComments.match(/"verified_at_utc"\s*=\s*[^\n]+/g) ?? [];
    ...
    it("client-supplied uploaded_at_utc_client is stored SEPARATELY from server-verified time (audit trail)", () => {
        ...
        expect(src).toMatch(/"uploaded_at_utc_client"\s*=\s*COALESCE\(\$5,\s*"uploaded_at_utc_client"\)/);
    });
    it("hash mismatch → part flipped to FAILED + bumps hash_mismatch counter + emits SecurityEvent", () => {
        expect(src).toMatch(/claimed && claimed !== input\.serverSha256[\s\S]*?bump\("upload_hash_mismatch_total"\)/);
        expect(src).toMatch(/failure_reason[\s\S]*?'hash_mismatch'/);
        expect(src).toMatch(/eventType:\s*"upload_part_hash_mismatch"/);
    });
    it("completeUploadSession refuses when ANY part is not VERIFIED + returns the pending list", () => {
        expect(src).toMatch(/WHERE "session_id" = \$1 AND "team_id" = \$2 AND "state" <> 'VERIFIED'/);
        expect(src).toMatch(/completion_blocked_pending_parts[\s\S]*?pendingPartIndices/);
    });
    it("idempotency: existing session with the same (team_id, idempotency_key) is reused", () => {
        expect(src).toMatch(/SELECT[\s\S]*?FROM\s+"evidence_upload_sessions"\s+WHERE "team_id" = \$1 AND "idempotency_key" = \$2/);
    ...
    it("every read is team-anchored — no cross-workspace lookups", () => {
        ...
        expect(src).toMatch(/WHERE "session_id" = \$1\s+AND "team_id" = \$2/);
    });
    it("abort never affects already-terminal sessions (no double-revoke)", () => {
        expect(src).toMatch(/AND "state" NOT IN \('COMPLETED', 'ABORTED', 'EXPIRED'\)/);
    });
    it("reapStaleUploadSessions targets only resumable states", () => {
        expect(src).toMatch(/WHERE "state" IN \('INITIATED', 'UPLOADING', 'VERIFYING'\)\s*AND "expires_at_utc" < NOW\(\)/);
    });
...
describe("Phase 30 — schema validation registration", () => {
    ...
    it("registers both new tables", () => {
        expect(src).toMatch(/name:\s*"evidence_upload_sessions"[\s\S]*?subsystem:\s*"core_evidence"/);
        expect(src).toMatch(/name:\s*"evidence_upload_session_parts"[\s\S]*?subsystem:\s*"core_evidence"/);
    });
    it("registers state + completion columns at CRITICAL severity", () => {
        expect(src).toMatch(/table:\s*"evidence_upload_sessions",\s*column:\s*"state",\s*severity:\s*"critical"/);
        expect(src).toMatch(/table:\s*"evidence_upload_sessions",\s*column:\s*"completed_at_utc",\s*severity:\s*"critical"/);
        expect(src).toMatch(/table:\s*"evidence_upload_session_parts",\s*column:\s*"state",\s*severity:\s*"critical"/);
        expect(src).toMatch(/table:\s*"evidence_upload_session_parts",\s*column:\s*"verified_at_utc",\s*severity:\s*"critical"/);
        expect(src).toMatch(/table:\s*"evidence_upload_session_parts",\s*column:\s*"server_sha256",\s*severity:\s*"critical"/);
    });
    it("registers the (session_id, part_index) UNIQUE index at CRITICAL severity", () => {
        expect(src).toMatch(/indexName:\s*"evidence_upload_session_parts_uk",\s*severity:\s*"critical"/);
    });
});
// =============================================================================
...
describe("Phase 30 — type-level invariants", () => {
    it("bounded vocabularies are typed catalogs", () => {
        const session = "INITIATED";
        const part = "PENDING";
        const denial = "session_not_found";
