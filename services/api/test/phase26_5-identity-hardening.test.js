...
import { SCIM_GROUP_MAPPED_ROLES, SCIM_GROUP_SCHEMA_URI, SEARCH_FORBIDDEN_OVERCLAIM_PHRASES, SSO_CALLBACK_STATE_TTL_SECONDS, computeSuspiciousSessionRisk, evaluateAdaptiveAuth, isSafeRedirectAfter, isSessionStale, sessionRiskLevel, shouldWriteHeartbeat, } from "@proovra/shared";
...
describe("Phase 26.5 — SSO hardening", () => {
    const src = readSource("../src/services/access-control/sso-hardening.service.ts");
...
describe("Phase 26.5 — suspicious-session detector", () => {
    const src = readSource("../src/services/access-control/suspicious-session.service.ts");
...
describe("Phase 26.5 — adaptive auth engine", () => {
    const src = readSource("../src/services/access-control/adaptive-auth.service.ts");
...
describe("Phase 26.5 — heartbeat + stale sweep", () => {
    const src = readSource("../src/services/access-control/session-inventory.service.ts");
...
describe("Phase 26.5 — SCIM Groups service", () => {
    const src = readSource("../src/services/access-control/scim-groups.service.ts");
...
describe("Phase 26.5 — SCIM routes", () => {
    ...
    it("all 5 Group endpoints registered", () => {
        ...
        const verbCount = (src.match(/app\.(post|get|patch|delete)\(\s*"\/v2\/scim\/Groups/g) ?? [])
            .length;
...
describe("Phase 26.5 — pure helpers reachable from API", () => {
    ...
    it("SCIM_GROUP_SCHEMA_URI is the RFC 7644 URI", () => {
        expect(SCIM_GROUP_SCHEMA_URI).toBe("urn:ietf:params:scim:schemas:core:2.0:Group");
    });
