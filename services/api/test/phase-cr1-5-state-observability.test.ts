...
describe("CR1.5 Test 7 — bounded /v1/users/me self-fetcher allow-list", () => {
   ...
   */
  const ALLOWED_SELF_FETCHERS: ReadonlyArray<{
    path: string; // path under apps/web/
    reason: string;
  }> = [
    {
  ...
  it("only the documented files self-fetch /v1/users/me", () => {
    ...
    const allowedNormalized = new Set(
      ALLOWED_SELF_FETCHERS.map(
        (e) => `/${e.path.replace(/\\/g, "/").replace(/^\/+/, "")}`,
      ),
    );
    ...
    for (const file of all) {
      ...
      if (!callsSelfMe) continue;
      const normalized = file.replace(/\\/g, "/");
      const rel = normalized.replace(/^.*\/apps\/web\/+/, "/");
      if (!allowedNormalized.has(rel)) offenders.push(rel);
    }
    expect(
      offenders,
      `Found new /v1/users/me self-fetchers outside the CR1.5 allow-list:\n${offenders.join("\n")}\nIf intentional, add to ALLOWED_SELF_FETCHERS with a reason + revisit phase.`,
    ).toEqual([]);
  });
...
describe("CR1.5 Test 8 — bounded useTeamWorkspaceGate callsite allow-list", () => {
   ...
   */
  const ALLOWED_GATE_CALLSITES: ReadonlyArray<{
    file: string;
    reason: string;
    revisitPhase: string;
  }> = [
  ...
  it("only the documented files call useTeamWorkspaceGate()", () => {
    ...
    const allowed = new Set(
      ALLOWED_GATE_CALLSITES.map(
        (e) => `/${e.file.replace(/\\/g, "/").replace(/^\/+/, "")}`,
      ),
    );
    const offenders: string[] = [];
    for (const file of all) {
      ...
      if (!/\buseTeamWorkspaceGate\s*\(/.test(src)) continue;
      const normalized = file.replace(/\\/g, "/");
      const rel = normalized.replace(/^.*\/apps\/web\/+/, "/");
      // The hook's own definition file isn't a callsite.
      if (rel.endsWith("/useTeamWorkspaceGate.ts")) continue;
      ...
      if (rel.endsWith("/lib/platform-context/index.ts")) continue;
      if (!allowed.has(rel)) offenders.push(rel);
    }
    expect(
      offenders,
      `Found new useTeamWorkspaceGate() callsites outside the CR1.5 allow-list:\n${offenders.join("\n")}`,
    ).toEqual([]);
  });
});
...
describe("CR1.5 Test 13 — observability utility is production-safe", () => {
  it("emit() is gated behind both NODE_ENV !== 'production' and the explicit opt-in flag", () => {
    ...
    const isFn = OBS.match(
      /function isObservabilityEnabled\(\)[\s\S]*?\n\}/,
    );
  ...
  it("SafePayload type forbids objects/arrays/blobs (only primitives + bounded strings)", () => {
    ...
    expect(OBS).toMatch(
      /SafePayloadValue\s*=\s*\|\s*boolean[\s\S]*?\|\s*null[\s\S]*?\|\s*string[\s\S]*?\|\s*number/,
    );
  });
