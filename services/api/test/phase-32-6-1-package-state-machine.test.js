...
describe("Phase 32.6.1 — worker catch arm distinguishes gate denial", () => {
    ...
    it("imports PackageGateDeniedError so the catch arm can type-test it", () => {
        expect(PROC_SRC).toMatch(/import \{ createVerificationPackage, PackageGateDeniedError \} from "\.\/verification-package\.js"/);
    });
    it("catch arm branches on PackageGateDeniedError via `instanceof`", () => {
        expect(PROC_SRC).toMatch(/if \(verificationError instanceof PackageGateDeniedError\)/);
    });
    ...
    it("gate-denial branch persists bounded metadata to evidence.verificationPackageMetadata", () => {
        expect(PROC_SRC).toMatch(/prisma\.evidence\.update\(\{[\s\S]{0,400}verificationPackageMetadata:\s*\{[\s\S]{0,400}blocked: true/);
    ...
    it("gate-denial bumps `package_generation_blocked_total` (separate from failed_total)", () => {
        ...
        expect(branchBody).toMatch(/bump\("package_generation_blocked_total"\)/);
    });
});
// =============================================================================
...
describe("Phase 32.6.1 — artifact status `blocked` projection", () => {
    ...
    it("helper accepts `evidenceVerificationPackageMetadata` parameter", () => {
        expect(SERVICE_SRC).toMatch(/evidenceVerificationPackageMetadata\?:\s*prismaPkg\.Prisma\.JsonValue \| null/);
    });
    it("helper reads the bounded blocked-metadata shape (NEVER throws on malformed input)", () => {
        ...
        expect(SERVICE_SRC).toMatch(/if \(raw == null \|\| typeof raw !== "object" \|\| Array\.isArray\(raw\)\) return null/);
    ...
    it("package is `blocked: true, pending: false` when metadata shows a gate denial AND no package row", () => {
        ...
        expect(code).toMatch(/packageBlocked = finalized && !latestPackage && blockedMeta !== null/);
        expect(code).toMatch(/packagePending\s*=\s*finalized &&\s*!latestPackage &&\s*!packageUnavailableForPersonalWorkspace &&\s*!packageBlocked/);
    });
    it("when a package row EXISTS, blocked is forced to false regardless of metadata", () => {
        ...
        expect(code).toMatch(/latestPackage\s*\?\s*\{[\s\S]{0,400}blocked: false,[\s\S]{0,200}blockedOutcome: null,/);
    });
});
// =============================================================================
...
describe("Phase 32.6.1 — verification-package download route structured response", () => {
    ...
    it("Phase 32.6.6 — the personal-workspace 410 branch is retired (personal evidence now generates a BASIC package)", () => {
        ...
        // 200 download.
        expect(ROUTES_SRC).not.toMatch(/return reply\.code\(410\)\.send\(\{[\s\S]{0,400}code:\s*"verification_package_unavailable"/);
        // No live `reason: "personal_workspace_..."` key in any reply.send().
        expect(ROUTES_SRC).not.toMatch(/^\s*reason:\s*"personal_workspace_no_team_governance_context"/m);
    });
    ...
    it("structured response NEVER leaks storage keys / bucket / signed URLs", () => {
        ...
        const lookAtResponses = slice.match(/return reply[\s\S]{0,800}code:\s*"verification_package_(blocked|pending|not_found)"[\s\S]{0,400}\}\)/g);
...
describe("Phase 32.6.1 — Redis connection event observability", () => {
    ...
    it("redisConnection attaches `ready` listener that logs a bounded reconnect line", () => {
        expect(QUEUE_SRC).toMatch(/redisConnection\.on\("ready"[\s\S]{0,400}event: "redis\.connection\.ready"/);
    });
    it("redisConnection attaches `error` listener with bounded log throttle (5s)", () => {
        ...
        expect(QUEUE_SRC).toMatch(/redisConnection\.on\("error"[\s\S]{0,800}event: "redis\.connection\.error"/);
    ...
    it("redisConnection attaches `close` listener that logs the previous-ready timestamp", () => {
        expect(QUEUE_SRC).toMatch(/redisConnection\.on\("close"[\s\S]{0,400}event: "redis\.connection\.close"/);
    ...
    it("error log includes the bounded NodeJS errno code (e.g. ECONNREFUSED)", () => {
        expect(QUEUE_SRC).toMatch(/code:\s*\(err as NodeJS\.ErrnoException\)\.code/);
    });
