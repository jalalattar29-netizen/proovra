...
const PREVIEW_SVC = readFileSync(apiPath("src/services/security/mfa-recovery-digest-preview.service.ts"), "utf8");
...
describe("R8.1.8 — admin digest preferences UI", () => {
    ...
    it("test 1: SPA renders the preferences card with data-cc-mfa-recovery-digest-preferences-card", () => {
        expect(ADMIN_SPA).toMatch(/data-cc-mfa-recovery-digest-preferences-card/);
    ...
    it("test 2: SPA calls GET + PATCH /v1/identity/mfa-admin/digest-preferences", () => {
        expect(ADMIN_SPA).toMatch(/apiFetch\(\s*["']\/v1\/identity\/mfa-admin\/digest-preferences["']/);
    ...
    it("test 3: snooze button onClick path constructs `suppressUntil = now + 7d` and PATCHes preferences", () => {
        ...
        expect(ADMIN_SPA).toMatch(/Date\.now\(\)\s*\+\s*days\s*\*\s*24\s*\*\s*60\s*\*\s*60\s*\*\s*1000/);
    ...
    it("test 4: resume button onClick PATCHes preferences with suppressUntil:null", () => {
        ...
        const doResume = ADMIN_SPA.match(/const doResume\s*=\s*async[\s\S]*?\};/);
    ...
    it("test 5: SPA copy explicitly states digest preferences do NOT suppress audit/security events", () => {
        expect(ADMIN_SPA).toMatch(/Digest preferences only affect notification emails\./);
        expect(ADMIN_SPA).toMatch(/Security events and audit logs are always preserved\./);
        expect(ADMIN_SPA).toMatch(/data-cc-mfa-recovery-preferences-disclaimer/);
    });
});
// =============================================================================
...
describe("R8.1.8 — digest preview endpoint + UI", () => {
    ...
    it("test 6: GET /v1/identity/mfa-admin/digest-preferences/preview wired in routes", () => {
        expect(ADMIN_ROUTES).toMatch(/app\.get\(\s*["']\/v1\/identity\/mfa-admin\/digest-preferences\/preview["']/);
    ...
    it("test 8: preview return type carries ONLY counts + team names + admin SPA URL", () => {
        ...
        const iface = PREVIEW_SVC.match(/export interface PreviewDigestTeam[\s\S]*?\n\}/);
        ...
        expect(PREVIEW_SVC).toMatch(/select:\s*\{\s*teamId:\s*true,\s*createdAt:\s*true\s*\}/);
        expect(PREVIEW_SVC).toMatch(/select:\s*\{\s*id:\s*true,\s*name:\s*true\s*\}/);
        expect(PREVIEW_SVC).toMatch(/select:\s*\{\s*teamId:\s*true,\s*digestEnabled:\s*true,\s*suppressUntil:\s*true\s*\}/);
    });
    // ---------------------------------------------------------------------------
    // 9. Preview groups by team.
    // ---------------------------------------------------------------------------
    it("test 9: preview builds a per-team count + oldest-age map and returns teams[]", () => {
        expect(PREVIEW_SVC).toMatch(/perTeam\s*=\s*new\s+Map/);
    ...
    it("test 10: SPA renders the preview card and does NOT include approve / reject buttons within it", () => {
        ...
        const previewBlock = ADMIN_SPA.match(/data-cc-mfa-recovery-digest-preview-card[\s\S]*?<\/section>/);
...
describe("R8.1.8 — HTML digest email", () => {
    ...
    it("test 11: worker sendAdminDigest builds an `html` body AND sends it to Resend", () => {
        ...
        expect(DIGEST_WORKER).toMatch(/body:\s*JSON\.stringify\(\s*\{\s*from,[\s\S]*?text,[\s\S]*?html,\s*\}\s*\)/);
    });
    // ---------------------------------------------------------------------------
    ...
    it("test 12: worker still builds the `text` body alongside `html`", () => {
        ...
        expect(DIGEST_WORKER).toMatch(/JSON\.stringify\(\s*\{[\s\S]*?text,[\s\S]*?html,/);
    });
    // ---------------------------------------------------------------------------
    ...
    it("test 13: HTML body iterates ONLY input.teams summaries — no raw tokens / OTPs / secrets / user-email enumeration", () => {
        ...
        expect(DIGEST_WORKER).not.toMatch(/teamRows\s*=[\s\S]*?recipients\.map/);
...
describe("R8.1.8 — scope guards", () => {
    ...
    it("test 16: auth.routes.ts + sso-auth.routes.ts + saml-auth.routes.ts under src/routes matching *auth* (R8.2 additive)", () => {
        ...
        expect(ADMIN_ROUTES).toMatch(/\/v1\/identity\/mfa-admin\/digest-preferences\/preview/);
    ...
    it("test 18: preview scopes by actor's ACTIVE OWNER/ADMIN memberships only", () => {
        ...
        expect(PREVIEW_SVC).toMatch(/teamMember\.findMany\(\{\s*where:\s*\{\s*userId:\s*input\.actorUserId,[\s\S]{0,200}status:\s*["']ACTIVE["'],[\s\S]{0,200}role:\s*\{\s*in:\s*\[["']OWNER["'],\s*["']ADMIN["']\]/);
        // The recovery-request query restricts teamId to the resolved
        // membership set.
        expect(PREVIEW_SVC).toMatch(/mfaRecoveryRequest\.findMany\(\{\s*where:\s*\{[\s\S]{0,300}teamId:\s*\{\s*in:\s*teamIds/);
    });
    // ---------------------------------------------------------------------------
...
describe("R8.1.8 bonus — analytics events wired via canonical service", () => {
    ...
    it("every R8.1.8 analytics event appears in the canonical routes file", () => {
        for (const evt of R8_1_8_EVENTS) {
            expect(ADMIN_ROUTES.match(new RegExp(`["']${evt}["']`))?.length ?? 0, `${evt} must appear in mfa-admin.routes.ts`).toBeGreaterThanOrEqual(1);
        }
