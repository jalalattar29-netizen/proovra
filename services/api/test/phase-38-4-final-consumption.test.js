...
describe("Phase 38.4 — operational density CSS rules", () => {
    it("declares CSS variables for all three modes", () => {
        expect(SHELL_CSS).toMatch(/\.app-shell-v2\[data-operational-density="comfortable"\]/);
        expect(SHELL_CSS).toMatch(/\.app-shell-v2\[data-operational-density="compact"\]/);
        expect(SHELL_CSS).toMatch(/\.app-shell-v2\[data-operational-density="spacious"\]/);
    });
...
describe("Phase 38.4 — workflowHelp library", () => {
    ...
    it("returns a complete entry for every (workflow, surface) pair", async () => {
        const { resolveWorkflowHelp } = await import("../../../apps/web/lib/platform-context/workflowHelp.js");
        const workflows = [
        ...
        for (const workflow of workflows) {
            for (const surface of surfaces) {
                ...
                for (const pattern of banned) {
                    expect(pattern.test(entry.title) || pattern.test(entry.body), `Banned language ${pattern} found in ${workflow}/${surface}`).toBe(false);
                }
            }
        }
    });
