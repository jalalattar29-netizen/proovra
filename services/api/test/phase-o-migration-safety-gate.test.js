...
async function loadAudit() {
    return (await import(REPO_ROOT + AUDIT_SCRIPT));
}
// ---------------------------------------------------------------------------
...
describe("Phase O — detectFindings", () => {
    it("flags CREATE TABLE IF NOT EXISTS as CRITICAL", async () => {
        ...
        const { findings } = detectFindings(`CREATE TABLE IF NOT EXISTS "widgets" (id UUID PRIMARY KEY);`);
    ...
    it("flags ALTER TABLE DROP COLUMN as CRITICAL", async () => {
        const { detectFindings } = await loadAudit();
        const { findings } = detectFindings(`ALTER TABLE "widgets" DROP COLUMN "x";`);
    ...
    it("flags ALTER TABLE RENAME as CRITICAL", async () => {
        const { detectFindings } = await loadAudit();
        const { findings } = detectFindings(`ALTER TABLE "widgets" RENAME COLUMN "a" TO "b";`);
    ...
    it("flags DROP TABLE, DROP INDEX, DROP TYPE, TRUNCATE, DELETE FROM as CRITICAL", async () => {
        ...
        for (const stmt of [
            ...
            expect.soft(findings.some((f) => f.risk === "CRITICAL"), `expected CRITICAL on: ${stmt}`).toBe(true);
        }
    });
    ...
    it("does NOT flag UPDATE with WHERE", async () => {
        const { detectFindings } = await loadAudit();
        const { findings } = detectFindings(`UPDATE "widgets" SET x = 1 WHERE x IS NULL;`);
    ...
    it("flags CREATE INDEX referencing unguarded columns as INDEX_COLUMN_RISK (CRITICAL)", async () => {
        const { detectFindings } = await loadAudit();
        const { findings } = detectFindings(`CREATE INDEX IF NOT EXISTS "i" ON "widgets" ("team_id");`);
...
`;
    ...
    it("flags ADD COLUMN without IF NOT EXISTS as HIGH", async () => {
        const { detectFindings } = await loadAudit();
        const { findings } = detectFindings(`ALTER TABLE "x" ADD COLUMN "y" UUID;`);
    ...
    it("accepts ADD COLUMN IF NOT EXISTS as clean", async () => {
        const { detectFindings } = await loadAudit();
        const { findings } = detectFindings(`ALTER TABLE IF EXISTS "x" ADD COLUMN IF NOT EXISTS "y" UUID;`);
...
describe("Phase O — CI gate on post-baseline migrations", () => {
    ...
    it("every migration with timestamp > baseline has ZERO CRITICAL findings", async () => {
        ...
        for (const name of dirs) {
            ...
            const crit = findings.filter((f) => f.risk === "CRITICAL" && !approved.has(f.kind));
            if (crit.length > 0) {
                violations.push({
                    migration: name,
        ...
        if (violations.length > 0) {
            const msg = violations
                .map((v) => `  ${v.migration}: ${v.criticalCount} CRITICAL findings — ${v.kinds.join(", ")}`)
                .join("\n");
            throw new Error(`Post-baseline migration(s) introduced CRITICAL findings. Fix or wrap in Phase O-Final-style DO blocks before commit:\n${msg}`);
        }
        expect(violations).toEqual([]);
    });
    it("the Phase O-Final repair migration itself passes the gate", async () => {
        const { detectFindings } = await loadAudit();
        const sql = read("services/api/prisma/migrations/20261006000000_phase_o_final_production_column_repair/migration.sql");
        const { findings } = detectFindings(sql);
...
describe("Phase O — detectNamingDriftInSql", () => {
    it("flags quoted camelCase identifiers as drift", async () => {
        ...
        const drift = detectNamingDriftInSql(`ALTER TABLE "widgets" ADD COLUMN "teamId" UUID;`);
    ...
    it("does not flag pure snake_case identifiers", async () => {
        const { detectNamingDriftInSql } = await loadAudit();
        const drift = detectNamingDriftInSql(`ALTER TABLE "widgets" ADD COLUMN "team_id" UUID;`);
