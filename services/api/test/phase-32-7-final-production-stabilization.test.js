...
function violationReasons(patch) {
    let reasons = [];
    withEnv(patch, () => {
        reasons = collectStartupViolations().map((v) => `${v.envName}:${v.reason}`);
    });
    return reasons;
}
// ===========================================================================
...
describe("32.7 Test 2 — .env.example is canonical (post-R8.C structure)", () => {
    ...
    it("contains every core required + feature-gated variable name", () => {
        ...
        for (const name of REQUIRED) {
            expect(EXAMPLE, `.env.example missing canonical variable: ${name}`).toMatch(new RegExp(`^${name}=`, "m"));
        }
    });
...
describe("32.7 Test 4 — artifact status polling is side-effect-free", () => {
    ...
    it("frontend capture polling hits the side-effect-free status endpoint, not the download endpoint", () => {
        const src = readWeb("app/(app)/capture/_hooks/useCaptureSessionOrchestration.ts");
...
describe("32.7 Test 7 — focus-refresh remains feature-gated and bounded", () => {
    it("PlatformContextProvider keeps the flag gate + 60s throttle", () => {
        ...
        expect(src).toMatch(/NEXT_PUBLIC_PLATFORM_CONTEXT_FOCUS_REFRESH_ENABLED\s*===\s*["']true["']/);
...
describe("32.7 Test 8 — stabilization contract: no new state library / nav surface", () => {
    it("apps/web/package.json still has no React Query / SWR / Redux / Zustand", () => {
        ...
        for (const forbidden of [
            ...
            expect(deps[forbidden], `Forbidden client-state library added: ${forbidden}.`).toBeUndefined();
        }
    });
});
// ===========================================================================
...
describe("32.7 Test 9 — capture / custody / report / package files untouched", () => {
    ...
    for (const { rel, expectedBytes } of PINS) {
        it(`${rel} stays within ±10% of CR1.6 baseline (${expectedBytes} bytes)`, () => {
            ...
            expect(st.size, `${rel} size ${st.size} outside ±10% window [${low}, ${high}].`).toBeGreaterThanOrEqual(low);
...
describe("32.7 Test 10 — documentation + registry updated", () => {
    it("docs/recovery/PHASE_32_7_FINAL_PRODUCTION_STABILIZATION.md exists + substantial", () => {
        const doc = readRepo("docs/recovery/PHASE_32_7_FINAL_PRODUCTION_STABILIZATION.md");
    ...
    it("MASTER_PHASE_REGISTRY.md registers Phase 32.7 with explicit status", () => {
        ...
        expect(registry).toMatch(/\|\s*(Phase )?32\.7\s*\|[\s\S]*?(CLOSED|CLOSED_WITH_DEFERRED_ITEMS)/);
    });
    it("registry records DEF item outcomes for items reviewed by 32.7", () => {
        ...
        for (const def of [
            ...
            expect(registry, `Registry no longer references ${def}; CR1.7 silent-debt rule violated.`).toContain(def);
        }
    });
});
// ---------------------------------------------------------------------------
