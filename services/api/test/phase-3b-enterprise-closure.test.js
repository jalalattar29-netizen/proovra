...
import { EXECUTIVE_METRICS_RANGES, EXECUTIVE_TRENDS_SCHEMA_VERSION, INTELLIGENCE_LIFECYCLE_CATEGORIES, INTELLIGENCE_LIFECYCLE_CODES, TREND_DIRECTIONS, buildTrendMetric, classifyLifecycleCategory, classifyTrend, rangeWindowMs, } from "@proovra/shared";
// ---------------------------------------------------------------------------
...
describe("Phase 3B Closure — shared contracts", () => {
    ...
    it("classifies lifecycle category correctly", () => {
        ...
        expect(classifyLifecycleCategory("CORRECTION_CREATED")).toBe("CORRECTION_LIFECYCLE");
        expect(classifyLifecycleCategory("PROVIDER_CALL_FAILED")).toBe("PROVIDER_LIFECYCLE");
        expect(classifyLifecycleCategory("BUDGET_HARD_LIMIT_REACHED")).toBe("BUDGET_LIFECYCLE");
    });
...
describe("Phase 3B Closure — Prisma + migration", () => {
    ...
    it("closure migration is Phase O-Final compliant", () => {
        const migPath = fileURLToPath(new URL("../prisma/migrations/20261216000000_phase_3b_enterprise_closure/migration.sql", import.meta.url));
...
describe("Phase 3B Closure — service module surface", () => {
    it("intelligence-activity emitter exports `emitLifecycleEvent`", async () => {
        const m = await import("../src/services/intelligence/intelligence-activity.service.js");
    ...
    it("intelligence-quality exposes provider / reviewer / team projections", async () => {
        const m = await import("../src/services/intelligence/intelligence-quality.service.js");
    ...
    it("executive-metrics exposes the trends projector", async () => {
        const m = await import("../src/services/intelligence/executive-metrics.service.js");
    ...
    it("provider-budget exposes scoped enforcement + spend + breach reads", async () => {
        const m = await import("../src/services/intelligence/provider-budget.service.js");
    ...
    it("reviewer-correction exposes the version-chain projection", async () => {
        const m = await import("../src/services/intelligence/reviewer-correction.service.js");
    ...
    it("verification-manifest exposes the four closure writers", async () => {
        const m = await import("../src/services/intelligence/intelligence-verification-manifest.service.js");
...
describe("Phase 3B Closure — scoped budget enforcement", () => {
    it("CASE budget honours `caseId` (no team-wide leakage)", async () => {
        const { decideBudgetGate } = await import("../src/services/intelligence/provider-budget.service.js");
            ...
            providerUsageEvent: {
                aggregate: async (args) => {
                    const filtered = usageRows.filter((r) => {
                        if (args.where.caseId && r.caseId !== args.where.caseId)
                            return false;
                        return r.decision !== "BLOCK";
                    });
                    const sum = filtered.reduce((acc, r) => acc + Number(r.estimatedCostUsdMicros), 0);
                    return {
    ...
    it("PROJECT budget honours `projectId` (no team-wide leakage)", async () => {
        ...
        const prismaStub = {
            ...
            providerUsageEvent: {
                aggregate: async (args) => {
                    const filtered = usageRows.filter((r) => !args.where.projectId || r.projectId === args.where.projectId);
                    const sum = filtered.reduce((acc, r) => acc + Number(r.estimatedCostUsdMicros), 0);
    ...
    it("CASE + PROJECT requires `scopeTargetId` on creation", async () => {
        const { createBudget } = await import("../src/services/intelligence/provider-budget.service.js");
        const denied = await createBudget({
            prisma: { providerBudget: { create: async () => ({ id: "x" }) } },
            teamId: "team",
            scope: "CASE",
            scopeTargetId: null,
            provider: null,
...
describe("Phase 3B Closure — audit transparency federator", () => {
    it("surfaces failureReason on provider usage rows + federates lifecycle table", async () => {
        const fed = await import("../src/services/intelligence/audit-transparency.service.js");
        const stub = {
            providerUsageEvent: {
                findMany: async () => [
                    {
...
describe("Phase 3B Closure — HTTP routes", () => {
    it("intelligence-platform.routes registers trends + quality + version-chain + breaches + spend", () => {
        const path = fileURLToPath(new URL("../src/routes/intelligence-platform.routes.ts", import.meta.url));
...
describe("Phase 3B Closure — UI + nav registry", () => {
    it("executive page exposes range selector + trend tiles + new families", () => {
        const path = fileURLToPath(new URL("../../../apps/web/app/(app)/executive/page.tsx", import.meta.url));
        const src = readFileSync(path, "utf8");
    ...
    it("intelligence quality page exists with bounded anchors", () => {
        const path = fileURLToPath(new URL("../../../apps/web/app/(app)/intelligence-quality/page.tsx", import.meta.url));
        const src = readFileSync(path, "utf8");
    ...
    it("budget center page exists with spend + breach tables", () => {
        const path = fileURLToPath(new URL("../../../apps/web/app/(app)/budget-center/page.tsx", import.meta.url));
        const src = readFileSync(path, "utf8");
    ...
    it("audit transparency page surfaces failure reason", () => {
        const path = fileURLToPath(new URL("../../../apps/web/app/(app)/audit-transparency/page.tsx", import.meta.url));
        const src = readFileSync(path, "utf8");
...
describe("Phase 3B Closure — module resolution", () => {
    it("activity emitter module imports cleanly", async () => {
        await expect(import("../src/services/intelligence/intelligence-activity.service.js")).resolves.toBeDefined();
    });
    it("quality module imports cleanly", async () => {
        await expect(import("../src/services/intelligence/intelligence-quality.service.js")).resolves.toBeDefined();
    });
