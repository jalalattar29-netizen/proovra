...
const GUIDANCE = readWeb("app/(app)/capture/_lib/CaptureWorkflowGuidance.tsx");
...
describe("Phase 38.11 — CommandCenter renders sections from registry + persona order", () => {
    it("declares a SECTION_RENDERERS registry mapping sectionId → renderer", () => {
        expect(COMMAND_CENTER).toMatch(/const\s+SECTION_RENDERERS\s*:\s*Record<\s*string,\s*SectionRendererEntry\s*>/);
    });
    ...
    it("renders sections via the registry-driven render loop — not hand-coded JSX per section", () => {
        ...
        expect(COMMAND_CENTER).toMatch(/buildSectionRenderPlan\(finalSectionOrder\)\.map\(/);
    ...
    it("removes the hand-coded ec-grid-2col section JSX (replaced by registry loop)", () => {
        ...
        expect(COMMAND_CENTER).not.toMatch(/<div\s+id="caseOperations"\s+data-section="caseOperations"/);
        expect(COMMAND_CENTER).not.toMatch(/<div\s+id="governancePosture"\s+data-section="governancePosture"/);
    });
    it("each persona priority list maps to registered section ids", () => {
        ...
        for (const id of REGISTERED) {
            expect(COMMAND_CENTER).toMatch(new RegExp(`${id}:\\s*\\{\\s*ariaLabel:`));
        }
    });
...
describe("Phase 38.11 — capture workflow guidance panel", () => {
    ...
    it("guidance copy is operational tone — no legal/marketing overclaims", () => {
        ...
        for (const pattern of FORBIDDEN) {
            expect(GUIDANCE_DATA, `guidance copy must not match ${pattern}`).not.toMatch(pattern);
        }
    });
});
// =============================================================================
...
describe("Phase 38.11 — route registry expansion (8 new routes)", () => {
    ...
    it("governance subroutes inherit ORGANIZATION_ONLY scope", () => {
        for (const id of [
            ...
            const block = REGISTRY.match(new RegExp(`id:\\s*"${id.replace(/\./g, "\\.")}"[\\s\\S]*?requiredActiveSpace:\\s*"([A-Z_]+)"`));
            expect(block?.[1], `${id} must be ORGANIZATION_ONLY`).toBe("ORGANIZATION_ONLY");
        }
    });
});
// =============================================================================
...
describe("Phase 38.11 — forbidden copy + safety statement final lock", () => {
    ...
    it("required safety statement remains on All Tools + persona + command palette", () => {
        ...
        expect(TOOLS).toMatch(/Workflow profiles personalize layout, defaults, and\s+recommendations\. They do not change permissions or remove tools\./);
        expect(PERSONA).toMatch(/workflow profile tunes ordering, defaults, and labels\. It does\s+NOT change/i);
