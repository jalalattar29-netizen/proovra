...
import {
  ...
  EXTERNAL_PORTAL_DENIAL_REASONS,
...
const SHARED = readSource(
  "../../../packages/shared/src/external-review-portal.ts",
);
const SHARED_INDEX = readSource("../../../packages/shared/src/index.ts");
const SCHEMA = readSource("../../../services/api/prisma/schema.prisma");
const MIGRATION = readSource(
  "../../../services/api/prisma/migrations/20261015000000_phase_2b_closure_invitation_delivery_sso/migration.sql",
);
...
const SVC_EMAIL = readSource(
  "../../../services/api/src/services/external-review/portal-invitation-email.service.ts",
);
const SVC_BULK = readSource(
  "../../../services/api/src/services/external-review/portal-bulk-invitations.service.ts",
);
const SVC_SSO = readSource(
  "../../../services/api/src/services/external-review/portal-sso.service.ts",
);
const SVC_PROJ = readSource(
  "../../../services/api/src/services/external-review/portal-projection.service.ts",
);
...
const UI_OPERATOR = readSource(
  "../../../apps/web/app/(app)/review/external/page.tsx",
);
const UI_ACCEPT = readSource(
  "../../../apps/web/app/portal/accept/[grantId]/page.tsx",
);
const UI_SSO_CALLBACK = readSource(
  "../../../apps/web/app/portal/sso/callback/[grantId]/route.ts",
);
...
describe("Phase 2B Closure — shared contracts", () => {
  ...
  it("activity catalog adds all closure events", () => {
    ...
    for (const code of required) {
      expect((EXTERNAL_PORTAL_ACTIVITY_CODES as readonly string[]).includes(code)).toBe(
        true,
      );
    }
  });
  ...
  it("denial catalog preserves Phase 2B bounded vocabulary", () => {
    for (const d of [
      ...
      expect(
        (EXTERNAL_PORTAL_DENIAL_REASONS as readonly string[]).includes(d),
      ).toBe(true);
    }
  });
...
describe("Phase 2B Closure — Prisma additions", () => {
  it("role assignment gains federation + adaptive auth columns", () => {
    expect(SCHEMA).toMatch(
      /authMethod\s+String\s+@default\("TOKEN"\)\s+@map\("auth_method"\)/,
    );
    expect(SCHEMA).toMatch(
      /ssoConnectionId\s+String\?\s+@map\("sso_connection_id"\)/,
    );
    expect(SCHEMA).toMatch(
      /allowedDomains\s+String\[\]\s+@default\(\[\]\)\s+@map\("allowed_domains"\)/,
    );
    expect(SCHEMA).toMatch(
      /ssoSubjectHash\s+String\?\s+@map\("sso_subject_hash"\)/,
    );
  ...
  it("Phase O-Final compliant SQL migration creates table + guarded indexes", () => {
    ...
    expect(MIGRATION).not.toMatch(
      /CREATE TABLE IF NOT EXISTS "external_review_invitation_deliveries"/,
    );
    expect(MIGRATION).toMatch(
      /ALTER TABLE IF EXISTS "external_reviewer_role_assignments"[\s\S]+auth_method/,
    );
...
describe("Phase 2B Closure — invitation delivery service", () => {
  ...
  it("PENDING → SENT on provider ok + INVITATION_EMAIL_SENT emitted", () => {
    ...
    expect(SVC_EMAIL).toMatch(
      /status:\s*"SENT"\s*satisfies InvitationDeliveryStatus/,
    );
  ...
  it("PENDING → FAILED on provider error + INVITATION_EMAIL_FAILED emitted", () => {
    expect(SVC_EMAIL).toMatch(
      /status:\s*"FAILED"\s*satisfies InvitationDeliveryStatus/,
    );
...
describe("Phase 2B Closure — bulk invitation service", () => {
  ...
  it("a failing row continues the loop (never aborts the batch)", () => {
    ...
    const failurePathContinues =
      /outcome: "INVALID_EMAIL"[\s\S]{0,500}continue;/m;
...
describe("Phase 2B Closure — portal session extension", () => {
  ...
  it("persists mfaSatisfiedAtUtc after a successful MFA challenge", () => {
    expect(SVC_SESSION).toMatch(
      /externalReviewerRoleAssignment\.update[\s\S]+mfaSatisfiedAtUtc:\s*new Date\(\)/,
    );
  });
...
describe("Phase 2B Closure — Operator Console UI", () => {
  ...
  it("hides the manual token reveal behind explicit break-glass UI", () => {
    ...
    expect(UI_OPERATOR).not.toMatch(
      /<textarea[^>]*data-invitation-raw-token/,
    );
  });
