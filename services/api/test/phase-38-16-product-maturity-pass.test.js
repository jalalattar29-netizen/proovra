...
const CONTEXTUAL_HELP = readWeb("components/contextual-help/ContextualHelp.tsx");
...
describe("Phase 38.16 — capture intake progress rail", () => {
    ...
    it("output is always exactly 5 stages, in canonical order", () => {
        ...
        expect(STAGES).toMatch(/const\s+order:\s*IntakeStageId\[\]\s*=\s*\[\s*"select_template",\s*"add_materials",\s*"map_context",\s*"review_readiness",\s*"finish",?\s*\]/);
    });
...
describe("Phase 38.16 — CaptureIntakeRail component contract", () => {
    ...
    it("active stage carries aria-current=\"step\"", () => {
        expect(RAIL).toMatch(/aria-current=\{stage\.status\s*===\s*"active"\s*\?\s*"step"\s*:\s*undefined\}/);
    });
    ...
    it("capture page does NOT use rail state to block finalization", () => {
        expect(CAPTURE).not.toMatch(/if\s*\(\s*!?rail[\s\S]{0,80}return\s+null/);
        expect(CAPTURE).not.toMatch(/if\s*\(\s*stages[\s\S]{0,80}(disable|return null|throw)/);
    });
});
// =============================================================================
...
describe("Phase 38.16 — density-aware CSS for owned panels", () => {
    ...
    it("compact + spacious modes override the variables", () => {
        expect(SHELL_CSS).toMatch(/data-operational-density="compact"\]\s*\[data-capture-intake-rail\]/);
        expect(SHELL_CSS).toMatch(/data-operational-density="spacious"\]\s*\[data-capture-intake-rail\]/);
    });
    it("each owned panel selector appears in the density block", () => {
        for (const selector of PANEL_SELECTORS) {
            expect(SHELL_CSS, `density block must target [${selector}]`).toMatch(new RegExp(`\\[${selector}\\]`));
        }
    });
    it("capture page emits the canonical data-operational-density attribute", () => {
        expect(CAPTURE).toMatch(/data-operational-density=\{[\s\S]{0,80}operationalDensityPreference/);
    });
});
// =============================================================================
...
describe("Phase 38.16 — ContextualHelp component", () => {
    it("imports the canonical resolveWorkflowHelp helper", () => {
        expect(CONTEXTUAL_HELP).toMatch(/import\s*\{[\s\S]{0,200}resolveWorkflowHelp[\s\S]{0,200}\}\s*from\s*["'][^"']*platform-context["']/);
    });
    ...
    it("dismiss is persisted in localStorage scoped per (workflow, surface)", () => {
        expect(CONTEXTUAL_HELP).toMatch(/DISMISS_KEY_PREFIX[\s\S]{0,40}workflow[\s\S]{0,40}surface/);
...
describe("Phase 38.16 — manual QA checklist for browser-side verification", () => {
    it("checklist file exists alongside the ContextualHelp component", () => {
        const checklistPath = webPath("components/contextual-help/MANUAL_QA_CHECKLIST.md");
        expect(existsSync(checklistPath), `${checklistPath} must exist`).toBe(true);
    });
    it("checklist enumerates Responsive + Accessibility sections", () => {
        const checklist = readWeb("components/contextual-help/MANUAL_QA_CHECKLIST.md");
    ...
    it("checklist references every panel selector under verification", () => {
        ...
        for (const sel of [
            ...
            expect(checklist, `checklist must reference [${sel}]`).toMatch(new RegExp(sel));
        }
    });
    it("checklist refuses to claim browser-verified pass without verification", () => {
        const checklist = readWeb("components/contextual-help/MANUAL_QA_CHECKLIST.md");
        expect(checklist).toMatch(/deferred|do not claim|cannot be verified/i);
    });
});
// =============================================================================
