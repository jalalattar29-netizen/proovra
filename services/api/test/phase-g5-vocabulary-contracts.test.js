 ...
 */
const TENANT_WORDING_ALLOWLIST = new Set([
...
describe("Phase G5.2 — `tenant` is never visible to operators", () => {
    it("no JSX-string usage of the word `tenant` anywhere in apps/web (outside allowlist)", () => {
        ...
        for (const f of files) {
            ...
            if (TENANT_WORDING_ALLOWLIST.has(rel))
                continue;
        ...
        if (offenders.length > 0) {
            throw new Error("User-facing `tenant` strings found in apps/web — must be Workspace / Matter:\n" +
                offenders
...
describe("Phase G5.2 — Artifact vocabulary (A2 contract) preserved", () => {
    it("no UI string says `Download report` without `PDF`", () => {
        ...
        if (offenders.length > 0) {
            throw new Error("Artifact-specific download string must say `Download Report PDF`:\n" +
                offenders
                    .map((o) => `  ${o.file}:${o.line} — ${o.excerpt}`)
                    .join("\n"));
        }
    ...
    it("no UI string says `Download verification package` without `ZIP`", () => {
        ...
        if (offenders.length > 0) {
            throw new Error("Artifact-specific download string must say `Download Verification Package ZIP`:\n" +
                offenders
                    .map((o) => `  ${o.file}:${o.line} — ${o.excerpt}`)
                    .join("\n"));
        }
...
// ---------------------------------------------------------------------------
const TEAM_WORDING_ALLOWLIST = new Set([
...
describe("Phase G5.2 — Team → Workspace carryover", () => {
    it("no NEW file outside the allowlist uses `Team` in user-facing UI strings", () => {
        ...
        const labelRe = /["'>](?:[^"'<>]*\b)?Team(?:s)?\b(?!Id|Member|Role|Workspace|Permission)/;
        for (const f of files) {
            const rel = relative(WEB_ROOT, f).replace(/\\/g, "/");
            if (TEAM_WORDING_ALLOWLIST.has(rel))
                continue;
            const stripped = stripComments(readFileSync(f, "utf8"));
            const lines = stripped.split("\n");
            for (let i = 0; i < lines.length; i++) {
        ...
        if (offenders.length > 0) {
            throw new Error("New `Team` UI strings outside the G5.2 allowlist — use `Workspace`:\n" +
                offenders
                    .map((o) => `  ${o.file}:${o.line} — ${o.excerpt}`)
                    .join("\n"));
        }
...
// ---------------------------------------------------------------------------
const CASE_WORDING_ALLOWLIST = new Set([
...
describe("Phase G5.2 — Case → Matter carryover", () => {
    it("no NEW file outside the allowlist uses operational `Case` in UI", () => {
        ...
        // "test case".
        const labelRe = /["'>](?:[^"'<>]*\b)?(?:Open|Add|Remove|Create|Edit|Delete|Manage|Assign)\s+(?:to\s+|from\s+|the\s+)?Case\b/;
        for (const f of files) {
            const rel = relative(WEB_ROOT, f).replace(/\\/g, "/");
            if (CASE_WORDING_ALLOWLIST.has(rel))
                continue;
            const stripped = stripComments(readFileSync(f, "utf8"));
            const lines = stripped.split("\n");
            for (let i = 0; i < lines.length; i++) {
        ...
        if (offenders.length > 0) {
            throw new Error("New `Case` UI strings outside the G5.2 allowlist — use `Matter`:\n" +
                offenders
                    .map((o) => `  ${o.file}:${o.line} — ${o.excerpt}`)
                    .join("\n"));
        }
