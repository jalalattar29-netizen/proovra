...
const API_SENTRY_SRC = readFileSync(
  fileURLToPath(new URL("../src/observability/sentry.ts", import.meta.url)),
  "utf8",
);
const WORKER_SENTRY_SRC = readFileSync(
  fileURLToPath(new URL("../../worker/src/sentry.ts", import.meta.url)),
  "utf8",
);
const API_OTEL_SRC = readFileSync(
  fileURLToPath(new URL("../src/observability/otel.ts", import.meta.url)),
  "utf8",
);
const WORKER_OTEL_SRC = readFileSync(
  fileURLToPath(new URL("../../worker/src/otel.ts", import.meta.url)),
  "utf8",
);
...
const WORKER_INDEX_SRC = readFileSync(
...
describe("O1.3 — Sentry init passes skipOpenTelemetrySetup gated on OTEL_ENABLED", () => {
  for (const [label, src] of [
    ...
    it(`${label} exposes an isOtelEnabled() helper that reads OTEL_ENABLED === "true"`, () => {
      ...
      expect(src).toMatch(
        /process\.env\.OTEL_ENABLED[\s\S]{0,80}toLowerCase\s*\(\s*\)\s*===\s*"true"/,
      );
    });
...
describe("O1.3 — entrypoint import order: otel-bootstrap precedes sentry", () => {
  it("api/server.ts side-effect-imports otel-bootstrap BEFORE importing sentry", () => {
    const bootstrapIdx = API_SERVER_SRC.indexOf(
      'import "./observability/otel-bootstrap.js"',
    );
    const sentryIdx = API_SERVER_SRC.indexOf(
  ...
  it("worker/index.ts side-effect-imports otel-bootstrap BEFORE importing sentry", () => {
    const bootstrapIdx = WORKER_INDEX_SRC.indexOf(
  ...
  for (const [label, src] of [
    ...
    it(`${label} only references token/secret/etc. inside the redactValue helper or scrubbing arms`, () => {
      ...
      const logLines = src.split("\n").filter(
        (l) =>
          /console\.(log|info|warn|error|debug)/.test(l) ||
...
describe("O1.3 — gate flips with OTEL_ENABLED at runtime", () => {
  ...
  function expectedIsOtelEnabled(env: NodeJS.ProcessEnv): boolean {
