...
import { PRESENCE_HEARTBEAT_TTL_MS, PRESENCE_MAX_VIEWERS_PER_KEY, } from "../src/services/presence/presence-backend.js";
import { RedisPresenceBackend } from "../src/services/presence/redis-presence-backend.js";
import { __currentSelectorBackendKind, __resetSelectorForTests, __setSelectorBackendForTests, listViewersAsyncViaSelector, recordHeartbeatViaSelector, } from "../src/services/presence/presence-selector.js";
// ---------------------------------------------------------------------------
...
describe("O2.1 — selector backend choice", () => {
    ...
    it("backends can be injected for test isolation", async () => {
        ...
        __setSelectorBackendForTests(new RedisPresenceBackend(mock));
...
describe("O2.1 — RedisPresenceBackend round-trip", () => {
    it("records + lists across simulated instances", async () => {
        ...
        const instanceA = new RedisPresenceBackend(redis);
        const instanceB = new RedisPresenceBackend(redis);
    ...
    it("evicts entries older than HEARTBEAT_TTL_MS at read time", async () => {
        const redis = new MockRedis();
        const backend = new RedisPresenceBackend(redis);
    ...
    it("backend is best-effort — Redis errors degrade to empty / no-op", async () => {
        ...
        await expect(backend.recordHeartbeat({
            ...stableInput(),
            userId: "u-a",
            displayName: "Anne",
    ...
    it("bounds hash size to MAX_VIEWERS_PER_KEY (evicts oldest when over)", async () => {
        ...
        for (let i = 0; i < PRESENCE_MAX_VIEWERS_PER_KEY + 5; i++) {
            ...
            h.set(`u-${i}`, JSON.stringify({ displayName: `User ${i}`, tMs: 1000 + i }));
        }
...
describe("O2.1 — route wires through the selector", () => {
    it("presence.routes.ts imports recordHeartbeatViaSelector + listViewersAsyncViaSelector", () => {
        const src = readFileSync(fileURLToPath(new URL("../src/routes/presence.routes.ts", import.meta.url)), "utf8");
