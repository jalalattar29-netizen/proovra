...
describe("projectIntakeLinkForExternalView", () => {
    ...
    it("derives isAnonymous from the intake mode", () => {
        expect(projectIntakeLinkForExternalView(fakeLink({ intakeMode: "EXTERNAL_ANONYMOUS" }))
            .isAnonymous).toBe(true);
        expect(projectIntakeLinkForExternalView(fakeLink({ intakeMode: "EXTERNAL_PSEUDONYMOUS" }))
            .isAnonymous).toBe(true);
        expect(projectIntakeLinkForExternalView(fakeLink({ intakeMode: "EXTERNAL_ONE_TIME" }))
            .isAnonymous).toBe(false);
        expect(projectIntakeLinkForExternalView(fakeLink({ intakeMode: "EXTERNAL_REUSABLE" }))
            .isAnonymous).toBe(false);
    });
});
