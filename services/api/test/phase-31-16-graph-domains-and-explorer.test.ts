...
describe("Phase 31.16 — REPORT graph domain", () => {
  ...
  it("SELECT JOINs reports → evidence + team-anchored", () => {
    expect(RECONCILER_SRC).toMatch(
      /FROM "reports" r[\s\S]*?JOIN "evidence" e ON e\."id" = r\."evidence_id"[\s\S]*?WHERE e\."team_id" = \$1[\s\S]*?AND e\."deleted_at" IS NULL/,
    );
  });
...
describe("Phase 31.16 — VERIFICATION_PACKAGE graph domain", () => {
  it("section header present", () => {
    expect(RECONCILER_SRC).toContain(
      "Phase 31.16 — VERIFICATION_PACKAGE domain reconciliation",
    );
  });

  it("SELECT JOINs verification_packages → evidence + team-anchored", () => {
    expect(RECONCILER_SRC).toMatch(
      /FROM "verification_packages" vp[\s\S]*?JOIN "evidence" e ON e\."id" = vp\."evidence_id"[\s\S]*?WHERE e\."team_id" = \$1[\s\S]*?AND e\."deleted_at" IS NULL/,
    );
  });

  it("upserts VERIFICATION_PACKAGE node with WORKSPACE_INTERNAL visibility", () => {
    expect(RECONCILER_SRC).toMatch(
      /upsertNode\(\s*client,\s*teamId,\s*"VERIFICATION_PACKAGE"[\s\S]*?"WORKSPACE_INTERNAL"/,
    );
  });

  it("emits GENERATED_PACKAGE edge with SYSTEM source + HIGH confidence", () => {
    expect(RECONCILER_SRC).toMatch(
      /upsertEdge\([\s\S]*?"GENERATED_PACKAGE"[\s\S]*?"SYSTEM"[\s\S]*?"HIGH"/,
    );
  });

  it("stale-sweep for VERIFICATION_PACKAGE nodes is team-anchored", () => {
    const block = RECONCILER_SRC.match(
      /UPDATE "investigation_graph_nodes" n[\s\S]*?n\."node_kind" = 'VERIFICATION_PACKAGE'[\s\S]*?\)\s*\)/,
    )?.[0];
...
describe("Phase 31.16 — EXPORT graph domain", () => {
  it("section header present", () => {
    expect(RECONCILER_SRC).toContain(
      "Phase 31.16 — EXPORT domain reconciliation",
    );
  });

  it("SELECT against governance_export_snapshots is team-anchored", () => {
    expect(RECONCILER_SRC).toMatch(
      /FROM "governance_export_snapshots"\s+WHERE "team_id" = \$1/,
    );
  });

  it("upserts EXPORT node with WORKSPACE_INTERNAL visibility", () => {
    expect(RECONCILER_SRC).toMatch(
      /upsertNode\(\s*client,\s*teamId,\s*"EXPORT"[\s\S]*?"WORKSPACE_INTERNAL"/,
    );
  });

  it("emits EXPORTED_AS edge with SYSTEM source + HIGH confidence, only when evidence_id is present", () => {
    expect(RECONCILER_SRC).toMatch(
      /if \(x\.evidence_id\)[\s\S]*?upsertEdge\([\s\S]*?"EXPORTED_AS"[\s\S]*?"SYSTEM"[\s\S]*?"HIGH"/,
    );
  });

  it("EXPORT stale-sweep is team-anchored on UPDATE + sub-select", () => {
    const block = RECONCILER_SRC.match(
      /UPDATE "investigation_graph_nodes" n[\s\S]*?n\."node_kind" = 'EXPORT'[\s\S]*?\)\s*\)/,
    )?.[0];
  ...
  it("EXPORT node label NEVER includes snapshot_payload (which can carry operator-private fields)", () => {
    const exportBlock = RECONCILER_SRC.match(
      /Phase 31\.16 — EXPORT domain reconciliation[\s\S]*?\}\s*catch\s*\{[\s\S]*?best-effort/,
    )?.[0];
...
describe("Phase 31.16 — anti-leak invariants on new domain steps", () => {
  it("no storage internals / signed URLs / private notes referenced anywhere in the 3 new steps", () => {
    const idxStart = RECONCILER_SRC.indexOf(
      "Phase 31.16 — REPORT domain reconciliation",
    );
...
describe("Phase 32.15 — Case Graph Explorer UI", () => {
  const src = readSource(
    "../../../apps/web/app/(app)/investigation/cases/[caseId]/graph/page.tsx",
  );
