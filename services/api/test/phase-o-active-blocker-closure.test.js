...
import { evaluateCaseMutationPermission, evaluateCrossTeamAttach, } from "../src/services/cases/case-permission.service.js";
import { STRIPE_SIGNATURE_TOLERANCE_SEC, StripeSignatureError, verifyStripeSignature, } from "../src/services/stripe.service.js";
...
describe("A-1 — case DELETE / MANAGE_SETTINGS role gate", () => {
    ...
    it("MEMBER access role CANNOT DELETE", () => {
        ...
        expect(r.allowed === false ? r.reason : "").toMatch(/OWNER or ADMIN/i);
    });
    ...
    it("CaseAssignment role (REVIEWER/INVESTIGATOR/GOVERNANCE/OBSERVER) does NOT elevate DELETE", () => {
        for (const assignment of [
            ...
            expect.soft(r.allowed, `MEMBER + ${assignment} must NOT delete`).toBe(false);
        }
    });
    it("MANAGE_SETTINGS (rename) follows the same gate", () => {
        expect(evaluateCaseMutationPermission({
...
describe("A-2 — cross-team evidence attach gate", () => {
    ...
    it("different teamIds are blocked with CROSS_TEAM_ATTACH_BLOCKED", () => {
        ...
        expect(r.allowed).toBe(false);
        expect(r.allowed === false ? r.code : "").toBe("CROSS_TEAM_ATTACH_BLOCKED");
    });
...
describe("A-4 — Stripe webhook signature timestamp tolerance", () => {
    ...
    it("accepts a signature within the tolerance window", () => {
        withEnv(() => {
            ...
            expect(() => verifyStripeSignature(Buffer.from(body), header, { nowMs: now })).not.toThrow();
        });
    });
    it("rejects a signature older than the tolerance window", () => {
        withEnv(() => {
            ...
            expect(thrown.code).toBe("TIMESTAMP_OUT_OF_TOLERANCE");
        });
    });
    ...
    it("rejects a garbage timestamp before HMAC compare", () => {
        withEnv(() => {
            ...
            expect(() => verifyStripeSignature(Buffer.from(body), header, { nowMs: Date.now() })).toThrow(StripeSignatureError);
        });
    });
    it("rejects a missing v1 segment", () => {
        withEnv(() => {
            ...
            expect(thrown).toBeInstanceOf(StripeSignatureError);
            expect(thrown.code).toBe("MISSING_FIELDS");
        });
    });
});
// ---------------------------------------------------------------------------
...
describe("C-1 — cover PDF compactValue is derived from tone", () => {
    ...
    it("immutable_storage warning tone does NOT render the word 'Verified'", () => {
        ...
        const m = src.match(/includes\("immutable storage"\)\)\s*\{([\s\S]*?)\}\s*\n/);
        expect(m).toBeTruthy();
        const body = m[1];
    ...
    it("core_integrity branch is also tone-derived (no hard-coded literal)", () => {
        const src = read(COVER);
        const m = src.match(/includes\("core integrity"\)\)\s*\{([\s\S]*?)\}\s*else/);
        expect(m).toBeTruthy();
        const body = m[1];
        expect.soft(body).toMatch(/params\.tone/);
    });
    it("trust signals NEVER render 'Verified' when tone is danger", () => {
        ...
        const literalAssigns = src.match(/compactValue\s*=\s*"Verified"(?!\s*[?:])/g);
        expect(literalAssigns, "No bare `compactValue = \"Verified\"` should remain.").toBeNull();
    });
});
// ---------------------------------------------------------------------------
...
describe("D-1 — frontend double-.json() sweep is complete", () => {
    it("no apps/web/** file has `const res = await apiFetch(...)\\n const X = await res.json()` pattern", () => {
        ...
        for (const path of FIXED_FILES) {
            ...
            const hits = src.match(/await\s+apiFetch[\s\S]{0,400}?await\s+(res|response)\.json\(\)/g) ??
                [];
            if (hits.length > 0)
                offenders.push(`${path} (${hits.length})`);
        }
        expect(offenders, `D-1 sweep incomplete in: ${offenders.join(", ")}`).toEqual([]);
    });
});
// ---------------------------------------------------------------------------
...
describe("B-4 — no silent `.catch(() => null)` on appendCustodyEvent", () => {
    ...
    it("evidence.routes.ts uses observable catch (no silent null swallow)", () => {
        ...
        expect.soft(src).not.toMatch(/appendCustodyEvent\([\s\S]{0,400}?\)\s*\.catch\(\s*\(\s*\)\s*=>\s*null\s*\)/);
    });
});
