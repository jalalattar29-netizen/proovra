...
import { getInstanceExportPolicySummary, getInstanceTimeline, projectStep, projectStepForReviewer, } from "../src/services/workflows/evidence-workflow-engine.service.js";
// -----------------------------------------------------------------------------
...
describe("Phase 23 — reviewer projection wires the privateReviewerNote", () => {
    const fixture = {
        ...
        waiverReason: null,
        privateReviewerNote: "INTERNAL: contributor flagged for repeat-submission review.",
        createdAt: new Date(),
        updatedAt: new Date(),
    };
    it("safe projection omits the private reviewer note", () => {
        const out = projectStep(fixture);
        const json = JSON.stringify(out);
        expect(json).not.toMatch(/INTERNAL/);
        expect(out.privateReviewerNote).toBeUndefined();
    });
    it("reviewer projection includes the private reviewer note verbatim", () => {
        const out = projectStepForReviewer(fixture);
        expect(out.privateReviewerNote).toMatch(/INTERNAL/);
    });
    it("reviewer projection inherits every safe-projection field", () => {
        const safe = projectStep(fixture);
        const reviewer = projectStepForReviewer(fixture);
...
describe("Phase 23 — new backend routes auth posture", () => {
    it("timeline + export-policy + templates routes all use requireAuth + 404 guard", async () => {
        ...
        expect(src).toMatch(/app\.get\(\s*"\/v1\/workflows\/instances\/:id\/timeline"[\s\S]{0,200}preHandler:\s*requireAuth/);
        expect(src).toMatch(/app\.get\(\s*"\/v1\/workflows\/instances\/:id\/export-policy"[\s\S]{0,200}preHandler:\s*requireAuth/);
        expect(src).toMatch(/app\.get\(\s*"\/v1\/workflows\/templates"[\s\S]{0,200}preHandler:\s*requireAuth/);
...
describe("Phase 23 — workflow detail page guards", () => {
    it("handles every documented engine error code", async () => {
        const { readFile } = await import("node:fs/promises");
        const { fileURLToPath } = await import("node:url");
        const src = await readFile(fileURLToPath(new URL("../../../apps/web/app/(app)/workflows/[id]/page.tsx", import.meta.url)), "utf8");
        for (const code of [
            "STEP_UP_REQUIRED",
...
describe("Phase 23 — workflow list page", () => {
    it("calls the new templates alias endpoint", async () => {
        const { readFile } = await import("node:fs/promises");
        const { fileURLToPath } = await import("node:url");
        const src = await readFile(fileURLToPath(new URL("../../../apps/web/app/(app)/workflows/page.tsx", import.meta.url)), "utf8");
