...
import {
  ...
  PLAN_LINE_ENTITLEMENTS,
} from "../src/services/packaging/entitlement.service.js";
...
} from "../src/services/packaging/webhooks/webhook-platform.service.js";
...
import {
  ...
  rejectChainTransfer,
  listChainTransfers,
} from "../src/services/exchange/chain-transfer.service.js";
...
import {
  ...
  currentTierFor,
  COST_PER_GB_PER_MONTH_USD_MICROS,
...
import {
  buildLifecycleManifestEntry,
  buildRetentionManifestEntry,
  buildLegalHoldManifestEntry,
  buildArchiveManifestEntry,
  buildDestructionManifestEntry,
  buildExchangeManifestEntry,
  buildTransferManifestEntry,
} from "../src/services/lifecycle/lifecycle-manifest.service.js";
  ...
  const base: Record<string, unknown> = {
    ...
    archiveTierTransition: {
      create: async (args: { data: Record<string, unknown>; select: unknown }) => ({
        ...
        costEstimateUsdMicros: BigInt(args.data.costEstimateUsdMicros as number ?? 0),
        ...args.data,
      }),
      findFirst: async () => null,
      findMany: async () => [],
...
describe("2. Prisma schema + migration", () => {
  ...
  const migrationPath = path.join(
    __dirname,
    "../prisma/migrations/20261230000000_phase_4b_packaging_and_lifecycle/migration.sql",
  );
  ...
  it("assertQuotaEntitlement consumed=5+limit=5+requested=1 → QUOTA_EXCEEDED", async () => {
    const prisma = makePrismaStub({
      entitlementGrant: {
        ...
        upsert: async (args: { create: { id?: string } }) => ({ id: "g1", ...args.create }),
      },
      entitlementUsage: {
...
describe("11. Archive tier transitions + costs", () => {
  it("transitionEvidenceTier HOT→COLD inserts row with costEstimateUsdMicros", async () => {
    let insertedCost: number | undefined;
    const prisma = makePrismaStub({
      evidence: {
        findUnique: async () => ({
          id: "ev-1",
      ...
      archiveTierTransition: {
        create: async (args: { data: Record<string, unknown>; select: unknown }) => {
          insertedCost = args.data.costEstimateUsdMicros as number;
          return {
            id: "trans-1",
            ...args.data,
            transitionedAtUtc: new Date(),
...
describe("12. Webhook signing + delivery", () => {
  it("createWebhookEndpoint inserts row with 32-byte secret", async () => {
    ...
    const prisma = makePrismaStub({
      lifecycleWebhookEndpoint: {
        create: async (args: { data: Record<string, unknown>; select: unknown }) => {
          capturedSecret = args.data.secret as string;
...
describe("13. Webhook delivery state machine", () => {
  ...
  it("200 response → DELIVERED", async () => {
    ...
    globalThis.fetch = async () =>
      ({ status: 200, text: async () => "ok" }) as Response;
    try {
  ...
  it("500 + attemptCount<8 → RETRYING with nextRetryAtUtc", async () => {
    ...
    const origFetch = globalThis.fetch;
    globalThis.fetch = async () =>
      ({ status: 500, text: async () => "error" }) as Response;
    try {
      const result = await deliverWebhookDelivery({
        prisma: prisma as never,
        deliveryId: "del-2",
  ...
  it("generateSignedUrl on READY package persists signedUrl + expiresAtUtc", async () => {
    ...
    const prisma = makePrismaStub({
      evidenceExchangePackage: {
        ...
        update: async (args: { data: Record<string, unknown> }) => {
          persistedUrl = args.data.signedUrl as string;
          persistedExpiry = args.data.signedUrlExpiresAtUtc as Date;
          return args.data;
        },
        create: async () => ({ id: "pkg-1" }),
        findMany: async () => [],
...
describe("17. Verification package wires 7 lifecycle manifests", () => {
  ...
  const lcPath = path.join(
    __dirname,
    "../../worker/src/verification-package-lifecycle.ts",
  );
