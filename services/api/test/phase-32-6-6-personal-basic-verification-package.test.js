...
function assertNoForbiddenVocabInRange(label, source, start, end) {
    const slice = source.slice(start, end).toLowerCase();
    for (const word of FORBIDDEN_VOCAB) {
        const re = new RegExp(`\\b${word}\\b`);
        if (re.test(slice)) {
            throw new Error(`Phase 32.6.6 vocabulary violation in ${label}: forbidden token "${word}" appears in operator-facing source.`);
        }
    }
}
// =============================================================================
...
describe("Phase 32.6.6 — createVerificationPackage personal-basic branch", () => {
    ...
    it("declares packageMode as `personal_basic | team_governed`", () => {
        expect(fn).toMatch(/packageMode\s*:\s*"personal_basic"\s*\|\s*"team_governed"\s*=\s*data\.teamId\s*&&\s*data\.isPersonalTeam\s*===\s*false\s*\?\s*"team_governed"\s*:\s*"personal_basic"/);
        expect(fn).not.toMatch(/packageMode\s*:\s*"personal_basic"\s*\|\s*"team_governed"\s*=\s*data\.teamId\s*\?\s*"team_governed"\s*:\s*"personal_basic"/);
    });
    ...
    it("personal mode skips the eligibility gate (gate runs only for team mode)", () => {
        ...
        expect(fn).toMatch(/if\s*\(\s*packageMode\s*===\s*"team_governed"\s*\)\s*\{[\s\S]{0,400}assertPackageEligibleOrDeny/);
    });
    ...
    it("personal-mode notice text uses neutral language (no overclaims in the literal strings)", () => {
        ...
        const teamNotice = "Team-governed package; workspace governance policy applied at build time";
        ...
        for (const text of [personalNotice, teamNotice]) {
            ...
            for (const word of FORBIDDEN_VOCAB) {
                const re = new RegExp(`\\b${word}\\b`);
                if (re.test(lower)) {
                    throw new Error(`Phase 32.6.6 vocabulary violation: forbidden token "${word}" appears in user-facing notice string: ${text}`);
                }
            }
        }
    });
...
describe("Phase 32.6.6 — route /v1/evidence/:id/verification-package 410 retired", () => {
    ...
    it("no longer returns a live 410 with `verification_package_unavailable`", () => {
        ...
        expect(SRC).not.toMatch(/reply\.code\(410\)[\s\S]{0,400}verification_package_unavailable/);
        ...
        expect(SRC).not.toMatch(/^\s*reason:\s*"personal_workspace_no_team_governance_context"/m);
    });
...
describe("Phase 32.6.6 — artifact-status helper personal-workspace unavailable retired", () => {
    ...
    it("packageUnavailableForPersonalWorkspace is now constant `false`", () => {
        expect(SRC).toMatch(/packageUnavailableForPersonalWorkspace\s*=\s*false/);
    });
    it("does NOT derive unavailable from `finalized && !params.evidenceTeamId`", () => {
        expect(SRC).not.toMatch(/packageUnavailableForPersonalWorkspace\s*=\s*finalized\s*&&\s*!params\.evidenceTeamId/);
    });
    it("unavailableReason is always `null` (no enum value emitted)", () => {
        ...
        expect(SRC).not.toMatch(/unavailableReason:\s*packageUnavailableForPersonalWorkspace\s*\?\s*"personal_workspace_no_team_governance_context"/);
    });
...
describe("Phase 32.6.6 — package mode catalog is bounded", () => {
    ...
    it("personal mode skips the package eligibility gate; team mode runs it", () => {
        const teamGatedIdx = SRC.indexOf('if (packageMode === "team_governed")');
