...
const REVIEWER_CONSOLE_ROUTES = readApi("src/routes/reviewer-console.routes.ts");
...
describe("Phase O Stream A — Sentry NODE-W /v1/ops/metrics", () => {
    ...
    it("returns bounded 400 WORKSPACE_CONTEXT_REQUIRED when no workspace", () => {
        expect(OPS_ROUTES).toMatch(/code: "WORKSPACE_CONTEXT_REQUIRED"[\s\S]{0,400}Select a workspace to view operational metrics/);
    });
    it("uses safeParse (never .parse) for the resolved object", () => {
        expect(OPS_ROUTES).toMatch(/\/v1\/ops\/metrics[\s\S]{0,2800}TeamIdQuery\.safeParse/);
    });
    it("emits requestId on the bounded error response", () => {
        expect(OPS_ROUTES).toMatch(/code: "WORKSPACE_CONTEXT_REQUIRED"[\s\S]{0,400}requestId: req\.id/);
    });
});
describe("Phase O Stream A — Sentry NODE-1G /v1/reviewer-ops/queue", () => {
    ...
    it("returns bounded 400 WORKSPACE_CONTEXT_REQUIRED when no workspace", () => {
        expect(REVIEWER_OPS_ROUTES).toMatch(/code: "WORKSPACE_CONTEXT_REQUIRED"[\s\S]{0,400}Select a workspace to view the reviewer queue/);
    });
    it("uses safeParse and surfaces INVALID_QUERY on failure", () => {
        expect(REVIEWER_OPS_ROUTES).toMatch(/QueueQuery\.safeParse/);
        expect(REVIEWER_OPS_ROUTES).toMatch(/code: "INVALID_QUERY"/);
    });
    it("preserves the limit cap at 100 with default 50", () => {
        expect(REVIEWER_OPS_ROUTES).toMatch(/\.max\(100\)\.optional\(\)\.default\(50\)/);
    });
});
describe("Phase O Stream A — Sentry NODE-11 /v1/reviewer-ops/console", () => {
    it("declares the prismaClient parameter as optional", () => {
        ...
        expect(REVIEWER_CONSOLE_ROUTES).toMatch(/prismaClient\?: PrismaClient,/);
    });
    it("re-anchors every service call to a local `client` constant", () => {
        expect(REVIEWER_CONSOLE_ROUTES).toMatch(/const client: PrismaClient = prismaClient \?\? prisma;/);
    ...
    it("uses safeParse on the query schema with bounded 400 on failure", () => {
        ...
        expect(REVIEWER_CONSOLE_ROUTES).toMatch(/code: "INVALID_QUERY"[\s\S]{0,400}requestId: req\.id/);
    });
    it("wraps the SLA composition defensively (degraded on undefined shape)", () => {
        ...
        expect(REVIEWER_CONSOLE_ROUTES).toMatch(/let slaSnapshot:[\s\S]{0,400}let slaStatus: SectionStatus/);
        expect(REVIEWER_CONSOLE_ROUTES).toMatch(/reviewer_console\.section_failed/);
    });
});
describe("Phase O Stream A — Sentry NODE-1D /v1/orgs/:id/members", () => {
    it("uses UuidParam.safeParse (never .parse) on :id", () => {
        expect(ORGANIZATIONS_ROUTES).toMatch(/\/v1\/orgs\/:id\/members[\s\S]{0,1400}UuidParam\.safeParse\(\s*\(req\.params as \{ id: string \}\)\.id,?\s*\)/);
    });
    it("returns bounded 400 INVALID_ORG_ID on invalid UUID", () => {
        expect(ORGANIZATIONS_ROUTES).toMatch(/code: "INVALID_ORG_ID"[\s\S]{0,400}Invalid organization id\./);
    });
    it("emits requestId on the bounded error response", () => {
        expect(ORGANIZATIONS_ROUTES).toMatch(/code: "INVALID_ORG_ID"[\s\S]{0,400}requestId: req\.id/);
    });
});
