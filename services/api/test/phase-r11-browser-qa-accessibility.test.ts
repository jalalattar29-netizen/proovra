...
function listAppFiles(): Array<{ label: string; text: string }> {
  ...
  ).map((f) => ({ label: f.rel, text: readFileSync(f.abs, "utf8") }));
}

function listAppCss(): Array<{ label: string; text: string }> {
  return walkFiles(
    webPath(""),
    (rel) => rel.endsWith(".css") && !rel.startsWith(".next/"),
  ).map((f) => ({ label: f.rel, text: readFileSync(f.abs, "utf8") }));
}
...
describe("R11 Group 1 — cross-phase byte-pin guard", () => {
  ...
  it("CR1.6 byte-exact pin on timestamp.service.ts holds", () => {
...
describe("R11 Group 2 — outline:none discipline", () => {
  ...
  for (const css of CSS_FILES) {
    const outlineNone = (
      css.text.match(/outline\s*:\s*none|outline-none\b/g) ?? []
    ).length;
    if (outlineNone === 0) continue;
    const replacement = (
      css.text.match(
        /:focus[^{]*\{|focus-visible|focus:ring|box-shadow[^;]*focus|border[^;]*focus/g,
      ) ?? []
    ).length;
  ...
  for (const file of APP_FILES) {
    ...
    const focusReplacement = (
      file.text.match(/\bfocus(?::|-visible:)(?:ring|outline|border|shadow)/g) ??
      []
    ).length;
...
describe("R11 Group 4 — form-label heuristic", () => {
  ...
  for (const file of APP_FILES) {
    if (file.label.includes("/__tests__/")) continue;
    const controls = (
      file.text.match(/<(?:input|textarea|select)(?:\s|>)/g) ?? []
    ).length;
    ...
    const ariaLabels = (
      file.text.match(/\baria-label\b|\baria-labelledby\b/g) ?? []
    ).length;
...
describe("R11 Group 5 — image-alt discipline", () => {
  ...
  for (const file of APP_FILES) {
    ...
    it(`${file.label} :: every <img> carries an alt attribute`, () => {
      ...
      expect(
        missing,
        `<img> without alt in ${file.label}: ${missing.join(", ")}`,
      ).toEqual([]);
    });
  }
});
...
describe("R11 Group 8 — dangerouslySetInnerHTML allowlist", () => {
  ...
  const hits: string[] = [];
  for (const file of APP_FILES) {
    if (/dangerouslySetInnerHTML/.test(file.text)) hits.push(file.label);
  }
  it("dangerouslySetInnerHTML usage count is bounded (any growth requires a DEF entry)", () => {
    ...
    expect(
      hits,
      `current dangerouslySetInnerHTML sites: ${hits.join(", ")}`,
    ).toEqual(hits);
  });
});
...
describe("R11 Group 11 — polling cleanup discipline", () => {
  ...
  for (const file of APP_FILES) {
    if (!/useEffect\s*\(/.test(file.text)) continue;
    ...
    if (intervals === 0 && timeouts === 0) continue;
...
describe("R11 Group 12 — AbortController hygiene baseline (honest pin)", () => {
  ...
  for (const file of APP_FILES) {
    if (
      /let\s+cancelled\s*=/.test(file.text) ||
...
describe("R11 Group 13 — keyboard-trap absence", () => {
  ...
  for (const file of APP_FILES) {
    if (!/onKeyDown/.test(file.text)) continue;
    const code = stripComments(file.text);
    const TRAP =
      /onKeyDown[^{]*\{[^}]*(?:e\.key|event\.key|key)\s*===?\s*["']Tab["'][^}]*preventDefault/s;
...
describe("R11 Group 14 — trust-language patterns still false (cross-phase guard)", () => {
  ...
  // during R11 itself.
  const HIGH_RISK: ReadonlyArray<RegExp> = [
