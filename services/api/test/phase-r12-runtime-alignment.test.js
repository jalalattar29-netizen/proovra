...
import { 
// Stage 3
DENIAL_REASONS, accessStateToDenialReason, accessGateKindToDenialReason, authorizationDenialCodeToDenialReason, anyDenialToCanonical, denialReasonHeadline, denialReasonGuidance, 
// Stage 4
TOM_PERSONA_KINDS, projectTomPersona, tomPersonaLabel, isSoloTomPersona, isOrgTomPersona, } from "@proovra/shared";
// =============================================================================
// Path helpers
// =============================================================================
...
describe("Phase R12 — Stage 2: canonical useActiveWorkspaceId", () => {
    const gateModule = read("apps/web/lib/platform-context/useTeamWorkspaceGate.ts");
    const tenantModule = read("apps/web/lib/platform-context/useTenantModel.ts");
    it("promotes useActiveWorkspaceId as PHASE 3 CANONICAL", () => {
        expect(gateModule).toMatch(/PHASE 3 CANONICAL/);
        expect(gateModule).toMatch(/export function useActiveWorkspaceId\(\): string \| null/);
    });
    it("marks useTeamId as @deprecated", () => {
        const block = gateModule.match(/@deprecated[\s\S]{0,1200}export function useTeamId/);
        expect(block).not.toBeNull();
    });
    it("marks useWorkspaceId as @deprecated", () => {
        const block = gateModule.match(/@deprecated[\s\S]{0,1200}export function useWorkspaceId/);
        expect(block).not.toBeNull();
    });
    it("marks useActiveSpaceId as @deprecated", () => {
        const block = tenantModule.match(/@deprecated[\s\S]{0,1200}export function useActiveSpaceId/);
        expect(block).not.toBeNull();
    });
});
// =============================================================================
...
describe("Phase R12 — Stage 3: canonical DenialReason vocabulary", () => {
    ...
    it("maps every AccessState to a DenialReason (or null for ALLOWED)", () => {
        ...
        expect(accessStateToDenialReason("DENIED_NO_CAPABILITY")).toBe("PERMISSION_REQUIRED");
        expect(accessStateToDenialReason("NEEDS_ORGANIZATION")).toBe("ORGANIZATION_REQUIRED");
        expect(accessStateToDenialReason("NEEDS_PERSONAL_OR_ORG")).toBe("WORKSPACE_REQUIRED");
        expect(accessStateToDenialReason("NEEDS_UPGRADE")).toBe("UPGRADE_REQUIRED");
        expect(accessStateToDenialReason("PLATFORM_ADMIN_ONLY")).toBe("PLATFORM_ADMIN_REQUIRED");
        expect(accessStateToDenialReason("RECOVERY_REQUIRED")).toBe("RECOVERY_REQUIRED");
    });
    it("maps AccessGateKind variants to the canonical reason", () => {
        expect(accessGateKindToDenialReason("PLAN_UPGRADE")).toBe("UPGRADE_REQUIRED");
        for (const k of [
            "ASK_ADMIN",
            "REQUEST_ACCESS",
            "CONTACT_OWNER",
        ...
        expect(accessGateKindToDenialReason("WORKSPACE_REQUIRED")).toBe("WORKSPACE_REQUIRED");
        expect(accessGateKindToDenialReason("FEATURE_UNAVAILABLE")).toBe("FEATURE_UNAVAILABLE");
    });
    it("maps every AuthorizationDenialCode to the canonical reason", () => {
        expect(authorizationDenialCodeToDenialReason("missing_actor")).toBe("AUTH_REQUIRED");
        expect(authorizationDenialCodeToDenialReason("no_actor")).toBe("AUTH_REQUIRED");
        expect(authorizationDenialCodeToDenialReason("missing_team_id")).toBe("WORKSPACE_REQUIRED");
        for (const c of [
            "member_not_active",
        ...
        for (const c of [
            ...
            expect(authorizationDenialCodeToDenialReason(c)).toBe("PERMISSION_REQUIRED");
        }
        expect(authorizationDenialCodeToDenialReason("authorization_unavailable")).toBe("OPERATIONAL_ERROR");
    });
    it("ensures personal users never get ORGANIZATION_REQUIRED for personal-safe routes", () => {
        ...
        expect(accessStateToDenialReason("NEEDS_PERSONAL_OR_ORG")).not.toBe("ORGANIZATION_REQUIRED");
        expect(accessStateToDenialReason("NEEDS_PERSONAL_OR_ORG")).toBe("WORKSPACE_REQUIRED");
        // Org-only pages still say organization required.
        expect(accessStateToDenialReason("NEEDS_ORGANIZATION")).toBe("ORGANIZATION_REQUIRED");
        // Permission denial is distinct from missing workspace.
        expect(accessStateToDenialReason("DENIED_NO_CAPABILITY")).not.toBe("WORKSPACE_REQUIRED");
        // Upgrade-required is distinct from permission-denied.
        expect(accessStateToDenialReason("NEEDS_UPGRADE")).not.toBe("PERMISSION_REQUIRED");
    });
    ...
    it("anyDenialToCanonical accepts inputs from any of the three vocabularies", () => {
        expect(anyDenialToCanonical("NEEDS_ORGANIZATION")).toBe("ORGANIZATION_REQUIRED");
...
describe("Phase R12 — Stage 4: canonical TOM persona projection", () => {
    ...
    it("projects personal vertical personas from the use-case profile", () => {
        ...
        expect(projectTomPersona({ ...base, useCaseProfile: "JOURNALIST" })).toBe("JOURNALIST");
        expect(projectTomPersona({ ...base, useCaseProfile: "LAWYER" })).toBe("LAWYER");
        expect(projectTomPersona({ ...base, useCaseProfile: "INVESTIGATOR" })).toBe("INVESTIGATOR");
        // Insurance is an investigator vertical.
        expect(projectTomPersona({ ...base, useCaseProfile: "INSURANCE" })).toBe("INVESTIGATOR");
        // Anything else → INDIVIDUAL.
        expect(projectTomPersona({ ...base, useCaseProfile: null })).toBe("INDIVIDUAL");
        expect(projectTomPersona({ ...base, useCaseProfile: "UNKNOWN" })).toBe("INDIVIDUAL");
    });
...
describe("Phase R12 — Stage 5: WorkspaceScope inline duplicates carry @deprecated", () => {
    ...
    for (const site of SITES) {
        it(`marks the inline WorkspaceScope in ${site} as @deprecated`, () => {
            ...
            const block = body.match(/@deprecated[\s\S]{0,800}export type WorkspaceScope = "PERSONAL" \| "TEAM"/);
            expect(block).not.toBeNull();
        });
    }
});
...
describe("Phase R12 — Stage 6: dead PERSONAL_*/ORG_* keys carry @deprecated", () => {
    ...
    it("flags every dead key with @deprecated in the API CAPABILITY_KEYS array", () => {
        const body = read("services/api/src/services/platform-context/types.ts");
        for (const key of DEAD_KEYS) {
            const pattern = new RegExp(`@deprecated PHASE 3[\\s\\S]{0,400}"${key}"`);
    ...
    it("retains the grants in capability-registry.ts (no behaviour change)", () => {
        const body = read("services/api/src/services/platform-context/capability-registry.ts");
...
describe("Phase R12 — Stage 7: canonical-workspace-resolver helpers exist", () => {
    const path = "services/api/src/services/access/canonical-workspace-resolver.ts";
    it("exposes resolveActiveOperationalWorkspace", () => {
        const body = read(path);
        expect(body).toMatch(/export async function resolveActiveOperationalWorkspace/);
...
describe("Phase R12 — shared package barrel exposes Phase 3 symbols", () => {
    ...
    it("places the new modules under packages/shared/src/architecture/", () => {
        expect(existsSync(join(sharedSrc, "architecture", "denial-vocabulary.ts"))).toBe(true);
        expect(existsSync(join(sharedSrc, "architecture", "canonical-persona.ts"))).toBe(true);
    });
});
// =============================================================================
...
describe("Phase R12 — Phase 3 constitutional non-negotiables", () => {
    ...
    it("does NOT rename teamId globally — resolver still returns teamId verbatim", () => {
        const body = read("services/api/src/services/access/canonical-workspace-resolver.ts");
