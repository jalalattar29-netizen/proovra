...
// =============================================================================
const PHASE2_TRUST_REL = "services/api/prisma/migrations/20270805000000_phase2_drift_repair_trust_status/migration.sql";
const PHASE2_GOV_REL = "services/api/prisma/migrations/20270806000000_phase2_drift_repair_governance/migration.sql";
const PHASE2_REDACTION_REL = "services/api/prisma/migrations/20270807000000_phase2_drift_repair_redaction/migration.sql";
const PHASE2_WEBHOOKS_REL = "services/api/prisma/migrations/20270808000000_phase2_drift_repair_lifecycle_webhooks/migration.sql";
const PHASE1_REL = "services/api/prisma/migrations/20270804000000_phase1_production_drift_stabilization/migration.sql";
const SENTRY_REL = "services/api/prisma/migrations/20270802000000_phase_sentry_batch_schema_drift_repair/migration.sql";
const GOV_REL = "services/api/prisma/migrations/20270803000000_phase_governance_additive_repair/migration.sql";
...
const PHASE2_DOC = tryReadRepo("docs/operations/phase2-full-prisma-drift-remediation.md");
// =============================================================================
// DDL-body helpers — strip `--` line comments so destructive-SQL guards run
// against EXECUTED SQL, not header text.
// =============================================================================
function ddlBody(migration) {
...
function mapTimestamptz(col, nullable = true) {
    ...
    return nullable
        ...
        : new RegExp(`DateTime\\s+(?:@default\\(now\\(\\)\\)|@updatedAt)\\s+@map\\("${col}"\\)\\s+@db\\.Timestamptz\\(6\\)`);
}
function mapUuid(col) {
    return new RegExp(`String\\?\\s+@map\\("${col}"\\)\\s+@db\\.Uuid`);
}
function mapVarchar(col, len) {
    return new RegExp(`String\\?\\s+@map\\("${col}"\\)\\s+@db\\.VarChar\\(${len}\\)`);
}
function mapBoolean(col) {
    return new RegExp(`Boolean\\s+@default\\(false\\)\\s+@map\\("${col}"\\)`);
}
function mapJson(col) {
...
describe("Phase 2 Drift Remediation — inventory + decision artifacts (GROUP A)", () => {
    ...
    it("A.2 — Manual decision table is non-empty (RedactionPolicyAudit table-name drift evidence intact)", () => {
        ...
        const phase3aTableCreate = readRepo("services/api/prisma/migrations/20261201000000_phase_3a_elite_closure_policy_video/migration.sql");
        ...
        expect(PHASE2_REDACTION_MIGRATION).toMatch(/redaction_policy_audit\b[\s\S]{0,200}redaction_policy_audits\b|redaction_policy_audits\b[\s\S]{0,200}redaction_policy_audit\b/);
    });
    it("A.3 — Allowlist permits all 4 Phase 2 migrations", () => {
        ...
        for (const { id } of PHASE2_MIGRATIONS) {
            expect(ALLOWLIST_GUARD, `PERMITTED_LATER_MIGRATIONS must include "${id}"`).toMatch(new RegExp(`"${id}"`));
        }
    });
});
// =============================================================================
    ...
    describe(`Phase 2 migration shape — ${m.id} (${m.domain}) (GROUP B)`, () => {
        ...
        it("B.2 — every non-empty non-comment statement terminates with ';'", () => {
            ...
            if (bodyOnly.length > 0) {
                expect(bodyOnly.endsWith(";"), `${m.id} DDL body must end with ';' (last 80 chars: ${bodyOnly.slice(-80).replace(/\n/g, "\\n")})`).toBe(true);
            }
        ...
        it("B.3 — every ADD COLUMN uses IF NOT EXISTS", () => {
            ...
            const addColGuarded = m.body.match(/\bADD\s+COLUMN\s+IF\s+NOT\s+EXISTS\b/gi) ?? [];
        ...
        it("B.4 — every CREATE INDEX uses IF NOT EXISTS", () => {
            ...
            const idxGuarded = m.body.match(/\bCREATE\s+(?:UNIQUE\s+)?INDEX\s+IF\s+NOT\s+EXISTS\b/gi) ??
                [];
        ...
        it("B.5 — every ALTER TABLE uses IF EXISTS", () => {
            ...
            const alterGuarded = m.body.match(/\bALTER\s+TABLE\s+IF\s+EXISTS\b/gi) ?? [];
        ...
        const colsForMigration = PHASE2_COLUMNS.filter((c) => c.migrationId === m.id);
        for (const c of colsForMigration) {
            it(`B.7.${c.table}.${c.column} — ADDs ${c.table}.${c.column} via ALTER TABLE IF EXISTS + ADD COLUMN IF NOT EXISTS + ${c.typeRe.source}`, () => {
                ...
                const re = new RegExp(`ALTER\\s+TABLE\\s+IF\\s+EXISTS\\s+"${c.table}"[\\s\\S]*?;`, "g");
                const matches = m.full.match(re) ?? [];
                expect(matches.length, `ALTER TABLE block for ${c.table} must exist in ${m.id}`).toBeGreaterThan(0);
                const joined = matches.join("\n");
                // Pin the column under IF NOT EXISTS + the type literal.
                const colRe = new RegExp(`ADD\\s+COLUMN\\s+IF\\s+NOT\\s+EXISTS\\s+"${c.column}"\\s+${c.typeRe.source}`);
...
describe("Phase 2 Drift Remediation — closure surface size (GROUP C)", () => {
    ...
    it("C.2 — every Phase 2 migration ID is referenced by at least one column pin", () => {
        ...
        for (const { id } of PHASE2_MIGRATIONS) {
            expect(referencedIds.has(id), `Migration ${id} should appear in PHASE2_COLUMNS`).toBe(true);
        }
    });
    it("C.3 — per-domain column counts match on-disk migrations (Trust=15, Gov=10, Redaction=19, Webhooks=5)", () => {
        ...
        expect(count("20270805000000_phase2_drift_repair_trust_status")).toBe(15);
        ...
        expect(count("20270808000000_phase2_drift_repair_lifecycle_webhooks")).toBe(5);
    });
    ...
    it("C.4 — on-disk ADD COLUMN count per migration matches column-pin count", () => {
        for (const m of PHASE2_MIGRATIONS) {
            const onDisk = (m.body.match(/\bADD\s+COLUMN\s+IF\s+NOT\s+EXISTS\b/gi) ?? []).length;
            const pinned = PHASE2_COLUMNS.filter((c) => c.migrationId === m.id).length;
            expect(onDisk, `Migration ${m.id} has ${onDisk} ADD COLUMN statements but ${pinned} column pins enumerated`).toBe(pinned);
        }
    });
});
// =============================================================================
...
describe("Phase 2 Drift Remediation — destructive-SQL guards across 7 closure migrations (GROUP D)", () => {
    for (const m of ALL_CLOSURE_MIGRATIONS) {
        ...
        it(`D.2 [${m.id}] — no ALTER COLUMN DROP NOT NULL`, () => {
            ...
            expect(m.body).not.toMatch(/\bALTER\s+COLUMN\b[\s\S]{0,80}DROP\s+NOT\s+NULL/i);
        });
...
describe("Phase 2 Drift Remediation — Prisma field pins (GROUP D)", () => {
    for (const c of PHASE2_COLUMNS) {
        it(`D-prisma.${c.model}.${c.column} — Prisma model ${c.model} declares column ${c.column}`, () => {
            ...
            expect(block[0], `Prisma model ${c.model} must declare column "${c.column}" matching ${c.mapRe.source}`).toMatch(c.mapRe);
        });
    }
});
// =============================================================================
...
describe("Phase 2 Drift Remediation — central handler sanity (GROUP E)", () => {
    ...
    it("E.2 — no new route file added during Phase 2 (routes count matches baseline)", () => {
        const routesDir = fileURLToPath(new URL("../../../services/api/src/routes", import.meta.url));
...
describe("Phase 2 Drift Remediation — no new v2 systems (GROUP F)", () => {
    it("F.1 — no *v2* route in services/api/src/routes and no *v2* page in apps/web/app", () => {
        ...
        const webAppDir = fileURLToPath(new URL("../../../apps/web/app", import.meta.url));
...
describe("Phase 2 Drift Remediation — no schema downgrade (GROUP G)", () => {
    for (const { model, tableMap } of PHASE2_MODEL_PINS) {
        it(`G.${model} — Prisma model ${model} present and @@map("${tableMap}") intact`, () => {
            ...
            const block = SCHEMA_PRISMA.match(modelRe);
            expect(block, `Prisma model ${model} must exist in schema.prisma`).toBeTruthy();
            expect(block[0], `${model} must @@map("${tableMap}")`).toMatch(new RegExp(`@@map\\("${tableMap}"\\)`));
        });
    }
});
