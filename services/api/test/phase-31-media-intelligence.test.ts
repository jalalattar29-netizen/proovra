...
import {
  computeClientServerGapSeconds,
  detectMimeExtensionMismatch,
  isScreenshotLikeFilename,
} from "../src/services/media-intelligence/analyzer.service.js";
...
describe("Phase 31 — media intelligence SQL drift patch", () => {
  ...
  it("CREATE TABLE IF NOT EXISTS (idempotent)", () => {
    expect(sql).toMatch(
      /CREATE TABLE IF NOT EXISTS\s+"media_intelligence_signals"/,
    );
  });
  ...
  it("signal_type CHECK constraint enumerates every catalog value (across all drift patches)", () => {
    ...
    for (const type of MEDIA_INTELLIGENCE_SIGNAL_TYPES) {
      expect(combined, `signal_type ${type} missing from any drift patch`).toMatch(
        new RegExp(`'${type}'`),
      );
    }
  });
  ...
  it("unique index on (evidence_id, COALESCE(material_id, sentinel), signal_type) for idempotent upsert", () => {
    expect(sql).toMatch(
      /CREATE UNIQUE INDEX IF NOT EXISTS "media_intelligence_signals_evidence_material_type_uk"[\s\S]*?COALESCE\("material_id",\s*'00000000-0000-0000-0000-000000000000'::uuid\),\s*"signal_type"/,
    );
  });
...
describe("Phase 31 — bounded catalogs", () => {
  it("signal types: 16 entries from the brief", () => {
    for (const expected of [
      ...
      expect(MEDIA_INTELLIGENCE_SIGNAL_TYPES).toContain(expected as never);
    }
  });
  ...
  it("display labels never use forbidden truth-claim wording", () => {
    ...
    for (const type of MEDIA_INTELLIGENCE_SIGNAL_TYPES) {
      ...
      expect(label, `label "${label}" uses forbidden wording`).not.toMatch(
        forbidden,
      );
    }
  });
...
describe("Phase 31 — safe-wording library", () => {
  ...
  it("OCR_AVAILABLE / TRANSCRIPT_AVAILABLE are neutral availability statements", () => {
    for (const input of [
      { type: "OCR_AVAILABLE" as const },
      { type: "TRANSCRIPT_AVAILABLE" as const },
    ]) {
...
describe("Phase 31 — pure heuristics", () => {
  describe("detectMimeExtensionMismatch", () => {
    ...
    it("returns null when extension is unknown (no signal — we can't say)", () => {
      expect(
        detectMimeExtensionMismatch("photo.xyz", "image/jpeg"),
      ).toBeNull();
    });

    it("returns null when families align (jpg ↔ image/jpeg, mp4 ↔ video/mp4)", () => {
      expect(
        detectMimeExtensionMismatch("photo.jpg", "image/jpeg"),
      ).toBeNull();
      expect(
        detectMimeExtensionMismatch("clip.mp4", "video/mp4"),
      ).toBeNull();
      expect(
        detectMimeExtensionMismatch("doc.pdf", "application/pdf"),
      ).toBeNull();
    });
    ...
    it("handles MIME parameter (e.g. 'image/jpeg; charset=utf-8')", () => {
      expect(
        detectMimeExtensionMismatch("photo.jpg", "image/jpeg; charset=utf-8"),
      ).toBeNull();
    });
  ...
  describe("isScreenshotLikeFilename", () => {
    ...
    it("handles paths (basename only)", () => {
      expect(
        isScreenshotLikeFilename("/Users/x/Pictures/Screen Shot 2026-05-20.png"),
      ).toBe(true);
      expect(
        isScreenshotLikeFilename("/Users/x/Pictures/vacation.jpg"),
      ).toBe(false);
    });
  ...
  describe("computeClientServerGapSeconds", () => {
    it("returns null when either side is null/missing", () => {
      ...
      expect(
        computeClientServerGapSeconds("2026-05-20T00:00:00Z", null),
      ).toBeNull();
      expect(
        computeClientServerGapSeconds("not-a-date", new Date()),
      ).toBeNull();
    });
...
describe("Phase 31 — analyzer source contract", () => {
  ...
  it("NEVER throws — always returns AnalyzerResult", () => {
    ...
    expect(src).toMatch(
      /export async function runMediaIntelligenceAnalysis[\s\S]*?try\s*\{[\s\S]*?\}\s*catch\s*\{[\s\S]*?return\s*\{[\s\S]*?ok:\s*false,\s*reason:\s*"service_unavailable"/,
    );
  });
  ...
  it("NEVER uses forbidden wording verbs in source strings", () => {
    ...
    for (const lit of stringLiterals) {
      expect(lit, `analyzer source string uses forbidden wording: ${lit}`).not.toMatch(
        forbidden,
      );
    }
  });
...
describe("Phase 31 — route source contract", () => {
  ...
  it("routes registered: GET signals + POST run + Phase 31.5 action", () => {
    expect(src).toMatch(
      /app\.get\(\s*"\/v1\/evidence\/:evidenceId\/media-intelligence"/,
    );
    expect(src).toMatch(
      /app\.post\(\s*"\/v1\/evidence\/:evidenceId\/media-intelligence\/run"/,
    );
  ...
  it("anti-enumeration: cross-team evidence returns 404 not_found", () => {
    expect(src).toMatch(
      /evidence\.teamId !== teamId[\s\S]*?reply\.code\(404\)\.send\(\{\s*error:\s*\{\s*code:\s*"not_found"\s*\}/,
    );
  });
  ...
  it("ORDER BY surfaces ATTENTION first (operator triage)", () => {
    expect(src).toMatch(
      /ORDER BY[\s\S]*?CASE\s*"severity"\s*WHEN 'ATTENTION' THEN 0/,
    );
  });
...
describe("Phase 31 — observability", () => {
  ...
  it("schema-validation registers the new table + indices at IMPORTANT severity (not CRITICAL — never blocks boot)", () => {
    ...
    expect(src).toMatch(
      /name:\s*"media_intelligence_signals",\s*severity:\s*"important"/,
    );
    expect(src).toMatch(
      /indexName:\s*"media_intelligence_signals_evidence_material_type_uk"/,
    );
  });
