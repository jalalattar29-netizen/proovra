...
// =============================================================================
// Drift map (preflight-derived). Every entry MUST appear in the migration
// as `ALTER TABLE IF EXISTS "<table>" ADD COLUMN IF NOT EXISTS "<col>"`.
// =============================================================================
...
const DRIFT_COLS: ReadonlyArray<DriftCol> = [
...
interface DriftIndex {
  ...
  readonly table: string;
  /** Regex source matching the column list ordering. */
  readonly colsRe: RegExp;
  readonly unique?: boolean;
}

const DRIFT_INDEXES: ReadonlyArray<DriftIndex> = [
  {
...
// Each of the 4 tables: pins that the Prisma model still declares every
// missing column (proves the migration brings DB up to schema, not the
// other way around).
interface SchemaPin {
  readonly model: string;
  readonly column: string;
  /** Regex matching the Prisma field declaration body. */
  ...
  for (const { table, column, typeRe } of DRIFT_COLS) {
    it(`A.2 — adds ${table}.${column} via ALTER TABLE IF EXISTS + ADD COLUMN IF NOT EXISTS`, () => {
      ...
      const blockRe = new RegExp(
        `ALTER\\s+TABLE\\s+IF\\s+EXISTS\\s+"${table}"[\\s\\S]{0,1200}?;`,
      );
  ...
  for (const { name, table, colsRe, unique } of DRIFT_INDEXES) {
    it(`A.3 — creates index ${name} on ${table}(${colsRe.source}) with IF NOT EXISTS`, () => {
      ...
      const idxRe = new RegExp(
        `CREATE\\s+(?:UNIQUE\\s+)?INDEX\\s+IF\\s+NOT\\s+EXISTS\\s+"${name}"[\\s\\S]{0,300}ON\\s+"${table}"\\s*\\(${colsRe.source}\\)`,
      );
