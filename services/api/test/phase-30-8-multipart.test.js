...
import { STORAGE_MULTIPART_LIFECYCLE_DENIAL_CODES, } from "../src/services/uploads/upload-session.service.js";
import { STORAGE_MULTIPART_DENIAL_CODES, buildMultipartStorageKey, } from "../src/services/uploads/storage-multipart.js";
...
describe("Phase 30.8 — multipart SQL drift patch", () => {
    const sql = readSource("../../../services/api/sql/drift-patches/2026-05-19-evidence-upload-multipart.sql");
    ...
    it("adds multipart bookkeeping columns to evidence_upload_sessions (idempotent)", () => {
        for (const col of [
            ...
            expect(sql).toMatch(new RegExp(`ADD COLUMN IF NOT EXISTS\\s+"${col}"`, "i"));
        }
    });
    ...
    it("creates unique (team_id, multipart_upload_id) index — prevents double-booking", () => {
        expect(sql).toMatch(/CREATE UNIQUE INDEX IF NOT EXISTS "evidence_upload_sessions_multipart_uk"[\s\S]*?\("team_id", "multipart_upload_id"\)[\s\S]*?WHERE "multipart_upload_id" IS NOT NULL/i);
    });
    it("creates reaper index targeting only live unresolved multipart sessions", () => {
        expect(sql).toMatch(/CREATE INDEX IF NOT EXISTS "evidence_upload_sessions_multipart_reaper_idx"[\s\S]*?WHERE "multipart_upload_id" IS NOT NULL[\s\S]*?"completed_at_storage_utc" IS NULL[\s\S]*?"aborted_at_storage_utc" IS NULL/i);
    });
    it("uses DO blocks for constraint existence checks (idempotent)", () => {
        expect(sql).toMatch(/DO \$\$[\s\S]*?SELECT 1 FROM pg_constraint[\s\S]*?conname = 'evidence_upload_sessions_completed_size_nonneg'/);
    });
...
describe("Phase 30.8 — storage-multipart abstraction", () => {
    const src = readSource("../../../services/api/src/services/uploads/storage-multipart.ts");
    ...
    it("canonical key builder refuses non-UUID inputs (anti-path-traversal)", () => {
        expect(src).toMatch(/uuidRegex[\s\S]*?test\(input\.evidenceId\)[\s\S]*?test\(input\.sessionId\)/);
        // Behavioral assertion: a bad input throws.
        expect(() => buildMultipartStorageKey({
    ...
    it("canonical key has the expected stable shape", () => {
        ...
        expect(buildMultipartStorageKey({ evidenceId, sessionId })).toBe(`evidence/${evidenceId}/multipart/${sessionId}/object`);
    });
    ...
    it("ETag is never assigned to a SHA-256 variable name", () => {
        ...
        for (const line of lines) {
            if (/sha256|Sha256|SHA256/.test(line) && /etag|ETag/i.test(line)) {
                ...
                expect(line, `mixed sha256+etag line: ${line}`).not.toMatch(/\b(sha256|serverSha256|clientSha256)\s*[:=]\s*.*[Ee][Tt]ag/);
            }
        }
    });
    it("Object Lock defaults are attached at CreateMultipartUpload time (NOT at presign)", () => {
        expect(src).toMatch(/CreateMultipartUploadCommand\(\{[\s\S]*?\.\.\.objectLock[\s\S]*?\}\)/);
        // UploadPartCommand must NOT carry Object Lock fields — those
        // belong on the create, not the part.
        expect(src).not.toMatch(/UploadPartCommand\(\{[\s\S]*?ObjectLockMode/);
    });
    ...
    it("abort treats NoSuchUpload as idempotent success", () => {
        expect(src).toMatch(/name === "NoSuchUpload"[\s\S]*?ok:\s*true,\s*alreadyAbsent:\s*true/);
    });
    it("complete returns multipart_not_found on NoSuchUpload (retriable signal)", () => {
        expect(src).toMatch(/completeMultipartUpload[\s\S]*?name === "NoSuchUpload"[\s\S]*?reason:\s*"multipart_not_found"/);
    });
    it("verifyCompletedObject streams through SHA-256 + compares case-insensitively to expected", () => {
        ...
        expect(src).toMatch(/expectedSha256\.toLowerCase\(\)\s*!==\s*serverSha256\.toLowerCase\(\)/);
    });
    ...
    it("complete sorts parts in ascending order (S3 requirement)", () => {
        expect(src).toMatch(/\.sort\(\(a, b\)\s*=>\s*a\.partNumber\s*-\s*b\.partNumber\)/);
    });
});
// =============================================================================
...
describe("Phase 30.8 — upload-session multipart lifecycle", () => {
    const src = readSource("../../../services/api/src/services/uploads/upload-session.service.ts");
    ...
    it("denial catalog includes both session + storage codes", () => {
        expect(STORAGE_MULTIPART_LIFECYCLE_DENIAL_CODES).toContain("session_not_found");
        expect(STORAGE_MULTIPART_LIFECYCLE_DENIAL_CODES).toContain("multipart_already_initiated");
        expect(STORAGE_MULTIPART_LIFECYCLE_DENIAL_CODES).toContain("multipart_not_initiated");
        expect(STORAGE_MULTIPART_LIFECYCLE_DENIAL_CODES).toContain("missing_part_etag");
    });
    it("initiateStorageMultipart is idempotent for sessions that already have an upload_id", () => {
        expect(src).toMatch(/if\s*\(session\.multipart_upload_id\)\s*\{[\s\S]*?storageBucket:\s*session\.storage_bucket/);
    });
    it("initiateStorageMultipart cleans up S3 on DB persist failure", () => {
        ...
        expect(src).toMatch(/catch\s*\{[\s\S]*?await abortMultipartUpload\([\s\S]*?multipartUploadId:\s*storage\.multipartUploadId/);
    });
    it("presign refuses if the session is not UPLOADING (no presigns from terminal sessions)", () => {
        expect(src).toMatch(/presignStorageUploadPart[\s\S]*?session\.state !== "UPLOADING"[\s\S]*?session_already_terminal/);
    });
    it("presign refuses if multipart not initiated yet", () => {
        expect(src).toMatch(/presignStorageUploadPart[\s\S]*?!session\.multipart_upload_id[\s\S]*?multipart_not_initiated/);
    });
    it("recordPartEtag rejects empty etag (missing_part_etag)", () => {
        expect(src).toMatch(/recordPartEtag[\s\S]*?!input\.etag\.trim\(\)[\s\S]*?missing_part_etag/);
    });
    it("completeStorageMultipart refuses when ANY part is missing an ETag", () => {
        expect(src).toMatch(/missing = parts[\s\S]*?\.filter\(\(p\)\s*=>\s*!p\.part_etag/);
        expect(src).toMatch(/missing\.length > 0[\s\S]*?reason:\s*"missing_part_etag"[\s\S]*?missingEtags:\s*missing/);
    });
    it("completeStorageMultipart refuses on terminal session states", () => {
        expect(src).toMatch(/completeStorageMultipart[\s\S]*?session\.state === "COMPLETED"[\s\S]*?session_already_terminal/);
    });
    it("hash mismatch in completeStorageMultipart flips session to FAILED + emits SecurityEvent", () => {
        expect(src).toMatch(/verify\.reason === "hash_mismatch"[\s\S]*?eventType:\s*"multipart_hash_mismatch"[\s\S]*?SET "state" = 'FAILED'/);
    });
    it("abortStorageMultipart refuses to abort already-COMPLETED sessions", () => {
        expect(src).toMatch(/abortStorageMultipart[\s\S]*?session\.state === "COMPLETED"[\s\S]*?session_already_completed/);
    });
    it("abortStorageMultipart uses server-side session row for bucket/key/uploadId (no client trust)", () => {
        expect(src).toMatch(/abortStorageMultipart[\s\S]*?bucket:\s*session\.storage_bucket[\s\S]*?key:\s*session\.storage_key[\s\S]*?multipartUploadId:\s*session\.multipart_upload_id/);
    });
    it("reapStaleMultipartUploads is bounded to 100 rows + idempotent", () => {
        ...
        expect(src).toMatch(/reapStaleMultipartUploads[\s\S]*?if\s*\(!result\.ok\)\s*\{[\s\S]*?failed \+= 1[\s\S]*?continue/);
    });
    it("every multipart query is team-anchored (no cross-workspace reads)", () => {
        ...
        for (const u of updates) {
            ...
            expect(u, `UPDATE missing team_id: ${u.slice(0, 120)}…`).toMatch(/"team_id"\s*=\s*\$/);
        }
    });
...
describe("Phase 30.8 — multipart web routes", () => {
    const src = readSource("../../../services/api/src/routes/upload-sessions.routes.ts");
    ...
    it("every multipart route uses authorizeOrFail with antiEnumeration: true + evidence.create", () => {
        for (const p of [
            ...
            expect(block, `${p} missing authorizeOrFail`).toMatch(/await authorizeOrFail\(/);
            expect(block, `${p} missing antiEnumeration`).toMatch(/antiEnumeration:\s*true/);
            expect(block, `${p} missing evidence.create permission`).toMatch(/permission:\s*"evidence\.create"/);
        }
    });
    ...
    it("complete surfaces missingPartIndices on missing_part_etag for actionable UI", () => {
        expect(src).toMatch(/missingPartIndices:\s*result\.missingEtags/);
    });
    ...
    it("the routes file never projects the storage column / S3 upload id in code", () => {
        ...
        const noComments = src
            .replace(/\/\*[\s\S]*?\*\//g, "")
            .replace(/\/\/[^\n]*/g, "");
        const body = noComments.slice(noComments.indexOf("export async function uploadSessionsRoutes"));
...
describe("Phase 30.8 — multipart API-key routes", () => {
    const src = readSource("../../../services/api/src/routes/integrations-uploads.routes.ts");
    ...
    const apiMultipartSectionStart = src.indexOf("Phase 30.8 — API-key S3 NATIVE MULTIPART ROUTES");
    ...
    it("every multipart route requires requireApiKey + integration.evidence.upload scope", () => {
        ...
        const scopeCalls = block.match(/requireApiScope\(req,\s*reply,\s*"integration\.evidence\.upload"\)/g) ?? [];
        expect(scopeCalls.length).toBe(4);
    });
    it("teamId always sourced from cred.teamId — never body / query / path", () => {
        const block = src.slice(apiMultipartSectionStart);
        const serviceCalls = block.match(/(initiateStorageMultipart|presignStorageUploadPart|completeStorageMultipart|abortStorageMultipart)\(\{[\s\S]*?\}\)/g) ?? [];
    ...
    it("API responses NEVER project storage_bucket / storage_key / multipart_upload_id", () => {
        ...
        for (const banned of [
            ...
            expect(block, `API multipart routes leak ${banned}`).not.toContain(banned);
        }
    });
...
describe("Phase 30.8 — custody / integrity invariants", () => {
    const serviceSrc = readSource("../../../services/api/src/services/uploads/upload-session.service.ts");
    const routesSrc = readSource("../../../services/api/src/routes/upload-sessions.routes.ts");
    const apiRoutesSrc = readSource("../../../services/api/src/routes/integrations-uploads.routes.ts");
    const storageSrc = readSource("../../../services/api/src/services/uploads/storage-multipart.ts");
    it("multipart routes NEVER emit custody events directly", () => {
        for (const [name, src] of [
            ...
            expect(src, `${name} references CustodyEvent`).not.toMatch(/CustodyEventType/);
        }
    });
    it("multipart routes NEVER write the authoritative Evidence.uploadedAt field", () => {
        ...
        for (const [name, src] of [
            ...
            expect(src, `${name} writes Evidence uploadedAtUtc`).not.toMatch(/uploadedAtUtc\s*:\s*now/);
            // Also never directly write an evidence.uploadedAtUtc.
            expect(src, `${name} mutates Evidence.uploadedAtUtc`).not.toMatch(/evidence\.uploadedAtUtc/i);
        }
    });
    it("completeStorageMultipart does NOT flip session.state to COMPLETED (gate stays authoritative)", () => {
        ...
        const completeBlock = serviceSrc.match(/completeStorageMultipart\([\s\S]*?^export /m)?.[0];
        expect(completeBlock).toBeTruthy();
        // No SET "state" = 'COMPLETED' anywhere in this block.
        expect(completeBlock).not.toMatch(/SET\s+"state"\s*=\s*'COMPLETED'/);
    });
    it("ETag is stored as storage metadata only — never compared as hash", () => {
        ...
        expect(storageSrc).not.toMatch(/sha256\w*\s*===\s*\w*[Ee][Tt]ag/);
        expect(storageSrc).not.toMatch(/\w*[Ee][Tt]ag\s*===\s*sha256/);
        // ETag is also never written into a server_sha256 column.
        expect(serviceSrc).not.toMatch(/"server_sha256"\s*=[^,;]*etag/i);
    });
    it("expected_sha256 is compared via verifyCompletedObject → sha256HexFromStream (NOT ETag)", () => {
        expect(storageSrc).toMatch(/verifyCompletedObject[\s\S]*?sha256HexFromStream/);
    });
    it("hash mismatch refuses to mark VERIFIED + flips session to FAILED", () => {
        expect(serviceSrc).toMatch(/verify\.reason === "hash_mismatch"[\s\S]*?SET "state" = 'FAILED'/);
    });
    ...
    it("missing_part_etag refuses to call S3 CompleteMultipartUpload (no completion on partial state)", () => {
        expect(serviceSrc).toMatch(/missing\.length > 0[\s\S]*?reason:\s*"missing_part_etag"/);
...
describe("Phase 30.8 — observability catalogs", () => {
    ...
    it("runtime-readiness registers multipart_storage subsystem", () => {
        ...
        expect(src).toMatch(/checkMultipartStorage\([\s\S]*?\)/);
        // The aggregator must invoke it.
        expect(src).toMatch(/Promise\.all\(\[[\s\S]*?checkMultipartStorage\(prisma\)/);
    });
    it("multipart_storage check reports CRITICAL when S3_BUCKET is missing", () => {
        const src = readSource("../../../services/api/src/runtime/runtime-readiness.ts");
        expect(src).toMatch(/!envPresent\("S3_BUCKET"\)[\s\S]*?status:\s*"CRITICAL"[\s\S]*?reasonCode:\s*"s3_bucket_missing"/);
    });
    it("schema validation registers the new critical multipart columns", () => {
        ...
        for (const col of [
            ...
            expect(src, `column ${col} not registered`).toMatch(new RegExp(`column:\\s*"${col}",\\s*severity:\\s*"critical"`));
        }
        expect(src).toMatch(/indexName:\s*"evidence_upload_sessions_multipart_uk"/);
    });
});
// =============================================================================
...
describe("Phase 30.8 — reaper wired into reconcile", () => {
    const opsSrc = readSource("../../../services/api/src/routes/ops.routes.ts");
    ...
    it("reaper failure does NOT crash the reconcile (best-effort)", () => {
        expect(opsSrc).toMatch(/reapStaleMultipartUploads[\s\S]*?\}\s*catch\s*\{[\s\S]*?\}/);
    });
    it("reaper publishes scanned / aborted / failed gauges", () => {
        expect(opsSrc).toMatch(/setGauge\(\s*"multipart_stale_scanned",/);
        expect(opsSrc).toMatch(/setGauge\(\s*"multipart_stale_aborted",/);
        expect(opsSrc).toMatch(/setGauge\(\s*"multipart_stale_failed",/);
    });
});
// =============================================================================
...
describe("Phase 30.8 — type-level invariants", () => {
    ...
    it("buildMultipartStorageKey rejects empty / non-UUID inputs", () => {
        expect(() => buildMultipartStorageKey({ evidenceId: "", sessionId: "" })).toThrow();
        expect(() => buildMultipartStorageKey({
