...
import { AUTHORITY_SCHEMA_VERSION, CAPABILITY_KEYS, CAPABILITY_SCHEMA_VERSION, } from "../src/services/platform-context/types.js";
...
const WEB_TEAMS_HOME = readWeb("components/workspace-admin/WorkspaceAdministrationHome.tsx");
// =============================================================================
// PART 1 — Types
// =============================================================================
describe("ENTERPRISE TENANT MODEL — types", () => {
    ...
    it("envelope declares account / personalSpace / organizations / activeSpace / duplicatePersonalCandidates fields", () => {
        ...
        expect(TYPES).toMatch(/\borganizations:\s*ReadonlyArray<PlatformContextOrganization>/);
        expect(TYPES).toMatch(/\bactiveSpace:\s*PlatformContextActiveSpace\b/);
        expect(TYPES).toMatch(/\bduplicatePersonalCandidates:\s*ReadonlyArray<PlatformContextDuplicatePersonalCandidate>/);
    });
    ...
    it("new canonical capability keys are part of CAPABILITY_KEYS", () => {
        ...
        for (const key of required) {
            expect(CAPABILITY_KEYS.includes(key), `CAPABILITY_KEYS must include ${key}`).toBe(true);
        }
    });
...
describe("ENTERPRISE TENANT MODEL — platform-context service", () => {
    ...
    it("emits PersonalSpace with the canonical 'Personal Space' label", () => {
        expect(SVC).toMatch(/personalSpace:\s*PlatformContextPersonalSpace\s*=[\s\S]{0,400}label:\s*"Personal Space"/);
    });
    ...
    it("detects duplicate personal-like rows via bounded heuristic", () => {
        expect(SVC).toMatch(/duplicatePersonalCandidates:\s*PlatformContextDuplicatePersonalCandidate\[\]/);
...
describe("ENTERPRISE TENANT MODEL — frontend types + hooks", () => {
    ...
    it("frontend types declare account/personalSpace/organizations/activeSpace on the envelope", () => {
        ...
        expect(WEB_TYPES).toMatch(/organizations\?:\s*ReadonlyArray<PlatformContextOrganization>/);
