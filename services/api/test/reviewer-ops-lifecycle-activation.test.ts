...
import {
  ...
  type SubsystemId,
} from "../src/runtime/schema-validation.js";
...
describe("Reviewer Ops activation [schema-validation] — EXPECTED_SCHEMA catalog", () => {
  function subsystemOf(o: ExpectedSchemaObject): SubsystemId {
    return o.subsystem;
  }

  it("includes every reviewer-ops critical table", () => {
    const tables = EXPECTED_SCHEMA.filter(
      (o) => o.kind === "table" && subsystemOf(o) === "reviewer_ops",
  ...
  it("flags reviewer-ops core tables as critical (not important)", () => {
    const critical = EXPECTED_SCHEMA.filter(
      (o) =>
        subsystemOf(o) === "reviewer_ops" &&
        o.severity === "critical" &&
        o.kind === "table",
    ).map((o) => (o as { name: string }).name);
  ...
  it("includes the Phase 25 additive columns on evidence_review_workflows", () => {
    const columns = EXPECTED_SCHEMA.filter(
      (o) =>
        o.kind === "column" &&
        (o as { table: string }).table === "evidence_review_workflows",
    ).map((o) => (o as { column: string }).column);
  ...
  it("includes the unique escalation fingerprint index (dedupe is critical)", () => {
    const indexes = EXPECTED_SCHEMA.filter((o) => o.kind === "index").map(
      (o) => (o as { indexName: string }).indexName,
    );
...
describe("Reviewer Ops activation [schema-status route]", () => {
  ...
  it("imports runSchemaValidation from the runtime module", () => {
    ...
    expect(src).toContain(
      'from "../runtime/schema-validation.js"',
    );
...
describe("Reviewer Ops activation [worker tick]", () => {
  it("worker module exports runReviewerReconciliation", () => {
    const src = readSource(
      "../../worker/src/reviewer-ops/reviewer-reconciliation.worker.ts",
    );
...
describe("Reviewer Ops activation [worker scheduler wiring]", () => {
  it("worker index.ts imports + starts + stops the reviewer reconciliation scheduler", () => {
    ...
    expect(src).toContain(
      './reviewer-ops/reviewer-reconciliation.worker.js',
    );
  ...
  it("the start hook is invoked AFTER the governance schedulers (ordering)", () => {
    const src = readSource("../../worker/src/index.ts");
    const startSection = src.slice(
      src.lastIndexOf("startCaptureDraftReaperScheduler();"),
      src.lastIndexOf("startObservabilityHeartbeat"),
    );
    expect(startSection).toMatch(
      /startImmutableStorageReconciliationScheduler\(\);[\s\S]*?startReviewerReconciliationScheduler\(\);/,
    );
  });
});
...
describe("Reviewer Ops activation [escalation storm]", () => {
  it("runReconcile records a GOVERNANCE incident when escalationsCreated ≥ threshold", () => {
    ...
    expect(src).toMatch(
      /recordIncident\([\s\S]*?category:\s*"GOVERNANCE"[\s\S]*?severity:\s*"HIGH"[\s\S]*?fingerprint:\s*`reviewer:escalation_storm/,
    );
