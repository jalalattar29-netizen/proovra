...
describe("Phase P2.2 — WORM Export Frontend", () => {
    it("/operations/exports page exists", () => {
        expect(exists("../../../apps/web/app/(app)/operations/exports/page.tsx")).toBe(true);
    });
    it("calls the P2.1 backend endpoints", () => {
        const p = readSource("../../../apps/web/app/(app)/operations/exports/page.tsx");
    ...
    it("immutable badge is gated on platformMode==='verified'", () => {
        ...
        expect(p).toMatch(/const\s+immutable\s*=\s*[\s\S]{0,80}objectLockMode\s*===\s*"verified"/);
...
describe("Phase P2.3 — Queue Operations Backend", () => {
    it("replay safety matrix exposes the bounded ReplayCategory enum", () => {
        const src = readSource("../src/services/operations/queue-replay-safety.service.ts");
    ...
    it("replay routes route through requireStepUpForSensitiveAction with QUEUE_JOB_REPLAY", () => {
        const r = readSource("../src/routes/operations-queues.routes.ts");
        expect(r).toMatch(/requireStepUpForSensitiveAction[\s\S]{0,300}purpose:\s*"QUEUE_JOB_REPLAY"/);
    });
    ...
    it("queue inventory service sanitises stack traces", () => {
        const src = readSource("../src/services/operations/queue-inventory.service.ts");
        expect(src).toContain("sanitiseStack");
        expect(src).toContain("<path>");
    });
    it("forbidden job replay returns bounded error code, emits audit", () => {
        const src = readSource("../src/services/operations/queue-replay-action.service.ts");
...
describe("Phase P2.4 — Queue Operations Frontend", () => {
    it("/operations/queues page exists and references the matrix endpoint", () => {
        const p = readSource("../../../apps/web/app/(app)/operations/queues/page.tsx");
    ...
    it("hides the replay button for forbidden / unknown categories", () => {
        ...
        expect(p).toMatch(/cat\s*===\s*"forbidden"\s*\|\|\s*cat\s*===\s*"unknown"/);
...
describe("Phase P2.5 — DR / Recovery Backend", () => {
    it("recovery validation service exists with the bounded outcome enum", () => {
        const src = readSource("../src/services/operations/recovery-validation.service.ts");
    ...
    it("restore validation route is step-up gated via RESTORE_VALIDATION_EXECUTE", () => {
        const r = readSource("../src/routes/operations-recovery.routes.ts");
        expect(r).toMatch(/requireStepUpForSensitiveAction[\s\S]{0,300}purpose:\s*"RESTORE_VALIDATION_EXECUTE"/);
    });
...
describe("Phase P2.6 — DR / Recovery Frontend", () => {
    it("/operations/recovery page exists", () => {
        expect(exists("../../../apps/web/app/(app)/operations/recovery/page.tsx")).toBe(true);
    });
    it("calls the P2.5 backend endpoints", () => {
        const p = readSource("../../../apps/web/app/(app)/operations/recovery/page.tsx");
