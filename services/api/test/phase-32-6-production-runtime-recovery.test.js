...
describe("Phase 32.6 — workers readiness startup grace period", () => {
    ...
    it("checkWorkers reads process uptime to compute the grace window", () => {
        expect(SRC).toMatch(/apiUptimeMs = Math\.round\(process\.uptime\(\) \* 1000\)/);
...
describe("Phase 32.6 — buildVerificationPackageAvailability fix", () => {
    const HELPERS_SRC = readSource("../../../apps/web/app/(app)/evidence/lib/evidence-library-helpers.ts");
    it("availability NO LONGER requires `detail.verificationPackage.url`", () => {
        ...
        const helperBody = code.slice(code.indexOf("export function buildVerificationPackageAvailability"), code.indexOf("export function hasPublicVerification"));
    ...
    it("availability now checks `generatedAtUtc` OR `version` (presence signals)", () => {
        expect(HELPERS_SRC).toMatch(/detail\?\.verificationPackage\?\.generatedAtUtc \|\|\s*detail\?\.verificationPackage\?\.version/);
    });
...
describe("Phase 32.6 — bounded observability counters", () => {
    ...
    it("registers all 8 brief-requested counters", () => {
        ...
        for (const name of required) {
            expect(METRICS_SRC, `metric ${name} must be registered`).toMatch(new RegExp(`"${name}"`));
        }
    });
    ...
    it("Phase 32.6.6 — personal-workspace skip site is retired (counter retained for catalog stability)", () => {
        ...
        expect(PROC_SRC).not.toMatch(/bump\("package_generation_skipped_personal_workspace_total"\)/);
    ...
    it("artifact_status_polled_total bumped from the artifact-status route (NOT report/latest)", () => {
        ...
        // endpoint.
        const reportLatestIdx = ROUTES_SRC.indexOf('"/v1/evidence/:id/report/latest"');
