...
describe("Phase 32.7.1 — public verify analytics writes are fire-and-forget", () => {
    ...
    it("the load-bearing `await prisma.$transaction([evidence.update, verificationView.create])` is gone", () => {
        ...
        expect(routeBody).not.toMatch(/await\s+prisma\.\$transaction\(\[[\s\S]{0,200}prisma\.evidence\.update\(\{[\s\S]{0,400}prisma\.verificationView\.create/);
    });
...
describe("Phase 32.7.1 — original-presign analytics write is fire-and-forget", () => {
    ...
    it("`lastAccessedAtUtc` update is no longer awaited", () => {
        ...
        expect(routeBody).not.toMatch(/await\s+prisma\.evidence\.update\(\{[\s\S]{0,200}lastAccessedByUserId/);
    });
    it("the update is wrapped in `void prisma.evidence.update(...).catch(...)`", () => {
        expect(routeBody).toMatch(/void\s+prisma\.evidence\s*\.\s*update\(\{[\s\S]{0,400}lastAccessedByUserId[\s\S]{0,400}\}\)\s*\.catch\(/);
    });
    it("failures emit a bounded WARN log line (NOT captureException)", () => {
        ...
        const updateIdx = routeBody.indexOf("void prisma.evidence");
    ...
    it("forensic custody event (EVIDENCE_VIEWED) still fires", () => {
        expect(routeBody).toMatch(/appendCustodyEvent\(\{[\s\S]{0,400}eventType:\s*prismaPkg\.CustodyEventType\.EVIDENCE_VIEWED/);
        expect(routeBody).toMatch(/accessMode:\s*"authenticated_original_access"/);
    });
});
// =============================================================================
...
describe("Phase 32.7.1 — custody/audit semantics preserved", () => {
    ...
    it("EVIDENCE_VIEWED custody event still emitted on authenticated original access", () => {
        expect(SRC).toMatch(/prismaPkg\.CustodyEventType\.EVIDENCE_VIEWED[\s\S]{0,400}accessMode:\s*"authenticated_original_access"/);
    });
    ...
    it("EXPORT_BLOCKED_BY_POLICY custody event still emitted on governance denial", () => {
        expect(SRC).toMatch(/prismaPkg\.CustodyEventType\.EXPORT_BLOCKED_BY_POLICY/);
    });
});
