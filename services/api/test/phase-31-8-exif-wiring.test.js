...
import { MEDIA_INTELLIGENCE_RUN_KINDS, } from "../src/services/media-intelligence/run-tracker.service.js";
import { safeSummary, SIGNAL_METADATA, MEDIA_INTELLIGENCE_SIGNAL_TYPES, } from "../src/services/media-intelligence/signal-catalog.js";
...
describe("Phase 31.8 — evidence_part_exif_summaries SQL drift patch", () => {
    const sql = readSource("../../../services/api/sql/drift-patches/2026-05-20-evidence-part-exif-summaries.sql");
    ...
    it("widens media_intelligence_runs.kind to include extract_exif", () => {
        expect(sql).toMatch(/ALTER TABLE "media_intelligence_runs"[\s\S]*?DROP CONSTRAINT IF EXISTS "media_intelligence_runs_kind_bounded"/);
        expect(sql).toMatch(/ADD CONSTRAINT "media_intelligence_runs_kind_bounded"[\s\S]*?'extract_exif'/);
    });
    it("idempotent CREATE TABLE", () => {
        expect(sql).toMatch(/CREATE TABLE IF NOT EXISTS\s+"evidence_part_exif_summaries"/);
    });
    ...
    it("dimensions bounded by CHECK (≤ 200000)", () => {
        expect(sql).toMatch(/CHECK \("image_width_px" IS NULL OR \("image_width_px" > 0 AND "image_width_px" <= 200000\)\)/);
        expect(sql).toMatch(/CHECK \("image_height_px" IS NULL OR \("image_height_px" > 0 AND "image_height_px" <= 200000\)\)/);
    });
    it("orientation bounded 1..8 per EXIF spec", () => {
        expect(sql).toMatch(/CHECK \("orientation" IS NULL OR \("orientation" >= 1 AND "orientation" <= 8\)\)/);
    });
    it("status bounded to known catalog", () => {
        ...
        // block.
        const block = sql.match(/CONSTRAINT "evidence_part_exif_summaries_status_bounded"[\s\S]*?\)\)/)?.[0];
    ...
    it("per-team unique on evidence_part_id — anti-enumeration defence", () => {
        expect(sql).toMatch(/CREATE UNIQUE INDEX IF NOT EXISTS "evidence_part_exif_summaries_team_part_uk"[\s\S]*?\("team_id", "evidence_part_id"\)/);
    });
    it("includes operator-readable Neon apply command in header", () => {
        expect(sql).toMatch(/psql "\$DATABASE_URL"[\s\S]*?2026-05-20-evidence-part-exif-summaries\.sql/);
    });
});
// =============================================================================
...
describe("Phase 31.8 — extract_exif registered in every catalog", () => {
    ...
    it("API-side queue catalog includes extract_exif", () => {
        const src = readSource("../src/queue/media-intelligence-queue.ts");
        expect(src).toMatch(/MEDIA_INTELLIGENCE_JOB_KINDS\s*=\s*\[[\s\S]*?"extract_exif"/);
    });
    it("worker-side queue catalog includes extract_exif", () => {
        const src = readSource("../../../services/worker/src/queue.ts");
    ...
    it("schema-validation registers the new table + key column", () => {
        ...
        expect(src).toContain('indexName: "evidence_part_exif_summaries_team_part_uk"');
    });
});
// =============================================================================
...
describe("Phase 31.8 — worker extract_exif branch", () => {
    const src = readSource("../../../services/worker/src/media-intelligence.processor.ts");
    ...
    it("never throws on FETCH_FAILED — records failure + continues to next part", () => {
        const fn = src.match(/async function processExtractExifJob\([\s\S]*?\n\}\s*\n*$/)?.[0];
        expect(fn).toBeTruthy();
        expect(fn).toMatch(/catch \(err\)\s*\{[\s\S]*?recordExifSummaryFailure[\s\S]*?status:\s*"FETCH_FAILED"[\s\S]*?continue/);
    });
    ...
    it("skips non-image MIME types up front (no wasted S3 fetch)", () => {
        expect(src).toMatch(/!part\.mime_type\.toLowerCase\(\)\.startsWith\("image\/"\)/);
    });
    ...
    it("marks run COMPLETED only when at least one part succeeds OR every part skipped", () => {
        const fn = src.match(/async function processExtractExifJob\([\s\S]*?\n\}\s*\n*$/)?.[0];
        expect(fn).toMatch(/if \(succeeded > 0 \|\| failed === 0\)\s*\{[\s\S]*?markRunCompleted/);
        expect(fn).toMatch(/else \{[\s\S]*?markRunFailed[\s\S]*?all_parts_failed/);
    });
    it("no storage internals leak into log payloads", () => {
        ...
        for (const block of logBlocks) {
            expect(block, `log block leaks storage internals: ${block}`).not.toMatch(/storage_(bucket|key)/);
        }
    });
});
// =============================================================================
...
describe("Phase 31.8 — EXIF summary persistence service", () => {
    const src = readSource("../../../packages/shared-runtime/src/media-intelligence/exif-summary.service.ts");
    it("upsert keyed by (team_id, evidence_part_id) — idempotent", () => {
        expect(src).toMatch(/ON CONFLICT \("team_id", "evidence_part_id"\) DO UPDATE/);
    });
...
describe("Phase 31.8 — analyzer reads persisted EXIF", () => {
    ...
    it("imports listExifSummariesForEvidence + computeExifVsServerGapSeconds", () => {
        expect(src).toMatch(/import\s*\{\s*listExifSummariesForEvidence\s*\}\s*from\s*"\.\/exif-summary\.service\.js"/);
        expect(src).toMatch(/import\s*\{\s*computeExifVsServerGapSeconds\s*\}\s*from\s*"\.\/exif-extractor\.service\.js"/);
    });
...
describe("Phase 31.8 — signal catalog", () => {
    it("catalog now includes DEVICE_METADATA_OBSERVATION", () => {
        expect(MEDIA_INTELLIGENCE_SIGNAL_TYPES).toContain("DEVICE_METADATA_OBSERVATION");
    });
    ...
    it("DEVICE_METADATA_OBSERVATION flagged implemented + INFO + MEDIUM", () => {
        ...
        expect(SIGNAL_METADATA.DEVICE_METADATA_OBSERVATION.defaultSeverity).toBe("INFO");
    });
...
describe("Phase 31.8 — apps/web client vocabulary", () => {
    const typesSrc = readSource("../../../apps/web/lib/media-intelligence/types.ts");
...
describe("Phase 31.8 — ops actions: retry + replay DLQ", () => {
    const queueSrc = readSource("../src/queue/media-intelligence-queue.ts");
    ...
    it("DLQ replay bounded to maxJobs (capped at 200)", () => {
        ...
        const fn = queueSrc.match(/export async function replayMediaIntelligenceDlq[\s\S]*?(?=\nexport |\n\/\*\*|$)/)?.[0];
    ...
    it("retry route registered + auth-gated with requireOpsActorAction", () => {
        expect(routeSrc).toMatch(/"\/v1\/ops\/media-intelligence\/runs\/:runId\/retry"[\s\S]*?requireOpsActorAction/);
    });
    it("replay route registered + auth-gated with requireOpsActorAction", () => {
        expect(routeSrc).toMatch(/"\/v1\/ops\/media-intelligence\/dlq\/replay"[\s\S]*?requireOpsActorAction/);
    });
    it("route bodies are strict (zod .strict() — no unknown fields)", () => {
        ...
        const replayBlock = routeSrc.match(/const ReplayDlqBody = z[\s\S]*?\.strict\(\);/)?.[0];
    ...
    it("response shape bounded — no Redis internals leak", () => {
        ...
        expect(routeSrc).toMatch(/reply\.code\(200\)\.send\(\{\s*runId,\s*retried:\s*true\s*\}\)/);
        expect(routeSrc).toMatch(/reply\.code\(200\)\.send\(\{\s*attempted:[\s\S]*?retried:[\s\S]*?skipped:/);
    });
});
// =============================================================================
    ...
    it("no forbidden truth-claim wording in any source literal", () => {
        ...
        for (let i = 0; i < sources.length; i++) {
            ...
            for (const lit of literals) {
                expect(lit, `source ${i} uses forbidden wording: ${lit}`).not.toMatch(forbidden);
            }
        }
    });
