...
const PUBLIC_KEY_FIXTURE = resolve(REPO_ROOT, "services/api/keys/signing-public.pem");
const PRIVATE_KEY_FIXTURE = resolve(REPO_ROOT, "services/api/keys/signing-private.pem");
...
describe("signing-key seed — 5-step public-key resolver", () => {
    ...
    it("(A1) exports resolvePublicKeyPemFromLocalPem for test reach-in", () => {
        expect(SEED_SRC).toMatch(/export\s+function\s+resolvePublicKeyPemFromLocalPem/);
    });
    ...
    it("(B1) aws-kms provider does not call the local-PEM resolver", () => {
        ...
        const kmsBlockMatch = SEED_SRC.match(/if\s*\(\s*provider\s*===\s*["']aws-kms["']\s*\)\s*\{([\s\S]*?)\n\s*\}\s*else\b/);
    ...
    it("(C4) TEST_ONLY constant is clearly marked AND public-only", () => {
        ...
        const constMatch = SEED_SRC.match(/TEST_ONLY_FALLBACK_PUBLIC_KEY_PEM\s*=\s*\[([\s\S]*?)\]\.join/);
    ...
    it("(E3) falls back to TEST_ONLY constant when env+files all miss AND non-prod", async () => {
        ...
        try {
            ...
            expect(result.pem).toMatch(/BEGIN PUBLIC KEY/);
            expect(["fixture", "TEST_ONLY"]).toContain(result.source.startsWith("fixture") ? "fixture" : "TEST_ONLY");
        }
        finally {
            process.env = prev;
        }
