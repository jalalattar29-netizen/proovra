...
const GOVERNED_EXPORT = readSource("../../../apps/web/components/governance/GovernedExportAction.tsx");
...
const COMMAND_PALETTE = readSource("../../../apps/web/components/navigation/CommandPalette.tsx");
// ===========================================================================
...
describe("Phase G2 (G1.1) — governance surface completion", () => {
    it("Matter Workspace Overview tab mounts the GovernanceSummary", () => {
        expect(MATTER_UI).toMatch(/import\s*\{\s*GovernanceSummary\s*\}\s*from\s+"\.\.\/governance\/GovernanceSummary"/);
        expect(MATTER_UI).toMatch(/<GovernanceSummary[\s\S]*?variant="matter"/);
    });
    it("GovernedExportAction wrapper exists and is read-only", () => {
        expect(GOVERNED_EXPORT).toMatch(/export function GovernedExportAction/);
...
describe("Phase G2 (C1.3) — Matter Workspace per-tab filter", () => {
    it("declares filterText state + filter input ref", () => {
        ...
        expect(MATTER_UI).toMatch(/filterInputRef\s*=\s*useRef<HTMLInputElement\s*\|\s*null>/);
    });
    ...
    it("filter input is suppressed on Overview / Graph / Risk tabs", () => {
        expect(MATTER_UI).toMatch(/activeTab\s*!==\s*"overview"\s*&&\s*\n?\s*activeTab\s*!==\s*"graph"\s*&&\s*\n?\s*activeTab\s*!==\s*"risk"/);
    });
    ...
    it("every filterable tab accepts an optional `filterText` prop", () => {
        ...
        for (const name of tabFns) {
            const re = new RegExp(`function\\s+${name}\\(\\s*\\{[\\s\\S]*?filterText[\\s\\S]*?\\}:\\s*\\{`);
...
describe("Phase G2 (C1.5) — Matter Workspace keyboard shortcuts", () => {
    ...
    it('binds `/` to focus the filter input', () => {
        expect(MATTER_UI).toMatch(/e\.key\s*===\s*"\/"[\s\S]*?filterInputRef\.current\?\.focus\(\)/);
    });
...
describe("Phase G2 (C2.5) — discussion advanced filters/search", () => {
    it("declares filterText + filterPreset state", () => {
        expect(DISCUSSION_PANEL).toMatch(/const \[filterText, setFilterText\]\s*=\s*useState/);
        expect(DISCUSSION_PANEL).toMatch(/const \[filterPreset, setFilterPreset\]\s*=\s*useState/);
    });
    it("renders bounded preset chips (all / unresolved / escalated / resolved)", () => {
        ...
        expect(DISCUSSION_PANEL).toMatch(/\[\s*"all"\s*,\s*"unresolved"\s*,\s*"escalated"\s*,\s*"resolved"\s*\]\s*as const/);
        expect(DISCUSSION_PANEL).toContain("data-discussion-filter-preset");
        expect(DISCUSSION_PANEL).toContain("data-discussion-filter-preset-active");
    });
    ...
    it("applies the filter client-side on the bounded thread list", () => {
        expect(DISCUSSION_PANEL).toMatch(/threads\s*\n?\s*\.filter\([\s\S]*?filterPreset[\s\S]*?filterText/);
    });
...
describe("Phase G2 (B.6) — operational quick-jump", () => {
    it("CommandPalette imports operationalGroupDescriptor from Phase B groups", () => {
        expect(COMMAND_PALETTE).toMatch(/import\s*\{\s*operationalGroupDescriptor[\s\S]*?\}\s*from\s+"\.\.\/\.\.\/lib\/navigation\/phaseBOperationalGroups"/);
    });
    it("renders a Phase B group chip per result", () => {
        expect(COMMAND_PALETTE).toContain("data-command-palette-group-chip");
        expect(COMMAND_PALETTE).toContain("data-command-palette-operational-group");
        expect(COMMAND_PALETTE).toMatch(/operationalGroupDescriptor\(item\.route\.id\)/);
    });
    it("preserves the global Cmd+K binding (B.6 trigger)", () => {
        expect(COMMAND_PALETTE).toMatch(/\(e\.metaKey\s*\|\|\s*e\.ctrlKey\)\s*&&\s*e\.key\.toLowerCase\(\)\s*===\s*"k"/);
    });
