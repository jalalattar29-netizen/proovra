...
import { ProductionConfigError, collectStartupViolations, envBool, envInt, getFeatureSnapshot, resolveSecret, } from "../src/config/index.js";
import { COUNTER_NAMES, GAUGE_NAMES, bump, readCounter, readGauge, setGauge, snapshotMetrics, __resetMetricsForTests, } from "../src/services/ops/metrics.service.js";
// -----------------------------------------------------------------------------
...
describe("Phase 20 — resolveSecret", () => {
    ...
    it("THROWS ProductionConfigError when env is missing in NODE_ENV=production", () => {
        ...
        expect(() => resolveSecret("COMMUNICATIONS_RECIPIENT_HASH_SECRET")).toThrow(ProductionConfigError);
    });
...
describe("Phase 20 — collectStartupViolations", () => {
    ...
    it("reports COMMUNICATIONS_RECIPIENT_HASH_SECRET only when COMMUNICATIONS_ENABLED=true", () => {
        ...
        expect(collectStartupViolations().some((v) => v.envName === "COMMUNICATIONS_RECIPIENT_HASH_SECRET")).toBe(false);
        process.env = {
            ...ORIGINAL,
            COMMUNICATIONS_ENABLED: "true",
            COMMUNICATIONS_RECIPIENT_HASH_SECRET: "",
        };
        expect(collectStartupViolations().some((v) => v.envName === "COMMUNICATIONS_RECIPIENT_HASH_SECRET")).toBe(true);
    });
    it("reports IDENTITY_SECURITY_HASH_SECRET only when IDENTITY_SECURITY_ENABLED=true", () => {
        ...
        expect(collectStartupViolations().some((v) => v.envName === "IDENTITY_SECURITY_HASH_SECRET")).toBe(true);
    });
    it("reports API_KEY_SECRET only when INTEGRATIONS_ENABLED=true", () => {
        ...
        expect(collectStartupViolations().some((v) => v.envName === "API_KEY_SECRET")).toBe(true);
    });
});
// -----------------------------------------------------------------------------
...
describe("Phase 20 — metrics", () => {
    ...
    it("bump silently drops ad-hoc / typo'd counter names", () => {
        ...
        bump("not_a_real_counter");
        const snap = snapshotMetrics();
        expect(snap.counters["not_a_real_counter"]).toBeUndefined();
    });
...
describe("Phase 20 — dev-fallback hash keys removed from production paths", () => {
    it("communication.service.ts uses resolveSecret (no literal phase18-dev-fallback)", async () => {
        ...
        const src = await readFile(fileURLToPath(new URL("../src/services/communications/communication.service.ts", import.meta.url)), "utf8");
    ...
    it("identity-security/risk.service.ts uses resolveSecret (no literal phase19-dev-fallback)", async () => {
        const { readFile } = await import("node:fs/promises");
        const { fileURLToPath } = await import("node:url");
        const src = await readFile(fileURLToPath(new URL("../src/services/identity-security/risk.service.ts", import.meta.url)), "utf8");
    ...
    it("identity-security/session-revocation.service.ts uses resolveSecret (no literal phase19-dev-fallback)", async () => {
        const { readFile } = await import("node:fs/promises");
        const { fileURLToPath } = await import("node:url");
        const src = await readFile(fileURLToPath(new URL("../src/services/identity-security/session-revocation.service.ts", import.meta.url)), "utf8");
        expect(src).not.toMatch(/phase19-dev-fallback-hash-key/);
        expect(src).toMatch(/resolveSecret/);
    });
});
...
describe("Phase 20 — safeEmitSecurityEvent observability", () => {
    it("source wires the failure path to the metrics module", async () => {
        const { readFile } = await import("node:fs/promises");
        const { fileURLToPath } = await import("node:url");
        const src = await readFile(fileURLToPath(new URL("../src/services/security/security-event.service.ts", import.meta.url)), "utf8");
...
describe("Phase 20 — ops routes auth posture", () => {
    it("/healthz and /readyz are public (no requireAuth preHandler)", async () => {
        const { readFile } = await import("node:fs/promises");
        const { fileURLToPath } = await import("node:url");
        const src = await readFile(fileURLToPath(new URL("../src/routes/ops.routes.ts", import.meta.url)), "utf8");
        const healthzBlock = src.match(/app\.get\(\s*"\/healthz"[\s\S]{0,300}/);
    ...
    it("/v1/ops/health and /v1/ops/metrics use requireAuth + 404-on-non-member", async () => {
        ...
        const { fileURLToPath } = await import("node:url");
        const src = await readFile(fileURLToPath(new URL("../src/routes/ops.routes.ts", import.meta.url)), "utf8");
        expect(src).toMatch(/app\.get\(\s*"\/v1\/ops\/health"[\s\S]{0,200}preHandler:\s*requireAuth/);
        expect(src).toMatch(/app\.get\(\s*"\/v1\/ops\/metrics"[\s\S]{0,200}preHandler:\s*requireAuth/);
    ...
    it("/v1/ops/reconcile is cron-secret protected (no requireAuth preHandler)", async () => {
        ...
        const { fileURLToPath } = await import("node:url");
        const src = await readFile(fileURLToPath(new URL("../src/routes/ops.routes.ts", import.meta.url)), "utf8");
        expect(src).toMatch(/app\.post\(\s*"\/v1\/ops\/reconcile"[\s\S]{0,400}/);
...
describe("Phase 20 — wording sweep (user-facing surfaces clean of overclaims)", () => {
    ...
    it("Phase 18 communications page contains no forbidden overclaim phrasing", async () => {
        const { readFile } = await import("node:fs/promises");
        const { fileURLToPath } = await import("node:url");
        const src = await readFile(fileURLToPath(new URL("../../../apps/web/app/(app)/communications/page.tsx", import.meta.url)), "utf8");
    ...
    it("Phase 19 security-center page contains no forbidden overclaim phrasing", async () => {
        const { readFile } = await import("node:fs/promises");
        const { fileURLToPath } = await import("node:url");
        const src = await readFile(fileURLToPath(new URL("../../../apps/web/app/(app)/security-center/page.tsx", import.meta.url)), "utf8");
        for (const re of FORBIDDEN) {
            expect(src).not.toMatch(re);
        }
    });
    it("Phase 17 identity surface (now /admin/identity) contains no forbidden overclaim phrasing", async () => {
        ...
        const { readFile } = await import("node:fs/promises");
        const { fileURLToPath } = await import("node:url");
        const src = await readFile(fileURLToPath(new URL("../../../apps/web/app/(app)/admin/identity/page.tsx", import.meta.url)), "utf8");
        for (const re of FORBIDDEN) {
            expect(src).not.toMatch(re);
        }
    });
    ...
    it("Shared communications body templates contain no forbidden overclaim phrasing", async () => {
        const { readFile } = await import("node:fs/promises");
        const { fileURLToPath } = await import("node:url");
        const src = await readFile(fileURLToPath(new URL("../../../packages/shared/src/communications.ts", import.meta.url)), "utf8");
        for (const re of FORBIDDEN) {
...
describe("Phase 20 — route permission matrix (high-risk surfaces)", () => {
    it("identity routes wire step-up on every documented sensitive purpose", async () => {
        const { readFile } = await import("node:fs/promises");
        const { fileURLToPath } = await import("node:url");
        const src = await readFile(fileURLToPath(new URL("../src/routes/identity.routes.ts", import.meta.url)), "utf8");
    ...
    it("governance routes wire step-up on every documented sensitive purpose", async () => {
        const { readFile } = await import("node:fs/promises");
        const { fileURLToPath } = await import("node:url");
        const src = await readFile(fileURLToPath(new URL("../src/routes/governance.routes.ts", import.meta.url)), "utf8");
        for (const purpose of [
            "LEGAL_HOLD_RELEASE",
    ...
    it("integrations routes wire step-up on API key create + revoke", async () => {
        const { readFile } = await import("node:fs/promises");
        const { fileURLToPath } = await import("node:url");
        const src = await readFile(fileURLToPath(new URL("../src/routes/integrations.routes.ts", import.meta.url)), "utf8");
    ...
    it("identity-security routes self-protect MFA policy update with step-up", async () => {
        const { readFile } = await import("node:fs/promises");
        const { fileURLToPath } = await import("node:url");
        const src = await readFile(fileURLToPath(new URL("../src/routes/identity-security.routes.ts", import.meta.url)), "utf8");
