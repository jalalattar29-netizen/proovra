...
import { ANALYTICS_DEFAULT_WINDOW_DAYS, ANALYTICS_MAX_WINDOW_DAYS, ANALYTICS_MIN_WINDOW_DAYS, clampWindow, } from "../src/services/analytics/analytics.service.js";
...
const PAGE = readWeb("app/(app)/ops/analytics/page.tsx");
...
describe("E4 Test 1 — bounded window", () => {
    ...
    it("clampWindow(Infinity) → default", () => {
        expect(clampWindow(Number.POSITIVE_INFINITY)).toBe(ANALYTICS_DEFAULT_WINDOW_DAYS);
    });
...
describe("E4 Test 3 — analytics service source contract", () => {
    ...
    it("Report + VerificationPackage scope through evidence.teamId (no fake teamId column)", () => {
        ...
        expect(SERVICE).toMatch(/prisma\.verificationPackage\.count[\s\S]*?evidence:\s*\{\s*teamId/);
    });
...
describe("E4 Test 4 — REST endpoints contract", () => {
    ...
    it.each(ENDPOINTS)("registers GET %s", (path) => {
        const re = new RegExp(`app\\.get\\(\\s*["']${path.replace(/\//g, "\\/")}["']`);
    ...
    it("server registers analyticsOperationsRoutes after automation routes", () => {
        ...
        const idxOps = SERVER.indexOf("app.register(analyticsOperationsRoutes)");
        const idxAutomation = SERVER.indexOf("app.register(automationWebhooksRoutes)");
...
describe("E4 Test 5 — route registry entry", () => {
    it("registers platform.analytics under /ops/analytics", () => {
        expect(ROUTE_REGISTRY).toMatch(/id:\s*["']platform\.analytics["']/);
        expect(ROUTE_REGISTRY).toMatch(/href:\s*["']\/ops\/analytics["']/);
    });
    it("route requires ANALYTICS_VIEW capability", () => {
        const block = ROUTE_REGISTRY.match(/id:\s*["']platform\.analytics["'][\s\S]*?sidebarEligible:[^\n]+/);
...
describe("E4 Test 6 — frontend page contract", () => {
    it("page exists at /ops/analytics", () => {
        expect(existsSync(webPath("app/(app)/ops/analytics/page.tsx"))).toBe(true);
    });
    ...
    it("bounded window selector lists only the canonical day options", () => {
        expect(PAGE).toMatch(/ANALYTICS_WINDOW_OPTIONS\s*=\s*\[\s*7,\s*14,\s*30,\s*60,\s*90,\s*180\s*\]/);
    });
});
// ===========================================================================
...
describe("E4 Test 9 — documentation + registry", () => {
    it("docs/product/PHASE_E4_ANALYTICS_OPERATIONAL_INTELLIGENCE.md exists + substantial", () => {
        const doc = readRepo("docs/product/PHASE_E4_ANALYTICS_OPERATIONAL_INTELLIGENCE.md");
    ...
    it("registry registers Phase E4 with explicit closure status", () => {
        ...
        expect(registry).toMatch(/\|\s*(Phase )?E4\s*\|[\s\S]*?(CLOSED|CLOSED_WITH_DEFERRED_ITEMS)/);
    });
});
