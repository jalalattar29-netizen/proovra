...
describe("case legal hold service", () => {
    it("place + release both emit CASE_LEGAL_HOLD_* custody events", async () => {
        ...
        const src = await readFile(fileURLToPath(new URL("../src/services/governance/case-legal-hold.service.ts", import.meta.url)), "utf8");
...
describe("retention sweeper service", () => {
    it("never auto-deletes; only flags via RETENTION_CANDIDATE_IDENTIFIED", async () => {
        const { readFile } = await import("node:fs/promises");
        const { fileURLToPath } = await import("node:url");
        const src = await readFile(fileURLToPath(new URL("../src/services/governance/retention-sweeper.service.ts", import.meta.url)), "utf8");
...
describe("publication service", () => {
    it("uses atomic where-status updateMany guard", async () => {
        const { readFile } = await import("node:fs/promises");
        const { fileURLToPath } = await import("node:url");
        const src = await readFile(fileURLToPath(new URL("../src/services/governance/publication.service.ts", import.meta.url)), "utf8");
    ...
    it("public verify route gates on publicVerifyState BEFORE returning data", async () => {
        ...
        const stateIdx = src.indexOf('if (evidence.publicVerifyState !== "PUBLISHED")');
...
describe("governance route surface", () => {
    it("retention reconciliation is cron-protected", async () => {
        ...
        expect(src).toMatch(/"\/v1\/governance\/reconcile-retention"[\s\S]*requireIntegrationCronSecret/);
    });
...
describe("export governance — gate matrix is preserved", () => {
    it("evidenceIsReviewed still requires APPROVED_INTERNAL / READY_FOR_EXTERNAL_REVIEW", async () => {
        const { reviewStatusSatisfiesGovernanceGate } = await import("../src/services/governance.service.js");
        expect(reviewStatusSatisfiesGovernanceGate("APPROVED_INTERNAL")).toBe(true);
        expect(reviewStatusSatisfiesGovernanceGate("READY_FOR_EXTERNAL_REVIEW")).toBe(true);
        for (const s of [
            "IN_REVIEW",
            "NEEDS_INFO",
            "QUEUED",
...
describe("Phase 14 privacy contracts", () => {
    ...
    it("custody chain captures suspension reason but public verify route does not", async () => {
        const { readFile } = await import("node:fs/promises");
        const { fileURLToPath } = await import("node:url");
        const publicationSrc = await readFile(fileURLToPath(new URL("../src/services/governance/publication.service.ts", import.meta.url)), "utf8");
        ...
        const evidenceRoutes = await readFile(fileURLToPath(new URL("../src/routes/evidence.routes.ts", import.meta.url)), "utf8");
        // The select clause used by /public/verify/:id MUST NOT pull the
        // suspension reason column.
        const verifyBlock = evidenceRoutes.slice(evidenceRoutes.indexOf('app.get("/public/verify/:id"'), evidenceRoutes.indexOf('app.get("/public/verify/:id"') + 6000);
