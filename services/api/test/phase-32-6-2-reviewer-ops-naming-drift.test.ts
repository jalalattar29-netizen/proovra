...
const SCALAR_TYPE_PATTERN =
  /^(String|Int|BigInt|Boolean|DateTime|Decimal|Float|Bytes|Json)(\?|\[\])?$/;
...
function parseModel(schema: string, modelName: string): PrismaModel {
...
describe("Phase 32.6.2 — reviewer_ops naming drift regression guard", () => {
      ...
      it("every camelCase scalar field carries @map(\"snake_case\")", () => {
        ...
        for (const field of model.fields) {
          ...
          if (!hasMapAnnotation(field.raw, expectedSnake)) {
            offenders.push(
              `  ${modelName}.${field.name} (line ${field.line}) — expected @map("${expectedSnake}")`,
            );
          }
        }
        if (offenders.length > 0) {
          throw new Error(
            `Phase 32.6.2 contract violation: reviewer_ops camelCase scalar fields must carry an @map annotation that pins them to the snake_case migration column. Offending fields:\n${offenders.join(
              "\n",
  ...
  it("repair migration file ships alongside the schema fix", () => {
    const migrationPath = fileURLToPath(
      new URL(
        "../prisma/migrations/20260620200000_reviewer_ops_naming_drift_repair/migration.sql",
        import.meta.url,
      ),
    );
