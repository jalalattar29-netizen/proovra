...
const MIGRATION = readApi("prisma/migrations/20260720100000_personal_workspace_bootstrap/migration.sql");
const BOOTSTRAP = readApi("src/services/platform-context/workspace-bootstrap.service.ts");
...
const WEB_REPORTS = readWeb("components/reports-experience/ReportsIndex.tsx");
...
describe("Phase EMERGENCY-RECOVERY — schema", () => {
    it("Team model declares an isPersonal boolean field", () => {
        expect(SCHEMA).toMatch(/isPersonal\s+Boolean\s+@default\(false\)\s+@map\("is_personal"\)/);
    });
...
describe("Phase EMERGENCY-RECOVERY — workspace-bootstrap service", () => {
    ...
    it("uses a fast-path findFirst before attempting create (idempotency)", () => {
        expect(BOOTSTRAP).toMatch(/client\.team\.findFirst[\s\S]{0,200}ownerUserId:\s*input\.userId/);
    });
...
describe("Phase EMERGENCY-RECOVERY — platform-context builder", () => {
    it("imports ensurePersonalWorkspace from the bootstrap service", () => {
        expect(SVC).toMatch(/import\s*\{\s*ensurePersonalWorkspace\s*\}\s*from\s*"\.\/workspace-bootstrap\.service\.js"/);
    });
...
describe("Phase EMERGENCY-RECOVERY — recovery actions", () => {
    ...
    it("envelope declares recoveryActions: ReadonlyArray<...> on both sides", () => {
        ...
        expect(TYPES).toMatch(/recoveryActions\??:\s*ReadonlyArray<PlatformContextRecoveryAction>/);
        expect(WEB_TYPES).toMatch(/recoveryActions\??:\s*ReadonlyArray<PlatformContextRecoveryAction>/);
    });
