...
const MIGRATION_SQL = readSource(
  "../prisma/migrations/20261002000000_phase_a2_pdf_artifact_status/migration.sql",
);
const SCHEMA = readSource("../prisma/schema.prisma");
const ARTIFACT_STATUS_SERVICE = readSource(
...
describe("Phase A2 — PDF artifact status (source contract)", () => {
  it("migration adds the four PDF artifact columns to reports", () => {
    expect(MIGRATION_SQL).toMatch(
      /ADD COLUMN IF NOT EXISTS pdf_signature_status\s+VARCHAR\(32\)/i,
    );
    expect(MIGRATION_SQL).toMatch(
      /ADD COLUMN IF NOT EXISTS pdf_signed_at_utc\s+TIMESTAMPTZ/i,
    );
    expect(MIGRATION_SQL).toMatch(
      /ADD COLUMN IF NOT EXISTS pdf_signer_key_id\s+VARCHAR\(120\)/i,
    );
    expect(MIGRATION_SQL).toMatch(
      /ADD COLUMN IF NOT EXISTS pdf_signing_warning\s+VARCHAR\(400\)/i,
    );
  });

  it("migration adds REPORT_PDF_SIGNED + REPORT_PDF_UNSIGNED_OPT_OUT custody events", () => {
    expect(MIGRATION_SQL).toMatch(
      /ALTER\s+TYPE\s+"CustodyEventType"\s+ADD\s+VALUE\s+IF\s+NOT\s+EXISTS\s+'REPORT_PDF_SIGNED'/i,
    );
    expect(MIGRATION_SQL).toMatch(
      /ALTER\s+TYPE\s+"CustodyEventType"\s+ADD\s+VALUE\s+IF\s+NOT\s+EXISTS\s+'REPORT_PDF_UNSIGNED_OPT_OUT'/i,
    );
  ...
  it("shared package exports the bounded artifact + signature vocabulary", () => {
    ...
    expect(ARTIFACT_LABELS.REPORT_PDF_UNSIGNED).toBe(
      "Unsigned Report PDF artifact",
    );
    expect(ARTIFACT_LABELS.VERIFICATION_PACKAGE_ZIP).toBe(
  ...
  it("artifact-status service projects pdfSignature for the Report block", () => {
    ...
    expect(ARTIFACT_STATUS_SERVICE).toContain(
  ...
  it("Evidence detail hero uses the disambiguated labels", () => {
    ...
    expect(
      executable.find((l) => />Download report</.test(l)),
    ).toBeUndefined();
    expect(
      executable.find((l) => />Download package</.test(l)),
    ).toBeUndefined();
  });
...
describe("Phase A2 — startup config validation (PDF signing)", () => {
  ...
  beforeEach(() => {
    snapshot = {
      ...
      PDF_ARTIFACT_SIGNATURE_OPT_OUT_ACK:
  ...
  it("non-production never raises the PDF signing violation", () => {
    ...
    expect(
      violations.find(
        (v) => v.reason === "pdf_signing_unconfigured_in_production",
      ),
    ).toBeUndefined();
  });
  ...
  it("production WITHOUT signing AND WITHOUT opt-out raises pdf_signing_unconfigured_in_production", () => {
    ...
    const v = violations.find(
      (x) => x.reason === "pdf_signing_unconfigured_in_production",
    );
    expect(v).toBeTruthy();
