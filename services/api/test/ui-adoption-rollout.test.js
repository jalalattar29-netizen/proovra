...
const BANNED_WORDING = /\btamper(ed|ing)?\b|\bforged\b|\bforgery\b|\baltered content\b|\bmanipulated evidence\b/i;
function assertNoBannedWordingInStringLiterals(src, label) {
    const stringLiterals = src.match(/"[^"\n]+"/g) ?? [];
    const all = stringLiterals.join(" ");
    expect(all, `banned wording leaked into ${label}`).not.toMatch(BANNED_WORDING);
}
// =============================================================================
...
describe("Reviewer Ops landing page (full adoption — Phase 32.8E architecture)", () => {
    ...
    const src = readSource("../../../apps/web/components/reviewer-experience/ReviewerCommandConsole.tsx");
    it("imports the runtime banner from the operational barrel", () => {
        expect(src).toMatch(/import\s*\{[\s\S]*?RuntimeStatusBanner[\s\S]*?\}\s*from\s*"[\.\/]+\/operational"/);
    });
    it("scopes RuntimeStatusBanner to reviewer_ops (Phase 32.7 invariant)", () => {
        expect(src).toMatch(/<RuntimeStatusBanner[\s\S]{0,400}forDomains=\{\s*\[\s*"reviewer_ops"\s*\]/);
    });
    it("renders an empty-state note when no review workflows exist", () => {
        expect(src).toMatch(/q\.items\.length === 0[\s\S]{0,400}No open review workflows in this workspace/);
    });
    ...
    it("no banned wording in this console's string literals", () => {
        assertNoBannedWordingInStringLiterals(src, "reviewer-experience/ReviewerCommandConsole.tsx");
    });
});
// =============================================================================
...
describe("Reviewer Ops SLA page (full adoption)", () => {
    const src = readSource("../../../apps/web/app/(app)/reviewer-ops/sla/page.tsx");
    it("imports NoWorkloadSnapshotsEmptyState + RuntimeStatusBanner from the operational barrel", () => {
        expect(src).toMatch(/import\s*\{[\s\S]*?NoWorkloadSnapshotsEmptyState[\s\S]*?RuntimeStatusBanner[\s\S]*?\}\s*from\s*"[\.\/]+components\/operational"/);
    });
    it("renders RuntimeStatusBanner only when teamId is known", () => {
        ...
        expect(src).toMatch(/teamId\s*\?\s*\(?[\s\S]{0,800}?<RuntimeStatusBanner[\s\S]{0,400}teamId=\{teamId\}/);
    });
...
describe("Reviewer Ops policy page (full adoption)", () => {
    ...
    const src = readSource("../../../apps/web/app/(app)/governance/policy/page.tsx");
    it("imports RuntimeStatusBanner from the operational barrel", () => {
        expect(src).toMatch(/import\s*\{[\s\S]*?RuntimeStatusBanner[\s\S]*?\}\s*from\s*"[\.\/]+components\/operational"/);
    });
    it("renders RuntimeStatusBanner only when teamId is known", () => {
        // Phase 32.7 — banner usage now optionally includes `forDomains`
        // for degradation isolation, and the JSX may be wrapped in `()`
...
describe("Governance dashboard (full adoption — Phase 32.8E architecture)", () => {
    ...
    const src = readSource("../../../apps/web/components/governance-experience/GovernanceControlPlane.tsx");
    it("imports the runtime banner from the operational barrel", () => {
        expect(src).toMatch(/import\s*\{[\s\S]*?RuntimeStatusBanner[\s\S]*?\}\s*from\s*"[\.\/]+\/operational"/);
    });
    it("scopes RuntimeStatusBanner to governance_lifecycle (Phase 32.7 invariant)", () => {
        expect(src).toMatch(/<RuntimeStatusBanner[\s\S]{0,400}forDomains=\{\s*\[\s*"governance_lifecycle"\s*\]/);
    });
    it("renders an empty-state note when no evidence-level holds exist", () => {
        expect(src).toMatch(/evidenceHolds\.length === 0[\s\S]{0,400}No evidence-level holds placed/);
    });
    it("renders an empty-state note when no case-level holds exist", () => {
        expect(src).toMatch(/caseHolds\.length === 0[\s\S]{0,400}No case-level legal holds placed/);
    });
    it("renders an empty-state note when no retention candidates exist", () => {
        expect(src).toMatch(/candidates\.length === 0[\s\S]{0,400}No retention candidates awaiting review/);
    });
    it("renders an empty-state note when no pending destruction reviews exist", () => {
        expect(src).toMatch(/pendingDestructionReviews\.length === 0[\s\S]{0,400}No destruction reviews pending/);
    });
    it("no banned wording in this control-plane's string literals", () => {
        assertNoBannedWordingInStringLiterals(src, "governance-experience/GovernanceControlPlane.tsx");
    });
});
// =============================================================================
// Observability dashboard
...
describe("Observability dashboard (full adoption)", () => {
    ...
    it("renders RuntimeStatusBanner inside the main render", () => {
        ...
        expect(src).toMatch(/<RuntimeStatusBanner[\s\S]{0,200}teamId=\{teamId\}/);
    });
    ...
    it("no banned wording in this page's string literals", () => {
        assertNoBannedWordingInStringLiterals(src, "ops/observability/page.tsx");
    });
});
// =============================================================================
// Evidence detail
...
describe("Evidence detail page (full adoption)", () => {
    const src = readSource("../../../apps/web/app/(app)/evidence/[id]/page.tsx");
    it("imports ExportPackageEligibilityBadge + GovernanceSnapshotPanel + OperationalTimelinePanel + RuntimeStatusBanner from the operational barrel", () => {
        expect(src).toMatch(/import\s*\{[\s\S]*?ExportPackageEligibilityBadge[\s\S]*?GovernanceSnapshotPanel[\s\S]*?OperationalTimelinePanel[\s\S]*?RuntimeStatusBanner[\s\S]*?\}\s*from\s*"[\.\/]+components\/operational"/);
    });
    it("renders RuntimeStatusBanner inside the evidence-detail-shell (operator sees runtime state above the hero)", () => {
        expect(src).toMatch(/evidence-detail-shell[\s\S]*?<RuntimeStatusBanner\s+teamId=\{workspace\.reviewWorkflow\.teamId\}/);
    });
    it("renders ExportPackageEligibilityBadge for both export and package kinds", () => {
        expect(src).toMatch(/<ExportPackageEligibilityBadge[\s\S]*?kind="export"/);
        expect(src).toMatch(/<ExportPackageEligibilityBadge[\s\S]*?kind="package"/);
    });
    it("eligibility state defaults to enabled (Phase 28-H hotfix — backend gate is authoritative)", () => {
        ...
        // gate.
        expect(src).toMatch(/<Button[\s\S]*?onClick=\{\(\) => void downloadReport\(\)\}[\s\S]*?disabled=\{exportDisabled\b/);
        expect(src).toMatch(/<Button[\s\S]*?onClick=\{\(\) => void downloadVerificationPackage\(\)\}[\s\S]*?disabled=\{packageDisabled\b/);
    });
    it("disabled-callback flips the button OFF only on a confirmed non-eligible decision", () => {
        ...
        expect(src).toMatch(/!s\.loading\s*&&\s*!s\.unknown\s*&&\s*!s\.eligible/);
        // The old (broken) fail-closed-on-loading expression must NOT exist.
        expect(src).not.toMatch(/s\.loading\s*\|\|\s*s\.unknown\s*\|\|\s*!s\.eligible/);
    });
    ...
    it("renders GovernanceSnapshotPanel on the overview tab when teamId is known", () => {
        expect(src).toMatch(/activeTab === "overview"[\s\S]*?<GovernanceSnapshotPanel/);
        expect(src).toMatch(/workspace\.reviewWorkflow\?\.teamId[\s\S]*?<GovernanceSnapshotPanel/);
    });
    it("renders OperationalTimelinePanel on the custody tab when teamId is known", () => {
        expect(src).toMatch(/activeTab === "custody"[\s\S]*?<OperationalTimelinePanel/);
        expect(src).toMatch(/workspace\.reviewWorkflow\?\.teamId[\s\S]*?<OperationalTimelinePanel/);
    });
    it("eligibility badges are placed in the hero block (operator sees them next to the action buttons)", () => {
        ...
        const downloadReportButtonIdx = src.indexOf("void downloadReport()");
    ...
    it("does not expose private notes / storage keys / signature material via the new wiring", () => {
        ...
        const importMatch = src.match(/import\s*\{([\s\S]*?)\}\s*from\s*"[\.\/]+components\/operational"/);
...
describe("Download routes (authoritative governance gate)", () => {
    const src = readSource("../../../services/api/src/routes/evidence.routes.ts");
    it("GET /v1/evidence/:id/report/latest exists and runs enforceSensitiveAction(\"download_report\", ...)", () => {
        ...
        expect(src).toMatch(/enforceSensitiveAction\(\s*"download_report"\s*,/);
    });
    it("GET /v1/evidence/:id/verification-package exists and runs enforceSensitiveAction(\"download_package\", ...)", () => {
        ...
        expect(src).toMatch(/enforceSensitiveAction\(\s*"download_package"\s*,/);
    });
    it("a blocked decision returns 403/503 — never a signed URL leak", () => {
        ...
        expect(src).toMatch(/decision\.code === "GOVERNANCE_CHECK_FAILED"\s*\?\s*503\s*:\s*403/);
    });
    it("custody event EXPORT_BLOCKED_BY_POLICY is appended when a download is blocked (no silent failure)", () => {
        expect(src).toMatch(/CustodyEventType\.EXPORT_BLOCKED_BY_POLICY/);
    });
});
// =============================================================================
...
describe("Phase 28-H [cross-page wiring invariants]", () => {
    ...
    it("every adopting page imports from the operational barrel exactly once", () => {
        for (const rel of ADOPTING_PAGES) {
            ...
            const matches = src.match(/from\s*"(?:[\.\/]+components\/operational|\.\.\/operational)"/g) ?? [];
    ...
    it("every adopting page either uses RuntimeStatusBanner or is the observability dashboard itself", () => {
        for (const rel of ADOPTING_PAGES) {
            ...
            expect(src, `RuntimeStatusBanner missing in ${rel}`).toMatch(/RuntimeStatusBanner/);
        }
    });
    ...
    it("no adopting page leaks process.env values into rendered text", () => {
        for (const rel of ADOPTING_PAGES) {
            ...
            // in JSX.
            expect(src, `env interpolation in ${rel}`).not.toMatch(/\{process\.env\.[A-Z_]+\}/);
        }
    });
});
