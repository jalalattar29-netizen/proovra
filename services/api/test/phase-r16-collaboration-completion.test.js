...
import { COLLABORATION_TEAM_ACCESS_REVIEW_DECISIONS, COLLABORATION_TEAM_ACCESS_REVIEW_STATUSES, COLLABORATION_TEAM_COMMENT_STATUSES, COLLABORATION_TEAM_COMMENT_TARGETS, COLLABORATION_TEAM_DIGEST_MODES, COLLABORATION_TEAM_GUEST_STATUSES, COLLABORATION_TEAM_MENTION_TYPES, COLLABORATION_TEAM_NOTIFICATION_TYPES, COLLABORATION_TEAM_SPECIAL_MENTIONS, buildCollaborationTeamUserDirectoryEntry, isSpecialCollaborationTeamMention, parseCollaborationTeamMentionHandles, sanitiseCollaborationTeamCommentBody, } from "@proovra/shared";
const repoRoot = fileURLToPath(new URL("../../../", import.meta.url));
function read(rel) {
    const full = join(repoRoot, rel);
    if (!existsSync(full))
        throw new Error(`R16: missing required file: ${rel}`);
...
describe("Phase R16 — mention parser", () => {
    it("extracts simple @handle tokens", () => {
        const out = parseCollaborationTeamMentionHandles("@alice please review with @bob");
        expect(out).toEqual(["alice", "bob"]);
    });
    it("recognises @team and @lead as special mentions", () => {
        const out = parseCollaborationTeamMentionHandles("Heads-up @team — @lead please weigh in.");
    ...
    it("does NOT pick up @ inside emails or URLs", () => {
        const out = parseCollaborationTeamMentionHandles("Ping support@example.com or https://x.com/@foo");
...
describe("Phase R16 — comment body sanitiser", () => {
    ...
    it("preserves mention tokens (parser needs them)", () => {
        const out = sanitiseCollaborationTeamCommentBody("Hello @alice and @team");
        if (out.ok) {
...
describe("Phase R16 — schema + migration", () => {
    ...
    it("ships the Phase 7 hardened migration", () => {
        const migPath = "services/api/prisma/migrations/20270301000000_phase_7_collaboration_completion/migration.sql";
    ...
    it("notifications carry readAt for in-app inbox semantics", () => {
        expect(schema).toMatch(/model CollaborationTeamNotification[\s\S]{0,2000}readAt\s+DateTime\?/);
    });
...
describe("Phase R16 — service module", () => {
    const svc = read("services/api/src/services/collaboration-team/collaboration-completion.service.ts");
    it("exports the canonical Phase 7 service functions", () => {
        ...
        for (const sym of required) {
            expect(svc, `missing export ${sym}`).toMatch(new RegExp(`export async function ${sym}\\b`));
        }
    });
    ...
    it("notification fanout never targets the actor themselves", () => {
        ...
        expect(svc).toMatch(/u\.mentionedUserId\s*!==\s*input\.actorUserId/);
    ...
    it("access review items only decidable by LEAD/ADMIN", () => {
        expect(svc).toMatch(/decideAccessReviewItem[\s\S]{0,800}role !== "LEAD" && role !== "ADMIN"/);
    });
});
// =============================================================================
// API routes
...
describe("Phase R16 — API routes", () => {
    const routes = read("services/api/src/routes/collaboration-completion.routes.ts");
    ...
    it("registers comments endpoints", () => {
        ...
        expect(routes).toContain('"/v1/collaboration-teams/:teamId/comments/:commentId"');
    });
    it("registers notification endpoints", () => {
        expect(routes).toContain('"/v1/collaboration-team-notifications"');
        expect(routes).toContain('"/v1/collaboration-team-notifications/:notificationId/read"');
        expect(routes).toContain('"/v1/collaboration-team-notifications/read-all"');
    });
    it("registers preferences endpoints", () => {
        expect(routes).toContain('"/v1/collaboration-teams/:teamId/notification-preferences"');
    });
    it("registers guest endpoints (invite + revoke + list)", () => {
        expect(routes).toContain('"/v1/collaboration-teams/:teamId/guests"');
        expect(routes).toContain('"/v1/collaboration-teams/:teamId/guests/invite"');
        expect(routes).toContain('"/v1/collaboration-teams/:teamId/guests/:guestId/revoke"');
    });
    it("registers access-review endpoints (open/decide/complete)", () => {
        ...
        expect(routes).toContain('"/v1/collaboration-teams/:teamId/access-review/items/:itemId"');
        expect(routes).toContain('"/v1/collaboration-teams/:teamId/access-review/:reviewId/complete"');
    });
    ...
    it("wires the routes into the Fastify app via server.ts", () => {
        ...
        expect(server).toMatch(/await app\.register\(collaborationCompletionRoutes\)/);
    });
...
describe("Phase R16 — frontend", () => {
    it("Phase 7 collaboration hub page exists", () => {
        expect(exists("apps/web/app/(app)/collaboration-teams/[teamId]/collaboration/page.tsx")).toBe(true);
    });
    ...
    it("collaboration hub renders the required testid hooks", () => {
        const hub = read("apps/web/app/(app)/collaboration-teams/[teamId]/collaboration/page.tsx");
        for (const t of [
            "collaboration-hub",
            "comments-panel",
            "comment-create-form",
            ...
            expect(hub, `missing testid ${t}`).toMatch(new RegExp(`data-testid="${t}"`));
        }
    });
...
describe("Phase R16 — constitutional rules", () => {
    ...
    it("no cross-workspace leak — every comment/notification read filters on workspace + team", () => {
        ...
        expect(svc).toMatch(/listMyNotifications[\s\S]{0,1500}userId:\s*input\.actorUserId[\s\S]{0,400}workspaceId:\s*input\.workspaceId/);
    });
});
