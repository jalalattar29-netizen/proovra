...
import { AI_ALLOWED_USE_CASES, AI_ALLOWED_USE_CASE_CONTENT, AI_CANONICAL_ADVISORY_DISCLAIMER, AI_FAILURE_TOLERANCE_CONTRACT, AI_FORBIDDEN_CATEGORIES, AI_FORBIDDEN_CATEGORY_DESCRIPTION, AI_OPERATIONAL_FORBIDDEN_OUTPUT_PATTERNS, AI_OPERATIONAL_REQUIRED_PHRASES, AI_PROMPT_INJECTION_PRINCIPLES, TRUST_CENTER_SECTIONS, } from "@proovra/shared-evidence-presentation";
// ---------------------------------------------------------------------------
// Helpers
// ---------------------------------------------------------------------------
...
const AI_OPERATIONAL_CONTENT_SRC = readPackages("shared-evidence-presentation/src/ai-operational-content.ts");
...
const CAPTURE_AI_COMPONENT = readWeb("components/ai/CaptureAiAssistant.tsx");
const CHAT_WIDGET_COMPONENT = readWeb("components/ai/ProovraChatWidget.tsx");
// ===========================================================================
...
describe("E9 Test 1 — canonical AI content module", () => {
    ...
    it.each(AI_ALLOWED_USE_CASES)("AI_ALLOWED_USE_CASE_CONTENT has a complete record for %s", (code) => {
        const c = AI_ALLOWED_USE_CASE_CONTENT[code];
        expect(c).toBeTruthy();
    ...
    it.each(AI_FORBIDDEN_CATEGORIES)("AI_FORBIDDEN_CATEGORY_DESCRIPTION has a clear rationale for %s", (code) => {
        const description = AI_FORBIDDEN_CATEGORY_DESCRIPTION[code];
...
describe("E9 Test 2 — advisory disclaimer alignment", () => {
    ...
    it("disclaimer surfaces the required boundary phrases", () => {
        for (const phrase of AI_OPERATIONAL_REQUIRED_PHRASES) {
            const inDisclaimer = AI_CANONICAL_ADVISORY_DISCLAIMER.toLowerCase().includes(phrase.toLowerCase());
            const inContentModule = AI_OPERATIONAL_CONTENT_SRC.toLowerCase().includes(phrase.toLowerCase());
            expect(inDisclaimer || inContentModule, `required AI boundary phrase "${phrase}" missing from both the disclaimer and the content module`).toBe(true);
        }
    });
    it("CaptureAiAssistant + ProovraChatWidget both render advisory-only disclaimer wording", () => {
        ...
        for (const src of [CAPTURE_AI_COMPONENT, CHAT_WIDGET_COMPONENT]) {
            ...
            expect(src).toMatch(/does not determine|not\s+legal\s+or\s+factual\s+determination|not\s+(?:legal|factual)\s+advice/i);
        }
    });
});
// ===========================================================================
...
describe("E9 Test 6 — AI service tree never mutates platform state", () => {
    ...
    for (const surface of AI_SOURCES) {
        describe(`surface — ${surface.label}`, () => {
            it.each(FORBIDDEN_MUTATION_CALLS)("does NOT call mutation primitive %s", (pattern) => {
...
describe("E9 Test 10 — forbidden output patterns absent from AI surfaces", () => {
    ...
    const SANITISED_POLICY = AI_POLICY_SRC.replace(/forbiddenPatterns\s*=\s*\[[\s\S]*?\];/g, "");
    const AI_SURFACES = [
    ...
    for (const surface of AI_SURFACES) {
        describe(`surface — ${surface.label}`, () => {
            it.each(AI_OPERATIONAL_FORBIDDEN_OUTPUT_PATTERNS)("does NOT match %s", (pattern) => {
                expect(surface.body).not.toMatch(pattern);
            });
        });
    }
...
// ===========================================================================
describe("E9 Test 14 — protected core files unchanged by E9", () => {
    const PINS = [
...
describe("E9 Test 15 — documentation + registry", () => {
    it("docs/product/PHASE_E9_AI_OPERATIONAL_INTELLIGENCE.md exists + substantial", () => {
        const doc = readRepo("docs/product/PHASE_E9_AI_OPERATIONAL_INTELLIGENCE.md");
    ...
    it("registry registers Phase E9 with explicit closure status", () => {
        ...
        expect(registry).toMatch(/\|\s*(Phase )?E9\s*\|[\s\S]*?(CLOSED|CLOSED_WITH_DEFERRED_ITEMS)/);
    });
    it("registry records the 4 new DEFs opened by E9", () => {
        const registry = readRepo("docs/recovery/MASTER_PHASE_REGISTRY.md");
        for (const def of ["DEF-033", "DEF-034", "DEF-035", "DEF-036"]) {
            expect(registry, `${def} missing from registry`).toMatch(new RegExp(`\\|\\s*${def}\\s*\\|`));
        }
    });
});
