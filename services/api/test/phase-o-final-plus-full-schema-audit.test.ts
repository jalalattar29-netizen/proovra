  ...
  return mod as {
    parsePrismaSchema: (src: string) => {
      models: Array<{
        ...
        indexes: Array<{ kind: string; fields: string[] }>;
      }>;
      enums: Array<{ name: string; dbName: string; values: string[] }>;
    };
    camelToSnake: (s: string) => string;
    snakeToCamel: (s: string) => string;
    expectedPgType: (field: {
      isEnum: boolean;
      dbType: string | null;
      baseType: string;
    }) => { acceptable: string[] | null; expectedUdt: string | null };
    classifyFinding: (f: { kind: string }) => string;
    suggestRepair: (f: Record<string, unknown>) => string | null;
    ...
    ) => { kind: string; actual: string } | null;
    scanMigrationsForRiskPatterns: (
      root: string,
    ) => Array<{ migration: string; table: string; line: number }>;
  };
}
...
describe("O-Final+ — scanMigrationsForRiskPatterns", () => {
  it("detects CREATE TABLE IF NOT EXISTS in real PROOVRA migrations", async () => {
    ...
    const out = scanMigrationsForRiskPatterns(
      REPO_ROOT + "services/api/prisma/migrations",
    );
    // The Phase 16 collaboration migration is the documented root-cause
    // exemplar — must appear with discussion_mentions.
    const phase16 = out.find(
      (r) =>
        r.migration === "20260525100000_add_collaboration_phase16" &&
