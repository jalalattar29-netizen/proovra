...
import { evaluateAlerts, formatPrometheusExposition, newTraceContext, OPERATIONAL_ALERT_THRESHOLDS, parseTraceParentHeader, safeLabelSet, sanitizePromLabelValue, traceParentHeader, withSpan, } from "@proovra/shared";
...
describe("Phase Y — Prometheus exposition", () => {
    ...
    it("attaches global labels to every sample", () => {
        ...
        expect(out).toContain('uptime_seconds{service="proovra-api",env="prod"} 100');
    });
    it("emits labelled samples for vector metrics", () => {
        ...
        expect(out).toContain('lifecycle_transition_total{from="ACTIVE",to="PENDING_DESTRUCTION"} 3');
        expect(out).toContain('lifecycle_transition_total{from="ACTIVE",to="ON_HOLD"} 1');
    });
    ...
    it("escapes backslash, double-quote, and newline in label values", () => {
        ...
        expect(sanitizePromLabelValue("path\\to\\file")).toBe("path\\\\to\\\\file");
...
describe("Phase Y — Trace context", () => {
    ...
    it("serializes to a W3C traceparent header", () => {
        ...
        expect(traceParentHeader(ctx)).toBe("00-0123456789abcdef0123456789abcdef-0123456789abcdef-01");
    });
    it("parses a W3C traceparent header back into a context", () => {
        const ctx = parseTraceParentHeader("00-0123456789abcdef0123456789abcdef-0123456789abcdef-01");
...
describe("Phase Y — withSpan", () => {
    it("reports an `ok` outcome with duration on success", async () => {
        ...
        const result = await withSpan({ name: "test.ok", sink, labels: { kind: "demo" } }, async () => 42);
...
describe("Phase Y — Route wiring", () => {
    ...
    it("exposes /metrics for Prometheus scrape", () => {
        ...
        expect(src).toContain('text/plain; version=0.0.4; charset=utf-8');
    });
...
describe("Phase Y — Worker wiring", () => {
    ...
    it("job outcome instrumentation fires on both completed and failed paths", () => {
        ...
        // both branches.
        const completedMatch = indexSrc.match(/workerInstance\.on\("completed"[\s\S]*?\}\);/);
        const failedMatch = indexSrc.match(/workerInstance\.on\("failed"[\s\S]*?\}\);/);
