...
const MIGRATION = readApi("prisma/migrations/20260629100000_phase328c_enterprise_gap_closure/migration.sql");
...
const REVIEWER_BACKEND = readApi("src/services/reviewer-ops/reviewer-command.service.ts");
const REVIEWER_TSX = readWeb("components/reviewer-experience/ReviewerCommandConsole.tsx");
const GOVERNANCE_TSX = readWeb("components/governance-experience/GovernanceControlPlane.tsx");
...
describe("Phase 32.8C FINAL-3 — Reviewer Ops + Governance gating fix", () => {
    it("Reviewer Ops does NOT early-return a plain text fallback for personal workspace", () => {
        ...
        expect(REVIEWER_TSX).not.toMatch(/<section className="cc-section" data-reviewer-personal>/);
    ...
    it("Governance does NOT early-return a plain text fallback for personal workspace", () => {
        expect(GOVERNANCE_TSX).not.toMatch(/<section className="cc-section" data-governance-personal>/);
    ...
    it("Reviewer Ops backend returns status='ok' (not 'not_applicable') for personal workspace", () => {
        expect(REVIEWER_BACKEND).toMatch(/scope === "PERSONAL"[\s\S]{0,400}status:\s*"ok"/);
        expect(REVIEWER_BACKEND).not.toMatch(/scope === "PERSONAL"[\s\S]{0,400}status:\s*"not_applicable"/);
    });
    it("Reviewer Ops personal banner labels team-only actions explicitly", () => {
        expect(REVIEWER_TSX).toMatch(/Team-only actions[\s\S]{0,200}disabled with clear labels/);
    });
    it("Governance personal banner labels team-only actions explicitly", () => {
        expect(GOVERNANCE_TSX).toMatch(/Team-only mutating actions[\s\S]{0,200}disabled with clear labels/);
    });
});
// =============================================================================
...
describe("Phase 32.8C FINAL-3 — header workspace display", () => {
    ...
    it("workspace switcher menu items render canonical labels, never raw UUIDs", () => {
        ...
        expect(TOPBAR_TSX).toMatch(/org\.displayName \?\?[\s\S]{0,200}Organization workspace/);
    });
});
// =============================================================================
// PART 3 — Schema
...
describe("Phase 32.8C FINAL-3 — schema additions", () => {
    ...
    it("OperationalGraphNode + Edge models exist with unique constraints", () => {
        ...
        expect(SCHEMA).toMatch(/@@unique\(\[teamId,\s*sourceNodeId,\s*targetNodeId,\s*edgeType\]\)/);
    });
...
describe("Phase 32.8C FINAL-3 — migration source-contract", () => {
    ...
    it("bulk_action_items has CASCADE FK to parent run", () => {
        expect(MIGRATION).toMatch(/bulk_operational_action_items_run_id_fkey[\s\S]*?ON DELETE CASCADE/);
    });
});
// =============================================================================
...
describe("Phase 32.8C FINAL-3 — bulk actions", () => {
    ...
    it("ops routes register POST + GET bulk-action endpoints", () => {
        expect(OPS_ROUTES).toMatch(/app\.post\(\s*"\/v1\/ops\/bulk-actions"/);
        expect(OPS_ROUTES).toMatch(/app\.get\(\s*"\/v1\/ops\/bulk-actions\/:id"/);
    });
    it("bulk-action POST uses requireOpsActorAction (mutating gate)", () => {
        const block = OPS_ROUTES.match(/app\.post\(\s*"\/v1\/ops\/bulk-actions"[\s\S]*?\}\s*,\s*\)/);
        expect(block).not.toBeNull();
...
describe("Phase 32.8C FINAL-3 — dashboard envelope wiring", () => {
    ...
    it("each new section lazy-generates then reads (failures wrapped)", () => {
        expect(COMMAND_CENTER).toMatch(/computeReviewerCapacityForWorkspace\(\{\s*teamId\s*\}\)\.catch\(/);
        expect(COMMAND_CENTER).toMatch(/projectOperationalGraphForWorkspace\(\{\s*teamId\s*\}\)\.catch\(/);
        expect(COMMAND_CENTER).toMatch(/recordOrgHealthSnapshotForWorkspace\(\{\s*teamId\s*\}\)\.catch\(/);
    });
    it("personal workspaces get reviewerCapacity status=not_applicable, not unavailable", () => {
        const block = COMMAND_CENTER.match(/async function runReviewerCapacityBoard[\s\S]*?\n\}\s*\n/);
        expect(block).not.toBeNull();
