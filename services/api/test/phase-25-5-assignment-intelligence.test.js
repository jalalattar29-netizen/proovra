...
import { ASSIGNMENT_INELIGIBILITY_CODES, ASSIGNMENT_RANK_REASON_CODES, ASSIGNMENT_RISK_FLAG_CODES, evaluateReviewerEligibility, evaluateTeamBalance, rankReviewerSuggestions, } from "@proovra/shared";
...
describe("Phase 25.5 — reviewer eligibility engine", () => {
    ...
    it("refuses reviewer-restricted workflows when the candidate lacks visibility", () => {
        const r = evaluateReviewerEligibility({ ...baseFacts, reviewerRestricted: true }, {
            ...baseCandidate,
            permissions: {
        ...
        expect(r.eligible).toBe(false);
        expect(r.reasons.map((x) => x.code)).toContain("reviewer_restricted_blocked");
    });
    it("refuses contributor-private workflows when the candidate lacks visibility", () => {
        const r = evaluateReviewerEligibility({ ...baseFacts, contributorPrivate: true }, {
            ...baseCandidate,
            permissions: {
                ...baseCandidate.permissions,
        ...
        expect(r.eligible).toBe(false);
        expect(r.reasons.map((x) => x.code)).toContain("contributor_private_blocked");
    });
    it("refuses legal-hold workflows when the candidate lacks hold permission", () => {
        const r = evaluateReviewerEligibility({ ...baseFacts, hasActiveLegalHold: true }, {
            ...baseCandidate,
            permissions: {
                ...baseCandidate.permissions,
    ...
    it("refuses terminal workflow states (COMPLETED / CANCELLED / REJECTED)", () => {
        for (const state of ["COMPLETED", "CANCELLED", "REJECTED"]) {
            const r = evaluateReviewerEligibility({ ...baseFacts, status: state }, baseCandidate);
            expect(r.eligible).toBe(false);
    ...
    it("refuses everyone except the named escalation owner", () => {
        ...
        expect(evaluateReviewerEligibility(facts, ownerCandidate).eligible).toBe(true);
        const otherResult = evaluateReviewerEligibility(facts, otherCandidate);
        expect(otherResult.eligible).toBe(false);
        expect(otherResult.reasons.map((x) => x.code)).toContain("escalation_owner_conflict");
    });
    it("refuses self-assignment by the actor", () => {
        const r = evaluateReviewerEligibility({ ...baseFacts, actorUserId: "actor-1" }, { ...baseCandidate, reviewerId: "actor-1" });
        expect(r.eligible).toBe(false);
        expect(r.reasons.map((x) => x.code)).toContain("reviewer_self_assignment_blocked");
    });
    ...
    it("refuses when a case-team membership is required but the candidate is not in it", () => {
        const r = evaluateReviewerEligibility({ ...baseFacts, requiredCaseTeamId: "case-team-x" }, { ...baseCandidate, isCaseTeamMember: false });
        expect(r.eligible).toBe(false);
        expect(r.reasons.map((x) => x.code)).toContain("reviewer_scope_blocked");
    });
...
describe("Phase 25.5 — reviewer suggestion ranker", () => {
    ...
    it("score is bounded to [0, 1000]", () => {
        ...
        const r = rankReviewerSuggestions({ ...baseFacts, requiredCaseTeamId: "case-x" }, [everything]);
    ...
    it("overloaded reviewers receive a HIGH-severity risk flag", () => {
        ...
        const { ranked } = rankReviewerSuggestions(baseFacts, [overloaded]);
        const overloadFlag = ranked[0].riskFlags.find((f) => f.code === "reviewer_overloaded");
    ...
    it("recommendation band scales monotonically with reviewer quality", () => {
        ...
        expect(bandOrder[idealResult.recommendationBand]).toBeGreaterThanOrEqual(bandOrder[minimalResult.recommendationBand]);
        // The ideal candidate clears ACCEPTABLE.
        expect(bandOrder[idealResult.recommendationBand]).toBeGreaterThanOrEqual(bandOrder.ACCEPTABLE);
    });
    it("reasons are sorted by delta descending (UI top-N rendering)", () => {
        ...
        for (let i = 1; i < ranked[0].reasons.length; i++) {
            expect(ranked[0].reasons[i].delta).toBeLessThanOrEqual(ranked[0].reasons[i - 1].delta);
        }
    });
    it("imbalance risk flag fires when this reviewer is carrying > 1.5× team mean", () => {
        ...
        expect(heavyResult.riskFlags.some((f) => f.code === "workload_imbalance")).toBe(true);
    });
});
// =============================================================================
...
describe("Phase 25.5 — team balance evaluator", () => {
    function reviewer(id, activeReviews, pressure, lastActivityDaysAgo) {
        return {
            ...baseCandidate,
            reviewerId: id,
            workload: {
                ...
                activeReviews,
                pressure,
                lastActivityAtEpochMs: Date.now() - lastActivityDaysAgo * 24 * 60 * 60 * 1000,
            },
        };
    }
...
describe("Phase 25.5 — source contract + privacy invariants", () => {
    const src = readSource("../../../packages/shared/src/reviewer-assignment.ts");
