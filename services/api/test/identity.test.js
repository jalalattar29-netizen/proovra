...
import { evaluateAccess, } from "../src/services/identity/access-policy.service.js";
...
function memberActor(overrides = {}) {
    return {
        ...
        member: {
            ...
            accessExpiresAtUtc: overrides.accessExpiresAtUtc === undefined
                ? null
...
describe("AccessReviewError + Contributor + External — stable codes", () => {
    it("AccessReviewError codes", () => {
        for (const c of ["review_not_found", "invalid_status_transition", "subject_missing"]) {
            const e = new AccessReviewError(c);
            expect(e.code).toBe(c);
        }
    });
    it("ContributorGovernanceError codes", () => {
        for (const c of ["session_not_found", "session_already_revoked", "session_already_terminal"]) {
...
describe("Access policy engine — member evaluation", () => {
    it("denies a null actor with no_actor", () => {
        ...
        if (!d.allowed)
            expect(d.reason).toBe("no_actor");
    });
    it("denies SUSPENDED members regardless of role", () => {
        const d = evaluateAccess(memberActor({ role: "ADMIN", status: "SUSPENDED" }), { permission: "evidence.read" });
        expect(d.allowed).toBe(false);
        if (!d.allowed)
            expect(d.reason).toBe("member_not_active");
    });
    it("denies REVOKED members regardless of role", () => {
        const d = evaluateAccess(memberActor({ role: "OWNER", status: "REVOKED" }), { permission: "evidence.read" });
        expect(d.allowed).toBe(false);
        if (!d.allowed)
            expect(d.reason).toBe("member_not_active");
    });
    it("denies a member whose accessExpiresAtUtc is in the past", () => {
        const d = evaluateAccess(memberActor({ role: "ADMIN", accessExpiresAtUtc: PAST }), { permission: "evidence.read" });
        expect(d.allowed).toBe(false);
        if (!d.allowed)
            expect(d.reason).toBe("member_access_expired");
    });
    ...
    it("allows via capability grant when role floor does NOT permit", () => {
        const d = evaluateAccess(memberActor({
            role: "MEMBER",
            capabilityGrants: [
                {
                    id: "g1",
        ...
        if (d.allowed) {
            ...
            if (d.via.kind === "capability_grant")
                expect(d.via.grantId).toBe("g1");
        }
    });
...
describe("Access policy engine — service account evaluation", () => {
    it("denies REVOKED service account", () => {
        ...
        expect(d.allowed).toBe(false);
        if (!d.allowed)
            expect(d.reason).toBe("service_account_revoked");
    });
    it("denies DISABLED service account", () => {
        ...
        expect(d.allowed).toBe(false);
        if (!d.allowed)
            expect(d.reason).toBe("service_account_disabled");
    });
    it("denies EXPIRED service account", () => {
        ...
        expect(d.allowed).toBe(false);
        if (!d.allowed)
            expect(d.reason).toBe("service_account_expired");
    });
    it("denies when scope is missing", () => {
        ...
        expect(d.allowed).toBe(false);
        if (!d.allowed)
            expect(d.reason).toBe("service_account_scope_missing");
    });
...
describe("Access policy engine — contributor evaluation", () => {
    it("denies revoked contributor session", () => {
        ...
        expect(d.allowed).toBe(false);
        if (!d.allowed)
            expect(d.reason).toBe("contributor_session_revoked");
    });
    it("denies expired contributor session", () => {
        ...
        expect(d.allowed).toBe(false);
        if (!d.allowed)
            expect(d.reason).toBe("contributor_session_expired");
    });
...
describe("Phase 17 migration — additive only", () => {
    it("uses IF NOT EXISTS / EXCEPTION blocks; never drops or renames", async () => {
        const { readFile } = await import("node:fs/promises");
        const { fileURLToPath } = await import("node:url");
        const src = await readFile(fileURLToPath(new URL("../prisma/migrations/20260526100000_add_identity_phase17/migration.sql", import.meta.url)), "utf8");
...
describe("integrations-auth middleware — Phase 17 hardening", () => {
    it("middleware calls verifyApiKeyDetailed (not the legacy boolean verify)", async () => {
        const { readFile } = await import("node:fs/promises");
        const { fileURLToPath } = await import("node:url");
        const src = await readFile(fileURLToPath(new URL("../src/middleware/integrations-auth.ts", import.meta.url)), "utf8");
