...
describe("Phase 38.9 — sidebar consumes only canonical registry + resolvers", () => {
    it("imports ROUTE_REGISTRY from the canonical registry", () => {
        expect(SIDEBAR).toMatch(/import\s*\{[\s\S]*ROUTE_REGISTRY[\s\S]*\}\s*from\s*["'][^"']*navigation\/routeRegistry["']/);
    });
    it("imports resolveRouteAccess from the canonical resolver", () => {
        expect(SIDEBAR).toMatch(/import\s*\{[\s\S]*resolveRouteAccess[\s\S]*\}\s*from\s*["'][^"']*navigation\/routeAccessResolver["']/);
    });
    it("imports resolveWorkflowExposure from the canonical exposure resolver", () => {
        expect(SIDEBAR).toMatch(/import\s*\{[\s\S]*resolveWorkflowExposure[\s\S]*\}\s*from\s*["'][^"']*navigation\/workflowExposureResolver["']/);
    });
    it("does NOT consume envelope.navigation.groups for rendering", () => {
        ...
        const stripped = SIDEBAR.replace(/\/\*[\s\S]*?\*\//g, "").replace(/\/\/.*$/gm, "");
    ...
    it("workflow is used only for bucketing — never as a sidebar access gate", () => {
        ...
        expect(SIDEBAR).not.toMatch(/if\s*\(\s*(primaryWorkflow|workflow)\s*[!=]==\s*["'][A-Z_]+["']\s*\)\s*\{?\s*return\s+null/m);
    });
...
describe("Phase 38.9 — route registry expansion", () => {
    ...
    it("notifications & integrations are sidebar-eligible (so they appear in nav buckets)", () => {
        const notif = REGISTRY.match(/id:\s*"workspace\.notifications"[\s\S]*?sidebarEligible:\s*(true|false)/);
        const integ = REGISTRY.match(/id:\s*"workspace\.integrations"[\s\S]*?sidebarEligible:\s*(true|false)/);
...
describe("Phase 38.9 — canonical workflow safety statement", () => {
    it("WorkflowSafetyNotice exposes the exact bounded statement constant", () => {
        expect(SAFETY).toMatch(/WORKFLOW_SAFETY_STATEMENT\s*=\s*["']Workflow profiles personalize layout, defaults, and recommendations\. They do not change permissions or remove tools\.["']/);
    });
