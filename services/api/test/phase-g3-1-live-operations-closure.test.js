...
const MIGRATION = readSource("../prisma/migrations/20261003000000_phase_g3_1_notification_preferences/migration.sql");
const PREF_SERVICE = readSource("../src/services/notifications/notification-preferences.service.ts");
const PREF_ROUTES = readSource("../src/routes/notification-preferences.routes.ts");
const ME_INBOX = readSource("../src/routes/me-inbox.routes.ts");
const SERVER = readSource("../src/server.ts");
const SECURITY_CATALOG = readSource("../../../packages/shared/src/security.ts");
const PRESENCE_INDICATOR = readSource("../../../apps/web/components/presence/PresenceIndicator.tsx");
const COLLISION_WARNING = readSource("../../../apps/web/components/presence/CollisionWarning.tsx");
const PREF_PANEL = readSource("../../../apps/web/components/notifications/NotificationPreferencesPanel.tsx");
...
describe("Phase G3.1 — notification preferences schema", () => {
    ...
    it("unique constraint on (userId, teamId, preferenceType, channel)", () => {
        expect(SCHEMA).toMatch(/@@unique\(\[userId,\s*teamId,\s*preferenceType,\s*channel\]\)/);
    });
    it("migration creates the table + enums + unique index", () => {
        ...
        expect(MIGRATION).toMatch(/CREATE TABLE "workspace_notification_preferences"/);
        expect(MIGRATION).toMatch(/CREATE UNIQUE INDEX[\s\S]*?\("user_id",\s*"team_id",\s*"preference_type",\s*"channel"\)/);
    });
});
// ===========================================================================
...
describe("Phase G3.1 — notification preferences service", () => {
    it("exports the bounded vocabularies", () => {
        expect(PREF_SERVICE).toContain('export const NOTIFICATION_PREFERENCE_TYPES =');
        expect(PREF_SERVICE).toContain('export const NOTIFICATION_PREFERENCE_CHANNELS =');
    });
    it("exports listNotificationPreferences + upsertNotificationPreference + isPreferenceEnabled", () => {
        expect(PREF_SERVICE).toMatch(/export (async )?function listNotificationPreferences/);
        expect(PREF_SERVICE).toMatch(/export (async )?function upsertNotificationPreference/);
        expect(PREF_SERVICE).toMatch(/export (async )?function isPreferenceEnabled/);
    });
    it("default for absent EMAIL row is FALSE (opt-in)", () => {
        expect(PREF_SERVICE).toMatch(/return channel\s*===\s*"IN_APP"/);
    });
});
// ===========================================================================
...
describe("Phase G3.1 — notification preferences routes", () => {
    it("registers GET + PUT /v1/me/notification-preferences", () => {
        expect(PREF_ROUTES).toMatch(/app\.get\(\s*"\/v1\/me\/notification-preferences"/);
        expect(PREF_ROUTES).toMatch(/app\.put\(\s*"\/v1\/me\/notification-preferences"/);
    });
...
describe("Phase G3.1 — inbox aggregator respects notification preferences", () => {
    it("imports isPreferenceEnabled from the new service", () => {
        expect(ME_INBOX).toMatch(/import\s*\{[\s\S]*?isPreferenceEnabled[\s\S]*?\}\s*from\s+"\.\.\/services\/notifications\/notification-preferences\.service\.js"/);
    });
    it("filters MENTION items through the preference check", () => {
        expect(ME_INBOX).toMatch(/if \(!\(await isAllowed\(m\.teamId,\s*"MENTION"\)\)\) continue/);
    });
    it("filters ASSIGNED_THREAD items through the preference check", () => {
        expect(ME_INBOX).toMatch(/if \(!\(await isAllowed\(t\.teamId,\s*"ASSIGNED_THREAD"\)\)\) continue/);
    });
...
describe("Phase G3.1 — PresenceIndicator component", () => {
    it("polls the existing /v1/me/presence/heartbeat endpoint every 30 seconds", () => {
        ...
        expect(PRESENCE_INDICATOR).toMatch(/HEARTBEAT_INTERVAL_MS\s*=\s*30_000/);
    });
...
describe("Phase G3.1 — CollisionWarning component", () => {
    it("compares initialUpdatedAtUtc with currentUpdatedAtUtc", () => {
        expect(COLLISION_WARNING).toMatch(/initialUpdatedAtUtc\s*!==\s*currentUpdatedAtUtc/);
    });
...
describe("Phase G3.1 — NotificationPreferencesPanel", () => {
    ...
    it("renders both IN_APP and EMAIL channels per row", () => {
        expect(PREF_PANEL).toMatch(/\["IN_APP",\s*"EMAIL"\]\s*as Channel\[\]/);
    });
