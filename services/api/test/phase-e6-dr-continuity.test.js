...
const FORBIDDEN_INFRA_PATTERNS = [
...
describe("E6 Test 2 — dependency / failure map coverage", () => {
    ...
    it.each(REQUIRED_SUBSYSTEMS)("phase doc dependency/failure map names subsystem %s", (pattern) => {
    ...
    it("phase doc includes a dependency map table", () => {
        ...
        expect(PHASE_DOC).toMatch(/Dependency\s*\|\s*Critical\?\s*\|\s*Failure impact\s*\|\s*Recovery method/i);
    });
});
// ===========================================================================
...
describe("E6 Test 4 — phase doc + runbooks free of fake-infra wording", () => {
    ...
    const sanitisedPhaseDoc = PHASE_DOC.replace(/### 5\. Forbidden fake infrastructure claims[\s\S]*?(?=\n### |\n## )/m, "");
    it.each(FORBIDDEN_INFRA_PATTERNS)("phase doc (outside the forbidden-list declaration) does NOT match %s", (pattern) => {
    ...
    for (const runbookName of RUNBOOK_FILES) {
        describe(`runbook ${runbookName}`, () => {
            ...
            it.each(FORBIDDEN_INFRA_PATTERNS)("does NOT match %s", (pattern) => {
...
describe("E6 Test 5 — Trust Center operational-reliability section extended", () => {
    const section = TRUST_CENTER_SECTIONS.find((s) => s.id === "operational-reliability");
    it("section exists", () => {
    ...
    it.each(FORBIDDEN_INFRA_PATTERNS)("operational-reliability content does NOT match %s", (pattern) => {
    ...
    for (const surface of SAFE_SURFACES) {
        describe(`safe surface — ${surface.label}`, () => {
            ...
            try {
                ...
                throw new Error(`E6 safe-surface reference missing: ${surface.label} at ${surface.path}`);
            }
            it.each(FORBIDDEN_INFRA_PATTERNS)("does NOT match %s", (pattern) => {
                expect(body).not.toMatch(pattern);
            });
...
describe("E6 Test 7 — no secret values exposed in phase doc / runbooks", () => {
    it.each(FORBIDDEN_SECRET_SHAPES)("phase doc does NOT contain a secret matching %s", (pattern) => {
        expect(PHASE_DOC).not.toMatch(pattern);
    });
    for (const runbookName of RUNBOOK_FILES) {
        describe(`runbook ${runbookName}`, () => {
            const body = readRunbook(runbookName);
            it.each(FORBIDDEN_SECRET_SHAPES)("does NOT contain a secret matching %s", (pattern) => {
                expect(body).not.toMatch(pattern);
            });
        });
    }
...
describe("E6 Test 10 — documentation + registry", () => {
    ...
    it("registry registers Phase E6 with explicit closure status", () => {
        ...
        expect(registry).toMatch(/\|\s*(Phase )?E6\s*\|[\s\S]*?(CLOSED|CLOSED_WITH_DEFERRED_ITEMS)/);
    });
