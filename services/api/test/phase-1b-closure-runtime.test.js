...
import { canonicalJson, canonicalUtf8Length, CanonicalJsonError, } from "@proovra/shared";
...
describe("Phase 1B Closure — Ed25519 signature round-trip", () => {
    ...
    it("hash mismatch breaks the round-trip", () => {
        ...
        // or different bytes.
        const tampered = Buffer.from(canonicalJson({ ...payload, assetHash: "0".repeat(64) }), "utf8");
...
// ===========================================================================
const SHARED_CANONICAL = readSource("../../../packages/shared/src/canonical-json.ts");
const SHARED_INDEX = readSource("../../../packages/shared/src/index.ts");
const API_CANONICAL = readSource("../../../services/api/src/services/capture-trust/canonical-json.ts");
const MOBILE_ED25519 = readSource("../../../apps/mobile/src/trust/ed25519.ts");
const MOBILE_SHA = readSource("../../../apps/mobile/src/trust/sha256.ts");
const MOBILE_KEY = readSource("../../../apps/mobile/src/trust/device-key.ts");
const MOBILE_REG = readSource("../../../apps/mobile/src/trust/device-registration.ts");
const MOBILE_ATTEST = readSource("../../../apps/mobile/src/trust/attestation.ts");
const MOBILE_ENV = readSource("../../../apps/mobile/src/trust/envelope.ts");
const MOBILE_QUEUE = readSource("../../../apps/mobile/src/trust/upload-queue.ts");
const MOBILE_INDEX = readSource("../../../apps/mobile/src/trust/index.ts");
const MOBILE_CAPTURE_SCREEN = readSource("../../../apps/mobile/app/(stack)/capture.tsx");
const CITIZEN_ROUTE = readSource("../../../services/api/src/routes/citizen-capture.routes.ts");
const CITIZEN_CLIENT = readSource("../../../apps/web/lib/citizen-capture/citizen-capture-client.ts");
const CITIZEN_PAGE = readSource("../../../apps/web/app/intake/[token]/capture/page.tsx");
const SERVER = readSource("../../../services/api/src/server.ts");
const EVIDENCE_ROUTES = readSource("../../../services/api/src/routes/evidence.routes.ts");
const VERIFY_PAGE = readSource("../../../apps/web/app/verify/[token]/page.tsx");
const VERIFICATION_PACKAGE = readSource("../../../services/worker/src/verification-package.ts");
const WORKER_PROVENANCE_LOADER = readSource("../../../services/worker/src/capture-trust/load-provenance-chain.ts");
const PROCESSOR = readSource("../../../services/worker/src/processor.ts");
...
describe("Phase 1B Closure — wiring: capture screen hook", () => {
    it("capture screen imports the trust runtime barrel", () => {
        expect(MOBILE_CAPTURE_SCREEN).toMatch(/from\s+"\.\.\/\.\.\/src\/trust"/);
    ...
    it("photo capture invokes the trust runtime", () => {
        expect(MOBILE_CAPTURE_SCREEN).toMatch(/takePictureAsync[\s\S]{0,2000}?runTrustCapture/);
    });
    it("video capture invokes the trust runtime", () => {
        expect(MOBILE_CAPTURE_SCREEN).toMatch(/recordAsync[\s\S]{0,3000}?runTrustCapture/);
    });
...
describe("Phase 1B Closure — wiring: verify page surfaces trust", () => {
    it("/public/verify response includes the captureTrust projection", () => {
        expect(EVIDENCE_ROUTES).toMatch(/projectVerifyCaptureTrust/);
...
describe("Phase 1B Closure — wiring: verification package + worker", () => {
    ...
    it("processor calls the loader before createVerificationPackage", () => {
        expect(PROCESSOR).toMatch(/loadProvenanceChainForPackage[\s\S]{0,400}?createVerificationPackage/);
    });
});
// ===========================================================================
// 4 — Helpers
