...
import { TRUST_CENTER_FORBIDDEN_PAGE_PATTERNS, TRUST_CENTER_PAGE_BOUNDARY_CALLOUT, TRUST_CENTER_PAGE_INTRO, TRUST_CENTER_REQUIRED_PHRASES, TRUST_CENTER_SECTIONS, TRUST_CENTER_SECTION_IDS, trustCenterDeepLink, } from "@proovra/shared-evidence-presentation";
// ---------------------------------------------------------------------------
// Helpers
// ---------------------------------------------------------------------------
...
describe("E5 Test 2 — required boundary phrases", () => {
    ...
    it("page-level boundary callout names the relevant external decision-makers", () => {
        expect(TRUST_CENTER_PAGE_BOUNDARY_CALLOUT).toMatch(/court, investigator, regulator, insurer, employer, or expert process/i);
        expect(TRUST_CENTER_PAGE_BOUNDARY_CALLOUT).toMatch(/does not independently prove factual truth/i);
    });
    it.each(TRUST_CENTER_REQUIRED_PHRASES)("rendered page surfaces required phrase: %s", (phrase) => {
        ...
        expect(inPage || inContent, `required boundary phrase "${phrase}" missing from both page and content module`).toBe(true);
    });
    it("AI limitations section states advisory-only verbatim", () => {
        ...
        expect(limitsBlob).toMatch(/does not determine.*?(?:truth|authorship|authenticity|admissibility)/i);
    });
});
// ===========================================================================
...
describe("E5 Test 3 — Trust Center page + content carry no forbidden trust-theatre wording", () => {
    ...
    const contentBody = TRUST_CENTER_CONTENT_SRC.replace(/TRUST_CENTER_FORBIDDEN_PAGE_PATTERNS\s*=\s*\[[\s\S]*?\]\s+as\s+const;/m, "");
    it.each(TRUST_CENTER_FORBIDDEN_PAGE_PATTERNS)("Trust Center page does NOT match %s", (pattern) => {
        expect(TRUST_CENTER_PAGE).not.toMatch(pattern);
    });
    it.each(TRUST_CENTER_FORBIDDEN_PAGE_PATTERNS)("Trust Center content module body does NOT match %s", (pattern) => {
    ...
    for (const surface of SAFE_SURFACES) {
        describe(`safe surface — ${surface.label}`, () => {
            ...
            try {
                ...
                throw new Error(`E5 safe-surface reference missing: ${surface.label} at ${surface.path}`);
            }
            ...
            const sanitised = body
                .replace(/(?:FORBIDDEN|BLOCK(?:ED|LIST)?_PATTERNS|UNSAFE_PHRASES?)\s*=\s*\[[\s\S]*?\]/g, "")
                .replace(/\bforbiddenPatterns\s*=\s*\[[\s\S]*?\];/g, "");
            it.each(TRUST_CENTER_FORBIDDEN_PAGE_PATTERNS)("does NOT match %s", (pattern) => {
...
describe("E5 Test 5 — page consumes the shared content module", () => {
    it("imports TRUST_CENTER_SECTIONS from the shared package", () => {
        expect(TRUST_CENTER_PAGE).toMatch(/from\s+["']@proovra\/shared-evidence-presentation["']/);
    ...
    it("renders sections via the shared content (no hard-coded section titles)", () => {
        ...
        for (const s of TRUST_CENTER_SECTIONS) {
            const titleAsLiteral = new RegExp(`>${s.title.replace(/[.*+?^${}()|[\\\]\\\\]/g, "\\\\$&")}<`);
            expect(TRUST_CENTER_PAGE, `page hard-codes section title "${s.title}" — should render from shared content`).not.toMatch(titleAsLiteral);
        }
    });
...
describe("E5 Test 8 — content aligns with the existing claims-matrix guard", () => {
    it("integrity-model section bullets reuse PROOVRA_ALLOWED_CLAIMS verbatim", () => {
        const integrity = TRUST_CENTER_SECTIONS.find((s) => s.id === "evidence-integrity-model");
        expect(integrity, "integrity-model section missing").toBeTruthy();
        // All five allowed claims appear as bullets.
        const allowedFromMatrix = CLAIMS_MATRIX.match(/PROOVRA_ALLOWED_CLAIMS\s*=\s*\[([\s\S]*?)\]\s*as\s*const;/);
        expect(allowedFromMatrix).toBeTruthy();
        const allowedClaims = Array.from(allowedFromMatrix[1].matchAll(/"([^"]+)"/g)).map((m) => m[1]);
        expect(allowedClaims.length).toBeGreaterThanOrEqual(5);
        for (const claim of allowedClaims) {
            expect(integrity.bullets.includes(claim), `allowed claim missing as integrity bullet: ${claim}`).toBe(true);
        }
    });
    it("integrity-model section limitations include each PROOVRA_FORBIDDEN_CLAIMS entry", () => {
        const integrity = TRUST_CENTER_SECTIONS.find((s) => s.id === "evidence-integrity-model");
        const forbiddenFromMatrix = CLAIMS_MATRIX.match(/PROOVRA_FORBIDDEN_CLAIMS\s*=\s*\[([\s\S]*?)\]\s*as\s*const;/);
        expect(forbiddenFromMatrix).toBeTruthy();
        const forbiddenClaims = Array.from(forbiddenFromMatrix[1].matchAll(/"([^"]+)"/g)).map((m) => m[1]);
        expect(forbiddenClaims.length).toBeGreaterThanOrEqual(10);
        for (const claim of forbiddenClaims) {
            const looksFor = `PROOVRA does not claim: ${claim}`;
            expect(integrity.limitations.includes(looksFor), `forbidden claim not pinned as Trust Center limitation: ${claim}`).toBe(true);
        }
    });
});
// ===========================================================================
...
describe("E5 Test 10 — documentation + registry", () => {
    ...
    it("registry registers Phase E5 with explicit closure status", () => {
        ...
        expect(registry).toMatch(/\|\s*(Phase )?E5\s*\|[\s\S]*?(CLOSED|CLOSED_WITH_DEFERRED_ITEMS)/);
    });
});
