...
const ENFORCEMENT_SVC = readFileSync(apiPath("src/services/security/login-mfa-enforcement.service.ts"), "utf8");
const POLICY_SVC = readFileSync(apiPath("src/services/identity-security/mfa-policy.service.ts"), "utf8");
...
describe("R8.1.5 — verified-email recovery preflight", () => {
    ...
    it("test 1: createRecoveryRequest writes status EMAIL_VERIFICATION_PENDING", () => {
        expect(RECOVERY_SVC).toMatch(/status:\s*["']EMAIL_VERIFICATION_PENDING["']/);
        // The Prisma model's default also matches so a row inserted
        // without an explicit status lands in the email-pending state.
        expect(SCHEMA).toMatch(/status\s+MfaRecoveryRequestStatus\s+@default\(EMAIL_VERIFICATION_PENDING\)/);
        // The status enum lists EMAIL_VERIFICATION_PENDING.
        expect(SCHEMA).toMatch(/enum MfaRecoveryRequestStatus\s*\{[\s\S]*?EMAIL_VERIFICATION_PENDING/);
    });
    // ---------------------------------------------------------------------------
    ...
    it("test 3: successful verify clears the hash + expiry so the same token cannot be re-used", () => {
        ...
        const verifyBlock = RECOVERY_SVC.match(/export async function verifyRecoveryRequestEmail[\s\S]*?\n\}/);
        ...
        expect(verifyBlock[0]).toMatch(/where:\s*\{[\s\S]*?status:\s*["']EMAIL_VERIFICATION_PENDING["']/);
    });
    // ---------------------------------------------------------------------------
    ...
    it("test 4: approveRecoveryRequest refuses with `request_not_email_verified` when status is EMAIL_VERIFICATION_PENDING", () => {
        expect(RECOVERY_SVC).toMatch(/request_not_email_verified/);
        // Branch order matters: the email-pending check happens BEFORE
        // the PENDING_ADMIN_REVIEW assertion.
        const approveBlock = RECOVERY_SVC.match(/export async function approveRecoveryRequest[\s\S]*?\n\}/);
...
describe("R8.1.5 — user self-cancel recovery", () => {
    ...
    it("test 6: cancelRecoveryRequest flips status to CANCELLED for owner from either preflight state", () => {
        const block = RECOVERY_SVC.match(/export async function cancelRecoveryRequest[\s\S]*?\n\}/);
...
describe("R8.1.5 — admin approval is not a session bypass", () => {
    ...
    it("test 10: rejectRecoveryRequest appends a platform audit log row", () => {
        const block = RECOVERY_SVC.match(/export async function rejectRecoveryRequest[\s\S]*?\n\}/);
        expect(block).toBeTruthy();
        expect(block[0]).toMatch(/appendPlatformAuditLog\(\{[\s\S]*?action:\s*["']mfa\.recovery\.rejected["']/);
    });
});
// =============================================================================
...
describe("R8.1.5 — per-org MFA enforcement fail-mode", () => {
    ...
    it("test 12: PATCH policy enforces the three bounded values via zod enum", () => {
        expect(ADMIN_ROUTES).toMatch(/mfaEnforcementFailMode:\s*z\s*\.\s*enum\(\s*\[\s*["']SMART["'],\s*["']FAIL_OPEN["'],\s*["']FAIL_CLOSED["']\s*\]/);
...
describe("R8.1.5 — scope guards: no parallel auth / workflow / tenant leak", () => {
    ...
    it("test 18: every state-mutating recovery operation scopes by teamId or owner", () => {
        ...
        expect(RECOVERY_SVC).toMatch(/teamMember\.findFirst\(\{\s*where:\s*\{[\s\S]{0,200}teamId:\s*input\.teamId/);
        // The approve admin check requires ACTIVE OWNER/ADMIN on the
        // request's team — never a different team.
        expect(RECOVERY_SVC).toMatch(/teamMember\.findFirst\(\{\s*where:\s*\{[\s\S]{0,200}teamId:\s*row\.teamId/);
