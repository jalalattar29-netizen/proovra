...
describe("useGlobalRuntimeState (Phase 28-J)", () => {
    const src = readSource("../../../apps/web/lib/useGlobalRuntimeState.ts");
    it("polls the three runtime-awareness endpoints", () => {
        ...
        expect(src).toMatch(/\/v1\/reviewer-ops\/escalations\?teamId=[^&]+&status=OPEN/);
    });
    ...
    it("derives severity with the documented precedence (CRITICAL > INCIDENT_ACTIVE > DEGRADED > UNKNOWN > HEALTHY)", () => {
        ...
        const fn = src.slice(src.indexOf("function deriveSeverity"), src.indexOf("function deriveSeverity") + 900);
...
describe("GlobalRuntimeIndicator (Phase 28-J)", () => {
    const src = readSource("../../../apps/web/components/operational/GlobalRuntimeIndicator.tsx");
...
describe("AppTopbarV2 — runtime indicator wiring", () => {
    const src = readSource("../../../apps/web/components/app-shell-v2/AppTopbarV2.tsx");
    it("imports GlobalRuntimeIndicator from the operational barrel", () => {
        expect(src).toMatch(/import\s*\{[^}]*GlobalRuntimeIndicator[^}]*\}\s*from\s*"\.\.\/operational"/);
    });
...
describe("AppSidebarV2 — IA + operational badges", () => {
    ...
    it("consumes the real useGlobalRuntimeState hook (real runtime data, not fake)", () => {
        expect(src).toMatch(/import\s*\{[\s\S]*?useGlobalRuntimeState[\s\S]*?\}\s*from\s*"\.\.\/\.\.\/lib\/useGlobalRuntimeState"/);
    ...
    it("escalation badge tone scales with severity (CRITICAL > HIGH > WARNING)", () => {
        ...
        expect(src).toMatch(/escalations\.some\(\s*\(e\)\s*=>\s*e\.severity === "CRITICAL"[\s\S]*?\)[\s\S]*?"critical"[\s\S]*?"high"[\s\S]*?"warning"/);
    });
    ...
    it("never emits a badge when the count is zero (no decorative chips)", () => {
        expect(src).toMatch(/runtime\.counts\.escalations > 0/);
...
describe("Sparkline (Phase 28-J)", () => {
    const src = readSource("../../../apps/web/components/operational/Sparkline.tsx");
...
describe("Observability page — Phase 28-J maturity", () => {
    ...
    it("imports Sparkline from the operational barrel", () => {
        expect(src).toMatch(/import\s*\{[\s\S]*?Sparkline[\s\S]*?\}\s*from\s*"[\.\/]+components\/operational"/);
    });
    ...
    it("worker + queue tiles read from real readiness subsystem entries", () => {
        ...
        expect(src).toMatch(/readiness\?\.subsystems\.find\(\(s\) => s\.id === "workers"\)/);
        expect(src).toMatch(/readiness\?\.subsystems\.find\(\(s\) => s\.id === "queues"\)/);
    });
    ...
    it("sparkline delta is computed from the rolling buffer, not a magic constant", () => {
        expect(src).toMatch(/const\s+delta\s*=\s*buffer\.length\s*>=\s*2\s*\?\s*last\s*-\s*first\s*:\s*null/);
    });
...
describe("Phase 28-J cross-surface invariants", () => {
    ...
    it("no operational surface uses dark-shell rgba(255,255,255,...) text on a light page", () => {
        ...
        for (const rel of LIGHT_SURFACES) {
            const src = readSource(rel);
            expect(src, `${rel} mixes dark-shell tokens with a light surface`).not.toMatch(/rgba\(\s*255\s*,\s*255\s*,\s*255\s*,/);
        }
    });
});
