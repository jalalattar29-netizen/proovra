...
const SIDEBAR_PATH = resolve(WEB_ROOT, "components/app-shell-v2/AppSidebarV2.tsx");
// ---------------------------------------------------------------------------
// Route registry
// ---------------------------------------------------------------------------
describe("Phase 10 — route registry route-ownership", () => {
    ...
    it("no registry entry uses the legacy href '/teams' (only a redirect target, not a canonical href)", () => {
        ...
        expect(offenders, `legacy /teams href found in registry: ${offenders.join(", ")}`).toEqual([]);
    });
});
// ---------------------------------------------------------------------------
