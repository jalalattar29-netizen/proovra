...
import { ACCESS_REVIEW_CAMPAIGN_KINDS, ACCESS_REVIEW_CAMPAIGN_STATES, ACCESS_REVIEW_ITEM_DECISIONS, AI_DISCLOSURE_SECTIONS, CROSS_ORG_REVIEW_STATES, DELEGATED_ADMIN_GRANT_STATES, DELEGATED_ADMIN_TIERS, DEPARTMENT_STATES, GOVERNANCE_DASHBOARD_SCHEMA_VERSION, GOVERNANCE_POLICY_ENFORCEMENT_MODES, GOVERNANCE_POLICY_KINDS, GOVERNANCE_POLICY_SCOPES, GOVERNANCE_POLICY_STATES, METHODOLOGY_SECTIONS, ORGANIZATION_STATES, SECURITY_SECTIONS, STATUS_COMPONENT_HEALTHS, STATUS_COMPONENT_KEYS, STATUS_INCIDENT_SEVERITIES, STATUS_INCIDENT_STATES, STATUS_PAGE_SCHEMA_VERSION, SUBPROCESSOR_DATA_CATEGORIES, SUBPROCESSOR_STATES, TRUST_AND_GOVERNANCE_LIMITATIONS, TRUST_ARTICLE_KINDS, TRUST_ARTICLE_STATES, TRUST_CENTER_SECTIONS, } from "@proovra/shared";
// ---------------------------------------------------------------------------
// 1. Shared contracts.
// ---------------------------------------------------------------------------
describe("Phase 4A — shared contracts", () => {
    ...
    it("Trust article kinds + states are bounded", () => {
        expect([...TRUST_ARTICLE_KINDS].sort()).toEqual(["AI_DISCLOSURE", "METHODOLOGY", "SECURITY", "TRUST_CENTER"]);
        expect([...TRUST_ARTICLE_STATES].sort()).toEqual(["DEPRECATED", "DRAFT", "PUBLISHED"]);
    });
    it("Delegated admin tiers cover the 7 enterprise roles", () => {
        ...
        expect([...DELEGATED_ADMIN_GRANT_STATES].sort()).toEqual(["ACTIVE", "EXPIRED", "REVOKED"]);
    });
    it("Governance policy kinds cover the 6 required categories", () => {
        ...
        expect([...GOVERNANCE_POLICY_SCOPES].sort()).toEqual(["DEPARTMENT", "ORGANIZATION", "WORKSPACE"]);
        expect([...GOVERNANCE_POLICY_ENFORCEMENT_MODES].sort()).toEqual(["AUDIT_ONLY", "BLOCK", "WARN"]);
        expect([...GOVERNANCE_POLICY_STATES].sort()).toEqual(["ACTIVE", "DEPRECATED", "DRAFT"]);
    });
    it("Access review campaign kinds + states + decisions bounded", () => {
        ...
        expect([...ACCESS_REVIEW_CAMPAIGN_STATES].sort()).toEqual(["CANCELLED", "CLOSED", "DRAFT", "OPEN"]);
        expect([...ACCESS_REVIEW_ITEM_DECISIONS].sort()).toEqual(["APPROVED", "ESCALATED", "PENDING", "REVOKED"]);
    });
    it("Cross-org review states + organization + department bounded", () => {
        expect([...CROSS_ORG_REVIEW_STATES].sort()).toEqual(["ACCEPTED", "DECLINED", "EXPIRED", "INVITED", "REVOKED"]);
        expect([...ORGANIZATION_STATES].sort()).toEqual(["ACTIVE", "ARCHIVED", "SUSPENDED"]);
    ...
    it("Status page vocabulary bounded", () => {
        ...
        expect([...STATUS_INCIDENT_SEVERITIES].sort()).toEqual(["CRITICAL", "INFO", "MAJOR", "MINOR"]);
    ...
    it("Subprocessor states + data categories bounded", () => {
        expect([...SUBPROCESSOR_STATES].sort()).toEqual(["ACTIVE", "DEPRECATED", "PROPOSED", "REMOVED"]);
        expect(SUBPROCESSOR_DATA_CATEGORIES.length).toBeGreaterThanOrEqual(8);
    });
    it("Governance dashboard schema version pinned", () => {
        expect(GOVERNANCE_DASHBOARD_SCHEMA_VERSION).toBe("PROOVRA_GOVERNANCE_DASHBOARD_V1");
    });
    it("Trust + governance limitations are surfaced", () => {
        expect(TRUST_AND_GOVERNANCE_LIMITATIONS.length).toBeGreaterThanOrEqual(5);
        expect(TRUST_AND_GOVERNANCE_LIMITATIONS).toContain("DELEGATED_ADMIN_TIERS_ARE_ENFORCED_SERVER_SIDE");
    });
});
// ---------------------------------------------------------------------------
// 2. Prisma additions + Phase O-Final compliant migration.
...
describe("Phase 4A — Prisma + migration", () => {
    ...
    it("migration is Phase O-Final compliant", () => {
        const migPath = fileURLToPath(new URL("../prisma/migrations/20261220000000_phase_4a_trust_and_governance/migration.sql", import.meta.url));
...
describe("Phase 4A — service module surface", () => {
    ...
    it("delegated-admin service exposes grant / revoke / list / hasTier", async () => {
        const m = await import("../src/services/governance/delegated-admin.service.js");
    ...
    it("governance-policy service exposes create / activate / assign / resolve", async () => {
        const m = await import("../src/services/governance/governance-policy.service.js");
    ...
    it("access-review service exposes campaign + item flow", async () => {
        const m = await import("../src/services/governance/access-review.service.js");
    ...
    it("cross-org-review service exposes invite / accept / decline / revoke", async () => {
        const m = await import("../src/services/governance/cross-org-review.service.js");
    ...
    it("governance-dashboard service exposes the bounded projector", async () => {
        const m = await import("../src/services/governance/governance-dashboard.service.js");
    ...
    it("verification-manifest service exposes the 5 closure writers", async () => {
        const m = await import("../src/services/trust/trust-verification-manifest.service.js");
...
describe("Phase 4A — delegated admin tier resolver", () => {
    it("GLOBAL_ADMIN matches every scope", async () => {
        const { hasDelegatedTier } = await import("../src/services/governance/delegated-admin.service.js");
        const prismaStub = {
            delegatedAdminGrant: {
                findMany: async () => [
                    {
...
describe("Phase 4A — governance policy inheritance + override", () => {
    it("Workspace override wins over Org assignment for the same policy kind+slug", async () => {
        const { resolveEffectivePolicies } = await import("../src/services/governance/governance-policy.service.js");
        const orgPolicy = {
            id: "policy-org",
            kind: "SECURITY",
            slug: "mfa",
...
describe("Phase 4A — HTTP routes", () => {
    it("trust-and-governance.routes registers every required endpoint", () => {
        const path = fileURLToPath(new URL("../src/routes/trust-and-governance.routes.ts", import.meta.url));
...
describe("Phase 4A — UI surfaces + nav registry", () => {
    it("Trust Center landing exists with bounded anchors", () => {
        const path = fileURLToPath(new URL("../../../apps/web/app/(app)/trust-center/page.tsx", import.meta.url));
        const src = readFileSync(path, "utf8");
    ...
    it("Methodology page exists", () => {
        const path = fileURLToPath(new URL("../../../apps/web/app/(app)/trust-center/methodology/page.tsx", import.meta.url));
        const src = readFileSync(path, "utf8");
        expect(src).toContain("METHODOLOGY");
        expect(src).toContain("Verification Methodology Center");
    });
    it("AI Disclosure page exists", () => {
        const path = fileURLToPath(new URL("../../../apps/web/app/(app)/trust-center/ai-disclosure/page.tsx", import.meta.url));
        const src = readFileSync(path, "utf8");
        expect(src).toContain("AI_DISCLOSURE");
        expect(src).toContain("AI Disclosure Center");
    });
    it("Security Center page exists", () => {
        const path = fileURLToPath(new URL("../../../apps/web/app/(app)/trust-center/security/page.tsx", import.meta.url));
        const src = readFileSync(path, "utf8");
    ...
    it("Subprocessors page exists with bounded table anchor", () => {
        const path = fileURLToPath(new URL("../../../apps/web/app/(app)/trust-center/subprocessors/page.tsx", import.meta.url));
        const src = readFileSync(path, "utf8");
    ...
    it("Status Page exists with bounded component + incident anchors", () => {
        const path = fileURLToPath(new URL("../../../apps/web/app/(app)/trust-center/status/page.tsx", import.meta.url));
        const src = readFileSync(path, "utf8");
    ...
    it("Governance Platform dashboard exists with bounded tiles", () => {
        const path = fileURLToPath(new URL("../../../apps/web/app/(app)/governance-platform/page.tsx", import.meta.url));
        const src = readFileSync(path, "utf8");
    ...
    it("Governance Platform subpages exist", () => {
        for (const sub of [
            ...
            const path = fileURLToPath(new URL(`../../../apps/web/app/(app)/governance-platform/${sub}/page.tsx`, import.meta.url));
            const src = readFileSync(path, "utf8");
...
describe("Phase 4A — report section integration", () => {
    it("intelligence-summary section source declares trustReferences shape + renders the block", () => {
        const path = fileURLToPath(new URL("../../worker/src/report-v2/sections/intelligence-summary.ts", import.meta.url));
        const src = readFileSync(path, "utf8");
...
describe("Phase 4A — module resolution", () => {
    it("every Phase 4A service module imports cleanly", async () => {
        await expect(import("../src/services/trust/trust-center.service.js")).resolves.toBeDefined();
        await expect(import("../src/services/trust/subprocessor.service.js")).resolves.toBeDefined();
        await expect(import("../src/services/trust/status-page.service.js")).resolves.toBeDefined();
        await expect(import("../src/services/trust/trust-verification-manifest.service.js")).resolves.toBeDefined();
        await expect(import("../src/services/governance/department.service.js")).resolves.toBeDefined();
        await expect(import("../src/services/governance/delegated-admin.service.js")).resolves.toBeDefined();
        await expect(import("../src/services/governance/governance-policy.service.js")).resolves.toBeDefined();
        await expect(import("../src/services/governance/access-review.service.js")).resolves.toBeDefined();
        await expect(import("../src/services/governance/cross-org-review.service.js")).resolves.toBeDefined();
        await expect(import("../src/services/governance/governance-dashboard.service.js")).resolves.toBeDefined();
    });
});
