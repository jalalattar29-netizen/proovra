...
const READINESS_PANEL = readWeb("app/(app)/capture/_lib/CaptureReadinessPanel.tsx");
...
describe("Phase 38.14 — capture readiness layer", () => {
    ...
    it("declares a bounded readiness level vocabulary (draft|developing|ready)", () => {
        expect(READINESS).toMatch(/ReadinessLevel\s*=\s*"draft"\s*\|\s*"developing"\s*\|\s*"ready"/);
    });
    ...
    it("operational-tone label set — no legal/forensic overclaim labels", () => {
        ...
        for (const pattern of FORBIDDEN) {
            expect(READINESS, `readiness data must not match ${pattern}`).not.toMatch(pattern);
            expect(READINESS_PANEL, `readiness panel must not match ${pattern}`).not.toMatch(pattern);
        }
    });
});
describe("Phase 38.14 — CaptureReadinessPanel contract", () => {
    ...
    it("emits a per-criterion satisfied attribute", () => {
        ...
        expect(READINESS_PANEL).toMatch(/data-capture-readiness-criterion-satisfied=/);
    });
    ...
    it("explicitly documents itself as informational + non-blocking", () => {
        ...
        // finalization.
        expect(READINESS_PANEL).toMatch(/never blocks finalization|operational completeness only/i);
    });
    ...
    it("capture page does NOT use the readiness signal to block finalization", () => {
        ...
        expect(CAPTURE).not.toMatch(/if\s*\(\s*!?readiness[\s\S]{0,80}return\s+null/);
        expect(CAPTURE).not.toMatch(/if\s*\(\s*readiness\.level\s*!==\s*"ready"\s*\)\s*\{?\s*(disable|return null|throw)/);
    });
});
// =============================================================================
...
describe("Phase 38.14 — dashboard subroute migrations", () => {
    ...
    it("dashboard/api-keys page file is deleted (canonical surface is /integrations)", () => {
        ...
        const apiKeysPath = url.fileURLToPath(new URL(`../../../apps/web/app/(app)/dashboard/api-keys/page.tsx`, import.meta.url));
