...
const REPAIR_MIGRATION = "services/api/prisma/migrations/20261006000000_phase_o_final_production_column_repair/migration.sql";
...
describe("O-Final — additive repair migration", () => {
    ...
    it("repairs the sister at-risk tables documented in the brief", () => {
        ...
        for (const table of [
            ...
            expect.soft(src).toMatch(new RegExp(`ALTER\\s+TABLE\\s+IF\\s+EXISTS\\s+"${table}"`, "i"));
        }
    });
});
// ---------------------------------------------------------------------------
...
describe("O-Final — production-variant coverage", () => {
    ...
    it("adds every Prisma-required column to discussion_mentions", () => {
        ...
        for (const col of [
            ...
            expect.soft(src).toMatch(new RegExp(`ADD\\s+COLUMN\\s+IF\\s+NOT\\s+EXISTS\\s+"${col}"`));
        }
    });
    it("deterministically backfills mentioned_user_id from legacy user_id", () => {
        ...
        expect(src).toMatch(/information_schema\.columns[\s\S]*?column_name\s*=\s*'user_id'/i);
        expect(src).toMatch(/UPDATE\s+"discussion_mentions"[\s\S]*?SET\s+"mentioned_user_id"\s*=\s*"user_id"/i);
    ...
    it("backfills thread_id via JOIN on discussion_messages (FK ensures totality)", () => {
        expect(src).toMatch(/information_schema\.tables[\s\S]*?table_name\s*=\s*'discussion_messages'/i);
        expect(src).toMatch(/UPDATE\s+"discussion_mentions"\s+dm[\s\S]*?FROM\s+"discussion_messages"\s+m[\s\S]*?WHERE\s+m\."id"\s*=\s*dm\."message_id"/i);
        expect(src).toMatch(/AND\s+dm\."thread_id"\s+IS\s+NULL/i);
    });
    it("backfills created_at from legacy created_at_utc", () => {
        expect(src).toMatch(/information_schema\.columns[\s\S]*?column_name\s*=\s*'created_at_utc'/i);
        expect(src).toMatch(/SET\s+"created_at"\s*=\s*"created_at_utc"/i);
    });
    ...
    it("every backfill UPDATE is wrapped in a column-existence DO block", () => {
        ...
        // a DO block.
        const executeUpdates = (src.match(/EXECUTE\s+\$upd\$[\s\S]*?UPDATE\s+"discussion_mentions"[\s\S]*?\$upd\$/gi) ?? []).length;
    ...
    it("every CREATE INDEX is wrapped in a column-existence DO block", () => {
        ...
        const executeIndexes = (stripped.match(/EXECUTE\s+'CREATE\s+(?:UNIQUE\s+)?INDEX/gi) ?? []).length;
...
describe("O-Final — index column safety", () => {
    ...
    function columnsAddedByMigrationToTable(src, table) {
        const block = new RegExp(`ALTER\\s+TABLE\\s+IF\\s+EXISTS\\s+"${table}"([\\s\\S]*?)(?=ALTER\\s+TABLE|DO\\s+\\$\\$|$)`, "gi");
        const out = new Set();
    ...
    it("every CREATE INDEX references only columns this migration adds or pre-existing known-safe columns", () => {
        const indexes = [
            ...src.matchAll(/CREATE\s+(?:UNIQUE\s+)?INDEX\s+IF\s+NOT\s+EXISTS\s+"[\w]+"\s+ON\s+"(\w+)"\s+\(([^)]+)\)/gi),
        ];
        ...
        for (const m of indexes) {
            ...
            for (const col of cols) {
                expect.soft(allowed.has(col), `CREATE INDEX on ${table}(${colList}) references column "${col}" that this migration does not add and is not in the known-pre-existing allowlist.`).toBe(true);
            }
        }
    });
});
