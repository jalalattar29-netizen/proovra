...
import { POLICY_ASSIGNMENT_PRECEDENCE, POLICY_ASSIGNMENT_SCOPES, POLICY_DETECTION_RULE_ACTIONS, REDACTION_POLICY_ACTIVITY_CODES, REDACTION_POLICY_DOCUMENT_SCHEMA_VERSION, REDACTION_POLICY_VERSION_STATES, REDACTION_POLICY_VERSION_TRANSITIONS, VIDEO_FRAME_EXTRACTORS, VIDEO_FRAME_SAMPLE_MS, VIDEO_FRAME_SAMPLE_PRESETS, VIDEO_TIMELINE_EVENT_CODES, VIDEO_TIMELINE_LAYERS, VIDEO_TRACK_BULK_OPS, VIDEO_TRACK_KINDS, VIDEO_TRACK_STATES, isAllowedPolicyVersionTransition, } from "@proovra/shared";
...
const MIGRATION = readSource("../../../services/api/prisma/migrations/20261201000000_phase_3a_elite_closure_policy_video/migration.sql");
const POLICY_STORE = readSource("../../../services/api/src/services/redaction/redaction-policy-store.service.ts");
const POLICY_SHIM = readSource("../../../services/api/src/services/redaction/redaction-policy.service.ts");
const POLICY_MANIFEST = readSource("../../../services/api/src/services/redaction/policy-verification-manifest.service.ts");
const VIDEO_FRAME = readSource("../../../services/api/src/services/redaction/video/video-frame.service.ts");
const VIDEO_TRACK = readSource("../../../services/api/src/services/redaction/video/video-track.service.ts");
const VIDEO_TIMELINE = readSource("../../../services/api/src/services/redaction/video/video-timeline.service.ts");
const VIDEO_TRACKING = readSource("../../../services/api/src/services/redaction/video/video-tracking.service.ts");
const VIDEO_VERIFICATION = readSource("../../../services/api/src/services/redaction/video/video-verification-manifest.service.ts");
const ROUTES = readSource("../../../services/api/src/routes/redaction.routes.ts");
const REPORT_SECTION = readSource("../../../services/worker/src/report-v2/sections/video-intelligence.ts");
const UI_POLICY = readSource("../../../apps/web/app/(app)/redaction/policy/page.tsx");
const UI_VIDEO_WORKSPACE = readSource("../../../apps/web/components/redaction/VideoReviewWorkspace.tsx");
// =============================================================================
// 1. Shared contracts
// =============================================================================
describe("Phase 3A Elite — shared contracts", () => {
    ...
    it("assignment scopes + bounded precedence catalog", () => {
        expect([...POLICY_ASSIGNMENT_SCOPES].sort()).toEqual(["CASE", "GLOBAL", "PROJECT", "WORKSPACE"].sort());
        expect(POLICY_ASSIGNMENT_PRECEDENCE.PROJECT).toBeGreaterThan(POLICY_ASSIGNMENT_PRECEDENCE.CASE);
        expect(POLICY_ASSIGNMENT_PRECEDENCE.CASE).toBeGreaterThan(POLICY_ASSIGNMENT_PRECEDENCE.WORKSPACE);
        expect(POLICY_ASSIGNMENT_PRECEDENCE.WORKSPACE).toBeGreaterThan(POLICY_ASSIGNMENT_PRECEDENCE.GLOBAL);
    });
    ...
    it("policy document schema version pin matches V1", () => {
        expect(REDACTION_POLICY_DOCUMENT_SCHEMA_VERSION).toBe("PROOVRA_REDACTION_POLICY_V1");
    });
...
describe("Phase 3A Elite — Prisma + migration", () => {
    ...
    it("video_frames is uniquely keyed per (team, evidence, frameIndex)", () => {
        expect(SCHEMA).toMatch(/@@unique\(\[teamId, evidenceId, frameIndex\]\)/);
        expect(SCHEMA).toMatch(/@@unique\(\[trackId, frameId\]\)/);
    });
    it("migration is Phase O-Final compliant (plain CREATE TABLE + guards)", () => {
        ...
        expect(MIGRATION).toMatch(/REFERENCES "redaction_policies"[\s\S]+CASCADE/);
...
describe("Phase 3A Elite — Prisma-backed policy store", () => {
    ...
    it("transitions enforce separation of duties for APPROVE + PUBLISH", () => {
        expect(POLICY_STORE).toMatch(/authoredByUserId === input\.actorUserId/);
    });
...
describe("Phase 3A Elite — policy HTTP routes", () => {
    ...
    it("policy writes require redaction.administer; reads require redaction.view", () => {
        expect(ROUTES).toMatch(/\/v1\/redaction\/policies"[\s\S]+?gate\(reply, ctx, "redaction\.administer"\)/);
        expect(ROUTES).toMatch(/\/v1\/redaction\/policy\/effective[\s\S]+?gate\(reply, ctx, "redaction\.view"\)/);
    });
});
// =============================================================================
...
describe("Phase 3A Elite — video HTTP routes", () => {
    ...
    it("video review-tier routes require redaction.detection.review", () => {
        expect(ROUTES).toMatch(/\/v1\/redaction\/video-tracks\/:id\/decision[\s\S]+?gate\(reply, ctx, "redaction\.detection\.review"\)/);
        expect(ROUTES).toMatch(/\/v1\/redaction\/videos\/:evidenceId\/decisions\/range[\s\S]+?gate\(reply, ctx, "redaction\.detection\.review"\)/);
    });
});
// =============================================================================
...
describe("Phase 3A Elite — cross-surface integrations", () => {
    ...
    it("report builder ships a video-intelligence section", () => {
        expect(REPORT_SECTION).toMatch(/export function renderVideoIntelligenceSection/);
    ...
    it("video verification manifest writer surfaces bounded counts only", () => {
        expect(VIDEO_VERIFICATION).toMatch(/export async function buildVideoTrackingVerificationEntry/);
    });
...
describe("Phase 3A Elite — runtime helpers", () => {
    it("IoU helper returns expected values for overlapping bboxes", () => {
        expect(iou({ x: 0, y: 0, width: 0.5, height: 0.5 }, { x: 0, y: 0, width: 0.5, height: 0.5 })).toBeCloseTo(1.0, 5);
        expect(iou({ x: 0, y: 0, width: 0.5, height: 0.5 }, { x: 0.5, y: 0.5, width: 0.5, height: 0.5 })).toBeCloseTo(0, 5);
        const partial = iou({ x: 0, y: 0, width: 0.4, height: 0.4 }, { x: 0.2, y: 0.2, width: 0.4, height: 0.4 });
