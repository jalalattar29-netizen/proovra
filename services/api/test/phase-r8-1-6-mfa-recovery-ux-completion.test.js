...
const ADMIN_SPA_PATH = webPath("app/(app)/security-center/mfa-recovery/page.tsx");
const RECOVERY_SVC_PATH = apiPath("src/services/security/mfa-recovery-request.service.ts");
...
describe("R8.1.6 — user-facing recovery verify page", () => {
    ...
    it("test 3: page POSTs to /v1/identity/mfa/recovery-requests/:id/verify-email", () => {
        expect(VERIFY_PAGE).toMatch(/\/v1\/identity\/mfa\/recovery-requests\/\$\{encodeURIComponent\([^)]+\)\}\/verify-email/);
...
describe("R8.1.6 — per-account recovery throttle", () => {
    ...
    it("test 6: createRecoveryRequest refuses with `rate_limited` when the user is over the per-account limit", () => {
        ...
        expect(RECOVERY_SVC).toMatch(/MFA_RECOVERY_PER_ACCOUNT_WINDOW_SECONDS\s*=\s*24\s*\*\s*60\s*\*\s*60/);
        expect(RECOVERY_SVC).toMatch(/recentCount\s*>=\s*MFA_RECOVERY_PER_ACCOUNT_LIMIT/);
    ...
    it("test 7: throttle uses prisma.count on mfaRecoveryRequest (no in-process Map / Set / counter)", () => {
        ...
        expect(RECOVERY_SVC).toMatch(/prisma\.mfaRecoveryRequest\.count\(\{\s*where:\s*\{\s*userId:\s*input\.userId,\s*createdAt:\s*\{\s*gte:\s*throttleWindowStart/);
        // The service file does NOT introduce a Map / Set / counter
        // for throttle state (those would defeat multi-instance
        // correctness).
        expect(RECOVERY_SVC).not.toMatch(/const\s+\w*[Tt]hrottle\w*\s*=\s*new\s+Map/);
        expect(RECOVERY_SVC).not.toMatch(/const\s+\w*[Tt]hrottle\w*\s*=\s*new\s+Set/);
    });
    // ---------------------------------------------------------------------------
    // 8. Throttle emits event.
    // ---------------------------------------------------------------------------
    it("test 8: throttle branch emits mfa_recovery_throttled with bounded counters", () => {
        expect(RECOVERY_SVC).toMatch(/eventType:\s*["']mfa_recovery_throttled["']/);
        // The route maps the result to HTTP 429.
        expect(ADMIN_ROUTES).toMatch(/result\.reason\s*===\s*["']rate_limited["'][\s\S]{0,200}reply\.code\(429\)/);
    });
});
// =============================================================================
...
describe("R8.1.6 — admin SPA quorum progress", () => {
    ...
    it("test 10: approve button is disabled until rq.emailVerified is true", () => {
        ...
        expect(ADMIN_SPA).toMatch(/Cannot approve until the user verifies their email/);
...
describe("R8.1.6 — pending recovery digest job", () => {
    ...
    it("test 13: digest body carries ONLY count + team name + admin SPA URL (no token / OTP / secret / user email enumeration)", () => {
        ...
        expect(DIGEST_SRC).not.toMatch(/body:\s*JSON\.stringify\([^)]*\bemail:\s*recipients/);
        ...
        const sendFn = DIGEST_SRC.match(/async function send(?:DigestEmail|AdminDigest)[\s\S]*?\n\}/);
...
describe("R8.1.6 — scope guards: no parallel auth / workflow / tenant leak", () => {
    ...
    it("test 18: digest scopes recipients by team membership; throttle scopes by userId only; digest log is per-team", () => {
        ...
        expect(DIGEST_SRC).toMatch(/teamMember\.findMany\(\{\s*where:\s*\{[\s\S]{0,200}status:\s*["']ACTIVE["'],\s*role:\s*\{\s*in:\s*\[["']OWNER["'],\s*["']ADMIN["']\]/);
        // Throttle count is keyed by user — never leaks across users.
        expect(RECOVERY_SVC).toMatch(/mfaRecoveryRequest\.count\(\{\s*where:\s*\{\s*userId:\s*input\.userId/);
        // Digest log is per-team.
        expect(SCHEMA).toMatch(/model MfaRecoveryDigestLog \{[\s\S]*?teamId\s+String/);
    });
    // ---------------------------------------------------------------------------
