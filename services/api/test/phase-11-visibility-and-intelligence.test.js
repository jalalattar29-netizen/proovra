...
const EVIDENCE_COMPLETE_SRC = resolve(API_ROOT, "src/services/evidence-complete.service.ts");
const EXTRACTION_SRC = resolve(API_ROOT, "src/services/intelligence/extraction.service.ts");
const ROUTE_REGISTRY_SRC = resolve(WEB_ROOT, "lib/navigation/routeRegistry.ts");
const DUPLICATES_PAGE_SRC = resolve(WEB_ROOT, "app/(app)/investigation/duplicates/page.tsx");
const DECISIONS_DOC = resolve(REPO_ROOT, "docs/architecture/phase-11-decisions.md");
...
describe("Phase 11 EVENT_WIRE — evidence-complete fan-out", () => {
    ...
    it("imports the existing search-queue helper (no v2 queue introduced)", () => {
        expect(src).toMatch(/import\s*\{[^}]*enqueueSearchIndexingJob[^}]*\}\s*from\s*["'][^"']*queue\/search-queue[^"']*["']/);
    });
    ...
    it("dispatches a best-effort graph reconcile via reconcileTeamGraph (existing reconciler)", () => {
        ...
        expect(src).toMatch(/import\(\s*["'][^"']*graph\/graph-builder\.service[^"']*["']\s*\)/);
    });
...
describe("Phase 11 EVENT_WIRE — extraction → entity extraction", () => {
    ...
    it("dynamic-imports the existing entity extractor (no v2 extractor introduced)", () => {
        expect(src).toMatch(/import\(\s*["'][^"']*entity-extraction\.service[^"']*["']\s*\)/);
    });
...
describe("Phase 11 NAV_VISIBILITY — core workspace routes remain reachable", () => {
     ...
     * the matching `}`.
     */
    function extractRouteBlock(routeId) {
...
describe("Phase 11 schema discipline — no new Prisma migrations added", () => {
     ...
     */
    const PRE_EXISTING_PHASE11_ALLOWLIST = new Set([
    ...
    it("prisma/migrations contains no NEW phase_11 / phase-11 folders beyond the documented allowlist", () => {
        ...
        for (const name of readdirSync(MIGRATIONS_DIR)) {
            ...
            if (!statSync(full).isDirectory())
                continue;
            if (!/phase[_-]?11/i.test(name))
                continue;
            if (PRE_EXISTING_PHASE11_ALLOWLIST.has(name))
                continue;
            offenders.push(name);
        }
