...
const VERIFY_PROJECTION_SVC = readFileSync(apiSrcPath("services/media-intelligence/verify-projection.service.ts"), "utf8");
const CLAIMS_MATRIX = readFileSync(sharedPath("claims-matrix.ts"), "utf8");
const TRUST_CENTER_CONTENT = readFileSync(sharedPath("trust-center-content.ts"), "utf8");
...
describe("CR4 Group 1 — file-size guards", () => {
    ...
    it("verify-projection.service.ts byte-exact pin (CR4 must not touch backend)", () => {
        const sz = statSync(apiSrcPath("services/media-intelligence/verify-projection.service.ts")).size;
    ...
    it("evidence-complete.service.ts pin (CR1.6 — 45,520 bytes after Phase 14)", () => {
        const sz = statSync(apiSrcPath("services/evidence-complete.service.ts")).size;
...
const E5_FORBIDDEN_SURFACE_PATTERNS = [
...
function allVerifySurfaceTexts() {
    const base = [
...
const CR4_FORBIDDEN_POSITIVE_CLAIMS = [
    /\btamper[- ]?proof\b/i,
...
describe("CR4 Group 5 — verify tree is side-effect free", () => {
    for (const surface of allVerifySurfaceTexts()) {
        ...
        it(`${surface.label} :: contains NO custody-event creation reference`, () => {
            expect(/createCustodyEvent|emitCustodyEvent/.test(surface.text)).toBe(false);
        });
        ...
        it(`${surface.label} :: contains NO finalize-evidence reference`, () => {
            expect(/completeEvidence|finalizeEvidence/.test(surface.text)).toBe(false);
        });
    }
});
// ---------------------------------------------------------------------------
...
describe("CR4 Group 7 — integrityProof mapping preferred", () => {
    ...
    it("at least 2 callsites use the integrityProof preference pattern", () => {
        const matches = TOKEN_PAGE.match(/integrityProof\s*\?\?\s*data?\.?\s*verification/g) ?? [];
        expect(matches.length).toBeGreaterThanOrEqual(2);
    });
...
// ---------------------------------------------------------------------------
const TSA_STATES = ["STAMPED", "FAILED", "UNAVAILABLE"];
    ...
    for (const forbidden of TSA_FORBIDDEN_STATES) {
        for (const surface of allVerifySurfaceTexts()) {
            it(`${surface.label} :: TSA-context forbidden state '${forbidden}' not paired with timestamp wording`, () => {
                ...
                const pattern = new RegExp(`(?:timestampStatus|TSA)[^\\n]{0,40}["'\`]${forbidden}["'\`]`, "i");
                expect(pattern.test(surface.text)).toBe(false);
            });
        }
    }
...
// ---------------------------------------------------------------------------
const OTS_STATES = ["PENDING", "ANCHORED", "FAILED", "UNAVAILABLE"];
    ...
    for (const forbidden of OTS_FORBIDDEN_STATES) {
        for (const surface of allVerifySurfaceTexts()) {
            it(`${surface.label} :: OTS-context forbidden state '${forbidden}' not paired with otsStatus wording`, () => {
                const pattern = new RegExp(`(?:otsStatus|OTS)[^\\n]{0,40}["'\`]${forbidden}["'\`]`, "i");
                expect(pattern.test(surface.text)).toBe(false);
            });
        }
    }
...
describe("CR4 Group 10 — custody vs access count separation", () => {
    it("token page filters on category === 'forensic' at least once", () => {
        expect(/category\s*===\s*["'`]forensic["'`]/.test(TOKEN_PAGE) ||
            /["'`]forensic["'`]\s*===\s*[\w.]*category/.test(TOKEN_PAGE)).toBe(true);
    });
    it("token page filters on category === 'access' at least once", () => {
        expect(/category\s*===\s*["'`]access["'`]/.test(TOKEN_PAGE) ||
            /["'`]access["'`]\s*===\s*[\w.]*category/.test(TOKEN_PAGE)).toBe(true);
    });
    ...
    const MIXED_AGG_PATTERN = /\b(?:custody|forensic)Count\s*\+\s*(?:access|publicView)Count\b/i;
    for (const surface of allVerifySurfaceTexts()) {
...
describe("CR4 Group 14 — E5 trust-content imports preserved", () => {
    it("token page imports PROOVRA_MULTIPART_LEGAL_BOUNDARY_NOTE from shared-evidence-presentation", () => {
        expect(/PROOVRA_MULTIPART_LEGAL_BOUNDARY_NOTE/.test(TOKEN_PAGE)).toBe(true);
    });
    ...
    it("token page imports PROOVRA_MULTIPART_REVIEWER_EXPLANATION from shared-evidence-presentation", () => {
        expect(/PROOVRA_MULTIPART_REVIEWER_EXPLANATION/.test(TOKEN_PAGE)).toBe(true);
    });
    ...
    const LOCAL_VERDICT_FN = /function\s+buildEvidenceTrustDecision\s*\(|const\s+buildEvidenceTrustDecision\s*=/;
    for (const surface of allVerifySurfaceTexts()) {
...
// ---------------------------------------------------------------------------
const AUTHENTICATED_ROUTE_TOKENS = [
    /["'`]\/v1\/teams\//,
    /["'`]\/v1\/governance\//,
    /["'`]\/admin\//,
    /["'`]\/v1\/admin\//,
...
describe("CR4 Meta — test harness", () => {
    ...
    it("verify-projection.service.ts privacy JSDoc is preserved", () => {
        expect(/NEVER exposes per-signal IDs/.test(VERIFY_PROJECTION_SVC)).toBe(true);
    });
