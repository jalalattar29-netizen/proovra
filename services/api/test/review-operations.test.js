...
import { ReviewOperationError, projectReviewWorkflow, } from "../src/services/review-operations/review-operations.service.js";
const NOW = new Date("2026-05-21T10:00:00Z");
function mockWorkflowRow(patch = {}) {
    return {
...
describe("review workflow projection — privacy + correctness", () => {
    it("does NOT expose workspaceType or raw status", () => {
        ...
        expect(projected.workspaceType).toBeUndefined();
        expect(projected.status).toBeUndefined();
        expect(projected.stage).toBe("QUEUED");
    });
    it("maps legacy DB status NOT_STARTED → QUEUED", () => {
        const projected = projectReviewWorkflow(mockWorkflowRow({ status: "NOT_STARTED" }));
        expect(projected.stage).toBe("QUEUED");
    });
    it("approvedForGovernanceGate=true only for APPROVED_INTERNAL", () => {
        const inReview = projectReviewWorkflow(mockWorkflowRow({ status: "IN_REVIEW" }));
        expect(inReview.approvedForGovernanceGate).toBe(false);
        const approved = projectReviewWorkflow(mockWorkflowRow({ status: "APPROVED_INTERNAL" }));
        expect(approved.approvedForGovernanceGate).toBe(true);
        const readyExternal = projectReviewWorkflow(mockWorkflowRow({ status: "READY_FOR_EXTERNAL_REVIEW" }));
...
describe("governance gate — source-level tightening", () => {
    it("evidenceIsReviewed only accepts APPROVED_INTERNAL / READY_FOR_EXTERNAL_REVIEW", async () => {
        ...
        const src = await readFile(fileURLToPath(new URL("../src/services/governance.service.ts", import.meta.url)), "utf8");
        ...
        const helper = src.match(/export function reviewStatusSatisfiesGovernanceGate[\s\S]*?return\s*\(/);
        expect(helper).not.toBeNull();
        if (helper) {
            const body = src.slice(helper.index ?? 0);
            const end = body.indexOf("\n}");
            const fullHelper = end > 0 ? body.slice(0, end) : body;
            expect(fullHelper).not.toMatch(/EvidenceReviewWorkflowStatus\.IN_REVIEW/);
            expect(fullHelper).toMatch(/EvidenceReviewWorkflowStatus\.APPROVED_INTERNAL/);
            expect(fullHelper).toMatch(/EvidenceReviewWorkflowStatus\.READY_FOR_EXTERNAL_REVIEW/);
        }
    });
});
describe("anti-enumeration — review-operations routes", () => {
    it("never responds 403 (404 on non-reviewer members)", async () => {
        const { readFile } = await import("node:fs/promises");
        const { fileURLToPath } = await import("node:url");
        const src = await readFile(fileURLToPath(new URL("../src/routes/review-operations.routes.ts", import.meta.url)), "utf8");
...
describe("bulk action — source-level safety contract", () => {
    it("ESCALATE and REQUEST_MORE_INFO require a note", async () => {
        const { readFile } = await import("node:fs/promises");
        const { fileURLToPath } = await import("node:url");
        const src = await readFile(fileURLToPath(new URL("../src/services/review-operations/review-operations.service.ts", import.meta.url)), "utf8");
