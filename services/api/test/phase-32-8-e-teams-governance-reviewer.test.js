...
const WORKSPACE_ADMIN_SVC = readApi("src/services/workspace-admin/workspace-admin.service.ts");
const GOVERNANCE_SVC = readApi("src/services/governance/governance-control-plane.service.ts");
const REVIEWER_SVC = readApi("src/services/reviewer-ops/reviewer-command.service.ts");
const ROUTES = readApi("src/routes/enterprise-aggregators.routes.ts");
const SERVER = readApi("src/server.ts");
const WORKSPACE_PANEL = readWeb("components/workspace-admin/WorkspaceAdminPanel.tsx");
const GOVERNANCE_PANEL = readWeb("components/governance-experience/GovernanceControlPlane.tsx");
const REVIEWER_PANEL = readWeb("components/reviewer-experience/ReviewerCommandConsole.tsx");
...
describe("Phase 32.8E — workspace-admin aggregator service contract", () => {
    it("exports buildWorkspaceAdmin entrypoint", () => {
        expect(WORKSPACE_ADMIN_SVC).toMatch(/export async function buildWorkspaceAdmin\(/);
    });
    ...
    it("never emits audit / analytics / security events", () => {
        for (const sym of [
            ...
            expect(WORKSPACE_ADMIN_SVC, `must not invoke ${sym}`).not.toContain(sym);
        }
    });
    it("never projects API key secrets, webhook secrets, or storage keys", () => {
        for (const sym of [
            ...
            expect(WORKSPACE_ADMIN_SVC, `service must not project ${sym}`).not.toContain(sym);
        }
    });
    it("declares bounded limits on every findMany", () => {
        ...
        for (const block of findMany) {
            expect(block, `unbounded findMany in workspace-admin: ${block.slice(0, 80)}`).toMatch(/take:/);
        }
    });
    it("personal vs team scope detected via memberCount", () => {
        expect(WORKSPACE_ADMIN_SVC).toMatch(/scope:\s*WorkspaceScope\s*=\s*memberCount\s*<=\s*1\s*\?\s*"PERSONAL"\s*:\s*"TEAM"/);
    });
    it("envelope exposes canonical sections (overview / access / governanceSnapshot / integrationsPosture / billing / operationalAccountability)", () => {
        for (const section of [
            ...
            expect(WORKSPACE_ADMIN_SVC).toMatch(new RegExp(`${section}:\\s*\\{`));
        }
    });
...
describe("Phase 32.8E — governance-control-plane aggregator service contract", () => {
    it("exports buildGovernanceControlPlane entrypoint", () => {
        expect(GOVERNANCE_SVC).toMatch(/export async function buildGovernanceControlPlane\(/);
    });
    it("never calls Prisma write methods", () => {
        for (const re of [
            /prisma\.[A-Za-z]+\.create\(/,
    ...
    it("case_legal_holds is treated as a CORE feature — subsystem-disabled banner reserved for genuine schema drift only", () => {
        ...
        expect(GOVERNANCE_SVC).toMatch(/caseLegalHoldsEnabled\s*=\s*true/);
        expect(GOVERNANCE_SVC).toMatch(/if\s*\(isPrismaTableOrColumnMissing\(err\)\)\s*\{[\s\S]{0,80}caseLegalHoldsEnabled\s*=\s*false/);
    });
    it("declares bounded limits on every findMany", () => {
        ...
        for (const block of findMany) {
            expect(block, `unbounded findMany in governance-control-plane: ${block.slice(0, 80)}`).toMatch(/take:/);
        }
    });
...
describe("Phase 32.8E — reviewer-command aggregator service contract", () => {
    it("exports buildReviewerCommand entrypoint", () => {
        expect(REVIEWER_SVC).toMatch(/export async function buildReviewerCommand\(/);
    });
    it("never calls Prisma write methods", () => {
        for (const re of [
            /prisma\.[A-Za-z]+\.create\(/,
    ...
    it("personal-workspace returns status='ok' with empty data (capability degradation, Phase 32.8C FINAL-3)", () => {
        ...
        for (const section of [
            ...
            expect(REVIEWER_SVC, `personal short-circuit missing ${section}`).toMatch(new RegExp(`${section}:\\s*\\{\\s*status:\\s*"ok"`));
        }
    });
...
describe("Phase 32.8E — enterprise aggregator routes", () => {
    it("registers GET /v1/teams/workspace-admin", () => {
        expect(ROUTES).toMatch(/app\.get\(\s*"\/v1\/teams\/workspace-admin"/);
    });
    it("registers GET /v1/governance/control-plane", () => {
        expect(ROUTES).toMatch(/app\.get\(\s*"\/v1\/governance\/control-plane"/);
    });
    it("registers GET /v1/reviewer-ops/command", () => {
        expect(ROUTES).toMatch(/app\.get\(\s*"\/v1\/reviewer-ops\/command"/);
    });
    it("every route requires authentication via preHandler: requireAuth", () => {
        const matches = ROUTES.match(/app\.get\(\s*"\/v1\/[^"]+",[\s\S]*?preHandler:\s*requireAuth/g) ?? [];
    ...
    it("workspace-membership gate uses 404-on-non-member, 403-on-inactive", () => {
        ...
        expect(ROUTES).toMatch(/code\(404\)\.send\(\{\s*error:\s*\{\s*code:\s*"not_found"/);
...
describe("Phase 32.8E — /teams workspace administration", () => {
    it("the /teams URL resolves to the canonical WorkspaceAdministrationHome via /workspaces", () => {
        ...
        expect(NEXT_CONFIG).toMatch(/source:\s*["']\/teams["'][\s\S]{0,200}destination:\s*["']\/workspaces["']/);
    });
    ...
    it("integrations posture explicitly notes secrets are not surfaced", () => {
        expect(WORKSPACE_PANEL).toMatch(/Secrets are never surfaced here/);
    });
    it("personal workspace gates out team-only tabs (Governance, Integrations, Accountability)", () => {
        expect(WORKSPACE_PANEL).toMatch(/key:\s*"governance",[\s\S]{0,80}visible:\s*isTeam/);
        expect(WORKSPACE_PANEL).toMatch(/key:\s*"integrations",[\s\S]{0,80}visible:\s*isTeam/);
        expect(WORKSPACE_PANEL).toMatch(/key:\s*"accountability",[\s\S]{0,80}visible:\s*isTeam/);
    });
...
describe("Phase 32.8E — /governance control plane", () => {
    ...
    it("preservation tab includes the explicit no-admissibility / no-authenticity disclaimer", () => {
        expect(GOVERNANCE_PANEL).toMatch(/does not\s+assert legal admissibility or authenticity of any record/);
    });
    it("never makes positive legal admissibility / authenticity / court-ready claims (quoted negations OK)", () => {
        ...
        for (const banned of [
            ...
            expect(GOVERNANCE_PANEL, `governance panel must not claim "${banned}"`).not.toMatch(positiveClaim(banned));
        }
    });
    it("policy edit link is gated via canonical GOVERNANCE_ACT capability", () => {
        ...
        expect(GOVERNANCE_PANEL).toMatch(/isAdmin\s*=\s*ctx\.can\(\s*['"]GOVERNANCE_ACT['"]\s*\)/);
    });
    ...
    it("does NOT call audited /v1/governance/case-legal-holds GET on mount (reads only the consolidated control-plane aggregator)", () => {
        expect(GOVERNANCE_PANEL).not.toMatch(/apiFetch\([^)]*\/v1\/governance\/case-legal-holds/);
        expect(GOVERNANCE_PANEL).not.toMatch(/apiFetch\([^)]*\/v1\/governance\/legal-holds/);
        expect(GOVERNANCE_PANEL).not.toMatch(/apiFetch\([^)]*\/v1\/governance\/retention-candidates/);
    });
});
// =============================================================================
...
describe("Phase 32.8E — /review canonical reviewer console mount", () => {
    ...
    it("personal workspace renders capability-degradation banner alongside the full surface (Phase 32.8C FINAL-3)", () => {
        ...
        expect(REVIEWER_PANEL).toMatch(/Personal workspace[\s\S]{0,200}read-only enterprise-lite mode/);
    });
    it("explicitly notes this is a triage surface — reviewer actions live on per-workflow pages", () => {
        expect(REVIEWER_PANEL).toMatch(/Reviewer actions live on the per-workflow inspector pages/);
    });
    ...
    it("never calls audited /v1/reviewer-ops/{queue,dashboard,workload,escalations} on mount (uses only the consolidated aggregator)", () => {
        expect(REVIEWER_PANEL).not.toMatch(/apiFetch\([^)]*\/v1\/reviewer-ops\/queue/);
        expect(REVIEWER_PANEL).not.toMatch(/apiFetch\([^)]*\/v1\/reviewer-ops\/dashboard/);
        expect(REVIEWER_PANEL).not.toMatch(/apiFetch\([^)]*\/v1\/reviewer-ops\/workload/);
        expect(REVIEWER_PANEL).not.toMatch(/apiFetch\([^)]*\/v1\/reviewer-ops\/escalations/);
    });
...
describe("Phase 32.8E — shared invariants", () => {
    it("none of the new frontend panels CALL side-effecting endpoints on mount", () => {
        ...
        for (const [name, src] of Object.entries(panels)) {
            ...
            expect(src, `${name} must not call /report/latest on mount`).not.toMatch(/apiFetch\([^)]*\/v1\/evidence\/[^)]*\/report\/latest/);
            expect(src, `${name} must not call /verification-package on mount`).not.toMatch(/apiFetch\([^)]*\/v1\/evidence\/[^)]*\/verification-package/);
            // Browse must not trigger team-export ZIP.
            expect(src, `${name} must not call /v1/cases/:id/export on mount`).not.toMatch(/apiFetch\([^)]*\/v1\/cases\/[^)]*\/export/);
            // Browse must not call the audited /v1/teams or /v1/teams/:id GETs.
            expect(src, `${name} must not call /v1/teams (audited list)`).not.toMatch(/apiFetch\("\/v1\/teams"/);
            expect(src, `${name} must not call /v1/teams/:id (audited GET)`).not.toMatch(/apiFetch\(`\/v1\/teams\/\$\{[^}]+\}\`[\s,}]/);
        }
    });
    ...
    it("no panel makes positive legal overclaim statements (quoted negations OK)", () => {
        ...
        for (const banned of [
            ...
            for (const [name, src] of Object.entries(panels)) {
                expect(src, `${name} must not claim "${banned}"`).not.toMatch(positiveClaim(banned));
            }
        }
    });
    ...
    it("canonical legacy routes remain intact (no accidental redirect breakage)", () => {
        ...
        for (const path of [
            ...
            expect(readWeb(path).length, `expected canonical legacy page ${path} to remain intact`).toBeGreaterThan(100);
        }
    });
});
