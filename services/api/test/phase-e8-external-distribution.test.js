...
import { EXTERNAL_ACCESS_FORBIDDEN_PATTERNS, EXTERNAL_ACCESS_PARTICIPANTS, EXTERNAL_PARTICIPANT_TYPES, EXTERNAL_SURFACE_INVENTORY, TRUST_CENTER_SECTIONS, getExternalParticipantContent, getExternalSurfaceRecord, listExternalSurfaceIds, } from "@proovra/shared-evidence-presentation";
// ---------------------------------------------------------------------------
// Helpers
// ---------------------------------------------------------------------------
...
const EXTERNAL_ACCESS_CONTENT_SRC = readPackages("shared-evidence-presentation/src/external-access-content.ts");
const WORKFLOW_INTAKE_TOKEN_SVC = readApi("src/services/workflow-intake-token.service.ts");
const WORKFLOW_INTAKE_ROUTES = readApi("src/routes/workflow-intake-links.routes.ts");
const EXTERNAL_INTAKE_ROUTES = readApi("src/routes/external-intake.routes.ts");
const EXTERNAL_REVIEW_ROUTES = readApi("src/routes/external-review.routes.ts");
const EXTERNAL_REVIEW_GRANT_SVC = readApi("src/services/external-review/external-review-grant.service.ts");
const EVIDENCE_REQUESTS_ROUTES = readApi("src/routes/evidence-requests.routes.ts");
...
describe("E8 Test 1 — external participant types", () => {
    ...
    it.each(EXTERNAL_PARTICIPANT_TYPES)("EXTERNAL_ACCESS_PARTICIPANTS has a complete record for %s", (type) => {
...
describe("E8 Test 3 — external-access content carries no forbidden trust wording", () => {
    for (const type of EXTERNAL_PARTICIPANT_TYPES) {
        describe(`participant ${type}`, () => {
            ...
            it.each(EXTERNAL_ACCESS_FORBIDDEN_PATTERNS)("does NOT match %s", (pattern) => {
                expect(blob).not.toMatch(pattern);
            });
        });
    }
    it("external-access-content.ts body (outside the forbidden-list declaration) is clean", () => {
        const sanitised = EXTERNAL_ACCESS_CONTENT_SRC.replace(/EXTERNAL_ACCESS_FORBIDDEN_PATTERNS[\s\S]*?\]\s*;/m, "");
...
describe("E8 Test 4 — backend external surfaces enforce the audited contract", () => {
    describe("workflow-intake-token.service.ts", () => {
        ...
        it("requires the secret to be present (rejects when unset)", () => {
            expect(WORKFLOW_INTAKE_TOKEN_SVC).toMatch(/WORKFLOW_INTAKE_TOKEN_SECRET/);
        });
    });
    describe("workflow-intake-links.routes.ts", () => {
        it("returns 503 when the feature flag is disabled", () => {
            ...
            expect(WORKFLOW_INTAKE_ROUTES).toMatch(/workflowIntakeFeatureDisabledReason|FEATURE_DISABLED/);
        });
...
describe("E8 Test 6 — Trust Center automation-auditability extended", () => {
    const section = TRUST_CENTER_SECTIONS.find((s) => s.id === "automation-auditability");
    it("section exists", () => {
...
describe("E8 Test 9 — documentation + registry", () => {
    ...
    it("registry registers Phase E8 with explicit closure status", () => {
        ...
        expect(registry).toMatch(/\|\s*(Phase )?E8\s*\|[\s\S]*?(CLOSED|CLOSED_WITH_DEFERRED_ITEMS)/);
    });
