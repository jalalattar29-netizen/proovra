...
const CANONICAL_GROUPS = readSource("../../../apps/web/lib/navigation/canonicalNavigationGroups.ts");
const GROUPING_RESOLVER = readSource("../../../apps/web/lib/navigation/navigationGroupingResolver.ts");
...
const WORKSPACES_PAGE = readSource("../../../apps/web/app/(app)/workspaces/page.tsx");
...
describe("Phase G0 (B.1) — sidebar uses Phase B canonical hierarchy", () => {
    it("ALLOWED_ROOT_GROUP_TITLES drops R2 titles and emits Phase B titles", () => {
        ...
        const allowed = CANONICAL_GROUPS.match(/ALLOWED_ROOT_GROUP_TITLES[\s\S]*?\];/);
    ...
    it("canonical group definitions expose the four Phase B groups", () => {
        expect(CANONICAL_GROUPS).toMatch(/SIDEBAR_GROUP_WORKSPACE\s*=\s*\{[\s\S]*?title:\s*"Workspace"/);
        expect(CANONICAL_GROUPS).toMatch(/SIDEBAR_GROUP_GOVERNANCE\s*=\s*\{[\s\S]*?title:\s*"Governance"/);
        expect(CANONICAL_GROUPS).toMatch(/SIDEBAR_GROUP_OUTPUTS\s*=\s*\{[\s\S]*?title:\s*"Outputs"/);
        expect(CANONICAL_GROUPS).toMatch(/SIDEBAR_GROUP_SYSTEM\s*=\s*\{[\s\S]*?title:\s*"System"/);
    });
    it("CR0 baseline test pin updated to the Phase B titles", () => {
        ...
        const allowedTitlesBlock = CR0_TEST.match(/Phase G0[\s\S]*?ALLOWED_ROOT_GROUP_TITLES\s*=\s*\[[\s\S]*?\];/);
    ...
    it("grouping resolver consumes phaseBOperationalGroups.ts for group membership", () => {
        expect(GROUPING_RESOLVER).toMatch(/import\s*\{\s*operationalGroupForRoute\s*\}\s*from\s+"\.\/phaseBOperationalGroups"/);
        expect(GROUPING_RESOLVER).toMatch(/operationalGroupForRoute\(item\.route\.id\)/);
    });
...
describe("Phase G0 (B.3) — terminology normalized in primary navigation", () => {
    it("route registry uses 'Workspaces' for the admin.teams entry label", () => {
        expect(ROUTE_REGISTRY).toMatch(/id:\s*"admin\.teams"[\s\S]*?label:\s*"Workspaces"/);
    });
...
describe("Phase G0 (B.4) — operations path alias preserved", () => {
    it("next.config.js still aliases /ops/reliability → /operations/reliability", () => {
        ...
        expect(cfg).toMatch(/source:\s*"\/ops\/reliability",\s*destination:\s*"\/operations\/reliability"/);
    });
});
// ===========================================================================
...
describe("Phase G0 (B0.3) — workspace switcher UX", () => {
    it("renders the workspace-vs-organization help block", () => {
        ...
        expect(TOPBAR).toMatch(/Organization\s*=\s*governance over one or more Workspaces/);
    });
    ...
    it("solo personal-only accounts still see the help (no org noise pollution)", () => {
        ...
        expect(TOPBAR).toMatch(/organizations\.length\s*>\s*0\s*\?\s*"\s*Organization/);
    });
});
// ===========================================================================
...
describe("Phase G0 (B0.5) — /workspaces canonical frontend route", () => {
    ...
    it("route registry's admin.teams entry now points at /workspaces", () => {
        expect(ROUTE_REGISTRY).toMatch(/id:\s*"admin\.teams"[\s\S]*?href:\s*"\/workspaces"/);
    });
