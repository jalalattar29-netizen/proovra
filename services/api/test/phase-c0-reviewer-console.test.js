...
const REVIEW_OPS_ROUTES = readSource("../src/routes/review-operations.routes.ts");
const CONSOLE_UI = readSource("../../../apps/web/components/reviewer-experience/ReviewerConsole.tsx");
const REVIEW_PAGE = readSource("../../../apps/web/app/(app)/review/page.tsx");
describe("Phase C0 — reviewer console aggregator (source contract)", () => {
    it("registers GET /v1/reviewer-ops/console", () => {
        expect(CONSOLE_ROUTES).toMatch(/app\.get\(\s*"\/v1\/reviewer-ops\/console"/);
    });
    it("composes the five sections + saved views in parallel via Promise.all", () => {
        ...
        const composeMatch = CONSOLE_ROUTES.match(/Promise\.all\(\[\s*([\s\S]*?)\s*\]\)/);
    ...
    it("uses anti-enumeration 404 for non-members", () => {
        expect(CONSOLE_ROUTES).toMatch(/reply\.code\(404\)\.send\(\{\s*error:\s*\{\s*code:\s*"not_found"/);
    });
...
describe("Phase C0 — sensitive review decision step-up enforcement", () => {
    ...
    it("the decision handler gates APPROVE_INTERNAL on requireStepUpForApprove", () => {
        expect(REVIEW_OPS_ROUTES).toMatch(/body\.decision\s*===\s*"APPROVE_INTERNAL"/);
    ...
    it("the decision handler gates REJECT_INSUFFICIENT on requireStepUpForReject", () => {
        expect(REVIEW_OPS_ROUTES).toMatch(/body\.decision\s*===\s*"REJECT_INSUFFICIENT"/);
...
describe("Phase C0 — reviewer console UI (source contract)", () => {
    ...
    it("keyboard navigation handles j / k / Enter / / / Cmd+K", () => {
        ...
        expect(CONSOLE_UI).toMatch(/\(e\.metaKey \|\| e\.ctrlKey\)[\s\S]*?e\.key\.toLowerCase\(\)\s*===\s*"k"/);
    });
