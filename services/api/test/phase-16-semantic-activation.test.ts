...
const EMBEDDING_PROVIDER_SRC = resolve(
  API_ROOT,
  "src/services/search/embedding-provider.ts",
);
const SEMANTIC_BACKFILL_SRC = resolve(
  API_ROOT,
  "src/services/search/semantic-backfill.service.ts",
);
const SEMANTIC_BUDGET_SRC = resolve(
  API_ROOT,
  "src/services/search/semantic-budget.service.ts",
);
const SEMANTIC_SERVICE_SRC = resolve(
  API_ROOT,
  "src/services/intelligence/semantic.service.ts",
);
const EVIDENCE_SEARCH_SRC = resolve(
  API_ROOT,
  "src/services/search/evidence-search.service.ts",
);
const SEARCH_ROUTES_SRC = resolve(API_ROOT, "src/routes/search.routes.ts");
const MI_EMBED_QUEUE_SRC = resolve(
  API_ROOT,
  "src/queue/mi-embed-queue.ts",
);
const WORKER_MI_EMBED_PROCESSOR_SRC = resolve(
  WORKER_ROOT,
...
const PHASE_16_MIGRATION_SQL = resolve(
  MIGRATIONS_DIR,
  "20270801000000_phase16_semantic_usage/migration.sql",
);
const PHASE_32_7_2_TEST_SRC = resolve(
  API_ROOT,
  "test/phase-32-7-2-security-event-mapping-drift.test.ts",
);
const PHASE_15_POLICY_DOC = resolve(
  DOCS_ROOT,
  "architecture/phase-15-semantic-policy.md",
);
...
describe("Phase 16 — OpenAI embedding provider gates + error vocabulary", () => {
  ...
  it("2. EmbeddingProviderError vocabulary includes OUTBOUND_NOT_ALLOWED, AUTH, RATE_LIMITED, UPSTREAM_5XX, TIMEOUT", () => {
    ...
    for (const code of [
      ...
      expect(body, `missing error code ${code}`).toMatch(
        new RegExp(`["']${code}["']`),
      );
    }
  ...
  it("4. Provider failure path in evidence-search.service.ts catches errors and sets a bounded fallbackReason", () => {
    ...
    expect(
      matched,
      "expected at least 3 bounded fallback reason strings in evidence-search.service.ts",
  ...
  it("5. No raw chunk-text logging across provider + backfill + worker processor", () => {
    ...
    for (const re of forbidden) {
      expect(
        provider,
        `raw chunk-text leak in embedding-provider.ts (${re})`,
      ).not.toMatch(re);
      expect(
        backfill,
        `raw chunk-text leak in semantic-backfill.service.ts (${re})`,
      ).not.toMatch(re);
      expect(
        worker,
        `raw chunk-text leak in mi-embed.processor.ts (${re})`,
      ).not.toMatch(re);
    }
  });
});
...
describe("Phase 16 — Backfill service + live indexing", () => {
  ...
  it("7. Backfill uses Prisma.sql / $queryRaw with workspaceId parameterised — never string-interpolated into SQL", () => {
    ...
    for (const re of forbidden) {
      expect(
        body,
        `forbidden string-interpolation of workspaceId into SQL (${re})`,
      ).not.toMatch(re);
    }
  });
...
describe("Phase 16 — Hybrid pgvector ranker + admin routes", () => {
  it("10. POST /v1/search/semantic/backfill registered in search.routes.ts behind platform-admin gate", () => {
    ...
    expect(body).toMatch(
      /["']\/v1\/search\/semantic\/backfill["'][\s\S]{0,400}requirePlatformAdmin/,
    );
...
describe("Phase 16 — /search page UI surfaces", () => {
  ...
  it("15. Mode selector disable wording references humanised fallbackReason strings", () => {
    ...
    expect(
      matched,
      `expected >= 4 humanised fallback phrases in the /search page; matched ${matched}`,
...
describe("Phase 16 — bounded guards (no duplication, no leak, no Search v2)", () => {
  ...
  it("17. NO /semantic-search route — apps/web/app/(app)/**/semantic* returns empty", () => {
    const appDir = resolve(WEB_ROOT, "app");
    const offenders = walk(appDir, (name) =>
      /^semantic/i.test(name) || /semantic-search/i.test(name),
    );
    expect(offenders, offenders.join("\n")).toEqual([]);
  });

  it("18. NO new top-level /v1 semantic endpoint — no services/api/src/routes/semantic*.ts", () => {
    const offenders = readdirSync(API_ROUTES_DIR).filter(
      (f) => /^semantic/i.test(f) && (f.endsWith(".ts") || f.endsWith(".tsx")),
    );
    expect(offenders, offenders.join("\n")).toEqual([]);
  ...
  it("20. Policy doc updated — phase-15-semantic-policy.md size exceeds the conservative Phase-16 baseline", () => {
    ...
    expect(
      body.length,
      `policy doc shrunk below Phase 16 floor: ${body.length} < ${POLICY_DOC_FLOOR_BYTES}`,
    ).toBeGreaterThan(POLICY_DOC_FLOOR_BYTES);
