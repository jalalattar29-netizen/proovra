...
const sharedPath = (rel) => resolve(REPO, "packages/shared", rel);
const SCHEMA = readFileSync(apiPath("prisma/schema.prisma"), "utf8");
const SHARED_SEC = readFileSync(sharedPath("src/security.ts"), "utf8");
const DIGEST_PREF_SVC = readFileSync(apiPath("src/services/security/mfa-digest-preference.service.ts"), "utf8");
const ADMIN_ROUTES = readFileSync(apiPath("src/routes/mfa-admin.routes.ts"), "utf8");
const RECOVERY_SVC = readFileSync(apiPath("src/services/security/mfa-recovery-request.service.ts"), "utf8");
...
const ADMIN_SPA = readFileSync(webPath("app/(app)/security-center/mfa-recovery/page.tsx"), "utf8");
// =============================================================================
...
describe("R8.1.7 — admin digest notification preferences", () => {
    ...
    it("test 1: MfaAdminDigestPreference Prisma model exists with the bounded fields", () => {
        ...
        expect(migrations.some((m) => m.endsWith("_r8_1_7_digest_preferences"))).toBe(true);
    });
    // ---------------------------------------------------------------------------
    ...
    it("test 2: schema default + service default + worker fallback all ENABLED", () => {
        ...
        const helper = DIGEST_PREF_SVC.match(/export function shouldSendDigest[\s\S]*?\n\}/);
    ...
    it("test 3: PATCH /v1/identity/mfa-admin/digest-preferences uses the authenticated actor as userId", () => {
        expect(ADMIN_ROUTES).toMatch(/app\.patch\(\s*["']\/v1\/identity\/mfa-admin\/digest-preferences["']/);
        // The route never reads userId from the body; it always uses
        // the session userId from getAuthUserId(req).
        const block = ADMIN_ROUTES.match(/app\.patch\(\s*["']\/v1\/identity\/mfa-admin\/digest-preferences["'][\s\S]*?\}\s*,\s*\)\s*;/);
        ...
        const bodySchema = ADMIN_ROUTES.match(/const PatchDigestPreferenceBody\s*=[\s\S]*?\}\);/);
    ...
    it("test 4: team-scoped preference requires ACTIVE membership", () => {
        expect(DIGEST_PREF_SVC).toMatch(/if\s*\(input\.teamId\)\s*\{[\s\S]{0,200}teamMember\.findFirst/);
        expect(DIGEST_PREF_SVC).toMatch(/teamId:\s*input\.teamId,\s*status:\s*["']ACTIVE["']/);
    ...
    it("test 5: preferences are read only by digest sender; not consulted in audit/security event paths", () => {
        ...
        const adminSvc = readFileSync(apiPath("src/services/security/mfa-admin-lifecycle.service.ts"), "utf8");
...
describe("R8.1.7 — multi-team digest grouping", () => {
    ...
    it("test 6: worker builds per-admin team-summary list + sends ONE email per admin (not per team)", () => {
        ...
        expect(DIGEST_WORKER).toMatch(/sendAdminDigest\(\{[\s\S]{0,200}teams:\s*includedSummaries/);
    ...
    it("test 7: MfaRecoveryAdminDigestLog is keyed by (userId, sentDate) UNIQUE", () => {
        ...
        expect(DIGEST_WORKER).toMatch(/mfaRecoveryAdminDigestLog\.findUnique\(\{\s*where:\s*\{\s*userId_sentDate:/);
    ...
    it("test 8: worker filters teams via isDigestAllowed before sending", () => {
        expect(DIGEST_WORKER).toMatch(/if\s*\(!isDigestAllowed\(prefs,\s*teamId,\s*now\)\)\s*continue/);
        ...
        expect(DIGEST_WORKER).toMatch(/if\s*\(includedSummaries\.length\s*===\s*0\)\s*\{[\s\S]{0,300}continue/);
    });
});
// =============================================================================
...
describe("R8.1.7 — digest failure handling", () => {
    ...
    it("test 9: per-admin digest log is written AFTER the transport returns OK", () => {
        ...
        // retry.
        const block = DIGEST_WORKER.match(/sent:\s*"ok"\s*\|\s*"skipped_no_transport"\s*\|\s*"failed";[\s\S]*?adminsSent \+= 1;/);
        expect(block).toBeTruthy();
        ...
        expect(block[0]).toMatch(/mfaRecoveryAdminDigestLog\.create[\s\S]*?if\s*\(sent\s*===\s*"ok"\)/);
    });
    // ---------------------------------------------------------------------------
    ...
    it("test 10: per-admin idempotency uses UNIQUE (userId, sentDate); replays skip", () => {
        ...
        expect(DIGEST_WORKER).toMatch(/mfaRecoveryAdminDigestLog\.findUnique\([^)]*userId_sentDate/);
        expect(DIGEST_WORKER).toMatch(/if\s*\(existing\)\s*continue/);
    ...
    it("test 11: worker email body carries only counts + team names + admin SPA URL", () => {
        ...
        // assembly.
        const sendFn = DIGEST_WORKER.match(/async function sendAdminDigest[\s\S]*?\n\}/);
...
describe("R8.1.7 — verify page session detection", () => {
    ...
    it("test 12: page calls /v1/auth/me AFTER the successful verify, not before", () => {
        ...
        expect(VERIFY_PAGE).toMatch(/setState\(\{\s*kind:\s*["']verified["'],\s*sessionPresent\s*\}\)/);
        // The two CTA branches are explicitly tagged.
        expect(VERIFY_PAGE).toMatch(/data-cc-mfa-recovery-verify-cta="continue-home"/);
        expect(VERIFY_PAGE).toMatch(/data-cc-mfa-recovery-verify-cta="return-to-sign-in"/);
    });
    // ---------------------------------------------------------------------------
...
describe("R8.1.7 — recovery verify analytics", () => {
    ...
    it("test 14: route emits analytics on every verify outcome via writeAnalyticsEvent", () => {
        ...
        expect(ADMIN_ROUTES).toMatch(/eventType:\s*["']mfa_recovery_verify_succeeded["']/);
        expect(ADMIN_ROUTES).toMatch(/eventType:\s*["']mfa_recovery_verify_failed["']/);
        expect(ADMIN_ROUTES).toMatch(/eventType:\s*["']mfa_recovery_verify_page_viewed["']/);
        // Payload metadata must not carry the raw verify token. Find
        // each analytics call's metadata object and assert.
        const calls = [
            ...ADMIN_ROUTES.matchAll(/writeAnalyticsEvent\(\{[\s\S]*?\}\)\.catch/g),
        ].map((m) => m[0]);
    ...
    it("test 15: analytics events go through the canonical writeAnalyticsEvent — no bespoke ingest service", () => {
        ...
        expect(ADMIN_ROUTES).toMatch(/from\s+["'][^"']*analytics-event\.service/);
...
describe("R8.1.7 — scope guards", () => {
    ...
    it("test 18: preference rows scope to (user, team); worker filters per-admin per-team; admin log is per user", () => {
        ...
        expect(SCHEMA).toMatch(/model MfaAdminDigestPreference[\s\S]*?userId\s+String[\s\S]*?teamId\s+String\?/);
        // Worker scopes recipients by team membership.
        expect(DIGEST_WORKER).toMatch(/teamMember\.findMany\(\{\s*where:\s*\{\s*teamId:\s*\{\s*in:\s*teamIds/);
        // Admin digest log is per-user.
        expect(SCHEMA).toMatch(/model MfaRecoveryAdminDigestLog \{[\s\S]*?userId\s+String/);
    });
    // ---------------------------------------------------------------------------
...
describe("R8.1.7 bonus — bounded SECURITY_EVENT_TYPES vocabulary", () => {
    ...
    it("every R8.1.7 event type appears EXACTLY once in SECURITY_EVENT_TYPES", () => {
        for (const evt of R8_1_7_EVENTS) {
            ...
            expect(matches?.length ?? 0, `${evt} must appear exactly once`).toBe(1);
        }
    });
