...
describe("Phase 37.97 — projection schema + migration", () => {
    ...
    const MIGRATION = readApi("prisma/migrations/20260720200000_dashboard_projections/migration.sql");
    it("declares OrgHealthProjection with the canonical tenant-keyed shape", () => {
        ...
        expect(SCHEMA).toMatch(/@@index\(\[teamId, sampledAtUtc\(sort: Desc\)\]\)/);
    ...
    it("migration is additive (IF NOT EXISTS) on every CREATE", () => {
        ...
        for (const line of creates) {
            expect(/IF NOT EXISTS/i.test(line), `Migration must use IF NOT EXISTS for additive safety: ${line}`).toBe(true);
        }
    });
    it("migration declares the tenant-keyed indexes", () => {
        expect(MIGRATION).toMatch(/CREATE INDEX IF NOT EXISTS "org_health_projections_team_sampled_desc_idx"/);
    });
});
describe("Phase 37.97 — refresh service is tenant-scoped", () => {
    const SRC = readApi("src/services/dashboard/projections/refresh-org-health.service.ts");
    ...
    it("every count query is scoped by `teamId` (no global aggregate)", () => {
        ...
        for (const m of counts) {
            expect(/teamId/.test(m[0]), `Every refresh count query must scope by teamId. Offender: ${m[0].slice(0, 200)}`).toBe(true);
        }
    });
    it("upsert uses the (teamId, sampledAtUtc) composite key only", () => {
        expect(SRC).toMatch(/upsert\(\s*\{[\s\S]*?teamId_sampledAtUtc:\s*\{\s*teamId,\s*sampledAtUtc\s*\}/);
    });
...
const { refreshOrgHealthProjection, readLatestOrgHealthProjection } = await import("../src/services/dashboard/projections/refresh-org-health.service.js");
beforeEach(() => {
...
describe("Phase 37.97 — refresh service tenant safety (behavioral)", () => {
    it("rejects an empty teamId loudly", async () => {
        await expect(refreshOrgHealthProjection({ teamId: "" })).rejects.toThrow(/teamId/);
    ...
    it("scopes EVERY count query by the input teamId", async () => {
        ...
        for (const call of allCalls) {
            const arg = call[0];
            expect(arg.where?.teamId, `Every count must filter by the input teamId. Offender args: ${JSON.stringify(arg)}`).toBe("teamA");
        }
    });
    it("upserts the row for the input teamId (and only that team)", async () => {
        ...
        const arg = mockClient.orgHealthProjection.upsert.mock.calls[0][0];
    ...
    it("readLatestOrgHealthProjection filters by teamId", async () => {
        ...
        const arg = mockClient.orgHealthProjection.findFirst.mock.calls[0][0];
