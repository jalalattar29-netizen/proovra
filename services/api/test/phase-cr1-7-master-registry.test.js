...
describe("CR1.7 Test 2 — required sections present", () => {
    ...
    it.each(REQUIRED_SECTION_HEADERS)("registry contains section header: %s", (header) => {
        expect(REGISTRY, `Missing required section header: ${header}`).toContain(header);
    });
});
// =============================================================================
...
describe("CR1.7 Test 3 — registry contains R8.C, CR1.5, CR1.6", () => {
    ...
    it.each(REQUIRED_PHASE_REFS)("registry references phase $label and its doc", ({ label, doc }) => {
        expect(REGISTRY, `Missing phase reference: ${label}`).toContain(label);
        expect(REGISTRY, `Missing doc reference: ${doc}`).toContain(doc);
    });
});
// =============================================================================
...
describe("CR1.7 Test 4 — deferred items table is well-formed", () => {
    it("has the canonical column headers", () => {
        ...
        for (const col of REQUIRED_COLUMNS) {
            expect(REGISTRY, `Deferred-items table missing column: ${col}`).toContain(col);
        }
    });
    ...
    it("every DEF-NNN id appears at least once in §6 deferred items table", () => {
        ...
        for (const id of ids) {
            ...
            expect(REGISTRY, `Cross-referenced ${id} does not have a row definition (orphan ref).`).toMatch(rowRegex);
        }
    });
});
// =============================================================================
...
describe("CR1.7 Test 6 — entry checklist + closure template present", () => {
    it("entry checklist lists at least the 8 mandatory questions", () => {
        ...
        for (const phrase of CHECKLIST_PHRASES) {
            expect(REGISTRY, `Entry checklist missing phrase: "${phrase}"`).toMatch(new RegExp(phrase, "i"));
        }
    });
    it("closure template lists the 10 mandatory final-report items", () => {
        ...
        for (const phrase of CLOSURE_PHRASES) {
            expect(REGISTRY, `Closure template missing phrase: "${phrase}"`).toContain(phrase);
        }
    });
});
// =============================================================================
...
describe("CR1.7 Test 7 — next safe phase is identified as 32.7", () => {
    ...
    it("registry enumerates 32.7's hard out-of-scope set", () => {
        ...
        for (const item of OUT_OF_SCOPE) {
            expect(REGISTRY, `Out-of-scope item not enumerated: ${item}`).toMatch(new RegExp(item, "i"));
        }
    });
});
// =============================================================================
...
describe("CR1.7 Test 8 — every deferred item carries a closure criterion", () => {
    ...
    it("each DEF-NNN row in §6 has 9 pipes and a non-empty closure-criteria cell", () => {
        ...
        // `| DEF-NNN | ...`.
        const rows = REGISTRY.split(/\r?\n/).filter((line) => /^\|\s*DEF-\d{3}\s*\|/.test(line));
        ...
        for (const row of rows) {
            ...
            expect(pipeCount, `Row malformed (expected 9 pipes for 8 columns): ${row}`).toBe(9);
            ...
            expect(closureCriteria.length, `Empty closure criteria in row: ${row}`).toBeGreaterThan(3);
        }
    });
});
// =============================================================================
...
describe("CR1.7 Test 10 — R8.C / CR1.5 / CR1.6 registered with explicit status", () => {
    it("R8.C is registered as CLOSED_WITH_DEFERRED_ITEMS", () => {
        // R8.C row in §3 or §4.
        expect(REGISTRY).toMatch(/\|\s*R8\.C\s*\|[\s\S]*?CLOSED_WITH_DEFERRED_ITEMS/);
    });
    it("CR1.5 is registered as CLOSED_WITH_DEFERRED_ITEMS", () => {
        expect(REGISTRY).toMatch(/\|\s*CR1\.5\s*\|[\s\S]*?CLOSED_WITH_DEFERRED_ITEMS/);
    });
    it("CR1.6 is registered as CLOSED_WITH_DEFERRED_ITEMS", () => {
        expect(REGISTRY).toMatch(/\|\s*CR1\.6\s*\|[\s\S]*?CLOSED_WITH_DEFERRED_ITEMS/);
    });
...
describe("CR1.7 Test 11 — known deferred items migrated into registry", () => {
    ...
    const REQUIRED_DEFERRED_ITEMS_PHRASES = [
    ...
    it.each(REQUIRED_DEFERRED_ITEMS_PHRASES)("registry mentions known deferred topic: $label", ({ label, regex }) => {
        expect(REGISTRY, `Required deferred topic not migrated into registry: "${label}"`).toMatch(regex);
    });
});
// =============================================================================
