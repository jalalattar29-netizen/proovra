...
import { AUTHORITY_SCHEMA_VERSION, CAPABILITY_KEYS, CAPABILITY_SCHEMA_VERSION, NAVIGATION_SCHEMA_VERSION, WORKSPACE_PLANS, WORKSPACE_ROLES, WORKSPACE_SCOPES, } from "../src/services/platform-context/types.js";
import { resolveCapabilities, resolvePersona, } from "../src/services/platform-context/capability-registry.js";
import { NAVIGATION_REGISTRY, filterNavigationRegistry, } from "../src/services/platform-context/navigation-registry.js";
...
describe("Phase 32.8 Foundation — capability registry", () => {
    ...
    it("resolvePersona returns role-shaped persona for team workspaces", () => {
        expect(resolvePersona({ scope: "TEAM", role: "OWNER" })).toBe("WORKSPACE_OWNER");
        expect(resolvePersona({ scope: "TEAM", role: "ADMIN" })).toBe("TEAM_ADMIN");
        expect(resolvePersona({ scope: "TEAM", role: "MEMBER" })).toBe("TEAM_MEMBER");
        expect(resolvePersona({ scope: "TEAM", role: "VIEWER" })).toBe("TEAM_VIEWER");
    });
});
// =============================================================================
...
describe("Phase 32.8 Foundation — navigation registry", () => {
    ...
    it("Teams nav visible to every team member AND to personal users (as create-team entry)", () => {
        ...
        const teamMemberNav = filterNavigationRegistry(resolveCapabilities({
            scope: "TEAM",
            role: "MEMBER",
            plan: "TEAM",
            isPlatformAdmin: false,
        ...
        const personalNav = filterNavigationRegistry(resolveCapabilities({
            scope: "PERSONAL",
            role: "OWNER",
            plan: "PRO",
            isPlatformAdmin: false,
        }));
        const personalAdminGroup = personalNav.find((g) => g.id === "administration");
        expect(personalAdminGroup?.items.some((i) => i.id === "admin.teams") ?? false).toBe(true);
    });
    ...
    it("Reviewer Ops nav: review.queue hidden in personal workspaces, visible in team workspaces", () => {
        ...
        const personalReview = personalNav.find((g) => g.id === "review_governance");
        expect(personalReview?.items.some((i) => i.id === "review.queue") ?? false).toBe(false);
        const teamNav = filterNavigationRegistry(resolveCapabilities({
            scope: "TEAM",
            role: "VIEWER",
            plan: "TEAM",
            isPlatformAdmin: false,
...
describe("Phase 32.8 Foundation — route", () => {
    it("registers GET /v1/platform/context and POST /v1/platform/context/switch-workspace only", () => {
        ...
        expect(ROUTE).toMatch(/app\.post\(\s*['"]\/v1\/platform\/context\/switch-workspace['"]/);
...
describe("Phase 32.8 Foundation — server registration", () => {
    it("server.ts imports platformContextRoutes", () => {
        expect(SERVER).toMatch(/import\s*\{\s*platformContextRoutes\s*\}\s*from\s*['"]\.\/routes\/platform-context\.routes\.js['"]/);
    });
...
describe("Phase 32.8 Foundation — frontend canonical layer (F-2)", () => {
    ...
    it("provider hits /v1/platform/context and /v1/platform/context/switch-workspace only", () => {
        expect(WEB_PROVIDER).toMatch(/apiFetch\(\s*['"]\/v1\/platform\/context['"]/);
        expect(WEB_PROVIDER).toMatch(/apiFetch\(\s*\n?\s*['"]\/v1\/platform\/context\/switch-workspace['"]/);
...
describe("Phase 32.8 Foundation — Teams restoration (F-4)", () => {
    ...
    it("/teams URL resolves to the workspace administration panel via canonical /workspaces", () => {
        ...
        expect(WEB_NEXT_CONFIG).toMatch(/source:\s*["']\/teams["'][\s\S]{0,200}destination:\s*["']\/workspaces["']/);
    });
...
describe("Phase 32.8 Foundation — ReviewerOps + Governance repair (F-5)", () => {
    const REVIEWER_CONSOLE = readWeb("components/reviewer-experience/ReviewerCommandConsole.tsx");
    const GOVERNANCE_CONSOLE = readWeb("components/governance-experience/GovernanceControlPlane.tsx");
