...
import { COLLABORATION_TEAM_ACTIVITY_EVENT_TYPES, COLLABORATION_TEAM_ASSIGNMENT_PRIORITIES, COLLABORATION_TEAM_ASSIGNMENT_STATUSES, COLLABORATION_TEAM_ASSIGNMENT_TARGETS, COLLABORATION_TEAM_DELIVERY_STATUSES, COLLABORATION_TEAM_INVITE_CHANNELS, COLLABORATION_TEAM_INVITE_STATUSES, COLLABORATION_TEAM_INVITE_TOKEN_PREFIX, COLLABORATION_TEAM_INVITE_TOKEN_RANDOM_BYTES, COLLABORATION_TEAM_MEMBER_STATUSES, COLLABORATION_TEAM_PLAN_LIMITS, COLLABORATION_TEAM_ROLES, COLLABORATION_TEAM_STATUSES, COLLABORATION_TEAM_TYPES, collaborationTeamRoleHasPermission, getCollaborationTeamPlanLimits, isWellFormedCollaborationTeamInviteToken, renderCollaborationTeamInvitationSmsBody, } from "@proovra/shared";
// =============================================================================
// Path helpers
// =============================================================================
...
function read(rel) {
    ...
    if (!existsSync(full))
        throw new Error(`R14: missing required file: ${rel}`);
...
describe("Phase R14 — Stage 11: plan limits", () => {
    it("ships limits for every plan tier", () => {
        for (const tier of ["FREE", "PAYG", "PRO", "TEAM", "ENTERPRISE"]) {
    ...
    it("getCollaborationTeamPlanLimits falls back to FREE for unknown plans", () => {
        expect(getCollaborationTeamPlanLimits(null)).toBe(COLLABORATION_TEAM_PLAN_LIMITS.FREE);
        expect(getCollaborationTeamPlanLimits("UNKNOWN_TIER")).toBe(COLLABORATION_TEAM_PLAN_LIMITS.FREE);
        expect(getCollaborationTeamPlanLimits("pro")).toBe(COLLABORATION_TEAM_PLAN_LIMITS.PRO);
    });
});
// =============================================================================
...
describe("Phase R14 — Stage 7: invite token format", () => {
    ...
    it("isWellFormedCollaborationTeamInviteToken validates shape", () => {
        ...
        expect(isWellFormedCollaborationTeamInviteToken("ctit_v1_ABCDEFGHIJKLMNOPQRSTUVWXYZ234567ABCDEFGHIJ")).toBe(true);
    });
...
describe("Phase R14 — Stage 2: schema + migration", () => {
    ...
    it("CollaborationTeam.workspaceId references the legacy `Team` row (workspace; rule 1)", () => {
        expect(schema).toMatch(/model CollaborationTeam[\s\S]{0,2000}workspace Team @relation\("CollaborationTeamWorkspace"/);
    });
    ...
    it("ships the Phase-O hardened migration", () => {
        const migPath = "services/api/prisma/migrations/20270201000000_phase_5_collaboration_teams/migration.sql";
...
describe("Phase R14 — Stage 3: service module public surface", () => {
    const svc = read("services/api/src/services/collaboration-team/collaboration-team.service.ts");
    it("exports the canonical functions for CRUD + members + invites + activity + assignments", () => {
        ...
        for (const sym of requiredExports) {
            expect(svc, `missing export ${sym}`).toMatch(new RegExp(`export (async function|function|const) ${sym}\\b`));
        }
    });
    ...
    it("creator is auto-added as LEAD on team creation", () => {
        expect(svc).toMatch(/createCollaborationTeam[\s\S]{0,2500}collaborationTeamMember\.create[\s\S]{0,400}role:\s*"LEAD"/);
    });
});
// =============================================================================
// Stage 4 — routes
...
describe("Phase R14 — Stage 4: API routes", () => {
    ...
    it("LINK-invite response returns rawToken + acceptUrl once; EMAIL/SMS never does", () => {
        ...
        const emailIdx = routes.indexOf('"/v1/collaboration-teams/:teamId/invites/email"');
        const smsIdx = routes.indexOf('"/v1/collaboration-teams/:teamId/invites/sms"');
        const linkIdx = routes.indexOf('"/v1/collaboration-teams/:teamId/invites/link"');
        const revokeIdx = routes.indexOf('"/v1/collaboration-teams/:teamId/invites/:inviteId/revoke"');
...
describe("Phase R14 — Stage 7: delivery wrappers", () => {
    const delivery = read("services/api/src/services/collaboration-team/collaboration-team-delivery.service.ts");
...
describe("Phase R14 — shared package barrel + module placement", () => {
    ...
    it("service module sits under services/api/src/services/collaboration-team/", () => {
        expect(exists("services/api/src/services/collaboration-team/collaboration-team.service.ts")).toBe(true);
        expect(exists("services/api/src/services/collaboration-team/collaboration-team-delivery.service.ts")).toBe(true);
    });
    it("routes module sits under services/api/src/routes/", () => {
        expect(exists("services/api/src/routes/collaboration-teams.routes.ts")).toBe(true);
    });
});
