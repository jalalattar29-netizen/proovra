...
import { REVIEWER_OPS_ALLOWED_LABELS, REVIEWER_OPS_ERROR_CODES, REVIEW_ESCALATION_REASONS, REVIEW_ESCALATION_STATUSES, SEARCH_FORBIDDEN_OVERCLAIM_PHRASES, computeReviewerOpsSlaSnapshot, isAllowedEscalationStatusTransition, isAllowedLifecycleTransition, isAllowedReviewerOpsLabel, rollupReviewerOpsSlaState, stringContainsForbiddenOverclaim, } from "@proovra/shared";
// -----------------------------------------------------------------------------
// Helpers
// -----------------------------------------------------------------------------
...
describe("Phase 25 — engine error codes", () => {
    const engineSrc = readSource("../src/services/reviewer-ops/reviewer-operations-engine.service.ts");
    const escSrc = readSource("../src/services/reviewer-ops/escalation-engine.service.ts");
    ...
    it("ReviewerOpsError catalog covers every brief code", () => {
        for (const code of [
            ...
            expect(REVIEWER_OPS_ERROR_CODES.includes(code)).toBe(true);
    ...
    it("Route layer maps engine errors to HTTP statuses (no throws leaking)", () => {
        ...
        for (const block of catchBlocks) {
            ...
            expect(/sendEngineError|throw err/.test(block), `catch block missing error handling: ${block.slice(0, 80)}`).toBe(true);
        }
    });
...
describe("Phase 25 — escalation engine fingerprint + safe summary", () => {
    const src = readSource("../src/services/reviewer-ops/escalation-engine.service.ts");
    it("fingerprint mixes workflowId + reason + dayBucket", () => {
        expect(src).toMatch(/dayBucket\(input\.now\)/);
        expect(src).toMatch(/\$\{input\.workflowId\}:\$\{input\.reason\}:\$\{dayBucket\(input\.now\)\}/);
    });
...
describe("Phase 25 — workload service", () => {
    const src = readSource("../src/services/reviewer-ops/workload.service.ts");
...
describe("Phase 25 — route layer auth posture", () => {
    ...
    it("reconcile route is cron-secret protected (NOT requireAuth)", () => {
        const reconcileBlock = src.match(/\/v1\/reviewer-ops\/reconcile["'][\s\S]*?\}\);/);
        ...
        const reconcileHeader = src.match(/app\.post\(\s*"\/v1\/reviewer-ops\/reconcile"[^)]*\)/);
