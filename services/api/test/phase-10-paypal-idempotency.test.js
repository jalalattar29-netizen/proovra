...
function findPhase10Migration() {
    ...
    for (const name of readdirSync(MIGRATIONS_DIR)) {
        ...
        if (!statSync(dir).isDirectory())
            continue;
        if (!/phase_10_paypal_webhook_idempotency$/i.test(name))
            continue;
...
describe("Phase 10 — paypal_webhook_events migration (Phase O hardened)", () => {
    ...
    it("a phase_10_paypal_webhook_idempotency migration file exists", () => {
        expect(migrationPath, "expected prisma/migrations/<*>_phase_10_paypal_webhook_idempotency/migration.sql to exist").not.toBeNull();
    });
    it("the migration creates the paypal_webhook_events table (idempotent guarded CREATE TABLE)", () => {
        ...
        const directIfNotExists = /CREATE\s+TABLE\s+IF\s+NOT\s+EXISTS\s+"?paypal_webhook_events"?/i.test(sql);
        const guardedCreate = /IF\s+NOT\s+EXISTS\s*\(\s*SELECT\s+1\s+FROM\s+pg_tables[\s\S]{0,400}CREATE\s+TABLE\s+"?paypal_webhook_events"?/i.test(sql);
        expect(directIfNotExists || guardedCreate, "expected idempotent CREATE TABLE (IF NOT EXISTS or pg_tables-guarded) for paypal_webhook_events").toBe(true);
    });
...
describe("Phase 10 — /paypal webhook handler wires the dedup model + redacts payload", () => {
    ...
    it("the handler references the canonical PaypalWebhookEvent / paypal_webhook_events surface", () => {
        ...
        expect(handler.includes("paypalWebhookEvent") ||
            handler.includes("paypal_webhook_events")).toBe(true);
    });
    it("the handler does NOT log the raw req.body / request.body (no provider payload leakage)", () => {
        ...
        const offenders = [];
        const re = /(?:req\.log|request\.log|app\.log|console)\.[a-z]+\([^)]*\b(?:req|request)\.body\b/g;
        let m;
        while ((m = re.exec(handler)) !== null) {
            offenders.push(m[0]);
        }
        expect(offenders, `forbidden raw-body logging in webhook:\n${offenders.join("\n")}`).toEqual([]);
    });
});
