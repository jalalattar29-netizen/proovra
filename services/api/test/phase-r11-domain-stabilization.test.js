...
import { TARGET_WORKSPACE_KINDS, LEGACY_WORKSPACE_SCOPES, FORBIDDEN_WORKSPACE_KIND_TOKENS, FORBIDDEN_WORKSPACE_KIND_UI_STRINGS, isPersonalWorkspaceKind, isOrganizationWorkspaceKind, coerceLegacyScopeToTargetKind, assertTargetWorkspaceKind, } from "@proovra/shared";
// =============================================================================
// Path helpers
// =============================================================================
 ...
 */
function stripCommentsAndStringLiterals(src) {
    return src
        .replace(/\/\*[\s\S]*?\*\//g, "")
        .replace(/(^|[^:])\/\/.*$/gm, "$1")
        .replace(/"(?:\\.|[^"\\])*"/g, '""')
...
// =============================================================================
const ALLOWLISTED_PATHS_FOR_FORBIDDEN_TOKENS = [
    "docs/architecture/",
    "docs/operations/",
    "docs/product/",
    "docs/recovery/",
...
function isAllowlistedPath(absPath) {
    const rel = relative(repoRoot, absPath).replace(/\\/g, "/");
    return ALLOWLISTED_PATHS_FOR_FORBIDDEN_TOKENS.some((prefix) => rel.startsWith(prefix));
}
// =============================================================================
    ...
    it("TARGET_WORKSPACE_KINDS contains no fake workspace types", () => {
        for (const forbidden of FORBIDDEN_WORKSPACE_KIND_TOKENS) {
            const forbiddenAsKind = forbidden
                .replace(/Workspace$/, "")
                .toUpperCase();
            expect(TARGET_WORKSPACE_KINDS).not.toContain(forbiddenAsKind);
        }
    });
...
describe("INV-2 — Fake workspace terminology forbidden in new code", () => {
    ...
    it("no non-allowlisted file in services/api/src introduces forbidden tokens", () => {
        ...
        expect(violations, `New code introduced forbidden workspace-kind tokens (INV-2). Each is a constitutional violation per architecture-invariants.md. Violations:\n${violations
    ...
    it("no non-allowlisted file in apps/web introduces forbidden tokens", () => {
        ...
        expect(violations, `New code introduced forbidden workspace-kind tokens (INV-2):\n${violations
            .map((v) => `  - ${v.file}: ${v.token}`)
            .join("\n")}`).toEqual([]);
    });
...
describe("INV-3 — Core personal routes are not ORGANIZATION_ONLY", () => {
    it("routeRegistry's core personal routes use PERSONAL_OR_ORG (or NONE), never ORGANIZATION_ONLY", () => {
        ...
        const registryPath = join(webSrc, "lib", "navigation", "routeRegistry.ts");
        ...
        for (const id of corePersonalRouteIds) {
            ...
            const blockRe = new RegExp(`id:\\s*"${id.replace(/\./g, "\\.")}"[\\s\\S]{0,2000}?requiredActiveSpace:\\s*"([^"]+)"`);
            ...
            expect(requiredActiveSpace, `INV-3 violation: route '${id}' has requiredActiveSpace='${requiredActiveSpace}'. Core personal routes must be NONE or PERSONAL_OR_ORG, never ORGANIZATION_ONLY.`).not.toBe("ORGANIZATION_ONLY");
        }
    });
});
// =============================================================================
...
describe("INV-8 — Capability names do not imply fake workspace types", () => {
    it("capability-registry.ts contains no fake-workspace capability names", () => {
        const capRegistry = readFileSync(join(apiSrc, "services", "platform-context", "capability-registry.ts"), "utf8");
        for (const token of FORBIDDEN_WORKSPACE_KIND_TOKENS) {
            ...
            const snake = token
                ...
                .toUpperCase();
            expect(capRegistry.includes(upper), `Capability registry contains '${upper}' — fake workspace name (INV-8)`).toBe(false);
            expect(capRegistry.includes(snake), `Capability registry contains '${snake}' — fake workspace name (INV-8)`).toBe(false);
        }
    });
});
// =============================================================================
...
describe("INV-9 — ACCESS_STATES has no fake-workspace-type entries", () => {
    it("routeAccessResolver.ts ACCESS_STATES contains no fake workspace types", () => {
        const src = readFileSync(join(webSrc, "lib", "navigation", "routeAccessResolver.ts"), "utf8");
        ...
        for (const state of forbiddenStates) {
            expect(src.includes(state), `routeAccessResolver.ts contains '${state}' — forbidden by INV-9`).toBe(false);
        }
    });
});
// =============================================================================
...
describe("Phase 2 — architecture documentation is present", () => {
    ...
    it("docs/architecture/architecture-invariants.md exists and lists INV-1..INV-15", () => {
        ...
        for (const inv of [
            ...
            expect(src.includes(inv), `architecture-invariants.md missing declaration of ${inv}`).toBe(true);
        }
    });
    it("docs/architecture/current-to-target-domain-map.md exists", () => {
        const path = join(docsArchitectureDir, "current-to-target-domain-map.md");
        expect(existsSync(path)).toBe(true);
        const src = readFileSync(path, "utf8");
        expect(src.length).toBeGreaterThan(2000);
    ...
    it("docs/architecture/phase-3-runtime-refactor-readiness.md exists", () => {
        const path = join(docsArchitectureDir, "phase-3-runtime-refactor-readiness.md");
        expect(existsSync(path)).toBe(true);
        const src = readFileSync(path, "utf8");
        expect(src.length).toBeGreaterThan(1500);
    });
