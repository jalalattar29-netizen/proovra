...
import { RUNTIME_OWNERSHIP_MAP, canonicalCanEnterPendingDestruction, canonicalEvaluateExportEligibility, canonicalEvaluateLifecycleTransition, canonicalIsAllowedDestructionReviewTransition, canonicalIsAllowedEvidenceLifecycleTransition, canonicalIsTerminalDestructionReviewStatus, canonicalIsTerminalLifecycleState, canEnterPendingDestruction, decisionAllow, decisionDeny, findRuntimeOwners, ingestCorrelationId, isAllowedDestructionReviewTransition, isAllowedEvidenceLifecycleTransition, isTerminalDestructionReviewStatus, isTerminalLifecycleState, isValidCorrelationId, newCorrelationId, newQueuePayloadEnvelope, newWorkerExecutionContext, QUEUE_RETRY_PROFILES, } from "@proovra/shared";
...
describe("Phase X — Canonical re-exports match underlying helpers", () => {
    it("canonicalIsAllowedEvidenceLifecycleTransition === isAllowedEvidenceLifecycleTransition", () => {
        expect(canonicalIsAllowedEvidenceLifecycleTransition).toBe(isAllowedEvidenceLifecycleTransition);
    });
    ...
    it("canonicalIsAllowedDestructionReviewTransition === isAllowedDestructionReviewTransition", () => {
        expect(canonicalIsAllowedDestructionReviewTransition).toBe(isAllowedDestructionReviewTransition);
    });
    it("canonicalIsTerminalDestructionReviewStatus === isTerminalDestructionReviewStatus", () => {
        expect(canonicalIsTerminalDestructionReviewStatus).toBe(isTerminalDestructionReviewStatus);
    });
});
// -----------------------------------------------------------------------------
...
describe("Phase X — Lifecycle transition canonical formula", () => {
    it("denies terminal-state transitions to anything other than self", () => {
        ...
        expect(decision.reason).toBe("blocked_by_lifecycle_terminal");
    });
    it("denies invalid transitions even without holds", () => {
        ...
        expect(decision.allowed).toBe(false);
        expect(decision.reason).toBe("blocked_by_invalid_transition");
    });
    ...
    it("denies destruction when retention is immutable", () => {
        ...
        expect(decision.allowed).toBe(false);
        expect(decision.reason).toBe("blocked_by_immutable");
    });
...
describe("Phase X — Destruction orchestrator delegates to canonical formula", () => {
    const workerSrc = readSource("../../worker/src/governance/destruction-orchestrator.worker.ts");
    ...
    it("removed the inline isHoldActiveForEvidence + isImmutableForEvidence helpers", () => {
        expect(workerSrc).not.toMatch(/function\s+isHoldActiveForEvidence\s*\(/);
        expect(workerSrc).not.toMatch(/function\s+isImmutableForEvidence\s*\(/);
    });
});
// -----------------------------------------------------------------------------
...
describe("Phase X — Runtime ownership map", () => {
    ...
    it("identifies a single authoritative writer for Evidence.lifecycleState", () => {
        ...
        const lifecycleStateOwners = owners.filter((o) => o.artifact === "Evidence.lifecycleState");
        expect(lifecycleStateOwners.length).toBe(1);
        expect(lifecycleStateOwners[0]?.authoritativeWriter).toBe("api:lifecycle-orchestrator.service");
    });
    it("identifies a single authoritative writer for DestructionReview", () => {
        const owners = findRuntimeOwners("destruction").filter((o) => o.artifact.startsWith("DestructionReview"));
        expect(owners.length).toBe(1);
        expect(owners[0]?.authoritativeWriter).toBe("api:destruction-review.service");
    });
    it("documents the known governance-notification cross-runtime gap", () => {
        const owners = findRuntimeOwners("governance_notification");
        const writer = owners.find((o) => o.artifact === "GovernanceNotification rows");
        expect(writer?.authoritativeWriter).toBe("api:governance-notification.service");
...
describe("Phase X — Operational contracts", () => {
    ...
    it("declares canonical retry profiles for every queue class", () => {
        ...
        expect(QUEUE_RETRY_PROFILES.reconciliation.attempts).toBeGreaterThanOrEqual(1);
...
describe("Phase X — Cross-runtime alignment", () => {
    ...
    it("retention → immutable → destruction: immutable retention blocks destruction at the lifecycle gate AND the destruction gate", () => {
        ...
        expect(lifecycleGate.reason).toBe("blocked_by_immutable");
        ...
        expect(destructionGate.reason).toBe("blocked_by_immutable");
    });
    it("publication → export → visibility: hold precedence is identical (a hold blocks both export and any destruction transition)", () => {
        ...
        expect(destructionR.reason).toBe("blocked_by_hold");
    });
