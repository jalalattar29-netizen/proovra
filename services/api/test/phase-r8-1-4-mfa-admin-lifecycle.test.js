...
const MFA_SVC = readFileSync(apiPath("src/services/security/mfa.service.ts"), "utf8");
const ADMIN_SVC = readFileSync(apiPath("src/services/security/mfa-admin-lifecycle.service.ts"), "utf8");
...
const WORKER_GC = readFileSync(workerPath("src/mfa-challenge-gc.ts"), "utf8");
const WORKER_INDEX = readFileSync(workerPath("src/index.ts"), "utf8");
const SCHEMA = readFileSync(apiPath("prisma/schema.prisma"), "utf8");
const SHARED_SEC = readFileSync(sharedPath("src/security.ts"), "utf8");
const SECURITY_CENTER = readFileSync(webPath("app/(app)/security-center/page.tsx"), "utf8");
const MFA_PAGE = readFileSync(webPath("app/auth/mfa-challenge/page.tsx"), "utf8");
// =============================================================================
...
describe("R8.1.4 — scheduled MFA challenge cleanup", () => {
    ...
    it("test 2: GC deletes via deleteMany keyed by the selected stale ids", () => {
        expect(WORKER_GC).toMatch(/prisma\.mfaPendingChallenge\.deleteMany\(\{\s*where:\s*\{\s*id:\s*\{\s*in:\s*stale\.map/);
    });
    // ---------------------------------------------------------------------------
    ...
    it("test 3: GC batch is bounded (<= 200 per call) AND idempotent under concurrency", () => {
        ...
        expect(WORKER_GC).toMatch(/updateMany\(\{\s*where:\s*\{[\s\S]{0,300}status:\s*\{\s*in:\s*\[\s*["']EMAIL_VERIFICATION_PENDING["'],\s*["']PENDING_ADMIN_REVIEW["']\s*\]/);
...
describe("R8.1.4 — enforcement circuit breaker", () => {
    ...
    it("test 4: smart-mode org-scoped fail-CLOSED path returns MFA_REQUIRED or ENROLLMENT_REQUIRED (NEVER NOT_REQUIRED)", () => {
        ...
        const orgClosedEndMarker = ENFORCEMENT_SVC.indexOf("// Personal user, factor lookup", orgClosedStart);
        expect(orgClosedStart).toBeGreaterThan(0);
        expect(orgClosedEndMarker).toBeGreaterThan(orgClosedStart);
        const orgClosedBlock = ENFORCEMENT_SVC.slice(orgClosedStart, orgClosedEndMarker);
...
describe("R8.1.4 — admin MFA lifecycle controls", () => {
    ...
    it("test 6: every admin lifecycle function calls the shared `assertAdminCanAct` scope guard", () => {
        ...
        for (const fn of publicFns) {
            ...
            expect(body, `${fn} must call assertAdminCanAct`).toMatch(/assertAdminCanAct/);
        }
    });
    // ---------------------------------------------------------------------------
    ...
    it("test 7: scope guard refuses cross-team — checks both admin and target team membership", () => {
        ...
        // 403/404.
        expect(ADMIN_SVC).toMatch(/teamMember\.findFirst\(\{\s*where:\s*\{\s*userId:\s*input\.actorUserId,\s*teamId:\s*input\.teamId/);
        expect(ADMIN_SVC).toMatch(/teamMember\.findFirst\(\{\s*where:\s*\{\s*userId:\s*input\.targetUserId,\s*teamId:\s*input\.teamId/);
    });
    // ---------------------------------------------------------------------------
