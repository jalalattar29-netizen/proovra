...
import { DESTRUCTION_REVIEW_REASONS, DESTRUCTION_REVIEW_STATUSES, EVIDENCE_LIFECYCLE_STATES, EXPORT_ELIGIBILITY_OUTCOMES, LIFECYCLE_EVENT_TYPES, RETENTION_POLICY_SCOPES, RETENTION_POLICY_STATUSES, RETENTION_PRECEDENCE_ORDER, RetentionPolicyCreateInputSchema, RetentionPolicyUpdateInputSchema, SECURITY_EVENT_TYPES, STEP_UP_PURPOSES, canEnterPendingDestruction, isAllowedDestructionReviewTransition, isAllowedEvidenceLifecycleTransition, isAllowedRetentionPolicyTransition, isTerminalDestructionReviewStatus, isTerminalLifecycleState, listAllowedEvidenceLifecycleTransitions, pickHighestPrecedencePolicy, } from "@proovra/shared";
...
describe("Phase 27 — Evidence lifecycle state machine", () => {
    ...
    it("rejects ACTIVE → DESTROYED (must go through PENDING_DESTRUCTION)", () => {
        expect(isAllowedEvidenceLifecycleTransition("ACTIVE", "DESTROYED")).toBe(false);
    });
    ...
    it("supports archive restoration: ARCHIVED → ACTIVE", () => {
        expect(isAllowedEvidenceLifecycleTransition("ARCHIVED", "ACTIVE")).toBe(true);
    });
    it("denies destruction from ON_HOLD even if state machine were permissive", () => {
        expect(canEnterPendingDestruction({
            fromState: "ON_HOLD",
            hasActiveHold: false,
    ...
    it("self-transition is treated as no-op (orchestrator heartbeat)", () => {
        ...
        expect(isAllowedEvidenceLifecycleTransition("DESTROYED", "DESTROYED")).toBe(true);
    });
    it("listAllowedEvidenceLifecycleTransitions matches the table", () => {
        expect(listAllowedEvidenceLifecycleTransitions("UNDER_REVIEW")).toContain("ACTIVE");
...
describe("Phase 27 — Retention policy state machine", () => {
    ...
    it("ACTIVE → PAUSED / SUPERSEDED / ARCHIVED allowed; reverse from ARCHIVED denied", () => {
        ...
        expect(isAllowedRetentionPolicyTransition("ACTIVE", "SUPERSEDED")).toBe(true);
        expect(isAllowedRetentionPolicyTransition("ACTIVE", "ARCHIVED")).toBe(true);
        expect(isAllowedRetentionPolicyTransition("ARCHIVED", "ACTIVE")).toBe(false);
    });
    it("SUPERSEDED can only move to ARCHIVED (cannot un-supersede)", () => {
        expect(isAllowedRetentionPolicyTransition("SUPERSEDED", "ARCHIVED")).toBe(true);
        expect(isAllowedRetentionPolicyTransition("SUPERSEDED", "ACTIVE")).toBe(false);
    });
});
...
describe("Phase 27 — Destruction review state machine", () => {
    ...
    it("PENDING → UNDER_REVIEW / DEFERRED / CANCELLED", () => {
        for (const to of ["UNDER_REVIEW", "DEFERRED", "CANCELLED"]) {
            expect(isAllowedDestructionReviewTransition("PENDING", to)).toBe(true);
        }
        expect(isAllowedDestructionReviewTransition("PENDING", "APPROVED")).toBe(false);
    });
    it("UNDER_REVIEW → APPROVED / DENIED / DEFERRED / CANCELLED", () => {
        for (const to of [
            ...
            expect(isAllowedDestructionReviewTransition("UNDER_REVIEW", to)).toBe(true);
        }
        expect(isAllowedDestructionReviewTransition("UNDER_REVIEW", "EXECUTED")).toBe(false);
    });
    it("APPROVED → EXECUTED / CANCELLED only", () => {
        expect(isAllowedDestructionReviewTransition("APPROVED", "EXECUTED")).toBe(true);
        expect(isAllowedDestructionReviewTransition("APPROVED", "DENIED")).toBe(false);
    });
    it("DENIED → RESTORED / CANCELLED only", () => {
        expect(isAllowedDestructionReviewTransition("DENIED", "RESTORED")).toBe(true);
        expect(isAllowedDestructionReviewTransition("DENIED", "APPROVED")).toBe(false);
    });
...
describe("Phase 27 — Service source wiring", () => {
    it("retention engine imports the shared schemas + emits the right events", () => {
        const src = readSource("../src/services/governance-lifecycle/retention-engine.service.ts");
    ...
    it("export governance returns one of the catalogued outcomes", () => {
        const src = readSource("../src/services/governance-lifecycle/export-governance.service.ts");
...
describe("Phase 27 — Route wiring", () => {
    const routesSrc = readSource("../src/routes/governance-lifecycle.routes.ts");
    ...
    it("exposes all documented retention-policy endpoints", () => {
        ...
        expect(routesSrc).toContain('"/v1/governance/retention-policies/:id"');
        expect(routesSrc).toContain('"/v1/governance/retention-policies/:id/transition"');
        expect(routesSrc).toContain('"/v1/governance/retention-policies/:id/versions"');
        expect(routesSrc).toContain('"/v1/governance/retention-policies/effective"');
    });
    it("exposes destruction-review endpoints + lifecycle + export gate + dashboard", () => {
        ...
        expect(routesSrc).toContain('"/v1/governance/destruction-reviews/:id"');
        expect(routesSrc).toContain('"/v1/governance/destruction-reviews/:id/transition"');
        expect(routesSrc).toContain('"/v1/governance/lifecycle/evidence/:id/events"');
        expect(routesSrc).toContain('"/v1/governance/lifecycle/evidence/:id/transition"');
...
describe("Phase 27 — UI wording sweep", () => {
    ...
    it("destruction queue wording is enterprise-grade", () => {
        assertCleanOf("../../../apps/web/app/(app)/governance/destruction/page.tsx");
    });
});
// -----------------------------------------------------------------------------
// Migration wiring
...
describe("Phase 27 — Migration safety", () => {
    const migrationSrc = readSource("../prisma/migrations/20260606100000_phase27_retention_lifecycle/migration.sql");
