...
import {
  probeRekognition,
  __setTestClient as __setRekognitionClient,
} from "../src/services/redaction/providers/rekognition-client.js";
import {
  probeAzureDocumentIntelligence,
  __setTestClient as __setAzureClient,
} from "../src/services/redaction/providers/azure-document-intelligence-client.js";
import {
  probeDeepgram,
  scanTranscriptForCandidates,
  __setTestClient as __setDeepgramClient,
} from "../src/services/redaction/providers/deepgram-client.js";
...
} from "../src/services/redaction/redaction-policy.service.js";
import {
  REDACTION_BULK_DECISION_MAX_ROWS,
  bulkRecordDetectionDecisions,
} from "../src/services/redaction/redaction-decision-bulk.service.js";
...
const REKOG_SRC = readSource(
  "../../../services/api/src/services/redaction/providers/rekognition-client.ts",
);
const AZURE_SRC = readSource(
  "../../../services/api/src/services/redaction/providers/azure-document-intelligence-client.ts",
);
const DEEPGRAM_SRC = readSource(
  "../../../services/api/src/services/redaction/providers/deepgram-client.ts",
);
const ORCH_SRC = readSource(
  "../../../services/api/src/services/redaction/redaction-detection.service.ts",
);
const POLICY_SRC = readSource(
  "../../../services/api/src/services/redaction/redaction-policy.service.ts",
);
const BULK_SRC = readSource(
...
const EVIDENCE_BYTES_SRC = readSource(
  "../../../services/api/src/services/redaction/evidence-bytes.service.ts",
);
...
describe("Phase 3A Closure — provider wrappers", () => {
  ...
  it("each probe honestly reports NOT_CONFIGURED with bounded reason when creds absent", () => {
    ...
    try {
      ...
      if (prevAk) process.env.AWS_ACCESS_KEY_ID = prevAk;
      if (prevSk) process.env.AWS_SECRET_ACCESS_KEY = prevSk;
      if (prevProf) process.env.AWS_PROFILE = prevProf;
      if (prevRole) process.env.AWS_ROLE_ARN = prevRole;
      if (prevWeb) process.env.AWS_WEB_IDENTITY_TOKEN_FILE = prevWeb;
      if (prevRegion) process.env.AWS_REGION = prevRegion;
      if (prevAzureE) process.env.AZURE_DOCUMENT_INTELLIGENCE_ENDPOINT = prevAzureE;
      if (prevAzureK) process.env.AZURE_DOCUMENT_INTELLIGENCE_KEY = prevAzureK;
      if (prevDg) process.env.DEEPGRAM_API_KEY = prevDg;
    }
  });
...
describe("Phase 3A Closure — orchestrator delegates to SDKs", () => {
  it("provider registry binds every cloud provider to a real wrapper", () => {
    ...
    expect(ORCH_SRC).toMatch(
      /AZURE_DOCUMENT_INTELLIGENCE:[\s\S]+azureAnalyzeDocumentLayout/,
    );
...
describe("Phase 3A Closure — bulk decisions", () => {
  ...
  it("an oversize batch is refused with bounded denial", async () => {
    ...
    if (!res.ok) expect(res.denial).toBe("POLICY_REJECTED");
  });
...
describe("Phase 3A Closure — HTTP routes", () => {
  ...
  it("policy PATCH requires redaction.administer", () => {
    expect(ROUTES_SRC).toMatch(
      /\/v1\/redaction\/policy[\s\S]+?gate\(reply, ctx, "redaction\.administer"\)/,
    );
  });
});
...
describe("Phase 3A Closure — runtime sanity", () => {
  it("Deepgram transcript scanner maps regex hits to bounded AUDIO_RANGE_MS regions", () => {
    ...
    const g = email?.suggestedRegionGeometry as { startMs: number; endMs: number };
