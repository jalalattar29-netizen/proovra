...
import { getOcrProducerMode, getTranscriptProducerMode, summariseProducerModes, OCR_PRODUCER_MODES, TRANSCRIPT_PRODUCER_MODES, } from "../src/services/media-intelligence/producer-mode.js";
describe("Phase 31.19 — OCR producer mode", () => {
    ...
    it("unknown / malformed values collapse to NOT_CONFIGURED", () => {
        expect(getOcrProducerMode({ OCR_PRODUCER_MODE: "bogus" })).toBe("NOT_CONFIGURED");
        expect(getOcrProducerMode({ OCR_PRODUCER_MODE: "" })).toBe("NOT_CONFIGURED");
        expect(getOcrProducerMode({ OCR_PRODUCER_MODE: "NOT_configured" })).toBe("NOT_CONFIGURED");
    });
});
describe("Phase 31.19 — Transcript producer mode", () => {
    ...
    it("returns the env value when it is a known mode", () => {
        for (const mode of TRANSCRIPT_PRODUCER_MODES) {
            expect(getTranscriptProducerMode({ TRANSCRIPT_PRODUCER_MODE: mode })).toBe(mode);
        }
    });
    it("unknown / malformed values collapse to NOT_CONFIGURED", () => {
        expect(getTranscriptProducerMode({ TRANSCRIPT_PRODUCER_MODE: "deepgram-cloud" })).toBe("NOT_CONFIGURED");
        expect(getTranscriptProducerMode({ TRANSCRIPT_PRODUCER_MODE: "" })).toBe("NOT_CONFIGURED");
    });
});
