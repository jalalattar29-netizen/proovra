 ...
 */
const FORBIDDEN_EXTRACTION_PROMISES = [
 ...
 */
const FORBIDDEN_LEGAL_OVERCLAIMS = [
    /\btampered?\b/i,
    /\btamper-?proof\b/i,
    /\bauthentic\b/i,
    /\badmissible\b/i,
 ...
 * claim.
 */
const LEGAL_DISCLAIMER_ALLOWLIST = new Set([
...
describe("Phase G5.1 — Honest-MI UI honesty contract", () => {
    it("no operator-facing string promises OCR / transcript extraction", () => {
        ...
        if (offenders.length > 0) {
            throw new Error("Forbidden OCR/transcript extraction promises found:\n" +
                offenders
                    .map((o) => `  ${o.file}:${o.line} — "${o.phrase}" — ${o.excerpt}`)
                    .join("\n"));
        }
    ...
    it("legal overclaim vocabulary stays absent from apps/web (outside disclaimer allowlist)", () => {
        ...
        for (const f of files) {
            ...
            if (LEGAL_DISCLAIMER_ALLOWLIST.has(rel))
                continue;
            const raw = readFileSync(f, "utf8");
            const stripped = stripComments(raw);
            const lines = stripped.split("\n");
    ...
    it("useMediaIntelligence hook is consumed by the bounded set of files only", () => {
        ...
        if (offenders.length > 0) {
            throw new Error("Unexpected useMediaIntelligence callers (re-read honest-mi-decision.md):\n" +
                offenders.map((o) => `  ${o}`).join("\n"));
        }
        expect(offenders).toEqual([]);
    });
    it("honest-mi-decision.md is present and pins the B-prime decision", () => {
        const doc = readFileSync(fileURLToPath(new URL("../../../docs/architecture/honest-mi-decision.md", import.meta.url)), "utf8");
        expect(doc).toContain("Decision **B-prime**");
        expect(doc).toContain("does not extract OCR text or audio/video transcripts");
