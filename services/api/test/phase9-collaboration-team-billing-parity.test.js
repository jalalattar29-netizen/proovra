...
const TEAMS_ROUTES_PATH = fileURLToPath(new URL("../src/routes/teams.routes.ts", import.meta.url));
const COLLAB_TEAMS_ROUTES_PATH = fileURLToPath(new URL("../src/routes/collaboration-teams.routes.ts", import.meta.url));
const COLLAB_TEAMS_SERVICE_PATH = fileURLToPath(new URL("../src/services/collaboration-team/collaboration-team.service.ts", import.meta.url));
...
describe.skip("Phase 9 — /v1/teams vs /v1/collaboration-teams plan-limit parity (Phase 10 debt)", () => {
    it("legacy /v1/teams route enforces maxOwnedTeams (baseline)", () => {
        ...
        expect(src.includes("maxOwnedTeams"), "teams.routes.ts must reference maxOwnedTeams (legacy enforcement baseline). If this assertion fails, the legacy per-user plan ceiling has been removed and Phase 10 parity work has lost its reference implementation.").toBe(true);
        expect(src.includes("assertUserCanCreateAnotherTeam") ||
            src.includes("TEAM_WORKSPACE_LIMIT_REACHED") ||
            src.includes("TEAM_CREATION_NOT_ALLOWED"), "teams.routes.ts must invoke the per-user plan-ceiling guard (assertUserCanCreateAnotherTeam) or emit one of its canonical error codes.").toBe(true);
    });
    it("collaboration-teams creation enforces the SAME maxOwnedTeams ceiling", () => {
        ...
        expect(combined.includes("maxOwnedTeams"), "Phase 10 debt: neither collaboration-teams.routes.ts nor collaboration-team.service.ts references maxOwnedTeams. The Phase 9 Billing audit found that POST /v1/collaboration-teams bypasses the per-user plan ceiling that POST /v1/teams enforces via assertUserCanCreateAnotherTeam.").toBe(true);
    });
    it("collaboration-teams creation emits a TEAM_WORKSPACE_LIMIT_REACHED-equivalent denial code", () => {
        ...
        const routeSrc = readSource(COLLAB_TEAMS_ROUTES_PATH);
        const combined = `${serviceSrc}\n${routeSrc}`;
        const hasOwnedCeilingDenial = combined.includes("TEAM_WORKSPACE_LIMIT_REACHED") ||
            combined.includes("TEAM_CREATION_NOT_ALLOWED") ||
            combined.includes("MAX_OWNED_TEAMS_REACHED");
        expect(hasOwnedCeilingDenial, "Phase 10 debt: collaboration-team creation does not emit a stable per-user-ceiling denial code. Choose one of TEAM_WORKSPACE_LIMIT_REACHED, TEAM_CREATION_NOT_ALLOWED, or MAX_OWNED_TEAMS_REACHED and emit it from the collaboration-team creation path when the actor's maxOwnedTeams is exhausted.").toBe(true);
    });
    it("collaboration-teams creation reads the actor's personal-workspace plan (not just the active workspace plan)", () => {
        ...
        const routeSrc = readSource(COLLAB_TEAMS_ROUTES_PATH);
        const combined = `${serviceSrc}\n${routeSrc}`;
        expect(combined.includes("getPersonalWorkspaceScope") ||
            combined.includes("personalScope"), "Phase 10 debt: collaboration-team creation does not consult the actor's personal-workspace plan. The legacy /v1/teams route calls getPersonalWorkspaceScope(ownerUserId) before reading maxOwnedTeams — mirror that here.").toBe(true);
    });
});
/**
...
describe("Phase 9 — collaboration-team billing-parity audit finding (active)", () => {
    it("documents the gap: collaboration-team service does not reference maxOwnedTeams today", () => {
        ...
        const routeSrc = readSource(COLLAB_TEAMS_ROUTES_PATH);
        const combined = `${serviceSrc}\n${routeSrc}`;
        expect(combined.includes("maxOwnedTeams"), "Phase 9 audit invariant broken: collaboration-team creation now references maxOwnedTeams. The Phase 10 debt may be closed — remove `.skip` from the parity describe block above and let those assertions run for real.").toBe(false);
    });
});
