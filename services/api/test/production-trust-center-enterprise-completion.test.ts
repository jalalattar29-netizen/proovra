...
const SECTION_LIST = readRepo(
  "apps/web/app/(app)/trust-center/_section-list.tsx",
);
const STATUS_PAGE = readRepo(
  "apps/web/app/(app)/trust-center/status/page.tsx",
);
const AI_DISCLOSURE_PAGE = readRepo(
  "apps/web/app/(app)/trust-center/ai-disclosure/page.tsx",
);
const SECURITY_PAGE = readRepo(
  "apps/web/app/(app)/trust-center/security/page.tsx",
);
const SUBPROCESSORS_PAGE = readRepo(
...
function countKind(source: string, kind: string): number {
...
describe("(6) AI_LEGAL_DISCLAIMER is imported AND surfaced via the constant", () => {
  it("trust-center.service.ts imports AI_LEGAL_DISCLAIMER from ai-policy.js", () => {
    expect(TRUST_SERVICE).toMatch(
      /import\s*\{[^}]*AI_LEGAL_DISCLAIMER[^}]*\}\s*from\s*"\.\.\/ai\/ai-policy\.js"/,
    );
  });
...
describe("(8) Security seed contains no fake certifications or unsupported claims", () => {
  ...
  it('any "SSE-KMS" mention is bounded to a disclaimer in the encryption slug', () => {
    ...
    for (const at of matches) {
      ...
      expect(
        pre,
        `SSE-KMS at offset ${at} is not bounded by a disclaimer`,
      ).toMatch(/(does not|not currently|never)/i);
    }
  });
...
describe("(10) ensureSubprocessorSeed uses prisma.subprocessor.upsert (idempotent)", () => {
  it("subprocessor.service.ts uses upsert keyed on teamId_slug composite", () => {
    ...
    expect(SUBPROCESSOR_SERVICE).toMatch(
      /teamId_slug:\s*\{\s*teamId[^}]+slug[^}]+\}/,
    );
  });
});
...
describe("(15) /v1/trust/* surface matches the canonical whitelist", () => {
  it("every /v1/trust path in routes is in the canonical whitelist", () => {
    ...
    const stripped = Array.from(
      new Set(found.map((s) => s.slice(1, -1))),
    );
...
describe("(21) AI_LEGAL_DISCLAIMER is imported at the top of trust-center.service.ts", () => {
  it("import sits in the top-of-file import block (before SEED_ARTICLES)", () => {
    const importIdx = TRUST_SERVICE.search(
      /import\s*\{[^}]*AI_LEGAL_DISCLAIMER[^}]*\}\s*from/,
    );
