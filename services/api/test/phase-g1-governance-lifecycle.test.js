...
const RETENTION_ENGINE = readSource("../src/services/governance-lifecycle/retention-engine.service.ts");
...
const METRICS_CATALOG = readSource("../../../packages/shared-runtime/src/ops/metrics.service.ts");
const LIFECYCLE_BADGE = readSource("../../../apps/web/components/governance/LifecycleStateBadge.tsx");
const GOVERNANCE_SUMMARY = readSource("../../../apps/web/components/governance/GovernanceSummary.tsx");
const EXPORT_PREFLIGHT = readSource("../../../apps/web/components/governance/ExportEligibilityPreflight.tsx");
const RETENTION_CONFLICT = readSource("../../../apps/web/components/governance/RetentionConflictAlert.tsx");
const RETENTION_PAGE = readSource("../../../apps/web/app/(app)/governance/retention/page.tsx");
// ===========================================================================
...
describe("Phase G1 (B0.4) — retention engine consumes inheritance resolver", () => {
    it("retention engine imports resolveTeamRetentionPolicy from the B0 service", () => {
        expect(RETENTION_ENGINE).toMatch(/import\s*\{\s*resolveTeamRetentionPolicy\s*\}\s*from\s+"\.\.\/organization\/retention-inheritance\.service\.js"/);
    });
    it("EffectiveRetentionDecision declares the bounded source attribution", () => {
        expect(RETENTION_ENGINE).toMatch(/source:\s*"team_policy"\s*\|\s*"org_policy_inherited"\s*\|\s*"none"/);
    });
    it("falls back to org template when no explicit policy matches", () => {
        expect(RETENTION_ENGINE).toMatch(/candidates\.length\s*===\s*0[\s\S]*?resolveTeamRetentionPolicy\(input\.teamId/);
        expect(RETENTION_ENGINE).toMatch(/inheritance\.source\s*===\s*"org_policy_inherited"[\s\S]*?source:\s*"org_policy_inherited"/);
    });
    it("none fallback when no policy at any tier", () => {
        expect(RETENTION_ENGINE).toMatch(/reason:\s*"no_active_policy"[\s\S]*?source:\s*"none"/);
    });
    ...
    it("bumps the inherited-total metric on org template fallback", () => {
        expect(RETENTION_ENGINE).toMatch(/bump\(\s*"retention_policy_inherited_total"\s*\)/);
    });
});
// ===========================================================================
...
describe("Phase G1 — tenancy observability metrics", () => {
    ...
    it("tenancy resolver bumps failure metric on team_not_found", () => {
        expect(TENANCY_RESOLVER).toMatch(/bump\(\s*"tenancy_resolution_failure_total"\s*\)[\s\S]*?team_not_found/);
    });
    it("tenancy resolver bumps orphan metric on team_org_missing", () => {
        expect(TENANCY_RESOLVER).toMatch(/bump\(\s*"orphan_governance_object_total"\s*\)[\s\S]*?team_org_missing/);
    });
    it("tenancy resolver bumps disagreement + cross_org_blocked on tenancy_disagreement", () => {
        expect(TENANCY_RESOLVER).toMatch(/bump\(\s*"tenancy_disagreement_total"\s*\);\s*\n\s*bump\(\s*"cross_org_resolution_blocked_total"\s*\)/);
    });
});
// ===========================================================================
...
describe("Phase G1 — public verify destroyed-state anti-enumeration", () => {
    it("returns 404 with generic message when evidence.lifecycleState === DESTROYED", () => {
        expect(EVIDENCE_ROUTES).toMatch(/lifecycleState\s*===\s*"DESTROYED"[\s\S]*?reply\.code\(404\)\.send\(\{\s*message:\s*"Evidence not found"\s*\}\)/);
    });
    it("records the suppressed outcome in the audit log without leaking state", () => {
        expect(EVIDENCE_ROUTES).toMatch(/outcome:\s*"lifecycle_destroyed"/);
    });
});
// ===========================================================================
...
describe("Phase G1 (F.1) — LifecycleStateBadge component", () => {
    ...
    it("supports inheritance source via the optional prop", () => {
        expect(LIFECYCLE_BADGE).toMatch(/inheritanceSource\?:\s*"team_policy"\s*\|\s*"org_policy_inherited"\s*\|\s*"none"/);
    });
...
describe("Phase G1 (F.2) — GovernanceSummary component", () => {
    ...
    it("renders retention-source labels for the three resolver outcomes", () => {
        ...
        expect(GOVERNANCE_SUMMARY).toContain('org_policy_inherited: "Inherited from organization"');
        expect(GOVERNANCE_SUMMARY).toContain('none: "No retention policy"');
    });
    it("supports the matter variant for Matter Workspace Overview", () => {
        expect(GOVERNANCE_SUMMARY).toMatch(/variant\??:\s*"evidence"\s*\|\s*"matter"/);
    });
    it("explicitly disclaims legal admissibility", () => {
        expect(GOVERNANCE_SUMMARY.replace(/\s+/g, " ")).toMatch(/Not a legal admissibility statement/);
    });
});
// ===========================================================================
...
describe("Phase G1 (F.3) — ExportEligibilityPreflight component", () => {
    it("consumes /v1/governance/export-eligibility with teamId + evidenceId", () => {
        expect(EXPORT_PREFLIGHT).toMatch(/\/v1\/governance\/export-eligibility\?teamId=\$\{encodeURIComponent/);
...
describe("Phase G1 (F.4) — RetentionConflictAlert + retention page mount", () => {
    ...
    it("is mounted on the /governance/retention page", () => {
        ...
        expect(RETENTION_PAGE).toMatch(/<RetentionConflictAlert\s+teamId=\{teamId\s*\?\?\s*null\}/);
    });
});
// ===========================================================================
