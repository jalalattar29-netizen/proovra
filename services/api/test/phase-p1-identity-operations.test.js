...
describe("Phase P1 — canonical sub-paths resolve to procurement-grade surfaces", () => {
    it("/settings/security/saml redirects to /security-center/sso (the R8.2.x SAML console)", () => {
        expect(exists("../../../apps/web/app/(app)/settings/security/saml/page.tsx")).toBe(true);
        const src = readSource("../../../apps/web/app/(app)/settings/security/saml/page.tsx");
    ...
    it("/settings/security/scim redirects to /admin/identity/scim via next.config.js (the SCIM token console)", () => {
        ...
        expect(cfg).toMatch(/source:\s*["']\/settings\/security\/scim["'][\s\S]{0,200}destination:\s*["']\/admin\/identity\/scim["']/);
        // The redirect page file is intentionally absent — routing layer
        // is the single source of truth.
        expect(exists("../../../apps/web/app/(app)/settings/security/scim/page.tsx")).toBe(false);
    });
    it("/settings/security/audit redirects to /admin/identity/timeline via next.config.js (the unified event feed)", () => {
        const cfg = readSource("../../../apps/web/next.config.js");
        expect(cfg).toMatch(/source:\s*["']\/settings\/security\/audit["'][\s\S]{0,200}destination:\s*["']\/admin\/identity\/timeline["']/);
        expect(exists("../../../apps/web/app/(app)/settings/security/audit/page.tsx")).toBe(false);
    });
    it("the SAML redirect docstring still names the backend endpoints it consumes", () => {
        const saml = readSource("../../../apps/web/app/(app)/settings/security/saml/page.tsx");
        ...
        const scimDest = readSource("../../../apps/web/app/(app)/admin/identity/scim/page.tsx");
        expect(scimDest).toContain("/v1/admin/identity/scim/tokens");
        const timelineDest = readSource("../../../apps/web/app/(app)/admin/identity/timeline/page.tsx");
...
describe("Phase P1 — step-up gating on dangerous identity admin mutations", () => {
    const SCIM_PAGE = readSource("../../../apps/web/app/(app)/admin/identity/scim/page.tsx");
    const SSO_PAGE = readSource("../../../apps/web/app/(app)/security-center/sso/page.tsx");
    it("SCIM token create + revoke route through useStepUpAction", () => {
        ...
        expect(SCIM_PAGE).toMatch(/revoke[\s\S]*?stepUp\.runStepUpAction/);
    });
    ...
    it("SAML certificate promotion routes through useStepUpAction", () => {
        ...
        expect(SSO_PAGE).toMatch(/handlePromoteNextCert[\s\S]*?stepUp\.runStepUpAction/);
    });
...
describe("Phase P1 — backend authority preserved (no frontend-only authorization)", () => {
    ...
    const SCIM_SERVICE = readSource("../src/services/access-control/scim.service.ts");
    const SESSION_INVENTORY = readSource("../src/services/access-control/session-inventory.service.ts");
