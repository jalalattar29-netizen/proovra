...
const MIGRATION = readApi(
  "prisma/migrations/20260628100000_phase328c_workflow_causality/migration.sql",
);
...
describe("Phase 32.8C FINAL-2 — migration source-contract", () => {
  ...
  it("workflow event has FK CASCADE to parent workflow", () => {
    expect(MIGRATION).toMatch(
      /operational_workflow_events_workflow_id_fkey[\s\S]*?ON DELETE CASCADE/,
    );
  });
...
describe("Phase 32.8C FINAL-2 — causality engine", () => {
  ...
  it("scan failure returns zero — never blocks dashboard", () => {
    expect(CAUSAL).toMatch(
      /return\s*\{\s*linksPersisted:\s*0,\s*chainsPersisted:\s*0,\s*failed:\s*1\s*\}/,
    );
  });
...
describe("Phase 32.8C FINAL-2 — API routes", () => {
  ...
  it("read routes (GET workflows + chains) use requireOpsActor (read), not action", () => {
    const block = OPS_ROUTES.slice(
      OPS_ROUTES.indexOf('"/v1/ops/workflows"'),
      OPS_ROUTES.indexOf('"/v1/ops/causality/chains/:id"'),
    );
...
describe("Phase 32.8C FINAL-2 — envelope changes", () => {
  ...
  it("envelope incidents section carries workflows + causalityChains", () => {
    expect(COMMAND_CENTER).toMatch(
      /workflows:\s*CommandCenterWorkflowItem\[\]/,
    );
    expect(COMMAND_CENTER).toMatch(
      /causalityChains:\s*CommandCenterCausalityChainItem\[\]/,
    );
  });
  ...
  it("envelope frontend types.ts mirrors the additions", () => {
    ...
    expect(CC_TYPES).toMatch(
      /causalityChains:\s*OperationalCausalityChainItem\[\]/,
    );
  });
});
...
describe("Phase 32.8C FINAL-2 — frontend rendering", () => {
  ...
  it("title summary lists chain + workflow + correlation counts when present", () => {
    expect(CC_TSX).toMatch(
      /chain\$\{causalityChains\.length === 1 \? "" : "s"\}/,
    );
    expect(CC_TSX).toMatch(
  ...
  it("empty state remains operationally meaningful when all four arrays are empty", () => {
    expect(CC_TSX).toMatch(
      /Workflows and causality chains follow incidents — when there are no incidents there is nothing to orchestrate/,
    );
  });
