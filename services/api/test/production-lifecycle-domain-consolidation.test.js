...
function readShared(rel) {
    return readFileSync(fileURLToPath(new URL(`../../../packages/shared/${rel}`, import.meta.url)), "utf8");
}
...
const LIFECYCLE_ROUTES = readApi("src/routes/product-and-lifecycle.routes.ts");
const GOVERNANCE_LIFECYCLE_ROUTES = readApi("src/routes/governance-lifecycle.routes.ts");
// =============================================================================
...
describe("Lifecycle consolidation — bounded error string", () => {
    it("renders a bounded operator-facing error message", () => {
        ...
        expect(SURFACE_A).toMatch(/could not be loaded|unexpected response shape/i);
    });
});
// =============================================================================
...
describe("Lifecycle consolidation — archive URL drift", () => {
    it("Surface A archive page posts to the SINGULAR /transition endpoint", () => {
        expect(SURFACE_A_ARCHIVE).toMatch(/apiFetch\(\s*["']\/v1\/lifecycle\/archive\/transition["']/);
    });
    it("Surface A archive page does NOT POST to the plural /transitions endpoint", () => {
        ...
        expect(SURFACE_A_ARCHIVE).not.toMatch(/apiFetch\(\s*["']\/v1\/lifecycle\/archive\/transitions["'][^)]*method:\s*["']POST["']/m);
    });
});
// =============================================================================
...
describe("Lifecycle consolidation — Surface B is read-only", () => {
    it("Surface B never issues a non-GET apiFetch", () => {
        ...
        expect(SURFACE_B).not.toMatch(/apiFetch\([^)]*method:\s*["'](?:POST|PATCH|DELETE|PUT)["']/m);
    });
});
// =============================================================================
...
describe("Lifecycle consolidation — no new lifecycle routes", () => {
    ...
    const CANONICAL_LIFECYCLE_PATHS = new Set([
    ...
    it("backend declares no /v1/lifecycle/* or /v1/governance/lifecycle/* path outside the canonical whitelist", () => {
        ...
        const matches = combined.match(/"\/v1\/(?:lifecycle|governance\/lifecycle)\/[^"]*"/g) ?? [];
...
describe("Lifecycle consolidation — GUARD: capability-status service", () => {
    it("services/api/src/services/lifecycle/capability-status.service.ts exports computeLifecycleCapabilityStatus", () => {
        const src = readApi("src/services/lifecycle/capability-status.service.ts");
        expect(src).toMatch(/export\s+async\s+function\s+computeLifecycleCapabilityStatus\s*\(/);
    });
});
// =============================================================================
...
describe("Lifecycle consolidation — GUARD: lifecycle-domain-responsibilities doc", () => {
    it("docs/architecture/lifecycle-domain-responsibilities.md exists and is >4000 bytes", () => {
        const docPath = fileURLToPath(new URL("../../../docs/architecture/lifecycle-domain-responsibilities.md", import.meta.url));
