...
function captureAiSurfaceFiles() {
...
const AUDIO_HOOK = readWeb("app/(app)/capture/_hooks/useCaptureAudioRecorder.ts");
...
describe("CR5 Group 1 — upload truth: backend-owned", () => {
    ...
    it("useCaptureSessionOrchestration.ts MUST NOT exceed pre-CR5 baseline", () => {
        expect(statSync(webPath("app/(app)/capture/_hooks/useCaptureSessionOrchestration.ts")).size).toBeLessThanOrEqual(PRE_CR5_ORCH_BYTES);
    });
    it("useResumableUploads.ts MUST NOT exceed pre-CR5 baseline", () => {
        expect(statSync(webPath("app/(app)/capture/_hooks/useResumableUploads.ts")).size).toBeLessThanOrEqual(PRE_CR5_RESUMABLE_BYTES);
    });
    ...
    it("orchestrator JSDoc preserves the 'NEVER calls /v1/evidence/:id/complete' contract", () => {
        expect(/NEVER calls \/v1\/evidence\/[^\n]*\/complete/.test(ORCH)).toBe(true);
    });
    ...
    const UPLOAD_AT_BROWSER_WRITE = /uploadedAt\s*[:=]\s*(?:new\s+Date|Date\.now\b)/;
...
describe("CR5 Group 3 — checklist enforcement", () => {
    ...
    const REQUIRED_BYPASS = /\bblocked\s*=\s*false\s*;\s*\/\/.*required|skipRequiredSteps|bypassChecklist|ignoreRequiredSteps/i;
    for (const surface of CAPTURE_FILES) {
...
describe("CR5 Group 5 — AI boundary: metadata-only", () => {
    ...
    for (const surface of CAPTURE_AI_FILES) {
        it(`${surface.label} :: AI surface does NOT POST a 'file' / 'image' / 'video' body field`, () => {
            const FILE_POST = /method\s*:\s*["'`]POST["'`][^]*?body\s*:[^]*?(?:file|image|video|audio|attachment|payload)/i;
...
describe("CR5 Group 8 — camera / audio cancel + stop safety", () => {
    ...
    it("camera hook exposes a cancel/stop pathway (stopMediaStream / stopVideoRecording / cancelCamera / abort)", () => {
        ...
        expect(/stopMediaStream|stopVideoRecording|cancelCamera|cancel\s*\(|abort\s*\(|stopCamera/
            .test(CAMERA_HOOK)).toBe(true);
    });
    it("audio hook exposes a cancel/stop pathway (stopAudioStream / stopAudioRecording / cancelRecording / abort)", () => {
        expect(/stopAudioStream|stopAudioRecording|cancelRecording|cancel\s*\(|abort\s*\(|stopRecording/
            .test(AUDIO_HOOK)).toBe(true);
    });
    // No device hook may auto-finalize on stop.
    const AUTO_FINALIZE_ON_STOP = /(?:onstop|onStop|stop\s*\(\s*\))[^]{0,200}?\/v1\/evidence\/.*\/complete/;
...
describe("CR5 Group 9 — canonical completeEvidence path", () => {
    ...
    it("the finalize endpoint /v1/evidence/:id/complete is called from EXACTLY the orchestrator (code-only projection)", () => {
        ...
        for (const surface of CAPTURE_FILES) {
            ...
            if (surface.label.endsWith("useCaptureSessionOrchestration.ts")) {
                ...
                expect(matches.length, `${surface.label} unexpectedly calls the finalize endpoint`).toBe(0);
            }
        }
    ...
    it("orchestrator documents 'session complete' precedes batch finalize", () => {
        expect(/COMPLETED|session\s+(?:complete|finalize)/i.test(ORCH)).toBe(true);
    });
});
// ---------------------------------------------------------------------------
...
describe("CR5 Group 10 — forbidden patterns", () => {
    ...
    const HASH_AS_TRUTH = /\bstatus\s*[:=]\s*["'`]VERIFIED["'`].*\bcomputeIntegrityFromBlob/s;
    for (const surface of CAPTURE_FILES) {
