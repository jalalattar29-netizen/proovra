...
import { MEDIA_INTELLIGENCE_RUN_KINDS, MEDIA_INTELLIGENCE_RUN_STATUSES, } from "../src/services/media-intelligence/run-tracker.service.js";
import { GRAPH_CONFIDENCES, GRAPH_EDGE_TYPES, GRAPH_NODE_KINDS, GRAPH_SOURCE_KINDS, GRAPH_VISIBILITY_SCOPES, MANUAL_EDGE_TYPES, MAX_GRAPH_TRAVERSAL_DEPTH, MAX_GRAPH_NODES_PER_RESPONSE, MAX_GRAPH_EDGES_PER_RESPONSE, } from "../src/services/graph/graph-catalog.js";
import { safeSummary, SIGNAL_METADATA, } from "../src/services/media-intelligence/signal-catalog.js";
...
describe("Phase 31.5 — media_intelligence_runs SQL drift patch", () => {
    const sql = readSource("../../../services/api/sql/drift-patches/2026-05-20-media-intelligence-runs.sql");
    ...
    it("idempotent CREATE TABLE", () => {
        expect(sql).toMatch(/CREATE TABLE IF NOT EXISTS\s+"media_intelligence_runs"/);
    });
    it("kind CHECK constraint enumerates every catalog value (across all drift patches)", () => {
        ...
        for (const kind of MEDIA_INTELLIGENCE_RUN_KINDS) {
            expect(combined, `kind ${kind} missing from any drift patch`).toMatch(new RegExp(`'${kind}'`));
        }
    });
    it("status CHECK constraint matches the run-tracker vocabulary", () => {
        expect(sql).toMatch(/CHECK \("status" IN \('PENDING', 'PROCESSING', 'COMPLETED', 'FAILED', 'DISMISSED'\)\)/);
    });
    it("attempt_count is bounded to [0, 5]", () => {
        expect(sql).toMatch(/CHECK \("attempt_count" >= 0 AND "attempt_count" <= 5\)/);
    });
    ...
    it("idempotency unique index per-team when key present", () => {
        expect(sql).toMatch(/CREATE UNIQUE INDEX IF NOT EXISTS "media_intelligence_runs_team_idemp_uk"[\s\S]*?WHERE "idempotency_key" IS NOT NULL/);
    });
});
describe("Phase 32 — investigation-graph SQL drift patch", () => {
    const sql = readSource("../../../services/api/sql/drift-patches/2026-05-20-investigation-graph.sql");
    it("creates all three tables idempotently", () => {
        expect(sql).toMatch(/CREATE TABLE IF NOT EXISTS\s+"investigation_graph_nodes"/);
        expect(sql).toMatch(/CREATE TABLE IF NOT EXISTS\s+"investigation_graph_edges"/);
    ...
    it("node_kind CHECK matches the catalog exactly", () => {
        for (const kind of GRAPH_NODE_KINDS) {
            expect(sql, `node_kind ${kind} missing`).toMatch(new RegExp(`'${kind}'`));
        }
    });
    ...
    it("manual_relationships edge_type restricted to MANUAL_EDGE_TYPES subset", () => {
        ...
        expect(sql).toMatch(/CONSTRAINT "manual_relationships_edge_type_bounded"[\s\S]*?CHECK[\s\S]*?'REFERENCES_SAME_INCIDENT'[\s\S]*?'MANUALLY_LINKED_TO'/);
    });
    it("upsert keys: nodes (team, kind, ext), edges (team, src, tgt, type)", () => {
        expect(sql).toMatch(/CREATE UNIQUE INDEX IF NOT EXISTS "investigation_graph_nodes_team_kind_ext_uk"/);
        expect(sql).toMatch(/CREATE UNIQUE INDEX IF NOT EXISTS "investigation_graph_edges_team_triple_uk"/);
    });
    it("traversal indexes are stale-aware (partial on stale_at_utc IS NULL)", () => {
        expect(sql).toMatch(/CREATE INDEX IF NOT EXISTS "investigation_graph_edges_outbound_idx"[\s\S]*?WHERE "stale_at_utc" IS NULL/);
        expect(sql).toMatch(/CREATE INDEX IF NOT EXISTS "investigation_graph_edges_inbound_idx"[\s\S]*?WHERE "stale_at_utc" IS NULL/);
    });
    it("edges cascade on node delete", () => {
        expect(sql).toMatch(/REFERENCES "investigation_graph_nodes"\("id"\) ON DELETE CASCADE/);
    });
});
// =============================================================================
...
describe("Phase 31.5 — run tracker", () => {
    const src = readSource("../../../packages/shared-runtime/src/media-intelligence/run-tracker.service.ts");
    it("bounded run kinds match the brief catalog", () => {
        for (const expected of [
            ...
            expect(MEDIA_INTELLIGENCE_RUN_KINDS).toContain(expected);
        }
    });
    ...
    it("idempotency check runs BEFORE insert", () => {
        const enqueueBlock = src.match(/enqueueMediaIntelligenceRun[\s\S]*?return\s*\{[\s\S]*?ok:\s*false,\s*reason:\s*"tracker_unavailable"/)?.[0];
    ...
    it("NEVER throws — every public function returns a bounded result", () => {
        ...
        const fnHeaders = src.match(/export async function \w+\([\s\S]*?\): Promise<[\s\S]*?\>/g) ?? [];
        ...
        for (const header of fnHeaders) {
            ...
            const fnBlock = src.match(new RegExp(`export async function ${fnName}[\\s\\S]*?^\\}`, "m"))?.[0];
...
describe("Phase 31.5 — enhanced analyzer signals", () => {
    const forbidden = /\b(tamper(ed|ing)?|forged|forgery|fake|authentic(ity)?|admissible|proves?|confirms?|manipulated|doctored|verified as real|identity confirmed|proof of)\b/i;
    ...
    it("analyzer source emits the 4 new signal types + OCR/transcript", () => {
        ...
        for (const sigType of [
            ...
            expect(src, `analyzer doesn't emit ${sigType}`).toContain(`signalType: "${sigType}"`);
        }
    });
    ...
    it("SIMILAR_FILE_CANDIDATE query excludes exact-hash duplicates (no double-counting)", () => {
        const src = readSource("../../../packages/shared-runtime/src/media-intelligence/analyzer.service.ts");
        expect(src).toMatch(/findSimilarFilenameSizeCount[\s\S]*?ep\."sha256" IS NULL OR ep\."sha256" <> COALESCE/);
    });
});
// =============================================================================
...
describe("Phase 31.5 — signal action endpoint", () => {
    ...
    it("action enum bounded to ACKNOWLEDGED | DISMISSED (no CONFIRMED/PROVEN)", () => {
        expect(src).toMatch(/action:\s*z\.enum\(\["ACKNOWLEDGED",\s*"DISMISSED"\]\)/);
    });
    it("anti-enumeration: signal not in caller's team → 404 not_found", () => {
        expect(src).toMatch(/owner\[0\]!\.team_id !== body\.teamId[\s\S]*?reply\.code\(404\)\.send\(\{\s*error:\s*\{\s*code:\s*"not_found"\s*\}/);
    });
    ...
    it("ack route requires evidence.update_metadata + authorizeOrFail antiEnumeration", () => {
        const ackBlock = src.match(/"\/v1\/media-intelligence\/signals\/:signalId\/action"[\s\S]*?\}\s*,\s*\)\s*;/)?.[0];
...
describe("Phase 32 — graph catalog", () => {
    it("node kinds match the brief (14 entries)", () => {
        ...
        for (const expected of [
            ...
            expect(GRAPH_NODE_KINDS).toContain(expected);
        }
    });
    it("edge types match the brief (16 entries)", () => {
        ...
        for (const expected of [
            ...
            expect(GRAPH_EDGE_TYPES).toContain(expected);
        }
    });
...
describe("Phase 32 — graph builder source contract", () => {
    const src = readSource("../../../packages/shared-runtime/src/graph/graph-builder.service.ts");
    ...
    it("reconcileTeamGraph NEVER throws — top-level try/catch around the body", () => {
        expect(src).toMatch(/export async function reconcileTeamGraph[\s\S]*?try\s*\{[\s\S]*?\}\s*catch\s*\{/);
    });
    ...
    it("upsertNode + upsertEdge use ON CONFLICT for idempotency", () => {
        expect(src).toMatch(/INSERT INTO "investigation_graph_nodes"[\s\S]*?ON CONFLICT[\s\S]*?DO UPDATE SET/);
        expect(src).toMatch(/INSERT INTO "investigation_graph_edges"[\s\S]*?ON CONFLICT[\s\S]*?DO UPDATE SET/);
    });
    it("stale-edge sweep marks edges inactive (never DELETE)", () => {
        expect(src).toMatch(/UPDATE "investigation_graph_edges"\s*"?e"?[\s\S]*?SET "stale_at_utc" = NOW\(\)/);
    ...
    it("manual relationship retraction marks audit row + stales edge (never deletes)", () => {
        expect(src).toMatch(/UPDATE "manual_relationships"\s*SET "status" = 'RETRACTED'/);
...
describe("Phase 32 — graph traversal", () => {
    ...
    it("depth clamped to [0, MAX_GRAPH_TRAVERSAL_DEPTH]", () => {
        expect(src).toMatch(/Math\.max\(\s*0,\s*Math\.min\(input\.depth \?\? 2,\s*MAX_GRAPH_TRAVERSAL_DEPTH\)/);
    });
    it("traversal filters stale_at_utc IS NULL on edges + nodes", () => {
        expect(src).toMatch(/"stale_at_utc" IS NULL[\s\S]*?source_node_id/);
        expect(src).toMatch(/investigation_graph_nodes[\s\S]*?"stale_at_utc" IS NULL/);
    });
    ...
    it("seed not found returns empty subgraph (anti-enumeration: same shape as 'no graph yet')", () => {
        expect(src).toMatch(/seedRows\.length === 0[\s\S]*?return\s*\{[\s\S]*?nodes:\s*\[\],\s*edges:\s*\[\]/);
    });
});
// =============================================================================
// PART 8 — Graph routes
...
describe("Phase 32 — graph routes", () => {
    ...
    it("registers all graph routes (Phase 32 base + Phase 32.5 additions)", () => {
        // Phase 32 base
        expect(src).toMatch(/app\.get\(\s*"\/v1\/graph\/evidence\/:evidenceId"/);
        expect(src).toMatch(/app\.post\(\s*"\/v1\/graph\/relationships\/manual"/);
        expect(src).toMatch(/app\.delete\(\s*"\/v1\/graph\/relationships\/manual\/:manualRelationshipId"/);
    ...
    it("every route uses authorizeOrFail + antiEnumeration: true", () => {
        const authorizeCalls = src.match(/await authorizeOrFail\(\s*req,\s*reply,\s*\{[\s\S]*?\}\s*\)/g) ?? [];
    ...
    it("GET uses evidence.read; POST/DELETE use evidence.update_metadata", () => {
        ...
        const updates = src.match(/permission:\s*"evidence\.update_metadata"/g) ?? [];
    ...
    it("POST refuses self-loop (source === target) with bounded 400 code", () => {
        expect(src).toMatch(/body\.sourceNodeId === body\.targetNodeId[\s\S]*?code:\s*"self_loop_not_allowed"/);
    });
    ...
    it("self-loop check runs BEFORE authorizeOrFail (bounded body validation first)", () => {
        const postBlock = src.match(/"\/v1\/graph\/relationships\/manual"[\s\S]*?retractManual/)?.[0];
...
describe("Phase 32 — observability + reconciliation wiring", () => {
    ...
    it("investigation_graph check never reports CRITICAL (advisory layer)", () => {
        const src = readSource("../../../services/api/src/runtime/runtime-readiness.ts");
        const fn = src.match(/async function checkInvestigationGraph\([\s\S]*?\n\}/)?.[0];
    ...
    it("master reconcile calls reconcileTeamGraph for bounded set of recent teams", () => {
        const src = readSource("../../../services/api/src/routes/ops.routes.ts");
        expect(src).toMatch(/prisma\.team\.findMany\(\{\s*orderBy:\s*\{\s*updatedAt:\s*"desc"\s*\},\s*take:\s*10/);
        expect(src).toMatch(/reconcileTeamGraph\(t\.id\)/);
        // Each call wrapped in try/catch so one team's failure can't
        // block the others.
        expect(src).toMatch(/for\s*\(const t of recentTeams\)\s*\{[\s\S]*?try\s*\{[\s\S]*?reconcileTeamGraph[\s\S]*?\}\s*catch/);
    });
...
describe("Phase 31.5 + 32 — anti-leak invariants", () => {
    ...
    it("no forbidden truth-claim wording in any source string", () => {
        const forbidden = /\b(tamper(ed|ing)?|forged|fake|authentic(ity)?|admissible|proves?|confirms?|verified as real|identity confirmed|manipulated|doctored)\b/i;
        for (let i = 0; i < allSources.length; i++) {
            const literals = allSources[i].match(/"[^"\n]+"/g) ?? [];
            for (const lit of literals) {
                expect(lit, `source ${i} string uses forbidden wording: ${lit}`).not.toMatch(forbidden);
            }
        }
    });
    it("no graph code uses ETag as integrity (no sha256 = etag pattern)", () => {
        for (let i = 0; i < allSources.length; i++) {
            const noComments = allSources[i]
                .replace(/\/\*[\s\S]*?\*\//g, "")
                .replace(/\/\/[^\n]*/g, "");
            expect(noComments, `source ${i} treats ETag as hash`).not.toMatch(/sha256\w*\s*===\s*\w*[Ee][Tt]ag/);
            expect(noComments, `source ${i} treats ETag as hash`).not.toMatch(/\w*[Ee][Tt]ag\s*===\s*sha256/);
        }
    });
});
