...
import { reviewStatusSatisfiesGovernanceGate, } from "../src/services/governance.service.js";
// -----------------------------------------------------------------------------
...
describe("reviewStatusSatisfiesGovernanceGate — Phase 13.5 strict matrix", () => {
    ...
    it("every other stage does NOT satisfy the gate", () => {
        for (const status of [
            ...
            expect(reviewStatusSatisfiesGovernanceGate(status), `${status} should NOT satisfy`).toBe(false);
        }
    });
...
describe("export-gate wiring — source-level", () => {
    ...
    it("canGenerateReport / canGeneratePackage / canPublishPublicVerify all gate on reviewState.isReviewed", async () => {
        ...
        for (const fnName of [
            ...
            const re = new RegExp(`export function ${fnName}[\\s\\S]*?return \\{ allowed: true \\};`);
...
describe("SLA cron route — production fail-closed", () => {
    it("uses requireIntegrationCronSecret + cron-secret middleware fails closed in prod", async () => {
        const { readFile } = await import("node:fs/promises");
        const { fileURLToPath } = await import("node:url");
        const route = await readFile(fileURLToPath(new URL("../src/routes/review-operations.routes.ts", import.meta.url)), "utf8");
        expect(route).toMatch(/requireIntegrationCronSecret/);
        const middleware = await readFile(fileURLToPath(new URL("../src/middleware/cron-secret.ts", import.meta.url)), "utf8");
...
describe("review decision → notification dispatch", () => {
    ...
    it("review-notification dispatch reuses safeSendEmailNotification", async () => {
        const { readFile } = await import("node:fs/promises");
        const { fileURLToPath } = await import("node:url");
        const src = await readFile(fileURLToPath(new URL("../src/services/review-operations/review-notifications.service.ts", import.meta.url)), "utf8");
