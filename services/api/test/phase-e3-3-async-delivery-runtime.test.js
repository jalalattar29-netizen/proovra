...
import { computeNextAttemptAt, isRetryableFailure, WEBHOOK_AUTO_DISABLE_THRESHOLD, WEBHOOK_MAX_TOTAL_ATTEMPTS, WEBHOOK_RETRY_BACKOFF_SECONDS, } from "../src/services/automation/automation-webhook.service.js";
// ---------------------------------------------------------------------------
// Helpers
// ---------------------------------------------------------------------------
...
const MIGRATION = readApi("prisma/migrations/20260803000000_phase_e3_3_async_delivery_runtime/migration.sql");
const PRISMA = readApi("prisma/schema.prisma");
const RUNTIME = readApi("src/services/automation/automation-delivery-runtime.service.ts");
...
describe("E3.3 Test 1 — retry constants are bounded", () => {
    ...
    it("total max wall-clock between first attempt and exhaustion is bounded (≤ 335 s)", () => {
        const totalBackoff = WEBHOOK_RETRY_BACKOFF_SECONDS.reduce((a, b) => a + b, 0);
...
describe("E3.3 Test 2 — isRetryableFailure classifier", () => {
    ...
    it.each([400, 401, 403, 404, 405, 410, 422])("non_2xx:%i (regular 4xx) is NOT retryable", (code) => {
...
describe("E3.3 Test 4 — migration extends status + health", () => {
    it("migration drops + recreates the delivery status CHECK with RETRY_SCHEDULED + RETRY_EXHAUSTED", () => {
        expect(MIGRATION).toMatch(/DROP CONSTRAINT "automation_webhook_deliveries_status_allowlist"/);
        expect(MIGRATION).toMatch(/automation_webhook_deliveries_status_allowlist[\s\S]*?'RETRY_SCHEDULED'/);
        expect(MIGRATION).toMatch(/automation_webhook_deliveries_status_allowlist[\s\S]*?'RETRY_EXHAUSTED'/);
    });
    it("migration adds 3 destination-health columns", () => {
        expect(MIGRATION).toMatch(/ADD COLUMN "consecutive_failure_count" INTEGER NOT NULL DEFAULT 0/);
        expect(MIGRATION).toMatch(/ADD COLUMN "auto_disabled_at" TIMESTAMPTZ/);
        expect(MIGRATION).toMatch(/ADD COLUMN "disabled_reason" VARCHAR\(200\)/);
    });
    it("migration adds partial index for the cron sweeper", () => {
        expect(MIGRATION).toMatch(/CREATE INDEX "automation_webhook_deliveries_status_nextattempt_idx"[\s\S]*?WHERE "status" = 'RETRY_SCHEDULED'/);
    });
...
describe("E3.3 Test 6 — runtime source-level safety", () => {
    ...
    it("runtime uses DB-level claim (updateMany filtered on status) to defend double-process", () => {
        ...
        expect(RUNTIME).toMatch(/status:\s*\{\s*in:\s*\[\s*["']PENDING["'],\s*["']RETRY_SCHEDULED["']\s*\]\s*\}/);
    });
    ...
    it("runtime auto-disables destination at the bounded threshold", () => {
        ...
        expect(RUNTIME).toMatch(/eventType:\s*["']automation_webhook_destination_auto_disabled["']/);
    });
...
describe("E3.3 Test 7 — action handler is async hand-off only", () => {
    it("imports enqueueDelivery from the runtime module", () => {
        expect(ACTIONS).toMatch(/from\s+["']\.\/automation-delivery-runtime\.service\.js["']/);
...
describe("E3.3 Test 10 — documentation + registry", () => {
    ...
    it("registry registers Phase E3.3 with explicit status", () => {
        ...
        expect(registry).toMatch(/\|\s*(Phase )?E3\.3\s*\|[\s\S]*?(CLOSED|CLOSED_WITH_DEFERRED_ITEMS)/);
    });
