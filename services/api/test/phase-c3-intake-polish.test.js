...
const EVIDENCE_REQUEST_SVC = readSource("../src/services/evidence-request.service.ts");
const CASE_ROUTES = readSource("../src/routes/case-workspace.routes.ts");
const INTAKE_PAGE = readSource("../../../apps/web/app/intake/[token]/page.tsx");
const INTAKE_CHECKLIST = readSource("../../../apps/web/components/intake/IntakeChecklist.tsx");
const INTAKE_PROGRESS = readSource("../../../apps/web/components/intake/IntakeCompletionProgress.tsx");
const INTAKE_RERE = readSource("../../../apps/web/components/intake/IntakeReReviewBanner.tsx");
const REQUEST_INSPECTOR = readSource("../../../apps/web/app/(app)/evidence-requests/[id]/page.tsx");
const MATTER_UI = readSource("../../../apps/web/components/cases-experience/MatterWorkspace.tsx");
// ===========================================================================
...
describe("Phase C3 — public intake projection extension", () => {
    it("ExternalRequestPublicView declares the new request-level status field", () => {
        expect(EVIDENCE_REQUEST_SVC).toMatch(/export type ExternalRequestPublicView[\s\S]*?status:\s*string/);
    });
    it("ExternalRequestPublicView declares the deterministic completion summary", () => {
        expect(EVIDENCE_REQUEST_SVC).toMatch(/completion:\s*\{[\s\S]*?requiredTotal[\s\S]*?requiredFulfilled[\s\S]*?optionalTotal[\s\S]*?optionalFulfilled[\s\S]*?completionPercent[\s\S]*?reviewReady[\s\S]*?needsMoreInfo/);
    });
    it("Deliverable projection surfaces fulfilledCount + captureAfterRequest", () => {
        expect(EVIDENCE_REQUEST_SVC).toMatch(/deliverables:\s*Array<[\s\S]*?fulfilledCount:\s*number/);
        expect(EVIDENCE_REQUEST_SVC).toMatch(/deliverables:\s*Array<[\s\S]*?captureAfterRequest:\s*boolean/);
    });
    it("projectRequestForExternalView computes the completion summary deterministically", () => {
        expect(EVIDENCE_REQUEST_SVC).toMatch(/const\s+isSatisfied\s*=\s*\(status:\s*string\)\s*=>[\s\S]*?"FULFILLED"[\s\S]*?"WAIVED"/);
        expect(EVIDENCE_REQUEST_SVC).toMatch(/const\s+completionPercent\s*=[\s\S]*?Math\.round\(\(allSatisfied\s*\/\s*allItems\)\s*\*\s*100\)/);
        expect(EVIDENCE_REQUEST_SVC).toMatch(/const\s+reviewReady\s*=[\s\S]*?requiredFulfilled\s*===\s*requiredItems\.length/);
    });
    it("review readiness never silently flips on optional-only requests", () => {
        expect(EVIDENCE_REQUEST_SVC).toMatch(/requiredItems\.length\s*===\s*0\s*\|\|\s*requiredFulfilled\s*===\s*requiredItems\.length/);
    });
});
// ===========================================================================
...
describe("Phase C3 — GET /v1/cases/:id/evidence-requests aggregator", () => {
    it("registers the endpoint behind requireAuth + requireCaseAccess", () => {
        ...
        const handler = CASE_ROUTES.match(/evidence-requests[\s\S]*?\}\s*,\s*\)\s*;/);
        expect(handler).toBeTruthy();
        expect(handler[0]).toContain("requireCaseAccess");
    });
    it("scope-binds the query to the case's teamId AND caseId", () => {
        expect(CASE_ROUTES).toMatch(/prisma\.evidenceRequest\.findMany\(\{[\s\S]*?teamId:\s*caseRow\.teamId[\s\S]*?caseId:\s*params\.id/);
    });
    it("returns per-request completion summary + bounded status counts", () => {
        ...
        expect(CASE_ROUTES).toMatch(/completionPercent\s*=[\s\S]*?Math\.round\(\(satisfied\s*\/\s*total\)\s*\*\s*100\)/);
...
describe("Phase C3 — /intake/[token] page surfaces the C3 components", () => {
    it("imports the three new components", () => {
        expect(INTAKE_PAGE).toMatch(/import\s*\{?\s*IntakeChecklist\s*\}?\s*from\s+"\.\.\/\.\.\/\.\.\/components\/intake\/IntakeChecklist"/);
        expect(INTAKE_PAGE).toMatch(/import\s*\{[\s\S]*?IntakeCompletionProgress[\s\S]*?\}\s*from\s+"\.\.\/\.\.\/\.\.\/components\/intake\/IntakeCompletionProgress"/);
        expect(INTAKE_PAGE).toMatch(/import\s*\{?\s*IntakeReReviewBanner\s*\}?\s*from\s+"\.\.\/\.\.\/\.\.\/components\/intake\/IntakeReReviewBanner"/);
    });
    it("renders the re-request banner when request.completion.needsMoreInfo", () => {
        expect(INTAKE_PAGE).toMatch(/request\.completion\.needsMoreInfo[\s\S]*?<IntakeReReviewBanner/);
    });
    it("renders the completion progress driven by request.completion", () => {
        expect(INTAKE_PAGE).toMatch(/<IntakeCompletionProgress\s+completion=\{request\.completion\}/);
    });
    it("renders the checklist driven by request.deliverables", () => {
        expect(INTAKE_PAGE).toMatch(/<IntakeChecklist\s+deliverables=\{request\.deliverables\}/);
    });
...
describe("Phase C3 — IntakeCompletionProgress component", () => {
    it("renders the backend-provided percent without recomputing it", () => {
        expect(INTAKE_PROGRESS).toMatch(/Math\.max\(0,\s*Math\.min\(100,\s*completion\.completionPercent\)\)/);
    });
    it("renders Review-ready chip ONLY when completion.reviewReady is true", () => {
        expect(INTAKE_PROGRESS).toMatch(/completion\.reviewReady\s*\?[\s\S]*?Review-ready[\s\S]*?:[\s\S]*?Required items remaining/);
    });
...
describe("Phase C3 — IntakeReReviewBanner component", () => {
    it("only surfaces the blocking required deliverables (status in PENDING/PARTIALLY_FULFILLED/REJECTED)", () => {
        expect(INTAKE_RERE).toMatch(/status\s*===\s*"PENDING"[\s\S]*?status\s*===\s*"PARTIALLY_FULFILLED"[\s\S]*?status\s*===\s*"REJECTED"/);
    });
...
describe("Phase C3 — /evidence-requests/[id] reviewer inspector", () => {
    ...
    it("invokes the existing audited waive endpoint", () => {
        expect(REQUEST_INSPECTOR).toMatch(/\/deliverables\/\$\{encodeURIComponent\(deliverableId\)\}\/waive/);
    });
    it("invokes the existing audited response review endpoint", () => {
        expect(REQUEST_INSPECTOR).toMatch(/\/responses\/\$\{encodeURIComponent\(responseId\)\}\/review/);
    });
...
describe("Phase C3 — Matter Workspace Evidence tab extension", () => {
    it("fetches the new /v1/cases/:id/evidence-requests aggregator", () => {
        expect(MATTER_UI).toContain("/v1/cases/${encodeURIComponent(envelope.case.id)}/evidence-requests");
    });
    ...
    it("preserves the C1 empty-state contract — EmptyState still fires when BOTH evidence and requests are empty", () => {
        expect(MATTER_UI).toMatch(/ev\.items\.length\s*===\s*0\s*&&\s*requestRows\.length\s*===\s*0/);
        const tabFn = MATTER_UI.match(/function\s+EvidenceTab\s*\([\s\S]*?\n\}\s*\n/);
