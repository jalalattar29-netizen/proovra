...
const SVC_ACTIVITY = readSource(
  "../../../services/api/src/services/external-review/portal-activity.service.ts",
);
const SVC_DECISIONS = readSource(
  "../../../services/api/src/services/external-review/portal-decisions.service.ts",
);
const SVC_COMMENTS = readSource(
  "../../../services/api/src/services/external-review/portal-comments.service.ts",
);
const SVC_WATERMARK = readSource(
  "../../../services/api/src/services/external-review/portal-watermark.service.ts",
);
...
const UI_REVIEW = readSource(
  "../../../apps/web/app/portal/[token]/work/[workflowId]/page.tsx",
);
const UI_WATERMARK = readSource(
  "../../../apps/web/components/external-portal/WatermarkOverlay.tsx",
);
const UI_CLIENT = readSource(
  "../../../apps/web/lib/external-portal/portal-client.ts",
);
const UI_INTERNAL = readSource(
  ...
  it("verifies a signature built with the same secret", async () => {
    ...
    try {
      const mod = await import(
        "../src/services/external-review/portal-watermark.service.js"
      );
      ...
      if (before === undefined) delete process.env.PORTAL_WATERMARK_HMAC;
      else process.env.PORTAL_WATERMARK_HMAC = before;
    }
  });
...
describe("Phase 2B — Prisma schema", () => {
  ...
  it("declares ExternalReviewComment (threaded)", () => {
    ...
    const block = SCHEMA.match(
      /^model ExternalReviewComment \{[\s\S]*?\n\}/m,
    )![0];
  ...
  it("declares ExternalReviewDecision with grant+workflow uniqueness", () => {
    ...
    const block = SCHEMA.match(
      /^model ExternalReviewDecision \{[\s\S]*?\n\}/m,
    )![0];
...
describe("Phase 2B — backend services", () => {
  ...
  it("invitation service exposes issue / revoke / accept / list", () => {
    ...
    expect(SVC_INVITE).toMatch(
      /export\s+async\s+function\s+listInvitationsForTeam/,
    );
  });
...
describe("Phase 2B — internal invitation admin", () => {
  ...
  it("registered in REVIEW pillar nav", () => {
    ...
    expect(PILLAR_REGISTRY).toMatch(
      /"workspace\.review_external"[\s,]+"REVIEW"/,
    );
  });
});
