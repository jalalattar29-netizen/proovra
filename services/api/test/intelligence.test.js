...
import { requestAiAssistance, setAiAssistanceProvider, } from "../src/services/intelligence/ai-assistance.service.js";
...
describe("AI assistance wrapper", () => {
    ...
    afterEach(() => {
        ...
        if (prevEnabled === undefined)
            ...
            process.env.OPENAI_AI_ENABLED = prevEnabled;
    });
    ...
    it("REJECTS provider output that contains forbidden authenticity phrases", async () => {
        ...
        expect(result.summary).toBeNull();
        expect(result.cautions.some((c) => /cannot present/i.test(c))).toBe(true);
        expect(result.disclaimer).toMatch(/advisory/i);
    });
...
describe("search service — source-level governance scope", () => {
    it("publishable scope adds publicVerifyState = PUBLISHED to the WHERE clause", async () => {
        ...
        const src = await readFile(fileURLToPath(new URL("../src/services/intelligence/search.service.ts", import.meta.url)), "utf8");
...
describe("intelligence routes — anti-enumeration + scope", () => {
    it("never responds 403 for non-members", async () => {
        const { readFile } = await import("node:fs/promises");
        const { fileURLToPath } = await import("node:url");
        const src = await readFile(fileURLToPath(new URL("../src/routes/intelligence.routes.ts", import.meta.url)), "utf8");
...
describe("similarity service — wording contract", () => {
    it("upsert path uses the canonical similaritySummaryFor (advisory wording)", async () => {
        const { readFile } = await import("node:fs/promises");
        const { fileURLToPath } = await import("node:url");
        const src = await readFile(fileURLToPath(new URL("../src/services/intelligence/similarity.service.ts", import.meta.url)), "utf8");
...
describe("semantic search foundation", () => {
    it("returns enabled=false when SEMANTIC_SEARCH_ENABLED is unset", async () => {
        ...
        try {
            const { searchSemantic } = await import("../src/services/intelligence/semantic.service.js");
            ...
            if (prev === undefined)
                ...
                process.env.SEMANTIC_SEARCH_ENABLED = prev;
        }
    });
});
describe("extraction service — provider abstraction", () => {
    it("no-op providers are the default when env is unset", async () => {
        ...
        try {
            const { configuredOcrProviderName, configuredTranscriptProviderName, } = await import("../src/services/intelligence/extraction.service.js");
            expect(configuredOcrProviderName()).toBe("noop");
            expect(configuredTranscriptProviderName()).toBe("noop");
        }
        finally {
            if (prevOcr === undefined)
                delete process.env.OCR_PROVIDER;
            else
                process.env.OCR_PROVIDER = prevOcr;
            if (prevTranscript === undefined)
                delete process.env.TRANSCRIPT_PROVIDER;
            else
                process.env.TRANSCRIPT_PROVIDER = prevTranscript;
        }
    });
});
