...
import { ACCESS_REVIEW_GRANT_KINDS, DEPARTMENT_MEMBERSHIP_ROLES, POLICY_EVALUATION_DECISIONS, SECURITY_CONTROL_CONFIDENCE, STATUS_PROBE_VERDICTS, TRUST_ARTICLE_DRIFT_STATES, TRUST_LIFECYCLE_CATEGORIES, TRUST_LIFECYCLE_CODES, classifyTrustLifecycleCategory, } from "@proovra/shared";
// ---------------------------------------------------------------------------
// Helpers
// ---------------------------------------------------------------------------
...
describe("Phase 4A Closure — shared contracts", () => {
    it("TRUST_ARTICLE_DRIFT_STATES is bounded (CURRENT / STALE / NEEDS_REVIEW)", () => {
        expect([...TRUST_ARTICLE_DRIFT_STATES].sort()).toEqual(["CURRENT", "NEEDS_REVIEW", "STALE"]);
    });
    it("ACCESS_REVIEW_GRANT_KINDS covers the propagation engine routes", () => {
        expect([...ACCESS_REVIEW_GRANT_KINDS].sort()).toEqual(["DELEGATED_ADMIN", "DEPARTMENT_MEMBERSHIP", "EXTERNAL_REVIEW"]);
    });
    it("POLICY_EVALUATION_DECISIONS is bounded (ALLOW / WARN / BLOCK)", () => {
        expect([...POLICY_EVALUATION_DECISIONS].sort()).toEqual(["ALLOW", "BLOCK", "WARN"]);
    });
    it("STATUS_PROBE_VERDICTS includes UNKNOWN as honest default", () => {
        expect([...STATUS_PROBE_VERDICTS].sort()).toEqual(["DEGRADED", "DOWN", "OPERATIONAL", "UNKNOWN"]);
    });
    it("SECURITY_CONTROL_CONFIDENCE is bounded", () => {
        expect([...SECURITY_CONTROL_CONFIDENCE].sort()).toEqual(["IMPLEMENTED", "PARTIAL", "PLANNED", "UNAVAILABLE"]);
    });
    it("DEPARTMENT_MEMBERSHIP_ROLES is bounded (MEMBER / LEAD / ADMIN)", () => {
        expect([...DEPARTMENT_MEMBERSHIP_ROLES].sort()).toEqual(["ADMIN", "LEAD", "MEMBER"]);
    });
    ...
    it("TRUST_LIFECYCLE_CATEGORIES is the disjoint 2-tuple", () => {
        expect([...TRUST_LIFECYCLE_CATEGORIES].sort()).toEqual(["GOVERNANCE_LIFECYCLE", "TRUST_LIFECYCLE"]);
    });
    it("classifyTrustLifecycleCategory routes codes into the right bucket", () => {
        expect(classifyTrustLifecycleCategory("TRUST_ARTICLE_CREATED")).toBe("TRUST_LIFECYCLE");
        expect(classifyTrustLifecycleCategory("SUBPROCESSOR_CREATED")).toBe("TRUST_LIFECYCLE");
        expect(classifyTrustLifecycleCategory("STATUS_INCIDENT_CREATED")).toBe("TRUST_LIFECYCLE");
        expect(classifyTrustLifecycleCategory("ACCESS_REVIEW_DECIDED")).toBe("GOVERNANCE_LIFECYCLE");
        expect(classifyTrustLifecycleCategory("DEPARTMENT_CREATED")).toBe("GOVERNANCE_LIFECYCLE");
        expect(classifyTrustLifecycleCategory("POLICY_VIOLATION")).toBe("GOVERNANCE_LIFECYCLE");
    });
});
// ---------------------------------------------------------------------------
// 2. Prisma + closure migration
...
describe("Phase 4A Closure — Prisma + closure migration", () => {
    ...
    it("closure migration is Phase O-Final compliant", () => {
        const sql = readSrc("../prisma/migrations/20261225000000_phase_4a_enterprise_closure/migration.sql");
...
describe("Phase 4A Closure — service module surface", () => {
    ...
    it("department-membership service exposes grant / revoke / list", async () => {
        const m = await import("../src/services/governance/department-membership.service.js");
    ...
    it("department-scope service exposes resolve / where / assert helpers", async () => {
        const m = await import("../src/services/governance/department-scope.service.js");
    ...
    it("policy-evaluation service exposes the six evaluators", async () => {
        const m = await import("../src/services/governance/policy-evaluation.service.js");
    ...
    it("security-claim-check service exposes run + list", async () => {
        const m = await import("../src/services/trust/security-claim-check.service.js");
    ...
    it("trust-and-governance-audit service exposes every subject emitter", async () => {
        const m = await import("../src/services/trust/trust-and-governance-audit.service.js");
    ...
    it("access-review-escalation service exposes escalate + list", async () => {
        const m = await import("../src/services/governance/access-review-escalation.service.js");
...
describe("Phase 4A Closure — delegated tier enforcement on routes", () => {
    ...
    it("trust article + subprocessor AUTHORING POSTs are tier-gated", () => {
        ...
        expect(src).toMatch(/"\/v1\/trust\/subprocessors"[\s\S]{0,400}requireDelegatedTierAny/);
    });
    it("trust SEED POSTs are NOT delegated-tier-gated (workspace-member self-heal)", () => {
        ...
        const seedArticleHandler = nextRouteIdx > 0
        ...
        const seedSubHandler = nextSubIdx > 0 ? seedSubSlice.slice(0, nextSubIdx) : seedSubSlice;
    ...
    it("status mutations are tier-gated", () => {
        expect(src).toMatch(/"\/v1\/trust\/status\/incidents"[\s\S]{0,400}requireDelegatedTierAny/);
        expect(src).toMatch(/"\/v1\/trust\/status\/incidents\/:id\/updates"[\s\S]{0,400}requireDelegatedTierAny/);
        expect(src).toMatch(/"\/v1\/trust\/status\/maintenance"[\s\S]{0,400}requireDelegatedTierAny/);
    });
    it("department + delegated-admin + membership mutations are tier-gated", () => {
        expect(src).toMatch(/"\/v1\/governance\/departments"[\s\S]{0,400}requireDelegatedTier\("ORG_ADMIN"\)/);
        expect(src).toMatch(/"\/v1\/governance\/departments\/:id\/archive"[\s\S]{0,400}requireDelegatedTier\("ORG_ADMIN"\)/);
        expect(src).toMatch(/"\/v1\/governance\/delegated-admin\/:id\/revoke"[\s\S]{0,400}requireDelegatedTier\("ORG_ADMIN"\)/);
        expect(src).toMatch(/"\/v1\/governance\/departments\/:id\/memberships"[\s\S]{0,400}requireDelegatedTier\("DEPARTMENT_ADMIN"\)/);
        expect(src).toMatch(/"\/v1\/governance\/departments\/memberships\/:id\/revoke"[\s\S]{0,400}requireDelegatedTier\("DEPARTMENT_ADMIN"\)/);
    });
    it("policy + access-review + cross-org mutations are tier-gated", () => {
        expect(src).toMatch(/"\/v1\/governance\/policies"[\s\S]{0,400}requireDelegatedTierAny/);
        expect(src).toMatch(/"\/v1\/governance\/policies\/:id\/activate"[\s\S]{0,400}requireDelegatedTierAny/);
        expect(src).toMatch(/"\/v1\/governance\/policies\/:id\/deprecate"[\s\S]{0,400}requireDelegatedTierAny/);
        expect(src).toMatch(/"\/v1\/governance\/policies\/:id\/assignments"[\s\S]{0,400}requireDelegatedTier\("ORG_ADMIN"\)/);
        expect(src).toMatch(/"\/v1\/governance\/access-reviews\/campaigns"[\s\S]{0,400}requireDelegatedTierAny/);
        expect(src).toMatch(/"\/v1\/governance\/access-reviews\/items\/:id\/decision"[\s\S]{0,400}requireDelegatedTierAny/);
        expect(src).toMatch(/"\/v1\/governance\/cross-org-review"[\s\S]{0,400}requireDelegatedTierAny/);
        expect(src).toMatch(/"\/v1\/governance\/cross-org-review\/:id\/accept"[\s\S]{0,400}requireDelegatedTier\("ORG_ADMIN"\)/);
    });
    it("drift + security-claims scan routes are tier-gated", () => {
        expect(src).toMatch(/"\/v1\/trust\/drift\/scan"[\s\S]{0,400}requireDelegatedTierAny/);
        expect(src).toMatch(/"\/v1\/trust\/security-claims\/scan"[\s\S]{0,400}requireDelegatedTierAny/);
    });
});
// ---------------------------------------------------------------------------
// 5. Trust mutation audit emissions
...
describe("Phase 4A Closure — trust mutation audit emissions", () => {
    ...
    it("cross-org-review.service emits emitCrossOrgEvent", () => {
        const src = readSrc("../src/services/governance/cross-org-review.service.ts");
        expect(src).toContain("emitCrossOrgEvent");
    });
    it("delegated-admin.service emits emitDelegatedAdminEvent", () => {
        const src = readSrc("../src/services/governance/delegated-admin.service.ts");
    ...
    it("access-review.service emits emitAccessReviewDecisionEvent", () => {
        const src = readSrc("../src/services/governance/access-review.service.ts");
...
describe("Phase 4A Closure — policy evaluation behaviour", () => {
    ...
    function buildPrismaStub(assignments) {
        return {
    ...
    it("evaluateIntelligencePolicy returns BLOCK when disallowedProviders matches + mode=BLOCK", async () => {
        const { evaluateIntelligencePolicy } = await import("../src/services/governance/policy-evaluation.service.js");
    ...
    it("evaluateSecurityPolicy returns BLOCK when requireMfa=true and mfaSatisfied=false", async () => {
        const { evaluateSecurityPolicy } = await import("../src/services/governance/policy-evaluation.service.js");
        const prismaStub = buildPrismaStub([
            buildPolicyAssignment({
                kind: "SECURITY",
                enforcementMode: "BLOCK",
...
describe("Phase 4A Closure — access review REVOKED propagates", () => {
    it("REVOKED on a delegated_admin grant calls revokeDelegatedAdmin", async () => {
        const delegated = await import("../src/services/governance/delegated-admin.service.js");
        const revokeSpy = vi
            .spyOn(delegated, "revokeDelegatedAdmin")
            .mockResolvedValue({ ok: true });
        const { recordItemDecision } = await import("../src/services/governance/access-review.service.js");
        ...
        const callArgs = revokeSpy.mock.calls[0]?.[0];
        expect(callArgs?.grantId).toBe(grantUuid);
        revokeSpy.mockRestore();
    });
    it("REVOKED on an external_review grant calls revokeInvitation", async () => {
        const invMod = await import("../src/services/external-review/portal-invitation.service.js");
        const invSpy = vi
        ...
        const callArgs = invSpy.mock.calls[0]?.[0];
        expect(callArgs?.grantId).toBe(grantUuid);
        invSpy.mockRestore();
    });
});
...
describe("Phase 4A Closure — cross-org accept calls portal", () => {
    it("acceptCrossOrgReview calls issueInvitation when externalReviewGrantId is null", async () => {
        ...
        const issueSpy = vi
            ...
            .mockResolvedValue({ ok: true, grantId: "portal-grant-1" });
...
describe("Phase 4A Closure — status probes", () => {
    it("probeApi returns OPERATIONAL with componentKey=API", async () => {
        const { probeApi } = await import("../src/services/trust/status-probes.service.js");
    ...
    it("probeRedis returns UNKNOWN without REDIS_URL", async () => {
        ...
        try {
            const { probeRedis } = await import("../src/services/trust/status-probes.service.js");
    ...
    it("probeStorage returns UNKNOWN without S3_EVIDENCE_BUCKET", async () => {
        ...
        try {
            const { probeStorage } = await import("../src/services/trust/status-probes.service.js");
            ...
            if (prior !== undefined)
                process.env.S3_EVIDENCE_BUCKET = prior;
        }
    });
});
// ---------------------------------------------------------------------------
...
describe("Phase 4A Closure — department scope", () => {
    it("resolveUserDepartmentScope returns unrestricted=true for GLOBAL_ADMIN", async () => {
        const { resolveUserDepartmentScope } = await import("../src/services/governance/department-scope.service.js");
        const prismaStub = {
    ...
    it("assertDepartmentAccess: unrestricted envelope is always ok", async () => {
        const { assertDepartmentAccess } = await import("../src/services/governance/department-scope.service.js");
    ...
    it("buildDepartmentScopeWhere returns undefined for unrestricted envelopes", async () => {
        const { buildDepartmentScopeWhere } = await import("../src/services/governance/department-scope.service.js");
...
describe("Phase 4A Closure — security claim drift detection", () => {
    it("runSecurityClaimChecks persists one row per SECURITY section + SCIM is UNAVAILABLE", async () => {
        const { runSecurityClaimChecks } = await import("../src/services/trust/security-claim-check.service.js");
...
describe("Phase 4A Closure — trust article drift detection", () => {
    it("runTrustArticleDriftScan flips an article with a missing reference to STALE", async () => {
        const { runTrustArticleDriftScan } = await import("../src/services/trust/trust-drift.service.js");
        const updates = [];
        const prismaStub = {
            trustCenterArticle: {
                findMany: async () => [
...
describe("Phase 4A Closure — verification package wires manifests", () => {
    it("verification-package.ts imports + invokes buildTrustAndGovernanceManifests", () => {
        const src = readSrc("../../worker/src/verification-package.ts");
    ...
    it("verification-package-trust-and-governance.ts produces the five Phase 4A manifest paths", () => {
        const src = readSrc("../../worker/src/verification-package-trust-and-governance.ts");
...
describe("Phase 4A Closure — audit federator surfaces trust + governance lifecycle", () => {
    it("audit-transparency.service handles TRUST_LIFECYCLE and GOVERNANCE_LIFECYCLE", () => {
        const src = readSrc("../src/services/intelligence/audit-transparency.service.ts");
...
describe("Phase 4A Closure — governance dashboard reality", () => {
    const src = readSrc("../src/services/governance/governance-dashboard.service.ts");
    it("securityHealth is derived from real queries — not a literal { mfaCoveragePct: 0, ... }", () => {
        expect(src).not.toMatch(/securityHealth\s*:\s*\{\s*mfaCoveragePct\s*:\s*0\s*,\s*samlEnabled\s*:\s*false\s*,\s*scimEnabled\s*:\s*false\s*\}/);
...
describe("Phase 4A Closure — UI completion", () => {
    it("per-campaign access review items page exists with bounded data anchor", () => {
        const relPath = "../../../apps/web/app/(app)/governance-platform/access-reviews/[campaignId]/page.tsx";
    ...
    it("trust-center page imports DriftBadge", () => {
        const src = readSrc("../../../apps/web/app/(app)/trust-center/page.tsx");
