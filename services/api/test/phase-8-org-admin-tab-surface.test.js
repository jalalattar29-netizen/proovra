...
describe("Phase 8 — admin route registry shape", () => {
    ...
    for (const { id, href } of ADMIN_ROUTE_IDS) {
        describe(`route ${id}`, () => {
            ...
            const blockMatch = registry.match(new RegExp(`id:\\s*"${id.replace(/\./g, "\\.")}"[\\s\\S]{0,1000}?sidebarEligible:\\s*(true|false)\\s*,`));
            it("appears as a registered route", () => {
                expect(blockMatch, `Route id ${id} not found in routeRegistry`).not
                    .toBeNull();
            });
            it(`declares href ${href}`, () => {
                expect(blockMatch?.[0]).toMatch(new RegExp(`href:\\s*"${href.replace(/[/:]/g, "\\$&")}"`));
            });
...
describe("Phase 8 — every admin tab page wraps in PageRouteGate", () => {
    for (const { pagePath } of ADMIN_ROUTE_IDS) {
        ...
        it(`${pagePath} imports PageRouteGate from the canonical path`, () => {
            const body = readWeb(pagePath);
            expect(body).toMatch(/from\s+["'].*components\/navigation\/PageRouteGate["']/);
        });
    }
});
// ---------------------------------------------------------------------------
...
describe("Phase 8 — admin routes map to the ADMIN pillar", () => {
    ...
    for (const { id } of ADMIN_ROUTE_IDS) {
        it(`pillarRegistry maps "${id}" → ADMIN`, () => {
            const re = new RegExp(`\\["${id.replace(/\./g, "\\.")}",\\s*"ADMIN"\\]`);
...
describe("Phase 8 — admin shell tab structure", () => {
    ...
    for (const segment of EXPECTED_TAB_SEGMENTS) {
        it(`declares the "${segment}" tab segment`, () => {
            expect(layout).toMatch(new RegExp(`segment:\\s*"${segment.replace(/-/g, "\\-")}"`));
        });
    }
    ...
    it("renders per-tab links with data-testid org-admin-tab-<id>", () => {
        expect(layout).toMatch(/data-testid=\{?`org-admin-tab-\$\{tab\.id\}`\}?/);
    });
});
// ---------------------------------------------------------------------------
...
describe("Phase 8 — admin shell deep-links to canonical Phase 4A pages (feature-area honesty)", () => {
    const governanceTab = readWeb("app/(app)/organizations/[id]/admin/governance/page.tsx");
    const auditTab = readWeb("app/(app)/organizations/[id]/admin/audit/page.tsx");
    const trustTab = readWeb("app/(app)/organizations/[id]/admin/trust/page.tsx");
    const securityTab = readWeb("app/(app)/organizations/[id]/admin/security/page.tsx");
    const retentionTab = readWeb("app/(app)/organizations/[id]/admin/retention/page.tsx");
