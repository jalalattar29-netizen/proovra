...
const MIGRATION =
  "services/api/prisma/migrations/20261008000000_phase_o_workflow_join_table_final_repair/migration.sql";
...
describe("Phase O — workflow join-table final repair: contract", () => {
  ...
  it("every ADD COLUMN uses IF NOT EXISTS", () => {
    const bare = stripSqlComments(read(MIGRATION)).match(
...
describe("Phase O — workflow join-table final repair: 5-finding coverage", () => {
  ...
  it("adds id UUID (nullable, with DEFAULT gen_random_uuid)", () => {
    expect(src).toMatch(
      /ALTER\s+TABLE\s+IF\s+EXISTS\s+"evidence_workflow_instance_evidence"[\s\S]*?ADD\s+COLUMN\s+IF\s+NOT\s+EXISTS\s+"id"\s+UUID/i,
    );
    expect(src).toMatch(
      /ALTER\s+COLUMN\s+"id"\s+SET\s+DEFAULT\s+gen_random_uuid\(\)/i,
    );
  });
  ...
  it("adds workflow_instance_id UUID and backfills from workflowInstanceId", () => {
    ...
    expect(src).toMatch(
      /SET\s+"workflow_instance_id"\s*=\s*"workflowInstanceId"/,
    );
  });
  ...
  it("adds created_at TIMESTAMPTZ(6) and backfills from createdAt", () => {
    expect(src).toMatch(
      /ADD\s+COLUMN\s+IF\s+NOT\s+EXISTS\s+"created_at"\s+TIMESTAMPTZ\(6\)/,
    );
  ...
  it("backfills id with gen_random_uuid() only where id is NULL", () => {
    expect(src).toMatch(
      /SET\s+"id"\s*=\s*gen_random_uuid\(\)[\s\S]*?WHERE\s+"id"\s+IS\s+NULL/i,
    );
  });
});
...
describe("Phase O — workflow join-table final repair: backfill safety", () => {
  ...
  it("every UPDATE on the join table is inside an EXECUTE inside a DO block", () => {
    ...
    const executeUpdates = (
      stripped.match(
        /EXECUTE\s+\$upd\$[\s\S]*?UPDATE\s+"evidence_workflow_instance_evidence"[\s\S]*?\$upd\$/gi,
      ) ?? []
    ).length;
  ...
  it("every camelCase→snake_case backfill DO block checks BOTH source and target columns exist", () => {
    ...
    for (const [src_col, dst_col] of [
      ...
      const re = new RegExp(
        `column_name\\s*=\\s*'${src_col}'[\\s\\S]*?column_name\\s*=\\s*'${dst_col}'`,
        "i",
      );
...
describe("Phase O — workflow join-table final repair: index safety", () => {
  ...
  it("indexes reference only columns this migration ADDs", () => {
    ...
    for (const m of stripped.matchAll(
      /CREATE\s+INDEX\s+IF\s+NOT\s+EXISTS\s+"[\w_]+"\s+ON\s+"[\w_]+"\s+\(([^)]+)\)/gi,
    )) {
      const cols = m[1]
        .split(",")
      ...
      for (const c of cols) {
        expect.soft(
          addedCols.has(c),
          `Index references column "${c}" not added by this migration`,
        ).toBe(true);
      }
    }
  });
...
describe("Phase O — workflow join-table final repair: passes the migration safety gate", () => {
  it("audit findings on this migration have zero CRITICAL", async () => {
    const audit = (await import(
      REPO_ROOT + "services/api/scripts/full-migration-audit.mjs"
    )) as {
