...
const BILLING_GUARDS_PATH = resolve(API_ROOT, "src/services/collaboration-team/billing-guards.ts");
const ROUTES_PATH = resolve(API_ROOT, "src/routes/collaboration-teams.routes.ts");
const SHARED_INDEX_PATH = resolve(REPO_ROOT, "packages/shared/src/index.ts");
const SHARED_CODES_PATH = resolve(REPO_ROOT, "packages/shared/src/collaboration-team-billing-codes.ts");
// ---------------------------------------------------------------------------
// Helper exports
// ---------------------------------------------------------------------------
describe("Phase 10 — collaboration-team billing-guards exports", () => {
    ...
    it("exports assertCanCreateCollaborationTeam", () => {
        expect(src).toMatch(/export\s+async\s+function\s+assertCanCreateCollaborationTeam\b/);
    });
    it("exports assertCollaborationTeamMemberLimit", () => {
        expect(src).toMatch(/export\s+async\s+function\s+assertCollaborationTeamMemberLimit\b/);
    });
    it("exports assertCanInviteCollaborationTeamMember", () => {
        expect(src).toMatch(/export\s+async\s+function\s+assertCanInviteCollaborationTeamMember\b/);
    });
    ...
    it("exports assertSubscriptionActiveOrGraceAllowed", () => {
        expect(src).toMatch(/export\s+async\s+function\s+assertSubscriptionActiveOrGraceAllowed\b/);
    });
...
describe("Phase 10 — shared error-code vocabulary is canonical + re-exported", () => {
    ...
    it("the six Phase 10 codes are present in the canonical list", () => {
        ...
        for (const code of expected) {
            expect(codesSrc, `missing code ${code}`).toMatch(new RegExp(`["']${code}["']`));
        }
    });
});
// ---------------------------------------------------------------------------
...
describe("Phase 10 — /v1/collaboration-teams handlers call the canonical guards", () => {
    ...
    it("POST /v1/collaboration-teams calls assertCanCreateCollaborationTeam", () => {
        ...
        const postCreateMatch = routes.match(/app\.post(?:<[^>]*>)?\(\s*["']\/v1\/collaboration-teams["']\s*,([\s\S]*?)\n\s*\)\s*;/);
        expect(postCreateMatch, "expected to locate the POST /v1/collaboration-teams handler block").not.toBeNull();
        const createBlock = postCreateMatch ? postCreateMatch[1] : "";
        expect(createBlock, "expected assertCanCreateCollaborationTeam call in POST /v1/collaboration-teams handler").toMatch(/assertCanCreateCollaborationTeam\s*\(/);
    });
    it("POST /v1/collaboration-teams/:teamId/members calls assertCollaborationTeamMemberLimit", () => {
        const membersSection = routes.split('"/v1/collaboration-teams/:teamId/members"')[1] ?? "";
        expect(membersSection, "expected assertCollaborationTeamMemberLimit in add-member handler").toMatch(/assertCollaborationTeamMemberLimit\s*\(/);
    });
    it("POST /v1/collaboration-teams/:teamId/invites/sms calls assertCanInviteCollaborationTeamMember with 'SMS'", () => {
        const smsSection = routes.split('"/v1/collaboration-teams/:teamId/invites/sms"')[1] ?? "";
        // Bounded: the handler MUST invoke the invite gate AND pass the
        // literal "SMS" channel argument.
        expect(smsSection, "expected assertCanInviteCollaborationTeamMember(... 'SMS' ...) in SMS-invite handler").toMatch(/assertCanInviteCollaborationTeamMember\s*\([\s\S]{0,200}["']SMS["']/);
    });
    it("POST /v1/collaboration-team-invites/:token/accept calls assertCollaborationTeamMemberLimit", () => {
        const acceptSection = routes.split('"/v1/collaboration-team-invites/:token/accept"')[1] ?? "";
        expect(acceptSection, "expected assertCollaborationTeamMemberLimit in invite-accept handler").toMatch(/assertCollaborationTeamMemberLimit\s*\(/);
    });
});
