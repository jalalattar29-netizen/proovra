...
const SIGNAL_CATALOG_SRC = readSource("../../../packages/shared-runtime/src/media-intelligence/signal-catalog.ts");
const INDEXER_SRC = readSource("../../../packages/shared-runtime/src/media-intelligence/ocr-transcript-indexer.service.ts");
const INDEXED_SIGNALS_SQL = readSource("../sql/drift-patches/2026-05-20-ocr-transcript-indexed-signals.sql");
// =============================================================================
...
describe("Phase 31.20 — OCR_INDEXED / TRANSCRIPT_INDEXED signal types", () => {
    ...
    it("SIGNAL_METADATA has entries for the new types", () => {
        ...
        expect(SIGNAL_CATALOG_SRC).toMatch(/OCR_INDEXED:\s*\{[^}]*displayLabel:\s*"OCR text indexed for search"/);
        expect(SIGNAL_CATALOG_SRC).toMatch(/TRANSCRIPT_INDEXED:\s*\{[^}]*displayLabel:\s*"Transcript indexed for search"/);
    });
    it("safeSummary library produces bounded wording for the new types", () => {
        ...
        expect(SIGNAL_CATALOG_SRC).toMatch(/case "OCR_INDEXED":[\s\S]*?OCR content indexed for search \(\$\{input\.chunkCount\}/);
        expect(SIGNAL_CATALOG_SRC).toMatch(/case "TRANSCRIPT_INDEXED":[\s\S]*?Transcript content indexed for search \(\$\{input\.segmentCount\}/);
    });
    it("SQL CHECK constraint patch lists both new signal types", () => {
        ...
        expect(INDEXED_SIGNALS_SQL).toMatch(/ALTER TABLE "media_intelligence_signals"[\s\S]*?DROP CONSTRAINT IF EXISTS "media_intelligence_signals_signal_type_bounded"/);
        expect(INDEXED_SIGNALS_SQL).toMatch(/ALTER TABLE "media_intelligence_signals"[\s\S]*?ADD CONSTRAINT "media_intelligence_signals_signal_type_bounded"/);
...
describe("Phase 31.20 — OCR / transcript indexer producer service", () => {
    it("filters to non-redacted, non-BLOCKED rows", () => {
        ...
        // OCR query filter
        expect(code).toMatch(/FROM "evidence_ocr_text"[\s\S]*?"redacted" = false[\s\S]*?"visibility_scope" <> 'BLOCKED'/);
        // Transcript query filter
        expect(code).toMatch(/FROM "evidence_transcript_segments"[\s\S]*?"redacted" = false[\s\S]*?"visibility_scope" <> 'BLOCKED'/);
    });
    ...
    it("idempotent upsert: ON CONFLICT keyed on (evidence_id, material_id, signal_type)", () => {
        expect(INDEXER_SRC).toMatch(/ON CONFLICT \(\s*"evidence_id",\s*COALESCE\("material_id", '00000000-0000-0000-0000-000000000000'::uuid\),\s*"signal_type"\s*\) DO UPDATE/);
    });
    ...
    it("bounded batch size — Math.max(1, Math.min(input.evidenceBatchSize ?? 500, 5000))", () => {
        expect(INDEXER_SRC).toMatch(/Math\.max\(\s*1,\s*Math\.min\(input\.evidenceBatchSize \?\? 500, 5000\),?\s*\)/);
    });
    it("uses material_id = NULL for whole-evidence signals", () => {
        ...
        expect(INDEXER_SRC).toMatch(/VALUES \(\$1, \$2, NULL, \$3, \$4, \$5, \$6, NULL\)/);
    });
...
// =============================================================================
const FFMPEG_PRODUCERS_SRC = readSource("../../worker/src/ffmpeg-derived-assets.ts");
describe("Phase 31.20 — ffmpeg-derived asset producers", () => {
    ...
    it("producers reject wrong-MIME inputs with UNSUPPORTED (anti-leak)", () => {
        ...
        expect(code).toMatch(/produceVideoFrame[\s\S]*?startsWith\("video\/"\)[\s\S]*?status: "unsupported"/);
        expect(code).toMatch(/produceLowResProxy[\s\S]*?startsWith\("video\/"\)[\s\S]*?status: "unsupported"/);
    });
    it("subprocess is bounded by FFMPEG_SUBPROCESS_TIMEOUT_MS", () => {
        expect(FFMPEG_PRODUCERS_SRC).toMatch(/const FFMPEG_SUBPROCESS_TIMEOUT_MS = \d+_000/);
        expect(FFMPEG_PRODUCERS_SRC).toMatch(/child\.kill\("SIGKILL"\)/);
    });
    it("output size is bounded per asset kind", () => {
        expect(FFMPEG_PRODUCERS_SRC).toMatch(/video_frame:\s*\d+\s*\*\s*1024\s*\*\s*1024/);
        expect(FFMPEG_PRODUCERS_SRC).toMatch(/audio_waveform:\s*\d+\s*\*\s*1024,?/);
        expect(FFMPEG_PRODUCERS_SRC).toMatch(/low_res_proxy:\s*\d+\s*\*\s*1024\s*\*\s*1024/);
    ...
    it("hash of derived bytes is computed (separate SHA-256 — never the source hash)", () => {
        const code = stripComments(FFMPEG_PRODUCERS_SRC);
        expect(code).toMatch(/const derivedSha256 = createHash\("sha256"\)\.update\(outBytes\)\.digest\("hex"\)/);
    });
    ...
    it("audio_waveform uses bounded showwavespic filter (no audio fingerprint)", () => {
        expect(FFMPEG_PRODUCERS_SRC).toMatch(/showwavespic=s=600x80/);
    });
});
// =============================================================================
// PART 4 — derived-assets processor dispatch
// =============================================================================
const DERIVED_ASSETS_PROCESSOR_SRC = readSource("../../worker/src/derived-assets.processor.ts");
describe("Phase 31.20 — derived-assets processor dispatch", () => {
    ...
    it("compact_review_preview stays UNSUPPORTED (reserved)", () => {
        expect(DERIVED_ASSETS_PROCESSOR_SRC).toMatch(/assetKind === "compact_review_preview"[\s\S]*?kind_reserved_for_future_phase/);
    });
    it("ffmpeg engine version is distinct from sharp engine version (provenance)", () => {
        expect(DERIVED_ASSETS_PROCESSOR_SRC).toMatch(/FFMPEG_ENGINE_VERSION = "ffmpeg-v0-phase31-20"/);
        // The persistence call accepts an optional engineVersion override.
        expect(DERIVED_ASSETS_PROCESSOR_SRC).toMatch(/engineVersion:\s*FFMPEG_ENGINE_VERSION/);
    });
    it("source bytes are pulled from getObjectRange with per-kind budget", () => {
        expect(DERIVED_ASSETS_PROCESSOR_SRC).toMatch(/SOURCE_READ_BUDGET\[assetKind\]/);
    });
...
// =============================================================================
const SUBSYSTEM_PROCESSORS_SRC = readSource("../../worker/src/subsystem-queue-processors.ts");
describe("Phase 31.20 — graph-reconcile invokes OCR/transcript indexer", () => {
    it("imports and conditionally invokes the indexer service via @proovra/shared-runtime", () => {
        ...
        expect(SUBSYSTEM_PROCESSORS_SRC).toMatch(/import\(\s*"@proovra\/shared-runtime\/media-intelligence"/);
        expect(SUBSYSTEM_PROCESSORS_SRC).toMatch(/const indexerResult = await indexExistingOcrAndTranscript\(\s*\{\s*teamId: job\.data\.teamId/);
    });
    it("gates on producer mode (skip when both modes are NOT_CONFIGURED)", () => {
        expect(SUBSYSTEM_PROCESSORS_SRC).toMatch(/modes\.ocr !== "NOT_CONFIGURED" \|\| modes\.transcript !== "NOT_CONFIGURED"/);
        expect(SUBSYSTEM_PROCESSORS_SRC).toMatch(/graph_reconcile\.ocr_transcript_indexer_skipped_not_configured/);
    });
    it("indexer failure is non-fatal — never throws to BullMQ", () => {
        // The indexer call is wrapped in try { … } catch (err) { logger.warn(...) }
        expect(SUBSYSTEM_PROCESSORS_SRC).toMatch(/graph_reconcile\.ocr_transcript_indexer_failed_non_fatal/);
    });
});
// =============================================================================
...
describe("Phase 31.20 — final 3 isolated queues", () => {
    it("queue.ts declares graph-domain-sync / graph-timeline-sync / graph-search-projection", () => {
        ...
        expect(QUEUE_SRC).toMatch(/graphTimelineSyncQueueName = "graph-timeline-sync"/);
        expect(QUEUE_SRC).toMatch(/graphSearchProjectionQueueName = "graph-search-projection"/);
    });
    it("each new queue has a deterministic idempotent job-id builder", () => {
        expect(QUEUE_SRC).toMatch(/function buildGraphDomainSyncJobId\(\s*teamId: string,\s*domain: string \| null \| undefined,?\s*\): string/);
        expect(QUEUE_SRC).toMatch(/function buildGraphTimelineSyncJobId\(teamId: string\): string \{\s*return `graph-timeline-sync-\$\{teamId\}`/);
        expect(QUEUE_SRC).toMatch(/function buildGraphSearchProjectionJobId\(teamId: string\): string \{\s*return `graph-search-projection-\$\{teamId\}`/);
    });
    it("each enqueue helper routes through genericIdempotentEnqueue", () => {
        expect(QUEUE_SRC).toMatch(/export async function enqueueGraphDomainSyncJob[\s\S]*?genericIdempotentEnqueue\(/);
        expect(QUEUE_SRC).toMatch(/export async function enqueueGraphTimelineSyncJob[\s\S]*?genericIdempotentEnqueue\(/);
        expect(QUEUE_SRC).toMatch(/export async function enqueueGraphSearchProjectionJob[\s\S]*?genericIdempotentEnqueue\(/);
    });
    it("graph-domain-sync payload allows bounded domain filter (catalog enum only)", () => {
        expect(QUEUE_SRC).toMatch(/domain\?:\s*\|\s*"CASE"\s*\|\s*"REPORT"\s*\|\s*"VERIFICATION_PACKAGE"\s*\|\s*"EXPORT"\s*\|\s*"REVIEW_TASK"\s*\|\s*"ESCALATION"\s*\|\s*"INCIDENT"\s*\|\s*"EXTERNAL_REVIEW"\s*\|\s*null/);
    });
...
describe("Phase 31.20 — final 3 queue processors", () => {
    ...
    it("graph-domain-sync invokes the bounded per-domain stale sweep", () => {
        const idx = SUBSYSTEM_PROCESSORS_SRC.indexOf("export async function processGraphDomainSyncJob");
    ...
    it("graph-timeline-sync invokes the real bounded timeline sync", () => {
        const idx = SUBSYSTEM_PROCESSORS_SRC.indexOf("export async function processGraphTimelineSyncJob");
        expect(idx).toBeGreaterThan(0);
    ...
    it("graph-search-projection invokes the real recent-signal-activity reindex", () => {
        const idx = SUBSYSTEM_PROCESSORS_SRC.indexOf("export async function processGraphSearchProjectionJob");
        expect(idx).toBeGreaterThan(0);
