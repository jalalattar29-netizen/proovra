...
const BADGE_PATH = resolve(WEB_ROOT, "components/billing/PlanLimitBadge.tsx");
const TEAMS_INDEX_PAGE = resolve(WEB_ROOT, "app/(app)/collaboration-teams/page.tsx");
const TEAM_DETAIL_PAGE = resolve(WEB_ROOT, "app/(app)/collaboration-teams/[teamId]/page.tsx");
const COLLAB_HUB_PAGE = resolve(WEB_ROOT, "app/(app)/collaboration-teams/[teamId]/collaboration/page.tsx");
// ---------------------------------------------------------------------------
// Component shape
// ---------------------------------------------------------------------------
describe("Phase 10 — PlanLimitBadge component shape", () => {
    ...
    it("the component declares the canonical Phase 10 prop shape (kind, current, max, planLabel)", () => {
        ...
        // pages import.
        expect(src).toMatch(/export\s+(?:function\s+PlanLimitBadge|const\s+PlanLimitBadge|default\s+function\s+PlanLimitBadge|\{[^}]*PlanLimitBadge[^}]*\})/);
    });
...
describe("Phase 10 — Collaboration Team pages surface PlanLimitBadge", () => {
    it("/collaboration-teams (index page) imports PlanLimitBadge", () => {
        ...
        expect(src).toMatch(/import\s+(?:\{[^}]*PlanLimitBadge[^}]*\}|PlanLimitBadge)\s+from\s+["'][^"']*PlanLimitBadge["']/);
    });
