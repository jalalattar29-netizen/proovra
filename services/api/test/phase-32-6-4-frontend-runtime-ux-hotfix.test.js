...
function assertNoForbiddenVocab(label, source) {
    ...
    for (const word of FORBIDDEN_VOCAB) {
        ...
        if (re.test(lower)) {
            throw new Error(`Phase 32.6.4 vocabulary violation in ${label}: forbidden token "${word}" appears in operator-facing source. Use bounded alternatives: "unavailable" / "temporarily unavailable" / "blocked by governance policy" / "still being generated" / "permission required" / "workspace required" / "retry shortly".`);
        }
    }
}
describe("Phase 32.6.4 — Governance page resilience (Phase 32.8E architecture)", () => {
    ...
    const panelSrc = readWebSource("components/governance-experience/GovernanceControlPlane.tsx");
    const serviceSrc = readFileSync(fileURLToPath(new URL("../src/services/governance/governance-control-plane.service.ts", import.meta.url)), "utf8");
    ...
    it("uses bounded UX wording (no forbidden vocabulary)", () => {
        ...
        assertNoForbiddenVocab("governance-experience/GovernanceControlPlane.tsx", panelSrc);
    });
...
describe("Phase 32.6.4 — Verification-package status-code handling", () => {
    ...
    it("downloadVerificationPackage does NOT silently swallow non-url 2xx", () => {
        ...
        const fallback = src.indexOf("Verification package is temporarily unavailable");
