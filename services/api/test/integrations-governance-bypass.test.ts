...
type PolicyFixture = {
  ...
  defaultAssignmentDueHours: number | null;
  defaultCompletionDueHours: number | null;
  defaultDueSoonHours: number | null;
...
function permissivePolicy(): PolicyFixture {
  return {
    source: "default",
...
const SERVICE_ACCOUNT_ROLE = "ADMIN" as const;
...
describe("governance.legal_hold_placed payload — privacy", () => {
  it("includes id/title/caseId/placedAtUtc but never the reason", () => {
    ...
    expect(Object.keys(safe).sort()).toEqual(
      ["caseId", "evidenceId", "legalHoldId", "placedAtUtc", "title"].sort(),
    );
    expect((safe as Record<string, unknown>).reason).toBeUndefined();
  });
});
...
describe("integration evidence projection — privacy", () => {
  it("does NOT include internalNotes, intakePlanJson, or submittedBy fields", async () => {
    ...
    const src = await readFile(
      fileURLToPath(
        new URL("../src/routes/integrations-api.routes.ts", import.meta.url),
      ),
      "utf8",
    );
