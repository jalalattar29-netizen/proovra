...
const MIGRATION_PATH = "services/api/prisma/migrations/20270809000000_phase_2_1_final_drift_closure/migration.sql";
...
const ALLOWLIST_TEST = readRepo("services/api/test/phase-32-7-2-security-event-mapping-drift.test.ts");
// =============================================================================
// GROUP A — file presence + ID allowlist
// =============================================================================
describe("Phase 2.1 — file + allowlist", () => {
    it("migration file exists at the expected path", () => {
        expect(existsSync(fileURLToPath(new URL(`../../../${MIGRATION_PATH}`, import.meta.url)))).toBe(true);
        expect(MIGRATION.length).toBeGreaterThan(800);
    });
    it("migration ID is appended to PERMITTED_LATER_MIGRATIONS", () => {
        expect(ALLOWLIST_TEST).toMatch(/"20270809000000_phase_2_1_final_drift_closure"/);
    });
...
describe("Phase 2.1 — item 1: redaction_policy_audits plural table", () => {
    ...
    it("safety-net ALTER ADD COLUMN IF NOT EXISTS covers every required column", () => {
        ...
        for (const col of [
            ...
            expect(DDL).toMatch(new RegExp(`ALTER\\s+TABLE\\s+IF\\s+EXISTS\\s+"redaction_policy_audits"[\\s\\S]{0,800}ADD\\s+COLUMN\\s+IF\\s+NOT\\s+EXISTS\\s+"${col}"`, "i"));
        }
    });
    ...
    it("plural-table indexes match the Prisma @@index declarations", () => {
        expect(DDL).toMatch(/CREATE\s+INDEX\s+IF\s+NOT\s+EXISTS\s+"redaction_policy_audits_policy_created_idx"/i);
        expect(DDL).toMatch(/CREATE\s+INDEX\s+IF\s+NOT\s+EXISTS\s+"redaction_policy_audits_policy_version_idx"/i);
        expect(DDL).toMatch(/CREATE\s+INDEX\s+IF\s+NOT\s+EXISTS\s+"redaction_policy_audits_team_idx"/i);
    });
...
describe("Phase 2.1 — items 2-4: 3-column safety net", () => {
    it("entitlements.team_seats — INTEGER NOT NULL DEFAULT 0", () => {
        expect(DDL).toMatch(/ALTER\s+TABLE\s+IF\s+EXISTS\s+"entitlements"[\s\S]{0,200}ADD\s+COLUMN\s+IF\s+NOT\s+EXISTS\s+"team_seats"\s+INTEGER\s+NOT\s+NULL\s+DEFAULT\s+0/i);
        // Prisma model still declares the field with @default(0).
        expect(SCHEMA).toMatch(/teamSeats\s+Int\s+@default\(0\)\s+@map\("team_seats"\)/);
    });
    it("verification_packages.package_type — VARCHAR(64) NULL", () => {
        expect(DDL).toMatch(/ALTER\s+TABLE\s+IF\s+EXISTS\s+"verification_packages"[\s\S]{0,200}ADD\s+COLUMN\s+IF\s+NOT\s+EXISTS\s+"package_type"\s+VARCHAR\(64\)/i);
        // Prisma model still declares the field as optional VarChar(64).
        expect(SCHEMA).toMatch(/packageType\s+String\?\s+@map\("package_type"\)\s+@db\.VarChar\(64\)/);
    });
    it("verification_packages.trust_decision_snapshot — JSONB NULL", () => {
        expect(DDL).toMatch(/ALTER\s+TABLE\s+IF\s+EXISTS\s+"verification_packages"[\s\S]{0,200}ADD\s+COLUMN\s+IF\s+NOT\s+EXISTS\s+"trust_decision_snapshot"\s+JSONB/i);
        // Prisma model still declares the field as optional Json.
        expect(SCHEMA).toMatch(/trustDecisionSnapshot\s+Json\?\s+@map\("trust_decision_snapshot"\)/);
    });
});
// =============================================================================
...
describe("Phase 2.1 — destructive-SQL guards", () => {
    ...
    it("the only CREATE TABLE is wrapped in the Phase O-Final pg_tables guard", () => {
        ...
        expect(DDL).toMatch(/IF\s+NOT\s+EXISTS\s*\(\s*SELECT\s+1\s+FROM\s+pg_tables[\s\S]{0,400}redaction_policy_audits[\s\S]{0,400}CREATE\s+TABLE\s+"redaction_policy_audits"/i);
    });
});
