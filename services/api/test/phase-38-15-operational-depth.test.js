...
const SUGGESTIONS_PANEL = readWeb("app/(app)/capture/_lib/CaptureSuggestionsPanel.tsx");
...
describe("Phase 38.15 — capture suggestions layer", () => {
    ...
    it("each suggestion is keyed on a readiness criterion id", () => {
        ...
        for (const criterionId of [
            ...
            expect(SUGGESTIONS).toMatch(new RegExp(`${criterionId}:\\s*\\{[\\s\\S]{0,60}id:\\s*"${criterionId}"`));
        }
    });
    it("suggestions catalog is bounded — tone limited to info|warning", () => {
        expect(SUGGESTIONS).toMatch(/tone:\s*"info"\s*\|\s*"warning"/);
    });
    ...
    it("operational-tone catalog — no legal/forensic overclaim labels", () => {
        ...
        for (const pattern of FORBIDDEN) {
            expect(SUGGESTIONS, `suggestions data must not match ${pattern}`).not.toMatch(pattern);
            expect(SUGGESTIONS_PANEL, `suggestions panel must not match ${pattern}`).not.toMatch(pattern);
        }
    });
});
describe("Phase 38.15 — CaptureSuggestionsPanel contract", () => {
    ...
    it("each suggestion action button is labelled with the suggestion title", () => {
        expect(SUGGESTIONS_PANEL).toMatch(/aria-label=\{`\$\{s\.action\.label\}\s*—\s*\$\{s\.title\}`\}/);
    });
    ...
    it("explicitly documents itself as informational + non-blocking", () => {
        expect(SUGGESTIONS_PANEL).toMatch(/Suggestions are informational|never block finalization|never blocks finalization/i);
    });
    it("hides when there are zero suggestions", () => {
        expect(SUGGESTIONS_PANEL).toMatch(/if\s*\(suggestions\.length\s*===\s*0\)\s*return\s+null/);
    });
    ...
    it("capture page does NOT use the suggestions signal to block finalization", () => {
        expect(CAPTURE).not.toMatch(/if\s*\(\s*suggestions[\s\S]{0,80}return\s+null/);
        expect(CAPTURE).not.toMatch(/if\s*\(\s*suggestions\.length\s*>\s*0\s*\)\s*\{?\s*(disable|return null|throw)/);
    });
});
// =============================================================================
