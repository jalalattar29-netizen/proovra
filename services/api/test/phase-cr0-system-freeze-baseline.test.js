...
describe("Phase CR0 — every (app) page wraps in <PageRouteGate> OR is documented exempt", () => {
    ...
    it("every page.tsx under app/(app)/ either has PageRouteGate or is on the CR0 exemption list", () => {
        ...
        const exemptSuffixes = new Set(DOCUMENTED_EXEMPTIONS.map((e) => e.page.replace(/\\/g, "/")));
        const offenders = [];
        for (const file of all) {
            const normalized = file.replace(/\\/g, "/");
            const rel = normalized.replace(/^.*\/app\/\(app\)\//, "");
            if (exemptSuffixes.has(rel))
                continue;
        ...
        expect(offenders, `Pages must either use <PageRouteGate> or be added to the CR0 DOCUMENTED_EXEMPTIONS list with a justification + revisit phase. Offenders:\n${offenders.join("\n")}`).toEqual([]);
    });
...
describe("Phase CR0 — sidebar root groups bounded to canonical vocabulary", () => {
    ...
    it("sidebar source + canonical group definitions only emit titles from the bounded vocabulary", () => {
        ...
        const titleMatches = Array.from(combined.matchAll(/title:\s*"([^"]+)"/g)).map((m) => m[1]);
        for (const t of titleMatches) {
            expect(ALLOWED_ROOT_GROUP_TITLES.includes(t), `Sidebar root group "${t}" is not in the bounded vocabulary. Add it to ALLOWED_ROOT_GROUP_TITLES intentionally (CR6) or remove.`).toBe(true);
        }
    });
        ...
        for (const rel of PRIMARY_SHELL_FILES) {
            ...
            lines.forEach((line, idx) => {
                if (/["']Unknown["']/.test(line)) {
                    if (/CR0-allowed:/i.test(line))
                        return;
                    offenders.push(idx + 1);
                }
            });
            expect(offenders, `${rel} contains user-facing "Unknown" at lines ${offenders.join(", ")} without a CR0-allowed comment.`).toEqual([]);
        }
    });
});
// =============================================================================
...
describe("Phase CR0 — server.ts route registration inventory is frozen (CR1 will purge)", () => {
    ...
    it("server.ts mounts the acknowledged-debt route handles and no new seed/orphan routes appear", () => {
        ...
        for (const pattern of FORBIDDEN_NEW_SEED_PATTERNS) {
            expect(server, `New seed/debug route registration matching ${pattern} is forbidden under CR0. Use a guarded test endpoint instead, or wait for CR1.`).not.toMatch(pattern);
        }
    });
    it("the recovery baseline document exists", () => {
        ...
        const docPath = fileURLToPath(new URL("../../../docs/recovery/CR0_SYSTEM_FREEZE_BASELINE.md", import.meta.url));
        expect(readFileSync(docPath, "utf8").length, `${docPath} must exist and be non-empty.`).toBeGreaterThan(500);
    });
});
// =============================================================================
...
describe("Phase CR0 — existing Phase 38 architectural locks still hold", () => {
    ...
    it("the workflow safety statement remains available as the canonical reassurance", () => {
        ...
        expect(notice).toMatch(/WORKFLOW_SAFETY_STATEMENT\s*=\s*["']Workflow profiles personalize layout, defaults, and recommendations\. They do not change permissions or remove tools\.["']/);
    });
});
