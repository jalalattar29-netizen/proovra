...
const EVIDENCE_SAVED_VIEWS = readSource("../src/routes/evidence.saved-views.routes.ts");
...
const CASE_DETAIL_PAGE = readSource("../../../apps/web/app/(app)/cases/[id]/page.tsx");
...
describe("Phase G4.6 — Personal-mode evidence resolves deterministically (G4.1)", () => {
    it("read-side compatibility helper is shipped and exported", () => {
        expect(TENANCY_RESOLVER).toContain("export async function resolveEvidenceTenancyForRead");
    });
    ...
    it("the helper is READ-ONLY — never mutates evidence", () => {
        ...
        const startIdx = TENANCY_RESOLVER.indexOf("export async function resolveEvidenceTenancyForRead");
        expect(startIdx).toBeGreaterThan(0);
        const endIdx = TENANCY_RESOLVER.indexOf("export async function checkEvidenceTenancyInvariant");
...
describe("Phase G4.6 — Classic matter view retired safely (G4.2)", () => {
    it("classic URL is now a permanent redirect in next.config.js to the canonical Matter Workspace", () => {
        ...
        expect(NEXT_CONFIG).toMatch(/source:\s*["']\/cases\/:id\/classic["'][\s\S]{0,200}destination:\s*["']\/cases\/:id["']/);
    });
    ...
    it("MatterWorkspace's empty-state copy no longer references the classic view", () => {
        ...
        expect(MATTER_WORKSPACE).not.toMatch(/Use the classic view to link evidence/);
        expect(MATTER_WORKSPACE).not.toMatch(/Assign[^"']*from the classic view/);
    });
});
// ---------------------------------------------------------------------------
...
describe("Phase G4.6 — evidence.routes.ts refactor preserves contracts", () => {
    it("saved-views routes file exists and exports the registration helper", () => {
        expect(EVIDENCE_SAVED_VIEWS).toContain("export async function evidenceSavedViewsRoutes");
    });
    it("all five saved-views URLs land in the extracted module", () => {
        expect(EVIDENCE_SAVED_VIEWS).toContain('"/v1/evidence/saved-views"');
        expect(EVIDENCE_SAVED_VIEWS).toContain('"/v1/evidence/saved-views/:id"');
        expect(EVIDENCE_SAVED_VIEWS).toContain('"/v1/evidence/saved-views/:id/default"');
    ...
    it("parent module registers the extracted routes exactly once", () => {
        const calls = (EVIDENCE_ROUTES.match(/evidenceSavedViewsRoutes\(app\)/g) ?? []).length;
        expect(calls).toBe(1);
    });
    it("the parent module no longer carries duplicate saved-view route handlers", () => {
        ...
        // handler text moved.
        expect(EVIDENCE_ROUTES).not.toContain('app.get("/v1/evidence/saved-views"');
        expect(EVIDENCE_ROUTES).not.toContain('app.post("/v1/evidence/saved-views"');
        expect(EVIDENCE_ROUTES).not.toContain('app.patch("/v1/evidence/saved-views/:id"');
        expect(EVIDENCE_ROUTES).not.toContain('app.delete("/v1/evidence/saved-views/:id"');
    });
    it("auth preHandler preserved on every extracted route", () => {
        const occurrences = (EVIDENCE_SAVED_VIEWS.match(/preHandler: requireAuth/g) ?? []).length;
