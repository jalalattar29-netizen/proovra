...
const REVIEW_DETAIL = readWeb("app/(app)/reviewer-ops/[reviewId]/page.tsx");
// =============================================================================
...
describe("Phase 38.12 — CommandCenter section registry supports grid grouping", () => {
    ...
    it("declares the bounded gridGroup vocabulary in source", () => {
        ...
        for (const g of GROUPS) {
            expect(COMMAND_CENTER, `gridGroup "${g}" missing`).toMatch(new RegExp(`"${g}"`));
        }
    });
    it("annotates representative section pairs with the same gridGroup", () => {
        // caseOperations + reviewerOrchestration share "case-ops"
        expect(COMMAND_CENTER).toMatch(/caseOperations:\s*\{[\s\S]{0,300}gridGroup:\s*"case-ops"/);
        expect(COMMAND_CENTER).toMatch(/reviewerOrchestration:\s*\{[\s\S]{0,300}gridGroup:\s*"case-ops"/);
        // pipelineDetail + governancePosture share "pipeline"
        expect(COMMAND_CENTER).toMatch(/pipelineDetail:\s*\{[\s\S]{0,300}gridGroup:\s*"pipeline"/);
        expect(COMMAND_CENTER).toMatch(/governancePosture:\s*\{[\s\S]{0,300}gridGroup:\s*"pipeline"/);
        // auditReadiness + organizationalIntelligence share "audit"
        expect(COMMAND_CENTER).toMatch(/auditReadiness:\s*\{[\s\S]{0,300}gridGroup:\s*"audit"/);
        expect(COMMAND_CENTER).toMatch(/organizationalIntelligence:\s*\{[\s\S]{0,300}gridGroup:\s*"audit"/);
    });
    ...
    it("band length is capped at 3 — keeps grids visually safe", () => {
        ...
        // cap is present.
        expect(COMMAND_CENTER).toMatch(/(last|band)\.entries\.length\s*<\s*3/);
    });
});
// =============================================================================
...
describe("Phase 38.12 — route registry expansion (5 new routes)", () => {
    ...
    it("review.queue_detail is org-only + reached from the queue (not a top-level discovery surface)", () => {
        expect(REGISTRY).toMatch(/id:\s*"review\.queue_detail"[\s\S]*?requiredActiveSpace:\s*"ORGANIZATION_ONLY"[\s\S]*?commandPaletteVisible:\s*false[\s\S]*?allToolsVisible:\s*false[\s\S]*?sidebarEligible:\s*false/);
    });
});
// =============================================================================
