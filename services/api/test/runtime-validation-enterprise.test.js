...
describe("Runtime readiness aggregator [structure]", () => {
    ...
    it("rollUpStatus precedence: CRITICAL > DEGRADED > HEALTHY > UNKNOWN", () => {
        expect(src).toMatch(/if \(subsystems\.some\(\(s\) => s\.status === "CRITICAL"\)\) return "CRITICAL"/);
        expect(src).toMatch(/if \(subsystems\.some\(\(s\) => s\.status === "DEGRADED"\)\) return "DEGRADED"/);
        expect(src).toMatch(/if \(subsystems\.every\(\(s\) => s\.status === "HEALTHY"\)\) return "HEALTHY"/);
    });
...
describe("Enterprise empty-state components", () => {
    ...
    it("fail-closed variants use appropriate severity tones", () => {
        ...
        expect(src).toMatch(/GovernanceSnapshotUnavailableNotice[\s\S]+?variant="unknown"/);
    });
