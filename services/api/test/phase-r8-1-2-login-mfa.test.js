...
import { signMfaPendingToken, verifyMfaPendingTokenSignature, MFA_PENDING_TTL_SECONDS, signJwt, } from "../src/services/jwt.js";
...
const CMD_CENTER_PATH = webPath("components/command-center/CommandCenter.tsx");
// =============================================================================
...
describe("R8.1.2 Part 1 — mfa-pending JWT primitives", () => {
    ...
    it("test 1: signMfaPendingToken returns a token with the `mfa: pending` discriminator", () => {
        const token = signMfaPendingToken({ sub: "user_abc", provider: "EMAIL", email: "u@example.com" }, SECRET);
        ...
        expect(payload.exp - now).toBeGreaterThanOrEqual(MFA_PENDING_TTL_SECONDS - 2);
        expect(payload.exp - now).toBeLessThanOrEqual(MFA_PENDING_TTL_SECONDS + 2);
    });
    ...
    it("test 3: verifyMfaPendingTokenSignature returns the same payload across calls (replay protection delegated to durable store)", () => {
        ...
        const token = signMfaPendingToken({ sub: "user_replay", provider: "EMAIL", email: "r@example.com" }, SECRET, "test-jti-replay-1234567890abcdef");
    ...
    it("test 4: verifyMfaPendingTokenSignature refuses a regular (non-pending) session token", () => {
        const normal = signJwt({ sub: "user_full", provider: "EMAIL", email: "f@example.com" }, SECRET, 60);
        expect(() => verifyMfaPendingTokenSignature(normal, SECRET)).toThrowError(/not_a_pending_token/);
    });
});
// =============================================================================
...
describe("R8.1.2 Part 2 — requireAuth rejects pending tokens", () => {
    it("test 5: middleware refuses payload.mfa === 'pending' with a generic 401", () => {
        ...
        const mfaRejectBlock = MIDDLEWARE_SRC.match(/payload\.mfa\s*===\s*["']pending["'][\s\S]{0,400}/);
...
describe("R8.1.2 Part 3 — gateLoginWithMfa wired into every login path", () => {
    ...
    it("test 7: gateLoginWithMfa runs in google + apple + email login paths", () => {
        ...
        expect(endpointBody("/v1/auth/email/login")).toMatch(/gateLoginWithMfa/);
    });
...
describe("R8.1.2 Part 6 — uses only bounded SECURITY_EVENT_TYPES", () => {
    it("test 16: auth.routes.ts emits only `auth_login_failed` from R8 vocabulary (no new event types)", () => {
        ...
        const emits = [
            ...AUTH_SRC.matchAll(/safeEmitSecurityEvent\s*\(\s*\{[\s\S]*?eventType:\s*["']([^"']+)["']/g),
        ].map((m) => m[1]);
...
describe("R8.1A Part 7 — CommandCenter.tsx build hygiene", () => {
    it("test 17: getPersonaSectionOrder is NOT imported directly into CommandCenter.tsx", () => {
        ...
        const importBlock = src.match(/import\s*\{[\s\S]*?\}\s*from\s*["'][^"']*platform-context["']\s*;/);
