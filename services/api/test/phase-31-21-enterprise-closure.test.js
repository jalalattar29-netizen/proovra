...
const LOCAL_CAP_SRC = readSource("../../worker/src/local-ocr-transcript-capability.ts");
const DOMAIN_SYNC_SRC = readSource("../../../packages/shared-runtime/src/graph/domain-sync.service.ts");
...
describe("Phase 31.21 — local OCR / Whisper capability probes are stubs", () => {
    ...
    it("both probes return { ok: false, reason: \"not_enabled\" }", () => {
        const code = stripComments(LOCAL_CAP_SRC);
        expect(code).toMatch(/detectLocalTesseractCapability[\s\S]{0,600}return \{\s*ok: false,\s*reason: "not_enabled",?\s*\}/);
        expect(code).toMatch(/detectLocalWhisperCapability[\s\S]{0,600}return \{\s*ok: false,\s*reason: "not_enabled",?\s*\}/);
    });
});
// =============================================================================
...
describe("Phase 31.21 — domain-sync.service.ts", () => {
    ...
    it("runSearchProjectionSync caps enqueue at 200 per run by default and clamps to 500 max", () => {
        ...
        expect(DOMAIN_SYNC_SRC).toMatch(/Math\.max\(\s*1,\s*Math\.min\(options\.enqueueCap \?\? MAX_REINDEX_ENQUEUES_PER_RUN, 500\),?\s*\)/);
    });
    it("runSearchProjectionSync windowMinutes clamped to 1..1440", () => {
        expect(DOMAIN_SYNC_SRC).toMatch(/Math\.max\(\s*1,\s*Math\.min\(options\.windowMinutes \?\? RECENT_SIGNAL_WINDOW_MINUTES, 1440\),?\s*\)/);
    });
    it("runSearchProjectionSync is team-anchored", () => {
        ...
        const slice = DOMAIN_SYNC_SRC.slice(idx, idx + 3500);
        expect(slice).toMatch(/FROM "media_intelligence_signals"\s*WHERE "team_id" = \$1/);
    });
    it("runSearchProjectionSync never throws — query failure returns { ok: false, reason }", () => {
        const idx = DOMAIN_SYNC_SRC.indexOf("export async function runSearchProjectionSync");
        const slice = DOMAIN_SYNC_SRC.slice(idx, idx + 3500);
        expect(slice).toMatch(/} catch \(err\) \{[\s\S]*?return \{\s*ok: false,[\s\S]*?reason:[\s\S]*?recent_signal_query_failed/);
    });
