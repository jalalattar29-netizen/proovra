...
import { ANALYTICS_WINDOWS, DEDUPE_KEY_MAX_LEN, DEFAULT_NOTIFICATION_CHANNELS, DESTRUCTION_EXECUTION_PHASES, DESTRUCTION_EXECUTION_STATUSES, DESTRUCTION_EXECUTION_STATUS_LABELS, GOVERNANCE_ANALYTICS_METRICS, GOVERNANCE_EXPORT_SNAPSHOT_KINDS, GOVERNANCE_NOTIFICATION_CHANNELS, GOVERNANCE_NOTIFICATION_DELIVERY_STATUSES, GOVERNANCE_NOTIFICATION_KINDS, GOVERNANCE_NOTIFICATION_KIND_LABELS, GOVERNANCE_NOTIFICATION_SEVERITIES, GOVERNANCE_RECONCILIATION_KINDS, GOVERNANCE_RECONCILIATION_STATUSES, GOVERNANCE_RECONCILIATION_TRIGGERS, GovernanceExportSnapshotInputSchema, IMMUTABLE_STORAGE_CHECK_OUTCOMES, IMMUTABLE_STORAGE_DRIFT_OUTCOMES, IMMUTABLE_STORAGE_OUTCOME_LABELS, NOTIFICATION_SUMMARY_MAX_LEN, NOTIFICATION_THROTTLE_SECONDS, NOTIFICATION_TITLE_MAX_LEN, NOTIFICATION_TO_INCIDENT_SEVERITY, SECURITY_EVENT_TYPES, analyticsWindowToMilliseconds, isDriftOutcome, isTerminalDestructionExecutionStatus, isValidDedupeKey, } from "@proovra/shared";
...
describe("Phase 27.5 — Notification catalog", () => {
    it("declares the documented kind set", () => {
        ...
        expect(GOVERNANCE_NOTIFICATION_KINDS).toContain("IMMUTABLE_RECONCILIATION_FAILURE");
    ...
    it("throttle windows compress with rising severity", () => {
        expect(NOTIFICATION_THROTTLE_SECONDS.INFO).toBeGreaterThan(NOTIFICATION_THROTTLE_SECONDS.WARNING);
        expect(NOTIFICATION_THROTTLE_SECONDS.WARNING).toBeGreaterThan(NOTIFICATION_THROTTLE_SECONDS.HIGH);
        expect(NOTIFICATION_THROTTLE_SECONDS.HIGH).toBeGreaterThan(NOTIFICATION_THROTTLE_SECONDS.CRITICAL);
    });
...
describe("Phase 27.5 — Service source wiring", () => {
    it("notification service dedupes by (teamId, kind, dedupeKey) with throttle gating", () => {
        const src = readSource("../src/services/governance-lifecycle/governance-notification.service.ts");
    ...
    it("export-lineage service captures bounded payload + hashes canonical JSON", () => {
        const src = readSource("../src/services/governance-lifecycle/export-lineage.service.ts");
    ...
    it("analytics service is read-only and bounded by the metric catalog", () => {
        const src = readSource("../src/services/governance-lifecycle/governance-analytics.service.ts");
...
describe("Phase 27.5 — Worker wiring", () => {
    const retentionSrc = readSource("../../worker/src/governance/retention-reconciliation.worker.ts");
    const destructionSrc = readSource("../../worker/src/governance/destruction-orchestrator.worker.ts");
    const immutableSrc = readSource("../../worker/src/governance/immutable-storage-reconciliation.worker.ts");
    const runLockSrc = readSource("../../worker/src/governance/reconciliation-run.ts");
...
describe("Phase 27.5 — Route wiring", () => {
    const routesSrc = readSource("../src/routes/governance-operations.routes.ts");
    ...
    it("routes expose analytics + notifications + snapshots + reconciliation", () => {
        ...
        expect(routesSrc).toContain('"/v1/governance/notifications/:id/acknowledge"');
        expect(routesSrc).toContain('"/v1/governance/export-snapshots"');
        expect(routesSrc).toContain('"/v1/governance/export-snapshots/:id"');
        expect(routesSrc).toContain('"/v1/governance/export-snapshots/:id/verify"');
...
describe("Phase 27.5 — Migration safety", () => {
    const migrationSrc = readSource("../prisma/migrations/20260613100000_phase27_5_governance_operationalization/migration.sql");
    ...
    it("enforces (team_id, kind, dedupe_key) unique constraint", () => {
        expect(migrationSrc).toContain("governance_notifications_team_kind_dedupe_unique");
    });
});
// -----------------------------------------------------------------------------
// UI wording sweep
...
describe("Phase 27.5 — UI wording sweep", () => {
    ...
    function assertCleanOf(rel) {
        ...
        for (const p of playful)
            expect(lower).not.toContain(p);
        for (const p of truthClaims)
            expect(lower).not.toContain(p);
    }
    it("analytics page wording is enterprise-grade", () => {
        assertCleanOf("../../../apps/web/app/(app)/governance/analytics/page.tsx");
    });
    it("notifications page wording is enterprise-grade", () => {
        assertCleanOf("../../../apps/web/app/(app)/governance/notifications/page.tsx");
    });
    it("LifecycleIndicators component is enterprise-grade", () => {
        assertCleanOf("../../../apps/web/components/governance/LifecycleIndicators.tsx");
    });
});
