...
describe("Phase 32.6.5 — checkMigrations stale-rolled-back repair", () => {
    ...
    it("selects `started_at` from _prisma_migrations (needed for dedup)", () => {
        expect(fn).toMatch(/SELECT[\s\S]*?migration_name[\s\S]*?finished_at[\s\S]*?rolled_back_at[\s\S]*?started_at[\s\S]*?FROM\s+"_prisma_migrations"/);
    });
    it("dedups by migration_name keeping the most recent row per name", () => {
        // Verify the Map-based dedup exists.
        expect(fn).toMatch(/latestPerMigration\s*=\s*new\s+Map<\s*string\s*,/);
    ...
    it("`failed` counts only currently-rolled-back live rows (NOT historical)", () => {
        expect(fn).toMatch(/failed\s*=\s*liveRows\.filter\(\(r\)\s*=>\s*r\.rolled_back_at\s*!=\s*null\)\.length/);
    });
    it("`pending` excludes rows that are rolled back (a rolled-back row has finished_at NULL too)", () => {
        expect(fn).toMatch(/pending\s*=\s*liveRows\.filter\([\s\S]*?r\.finished_at\s*==\s*null\s*&&\s*r\.rolled_back_at\s*==\s*null[\s\S]*?\)\.length/);
    });
...
describe("Phase 32.6.5 — Sentry filter catches `Stream isn't writeable`", () => {
    ...
    it("API beforeSend filter includes `Stream isn't writeable`", () => {
        expect(API_SENTRY).toMatch(/message\.includes\(\s*"Stream isn't writeable"\s*\)/);
    });
    it("Worker beforeSend filter includes `Stream isn't writeable`", () => {
        expect(WORKER_SENTRY).toMatch(/message\.includes\(\s*"Stream isn't writeable"\s*\)/);
    });
    ...
    it("filter is guarded by an explicit `if (message.includes(...))` (no broad message drop)", () => {
        for (const src of [API_SENTRY, WORKER_SENTRY]) {
            ...
            const returnEventCount = (beforeSendBlock.match(/return\s+event;/g) ?? []).length;
            ...
            const returnNullCount = (beforeSendBlock.match(/return\s+null;/g) ?? []).length;
...
describe("Phase 32.6.5 — checkWorkers queries security_events (the writer's table)", () => {
    ...
    it("queries prisma.securityEvent (NOT prisma.adminAuditLog) for reviewer_reconcile_run", () => {
        ...
        expect(fn).not.toMatch(/prisma\.adminAuditLog\.findFirst\([\s\S]{0,200}reviewer_reconcile_run/);
    });
    it("uses `eventType` field on the SecurityEvent model (not `action`)", () => {
        ...
        expect(fn).toMatch(/where:\s*\{\s*eventType:\s*(heartbeatWireString|"reviewer_reconcile_run")\s*\}/);
    });
...
describe("Phase 32.6.5 — reconcile writer/reader alignment", () => {
    ...
    const WRITER_SRC = readSource("../src/services/reviewer-ops/reviewer-operations-engine.service.ts");
    it("writer emits SecurityEvent with the canonical WORKER_HEARTBEAT event", () => {
        ...
        expect(WRITER_SRC).toMatch(/eventType:\s*canonicalOperationalWireStringFor\([\s\S]{0,100}"WORKER_HEARTBEAT"/);
    });
    it("reader queries the same table (SecurityEvent) using the canonical wire string", () => {
        ...
        expect(READER_SRC).toMatch(/prisma\.securityEvent\.findFirst\(\s*\{\s*where:\s*\{\s*eventType:\s*heartbeatWireString/);
    });
    it("no orphan reader still pointing at adminAuditLog for reviewer_reconcile_run", () => {
        expect(READER_SRC).not.toMatch(/prisma\.adminAuditLog\.findFirst\(\s*\{\s*where:\s*\{\s*action:\s*"reviewer_reconcile_run"/);
    });
});
// =============================================================================
