...
describe("App sidebar — Platform Health group (Phase 32.8B)", () => {
    ...
    const src = readSource("../../../apps/web/components/app-shell-v2/AppSidebarV2.tsx");
...
describe("/ops/runbooks page", () => {
    const src = readSource("../../../apps/web/app/(app)/ops/runbooks/page.tsx");
...
describe("Operational component contrast (Phase 28-I)", () => {
    const TOKENS_SRC = readSource("../../../apps/web/components/operational/tokens.ts");
    ...
    it("no operational component uses rgba(255,255,255,...) text or borders", () => {
        for (const rel of COMPONENT_FILES) {
            ...
            expect(src, `${rel} still uses dark-shell white tokens (contrast bug)`).not.toMatch(/rgba\(\s*255\s*,\s*255\s*,\s*255\s*,/);
        }
    });
    ...
    it("OperationalEmptyState renders Link with underlined readable color (no invisible text)", () => {
        const src = readSource("../../../apps/web/components/operational/OperationalEmptyState.tsx");
...
describe("Empty-state runbook links", () => {
    ...
    it("no operational surface links to the broken /docs/runbooks", () => {
        for (const rel of PRESET_SOURCES) {
            const src = readSource(rel);
            expect(src, `${rel} links to broken /docs/runbooks`).not.toContain("/docs/runbooks");
        }
    });
...
describe("Schema validation — saved_search_views drift coverage", () => {
    ...
    it("registers the saved_search_views table", () => {
        expect(src).toMatch(/kind:\s*"table",\s*name:\s*"saved_search_views"/);
    });
    it("registers saved_search_views.scope as a CRITICAL column under reviewer_ops", () => {
        ...
        // failing at runtime.
        expect(src).toMatch(/table:\s*"saved_search_views",\s*column:\s*"scope",\s*severity:\s*"critical",\s*subsystem:\s*"reviewer_ops"/);
    });
    it("registers the other reviewer-ops saved-view columns the projection depends on", () => {
        for (const col of [
            ...
            expect(src, `column ${col} not registered`).toMatch(new RegExp(`table:\\s*"saved_search_views",\\s*column:\\s*"${col}"`));
        }
    });
});
// =============================================================================
...
describe("Saved-views route — projection contract", () => {
    ...
    it("declares the three /v1/reviewer-ops/saved-views handlers", () => {
        ...
        expect(src).toMatch(/app\.delete\([\s\S]*?"\/v1\/reviewer-ops\/saved-views\/:id"/);
    });
...
describe("Saved-views service — scope filter regression guard", () => {
    const src = readSource("../../../services/api/src/services/reviewer-ops/saved-queue-views.service.ts");
...
describe("SQL drift patch — saved_search_views.scope", () => {
    const src = readSource("../../../services/api/sql/drift-patches/2026-05-19-saved-search-views-scope.sql");
    ...
    it("recreates the supporting indexes with CREATE INDEX IF NOT EXISTS (idempotent)", () => {
        expect(src).toMatch(/CREATE INDEX IF NOT EXISTS\s+saved_search_views_team_scope_idx/i);
        expect(src).toMatch(/CREATE INDEX IF NOT EXISTS\s+saved_search_views_team_scope_visibility_idx/i);
    });
