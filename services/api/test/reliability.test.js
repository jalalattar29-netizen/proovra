...
import { checkUploadSize, getStaleThresholds, getUploadSizeLimits, } from "../src/services/reliability/upload-limits.service.js";
import { listQueuePolicies, OTS_UPGRADE_QUEUE_POLICY, REPORT_QUEUE_POLICY, } from "../src/services/reliability/queue-policy.service.js";
...
describe("upload size limits", () => {
    ...
    it("clamps absurdly small env to floor", () => {
        ...
        const limits = getUploadSizeLimits();
        expect(limits.maxUploadFileSizeBytes).toBeGreaterThanOrEqual(1024 * 1024);
    });
    ...
    it("checkUploadSize recommends multipart above the threshold", () => {
        ...
        if (small.ok)
            expect(small.recommendMultipart).toBe(false);
        if (big.ok)
            expect(big.recommendMultipart).toBe(true);
    });
...
describe("queue policies", () => {
    ...
    it("documented retry counts match the worker source", async () => {
        ...
        const src = await readFile(fileURLToPath(new URL("../../worker/src/queue.ts", import.meta.url)), "utf8");
...
describe("upload session projection — privacy", () => {
    it("does NOT expose multipartUploadId", () => {
        ...
        expect(projected.multipartUploadId).toBeUndefined();
...
describe("anti-enumeration — reliability routes", () => {
    it("uses 404 (not 403) for non-admin members", async () => {
        const { readFile } = await import("node:fs/promises");
        const { fileURLToPath } = await import("node:url");
        const src = await readFile(fileURLToPath(new URL("../src/routes/reliability.routes.ts", import.meta.url)), "utf8");
...
describe("finalize idempotency — source-level guard", () => {
    it("evidence-complete.service.ts gates the SIGNED update by status", async () => {
        const { readFile } = await import("node:fs/promises");
        const { fileURLToPath } = await import("node:url");
        const src = await readFile(fileURLToPath(new URL("../src/services/evidence-complete.service.ts", import.meta.url)), "utf8");
...
describe("capture upload retry — source-level check", () => {
    it("useCaptureSessionOrchestration.ts retries transient PUT failures", async () => {
        const { readFile } = await import("node:fs/promises");
        const { fileURLToPath } = await import("node:url");
        const src = await readFile(fileURLToPath(new URL("../../../apps/web/app/(app)/capture/_hooks/useCaptureSessionOrchestration.ts", import.meta.url)), "utf8");
