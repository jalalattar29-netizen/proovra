...
import { OperationalSeedError, SEED_SCENARIOS, assertSeedingEnabled, assertSeedingSecret, } from "../src/services/ops/operational-seed.service.js";
...
describe("Operational seed [env gates]", () => {
    ...
    it("refuses when OPERATIONAL_SEEDING_ENABLED is unset", () => {
        ...
        try {
            ...
            expect(err.code).toBe("SEEDING_DISABLED");
    ...
    it("refuses the secret check when no secret is configured (fail closed)", () => {
        expect(() => assertSeedingSecret("anything")).toThrow(/SEEDING_SECRET_NOT_CONFIGURED/);
    });
    it("refuses a mismatched secret with 401", () => {
        ...
        try {
            ...
            expect(err.code).toBe("SEEDING_SECRET_INVALID");
...
describe("Operational seed [real-engine wiring]", () => {
    ...
    it("imports ensureReviewWorkflow from the real review-operations service", () => {
        expect(src).toMatch(/import \{ ensureReviewWorkflow \} from ".*review-operations\.service\.js"/);
    });
    it("imports runReconcile from the real reviewer-operations engine", () => {
        expect(src).toMatch(/import \{[\s\S]*?runReconcile,[\s\S]*?\} from ".*reviewer-operations-engine\.service\.js"/);
    });
    it("imports createEscalation / acknowledgeEscalation / resolveEscalation from the real engine", () => {
        expect(src).toMatch(/import \{[\s\S]*?createEscalation,[\s\S]*?acknowledgeEscalation,[\s\S]*?resolveEscalation,[\s\S]*?\} from ".*escalation-engine\.service\.js"/);
    });
    it("imports snapshotWorkspaceWorkload from the real workload service", () => {
        expect(src).toMatch(/import \{ snapshotWorkspaceWorkload \} from ".*workload\.service\.js"/);
    });
    it("imports recordIncident from the canonical incident service", () => {
        expect(src).toMatch(/import \{ recordIncident \} from ".*incident\.service\.js"/);
    });
...
describe("Operational seed [E2E chain — every link is wired]", () => {
    ...
    it("step 6: GOVERNANCE incident raised via recordIncident with seed metadata", () => {
        expect(src).toMatch(/recordIncident\(\s*\{[\s\S]*?category:\s*"GOVERNANCE"[\s\S]*?metadata:\s*\{[\s\S]*?seeded:\s*true[\s\S]*?seedRunId/);
    });
...
describe("Operational seed [cleanup safety]", () => {
    ...
    it("escalation cleanup requires safeSummary to start with the seed prefix (defense in depth)", () => {
        expect(src).toMatch(/reviewEscalation\.deleteMany\([\s\S]*?safeSummary:\s*\{\s*startsWith:\s*seedPrefix\s*\}/);
    });
...
describe("Operational seed [no bypass]", () => {
    ...
    it("never bypasses report-v2 / verify / package builders (no imports)", () => {
        ...
        const importStatements = src
            ...
            .filter((l) => l.trim().startsWith("import"))
            .join("\n");
        expect(importStatements).not.toMatch(/report-v2|verification-package|verify\.routes/);
    });
    it("never touches OTS / TSA / anchor proofs (no imports)", () => {
        const importStatements = src
            ...
            .filter((l) => l.trim().startsWith("import"))
            .join("\n");
        expect(importStatements).not.toMatch(/decideOtsPackageArtifact|deriveAnchorSemantics|opentimestamps|tsaTokenBase64/);
    });
});
