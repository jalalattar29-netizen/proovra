...
const TOPBAR = readSource("../../../apps/web/components/app-shell-v2/AppTopbarV2.tsx");
const TOPBAR_INDICATOR = readSource("../../../apps/web/components/app-shell-v2/InboxIndicator.tsx");
// ===========================================================================
...
describe("Phase C2 — /v1/me/inbox surfaces discussion signals", () => {
    ...
    it("queries DiscussionMention scoped to caller's teamIds + unread (notifiedAtUtc null)", () => {
        expect(ME_INBOX_ROUTES).toMatch(/prisma\.discussionMention\.findMany\(\{[\s\S]*?teamId:\s*\{\s*in:\s*teamIds\s*\}[\s\S]*?mentionedUserId:\s*userId[\s\S]*?notifiedAtUtc:\s*null/);
    });
    it("queries DiscussionThread scoped to caller's teamIds + assignedToUserId + active status", () => {
        expect(ME_INBOX_ROUTES).toMatch(/prisma\.discussionThread\.findMany\(\{[\s\S]*?teamId:\s*\{\s*in:\s*teamIds\s*\}[\s\S]*?assignedToUserId:\s*userId[\s\S]*?status:\s*\{\s*notIn:\s*\["RESOLVED",\s*"CLOSED"\]\s*\}/);
    });
    it("emits inbox items with deep-link href into evidence ?tab=discussion&thread=", () => {
        expect(ME_INBOX_ROUTES).toContain("?tab=discussion&thread=${encodeURIComponent");
...
describe("Phase C2 — /v1/me/inbox/summary topbar counter endpoint", () => {
    ...
    it("returns bounded { unreadMentions, openAssignments } counters", () => {
        expect(ME_INBOX_ROUTES).toMatch(/reply\.code\(200\)\.send\(\{[\s\S]*?unreadMentions[\s\S]*?openAssignments/);
    });
    it("is workspace-scoped — counts narrow to teamIds the caller is a member of", () => {
        ...
        expect(ME_INBOX_ROUTES).toMatch(/discussionMention\.count\(\{[\s\S]*?teamId:\s*\{\s*in:\s*teamIds\s*\}[\s\S]*?mentionedUserId:\s*userId[\s\S]*?notifiedAtUtc:\s*null/);
        expect(ME_INBOX_ROUTES).toMatch(/discussionThread\.count\(\{[\s\S]*?teamId:\s*\{\s*in:\s*teamIds\s*\}[\s\S]*?assignedToUserId:\s*userId/);
    });
    it("short-circuits to zero when caller belongs to no workspace", () => {
        expect(ME_INBOX_ROUTES).toMatch(/if\s*\(teamIds\.length\s*===\s*0\)\s*\{[\s\S]*?unreadMentions:\s*0[\s\S]*?openAssignments:\s*0/);
    });
});
// ===========================================================================
...
describe("Phase C2 — POST /v1/collaboration/threads/:id/mark-mentions-read", () => {
    it("registers the endpoint behind requireAuth", () => {
        expect(COLLAB_ROUTES).toMatch(/app\.post\(\s*"\/v1\/collaboration\/threads\/:id\/mark-mentions-read"[\s\S]*?preHandler:\s*requireAuth/);
    });
    it("requires reviewer-member access on the workspace (anti-enumeration)", () => {
        const handler = COLLAB_ROUTES.match(/mark-mentions-read[\s\S]*?async \(req, reply\)[\s\S]*?\}\s*,\s*\)\s*;/);
        expect(handler).toBeTruthy();
        expect(handler[0]).toContain("requireReviewerMember");
    });
    it("validates the thread belongs to the workspace before updating", () => {
        expect(COLLAB_ROUTES).toMatch(/thread\.teamId\s*!==\s*query\.teamId/);
    });
    it("updates only the caller's unread mentions on the target thread", () => {
        expect(COLLAB_ROUTES).toMatch(/discussionMention\.updateMany\(\{[\s\S]*?threadId:\s*params\.id[\s\S]*?mentionedUserId:\s*ok\.userId[\s\S]*?notifiedAtUtc:\s*null/);
    });
...
describe("Phase C2 — GET /v1/cases/:id/discussion-threads aggregator", () => {
    it("registers the endpoint behind requireAuth + requireCaseAccess", () => {
        ...
        const handler = CASE_ROUTES.match(/discussion-threads[\s\S]*?\}\s*,\s*\)\s*;/);
        expect(handler).toBeTruthy();
        expect(handler[0]).toContain("requireCaseAccess");
    });
    it("scope-binds the query to the case's teamId + linked evidence", () => {
        expect(CASE_ROUTES).toMatch(/prisma\.discussionThread\.findMany\(\{[\s\S]*?teamId:\s*caseRow\.teamId[\s\S]*?evidenceId:\s*\{\s*in:\s*evidenceIds\s*\}/);
    });
...
describe("Phase C2 — /evidence/[id] Discussion tab", () => {
    it("declares discussion in the EvidenceDetailTab union", () => {
        expect(EVIDENCE_PAGE).toMatch(/type EvidenceDetailTab[\s\S]*?\|\s*"discussion"/);
    });
    it("adds Discussion entry to the DETAIL_TABS catalog with MessageSquare icon", () => {
        expect(EVIDENCE_PAGE).toMatch(/id:\s*"discussion",\s*label:\s*"Discussion",\s*icon:\s*MessageSquare/);
    });
    it("seeds initial tab + thread from URL search params", () => {
        expect(EVIDENCE_PAGE).toContain("useSearchParams");
        expect(EVIDENCE_PAGE).toMatch(/searchParams\?\.get\("tab"\)/);
        expect(EVIDENCE_PAGE).toMatch(/searchParams\?\.get\("thread"\)/);
    });
    it("renders the EvidenceDiscussionPanel when the tab is active", () => {
        expect(EVIDENCE_PAGE).toMatch(/activeTab\s*===\s*"discussion"[\s\S]*?<EvidenceDiscussionPanel/);
    });
    it("imports EvidenceDiscussionPanel from the local components folder", () => {
        expect(EVIDENCE_PAGE).toMatch(/import\s+EvidenceDiscussionPanel\s+from\s+"\.\/components\/EvidenceDiscussionPanel"/);
    });
});
// ===========================================================================
...
describe("Phase C2 — EvidenceDiscussionPanel contract", () => {
    it("consumes the existing audited collaboration endpoints", () => {
        ...
        expect(DISCUSSION_PANEL).toContain("/mark-mentions-read?teamId=");
    });
    ...
    it("locks composition when the thread is RESOLVED or CLOSED", () => {
        expect(DISCUSSION_PANEL).toMatch(/status\s*===\s*"RESOLVED"\s*\|\|[\s\S]*?status\s*===\s*"CLOSED"/);
...
describe("Phase C2 — Matter Workspace Communications tab", () => {
    it("fetches the new /v1/cases/:id/discussion-threads aggregator", () => {
        expect(MATTER_UI).toContain("/v1/cases/${encodeURIComponent(envelope.case.id)}/discussion-threads");
    });
    ...
    it("preserves the C1 empty-state contract (EmptyState reference still present)", () => {
        ...
        const tabFn = MATTER_UI.match(/function\s+CommunicationsTab\s*\([\s\S]*?\n\}\s*\n/);
