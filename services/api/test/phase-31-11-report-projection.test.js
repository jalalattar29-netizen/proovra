...
import { projectMediaIntelligenceForReport, } from "../src/services/media-intelligence/report-projection.service.js";
...
describe("Phase 31.11 — projection source contract", () => {
    ...
    it("DISMISSED signals filtered out at the SQL layer (legal distribution invariant)", () => {
        expect(src).toMatch(/s\."status" IN \('PENDING','ACKNOWLEDGED'\)/);
        expect(src).not.toMatch(/s\."status" IN \([^)]*'DISMISSED'/);
    });
    it("severity-ordered SQL: ATTENTION → REVIEW_RECOMMENDED → INFO", () => {
        expect(src).toMatch(/CASE s\."severity"[\s\S]*?WHEN 'ATTENTION' THEN 0[\s\S]*?WHEN 'REVIEW_RECOMMENDED' THEN 1[\s\S]*?ELSE 2/);
    });
...
describe("Phase 31.11 — bounded result types", () => {
    it("ProjectedSignal carries ONLY the bounded report-side fields", () => {
        ...
        for (const k of Object.keys(sample)) {
            expect(allowed.has(k), `unexpected key on ProjectedSignal: ${k}`).toBe(true);
        }
    });
});
// =============================================================================
...
describe("Phase 31.11 — worker report bridge", () => {
    const src = readSource("../../../services/worker/src/media-intelligence-report-bridge.ts");
    ...
    it("lazy-imports the projection service via @proovra/shared-runtime (Phase 31.22 boundary fix)", () => {
        expect(src).toMatch(/await import\(\s*"@proovra\/shared-runtime\/media-intelligence"/);
    });
    it("never throws — try/catch around the entire projection call returns null on failure", () => {
        expect(src).toMatch(/try\s*\{[\s\S]*?\}\s*catch \(err\)[\s\S]*?return\s+null/);
    });
    ...
    it("passes worker's prisma explicitly to the projection (so the api default isn't used)", () => {
        ...
        expect(src).toMatch(/projectMediaIntelligenceForReport\(\s*\{[\s\S]*?\},\s*prisma,?\s*\)/);
    });
});
// =============================================================================
...
describe("Phase 31.11 — processor wiring", () => {
    ...
    it("imports buildReportMediaIntelligence from the worker bridge", () => {
        expect(src).toMatch(/import\s*\{\s*buildReportMediaIntelligence\s*\}\s*from\s*"\.\/media-intelligence-report-bridge\.js"/);
    });
    it("ReportBuildParams type carries OPTIONAL mediaIntelligence field (legacy callers unaffected)", () => {
        expect(src).toMatch(/mediaIntelligence\?:\s*Parameters<typeof buildReportPdfV2>\[0\]\["mediaIntelligence"\]/);
    });
    it("BOTH buildReportPdfV2 call sites attach mediaIntelligence", () => {
        ...
        const occurrences = src.match(/buildReportMediaIntelligence\(/g) ?? [];
        expect(occurrences.length).toBeGreaterThanOrEqual(2);
    });
    it("projection runs against evidence.teamId — anti-enumeration", () => {
        expect(src).toMatch(/buildReportMediaIntelligence\(\s*\{[\s\S]*?teamId:\s*evidence\.teamId\s*\?\?\s*null/);
    });
    it("projection result attached as `mediaIntelligence:` on the ReportBuildParams payload", () => {
        ...
        expect(src).toMatch(/mediaIntelligence:\s*reportMediaIntelligence/);
        expect(src).toMatch(/mediaIntelligence:\s*finalizedReportMediaIntelligence/);
    });
});
// =============================================================================
