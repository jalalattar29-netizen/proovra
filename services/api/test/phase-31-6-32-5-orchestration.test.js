...
describe("Phase 31.6 — API enqueue producer", () => {
    const src = readSource("../../../services/api/src/queue/media-intelligence-queue.ts");
    ...
    it("buildMediaIntelligenceJobId uses the bounded mi-<kind>-<evidenceId> pattern", () => {
        expect(src).toMatch(/export function buildMediaIntelligenceJobId\([\s\S]*?return `mi-\$\{kind\}-\$\{evidenceId\}`/);
    });
...
describe("Phase 31.6 — worker processor", () => {
    ...
    it("bumps started_total at the top of the processor", () => {
        expect(src).toMatch(/tryBump\("media_intelligence_processor_started_total"\)/);
    });
    it("bumps deferred_total for the 4 reserved kinds (extract/compute/reindex)", () => {
        expect(src).toMatch(/tryBump\("media_intelligence_processor_deferred_total"\)/);
    });
    it("bumps completed_total on the success path", () => {
        expect(src).toMatch(/tryBump\("media_intelligence_processor_completed_total"\)/);
    });
    it("bumps failed_total + dlq_total on the failure path", () => {
        expect(src).toMatch(/tryBump\("media_intelligence_processor_failed_total"\)/);
    ...
    it("never throws to BullMQ on analyzer refusal (returns success-from-queue)", () => {
        expect(src).toMatch(/media_intelligence\.analyzer_refused[\s\S]*?return \{ ok: true, signalsEmitted: 0 \}/);
    });
    ...
    it("bounded job payload + bounded log/response shapes — no storage internals leak to caller", () => {
        ...
        const payloadDestructures = noComments.match(/const\s*\{[^}]*\}\s*=\s*job\.data/g) ?? [];
...
describe("Phase 32.5 — graph routes", () => {
    ...
    it("each new route is gated by authorizeOrFail with evidence.read + antiEnumeration", () => {
        // Cases route block.
        const casesBlock = src.match(/"\/v1\/graph\/cases\/:caseId"[\s\S]*?\n\s*\}\s*,?\s*\n\s*\)/)?.[0];
        ...
        const searchBlock = src.match(/"\/v1\/graph\/search"[\s\S]*?\n\s*\}\s*,?\s*\n\s*\)/)?.[0];
        expect(searchBlock).toMatch(/authorizeOrFail/);
        expect(searchBlock).toMatch(/antiEnumeration:\s*true/);
        const timelineBlock = src.match(/"\/v1\/graph\/timeline"[\s\S]*?\n\s*\}\s*,?\s*\n\s*\)/)?.[0];
...
describe("Phase 31.6 — runtime-readiness media intelligence check", () => {
    ...
    it("imports setGauge from the metrics service", () => {
        expect(src).toMatch(/import\s*\{\s*setGauge\s*\}\s*from\s*"\.\.\/services\/ops\/metrics\.service\.js"/);
    });
    it("checkMediaIntelligence sets all 5 new gauges", () => {
        ...
        expect(fn).toMatch(/setGauge\(\s*"media_intelligence_oldest_pending_age_seconds"/);
...
describe("Phase 31.6 — apps/web hook + types source contracts", () => {
    ...
    const hookSrc = readSource("../../../apps/web/lib/media-intelligence/useMediaIntelligence.ts");
    ...
    it("severityLabel returns advisory wording, NEVER alarmist", () => {
        ...
        const labelFn = typesSrc.match(/function severityLabel[\s\S]*?\n\}/)?.[0];
        expect(labelFn).toBeTruthy();
        for (const banned of ["WARNING", "CRITICAL", "ALERT", "DANGER", "BLOCKED"]) {
            expect(labelFn, `severityLabel uses ${banned}`).not.toMatch(new RegExp(`"\\s*${banned}`, "i"));
        }
    });
    ...
    it("ack supports both ACKNOWLEDGED and DISMISSED (bounded vocabulary)", () => {
        expect(hookSrc).toMatch(/Extract<ClientStatus,\s*"ACKNOWLEDGED"\s*\|\s*"DISMISSED">/);
    });
...
describe("Phase 31.6 — MediaIntelligencePanel source contract", () => {
    ...
    it("renders advisory-only language (no truth-claims, no alarmist labels)", () => {
        ...
        const forbidden = /\b(tamper(ed|ing)?|forged|fake|authentic(ity)?|admissible|proves?|confirms?|verified as real|manipulated|doctored)\b/i;
        for (const lit of literals) {
            expect(lit, `MediaIntelligencePanel uses forbidden truth-claim wording: ${lit}`).not.toMatch(forbidden);
        }
    });
...
describe("Phase 31.6 — run tracker dismiss counter", () => {
    ...
    it("dismissRun is idempotent (only transitions from non-terminal states)", () => {
        const fn = src.match(/export async function dismissRun[\s\S]*?\n\}/)?.[0];
        expect(fn).toMatch(/"status" IN \('PENDING', 'FAILED', 'PROCESSING'\)/);
    });
});
