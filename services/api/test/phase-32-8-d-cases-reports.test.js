...
describe("Phase 32.8D — case-workspace service contract", () => {
    ...
    it("never emits audit / analytics / security events on load", () => {
        for (const sym of [
            ...
            expect(CASE_SVC, `case service must not invoke ${sym}`).not.toContain(sym);
        }
    });
    it("declares bounded limits on every list query", () => {
        ...
        for (const block of findMany) {
            expect(block, `case service has unbounded findMany: ${block.slice(0, 80)}`).toMatch(/take:/);
        }
    });
    it("personal-case `reviewCoordination` short-circuits to `not_applicable`", () => {
        expect(CASE_SVC).toMatch(/scope === "PERSONAL"[\s\S]{0,200}status:\s*"not_applicable"/);
    });
...
describe("Phase 32.8D — reports aggregator service contract", () => {
    it("exports listWorkspaceArtifacts as the canonical entrypoint", () => {
        expect(REPORTS_SVC).toMatch(/export async function listWorkspaceArtifacts\(/);
    });
    it("never calls Prisma write methods", () => {
        for (const re of [
            /prisma\.[A-Za-z]+\.create\(/,
    ...
    it("never imports or invokes report / package generation or signed-URL engines", () => {
        ...
        for (const sym of [
            ...
            expect(REPORTS_SVC, `reports service must not invoke ${sym}`).not.toContain(sym);
        }
    });
    ...
    it("declares bounded lifecycle enums (report + package)", () => {
        expect(REPORTS_SVC).toMatch(/export type ReportLifecycle\s*=[\s\S]{0,200}"not_requested"[\s\S]{0,200}"pending"[\s\S]{0,200}"ready"[\s\S]{0,200}"failed"[\s\S]{0,200}"unavailable"/);
        expect(REPORTS_SVC).toMatch(/export type PackageLifecycle\s*=[\s\S]{0,200}"not_requested"[\s\S]{0,200}"pending"[\s\S]{0,200}"ready"[\s\S]{0,200}"blocked"[\s\S]{0,200}"failed"[\s\S]{0,200}"unavailable"/);
    });
    ...
    it("never projects raw storage keys, signed URLs, file SHA, or privileged legal text", () => {
        for (const sym of [
            ...
            expect(REPORTS_SVC, `reports service must not project ${sym}`).not.toContain(sym);
        }
    });
});
// =============================================================================
...
describe("Phase 32.8D — /cases list page (CasesIndex)", () => {
    it("delegates the cases page to CasesIndex", () => {
        ...
        expect(CASES_PAGE).toMatch(/import\s*\{\s*CasesIndex\s*\}\s*from\s*"[^"]*components\/cases-experience\/CasesIndex"/);
    });
...
describe("Phase 32.8D — /cases/[id] tabbed workspace (CaseWorkspace)", () => {
    ...
    it("never makes legal overclaiming statements", () => {
        for (const banned of [
            ...
            expect(CASE_WORKSPACE, `case workspace must not say "${banned}"`).not.toContain(banned);
        }
    });
...
describe("Phase 32.8D — /reports artifact lifecycle (ReportsIndex)", () => {
    ...
    function stripClickHandlers(src) {
        return src.replace(/const\s+(?:triggerReport|triggerPackage)\s*=\s*async[\s\S]*?\n  \};/g, "");
    }
    it("never CALLS the side-effecting `/v1/evidence/:id/report/latest` on mount (docstring mentions OK)", () => {
        ...
        expect(onMount).not.toMatch(/apiFetch\([^)]*\/v1\/evidence\/[^)]*\/report\/latest/);
        expect(onMount).not.toMatch(/fetch\([^)]*\/v1\/evidence\/[^)]*\/report\/latest/);
    });
    it("never CALLS the side-effecting `/v1/evidence/:id/verification-package` on mount (docstring mentions OK)", () => {
        const onMount = stripClickHandlers(REPORTS_INDEX);
        expect(onMount).not.toMatch(/apiFetch\([^)]*\/v1\/evidence\/[^)]*\/verification-package/);
        expect(onMount).not.toMatch(/fetch\([^)]*\/v1\/evidence\/[^)]*\/verification-package/);
    });
    it("does not call any /v1/governance/export-eligibility on mount (no governance probe side effects)", () => {
        expect(REPORTS_INDEX).not.toMatch(/apiFetch\([^)]*\/v1\/governance\/export-eligibility/);
        expect(REPORTS_INDEX).not.toMatch(/fetch\([^)]*\/v1\/governance\/export-eligibility/);
    });
    ...
    it("explicitly tells the operator that browsing is side-effect-free", () => {
        expect(REPORTS_INDEX).toMatch(/Browsing this page never triggers report or package generation/);
    });
    it("never makes legal-admissibility / authenticity / court-ready / truth claims (negations and quoted disclaimers are OK)", () => {
        ...
        for (const banned of [
            ...
            expect(REPORTS_INDEX, `reports index must not make positive claim "${banned}"`).not.toMatch(positiveClaim(banned));
        }
    });
    it("includes the bounded no-claim disclaimer at the top of the page", () => {
        expect(REPORTS_INDEX).toMatch(/record integrity at the time of\s+generation and do NOT assert legal admissibility/);
    });
    ...
    it("renders package-blocked rows with the bounded reason chip", () => {
        expect(REPORTS_INDEX).toMatch(/data-reports-package-blocked-reason=\{row\.package\.blockedReason\}/);
...
describe("Phase 32.8D — shared invariants", () => {
    it("aggregator services do not import the OTS / TSA / custody / billing modules", () => {
        ...
        for (const re of forbiddenImports) {
            ...
            expect(REPORTS_SVC, `reports service must not import ${re}`).not.toMatch(re);
        }
    });
    ...
    it("frontend cases + reports surfaces never make positive overclaim statements", () => {
        ...
        for (const banned of [
            ...
            for (const [name, src] of Object.entries(sources)) {
                expect(src, `${name} must not contain positive overclaim "${banned}"`).not.toMatch(positiveClaim(banned));
            }
        }
    });
    it("none of the new frontend components CALL side-effecting download endpoints (docstring mentions are OK)", () => {
        ...
        const stripDownloadHandlers = (s) => s.replace(/const\s+(?:triggerReport|triggerPackage)\s*=\s*async[\s\S]*?\n  \};/g, "");
        const sources = {
    ...
    it("personal-workspace governance/reviewer sections honor not_applicable from the server", () => {
        ...
        expect(CASE_SVC).toMatch(/reviewCoordination[\s\S]{0,400}scope === "PERSONAL"[\s\S]{0,200}not_applicable/);
