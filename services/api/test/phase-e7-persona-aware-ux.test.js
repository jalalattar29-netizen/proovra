...
import { PERSONA_CODES, PERSONA_CONTENT, PERSONA_CONTENT_FORBIDDEN_PATTERNS, PERSONA_DISPLAY_LABEL, PERSONA_CASE_TERM, PERSONA_DOMAIN_GROUP, getPersonaContent, getPersonaRecommendedNextStep, getPersonaRecommendedTemplates, getPersonaEmptyStateHint, getPersonaCaseTerm, getPersonaDashboardPriority, } from "@proovra/shared-evidence-presentation";
...
const PERSONA_CONTENT_SRC = readPackages("shared-evidence-presentation/src/persona-content.ts");
const CAPABILITY_REGISTRY_SRC = readApi("src/services/platform-context/capability-registry.ts");
const PERSONA_PROFILE_SERVICE_SRC = readApi("src/services/platform-context/persona-profile.service.ts");
const USE_PERSONA_PROFILE_SRC = readWeb("lib/platform-context/usePersonaProfile.ts");
const PERSONA_SECTION_ORDER_SRC = readWeb("lib/platform-context/personaSectionOrder.ts");
const PERSONA_RECOMMENDED_ACTIONS_SRC = readWeb("lib/platform-context/personaRecommendedActions.ts");
const ONBOARDING_RESOLVER_SRC = readWeb("lib/onboarding/resolveOnboardingState.ts");
// ===========================================================================
...
describe("E7 Test 1 — persona content shape + completeness", () => {
    it("PERSONA_CODES matches the backend WORKSPACE_PERSONA_PROFILES enum", () => {
        expect([...PERSONA_CODES].sort()).toEqual([...WORKSPACE_PERSONA_PROFILES].sort());
    });
    it.each(PERSONA_CODES)("PERSONA_CONTENT has a complete record for %s", (code) => {
        ...
        expect(Object.keys(content.emptyStateHints).length).toBeGreaterThanOrEqual(3);
    });
    ...
    it.each(PERSONA_CODES)("case-term is defined for %s", (code) => {
        expect(PERSONA_CASE_TERM[code]).toMatch(/^(Case|Matter|Claim|Investigation)$/);
    });
...
describe("E7 Test 2 — persona content respects the E5 trust-language boundary", () => {
    for (const code of PERSONA_CODES) {
        describe(`persona ${code}`, () => {
            ...
            it.each(PERSONA_CONTENT_FORBIDDEN_PATTERNS)("does NOT match %s", (pattern) => {
                expect(blob).not.toMatch(pattern);
            });
        });
    }
    it("persona-content.ts file body (outside the forbidden-list declaration) is clean", () => {
        const sanitised = PERSONA_CONTENT_SRC.replace(/PERSONA_CONTENT_FORBIDDEN_PATTERNS[\s\S]*?\]\s*;/m, "");
...
describe("E7 Test 3 — capability registry has zero persona dependency", () => {
    it("CapabilityResolverInput shape does NOT include persona / personaCode / primaryProfile", () => {
        ...
        const inputBlock = CAPABILITY_REGISTRY_SRC.match(/CapabilityResolverInput\s*=\s*\{[\s\S]*?\};/m);
    ...
    it("persona-profile.service.ts states the 'NEVER grants capabilities' contract verbatim", () => {
        expect(PERSONA_PROFILE_SERVICE_SRC).toMatch(/NEVER grants capabilities/);
    });
});
// ===========================================================================
...
describe("E7 Test 4 — frontend persona consumers", () => {
    it("usePersonaProfile.ts declares the 'NEVER replaces a capability check' rule", () => {
        expect(USE_PERSONA_PROFILE_SRC).toMatch(/NEVER replaces a capability check/);
    });
    it("personaSectionOrder.ts states the bounded-section-id rule", () => {
        expect(PERSONA_SECTION_ORDER_SRC).toMatch(/Capabilities remain authoritative/);
    ...
    it("personaRecommendedActions.ts states the 'never replaces capability check' rule", () => {
        expect(PERSONA_RECOMMENDED_ACTIONS_SRC).toMatch(/NEVER replaces a capability check/);
    });
...
describe("E7 Test 5 — onboarding resolver: persona hint is optional + additive", () => {
    it("resolveOnboardingState.ts wires persona as an OPTIONAL field", () => {
        ...
        expect(ONBOARDING_RESOLVER_SRC).toMatch(/input\.personaCode\s*\?\s*getPersonaRecommendedNextStep/);
    });
...
describe("E7 Test 10 — documentation + registry", () => {
    ...
    it("registry registers Phase E7 with explicit closure status", () => {
        ...
        expect(registry).toMatch(/\|\s*(Phase )?E7\s*\|[\s\S]*?(CLOSED|CLOSED_WITH_DEFERRED_ITEMS)/);
    });
});
