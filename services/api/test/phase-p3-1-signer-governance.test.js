...
import { buildCanonicalPayload, CUSTODY_ATTESTATION_OUTCOMES, hashCanonicalPayload, } from "../src/services/operations/custody-attestation.service.js";
import { SIGNER_PROVIDERS, SIGNER_PURPOSES, SIGNER_STATUSES, getCurrentActiveSigners, } from "../src/services/operations/signer-registry.service.js";
...
describe("Phase P3.1 — Bounded enums", () => {
    it("SIGNER_PURPOSES is exactly the 4 documented purposes", () => {
        expect(new Set(SIGNER_PURPOSES)).toEqual(new Set([
            "report_pdf",
    ...
    it("SIGNER_PROVIDERS is exactly the 3 documented providers", () => {
        expect(new Set(SIGNER_PROVIDERS)).toEqual(new Set(["aws_kms", "local_pem", "disabled"]));
    });
    it("SIGNER_STATUSES is exactly the 6 documented statuses", () => {
        expect(new Set(SIGNER_STATUSES)).toEqual(new Set([
            "active",
            "staged",
            "retiring",
            "retired",
    ...
    it("CUSTODY_ATTESTATION_OUTCOMES is exactly the 7 bounded outcomes", () => {
        expect(new Set(CUSTODY_ATTESTATION_OUTCOMES)).toEqual(new Set([
            "verified",
            "missing_attestation",
            "signature_invalid",
...
describe("Phase P3.1 — Canonical payload determinism", () => {
    ...
    it("different fields → different hash", () => {
        ...
        const h2 = hashCanonicalPayload(buildCanonicalPayload({ ...base, sequence: 2 }));
        expect(h1).not.toBe(h2);
    });
    it("canonical projection does NOT include payload, ip, or userAgent", () => {
        const src = readSource("../src/services/operations/custody-attestation.service.ts");
...
describe("Phase P3.1 — Current active signers from env", () => {
    it("derives 4 signers — one per purpose", () => {
        ...
        expect(new Set(list.map((s) => s.signerPurpose))).toEqual(new Set(SIGNER_PURPOSES));
    });
...
describe("Phase P3.1 — Routes registered + step-up gated", () => {
    ...
    it("promote / retire / revoke / backfill route through step-up purposes", () => {
        const r = readSource("../src/routes/operations-signers.routes.ts");
        expect(r).toMatch(/requireStepUpForSensitiveAction[\s\S]{0,200}purpose:\s*"SIGNER_PROMOTE"/);
        expect(r).toMatch(/requireStepUpForSensitiveAction[\s\S]{0,200}purpose:\s*"SIGNER_RETIRE"/);
        expect(r).toMatch(/requireStepUpForSensitiveAction[\s\S]{0,200}purpose:\s*"SIGNER_REVOKE"/);
        expect(r).toMatch(/requireStepUpForSensitiveAction[\s\S]{0,200}purpose:\s*"CUSTODY_ATTESTATION_BACKFILL"/);
    });
...
describe("Phase P3.1 — Bounded registries extended", () => {
    it("step-up purposes include the 4 new entries", () => {
        const idn = readSource("../../../packages/shared/src/identity-security.ts");
        for (const p of [
            '"SIGNER_PROMOTE"',
            '"SIGNER_RETIRE"',
            '"SIGNER_REVOKE"',
...
describe("Phase P3.1 — Frontend signer governance page", () => {
    it("/operations/signers page exists", () => {
        expect(exists("../../../apps/web/app/(app)/operations/signers/page.tsx")).toBe(true);
    });
    it("page calls all documented backend endpoints", () => {
        const p = readSource("../../../apps/web/app/(app)/operations/signers/page.tsx");
        for (const ep of [
            "/v1/operations/signers",
            "/v1/operations/custody-attestations",
            "/health",
...
describe("Phase P3.1 — Historical artifact immutability", () => {
    it("rotation service never mutates Report or VerificationPackage rows", () => {
        const src = readSource("../src/services/operations/signer-rotation.service.ts");
