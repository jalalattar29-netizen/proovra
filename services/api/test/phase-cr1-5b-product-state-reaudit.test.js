...
const REAUDIT_DOC = readRepo("docs/product/CR1_5_STATE_ORCHESTRATION_OBSERVABILITY.md");
// ===========================================================================
...
describe("CR1.5B Test 1 — re-audit documentation exists + substantial", () => {
    it("docs/product/CR1_5_STATE_ORCHESTRATION_OBSERVABILITY.md is present and >= 10KB", () => {
        ...
        expect(REAUDIT_DOC).toMatch(/docs\/recovery\/CR1_5_STATE_ORCHESTRATION_OBSERVABILITY\.md/);
    });
...
describe("CR1.5B Test 2 — dead-code `ShellNoWorkspace()` removed by CR1.6", () => {
    ...
    it.each(CLEANED_SITES)("$file: `no_workspace` LoadState branch and `$symbol()` are gone", ({ file, symbol }) => {
        const src = readWeb(file);
        // CR1.6 — the dead LoadState union value must be removed.
        expect(src, `${file}: LoadState should not declare 'no_workspace' (dead branch removed in CR1.6).`).not.toMatch(/status:\s*["']no_workspace["']/);
        // CR1.6 — the dead render-switch arm must be removed.
        expect(src, `${file}: render switch should not check 'no_workspace' (dead branch removed in CR1.6).`).not.toMatch(new RegExp(`state\\.status\\s*===\\s*["']no_workspace["']`));
        // CR1.6 — the dead function definition must be removed.
        expect(src, `${file}: function ${symbol}() should be removed (dead code removed in CR1.6).`).not.toMatch(new RegExp(`function ${symbol}\\s*\\(`));
...
describe("CR1.5B Test 3 — `Personal Space` label is canonical (no drift)", () => {
    it("backend platform-context.service.ts emits the exact string `Personal Space`", () => {
        const src = readApi("src/services/platform-context/platform-context.service.ts");
...
describe("CR1.5B Test 5 — fallback label is operationally neutral", () => {
    it("STATE_FALLBACK_LABEL exports the exact string `Status pending`", () => {
        const src = readWeb("lib/product-language/stateLabels.ts");
        expect(src).toMatch(/STATE_FALLBACK_LABEL\s*=\s*"Status pending"\s*as\s*const/);
    });
});
// ===========================================================================
...
describe("CR1.5B Test 9 — observability event vocabulary covers production emit sites", () => {
    it("STATE_OBSERVABILITY_EVENTS includes every event actually emitted in production code", () => {
        ...
        for (const evt of EMITTED_IN_PROD) {
            expect(obs, `STATE_OBSERVABILITY_EVENTS missing event emitted in prod code: ${evt}`).toContain(`"${evt}"`);
        }
    });
...
describe("CR1.5B Test 11 — no new client-side state library introduced", () => {
    it("apps/web/package.json does NOT depend on React Query, SWR, Redux, or Zustand", () => {
        ...
        for (const name of FORBIDDEN) {
            expect(allDeps[name], `Forbidden client-state library introduced: ${name}. Adding a parallel cache library requires CR-level decision per CR1.5 §9.`).toBeUndefined();
        }
    });
});
// ===========================================================================
    ...
    it("no apps/web/ file references a forbidden product-state localStorage key", () => {
        ...
        for (const file of files) {
            ...
            while ((m = re.exec(src)) !== null) {
                ...
                const hit = FORBIDDEN_SUBSTRINGS.some((s) => lower.includes(s.toLowerCase()));
                if (!hit)
                    continue;
        ...
        expect(offenders, `Forbidden product-state localStorage key(s) introduced:\n${offenders.join("\n")}\nAdd to ALLOWED_KEYS only with explicit CR1.5 doc justification — the canonical state source is PlatformContextEnvelope.`).toEqual([]);
    });
});
// ===========================================================================
...
describe("CR1.5B Test 14 — canonical workspace-state hooks remain exported", () => {
    it("lib/platform-context exports the canonical hooks used by every surface", () => {
        ...
        for (const name of REQUIRED_EXPORTS) {
            expect(idx, `lib/platform-context/index.ts must export the canonical hook ${name}`).toMatch(new RegExp(`\\b${name}\\b`));
        }
    });
});
