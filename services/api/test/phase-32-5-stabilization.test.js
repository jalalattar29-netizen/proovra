...
describe("Phase 32.5 → 32.6.6 — verification package personal-workspace handling", () => {
    ...
    it("Phase 32.6.6 — `verification_package.skipped_personal_workspace` log line is removed", () => {
        expect(PROCESSOR_SRC).not.toMatch(/verification_package\.skipped_personal_workspace/);
    });
...
describe("Phase 32.5 → 32.6.6 — artifact status projection", () => {
    const SERVICE_SRC = readSource("../src/services/evidence-artifact-status.service.ts");
    it("EvidenceArtifactStatus.verificationPackage carries `unavailable` + `unavailableReason` (shape retained for backward-compat)", () => {
        ...
        expect(SERVICE_SRC).toMatch(/unavailable: boolean;\s*unavailableReason: VerificationPackageUnavailableReason \| null;/);
    });
    it("Phase 32.6.6 — `unavailable` for personal-workspace path is now always false", () => {
        ...
        expect(SERVICE_SRC).toMatch(/packageUnavailableForPersonalWorkspace\s*=\s*false/);
        expect(SERVICE_SRC).not.toMatch(/packageUnavailableForPersonalWorkspace\s*=\s*finalized\s*&&\s*!params\.evidenceTeamId/);
    });
    it("Phase 32.6.6 — packagePending derivation still excludes blocked + unavailable", () => {
        const code = stripComments(SERVICE_SRC);
        expect(code).toMatch(/packagePending\s*=\s*finalized\s*&&\s*\n?\s*!latestPackage\s*&&\s*\n?\s*!packageUnavailableForPersonalWorkspace\s*&&\s*\n?\s*!packageBlocked/);
    });
    it("Phase 32.6.6 — `unavailableReason` enum no longer emits a value (reserved for future cases)", () => {
        ...
        expect(SERVICE_SRC).toMatch(/export type VerificationPackageUnavailableReason\s*=\s*(never|\|\s*"[^"]+")\s*;/);
    ...
    it("route passes evidenceTeamId through to the helper", () => {
        ...
        expect(ROUTES_SRC).toMatch(/buildEvidenceArtifactStatus\(\{[\s\S]{0,400}evidenceTeamId:\s*evidenceRecord\.teamId\s*\?\?\s*null/);
    });
});
// =============================================================================
...
describe("Phase 32.5 — evidence detail page artifact polling", () => {
    const PAGE_SRC = readSource("../../../apps/web/app/(app)/evidence/[id]/page.tsx");
    it("polls /v1/evidence/:id/artifacts/status (the side-effect-free endpoint)", () => {
        expect(PAGE_SRC).toMatch(/\/v1\/evidence\/\$\{evidenceId\}\/artifacts\/status/);
    ...
    it("polling stops when both report + package are no longer pending", () => {
        expect(PAGE_SRC).toMatch(/reportStillPending\s*\|\|\s*packageStillPending/);
    ...
    it("polling triggers loadWorkspace ONLY on state transition (not every tick)", () => {
        const code = stripComments(PAGE_SRC);
        expect(code).toMatch(/stateChanged\s*=\s*reportNowAvailable !== priorReportAvailable \|\|\s*packageNowAvailable !== priorPackageAvailable/);
    ...
    it("polling is gated on finalized status (SIGNED or REPORTED) — no polling for CREATED/UPLOADING", () => {
        expect(PAGE_SRC).toMatch(/const finalized = status === "SIGNED" \|\| status === "REPORTED";/);
    });
});
// =============================================================================
...
describe("Phase 32.5 — governance route schema-drift bounded handler", () => {
    ...
    it("the CORE governance read routes consume runGovernanceHandler", () => {
        ...
        const wrapsCount = (ROUTES_SRC.match(/await runGovernanceHandler\(reply,/g) ?? []).length;
        ...
        // contract.
        const caseGetIdx = ROUTES_SRC.indexOf('app.get(\n    "/v1/governance/case-legal-holds"');
...
describe("Phase 32.5 — workspace profile + sidebar visibility", () => {
    const PROFILE_SRC = readSource("../../../apps/web/lib/workspace-profile.ts");
    const SIDEBAR_SRC = readSource("../../../apps/web/components/app-shell-v2/AppSidebarV2.tsx");
    ...
    const NAV_CONFIG_SRC = readSource("../../../apps/web/lib/navigation-config.ts");
    it("workspace profile catalog has 6 bounded values including INDIVIDUAL and ENTERPRISE_ADMIN", () => {
        const m = PROFILE_SRC.match(/export const WORKSPACE_PROFILES = \[([\s\S]*?)\] as const/);
        expect(m).toBeTruthy();
    ...
    it("workspace role catalog includes the bounded set", () => {
        const m = PROFILE_SRC.match(/export const WORKSPACE_ROLES = \[([\s\S]*?)\] as const/);
        expect(m).toBeTruthy();
        const entries = (m[1].match(/"[A-Z_]+"/g) ?? []).map((e) => e.slice(1, -1));
    ...
    it("filterByVisibility honours platform-admin + roles + profiles predicates", () => {
        ...
        expect(code).toMatch(/if \(v\.requiresPlatformAdmin && !context\.isPlatformAdmin\) return false/);
    ...
    it("administration items gate visibility on OWNER/ADMIN role (Phase 32.8B nav config)", () => {
        ...
        const adminGroupIdx = code.indexOf("const ADMINISTRATION_GROUP: NavGroup");
...
describe("Phase 32.5 — consolidated migration absorbs drift patches", () => {
    const MIGRATION_PATH = "../prisma/migrations/20260620100000_phase24_31_consolidated_drift_patches/migration.sql";
    ...
    it("migration absorbs every drift patch listed in services/api/sql/drift-patches/", () => {
        const driftDir = fileURLToPath(new URL("../../api/sql/drift-patches/", import.meta.url));
        ...
        for (const patch of patches) {
            expect(MIGRATION_SRC.includes(`BLOCK: ${patch}`) ||
                MIGRATION_SRC.includes(patch), `Migration must absorb drift patch ${patch}`).toBe(true);
        }
    });
    it("migration creates reviewer_ops_reminders (which had no creator anywhere before)", () => {
        expect(MIGRATION_SRC).toMatch(/CREATE TABLE IF NOT EXISTS "reviewer_ops_reminders"/);
        expect(MIGRATION_SRC).toMatch(/reviewer_ops_reminders_team_kind_dedup_uk/);
    });
...
describe("Phase 32.5 — root layout viewport meta", () => {
    ...
    it("root layout declares the mobile viewport meta tag", () => {
        expect(LAYOUT_SRC).toMatch(/<meta\s+name="viewport"\s+content="width=device-width, initial-scale=1\.0[^"]*"\s*\/>/);
    });
