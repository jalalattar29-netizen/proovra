...
const SHARED_SECURITY = readFileSync(fileURLToPath(new URL(`../../../packages/shared/src/security.ts`, import.meta.url)), "utf8");
// =============================================================================
...
describe("R8 Part 2 — bounded R8 identity event vocabulary", () => {
    ...
    it("SECURITY_EVENT_TYPES contains every R8 identity event", () => {
        for (const event of R8_EVENTS) {
            expect(SHARED_SECURITY, `SECURITY_EVENT_TYPES must include R8 event "${event}"`).toMatch(new RegExp(`"${event}"`));
        }
    });
...
describe("R8 Part 4 — no parallel identity infrastructure introduced", () => {
    function listAllRouteFiles() {
        const out = [];
        const dir = apiPath("src/routes");
        for (const name of readdirSync(dir)) {
            const full = `${dir}/${name}`;
            try {
                if (statSync(full).isFile() && /\.ts$/.test(name))
                    out.push(name);
            }
            catch {
                /* ignore */
    ...
    it("no new parallel auth route file (`mfa.routes.ts` is an identity sub-domain shipped by R8.1.1, NOT a parallel auth system)", () => {
        ...
        for (const forbidden of FORBIDDEN_NAMES) {
            expect(files.includes(forbidden), `R8 forbids parallel auth route file ${forbidden} — use canonical routes/auth.routes.ts + routes/sso-auth.routes.ts + routes/identity-security.routes.ts.`).toBe(false);
        }
    });
    it("no new SCIM route file (no `scim2.routes.ts` / `scim-v2.routes.ts`)", () => {
        ...
        const files = listAllRouteFiles();
        for (const forbidden of FORBIDDEN_NAMES) {
            expect(files.includes(forbidden), `R8 forbids parallel SCIM route file ${forbidden} — use canonical routes/scim.routes.ts.`).toBe(false);
        }
    });
    it("no parallel security-event service (canonical is services/security/security-event.service.ts)", () => {
        function listAllSvcFiles(dirAbs) {
            const out = [];
            const stack = [dirAbs];
            while (stack.length > 0) {
                ...
                for (const name of entries) {
                    ...
                    try {
                        ...
                        if (st.isFile() && /security-event/.test(name))
                            out.push(full);
        ...
        const canonical = matches.find((f) => f.replace(/\\/g, "/").endsWith("/services/security/security-event.service.ts"));
        expect(canonical, "canonical security-event.service.ts must exist under services/security/").toBeTruthy();
        expect(matches.length, "only one security-event service file allowed").toBe(1);
    });
});
// =============================================================================
...
describe("R8 Part 5 — canonical identity files preserved in size", () => {
    ...
    for (const { rel, expectedBytes } of PINS) {
        it(`${rel} is within ±5% of the R8 audit baseline (${expectedBytes} bytes)`, () => {
            ...
            expect(st.size, `${rel} size ${st.size} drifted out of R8 window [${low}, ${high}]. R8 should NOT modify canonical auth/identity files.`).toBeGreaterThanOrEqual(low);
    ...
    for (const rel of SWEEP_FILES) {
        it(`${rel} contains no raw token/secret/OTP logging`, () => {
            ...
            for (const pattern of FORBIDDEN_PATTERNS) {
                expect(src, `${rel} contains forbidden logging anti-pattern ${pattern}`).not.toMatch(pattern);
            }
        });
    }
});
...
describe("R8 Part 9 — no workflow/persona authorization regression", () => {
    it("routeAccessResolver still consults capabilities + active-space only", () => {
        const src = readFileSync(fileURLToPath(new URL(`../../../apps/web/lib/navigation/routeAccessResolver.ts`, import.meta.url)), "utf8");
    ...
    it("workflowExposureResolver still documents the no-authorization contract", () => {
        const src = readFileSync(fileURLToPath(new URL(`../../../apps/web/lib/navigation/workflowExposureResolver.ts`, import.meta.url)), "utf8");
