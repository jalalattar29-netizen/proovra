...
import { buildEvidenceProjection, buildWorkflowInstanceProjection, computeReviewerPriority, detectStuckWorkflow, PRIORITY_REASON_CODES, sanitiseSearchBody, sanitiseSearchString, sanitiseSearchTags, STUCK_REASON_CODES, summarisePriorityReasons, } from "@proovra/shared";
...
describe("Phase 25 — shared search projection engine", () => {
    const sourceSrc = readSource("../../../packages/shared/src/search-projection.ts");
    ...
    it("the API indexer + worker BOTH import from @proovra/shared", () => {
        const apiSrc = readSource("../../../services/api/src/services/search/evidence-indexing.service.ts");
        const workerSrc = readSource("../../../services/worker/src/search-indexing.processor.ts");
        // The import block may span multiple lines with several
        // co-imported types — widen the gap allowance accordingly.
        expect(apiSrc).toMatch(/buildEvidenceProjection[\s\S]{0,600}from\s+"@proovra\/shared"/);
        expect(workerSrc).toMatch(/buildEvidenceProjection[\s\S]{0,600}from\s+"@proovra\/shared"/);
    });
    it("sanitiseSearchString strips overclaim phrases + control chars + trims to max", () => {
        expect(sanitiseSearchString("legally admissible record", 200)).toBe("[redacted-overclaim] record");
    ...
    it("buildEvidenceProjection asks for delete-from-index when the row is deleted / DESTROYED / PENDING_DESTRUCTION", () => {
        ...
        for (const variant of [
            { ...base, deletedAt: new Date(), expectedReason: "deleted" },
            {
                ...base,
                deletedAt: null,
                lifecycleState: "DESTROYED",
                expectedReason: "lifecycle_destroyed",
            },
            {
                ...base,
                deletedAt: null,
                lifecycleState: "PENDING_DESTRUCTION",
                expectedReason: "lifecycle_pending_destruction",
            },
        ]) {
...
describe("Phase 25 — search worker rebuild source contract", () => {
    ...
    it("delegates projection to @proovra/shared (no duplicated logic)", () => {
        ...
        const importMatch = workerSrc.match(/import\s*\{([\s\S]*?)\}\s*from\s+"@proovra\/shared"/);
        ...
        expect(workerSrc).not.toMatch(/function buildEvidenceProjection\(/);
        expect(workerSrc).not.toMatch(/function buildWorkflowInstanceProjection\(/);
    });
    ...
    it("only unwinds OCR + transcript indexing-lag pointers AFTER a successful upsert", () => {
        ...
        expect(workerSrc).toMatch(/if \(upserted && projectedEvidenceId\)[\s\S]*?UPDATE "evidence_ocr_text"[\s\S]*?SET "indexed_at_utc"/);
    });
    it("OCR + transcript reads filter to TEAM scope + non-redacted (governance gate)", () => {
        expect(workerSrc).toMatch(/FROM "evidence_ocr_text"[\s\S]*?"visibility_scope" = 'TEAM'[\s\S]*?"redacted" = FALSE/);
        expect(workerSrc).toMatch(/FROM "evidence_transcript_segments"[\s\S]*?"visibility_scope" = 'TEAM'[\s\S]*?"redacted" = FALSE/);
    });
...
describe("Phase 25 — reviewer priority scoring engine", () => {
    ...
    it("legal-hold + SLA-breached + CRITICAL escalation rank URGENT (governance-first)", () => {
        ...
        expect(r.reasons.map((x) => x.code)).toEqual(expect.arrayContaining([
            "legal_hold_active",
            "sla_breached",
            "escalation_critical",
        ]));
    ...
    it("reason labels are operator-readable, not free-text from input", () => {
        ...
        expect(r.reasons.some((x) => x.label === "Reviewer SLA breached")).toBe(true);
    });
...
describe("Phase 25 — stuck workflow detector", () => {
    ...
    it("submitted > 24h without assignment is stuck (HIGH)", () => {
        ...
        expect(r.reasons.some((x) => x.code === "submitted_never_assigned")).toBe(true);
    ...
    it("SLA breached without an escalation is stuck (CRITICAL)", () => {
        ...
        expect(r.reasons.some((x) => x.code === "sla_overdue_no_escalation")).toBe(true);
    });
    it("escalated but unacknowledged > 8h is stuck (HIGH)", () => {
        ...
        expect(r.isStuck).toBe(true);
        expect(r.reasons.some((x) => x.code === "escalated_unacknowledged")).toBe(true);
    });
    it("approved but export still blocked is stuck (WARNING)", () => {
        ...
        expect(r.isStuck).toBe(true);
        expect(r.reasons.some((x) => x.code === "approved_export_blocked")).toBe(true);
    });
    it("every reason code is in the bounded catalog", () => {
        const r = detectStuckWorkflow({
            ...baseFacts,
