...
import { NAVIGATION_REGISTRY, buildNavigationProjection, filterNavigationRegistry, } from "../src/services/platform-context/navigation-registry.js";
// ===========================================================================
...
describe("Phase ROUTE-FIX — sidebar / account-menu projection", () => {
    ...
    it("Pricing is visible in the account menu for EVERY scope (FREE personal, PRO personal, TEAM)", () => {
        for (const input of [
            { scope: "PERSONAL", role: "OWNER", plan: "FREE" },
            { scope: "PERSONAL", role: "OWNER", plan: "PRO" },
            { scope: "TEAM", role: "MEMBER", plan: "TEAM" },
            { scope: "TEAM", role: "VIEWER", plan: "TEAM" },
        ]) {
            const caps = resolveCapabilities({
                ...input,
                isPlatformAdmin: false,
            ...
            const { accountMenu } = buildNavigationProjection(caps);
            const ids = accountMenu.items.map((i) => i.id);
            expect(ids, `Pricing missing for ${JSON.stringify(input)}`).toContain("account.pricing");
        }
    });
});
// ===========================================================================
...
describe("Phase ROUTE-FIX — public routes are not workspace-gated", () => {
    it("Pricing route exists on disk at app/pricing/page.tsx", async () => {
        ...
        const path = fileURLToPath(new URL("../../../apps/web/app/pricing/page.tsx", import.meta.url));
        expect(() => readFileSync(path, "utf8")).not.toThrow();
    });
    it("Support route exists on disk at app/support/page.tsx", async () => {
        const { readFileSync } = await import("node:fs");
        const { fileURLToPath } = await import("node:url");
        const path = fileURLToPath(new URL("../../../apps/web/app/support/page.tsx", import.meta.url));
        expect(() => readFileSync(path, "utf8")).not.toThrow();
    });
    it("Pricing + Help are NOT inside the (app) workspace layout (no workspace gating)", async () => {
        ...
        const inAppLayout = fileURLToPath(new URL("../../../apps/web/app/(app)/pricing", import.meta.url));
        expect(existsSync(inAppLayout)).toBe(false);
        const helpInAppLayout = fileURLToPath(new URL("../../../apps/web/app/(app)/support", import.meta.url));
...
describe("Phase ROUTE-FIX — topbar wires the canonical account menu", () => {
    it("AppTopbarV2 reads navigation.accountMenu.items from the envelope", async () => {
        const { readFileSync } = await import("node:fs");
        const { fileURLToPath } = await import("node:url");
        const src = readFileSync(fileURLToPath(new URL("../../../apps/web/components/app-shell-v2/AppTopbarV2.tsx", import.meta.url)), "utf8");
