...
const MIGRATION = readApi(
  "prisma/migrations/20260625100000_phase328cpppp_dashboard_intelligence_closure/migration.sql",
);
...
describe("Phase 32.8C++++ — migration file source-contract", () => {
  ...
  it("ALTER TABLE statements use ADD COLUMN IF NOT EXISTS (additive, idempotent)", () => {
    expect(MIGRATION).toMatch(
      /ALTER TABLE "evidence_reviewer_comments"[\s\S]*?ADD COLUMN IF NOT EXISTS "resolved_at_utc"/,
    );
    expect(MIGRATION).toMatch(
      /ALTER TABLE "evidence_annotations"[\s\S]*?ADD COLUMN IF NOT EXISTS "resolved_at_utc"/,
    );
  });

  it("integrity snapshot CASCADEs on evidence delete (1:1 mirror)", () => {
    expect(MIGRATION).toMatch(
      /evidence_integrity_snapshots_evidence_id_fkey[\s\S]*?ON DELETE CASCADE/,
    );
  });
...
describe("Phase 32.8C++++ — dashboard wiring", () => {
  it("imports the integrity snapshot reader + backfill", () => {
    expect(COMMAND_CENTER).toMatch(
      /import\s*\{[^}]*backfillIntegritySnapshots[^}]*\}\s*from\s*"\.\/integrity-snapshot\.service\.js"/,
    );
    expect(COMMAND_CENTER).toMatch(
      /import\s*\{[^}]*listWorkspaceIntegritySnapshots[^}]*\}\s*from\s*"\.\/integrity-snapshot\.service\.js"/,
    );
  });

  it("imports the access anomaly reader + classifier", () => {
    expect(COMMAND_CENTER).toMatch(
      /import\s*\{[^}]*listWorkspaceAccessAnomalies[^}]*\}\s*from\s*"\.\/access-anomaly\.service\.js"/,
    );
    expect(COMMAND_CENTER).toMatch(
      /import\s*\{[^}]*runClassifierForWorkspace[^}]*\}\s*from\s*"\.\/access-anomaly\.service\.js"/,
    );
  });
