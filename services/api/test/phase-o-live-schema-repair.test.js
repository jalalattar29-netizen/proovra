...
const MIGRATION = "services/api/prisma/migrations/20261007000000_phase_o_live_schema_compatibility_repair/migration.sql";
...
describe("Phase O — audit parser excludes reverse-1-to-1 relation fields", () => {
    ...
    for (const { model, field } of RELATION_FIELDS) {
        it(`${model}.${field} is NOT a scalar column in the parsed model`, async () => {
            ...
            expect(present, `Relation field ${model}.${field} must NOT appear in scalar field list`).toBeUndefined();
        });
    }
...
describe("Phase O — audit suggestRepair never emits TYPE_TBD on the real schema", () => {
    it("fieldToSqlType returns a concrete type for every scalar field in the real schema", async () => {
        const mod = await import(REPO_ROOT + AUDIT_SCRIPT);
        const { parsePrismaSchema, fieldToSqlType } = mod;
        ...
        expect(unmapped, `Every non-enum scalar field must map to a concrete SQL type (no TYPE_TBD). Unmapped: ${JSON.stringify(unmapped)}`).toEqual([]);
    });
});
// ---------------------------------------------------------------------------
...
describe("Phase O — backfill safety", () => {
    ...
    it("every backfill UPDATE pairs WHERE target IS NULL with source check", () => {
        ...
        for (const upd of updates) {
            ...
            const hasIsColumnsCheck = /information_schema\.columns/i.test(ctxBefore);
            const hasNullGuard = /IS\s+NULL/i.test(ctxAfter) || /%I\s+IS\s+NULL/i.test(ctxAfter);
            const isFormatCall = /format\([\s\S]*?UPDATE/i.test(ctxBefore + upd);
            expect.soft(hasIsColumnsCheck, `UPDATE "${upd}" must be inside a DO block that first checks information_schema.columns for the source column.`).toBe(true);
            expect.soft(hasNullGuard || isFormatCall, `UPDATE "${upd}" must use a WHERE target IS NULL guard (literal or via format()).`).toBe(true);
        }
    });
...
describe("Phase O — REPAIR_NOW column coverage", () => {
    ...
    for (const { table, col } of REPAIR_NOW) {
        it(`${table}.${col} ADDed by migration`, () => {
            const re = new RegExp(`ALTER\\s+TABLE\\s+IF\\s+EXISTS\\s+"${table}"[\\s\\S]*?ADD\\s+COLUMN\\s+IF\\s+NOT\\s+EXISTS\\s+"${col}"`, "i");
...
describe("Phase O — MANUAL_DECISION items are NOT in the migration", () => {
    ...
    it("evidence_workflow_instance_evidence is NOT touched (missing PK id is operator decision)", () => {
        expect(src).not.toMatch(/ALTER\s+TABLE\s+IF\s+EXISTS\s+"evidence_workflow_instance_evidence"/i);
    });
});
// ---------------------------------------------------------------------------
