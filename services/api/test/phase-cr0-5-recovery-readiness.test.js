...
function listAllFiles(dirAbs, extRegex) {
    const out = [];
    const stack = [dirAbs];
    while (stack.length > 0) {
        ...
        for (const name of entries) {
            ...
            try {
                ...
                if (stat.isFile() && extRegex.test(name))
                    out.push(full);
...
describe("Phase CR0.5 — readiness document + roadmap lock", () => {
    it("CR0.5 readiness document exists at the canonical path", () => {
        const path = fileURLToPath(new URL("../../../docs/recovery/CR0_5_RECOVERY_READINESS.md", import.meta.url));
    ...
    it("recovery roadmap order is locked in the doc — 23 phases CR0 → CR8", () => {
        ...
        for (const phase of REQUIRED_PHASES) {
            expect(doc, `recovery roadmap missing phase "${phase}"`).toMatch(new RegExp(`\\*\\*${phase.replace(/\./g, "\\.")}\\*\\*`));
        }
    });
    it("CR0 baseline document still exists (CR0.5 extends, does not replace)", () => {
        const path = fileURLToPath(new URL("../../../docs/recovery/CR0_SYSTEM_FREEZE_BASELINE.md", import.meta.url));
        const src = readFileSync(path, "utf8");
...
describe("Phase CR0.5 — useTeamWorkspaceGate live consumer count is pinned", () => {
    ...
    it("only the pinned production files import useTeamWorkspaceGate", () => {
        ...
        expect(offenders, `New code must NOT use useTeamWorkspaceGate. Migrate to useActiveSpaceId. Offenders:\n${offenders.join("\n")}`).toEqual([]);
    });
});
// =============================================================================
...
describe("Phase CR0.5 — production route registration debt pinned (CR1 will purge)", () => {
    ...
    it("the auditMiddleware HIGH-RISK hook was removed in CR1 Phase D", () => {
        ...
        const server = readApi("src/server.ts");
        expect(server, "auditMiddleware import must not return.").not.toMatch(/^import[^\n]*\bauditMiddleware\b/m);
        expect(server, "auditMiddleware addHook wiring must not return.").not.toMatch(/\bapp\.addHook\([^)]*\bauditMiddleware\b/);
    });
...
describe("Phase CR0.5 — capture/finalization/custody guard (CR0.5 must not touch)", () => {
    ...
    for (const pin of PINNED_FILES) {
        it(`${pin.rel} is within ±${(pin.tolerance * 100).toFixed(0)}% of CR0.5 baseline (DO NOT REFACTOR in CR0.5)`, () => {
            ...
            expect(bytes, `${pin.rel} drifted from CR0.5 baseline of ${pin.expectedBytes} bytes (current: ${bytes}). If this is intentional, update the pin and justify in your phase report.`).toBeGreaterThanOrEqual(min);
    ...
    it("custody-events code is still duplicated between API and worker (CR1 will extract to shared package)", () => {
        ...
        const workerCopy = readFileSync(fileURLToPath(new URL("../../worker/src/custody-events.ts", import.meta.url)), "utf8");
...
describe("Phase CR0.5 — no workflow/persona authorization gate introduced", () => {
    it("no page returns null based on workflow / persona checks (extends Phase 38.7 lock)", () => {
        ...
        expect(offenders, `Workflow / persona must NEVER be used as a route gate. Offenders:\n${offenders.join("\n")}`).toEqual([]);
    });
});
// =============================================================================
...
describe("Phase CR0.5 — no new duplicate navigation source", () => {
    ...
    it("no new file exports a function whose name suggests an alternative navigation builder", () => {
        ...
        expect(offenders, `Duplicate navigation builders are forbidden — extend ROUTE_REGISTRY + resolvers instead.\nOffenders:\n${offenders.join("\n")}`).toEqual([]);
    });
});
