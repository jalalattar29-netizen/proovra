...
describe("Phase 27.5/28.5 — external-review.routes.ts source contract", () => {
    const src = readSource("../../../services/api/src/routes/external-review.routes.ts");
    it("declares all 6 brief-mandated routes", () => {
        ...
        expect(src).toMatch(/app\.post\(\s*"\/v1\/external-review\/grants"/);
        expect(src).toMatch(/app\.get\(\s*"\/v1\/external-review\/grants"/);
    });
    it("imports the canonical service helpers from external-review-grant.service.js", () => {
        expect(src).toMatch(/import\s*\{[\s\S]*?issueExternalReviewGrant[\s\S]*?listExternalReviewGrants[\s\S]*?lookupExternalReviewGrantByToken[\s\S]*?recordExternalReviewAccess[\s\S]*?transitionExternalReviewGrant[\s\S]*?\}\s*from\s+"\.\.\/services\/external-review\/external-review-grant\.service\.js"/);
    });
    it("operator routes use authorizeOrFail with antiEnumeration: true", () => {
        expect(src).toMatch(/authorizeOrFail\(req, reply,[\s\S]*?antiEnumeration:\s*true/);
        // Every authorizeOrFail call MUST include antiEnumeration: true to
        // prevent cross-team probing.
        const authorizeCalls = src.match(/authorizeOrFail\([\s\S]*?\}\s*\)/g) ?? [];
        expect(authorizeCalls.length).toBeGreaterThan(0);
        for (const call of authorizeCalls) {
            expect(call, `authorize call without antiEnumeration: ${call}`).toMatch(/antiEnumeration:\s*true/);
        }
    });
    it("write operations require governance.legal_hold.manage", () => {
        expect(src).toMatch(/authorizeOrFail\([\s\S]*?permission:\s*"governance\.legal_hold\.manage"/);
    });
    it("read operations require audit.read (least-privilege)", () => {
        expect(src).toMatch(/authorizeOrFail\([\s\S]*?permission:\s*"audit\.read"/);
    });
    it("reviewer-side routes (token-based) are NOT behind requireAuth", () => {
        ...
        const acceptRoute = src.slice(src.indexOf('"/v1/external-review/access/:token"'), src.indexOf('"/v1/external-review/access/:token"') + 500);
        const contextRoute = src.slice(src.indexOf('"/v1/external-review/access/:token/context"'), src.indexOf('"/v1/external-review/access/:token/context"') + 500);
    ...
    it("token parameter is validated as 32+ char hex (anti-fuzz)", () => {
        expect(src).toMatch(/TokenParamSchema\s*=\s*z\.object\(\{\s*token:\s*z\.string\(\)\.min\(32\)\.max\(128\)\.regex\(\/\^\[a-f0-9\]\+\$\/i\)/);
    });
    it("anti-enumeration: token-failure responses all collapse to 401 grant_not_active", () => {
        ...
        const code401Calls = slice.match(/reply\.code\(401\)\.send\(\{[\s\S]*?\}\)/g) ?? [];
        expect(code401Calls.length).toBeGreaterThanOrEqual(2);
        for (const call of code401Calls) {
            expect(call).toMatch(/code:\s*"grant_not_active"/);
    ...
    it("reviewer projection never includes raw grant id / token_hash / team_id / invited_by", () => {
        ...
        for (const forbidden of [
            ...
            expect(projFn, `${forbidden} leaked into reviewer projection`).not.toContain(forbidden);
        }
    });
    it("reviewer projection only exposes the bounded narrow shape", () => {
        ...
        for (const allowed of [
            ...
            expect(projFn, `${allowed} missing from reviewer projection`).toContain(allowed);
        }
    });
    it("the reviewer accept route returns the projection, NEVER the raw grant", () => {
        const acceptRouteIdx = src.indexOf('"/v1/external-review/access/:token"');
        ...
        expect(acceptRoute).toMatch(/reply\.code\(200\)\.send\(\{\s*context:\s*projectGrantForReviewer\(grant\)/);
        expect(acceptRoute).not.toMatch(/reply\.code\(200\)\.send\(\{\s*grant:\s*grant\b/);
    });
    it("the context route returns the projection, NEVER the raw grant", () => {
        const contextRouteIdx = src.indexOf('"/v1/external-review/access/:token/context"');
        const contextRoute = src.slice(contextRouteIdx, contextRouteIdx + 1500);
        expect(contextRoute).toMatch(/reply\.code\(200\)\.send\(\{\s*context:\s*projectGrantForReviewer\(lookup\.grant\)/);
    });
    ...
    it("issue route returns rawToken EXACTLY ONCE in the 201 response", () => {
        ...
        // again).
        expect(issueRoute).toMatch(/reply\.code\(201\)\.send\(\{\s*grant:\s*result\.grant,\s*rawToken:\s*result\.rawToken/);
    });
    it("revoke route maps service denial codes to the right HTTP status (404 / 409 / 400)", () => {
        ...
        expect(revokeRoute).toMatch(/result\.reason === "invalid_transition"[\s\S]*?409/);
    });
});
// =============================================================================
// Server registration
...
describe("Phase 27.5/28.5 — server registration", () => {
    ...
    it("imports the externalReviewRoutes module", () => {
        expect(src).toMatch(/import\s*\{\s*externalReviewRoutes\s*\}\s*from\s+"\.\/routes\/external-review\.routes\.js"/);
    });
...
describe("Phase 27.5/28.5 — privacy + governance invariants", () => {
    ...
    it("scopeKind body validator is bounded to the canonical 3-state catalog", () => {
        expect(src).toMatch(/scopeKind:\s*z\.enum\(EXTERNAL_REVIEW_SCOPE_KINDS\)/);
    });
...
describe("Phase 27.5/28.5 — bounded denial vocabulary", () => {
    ...
    it("operator denial code 'grant_issue_denied' uses the bounded service reason payload", () => {
        expect(src).toMatch(/code:\s*"grant_issue_denied",\s*reason:\s*result\.reason/);
    });
    it("operator denial code 'revoke_denied' uses the bounded service reason payload", () => {
        expect(src).toMatch(/code:\s*"revoke_denied",\s*reason:\s*result\.reason/);
    });
