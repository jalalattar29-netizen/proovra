...
import { redactMetadata, safeJsonSnapshot, } from "../src/services/observability/redact.js";
...
import { buildObservabilityHealth, getObservabilityProvider, isObservabilityFeatureEnabled, setObservabilityProviderForTests, } from "../src/services/observability/registry.js";
import { IncidentError } from "../src/services/observability/incident.service.js";
import { __resetAlertThrottleForTests, dispatchOpsAlert, setAlertProviderForTests, } from "../src/services/alerts/alert.service.js";
...
describe("Phase 21 — metadata redactor", () => {
    ...
    it("bounds long strings", () => {
        const out = redactMetadata({ value: "X".repeat(5000) });
        const s = out["value"];
    ...
    it("bounds large arrays", () => {
        ...
        const out = redactMetadata({ values: arr });
        const v = out["values"];
...
describe("Phase 21 — NoopObservabilityProvider", () => {
    it("never throws on any method", () => {
        ...
        expect(() => p.captureException({ err: new Error("x"), severity: "HIGH" })).not.toThrow();
...
describe("Phase 21 — auto-incident wiring", () => {
    ...
    it("communications webhook bumps webhook_invalid_signature_total on bad signature", async () => {
        const { readFile } = await import("node:fs/promises");
        const { fileURLToPath } = await import("node:url");
        const src = await readFile(fileURLToPath(new URL("../src/routes/communications.routes.ts", import.meta.url)), "utf8");
...
describe("Phase 21 — incident routes auth posture", () => {
    it("incident routes use requireAuth + 404-on-non-member", async () => {
        ...
        expect(src).toMatch(/app\.get\(\s*"\/v1\/ops\/incidents"[\s\S]{0,200}preHandler:\s*requireAuth/);
        expect(src).toMatch(/app\.post\(\s*"\/v1\/ops\/incidents\/:id\/ack"[\s\S]{0,200}preHandler:\s*requireAuth/);
        expect(src).toMatch(/app\.post\(\s*"\/v1\/ops\/incidents\/:id\/resolve"[\s\S]{0,200}preHandler:\s*requireAuth/);
        expect(src).toMatch(/app\.post\(\s*"\/v1\/ops\/incidents\/:id\/suppress"[\s\S]{0,200}preHandler:\s*requireAuth/);
...
describe("Phase 21 migration — additive only", () => {
    it("uses IF NOT EXISTS / EXCEPTION blocks; never drops or renames", async () => {
        const { readFile } = await import("node:fs/promises");
        const { fileURLToPath } = await import("node:url");
        const src = await readFile(fileURLToPath(new URL("../prisma/migrations/20260529100000_add_operational_incidents_phase21/migration.sql", import.meta.url)), "utf8");
