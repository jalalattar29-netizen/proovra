...
const PLATFORM_CONTEXT_DIR = resolve(WEB_ROOT, "lib/platform-context");
/**
 ...
 */
const TENANCY_TELEMETRY_ALLOWLIST = new Set([
...
describe("Phase G4.3 — Frontend tenancy reads are centralized", () => {
    ...
    it("envelope.workspace.* is read ONLY from inside lib/platform-context/", () => {
        ...
        const offenders = [];
        for (const f of files) {
            if (f.startsWith(PLATFORM_CONTEXT_DIR))
                continue; // canonical reader
            if (TENANCY_TELEMETRY_ALLOWLIST.has(f))
                continue;
            const src = readFileSync(f, "utf8");
            const lines = src.split("\n");
            for (let i = 0; i < lines.length; i++) {
        ...
        if (offenders.length > 0) {
            ...
            // to migrate.
            throw new Error("Direct envelope.workspace reads found outside lib/platform-context:\n" +
                offenders
