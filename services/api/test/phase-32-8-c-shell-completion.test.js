...
describe("Phase 32.8C FINAL-4 — topbar workspace cluster", () => {
    ...
    it("never repeats 'Workspace' as a literal twice in the cluster fallback chain", () => {
        ...
        const cluster = TOPBAR.match(/<div className="app-topbar-v2-workspace-copy">[\s\S]*?<\/div>/);
...
describe("Phase 32.8C FINAL-4 — bulk actions toolbar", () => {
    ...
    it("toolbar is read-only: chips link to /ops/observability, no inline mutations", () => {
        const block = CC_TSX.match(/function BulkActionsToolbar\([\s\S]*?\n\}\s*\n/);
    ...
    it("footnote explicitly states the dashboard is read-only", () => {
        expect(CC_TSX).toMatch(/dashboard is read-only — chips above link to the canonical surface/);
    });
});
// =============================================================================
...
describe("Phase 32.8C FINAL-4 — no-regression invariants", () => {
    ...
    it("persona priority strip never renders raw UUIDs", () => {
        const strip = CC_TSX.match(/<nav\s+className="ec-persona-priority"[\s\S]*?<\/nav>/);
    ...
    it("bulk toolbar is gated by scope + persona — never bypasses auth", () => {
        expect(CC_TSX).toMatch(/canAct:\s*capabilities\.bulkActions/);
    });
