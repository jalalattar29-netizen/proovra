...
const SSO_SRC = readFileSync(apiPath("src/routes/sso-auth.routes.ts"), "utf8");
...
function endpointBody(path) {
    const anchor = AUTH_SRC.indexOf(`app.post("${path}"`);
    if (anchor < 0)
        return "";
    const tail = AUTH_SRC.slice(anchor + 1);
    const nextRoute = tail.search(/\bapp\.(post|get|delete|put|patch)\(\s*["'][^"']+["']/);
...
describe("R8.1.3 — durable MFA pending challenge model + enforcement", () => {
    ...
    it("test 1: MfaPendingChallenge model exists in schema with the required fields", () => {
        ...
        expect(migrations.some((m) => m.endsWith("_r8_1_3_mfa_pending_challenges"))).toBe(true);
    });
    // ---------------------------------------------------------------------------
    ...
    it("test 4: consume uses a single UPDATE keyed by consumedAt: null AND expiresAt > now", () => {
        ...
        const block = MFA_SVC.match(/prisma\.mfaPendingChallenge\.updateMany\(\{[\s\S]*?\}\s*\)/);
    ...
    it("test 11: mfa-policy.service.ts emits an MFA policy event on update", () => {
        const policySvc = readFileSync(apiPath("src/services/identity-security/mfa-policy.service.ts"), "utf8");
    ...
    it("test 16: resolver scopes membership lookups to userId only (no cross-tenant query)", () => {
        ...
        expect(ENFORCEMENT_SVC).toMatch(/prisma\.teamMember\.findMany\(\{\s*where:\s*\{\s*userId:\s*input\.userId/);
        // The org policy lookup is keyed by the explicit set of teamIds
        // the user belongs to. NEVER by `team: { somethingElse: ... }`.
        expect(ENFORCEMENT_SVC).toMatch(/prisma\.organizationSecurityPolicy\.findMany\(\{\s*where:\s*\{\s*teamId:\s*\{\s*in:\s*teamIds/);
    });
    // ---------------------------------------------------------------------------
    ...
    it("test 17: R8.1.3 does not import capture / upload / custody / report / package modules anywhere", () => {
        ...
        for (const src of surfaces) {
            for (const needle of forbiddenSubstrings) {
                expect(src.includes(`from "${needle}`) || src.includes(`from '${needle}`), `R8.1.3 surface must not import ${needle}`).toBe(false);
            }
        }
    });
});
