...
import { constantTimeEqualHex, filterValidScopes, hashIncomingApiKey, integrationsFeatureDisabledReason, isIntegrationsFeatureEnabled, isValidScope, issueApiKey, scopesGrantPermission, } from "../src/services/integrations/api-keys.service.js";
...
describe("api credentials — feature flag", () => {
    ...
    afterEach(() => {
        if (originalFlag === undefined)
            ...
            process.env.INTEGRATIONS_ENABLED = originalFlag;
...
describe("api credentials — issue + verify", () => {
    ...
    it("issueApiKey produces a valid pwk-shaped key + matching hash", () => {
        withEnv({ INTEGRATIONS_ENABLED: "true", API_KEY_SECRET: TEST_SECRET }, () => {
...
describe("api credentials — scope catalog", () => {
    ...
    it("scopesGrantPermission performs exact-match lookup", () => {
        ...
        expect(scopesGrantPermission(scopes, "integration.evidence.read")).toBe(true);
        expect(scopesGrantPermission(scopes, "integration.evidence.create")).toBe(false);
    });
});
