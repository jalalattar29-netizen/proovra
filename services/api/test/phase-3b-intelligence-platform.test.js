...
import { AUDIT_TRANSPARENCY_CATEGORIES, EXECUTIVE_METRICS_LIMITATIONS, EXECUTIVE_METRICS_SCHEMA_VERSION, INTELLIGENCE_CONFIDENCE_BANDS, INTELLIGENCE_PROVENANCE_LIMITATIONS, MEDIA_INTELLIGENCE_ACTIVITY_CODES, MEDIA_INTELLIGENCE_MODALITIES, MEDIA_INTELLIGENCE_PROVIDERS, MEDIA_INTELLIGENCE_RECORD_KINDS, MEDIA_INTELLIGENCE_RECORD_STATES, PROVIDER_ADAPTER_OPERATIONS, PROVIDER_ADAPTER_STATES, PROVIDER_BUDGET_DECISIONS, PROVIDER_BUDGET_PERIODS, PROVIDER_BUDGET_SCOPES, PROVIDER_COST_UNITS, REVIEWER_CORRECTION_KINDS, REVIEWER_CORRECTION_STATES, classifyIntelligenceConfidence, } from "@proovra/shared";
...
import { getAdapter, listAdapterProbes, listAdapters, } from "../src/services/intelligence/providers/provider-adapter.js";
function readSource(rel) {
    return readFileSync(fileURLToPath(new URL(rel, import.meta.url)), "utf8");
}
const SHARED = readSource("../../../packages/shared/src/media-intelligence-platform.ts");
const SHARED_INDEX = readSource("../../../packages/shared/src/index.ts");
const SCHEMA = readSource("../../../services/api/prisma/schema.prisma");
const MIGRATION = readSource("../../../services/api/prisma/migrations/20261215000000_phase_3b_intelligence_platform/migration.sql");
const ADAPTER_IFACE = readSource("../../../services/api/src/services/intelligence/providers/provider-adapter.ts");
const ADAPTER_AZURE = readSource("../../../services/api/src/services/intelligence/providers/azure-document-intelligence-adapter.ts");
const ADAPTER_DEEPGRAM = readSource("../../../services/api/src/services/intelligence/providers/deepgram-adapter.ts");
const ADAPTER_REK = readSource("../../../services/api/src/services/intelligence/providers/rekognition-adapter.ts");
const ADAPTER_OPENAI = readSource("../../../services/api/src/services/intelligence/providers/openai-adapter.ts");
const SVC_MEDIA = readSource("../../../services/api/src/services/intelligence/media-intelligence.service.ts");
const SVC_CORR = readSource("../../../services/api/src/services/intelligence/reviewer-correction.service.ts");
const SVC_USAGE = readSource("../../../services/api/src/services/intelligence/provider-usage.service.ts");
const SVC_BUDGET = readSource("../../../services/api/src/services/intelligence/provider-budget.service.ts");
const SVC_EXEC = readSource("../../../services/api/src/services/intelligence/executive-metrics.service.ts");
const SVC_AUDIT = readSource("../../../services/api/src/services/intelligence/audit-transparency.service.ts");
const SVC_MANIFEST = readSource("../../../services/api/src/services/intelligence/intelligence-verification-manifest.service.ts");
const ROUTES = readSource("../../../services/api/src/routes/intelligence-platform.routes.ts");
const SERVER = readSource("../../../services/api/src/server.ts");
const NAV = readSource("../../../services/api/src/services/platform-context/navigation-registry.ts");
const REPORT_SECTION = readSource("../../../services/worker/src/report-v2/sections/intelligence-summary.ts");
const UI_EXEC = readSource("../../../apps/web/app/(app)/executive/page.tsx");
const UI_AUDIT = readSource("../../../apps/web/app/(app)/audit-transparency/page.tsx");
const UI_PLATFORM = readSource("../../../apps/web/app/(app)/intelligence-platform/page.tsx");
// ===========================================================================
// 1. Shared contracts
// ===========================================================================
describe("Phase 3B — shared contracts", () => {
    ...
    it("correction kinds + states are bounded", () => {
        ...
        expect([...REVIEWER_CORRECTION_STATES].sort()).toEqual(["ACCEPTED", "DRAFT", "REVERTED"].sort());
    });
    it("adapter operation / state / cost / budget catalogs are bounded", () => {
        ...
        expect([...PROVIDER_COST_UNITS].sort()).toEqual(["CALL", "IMAGE", "MEGABYTE", "MINUTE", "PAGE", "TOKEN"].sort());
        expect([...PROVIDER_BUDGET_PERIODS].sort()).toEqual(["ANNUAL", "DAILY", "MONTHLY", "QUARTERLY", "WEEKLY"].sort());
        expect([...PROVIDER_BUDGET_SCOPES].sort()).toEqual(["CASE", "PROJECT", "PROVIDER", "TEAM", "WORKSPACE"].sort());
    ...
    it("executive metrics schema version pin matches V1 + bounded limitations", () => {
        expect(EXECUTIVE_METRICS_SCHEMA_VERSION).toBe("PROOVRA_EXECUTIVE_METRICS_V1");
        for (const l of [
...
describe("Phase 3B — Prisma + migration", () => {
    ...
    it("record table is uniquely keyed per (team, evidence, provider, providerRecordKey)", () => {
        expect(SCHEMA).toMatch(/@@unique\(\[teamId, evidenceId, provider, providerRecordKey\]\)/);
    });
    it("corrections cascade off records; entities cascade off records", () => {
        expect(SCHEMA).toMatch(/record\s+MediaIntelligenceRecord\s+@relation\([^)]+onDelete: Cascade\)/);
    });
    it("Phase O-Final compliant migration", () => {
        expect(MIGRATION).toMatch(/CREATE TABLE "media_intelligence_records"/);
        expect(MIGRATION).not.toMatch(/CREATE TABLE IF NOT EXISTS "media_intelligence_records"/);
        expect(MIGRATION).toMatch(/information_schema\.columns/);
        expect(MIGRATION).toMatch(/REFERENCES "media_intelligence_records"[\s\S]+CASCADE/);
    });
});
// ===========================================================================
...
describe("Phase 3B — runtime helpers", () => {
    ...
    it("adapter registry surfaces probes for every registered provider", () => {
        ...
        for (const expected of [
            ...
            expect(providers.has(expected)).toBe(true);
        }
    });
});
