...
describe("Phase 31.14 — derived-assets bytes proxy route", () => {
    ...
    it("route registered at /v1/evidence/:evidenceId/derived-assets/:assetId/bytes", () => {
        expect(src).toMatch(/app\.get\(\s*"\/v1\/evidence\/:evidenceId\/derived-assets\/:assetId\/bytes"/);
    });
    it("gated by authorizeOrFail + evidence.read + antiEnumeration", () => {
        const block = src.match(/"\/v1\/evidence\/:evidenceId\/derived-assets\/:assetId\/bytes"[\s\S]*?\n\s*\}\s*,\s*\n\s*\)/)?.[0];
    ...
    it("anti-enumeration on evidence: cross-team id returns 404 `not_found`", () => {
        ...
        expect(block).toMatch(/reply\.code\(404\)\.send\(\{\s*error:\s*\{\s*code:\s*"not_found"\s*\}/);
    });
    it("asset row team + evidence binding (anti-enumeration)", () => {
        const block = src.match(/"\/v1\/evidence\/:evidenceId\/derived-assets\/:assetId\/bytes"[\s\S]*?\n\s*\}\s*,\s*\n\s*\)/)?.[0];
        expect(block).toMatch(/WHERE "id" = \$1[\s\S]*?AND "team_id" = \$2[\s\S]*?AND "evidence_id" = \$3/);
    });
    ...
    it("GET list projection attaches bytesUrl for COMPLETED + null otherwise", () => {
        expect(src).toMatch(/bytesUrl:[\s\S]*?a\.status === "COMPLETED"[\s\S]*?:\s*null/);
    });
    it("bytesUrl is an INTERNAL proxy path (NOT a storage URL)", () => {
        expect(src).toMatch(/`\/v1\/evidence\/\$\{encodeURIComponent\(evidenceId\)\}\/derived-assets\/\$\{encodeURIComponent\(a\.id\)\}\/bytes/);
    });
});
// =============================================================================
// PART 2 — UI hook
...
describe("Phase 31.14 — useDerivedAssets hook", () => {
    const src = readSource("../../../apps/web/lib/media-intelligence/useDerivedAssets.ts");
...
describe("Phase 31.14 — MediaIntelligencePanel real-image rendering", () => {
    const src = readSource("../../../apps/web/components/media-intelligence/MediaIntelligencePanel.tsx");
    ...
    it("button is keyboard accessible — has aria-label + type='button'", () => {
        const thumb = src.match(/function DerivedAssetThumbnail[\s\S]*?return \([\s\S]*?\);\s*\n\}/)?.[0];
    ...
    it("preview modal: dialog role + aria-modal + Escape-to-close + body scroll suppression", () => {
        const modal = src.match(/function DerivedAssetPreviewModal[\s\S]*?return \([\s\S]*?\);\s*\n\}/)?.[0];
        ...
        const strip = src.match(/function DerivedAssetsStrip[\s\S]*?\n\}\s*\n/)?.[0];
...
describe("Phase 31.14 — report projection derivedThumbnails wiring", () => {
    const src = readSource("../../../packages/shared-runtime/src/media-intelligence/report-projection.service.ts");
    it("now returns null only when BOTH signals + thumbnails are empty", () => {
        expect(src).toMatch(/signals\.length === 0 && derivedThumbnails\.length === 0/);
    });
    it("loadDerivedThumbnailsForReport: bounded thumbnail count + byte cap", () => {
        expect(src).toMatch(/MAX_REPORT_THUMBNAILS\s*=\s*6/);
        expect(src).toMatch(/MAX_REPORT_THUMBNAIL_BYTES\s*=\s*256\s*\*\s*1024/);
    });
    it("SELECTs only COMPLETED image_thumbnail assets", () => {
        const fn = src.match(/async function loadDerivedThumbnailsForReport[\s\S]*?\n\}/)?.[0];
    ...
    it("anti-leak: no storage_bucket / storage_key referenced", () => {
        ...
        for (const banned of [
            ...
            expect(noComments, `report projection leaks ${banned}`).not.toContain(banned);
        }
    });
});
// =============================================================================
...
describe("Phase 31.14 — verification package intelligence bridge", () => {
    const src = readSource("../../../services/worker/src/verification-package-intelligence-bridge.ts");
    ...
    it("never throws — returns null on any error", () => {
        expect(src).toMatch(/try\s*\{[\s\S]*?\}\s*catch\s*\(err\)[\s\S]*?return\s+null/);
    });
    ...
    it("returns null when there are no signals AND no derived assets", () => {
        expect(src).toMatch(/mediaSignals\.length === 0 && derivedAssets\.length === 0[\s\S]*?return\s+null/);
    });
...
describe("Phase 31.14 — worker processor wires verification-package intelligence", () => {
    ...
    it("imports the bridge", () => {
        expect(src).toMatch(/import\s*\{\s*buildVerificationPackageIntelligence\s*\}\s*from\s*"\.\/verification-package-intelligence-bridge\.js"/);
    });
    it("call site populates intelligence on createVerificationPackage", () => {
        expect(src).toMatch(/buildVerificationPackageIntelligence\(\{/);
        expect(src).toMatch(/createVerificationPackage\(\{[\s\S]*?intelligence:\s*verificationPackageIntelligence/);
    });
});
// =============================================================================
...
describe("Phase 31.14 — cross-source anti-leak", () => {
    ...
    it("no forbidden truth-claim vocabulary in any source literal", () => {
        ...
        for (let i = 0; i < sources.length; i++) {
            ...
            const literals = noComments.match(/"[^"\n]+"/g) ?? [];
            for (const lit of literals) {
                expect(lit, `src ${i} forbidden wording: ${lit}`).not.toMatch(forbidden);
            }
        }
    });
});
