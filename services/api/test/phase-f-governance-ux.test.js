...
const ROUTE_SRC = readSource("../src/routes/governance-lifecycle.routes.ts");
const PREVIEW_UI = readSource("../../../apps/web/components/governance/DestructionImpactPreview.tsx");
const CERT_UI = readSource("../../../apps/web/components/governance/DestructionCertificate.tsx");
const INHERIT_UI = readSource("../../../apps/web/components/governance/RetentionInheritanceSummary.tsx");
const DESTRUCTION_PAGE = readSource("../../../apps/web/app/(app)/governance/destruction/page.tsx");
...
describe("Phase F — backend governance endpoints", () => {
    it("registers GET /v1/governance/destruction-reviews/:id/preview", () => {
        expect(ROUTE_SRC).toMatch(/app\.get\(\s*"\/v1\/governance\/destruction-reviews\/:id\/preview"/);
    });
    it("registers GET /v1/governance/destruction-reviews/:id/certificate", () => {
        expect(ROUTE_SRC).toMatch(/app\.get\(\s*"\/v1\/governance\/destruction-reviews\/:id\/certificate"/);
    });
    it("registers GET /v1/governance/retention/inheritance", () => {
        expect(ROUTE_SRC).toMatch(/app\.get\(\s*"\/v1\/governance\/retention\/inheritance"/);
    });
    it("imports resolveTeamRetentionPolicy from the B0 service", () => {
        expect(ROUTE_SRC).toMatch(/import\s*\{\s*resolveTeamRetentionPolicy\s*\}\s*from\s+"\.\.\/services\/organization\/retention-inheritance\.service\.js"/);
    });
});
// ===========================================================================
...
describe("Phase F — governance read endpoints are gated + audit-free", () => {
    ...
    it("each new handler invokes requireMember + governance.policy.read", () => {
        for (const tag of newHandlers) {
            const block = ROUTE_SRC.match(new RegExp(`${tag.replace(/[\/:]/g, "\\$&")}[\\s\\S]*?\\}\\s*,\\s*\\)\\s*;`));
    ...
    it("each new handler emits no audit (reads are not auditable)", () => {
        for (const tag of newHandlers) {
            ...
            expect(block[0]).not.toMatch(/appendCustodyEvent|appendPlatformAuditLog|writeAnalyticsEvent|appendReviewerAuditEvent/);
        }
    });
});
// ===========================================================================
...
describe("Phase F — destruction impact preview payload", () => {
    ...
    it("computes blockers from active holds + immutable policy + destroyed state", () => {
        expect(ROUTE_SRC).toMatch(/blockedBy\.push\("evidence_legal_hold"\)/);
        expect(ROUTE_SRC).toMatch(/blockedBy\.push\("case_legal_hold"\)/);
        expect(ROUTE_SRC).toMatch(/blockedBy\.push\("retention_policy_immutable"\)/);
...
describe("Phase F — destruction certificate payload", () => {
    it("only serves certificates for EXECUTED reviews (409 otherwise)", () => {
        expect(ROUTE_SRC).toMatch(/review\.status\s*!==\s*"EXECUTED"[\s\S]*?reply\.code\(409\)\.send\(\{\s*error:\s*\{\s*code:\s*"certificate_unavailable"/);
    });
    it("emits the canonical certificate fields + caveats", () => {
        const handler = ROUTE_SRC.match(/certificate[\s\S]*?reply\.code\(200\)\.send\(\{[\s\S]*?caveats:/);
...
describe("Phase F — retention inheritance endpoint", () => {
    it("delegates entirely to the existing resolveTeamRetentionPolicy", () => {
        const handler = ROUTE_SRC.match(/retention\/inheritance[\s\S]*?\}\s*,\s*\)\s*;/);
        expect(handler).toBeTruthy();
        expect(handler[0]).toMatch(/resolveTeamRetentionPolicy\(query\.teamId\)/);
    });
    it("returns the resolver verdict verbatim under `resolution`", () => {
        expect(ROUTE_SRC).toMatch(/retention\/inheritance[\s\S]*?reply\.code\(200\)\.send\(\{\s*resolution\s*\}\)/);
    });
});
// ===========================================================================
...
describe("Phase F — DestructionImpactPreview component", () => {
    it("consumes the new preview endpoint", () => {
        ...
        expect(PREVIEW_UI).toMatch(/encodeURIComponent\(\s*\n?\s*reviewId,?\s*\n?\s*\)/);
    ...
    it("explicitly disclaims legal admissibility", () => {
        ...
        expect(PREVIEW_UI.replace(/\s+/g, " ")).toMatch(/not a legal admissibility statement/);
    });
...
describe("Phase F — DestructionCertificate component", () => {
    it("consumes the new certificate endpoint", () => {
        ...
        expect(CERT_UI).toMatch(/encodeURIComponent\(\s*\n?\s*reviewId,?\s*\n?\s*\)/);
...
describe("Phase F — RetentionInheritanceSummary component", () => {
    it("consumes the new inheritance endpoint", () => {
        expect(INHERIT_UI).toMatch(/\/v1\/governance\/retention\/inheritance\?teamId=/);
    });
...
describe("Phase F — governance page integration", () => {
    ...
    it("destruction page gates certificate button on r.status === EXECUTED", () => {
        expect(DESTRUCTION_PAGE).toMatch(/r\.status\s*===\s*"EXECUTED"[\s\S]{0,400}view-certificate/);
    });
    it("retention page mounts the inheritance summary", () => {
        expect(RETENTION_PAGE).toContain("RetentionInheritanceSummary");
        expect(RETENTION_PAGE).toMatch(/<RetentionInheritanceSummary\s+teamId=\{teamId\s*\?\?\s*null\}/);
    });
});
// ===========================================================================
