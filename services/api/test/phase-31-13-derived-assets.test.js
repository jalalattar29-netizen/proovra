...
describe("Phase 31.13 — evidence_part_derived_assets SQL drift patch", () => {
    const sql = readSource("../../../services/api/sql/drift-patches/2026-05-20-evidence-part-derived-assets.sql");
    ...
    it("idempotent CREATE TABLE", () => {
        expect(sql).toMatch(/CREATE TABLE IF NOT EXISTS\s+"evidence_part_derived_assets"/);
    });
    it("asset_kind CHECK bounded to known catalog", () => {
        const block = sql.match(/CONSTRAINT "evidence_part_derived_assets_kind_bounded"[\s\S]*?\)\)/)?.[0];
    ...
    it("status CHECK bounded to lifecycle catalog including UNSUPPORTED", () => {
        const block = sql.match(/CONSTRAINT "evidence_part_derived_assets_status_bounded"[\s\S]*?\)\)/)?.[0];
        expect(block).toBeTruthy();
        for (const s of [
            "'PENDING'",
            "'PROCESSING'",
    ...
    it("size cap (50MB) + dimensions cap (200000 px) to prevent runaway storage", () => {
        ...
        expect(sql).toMatch(/"width_px" > 0 AND "width_px" <= 200000/);
        expect(sql).toMatch(/"height_px" > 0 AND "height_px" <= 200000/);
    });
    it("per-team unique on (evidence_part_id, asset_kind)", () => {
        expect(sql).toMatch(/CREATE UNIQUE INDEX IF NOT EXISTS "evidence_part_derived_assets_team_part_kind_uk"[\s\S]*?\("team_id", "evidence_part_id", "asset_kind"\)/);
    });
    it("schema-validation registers the new table + key column + index", () => {
        ...
        expect(src).toContain('indexName: "evidence_part_derived_assets_team_part_kind_uk"');
    });
});
// =============================================================================
...
describe("Phase 31.13 — derived assets persistence service", () => {
    const src = readSource("../../../packages/shared-runtime/src/media-intelligence/derived-assets.service.ts");
    it("upsert keyed by (team_id, evidence_part_id, asset_kind) — idempotent", () => {
        expect(src).toMatch(/ON CONFLICT \("team_id", "evidence_part_id", "asset_kind"\) DO UPDATE/);
    });
    it("read projection NEVER includes storage internals", () => {
        const projectRowFn = src.match(/function projectRow\([\s\S]*?\n\}/)?.[0];
        ...
        for (const banned of [
            ...
            expect(projectRowFn, `projectRow leaks ${banned}`).not.toContain(banned);
        }
    });
    it("read SQL SELECT clause does NOT request storage_bucket / storage_key", () => {
        const listFn = src.match(/export async function listDerivedAssetsForEvidence[\s\S]*?\n\}/)?.[0];
    ...
    it("never throws — every code path returns a bounded result", () => {
        for (const fn of [
            ...
            const block = src.match(new RegExp(`export async function ${fn}\\([\\s\\S]*?\\n\\}`))?.[0];
...
describe("Phase 31.13 — API derived-assets queue helper", () => {
    ...
    it("deterministic job id da-<kind>-<evidencePartId>", () => {
        expect(src).toMatch(/return `da-\$\{assetKind\}-\$\{evidencePartId\}`/);
    });
    ...
    it("Redis outage returns { enqueued: false, reason } — never throws", () => {
        expect(src).toMatch(/catch \(err\)\s*\{[\s\S]*?return\s*\{\s*enqueued:\s*false,\s*reason:/);
    });
...
describe("Phase 31.13 — worker derived-assets processor", () => {
    const src = readSource("../../../services/worker/src/derived-assets.processor.ts");
    ...
    it("derived bytes stored under separate `derived-assets/` S3 prefix", () => {
        expect(src).toMatch(/DERIVED_ASSET_S3_PREFIX\s*=\s*"derived-assets"/);
        expect(src).toMatch(/\$\{DERIVED_ASSET_S3_PREFIX\}\/\$\{evidenceId\}\/\$\{evidencePartId\}/);
    });
    ...
    it("never throws on structural problems (missing part / non-image / no storage ref)", () => {
        ...
        expect(src).toMatch(/return\s*\{\s*ok:\s*true,\s*status:\s*"UNSUPPORTED"\s*\}/);
    });
    ...
    it("originals are never mutated — only reads source via getObjectRange, writes to derived prefix", () => {
        ...
        expect(noComments).not.toMatch(/putObjectBuffer\(\{[^}]*?key:\s*part\.storage_key/);
    });
    ...
    it("DLQ counter bumped only on final attempt exhaustion", () => {
        expect(src).toMatch(/\(job\.attemptsMade \?\? 0\) >= attemptsAllowed - 1[\s\S]*?tryBump\("derived_assets_dlq_total"\)/);
    });
});
// =============================================================================
...
describe("Phase 31.13 — capability detection", () => {
    const src = readSource("../../../services/worker/src/derived-assets-capability.ts");
    ...
    it("never throws — every code path returns a bounded result", () => {
        const fn = src.match(/export async function detectDerivedAssetCapability[\s\S]*?\n\}/)?.[0];
        expect(fn).toBeTruthy();
...
describe("Phase 31.13 — derived-assets API routes", () => {
    ...
    it("GET route registered + gated by authorizeOrFail + antiEnumeration", () => {
        const block = src.match(/"\/v1\/evidence\/:evidenceId\/derived-assets"[\s\S]*?\n\s*\}\s*,\s*\n\s*\)/)?.[0];
        expect(block).toBeTruthy();
    ...
    it("POST run route requires evidence.update_metadata (not just read)", () => {
        const block = src.match(/"\/v1\/evidence\/:evidenceId\/derived-assets\/run"[\s\S]*?\n\s*\}\s*,\s*\n\s*\)/)?.[0];
        expect(block).toBeTruthy();
...
describe("Phase 31.13 — UI hook + panel extension", () => {
    const hookSrc = readSource("../../../apps/web/lib/media-intelligence/useDerivedAssets.ts");
    const panelSrc = readSource("../../../apps/web/components/media-intelligence/MediaIntelligencePanel.tsx");
    ...
    it("polling interval clamped [2000, 60000]", () => {
        expect(hookSrc).toMatch(/Math\.max\(2_?000,\s*Math\.min\(60_?000/);
    });
    ...
    it("panel renders the DerivedAssetsStrip only when assets exist", () => {
        ...
        expect(panelSrc).toMatch(/derived\.state\.assets\.length > 0\s*\?\s*\(?\s*<DerivedAssetsStrip/);
    });
    it("panel categorizes assets by status (completed / failed / unsupported / pending)", () => {
        const strip = panelSrc.match(/function DerivedAssetsStrip[\s\S]*?\n\}\s*\n/)?.[0];
    ...
    it("no forbidden vocabulary in panel literals", () => {
        ...
        for (const lit of literals) {
            expect(lit, `panel uses forbidden wording: ${lit}`).not.toMatch(forbidden);
        }
    });
...
describe("Phase 31.13 — derived-assets worker bootstrap", () => {
    const indexSrc = readSource("../../../services/worker/src/index.ts");
    it("derived-assets worker registered via safeRegisterWorker", () => {
        expect(indexSrc).toMatch(/safeRegisterWorker\(\s*"derived-assets"\s*,\s*\(\)\s*=>/);
    });
    ...
    it("shutdown null-checks derivedAssetsWorker before close", () => {
        expect(indexSrc).toMatch(/if\s*\(derivedAssetsWorker\)\s*\{[\s\S]*?await derivedAssetsWorker\.close/);
    });
...
describe("Phase 31.13 — anti-leak across new surfaces", () => {
    ...
    it("no forbidden truth-claim vocabulary in any source literal", () => {
        ...
        for (let i = 0; i < sources.length; i++) {
            ...
            const literals = noComments.match(/"[^"\n]+"/g) ?? [];
            for (const lit of literals) {
                expect(lit, `source ${i} forbidden wording: ${lit}`).not.toMatch(forbidden);
            }
        }
    });
