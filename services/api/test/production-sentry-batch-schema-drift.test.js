...
// =============================================================================
const REPAIR_MIGRATION = readRepo("services/api/prisma/migrations/20270802000000_phase_sentry_batch_schema_drift_repair/migration.sql");
const OPS_ROUTES = readRepo("services/api/src/routes/ops.routes.ts");
const REVIEWER_OPS_ROUTES = readRepo("services/api/src/routes/reviewer-ops.routes.ts");
const REVIEWER_CONSOLE_ROUTES = readRepo("services/api/src/routes/reviewer-console.routes.ts");
const ORGANIZATIONS_ROUTES = readRepo("services/api/src/routes/organizations.routes.ts");
const REVIEWER_WORKSPACE_ROUTES = readRepo("services/api/src/routes/reviewer-workspace.routes.ts");
const TRUST_GOV_ROUTES = readRepo("services/api/src/routes/trust-and-governance.routes.ts");
const READINESS_ROUTES = readRepo("services/api/src/routes/runtime-readiness.routes.ts");
const READINESS_AGGREGATOR = readRepo("services/api/src/runtime/runtime-readiness.ts");
const CODING_SCHEMA_SERVICE = readRepo("services/api/src/services/reviewer-workspace/coding-schema.service.ts");
...
describe("Phase O — repair migration covers every Sentry schema-drift item", () => {
    ...
    it("adds evidence_exchange_packages.updated_at (NODE-1R)", () => {
        expect(REPAIR_MIGRATION).toMatch(/ALTER\s+TABLE\s+IF\s+EXISTS\s+"evidence_exchange_packages"[\s\S]{0,300}ADD\s+COLUMN\s+IF\s+NOT\s+EXISTS\s+"updated_at"\s+TIMESTAMPTZ\(6\)\s+NOT\s+NULL\s+DEFAULT\s+NOW\(\);/);
    });
    // (3) NODE-1H — provider_budgets.archived_at
    it("adds provider_budgets.archived_at (NODE-1H)", () => {
        expect(REPAIR_MIGRATION).toMatch(/ALTER\s+TABLE\s+IF\s+EXISTS\s+"provider_budgets"[\s\S]{0,300}ADD\s+COLUMN\s+IF\s+NOT\s+EXISTS\s+"archived_at"\s+TIMESTAMPTZ\(6\);/);
    });
    // (4) NODE-1Q — chain_transfers.updated_at
    it("adds chain_transfers.updated_at (NODE-1Q)", () => {
        expect(REPAIR_MIGRATION).toMatch(/ALTER\s+TABLE\s+IF\s+EXISTS\s+"chain_transfers"[\s\S]{0,300}ADD\s+COLUMN\s+IF\s+NOT\s+EXISTS\s+"updated_at"\s+TIMESTAMPTZ\(6\)\s+NOT\s+NULL\s+DEFAULT\s+NOW\(\);/);
    });
    ...
    it("adds redaction_policy_assignments.version_id + DO-block backfill (NODE-1N)", () => {
        expect(REPAIR_MIGRATION).toMatch(/ALTER\s+TABLE\s+IF\s+EXISTS\s+"redaction_policy_assignments"[\s\S]{0,300}ADD\s+COLUMN\s+IF\s+NOT\s+EXISTS\s+"version_id"\s+UUID;/);
        // DO-block guarded backfill from legacy `policy_version_id`.
        expect(REPAIR_MIGRATION).toMatch(/DO\s+\$\$[\s\S]*?information_schema\.columns[\s\S]*?policy_version_id[\s\S]*?version_id[\s\S]*?UPDATE\s+"redaction_policy_assignments"[\s\S]*?SET\s+"version_id"\s*=\s*"policy_version_id"[\s\S]*?WHERE\s+"version_id"\s+IS\s+NULL\s+AND\s+"policy_version_id"\s+IS\s+NOT\s+NULL/);
    });
    // (6) NODE-1M — redaction_projects.closed_at_utc
    it("adds redaction_projects.closed_at_utc (NODE-1M)", () => {
        expect(REPAIR_MIGRATION).toMatch(/ALTER\s+TABLE\s+IF\s+EXISTS\s+"redaction_projects"[\s\S]{0,300}ADD\s+COLUMN\s+IF\s+NOT\s+EXISTS\s+"closed_at_utc"\s+TIMESTAMPTZ\(6\);/);
    });
    ...
    it("adds delegated_admin_grants.{granted_to_user_id,scope_target_id,created_at,updated_at} + backfill (NODE-1K)", () => {
        const block = REPAIR_MIGRATION.match(/ALTER\s+TABLE\s+IF\s+EXISTS\s+"delegated_admin_grants"[\s\S]{0,800}?;/);
        ...
        expect(block[0]).toMatch(/ADD\s+COLUMN\s+IF\s+NOT\s+EXISTS\s+"created_at"\s+TIMESTAMPTZ\(6\)\s+NOT\s+NULL\s+DEFAULT\s+NOW\(\)/);
        expect(block[0]).toMatch(/ADD\s+COLUMN\s+IF\s+NOT\s+EXISTS\s+"updated_at"\s+TIMESTAMPTZ\(6\)\s+NOT\s+NULL\s+DEFAULT\s+NOW\(\)/);
        // Backfill: legacy `grantee_user_id` → `granted_to_user_id`.
        expect(REPAIR_MIGRATION).toMatch(/DO\s+\$\$[\s\S]*?grantee_user_id[\s\S]*?granted_to_user_id[\s\S]*?UPDATE\s+"delegated_admin_grants"[\s\S]*?SET\s+"granted_to_user_id"\s*=\s*"grantee_user_id"[\s\S]*?WHERE\s+"granted_to_user_id"\s+IS\s+NULL\s+AND\s+"grantee_user_id"\s+IS\s+NOT\s+NULL/);
    });
    // (8) NODE-1E — status_components.updated_at
    it("adds status_components.updated_at (NODE-1E)", () => {
        expect(REPAIR_MIGRATION).toMatch(/ALTER\s+TABLE\s+IF\s+EXISTS\s+"status_components"[\s\S]{0,300}ADD\s+COLUMN\s+IF\s+NOT\s+EXISTS\s+"updated_at"\s+TIMESTAMPTZ\(6\)\s+NOT\s+NULL\s+DEFAULT\s+NOW\(\);/);
    });
    // (9) NODE-1J — subprocessors.category / country / description
    it("adds subprocessors.{category,country,description} (NODE-1J)", () => {
        const block = REPAIR_MIGRATION.match(/ALTER\s+TABLE\s+IF\s+EXISTS\s+"subprocessors"[\s\S]{0,600}?;/);
        expect(block, "subprocessors ALTER block must exist").toBeTruthy();
        expect(block[0]).toMatch(/ADD\s+COLUMN\s+IF\s+NOT\s+EXISTS\s+"category"\s+VARCHAR\(80\)\s+NOT\s+NULL\s+DEFAULT\s+'PROVIDER'/);
    ...
    it("defensively re-asserts R7 reviewer-workspace columns on coding_schemas + coding_fields (NODE-1P)", () => {
        expect(REPAIR_MIGRATION).toMatch(/ALTER\s+TABLE\s+IF\s+EXISTS\s+"coding_schemas"[\s\S]{0,800}ADD\s+COLUMN\s+IF\s+NOT\s+EXISTS\s+"updated_at"\s+TIMESTAMPTZ\(6\)\s+NOT\s+NULL\s+DEFAULT\s+NOW\(\)/);
        expect(REPAIR_MIGRATION).toMatch(/ALTER\s+TABLE\s+IF\s+EXISTS\s+"coding_fields"[\s\S]{0,400}ADD\s+COLUMN\s+IF\s+NOT\s+EXISTS\s+"options"\s+JSONB/);
        expect(REPAIR_MIGRATION).toMatch(/ALTER\s+TABLE\s+IF\s+EXISTS\s+"coding_fields"[\s\S]{0,400}ADD\s+COLUMN\s+IF\s+NOT\s+EXISTS\s+"order_index"\s+INTEGER\s+NOT\s+NULL\s+DEFAULT\s+0/);
    });
});
// =============================================================================
...
describe("Phase O Stream A — /v1/ops/metrics (NODE-W)", () => {
    it("resolves teamId from currentWorkspaceId BEFORE running Zod", () => {
        expect(OPS_ROUTES).toMatch(/\/v1\/ops\/metrics[\s\S]{0,2400}currentWorkspaceId: true/);
    });
    it("emits bounded 400 WORKSPACE_CONTEXT_REQUIRED with requestId when no workspace", () => {
        expect(OPS_ROUTES).toMatch(/code:\s*"WORKSPACE_CONTEXT_REQUIRED"[\s\S]{0,400}Select a workspace to view operational metrics[\s\S]{0,400}requestId:\s*req\.id/);
    });
    it("uses safeParse for query (no unhandled .parse(req.query) leak inside the handler)", () => {
        ...
        const slice = OPS_ROUTES.slice(idx, idx + 1 + (nextRoute > 0 ? nextRoute : 3000));
...
describe("Phase O Stream A — /v1/reviewer-ops/queue (NODE-1G)", () => {
    it("clamps limit to ≤100 via z.coerce.number().max(100)", () => {
        expect(REVIEWER_OPS_ROUTES).toMatch(/limit:\s*z\.coerce\.number\(\)\.int\(\)\.min\(1\)\.max\(100\)\.optional\(\)\.default\(50\)/);
    });
    it("uses safeParse and emits bounded 400 INVALID_QUERY on failure", () => {
        expect(REVIEWER_OPS_ROUTES).toMatch(/QueueQuery\.safeParse\(\s*\{[\s\S]{0,200}teamId:\s*resolvedTeamId/);
        expect(REVIEWER_OPS_ROUTES).toMatch(/code:\s*"INVALID_QUERY"[\s\S]{0,400}Query parameters invalid[\s\S]{0,400}requestId:\s*req\.id/);
    });
    it("resolves teamId from currentWorkspaceId before parsing", () => {
        expect(REVIEWER_OPS_ROUTES).toMatch(/\/v1\/reviewer-ops\/queue[\s\S]{0,2400}currentWorkspaceId: true/);
    });
});
// =============================================================================
...
describe("Phase O Stream A — /v1/reviewer-ops/console (NODE-11)", () => {
    it("signature accepts optional prismaClient (defaults via local ?? prisma)", () => {
        ...
        expect(REVIEWER_CONSOLE_ROUTES).toMatch(/const\s+client:\s*PrismaClient\s*=\s*prismaClient\s*\?\?\s*prisma/);
    });
    it("uses safeParse for query + emits bounded 400 INVALID_QUERY", () => {
        expect(REVIEWER_CONSOLE_ROUTES).toMatch(/ConsoleQuery\.safeParse\(/);
        expect(REVIEWER_CONSOLE_ROUTES).toMatch(/code:\s*"INVALID_QUERY"[\s\S]{0,400}requestId:\s*req\.id/);
    });
});
// =============================================================================
...
describe("Phase O Stream A — /v1/orgs/:id/members (NODE-1D)", () => {
    it("uses UuidParam.safeParse and emits INVALID_ORG_ID 400", () => {
        ...
        expect(slice).toMatch(/code:\s*"INVALID_ORG_ID"[\s\S]{0,300}Invalid organization id[\s\S]{0,300}requestId:\s*req\.id/);
...
describe("Phase O Stream C — POST /v1/coding/schemas/seed-defaults (NODE-1P)", () => {
    it("seed service is idempotent via natural-key findFirst + skip-or-create", () => {
        expect(CODING_SCHEMA_SERVICE).toMatch(/prisma\.codingSchema\.findFirst\(\s*\{\s*where:\s*\{\s*teamId:\s*input\.teamId,\s*slug:\s*spec\.slug/);
    ...
    it("each spec runs in its own try/catch so a single failure never poisons the pass", () => {
        expect(CODING_SCHEMA_SERVICE).toMatch(/for\s*\(\s*const\s+spec\s+of\s+DEFAULT_SCHEMAS\s*\)\s*\{\s*try\s*\{/);
        expect(CODING_SCHEMA_SERVICE).toMatch(/\}\s*catch\s*\{[\s\S]{0,200}failed\s*\+=\s*1/);
    });
    it("seed payload provides every CodingSchema required + R7-additive field (teamId/slug/label/category/description/createdByUserId)", () => {
        ...
        expect(CODING_SCHEMA_SERVICE).toMatch(/createSchema\(\s*\{[\s\S]{0,400}teamId:\s*input\.teamId/);
        expect(CODING_SCHEMA_SERVICE).toMatch(/createSchema\(\s*\{[\s\S]{0,400}authorUserId:\s*input\.authorUserId/);
        expect(CODING_SCHEMA_SERVICE).toMatch(/createSchema\(\s*\{[\s\S]{0,400}slug:\s*spec\.slug/);
        expect(CODING_SCHEMA_SERVICE).toMatch(/createSchema\(\s*\{[\s\S]{0,400}label:\s*spec\.label/);
        expect(CODING_SCHEMA_SERVICE).toMatch(/createSchema\(\s*\{[\s\S]{0,400}category:\s*spec\.category/);
        expect(CODING_SCHEMA_SERVICE).toMatch(/createSchema\(\s*\{[\s\S]{0,400}description:\s*spec\.description/);
        expect(CODING_SCHEMA_SERVICE).toMatch(/createSchema\(\s*\{[\s\S]{0,400}fields:\s*spec\.fields/);
    });
...
describe("Phase O Stream C — GET /v1/trust/status (NODE-1E)", () => {
    it("wraps projectStatusPage in try/catch and degrades on P2022/P2021", () => {
        ...
        expect(slice).toMatch(/code\s*===\s*"P2022"\s*\|\|\s*code\s*===\s*"P2021"/);
        expect(slice).toMatch(/status:\s*null[\s\S]{0,200}degraded:\s*true[\s\S]{0,200}reason:\s*"SCHEMA_NOT_READY"/);
    });
});
// =============================================================================
...
describe("Phase O — runtime-readiness aggregator probe isolation (NODE-1Q + NODE-1J)", () => {
    ...
    it("readiness route adds an OUTER P2022/P2021 catch as belt-and-braces (NODE-1Q + NODE-1J)", () => {
        expect(READINESS_ROUTES).toMatch(/\/admin\/runtime\/readiness[\s\S]{0,2000}try\s*\{[\s\S]{0,400}runReadinessCheck/);
        expect(READINESS_ROUTES).toMatch(/code\s*===\s*"P2022"\s*\|\|\s*code\s*===\s*"P2021"[\s\S]{0,400}degraded:\s*true[\s\S]{0,200}reason:\s*"SCHEMA_NOT_READY"/);
    });
});
// =============================================================================
...
describe("Phase O Stage 4 — central error handler ZodError mapping", () => {
    it("maps raw ZodError to 400 INVALID_INPUT with bounded fields[]", () => {
        ...
        expect(SERVER_TS).toMatch(/!isAppError\(err\)\s*&&\s*err\s+instanceof\s+ZodError/);
...
describe("Phase O Stage 4 — central error handler Prisma drift mapping", () => {
    it("maps Prisma P2022/P2021 to 503 SCHEMA_NOT_READY (operator-safe)", () => {
        ...
        expect(SERVER_TS).toMatch(/reply\.code\(500\)\.send\(\s*\{\s*error:\s*\{\s*code:\s*"DATABASE_ERROR"/);
    });
});
// =============================================================================
...
describe("Phase O — bounded stability guards", () => {
    ...
    it("no raw process.env reads or stack traces leaked to client-bound degraded payloads", () => {
        ...
        for (const text of [
            ...
            const matches = text.match(/reply\.code\(\d{3}\)\.send\(\s*\{[\s\S]{0,800}?reason:\s*"SCHEMA_NOT_READY"[\s\S]{0,200}?\}\s*\)/g) ?? [];
            ...
            for (const m of matches) {
                expect(m, "degraded SCHEMA_NOT_READY payload must not leak process.env").not.toMatch(/process\.env\./);
                expect(m, "degraded SCHEMA_NOT_READY payload must not leak err.stack").not.toMatch(/err\.stack|error\.stack/);
                expect(m, "degraded SCHEMA_NOT_READY payload must not embed raw error JSON").not.toMatch(/JSON\.stringify\(err\)/);
            }
        }
        ...
        const central = SERVER_TS.match(/reply\.code\(503\)\.send\(\s*\{\s*error:\s*\{[\s\S]{0,400}?code:\s*"SCHEMA_NOT_READY"[\s\S]{0,400}?\}\s*\)/);
