...
describe("Phase 1A — pillar enum", () => {
    it("declares exactly 8 pillars", () => {
        const match = PILLAR_REGISTRY.match(/export\s+const\s+PROOVRA_PILLARS\s*=\s*\[([^\]]+)\]/);
...
describe("Phase 1A — route coverage", () => {
    it("every registered route id is mapped to a pillar", () => {
        ...
        const routeIdMatches = Array.from(ROUTE_REGISTRY.matchAll(/\bid:\s*"([a-z0-9_.\-]+)"/gi)).map((m) => m[1]);
        ...
        const mappingBlock = PILLAR_REGISTRY.match(/PILLAR_FOR_ROUTE_ID:[\s\S]*?new\s+Map\(\[([\s\S]*?)\]\)/);
        expect(mappingBlock).toBeTruthy();
        const mappedIds = new Set(Array.from(mappingBlock[1].matchAll(/"([a-z0-9_.\-]+)"\s*,/gi)).map((m) => m[1]));
        const unmapped = routeIds.filter((id) => !mappedIds.has(id));
        expect(unmapped, `Route ids missing from PILLAR_FOR_ROUTE_ID: ${unmapped.join(", ")}`).toEqual([]);
    });
});
// ===========================================================================
// 3 — Universal pillars
...
describe("Phase 1A — universal pillars", () => {
    it("HOME and TRUST are in the UNIVERSAL_PILLARS set", () => {
        const block = PILLAR_REGISTRY.match(/UNIVERSAL_PILLARS[\s\S]*?=\s*\[([^\]]*)\]/);
...
describe("Phase 1A — server-side pillar projection", () => {
    it("exports buildPillarProjection", () => {
        expect(SERVER_NAV_REGISTRY).toMatch(/export\s+function\s+buildPillarProjection\s*\(/);
    });
    it("declares pillar order matching the canonical client-side order", () => {
        const pos = (token) => SERVER_NAV_REGISTRY.indexOf(`"${token}"`);
        for (let i = 0; i < CANONICAL_PILLAR_ORDER.length - 1; i++) {
            const a = CANONICAL_PILLAR_ORDER[i];
            const b = CANONICAL_PILLAR_ORDER[i + 1];
            const aPos = pos(a);
    ...
    it("envelope navigation type declares sidebar.pillars", () => {
        expect(PLATFORM_CONTEXT_TYPES).toMatch(/sidebar:\s*\{[\s\S]*?pillars:/);
    });
    it("NAVIGATION_SCHEMA_VERSION is bumped to 2", () => {
        expect(PLATFORM_CONTEXT_TYPES).toMatch(/NAVIGATION_SCHEMA_VERSION\s*=\s*2/);
    });
    it("PROOVRA_PILLARS is exported from server types", () => {
        expect(PLATFORM_CONTEXT_TYPES).toMatch(/export\s+const\s+PROOVRA_PILLARS\s*=/);
    });
});
// ===========================================================================
...
describe("Phase 1A — Trust pillar surface", () => {
    it("workspace.trust is registered in the route registry", () => {
        ...
        expect(ROUTE_REGISTRY).toMatch(/workspace\.trust[\s\S]*?sidebarEligible:\s*true/);
    });
...
describe("Phase 1A — legacy IA redirects", () => {
    ...
    for (const [from, to] of REDIRECTS) {
        it(`redirects ${from} → ${to}`, () => {
            ...
            const pattern = new RegExp(`source:\\s*"${escaped(from)}"[\\s\\S]{0,200}?destination:\\s*"${escaped(to)}"`);
...
describe("Phase 1A — grouping resolver", () => {
    it("imports from pillarRegistry", () => {
        expect(GROUPING_RESOLVER).toMatch(/from\s+"\.\/pillarRegistry"/);
    });
...
describe("Phase 1A — pillar bound", () => {
    it("PILLAR_DEFINITIONS has exactly 8 entries", () => {
        const idMatches = Array.from(PILLAR_REGISTRY.matchAll(/^\s{2}\{[\s\S]*?id:\s*"([A-Z]+)"/gm)).map((m) => m[1]);
        // Filter to only the PILLAR_DEFINITIONS array (the ids should be in
        // CANONICAL_PILLAR_ORDER).
        const pillarIds = idMatches.filter((id) => CANONICAL_PILLAR_ORDER.includes(id));
