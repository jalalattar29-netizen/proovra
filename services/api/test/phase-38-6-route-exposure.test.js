...
const EXPOSURE_RESOLVER = readWeb("lib/navigation/workflowExposureResolver.ts");
...
describe("Phase 38.6 — route registry", () => {
    ...
    it("ROUTE_REGISTRY has unique ids", async () => {
        const mod = await import("../../../apps/web/lib/navigation/routeRegistry.js");
        const ids = mod.ROUTE_REGISTRY.map((r) => r.id);
    ...
    it("ACCOUNT-domain routes always declare requiredActiveSpace='NONE'", async () => {
        const mod = await import("../../../apps/web/lib/navigation/routeRegistry.js");
        const accountRoutes = mod.ROUTE_REGISTRY.filter((r) => r.domain === "ACCOUNT");
        expect(accountRoutes.length).toBeGreaterThan(0);
        for (const r of accountRoutes) {
            expect(r.requiredActiveSpace, `${r.id} is ACCOUNT domain but requires ${r.requiredActiveSpace}`).toBe("NONE");
        }
    });
...
describe("Phase 38.6 — resolveRouteAccess workflow independence", () => {
    ...
    it("ALLOWED state requires every requiredCapability to be true", async () => {
        const mod = await import("../../../apps/web/lib/navigation/routeAccessResolver.js");
...
describe("Phase 38.6 — resolveWorkflowExposure invariants", () => {
    ...
    it("every canSeeNav route lands in at least one bucket", async () => {
        const mod = await import("../../../apps/web/lib/navigation/workflowExposureResolver.js");
        ...
        for (const r of routes) {
            if (r.access.canSeeNav) {
                expect(bucketed.has(r.route.id), `${r.route.id} is canSeeNav but missing from every bucket`).toBe(true);
            }
        }
    });
...
describe("Phase 38.6 — PageRouteGate", () => {
    ...
    it("renders a structured panel (not null) for PLATFORM_ADMIN_ONLY — production fix", () => {
        ...
        const branchIdx = PAGE_GATE.indexOf('access.accessState === "PLATFORM_ADMIN_ONLY"');
...
describe("Phase 38.6 — All Tools surface", () => {
    ...
    it("renders the 'Workflow profiles personalize layout' safety copy", () => {
        expect(TOOLS_PAGE).toMatch(/Workflow profiles personalize layout, defaults, and\s+recommendations\. They do not change permissions or remove tools\./);
    });
    ...
    it("ROUTE_REGISTRY: every sidebarEligible route is also allToolsVisible (or carries a noted exception)", async () => {
        ...
        for (const r of mod.ROUTE_REGISTRY) {
            if (r.sidebarEligible) {
                expect(r.allToolsVisible, `${r.id} is sidebarEligible but not allToolsVisible — would be invisible if workflow demotes it`).toBe(true);
            }
        }
    });
    it("ROUTE_REGISTRY: at least one canonical route is allToolsVisible (smoke check)", async () => {
        const mod = await import("../../../apps/web/lib/navigation/routeRegistry.js");
        const allTools = mod.ROUTE_REGISTRY.filter((r) => r.allToolsVisible);
...
// Fixtures
// =============================================================================
function makeFixtureRoutes(canSeeNav) {
    return [
        {
            route: {
                id: "workspace.home",
