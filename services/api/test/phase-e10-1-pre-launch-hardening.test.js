...
const MIGRATION = readApi("prisma/migrations/20260804000000_phase_e10_1_stripe_webhook_idempotency/migration.sql");
const DRIFT_TEST = readApi("test/phase-32-7-2-security-event-mapping-drift.test.ts");
...
describe("E10.1 Test 1 — DEF-037: auth route rate limits", () => {
    it("auth.routes.ts imports enforceRateLimit from the existing rate-limit service", () => {
        expect(AUTH_ROUTES).toMatch(/import\s*\{\s*enforceRateLimit\s*\}\s*from\s*["']\.\.\/services\/rate-limit\.js["']/);
    });
    it("login + password-reset routes call enforceRateLimit before body parsing", () => {
        const loginBlock = AUTH_ROUTES.slice(AUTH_ROUTES.indexOf('app.post("/v1/auth/email/login"'), AUTH_ROUTES.indexOf('app.post("/v1/auth/password-reset/request"'));
        const resetBlock = AUTH_ROUTES.slice(AUTH_ROUTES.indexOf('app.post("/v1/auth/password-reset/request"'), AUTH_ROUTES.indexOf('app.post("/v1/auth/password-reset/confirm"'));
    ...
    it("bounded per-IP buckets are tight: login ≤ 15/min; password-reset ≤ 10/min", () => {
        ...
        const loginMax = AUTH_ROUTES.match(/AUTH_LOGIN_RATE_LIMIT_PER_IP_PER_MIN\s*=\s*(\d+)/);
        const resetMax = AUTH_ROUTES.match(/AUTH_PASSWORD_RESET_RATE_LIMIT_PER_IP_PER_MIN\s*=\s*(\d+)/);
    ...
    it("rate-limit-blocked path returns 429 + Retry-After header + auditAuthEvent({outcome: 'blocked'})", () => {
        ...
        const blocks = [
            AUTH_ROUTES.slice(AUTH_ROUTES.indexOf('app.post("/v1/auth/email/login"'), AUTH_ROUTES.indexOf('app.post("/v1/auth/password-reset/request"')),
            AUTH_ROUTES.slice(AUTH_ROUTES.indexOf('app.post("/v1/auth/password-reset/request"'), AUTH_ROUTES.indexOf('app.post("/v1/auth/password-reset/confirm"')),
        ];
...
describe("E10.1 Test 2 — DEF-038: Stripe webhook idempotency", () => {
    ...
    it("migration creates the stripe_webhook_events table with the unique index", () => {
        ...
        expect(MIGRATION).toMatch(/CREATE UNIQUE INDEX\s+"stripe_webhook_events_stripe_event_id_key"\s+ON\s+"stripe_webhook_events"\(\s*"stripe_event_id"\s*\)/);
    ...
    it("webhook handler verifies signature BEFORE the idempotency check", () => {
        const handler = WEBHOOK_ROUTES.slice(WEBHOOK_ROUTES.indexOf('app.post("/stripe"'), WEBHOOK_ROUTES.indexOf('app.post("/paypal"'));
    ...
    it("webhook handler inserts the stripeWebhookEvent row BEFORE processing the event payload", () => {
        const handler = WEBHOOK_ROUTES.slice(WEBHOOK_ROUTES.indexOf('app.post("/stripe"'), WEBHOOK_ROUTES.indexOf('app.post("/paypal"'));
        const idemIdx = handler.indexOf("stripeWebhookEvent.create");
        const firstProcessingIdx = handler.indexOf('if (event.type === "checkout.session.completed")');
        expect(idemIdx).toBeGreaterThan(-1);
...
describe("E10.1 Test 3 — migration drift allow-list updated", () => {
    it("32.7-2 migration drift test includes the E10.1 migration", () => {
        expect(DRIFT_TEST).toContain("20260804000000_phase_e10_1_stripe_webhook_idempotency");
    });
});
// ===========================================================================
...
describe("E10.1 Test 5 — no expansion beyond blocker closure", () => {
    ...
    it("capability registry is unchanged — E10.1 adds no new capability key", () => {
        const capRegistry = readApi("src/services/platform-context/capability-registry.ts");
...
describe("E10.1 Test 8 — master registry records closures", () => {
    it("registry registers Phase E10.1 with explicit closure status", () => {
        expect(REGISTRY).toMatch(/\|\s*Phase\s+E10\.1\s*\|[\s\S]*?(CLOSED|CLOSED_WITH_DEFERRED_ITEMS)/);
    });
