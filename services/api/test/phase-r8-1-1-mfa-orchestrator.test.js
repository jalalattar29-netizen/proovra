...
describe("R8.1.1 Part 1 — Prisma migration for R8.1 schema", () => {
    ...
    it("migration SQL creates the canonical tables + enums", () => {
        const sql = readApi("prisma/migrations/20260722000000_r8_1_mfa_activation/migration.sql");
...
describe("R8.1.1 Part 2 — orchestrator composes R8.1 helpers", () => {
    ...
    it("activation requires a successful TOTP verify (no auto-activate)", () => {
        ...
        expect(ORCH_SRC).toMatch(/if\s*\(\s*!verifyTotpCode\([^)]+\)\s*\)\s*\{[\s\S]{0,600}return\s*\{\s*ok:\s*false/);
    });
    ...
    it("regenerate invalidates the previous batch atomically", () => {
        expect(ORCH_SRC).toMatch(/mfaRecoveryCode\.updateMany[\s\S]{0,400}batchInvalidatedAt[\s\S]{0,400}mfaRecoveryCode\.createMany/);
    });
    it("readMfaStatus does NOT return secrets / verifiers / lookup hashes", () => {
        ...
        const selectMatch = ORCH_SRC.match(/readMfaStatus[\s\S]*?prisma\.mfaFactor\.findMany\(\{[\s\S]*?select:\s*\{([\s\S]*?)\}/);
...
describe("R8.1.1 Part 3 — REST endpoints registered", () => {
    it("exposes the 6 canonical MFA endpoints under /v1/identity/mfa/*", () => {
        ...
        expect(ROUTES_SRC).toMatch(/app\.post\(\s*"\/v1\/identity\/mfa\/enroll\/verify"/);
        ...
        expect(ROUTES_SRC).toMatch(/app\.delete(?:<[^>]+>)?\(\s*"\/v1\/identity\/mfa\/factors\/:id"/);
        expect(ROUTES_SRC).toMatch(/app\.post\(\s*"\/v1\/identity\/mfa\/recovery-codes\/regenerate"/);
        expect(ROUTES_SRC).toMatch(/app\.post\(\s*"\/v1\/identity\/mfa\/challenge\/verify"/);
    });
    it("every endpoint requires authentication via the canonical requireAuth preHandler", () => {
        const preHandlerCount = (ROUTES_SRC.match(/preHandler:\s*requireAuth/g) ?? []).length;
...
describe("R8.1.1 Part 4 — recovery codes surfaced only at issuance", () => {
    it("verify-enroll endpoint returns recoveryCodes (one-time)", () => {
        ...
        expect(ROUTES_SRC).toMatch(/enroll\/verify[\s\S]{0,2000}recoveryCodes:\s*\w+\.recoveryCodes/);
    });
    it("regenerate endpoint returns recoveryCodes (one-time)", () => {
        expect(ROUTES_SRC).toMatch(/recovery-codes\/regenerate[\s\S]{0,800}recoveryCodes:\s*\w+\.recoveryCodes/);
    });
    it("factors endpoint (status) does NOT return recoveryCodes plaintext", () => {
        ...
        const statusReturnBlock = ORCH_SRC.match(/readMfaStatus[\s\S]*?return\s*\{([\s\S]*?)\};/);
...
describe("R8.1.1 Part 5 — rate limiting on verify endpoints", () => {
    ...
    it("enroll-verify checks the rate limiter and returns 429 when exceeded", () => {
        expect(ROUTES_SRC).toMatch(/enroll\/verify[\s\S]{0,600}isRateLimited[\s\S]{0,400}reply\.code\(429\)/);
    });
    it("challenge-verify checks the rate limiter and returns 429 when exceeded", () => {
        expect(ROUTES_SRC).toMatch(/challenge\/verify[\s\S]{0,600}isRateLimited[\s\S]{0,400}reply\.code\(429\)/);
    });
    it("rate-limit hits emit mfa_verification_failed with reason=rate_limited", () => {
        expect(ROUTES_SRC).toMatch(/isRateLimited[\s\S]{0,400}reason:\s*"rate_limited"/);
    });
});
// =============================================================================
