...
import { REVIEWER_OPS_BULK_ACTIONS, REVIEWER_OPS_REMINDER_KINDS, REVIEWER_OPS_SAVED_VIEW_SCOPE, REVIEWER_OPS_SLA_POLICY_HOURS_MAX, SAVED_VIEW_SCOPES, SEARCH_FORBIDDEN_OVERCLAIM_PHRASES, isReviewerInactive, resolveReviewerOpsSlaPolicy, } from "@proovra/shared";
...
describe("Phase 25.5 — SLA policy resolver wiring", () => {
    const slaSrc = readSource("../src/services/reviewer-ops/sla-policy.service.ts");
...
describe("Phase 25.5 — bulk triage service", () => {
    const src = readSource("../src/services/reviewer-ops/bulk-triage.service.ts");
...
describe("Phase 25.5 — saved queue views", () => {
    const src = readSource("../src/services/reviewer-ops/saved-queue-views.service.ts");
...
describe("Phase 25.5 — reminder engine", () => {
    const src = readSource("../src/services/reviewer-ops/reminder-engine.service.ts");
...
describe("Phase 25.5 — step-up wiring", () => {
    ...
    it("approve / reject / escalation-resolve / escalation-suppress routes invoke the gate", () => {
        ...
        const matches = src.match(/await enforceStepUpIfFlagged\(/g) ?? [];
    ...
    it("SLA policy edit route requires GOVERNANCE_POLICY_UPDATE step-up", () => {
        ...
        expect(src).toMatch(/app\.post\(\s*"\/v1\/reviewer-ops\/sla-policy"/);
...
describe("Phase 25.5 — analytics service", () => {
    const src = readSource("../src/services/reviewer-ops/analytics.service.ts");
