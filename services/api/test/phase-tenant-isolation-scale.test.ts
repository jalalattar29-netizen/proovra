  ...
  try {
    ...
    for (const name of readdirSync(absDir)) {
      ...
      try {
        ...
        if (stat.isFile() && name.endsWith(ext)) out.push(full);
      } catch {
        /* ignore */
      }
    }
...
const SERVICES_DIR = fileURLToPath(
  new URL("../src/services/", import.meta.url),
);
const WORKER_SRC_DIR = fileURLToPath(
 ...
 */
const NON_OPERATIONAL_ROUTES: ReadonlySet<string> = new Set([
  "auth.routes.ts",
  "sso-auth.routes.ts",
...
describe("Tenant isolation — operational route classification", () => {
  const routeFiles = listFiles(ROUTES_DIR, ".ts").filter((p) =>
    isRouteFile(p.split(/[\\/]/).pop() ?? ""),
  );
  ...
  for (const fullPath of routeFiles) {
    ...
    if (NON_OPERATIONAL_ROUTES.has(name)) continue;
    it(`${name} consumes the canonical authorize helper OR carries an approved TENANT_SCOPE_EXCEPTION`, () => {
      const src = readFileSync(fullPath, "utf8");
      const hasAuthorize =
        /authorizeOrFail|requireAuthorize\(|evaluateMemberAccess|evaluateAuthorize|requireEvidenceAccess|requireCaseAccess|requireReportAccess|requirePackageAccess|requireActiveSpaceAccess/.test(
          src,
        );
      ...
      const isExplicitNoOp =
        /Intentionally disabled|Intentionally left as a no-op|LEGACY[^\n]*no-op/i.test(
          src,
        );
      expect(
        hasAuthorize || hasException || hasTenantMarkers || isExplicitNoOp,
        `${name} must use authorizeOrFail/requireAuthorize, a TENANT_SCOPE_EXCEPTION comment, or reference both userId and teamId. None found.`,
      ).toBe(true);
    });
  }
});
...
describe("Tenant isolation — bounded list queries", () => {
  ...
  for (const entry of readdirSync(SERVICES_DIR)) {
    ...
    try {
      const stat = statSync(full);
      if (stat.isFile() && entry.endsWith(".ts")) serviceFiles.push(full);
      else if (stat.isDirectory()) {
        for (const sub of readdirSync(full)) {
          if (sub.endsWith(".ts")) serviceFiles.push(`${full}/${sub}`);
        }
      }
    } catch {
      /* ignore */
  ...
  for (const model of LARGE_OPERATIONAL_MODELS) {
    it(`every ${model}.findMany call sits in a file with a bounding token or an approved exception`, () => {
      ...
      expect(
        offenders,
        `${model}.findMany must be bounded by take/cursor/limit, scoped to a tenant/parent id, or carry an approved TENANT_SCOPE_EXCEPTION. Offenders:\n${offenders.join("\n")}`,
      ).toEqual([]);
    });
  }
});
...
describe("Tenant isolation — worker job ownership verification", () => {
  ...
  function walk(dir: string) {
    try {
      for (const name of readdirSync(dir)) {
        ...
        try {
          const stat = statSync(full);
          if (stat.isFile() && name.endsWith(".ts")) workerFiles.push(full);
  ...
  for (const handler of HANDLER_PATTERNS) {
    const candidate = workerFiles.find((f) =>
      f.replace(/\\/g, "/").endsWith(`/${handler}`),
    );
    if (!candidate) continue; // file moved or renamed — silently skip
    it(`worker handler ${handler} scopes by resource id from the job payload`, () => {
      ...
      const fetchesResource = /prisma\.(evidence|report|verificationPackage|evidencePart|otsRecord|otsState|searchDocument|mediaIntelligenceRun)\.(findUnique|findFirst|findMany)/.test(
        src,
      );
      const referencesResourceId =
        /\b(evidenceId|caseId|reportId|packageId|partId|runId|verificationPackageId)\b/.test(
          src,
        );
      const hasException = hasApprovedException(src);
      expect(
        fetchesResource || referencesResourceId || hasException,
        `${handler} must fetch the resource by id, reference a job-payload resource id, or carry an approved TENANT_SCOPE_EXCEPTION`,
      ).toBe(true);
    });
  }
});
...
describe("Tenant isolation — list endpoints accept pagination", () => {
  ...
  for (const name of LIST_ROUTE_FILES) {
    ...
    it(`${name} accepts cursor / limit / take pagination on its list endpoints`, () => {
      ...
      expect(
        hasPaginationToken,
        `${name} must accept cursor/limit pagination on its list endpoints`,
      ).toBe(true);
    });
  }
});
...
describe("Tenant isolation — schema indexes for tenant-scoped scans", () => {
  ...
  const REQUIRED_INDEX_TABLES: ReadonlyArray<{
    model: string;
    /**
     * Leading tenant/parent key. Indexes whose first column matches one of
     * these are accepted.
     */
    leadingKeys: ReadonlyArray<string>;
  }> = [
  ...
  for (const entry of REQUIRED_INDEX_TABLES) {
    it(`${entry.model} declares at least one index leading with a tenant/parent key`, () => {
      ...
      expect(
        hasMatchingIndex,
        `${entry.model} must declare an @@index leading with one of: ${entry.leadingKeys.join(", ")}. Found indexes:\n${indexLines.join("\n")}`,
      ).toBe(true);
    });
  }
});
