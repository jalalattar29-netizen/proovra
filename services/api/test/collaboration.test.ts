...
} from "../src/services/collaboration/discussion.service.js";
...
describe("Thread projection — privacy", () => {
  it("omits resolutionNote + escalationReason from list projection", () => {
    const projected = projectDiscussionThread({
      ...
      kind: "EVIDENCE_GENERAL" as never,
    ...
    expect((projected as Record<string, unknown>).resolutionNote).toBeUndefined();
    expect((projected as Record<string, unknown>).escalationReason).toBeUndefined();
  });
});
...
describe("Message projection — shape", () => {
  it("includes body text (internal channel) but never raw deleted/edited internals", () => {
    ...
    expect((projected as Record<string, unknown>).deletedByUserId).toBeUndefined();
    expect((projected as Record<string, unknown>).contributorIntakeSessionId).toBeUndefined();
  });
});
...
describe("Notification templates — collaboration cases use ReviewAssigned context", () => {
  it("five new case branches present + each uses expectCtx(ctx, ReviewAssigned)", async () => {
    ...
    for (const evt of [
      ...
      const re = new RegExp(
        `case\\s+"${evt}":[\\s\\S]{0,300}expectCtx\\(ctx,\\s*"ReviewAssigned"\\)`,
      );
