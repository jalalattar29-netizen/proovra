...
describe("Phase 25.7 — queue intelligence service", () => {
  ...
  it("imports the SHARED engines (no duplicated logic)", () => {
    expect(src).toMatch(
      /import\s*\{[\s\S]*?computeReviewerPriority[\s\S]*?detectStuckWorkflow[\s\S]*?rankReviewerSuggestions[\s\S]*?\}\s*from\s+"@proovra\/shared"/,
    );
  });

  it("exports projectQueueIntelligence with a bounded input contract", () => {
    expect(src).toMatch(
      /export async function projectQueueIntelligence\([\s\S]*?QueueIntelligenceInput/,
    );
...
describe("Phase 25.7 — POST /v1/reviewer-ops/queue-intelligence route", () => {
  ...
  it("declares the route gated by requireReviewerActor (RBAC enforced at the boundary)", () => {
    ...
    expect(src).toMatch(
      /\/v1\/reviewer-ops\/queue-intelligence"[\s\S]*?requireReviewerActor\(req, reply, body\.teamId\)/,
    );
  });

  it("validates body with bounded workflowIds (max 100, min 1)", () => {
    expect(src).toMatch(
      /workflowIds:\s*z\.array\(z\.string\(\)\.uuid\(\)\)\.min\(1\)\.max\(100\)/,
    );
  });

  it("imports projectQueueIntelligence from the service (no inline orchestration)", () => {
    expect(src).toMatch(
      /import\s*\{\s*projectQueueIntelligence\s*\}\s*from\s+"\.\.\/services\/reviewer-ops\/queue-intelligence\.service\.js"/,
    );
  });

  it("propagates actor + isReviewerCapable so the engine enforces self-assignment + reviewer-restricted rules", () => {
    expect(src).toMatch(
      /projectQueueIntelligence\(\{[\s\S]*?actorUserId:\s*ctx\.actorUserId[\s\S]*?isReviewerCapable:\s*ctx\.isReviewerCapable/,
    );
  });
});
...
describe("Phase 25.7 — stuck workflow sweep wired into runReconcile", () => {
  ...
  it("imports detectStuckWorkflow from @proovra/shared", () => {
    expect(src).toMatch(
      /import\s*\{[\s\S]*?detectStuckWorkflow[\s\S]*?\}\s*from\s+"@proovra\/shared"/,
    );
  });

  it("loads non-terminal workflows in a bounded scan (take: 200)", () => {
    expect(src).toMatch(
      /evidenceReviewWorkflow\.findMany\(\{[\s\S]*?status:\s*\{\s*notIn:\s*\[[\s\S]*?"CLOSED"[\s\S]*?"APPROVED_INTERNAL"[\s\S]*?"REJECTED_INSUFFICIENT"[\s\S]*?\]\s*as never[\s\S]*?\}[\s\S]*?take:\s*200/,
    );
  });

  it("computes detectStuckWorkflow per row inside the reconcile sweep", () => {
    expect(src).toMatch(
      /const classification:\s*StuckClassification = detectStuckWorkflow\(/,
    );
  });
  ...
  it("only escalates CRITICAL-severity stuck rows (never WARNING/HIGH — guards against storm)", () => {
    expect(src).toMatch(
      /classification\.topSeverity === "CRITICAL"[\s\S]*?!wf\.activeEscalationId/,
    );
  });

  it("escalation reuses the existing WORKFLOW_STALLED reason (existing fingerprint dedup)", () => {
    expect(src).toMatch(
      /createEscalation\(\s*\{[\s\S]*?reason:\s*"WORKFLOW_STALLED"/,
    );
  });
  ...
  it("stuck sweep is best-effort — failure does not abort reconcile (try/catch around the sweep)", () => {
    expect(src).toMatch(
      /\/\/ Phase 25\.7 — stuck workflow sweep[\s\S]*?try\s*\{[\s\S]*?\}\s*catch\s*\{[\s\S]*?\/\/ Stuck sweep is best-effort/,
    );
  });
...
describe("Phase 25.7 — privacy + governance invariants", () => {
  ...
  it("route handler does NOT call Prisma directly (orchestration stays in the service)", () => {
    ...
    const handler = routeSrc.slice(
      routeSrc.indexOf('"/v1/reviewer-ops/queue-intelligence"'),
      routeSrc.indexOf('"/v1/reviewer-ops/queue-intelligence"') + 1200,
    );
