...
const TYPES_SRC = readSource("../src/services/platform-context/types.ts");
const CTX_SERVICE = readSource("../src/services/platform-context/platform-context.service.ts");
...
const ORG_AUDIT_SERVICE = readSource("../src/services/organization/org-audit.service.ts");
const RETENTION_RESOLVER = readSource("../src/services/organization/retention-inheritance.service.ts");
const NAV_REGISTRY = readSource("../src/services/platform-context/navigation-registry.ts");
describe("Phase B0 — workspace URL alias", () => {
    ...
    it("rewrite is one-way (workspaces → teams; teams is not rewritten)", () => {
        ...
        const sourceLines = lines.filter((l) => l.includes("startsWith(") || l.includes("rawUrl.startsWith"));
    ...
    it("rewrite is path-prefix-safe (rejects /v1/workspacesfoo)", () => {
        ...
        expect(ALIAS_PLUGIN).toMatch(/nextChar !== "" && nextChar !== "\/" && nextChar !== "\?"/);
    });
...
describe("Phase B0 — platform-context envelope v3", () => {
    it("AUTHORITY_SCHEMA_VERSION is bumped to 3", () => {
        expect(TYPES_SRC).toMatch(/export\s+const\s+AUTHORITY_SCHEMA_VERSION:\s*number\s*=\s*3/);
    });
...
describe("Phase B0 — organization governance write surfaces", () => {
    ...
    it("retention publish emits ORG_POLICY_RETENTION_PUBLISHED audit", () => {
        expect(GOV_ROUTES).toContain('"ORG_POLICY_RETENTION_PUBLISHED"');
    ...
    it("write endpoints emit anti-enumeration 404 — never 403 — on access denial", () => {
        ...
        // is not enumerable.
        expect(GOV_ROUTES).toMatch(/if \(result\.kind !== "ok"\)[\s\S]*?return\s*\{\s*ok:\s*false,\s*code:\s*404/);
    });
});
describe("Phase B0 — retention inheritance resolver", () => {
    ...
    it("resolution order is team-first → org-inherited → none", () => {
        ...
        const fn = RETENTION_RESOLVER.slice(RETENTION_RESOLVER.indexOf("export async function resolveTeamRetentionPolicy"));
    ...
    it("resolver is read-only — never creates a team policy from an inherited template", () => {
        expect(RETENTION_RESOLVER).not.toMatch(/\bevidenceRetentionPolicy\.create\(/);
        expect(RETENTION_RESOLVER).not.toMatch(/\bevidenceRetentionPolicy\.upsert\(/);
    });
