...
import { DISCOVERY_FORBIDDEN_FIELDS, EXTERNAL_REVIEW_ACCESS_STATES, EXTERNAL_REVIEW_EVIDENCE_FORBIDDEN_FIELDS, EXTERNAL_REVIEW_EVIDENCE_SAFE_FIELDS, INDEXING_EVENT_KINDS, PACKAGE_ELIGIBILITY_OUTCOMES, SEARCHABLE_ENTITY_KINDS, applyDiscoveryFilter, canonicalEvaluatePackageEligibility, emitIndexingEvent, evaluateExternalReviewAccess, isActiveExternalReviewState, isAllowedExternalReviewTransition, isTerminalExternalReviewState, projectEvidenceForExternalReview, registerIndexingEventSink, } from "@proovra/shared";
...
describe("Phase 28-E [worker gate] — source contract", () => {
    const gateSrc = readSource("../../worker/src/governance/package-eligibility-gate.ts");
    it("calls the canonical helper, never inlines the rules", () => {
        expect(gateSrc).toMatch(/import\s*\{[^}]*canonicalEvaluatePackageEligibility[^}]*\}\s*from\s*"@proovra\/shared"/);
    ...
    it("treats any prisma error as denial (GOVERNANCE_STATE_UNAVAILABLE)", () => {
        expect(gateSrc).toMatch(/catch[^}]*GOVERNANCE_STATE_UNAVAILABLE/s);
    });
    ...
    it("emits an operational log on denial", () => {
        expect(gateSrc).toMatch(/logger\.warn\([\s\S]+?"package_generation\.denied"/);
    });
...
describe("Phase 28-E [worker gate] — wiring into createVerificationPackage", () => {
    const pkgSrc = readSource("../../worker/src/verification-package.ts");
    ...
    it("processor.ts now passes teamId to createVerificationPackage", () => {
        ...
        expect(procSrc).toMatch(/createVerificationPackage\(\{[\s\S]+?teamId:\s*evidence\.teamId/);
    });
});
// =============================================================================
...
describe("Phase 28-E [external review] — lifecycle state machine", () => {
    ...
    it("INVITED can transition to ACTIVE / REVOKED / EXPIRED / BLOCKED_BY_POLICY", () => {
        ...
        expect(isAllowedExternalReviewTransition("INVITED", "BLOCKED_BY_POLICY")).toBe(true);
    });
    it("BLOCKED_BY_POLICY can recover to ACTIVE if policy lifts", () => {
        expect(isAllowedExternalReviewTransition("BLOCKED_BY_POLICY", "ACTIVE")).toBe(true);
    });
});
describe("Phase 28-E [external review] — access evaluation", () => {
    ...
    it("active legal hold revokes external access by default", () => {
        ...
        expect(d.allowed).toBe(false);
        if (!d.allowed)
            expect(d.reason).toBe("invalidated_by_hold");
    });
    ...
    it("governance blocker beats everything else (most-restrictive)", () => {
        ...
        expect(d.allowed).toBe(false);
        if (!d.allowed)
            expect(d.reason).toBe("blocked_by_governance");
    });
    ...
    it("BLOCKED_BY_POLICY denies", () => {
        ...
        expect(d.allowed).toBe(false);
        if (!d.allowed)
            expect(d.reason).toBe("blocked_by_policy");
    });
});
// =============================================================================
...
describe("Phase 28-E [discovery] — applyDiscoveryFilter visibility gates", () => {
    ...
    it("external reviewer cannot see operator_only documents", () => {
        ...
        expect(d.allowed).toBe(false);
        if (!d.allowed)
            expect(d.reason).toBe("visibility_redacted");
    });
    ...
    it("DESTROYED / PENDING_DESTRUCTION / RETENTION_LOCKED are hidden from external reviewers", () => {
        for (const state of [
            ...
            expect(d.allowed).toBe(false);
            if (!d.allowed)
                expect(d.reason).toBe("blocked_by_lifecycle");
        }
    });
});
...
describe("Phase 28-E [export parity audit] — every governance-sensitive path consults canonical helpers", () => {
    it("governance-snapshot service routes export decision through canonicalEvaluateExportEligibility", () => {
        const src = readSource("../src/services/governance-lifecycle/governance-snapshot.service.ts");
    ...
    it("export-governance.service.ts is the only inline implementation, and uses the canonical helper for decisions", () => {
        ...
        // shared catalog.
        expect(src).toMatch(/ExportEligibilityResult|ExportEligibilityOutcome/);
    });
});
// =============================================================================
...
describe("Phase 28-E [privacy] — projections never expose forbidden fields", () => {
    ...
    const discoverySrc = readSource("../../../packages/shared/src/discovery-foundation.ts");
    it("external-review projection enumerates ONLY safe fields", () => {
        ...
        for (const forbidden of EXTERNAL_REVIEW_EVIDENCE_FORBIDDEN_FIELDS) {
            ...
            expect(externalSrc).not.toMatch(new RegExp(`${forbidden}:\\s*(pick|String|raw|maybeString)`));
        }
    });
