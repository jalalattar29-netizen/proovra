...
const PLATFORM_CTX_PROVIDER_SRC = readWeb("lib/platform-context/PlatformContextProvider.tsx");
const PLATFORM_CTX_TYPES_WEB_SRC = readWeb("lib/platform-context/types.ts");
const GLOBAL_RUNTIME_INDICATOR_SRC = readWeb("components/operational/GlobalRuntimeIndicator.tsx");
const CASES_INDEX_SRC = readWeb("components/cases-experience/CasesIndex.tsx");
const NEXT_CONFIG_SRC = readWeb("next.config.js");
const PLATFORM_CTX_API_SRC = readApi("src/services/platform-context/platform-context.service.ts");
const PLATFORM_CTX_TYPES_API_SRC = readApi("src/services/platform-context/types.ts");
const CAPABILITY_REGISTRY_SRC = readApi("src/services/platform-context/capability-registry.ts");
// =============================================================================
...
describe("Phase R10 — Stage 1: navigation schema version parity", () => {
    it("API and web client export the SAME NAVIGATION_SCHEMA_VERSION literal", () => {
        ...
        const apiMatch = PLATFORM_CTX_TYPES_API_SRC.match(/export\s+const\s+NAVIGATION_SCHEMA_VERSION\s*=\s*(\d+)\s+as\s+const/);
        const webMatch = PLATFORM_CTX_TYPES_WEB_SRC.match(/export\s+const\s+NAVIGATION_SCHEMA_VERSION\s*=\s*(\d+)\s+as\s+const/);
    ...
    it("API platform-context.service references NAVIGATION_SCHEMA_VERSION when emitting envelope", () => {
        expect(PLATFORM_CTX_API_SRC).toMatch(/navigationSchemaVersion:\s*NAVIGATION_SCHEMA_VERSION/);
    });
    it("web client whitelist (ACCEPTED_NAVIGATION_SCHEMA_VERSIONS) contains the current constant", () => {
        ...
        // number literals.
        const versionMatch = PLATFORM_CTX_TYPES_WEB_SRC.match(/export\s+const\s+NAVIGATION_SCHEMA_VERSION\s*=\s*(\d+)\s+as\s+const/);
        expect(versionMatch).not.toBeNull();
        const currentVersion = Number(versionMatch[1]);
        const whitelistMatch = PLATFORM_CTX_TYPES_WEB_SRC.match(/export\s+const\s+ACCEPTED_NAVIGATION_SCHEMA_VERSIONS[\s\S]*?\[([^\]]+)\]/);
    ...
    it("SCHEMA_VERSION_MISMATCH appears in the PlatformContextErrorCode union", () => {
        ...
        expect(PLATFORM_CTX_TYPES_WEB_SRC).toMatch(/export\s+type\s+PlatformContextErrorCode\s*=[\s\S]*?\|\s*"SCHEMA_VERSION_MISMATCH"/);
    });
    it("PlatformContextProvider transitions to FAILED (NOT LOADING_CONTEXT) on schema mismatch", () => {
        ...
        expect(PLATFORM_CTX_PROVIDER_SRC).toMatch(/name:\s*"FAILED"[\s\S]{0,200}errorCode:\s*"SCHEMA_VERSION_MISMATCH"/);
    });
    it("schema-mismatch path does NOT downgrade to LOADING_CONTEXT", () => {
        ...
        const mismatchIdx = PLATFORM_CTX_PROVIDER_SRC.indexOf('errorCode: "SCHEMA_VERSION_MISMATCH"');
        ...
        const window = PLATFORM_CTX_PROVIDER_SRC.slice(Math.max(0, mismatchIdx - 200), mismatchIdx + 600);
...
describe("Phase R10 — Stage 2: resolveRouteAccess call sites pass envelope context", () => {
    ...
    const LEGACY_FRAGMENTS_RE = /resolveRouteAccess\(\{[\s\S]{0,800}workspace:\s*envelope\?\.workspace[\s\S]{0,400}personalSpace:\s*envelope\?\.personalSpace/;
    function hasCentralizedFragments(src) {
        return (/useWorkspaceFragment\b/.test(src) &&
    ...
    it("AppSidebarV2 passes both workspace and personalSpace into resolveRouteAccess", () => {
        expect(LEGACY_FRAGMENTS_RE.test(APP_SIDEBAR_SRC) ||
            hasCentralizedFragments(APP_SIDEBAR_SRC)).toBe(true);
    });
    it("CommandPalette passes both workspace and personalSpace into resolveRouteAccess", () => {
        expect(LEGACY_FRAGMENTS_RE.test(COMMAND_PALETTE_SRC) ||
            hasCentralizedFragments(COMMAND_PALETTE_SRC)).toBe(true);
    });
    it("tools/page.tsx passes both workspace and personalSpace into resolveRouteAccess", () => {
        expect(LEGACY_FRAGMENTS_RE.test(TOOLS_PAGE_SRC) ||
            hasCentralizedFragments(TOOLS_PAGE_SRC)).toBe(true);
    });
});
describe("Phase R10 — Stage 2: capability registry personal-first rules", () => {
    async function loadCapReg() {
        return import("../src/services/platform-context/capability-registry.js");
    }
...
describe("Phase R10 — Stage 2: GlobalRuntimeIndicator hides for personal users", () => {
    it("returns null when teamId is null (no operator runtime to report)", () => {
        ...
        expect(GLOBAL_RUNTIME_INDICATOR_SRC).toMatch(/if\s*\(\s*teamId === null\s*\)\s*\{\s*return null;/);
    });
    it("AppTopbarV2 passes teamId=null for non-TEAM workspaces (source contract on sidebar derivation)", () => {
        ...
        expect(APP_SIDEBAR_SRC).toMatch(/workspace\?\.status === "active"\s*&&\s*envelope\.workspace\.scope === "TEAM"/);
    });
});
...
describe("Phase R10 — Stage 2/3: next.config.js redirect cleanliness", () => {
    ...
    it("for every /ops/* page on disk, no /ops/* -> /operations/* redirect exists (unless the destination also exists on disk)", () => {
        ...
        for (const page of pages) {
            ...
            const sourceRe = new RegExp(`source:\\s*["']${page.replace(/\//g, "\\/")}["']`);
            if (!sourceRe.test(NEXT_CONFIG_SRC))
                continue;
            ...
            expect(existsSync(destPath), `redirect ${page} -> ${dest} but ${destPath} does not exist`).toBe(true);
        }
    });
});
// =============================================================================
...
describe("Phase R10 — isProAccount derives from accountPlan (not workspace.plan)", () => {
    it("isProAccount literal expression uses accountPlan + PRO_PLAN_KEYS (NOT workspace.plan)", () => {
        ...
        expect(PLATFORM_CTX_API_SRC).toMatch(/const\s+isProAccount\s*=\s*accountPlan\s*\?\s*PRO_PLAN_KEYS\.has\(accountPlan\)\s*:\s*false/);
    });
    it("the isProAccount derivation block does NOT read workspace.plan", () => {
        ...
        const windowSrc = PLATFORM_CTX_API_SRC.slice(Math.max(0, idx - 50), idx + 200);
...
describe("Phase R10 — PageRouteGate warns on unknown routeId in development", () => {
    it("development-only console.warn fires for unregistered routeId", () => {
        ...
        expect(PAGE_GATE_SRC).toMatch(/process\.env\.NODE_ENV\s*!==\s*"production"[\s\S]{0,200}console\.warn\([\s\S]{0,200}routeId/);
    });
});
// =============================================================================
...
describe("Phase R10 — Stage 3: route registry enterprise entries", () => {
    it("workspace.evidence_lifecycle entry is registered with the canonical href", () => {
        expect(ROUTE_REGISTRY_SRC).toMatch(/id:\s*"workspace\.evidence_lifecycle"[\s\S]{0,400}href:\s*"\/evidence-lifecycle"/);
    });
    it("workspace.governance_platform entry is registered with the canonical href", () => {
        expect(ROUTE_REGISTRY_SRC).toMatch(/id:\s*"workspace\.governance_platform"[\s\S]{0,400}href:\s*"\/governance-platform"/);
    });
    it("both entries declare ORGANIZATION_ONLY requiredActiveSpace (still blocked for personal)", () => {
        ...
        const lifecycleBlock = ROUTE_REGISTRY_SRC.slice(ROUTE_REGISTRY_SRC.indexOf('id: "workspace.evidence_lifecycle"'), ROUTE_REGISTRY_SRC.indexOf('id: "workspace.evidence_lifecycle"') + 600);
        const governanceBlock = ROUTE_REGISTRY_SRC.slice(ROUTE_REGISTRY_SRC.indexOf('id: "workspace.governance_platform"'), ROUTE_REGISTRY_SRC.indexOf('id: "workspace.governance_platform"') + 600);
        expect(lifecycleBlock).toMatch(/requiredActiveSpace:\s*"ORGANIZATION_ONLY"/);
        expect(governanceBlock).toMatch(/requiredActiveSpace:\s*"ORGANIZATION_ONLY"/);
    });
});
// =============================================================================
...
describe("Phase R10 — behavioural: resolveRouteAccess persona matrix", () => {
    async function loadResolver() {
        return import("../../../apps/web/lib/navigation/routeAccessResolver.js");
    }
    function makeRoute(requiredActiveSpace, opts) {
        return {
            id: "test.route",
            href: "/test",
            label: "Test",
...
describe("Phase R10 — registry-page-existence invariant", () => {
    async function loadRegistry() {
        const mod = await import("../../../apps/web/lib/navigation/routeRegistry.js");
        return mod.ROUTE_REGISTRY;
    }
