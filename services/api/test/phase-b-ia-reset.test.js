...
const PHASE_B_GROUPS = readSource("../../../apps/web/lib/navigation/phaseBOperationalGroups.ts");
const ROUTE_REGISTRY_SRC = readSource("../../../apps/web/lib/navigation/routeRegistry.ts");
const NEXT_CONFIG = readSource("../../../apps/web/next.config.js");
const BREADCRUMB = readSource("../../../apps/web/components/navigation/OperationalBreadcrumb.tsx");
const CASE_PAGE = readSource("../../../apps/web/app/(app)/cases/[id]/page.tsx");
const EVIDENCE_REQUEST_PAGE = readSource("../../../apps/web/app/(app)/evidence-requests/[id]/page.tsx");
// ---------------------------------------------------------------------
// Helper: extract the set of route ids declared in the registry by
// parsing the source for `id: "..."`.
// ---------------------------------------------------------------------
function registryRouteIds() {
    const ids = new Set();
    const re = /^\s*id:\s*"([^"]+)"/gm;
    let m;
...
describe("Phase B — canonical operational hierarchy", () => {
    it("declares exactly four operational groups", () => {
        const groups = ["WORKSPACE", "GOVERNANCE", "OUTPUTS", "SYSTEM"];
        ...
        const groupEntryRe = /id:\s*"(WORKSPACE|GOVERNANCE|OUTPUTS|SYSTEM)",\s*\n\s*title:/g;
        const entries = [...PHASE_B_GROUPS.matchAll(groupEntryRe)].map((m) => m[1]);
    ...
    it("each group declares a title, hint, primary[], and secondary[]", () => {
        for (const g of ["WORKSPACE", "GOVERNANCE", "OUTPUTS", "SYSTEM"]) {
            expect(PHASE_B_GROUPS).toMatch(new RegExp(`id:\\s*"${g}",\\s*\\n\\s*title:[\\s\\S]*?hint:[\\s\\S]*?primary:[\\s\\S]*?secondary:`));
        }
    });
    it("exposes operationalGroupForRoute() + operationalGroupDescriptor() helpers", () => {
        ...
        expect(PHASE_B_GROUPS).toMatch(/export function operationalGroupDescriptor/);
    });
    it("declares the destination budget constant", () => {
        expect(PHASE_B_GROUPS).toMatch(/OPERATIONAL_DESTINATION_TARGET\s*=\s*\{[\s\S]*?target:\s*25[\s\S]*?ceiling:\s*30/);
    });
});
// ===========================================================================
// 2. Mapping coverage
...
describe("Phase B — registry ↔ Phase B group coverage", () => {
    it("every registered route id is mapped to a Phase B group", () => {
        ...
        const mapped = new Set([...primary, ...secondary]);
...
describe("Phase B — route registry additions", () => {
    it("registers workspace.review pointing at /review", () => {
        expect(ROUTE_REGISTRY_SRC).toMatch(/id:\s*"workspace\.review"[\s\S]*?href:\s*"\/review"/);
    });
    it("registers review.operations pointing at /review/operations", () => {
        expect(ROUTE_REGISTRY_SRC).toMatch(/id:\s*"review\.operations"[\s\S]*?href:\s*"\/review\/operations"/);
    });
    it("registers workspace.evidence_requests pointing at /evidence-requests", () => {
        expect(ROUTE_REGISTRY_SRC).toMatch(/id:\s*"workspace\.evidence_requests"[\s\S]*?href:\s*"\/evidence-requests"/);
    });
    it("keeps the legacy review.queue entry (per Phase Final-Vocab-Alignment, the id-binding survives but its href is the canonical /review console)", () => {
        ...
        expect(ROUTE_REGISTRY_SRC).toMatch(/id:\s*"review\.queue"[\s\S]*?href:\s*"\/review"/);
    });
});
// ===========================================================================
...
describe("Phase B — next.config.js redirects", () => {
    it("removed the legacy /review → /reviewer-ops redirect (was bypassing Phase C0)", () => {
        ...
        expect(code).not.toMatch(/source:\s*"\/review",\s*destination:\s*"\/reviewer-ops"/);
    });
    it("added the /ops/reliability → /operations/reliability alias", () => {
        expect(NEXT_CONFIG).toMatch(/source:\s*"\/ops\/reliability",\s*destination:\s*"\/operations\/reliability"/);
    });
...
describe("Phase B — OperationalBreadcrumb component", () => {
    ...
    it("renders the Phase B operational group via operationalGroupDescriptor", () => {
        expect(BREADCRUMB).toMatch(/import\s*\{\s*operationalGroupDescriptor[\s\S]*?\}\s*from\s+"\.\.\/\.\.\/lib\/navigation\/phaseBOperationalGroups"/);
...
describe("Phase B — breadcrumb integration", () => {
    it("Matter Workspace page renders the breadcrumb above the workspace", () => {
        ...
        expect(CASE_PAGE).toMatch(/<OperationalBreadcrumb[\s\S]*?routeId="workspace\.cases"/);
    });
    it("Evidence Request inspector renders the breadcrumb pointing at the matter when one exists", () => {
        ...
        expect(EVIDENCE_REQUEST_PAGE).toMatch(/<OperationalBreadcrumb[\s\S]*?routeId="workspace\.evidence_requests"/);
        expect(EVIDENCE_REQUEST_PAGE).toMatch(/href:\s*`\/cases\/\$\{encodeURIComponent\(data\.caseId\)\}`/);
    });
});
// ===========================================================================
