...
const GRAPH_BUILDER = resolve(SHARED_RUNTIME_ROOT, "src/graph/graph-builder.service.ts");
const GRAPH_CATALOG = resolve(SHARED_RUNTIME_ROOT, "src/graph/graph-catalog.ts");
const WORKER_PROCESSOR = resolve(WORKER_ROOT, "src/media-intelligence.processor.ts");
const WORKER_QUEUE = resolve(WORKER_ROOT, "src/queue.ts");
const ENTITY_EXTRACTION_SERVICE = resolve(API_ROOT, "src/services/intelligence/entity-extraction.service.ts");
const SEARCH_INDEXING_SERVICE = resolve(API_ROOT, "src/services/search/evidence-indexing.service.ts");
const INTELLIGENCE_ROUTES = resolve(API_ROOT, "src/routes/intelligence.routes.ts");
const INTELLIGENCE_CLIENT = resolve(WEB_ROOT, "lib/api/intelligence.ts");
const INVESTIGATION_PAGE = resolve(WEB_ROOT, "app/(app)/investigation/page.tsx");
...
const PHASE_13_MIGRATION_DIR = resolve(MIGRATIONS_DIR, "20270601000000_phase13_intelligence_chain");
const PHASE_13_MIGRATION_SQL = resolve(PHASE_13_MIGRATION_DIR, "migration.sql");
const WORKER_SRC_DIR = resolve(WORKER_ROOT, "src");
const ROUTE_REGISTRY = resolve(WEB_ROOT, "lib/navigation/routeRegistry.ts");
const PHASE_13_DOC = resolve(DOCS_ROOT, "architecture/phase-13-intelligence-chain.md");
...
describe("Phase 13 S4.3 — single additive Phase 13 migration", () => {
    ...
    it("adds graph_edge_id with ADD COLUMN IF NOT EXISTS + terminating ;", () => {
        expect(sql).toMatch(/ADD\s+COLUMN\s+IF\s+NOT\s+EXISTS\s+"graph_edge_id"[\s\S]*?UUID[^;]*;/i);
    });
    it("creates the partial graph_edge_id index with terminating ;", () => {
        expect(sql).toMatch(/CREATE\s+INDEX\s+IF\s+NOT\s+EXISTS\s+"evidence_similarities_graph_edge_id_idx"[\s\S]*?;/i);
    });
...
describe("Phase 13 S4.4 — schema.prisma mirrors the additive column", () => {
    ...
    it("declares graphEdgeId mapped to graph_edge_id on EvidenceSimilarity", () => {
        expect(schema).toMatch(/graphEdgeId[\s\S]{0,80}@map\(\s*"graph_edge_id"\s*\)/);
    });
...
describe("Phase 13 S4.13 — /search renders bounded semantic-search disabled pill", () => {
    ...
    it("uses the data-semantic-search-status attribute for the chip", () => {
        ...
        expect(src).toMatch(/data-semantic-search-status=(?:"disabled"|\{status\})/);
    });
    it("does not leak internal config or env-var names in chip copy", () => {
        ...
        expect(chipText).not.toMatch(/SEMANTIC_SEARCH_ENABLED|OPENAI_EMBEDDING_MODEL|pgvector|qdrant|weaviate/i);
    });
});
// ===========================================================================
// S4.14 — DOC_EXISTS
...
describe("Phase 13 G1 — no v2 / _new / _v2 files in any affected tree", () => {
    ...
    const FORBIDDEN_BASENAMES = /^(ocr|transcript|entity|graph|timeline|search|similarity|relationship|intelligence)[._-]v2\.(ts|tsx|js)$/i;
    function offendersUnder(root) {
        return walk(root)
            .map((p) => p.replace(/\\/g, "/"))
            .filter((p) => {
    ...
    it("apps/web has no v2-duplicate file in any of the listed families", () => {
        const offenders = offendersUnder(resolve(WEB_ROOT, "app")).concat(offendersUnder(resolve(WEB_ROOT, "components")), offendersUnder(resolve(WEB_ROOT, "lib")));
...
describe("Phase 13 G2 — exactly one phase13_intelligence_chain migration", () => {
    it("matches the canonical Phase 13 intelligence-chain slug exactly once", () => {
        ...
        const intelligenceChain = entries.filter((name) => /phase13_intelligence_chain/i.test(name));
    ...
    it("no sibling migration shares the 20270601000000_ timestamp prefix", () => {
        const entries = readdirSync(MIGRATIONS_DIR);
        const siblings = entries.filter((name) => name.startsWith("20270601000000_"));
