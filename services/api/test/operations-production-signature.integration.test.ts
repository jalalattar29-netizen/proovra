...
/**
 * The EXPAND half is the entirety of what this release ships.
 *
 * It removes nothing and is safe before the code deploys — it is what unblocks
 * the writer. The CONTRACT half that drops the legacy columns is held back:
 * `prisma migrate deploy` applies every pending migration in one pass, so
 * shipping both would race a removal against the image deploy, and the cases
 * proving that half's conservation refusal travel with it.
 *
 * So every case below runs against the schema the deployment actually leaves
 * behind — canonical columns rebound, legacy columns still present — which is
 * the stricter state, not the weaker one.
 */
...
describe("Operations production signature (live PostgreSQL 16)", () => {
  ...
  /** What this release deploys: the EXPAND half, and nothing else. */
  const converge = convergeExpandOnly;
  ...
  it("is idempotent — re-applying EXPAND on an already-converged database is a no-op", async () => {
    await applyHybridDrift();
    await converge();
    ...
    const first = await reconcile();
    expect(first.run.sources.failedSources).toEqual([]);
    const columns = await legacyColumnCount();

    // Re-running it must be a no-op rather than an error: a deploy can be
    // retried, and a migration that only works once is a migration that turns
    // a retry into an outage.
    await converge();
    expect(await legacyColumnCount()).toBe(columns);

    await prisma.operationalIncident.deleteMany({
      where: scope.workspaceIncidentWhere(personal.teamId),
    });
    const second = await reconcile();
    expect(second.run.sources.failedSources).toEqual([]);
    expect(second.run.readiness).toBe("READY");
    expect(await incidentCount()).toBeGreaterThan(0);
  });
});
