...
import { AUTHORIZATION_DENIAL_CODES, evaluateAuthorize, } from "../src/middleware/authorize.js";
...
describe("Phase 26 — bounded authorization denial codes", () => {
    ...
    it("includes every AccessDenyReason from the access-policy engine (no drift)", () => {
        ...
        for (const reason of [
            ...
            expect(AUTHORIZATION_DENIAL_CODES.includes(reason)).toBe(true);
        }
    });
    it("includes route-side codes the access engine does not emit", () => {
        expect(AUTHORIZATION_DENIAL_CODES.includes("missing_team_id")).toBe(true);
        expect(AUTHORIZATION_DENIAL_CODES.includes("missing_actor")).toBe(true);
        expect(AUTHORIZATION_DENIAL_CODES.includes("authorization_unavailable")).toBe(true);
    });
...
describe("Phase 26 — evaluateAuthorize behaviour", () => {
    it("returns 'missing_actor' when no session is attached", async () => {
        const outcome = await evaluateAuthorize(makeReq(null), {
    ...
    it("returns 'missing_team_id' when teamId is empty (400 by default)", async () => {
        const outcome = await evaluateAuthorize(makeReq("u1"), {
            teamId: null,
            permission: "audit.read",
        });
        expect(outcome.allowed).toBe(false);
...
describe("Phase 26 — authorize helper source contract", () => {
    ...
    it("never sends a free-text reason on a 403 deny (bounded vocabulary only)", () => {
        ...
        expect(src).toMatch(/reply\.code\(403\)\.send\(\{\s*error:\s*\{\s*code:\s*"permission_denied",\s*reason:\s*reasonCode\s*,?\s*\}\s*,?\s*\}\)/);
    });
    it("404 anti-enumeration response shape never carries a reason field (no info leak)", () => {
        expect(src).toMatch(/reply\.code\(404\)\.send\(\{\s*error:\s*\{\s*code:\s*"not_found"\s*\}\s*\}\)/);
    });
    it("401 unauthenticated response shape carries only the bounded code", () => {
        expect(src).toMatch(/reply\.code\(401\)\.send\(\{\s*error:\s*\{\s*code:\s*"unauthenticated"\s*\}\s*\}\)/);
    });
    it("503 fail-closed response never leaks Prisma / exception messages", () => {
        expect(src).toMatch(/reply\.code\(503\)\.send\(\{\s*error:\s*\{[\s\S]*?code:\s*"authorization_unavailable"/);
    ...
    it("wraps every evaluateMemberAccess call in try/catch (fail-closed contract)", () => {
        expect(src).toMatch(/try\s*\{\s*decision\s*=\s*await\s+evaluateMemberAccess/);
        expect(src).toMatch(/catch\s*\{[\s\S]*?bump\("authorize_failed_closed_total"\)/);
    });
    ...
    it("attaches the resolved (actorUserId, teamId) to the request so handlers can re-use without re-fetching", () => {
        expect(src).toMatch(/\(req as FastifyRequest & \{[\s\S]*?authorized\?:\s*\{[\s\S]*?actorUserId:\s*string;\s*teamId:\s*string/);
    });
    it("imports the canonical Permission type from @proovra/shared (no string-typed permissions)", () => {
        expect(src).toMatch(/import\s+type\s*\{\s*Permission\s*\}\s+from\s+"@proovra\/shared"/);
    });
    it("antiEnumeration only coerces membership-class denials, not permission-not-granted", () => {
        ...
        // anti-enumeration.
        expect(src).toMatch(/isMembershipFailure\s*=[\s\S]*?"no_actor"[\s\S]*?"member_not_active"[\s\S]*?"member_access_expired"/);
        expect(src).toMatch(/options\.antiEnumeration && isMembershipFailure\s*\?\s*404\s*:\s*403/);
    });
});
// =============================================================================
...
describe("Phase 26 — type-level invariants", () => {
    it("AuthorizationDenialCode is exactly the keyof the bounded array", () => {
        ...
        const sample = "permission_not_granted";
...
describe("Phase 26 — privacy + governance invariants", () => {
    ...
    it("metric catalogue registers the three new Phase 26 counters", () => {
        const metricSrc = readSource("../../../packages/shared-runtime/src/ops/metrics.service.ts");
