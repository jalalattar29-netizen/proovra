...
function readShared(rel) {
    return readFileSync(fileURLToPath(new URL(`../../../packages/shared-runtime/${rel}`, import.meta.url)), "utf8");
}
...
describe("Phase 32.7 — canonical operational event contract", () => {
    ...
    it("declares the typed CanonicalOperationalEvent union from the catalog", () => {
        expect(SRC).toMatch(/export type CanonicalOperationalEvent\s*=\s*\(typeof CANONICAL_OPERATIONAL_EVENTS\)\[number\]/);
    });
    it("maps WORKER_HEARTBEAT to the preserved wire string `reviewer_reconcile_run`", () => {
        ...
        expect(SRC).toMatch(/OPERATIONAL_EVENT_WIRE_STRINGS\s*:\s*Record<\s*CanonicalOperationalEvent\s*,\s*string\s*>/);
        expect(SRC).toMatch(/WORKER_HEARTBEAT:\s*"reviewer_reconcile_run"/);
    });
    it("exposes `wireStringFor()` for cross-subsystem resolution", () => {
        expect(SRC).toMatch(/export function wireStringFor\(\s*name:\s*CanonicalOperationalEvent\s*\):\s*string/);
    });
...
describe("Phase 32.7 — worker heartbeat writer uses the canonical constant", () => {
    const SRC = readApi("src/services/reviewer-ops/reviewer-operations-engine.service.ts");
    it("imports `wireStringFor` from @proovra/shared-runtime/ops", () => {
        expect(SRC).toMatch(/import\s*\{[\s\S]*?wireStringFor[\s\S]*?\}\s*from\s*"@proovra\/shared-runtime\/ops"/);
    });
    it("emits the SecurityEvent with the canonical wire string (not a literal)", () => {
        ...
        expect(SRC).toMatch(/eventType:\s*canonicalOperationalWireStringFor\([\s\S]{0,100}"WORKER_HEARTBEAT"[\s\S]{0,20}\)/);
...
describe("Phase 32.7 — workers readiness check uses the canonical constant", () => {
    ...
    it("resolves the heartbeat wire string through the canonical contract", () => {
        expect(fn).toMatch(/wireStringFor\s*\}\s*=\s*await import\(\s*"@proovra\/shared-runtime\/ops"\s*\)/);
    ...
    it("queries SecurityEvent with the canonical wire string variable (no inline literal)", () => {
        expect(fn).toMatch(/prisma\.securityEvent\.findFirst\(\s*\{\s*where:\s*\{\s*eventType:\s*heartbeatWireString\s*\}/);
        ...
        expect(fn).not.toMatch(/where:\s*\{\s*eventType:\s*"reviewer_reconcile_run"\s*\}/);
    });
});
// =============================================================================
...
describe("Phase 32.7 — subsystem→domain map", () => {
    ...
    it("typed Record<SubsystemId, SubsystemAffectedDomain> assigns a domain to every subsystem", () => {
        expect(SRC).toMatch(/SUBSYSTEM_AFFECTED_DOMAIN:\s*Record<SubsystemId,\s*SubsystemAffectedDomain>/);
    ...
    it("`runReadinessCheck` injects affectedDomain into every subsystem result", () => {
        expect(SRC).toMatch(/subsystems:\s*SubsystemReadiness\[\]\s*=\s*rawSubsystems\.map\(\s*\(s\)\s*=>\s*\(\{\s*\.\.\.s,\s*affectedDomain:\s*SUBSYSTEM_AFFECTED_DOMAIN\[s\.id\]/);
    });
});
// =============================================================================
...
describe("Phase 32.7 — RuntimeStatusBanner degradation boundary", () => {
    ...
    it("props expose optional `forDomains` for page-level scoping", () => {
        expect(SRC).toMatch(/forDomains\?:\s*ReadonlyArray<RuntimeOperationalDomain>/);
    });
...
describe("Phase 32.7 — page consumers scope the runtime banner", () => {
    ...
    for (const c of cases) {
        it(`${c.file} scopes RuntimeStatusBanner to ${c.domain}`, () => {
            ...
            const re = new RegExp(`RuntimeStatusBanner[\\s\\S]{0,200}forDomains=\\{\\s*\\[\\s*"${c.domain}"\\s*\\]\\s*\\}`);
            expect(src).toMatch(re);
        });
    }
    it("ops/observability page intentionally keeps the unscoped banner (it IS the runtime visibility surface)", () => {
        const src = readFileSync(fileURLToPath(new URL("../../../apps/web/app/(app)/ops/observability/page.tsx", import.meta.url)), "utf8");
...
describe("Phase 32.7 — no regression on the existing readiness contract", () => {
    ...
    it("rollUpStatus still escalates CRITICAL > DEGRADED > UNKNOWN > HEALTHY", () => {
        expect(SRC).toMatch(/if\s*\(\s*subsystems\.some\(\(s\)\s*=>\s*s\.status === "CRITICAL"\)\)\s*return\s*"CRITICAL"/);
        expect(SRC).toMatch(/if\s*\(\s*subsystems\.some\(\(s\)\s*=>\s*s\.status === "DEGRADED"\)\)\s*return\s*"DEGRADED"/);
        expect(SRC).toMatch(/if\s*\(\s*subsystems\.every\(\(s\)\s*=>\s*s\.status === "HEALTHY"\)\)\s*return\s*"HEALTHY"/);
