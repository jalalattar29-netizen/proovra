...
import { GRAPH_EDGE_TYPES, GRAPH_NODE_KINDS, } from "../src/services/graph/graph-catalog.js";
...
describe("Phase 31.15 — CASE reconciler source contract", () => {
    ...
    it("SELECT on cases is team-anchored", () => {
        expect(src).toMatch(/SELECT "id", "name"[\s\S]*?FROM "cases"[\s\S]*?WHERE "team_id" = \$1/);
    });
    it("upserts a CASE node per row with bounded label (<=240 chars)", () => {
        const block = src.match(/Phase 31\.15 — CASE domain reconciliation[\s\S]*?the rest of the reconcile continues/)?.[0];
    ...
    it("CASE node visibility scope is WORKSPACE_INTERNAL", () => {
        const block = src.match(/Phase 31\.15 — CASE domain reconciliation[\s\S]*?the rest of the reconcile continues/)?.[0];
        expect(block).toMatch(/upsertNode\([\s\S]*?"CASE"[\s\S]*?"WORKSPACE_INTERNAL"/);
    });
    it("stale-tombstones CASE nodes whose external_id is gone from `cases`", () => {
        const block = src.match(/Phase 31\.15 — CASE domain reconciliation[\s\S]*?the rest of the reconcile continues/)?.[0];
        expect(block).toMatch(/UPDATE "investigation_graph_nodes" n[\s\S]*?SET "stale_at_utc" = NOW\(\)[\s\S]*?n\."node_kind" = 'CASE'[\s\S]*?NOT EXISTS[\s\S]*?FROM "cases" c/);
    });
    it("stale sweep is team-anchored on BOTH the nodes table and the cases sub-select", () => {
        const block = src.match(/UPDATE "investigation_graph_nodes" n[\s\S]*?NOT EXISTS[\s\S]*?\)/)?.[0];
        expect(block).toBeTruthy();
    ...
    it("BELONGS_TO_CASE edges emitted only for evidence with non-null caseId", () => {
        ...
        expect(src).toMatch(/SELECT "id", "case_id"[\s\S]*?FROM "evidence"[\s\S]*?WHERE "team_id" = \$1[\s\S]*?AND "case_id" IS NOT NULL[\s\S]*?AND "deleted_at" IS NULL/);
    });
    it("BELONGS_TO_CASE edges are SYSTEM source, HIGH confidence", () => {
        expect(src).toMatch(/upsertEdge\([\s\S]*?"BELONGS_TO_CASE"[\s\S]*?"SYSTEM"[\s\S]*?"HIGH"/);
    });
    ...
    it("entire CASE step wrapped in try/catch — best-effort, never blocks the rest of reconcile", () => {
        const block = src.match(/Phase 31\.15 — CASE domain reconciliation[\s\S]*?\}\s*catch\s*\{[\s\S]*?the rest of the reconcile continues[\s\S]*?\}/)?.[0];
        expect(block).toBeTruthy();
    });
});
// =============================================================================
...
describe("Phase 31.15 — CASE reconciler anti-leak", () => {
    ...
    it("CASE step does NOT touch storage internals / signed URLs / private notes", () => {
        const block = src.match(/Phase 31\.15 — CASE domain reconciliation[\s\S]*?\}\s*catch\s*\{/)?.[0];
        expect(block).toBeTruthy();
...
describe("Phase 32.14 — Investigation Timeline UI", () => {
    const src = readSource("../../../apps/web/app/(app)/investigation/timeline/page.tsx");
    ...
    it("no forbidden truth-claim vocabulary in user-facing literals", () => {
        ...
        const forbidden = /\b(tamper(ed|ing)?|forged|fake|authentic(ity)?|admissible|proves?|confirms?|manipulated|doctored)\b/i;
        for (const lit of literals) {
            expect(lit, `timeline UI uses forbidden wording: ${lit}`).not.toMatch(forbidden);
        }
    });
    ...
    it("empty state has next-action guidance (not a dead-end UI)", () => {
        ...
        const flat = src.replace(/\s+/g, " ");
        expect(flat).toMatch(/No (?:events recorded|workspace events)[\s\S]*?(?:graph reconcile|reconcile|analyzer|Capture|Cases)/i);
    });
});
