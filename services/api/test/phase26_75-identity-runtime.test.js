...
import { GEO_LOOKUP_DEFAULT_TIMEOUT_MS, GEO_LOOKUP_MAX_TIMEOUT_MS, HIGH_RISK_INCIDENT_DEFAULT_THRESHOLD, PRIVILEGED_ACTIONS, RUNTIME_RISK_RECOMPUTE_DEFAULT_MINUTES, SESSION_QUARANTINE_DEFAULT_HOURS, SESSION_QUARANTINE_MAX_HOURS, SEARCH_FORBIDDEN_OVERCLAIM_PHRASES, TRUST_DECAY_DEFAULT_MAX, TRUST_DECAY_STALE_DAYS, computeTrustDecay, isForcedReauthAllowed, isSessionDueForRiskRecompute, privilegedActionRequiresFreshAuth, } from "@proovra/shared";
...
describe("Phase 26.75 — adaptive runtime gate", () => {
    const src = readSource("../src/services/access-control/adaptive-runtime-gate.service.ts");
...
describe("Phase 26.75 — session quarantine", () => {
    const src = readSource("../src/services/access-control/session-quarantine.service.ts");
...
describe("Phase 26.75 — geo intelligence", () => {
    const src = readSource("../src/services/access-control/geo-intelligence.service.ts");
...
describe("Phase 26.75 — runtime risk recompute", () => {
    const src = readSource("../src/services/access-control/runtime-risk.service.ts");
...
describe("Phase 26.75 — trusted device decay", () => {
    const src = readSource("../src/services/access-control/trusted-device-decay.service.ts");
...
describe("Phase 26.75 — admin runtime routes", () => {
    ...
    it("emergency revoke requires step-up", () => {
        const emergencyBlock = src.match(/\/v1\/admin\/identity\/emergency-revoke["'][\s\S]{0,1500}?async/);
        expect(emergencyBlock).toBeTruthy();
        // The step-up call is inside the handler.
        expect(src).toMatch(/purpose: ["']ORG_SECURITY_POLICY_UPDATE["'][\s\S]{0,200}?resourceKind: ["']team["']/);
    });
...
describe("Phase 26.75 — pure helpers", () => {
    ...
    it("privilegedActionRequiresFreshAuth: fresh session is OK", () => {
        const now = new Date();
        expect(privilegedActionRequiresFreshAuth({
    ...
    it("isSessionDueForRiskRecompute respects window", () => {
        const now = new Date();
        expect(isSessionDueForRiskRecompute({
