...
const LIFECYCLE_DIR = resolve(
  REPO_ROOT,
  "apps/web/app/(app)/evidence-lifecycle",
);
const SUB_PAGES: Array<{ slug: string; label: string }> = [
...
function readLifecycleFile(rel: string): string {
...
describe("Evidence Lifecycle Final Fix — segment chrome", () => {
  ...
  it("(4) global error.tsx still exists as last-resort boundary", () => {
    ...
    // (2026-07-21) The boundary is now the canonical `ProovraSystemState`:
    // calm reassurance + retry/escape + the trace id as a copyable support
    // reference. It must NOT regress to the old vague "Something went wrong".
    expect(src).toMatch(/ProovraSystemState/);
...
describe("Evidence Lifecycle Final Fix — per-page wiring", () => {
  ...
  for (const sub of SUB_PAGES_WITH_BOUNDARY) {
    it(`(11.${sub.slug}) ${sub.label} sub-page wraps Shell in LifecycleSectionBoundary`, () => {
      ...
      expect(src).toMatch(
        /<LifecycleSectionBoundary[\s\S]*?<Shell\s*\/>[\s\S]*?<\/LifecycleSectionBoundary>/,
      );
...
describe("Evidence Lifecycle Final Fix — backend hardening", () => {
  ...
  it("(17) no new lifecycle migration was added — Phase 2.1 (20270809) remains latest lifecycle-touching migration", () => {
    ...
    const dirs = readdirSync(migrationsDir).filter((n) =>
      statSync(resolve(migrationsDir, n)).isDirectory(),
    );
    // No directory with the strings "lifecycle-v2" or "lifecycle_final_fix"
    // should appear — this fix is code-only.
    const offenders = dirs.filter(
      (n) => /lifecycle.*v2/i.test(n) || /lifecycle.final.fix/i.test(n),
    );
