...
function parseArgs(): {
  ...
  for (const a of args) {
    if (a.startsWith("--teamId=")) out.teamId = a.slice("--teamId=".length).trim();
      ...
      if (Number.isFinite(n)) out.maxBatches = n;
      ...
      if (Number.isFinite(n) && n >= 1) out.repeat = Math.min(n, 1000);
    }
  }
  return out;
}
...
async function main(): Promise<void> {
  ...
  if (!args.teamId) {
    ...
    console.error(
      "backfill-semantic-embeddings: --teamId=<workspace-uuid> is required",
    );
    process.exitCode = 2;
    return;
  }
