...
export const PROPOSED_ADDITIONS = {
  ...
  // UC-0 (2026-09-17) — the acquisition/provenance trust foundation.
  "20280601000000_uc0_acquisition_provenance_foundation":
    "REQUIRED_RELEASE_MIGRATION — BACKFILL, SAFE_TO_APPLY_NOW, and it must be applied BEFORE the image that reads it. Adds evidence.acquisition_mode + acquisition_mode_source (nullable, CHECK-bounded, set-once by a BEFORE UPDATE trigger), evidence_parts.artifact_class (default ORIGINAL), reports.acquisition_mode_snapshot, capture-session nonce/lifecycle columns and the ACTIVE/BOUND/INTERRUPTED enum values, capture_device_attestations.verifier_version, and derivative descriptor columns with a new (team_id, evidence_part_id, asset_kind, variant_key) unique index (the old narrow unique index is RETAINED — expand only). The single backfill writes SECURE_INTAKE_LINK only for records linked through workflow_intake_sessions, marked BACKFILL_INTAKE_SESSION_LINK, and only where acquisition_mode IS NULL, so re-running is a no-op; every other historical record stays NULL and reads as Not recorded. The previous image never reads the new columns; the new image writes acquisition_mode on every create, so deploying it first would fail evidence creation.",
  // UC-1 (2026-09-17) — first-party extension OAuth authorization codes.
  "20280620000000_uc1_extension_oauth_codes":
    "REQUIRED_RELEASE_MIGRATION — EXPAND, SAFE_TO_APPLY_NOW, apply BEFORE the UC-1 image. One new table extension_auth_codes for the extension's Authorization Code + PKCE S256 flow (the code is never stored, only its SHA-256; single-use via used_at_utc; short-lived; bound to client, redirect and code_challenge; FK to users ON DELETE CASCADE). Nothing existing is altered, no row read or written, so it is inert under the pre-UC-1 image and read by the new image's /v1/oauth/extension/* routes. Idempotent (CREATE TABLE/INDEX IF NOT EXISTS + guarded FK). Clean-boot rehearsed on disposable PostgreSQL 16; drift OK. NOT APPLIED TO PRODUCTION.",
  // UC-1 (2026-09-17) — Direct Web Capture acquisition mode.
  "20280610000000_uc1_direct_web_capture_acquisition_mode":
    "REQUIRED_RELEASE_MIGRATION — EXPAND, SAFE_TO_APPLY_NOW, apply BEFORE the UC-1 image. Widens the two acquisition_mode CHECK constraints (evidence_acquisition_mode_check, capture_sessions_acquisition_mode_check) to admit DIRECT_WEB_CAPTURE_EXTENSION — the first isDirectCapture mode, produced by the PROOVRA browser extension in a server-issued capture session. A CHECK IN-list cannot be extended in place, so each constraint is DROPPED and RE-ADDED with the same name and a strictly wider value set — a constraint SWAP, not a removal: every existing row already satisfies the narrower set. Safe on either image (the pre-UC-1 image writes only the three UC-0 modes, still admitted). Idempotent (DROP CONSTRAINT IF EXISTS + ADD). Clean-boot rehearsed on disposable PostgreSQL 16; drift OK. NOT APPLIED TO PRODUCTION.",
  // UC-3 (2026-09-17) — Android Continuous Screen Capture acquisition mode.
  "20280640000000_uc3_continuous_screen_capture_acquisition_mode":
    "REQUIRED_RELEASE_MIGRATION — EXPAND, SAFE_TO_APPLY_NOW, apply BEFORE the UC-3 image. Widens the two acquisition_mode CHECK constraints (evidence_acquisition_mode_check, capture_sessions_acquisition_mode_check) to admit DIRECT_SCREEN_CAPTURE_ANDROID_CONTINUOUS — the third isDirectCapture mode, produced by the PROOVRA Android app recording a continuous screen session (ordered segments) via MediaProjection. A CHECK IN-list cannot be extended in place, so each constraint is DROPPED and RE-ADDED with the same name and a strictly wider value set — a constraint SWAP, not a removal: every existing row already satisfies the narrower set. Safe on either image (the pre-UC-3 image writes only the five prior modes, still admitted). Idempotent (DROP CONSTRAINT IF EXISTS + ADD). Clean-boot rehearsed on disposable PostgreSQL 16; drift OK. NOT APPLIED TO PRODUCTION.",
  // UC-2 (2026-09-17) — Android Direct Screen Capture acquisition mode.
  "20280630000000_uc2_screen_capture_acquisition_mode":
    "REQUIRED_RELEASE_MIGRATION — EXPAND, SAFE_TO_APPLY_NOW, apply BEFORE the UC-2 image. Widens the two acquisition_mode CHECK constraints (evidence_acquisition_mode_check, capture_sessions_acquisition_mode_check) to admit DIRECT_SCREEN_CAPTURE_ANDROID — the second isDirectCapture mode, produced by the PROOVRA Android app via MediaProjection in a server-issued capture session. A CHECK IN-list cannot be extended in place, so each constraint is DROPPED and RE-ADDED with the same name and a strictly wider value set — a constraint SWAP, not a removal: every existing row already satisfies the narrower set. Safe on either image (the pre-UC-2 image writes only the four prior modes, still admitted). Idempotent (DROP CONSTRAINT IF EXISTS + ADD). Clean-boot rehearsed on disposable PostgreSQL 16; drift OK. NOT APPLIED TO PRODUCTION.",
  // UC-5 (2026-09-18) — iOS Native Screen Capture acquisition mode.
  "20280660000000_uc5_ios_screen_capture_acquisition_mode":
    "REQUIRED_RELEASE_MIGRATION — EXPAND, SAFE_TO_APPLY_NOW, apply BEFORE the UC-5 image. Widens the two acquisition_mode CHECK constraints (evidence_acquisition_mode_check, capture_sessions_acquisition_mode_check) to admit DIRECT_SCREEN_CAPTURE_IOS — the iOS direct-screen mode, produced by the PROOVRA iOS app recording the device screen through Apple's user-authorised system broadcast (ReplayKit + a PROOVRA Broadcast Upload Extension), preserved as ordered segments. A CHECK IN-list cannot be extended in place, so each constraint is DROPPED and RE-ADDED with the same name and a strictly wider value set — a constraint SWAP, not a removal: every existing row already satisfies the narrower set. Safe on either image (the pre-UC-5 image writes only the six prior modes, still admitted). Idempotent (DROP CONSTRAINT IF EXISTS + ADD). Clean-boot rehearsed on disposable PostgreSQL 16; drift OK. NOT APPLIED TO PRODUCTION.",
  // UC-0 (A3, 2026-09-17) — the derivative variant contract.
  "20280602000000_derived_asset_variant_contract":
    "REQUIRED_LATER_CONTRACT_MIGRATION — CONTRACT_DROP, self-guarded, APPLY AFTER the UC-0 image. Drops the narrow unique key evidence_part_derived_assets_team_part_kind_uk (team_id, evidence_part_id, asset_kind), leaving the composite key (…, variant_key) added by 20280601000000. The narrow key blocked a second variant of one kind on the same part (e.g. keyframes); the composite key keeps identical variants idempotent while letting distinct variants coexist. A DO-block RAISEs unless the composite key exists, so the drop can never leave derived assets with no uniqueness. Ordering hazard in one direction only: the pre-UC-0 image upserts ON CONFLICT (team_id, evidence_part_id, asset_kind) against the narrow key, so this must not be applied until the UC-0 image (which upserts against the composite key) is live everywhere. Idempotent: DROP INDEX IF EXISTS.",
  ...
  // SEARCH TRUTH (2026-09-05) — which build of the projection wrote a document.
  "20280401000000_search_projection_version":
    "REQUIRED_RELEASE_MIGRATION — EXPAND, SAFE_TO_APPLY_NOW. One `ADD COLUMN IF NOT EXISTS evidence_search_documents.projection_version INTEGER NOT NULL DEFAULT 1` and one index on (team_id, projection_version); destructiveStatements is empty, no existing column is altered or dropped, and no row is read, written or deleted. WHY: a search document is a CACHE of buildEvidenceProjection, and when the projection began indexing External Intake identity — Customer ID, recipient name, address, phone — every document already written became unable to answer a question about those fields while remaining entirely valid-looking. Nothing could detect it: the source row was untouched so no timestamp moved, and the reindex only ever selected evidence rows with NO document (esd.id IS NULL), so a record that already had one was invisible to the cron sweep, POST /v1/search/reconcile and the backfill CLI alike. Proven against a live database rather than argued: a document with the identity stripped from its body failed all four identity probes, a full workspace reconcile reported 14 documents indexed, and the stripped body came back byte-for-byte identical. THE DEFAULT IS 1 AND NOT THE CURRENT VERSION, deliberately: every existing row predates the identity fields and must report as stale so the reindex refreshes it — defaulting to the current version would declare the entire production index healthy and leave the defect behind a green number. NO BACKFILL IN THE MIGRATION: the repair is a REINDEX, not an UPDATE. Stamping the version without rebuilding the body would mark stale documents current and permanently hide the very records this exists to find. The refresh runs through the same canonical indexEvidence, per workspace, in bounded batches, under the existing durable per-workspace lock, and is idempotent — a second run reports zero stale. Safe to apply BEFORE the code: the column has a default so the ALTER does not rewrite the table on PostgreSQL 11+, and an older build neither reads nor writes it. Safe AFTER a rollback: an unread integer. REHEARSED against live PostgreSQL 16 (pgvector/pgvector:pg16): applies on a clean database from zero (262 migrations, exit 0) and on one already carrying the previous schema with 229 existing search documents, converging on an identical evidence_search_documents column signature in both with the index present in each, and 229 rows conserved with every pre-existing row taking version 1 without an UPDATE touching one of them.",
