...
import {
  safeSummary,
  SIGNAL_METADATA,
  type MediaIntelligenceConfidence,
  type MediaIntelligenceSeverity,
  type MediaIntelligenceSignalType,
} from "./signal-catalog.js";
...
export type AnalyzerResult = {
  ...
  signalsByType: Partial<Record<MediaIntelligenceSignalType, number>>;
  /** True if the analyzer reached the persistence step. False on
   *  fail-closed (evidence not found, governance block). */
  ok: boolean;
  /** Bounded reason — only present when `ok === false`. */
  reason?:
    | "evidence_not_found"
    | "team_mismatch"
    | "service_unavailable";
 ...
 */
export async function runMediaIntelligenceAnalysis(
  input: AnalyzerInput,
  ...
  try {
    ...
    const exifSummaries = await listExifSummariesForEvidence(
      input.teamId,
      input.evidenceId,
      client,
    );
    const exifByPart = new Map(
      exifSummaries.map((s) => [s.evidencePartId, s] as const),
    );

    const emitted: Array<{
      signalType: MediaIntelligenceSignalType;
      materialId: string | null;
      severity: MediaIntelligenceSeverity;
      confidence: MediaIntelligenceConfidence;
      summary: string;
      detailsJson: unknown;
    }> = [];
    ...
    for (const part of parts) {
      ...
      if (persistedExif && persistedExif.status === "OK") {
        const exifIso =
          persistedExif.dateTimeOriginalUtc ??
          persistedExif.createDateUtc ??
          null;
        const gap = computeExifVsServerGapSeconds(
          exifIso,
      ...
      const gap = computeClientServerGapSeconds(
        partSignals.captureTimeUtc ?? evidence.deviceTimeIso ?? null,
        evidence.uploadedAtUtc,
      );
      if (gap !== null && gap >= 60 * 60) {
      ...
      if (part.sha256) {
        const dupes = await findDuplicateHashCount(
          client,
          input.teamId,
          part.sha256,
          part.id,
      ...
      if (filename && part.sizeBytes != null) {
        const candidates = await findSimilarFilenameSizeCount(
          client,
          input.teamId,
          filename,
          BigInt(part.sizeBytes),
          part.sha256,
          part.id,
        );
        if (candidates > 0) {
    ...
    const ocrAvailable = await checkIntelligenceJobAvailability(
      client,
      input.evidenceId,
      "OCR",
    );
    ...
    const transcriptAvailable = await checkIntelligenceJobAvailability(
      client,
      input.evidenceId,
      "TRANSCRIPT",
    );
    ...
    for (const sig of emitted) {
      try {
        await client.$executeRawUnsafe(
          ...
          sig.confidence,
          sig.summary,
          JSON.stringify(sig.detailsJson ?? null),
        );
    ...
    const signalsByType: Partial<
      Record<MediaIntelligenceSignalType, number>
    > = {};
    for (const sig of emitted) {
...
// =============================================================================
// Pure heuristics (exported for tests)
// =============================================================================

export type MimeExtensionMismatch = {
  extension: string;
  mimeType: string;
};
 ...
 */
export function detectMimeExtensionMismatch(
  filename: string,
  mimeType: string,
): MimeExtensionMismatch | null {
  ...
  if (!expected) return null; // unknown extension — no signal
  const actualFamily = mimeFamily(normalizedMime);
  if (!actualFamily) return null;
...
const EXTENSION_TO_MIME_FAMILY: Readonly<Record<string, string>> = {
  jpg: "image",
  jpeg: "image",
  png: "image",
  gif: "image",
...
export function mimeFamily(mime: string): string | null {
  if (!mime) return null;
 ...
 */
export function computeClientServerGapSeconds(
  clientTimeIso: string | null | undefined,
  serverTime: Date | null | undefined,
): number | null {
  if (!clientTimeIso || !serverTime) return null;
...
type ClientSignalsShape = {
  captureTimeUtc?: string | null;
  exifPresent?: boolean | null;
  screenshotLike?: boolean | null;
  // Phase 31.5 — used by VIDEO_DURATION_OBSERVATION.
  durationMs?: number | null;
};

function readClientSignals(raw: unknown): ClientSignalsShape {
  if (!raw || typeof raw !== "object") return {};
  const o = raw as Record<string, unknown>;
  return {
    captureTimeUtc:
      typeof o.captureTimeUtc === "string" ? o.captureTimeUtc : null,
    exifPresent:
      typeof o.exifPresent === "boolean" ? o.exifPresent : null,
    screenshotLike:
      typeof o.screenshotLike === "boolean" ? o.screenshotLike : null,
    durationMs:
...
async function findDuplicateHashCount(
  client: PrismaClient,
  teamId: string,
  sha256: string,
 ...
 */
async function findSimilarFilenameSizeCount(
  client: PrismaClient,
  teamId: string,
  filename: string,
  sizeBytes: bigint,
    ...
    const rows = (await client.$queryRawUnsafe(
      `SELECT COUNT(*)::int AS "n"
         ...
         WHERE e."team_id" = $1
           ...
           AND ep."size_bytes" = $3
           AND ep."id" <> $4
           AND (ep."sha256" IS NULL OR ep."sha256" <> COALESCE($5, ''))`,
      teamId,
      filename,
      sizeBytes,
      excludePartId,
 ...
 */
async function checkIntelligenceJobAvailability(
  client: PrismaClient,
  evidenceId: string,
  kind: "OCR" | "TRANSCRIPT",
): Promise<boolean> {
