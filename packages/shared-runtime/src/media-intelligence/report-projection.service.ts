...
/**
 * Bounded report shape — mirror of `MediaIntelligenceReportInput`
 * in `services/worker/src/report-v2/types.ts`. Re-declared here so
 * the api package doesn't have to import worker types; the contract
 * test asserts the shapes stay in lockstep.
 */
export type ReportProjectionResult = {
  signals: ReadonlyArray<ProjectedSignal>;
  derivedThumbnails: ReadonlyArray<ProjectedThumbnail>;
  ocrTranscript: ReadonlyArray<ProjectedOcrTranscript>;
};
...
export type ProjectedThumbnail = {
  materialId: string;
  dataUrl: string;
  assetKind: string;
};

export type ProjectedOcrTranscript = {
  materialId: string;
...
const SEVERITY_RANK: Record<ProjectedSignal["severity"], number> = {
  ATTENTION: 0,
  REVIEW_RECOMMENDED: 1,
  INFO: 2,
};

const ALLOWED_SEVERITIES = new Set<ProjectedSignal["severity"]>([
  "INFO",
  "REVIEW_RECOMMENDED",
  "ATTENTION",
]);
const ALLOWED_CONFIDENCES = new Set<ProjectedSignal["confidence"]>([
  "LOW",
  "MEDIUM",
  "HIGH",
]);
const ALLOWED_STATUSES = new Set<ProjectedSignal["status"]>([
  "PENDING",
  "ACKNOWLEDGED",
  "DISMISSED",
]);
 ...
 */
/**
 * Phase 31.22 — caller-injected storage fetcher for derived
 * thumbnails. The shared-runtime package MUST NOT bundle an S3
 * adapter; the caller (services/worker) passes its own helper.
 * When no fetcher is provided, the projection omits thumbnails.
 */
export type ReportThumbnailStorageFetcher = (input: {
  bucket: string;
  key: string;
}) => Promise<NodeJS.ReadableStream | AsyncIterable<Buffer | string>>;

export async function projectMediaIntelligenceForReport(
  input: ReportProjectionInput,
  client: PrismaClient = getRegisteredPrisma(),
  storage?: ReportThumbnailStorageFetcher,
): Promise<ReportProjectionResult | null> {
  if (!input.teamId || !input.evidenceId) return null;

  let signals: ProjectedSignal[] = [];
  try {
    ...
    signals = rows
      .map(projectRow)
      .filter((s): s is ProjectedSignal => s != null);
  } catch {
  ...
  // stays controlled.
  const derivedThumbnails = storage
    ? await loadDerivedThumbnailsForReport(
        input.teamId,
        input.evidenceId,
        client,
        storage,
...
function projectRow(raw: {
  ...
  if (!ALLOWED_SEVERITIES.has(raw.severity as ProjectedSignal["severity"])) {
    return null;
  }
  if (!ALLOWED_CONFIDENCES.has(raw.confidence as ProjectedSignal["confidence"])) {
    return null;
  }
  if (!ALLOWED_STATUSES.has(raw.status as ProjectedSignal["status"])) {
    return null;
  }
  ...
  return {
    ...
    severity: raw.severity as ProjectedSignal["severity"],
    confidence: raw.confidence as ProjectedSignal["confidence"],
    safeSummary: raw.safe_summary.slice(0, MAX_SAFE_SUMMARY_CHARS),
    status: raw.status as ProjectedSignal["status"],
 ...
 */
function sanitizeMaterialLabel(s: string | null): string | null {
...
// Phase 31.22 — caller-injected storage fetcher (see public type
// definition above). Signature matches `getObjectStream` from
// services/worker/src/storage.ts so the worker can pass it directly.
async function loadDerivedThumbnailsForReport(
  teamId: string,
  evidenceId: string,
  client: PrismaClient,
  getObjectStream: ReportThumbnailStorageFetcher,
): Promise<
  ReadonlyArray<{ materialId: string; dataUrl: string; assetKind: string }>
> {
  try {
    const rows = (await client.$queryRawUnsafe(
      `SELECT "id", "evidence_part_id", "asset_kind"
